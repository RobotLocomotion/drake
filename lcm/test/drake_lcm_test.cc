...
// A mudpm URL that is not the default.  We'll transmit here in our tests.
constexpr const char* const kMudpmUrl = "udpm://239.255.76.67:7670";
...
TEST_F(DrakeLcmTest, CustomUrlTest) {
  dut_ = std::make_unique<DrakeLcm>(kMudpmUrl);
  EXPECT_EQ(dut_->get_requested_lcm_url(), kMudpmUrl);
  EXPECT_EQ(dut_->get_lcm_url(), kMudpmUrl);
}
...
TEST_F(DrakeLcmTest, QueueCapacityTest) {
  ...
  for (int i = 0; i < 3; ++i) {
    Publish(dut_.get(), channel_name, message_);
    // Let lcm_mudpm's receive thread get scheduled.

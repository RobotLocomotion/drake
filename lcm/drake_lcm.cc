...
class DrakeLcm::Impl {
 public:
  ...
  // Housekeeping: scrub any deallocated subscribers.
  void CleanUpOldSubscribers() {
...
class DrakeSubscription final : public DrakeSubscriptionInterface {
 public:
  ...
  static std::shared_ptr<DrakeSubscription> CreateSingleChannel(
    ...
    return Create(native_instance, channel_regex, single_channel_handler, {});
  }
  ...
  static std::shared_ptr<DrakeSubscription> CreateMultichannel(
    ...
    return Create(native_instance, ".*", {}, multichannel_handler);
  }

  static std::shared_ptr<DrakeSubscription> Create(
      ::lcm::LCM* native_instance, const std::string& channel_regex,
      HandlerFunction single_channel_handler,
      MultichannelHandlerFunction multichannel_handler) {
    ...
    result->user_single_channel_callback_ = std::move(single_channel_handler);
    result->user_multichannel_callback_ = std::move(multichannel_handler);
    ...
    DRAKE_DEMAND((result->user_single_channel_callback_ != nullptr)
                 || (result->user_multichannel_callback_ != nullptr));
  ...
  void Detach() {
    ...
    user_single_channel_callback_ = {};
    user_multichannel_callback_ = {};
 ...
 private:
  void InstanceCallback(const std::string& channel,
                        const ::lcm::ReceiveBuffer* buffer) {
    DRAKE_DEMAND(!weak_self_reference_.expired());
    if (user_single_channel_callback_ != nullptr) {
      user_single_channel_callback_(buffer->data, buffer->data_size);
    } else if (user_multichannel_callback_ != nullptr) {
      user_multichannel_callback_(channel, buffer->data, buffer->data_size);
    }
  }
  ...
  DrakeLcmInterface::HandlerFunction user_single_channel_callback_;
  DrakeLcmInterface::MultichannelHandlerFunction user_multichannel_callback_;

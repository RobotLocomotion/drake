...
class DrakeSubscription final : public DrakeSubscriptionInterface {
 public:
  ...
  static std::shared_ptr<DrakeSubscription> CreateSingleChannel(
    ...
    return Create(native_instance,
                  channel_regex, std::move(single_channel_handler), {});
  }
 ...
 private:
  ...
  // The user's function that handles raw message data.  For an active
  // subscription, exactly one will be set non-null; when detached, both will
  // be set to null.

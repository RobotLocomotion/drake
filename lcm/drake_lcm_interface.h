...
class DrakeLcmInterface {
 public:
   ...
   * `true`.  Refer to the DrakeSubscriptionInterface class overview for
   * details.
   *
   * @warning Failure to manage the return value may cause the subscription
   * to be removed immediately!
   */
  DRAKE_NODISCARD virtual std::shared_ptr<DrakeSubscriptionInterface> Subscribe(
 ...
 *
 * It is safe to call this object's destructor from any thread.  After the
 * destructor has completed (and assuming unsubscribe-on-delete is true), the
 * message handler will not be called again BUT any resources pinned to the
 * subscription may not have been destroyed yet (e.g., objects bound into the
 * user-provided message handler callback lambda).
 */
class DrakeSubscriptionInterface {
 public:
   ...
   * received messages will be discarded.
   */
 ...
 * for multi-threaded locking; it should only be used in cases where handlers
 * are called from the same thread that owns all copies of this object.
 */
template <typename Message>

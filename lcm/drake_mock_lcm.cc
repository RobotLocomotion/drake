...
// TODO(jwnimmer-tri) Once DrakeMockLcm's deprecated methods are removed, we
// should refactor this class to be a thin wrapper around the memq:// LCM URL.
...
void DrakeMockLcm::Publish(const std::string& channel, const void* data,
  ...
  LastPublishedMessage* record{};
  auto iter = last_published_messages_.find(channel);
  if (iter != last_published_messages_.end()) {
    record = &iter->second;
  } else {
    record = &last_published_messages_[channel];
  }

  const uint8_t* bytes = static_cast<const uint8_t*>(data);
  record->data = std::vector<uint8_t>(&bytes[0], &bytes[data_size]);
...
std::unique_ptr<DrakeSubscription> DrakeMockLcm::Subscribe(
    const std::string& channel, int queue_capacity, HandlerFunction handler) {
  DRAKE_THROW_UNLESS(!channel.empty());
  if (queue_capacity != 1) {
    throw std::logic_error(
        "Only queue_capacity == 1 is available in mock for now.");
  }

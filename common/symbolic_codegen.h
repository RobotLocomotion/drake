...
/// stores the evaluation result of the symbolic matrix @p M using a specific
/// double assignment for each symbolic variable in @p M. `<function_name>`
/// takes one input parameter `p` and three output parameters (`outer_indicies`,
/// `inner_indices`, and `values`).
///
///  - const double* p : An array of doubles for input parameters.

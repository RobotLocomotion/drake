...
// A concept that will accept POD pointers as well as smart pointers.
template <typename T>
concept Dereferenceable = requires(T t) { *t; };

// Note: we pass the dereferenceable type by reference to avoid attempts at
// copying smart pointers.
template <Dereferenceable T>
T& PassNonNull(T& ptr, const char* condition, const char* func,
               const char* file, int line) {
  if (ptr == nullptr) {
...
#define DRAKE_THROW_IF_NULL(ptr)                                              \
  ::drake::internal::PassNonNull(ptr, #ptr " != nullptr", __func__, __FILE__, \
                                 __LINE__)

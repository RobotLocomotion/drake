...
/// @file
/// Provides Drake's assertion implementation. They come in two flavors:
/// DRAKE_ASSERT which can be armed or disarmed independently from the
/// system-wide asserts and DRAKE_THROW_UNLESS which is always armed.
/// DRAKE_THROW_UNLESS is similar to an assertion, but uses exceptions instead
/// of `::abort()`. These are intended to be used both within Drake and by other
/// software.
...
//
// Note: supported types are explicitly enumerated in drake_assert.cc. */
template <typename T>
...
///
/// In addition to the @p condition, up to four value expressions can be

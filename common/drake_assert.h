...
/// unreachable (2) there is a compiler warning if this macro were removed.
/// The most common valid use is with a switch-case-return block where all
/// cases are accounted for but the enclosing function is supposed to return a
/// value.  Do *not* use this macro as a "logic error" assertion; it should
/// *only* be used to silence false positive warnings.
...
#define DRAKE_UNREACHABLE()                                             \
  ::drake::detail::Abort(                                               \
      "Unreachable code", __func__, __FILE__, __LINE__)

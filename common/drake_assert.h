...
// A type trait to select for raw, smart, and shared pointers.
template <typename T>
struct is_ptr_like : std::is_pointer<T> {};

template <typename T, typename D>
struct is_ptr_like<std::unique_ptr<T, D>> : std::true_type {};
template <typename T, typename D>
struct is_ptr_like<const std::unique_ptr<T, D>> : std::true_type {};

template <typename T>
struct is_ptr_like<std::shared_ptr<T>> : std::true_type {};
template <typename T>
struct is_ptr_like<const std::shared_ptr<T>> : std::true_type {};
...
auto SafeDereference(PtrType& ptr, const char* condition, const char* func,
  ...
  requires is_ptr_like<PtrType>::value
{
  if (ptr == nullptr) {
...
/// E.g., this includes function calls that return points and pointers to arrays
/// (&x[0]).
///
/// @endcode

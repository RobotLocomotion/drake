...
// @see EigenPtr for (only) usage.
// N.B. This is used because `drake::optional<>` does not work with
// `Eigen::Ref<>` because it deletes the necessary `operator=` overload for
...
class reassignable {
 public:
  ...
  void set_value(U&& value_in) {
    ...
    new (&raw_value()) T(std::forward<U>(value_in));
    has_value_ = true;
  }
 ...
 private:
  ...
  T& raw_value() { return reinterpret_cast<T&>(storage_); }
...
class EigenPtr {
 ...
 private:
  // Use mutable `reassignable` to permit pointer-like semantics (with

...
namespace internal {
/*
 * Adds [RaisePower(b, m) for m in PolynomialBasis(vars, degree)] to @p bin,
 * where PolynomialBasis(vars, degree) returns all the PolynomialBasisElement
 * objects with variables `vars` and total degree = `degree`. RaisePower is the
 * operation that merges the two maps b.var_to_degree_map() and
 * m.var_to_degree_map(). The map var_to_degree_map contains the map from a
 * variable `v` to its degree. When we merge the two maps b.var_to_degree_map()
 * and m.var_to_degree_map(), for each of the variable `v` in either or both of
 * the map, we sum the degree in both map. Namely when there is a shared key `v`
 * in both maps `b` and `m`, then the returned PolynomialBasisElement contains
 * `v` to degree `b.var_to_degree_map().at(v)+m.var_to_degree_map().at(v)`. When
 * the key `v` appears in only one of the map `b` or `m`, then the returned
 * PolynomialBasisElement contains `v` to the degree
 * b.var_to_degree_map().at(v). For example, if both b and m are
 * MonomialBasisElements, b = x²y, m=xy²z, then RaisePower(b, m) = x³y³z.
 * TODO(hongkai.dai): AddPolynomialBasisElementOfDegreeN will replace
 * AddMonomialsOfDegreeN in symbolic_monomial_util.h once drake issue #13803 is
 * resolved.

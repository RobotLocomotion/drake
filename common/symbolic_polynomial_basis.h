 ...
 * with variables `vars` and total degree =`degree`. RaisePower is the operation
 * that returns b.var_to_degree_map() + m.var_to_degree_map(), namely for any
 * variable `v` shows up in both or both `b` and `m`, then the returned
 * PolynomialBasisElement contains `v` to degree
 * `b.var_to_degree_map()(v)+m.var_to_degree_map().at(v)`. For example, if both
 * b and m are MonomialBasisElements, b = x²y, m=xy²z, then RaisePower(b, m) =
 * x³y³z.
 */

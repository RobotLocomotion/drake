...
///
/// @note Previously, we use the "friend" trick explained in the Meyer's
/// effective C++ 3rd. (item 46) to provide a single `operator&&`
/// definition. The trick allows the definition to support not only `Bool<T> &&
/// Bool<T>` case but also `Bool<T>::value && Bool<T>` and `Bool<T> &&
/// Bool<T>::value_type` cases. However, because of this extra constraint
/// `Bool<T>::value_type != bool`, we cannot use this friend trick anymore. As a
/// result, we need to provide three definitions explicitly.
template <typename T>

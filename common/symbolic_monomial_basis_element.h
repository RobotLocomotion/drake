...
class MonomialBasisElement : public PolynomialBasisElement {
 public:
  ...
  /**
   * Convert this monomial to Chebyshev polynomial basis. For example
   * x² = 0.5T₂(x) + 0.5T₀(x)
   * x²y³ = 1/8T₂(x)T₃(y) + 3/8T₂(x)T₁(y) + 1/8T₀(x)T₃(y)+3/8T₀(x)T₁(y)
   * We return the  map from each ChebyshevBasisElement to its coefficient.
   * For example, when this = x², then we return
   * {[T₂(x)⇒0.5], [T₀(x)⇒0.5]}
   * when this = x²y³, then we return
   * {[T₂(x)T₃(y)⇒1/8], [T₂(x)T₁(y)⇒3/8], [T₀(x)T₃(y)⇒1/8], [T₀(x)T₁(y)⇒3/8]}
   */

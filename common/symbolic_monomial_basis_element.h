 ...
 * with non-negative integer exponents. Note that it doesn not include the
 * coefficient part of a monomial. So x, x³y, xy²z are all valid
 * MonomialBasisElement, but 1+x or 2xy²z are not.
 */
class MonomialBasisElement : public PolynomialBasisElement {
 public:
  ...
  MonomialBasisElement& operator*=(const MonomialBasisElement& m);
  ...
  /**
   * Differentiates a MonomialBasisElement.
  ...
  /**
   * Integrates a MonomialBasisElement on a variable.

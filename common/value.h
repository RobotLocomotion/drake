...
class AbstractValue {
 public:
  ...
  /// AbstractValue::Make<T>() or Value<T>::Value().
  /// If T does not match, a std::logic_error will probably be thrown with a
  /// helpful error message (and is guaranteed to be thrown in Debug builds).
  template <typename T>
  ...
  /// @tparam T The originally declared type of this AbstractValue, e.g., from
  /// AbstractValue::Make<T>() or Value<T>::Value().
  /// If T does not match, probably returns nullptr (and is guaranteed to
  /// return nullptr in Debug builds).
  template <typename T>
  ...
  /// with this object, a std::logic_error will probably be thrown with a
  /// helpful error message (and is guaranteed to be thrown in Debug builds).

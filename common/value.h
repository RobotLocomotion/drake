...
constexpr void hash_template_argument_from_pretty_func(
  ...
  for (; (*p != '<') && (*p != ';') && (*p != ']'); ++p) {
    // Elide whitespace in typenames (e.g., GCC uses "int(*)(int)" for function
    // pointers, Clang uses "int (*)(int)").
    if (*p == ' ') { continue; }
    result->add_byte(*p);
  }
}
...
// Computes a typename hash into a static constant.  By putting it into a static

...
spdlog::level::level_enum ToSpdlogLevel(drake::logging::level_enum x) {
  switch (x) {
  ...
  throw std::runtime_error("ToSpdlogLevel should not reach here!");
}
...
bool logger::should_log(level_enum severity) const {
  ...
  return impl->should_log(ToSpdlogLevel(severity));
}
...
void logger::set_level(level_enum severity) const {
  ...
  return impl->set_level(ToSpdlogLevel(severity));
}
...
void logger::LogMessage(level_enum severity, std::string_view message) {
  auto* const impl = static_cast<spdlog::logger*>(impl_);
  return impl->log(ToSpdlogLevel(severity), message);
}

...
GTEST_TEST(StochasticTest, ScalarTest) {
  ...
  EXPECT_EQ(SaveYamlString("root", variants), R"""(root:
  vec:
    - !Deterministic
      value: 5.0
    - !Gaussian
  ...
  const auto variants_floats = LoadYamlString<DistributionStruct>(floats);
  vec = Sample(variants_floats.vec, &generator);

...
Eigen::VectorXd Monomial::Evaluate(
    ...
    const Eigen::Ref<const Eigen::MatrixXd>& vars_val) const {
  DRAKE_DEMAND(vars.rows() == vars_val.rows());
  Eigen::ArrayXd monomial_vals = Eigen::ArrayXd::Ones(vars_val.cols());
  for (int i = 0; i < vars.rows(); ++i) {
    auto it = powers_.find(vars(i));
    if (it != powers_.end()) {
      monomial_vals *= vars_val.row(i).array().pow(it->second);
    }
  }

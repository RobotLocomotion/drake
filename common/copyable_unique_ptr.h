...
class copyable_unique_ptr : public std::unique_ptr<T> {
 public:
  ...
  // TODO(15344) We need to shore up this const corretness hole. Rather than an
  ...
  /** Return a writable reference to the contained object (if T is itself not
   ...
   We *strongly* recommended, that, if dereferencing a %copyable_unique_ptr
   ...
   %copyable_unique_pptr. As "copy-on-write" behavior is introduced in the

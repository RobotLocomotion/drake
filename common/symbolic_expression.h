...
class Expression {
 public:
  ...
  friend Expression& operator+=(Expression& lhs, const Expression& rhs) {
    ...
    if (std::isfinite(lhs.value_) && std::isfinite(rhs.value_)) {
      lhs.value_ += rhs.value_;
      DRAKE_ASSERT(!std::isnan(lhs.value_));
      return lhs;
    }
    lhs.DoAdd(rhs);
    return lhs;
 ...
 private:
  explicit Expression(std::unique_ptr<ExpressionCell> ptr);
  ...
  // When value_ is NaN, that means it's actually a boxed shared_ptr.

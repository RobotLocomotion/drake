...
class Expression {
 ...
 private:
  ...
  // Note: We use "non-const" ExpressionCell type. This allows us to
  // perform destructive updates on the pointed cell if the cell is not shared
  // with other Expressions (that is, ptr_.use_count() == 1).
  //
  // See
  // https://github.com/RobotLocomotion/drake/issues/10900#issuecomment-472161681

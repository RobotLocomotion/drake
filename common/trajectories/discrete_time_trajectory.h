...
those times (up to a tolerance) with throw.  This trajectory does *not* have
...
of a the discrete-time solution to the finite-horizon linear quadratic regulator
...
class DiscreteTimeTrajectory final : public Trajectory<T> {
 public:
  ...
  @pre @p times must differ by more than kTolerance and be monotonically
  increasing.
  @pre @p values must have times.size() columns.
  @pre @p time_comparison_tolerance must be >= 0.
  @pre When T=symbolic:Expression, @p times must be constants. */
  ...
  @pre @p times should differ by more than kTolerance and be monotonically
       increasing.
 ...
 private:
  ...
  double kTimeComparisonTolerance_;
};

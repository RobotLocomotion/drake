...
class BezierCurve final : public trajectories::Trajectory<T> {
 public:
  ...
  /** We often write optimizations using the control points as decision
   variables.  This returns the matrix, `M`, defining the control points of the
   `order` derivative in the form:
   <pre>
     derivative.control_points().row(i) = this.control_points().row(i) * M.
   </pre>
   Notice that the matrix, `M`, is the same for all rows, `i`; this structure
   is the reason that we do not return, for instance,
   `vec(all_deriv_control_points) = N * vec(original_control_points)`.
   @pre derivative_order >= 0. */
  Eigen::SparseMatrix<double> DerivativeControlPointCoefficients(

...
class PiecewisePoseTrajectoryTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    dut_ =
        PiecewisePoseTrajectory<double>::MakeCubicLinearWithEndLinearVelocity(
            times, samples, vel0, vel1);
  ...
  PiecewisePoseTrajectory<double> dut_;
...
TEST_F(PiecewisePoseTrajectoryTest, TestEndLinearVelocity) {
...
TEST_F(PiecewisePoseTrajectoryTest, TestPose) {
...
TEST_F(PiecewisePoseTrajectoryTest, TestVelocity) {
  for (double time : test_times_) {
...
// PiecewisePolynomial.
TEST_F(PiecewisePoseTrajectoryTest, TestAccelertaion) {
  for (double time : test_times_) {
    Vector6<double> expected;
...
// Tests getters.
TEST_F(PiecewisePoseTrajectoryTest, TestGetTrajectory) {
...
TEST_F(PiecewisePoseTrajectoryTest, TestConstructor) {
  {
    PiecewisePoseTrajectory<double> equal(dut_.get_position_trajectory(),
...
// Tests is_approx().
TEST_F(PiecewisePoseTrajectoryTest, TestIsApprox) {
  ...
  {
    PiecewisePoseTrajectory<double> diff_position(new_pos_traj, orientation_);
    EXPECT_TRUE(!diff_position.is_approx(dut_, 1e-12));
  }

  {
    PiecewisePoseTrajectory<double> diff_orientation(position_, new_rot_traj);

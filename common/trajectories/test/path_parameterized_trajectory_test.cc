...
TEST_F(PathParameterizedTrajectoryTest, TestValue) {
  for (double t = 1.0; t < 2.0; t += 0.12) {
    EXPECT_TRUE(CompareMatrices(
        dut_->value(t), path_.value(time_scaling_.scalarValue(t)), 1e-14));
  }
}
...
TEST_F(PathParameterizedTrajectoryTest, TestDerivatives) {
  for (double t = 1.0; t < 2.0; t += 0.12) {
    ...
    double s = time_scaling_.scalarValue(t);
    double s_dot = time_scaling_.EvalDerivative(t, 1)(0, 0);
    VectorX<double> x_dot = path_.EvalDerivative(s, 1) * s_dot;
    EXPECT_TRUE(CompareMatrices(dut_->EvalDerivative(t, 1), x_dot, 1e-14));

    double s_ddot = time_scaling_.EvalDerivative(t, 2)(0, 0);
    ...
    double s_3dot = time_scaling_.EvalDerivative(t, 3)(0, 0);

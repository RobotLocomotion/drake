...
class PathParameterizedTrajectoryTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    parameterization_ = PiecewisePolynomial<double>::CubicShapePreserving(
        times, s_samples.transpose(), true);
    path_ = PiecewisePolynomial<double>::CubicShapePreserving(
        Vector2<double>(s_samples(0), s_samples(2)), x_samples, true);
    dut_ = std::make_unique<PathParameterizedTrajectory<double>>(
        path_, parameterization_);
  }
  PiecewisePolynomial<double> path_;
  PiecewisePolynomial<double> parameterization_;
...
TEST_F(PathParameterizedTrajectoryTest, TestValue) {
  for (double t = 1.0; t < 2.0; t += 0.12) {
    EXPECT_TRUE(CompareMatrices(
        dut_->value(t), path_.value(parameterization_.scalarValue(t)), 1e-14));
  }
}
...
TEST_F(PathParameterizedTrajectoryTest, TestDerivatives) {
  for (double t = 1.0; t < 2.0; t += 0.12) {
    ...
    double s = parameterization_.scalarValue(t);
    double s_dot = parameterization_.EvalDerivative(t, 1)(0, 0);
    VectorX<double> x_dot = path_.EvalDerivative(s, 1) * s_dot;
    EXPECT_TRUE(CompareMatrices(dut_->EvalDerivative(t, 1), x_dot, 1e-14));

    double s_ddot = parameterization_.EvalDerivative(t, 2)(0, 0);
    ...
    double s_3dot = parameterization_.EvalDerivative(t, 3)(0, 0);
...
TEST_F(PathParameterizedTrajectoryTest, TestGetTrajectory) {
  EXPECT_TRUE(dynamic_cast<const PiecewisePolynomial<double>&>(dut_->path())
                  .isApprox(path_, 1e-12));
  EXPECT_TRUE(
      dynamic_cast<const PiecewisePolynomial<double>&>(dut_->parameterization())
          .isApprox(parameterization_, 1e-12));
}
...
void TestScalarType() {
  ...
  PiecewisePolynomial<T> parameterization =
  ...
  PathParameterizedTrajectory<T> trajectory(path, parameterization);
}

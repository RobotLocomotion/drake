...
double DiscreteTimeTrajectory<T>::time_comparison_tolerance() const {
  return kTimeComparisonTolerance_;
}
...
std::unique_ptr<Trajectory<T>> DiscreteTimeTrajectory<T>::Clone() const {
  return std::make_unique<DiscreteTimeTrajectory<T>>(times_, values_,
                                                     kTimeComparisonTolerance_);
}

template <typename T>
MatrixX<T> DiscreteTimeTrajectory<T>::value(const T& t) const {
  using std::abs;
  const char* no_matching_time_str =
      "Value requested at time {} does not match any of the trajectory times "
      "within tolerance {}.";
  for (int i = 0; i < static_cast<int>(times_.size()); ++i) {
    if (t < times_[i] - kTimeComparisonTolerance_) {
      throw std::runtime_error(fmt::format(no_matching_time_str,
                                           ExtractDoubleOrThrow(t),
                                           kTimeComparisonTolerance_));
    }
    if (abs(t - times_[i]) <= kTimeComparisonTolerance_) {
      return values_[i];
    }
  }
  throw std::runtime_error(fmt::format(no_matching_time_str,

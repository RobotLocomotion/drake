...
std::unique_ptr<Trajectory<T>> PiecewisePose<T>::DoMakeDerivative(
  ...
  std::unique_ptr<Trajectory<T>> orientation_deriv =
  ...
  for (size_t ii = 0; ii < breaks.size(); ii++) {
    MatrixX<Polynomial<T>> segment_derivative(6, 1);
    // segment_derivative.template topRows<3>() =
    //     orientation_deriv->value(breaks[ii]);
    segment_derivative(0, 0) = orientation_deriv->value(breaks[ii])(0);
    segment_derivative(1, 0) = orientation_deriv->value(breaks[ii])(1);
    segment_derivative(2, 0) = orientation_deriv->value(breaks[ii])(2);

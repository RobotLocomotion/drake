...
class PiecewisePolynomial final : public PiecewiseTrajectory<T> {
 public:
   ...
   * methods differ by the continuity constraints that they enforce at sample
   ...
   *
   * @param periodic_end_condition Determines whether the "not-a-sample"
   * (`false`) or the periodic spline (`true`) end condition is used.
   *
   * @pre `breaks` and `samples` must have at least 3 elements. The "not-a-sample"
   * condition is ill-defined for two samples, and the "periodic" condition

...
class PiecewisePolynomial final : public PiecewiseTrajectory<T> {
 public:
  ...
  /**
   * Extracts a trajectory representing a block of size (p,q) starting at (i,j)
   * from the PiecewisePolynomial.
   * @returns a PiecewisePolynomial such that
   *   ret.value(t) = this.value(t).block(i,j,p,q);
   */
  PiecewisePolynomial Block(int i, int j, int p, int q) const;

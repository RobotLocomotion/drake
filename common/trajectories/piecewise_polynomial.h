...
class PiecewisePolynomial final : public PiecewiseTrajectory<T> {
 public:
   ...
   *          by @p t.  If T=symbolic::Expression, then this method will throw
   *          if t cannot be cast to double, or return only the
   *          symbolic::Expression for the current segment if @p t can be cast
   *          to double.

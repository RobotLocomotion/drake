...
class PiecewisePolynomial final : public PiecewiseTrajectory<T> {
 public:
  ...
  /**
   * Version of CubicWithContinuousSecondDerivatives(breaks, samples,
   * sample_dot_start, sample_dot_end) that uses vector samples and Eigen
   * VectorXd / MatrixX<T> inputs. Each column of `samples` represents a
   * sample point.
   *
   * @pre `samples.cols() == breaks.size()`.
   * @throws std::runtime_error under the conditions specified under

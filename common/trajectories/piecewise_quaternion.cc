...
std::unique_ptr<Trajectory<T>> PiecewiseQuaternionSlerp<T>::MakeDerivative(
  ...
  if (derivative_order == 1) {
    std::vector<Eigen::MatrixXd> velocities;
    std::copy(angular_velocities_.begin(), angular_velocities_.end(),
              velocities.begin());
    return PiecewisePolynomial<T>::ZeroOrderHold(this->breaks(), velocities)
        .Clone();
  }
  // All higher derivatives are simply zero.
  return std::make_unique<PiecewisePolynomial<T>>(Eigen::Vector3d::Zero());
}

...
template <typename T>
const Polynomial<T>&
PiecewisePolynomial<T>::getPolynomial(int segment_index,
                                                    Eigen::Index row,
...
operator+=(const PiecewisePolynomial<T>& other) {
  if (!this->SegmentTimesEqual(other, kEpsilonTime))
    throw runtime_error(
...
void PiecewisePolynomial<T>::
  ...
  for (size_t i = 0; i < times.size() - 1; i++) {
    if (times[i + 1] - times[i] < kEpsilonTime) {
...
template <typename T>
Eigen::Matrix<T, 4, 1>
PiecewisePolynomial<T>::ComputeCubicSplineCoeffs(
    double dt, T y0, T y1, T yd0,
    T yd1) {
  if (dt < kEpsilonTime) {

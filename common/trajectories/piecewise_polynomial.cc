...
template <typename T>
PiecewisePolynomial<T> PiecewisePolynomial<T>::Block(int i, int j, int p,
                                                     int q) const {
  DRAKE_DEMAND(i >= 0 && i < rows());
  DRAKE_DEMAND(j >= 0 && j < cols());
  DRAKE_DEMAND(p >= 0 && i + p <= rows());
  DRAKE_DEMAND(q >= 0 && j + q <= cols());
  ...
  std::transform(polynomials_.begin(), polynomials_.end(),
                 ...
                 [i, j, p, q](const PolynomialMatrix& matrix) {

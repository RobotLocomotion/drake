...
/* The StringifyParam family of functions takes a value passed to
 DRAKE_THROW_UNLESS and stringifies it. The family includes a number of
 overloads for built-in C++ types and a templated catch-all option. */
std::string StringifyParam(const char* value);
std::string StringifyParam(char value);
std::string StringifyParam(double value);
std::string StringifyParam(float value);
std::string StringifyParam(const std::filesystem::path& value);
std::string StringifyParam(const std::string& value);
std::string StringifyParam(const std::string_view& value);
template <typename T>
std::string StringifyParam(const T& value) {
  ...
  [&]<size_t... I>(std::integer_sequence<size_t, I...> &&) {
    (((buffer.values[I].first = std::get<2 * I>(std::move(pairs_tuple)),
       buffer.values[I].second =
           StringifyParam(std::get<2 * I + 1>(std::move(pairs_tuple)))),
      ...));
  }
...
This utility uses fmt::format to stringify the message values. Therefore, the
type passed must be compatible with fmt::to_string(). For eigen types, this
requires application of the fmt_eigen function. Such as:
...
message. Similarly, some types require fmt_streamed() such as:

...
Expression::Expression(std::unique_ptr<ExpressionCell> ptr) {
  set_shared_cell(ptr.release());
}
...
void Expression::set_shared_cell(const ExpressionCell* owned) noexcept {
  ...
  ++(owned->use_count());
  uintptr_t buffer;
  std::memcpy(&buffer, &owned, 8);
  buffer |= (0xFFFull << 52) | 1;
  std::memcpy(&value_, &buffer, 8);
...
const ExpressionCell& Expression::cell() const {
  DRAKE_ASSERT(std::isnan(value_));
  uintptr_t buffer;
  std::memcpy(&buffer, &value_, 8);
  buffer &= ~((0xFFFull << 52) | 1);
  ExpressionCell* owned;
  std::memcpy(&owned, &buffer, 8);
  return *owned;
}
...
Expression Expression::NaN() {
  return Expression{make_unique<ExpressionNaN>()};
}

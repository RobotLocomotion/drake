...
namespace drake {
namespace symbolic {
class PolynomialBasisTest : public ::testing::Test {
 protected:
  const Variable x_{"x"};
  const Variable y_{"y"};
  const Variable z_{"z"};
...
TEST_F(PolynomialBasisTest, ComputePolynomialBasisUpToDegree1) {
  ...
  // The chebyshev basis of (x, y) up to degree 0 is [1]
  ...
  // T1(z),T2(x) T1(y)T1(x), T1(z)T1(x), T2(y), T1(z)T1(y), T2(z)]
  ...
  // T1(z),T2(x) T1(y)T1(x), T1(z)T1(x), T2(y), T1(z)T1(y), T2(z), T3(x),
...
TEST_F(PolynomialBasisTest, ComputePolynomialBasisUpToDegree2) {
  // Test ComputePolynomialBasisUpToDegree with degree_type=DegreeType::kEven.

  // The chebyshev basis of x up to degree 0 (with even degrees) is [1]
  const auto result1 =
      ComputePolynomialBasisUpToDegree<Eigen::Dynamic, ChebyshevBasisElement>(
          Variables({x_}), 0, internal::DegreeType::kEven);
  EXPECT_EQ(result1.rows(), 1);
  EXPECT_EQ(result1(0), ChebyshevBasisElement());

  // The chebyshev basis of x up to degree 1 (with even degrees) is [1]
  const auto result2 =
      ComputePolynomialBasisUpToDegree<Eigen::Dynamic, ChebyshevBasisElement>(
          Variables({x_}), 1, internal::DegreeType::kEven);
  EXPECT_EQ(result2.rows(), 1);
  EXPECT_EQ(result2(0), ChebyshevBasisElement());

  // The chebyshev basis of x up to degree 2 (with even degrees) is [1, T2(x)]
  const auto result3 =
      ComputePolynomialBasisUpToDegree<Eigen::Dynamic, ChebyshevBasisElement>(
          Variables({x_}), 2, internal::DegreeType::kEven);
  EXPECT_EQ(result3.rows(), 2);
  EXPECT_EQ(result3(0), ChebyshevBasisElement());
  EXPECT_EQ(result3(1), ChebyshevBasisElement(x_, 2));

  // The chebyshev basis of x up to degree 3 (with even degrees) is [1, T2(x)]
  const auto result4 =
      ComputePolynomialBasisUpToDegree<Eigen::Dynamic, ChebyshevBasisElement>(
          Variables({x_}), 3, internal::DegreeType::kEven);
  EXPECT_EQ(result4.rows(), 2);
  EXPECT_EQ(result4(0), ChebyshevBasisElement());
  EXPECT_EQ(result4(1), ChebyshevBasisElement(x_, 2));

  // The chebyshev basis of (x, y) up to degree 2 (with even degrees) is [1,
  // T2(x), T1(x)T1(y) T2(y)].
  const auto result5 =
      ComputePolynomialBasisUpToDegree<Eigen::Dynamic, ChebyshevBasisElement>(
          Variables({x_, y_}), 2, internal::DegreeType::kEven);
  EXPECT_EQ(result5.rows(), 4);
  EXPECT_EQ(result5(0), ChebyshevBasisElement());
  EXPECT_EQ(result5(1), ChebyshevBasisElement(x_, 2));
  EXPECT_EQ(result5(2), ChebyshevBasisElement({{x_, 1}, {y_, 1}}));
  EXPECT_EQ(result5(3), ChebyshevBasisElement(y_, 2));

  // The chebyshev basis of (x, y) up to degree 3 (with even degrees) is [1,
  // T2(x), T1(x)T1(y) T2(y)].
  const auto result6 =
      ComputePolynomialBasisUpToDegree<Eigen::Dynamic, ChebyshevBasisElement>(
          Variables({x_, y_}), 3, internal::DegreeType::kEven);
  EXPECT_EQ(result6.rows(), 4);
  EXPECT_EQ(result6(0), ChebyshevBasisElement());
  EXPECT_EQ(result6(1), ChebyshevBasisElement(x_, 2));
  EXPECT_EQ(result6(2), ChebyshevBasisElement({{x_, 1}, {y_, 1}}));
  EXPECT_EQ(result6(3), ChebyshevBasisElement(y_, 2));
}

TEST_F(PolynomialBasisTest, ComputePolynomialBasisUpToDegree3) {
  ...
  // The chebyshev basis of x up to degree 1 (with odd degrees) is [T1(x)]
  const auto result2 =
      ComputePolynomialBasisUpToDegree<Eigen::Dynamic, ChebyshevBasisElement>(
          Variables({x_}), 1, internal::DegreeType::kOdd);
  EXPECT_EQ(result2.rows(), 1);
  EXPECT_EQ(result2(0), ChebyshevBasisElement(x_));

  // The chebyshev basis of x up to degree 2 (with odd degrees) is [T1(x)]
  const auto result3 =
      ComputePolynomialBasisUpToDegree<Eigen::Dynamic, ChebyshevBasisElement>(
          Variables({x_}), 2, internal::DegreeType::kOdd);
  EXPECT_EQ(result3.rows(), 1);
  EXPECT_EQ(result3(0), ChebyshevBasisElement(x_));

  // The chebyshev basis of x up to degree 3 (with odd degrees) is [T1(x),
  // T3(x)]
  const auto result4 =
      ComputePolynomialBasisUpToDegree<Eigen::Dynamic, ChebyshevBasisElement>(
          Variables({x_}), 3, internal::DegreeType::kOdd);
  EXPECT_EQ(result4.rows(), 2);
  EXPECT_EQ(result4(0), ChebyshevBasisElement(x_));
  EXPECT_EQ(result4(1), ChebyshevBasisElement(x_, 3));

  // The chebyshev basis of (x, y) up to degree 3 (with odd degrees) is [T1(x),
  // T1(y), T3(x), T2(x)T1(y), T1(x)T2(y), T3(y)]
  const auto result5 =
      ComputePolynomialBasisUpToDegree<Eigen::Dynamic, ChebyshevBasisElement>(
          Variables({x_, y_}), 3, internal::DegreeType::kOdd);
  EXPECT_EQ(result5.rows(), 6);
...
TEST_F(PolynomialBasisTest, ComputePolynomialBasisUpToDegree4) {
  // Test ComputePolynomialBasisUpToDegree with BasisElement = Monomial
  // The monomial basis of (x, y) up to degree 2 is [1, x, y, x^2, xy, y^2]
  const auto result1 =

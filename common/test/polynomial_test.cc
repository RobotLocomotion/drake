...
GTEST_TEST(PolynomialTest, IsApproxTest) {
  ...
  EXPECT_TRUE((100*x).IsApprox(99*x, 0.1, ToleranceType::relative));
  EXPECT_FALSE((100*x).IsApprox(99*x, 0.1, ToleranceType::absolute));
  EXPECT_FALSE((0.01*x).IsApprox(0.02*x, 0.1, ToleranceType::relative));
  EXPECT_TRUE((0.01*x).IsApprox(0.02*x, 0.1, ToleranceType::absolute));

  // Test missing monomials.
  EXPECT_FALSE((x + y + 0.01 * x * x).IsApprox(x + y));
  EXPECT_FALSE((x + y).IsApprox(x + y + 0.01 * x * x));
  // Missing is ok if their coefficient are less than tol (regardless of
  // ToleranceType).
  EXPECT_TRUE(
      (x + y + 0.01 * x * x).IsApprox(x + y, 0.02, ToleranceType::absolute));
  EXPECT_TRUE(
      (x + y).IsApprox(x + y + 0.01 * x * x, 0.02, ToleranceType::absolute));
  EXPECT_TRUE(
      (x + y + 0.01 * x * x).IsApprox(x + y, 0.02, ToleranceType::relative));
  EXPECT_TRUE(
      (x + y).IsApprox(x + y + 0.01 * x * x, 0.02, ToleranceType::relative));
}

...
TEST_F(SymbolicChebyshevBasisElementTest, Integration) {
  ...
  // ∫T2(x)dx = 1/5*T3(x) - 1/3*T1(x)
  const auto result3 = ChebyshevBasisElement({{x_, 2}}).Integration(x_);
  EXPECT_EQ(result3.size(), 2);
  EXPECT_EQ(result3.at(ChebyshevBasisElement({{x_, 3}})), 1. / 5);
  EXPECT_EQ(result3.at(ChebyshevBasisElement({{x_, 1}})), -1. / 3);

  // ∫T3(x)T2(y)dx = 1/7*T4(x)T2(y) - 1/5*T2(x)T2(y)
  const auto result4 =
      ChebyshevBasisElement({{x_, 3}, {y_, 2}}).Integration(x_);
  EXPECT_EQ(result4.size(), 2);
  EXPECT_EQ(result4.at(ChebyshevBasisElement({{x_, 4}, {y_, 2}})), 1. / 7);
  EXPECT_EQ(result4.at(ChebyshevBasisElement({{x_, 2}, {y_, 2}})), -1. / 5);
}

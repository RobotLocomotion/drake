...
TEST_F(AutoDiffXdTest, TieBreakingCheckMinBothNonEmpty) {
  ...
  EXPECT_EQ(min(v1, v2).derivatives()[0], 1.0);  // Returns v1, not v2.
}
...
TEST_F(AutoDiffXdTest, TieBreakingCheckMinOneNonEmpty) {
  ...
  const AutoDiffXd v2{1.0, Vector1<double>(2.)};
  EXPECT_EQ(min(v1, v2).derivatives()[0], 2.0);  // Returns v2, not v1.
  EXPECT_EQ(min(v2, v1).derivatives()[0], 2.0);  // Returns v2, not v1.
}

...
// Check that union_expected equals to the union of vars1 and vars2.
void CheckEqualToUnion(const symbolic::Variables& vars1,
                       const symbolic::Variables& vars2,
                       const symbolic::Variables& union_expected) {
  symbolic::Variables union_vars = vars1;
  union_vars.insert(vars2);
  EXPECT_EQ(union_vars, union_expected);
}
...
TEST_F(SymbolicPolynomialTest, AdditionPolynomialMonomial) {
  ...
  for (const Expression& e : exprs_) {
    ...
    for (int i = 0; i < monomials_.size(); ++i) {
      ...
      CheckEqualToUnion(p.indeterminates(), m.GetVariables(),
                        sum1.indeterminates());
      CheckEqualToUnion(p.indeterminates(), m.GetVariables(),
                        sum2.indeterminates());
      EXPECT_EQ(p.decision_variables(), sum1.decision_variables());
      EXPECT_EQ(p.decision_variables(), sum2.decision_variables());
    }
  }
  ...
  for (const Expression& e : exprs_) {
    for (int i = 0; i < monomials_.size(); ++i) {
      ...
      EXPECT_PRED2(ExprEqual, p.ToExpression(),
                   (e + m.ToExpression()).Expand());
      CheckEqualToUnion(e_indeterminates, m.GetVariables(), p.indeterminates());
...
TEST_F(SymbolicPolynomialTest, MultiplicationPolynomialPolynomial1) {
  ...
  for (const Expression& e1 : exprs_) {
    for (const Expression& e2 : exprs_) {
      ...
      // variables.
      CheckEqualToUnion(p1.indeterminates(), p2.indeterminates(),
                        product.indeterminates());
      CheckEqualToUnion(p1.decision_variables(), p2.decision_variables(),
                        product.decision_variables());
    }
  }
  // Test Polynomial& operator*=(Polynomial& c);
  for (const Expression& e1 : exprs_) {
    for (const Expression& e2 : exprs_) {
      ...
      EXPECT_PRED2(ExprEqual, p.ToExpression(),
                   (e1.Expand() * e2.Expand()).Expand());
      CheckEqualToUnion(e1_indeterminates, p2.indeterminates(),
                        p.indeterminates());
      CheckEqualToUnion(e1_decision_variables, p2.decision_variables(),
  ...
  for (const Expression& e : exprs_) {
    for (int i = 0; i < monomials_.size(); ++i) {
      ...
      EXPECT_PRED2(ExprEqual, product2.ToExpression(),
                   (m.ToExpression() * e).Expand());
      CheckEqualToUnion(p.indeterminates(), m.GetVariables(),
                        product1.indeterminates());
      CheckEqualToUnion(p.indeterminates(), m.GetVariables(),
                        product2.indeterminates());
      EXPECT_EQ(p.decision_variables(), product1.decision_variables());
      EXPECT_EQ(p.decision_variables(), product2.decision_variables());
    }
  }
  // Test Polynomial& operator*=(const Monomial& m);
  for (const Expression& e : exprs_) {

...
GTEST_TEST(EigenDenseStoragePoolsTest, DenseStoragePoolsBasics) {
  ...
  DenseStoragePools empty_mirror(empty);
  EXPECT_EQ(empty_mirror.rows(), 0);
  EXPECT_EQ(empty_mirror.data(), nullptr);

  DenseStoragePools some_mirror(some);
  ...
  EXPECT_NE(empty_assigned.data(), nullptr);
  ...
  DenseStoragePools empty_moved(std::move(empty_movee0));
  EXPECT_EQ(empty_moved.rows(), 0);
  EXPECT_EQ(empty_moved.data(), nullptr);

  DenseStoragePools some_movee0(10, 10, 1);
  DenseStoragePools some_moved(std::move(some_movee0));
  EXPECT_EQ(some_moved.rows(), 10);
  EXPECT_NE(some_moved.data(), nullptr);

  DenseStoragePools empty_movee1;
  DenseStoragePools empty_move_assigned(10, 10, 1);
  empty_move_assigned = std::move(empty_movee1);
  EXPECT_EQ(empty_move_assigned.rows(), 0);
  EXPECT_EQ(empty_move_assigned.data(), nullptr);
...
GTEST_TEST(EigenDenseStoragePoolsTest, DenseStoragePoolsConservativeResize) {

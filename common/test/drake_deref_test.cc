...
GTEST_TEST(DrakeDerefTest, Acceptance) {
  ...
  // macro, we need to launder it through a function.
  auto filter = [](bool value) {
    return value;
  };

  EXPECT_TRUE(filter(std::is_same_v<decltype(DRAKE_DEREF(p1)), int&>));
  EXPECT_TRUE(filter(std::is_same_v<decltype(DRAKE_DEREF(p2)), int&>));
  EXPECT_TRUE(filter(std::is_same_v<decltype(DRAKE_DEREF(p3)), const int&>));
  EXPECT_TRUE(filter(std::is_same_v<decltype(DRAKE_DEREF(p4)), const int&>));
  ...
  EXPECT_TRUE(filter(std::is_same_v<decltype(DRAKE_DEREF(up1)), int&>));
  EXPECT_TRUE(filter(std::is_same_v<decltype(DRAKE_DEREF(up2)), int&>));
  EXPECT_TRUE(filter(std::is_same_v<decltype(DRAKE_DEREF(up3)), const int&>));
  EXPECT_TRUE(filter(std::is_same_v<decltype(DRAKE_DEREF(up4)), const int&>));
  ...
  EXPECT_TRUE(filter(std::is_same_v<decltype(DRAKE_DEREF(sp1)), int&>));
  EXPECT_TRUE(filter(std::is_same_v<decltype(DRAKE_DEREF(sp2)), int&>));
  EXPECT_TRUE(filter(std::is_same_v<decltype(DRAKE_DEREF(sp3)), const int&>));
  EXPECT_TRUE(filter(std::is_same_v<decltype(DRAKE_DEREF(sp4)), const int&>));
}

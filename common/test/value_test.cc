...
// not the possibly-more-derived type of the contained value. See #15434.
GTEST_TEST(ValueTest, TypesInBadCastMessage) {
  using systems::MyVector;

  // Make a base-typed value container with a more-derived value inside.
  auto value =
      std::make_unique<Value<BasicVector<double>>>(MyVector<double, 1>{});
  AbstractValue& abstract = *value;

  // This request looks like it should work, but doesn't. The error message
  // should indicate why.
  DRAKE_EXPECT_THROWS_MESSAGE(
      (abstract.get_value<MyVector<double, 1>>()),
      ".*request.*MyVector.*actual.*BasicVector.*");

...
void CheckCalcProbabilityDensityUniform() {
  ...
  for (const auto& x :
    ...
    T density = CalcProbabilityDensity(RandomDistribution::kUniform, x);
...
void CheckCalcProbabilityDensityGaussian() {
  for (const auto& x :
    ...
    const T density = CalcProbabilityDensity(RandomDistribution::kGaussian, x);
...
void CheckCalcProbabilityDensityExponential() {
  ...
  for (const auto& x : {Vector3<T>(0.1, 0.5, 0.), Vector3<T>(0.1, 0.9, 1.5)}) {
    ...
    EXPECT_NEAR(ExtractDoubleOrThrow(CalcProbabilityDensity(
  ...
  for (const Vector3<T>& x_zero_prob :
    ...
    EXPECT_EQ(ExtractDoubleOrThrow(CalcProbabilityDensity(

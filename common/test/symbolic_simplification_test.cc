...
std::function<Expression(const Variable& x)>UnaryOpToF(UnaryTestOp op) {
  switch (op) {
...
TEST_P(SymbolicUnificationTestUnary, Check) {
  const Expression& lhs = UnaryOpToF(GetParam())(x_);
  const RewritingRule rule{lhs, x_};
  const Rewriter rewriter = MakeRuleRewriter(rule);
  const Expression& e1 = UnaryOpToF(GetParam())(a_);
...
TEST_P(SymbolicUnificationTestBinary, Check) {
  const Expression& lhs = BinaryOpToF(GetParam())(x_, y_);
  const RewritingRule rule{lhs, x_ + y_};
  const Rewriter rewriter = MakeRuleRewriter(rule);
  const Expression& e1 = BinaryOpToF(GetParam())(a_, b_);
  EXPECT_PRED2(ExprEqual, rewriter(e1), a_ + b_);

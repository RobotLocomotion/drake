...
GTEST_TEST(DrakeThrowTest, ThrowWithValues) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(f1(1), ".*a = 1.5.*");
  DRAKE_EXPECT_THROWS_MESSAGE(f1(2), ".*a = 1.5, b = 2.5.");
  DRAKE_EXPECT_THROWS_MESSAGE(f1(3), ".*a = 1.5, b = 2.5, c = 3.5.");
  DRAKE_EXPECT_THROWS_MESSAGE(f1(4), ".*a = 1.5, b = 2.5, c = 3.5, d = 4.5.");
}

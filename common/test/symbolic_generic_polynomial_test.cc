...
TEST_F(SymbolicGenericPolynomialTest, DefaultConstructors) {
  ...
  const GenericPolynomial<MonomialBasisElement> p3(0);
  EXPECT_TRUE(p3.basis_element_to_coefficient_map().empty());

  const GenericPolynomial<ChebyshevBasisElement> p4(0);
...
TEST_F(SymbolicGenericPolynomialTest, DifferentiateForChebyshevBasis) {
  ...
  EXPECT_PRED2(GenericPolyEqual<ChebyshevBasisElement>, p.Differentiate(var_z_),
               GenericPolynomial<ChebyshevBasisElement>(0));

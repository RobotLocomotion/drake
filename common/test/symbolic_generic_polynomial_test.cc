...
void CheckAdditionPolynomialDouble(const std::vector<Expression>& exprs,
  ...
  for (const Expression& e : exprs) {
    for (const double c : doubles) {
      EXPECT_PRED2(
          ExprEqual,
          (GenericPolynomial<MonomialBasisElement>(e) + c).ToExpression(),
          (e + c).Expand());
      EXPECT_PRED2(
          ExprEqual,
          (c + GenericPolynomial<MonomialBasisElement>(e)).ToExpression(),
          (c + e).Expand());
    }
  }
...
void CheckAdditionPolynomialVariable(const std::vector<Expression>& exprs,
  ...
  for (const Expression& e : exprs) {
    for (const Variable& v : vars) {
      EXPECT_PRED2(
          ExprEqual,
          (GenericPolynomial<MonomialBasisElement>(e) + v).ToExpression(),
          (e + v).Expand());
      EXPECT_PRED2(
          ExprEqual,
          (v + GenericPolynomial<MonomialBasisElement>(e)).ToExpression(),
          (v + e).Expand());
    }
  }

...
std::map<MonomialBasisElement, double> operator*(
  ...
  std::map<Variable, int> var_to_degree_map_product;
  auto it1 = m1.var_to_degree_map().begin();
  auto it2 = m2.var_to_degree_map().begin();
  while (it1 != m1.var_to_degree_map().end() &&
         it2 != m2.var_to_degree_map().end()) {
    const Variable& var1 = it1->first;
    const Variable& var2 = it2->first;
    if (var1.less(var2)) {
      var_to_degree_map_product.emplace(var1, it1->second);
      it1++;
    } else if (var2.less(var1)) {
      var_to_degree_map_product.emplace(var2, it2->second);
      it2++;
    } else {
      // var1 == var2
      var_to_degree_map_product.emplace(var1, it1->second + it2->second);
      it1++;
      it2++;
    }
  }
  while (it1 != m1.var_to_degree_map().end()) {
    var_to_degree_map_product.emplace(it1->first, it1->second);
    it1++;
  }
  while (it2 != m2.var_to_degree_map().end()) {
    var_to_degree_map_product.emplace(it2->first, it2->second);
    it2++;
  }

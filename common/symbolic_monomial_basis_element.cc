...
MonomialBasisElement& MonomialBasisElement::operator*=(
    const MonomialBasisElement& m) {
  for (const auto& p : m.var_to_degree_map()) {
    const Variable& var{p.first};
    const int exponent{p.second};
    auto it = get_mutable_var_to_degree_map()->find(var);
    if (it == var_to_degree_map().end()) {
      get_mutable_var_to_degree_map()->insert(p);
    } else {
      it->second += exponent;
    }
    (*get_mutable_total_degree()) += exponent;
  }
  return *this;
}
...
std::map<MonomialBasisElement, double> operator*(
    MonomialBasisElement m1, const MonomialBasisElement& m2) {
  m1 *= m2;
  return std::map<MonomialBasisElement, double>{{{m1, 1.}}};
}

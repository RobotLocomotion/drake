...
pair<int, Monomial> DifferentiateMonomial(const Monomial& m,
  ...
  map<Variable, int, VariableLess> powers{m.get_powers()};
...
Polynomial Polynomial::SubstituteAndExpand(
  ...
  auto compute_substituted_monomial_expansion =
    ...
    if (nearest_cached_monomial == Monomial()) {
      ...
      std::map<Variable, int, VariableLess> halved_powers;
      ...
      std::map<Variable, int, VariableLess> remaining_powers;
      const std::map<Variable, int, VariableLess>& cached_powers{

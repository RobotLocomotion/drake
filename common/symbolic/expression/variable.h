  ...
  class Id {
   ...
   private:
    ...
    Type get_type() const {
      ...
      return static_cast<Type>(hi_ >> (64 - 8));
    }

    bool is_default() const { return lo_ == 0 && hi_ == 0; }

    // Id stores three pieces of information:
    ...
    //     time it's needed; all variables created within the process have the
    //     same nonce; this helps distinguish variables created by one process
    //     from those created by another process when saving them to disk.
    //
    // For efficiency, we pack these three pieces into two member fields:
    // - the 8-bit type is stored in the upper byte of hi_;
    // - the 88-bit nonce is stored as:
    //   - 56 bits in the lower 7 bytes of hi_ and
    //   - 32 bits in the upper 4 bytes of lo_;
    ...
    // which can therfore provide a run-to-run stable hash key when an Id is

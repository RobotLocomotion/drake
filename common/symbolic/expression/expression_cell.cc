...
void AssertAlreadyExpanded(const Expression& e, int line) {
  if constexpr (kDrakeAssertIsArmed) {
    ...
    if (!e.EqualTo(e2)) {
      throw std::runtime_error(fmt::format(
          ...
          " changed to '{}' after another expansion", __FILE__, line, e, e2));
    }
  }
}
...
Expression ExpandPow(const Expression& base, const int n) {
  ...
  AssertAlreadyExpanded(base, __LINE__);
  DRAKE_ASSERT(n >= 1);
  if (n == 1) {
    return base;
  }
...
Expression ExpandPow(const Expression& base, const Expression& exponent) {
  ...
  AssertAlreadyExpanded(base, __LINE__);
  AssertAlreadyExpanded(exponent, __LINE__);

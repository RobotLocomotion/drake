...
const map<Expression, double, ExpressionLess>&
...
const map<Expression, Expression, ExpressionLess>&
...
void Gemm<reverse>::CalcDV(const MatrixRef<double>& D,
  ...
  for (int i = 0; i < max_i; ++i) {
    ...
    std::sort(inner_vars.begin(), inner_vars.end(), VariableLess{});
    ...
    for (int j = 0; j < max_j; ++j) {
      ...
      using TermsMap = map<Expression, double, ExpressionLess>;

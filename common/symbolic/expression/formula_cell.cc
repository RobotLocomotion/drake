...
NaryFormulaCell::NaryFormulaCell(const FormulaKind k,
                                 set<Formula, FormulaLess> formulas)
...
string NaryFormulaCell::DisplayWithOp(const string& op) const {
  std::stringstream os;
  const set<Formula, FormulaLess>& formulas{get_operands()};
...
FormulaAnd::FormulaAnd(const set<Formula, FormulaLess>& formulas)
...
FormulaAnd::FormulaAnd(const Formula& f1, const Formula& f2)
    : NaryFormulaCell{FormulaKind::And, set<Formula, FormulaLess>{f1, f2}} {}
...
FormulaOr::FormulaOr(const set<Formula, FormulaLess>& formulas)
    : NaryFormulaCell{FormulaKind::Or, formulas} {
  DRAKE_ASSERT(get_operands().size() > 1U);
}

FormulaOr::FormulaOr(const Formula& f1, const Formula& f2)
    : NaryFormulaCell{FormulaKind::Or, set<Formula, FormulaLess>{f1, f2}} {}

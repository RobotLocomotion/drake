...
This only used for expressions with `kind != ExpressionKind::Constant`; for
...
In the following sections, we'll summare the reasons we're using nanboxing,
...
(2) Speculative arthmetic.

For floating-point arithmetic in particular, representing pointers as NaNs
allows us to use _speculative_ arthmetic for even more throughput.
...
evaluation. Those 8 branches (>1 per flop) are not cheap, though! Ideally we
would like to see very few branches per flop on average, for best throughput.
...
subtracting infininty from itself, or dividing by zero). This is a rare case,
...
class BoxedCell {
 public:
  ...
  /* Sets this to point at the given cell, incrementing its use_count.
  of the cell_to_share.
 ...
 private:
  ...
  comment near the `value_` member field for additional details. */

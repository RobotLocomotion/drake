...
struct GradedReverseLexOrder {
  ...
  bool operator()(const Monomial& m1, const Monomial& m2) const {
    ...
    const std::map<Variable, int, VariableLess>& powers1{m1.get_powers()};
    const std::map<Variable, int, VariableLess>& powers2{m2.get_powers()};
    std::map<Variable, int, VariableLess>::const_iterator it1{powers1.cbegin()};
    std::map<Variable, int, VariableLess>::const_iterator it2{powers2.cbegin()};
 ...
 * AddPolynomialBasisElementsOfDegreeN in polynomial_basis.h instead when we
 * deprecate Monomial class.
 */
...
Eigen::Matrix<Monomial, rows, 1> ComputeMonomialBasis(
  ...
  // 1. Collect monomials.
  std::set<Monomial, GradedReverseLexOrder<VariableLess>> monomials;
  int start_degree = 0;
  int degree_stride = 1;
  switch (degree_type) {

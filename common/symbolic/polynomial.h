...
class Polynomial {
 public:
  ...
  /// An encapuslated data type for use with the method SubstituteAndExpand.
  ...
  /// necessary.
  /// @param[in,out] substitutions_cached_data A map caching the higher order
  /// expansions of the `indeterminate_substitutions`. Typically, the first time
  /// an indeterminate_substitution is performed, substitutions will be either a
  ...
  /// = x⁷. A user map call p1.SubstituteAndExpand({x : a(1-y),
  /// substitutions_cached_data}) where substitutions is a pointer to an empty
  /// map. As part of computing the expansion of p1, the expansion of x⁷ may get
  /// computed and stored in substitutions, and so a subsequent call of
  ...
  /// passed substitutions_cached_data object is consistent with

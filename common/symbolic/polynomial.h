...
/**
 Compute the polynomial as m(x)ᵀ * Q * m(x), where m(x) is the monomial basis,
 and Q is the Gram matrix.
 @param monomial_basis m(x) in the documentation. A vector of monomials
 @pagram gram_lower Stacking each column of the lower triangular entries in Q
 into a vector.
 */
...
CalcPolynomialWGramLower(const Eigen::MatrixBase<Derived1>& monomial_basis,

...
class DerivedBasisA : public PolynomialBasisElement {
 public:
  ...
  std::pair<double, DerivedBasisA> EvaluatePartial(
    ...
    double coeff;
    std::map<Variable, int, VariableLess> new_var_to_degree_map;
...
TEST_F(SymbolicPolynomialBasisElementTest, Constructor) {
  ...
  const DerivedBasisA p4(std::map<Variable, int, VariableLess>({}));
...
TEST_F(SymbolicPolynomialBasisElementTest, degree) {
  ...
  EXPECT_EQ(DerivedBasisA(std::map<Variable, int, VariableLess>()).degree(x_),
            0);
}
...
TEST_F(SymbolicPolynomialBasisElementTest, GetVariables) {
  ...
  const symbolic::DerivedBasisA p4(std::map<Variable, int, VariableLess>({}));
...
TEST_F(SymbolicPolynomialBasisElementTest, EigenMatrix) {
  ...
  M << DerivedBasisA(std::map<Variable, int, VariableLess>({})),
...
TEST_F(SymbolicPolynomialBasisElementTest, BasisElementGradedReverseLexOrder) {
  ...
  BasisElementGradedReverseLexOrder<VariableLess, DerivedBasisA> compare;
  // y^0 = x^0 = 1.
  ...
  derived_basis_all.emplace_back(
      std::map<Variable, int, VariableLess>{{x_, 0}});
  derived_basis_all.emplace_back(
      std::map<Variable, int, VariableLess>{{x_, 1}});
  derived_basis_all.emplace_back(
      std::map<Variable, int, VariableLess>{{y_, 1}});
  derived_basis_all.emplace_back(
      std::map<Variable, int, VariableLess>{{z_, 1}});
  derived_basis_all.emplace_back(
      std::map<Variable, int, VariableLess>{{x_, 2}});
  derived_basis_all.emplace_back(
      std::map<Variable, int, VariableLess>{{x_, 1}, {y_, 1}});
  derived_basis_all.emplace_back(
      std::map<Variable, int, VariableLess>{{x_, 1}, {z_, 1}});
  derived_basis_all.emplace_back(
      std::map<Variable, int, VariableLess>{{y_, 2}});
  derived_basis_all.emplace_back(
      std::map<Variable, int, VariableLess>{{y_, 1}, {z_, 1}});
  derived_basis_all.emplace_back(
      std::map<Variable, int, VariableLess>{{z_, 2}});

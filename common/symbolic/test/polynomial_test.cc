...
TEST_F(SymbolicPolynomialTest, ConstructorFromExpressionAndIndeterminates3) {
  ...
  const symbolic::Expression e1{0 * pow(x_, 2)};
  const symbolic::Polynomial p1{e1, {var_x_}};
  EXPECT_TRUE(p1.monomial_to_coefficient_map().empty());

  const symbolic::Expression e2{3 * pow(x_ + 0 * y_, 2)};
  const symbolic::Polynomial p2{e2, {var_x_}};
  EXPECT_EQ(p2.monomial_to_coefficient_map().size(), 1u);
  EXPECT_EQ(p2.monomial_to_coefficient_map().at(Monomial(var_x_, 2)), 3);
}

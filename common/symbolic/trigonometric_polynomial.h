 ...
 * If the indeterminates of `e` include cosθ and sinθ, and e is a multilinear
 * polynomial of cosθ and sinθ with no cosθᵢ and sinθᵢ appear in the same
 * monomial, then we replace cosθᵢ with (1-tᵢ²)/(1+tᵢ²), and sinθᵢ with
 * 2tᵢ/(1+tᵢ²), and get a rational polynomial. The indeterminates of this
 * rational polynomial are t together with the indeterminates in `e` that are
 * not cosθ or sinθ. If the input expression doesn't contain
 * the sine and cosine functions, then the returned rational has denominator
 * being 1. Notice that the indeterminates of `e` can include
 * variables other than cosθ and sinθ, and we impose no requirements on these
 * variables that are not cosθ or sinθ.
 *
 * For example,

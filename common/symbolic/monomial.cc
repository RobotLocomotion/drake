...
int TotalDegree(const map<Variable, int, Variable::CompareLess>& powers) {
...
// symbolic::Expression&).
map<Variable, int, Variable::CompareLess> ToMonomialPower(const Expression& e) {
  // TODO(soonho): Re-implement this function by using a Polynomial visitor.
  DRAKE_DEMAND(e.is_polynomial());
  map<Variable, int, Variable::CompareLess> powers;
...
// types of arguments.
map<Variable, int, Variable::CompareLess> ToMonomialPower(
...
Monomial::Monomial(const map<Variable, int, Variable::CompareLess>& powers)

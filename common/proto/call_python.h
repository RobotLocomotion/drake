...
#include "drake/common/proto/python_remote_procedure_call.pb.h"
...
using MatlabRPC
    DRAKE_DEPRECATED("2019-12-01", "Use PythonRemoteProcedureCall instead.")
    = PythonRemoteProcedureCall;
...
inline void AssembleRemoteProcedureCall(PythonRemoteProcedureCall*) {
  // Intentionally left blank.  Base case for template recursion.
}

template <typename T, typename... Types>
void AssembleRemoteProcedureCall(PythonRemoteProcedureCall* rpc, T first,
                               Types... args) {
  ToPythonRemoteData(first, rpc->add_rhs());
  AssembleRemoteProcedureCall(rpc, args...);
}
...
void PublishCallPython(const PythonRemoteProcedureCall& rpc);
...
PythonRemoteVariable CallPython(const std::string& function_name,
  ...
  PythonRemoteProcedureCall rpc;
  rpc.add_lhs(output.unique_id());
  internal::AssembleRemoteProcedureCall(&rpc, args...);
  rpc.set_function_name(function_name);
  internal::PublishCallPython(rpc);
  return output;
}

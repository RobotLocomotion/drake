...
class CallPythonClient(object):
    ...
    def _handle_messages_threaded(self):
        ...
        # Clear the queue to release blocking on `queue.join()`.
        while not queue.empty():
            queue.get()
            queue.task_done()
        producer.join()
    ...
    def _get_file(self):
        ...
        if self._file is None:
            # Open FIFO as non-blocking, so that we may easily handle looping.
            # TODO(eric.cousineau): This is different than normal files because
            # O_NONBLOCK generally only affects opening files. However, for
            # FIFO files, it also affects how reading occurs.
            # Consider using a timeout instead if this becomes a problem.

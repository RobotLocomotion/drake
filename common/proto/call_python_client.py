...
class CallPythonClient(object):
    ...
    def _execute_message_impl(self, msg):
        ...
        for i, arg in enumerate(msg.rhs):
            ...
            if data_type == lcmt_call_python_data.REMOTE_VARIABLE_REFERENCE:
            ...
            elif data_type == lcmt_call_python_data.DOUBLE:
                value = self._to_array(arg, np.double)
            elif data_type == lcmt_call_python_data.CHAR:
                assert arg.rows == 1
                value = arg.data.decode('utf8')
            elif data_type == lcmt_call_python_data.LOGICAL:
                value = self._to_array(arg, np.bool)
            elif data_type == lcmt_call_python_data.INT:

...
class CallPythonClient(object):
    ...
    def _to_array(self, arg, dtype):
        ...
        # scalar).
        np_raw = np.frombuffer(bytes(arg['data']), dtype=dtype)
    ...
    def _execute_message_impl(self, msg):
        ...
        kwargs = None
        for i, arg in enumerate(args):
            arg_raw = bytes(arg['data'])
            value = None
            arg_type = arg['type']
            if arg_type == PythonRemoteData.REMOTE_VARIABLE_REFERENCE:
            ...
            elif arg_type == PythonRemoteData.DOUBLE:
                value = self._to_array(arg, np.double)
            elif arg_type == PythonRemoteData.CHAR:
                assert arg['rows'] == 1
                value = arg_raw.decode('utf8')
            elif arg_type == PythonRemoteData.LOGICAL:
                value = self._to_array(arg, np.bool)
            elif arg_type == PythonRemoteData.INT:
    ...
    def _read_fifo_message(self, fifo):
        ...
        while not self._done:
            ...
            if len(buffer) == datagram_size:
                byte = fifo.read(1)
                assert byte == b'\0'  # EOM
                message = umsgpack.unpackb(bytes(buffer))
                return message

...
void internal::ToPythonRemoteDataMatrix(
...
void PublishCall(
    ...
    const PythonRemoteProcedureCall& rpc) {
  ...
  {  // Defines the lifetime of the CodedOutputStream.
    ...
    if (buffer != nullptr) {
      ...
      rpc.SerializeWithCachedSizesToArray(buffer);
    } else {
      // Slightly-slower path when the message is multiple buffers.
      rpc.SerializeWithCachedSizes(&output);
...
void internal::PublishCallPython(const PythonRemoteProcedureCall& rpc) {
  PublishCall(GetPythonOutputStream(), rpc);
}

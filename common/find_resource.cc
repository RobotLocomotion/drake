...
// Assume root is Drake's resource root, check for the sentinel file and return
// the found resource_path.
  ...
  for (const auto& directory : {
    ...
    if (StartsWith(substr, directory)) {
      ...
      if (rlocation_or_error.error.empty()) {
        return rlocation_or_error.rlocation_abspath;
      }
    }
  }
  return nullopt;
...
optional<string> MaybeGetInstallResourceRoot() {
  ...
  if (libdrake_dir) {
    spruce::path root = *libdrake_dir + "/../share";
    if (root.isDir()) {
      return root.getStr();
    } else {
      log()->debug("FindResource ignoring CMake install candidate '{}' "
                   "because it does not exist", root.getStr());
    }
  } else {
  ...
  if (internal::HasRunfiles()) {
    ...
    if (rlocation_or_error.error.empty()) {
      return Result::make_success(
          resource_path, rlocation_or_error.rlocation_abspath);
    }

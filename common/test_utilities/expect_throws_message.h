...
omitted.  New code should always omit it, because we intend to deprecate its
use in the future.  In other words, call this macro as (expression, regexp),
without the middle argument for which subclass of exception to expect.
...
#define DRAKE_OVERLOAD_THROWS_MESSGE_MACRO_ARITY(_1, _2, _3, NAME, ...) NAME
#define DRAKE_EXPECT_THROWS_MESSAGE(...) \
  DRAKE_OVERLOAD_THROWS_MESSGE_MACRO_ARITY(__VA_ARGS__, \

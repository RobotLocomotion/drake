...
@see DRAKE_ASSERT_ERROR_MESSAGE
@see DRAKE_EXPECT_ERROR_MESSAGE_IF_ARMED, DRAKE_ASSERT_ERROR_MESSAGE_IF_ARMED */
#define DRAKE_EXPECT_ERROR_MESSAGE(expression, exception, regexp)

/** Fatal error version of `DRAKE_EXPECT_ERROR_MESSAGE`.
@see DRAKE_EXPECT_ERROR_MESSAGE */
#define DRAKE_ASSERT_ERROR_MESSAGE(expression, exception, regexp)

/** Same as `DRAKE_EXPECT_ERROR_MESSAGE` in Debug builds, but doesn't require
...
/** Same as `DRAKE_ASSERT_ERROR_MESSAGE` in Debug builds, but doesn't require
a throw in Release builds. However, if the Release build does throw it must
throw the right message. More precisely, the thrown message is required
whenever `DRAKE_ENABLE_ASSERTS` is defined, which Debug builds do be default.
@see DRAKE_ASSERT_ERROR_MESSAGE */
...
#define DRAKE_EXPECT_ERROR_MESSAGE_HELPER(expression, exception, regexp, \
...
#define DRAKE_EXPECT_ERROR_MESSAGE(expression, exception, regexp) \
  DRAKE_EXPECT_ERROR_MESSAGE_HELPER(expression, exception, regexp, \

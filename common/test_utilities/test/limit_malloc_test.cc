...
// A call to realloc() that does not change the size should not be guarded.
GTEST_TEST(LimitCallocTest, ChangingSizeTest) {
  void* dummy = malloc(16);;
  g_dummy = dummy;
...
// A call to realloc() that does not change the size should not be guarded.
GTEST_TEST(LimitCallocDeathTest, ChangingSizeTest) {
  ::testing::FLAGS_gtest_death_test_style = "threadsafe";
  void* dummy = malloc(16);;
  g_dummy = dummy;
  ASSERT_TRUE(g_dummy != nullptr);

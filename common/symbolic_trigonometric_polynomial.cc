...
class SinCosVisitor {
 public:
  ...
  explicit SinCosVisitor(SinCosSubstitution s, bool half_angle = false)
      : subs_{std::move(s)}, half_angle_{half_angle} {};
  ...
  [[nodiscard]] Expression VisitVariable(const Expression& e,
    ...
    if (iter == subs_.end()) {
      ...
      return half_angle_ ? 2 * iter->second.s * iter->second.c : iter->second.s;
    } else if (status == kInsideCos) {
      return half_angle_ ? 1 - 2 * iter->second.s * iter->second.s
                         : iter->second.c;
    }
    return e;
  }
...
Expression Substitute(const Expression& e, const SinCosSubstitution& subs,
                      bool half_angle) {
  return SinCosVisitor(subs, half_angle).Substitute(e);
}

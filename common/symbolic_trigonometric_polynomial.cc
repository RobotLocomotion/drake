...
bool CheckPolynomialIndeterminatesAreSinCos(
  ...
  for (const auto& [sin_var, cos_var] : sin_cos) {
    sin_cos_vars.insert(sin_var);
    sin_cos_vars.insert(cos_var);
  }
...
void SubstituteStereographicProjectionImpl(
  ...
  for (const auto& pair : e_poly.monomial_to_coefficient_map()) {
    ...
    for (int i = 0; i < static_cast<int>(sin_cos.size()); ++i) {
      const auto& sin_var = sin_cos[i].first;
      const auto& cos_var = sin_cos[i].second;
  ...
  for (const auto& [monomial, coeff] : e_poly.monomial_to_coefficient_map()) {
    ...
    for (int angle_index : angle_indices) {
      const auto& sin_var = sin_cos[angle_index].first;
      const auto& cos_var = sin_cos[angle_index].second;
...
symbolic::RationalFunction SubstituteStereographicProjection(
  ...
  symbolic::Variables sin_cos_vars;
  std::vector<std::pair<symbolic::Variable, symbolic::Variable>> sin_cos_vec(
      subs.size());
  ...
  for (const auto& [theta, t] : subs) {
    ...
    sin_cos_vec[var_count] = std::make_pair(sin_cos.s, sin_cos.c);
    t_vars(var_count) = t;
    var_count++;
  }

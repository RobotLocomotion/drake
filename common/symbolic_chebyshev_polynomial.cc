...
namespace drake {
namespace symbolic {
ChebyshevPolynomial::ChebyshevPolynomial(const symbolic::Variable& var,
                                         int degree)
    : var_{var}, degree_{degree} {
...
symbolic::Polynomial ChebyshevPolynomial::ToPolynomial() const {
  if (degree_ == 0) {
    // Return 1.
    return symbolic::Polynomial(symbolic::Monomial());
  } else if (degree_ == 1) {
    // Return x.
    return symbolic::Polynomial(symbolic::Monomial(var_, 1));
  } else if (degree_ == 2) {
    // Return 2x^2 - 1
    return symbolic::Polynomial(
        {{symbolic::Monomial(var_, 2), 2}, {symbolic::Monomial(), -1}});
  } else {
    ...
    symbolic::Polynomial::MapType monomial_to_coefficient_map;
    for (int i = 0; i <= degree_; ++i) {
      if (monomial_coeffs(i, next_col_index) != 0) {
        monomial_to_coefficient_map.emplace(symbolic::Monomial(var_, i),
                                            monomial_coeffs(i, next_col_index));
      }
    }
    return symbolic::Polynomial(monomial_to_coefficient_map);
  }
}
...
bool ChebyshevPolynomial::operator==(const ChebyshevPolynomial& other) const {
  ...
  return this->var().equal_to(other.var()) && this->degree() == other.degree();
}
...
bool CompareChebyshevPolynomial::operator()(const ChebyshevPolynomial& lhs,
  ...
  if (lhs.degree() == 0 && rhs.degree() == 0) {
    // Both lhs and rhs are 1.
    return false;
  } else if (lhs.degree() == 0) {
    // lhs is 1, rhs is larger than lhs.
    return true;
  } else if (rhs.degree() == 0) {
    // rhs is 1, lhs is larger than rhs.
    return false;
  } else if (lhs.var().get_id() < rhs.var().get_id()) {
    return true;
  } else if (lhs.var() == rhs.var() && lhs.degree() < rhs.degree()) {
    return true;
  }
  return false;
}

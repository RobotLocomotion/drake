...
void ArgSweep(benchmark::internal::Benchmark* benchmark, bool use_double) {
  for (const auto& left_side : {kDerEmpty, kDerDense}) {
    for (const auto& right_side : {kDerNone, kDerEmpty, kDerDense}) {
      if (right_side == kDerNone && !use_double) {
        continue;
      }
      if (left_side == kDerEmpty && right_side == kDerDense) {

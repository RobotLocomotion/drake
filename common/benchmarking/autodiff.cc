...
void ArgSweep(benchmark::Benchmark* benchmark, bool use_double) {
  for (const auto& left_side :
       {kDerivativesEmpty, kDerivativesUnit, kDerivativesDense}) {
    for (const auto& right_side : {kDerivativesNone, kDerivativesEmpty,
                                   kDerivativesUnit, kDerivativesDense}) {

...
class ChebyshevBasisElement : public PolynomialBasisElement {
 public:
  ...
  /** Merge this Chebyshev basis element with another Chebyshev basis element by
   * merging their var_to_degree_map. After merging, the degree of each variable
   * is rasied to the sum of the degree in each basis element (if a variable
   * does not show up in either one of the basis element, we regard its degree
   * to be 0). For example, merging T₁(x)T₃(y) and T₂(x)T₄(z) gets
   * T₃(x)T₃(y)T₄(z).
   */

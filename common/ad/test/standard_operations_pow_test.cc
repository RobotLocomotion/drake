...
// Our pow(ADS, double) implementation has special cases for small integer exp.
// Check those against Eigen's reference implementation.
TEST_F(StandardOperationsTest, PowAdsSmallInt) {
  CHECK_BINARY_FUNCTION_ADS_SCALAR(pow, x, y, -3.0);
  CHECK_BINARY_FUNCTION_ADS_SCALAR(pow, x, y, -2.0);
  CHECK_BINARY_FUNCTION_ADS_SCALAR(pow, x, y, -1.0);
  CHECK_BINARY_FUNCTION_ADS_SCALAR(pow, x, y, 1.0);
  CHECK_BINARY_FUNCTION_ADS_SCALAR(pow, x, y, 2.0);
  CHECK_BINARY_FUNCTION_ADS_SCALAR(pow, x, y, 3.0);
}

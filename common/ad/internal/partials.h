...
class Partials {
 ...
 private:
  ...
  /* Multiplies this by the given factor, but leaves along any zero. */
  void SparseMul(double factor);
  void SparseDiv(double factor);

  // Our effective value is `coeff_ * storage_`; we store them separately so
  // that re-scaling is fast (we can just scale the coeff).
  //
  // We maintain an invariant that `coeff_` is always finite. If it would become
  // non-finite, instead we multiply it through the `storage_` and reset it back
  // to `1.0`. This is required to meet our API contract of "any zero values
  // will remain zero, even if the factor is ±∞ or NaN" per our class overview.
  double coeff_{0.0};

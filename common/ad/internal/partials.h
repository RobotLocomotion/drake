...
class Partials {
 ...
 private:
  ...
  // that re-scaling is fast (we can just scale the coeff). We maintain an
  // invariant that `coeff_` is always finite. If it would become non-finite,
  // instead we multiply it through the `storage_` and reset it back to `1.0`.
  double coeff_{0.0};

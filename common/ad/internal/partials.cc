...
void Partials::AddScaled(double scale, const Partials& other) {
  ...
  // the special case(s) where where one or both of the `coeff_` factors are
  // exactly 0.0 or 1.0. So far, it doesn't benchmark faster in any suites we
  // have, but maybe it will in the future.

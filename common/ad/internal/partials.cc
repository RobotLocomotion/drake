...
int GetStaticUnitVectorOffset(const double* data) {
  const int offset = GetStaticUnitVector(0) - data;
  DRAKE_ASSERT(0 <= offset && offset < StorageVec::kMaxUnownedSize);
  return offset;
}
...
StorageVec StorageVec::MakeUnit(int size, int offset) {
  ...
  if (size <= StorageVec::kMaxUnownedSize) {
    ...
    // For large sizes, we can't use GetStaticUnitVector() for readback, so the
    // storage will need to live on the heap. If this ends up causing problems,
    // we could increase kMaxUnownedSize or switch to a more complicated storage
    // option.
...
void StorageVec::ConvertToOwned(double* coeff) {
  DRAKE_ASSERT(coeff != nullptr);
  DRAKE_ASSERT(std::isfinite(*coeff));
  DRAKE_ASSERT(!is_owned());
  DRAKE_ASSERT(size_ >= 2);
  auto* owned_data = new double[size_];
  std::fill(owned_data, owned_data + size_, 0.0);
  if (data_ != GetStaticZeroVector()) {
    const int offset = GetStaticUnitVectorOffset(data_);
    owned_data[offset] = (*coeff);
  }
  data_ = owned_data;
  is_owned_ = true;
  DRAKE_ASSERT_VOID(CheckInvariants());
...
void StorageVec::SetZero() {
  if (size_ > 0) {
    if (size_ <= StorageVec::kMaxUnownedSize) {
      *this = StorageVec::MakeZero(size_);
    } else {
...
void StorageVec::CheckInvariants() const {
  ...
  if (size_ == 0) {
    ...
    DRAKE_DEMAND(is_owned_ == false);
    DRAKE_DEMAND(data_ != nullptr);
    DRAKE_DEMAND(*data_ == 1.0 || *data_ == 0.0);
  } else /* size_ >= 2 */ {
    if (size_ > kMaxUnownedSize) {
      DRAKE_DEMAND(is_owned_ == true);
    }
    DRAKE_DEMAND(data_ != nullptr);
  }
}
...
void Partials::MatchSizeOf(const Partials& other) {
  ...
  if (size() == 0) {
    storage_ = StorageVec::MakeZero(other.size());
    coeff_ = 1.0;
    return;
  }
...
void Partials::Mul(double factor) {
  if (std::isfinite(factor)) [[likely]] {
    ...
    ad::DerivativesMutableXpr partials_xpr = MakeMutableXpr();
    for (int i = 0; i < size(); ++i) {
      const double x = partials_xpr[i];
      if ((x < 0) || (x > 0)) {
        partials_xpr[i] = x * factor;
      }
    }
  }
}
...
void Partials::AddScaled(double scale, const Partials& other) {
  ...
  // Special case: this is a unit vector (but other is not).
  if (!storage_.is_owned()) {
    storage_.ConvertToOwned(&coeff_);
    mutable_storage_view().noalias() +=
        (scale * other.coeff_) * other.storage_view();
    return;
  }

  // TODO(jwnimmer-tri) It's probably worth special casing when the other is
  // unit vector a unit vector (but this is not).
  ...
  // 1.0. So far, it doesn't benchmark faster in any suites we have, but maybe
  // it will in the future.
  //
  // N.B. Using a single Eigen expression for the computation is crucial for
  // performance. It compiles this better than a hand-written for-loop.
  mutable_storage_view().noalias() =
...
DerivativesMutableXpr Partials::MakeMutableXpr() {
  ...
  if (size() == 0) {
    ...
    // We'll just store it directly in `coeff_`. Per the StorageVec class
    // invariant, the storage_.data() points to "1.0" static const storage.
    data = &coeff_;
  } else {
    if (storage_.is_owned()) {
      if (coeff_ != 1.0) {

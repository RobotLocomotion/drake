...
AutoDiff pow(AutoDiff base_ad, const AutoDiff& exp_ad) {
  ...
  // When dealing with infinites (or a zero base where a sign-change to the
  ...
  // are grad(exp) are both zero we'll leave grad(result) as zero, but otherwise
  ...
  //  ∂/∂x bˣ = (x bˣ⁻¹) b' + (bˣ ln(b)) x'
  // https://www.wolframalpha.com/input?i2d=true&i=Divide%5Bd%2Cdp%5D%5C%2840%29Power%5Bb%5C%2840%29p%5C%2841%29%2Cx%5C%2840%29p%5C%2841%29%5D%5C%2841%29

  // Account for the contribution of grad(base) on grad(result).
  // Don't try to compute (x bˣ⁻¹) with x == 0, in case it comes out as a NaN

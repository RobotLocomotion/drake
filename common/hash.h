...
///
/// The short version is that Drake types implement a `hash_append` function.
...
/// @endcode
///
/// Types that implement this pattern can be used in unordered collections:
/// @code
/// std::unordered_set<MyValue, drake::DefaultHash> foo;
/// @endcode
/// Drake may also specialize `std::hash<MyValue>` to use `DefaultHash`, so
/// that the second template argument to `std::unordered_set` can be omitted.
///
/// TBD Explain that hash_append should be noexcept.
///
/// TBD Forbid overloading hash_append for std::unordered_foo collections.
///
/// @}

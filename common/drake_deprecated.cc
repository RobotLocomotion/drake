...
// Drake's regression tests use this to fail-fast in case Drake is supriously
// using deprecated self-calls. Note that the same constant is used for both
// C++ code and Python code.
constexpr char kDrakeDeprecationIsError[] = "_DRAKE_DEPRECATION_IS_ERROR";

}  // namespace

void WarnDeprecated::DoWarnDeprecated(std::string_view removal_date,
  ...
  const char* const is_error = std::getenv(kDrakeDeprecationIsError);

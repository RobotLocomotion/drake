...
Expression ExpressionAbs::Differentiate(const Variable& x) const {
  if (GetVariables().include(x)) {
    const Expression& arg{get_argument()};
    return if_then_else(arg < 0, -arg.Differentiate(x),
                        if_then_else(arg == 0, Expression::NaN(),
                                     arg.Differentiate(x)));
  } else {
    return Expression::Zero();
  }
...
Expression ExpressionCeiling::Differentiate(const Variable& x) const {
  if (GetVariables().include(x)) {
    const Expression& arg{get_argument()};
    // FYI:  'ceil(x) - floor(x) == 0` is the same as `x % 1 == 0`.
    return if_then_else(ceil(arg) - floor(arg) == 0,
                        Expression::NaN(),
                        Expression::Zero());
  } else {
    return Expression::Zero();
...
Expression ExpressionIfThenElse::Differentiate(const Variable& x) const {
  if (GetVariables().include(x)) {
    // TODO(ggould-tri): This does not return NaN as promised when the
    // condition is exactly on the boundary between true and false, and it's
    // not clear how it would.
    return if_then_else(f_cond_,

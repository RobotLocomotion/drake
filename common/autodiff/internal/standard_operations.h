...
(<,<=,>,>=,==,!=) but also argument-dependent lookup compatiblity with the
standard library's mathematical functions (abs, sin, round, isfinite, etc.)
...
inline AutoDiff atan2(AutoDiff a, const AutoDiff& b) {
  a.partials().Mul(b.value());
  a.partials().AddScaled(-a.value(), b.partials());
  a.partials().Mul(1.0 / (a.value() * a.value() + b.value() * b.value()));

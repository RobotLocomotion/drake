...
int IndexToInt(Eigen::Index index) {
  // XXX Add bounds checking (both low and high).
...
Partials::Partials(Eigen::Index size, Eigen::Index offset)
    : size_{IndexToInt(size)},
      ...
      coeff_{1.0} {
  // XXX Add better error messages.
  DRAKE_DEMAND(offset >= 0);
  DRAKE_DEMAND(offset < size);
...
void Partials::CheckInvariants() const {
  ...
  if (size_ == 0) {
    ...
    // XXX This one isn't true; needs rework???
...
// XXX this is as bad name w/o the coeff.
const double* Partials::view_as_eigen_data() const {
  if (empty()) {
    return nullptr;
  }
  if (unit_ > 0) {
    return GetStaticUnitVector(unit_ - 1);
  }
...
void Partials::AddScaledImpl(double scale, const Partials& other) {
  ...
  if ((size_ > 0) && (size_ != other.size_)) {
    throw std::logic_error(fmt::format(
        ...
        size_, other.size_, view_as_eigen().eval(),
        other.view_as_eigen().eval()));
  }
  ...
  if (is_unit()) {
    ...
    if (other.is_unit()) {
      ...
      new_data[get_unit()] = coeff_;
      new_data[other.get_unit()] = scale * other.coeff_;
      coeff_ = 1.0;
      unit_ = 0;
      DRAKE_ASSERT_VOID(CheckInvariants());
      return;
    ...
    new_data[get_unit()] += this_relative_partial;
    coeff_ = new_coeff;
    unit_ = 0;
    DRAKE_ASSERT_VOID(CheckInvariants());
    return;
  ...
  if (storage_.get_use_count() == 1) {
    ...
    if (other.is_unit()) {
      this_data[other.get_unit()] += scale * other.coeff_ / coeff_;
      return;
    }
  ...
  if (other.is_unit()) {
    ...
    new_this_data[other.get_unit()] = scale * other.coeff_ / coeff_;
...
PartialsXpr Partials::MakeMutableXpr() {
  DRAKE_ASSERT_VOID(CheckInvariants());

  // If we have no data, then there's no preparation required.
  // If we do have data, then we need to copy-on-write now.
  if (size_ == 0) {
    DRAKE_DEMAND(storage_.data() == nullptr);
  } else {
    *this = Partials(view_as_eigen());
    DRAKE_DEMAND(size_ > 0);
    DRAKE_DEMAND(storage_.data() != nullptr);
  }

  return PartialsXpr{this};
}

PartialsXpr::PartialsXpr(Partials* backreference)
    : Eigen::Map<Eigen::VectorXd>(
          // The const_cast is safe because only MakeMutableXpr calls this.
          const_cast<double*>(backreference->storage_.data()),
          backreference->size_),
      backreference_{backreference} {}

void PartialsXpr::resize(Eigen::Index size) {
  *this = Eigen::VectorXd::Zero(size);
}

void PartialsXpr::resize(Eigen::Index rows, Eigen::Index cols) {
  DRAKE_THROW_UNLESS(cols == 1);
  *this = Eigen::VectorXd::Zero(rows);
}

PartialsXpr Partials::SetFrom(const Eigen::Ref<const Eigen::VectorXd>& other) {
  *this = Partials(other);

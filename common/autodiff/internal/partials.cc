...
#include "drake/common/autodiff/internal/static_data.h"
...
Partials::Partials(const autodiff::AutoDiffDerivativesConstXpr& value)
    : magic_size_{IndexToInt(value.size())} {
...
AutoDiffDerivativesConstXpr Partials::make_const_xpr() const {
  ...
  return AutoDiffDerivativesConstXpr{coeff_, data, size(), stride};
}
...
AutoDiffDerivativesMutableXpr Partials::MakeMutableXpr() {
  ...
  return AutoDiffDerivativesMutableXpr{this, xpr_data, size()};
}

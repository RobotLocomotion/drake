...
using Stride = internal::AutoDiffDerivativesStride;
...
AutoDiffDerivativesConstXpr::with_explicit_zeros(Eigen::Index size) {
  ...
  if (NeedsExplicitZeros(this->size(), size)) {
    ...
    const int stride = 0;
    return AutoDiffDerivativesConstXpr(coeff, &kZero, size, stride);
  }
  return *this;
}

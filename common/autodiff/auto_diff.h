...
class AutoDiff {
 public:
  ...
  The return type is an Eigen XprType, i.e., an object that behaves like an
  Eigen::VectorXd readonly reference. It can be assigned to a VectorXd, or
  accessed directly via operator[], or etc. */
  auto derivatives() const {
    return partials_.view_as_eigen();
  }
  ...
  preferable to set them directly in the constructor.

  The return type is an Eigen XprType, i.e., an object that behaves like an
  Eigen::VectorXd mutable reference. It can be assigned from a VectorXd, or
  accessed directly via operator[], or etc.
  ...
  Calling this function copies the derivatives prior to returning the reference,
  so is O(N) runtime in size(). */
  auto derivatives() {

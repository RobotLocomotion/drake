...
class YamlReadArchive final {
 ...
 private:
  ...
  void VisitOptional(const NVP& nvp) {
    ...
    // or has an empty value.  If the YAML has no data, ensure that the C++
  ...
  void ParseScalar(const std::string& value, T* result) {
    ...
    // Generally, all of the POD types are supported, plus std::string.
  ...
  // with constructing a new Map node.  The root_ vs key_,value_ representations
  // are mutually exclusive -- when root_ is null, both the key_ and value_ must

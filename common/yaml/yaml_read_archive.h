...
/// (Advanced) YamlReadArchive is a helper class for yaml_io.h that loads data
/// from a YAML file into a C++ structure using the Serialize / Archive pattern.
///
...
class YamlReadArchive final {
 public:
  ...
  DRAKE_DEPRECATED("2022-02-01", "Use the functions in yaml_io.h, instead.")
  explicit YamlReadArchive(const YAML::Node& root);

  /// (Advanced) Creates an archive that reads from @p root, with @p options
  /// that allow for less restrictive parsing.
  DRAKE_DEPRECATED("2022-02-01", "Use the functions in yaml_io.h, instead.")

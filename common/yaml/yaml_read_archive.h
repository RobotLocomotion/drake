...
class YamlReadArchive final {
 ...
 private:
  ...
  void VisitOptional(const NVP& nvp) {
    ...
    const auto& sub_node = GetSubNode(nvp.name());
  ...
  void VisitVariant(const NVP& nvp) {
    const YAML::Node sub_node = GetSubNode(nvp.name());
    if (!sub_node) {
  ...
  YAML::Node GetSubNode(const char*) const;
  ...
  // These jointly denote the YAML::Node root that we are Accept() will read
  // from.  For performance reasons, we'll use a few different members all for
  // the same purpose.  Never access these directly from Visit methods -- use
  // GetSubNode() instead.
  // @{

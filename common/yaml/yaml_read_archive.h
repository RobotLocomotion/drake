...
class YamlReadArchive final {
 public:
  ...
  struct Options {
    ...
    /// The default Options are strict -- all irregularities during parsing
    /// will result in exception.
    Options() {}

    /// Refer to the member fields for details on these meanings.
    Options(bool allows_yaml_with_no_cpp, bool allows_cpp_with_no_yaml)
      : allow_yaml_with_no_cpp(allows_yaml_with_no_cpp),
        allow_cpp_with_no_yaml(allows_cpp_with_no_yaml) {}

    friend std::ostream& operator<<(std::ostream& os, const Options& x);

    /// Allows YAML Maps to have extra key-value pairs that are not defined by
    /// the Serializable being parsed into.  In other words, the Serializable
    /// types only form an incomplete schema for the YAML data.
    bool allow_yaml_with_no_cpp{false};

    /// Allows Serializable structs to provide more key-value pairs than are
    /// present in the YAML.  In other words, the structs have default values
    /// that are left intact unless the YAML data provides a value.

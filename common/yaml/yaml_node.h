...
class Node final {
 public:
  ...
  Any iterators based on GetMapping() remain valid.
  This function is only available for mutable Nodes; when using a const Node,
  call GetMapping and use the std::map<> API instead.
  ...
  struct MappingData final {
    ...
    // map here.  Providing an unordered_map in a public API is a recipe for
    // program non-determinism and reproducibility disasters.

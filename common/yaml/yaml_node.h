...
class Node final {
 ...
 private:
  ...
  // Having a YAML tag on a YamlNode is not required (our default value is the
  // empty string), but when present can be set to either a well-known enum or a
  // bespoke string. The representation here is not canonical -- it's possible

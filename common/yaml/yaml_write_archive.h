...
class YamlWriteArchive final {
 ...
 private:
  ...
  void VisitSerializable(const NVP& nvp) {
    ...
    internal::Node node = std::move(sub_archive.root_);
  ...
  void VisitVariant(const NVP& nvp) {
    ...
    std::visit([this, name, index](auto&& unwrapped) {
      ...
      if (index != 0) {
        ...
        const std::string tag = YamlWriteArchive::GetVariantTag<T>();
        root_.At(name).SetTag(tag);
      }
    }, variant);
  ...
  void VisitMap(const NVP& nvp) {
    ...
    auto sub_node = internal::Node::MakeMap();
  ...
  internal::Node root_ = internal::Node::MakeMap();

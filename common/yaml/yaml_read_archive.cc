...
// Convert a yamp-cpp YAML::Node `node` to a Drake yaml::internal::Node.
...
const internal::Node* YamlReadArchive::GetSubNodeScalar(
  ...
  const internal::Node* result = DoGetSubNode(
      name, internal::NodeType::kScalar);
  if ((result != nullptr) && result->IsEmptyScalar()) {
...
const internal::Node* YamlReadArchive::GetSubNodeSequence(
    const char* name) const {
  return DoGetSubNode(name, internal::NodeType::kSequence);
}

const internal::Node* YamlReadArchive::GetSubNodeMapping(
    const char* name) const {
  return DoGetSubNode(name, internal::NodeType::kMapping);
}

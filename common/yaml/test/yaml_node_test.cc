...
TEST_P(YamlNodeParamaterizedTest, GetSetMark) {
  ...
  EXPECT_FALSE(dut.GetMarkFilename().has_value());
  EXPECT_FALSE(dut.GetMark().has_value());

  dut.SetMarkFilename("example.yaml");
  dut.SetMark(Node::Mark{2, 3});
  EXPECT_EQ(dut.GetMarkFilename().value(), "example.yaml");
...
TEST_P(YamlNodeParamaterizedTest, EqualityPerMarkFilename) {
  Node dut = MakeEmptyDut();
  Node dut2 = MakeEmptyDut();
  EXPECT_TRUE(dut == dut2);
  dut2.SetMarkFilename("example.yaml");
  EXPECT_FALSE(dut == dut2);
}

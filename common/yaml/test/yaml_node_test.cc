 ...
 protected:
  ...
  Node MakeEmptyDut() {
    switch (GetExpectedType()) {
      ...
      case NodeType::kMap:      return Node::MakeMap();
    }
    DRAKE_UNREACHABLE();
  }
...
TEST_P(YamlNodeParamaterizedTest, GetType) {
  ...
  EXPECT_EQ(dut.IsMap(), GetExpectedType() == NodeType::kMap);
}
...
TEST_P(YamlNodeParamaterizedTest, EqualityPerType) {
  ...
  EXPECT_EQ(dut == Node::MakeMap(), dut.IsMap());
}
...
// Check Map-specific operations.
TEST_P(YamlNodeParamaterizedTest, MapOps) {
  Node dut = MakeEmptyDut();
  if (dut.IsMap()) {
    ...
    Node dut2 = MakeNonEmptyDut();
    EXPECT_FALSE(dut == dut2);
    ASSERT_EQ(dut2.GetMap().size(), 1);
    EXPECT_EQ(dut2.GetMap().begin()->first, "key");
    EXPECT_EQ(dut2.At("key").GetScalar(), "value");
    dut2.Remove("key");
    EXPECT_TRUE(dut.GetMap().empty());
  } else {
    DRAKE_EXPECT_THROWS_MESSAGE(
        dut.GetMap(), GetExpectedCannot("GetMap"));
    DRAKE_EXPECT_THROWS_MESSAGE(

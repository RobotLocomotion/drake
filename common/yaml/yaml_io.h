...
/// Saves data from the Serializable struct into a YAML file.
/// The data will appear under the `root_name` key in a top-level mapping.
template <typename Serializable>
void SaveYamlFile(
    const std::string& filename,
...
static Serializable LoadYamlFile(
  ...
  return internal::LoadNode(std::move(node), child_name, defaults, options);
}
...
void SaveYamlFile(
    ...
    const std::string& root_name,
    const Serializable& data) {
  YamlWriteArchive::WriteFile(filename, SaveYamlString(root_name, data));
}

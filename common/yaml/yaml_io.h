...
/// Like LoadFile, but loads from a YAML string instead of a file.
template <typename Serializable>
static Serializable LoadString(
...
template <typename Serializable>
void SaveFile(const std::string& filename, const Serializable& data);
...
template <typename Serializable>
std::string SaveString(const std::string& root_name, const Serializable& data);
...
static Serializable LoadFile(
  ...
  return LoadNode(std::move(node), child_name, defaults, options);
}

...
void RecursiveEmit(const internal::Node& node, YAML::EmitFromEvents* sink) {
  ...
  node.Visit(overloaded {
    [](std::monostate) { /* no-op for Null node type */ },
    ...
    [&](const internal::Node::MapData& data) {
      ...
      if (node.GetMap().count(kKeyOrder)) {
        const internal::Node& key_order_node = node.GetMap().at(kKeyOrder);
...
std::string YamlWriteArchive::EmitString(const std::string& root_name) const {
  ...
  if (root_.GetMap().empty()) {
    ...
    if (root_name.empty()) {
      ...
      auto document = internal::Node::MakeMap();
...
void DoEraseMatchingMaps(internal::Node* x, const internal::Node* y) {
  ...
  for (auto&& [key, x_value] : x->GetMap()) {
    ...
    auto& x_mutable_value = x->At(key);
    DoEraseMatchingMaps(&x_mutable_value, &y_value);
  }
}

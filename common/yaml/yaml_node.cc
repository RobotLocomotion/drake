...
template <typename T> std::string_view GetNiceVariantName(const T&) {
  if constexpr (std::is_same_v<T, Node::ScalarData>) {
    ...
    return "Sequence";
  } else if constexpr (std::is_same_v<T, Node::MapData>) {
    return "Map";
  }
}
...
std::string_view Node::GetTypeString(NodeType type) {
  switch (type) {
    ...
    case NodeType::kMap: {
      return GetNiceVariantName(MapData{});
    }
  }
  DRAKE_UNREACHABLE();
}
...
bool Node::IsMap() const {
  return std::holds_alternative<MapData>(data_);
}
...
bool operator==(const Node::MapData& a, const Node::MapData& b) {
  return a.map == b.map;
}
...
const std::map<std::string, Node>& Node::GetMap() const {
  return std::visit(overloaded {
    [](const MapData& data) -> const std::map<std::string, Node>& {
      return data.map;
    },
    [](auto&& data) -> const std::map<std::string, Node>& {
      throw std::logic_error(fmt::format(
          "Cannot Node::GetMap on a {}", GetNiceVariantName(data)));
    },
  }, data_);
}
...
void Node::Add(std::string key, Node value) {
  return std::visit(overloaded {
      [&key, &value](MapData& data) -> void {
      data.map.insert({std::move(key), std::move(value)});
    },
    [](auto&& data) -> void {
      throw std::logic_error(fmt::format(

...
The follow sections explain each of these steps in more detail, along with the
customization options that are available for each one.
...
function. See drake::math::BsplineBasis for an example of this situation.
...
@code{cpp}
MyData LoadMyData1(const std::string& filename) {
...
the yaml file's contents will be overwrite the provided defaults, but any
fields that are not mentioned in the yaml fill we remain intact at their
default values.

When merging file data atop any defaults, any `std::map` or `std::unordered_map`
collections will merge the contents of the file alonside the existing map
values, keeping anything in the default that is unchanged. Any other
collections such as `std::vector` are entirely reset, even if they already had
some values values already in place (in particular, they are not merely
appended to).

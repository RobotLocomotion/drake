...
* If you are building from source or using the `drake` C++ library, what
compiler are you using, and which version?

    > Execute `bash -x -c 'echo ${CC}; echo ${CXX}'`. If it prints out
    values for these variables, include the output, including
    `${CXX} --version`.

    > If the above command does not reveal your compiler, please determine
    which compiler is beign used, and print out its `--version`.

* If [building from source](http://drake.mit.edu/from_source.html):
...
* If using [from a binary release](http://drake.mit.edu/from_binary.html):

...
DiscreteTimeFiniteHorizonLinearQuadraticRegulator(
  ...
  auto Qf = options.Qf.value_or(Eigen::MatrixXd::Zero(num_states, num_states));
  auto xdf = (options.xd != nullptr)
                 ? options.xd->value(tf)
                 : ((options.x0 != nullptr) ? options.x0->value(tf)
                                            : x0_trj->value(tf));
  S.back() = Qf;
  if (options.use_square_root_method) {

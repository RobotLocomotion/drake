...
void PidControlledSystem<T>::Initialize(
  ...
  plant_ = builder.template AddSystem<System<T>>(std::move(plant));
...
PidControlledSystem<T>::ConnectController(
  ...
  return ConnectResult{plant_input_adder->get_input_port(1),
                       controller->get_input_port_desired_state()};
}

template <typename T>
typename PidControlledSystem<T>::ConnectResult
PidControlledSystem<T>::ConnectController(const InputPort<T>& plant_input,
  ...
  return ConnectController(
      ...
      MatrixX<double>::Identity(plant_output.size(), plant_output.size()), Kp,
      Ki, Kd, builder);
}
...
PidControlledSystem<T>::ConnectControllerWithInputSaturation(
  ...
  return PidControlledSystem<T>::ConnectController(
      saturation->get_input_port(), plant_output, feedback_selector, Kp, Ki, Kd,
      builder);
}
...
PidControlledSystem<T>::ConnectControllerWithInputSaturation(
  ...
  return ConnectControllerWithInputSaturation(
      plant_input, plant_output,
      MatrixX<double>::Identity(plant_output.size(), plant_output.size()), Kp,
      Ki, Kd, min_plant_input, max_plant_input, builder);
}

...
JointStiffnessController<T>::JointStiffnessController(
    ...
    : LeafSystem<T>(SystemTypeTag<JointStiffnessController>{}),
      ...
      kp_(kp),
      kd_(kd) {
  // At least one of owned_plant_ or plant should have ben nullptr.
  ...
  DRAKE_DEMAND(num_q == plant_->num_positions());
...
std::unique_ptr<multibody::MultibodyPlant<T>> ConvertOwnedPlant(
  ...
  if (!owned_plant) {
    throw std::runtime_error(
        "To use scalar conversion, you must use the "
...
void JointStiffnessController<T>::CalcMultibodyForces(
  ...
  plant_->CalcForceElementsContribution(plant_context,
                                                  cache_value);
}

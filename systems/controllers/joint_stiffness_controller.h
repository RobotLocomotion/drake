 ...
 * such as the JointImpedanceControl mode on Kuka's iiwa of Franka's Panda, are
 ...
 * resembles a spring-damper dynamics at the joints around the set:
 * <pre>
 *   M(q)vd + C(q,v)v + kp⊙(q - q_d) + kd⊙(v - v_d) = τ_ext,
 * </pre>
 * where `M(q)vd + C(q,v)v` are the original multibody mass and Coriolis terms,
 * and `τ_ext` are any external generalized forces that can arise, e.g. from
 * contact forces.
 *
 * The controller currently assumes that plant.num_positions() ==
 ...
 * Franka's Panda is best modeled as a stiffness controller (unless one were to

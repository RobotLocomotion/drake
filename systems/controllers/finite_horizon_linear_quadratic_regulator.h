...
struct FiniteHorizonLinearQuadraticRegulatorOptions {
  /**
  A num_states x num_states positive semi-definite matrix which specified he
  cost at the final time. If unset, then Qf will be set to the zero matrix.
  */
  std::optional<Eigen::MatrixXd> Qf;

  /**
  Specify a nominal state trajectory.  The system is linearized about this
  trajectory, and the objective is to regulate to this trajectory -- the state
  component of the quadratic running cost is (x-x0(t))'*Q*(x-x0(t)).  If
  nullptr, x0(t) = 0 ∀t.  x0 must be defined over the entire interval [t0, tf].
  */
  const trajectories::Trajectory<double>* x0{nullptr};

  /**
  Specify a nominal input trajectory.  The system is linearized about this
  trajectory, and the objective is specified relative to this trajectory -- the
  input component of the quadratic running cost is (u - u0(t))'*R*(u-u0(t)).  If
  nullptr, then u0(t) = 0 ∀t. u0 must be defined over the entire interval [t0,
  tf].
  */
...
/**
A structure that contains the basic FiniteHorizonLinearQuadraticRegular results.
The finite-horizon cost-to-go is given by (x-x0(t))'*S(t)*(x-x0(t)) and the
optimal controller is given by u-u0(t) = -K(t)*(x-x0(t)).
*/
struct FiniteHorizonLinearQuadraticRegulatorResult {
  std::unique_ptr<trajectories::PiecewisePolynomial<double>> K;
  std::unique_ptr<trajectories::PiecewisePolynomial<double>> S;
};
...
optimal cost-to-go for the finite-horizon linear quadratic regulator:
  ...
  min_u (x-x0(tf))'*Qf*(x-x0(tf) + \int_{t0}^{tf} (x-x0(t))'*Q*(x-x0(t)) dt
        \int_{t0}^{tf} (u-u0(t))'*R*(u-u0(t)) dt s.t. \dot{x} = A(t)*(x - x0(t))
        + B(t)*(u - u0(t)) + c(t)

where A(t), B(t), and c(t) are taken from the gradients of the continuous-time
  dynamics \dot{x} = f(t,x,u), as A(t) = dfdx(t, x0(t), u0(t)) B(t) = dfdu(t,
  x0(t), u0(t)) c(t) = f(t, x0(t), u0(t))

@param system must be a System<double> with (only) n continuous state variables

...
FittedValueIteration(
  ...
  const int kNumStateIndices = state_mesh.get_num_interpolants();
  ...
  {  // Build transition matrices.
    ...
    Eigen::VectorXi Tind_tmp(kNumStateIndices);
    Eigen::VectorXd T_tmp(kNumStateIndices);

    for (int input = 0; input < kNumInputs; input++) {
      std::cout << ".";
      Tind[input].resize(kNumStateIndices, kNumStates);
      T[input].resize(kNumStateIndices, kNumStates);
  ...
  while (max_diff > options.convergence_tol) {
    for (int state = 0; state < kNumStates; state++) {
      ...
      for (int input = 0; input < kNumInputs; input++) {
        // Q(state,input) = g(x,u) + γ J(f(x,u)).
        double Q = cost[input](state);
        for (int index = 0; index < kNumStateIndices; index++) {

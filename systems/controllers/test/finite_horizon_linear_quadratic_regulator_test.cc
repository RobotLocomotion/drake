...
// system alone, but set B*ud = -c.  The steady-state solution to the
// finite-horizon LQR problem should lots of the affine terms in order to get
...
GTEST_TEST(FiniteHorizonLQRTest, AffineSystemTest) {
  ...
  // all of the affine terms must be correct to cancel each other out.
  EXPECT_TRUE(CompareMatrices(result.S->value(t0), lqr_result.S, 1e-5));

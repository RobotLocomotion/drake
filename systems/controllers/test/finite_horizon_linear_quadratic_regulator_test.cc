...
GTEST_TEST(DiscreteTimeFiniteHorizonLQRTest,
           ContinuousTimeDoubleIntegratorWithNonZeroGoal) {
  Eigen::Matrix2d A;
  Eigen::Vector2d B;
  A << 0, 1, 0, 0;
  B << 0, 1;
  ...
  std::vector<Eigen::MatrixXd> x0_, xd_;
  for (int n = 0; n <= std::round((tf - t0) / h); ++n) {
    t_.push_back(t0 + h * n);
    x0_.push_back(Eigen::Vector2d(0, 0));
    xd_.push_back(xdv);
  }
  trajectories::DiscreteTimeTrajectory<double> x0_traj(t_, x0_);
  trajectories::DiscreteTimeTrajectory<double> xd_traj(t_, xd_);
  options.x0 = &x0_traj;
  options.xd = &xd_traj;

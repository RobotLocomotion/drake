...
GTEST_TEST(JointStiffnessControllerTest, SimpleDoublePendulum) {
  ...
  EXPECT_TRUE(CompareMatrices(tau, tau_expected, 1e-12));
}
...
GTEST_TEST(JointStiffnessControllerTest, ScalarConversion) {
  ...
  JointStiffnessController<double> controller_without_ownership(*mbp, kp, kd);
  DRAKE_EXPECT_THROWS_MESSAGE(controller_without_ownership.ToAutoDiffXd(),
                              ".*constructor which takes ownership.*");

  JointStiffnessController<double> controller(std::move(mbp), kp, kd);

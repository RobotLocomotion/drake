...
/// @{
/// Drake provides a variety of capabilies for sensor modeling. Some sensors are
/// offered as Systems in a block diagram. These sensors are listed in the
...
/// "get_reaction_forces_output_port()" that returns the reaction
/// @ref drake::multibody::SpatialForce "SpatialForce" of all the joints.
/// The users only need to extract the joints they are interested. To simulate a
/// Force/Torque sensor, the users need to either a) define a fixed joint for
/// the force/torque sensor in the model file or b) add a
/// @ref drake::multibody::WeldJoint "WeldJoint" between two bodies.
/// This joint will then serve as the Force/Torque sensor. The users can also
/// refer to *planar_gripper* for example.
/// @}

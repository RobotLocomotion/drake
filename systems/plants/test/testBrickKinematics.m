...
for fb = {'rpy','RPY'};%,'quat'};
  ...
  for i=1:25
    ...
    [x,J] = geval(1,@terrainContactPointsInWorld,m,q,options);
    
    options.grad_method = 'taylorvar';
    [x,J,dJ] = geval(1,@terrainContactPointsInWorld,m,q,options);
    kinsol = doKinematics(m,q,false,false,qd);
    %[~,~,Jdot] = contactPositionsJdot(m,kinsol);
    [~,~,Jdot] = terrainContactPointsInWorld(m,kinsol,true);

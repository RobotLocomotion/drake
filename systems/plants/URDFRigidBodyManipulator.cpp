...
bool parseVisual(RigidBody* body, TiXmlElement* node, URDFRigidBodyManipulator* model)
{
  // todo:  consider implementing this, but I don't need it yet
  return true;
}

bool parseCollision(int body_index, TiXmlElement* node, URDFRigidBodyManipulator* model)
{
  Isometry3d T = Isometry3d::Identity();
  TiXmlElement* origin = node->FirstChildElement("origin");
  if (origin) poseAttributesToTransform(origin,T.matrix());

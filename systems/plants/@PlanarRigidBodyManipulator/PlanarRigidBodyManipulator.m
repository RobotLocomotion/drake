      ...
      switch (lower(type))
        case 'linearspringdamper'
          ...
          if linkNode.hasAttribute('xyz')
            fe.pos1 = [model.x_axis'; model.y_axis']*reshape(parseParamString(model,robotnum,char(linkNode.getAttribute('xyz'))),3,1);
          end
          linkNode = node.getElementsByTagName('link2').item(0);
          fe.body2 = findLinkInd(model,char(linkNode.getAttribute('link')),robotnum);
          if linkNode.hasAttribute('xyz')
            fe.pos2 = [model.x_axis'; model.y_axis']*reshape(parseParamString(model,robotnum,char(linkNode.getAttribute('xyz'))),3,1);
          end

classdef MultipleTimeKinematicConstraint < RigidBodyConstraint
  ...
  methods
    ...
    function cnstr = generateConstraint(obj,t)
      % generate a FunctionHandleDifferentiableConstraint for postures at all time t
      t = t(:)';
      valid_t_idx = obj.isTimeValid(t);
      t_idx = (1:length(t));
      ...
      if(num_valid_t >= 2)
        [lb,ub] = obj.bounds(t);
        nq = obj.robot.getNumDOF;
        cnstr = {FunctionHandleDifferentiableConstraint(lb,ub,length(t)*nq,@(varargin) obj.eval(t,varargin(numel(t)+(1:numel(t)))))};

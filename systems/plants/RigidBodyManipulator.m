...
%      obj = obj.setNumVelocityConstraints(0);%size([obj.model.body.ground_contact],2));
    ...
    function [H,C,B,dH,dC,dB] = manipulatorDynamics(obj,q,qd)
      ...
      if (nargout>3)
        if (obj.model.mex_model_ptr && isnumeric(q) && isnumeric(qd))
            [H,C,dH,dC] = HandCmex(obj.model.mex_model_ptr,q,qd);
        else
        ...
        if (obj.model.mex_model_ptr && isnumeric(q) && isnumeric(qd))
          [H,C] = HandCmex(obj.model.mex_model_ptr,q,qd);
        else
    ...
    function [phi,n,D,mu,dn,dD] = contactConstraints(obj,q,qd)
      ...
      use_mex = false;
      if (obj.model.mex_model_ptr && isnumeric(q) && (nargin<3 || isnumeric(qd)))
        use_mex = true;
      end
...
%      figure(1); clf;  % for debugging
      for i=1:length(obj.model.body)
        nC = size(obj.model.body(i).contact_pts,2);
        if nC>0
          if (nargout>4)
            [contact_pos(:,count+(1:nC)),J(3*count+(1:3*nC),:),dJ(3*count+(1:3*nC),:)] = forwardKin(obj.model,i,obj.model.body(i).contact_pts,use_mex);
          elseif (nargout>1)
            [contact_pos(:,count+(1:nC)),J(3*count+(1:3*nC),:)] = forwardKin(obj.model,i,obj.model.body(i).contact_pts,use_mex);
          else
            contact_pos(:,count+(1:nC)) = forwardKin(obj.model,i,obj.model.body(i).contact_pts,use_mex);
          end
          count = count + nC;

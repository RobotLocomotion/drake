classdef RelativeOrientConstraint < OrientConstraint
  ...
  methods
    function obj = RelativeOrientConstraint(robot,tspan,bodyA_idx,bodyB_idx,trans_bp2b,tol)
      typecheck(bodyA_idx,'double');
      typecheck(bodyB_idx,'double');
      sizecheck(trans_bp2b,[4,1]);

      obj = obj@OrientConstraint(robot, tspan, tol);
    ...
    function [orient_prod, dorient_prod] = getOrientationProduct(obj,kinsol)
    ...
    function name = getConstraintName(obj,t)
      if(t>=obj.tspan(1)&&t<=obj.tspan(end))||isempty(t)
        name = {sprintf('%s relative to %s orientation constraint at time %10.4f', ...

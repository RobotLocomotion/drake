...
for i=1:N,
  ...
  if ~isempty(contact_seq{i})    
    ...
    if obj.dim == 2
      ...
      J = [n(contact_seq{i}(1),:); D{1}(contact_seq{i}(1),:); D{2}(contact_seq{i}(1),:)];
    end
    
    %add constraint
    ind = contact_seq{i}(1);
    if obj.dim == 2
      xA = obj.T_2D_to_3D'*xA;
      xB = obj.T_2D_to_3D'*xB;
    end
    position_fun = drakeFunction.kinematic.RelativePosition(cplant,idxB(ind),idxA(ind),xB(:,ind));
    position_constraint = DrakeFunctionConstraint(xA(:,ind),xA(:,ind),position_fun);
    ...
    for j=2:length(contact_seq{i})
      if cond([J;n(contact_seq{i}(j),:)]) < 1e2
        coord_inds = obj.dim;
      ...
      if obj.dim == 3,
        if cond([J;D{2}(contact_seq{i}(j),:)]) < 1e2
          coord_inds = [2;coord_inds];
          J = [J;D{2}(contact_seq{i}(j),:)];
        end
      end
      ...
      if ~isempty(coord_inds)
        posA = xA(coord_inds,ind);
        position_fun = drakeFunction.kinematic.RelativePosition(cplant,idxB(ind),idxA(ind),xB(:,ind),coord_inds);

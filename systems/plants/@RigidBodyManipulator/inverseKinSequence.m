...
contact_state = cell(1,nSample);
contact_affs = cell(1,nSample);
contact_dist = cell(1,nSample);
...
for i = 1:nSample
    ...
    while j<length(ikargs)
        ...
        if(body_ind{i}(n) == 0)
            ...
            contact_state{i}{n} = ikargs{j+1}.contact_state;
            j = j+2;
            ...
            contact_affs{i}{n} = ikargs{j+2}.contact_affs;
            contact_dist{i}{n} = ikargs{j+2}.contact_dist;
            j = j+3;
            [rows,mi] = size(body_pos{i}{n});
            if(rows~=3) error('bodypos must be 3xmi');end
        end
        ...
        static_pts_flag{i}{n} = any(cell2mat(contact_state{i}{n}') == ActionKinematicConstraint.BREAK_CONTACT,1) | ...
                          any(cell2mat(contact_state{i}{n}') == ActionKinematicConstraint.STATIC_PLANAR_CONTACT,1)|...
                          any(cell2mat(contact_state{i}{n}') == ActionKinematicConstraint.STATIC_GRIP_CONTACT,1);
        ...
        if(body_ind{i}(n) ~=0 )
            ...
            for aff_ind = 1:num_contact_aff
                ...
                contact_body_pts_ind{i}{n}{aff_ind} = find(contact_state{i}{n}{aff_ind} ~= ActionKinematicConstraint.UNDEFINED_CONTACT);
                num_contact_body_pts{i}{n}(aff_ind) = length(contact_body_pts_ind{i}{n}{aff_ind});
                if(num_contact_body_pts{i}{n}(aff_ind)>0)
                    contact_dist_ub = contact_dist{i}{n}{aff_ind}.max;
                    contact_dist_ub = contact_dist_ub(:,contact_body_pts_ind{i}{n}{aff_ind});
                    contact_dist_lb = contact_dist{i}{n}{aff_ind}.min;
...
if(options.optimizeSparsity)
    ...
    for i = 1:nTrials
        [~,G_tmp(:,i)] = ik_tmp(w_tmp(:,i),obj,nq,nSample+1,nF,nG,...
            ...
            contact_state,static_pts_flag,static_pts_ind,num_static_pts,...
            prev_body_ind,prev_pts_ind,shrinkFactor,contact_affs,contact_dist,...
    ...
    ik = @(w) ik_tmp(w,obj,nq,nSample+1,nF,nG,options.Qa,options.Qv,...
        ...
        options.quasiStaticFlag,nonzero_ind,contact_state,static_pts_flag,...
        static_pts_ind,num_static_pts,prev_body_ind,prev_pts_ind,shrinkFactor,...
        contact_affs,contact_dist,contact_body_pts_ind,num_contact_body_pts);
else
...
function [f,G] = ik_tmp(w,obj,nq,nT,nF,nG,Qa,Qv,Q,body_ind,body_pos,...
    ...
    contact_state,static_pts_flag,static_pts_ind,num_static_pts,prev_body_ind,...
    prev_pts_ind,shrinkFactor,contact_affs,contact_dist,contact_body_pts_ind,...

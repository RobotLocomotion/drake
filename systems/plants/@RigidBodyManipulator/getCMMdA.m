function [A, dA] = getCMMdA(robot, kinsol, robotnum, in_terms_of_qdot)
...
if ~isstruct(kinsol)  
  % treat input as getCMMdA(model,q)
  q = kinsol;
  compute_kinematics = true;
else
  if ~isfield(kinsol, 'v')
    kinsol_options.use_mex = kinsol.mex;
    robot.warning_manager.warnOnce('Drake:getCMMdA:OldKinsol','You called getCMMdA with a kinsol in the old format. Redoing kinematics in new format using q from old kinsol');
    q = kinsol.q;
    compute_kinematics = true;
  end
end

function [q,info] = approximateIK(obj,q_seed,q_nom,Q,varargin)
% Takes same input as inverseKin, but solves it as a QP.
% (Hongkai will update this documentation)

use_mex = true;
if(use_mex)
  num_constraint = length(varargin);
  ...
  [q,info] = approximateIKmex(obj.mex_model_ptr,q_seed,q_nom,Q,constraint_ptr_cell{:});
else
t = [];
kc_cell = {};
[joint_min,joint_max] = obj.getJointLimits();
for j = 1:length(varargin)-1
  if(isa(varargin{j},'KinematicConstraint'))

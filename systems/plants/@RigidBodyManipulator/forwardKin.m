...
if obj.use_new_kinsol
  ...
  if (kinsol.mex)
    ...
    if nargout > 2
      [x,J,dJ] = rigidBodyManipulatormex(obj.FORWARD_KIN_COMMAND,obj.mex_model_ptr,kinsol.q,body_or_frame_id,pts,options.rotation_type);
    elseif nargout > 1
      [x,J] = rigidBodyManipulatormex(obj.FORWARD_KIN_COMMAND,obj.mex_model_ptr,kinsol.q,body_or_frame_id,pts,options.rotation_type);
    else
      x = rigidBodyManipulatormex(obj.FORWARD_KIN_COMMAND,obj.mex_model_ptr,kinsol.q,body_or_frame_id,pts,options.rotation_type);
    end
...
if (kinsol.mex)
  ...
  if nargout > 2
    [x, J, dJ] = rigidBodyManipulatormex(obj.FORWARD_KIN_V_COMMAND,obj.mex_model_ptr, body_or_frame_id, points, rotation_type, base_or_frame_id, in_terms_of_qdot);
    dJ = reshape(dJ, size(J, 1), []); % convert to strange second derivative output format
  elseif nargout > 1
    [x, J] = rigidBodyManipulatormex(obj.FORWARD_KIN_V_COMMAND,obj.mex_model_ptr, body_or_frame_id, points, rotation_type, base_or_frame_id, in_terms_of_qdot);
  else
    x = rigidBodyManipulatormex(obj.FORWARD_KIN_V_COMMAND,obj.mex_model_ptr, body_or_frame_id, points, rotation_type, base_or_frame_id, in_terms_of_qdot);
  end
else

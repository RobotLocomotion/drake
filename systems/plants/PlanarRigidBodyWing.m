classdef PlanarRigidBodyWing < RigidBodyForceElement
  ...
  properties
    ...
    origin=zeros(2,1);%xz of the reference point for the wing.
...
%}   
      ...
      if ~isa(parent_body, 'numeric');
        error('Drake:PlanarRigidBodyWing:InvalidParent','Force Type Wing does not have a proper RigidBody parent');
      end
      ...
      elseif strcmpi(profile, 'flat plate')
        ...
        elseif strcmpi(profile(end-3:end), '.dat') %Path to a .dat file
          ...
          if ~linux %Because Windows filenames.
        ...
        avlfile = regexprep(avlfile, '\$Zorig', sprintf('%.2f',aero_origin(2)));
        ...
        try
          ...
          if(linux)
            commandstring = sprintf('avl %s %s < %s > %s', avlfilepath, runfilepath, commandfilepath, fullfile(tempdir,'avlScreenOutput.txt'));
            ...
            disp('Non-unix implementation not fully supported yet')
    ...
    function f_ext = computeSpatialForce(obj,manip,q,qd)
      ...
      torque = ppvalSafe(obj.fCm,aoa, false, false)*airspeed*airspeed;
      ...
      f_ext(:,obj.bodyind) = [torque;0;0] + ...
        cartesianForceToSpatialForce(manip, kinsol, obj.bodyind, obj.origin(1:2),lift_world+drag_world);
      f_ext;
    end

...
%  with a single (vector) input u, a single (vector) output y, and a single (vector) state x composed of a combination of continuous time and discrete time variables
  ...
  methods
    ...
    function newsys = feedbackControl(plant,control)
      % Creates a slightly more specific form of feedback combination
      % useful for building control hierarchies.  Here the input to the 
      % control system is expected to be a MultiCoordinateFrame, where 
      % exactly one of the sub-CoordinateFrames is related to the plant 
      % output (possibly through a transform), then the remaining
      % coordinate frame inputs are the input to the combined system.
      ...
      if ~typecheck(control.getInputFrame(),'MultiCoordinateFrame')
        error('Drake:DynamicalSystem:BadInputFrame','This feedback combination requires that the control block has a multicoordinateframe input');
      end
      
      control = control.inOutputFrame(plant.getInputFrame());
      [tf,fid] = findChildReverseTransform(control.getInputFrame(),plant.getOutputFrame(),struct('throw_error_if_fail',true));
      externalInputFrame = subFrameByFrameID(control.getInputFrame(),[1:fid-1,fid+1:length(control.getInputFrame().frame)]);
      feedbackIndices = control.getInputFrame().coord_ids{fid};
      externalInputIndices = setdiff(1:control.getInputFrame.dim,feedbackIndices);
      ...
      load_system('simulink3');
      add_block('simulink3/Sources/In1',[mdl,'/in']);
      add_block('simulink3/Signals & Systems/Mux',[mdl,'/mux']);
      add_block('simulink3/Signals & Systems/Selector',[mdl,'/selector'],'IndexParamArray',{mat2str([externalInputIndices,feedbackIndices])},'InputPortWidth',num2str(control.getInputFrame().dim));
      add_line(mdl,'in/1','mux/1');
      add_line(mdl,'mux/1','selector/1');
      
      add_block('simulink3/Subsystems/Subsystem',[mdl,'/control']);
      Simulink.SubSystem.deleteContents([mdl,'/control']);
      Simulink.BlockDiagram.copyContentsToSubSystem(control.getModel(),[mdl,'/control']);
      add_block('simulink3/Subsystems/Subsystem',[mdl,'/plant']);
      Simulink.SubSystem.deleteContents([mdl,'/plant']);
      Simulink.BlockDiagram.copyContentsToSubSystem(plant.getModel(),[mdl,'/plant']);

      add_line(mdl,'selector/1','control/1');
      add_line(mdl,'control/1','plant/1');

      if isempty(tf)
        add_line(mdl,'plant/1','mux/2');
      else
        add_block('simulink3/Subsystems/Subsystem',[mdl,'/coordinateTransform']);
        Simulink.SubSystem.deleteContents([mdl,'/coordinateTransform']);
        Simulink.BlockDiagram.copyContentsToSubSystem(tf.getModel(),[mdl,'/coordinateTransform']);
        add_line(mdl,'plant/1','coordinateTransform/1');
        add_line(mdl,'coordinateTransform/1','mux/2');
      end

      add_block('simulink3/Sinks/Out1',[mdl,'/out']);
      add_line(mdl,'plant/1','out/1');
      
      newsys = SimulinkModel(mdl,externalInputFrame.dim);
      newsys = setInputFrame(newsys,externalInputFrame);
      if (getNumStates(control)==0 && (isempty(tf) || getNumStates(tf)==0))
        newsys = setStateFrame(newsys,getStateFrame(plant));
      elseif (getNumStates(plant)==0 && (isempty(tf) || getNumStates(tf)==0))
        newsys = setStateFrame(newsys,getStateFrame(control));
      end
      newsys = setOutputFrame(newsys,getOutputFrame(plant));
      newsys.time_invariant_flag = control.isTI() && plant.isTI() && (isempty(tf) || tf.isTI());
      newsys.simulink_params = catstruct(control.simulink_params,plant.simulink_params);
      if ~isempty(tf)
        newsys.simulink_param = catstruct(tf.simulink_params,newsys.simulink_param);
      end
    end

 ...
 *
 * You may provide an LCM interface yourself or allow %LcmPublisherSystem
 * to allocate and maintain one internally.
 *
...
class LcmPublisherSystem : public LeafSystem<double> {
 public:
   ...
   *
   * @param lcm Optionally, a pointer to the LCM subsystem. The pointer must
  ...
  /**
   * Specifies an initialization channel name and a message-publishing function
   * to be invoked once from an initialization event. If this method is not
   * called, no initialization event will be created.
   *
   * You can only call this method once.
   * @throws std::logic_error if called a second time.
   *
   * @pre Neither the channel name nor the publisher function may be empty.
   */
  ...
  /**
   * Forces sending of the initialization message if a call to
   * AddInitializationMessage() was made, otherwise does nothing. Normally
   * this method is invoked by an initialization even triggered at the start
   * of a simulation using drake::systems::Simulator on a Diagram that contains
   * an %LcmPublisherSystem.
   */
  void SendInitializationMessage() const;
  ...
  /**
   * Returns the channel name supplied in AddInitializationMessage(), if that
   * was called, otherwise an empty string.
   */
  const std::string& get_initialization_channel_name() const {
    return initialization_channel_;
  }
  ...
  /**
   * Returns the LCM object in use by this publisher. This may have been
   * supplied in the constructor or may be an internally-maintained object of
   * type drake::lcm::DrakeLcm.
   */
 ...
 private:
  ...
  // Optionally, the channel on which to publish an initialization message
  // using the initialization_publisher (empty if none).
  std::string initialization_channel_;
  std::function<void(const std::string& channel,
                     drake::lcm::DrakeLcmInterface* lcm)>
      initialization_publisher_;

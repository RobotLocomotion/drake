 ...
 * will output new values based on new data received on our LCM URL.
 *
 ...
 * LCM stack has message(s) waiting.
 *
 * Note that because this implemented DrakeLcmInterface, any subscriber to that
 * interface can be serviced during simulation, not just %LcmSubscriberSystem.
 *
 * @code{cpp}
...
class LcmInterfaceSystem final
 ...
 public:
  ...
  DRAKE_NODISCARD std::unique_ptr<drake::lcm::DrakeSubscription> Subscribe(
      const std::string&, int, HandlerFunction) final;

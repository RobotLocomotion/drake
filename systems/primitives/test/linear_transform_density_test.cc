...
void CheckDensityGradient(RandomDistribution distribution,
  ...
  auto b = math::InitializeAutoDiffFromValueMatrix(Eigen::Vector2d(2, 4));
  Matrix2<AutoDiffXd> A =
      Eigen::Vector2d(2, -3).cast<AutoDiffXd>().asDiagonal();
  Eigen::Matrix2d A_val = math::ExtractValueMatrixFromAutoDiff(A);
  dut.FixConstantA(context.get(), A);
  dut.FixConstantB(context.get(), b);
  ...
  dut.FixConstantA(context.get(), A);
  A_val = math::ExtractValueMatrixFromAutoDiff(A);
  ...
  Eigen::Vector2d w_out_val = math::ExtractValueMatrixFromAutoDiff(
      dut.get_output_port_w_out().Eval(*context));
  Eigen::Vector2d b_val = math::ExtractValueMatrixFromAutoDiff(b);

...
GTEST_TEST(PortSwitchTest, VectorTest) {
  ...
  context->FixInputPort(port_A.get_index(), a);
  context->FixInputPort(port_B.get_index(), b);

  context->FixInputPort(system.get_port_selector_input_port().get_index(),
                        Value<InputPortIndex>(port_A.get_index()));
  EXPECT_TRUE(CompareMatrices(system.get_output_port().Eval(*context), a));

  context->FixInputPort(system.get_port_selector_input_port().get_index(),
                        Value<InputPortIndex>(port_B.get_index()));
...
GTEST_TEST(PortSwitchTest, AbstactValueTest) {
  ...
  context->FixInputPort(port_A.get_index(), Value<std::string>("got A"));
  context->FixInputPort(port_B.get_index(), Value<std::string>("got B"));

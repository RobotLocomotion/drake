...
const double kTwoHertz = 0.5;
const int kLength = 3;

// A simple type containing a vector, to simplify checking expected values for
// a vector-valued TimeDelay (`BasicValue`) and an abstract-valued TimeDelay
...
class TimeDelayTest : public ::testing::TestWithParam<bool> {
 protected:
  TimeDelayTest()
  ...
  void SetUp() override {
    ...
    if (!is_abstract_) {
      hold_ =
          std::make_unique<TimeDelay<double>>(kTwoHertz, kTenHertz, kLength);
    } else {
      // Initialize to zero.
      hold_ = std::make_unique<TimeDelay<double>>(kTwoHertz, kTenHertz,
...
// // Tests that the time delay has abstract state with the desired initial
// // value.
// TEST_P(TimeDelayTest, InitialState) {
...
// // Tests that the output is the delayed state.
...
TEST_P(TimeDelayTest, NextUpdateTimeMustNotBeCurrentTime) {
...
TEST_P(TimeDelayTest, NextUpdateTimeIsInTheFuture) {
  // Calculate the next update time.
...
TEST_P(TimeDelayTest, ToAutoDiff) {
  EXPECT_TRUE(is_autodiffxd_convertible(*hold_));
}

TEST_P(TimeDelayTest, ToSymbolic) {
  EXPECT_TRUE(is_symbolic_convertible(*hold_));
}

// Instantiate parameterized test cases for is_abstract_ = {false, true}
INSTANTIATE_TEST_CASE_P(test, TimeDelayTest, ::testing::Values(false));

// class SymbolicTimeDelayTest : public ::testing::Test {
//  protected:
//   void SetUp() override {
//     const double delay_sec = 0.5;
//     const double update_sec = 0.1;
//     const int size = 1;
//     hold_ = std::make_unique<TimeDelay<symbolic::Expression>>(
//         delay_sec, update_sec, size);

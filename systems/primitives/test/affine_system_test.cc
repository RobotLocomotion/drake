...
TEST_F(AffineSystemTest, InitialConditions) {
  ...
  dut_->set_initial_state_covariance(cov);
  ...
  // We expect the sample mean to be within sqrt(n/N) via Bai, Yin in
  // https://case.edu/artsci/math/mwmeckes/perspectivesInHighDimensions/litvak.pdf
  // // noqa NOTE(russt): I expected to need a fudge factor, but did not.

...
TEST_F(SymbolicVectorSystemAutoDiffXdTest, AutodiffXdFullGradient) {
  const VectorX<AutoDiffXd> txup =
      math::InitializeAutoDiffFromValueMatrix(txupval_);
  SetContext(txup);

  const auto xdotval =
      autodiff_system_->EvalTimeDerivatives(*context_).CopyToVector();
  EXPECT_TRUE(
      CompareMatrices(math::ExtractValueMatrixFromAutoDiff(xdotval),
  ...
  EXPECT_TRUE(CompareMatrices(
      math::ExtractValueMatrixFromAutoDiff(yval),
...
TEST_F(SymbolicVectorSystemAutoDiffXdTest, AutodiffXdGradientRelativeToInput) {
  ...
  gradient(3, 1) = 1.0;  // u₁
  const VectorX<AutoDiffXd> txup =
      math::InitializeAutoDiffFromValueAndGradientMatrix(
...
TEST_F(SymbolicVectorSystemAutoDiffXdTest, AutodiffXdGradientRelativeToState) {
  ...
  txup.head<2>() = math::InitializeAutoDiffFromValueMatrix(txupval_.head<2>());
  SetContext(txup);

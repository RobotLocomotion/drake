...
GTEST_TEST(SymbolicVectorSystemBuilderTest, BasicPattern) {
  ...
  EXPECT_TRUE(SymbolicVectorSystemBuilder().time(t).time_var_->equal_to(t));

  EXPECT_TRUE(
      SymbolicVectorSystemBuilder().state(x[1]).state_vars_[0].equal_to(x[1]));

  EXPECT_TRUE(
      SymbolicVectorSystemBuilder().state(x).state_vars_[1].equal_to(x[1]));

  EXPECT_TRUE(
      SymbolicVectorSystemBuilder().input(u[1]).input_vars_[0].equal_to(u[1]));

  EXPECT_TRUE(
      SymbolicVectorSystemBuilder().input(u).input_vars_[1].equal_to(u[1]));
  ...
  EXPECT_EQ(SymbolicVectorSystemBuilder().time_period(3.2).time_period_, 3.2);
...
GTEST_TEST(SymbolicVectorSystemTest, DanglingReferenceTest) {
  ...
  builder1.state(x);
  EXPECT_TRUE(builder1.state_vars_[0].equal_to(x[0]));

  const auto& builder2 = SymbolicVectorSystemBuilder().state(x);
  EXPECT_TRUE(builder2.state_vars_[0].equal_to(x[0]));
}
...
GTEST_TEST(SymbolicVectorSystemTest, ScalarPassThrough) {
  ...
  SymbolicVectorSystem<double> system(
      {}, EmptyVector<Variable>{}, Vector1<Variable>{u},
      EmptyVector<Expression>{}, Vector1<Expression>{u});
...
GTEST_TEST(SymbolicVectorSystemTest, VectorPassThrough) {
  // y = u.
  Vector2<Variable> u{Variable{"u0"}, Variable{"u1"}};
  SymbolicVectorSystem<double> system({}, EmptyVector<Variable>{}, u,
...
GTEST_TEST(SymbolicVectorSystemTest, OutputScaledTime) {
  Variable t("t");
  SymbolicVectorSystem<double> system(
      t, EmptyVector<Variable>{}, EmptyVector<Variable>{},
      EmptyVector<Expression>{}, Vector1<Expression>{2. * t});
...
GTEST_TEST(SymbolicVectorSystemTest, ContinuousStateOnly) {
  ...
  Variable x{"x"};
  SymbolicVectorSystem<double> system(
      {}, Vector1<Variable>{x}, EmptyVector<Variable>{},
      Vector1<Expression>{-x + pow(x, 3)}, EmptyVector<Expression>{});
...
GTEST_TEST(SymbolicVectorSystemTest, DiscreteStateOnlyTest) {
  ...
  SymbolicVectorSystem<double> system(
      {}, Vector1<Variable>{x}, EmptyVector<Variable>{},
      Vector1<Expression>{-x + pow(x, 3)}, EmptyVector<Expression>{}, 0.1);

...
TEST_F(SymbolicVectorSystemTest, TestAutodiffXd) {
  ...
  const Vector2d pval{2.0, 12.0};
  Vector6d txupval;
  txupval << tval, xval, uval, pval;

  const VectorX<AutoDiffXd> txup = math::initializeAutoDiff(txupval);

  context->SetTime(txup[0]);
  context->SetContinuousState(txup.segment<1>(1));
  context->FixInputPort(0, txup.segment<2>(2));
  context->get_mutable_numeric_parameter(0).SetFromVector(txup.tail<2>());

  const auto xdotval =
      autodiff_system->EvalTimeDerivatives(*context).CopyToVector();

  EXPECT_TRUE(CompareMatrices(math::autoDiffToValueMatrix(xdotval),
                              Vector1d{-xval * pval[0] + xval * xval * xval}));

  {
    Vector6d expected;
    expected << 0.0, -pval[0] + 3.0 * xval * xval, 0.0, 0.0, -xval, 0.0;
    EXPECT_TRUE(CompareMatrices(xdotval[0].derivatives(), expected));
  }
  ...
  const auto& yval = autodiff_system->get_output_port(0)
                         ...
                         .get_value();
  EXPECT_TRUE(
      CompareMatrices(math::autoDiffToValueMatrix(yval),
                      Vector1d{tval + 2 * uval[0] + 3 * uval[1] + pval[1]}));
  {
    Vector6d expected;
    expected << 1.0, 0.0, 2.0, 3.0, 0.0, 1.0;
    EXPECT_TRUE(CompareMatrices(yval[0].derivatives(), expected));
  }
}

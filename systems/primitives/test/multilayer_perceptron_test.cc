...
GTEST_TEST(MultilayerPerceptronTest, RandomParameters) {
  ...
  EXPECT_TRUE(
      CompareMatrices(mlp.GetWeights(mean, 0), VectorXd::Zero(100), 0.3));
  EXPECT_TRUE(
      CompareMatrices(mlp.GetBiases(mean, 0), VectorXd::Zero(100), 0.3));
  EXPECT_TRUE(CompareMatrices(mlp.GetWeights(std_dev, 0),
                              VectorXd::Constant(100, 1.0), 0.2));
  ...
  EXPECT_TRUE(CompareMatrices(mlp.GetWeights(mean, 1),
                              Eigen::RowVectorXd::Zero(100), 0.03));
  ...
  EXPECT_TRUE(CompareMatrices(mlp.GetWeights(std_dev, 1),
                              Eigen::RowVectorXd::Constant(100, 0.1), 0.02));

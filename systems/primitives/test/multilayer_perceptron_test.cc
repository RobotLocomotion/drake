...
void BasicTest() {
  ...
  EXPECT_EQ(mlp.activation_type(), PerceptronActivationType::kReLU);
  ...
  EXPECT_FALSE(CompareMatrices(mlp.GetBiases(*context, 2), b));
  ...
  EXPECT_FALSE(CompareMatrices(mlp.GetBiases(params, 2), b));
...
template <typename T>
void CalcOutputTest(PerceptronActivationType type) {
  MultilayerPerceptron<T> mlp({2, 3, 3, 2}, type);
  auto context = mlp.CreateDefaultContext();
...
GTEST_TEST(MultilayerPerceptronTest, CalcOutput) {
  for (PerceptronActivationType type : {kIdentity, kReLU, kTanh}) {  
    CalcOutputTest<double>(type);
    CalcOutputTest<AutoDiffXd>(type);
    CalcOutputTest<symbolic::Expression>(type);
  }
}

// Check that backprop gives the same gradients as AutoDiffXd.
GTEST_TEST(MultilayerPerceptionTest, Backprop) {
  // TODO(russt): loop over types.
  std::vector<int> layers({2, 3, 3, 2});
  MultilayerPerceptron<double> mlp(layers, kIdentity);
  MultilayerPerceptron<AutoDiffXd> mlp_ad(layers, kIdentity);
  ...
  auto MSE_loss = [Y_desired](const Eigen::Ref<const Eigen::MatrixXd>& Y,
                                   Eigen::MatrixXd* dloss_dY) {
    *dloss_dY = 2 * (Y - Y_desired) / Y.cols();
    return (Y - Y_desired).squaredNorm() / Y.cols();
  };
  double loss = mlp.Backpropagation(*context, X, MSE_loss, &dloss_dparams);

...
GTEST_TEST(ContinuousToDiscrete, test) {
  ...
  EXPECT_ANY_THROW(ContinuousToDiscrete<double>(
  ...
  auto discrete_affine_sys =
      ContinuousToDiscrete<double>(continuous_affine_sys, h);

  const double epsilon = 1e-10;
  EXPECT_TRUE(discrete_affine_sys->A().isApprox(Ad, epsilon));
  EXPECT_TRUE(discrete_affine_sys->B().isApprox(Bd, epsilon));
  EXPECT_TRUE(discrete_affine_sys->f0().isApprox(f0d, epsilon));
  EXPECT_TRUE(discrete_affine_sys->C().isApprox(Cd, epsilon));
  EXPECT_TRUE(discrete_affine_sys->D().isApprox(Dd, epsilon));
  EXPECT_TRUE(discrete_affine_sys->y0().isApprox(y0d, epsilon));

  LinearSystem<double> continuous_linear_sys(A, B, C, D);
  auto discrete_linear_sys =
      ContinuousToDiscrete<double>(continuous_affine_sys, h);

  EXPECT_TRUE(discrete_linear_sys->A().isApprox(Ad, epsilon));
  EXPECT_TRUE(discrete_linear_sys->B().isApprox(Bd, epsilon));
  EXPECT_TRUE(discrete_linear_sys->C().isApprox(Cd, epsilon));
  EXPECT_TRUE(discrete_linear_sys->D().isApprox(Dd, epsilon));
}

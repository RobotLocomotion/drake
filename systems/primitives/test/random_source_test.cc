...
GTEST_TEST(RandomSourceTest, UniformWhiteNoise) {
  ...
  EXPECT_EQ(random_source->sampling_interval_sec(), 0.0025);
...
GTEST_TEST(RandomSourceTest, UniformWhiteNoiseAutoDiff) {
  ...
  // Now take a single sample from this source. Make sure the scalar type of the sample is AutoDiffXd, and the gradient is empty.
  auto context = random_source->CreateDefaultContext();
  auto sample = random_source->get_output_port().Eval(*context);
  static_assert(
      std::is_same<decltype(sample)::Scalar, AutoDiffXd>::value,

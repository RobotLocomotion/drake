 ...
 protected:
  int max_num_allocations() const {
    if constexpr (std::is_same_v<T, symbolic::Expression>) {
      // symbolic:: generates heap traffic on its own.
      return 5998;
    } else {
      return 0;
    }
  ...
  VectorLog<T> log_{3};
  Vector3<T> data_{1.1, 2.2, 3.3};
  // Convert our vector immediately to avoid copy-construction at run time.
  VectorX<T> record_{data_};
};

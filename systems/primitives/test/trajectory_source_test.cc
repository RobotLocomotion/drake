...
GTEST_TEST(AdditionalTrajectorySourceTests, ScalarConversion) {
  ...
  for (bool zero_derivatives_beyond_limits : {true, false}) {
    ...
    // We can call UpdateTrajectory with a Trajectory<Expression>.
    auto pp_sym = PiecewisePolynomial<Expression>::ZeroOrderHold(
        Vector3<Expression>{0, 1, 2}, RowVector3<Expression>{1.2, 3, 1.5});
    source_sym->UpdateTrajectory(pp_sym);

    // TODO(russt): Evaluate the output port with the symbolic time and confirm
    // the results. At present, the trajectory classes don't support non-double
    // symbolic times; they throw when looking up the segment.
  }
}

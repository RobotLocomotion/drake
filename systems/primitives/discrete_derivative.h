...
class DiscreteDerivative final : public LeafSystem<T> {
 public:
  ...
  DRAKE_DEPRECATED("Per GSG, use set_state(context, u) instead.")
...
class StateInterpolatorWithDiscreteDerivative final : public Diagram<T> {
 public:
  ...
  /// Constructor taking @p num_inputs, the size of the position vector to be
  /// differentiated, and @p time_step, the sampling interval.
  StateInterpolatorWithDiscreteDerivative(int num_inputs, double time_step);

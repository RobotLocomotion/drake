...
class StateInterpolatorWithDiscreteDerivative final : public Diagram<T> {
 public:
  ...
  /// Convenience method to set the entire position history for the
  /// discrete-time derivative to as if the most recent input was @p
  /// position, and the input before that was whatever was required to
  /// produce the output velocity @p velocity.  @p position and @p velocity
  /// must be the same size as the input/output ports.

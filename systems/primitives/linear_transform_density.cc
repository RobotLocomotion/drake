...
T LinearTransformDensity<T>::CalcDensity(const Context<T>& context) const {
  ...
  if (input_size_ == output_size_) {
    ...
    if (qr_solver.isInvertible()) {
      ...
      // observe the value of the output sample w_out, and we want to fix the
      // output to this sampled value, and compute the gradient of the
      // probability for taking this fixed output sample. If we were to include
      // the gradient of w_out (which equals to A_in * w_in + b), then we would
      // essentially compute the gradient of the density(w_in) w.r.t A and b,
      // the gradient of ∂w_in/ ∂A and ∂w_in/ ∂b is zero, leading to zero
      // gradient of density(w_in). This zero gradient isn't what we want (we
      // want the gradient of density(w_out), not the gradient of
      // density(w_in)).

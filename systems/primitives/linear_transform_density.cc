...
T LinearTransformDensity<T>::CalcDensity(const Context<T>& context) const {
  ...
  if (input_size_ == output_size_) {
    ...
    if (qr_solver.isInvertible()) {
      ...
      // gradient of w_out = A_in * w_in + b, then we would essentially compute
      // the gradient of the density(w_in) w.r.t A and b, the gradient of ∂w_in/
      // ∂A and ∂w_in/ ∂b would be zero, leading to zero gradient of
      // density(w_in). This zero gradient isn't what we want (we want the
      // gradient of density(w_out)).

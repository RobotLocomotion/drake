...
DiscreteDerivative<T>::DiscreteDerivative(int num_inputs, double time_step,
  ...
  this->DeclarePeriodicDiscreteUpdateEvent(time_step_, 0.0,
                                           &DiscreteDerivative<T>::CalcUpdate);
  this->DeclareForcedDiscreteUpdateEvent(&DiscreteDerivative<T>::CalcUpdate);
}
...
template <typename T>
EventStatus DiscreteDerivative<T>::CalcUpdate(

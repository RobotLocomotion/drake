...
StateInterpolatorWithDiscreteDerivative<
    T>::StateInterpolatorWithDiscreteDerivative(int num_inputs,
                                                double time_step) {
  DiagramBuilder<T> builder;

  auto pass_through = builder.template AddSystem<PassThrough>(num_inputs);
  derivative_ =
      builder.template AddSystem<DiscreteDerivative>(num_inputs, time_step);
  auto mux = builder.template AddSystem<Multiplexer>(
      std::vector<int>{num_inputs, num_inputs});
...
void StateInterpolatorWithDiscreteDerivative<T>::set_initial_position(
  ...
  derivative_->set_state(context, position);
}

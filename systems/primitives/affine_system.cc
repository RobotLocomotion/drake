...
// correct size.
int CalcNumStates(const std::optional<Eigen::Ref<const Eigen::MatrixXd>>& A,
                  const std::optional<Eigen::Ref<const Eigen::MatrixXd>>& B,
                  const std::optional<Eigen::Ref<const Eigen::VectorXd>>& f0,
                  const std::optional<Eigen::Ref<const Eigen::MatrixXd>>& C) {
  int num_states = 0;
...
// correct size.
int CalcNumInputs(const std::optional<Eigen::Ref<const Eigen::MatrixXd>>& B,
                  const std::optional<Eigen::Ref<const Eigen::MatrixXd>>& D) {
  int num_inputs = 0;
  if (B) {
    num_inputs = B->cols();
...
// correct size.
int CalcNumOutputs(const std::optional<Eigen::Ref<const Eigen::MatrixXd>>& C,
                   const std::optional<Eigen::Ref<const Eigen::MatrixXd>>& D,
                   const std::optional<Eigen::Ref<const Eigen::VectorXd>>& y0) {
  int num_outputs = 0;
  if (C) {
...
bool IsMeaningful(const std::optional<Eigen::Ref<const Eigen::MatrixXd>>& m) {
  return m && m->size() > 0 && (m->array() != 0).any();
}

...
TimeVaryingAffineSystem<T>::TimeVaryingAffineSystem(
  ...
  if (num_outputs_ > 0) {
    // Ensure we have the base prerequisites used in this class. Derived
    // classes will elect the individual prerequisites they need, which may not
    // encompass the prerequisites for an actual time-varying affine system.
    output_prerequisites.insert(this->all_state_ticket());
    this->DeclareVectorOutputPort(BasicVector<T>(num_outputs_),
                                  &TimeVaryingAffineSystem::CalcOutputY,
                                  std::move(output_prerequisites));
  }
}
...
std::set<DependencyTicket> OutputTickets(const Eigen::MatrixXd& D) {
  if (IsMeaningful(D)) {
    return {

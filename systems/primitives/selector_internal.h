...
class SelectorProgram {
 public:
  ...
  /* The recipe for a single output element (i.e., a single scalar). Which input
  port and output port these offets apply to is not stored here; rather, it is
  inferred from the container(s) that store this element. */
  struct Element {

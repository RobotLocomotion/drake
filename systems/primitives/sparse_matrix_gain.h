...
/// @f]
///
/// Note that, unlike MatrixGain, this system is does not derive from
/// LinearSystem (which does not yet support sparse matrices). However, we use
/// `D` as the name for the gain matrix here to be consistent.
///
/// @system
...
class SparseMatrixGain final : public LeafSystem<T> {
 public:
  ...
  ~SparseMatrixGain() final = default;

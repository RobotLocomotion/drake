...
/// output `delayed_u` which is previously received input, delayed by the
/// given amount.  The initial output will be a vector of zeros until the delay
/// time has passed.
///
...
class DiscreteTimeDelay final : public LeafSystem<T> {
 public:
  ...
  /// (Advanced) Manually sample the input port and updates the state of the

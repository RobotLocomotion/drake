...
/// vector of zeros until the delay time has passed.
///
...
class DiscreteTimeDelay final : public LeafSystem<T> {
 public:
  ...
  /// (Advanced) Manually returns the delay buffer state for an abstract-valued
  /// %DiscreteTimeDelay, with the oldest value in the buffer being the first.
  /// This is mostly useful for testing.
  std::unique_ptr<AbstractValues> GetAbstractBuffer(Context<T>* context);

  /// (Advanced) Manually sets the delay buffer state for an abstract-valued
  /// %DiscreteTimeDelay, with the oldest value in the buffer being the first.
  /// This is mostly useful for testing.
  void SetAbstractBuffer(Context<T>* context, AbstractValues* buffer_state);

...
class PortSwitch final : public LeafSystem<T> {
 public:
  ...
  conversion). The type of `model_value` must be default_constructable.  */
 ...
 private:
  ...
  // Use shared_ptr here because the model values will be the same for all
  // scalar types if the model_value is not templated on scalar type.  Make
  // sure to keep them const because it is easy/convenient to share these
  // values even across scalar-converted copies of the system.

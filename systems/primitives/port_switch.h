...
class PortSwitch final : public LeafSystem<T> {
  ...
  * DeclareInputPort() will be vector-valued ports of size `vector_size`. */
  ...
  // TODO(russt): Add support for scalar-typed abstract values (likely by
  //  accepting model_values for each scalar type).  This constructor
  //  assumes they are scalar agnostic.
  /** Constructs a %PortSwitch using `model_value` as the model for the output
  port.  All input ports declared via DeclareInputPort() will be

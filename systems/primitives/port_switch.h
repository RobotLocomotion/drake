...
// TODO(russt): Consider adding (opt-in) support for zero-crossing detection
// when the port selection changes.  This will require adding a state variable
...
class PortSwitch final : public LeafSystem<T> {
 public:
  ...
  explicit PortSwitch(const OutputType& model_value)
      : PortSwitch(-1, new Value<OutputType>(model_value)) {}
 ...
 private:
  PortSwitch(int vector_size, const AbstractValue* model_value);
  ...
  const copyable_unique_ptr<const AbstractValue> model_value_;
};

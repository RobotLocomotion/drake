...
class Demultiplexer final : public LeafSystem<T> {
 public:
  ...
  ///
  /// @p output_ports_sizes must not be a zero length vector.
  ///
  ...
  /// output_ports_sizes will be the size of the input port. Drake will throw if
  /// the input port size is 0.
  ...
  ///
  /// @p output_ports_size must exactly divide @p size. Otherwise this
 ...
 private:
  ...
  // Return a std vector has the same size as the input @p output_ports_sizes.
  // The values are calculated as the start index of the output port inside the
  // input port vector.
  static const std::vector<int> CalOutputPortsStart(
    ...
    output_ports_start.reserve(num_output_ports);

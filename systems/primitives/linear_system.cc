...
std::unique_ptr<AffineSystem<double>> DoFirstOrderTaylorApproximation(
  ...
  if (num_states > 0) {
    if (autodiff_context->has_only_continuous_state()) {
      ...
      const Eigen::MatrixXd AB =
          math::ExtractGradientMatrixFromAutoDiff(autodiff_xdot_vec);
      A = AB.leftCols(num_states);
      B = AB.rightCols(num_inputs);

      const Eigen::VectorXd xdot0 =
          math::ExtractValueMatrixFromAutoDiff(autodiff_xdot_vec);
      ...
      const Eigen::MatrixXd AB =
          math::ExtractGradientMatrixFromAutoDiff(autodiff_x1_vec);
      A = AB.leftCols(num_states);
      B = AB.rightCols(num_inputs);

      const Eigen::VectorXd x1 =
          math::ExtractValueMatrixFromAutoDiff(autodiff_x1_vec);
  ...
  if (output_port) {
    ...
    const Eigen::MatrixXd CD =
        math::ExtractGradientMatrixFromAutoDiff(autodiff_y0);
    C = CD.leftCols(num_states);
    D = CD.rightCols(num_inputs);

    const Eigen::VectorXd y = math::ExtractValueMatrixFromAutoDiff(autodiff_y0);

...
class TrajectorySource : public SingleOutputVectorSource<T> {
 public:
  ...
  explicit TrajectorySource(const trajectories::Trajectory& trajectory,
 ...
 private:
  const std::unique_ptr<trajectories::Trajectory> trajectory_;
  const bool clamp_derivatives_;
  std::vector<std::unique_ptr<trajectories::Trajectory>> derivatives_;
};

...
class MultilayerPerceptron final : public LeafSystem<T> {
 public:
  ...
  /** Constructs the MLP with an additional option to transform the input vector
   ...
   transform [θ, θ̇] => [sin θ, cos θ, θ̇]. This would be accomplished by
   ...
   @param use_sin_cos_for_input is a boolean vector that determines the number
   of inputs in the network and whether the sin/cos transform is applied to each
   element.

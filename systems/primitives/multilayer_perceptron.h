...
class MultilayerPerceptron final : public LeafSystem<T> {
 public:
  ...
  /** Constructs the MLP with the same activation type for every layer.
   ...
   _every_ layer of the network (including the last layer).
  ...
  /** Sets all of the parameters (all weights and biases) in the `parameters`
   from a zero-mean, unit-covariance Gaussian distribution. This is typically
   called via System<T>::SetRandomContext. By contrast,

...
template <>
void SymbolicVectorSystem<Expression>::CalcOutput(
    const Context<Expression>& context,
    BasicVector<Expression>* output_vector) const {
  DRAKE_DEMAND(output_.size() > 0);
...
void SymbolicVectorSystem<double>::DoCalcTimeDerivatives(
  ...
  VectorBase<double>& xdot = derivatives->get_mutable_vector();
...
template <>
void SymbolicVectorSystem<Expression>::DoCalcTimeDerivatives(
    const Context<Expression>& context,
  ...
  VectorBase<Expression>& xdot = derivatives->get_mutable_vector();
  for (int i = 0; i < dynamics_.size(); i++) {
...
void SymbolicVectorSystem<double>::DoCalcDiscreteVariableUpdates(
  ...
  VectorBase<double>& xnext = updates->get_mutable_vector();
  for (int i = 0; i < dynamics_.size(); i++) {
...
template <>
void SymbolicVectorSystem<Expression>::DoCalcDiscreteVariableUpdates(
    const drake::systems::Context<Expression>& context,
    const std::vector<const drake::systems::DiscreteUpdateEvent<Expression>*>&
        events,
    drake::systems::DiscreteValues<Expression>* updates) const {
  unused(events);
  DRAKE_DEMAND(time_period_ > 0.0);
  DRAKE_DEMAND(dynamics_.size() > 0);
  symbolic::Substitution s;
  ...
  VectorBase<Expression>& xnext = updates->get_mutable_vector();
  for (int i = 0; i < dynamics_.size(); i++) {

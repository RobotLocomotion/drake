...
///
/// By default, sampling is performed every time the Simulator completes an
/// internal time step (that is, via a per-step Publish event). That means the
/// samples will in general be unevenly spaced in time. You may
/// optionally specify a "publish period" in which case sampling occurs
/// periodically, with the first sample occurring at time 0. Alternatively (not
/// common), you can specify that logging should only occur at "forced publish"
/// events, meaning explicit calls to System::Publish().
///
/// @warning This System is _not_ thread-safe because it writes to a mutable
...
class SignalLogger : public LeafSystem<T> {
 public:
  ...
  // TODO(sherm1) This is an O(n²) storage expansion scheme. Should use an
  // amortized-O(n) scheme like std::vector does (e.g. double each time).
  ...
  /// only if set_forced_publish_only() hasn't been called.
  ...
  /// to System::Publish() issued directly or by the Simulator. This method

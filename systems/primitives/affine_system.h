...
class TimeVaryingAffineSystem : public LeafSystem<T> {
 public:
  ...
  /// Sets the initial conditions of the system in the default context.
  /// `x0` must be a vector of length `num_states`.
  void set_initial_state(const Eigen::Ref<const VectorX<T>>& x0);

  /// Sets the initial conditions to be drawn from a Gaussian (when called
  /// initialized from the `SetRandomContext` methods).  The mean
  /// of the distribution will be the initial conditions set using
  /// `SetInitialState`.  `covariance` must have size `num_states` x
  /// `num_states` and must be symmetric and positive semi-definite.
  void set_initial_state_covariance(

...
// @returns true if the vector has any derivatives.
bool HasDerivatives(const VectorX<AutoDiffXd>& vector) {
  for (int k = 0; k < vector.size(); ++k) {
    if (vector[k].derivatives().size() > 0) {
      return true;
    }
  }
  return false;
...
Saturation<T>::Saturation(const Saturation<U>& other)
  ...
  if constexpr (std::is_same_v<U, AutoDiffXd>) {
    ...
    // Refuse conversion if derivatives would be discarded.
    DRAKE_THROW_UNLESS(!HasDerivatives(other.min_value_));
    DRAKE_THROW_UNLESS(!HasDerivatives(other.max_value_));
  }
}
...
Saturation<T>::Saturation(bool min_max_ports_enabled, int input_size,
  ...
  this->DeclareVectorOutputPort(kUseDefaultName, input_size_,
                                ...
                                // This seems unnecessary (the default
                                // all_sources_ticket is a semantic superset),
                                // but it is the only ticket special-cased in

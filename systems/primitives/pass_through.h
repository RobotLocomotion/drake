...
class PassThrough final : public LeafSystem<T> {
 public:
  ...
  /// When no input is connected, the output will be vector of all zeros.
  explicit PassThrough(int vector_size)
      : PassThrough(VectorX<T>::Zero(vector_size), nullptr) {}

  /// Constructs a system with vector-valued input/output ports.
  /// @param value The constant value to emit, when no input is connected.
  explicit PassThrough(const Eigen::Ref<const VectorX<T>>& value)
  ...
  explicit PassThrough(const AbstractValue& abstract_model_value)
      : PassThrough(Vector0<T>(), abstract_model_value.Clone()) {}
 ...
 private:
  ...
  PassThrough(const Eigen::Ref<const VectorX<T>>& model_vector_,

...
VectorLogSink<T>::VectorLogSink(const VectorLogSink<U>& other)
    : VectorLogSink<T>(other.get_input_port().size(), other.publish_triggers_,
                       other.publish_period_) {}

template <typename T>
const VectorLog<T>& VectorLogSink<T>::GetLog(const Context<T>& context) const {
...
template <typename T>
VectorLog<T>& VectorLogSink<T>::GetMutableLog(Context<T>* context) const {
  return GetLogFromCache(*context);
}

template <typename T>
const VectorLog<T>& VectorLogSink<T>::FindLog(
    const Context<T>& root_context) const {
  return GetLogFromCache(this->GetMyContextFromRoot(root_context));
}

template <typename T>
VectorLog<T>& VectorLogSink<T>::FindMutableLog(Context<T>* root_context) const {
  return GetLogFromCache(this->GetMyMutableContextFromRoot(root_context));
}

template <typename T>
VectorLog<T>& VectorLogSink<T>::GetLogFromCache(
    const Context<T>& context) const {
  this->ValidateContext(context);
  CacheEntryValue& value = this->get_cache_entry(log_cache_index_)
...
EventStatus VectorLogSink<T>::WriteToLog(const Context<T>& context) const {
  GetLogFromCache(context).AddData(context.get_time(),

...
/// context's `seed` parameter to `per_instance_seed`.  Therefore, the system's
/// parameters, state, and outputs will be identical for all simulations that
/// start from a default context within the current executable process.
///
/// 3. By default, `source.SetRandomContext()` will choose a new, arbitrary
/// values for the context's `seed` parameter, which means that the system's

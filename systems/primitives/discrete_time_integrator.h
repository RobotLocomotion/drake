...
/** Defines the integration methods supported by the DiscreteTimeIntegrator. */
enum IntegrationMethod {
  kExplicitEuler
  // TODO(russt): Add additional methods as needed, matching the names used
  // in https://drake.mit.edu/doxygen_cxx/group__integrators.html .
};

/** A discrete-time integrator for a vector input.
...
For example, the discrete state space dynamics of %DiscreteTimeIntegrator with
time step `h` using the integration method `kExplicitEuler` is:
```
xₙ₊₁ = xₙ + h uₙ  // update
yₙ = xₙ           // output
x₀ = xᵢₙᵢₜ        // initialize
```
where xᵢₙᵢₜ = 0 by default. Use set_integral_value() to set x_n in the context.
The output at time `t` is `xₙ` where `n = floor(t/h)`.
...
class DiscreteTimeIntegrator final : public LeafSystem<T> {
 public:
  ...
  DiscreteTimeIntegrator(int size, double time_step,
                         IntegrationMethod method = kExplicitEuler);
  ...
  /// Returns the integration method used by the integrator.
  IntegrationMethod integration_method() const { return method_; }
 ...
 private:
  ...
  double time_step_{};
  IntegrationMethod method_{kExplicitEuler};
};

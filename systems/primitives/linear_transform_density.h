 ...
 *
 * @tparam T Nonsymbolic default scalars.
 */
template <typename T>
class LinearTransformDensity final : public LeafSystem<T> {
 public:
   ...
   * density(w_out_sample). Namely we given an output sample, we want to know
   ...
   * @throw exception if A is not an invertible matrix.
   */

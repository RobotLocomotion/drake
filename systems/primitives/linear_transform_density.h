...
class LinearTransformDensity final : public LeafSystem<T> {
 public:
   ...
   * density(w_out_sample) w.r.t some other variables. Here w_out_sample is
   * FIXED to the sampled output, its value doesn't change with the variable we
   * take gradient with, namely the gradient tells us how the probability of
   * sampling w_out_sample would change, when A and b changes.
   * @pre If matrix A must be invertible. Otherwise throw a runtime error.
   */

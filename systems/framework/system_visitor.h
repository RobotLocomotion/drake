...
each step, we need to know if the current System is a Diagram (or not).  Rather
than performing the dynamic_cast, or adding a virtual method to System, you may
use the visitor pattern enabled by this class, e.g.:
...
class SystemVisitor {
 public:
  ...
  Diagram<T>. */
  virtual void VisitNonDiagram(const System<T>& system) = 0;

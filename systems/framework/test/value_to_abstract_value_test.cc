...
GTEST_TEST(ValueToAbstractValue, AbstractPolicyOnVectors) {
  ...
  // More complicated Eigen objects should throw.
  const Eigen::Vector4d long_vec(.25, .5, .75, 1.);
  DRAKE_EXPECT_THROWS_MESSAGE(
      AbstractPolicy::ToAbstract("MyApi", long_vec.tail(3)),  // (3)
      std::logic_error,
      ".*MyApi.*given Eigen object.*not suitable for.*Drake abstract.*");

  DRAKE_EXPECT_THROWS_MESSAGE(
      AbstractPolicy::ToAbstract("MyApi", long_vec.segment<2>(1)),  // (3)
      std::logic_error,
      ".*MyApi.*given Eigen object.*not suitable for.*Drake abstract.*");

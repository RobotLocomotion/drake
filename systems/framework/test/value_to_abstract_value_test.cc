...
// Test each of the three ToAbstract() methods implemented for AbstractPolicy.
// For reference:
...
// (3) ToAbstract<V>(V) creates Value<V> for copyable or cloneable types V,
//                      or Value<B> if V's clone returns a base type B.
// Method (3) is supposed to prefer the copy constructor if it and Clone() are
// both available.
//
...
GTEST_TEST(ValueToAbstractValue, AbstractPolicy) {
  ...
  // Check signature (3) handling of simple objects.

  const auto int_val = AbstractPolicy::ToAbstract(5);  // (3)
  EXPECT_EQ(int_val->get_value<int>(), 5);

  const auto str_val =
      AbstractPolicy::ToAbstract(std::string("string"));  // (3)
  EXPECT_EQ(str_val->get_value<std::string>(), "string");

  // Check signature (3) handling of objects that are: just copyable,
  // just cloneable (to self or base class), and both copyable and cloneable.

  const auto copyable_val =
      AbstractPolicy::ToAbstract(CopyConstructible(1.25));  // (3, copy)
  EXPECT_EQ(copyable_val->get_value<CopyConstructible>().value(), 1.25);

  const auto cloneable_val =
      AbstractPolicy::ToAbstract(JustCloneable(0.75));  // (3, clone)
  ...
  const auto base_cloneable_val = AbstractPolicy::ToAbstract(
      BaseClassCloneable("base_cloneable", 1.5));  // (3, clone-to-base)
  ...
  const auto derived_cloneable_val =
      AbstractPolicy::ToAbstract(
          DerivedClassCloneable("derived_cloneable", 0.25));  // (3, clone)
  ...
  const auto cc_val_abstract_copy =
      AbstractPolicy::ToAbstract(cc_value);  // (3, prefer copy)
  ...
  // These are also (3) but should trigger a nice static_assert message if
...
// This is just a sanity check that the AbstractPolicy does not treat vector
// types specially. They should all be handled through signature (3) as
// ordinary objects.
GTEST_TEST(ValueToAbstractValue, AbstractPolicyOnVectors) {
  ...
  const auto scalar_val = AbstractPolicy::ToAbstract(expected_scalar);
  EXPECT_EQ(scalar_val->get_value<double>(), expected_scalar);

  // An Eigen vector should not be stored in a BasicVector.
  const auto vec3_val = AbstractPolicy::ToAbstract(expected_vec);
  ...
  auto tail3_val = AbstractPolicy::ToAbstract(long_vec.tail(3));
  ...
  auto segment_val = AbstractPolicy::ToAbstract(long_vec.segment<2>(1));
  ...
  const auto basic_vector_val = AbstractPolicy::ToAbstract(basic_vector3);
  ...
  // handled like any other cloneable object by signature (3) and stored as
  // Value<MyVector>. That's different from the VectorPolicy treatment.
  const MyVector3d my_vector3(expected_vec);
  auto my_vector3_val = AbstractPolicy::ToAbstract(my_vector3);
...
GTEST_TEST(ValueToVectorValue, VectorPolicy) {
  ...
  auto tail3_val = VectorPolicy::ToAbstract("MyApi", last3);  // (1)
  ...
  const auto basic_vector_val =
      VectorPolicy::ToAbstract(__func__, basic_vector3);  // (2)
  EXPECT_EQ(basic_vector_val->get_value<BasicVector<double>>().get_value(),
            Eigen::Vector3d(7., 8., 9.));

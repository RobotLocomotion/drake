...
class DummyVector : public VectorBase<T> {
 public:
  ...
  DummyVector() {
    this->AppendInequalityConstraintUpperBound(static_cast<T>(10));
    this->AppendInequalityConstraintLowerBound(static_cast<T>(-10));
    this->AppendInequalityConstraintBounds(T(-5), T(20));
  }

...
void VerifyClonedState(const State<double>& clone) {
  ...
  EXPECT_EQ(1.0, xc.get_generalized_position().at(0));
  EXPECT_EQ(2.0, xc.get_generalized_position().at(1));

  EXPECT_EQ(kGeneralizedVelocitySize, xc.get_generalized_velocity().size());
  EXPECT_EQ(3.0, xc.get_generalized_velocity().at(0));
  EXPECT_EQ(5.0, xc.get_generalized_velocity().at(1));

  EXPECT_EQ(kMiscContinuousStateSize, xc.get_misc_continuous_state().size());
  EXPECT_EQ(8.0, xc.get_misc_continuous_state().at(0));
}
...
TEST_F(LeafContextTest, Clone) {
  ...
  xc.get_mutable_generalized_velocity().at(1) = 42.0;
  EXPECT_EQ(42.0, xc[3]);
  EXPECT_EQ(5.0, context_.get_continuous_state_vector().at(3));

  // -- Discrete
  BasicVector<double>& xd1 = clone->get_mutable_discrete_state(1);
  xd1.at(0) = 1024.0;
  EXPECT_EQ(1024.0, clone->get_discrete_state(1).at(0));
  EXPECT_EQ(256.0, context_.get_discrete_state(1).at(0));

  // Check State indexed discrete methods too.
  State<double>& state = clone->get_mutable_state();
  EXPECT_EQ(1024.0, state.get_discrete_state(1).at(0));
  EXPECT_EQ(1024.0, state.get_mutable_discrete_state(1).at(0));
  ...
  EXPECT_EQ(1.0, context_.get_numeric_parameter(0).at(0));
}
...
TEST_F(LeafContextTest, SetTimeStateAndParametersFrom) {
  ...
  EXPECT_EQ(128.0, target.get_discrete_state(0).at(0));
  // Verify that parameters were set.
  target.get_numeric_parameter(0);
  EXPECT_EQ(2.0, (target.get_numeric_parameter(0).at(1).value()));

...
TEST_F(SystemTest, ExactlyOneInputPortConvenience) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(system_.get_input_port(), ".*has 2 inputs.*");
  ...
  // The error message is lame, but doesn't seem worth fixing.
  const_cast<InputPort<double>&>(system_.get_input_port(2))
      .set_deprecation("deprecated");
  DRAKE_EXPECT_THROWS_MESSAGE(system_.get_input_port(), ".*has 0 inputs.*");
}
...
TEST_F(SystemTest, ExactlyOneOutputPortConvenience) {
  ...
  // Three ports (two non-deprecated): fail.
  system_.AddAbstractOutputPort();
  DRAKE_EXPECT_THROWS_MESSAGE(system_.get_output_port(), ".*has 2 outputs.*");
  ...
  const_cast<OutputPort<double>&>(system_.get_output_port(2))
      .set_deprecation("deprecated");
  DRAKE_EXPECT_THROWS_MESSAGE(system_.get_output_port(), ".*has 0 outputs.*");
}

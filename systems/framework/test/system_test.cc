...
TEST_F(SystemTest, TransmogrifyNotSupported) {
  ...
  EXPECT_THROW(System<double>::ToScalarType<drake::AutoDiffXd>(system_),
               std::exception);
  EXPECT_THROW(
      System<double>::ToScalarType<drake::symbolic::Expression>(system_),
      std::exception);
  ...
  EXPECT_THROW(system_.ToScalarType<drake::AutoDiffXd>(), std::exception);
  EXPECT_THROW(system_.ToScalarType<drake::symbolic::Expression>(),
               std::exception);
  ...
  EXPECT_EQ(system_.ToScalarTypeMaybe<drake::AutoDiffXd>(), nullptr);
  EXPECT_EQ(system_.ToScalarTypeMaybe<drake::symbolic::Expression>(), nullptr);

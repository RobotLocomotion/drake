...
class DiscreteValuesLimitMallocFixture :public testing::Test {
 public:
  DiscreteValuesLimitMallocFixture() {
    // Build a first object with some arbitrary data.
    a_.AppendGroup(BasicVector<double>::Make({1.0, 2.0}));
    a_.AppendGroup(BasicVector<double>::Make({3.0, 4.0}));

    // Make a second object with the same "data shape".
    b_.AppendGroup(std::make_unique<BasicVector<double>>(2));
    b_.AppendGroup(std::make_unique<BasicVector<double>>(2));
  }

  DiscreteValues<double> a_;
  DiscreteValues<double> b_;
};

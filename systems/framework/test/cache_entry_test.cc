...
TEST_F(CacheTest, DisableCacheWorks) {
  ...
  EXPECT_EQ(int_val.serial_number(), ser_int + 2);
  EXPECT_EQ(str_val.serial_number(), ser_str + 2);
  EXPECT_EQ(vec_val.serial_number(), ser_vec + 2);
}
...
TEST_F(CacheTest, Copy) {
  ...
  EXPECT_EQ(cache_value(string_index_, &clone_cache).ticket(),
  ...
  cache_value(index2_, &clone_cache)

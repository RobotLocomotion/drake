  ...
  MySystemBase()
      ...
      : entry0_(DeclareCacheEntry("entry0", Alloc3, Calc99)),
        ...
        entry4_(DeclareCacheEntry("entry4",
                                  ...
                                  {time_ticket()})),
        // 6. Use the method that takes just a member calculator
...
class CacheEntryTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    // Next two should be value-initialized.
  ...
  CacheIndex index0_, index1_, index2_, index3_, index4_;
...
TEST_F(CacheEntryTest, Copy) {
  ...
  entry4().EvalAbstract(context_); string_entry().EvalAbstract(context_);
  vector_entry().EvalAbstract(context_);

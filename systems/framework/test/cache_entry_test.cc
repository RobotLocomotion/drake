...
GTEST_TEST(CacheEntryAllocTest, BadAllocGetsCaught) {
  ...
  // right message.
  DRAKE_EXPECT_ERROR_MESSAGE(system.AllocateContext(), std::logic_error,
...
class CacheEntryTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    DRAKE_EXPECT_ERROR_MESSAGE(vector_entry().Get<MyVector3d>(context_),
                               std::logic_error,
...
TEST_F(CacheEntryTest, ValueMethodsWork) {
  ...
  DRAKE_EXPECT_ERROR_MESSAGE(entry0().Get<int>(context_),
  ...
  DRAKE_EXPECT_ERROR_MESSAGE(string_entry().GetAbstract(context_),
                             std::logic_error,
                             ".*GetAbstract().*value out of date.*");
  DRAKE_EXPECT_ERROR_MESSAGE(string_entry().Get<string>(context_),
                             std::logic_error, ".*Get().*value out of date.*");
...
// In Debug builds, try a bad swap and expect it to be caught.
  ...
  DRAKE_EXPECT_ERROR_MESSAGE(entry_value.swap_value(&empty_ptr),
                             std::logic_error,

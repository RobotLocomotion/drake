...
class DiagramContextTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    xc.get_mutable_vector().at(0) = 42.0;
    xc.get_mutable_vector().at(1) = 43.0;

    DiscreteValues<double>& xd = context_->get_mutable_discrete_state();
    xd.get_mutable_vector(0).at(0) = 44.0;

    context_->get_mutable_numeric_parameter(0).at(0) = 76.0;
    context_->get_mutable_numeric_parameter(0).at(1) = 77.0;
...
void VerifyClonedState(const State<double>& clone) {
  ...
  EXPECT_EQ(42.0, xc.get_vector().at(0));
  EXPECT_EQ(43.0, xc.get_vector().at(1));
  // - Discrete
  const DiscreteValues<double>& xd = clone.get_discrete_state();
  EXPECT_EQ(44.0, xd.get_vector(0).at(0));
  // - Abstract
...
void VerifyClonedParameters(const Parameters<double>& params) {
  ...
  EXPECT_EQ(76.0, params.get_numeric_parameter(0).at(0));
  EXPECT_EQ(77.0, params.get_numeric_parameter(0).at(1));
...
TEST_F(DiagramContextTest, MutableEverythingNotifications) {
  ...
  clone->get_mutable_discrete_state(0).at(0) = new_xd;
  clone->get_mutable_abstract_state<int>(0) = new_xa;
  clone->get_mutable_numeric_parameter(0).at(0) = new_pn;
...
TEST_F(DiagramContextTest, State) {
  ...
  ContinuousState<double>& integrator1_xc =
      context_->GetMutableSubsystemContext(SubsystemIndex(3))
          .get_mutable_continuous_state();
  EXPECT_EQ(42.0, integrator0_xc.get_vector().at(0));
  EXPECT_EQ(43.0, integrator1_xc.get_vector().at(0));
  // - Discrete
  DiscreteValues<double>& discrete_xd =
      context_->GetMutableSubsystemContext(SubsystemIndex(4))
          .get_mutable_discrete_state();
  EXPECT_EQ(44.0, discrete_xd.get_vector(0).at(0));

  // Changes to constituent system states appear in the diagram state.
  // - Continuous
  integrator1_xc.get_mutable_vector().at(0) = 1000.0;
  EXPECT_EQ(1000.0, xc.get_vector().at(1));
  // - Discrete
  discrete_xd.get_mutable_vector(0).at(0) = 1001.0;
  EXPECT_EQ(1001.0, xd.get_vector(0).at(0));
}
...
TEST_F(DiagramContextTest, Clone) {
  ...
  // context.
  clone->get_mutable_continuous_state_vector().at(0) = 1024.0;

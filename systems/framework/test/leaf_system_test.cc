...
class TestSystem : public LeafSystem<T> {
 public:
  ...
  const AbstractValue& GetVanillaAbstractParameters(
      const Context<T>& context) const {
    return this->GetAbstractParameter(context, 0 /* index */);
  }

  AbstractValue& GetVanillaMutableAbstractParameters(
      Context<T>* context) const {
    return this->GetMutableAbstractParameter(context, 0 /* index */);
  }
};
...
TEST_F(LeafSystemTest, AbstractParameters) {
  std::unique_ptr<Context<double>> context = system_.CreateDefaultContext();
  const std::string& param = system_.GetVanillaAbstractParameters(*context)
                                 .GetValueOrThrow<std::string>();
  EXPECT_EQ(param, "parameter value");
  std::string& mutable_param =
      system_.GetVanillaMutableAbstractParameters(context.get())
          .GetMutableValueOrThrow<std::string>();
  mutable_param = "modified parameter value";
  EXPECT_EQ("modified parameter value", mutable_param);
}

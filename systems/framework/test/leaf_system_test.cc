...
GTEST_TEST(ImplicitTimeDerivatives, DefaultImplementation) {
  ...
  xdot->SetFromVector(dut.fixed_derivative());
  ...
  dut.CalcImplicitTimeDerivativesResidual(*context, *xdot, &residual);
  EXPECT_EQ(residual, -dut.fixed_derivative());  // Yes, exactly.
  ...
  EXPECT_EQ(residual3, -dut.fixed_derivative());
  ...
  expected_residual.segment(1, 3) = -dut.fixed_derivative();

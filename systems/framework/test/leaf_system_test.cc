...
class DefaultExplicitSystem : public LeafSystem<double> {
 public:
  ...
  const Vector3d kDerivative{1., 2., 3.};
 private:
...
GTEST_TEST(ImplicitTimeDerivatives, DefaultImplementation) {
  ...
  // Proposing zero for the derivative should return a residual of
  // -actual_derivative.
  ...
  EXPECT_EQ(residual, -dut.kDerivative);
  ...
  EXPECT_EQ(residual3, -dut.kDerivative);
  ...
  expected_residual.segment(1, 3) = -dut.kDerivative;

...
TEST_F(ParametersTest, Numeric) {
  ...
  EXPECT_EQ(3.0, params_->get_numeric_parameter(0).at(0));
  EXPECT_EQ(6.0, params_->get_numeric_parameter(0).at(1));
  EXPECT_EQ(9.0, params_->get_numeric_parameter(1).at(0));
  EXPECT_EQ(12.0, params_->get_numeric_parameter(1).at(1));

  params_->get_mutable_numeric_parameter(0).at(1) = 42.0;
  EXPECT_EQ(42.0, params_->get_numeric_parameter(0).at(1));
}
...
TEST_F(ParametersTest, Clone) {
  ...
  EXPECT_EQ(3.0, clone->get_numeric_parameter(0).at(0));
  ...
  // - numeric
  clone->get_mutable_numeric_parameter(0).at(1) = 42.0;
  EXPECT_EQ(6.0, params_->get_numeric_parameter(0).at(1));
  // - abstract
...
TEST_F(ParametersTest, SetSymbolicFromDouble) {
  ...
  EXPECT_EQ("3", p0.at(0).to_string());
  EXPECT_EQ("6", p0.at(1).to_string());
  const auto& p1 = symbolic_params->get_numeric_parameter(1);
  EXPECT_EQ("9", p1.at(0).to_string());
  EXPECT_EQ("12", p1.at(1).to_string());
...
TEST_F(ParametersTest, SetAutodiffFromDouble) {
  ...
  EXPECT_EQ(3.0, p0.at(0).value());
  EXPECT_EQ(6.0, p0.at(1).value());
  const auto& p1 = autodiff_params->get_numeric_parameter(1);
  EXPECT_EQ(9.0, p1.at(0).value());
  EXPECT_EQ(12.0, p1.at(1).value());

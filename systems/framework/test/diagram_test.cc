...
// An internal property has a lifetime that is the union of the builder and the
// resulting diagram.
GTEST_TEST(InternalProperties, Lifetime) {
  std::weak_ptr<int> spy;
  {
    auto do_build = [&]() {
      ...
      spy = thing;
      DiagramBuilder<double> builder;
      internal::get_mutable_properties(&builder).emplace("thing",
                                                         std::move(thing));
      // Thing is living inside builder.
      EXPECT_FALSE(internal::get_mutable_properties(&builder).empty());
      EXPECT_FALSE(spy.expired());
      builder.AddSystem<EmptySystem<double>>();
      auto result = builder.Build();
      // Thing has been transferred to diagram.
      EXPECT_TRUE(internal::get_mutable_properties(&builder).empty());
      EXPECT_FALSE(spy.expired());
      return result;
    };

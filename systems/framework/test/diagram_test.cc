...
GTEST_TEST(DiscreteStateDiagramTest, EvalUniquePeriodicDiscreteUpdate) {
  ...
  DiagramBuilder<double> builder;
  builder.template AddSystem<SystemWithDiscreteState>(7, 5.0);
  builder.template AddSystem<SystemWithDiscreteState>(8, 5.0);
  builder.template AddSystem<SystemWithDiscreteState>(9, 0.0);  // no update
  ...
  // TODO(sherm1) Check that we properly handle the case where the Diagram
  //  also contains a non-periodic Discrete Update event (should be ignored).
}

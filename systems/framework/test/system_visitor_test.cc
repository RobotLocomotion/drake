...
class MyVisitor : public SystemVisitor<double> {
 public:
  void VisitNonDiagram(const System<double>& system) override {
    visited_non_diagrams_.push_back(&system);
  }
  void VisitDiagram(const Diagram<double>& diagram) override {
    visited_diagrams_.push_back(&diagram);
  }

  const Systems& visited_non_diagrams() const { return visited_non_diagrams_; }

  const Systems& visited_diagrams() const { return visited_diagrams_; }

 private:
  Systems visited_non_diagrams_{};
  Systems visited_diagrams_{};
};

void VisitAndCheck(const System<double>& system,
                   const Systems& expected_visited_non_diagrams,
                   const Systems& expected_visited_diagrams) {
  MyVisitor visitor;
  system.Accept(&visitor);
  EXPECT_EQ(visitor.visited_non_diagrams(), expected_visited_non_diagrams);

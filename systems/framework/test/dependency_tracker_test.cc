...
// it subscribers, and that the suppress_notifications flag is copied
...
TEST_F(HandBuiltDependencies, SuppressNotifications) {
  ...
  const DependencyTracker& clone_middle1_ =
      context_.get_dependency_graph().get_tracker(middle1_->ticket());
  ...
  const DependencyTracker& clone_upstream1_ =
      context_.get_dependency_graph().get_tracker(upstream1_->ticket());

...
GTEST_TEST(DiagramBuilderTest, InputPortNamesFanout) {
  ...
  EXPECT_TRUE(builder.ConnectLikewise(
...
// A diagram can use ConnectLikewise.
// Source-->Gain
//       |->Sink
TEST_F(DiagramBuilderSolePortsTest, SourceGainSink2) {
  builder_.Connect(*out1_, *in1out1_);
  EXPECT_TRUE(builder_.ConnectLikewise(
...
// Using ConnectLikewise on a disconnected input is a no-op.
TEST_F(DiagramBuilderSolePortsTest, ConnectLikewiseNothing) {
  EXPECT_FALSE(builder_.ConnectLikewise(
      in1out1_->get_input_port(), in1_->get_input_port()));
  auto diagram = builder_.Build();

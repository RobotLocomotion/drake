...
TEST_F(DiagramBuilderSolePortsTest, DisconnectError) {
  const OutputPort<double>& src = out1_->get_output_port();
  const InputPort<double>& dest = in1_->get_input_port();
  builder_.Connect(src, dest);

  // Wrong `src`.
  const OutputPort<double>& src_wrong = in1out1_->get_output_port();
  DRAKE_EXPECT_THROWS_MESSAGE(builder_.Disconnect(src_wrong, dest),
                              ".*src=y0, dest=in.*not.*connected.*");

  // Wrong `dest`.
  const InputPort<double>& dest_wrong = in1out1_->get_input_port();
  DRAKE_EXPECT_THROWS_MESSAGE(builder_.Disconnect(src, dest_wrong),
                              ".*src=y0, dest=u0.*not.*connected.*");
}

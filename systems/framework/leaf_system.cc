...
std::multimap<int, int> LeafSystem<T>::GetDirectFeedthroughs() const {
  ...
  for (OutputPortIndex o{0}; o < this->num_output_ports(); ++o) {
    ...
    // time, state, parameters, etc -- not any cache entries. In this case
    // there is nothing to do: no feedthrough to add to the return value and
    // no candidates to add to the `unknown` worklist.

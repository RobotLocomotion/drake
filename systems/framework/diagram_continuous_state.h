...
class DiagramContinuousState final: public ContinuousState<T> {
 public:
  ...
  explicit DiagramContinuousState(std::vector<ContinuousState<T>*> substates)
    ...
    DRAKE_ASSERT_VOID(internal::CheckNonNull(substates_));
  }
  ...
  explicit DiagramContinuousState(
    ...
    DRAKE_ASSERT_VOID(internal::CheckNonNull(owned_substates_));
  }
  ...
  std::unique_ptr<DiagramContinuousState> Clone() const {
    return std::unique_ptr<DiagramContinuousState>(DoClone());
  }
  ...
  const ContinuousState<T>& get_substate(int index) const {
    DRAKE_DEMAND(0 <= index && index < get_num_substates());
 ...
 private:
  ...
  DiagramContinuousState* DoClone() const override {
    std::vector<std::unique_ptr<ContinuousState<T>>> owned_states;
    // Make deep copies regardless of whether they were owned.
    for (auto state : substates_)
      owned_states.push_back(state->Clone());
    return new DiagramContinuousState(std::move(owned_states));
  }

...
class DiagramBuilder {
 public:
  ...
  InputPortIndex ExportInput(const InputPort<T>& input,
    ...
    const std::string port_name = name.empty() ?
            // The requirement that subsystem names are unique guarantees
            // uniqueness of the port names.
            input.get_system()->get_name() + ":" + input.get_name()
            : std::move(name);

    input_port_default_names_.push_back(port_name);
 ...
 private:
  ...
  std::unique_ptr<typename Diagram<T>::Blueprint> Compile() {
    ...
    blueprint->input_port_default_names = input_port_default_names_;
  ...
  std::vector<std::string> input_port_default_names_;

...
class CacheEntry {
 public:
  ...
  using AllocCallback
      DRAKE_DEPRECATED("2021-10-01",
          "Use ValueCalcFunction::AllocateCallback instead.")
      = ValueCalcFunction::AllocateCallback;

  /** Signature of a function suitable for calculating a value of a particular
  cache entry, given a place to put the value. */
  using CalcCallback
      DRAKE_DEPRECATED("2021-10-01",
          "Use ValueCalcFunction::CalcCallback instead.")
      = ValueCalcFunction::CalcCallback;
  ...
  The ValueCalcFunction embeds both an allocator and calculator function.
 ...
 private:
  ...
  const ValueCalcFunction value_calc_function_;

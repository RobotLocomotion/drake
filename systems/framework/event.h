 ...
 * writes the updates to a mutable, non-null DiscreteValues object.  @see
...
class DiscreteUpdateEvent final : public Event<T> {
 public:
  ...
  /**@{*/
  /**
   * Callback function that processes a discrete update event. Receives const
   * references to the context and this discrete update event, and a writeable
   * pointer to discrete state.
   */
  ...
  /**
   * Callback function that processes a discrete update event. Receives const
   * references to the system, the context, and this discrete update event, and
   * a writeable pointer to discrete state.
   */
  typedef std::function<void(const System<T>&, const Context<T>&,
  ...
  // the system_callback function `callback`.
 ...
 * event, and writes the updates to a mutable, non-null State object. @see
...
class UnrestrictedUpdateEvent final : public Event<T> {
 public:
  ...
  /**@{*/
  /**
   * Callback function that processes an unrestricted update event. Receives
   * const references to the context and this unrestricted update event, and a
   * writeable pointer to state.
   */
  ...
  /**
   * Callback function that processes an unrestricted update event. Receives
   * const references to the system, the context, and this unrestricted update
   * event, and a writeable pointer to state.
   */
  typedef std::function<void(const System<T>&, const Context<T>&,
                             const UnrestrictedUpdateEvent<T>&, State<T>*)>
      SystemCallback;
  ...
  // Makes an UnrestrictedUpdateEvent with `trigger_type` and system_callback
  // function `system callback`. `system_callback` can be null.
  UnrestrictedUpdateEvent(const TriggerType& trigger_type,
                          const SystemCallback& system_callback)
      : Event<T>(trigger_type), system_callback_(system_callback) {}

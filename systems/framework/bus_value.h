...
name and stored using an AbstactValue.
...
class BusValue final {
 public:
  ...
  The return value is invalidated by a call to any non-const method on this. */
  const AbstractValue* Find(std::string_view name) const;

  /** Removes all signals from this. Invalidates all iterators. */
  void Clear();

  /** Sets one signal value. Invalidates all iterators. */
...
class BusValue::Iterator {
 public:
  ...
  using value_type = std::pair<std::string_view, const AbstractValue&>;

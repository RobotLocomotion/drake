...
class LeafSystem : public System<T> {
 ...
 protected:
  ...
  /// initial values for these variables. If multiple calls are made to
  /// DeclareContinuousState() methods, only the last call has any effect.
  /// Also, these methods have no effect if AllocateContinuousState() is
  ...
  /// variables. Each call to a DeclareDiscreteState() method produces another

...
class LeafSystem : public System<T> {
 ...
 protected:
  ...
  // Clones the model value as provided in DeclareContinuousState() call.
  ...
  // Clones the model values as provided in DeclareDiscreteState() calls.
  std::unique_ptr<DiscreteValues<T>> AllocateDiscreteState() const {
    return model_discrete_state_.Clone();
  }

  // Clones the abstract states declared through DeclareAbstractState() calls.
  ...
  // Clones the model values for all parameters declared via
  // DeclareNumericParameter() and DeclareAbstractParameter() calls.

...
class LeafSystem : public System<T> {
 ...
 protected:
  ...
  size.  @p calc is a calculator function that is a class member function
  ...
  LeafOutputPort<T>& DeclareVectorOutputPort(
      ...
      std::set<DependencyTicket> prerequisites_of_calc = {
          all_sources_ticket()}) {
    return DeclareVectorOutputPort(name, BasicVector<T>(size), calc,
  ...
  and size @p size, using a function for calculating the port's value at
  runtime. Note that this takes the calculator function in its most generic
  form; if you have a member function available use one of the other signatures.
  @see LeafOutputPort::CalcVectorCallback
  ...
  LeafOutputPort<T>& DeclareVectorOutputPort(
      ...
      std::set<DependencyTicket> prerequisites_of_calc = {
          all_sources_ticket()}) {
    return DeclareVectorOutputPort(name, BasicVector<T>(size),
                                   vector_calc_function, prerequisites_of_calc);
  }

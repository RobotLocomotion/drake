    ...
    if (!unknown.empty()) {
      ...
      for (const auto& input_output : orig_unknown) {
        ...
        if (!value.is_out_of_date()) {
          ...
          // result and we can remove it from unknown.  (Alternatively, the
          // System's stated dependencies were inaccurate, but we can't really
          // repair that here.)
          remove_unknown(input_output);
        }
        // Undo the mark_up_to_date(), above.  It shouldn't matter at all on
        // this throwaway context, but perhaps it's best not to leave garbage
        // values marked valid for longer than required.

...
class LeafSystem : public System<T> {
 ...
 protected:
  ...
  /// Methods in the this group declare that this System has a simple,
  /// fixed-period event. The first periodic trigger will occur at
  ...
  /// triggered, or simpler class member functions to be invoked instead.
  ///
  ...
  /// Drake Simulator's StepTo() method takes a substep that advances the
  ...
  /// missing anything. For example, if you want to record in a state variable
  /// the minimum or maximum value achieved so far by some quantity, a per-step
  /// event guarantees you will see it, while a periodic event could miss the
  /// extremum.
  ///
  ...
  ///
  /// Per-step events are issued as follows: First, the Simulator's Initialize()
  ...
  /// @see DeclarePerStepPublishEvent()
  /// @see DeclarePerStepDiscreteUpdateEvent()
  /// @see DeclarePerStepUnrestrictedUpdate()
  ///
  /// See @ref declare_per-step_events "Declare per-step events" for more
  /// information.
  ///
  ...
  /// beginning of the first StepTo() call. On the other hand, initialization

...
void DiagramBuilder<T>::ThrowIfSystemNotRegistered(
  ...
  if (systems_.count(system) == 0) {
    ...
    throw std::logic_error(fmt::format(
        ...
        "DiagramBuilder using AddSystem.\n\nThe systems currently registered "
        "to this builder are: {}.\n\nIf '{}' was registered as a subsystem "
        "to one of these, you must export the input or output port using "
        "ExportInput/ExportOutput and then connect to the exported "
        "port.",

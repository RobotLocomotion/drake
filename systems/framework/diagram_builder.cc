...
void DiagramBuilder<T>::ConnectInput(
    const InputPort<T>& input, const std::string& port_name) {
  DRAKE_THROW_UNLESS(diagram_input_indices_.count(port_name));
  ConnectInput(input, diagram_input_indices_[port_name]);
}

template <typename T>
void DiagramBuilder<T>::ConnectInput(
    const InputPort<T>& input, InputPortIndex port_index) {
  InputPortLocator id{&input.get_system(), input.get_index()};

...
InputPortIndex DiagramBuilder<T>::ExportInput(
  ...
  InputPortIndex result = DeclareInput(input, name);
  ConnectInput(result, input);
  return result;
}
...
void DiagramBuilder<T>::ConnectInput(
    const std::string& port_name, const InputPort<T>& input) {
  DRAKE_THROW_UNLESS(diagram_input_indices_.count(port_name));
  ConnectInput(diagram_input_indices_[port_name], input);
}

template <typename T>
void DiagramBuilder<T>::ConnectInput(
    InputPortIndex port_index, const InputPort<T>& input) {
  InputPortLocator id{&input.get_system(), input.get_index()};
  ThrowIfInputAlreadyWired(id);
  ThrowIfSystemNotRegistered(&input.get_system());
  DRAKE_THROW_UNLESS(port_index < InputPortIndex(diagram_input_data_.size()));

  // Check that port types match.
  const ExportedInputData& data = diagram_input_data_[port_index];

...
SystemBase::GraphvizFragment SystemBase::DoGetGraphvizFragment(
  ...
  const bool split_in_twain = [&params]() {
    auto iter = params.options.find("split_in_twain");
    return (iter != params.options.end()) && (iter->second == "1");
  }();
  ...
  // output ports are on the right (col 1). A nullopt labels indicates that the
  ...
  {
    std::string node_id_input = params.node_id;
    std::string node_id_output = params.node_id;
    if (split_in_twain) {
      node_id_input += "in";
      node_id_output += "out";
    }
  ...
  // Decide how to stylize the table.
  MatrixX<std::string> port_styles(max_num_inputs_outputs, 2);
  // Expand the shorter of the two columns to match the other, longer column.
  ...
  // When using 'split_in_twain' mode, we'll emit two nodes (the input node,
  // then the output node); otherwise we'll emit just one node.
  const int num_nodes = split_in_twain ? 2 : 1;
  for (int node = 0; node < num_nodes; ++node) {
    const std::string& node_id =
        (split_in_twain && node == 0)   ? params.node_id + "in"
        : (split_in_twain && node == 1) ? params.node_id + "out"
                                        : params.node_id;
      ...
      for (int j = 0; j < 2; ++j) {
        if (split_in_twain && j != node) {
          ...
          if (i == 0) {
            result.fragments.push_back(fmt::format(
                R"""(
<TD BORDER="1" STYLE="dotted" COLOR="cadetblue" ROWSPAN="{}">

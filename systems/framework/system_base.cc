...
const AbstractValue* SystemBase::EvalAbstractInputImpl(
  ...
  if (input_ports_[port_index]->get_deprecation().has_value())
    WarnPortDeprecation(true, port_index);
...
void SystemBase::WarnPortDeprecation(bool input, int port_index) const {
  ...
  // We'd like to warn at most onces *per process*; therefore, we have a second

...
@code
  auto my_calc = [](const Context<T>& context, std::string* output) {
...
class ValueProducer final {
 public:
  ...
  † Do not use (2) nor (4) unless SomeOutput is cheap to copy.
  ...
  aliased by the ValueProducer, so must outlive this object.

 ...
 private:
  ...
  std::unique_ptr<typename Diagram<NewType>::Blueprint> ConvertScalarType()
    ...
    for (const InputPortLocator& id : input_port_ids_) {
      ...
      blueprint->input_port_default_names.emplace_back(
          this->get_input_port(port).get_name());
    }
  ...
  struct Blueprint {
    ...
    std::vector<std::string> input_port_default_names;
  ...
  void Initialize(std::unique_ptr<Blueprint> blueprint) {
    ...
    DRAKE_DEMAND(input_port_ids_.size() ==
                 blueprint->input_port_default_names.size());
    std::vector<std::string>::iterator name_iter =
        blueprint->input_port_default_names.begin();
    for (const InputPortLocator& id : input_port_ids_) {

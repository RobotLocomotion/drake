...
class SystemConstraintBounds final {
 public:
  ...
  /// Creates a constraint with the given upper and lower bounds for `x`.  The
  /// type() of this constraint will be kInequality, except in the unusual case
  /// where both lower and upper are all zeros (in which case it is kEquality).
  SystemConstraintBounds(
      const Eigen::Ref<const Eigen::VectorXd>& lower,
      const Eigen::Ref<const Eigen::VectorXd>& upper);

  /// Creates an inequality constraint with the given lower bounds for `x`.
  /// The upper bounds are all positive infinity.
  SystemConstraintBounds(
      const Eigen::Ref<const Eigen::VectorXd>& lower,
      stx::nullopt_t);

  /// Creates an inequality constraint with the given upper bounds for `x`.

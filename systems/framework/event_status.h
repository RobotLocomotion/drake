...
(Advanced) In case you are writing an event dispatcher (that is, overriding
LeafSystem::DoPublish(), LeafSystem::DoCalcDiscreteVariableUpdates(),
or LeafSystem::DoCalcUnrestrictedUpdate(); not recommended), the dispatcher's
return status should be the returned status of highest severity in a series of
event handlers invoked for simultaneous events. In case of multiple returns at
the same severity, the first one should win. Simultaneous event handler
dispatching should only return early (without processing all events), if an
event handler returns EventStatus::kFailed. */
class EventStatus {
 public:
  ...
  /** Returns `true` if the status is Failed. There will also be a message with
  more detail. */

...
class System : public SystemBase {
 public:
  ...
  /// subsystem, from within the Context for a containing Diagram. There are
  /// two common circumstances where this is needed:
  ///
  /// 1. You are given a Diagram and its Context, and have a reference to a
  ///    particular subsystem of that Diagram. You can ask the Diagram to
  ///    extract the subcontext of that subsystem, using GetSubsystemContext()
  ///    or GetMutableSubsystemContext().
  /// 2. You are given the root Context for a complete Diagram (typically by
  ///    the Simulator as part of a generated trajectory). You have a reference
  ///    to a particular subsystem and would like it to retrieve its subcontext
  ///    from within the root Context. Use GetMyContextFromRoot() or
  ...
  /// @pre The given `context` is compatible with `this` %System.
  ...
  /// Returns the const Context for `this` subsystem, given the root
  /// context for the containing Diagram. If `this` %System is already the
  /// top level (root) %System, just returns the root_context. (A root Context
  /// is one that does not have a parent Context.)
  ...
  /// Returns the mutable subsystem context for `this` system, given the root
  /// context for the containing Diagram. If `this` system is already the
  /// top level system, just returns the root_context.
  Context<T>& GetMyMutableContextFromRoot(Context<T>* root_context) const {
    DRAKE_ASSERT(root_context != nullptr);
    // Make use of the const method to avoid code duplication.

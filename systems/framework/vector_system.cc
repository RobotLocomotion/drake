...
template <typename T>
void VectorSystem<T>::DoCalcDiscreteVariableUpdates(
    const Context<T>& context,
    const std::vector<const DiscreteUpdateEvent<T>*>& events,
    DiscreteValues<T>* discrete_state) const {
  for (const DiscreteUpdateEvent<T>* event : events) {
    const EventStatus status = event->handle(*this, context, discrete_state);
    if (status.severity() == EventStatus::Severity::kDidNothing) {
      // Handle any deprecated no-handler events.
      CalcDiscreteUpdate(context, discrete_state);
    }
  }
}

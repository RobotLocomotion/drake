...
const AbstractValue& LeafOutputPort<T>::DoEval(
  ...
  // TODO(sherm1) Remove this kludge asap.
  // Force recalculation of output port value until invalidation propagation
  // is wired up. We'll use the value allocated for the cache entry, but
  // apply Calc() to that value rather than Eval(). The code should read just:
  //   return entry.EvalAbstract(context);
  const AbstractValue& value =
      entry.get_cache_entry_value(context).PeekAbstractValueOrThrow();
  entry.Calc(context, const_cast<AbstractValue*>(&value));
  return value;
  // TODO(sherm1) Remove the above four lines.
}

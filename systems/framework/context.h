...
class Context : public ContextBase {
 public:
  ...
  // Sets the Context time to `time` but notes that this is not the true
  // current time but some small perturbation away from it. The true current
  // time is recorded and propagated. This is used by Simulator::Initialize()
  // to ensure that initialize-time periodic and scheduled events are
  // not missed due to "right now" events.
  ...
  // If time was set with PerturbTime(), returns the true time. If this
  // is empty then the true time is just get_time(). This is used for

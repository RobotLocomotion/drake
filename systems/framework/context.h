...
class Context : public ContextBase {
 public:
  ...
  /// in the context, and "dangerous" methods return a mutable reference. The
  /// dangerous methods contain `get_mutable` or `GetMutable` in their names.
  /// Note that some advanced methods set time (safely) but return a mutable
  /// reference to some parts of the state. In addition the `FixInputPort`
  /// methods return an object that has `get_mutable` methods with the same
  /// dangers (see FixedInputPortValue). Prefer the safe methods when possible.
  ///
  /// <h4>Safe "Set" methods</h4>
  /// The `set` and `Set` methods that don't also contaion `GetMutable` in their

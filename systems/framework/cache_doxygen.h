...
states, input ports, accuracy) and _computed_ values (e.g. derivatives, output
ports) that depend on some or all of the source values. We call the particular
...
values is rigorously tracked via their associated dependency trackers. Computed
...
(called “subscribers”) so that it can perform "out of date" notifications at run
time, and is registered with its prerequisites so that it can be properly
notified when it may be invalid.
...
specifying `{nothing_ticket()}`. The available tickets are defined in the
SystemBase class.
...
method to use in a prerequisite list as shown in the previous section. The
numerical values of these tickets are defined in Drake's
`systems/framework/framework_common.h` header. Ticket methods followed by `(i)`
in the table below are assigned as the indicated resource is allocated; they do
not have pre-assigned ticket numbers.
...
3. Until issue [https://github.com/RobotLocomotion/drake/issues/9171](#9171)
...
The “out of date” boolean is cleared when a new cache entry value is
assigned; it is set by the associated DependencyTracker as a
...
is accessed. The out of date flag operates even when caching is disabled but is
ignored in that case by Eval() methods. The sense of these flags is chosen so
that only a single check that an int is zero is required to know that a value
may be returned without computation.
...
be serialized then deserialized to use later, provided that a compatible System
is available.

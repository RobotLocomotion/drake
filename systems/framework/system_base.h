...
class SystemBase : public internal::SystemMessageInterface {
 ...
 protected:
  ...
  /** Throws std::logic_error because some called API method `func` that is
  only allowed for declared-vector input ports on the abstract port whose
  index is given here. */
  [[noreturn]] void ThrowNotAVectorInputPort(const char* func,
                                             InputPortIndex port_index) const;

  /** Throws std::logic_error because some called API method `func` claiming
  ...
  /** Throws std::logic_error because some called API method `func` that
  requires this input port to be evaluatable, but it was neither freestanding
  nor connected. */

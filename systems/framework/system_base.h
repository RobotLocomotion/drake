...
class SystemBase : public internal::SystemMessageInterface {
 public:
  ...
  flags that are only understood by particular systems, but Drake has a one
  built-in flag that is generally applicable: `"split"`. When set to `"1"`, the
  system will be added as two nodes with all input on one node and all output on
  the other; this is useful for systems that might otherwise cause problematic
  visual cycles.
  ...
  prefix the option name with the subsystem's path, e.g., use "plant/split"="1"
  to set the "split" option on the subsystem named "plant". */

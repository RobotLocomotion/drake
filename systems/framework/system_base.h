...
class SystemBase : public internal::SystemMessageInterface {
 public:
  ...
  - The nodes the graph are systems, and the solid edges are connections between
  ...
  flags that are only understood by particular systems, but Drake has a few
  built-in flags that are generally applicable:
  - "split": When set to "1", the system will be added as two nodes with all
     input on one node and all output on the other; this is useful for systems
     that might otherwise cause problematic visual cycles.
  ...
  prefix the option name with the subsystem's path, e.g., use "plant/split" to
  set the "split" option on the subsystem named "plant". */
  std::string GetGraphvizString(
      std::optional<int> max_depth = {},
      const std::map<std::string, std::string>& options = {}) const;

  /** The return type of GetGraphvizFragment(). */
 ...
 protected:
  ...
  /** The arguments to DoGetGraphvizFragment(). These parameters contitute a
  polite request; a user's %System's implementation DoGetGraphvizFragment() is
  not strictly required to honor any of these parameters, but generally should
  attempt to honor as much as possible. */
 ...
 private:
  ...
  // size-0 ports as documented in GetGraphvizString().

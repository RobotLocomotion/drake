...
/** A functor that calculates from a const ContextBase into a mutable
AbstractValue, that can also pre-allocate storage for the type-erased output.
...
Sugar is provided to bind class member functions that use typed output
arguments, so that the user can ignore the details of type erasure
...
class ValueCalcFunction final {
 public:
  ...
  ValueCalcFunction(
      const SomeInstance* instance,
      SomeOutput (SomeClass::*instance_func)(const SomeContext&) const) {
    ...
    alloc_ = internal::AbstractValueCloner(
        AbstractValue::Make<SomeOutput>(SomeOutput{}));
    calc_ = [typed_instance, instance_func](const ContextBase& context,
      ...
      typed_result = (typed_instance->*instance_func)(typed_context);
    };
  }

...
@param dest_channels Number of destination channels (typically 1, 3, or 4).
 Must not be less than source_channels (i.e., we can pad a channel while
 copying, but we cannot discard channels), and must not be more than 1 channel
 greater than source_channels (i.e., we can pad at most 1 channel).
...
void CopyAndFlipRaw(const T* source, T* dest, int width, int height) {
  static_assert(dest_channels >= source_channels);

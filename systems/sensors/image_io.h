...
The only file formats supported are JPEG, PNG, or TIFF. The file format is
...
- When writing an image to disk, the filename extension will determine the
  format. In this case, the filename extension must be a conventional choice
  (`.jpg`, `.jpeg`, `.png`, `.tif`, or `.tiff`).
- When writing an image to memory, there is no default format; this will throw
  an exception.
...
class ImageIo {
 public:
  ...
  /** (Internal use only) Configures error handling. */
  drake::internal::DiagnosticPolicy& diagnostic() { return diagnostic_; }
 ...
 private:
  ...
  // Helpers.
  struct LoaderTools;
  LoaderTools MakeLoaderTools(
      InputAny input_any,
      const drake::internal::DiagnosticPolicy* diagnostic) const;

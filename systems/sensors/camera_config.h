...
struct CameraConfig {
  ...
  /** @name Renderer properties
   ...
   `renderer_name`) and configured by `renderer_class`. Each RenderEngine
   instance must have a unique name. Rendering will be more efficient if
   multiple cameras share the same RenderEngine instance. To share RenderEngine
   instances, cameras must have _same_ value for `renderer_name`.

   Each camera can also provide the configuration of its supporting
   RenderEngine (`renderer_class`). However, it is an error for two cameras to
   specify the same `renderer_name` but provide _conflicting_ renderer
   configurations. It is the _user's_ responsibility to make sure that for every
   shared `renderer_name` value in a set of %CameraConfig instances that the
   corresponding `renderer_class` values are compatible. How that is achieved
   depends on the medium of specification. While there are multiple possible
   mechanisms, these examples consist of the simplest.

   <h4>Configuring compatible `renderer_class` in YAML</h4>

   The simplest solution in YAML is to use the merge operator (`<<:`) to
   guarantee that multiple cameras use the exact same `renderer_class`. This
   terse example shows how that might be done.
   ...
   `common_renderer`. The merge operator guarantees that `depth_camera` has
   exactly duplicated `base_camera`'s `renderer_class` value. However, it
   tweaks the camera definition by disabling the rgb and label images. Edits
   to the `RenderEngineVtk` parameters in a single location are kept in sync
   across all cameras that are supposed to share.

   <h4>Configuring compatible `renderer_class` in C++</h4>

   The same trick for coordinating multiple %CameraConfig instances in yaml
   will also work in C++. Simply instantiate a single set of RenderEngine
   parameters and assign it to every instance.
   ...
   `renderer_class` value for the first instance and leave it blank in
   subsequent instances which share the same `renderer_name`. It is important to
   configure the RenderEngine on the _first_ appearance of a `renderer_name`
   value.
   ...
   second config instance. It will be compatible in the following cases:
     ...
     - The second config instance's `renderer_class` value is the *name* of the
       type of RenderEngine instance that was already created.
     - The second config instance's `renderer_class` contains the appropriate
       ...
       created _and_ the parameter values match those in the instantiated
       engine.

   Every other `renderer_class` value in the second config instance (e.g.,
   naming a different type of RenderEngine, or specifying different parameters)
   is considered a conflicting specification and will give rise to runtime
   errors. */
  //@{
  ...
  /** The configuration of the camera's supporting RenderEngine.
   ...
   The value can be one of:
      ...
      - The set of parameters for one of those engines: RenderEngineVtkParams,
   ...
   according to the following rules.
      ...
      - Parameter set: a RenderEngine of the type compatible with the parameter
        set will be instantiated, with those parameter values.

   If, however, the `renderer_name` is not unique, as documented
   @ref camera_config_compatible_renderer "above", the values of `renderer_class`
   must be _compatible_ with each other.

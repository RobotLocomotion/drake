...
struct CameraConfig {
  ...
  /** Phase offset (in seconds) for image capture, relative to the simulator's
   ...
   Refer to the the RgbdSensorAsync class for a comprehensive description.
   @pre capture_offset is finite and non-negative.
   @warning This is currently only supported when output_delay is non-zero.
   We intend to remove this restriction in the future. */

...
struct CameraConfig {
  ...
  /** The choice of render engine implementation to use. It can be specified
   simply by providing a `string` containing the _name_ of the RenderEngine
   class to use (i.e., `"RenderEngineVtk"`, `"RenderEngineGl"`, or
   `"RenderEngineGltfClient"`). Or, it can be specified by providing parameters
   for one of those engines: RenderEngineParamsVtk, RenderEngineParamsGl, or
   ...
   4. Use the RenderEngineVtk with a customized clear color (black).
   @code{yaml}
   ...
   Things to note:
     ...
     - When providing a providing the _parameters_ for the engine, the
       declaration must begin with `!` to announce it as a type (examples 2, 3,
       and 4).

...
struct CameraConfig {
  ...
  /** Focal length(s) of the camera. It can be specified in one of several ways:
     - A single scalar which represents the symmetric focal lengths (in pixels).
     - An Anisotropic Anisotropic which specifies the focal lengths (in pixels)
       in the x- and y-directions.
     - An FovDegrees which defines focal length *implicitly* by deriving it from
       field of view (in degrees). If only one fov value is given (in either
       direction), the focal length is computed from that one direction and is
       assumed to be symmetric.
   @pre focal length(s) is a/are finite, positive number(s). */
  std::variant<double, Anisotropic, FovDegrees> focal{FovDegrees{ .y = 45 }};

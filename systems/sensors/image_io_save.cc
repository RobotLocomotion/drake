...
void ImageIo::SaveImpl(ImageAnyConstPtr image_any, OutputAny output_any) const {
  ...
  const ImageFileFormat format = [this, output_any]() {
    if (format_.has_value()) {
      ...
      throw std::logic_error(
          "ImageIo::Save() to a memory buffer requires SetFileFormat() to be "
          "called first, to select the file format.");
    }
  }();
  ...
  visit(
      [&vtk_image](const auto* image) {
        CopyImage(*image, vtk_image.Get());
      },
      image_any);

...
it can run on a background thread. We do that by allocating a standalone Context
and fixing its input port to be a copy of the scene graph FramePoseVector input
port. */
...
EventStatus RgbdSensorAsync::Initialize(const Context<double>& context,
  ...
  // TODO(jwnimmer-tri) Adjust this so we don't get logger warning spam.

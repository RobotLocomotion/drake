...
RgbdSensorAsync::RgbdSensorAsync(const SceneGraph<double>* scene_graph,
  ...
  DRAKE_THROW_UNLESS(defaults_.color_camera.has_value() ||
                     defaults_.depth_camera.has_value());
  DRAKE_THROW_UNLESS(!render_label_image || defaults_.color_camera.has_value());
  ...
  if (defaults_.color_camera.has_value()) {
    DeclareAbstractOutputPort("color_image", &Self::CalcColor, state);
  }
  if (defaults_.depth_camera.has_value()) {
...
double RgbdSensorAsync::default_fps() const {
  return defaults_.fps;
}

void RgbdSensorAsync::set_default_fps(double fps) {
  defaults_.fps = fps;
}

double RgbdSensorAsync::GetFps(const Context<double>& context) const {
  this->ValidateContext(context);
  return context.get_abstract_parameter(parameter_index_)
      .get_value<RgbdSensorAsyncParameters>()
      .fps;
...
void RgbdSensorAsync::SetFps(Context<double>* context, double fps) const {
  this->ValidateContext(context);
  context->get_mutable_abstract_parameter(parameter_index_)
      .template get_mutable_value<RgbdSensorAsyncParameters>()
      .fps = fps;
...
double RgbdSensorAsync::default_capture_offset() const {
  return defaults_.capture_offset;
}

void RgbdSensorAsync::set_default_capture_offset(double capture_offset) {
  defaults_.capture_offset = capture_offset;
}

double RgbdSensorAsync::GetCaptureOffset(const Context<double>& context) const {
  this->ValidateContext(context);
  return context.get_abstract_parameter(parameter_index_)
      .get_value<RgbdSensorAsyncParameters>()
      .capture_offset;
...
void RgbdSensorAsync::SetCaptureOffset(Context<double>* context,
                                       double capture_offset) const {
  this->ValidateContext(context);
  context->get_mutable_abstract_parameter(parameter_index_)
      .template get_mutable_value<RgbdSensorAsyncParameters>()
      .capture_offset = capture_offset;
...
double RgbdSensorAsync::default_output_delay() const {
  return defaults_.output_delay;
}

void RgbdSensorAsync::set_default_output_delay(double output_delay) {
  defaults_.output_delay = output_delay;
}

double RgbdSensorAsync::GetOutputDelay(const Context<double>& context) const {
  this->ValidateContext(context);
  return context.get_abstract_parameter(parameter_index_)
      .get_value<RgbdSensorAsyncParameters>()
      .output_delay;
...
void RgbdSensorAsync::SetOutputDelay(Context<double>* context,
                                     double output_delay) const {
  this->ValidateContext(context);
  context->get_mutable_abstract_parameter(parameter_index_)
      .template get_mutable_value<RgbdSensorAsyncParameters>()
      .output_delay = output_delay;
...
void RgbdSensorAsync::set_default_color_render_camera(
  ...
  DRAKE_THROW_UNLESS(color_camera.has_value() == HasColorCamera());
...
void RgbdSensorAsync::set_default_depth_render_camera(
  ...
  DRAKE_THROW_UNLESS(depth_camera.has_value() == HasDepthCamera());
...
bool RgbdSensorAsync::HasColorCamera() const {
  return HasOutputPort("color_image");
}

bool RgbdSensorAsync::HasDepthCamera() const {
  return HasOutputPort("depth_image_32f");
}
...
EventStatus RgbdSensorAsync::Initialize(const Context<double>& context,
  ...
  std::optional<ColorRenderCamera> color = GetColorRenderCamera(context);
  std::optional<DepthRenderCamera> depth = GetDepthRenderCamera(context);
  if (!HasColorCamera()) {
  ...
  auto sensor = std::make_shared<const SnapshotSensor>(
      scene_graph_, GetParentFrameId(context), GetX_PB(context),
      std::move(*color), std::move(*depth));
  next_state.worker =
      std::make_shared<Worker>(std::move(sensor), HasColorCamera(),
                               HasDepthCamera(), render_label_image_);
  next_state.output = {};
...
namespace {
/* If the size of `image` already matches `camera`, or is 0 in both dimensions,
...
void RgbdSensorAsync::CalcColor(const Context<double>& context,
  ...
  if (std::isfinite(get_state(context).output.time)) {

...
it can run on a background thread. The Worker accomplishes that using two helper
classes: QueryObjectChef and SnapshotSensor, connected in series. The Worker
allocates a standalone Context and fixes the QueryObjectChef's input port(s) to
be a copy of the scene graph's FramePoseVector input port(s). Then those two
helper classes transform the pose vector into a rendered image. */

...
/** An RGB-D camera system that provides RGB, depth, and label images using
 ...
 Output port image formats:
   - color_image: Four channels, uint8_t, in the following order:
     red, green, blue, and alpha.

   - depth_image_32f: One channel, 32-bit float, representing *the Z value in
     `D`* in meters. The values 0 and infinity are reserved for out-of-range
     depth returns (too close or too far, as defined by DepthCameraProperties).

   - depth_image_16u: One channel, 16-bit uint16_t, representing the Z value in
     `D` in *millimeters*. The values 0 and 65535 are reserved for out-of-range
     depth returns. Additionally, due to using uint16_t, the maximum valid
     depth return is 65534mm.

...
// (Internal use only) This file provides helper functions that encapsulate
// VTK's reader and writer classes for the file formats Drake cares about.
...
/* Constructs a VTK image reader for a specific file format, already connected
to read the data from the given memory buffer.
@warning The reader retains a pointer to the `input`, so it can read from it
when requesetd to. The `input` buffer must outlive the returned writer. */

...
const RigidTransformd& RgbdSensor::GetPoseInParent(
...
void RgbdSensor::CalcColorImage(const Context<double>& context,
  ...
  query_object.RenderColorImage(color_camera_, parent_frame_id_,
                                GetPoseInParent(context), color_image);
}

void RgbdSensor::CalcDepthImage32F(const Context<double>& context,
                                   ImageDepth32F* depth_image) const {
  const QueryObject<double>& query_object = get_query_object(context);
  query_object.RenderDepthImage(depth_camera_, parent_frame_id_,
                                GetPoseInParent(context), depth_image);
}
...
void RgbdSensor::CalcLabelImage(const Context<double>& context,
  ...
  const QueryObject<double>& query_object = get_query_object(context);
  query_object.RenderLabelImage(color_camera_, parent_frame_id_,
                                GetPoseInParent(context), label_image);
}
...
void RgbdSensor::CalcX_WB(const Context<double>& context,
  ...
  if (parent_frame_id_ == SceneGraph<double>::world_frame_id()) {
    *X_WB = GetPoseInParent(context);
  } else {
    const QueryObject<double>& query_object = get_query_object(context);
    *X_WB = query_object.GetPoseInWorld(parent_frame_id_) *
            GetPoseInParent(context);
  }
}

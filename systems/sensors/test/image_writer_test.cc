...
class ImageWriterTest : public ::testing::Test {
 public:
  static void SetUpTestCase() {
    ...
    ASSERT_TRUE(ImageWriterTester(ImageWriter(temp_dir_)).can_write());
  }
  ...
  static const std::string& temp_dir() { return temp_dir_; }
...
std::string ImageWriterTest::temp_dir_;
int ImageWriterTest::img_count_ = 0;
...
// Tests the writeability of a png writer based on the validity of the directory
// path.
...
TEST_F(ImageWriterTest, NoWritingBeforeStartTime) {
  ...
  for (int i = 0; i < COUNT; ++i) {
    const double t = start_time / (COUNT + 1) * i;

...
class SimRgbdSensorTest : public ::testing::Test {
 protected:
  void SetUp() override {
    auto [plant, scene_graph] = AddMultibodyPlantSceneGraph(&builder_, 1e-2);
    bodyA_ =
        &plant.AddRigidBody("bodyA", SpatialInertia<double>::MakeUnitary());
    bodyB_ =
        &plant.AddRigidBody("bodyB", SpatialInertia<double>::MakeUnitary());

    X_AF_ = RigidTransformd(Vector3d(-0.5, 0.5, 0.75));
    frame_F_ = &plant.AddFrame(std::make_unique<FixedOffsetFrame<double>>(

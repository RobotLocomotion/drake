...
fs::path GetTempPath(std::string_view filename) {
  return fs::path(temp_directory()) / filename;
}
...
TEST_P(ImageIoParameterizedTest, RoundTrip) {
  ...
  const PixelScalar pixel_scalar = param.pixel_scalar();
  ...
  // Obtain the raw storage for the two images.
  ...
  if (format == ImageFileFormat::kJpeg) {
    ...
    auto* loaded = reinterpret_cast<const uint8_t*>(readback_buffer);
  ...
  if (pixel_scalar == PixelScalar::k8U) {
    auto* orig = reinterpret_cast<const uint8_t*>(original_buffer);
    auto* loaded = reinterpret_cast<const uint8_t*>(readback_buffer);
    ...
    auto* orig = reinterpret_cast<const uint16_t*>(original_buffer);
    auto* loaded = reinterpret_cast<const uint16_t*>(readback_buffer);
    ...
    auto* orig = reinterpret_cast<const float*>(original_buffer);
    auto* loaded = reinterpret_cast<const float*>(readback_buffer);

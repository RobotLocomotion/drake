...
GTEST_TEST(TestAccelerometer, DefaultRotation) {
  ...
  const math::RigidTransform<double> X_BS(Eigen::Vector3d(0, 0, -r_BS));
  const Eigen::Vector3d& gravity = plant.gravity_field().gravity_vector();
  const auto& accelerometer =
      *builder.AddSystem<Accelerometer>(arm_body_index, X_BS, gravity);
...
GTEST_TEST(TestAccelerometer, Rotated) {
  double tol = 10 * std::numeric_limits<double>::epsilon();
  ...
  Eigen::Vector3d g_S(-cos(angle) * gravity(2) - sin(angle) * gravity(0),
                      gravity(1),
                      cos(angle) * gravity(0) - sin(angle) * gravity(2));
  expected_result -= g_S;

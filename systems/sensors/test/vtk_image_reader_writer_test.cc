...
// - format:        the image encoding to test
// - write_to_file: whether to use files or memory buffers
// - wide:          whether to use fat pixels (uint16_t vs uint8_t)
// clang-format off
using ReaderWriterParams = std::tuple<
...
TEST_P(VtkImageReaderWriterParamsTest, RoundTrip) {
  const ImageFileFormat format = std::get<0>(GetParam());
  const bool write_to_file = std::get<1>(GetParam());
  const bool wide = std::get<2>(GetParam());
  ...
  if (write_to_file) {
    filename = fs::path(temp_directory()) /
               fmt::format("FileRoundTrip_{}.image", format);
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Values(  // A tuple of (format, write_to_file, wide).
        std::make_tuple(ImageFileFormat::kJpeg, true, false),
        std::make_tuple(ImageFileFormat::kJpeg, false, false),
        std::make_tuple(ImageFileFormat::kPng, true, false),
        std::make_tuple(ImageFileFormat::kPng, false, false),
        std::make_tuple(ImageFileFormat::kPng, true, true),
        std::make_tuple(ImageFileFormat::kPng, false, true),
        std::make_tuple(ImageFileFormat::kTiff, true, false)

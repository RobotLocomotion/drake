...
struct TestParams {
  ...
  friend std::ostream& operator<<(std::ostream& os, const TestParams& self) {
    os << fmt::format("format = {}; write_to_file = {}; wide = {}", self.format,
                      self.write_to_file, self.wide);
    return os;
  }
};
...
TEST_P(VtkImageReaderWriterTest, RoundTrip) {
  ...
  if (format == ImageFileFormat::kJpeg) {
    ...
    EXPECT_LE(increments.abs().maxCoeff(), max_increment) << increments;
    return;
  }
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Values(
        ...
        TestParams{.format = kPng, .write_to_file = true, .wide = true},
        TestParams{.format = kPng, .write_to_file = false, .wide = true},
        // NOLINTNEXTLINE(whitespace/braces)
        TestParams{.format = kTiff, .write_to_file = true}
        // No "false" case here; VTK TIFF doesn't support writing to memory.
        // We have a expect-throws test immediately below for this case.

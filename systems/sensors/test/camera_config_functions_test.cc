...
TEST_F(CameraConfigFunctionsTest, RenderEngineRequest) {
  ...
  ASSERT_TRUE(scene_graph_->HasRendererOrThrow(default_config.renderer_name,
                                               "RenderEngineVtk"));
  ...
  ASSERT_TRUE(scene_graph_->HasRendererOrThrow(vtk_config.renderer_name,
                                               "RenderEngineVtk"));
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      ".*has a renderer named .+ but it is not of type .+");

  // Using existing name but *no* render engine uses existing engine.
  // TODO(SeanCurtis-TRI): Ran out of time, add this test.
  ...
  if (geometry::render::kHasRenderEngineGl) {
    ...
    ASSERT_TRUE(scene_graph_->HasRendererOrThrow(gl_config.renderer_name,
                                                 "RenderEngineGl"));
  } else {
    DRAKE_EXPECT_THROWS_MESSAGE(
        ApplyCameraConfig(gl_config, plant_, &builder_, scene_graph_, &lcm_),

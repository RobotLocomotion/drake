...
/* Simply make a config where none of the values are defaulted. */
...
GTEST_TEST(CameraConfigTest, NonDefault) {
  const CameraConfig config = MakeConfig();
  const CameraConfig default_config;
...
TEST_F(CameraConfigFunctionsTest, AllParametersCount) {
  ...
  config.rgb = true;  // We just need one image type so that we get a sensor.
  config.depth = true;

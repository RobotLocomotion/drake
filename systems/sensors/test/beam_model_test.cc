...
GTEST_TEST(BeamModelTest, TestProbabilityDensity) {
  ...
  auto w_event = builder.AddSystem<RandomSourced>(RandomDistribution::kUniform,
                                                1, 0.0025);
  builder.Connect(w_event->get_output_port(0),
                  beam_model->get_event_random_input_port());

  auto w_hit = builder.AddSystem<RandomSourced>(RandomDistribution::kGaussian,
                                                1, 0.0025);
  ...
  auto w_uniform =
      builder.AddSystem<RandomSourced>(RandomDistribution::kUniform, 1, 0.0025);

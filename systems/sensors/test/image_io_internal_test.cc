...
GTEST_TEST(ImageIoInternalTest, NeedsSwizzle) {
  ...
  constexpr bool swizzle_bgr = NeedsSwizzle<PixelType::kBgr8U>();
...
GTEST_TEST(ImageIoInternalTest, CopyRaw) {
  ...
  CopyRaw<uint8_t, source_channels, /* swizzle = */ false, dest_channels,
  ...
  CopyRaw<uint8_t, source_channels, /* swizzle = */ true, dest_channels,
          dest_pad>(source.data(), dest.data(), width, height);
  // clang-format off
  EXPECT_THAT(dest, testing::ElementsAre(

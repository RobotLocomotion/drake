...
TEST_F(RgbdSensorAsyncTest, ConstructorAndSimpleAccessors) {
  ...
  EXPECT_EQ(dut.default_fps(), fps);
  EXPECT_EQ(dut.default_capture_offset(), capture_offset);
  EXPECT_EQ(dut.default_output_delay(), output_delay);
...
TEST_F(RgbdSensorAsyncTest, Parameters) {
  ...
  const double other_fps = 2;
  const double other_capture_offset = 0.002;
  const double other_output_delay = 0.100;
  SourceId source_id;
  ...
  dut.set_default_fps(other_fps);
  dut.set_default_capture_offset(other_capture_offset);
  dut.set_default_output_delay(other_output_delay);
  ...
  EXPECT_NE(dut.default_fps(), dut.GetFps(*context));
  EXPECT_NE(dut.default_capture_offset(), dut.GetCaptureOffset(*context));
  EXPECT_NE(dut.default_output_delay(), dut.GetOutputDelay(*context));
  ...
  EXPECT_EQ(dut.default_fps(), dut.GetFps(*other_context));
  EXPECT_EQ(dut.default_capture_offset(), dut.GetCaptureOffset(*other_context));
  EXPECT_EQ(dut.default_output_delay(), dut.GetOutputDelay(*other_context));
  ...
  dut.SetFps(other_context.get(), dut.GetFps(*context));
  dut.SetCaptureOffset(other_context.get(), dut.GetCaptureOffset(*context));
  dut.SetOutputDelay(other_context.get(), dut.GetOutputDelay(*context));
  ...
  EXPECT_FALSE(CompareMatrices(dut.default_X_PB().GetAsMatrix4(),
                               dut.GetX_PB(*other_context).GetAsMatrix4()));
  EXPECT_NE(dut.default_fps(), dut.GetFps(*other_context));
  EXPECT_NE(dut.default_capture_offset(), dut.GetCaptureOffset(*other_context));
  EXPECT_NE(dut.default_output_delay(), dut.GetOutputDelay(*other_context));
  EXPECT_NE(width(dut.default_color_render_camera()),
            width(dut.GetColorRenderCamera(*other_context)));
  EXPECT_NE(width(dut.default_depth_render_camera()),
            width(dut.GetDepthRenderCamera(*other_context)));

...
std::string GetEngineName(const ParamsType&) {
  ...
  if constexpr (std::is_same_v<ParamsType, RenderEngineVtkParams>) {
    ...
    // NOLINTNEXTLINE(readability/braces) The line break confuses things.
  ...
  if (already_exists) {
    std::visit(
        overloaded{
            ...
            [&type_name, &config](auto&& params) -> void {
              const std::string class_name = GetEngineName(params);
              if (LookupEngineType(class_name) != type_name) {
                throw std::logic_error(fmt::format(
                    "Invalid camera configuration; requested renderer_name = "
                    "'{}' with renderer parameters for '{}'. The name is "

...
void ApplyCameraConfig(const CameraConfig& config,
  ...
  if (config.output_delay == 0.0) {
    ...
    auto* sensor = builder->AddSystem<RgbdSensorAsync>(
        scene_graph, body_A_id, X_AB, config.fps, capture_offset,
        config.output_delay,
    ...
    // TODO(jwnimmer-tri) Because concurrent update + publish events are not run
    // concurrently, we need to fudge the publish time here so that the image is
    // actually correct. Once the framework allows for publish events with the
    // timing we want, we should remove this fudge factaor.
    const double lcm_offset = capture_offset + (1.001 * config.output_delay);
    // XXX Actually use the lcm_offset.
    (void)(lcm_offset);
    auto publisher =

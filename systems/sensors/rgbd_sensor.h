...
class RgbdSensor final : public LeafSystem<double> {
 public:
  ...
  /** Constructs an %RgbdSensor with fully specified render camera models for
   both the depth and color cameras. The color camera in inferred from the
   `depth_camera`; it shares the same geometry::render::RenderCameraCore and is
   configured to show the window based on the value of `show_color_window`.

...
/** A sensor similar to %RgbdSensorDiscrete but with distinctly sampled input
and output timing, which allows the rendering to occur on a background thread
...
cameras in the same scene to trigger at different times if desired.
...
rendering. This helps smooth over the runtime latency associated with rendering.
...
resuming the simulation. This hold true for changes to both SceneGraph's model
geometry and the copy of the geometry in a scene graph Context.
...
class RgbdSensorAsync final : public LeafSystem<double> {
 public:
  ...
  @param scene_graph The SceneGraph to use for image rendering. This pointer is
    ...
    object. This system's `geometry_query` input port must be connected to the
    relevant output port of the given `scene_graph`.

...
/** A sensor similar to %RgbdSensor but with discretely sampled input and
output timing, which allows the rendering to occur on a background thread
...
@warning As the moment, this implementation does *not* respond to changes to
geometry shapes, textures, etc. after a simulation has started. The only thing
it repsonds to are changes to geometry poses. If you change anything beyond
poses, you must manually call Simulator::Initialize() to reset things before
resuming the simulation.

@warning The VTK render engine (created by MakeRenderEngineVtk() or
ApplyCameraConfig()) is not thread-safe, so must not be used with this sensor
system. */
class RgbdSensorAsync final : public LeafSystem<double> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(RgbdSensorAsync);

  /** Constructs a sensor with the given parameters.
  @param scene_graph The SceneGraph to use for image rendering. This pointer is
    ...
    object.
  @param parent_id The frame "P" to which the camera base is affixed.
    See RgbdSensor for details.
  @param X_PB The transform relating parent and base.
    See RgbdSensor for details.
  ...
  @param depth_camera The properties for the `depth_image_32f` and
    `depth_image_16u` and output ports.
  ...
  math::RigidTransformd X_PB() const { return X_PB_; }

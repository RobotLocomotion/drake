...
class RgbdSensorAsync final : public LeafSystem<double> {
 public:
  ...
  DRAKE_DEPRECATED("2024-03-01",
                   "X_PB is now a system parameter; use X_PB(context) to query "
                   "the value")
  const math::RigidTransformd& X_PB() const { return X_PB_from_constructor_; }

  /** Returns the `X_PB` parameter value stored in the given `context`. */
  const math::RigidTransformd& X_PB(
      const systems::Context<double>& context) const;

  /** Sets the value for the `X_PB` parameter value into the given `context`. */
  void SetX_PB(systems::Context<double>* context,
 ...
 private:
  ...
  const math::RigidTransformd X_PB_from_constructor_;
  int X_PB_index_{};
  const double fps_;

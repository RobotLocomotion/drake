...
class RgbdSensorAsync final : public LeafSystem<double> {
 public:
  ...
  @throws std::exception when lbel output is not enabled. */
  ...
  capture timestamp of the image outputs. When there are no images available
  on the output ports (e.g., at the beginning of a simulation), the value will
  be NaN. */

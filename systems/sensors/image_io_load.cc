...
struct DRAKE_NO_EXPORT ImageIo::LoaderTools {
  ...
  // The clearinghouse for I/O errors and warning.
  ...
  vtkSmartPointer<vtkImageExport> loader;
};
...
ImageIo::LoaderTools ImageIo::MakeLoaderTools(
  ...
  tools.diagnostic.SetActionForWarnings(
  ...
  tools.reader->Update();
  tools.loader = vtkSmartPointer<vtkImageExport>::New();
  ...
  if (std::optional<PixelScalar> scalar =
          internal::GetDrakeScalarType(tools.loader->GetDataScalarType())) {
    tools.metadata.scalar = *scalar;
...
void CopyVtkToDrakeImage(const LoaderTools& tools, Image<kPixelType>* image) {
  ...
  // Reject unsupported depths.
  if (metadata.depth != 1) {
    tools.diagnostic.Error(fmt::format(
        "Can't load image with depth={}; only depth=1 is supported.",
        metadata.depth));
    return;
  }
  ...
  if constexpr (sizeof(T) > 1) {
    ...
    if (add_alpha) {
      ...
      internal::CopyAndFlipRaw<T, 3, swizzle, 4, 0xFF>(source, dest, width,
                                                       height);
    } else if (drop_alpha) {
...
std::optional<Metadata> ImageIo::LoadMetadataImpl(InputAny input_any) const {
  ...
  if (num_errors > 0 || tools.metadata.channels == 0) {
    return std::nullopt;
  }
...
ImageAny ImageIo::LoadImpl(InputAny input_any) const {
  ...
  if (tools.metadata.channels == 0) {
    tools.diagnostic.Error("Can't parse image header");
    return {};
  }
  const Metadata& metadata = tools.metadata;

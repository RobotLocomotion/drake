...
template <typename T>
bool Simulator<T>::CheckForEventFailureAndMaybeThrow(
...
SimulatorStatus Simulator<T>::AdvanceTo(const T& boundary_time) {
  ...
  while (true) {
    ...
    // Break out of the loop if we've reached a termination condition. Note that
    // this is after step, timed, and witnessed events have been merged in to
    // the event collection and after any publishes. Those "straggler" events
    // can be handled at the end of a simulation with AdvancePendingEvents().
    if (!simulator_status.succeeded() || context_->get_time() >= boundary_time)
      break;
  }

  // We reach here after hitting a termination condition or a failure.

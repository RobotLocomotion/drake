...
Initialize(), with default parameters, can be viewed as a "0ᵗʰ step" that
occurs before the first Step() call as described above. Like Step(),
Initialize() first performs pending updates (in this case only initialization
events can be "pending"). Time doesn't advance so there is no continuous update
phase and witnesses cannot trigger. Finally, again like Step(), the initial
...
events, and periodic or timed publish events that trigger at t₀, followed by a
call to the monitor() function if one has been defined (a monitor is
semantically identical to a per-step publish).
...
class Simulator {
 public:
  ...
  /// satisfied by the initial conditions. In particular, if you suppress
  /// initialization events, then correctness of initial state is your
  /// responsibility.
  ///

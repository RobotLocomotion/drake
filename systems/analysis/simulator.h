...
class Simulator {
 public:
...
#ifndef DRAKE_DOXYGEN_CXX
  // (Internal use only) Make a Simulator which accepts a context via shared
  // pointer.
  //
  // The shared pointer signature is useful for implementing pydrake memory
  // management, because it permits supplying a custom deleter.
  static std::unique_ptr<Simulator<T>> make_with_shared_context(
  ...
  /// current Context is replaced by the new one. This is useful for supplying
  /// a new set of initial conditions. You should invoke Initialize() after
  /// replacing the Context.  @param context The new context, which may be
  /// null. If the context is null, a new context must be set before attempting
  /// to step the system forward.
  ...
  // different one, via shared pointer.  The current Context replaced by the
  // new one.  This is useful for supplying a new set of initial conditions.
  // You should invoke Initialize() after replacing the Context.
  //
  // The shared pointer signature is useful for implementing pydrake memory
  // management, because it permits supplying a custom deleter.
  //

...
this policy:
 - If any unrestricted update event fails, we leave the state unchanged, and
   report failure. We leave unspecified whether the other simultaneous
   unrestricted update events are handled or ignored in this case.
 - Next, if any discrete update event fails, we report failure. In this case
   ...
   unchanged. Other simultaneous discrete events may have been handled or not.
 - Next, if any publish event fails, we _continue_ handling all simultaneous
   publish events, and report failure after they have all been handled. The
   state is returned as updated since publish events can have external
   consequences based on that updated state.
 - A "reached termination" status from any event handler permits further

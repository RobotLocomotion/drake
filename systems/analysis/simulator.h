...
this policy:
 - If any unrestricted update event fails, we stop handling events, leave the
   state unchanged, and report failure.
 - Next, if any discrete update event fails, we stop handling discrete
   events, leave the state as updated by unrestricted events, and report
   failure.
 - Next, if any publish event fails, we _continue_ handling publish events,
   and report failure after they have all been handled.

...
class Simulator {
 public:
...
#ifndef DRAKE_DOXYGEN_CXX
  // (Internal use only) Create a Simulator which accepts a context via shared
  // pointer.
  //
  ...
  Simulator(const System<T>& system, std::shared_ptr<Context<T>> context);
#endif
  ...
  // different one, via shared pointer.  The current Context is deleted.  This

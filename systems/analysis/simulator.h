...
class Simulator {
 ...
 private:
  ...
  // @param requires_owned_system  If true, `owned_system` must not be null. If
  //                               false, `system` mot not be null.
  Simulator(const System<T>* system,
            std::unique_ptr<const System<T>> owned_system,
            std::shared_ptr<Context<T>> context, bool requires_owned_system);

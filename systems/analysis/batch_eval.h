...
evaluation of the dynamics. The return value will a matrix with each column
corresponding to the next state of the system evaluated `time_step` seconds
after the provided time, using the `time_step` that is reported by
...
no input port is selected, then the inputs argument will be ignored.
...
system parameters.
@param times A 1 x N vector of times at which to evaluate the dynamics.
@param states A num_continuous_states x N matrix of continuous states at which

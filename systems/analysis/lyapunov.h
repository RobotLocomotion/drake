...
///
/// @param basis_functions must
/// define an AutoDiffXd function that takes the state vector as an input
/// argument and returns the vector of values of the basis functions at that
/// state.  The Lyapunov function will then have the form
///   Vₚ(x) = ∑ pᵢ φᵢ(x),
...
/// conditions described above.  The resulting Lyapunov funciton is
///   Vₚ(x) = ∑ pᵢ φᵢ(x),
///
// TODO(russt): Consider passing a Context into the basis functions instead of
// just the vector.

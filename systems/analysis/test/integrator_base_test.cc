...
GTEST_TEST(IntegratorBaseTest, Clone) {
  ...
  for (auto& scheme : GetIntegrationSchemes()) {
    ...
    EXPECT_EQ(&integrator->get_context(),
                 &original.get_context());
    EXPECT_EQ(&integrator->get_system(),
                 &original.get_system());
    EXPECT_EQ(integrator->is_initialized(),
    ...
    EXPECT_EQ(integrator->get_stretch_factor(),
                 original.get_stretch_factor());
    ...
    EXPECT_TRUE(CompareMatrices(
        integrator->get_context().get_continuous_state().CopyToVector(),
        xf, 1e-10));

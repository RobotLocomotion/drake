...
class ScalarViewDenseOutputTest : public ::testing::Test {
 protected:
  std::unique_ptr<DenseOutput<T>> CreateDummyDenseOutput() {
    auto dense_output = std::make_unique<HermitianDenseOutput<T>>();
...
TYPED_TEST(ScalarViewDenseOutputTest, ExtensionConsistency) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ScalarViewDenseOutput<TypeParam>(this->CreateDummyDenseOutput(),
                                       this->kInvalidElementIndex),
      ".*out of.*dense output.*range.*");

  // Instantiates scalar continuous extension properly.
  ScalarViewDenseOutput<TypeParam> dense_output(this->CreateDummyDenseOutput(),
                                                this->kValidElementIndex);

  // Retrieves dummy base continuous extension.
  const DenseOutput<TypeParam>* base_output = dense_output.get_base_output();

  // Checks basic getters for consistency.
  EXPECT_EQ(dense_output.is_empty(), base_output->is_empty());
  EXPECT_EQ(dense_output.start_time(), base_output->start_time());
  EXPECT_EQ(dense_output.end_time(), base_output->end_time());
  ...
  EXPECT_EQ(dense_output.EvaluateScalar(this->kMidTime),

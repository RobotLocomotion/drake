...
TYPED_TEST(DiscreteTimeApproxScalarConversionTest, Convertable) {
  using T = TypeParam;
  Integrator<T> sys(1);
  auto discrete_sys = DiscreteTimeApproximation(sys, 0.1);

  if constexpr (std::is_same<T, double>::value) {
    EXPECT_NO_THROW(discrete_sys->ToAutoDiffXd());
    auto converted = discrete_sys->ToAutoDiffXdMaybe();
    EXPECT_NE(converted, nullptr);
  } else {
    EXPECT_NO_THROW(discrete_sys->template ToScalarType<double>());
    auto converted = discrete_sys->template ToScalarTypeMaybe<double>();
    EXPECT_NE(converted, nullptr);
  }
  EXPECT_NO_THROW(discrete_sys->Clone());
}

template <typename T>
class DummySystem : public LeafSystem<T> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(DummySystem);

  DummySystem(int num_input_ports, int num_output_ports,
              const std::multimap<int, int>& direct_feedthroughs) {
    this->DeclareContinuousState(1);

    for (int i = 0; i < num_input_ports; ++i) {
      this->DeclareAbstractInputPort(fmt::format("u{}", i),
...
TYPED_TEST(DiscreteTimeApproxScalarConversionTest, NotConvertable) {
  using T = TypeParam;
  DummySystem<T> sys(0, 0, {});
  auto discrete_sys = DiscreteTimeApproximation(sys, 0.1);

  if constexpr (std::is_same<T, double>::value) {
    EXPECT_ANY_THROW(discrete_sys->ToAutoDiffXd());
    auto converted = discrete_sys->ToAutoDiffXdMaybe();
    EXPECT_EQ(converted, nullptr);
  } else {
    EXPECT_ANY_THROW(discrete_sys->template ToScalarType<double>());
    auto converted = discrete_sys->template ToScalarTypeMaybe<double>();
    EXPECT_EQ(converted, nullptr);
  }
  EXPECT_ANY_THROW(discrete_sys->Clone());
}
...
GTEST_TEST(DiscreteTimeApproxAbstractOutputTest, NoFeedthrough) {
  ...
  //   ────┼ u0 ─   ─ y0 ┼────
  //       └─────────────┘
  DummySystem<double> dummy_sys(1, 1, {});
...
GTEST_TEST(DiscreteTimeApproxAbstractOutputTest, SingleFeedthrough) {
  ...
  //   ────┼ u0 ───── y0 ┼────
  //       └─────────────┘
  DummySystem<double> dummy_sys(1, 1, {{0, 0}});
...
GTEST_TEST(DiscreteTimeApproxAbstractOutputTest, DirectFeedback) {
  ...
  //     │ └──────────────┘ │
  //     └──────────────────┘
  DummySystem<double> dummy_sys(2, 1, {{0, 0}});
...
GTEST_TEST(DiscreteTimeApproxAbstractOutputTest, MultipleDirectFeedback) {
  ...
  //   │ └──────────────────┘ │
  //   └──────────────────────┘
  DummySystem<double> dummy_sys(3, 3, {{0, 0}, {1, 1}, {2, 2}});

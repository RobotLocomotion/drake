...
class OutputTimeStateSystem final : public LeafSystem<T> {
 public:
  ...
  OutputTimeStateSystem() {
    ContinuousStateIndex state_index = this->DeclareContinuousState(1);
...
GTEST_TEST(DiscreteTimeApproxTest, DiscreteUpdateShouldNotChangeTimeState) {
  ...
  // The discrete state have yet changed, so the state output should remain the
  // same as it was originally set.
  EXPECT_EQ(sys->get_output_port(1).Eval(*context)[0], x[0]);

  // This changes the discrete state.

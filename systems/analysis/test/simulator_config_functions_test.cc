...
template <typename T>
class CreateIntegratorFromFlagsTest : public ::testing::Test {};
using DefaultScalars =
    ::testing::Types<double, AutoDiffXd, symbolic::Expression>;
TYPED_TEST_SUITE(CreateIntegratorFromFlagsTest, DefaultScalars);

TYPED_TEST(CreateIntegratorFromFlagsTest, test) {
  using T = TypeParam;
  ...
  for (const auto& [scheme, type_name, support_symbolic] : suites) {
    ...
    if (std::is_same_v<T, symbolic::Expression> && !support_symbolic) {
      DRAKE_EXPECT_THROWS_MESSAGE(
          CreateIntegratorFromFlags(system, config),
          ".+ does not support scalar type " +
              NiceTypeName::Get<symbolic::Expression>());
    } else {
      auto integrator = CreateIntegratorFromFlags(system, config);

...
// Matches RateReport arg with RateReport ref if their values match (with rate
// values considered "matching" within the specified tolerance).
MATCHER_P2(RateReportNear, tolerance, ref, "") {
...
GTEST_TEST(RealtimeRateCalculatorTest, Calculation) {
  ...
  const std::vector<Sample> samples{
    ...
    // relative to zero.
    {1,    123,   RateReport::Initialized(), "Initialization"},
    // "Repeated" invocation is no-op; t_w = 0 equals the reset timer value.
    {1,    0,     RateReport::Empty(), "Repeat sim time"},
    ...
    {3,    1.5,     RateReport::Computed(1, 2 / 1.5), "Past first boundary"},
    ...
    {6,    5,       RateReport::Computed(4, 11 / 12.0), "Four boundaries"},
  ...
  for (const auto& sample : samples) {
    ...
    EXPECT_THAT(report, RateReportNear(1e-14, sample.expected_report));

...
GTEST_TEST(SimulatorTest, SinusoidalHybridSystem) {
  ...
  auto logger = builder.AddSystem<VectorLogSink<double>>(1 /* input size */);
  logger->SetPublishPeriod(h);
...
GTEST_TEST(SimulatorTest, Issue10443) {
  ...
  auto& periodic_logger = *builder.AddSystem<VectorLogSink<double>>(kSize);
  periodic_logger.SetPublishPeriod(1.0 / kFrequency);

...
GTEST_TEST(SimulatorTest, MissedPublishEventIssue13296) {
  ...
  simulator.get_mutable_context().SetTime(.125);
  ...
  EXPECT_EQ(simulator.get_context().get_time(), .125);
  EXPECT_EQ(next_update_time, .125);  // Normal behavior.

  simulator.get_mutable_context().PerturbTime(.0625, .125);
  next_update_time = simulator.get_system().CalcNextUpdateTime
      (simulator.get_context(), events.get());
  EXPECT_EQ(simulator.get_context().get_time(), .0625);
  EXPECT_EQ(next_update_time, .125);  // Return true time, not current time.

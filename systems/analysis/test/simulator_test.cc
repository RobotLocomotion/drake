...
GTEST_TEST(SimulatorTest, MissedEventIssue12620) {
  // After time reaches .25s, this will start returning the given trigger
  // time but with no Event object.
  ...
  Simulator<double> simulator(
      std::make_unique<TriggerTimeButNoEventSystem>(NAN));
  simulator.AdvanceTo(0.125);
  simulator.AdvanceTo(0.25);
  DRAKE_EXPECT_THROWS_MESSAGE(
      simulator.AdvanceTo(0.375), std::exception,
  ...
  Simulator<double> simulator2(
      std::make_unique<TriggerTimeButNoEventSystem>(0.375));
  simulator2.AdvanceTo(0.125);
  simulator2.AdvanceTo(0.25);
  DRAKE_EXPECT_THROWS_MESSAGE(
      simulator2.AdvanceTo(0.375), std::exception,
      ".*CalcNextUpdateTime.*TriggerTimeButNoEventSystem.*MyTriggerSystem.*"
...
GTEST_TEST(SimulatorTest, ForgotToSetTheUpdateTime) {
  // Returns infinity up until t=0.25, after that it fails to set the time.
  class ForgotToSetTimeSystem : public LeafSystem<double> {
   public:
    ForgotToSetTimeSystem() {
      this->set_name("MyForgetfulSystem");
    }
   ...
   private:
    void DoCalcNextUpdateTime(const Context<double>& context,
      ...
      if (context.get_time() < 0.25)
        *next_update_time = std::numeric_limits<double>::infinity();
      // At 0.25 we forget to set the time.
    }
  };

  Simulator<double> simulator(std::make_unique<ForgotToSetTimeSystem>());

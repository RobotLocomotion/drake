...
// Pretend these integrators followed conventional template arguments.
template <typename T> using Radau1Integrator = RadauIntegrator<T, 1>;
template <typename T> using Radau3Integrator = RadauIntegrator<T, 2>;
...
const vector<string>& GetIntegratorSchemes() {
  static const never_destroyed<vector<string>> result{[]() {
    vector<string> names;
    const auto& name_func_pairs = GetSupportedIntegrators<double>();
    for (const auto& [one_name, one_func] : name_func_pairs) {

...
class DiscreteTimeSystem final : public LeafSystem<T> {
 ...
 private:
  ...
  void DiscreteUpdate(const Context<T>& context, DiscreteValues<T>* out) const {
    ...
    // The integrator has just modified the time and the state in the
    ...
    // Otherwise, the resulting continuous context from the next call to
    // `get_uptodate_continuous_context(context)` will not have "up-to-date"
    // time and state matching the discrete `context`.

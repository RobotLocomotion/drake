 ...
 private:
  ...
  void Initialize() {
    ...
    for (int j = 0; j < num_output_ports; ++j) {
      ...
      // The discrete system output port relays to the continuous system output
      // port, so no need to explicit specify the "deprecate" attribute for this
      // discrete system output port.
    }

 ...
 private:
  ...
  void Initialize() {
    ...
    continuous_context_cache_entry_ = &this->DeclareCacheEntry(
        ...
        &DiscreteTimeSystem<T>::CopyAllContext,
  ...
  void CopyAllContext(const Context<T>& from_discrete_context,
                      Context<T>* to_continuous_context) const {
    CopyAllContextExceptInput(from_discrete_context, to_continuous_context);
    // Copy fixed input port values.
    for (int i = 0; i < continuous_system_->num_input_ports(); ++i) {
      const AbstractValue* value_ptr =
          this->EvalAbstractInput(from_discrete_context, i);
      if (value_ptr != nullptr) {
        to_continuous_context->FixInputPort(i, *value_ptr);
      }
    }
  }

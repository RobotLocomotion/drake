...
class DiscreteTimeSystem final : public LeafSystem<T> {
 ...
 private:
  ...
  void DiscreteUpdate(const Context<T>& context, DiscreteValues<T>* out) const {
    ...
    // The integrator have just modified the time and the state in the
    // continuous context. We need to mark them as out-of-date.
...
std::unique_ptr<System<T>> DiscreteTimeApproximation(
  ...
  return std::make_unique<DiscreteTimeSystem<T>>(
      std::move(system), time_period, time_offset, integrator_config);
}
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((
    static_cast<std::unique_ptr<System<T>> (*)(
        std::shared_ptr<const System<T>>, double, double,
        const SimulatorConfig&)>(&DiscreteTimeApproximation<T>),

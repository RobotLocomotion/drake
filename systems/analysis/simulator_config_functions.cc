...
// Returns (scheme, functor) pair to implement configure for this `Integrator`.
...
// The functor returned value is nullptr if the scheme does not support the
// scalar type T.
template <typename T, template <typename> class Integrator>
NamedConfigureIntegratorFunc<T> MakeResetter() {
  ...
  return NamedConfigureIntegratorFunc<T>(
      ...
      [](Simulator<T>* simulator, const System<T>& system,
        ...
        if constexpr (std::is_same_v<T, symbolic::Expression> &&
                      !is_fixed_step) {
          return (simulator != nullptr) ? nullptr
                                        : std::unique_ptr<IntegratorBase<T>>();
        }
        // Returns the integrator owned by the simulator if simulator is
        // non-null.
        ...
        } else {  // Returns a new integrator if the simulator is null.
          if constexpr (is_fixed_step) {
            auto result =

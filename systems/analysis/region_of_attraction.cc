...
Expression FixedLyapunovConvex(const solvers::VectorXIndeterminate& x,
  ...
  std::cout << V.indeterminates() << std::endl;
  std::cout << (V - Polynomial(rho)).indeterminates() << std::endl;
  ...
  prog.AddSosConstraint(
      ((V - Polynomial(rho)) * Polynomial(pow((x.transpose() * x)[0], d)) -
       lambda * Vdot)
          .ToExpression());
...
Expression RegionOfAttraction(const System<double>& system,
  ...
  if (user_provided_lyapunov_candidate) {
    ...
    const auto A = Evaluate(Jacobian(f, x_bar), x0env);
  ...
  V = FixedLyapunovConvex(x_bar,
                          Polynomial(V, Variables(prog.indeterminates())),
                          Polynomial(Vdot, Variables(prog.indeterminates())));

...
// Assumes V>0 and the Hessian of Vdot<0 at the origin.
// Use Vdot = 0 => V >= rho (or x=0) via
//   maximize   rho
//   subject to (V-rho)*x'*x - Lambda*Vdot is SOS.
symbolic::Expression FixedLyapunovConvex(const solvers::VectorXIndeterminate& x,
                                         const Expression& V,
  ...
  const int V_degree = Polynomial(V).TotalDegree();
  const int Vdot_degree = Polynomial(Vdot).TotalDegree();
  const int lambda_degree = Vdot_degree;
  const auto lambda =
      prog.NewFreePolynomial(Variables(x), lambda_degree).ToExpression();
  ...
  prog.AddSosConstraint((V - rho) * pow((x.transpose() * x)[0], d) -
                        lambda * Vdot);
...
Expression RegionOfAttraction(const System<double>& system,
  ...
  MathematicalProgram prog;
  const auto x = prog.NewIndeterminates(num_states, "x");

  Environment x0env;
  for (int i = 0; i < num_states; i++) {
    x0env.insert(x(i), x0(i));
  }

  // Evaluate the dynamics (in relative coordinates).
  symbolic_context->SetContinuousState(x0 + x);
  const auto f = symbolic_system->EvalTimeDerivatives(*symbolic_context)
                     .get_vector()
                     .CopyToVector();

  Expression V = options.lyapunov_candidate;

  if (V.EqualTo(Expression::Zero())) {
    // Then the caller did NOT pass in a candidate Lyapunov function.
    // Solve a Lyapunov equation to find a candidate.
    const auto A = Evaluate(Jacobian(f, x), x0env);
    const auto Q = Eigen::MatrixXd::Identity(num_states, num_states);
    const auto P = math::RealContinuousLyapunovEquation(A, Q);
    V = x.dot(P * x);
  } else {
    // The caller did pass in a candidate Lyapunov function.
    // If necessary, replace the state variables.
    if (options.state_variables.rows() > 0) {
      Substitution subs;
      subs.reserve(num_states);
      for (int i = 0; i < num_states; i++) {
        subs.emplace(options.state_variables(i), x(i));
      }
      V = V.Substitute(subs);
    }
  ...
  const auto Vdot = V.Jacobian(x).dot(f);

  if (!options.lyapunov_candidate.EqualTo(Expression::Zero())) {
    // Check that the Hessian of Vdot is negative definite.
    const Eigen::MatrixXd H =
        symbolic::Evaluate(symbolic::Jacobian(Vdot.Jacobian(x), x), x0env);
  ...
  V = FixedLyapunovConvex(x, V, Vdot);
  ...
  if (options.state_variables.rows() > 0) {
    for (int i = 0; i < num_states; i++) {
      subs.emplace(x(i), options.state_variables(i) - x0(i));
    }
  } else {
    for (int i = 0; i < num_states; i++) {
      subs.emplace(x(i), x(i) - x0(i));
    }
  }
  V = V.Substitute(subs);

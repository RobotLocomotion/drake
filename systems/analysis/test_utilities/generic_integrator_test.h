...
template <class T>
class TrivialIntegratorTest : public ::testing::Test {
 protected:
  void SetUp() {
...
TYPED_TEST_SUITE_P(TrivialIntegratorTest);

// Confirms that integration supports fixed steps on systems with no continuous
// state. Uses an empty plant.
TYPED_TEST_P(TrivialIntegratorTest, TrivialFixedStepEmptyPlant) {
...
TYPED_TEST_P(TrivialIntegratorTest, TrivialErrorControlledStepEmptyPlant) {
  ASSERT_EQ(this->context_->get_continuous_state().size(), 0);
...
// IntegratorTestFactory<RungeKutta3Integrator<double>>).
template <class T>
class GenericIntegratorTest : public TrivialIntegratorTest<T> {
 protected:

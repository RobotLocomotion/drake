...
template <typename T>
SpringMassSystem<T>::SpringMassSystem(SystemScalarConverter converter,
  ...
  this->DeclareContinuousState(SpringMassStateVector<T>(), 1 /* num_q */,
                               1 /* num_v */, 1 /* num_z */);
}
...
SpringMassSystem<T>::SpringMassSystem(double spring_constant_N_per_m,
                                      double mass_kg, bool system_is_forced)
    : SpringMassSystem(SystemTypeTag<SpringMassSystem>{},
                       spring_constant_N_per_m, mass_kg, system_is_forced) {}

template <typename T>
template <typename U>
SpringMassSystem<T>::SpringMassSystem(const SpringMassSystem<U>& other)
    : SpringMassSystem(other.get_spring_constant(), other.get_mass(),
...
T SpringMassSystem<T>::DoCalcPotentialEnergy(const Context<T>& context) const {
  const double k = spring_constant_N_per_m_;
  const T& x = get_position(context);
  const T& x0 = 0.0;  // TODO(david-german-tri) should be a parameter.
  const T& stretch = x - x0;
  const T& pe = k * stretch * stretch / 2;
  return pe;
}
...
void SpringMassSystem<T>::DoCalcTimeDerivatives(
  ...
  // the mass over time.
  derivative_vector.set_conservative_work(this->CalcConservativePower(context));
}

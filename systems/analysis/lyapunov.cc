...
Eigen::VectorXd SampleBasedLyapunovAnalysis(
  ...
  const VectorXd phi0 =
      math::ExtractValueMatrixFromAutoDiff(basis_functions(V_zero_state));
  ...
  for (int si = 0; si < num_samples; si++) {
    ...
    math::InitializeAutoDiffFromValueMatrix(state, &autodiff_state, state_size);
    const VectorX<AutoDiffXd> phi = basis_functions(autodiff_state);
    const Expression V = params.dot(math::ExtractValueMatrixFromAutoDiff(phi));

    context_state.SetFromVector(state);
    system.CalcTimeDerivatives(*my_context, derivatives.get());

    const Eigen::VectorXd phidot =
        math::ExtractGradientMatrixFromAutoDiff(phi) *

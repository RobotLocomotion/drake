...
RateReport RateReport::Initialized() {
  return {.initialized = true};
}

RateReport RateReport::Empty() {
  return {.initialized = false, .period_count = 0};
}

RateReport RateReport::Computed(int period_count, double rate) {
...
RealtimeRateCalculator::RateReport RealtimeRateCalculator::UpdateAndRecalculate(
  ...
  return RateReport::Computed(period_count, rate);
}

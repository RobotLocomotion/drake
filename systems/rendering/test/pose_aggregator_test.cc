...
PoseBundle<AutoDiffXd> ToAutoDiffXd(const PoseBundle<double>& original) {
  ...
  for (int i = 0; i < original.get_num_poses(); i++) {
    RigidTransform<AutoDiffXd> pose(original.transform(i).cast<AutoDiffXd>());
...
TEST_F(PoseAggregatorTest, CompositeAggregation) {
  ...
  const RigidTransformd& generic_pose_0 = bundle.transform(0);
  ...
  const RigidTransformd& generic_pose_1 = bundle.transform(1);
  ...
  const RigidTransformd& xv_pose = bundle.transform(2);
  ...
  const RigidTransformd& x_pose = bundle.transform(3);
  EXPECT_TRUE(CompareMatrices(
  ...
  for (int i = 0; i < bundle.get_num_poses(); i++) {
    EXPECT_TRUE(CompareMatrices(
        bundle.transform(i).matrix(),
        math::autoDiffToValueMatrix(autodiff_bundle.transform(i).matrix())));
    EXPECT_TRUE(

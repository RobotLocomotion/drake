  ...
  methods  % for constructing and manipulating polynomial systems
    ...
    function sys = feedback(sys1,sys2)
      ...
      sys = SpotPolynomialSystem(sys1.getNumContStates()+sys2.getNumContStates(),...
        ...
        sys1.getNumInputs(), sys1.getNumOutputs(), sys1.isDirectFeedthrough, true);
      ...
      if (sys1.getNumContStates()>0)
        sys.p_dynamics_rhs=[sys.p_dynamics_rhs;subss(p1_dynamics_rhs,[sys1.getStateFrame.poly;p_u],[p_x1;p_y2+p_u])];
      end
      if (sys2.getNumContStates()>0)
        sys.p_dynamics_rhs=[sys.p_dynamics_rhs;subss(p2_dynamics_rhs,[sys2.getStateFrame.poly;sys2.getInputFrame.poly],[p_x2;p_y1])];
      end
      if (sys1.getNumDiscStates()>0)
        sys.p_update = [sys.p_update;subss(p1_update,[sys1.getStateFrame.poly;p_u],[p_x1;p_y2+p_u])];
      end
      ...
      if (sys1.getNumStateConstraints()>0)
        sys.p_state_constraints=[p_state_constraints;subss(p1_state_constraints,sys1.getStateFrame.poly,p_x1)];
      end
      if (sys2.getNumStateConstraints()>0)
        sys.p_state_constraints=[p_state_constraints;subss(p2_state_constraints,sys2.getStateFrame.poly,p_x2)];
      end
      ...
      if isRational(sys1)
        ...
        if isRational(sys2)
          sys.p_dynamics_lhs=blkdiag(eye(getNumContStates(sys1)),subss(sys2.p_dynamics_rhs,sys2.getStateFrame.poly,p_x2));
        else
          sys.p_dynamics_lhs=[];
        end
      end

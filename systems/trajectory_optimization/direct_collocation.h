...
class DirectCollocation : public MultipleShooting {
 public:
  ...
  /// cannot impact the dynamics of the continuous states.
...
class DirectCollocationConstraint : public solvers::Constraint {
 ...
 private:
  ...
  const std::unique_ptr<System<AutoDiffXd>> system_{nullptr};
  std::unique_ptr<Context<AutoDiffXd>> context_{nullptr};
  const InputPort<AutoDiffXd>* input_port_{nullptr};
  FixedInputPortValue* input_port_value_{nullptr};
  std::unique_ptr<ContinuousState<AutoDiffXd>> derivatives_{nullptr};

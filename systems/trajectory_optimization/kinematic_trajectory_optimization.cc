...
class WrappedVelocityConstraint : public Constraint {
 public:
  WrappedVelocityConstraint(std::shared_ptr<Constraint> wrapped_constraint,
                            const Eigen::MatrixXd& M_pos,
                            const Eigen::MatrixXd& M_vel)
...
void KinematicTrajectoryOptimization::AddVelocityConstraintAtScaledTime(
  ...
  auto con =
      std::make_shared<WrappedVelocityConstraint>(constraint, M_pos, M_vel);

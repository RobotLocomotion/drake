...
GTEST_TEST(PathDeprecation, DirectCollocationAndConstraint) {
  ...
  EXPECT_NO_THROW(DirectCollocationConstraint(plant, *context));

  EXPECT_TRUE(std::is_function_v<decltype(AddDirectCollocationConstraint)>);
}

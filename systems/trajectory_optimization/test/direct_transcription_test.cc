...
GTEST_TEST(DirectTranscriptionTest, DeprecatedDiscreteTimeConstraintTest) {
...
// system.
GTEST_TEST(DirectTranscriptionTest, DeprecatedContinuousTimeConstraintTest) {
...
GTEST_TEST(DirectTranscriptionTest,
           DeprecatedDiscreteTimeSymbolicConstraintTest) {
...
// system.
GTEST_TEST(DirectTranscriptionTest,
           DeprecatedContinuousTimeSymbolicConstraintTest) {
...
void DeprecatedSolvePendulumTrajectory(bool continuous_time) {
...
GTEST_TEST(DirectTranscriptionTest, DeprecatedMultibodyDiscreteTest) {
  DeprecatedSolvePendulumTrajectory(false);
}

GTEST_TEST(DirectTranscriptionTest, DeprecatedMultibodyContinuousTest) {
  DeprecatedSolvePendulumTrajectory(true);
}

GTEST_TEST(DirectTranscriptionTest, DeprecatedDiscreteTimeLinearSystemTest) {
  Eigen::Matrix2d A, B;
  // clang-format off
  A << 1, 2,
      3, 4;
...
// constructor.
GTEST_TEST(DirectTranscriptionTest, DeprecatedTimeVaryingLinearSystemTest) {
...
GTEST_TEST(DirectTranscriptionTest, DeprecatedAddRunningCostTest) {
  const double kTimeStep = 0.1;
  std::unique_ptr<AffineSystem<double>> system = MakeAffineSystem(kTimeStep);
...
GTEST_TEST(DirectTranscriptionTest, DeprecatedLinearSystemWParamsTest) {
...
GTEST_TEST(DirectTranscriptionTest, AddRunningCostTest) {
  ...
  const solvers::MathematicalProgramResult result = Solve(dirtran);
  EXPECT_TRUE(result.is_success());

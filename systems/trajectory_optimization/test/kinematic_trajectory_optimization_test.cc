...
TEST_F(KinematicTrajectoryOptimizationTest, AddVelocityConstraintAtScaledTime) {
  EXPECT_EQ(trajopt_.prog().generic_constraints().size(), 0);
  Vector6d x_desired;
  ...
  trajopt_.AddVelocityConstraintAtScaledTime(
...
TEST_F(KinematicTrajectoryOptimizationTest, AddVelocityBounds) {
  ...
  for (double t = 0; t <= 1.0; t += 0.01) {
    EXPECT_GE(qdot->value(t).minCoeff(),
...
TEST_F(KinematicTrajectoryOptimizationTest, AddAccelerationBounds) {
  ...
  auto qdot = q.MakeDerivative();
  auto qddot = qdot->MakeDerivative();
  EXPECT_LT(qddot->value(0.0).minCoeff(), -1.0);  // Some value is < -1.0.

  EXPECT_EQ(trajopt_.prog().generic_constraints().size(), 0);
  trajopt_.AddAccelerationBounds(-Vector3d::Ones(),
                                 Vector3d::Ones());
  ...
  for (double t = 0; t <= 1.0; t += 0.01) {
    EXPECT_GE(qddot->value(t).minCoeff(),
              -1.0 - kTol);  // All values are >= -1.0.
  }
}
...
TEST_F(KinematicTrajectoryOptimizationTest, AddJerkBounds) {
  ...
  EXPECT_LT(qdddot->value(0.0).minCoeff(), -1.0);  // Some value is < -1.0.
  ...
  const double kTol = 1e-5;
  for (double t = 0; t <= 1.0; t += 0.01) {
    EXPECT_GE(qdddot->value(t).minCoeff(),
              -1.0 - kTol);  // All values are >= -1.0.
  }
}

...
GTEST_TEST(DirectCollocation, InputPortSelection) {
  ...
  prog.AddEqualTimeIntervalsConstraints();
  const double torque_limit = 3.0;  // N*m.
  const auto& u = prog.input();
  prog.AddConstraintToAllKnotPoints(-torque_limit <= u[0]);
  prog.AddConstraintToAllKnotPoints(u[0] <= torque_limit);

  Eigen::Vector2d initial_state = Eigen::Vector2d::Zero();
  Eigen::Vector2d final_state{M_PI, 0.0};
...
// following is a simple example of finding the passive limit cycle with
// direct collocation; this was a motivating example for supporting
...
GTEST_TEST(DirectCollocation, IgnoreNonContinuousState) {
  ...
  prog.AddConstraint(prog.initial_state()[1] ==
                     prog.final_state()[1] * cos(2 * alpha));

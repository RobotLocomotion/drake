...
GTEST_TEST(DirectCollocation, DeprecatedTestAddRunningCost) {
...
GTEST_TEST(DirectCollocation, DeprecatedTestCollocationConstraint) {
  const std::unique_ptr<LinearSystem<double>> system = MakeSimpleLinearSystem();
...
GTEST_TEST(DirectCollocation, DeprecatedTestReconstruction) {
  const std::unique_ptr<LinearSystem<double>> system = MakeSimpleLinearSystem();
  const auto context = system->CreateDefaultContext();
...
GTEST_TEST(DirectCollocation, DeprecatedDoubleIntegratorTest) {
...
// time.
GTEST_TEST(DirectCollocation, DeprecatedMinimumTimeTest) {
  const auto double_integrator = MakeDoubleIntegrator();
  auto context = double_integrator->CreateDefaultContext();
  const int kNumSampleTimes{10};
...
GTEST_TEST(DirectCollocation, DeprecatedNoInputs) {
  // xdot = -x.
...
GTEST_TEST(DirectCollocation, DeprecatedAddDirectCollocationConstraint) {
  const auto double_integrator = MakeDoubleIntegrator();
  auto context = double_integrator->CreateDefaultContext();
...
GTEST_TEST(DirectCollocation, DeprecatedInputPortSelection) {
...
// states.
GTEST_TEST(DirectCollocation, DeprecatedIgnoreNonContinuousState) {
...
GTEST_TEST(DirectCollocation, DoubleIntegratorTest) {
  ...
  const solvers::MathematicalProgramResult result = Solve(dircol);
  EXPECT_TRUE(result.is_success());
...
GTEST_TEST(DirectCollocation, InputPortSelection) {
  ...
  const auto result = Solve(dircol);
  EXPECT_TRUE(result.is_success());
}

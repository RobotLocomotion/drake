...
GTEST_TEST(MultipleShootingTest, DeprecatedFixedTimestepTest) {
...
GTEST_TEST(MultipleShootingTest,
           DeprecatedFixedTimestepTestWithPlaceholderVariables) {
...
GTEST_TEST(MultipleShootingTest,
           DeprecatedVariableTimestepTestWithPlaceholderVariables) {
  const VectorX<symbolic::Variable> input{
      symbolic::MakeVectorContinuousVariable(3, "my_input")};
  const VectorX<symbolic::Variable> state{
      symbolic::MakeVectorContinuousVariable(4, "my_state")};
...
GTEST_TEST(MultipleShootingTest, DeprecatedVariableTimestepTest) {
  const int kNumInputs{1};
  const int kNumStates{2};
  const int kNumSampleTimes{2};
  const double kMinTimeStep{0.1};
...
GTEST_TEST(MultipleShootingTest, DeprecatedPlaceholderVariableTest) {
  const int kNumInputs{1};
  const int kNumStates{2};
  const int kNumSampleTimes{2};
...
GTEST_TEST(MultipleShootingTest, DeprecatedPlaceholderVariableNames) {
  const int kNumInputs{1};
  const int kNumStates{2};
  const int kNumSampleTimes{2};
  const double kFixedTimeStep{0.1};
...
GTEST_TEST(MultipleShootingTest, DeprecatedTimeIntervalBoundsTest) {
  const int kNumInputs{1};
...
GTEST_TEST(MultipleShootingTest, DeprecatedEqualTimeIntervalsTest) {
  const int kNumInputs{1};
  const int kNumStates{2};
  const int kNumSampleTimes{3};
  const double kMinTimeStep{0.01};
...
GTEST_TEST(MultipleShootingTest, DeprecatedDurationConstraintTest) {
...
GTEST_TEST(MultipleShootingTest, DeprecatedConstraintAllKnotsTest) {
  const int kNumInputs{1};
  const int kNumStates{2};
  const int kNumSampleTimes{3};
  const double kMinTimeStep{0.01};
...
GTEST_TEST(MultipleShootingTest, DeprecatedFinalCostTest) {
  const int kNumInputs{1};
  const int kNumStates{2};
  const int kNumSampleTimes{2};
  const double kFixedTimeStep{0.1};
...
GTEST_TEST(MultipleShootingTest, DeprecatedTrajectoryCallbackTest) {
  const int kNumInputs{1};
  const int kNumStates{2};
  const int kNumSampleTimes{3};
  const double kFixedTimeStep{0.1};
...
GTEST_TEST(MultipleShootingTest, DeprecatedInitialGuessTest) {
  const int kNumInputs{1};
  const int kNumStates{1};
  const int kNumSampleTimes{3};
  const double kMinTimeStep{0.01};
...
GTEST_TEST(MultipleShootingTest, DeprecatedResultSamplesTest) {
  const int kNumInputs{1};
  const int kNumStates{2};
  const int kNumSampleTimes{2};
  const double kFixedTimeStep{0.1};
...
GTEST_TEST(MultipleShootingTest, DeprecatedNewSequentialVariableTest) {
  const int kNumInputs{1};
  const int kNumStates{2};
  const int kNumSampleTimes{3};
  const double kMinTimeStep{0.01};
...
GTEST_TEST(MultipleShootingTest, VariableTimestepTestWithPlaceholderVariables) {
  ...
  const solvers::MathematicalProgramResult result = Solve(trajopt);
  ASSERT_TRUE(result.is_success());
  const Eigen::VectorXd times = trajopt.GetSampleTimes(result);
  EXPECT_EQ(times.size(), 2);
  EXPECT_NEAR(times[0], 0.0, kSolverTolerance);
...
GTEST_TEST(MultipleShootingTest, EqualTimeIntervalsTest) {
  ...
  const solvers::MathematicalProgramResult result =
      Solve(trajopt, prog.initial_guess());
  ASSERT_TRUE(result.is_success());
...
GTEST_TEST(MultipleShootingTest, ConstraintAllKnotsTest) {
  ...
  solvers::MathematicalProgramResult result = Solve(trajopt);
  ASSERT_TRUE(result.is_success());
  for (int i = 0; i < kNumSampleTimes; i++) {
    // osqp can fail in polishing step, such that the accuracy cannot reach
    // 1E-6.

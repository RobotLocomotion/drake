...
TEST_F(SimIiwaDriverTest, AddSimIiwaDriver) {
  for (const auto& mode :
       {IiwaControlMode::kPositionOnly, IiwaControlMode::kTorqueOnly,
        IiwaControlMode::kPositionAndTorque}) {
    SCOPED_TRACE(fmt::format("mode = {}", static_cast<int>(mode)));
    ...
    const System<double>* const dut = &SimIiwaDriver<double>::AddSimIiwaDriver(
        *sim_plant, iiwa_instance_, *controller_plant_, &builder,

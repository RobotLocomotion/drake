...
class SimIiwaDriverTest : public ::testing::Test {
 ...
 protected:
  void SetUp() {
    ...
    // Check the disturbance. CompareMatrices() is mostly here for fancy
    // failure messages.
    VectorXd iota = VectorXd::LinSpaced(7, 0, 6);
    ASSERT_FALSE(
        CompareMatrices(iota, sim_plant_->MakeActuationMatrix() * iota));
    ASSERT_FALSE(CompareMatrices(
        iota, sim_plant_->MakeActuationMatrixPseudoinverse() * iota));
  ...
  void TestSimIiwaDriverPorts(IiwaControlMode mode, const System<double>& dut) {
    ...
    VectorXd test_input(7);
    test_input << 0, 0, 0, 0.01, 0, 0, 0.02;
    ...
    if (position_enabled(mode)) {
      context.FixInputPort(dut.GetInputPort("position").get_index(),
                           Value<BasicVector<double>>(test_input));
    }
    if (torque_enabled(mode)) {
    ...
    if (position_enabled(mode)) {
      ...
      // wired correctly.
      EXPECT_EQ(evaluate("position_commanded"), test_input);

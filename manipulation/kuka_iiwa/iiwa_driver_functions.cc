...
void ApplyDriverConfig(
  ...
  MultibodyPlant<double>* controller_plant =
      SharedPointerSystem<double>::AddToBuilder(
          builder,
          internal::MakeArmControllerModel(sim_plant, arm_model, hand_model));
  ...
  if (lcm->get_lcm_url() == LcmBuses::kLcmUrlMemqNull) {
    BuildSimplifiedIiwaControl(
        sim_plant, arm_model.model_instance, *controller_plant, builder,
        driver_config.ext_joint_filter_tau, desired_iiwa_kp_gains, control_mode,

...
struct Scenario {
  ...
  //
  // @note  The seed is always deterministic in the `Scenario`; a caller
  // who wants randomness must populate this value from their own randomness.
  std::uint64_t random_seed{0};

  // The maximum simulation time (in seconds).  The simulator will attempt
  // to run until this time and then terminate.
...
//
// @param scenario_text amends/updates the scenario with additional YAML
// schema.
...
//
// @param verbose When saving the scenario, whether or not default values
// should be written out to be explicit (true) or omitted (false).  Verbosity
// might be helpful to defend testset scenarios against changing defaults.

...
/// If we reset the diagram containing the ScunkWsgTrajectoryGenerator, we should obtain the same output.
GTEST_TEST(SchunkWsgTrajectoryGeneratorTest, ResetSim) {
  ...
  auto max_force_logger = LogVectorOutput(dut->get_max_force_output_port(), &builder);
  ...
  auto get_and_clear_logger = [](const systems::VectorLogSink<double>& sink,
    ...
    auto& log = sink.GetMutableLog(&log_context);
    const auto data = log.data();
    log.Clear();
    return data;
  };
  const auto target_data_expected =
      get_and_clear_logger(*target_logger, &(simulator.get_mutable_context()));
  const auto max_force_data_expected =
      get_and_clear_logger(*max_force_logger, &(simulator.get_mutable_context()));

  // Simulate the diagram again with reset_context.
  simulator.get_mutable_context().SetTimeStateAndParametersFrom(
      *reset_context);
  simulator.Initialize();
  simulator.AdvanceTo(0.1);
  const auto target_data =
      get_and_clear_logger(*target_logger, &(simulator.get_mutable_context()));
  const auto max_force_data =
      get_and_clear_logger(*max_force_logger, &(simulator.get_mutable_context()));

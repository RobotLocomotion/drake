...
void SchunkWsgPdController::DoCalcTimeDerivatives(
  ...
  // Assuming all the force_limit is going to come from the integral term,
  // this is how big the integrator needs to be.
  const double cap = force_limit / ki_command_;

  // Don't integrator pass cap.
  if (context.get_continuous_state_vector()[0] > cap && err > 0) {
    (*derivatives)[0] = 0;
  } else if (context.get_continuous_state_vector()[0] < -cap && err < 0) {
    (*derivatives)[0] = 0;
  } else {
    // I need to make the integrator crank up faster. This is also dependent
    // on desired_state[0]'s trajectory..

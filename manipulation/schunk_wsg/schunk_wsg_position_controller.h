...
/// controller to track -q₀=q₁=q_d (the desired position).  We model that here
/// with two PD controllers -- one that implements the physical constraint
...
///   kp_constraint >> kp_command  and  kd_constraint >> kd_command.
///
...
///
/// The desired_position and force_inputs are scalars (BasicVector<double> of
...
class SchunkWsgPositionController : public systems::LeafSystem<double> {
 ...
 private:
  ...
  int desired_position_input_port_{};
  int force_limit_input_port_{};
  int state_input_port_{};
  int generalized_force_output_port_{};
  int grip_force_output_port_{};
};

...
integrates successive velocity commands open loop.  In practice, we find that
this can lead to much more stable behavior than using the measured positions.
...
class DifferentialInverseKinematicsIntegrator
 ...
 public:
  ...
  @param context Optional Context of the MultibodyPlant.  The position values of
  this context will be overwritten during integration; you only need to pass
  this in if the robot has any non-default parameters.  @default
  ...
  */
  ...
  /** Sets the current positions in the context. */

...
DifferentialInverseKinematicsResult DoDifferentialInverseKinematics(
  ...
  Vector6<double> V_WE_E_with_gains;
  MatrixX<double> J_WE_E_with_gains{6, num_columns};
  int num_cart_constraints = 0;
  for (int i = 0; i < 6; i++) {
    const double gain{parameters.get_end_effector_velocity_gain()(i)};
    if (gain > 0) {
      J_WE_E_with_gains.row(num_cart_constraints) = J_WE_E.row(i);
      V_WE_E_with_gains(num_cart_constraints) = V_WE_E[i];
      num_cart_constraints++;
    }
  }

  return DoDifferentialInverseKinematics(
      q_current, v_current, V_WE_E_with_gains.head(num_cart_constraints),
      J_WE_E_with_gains.topRows(num_cart_constraints), parameters);
}
}  // namespace internal
...
DifferentialInverseKinematicsParameters::
    ...
    : num_positions_(num_positions),
      num_velocities_(num_velocities ? *num_velocities : num_positions),
...
DifferentialInverseKinematicsResult DoDifferentialInverseKinematics(
  ...
  const double kMinScaling = 1e-2;
  if (alpha_sol < kMinScaling) {

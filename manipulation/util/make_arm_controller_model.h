...
/* Constructs a separate plant (different from the simulated plant!) that the
 controller will do inverse dynamics on.  The difference between this plantvand
 @p simulation_plant is the dynamics modeling error.
 ...
 togit dif measure the gripper's inertia and attach a single non-articulated
 rigid body that matches the gripper's inertia. Additionally, if `grasp_frame`
 is present in the gripper model and welded to @p gripper_info.child_frame_name,

...
void CompareInertialTerms(const MultibodyPlant<double>& sim_plant,
                          ...
                          Context<double>* sim_plant_context) {
  ...
  const SpatialInertia<double> M_CIo_W_sim =
      sim_plant.CalcSpatialInertia(*sim_plant_context, sim_plant.world_frame(),
                                   GetAllBodyIndices(sim_plant));
...
/** Creates a Panda arm, as a way to debug stuff. */

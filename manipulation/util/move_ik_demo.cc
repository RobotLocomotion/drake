...
MoveIkDemo::MoveIkDemo(const std::string& robot_description,
...
MoveIkDemo::~MoveIkDemo() {}

void MoveIkDemo::set_joint_velocity_limits(
...
std::optional<robotlocomotion::robot_plan_t> MoveIkDemo::HandleStatus(
  ...
  if (status_count_ % print_interval_ == 1) {
    ...
    const math::RigidTransform<double> ik_pose =
        plant_.EvalBodyPoseInWorld(
            *context_, plant_.GetBodyByName(ik_link_));
    const math::RollPitchYaw<double> rpy(ik_pose.rotation());
    drake::log()->info("{} at: {} {}",
                       ik_link_,
                       ik_pose.translation().transpose(),

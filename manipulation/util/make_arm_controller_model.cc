...
std::unique_ptr<MultibodyPlant<double>> MakeArmControllerModel(
  ...
  if (gripper_info) {
    ...
    const std::string gripper_child_frame_name = gripper_info->child_frame_name;
    const Frame<double>& G = simulation_plant.GetFrameByName(
        gripper_child_frame_name, gripper_info->model_instance);
    ...
    const RigidBody<double>& C =
        plant->AddRigidBody(gripper_child_frame_name, arm_model_index, M_CGo_G);

 ...
 * specified RigidBodyFrames. Frame information is communicated via a
 * FramePoseVector object. Frames can be specified at construction in one of
 * two ways: (1) by providing a set of RigidBody names (of bodies already
 * existing in the RigidBodyTree), which specify which RigidBodies these newly
 * created frames should be attached to, or (2) by providing a set of
 * RigidBodyFrames directly.  The former simply searches the RigidBodyTree and
 * assigns a RigidBodyFrame to the specified body. Frame pose w.r.t. the base
 * RigidBody frame can also be specified.  This system takes an abstract valued
 * input of type KinematicResults<double> and generates an abstract value
 * output of type geometry::FramePoseVector<double>.
 */

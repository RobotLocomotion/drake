...
If your inertia matrix comes out as all zeros, rescale the mesh in Meshlab (but
don't save it!) prior to computing the Geometric Measures.  Be sure --scale
passed to this tool reflects your rescaling.
"""
...
def convert(log_text, scale, mass_kg):
    ...
    # in https://docs.python.org/3.4/library/string.html, we'll deter the Fixed
    # Point precision of the maximum element that yields the correct number of
    # sig figs.  Then we'll apply that to all of the other elements.
    inertia_sigfigs = 3
    max_element = max([ixx, ixy, ixz, iyy, iyz, izz])
    exp = int("{:.{}e}".format(max_element, inertia_sigfigs).split('e')[-1])
    inertia_precision = inertia_sigfigs - 1 - exp
...
def main():
    ...
    parser.add_argument(
        "filename", nargs=1,
        help="If given a .txt file, assumes it is meshlab log output.")
    ...
    if ext == ".txt":
        ...
        print("Can't handle a '{}' file".format(ext))
        return 1

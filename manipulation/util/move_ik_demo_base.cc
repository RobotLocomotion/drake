...
std::optional<robotlocomotion::robot_plan_t> MoveIkDemoBase::HandleStatus(
  ...
  if (status_count_ == 1) {
    ConstraintRelaxingIk ik(robot_description_, ik_link_);
    ...
    const bool result =
        ik.PlanSequentialTrajectory(waypoints, q, &q_sol);

...
void SanityCheckMessage(const lcmt_jaco_command& message) {
  DRAKE_DEMAND(
      message.num_joints == static_cast<int>(message.joint_position.size()));
  DRAKE_DEMAND(
      message.num_joints == static_cast<int>(message.joint_velocity.size()));
  DRAKE_DEMAND(
      message.num_fingers == static_cast<int>(message.finger_position.size()));
  DRAKE_DEMAND(
      message.num_fingers == static_cast<int>(message.finger_velocity.size()));
}

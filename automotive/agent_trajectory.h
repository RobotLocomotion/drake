...
class PoseVelocity final {
 public:
  ...
  Eigen::Translation<double, 3> translation() const { return translation_; }

  /// Accesses R_WA, the rotation component of the pose of A, expressed in
  /// world-frame coordinates W.
  Eigen::Quaternion<double> rotation() const { return rotation_; }
  ...
  /// order enumerated within PoseVelocity::Indices.
  ///
  /// Note that this accessor is for use within AgentTrajectory and callers
  /// should NOT use it; instead use one of the above accessors.
...
class AgentTrajectory final {
 public:
  ...
  /// `breaks` is not strictly increasing, and the inputs are otherwise
  ...
  PoseVelocity value(double time) const {
    return PoseVelocity{trajectory_.value(time)};
  }

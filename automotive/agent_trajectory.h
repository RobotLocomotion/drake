...
/// A class that wraps a piecewise trajectory instantiated based on PoseVelocity
/// data.  The underlying trajectory is a configurable PiecewisePolynomial for
/// the translation component of pose and a PiecewiseQuaternionSlerp for the
/// rotational component.  The velocity is computed internally to fully specify
/// a PoseVelocity structure when evaluated at a given time.  The result, when
/// evaluated at each knot point, is expected to match the input pose data
/// exactly.
class AgentTrajectory final {
 public:
  ...
  /// common/trajectories/piecewise_polynomial.h for details on each of the

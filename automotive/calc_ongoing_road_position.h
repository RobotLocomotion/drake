...
/// If kMemorize, configures the calling system (e.g. IdmController,
/// MobilPlanner) to declare an abstract state that memorizes the last-computed
/// RoadPosition.  If kExhaustiveSearch, then the system will contain no
/// abstract states.  Note that the memory option is for performance speedup (at
/// the expense of optimizer compatibility) by preventing a potentially sizeable
/// computation within RoadGeometry::ToRoadPosition().
enum class RoadPositionStrategy { kMemorize, kExhaustiveSearch };

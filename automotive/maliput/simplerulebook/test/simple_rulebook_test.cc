...
GTEST_TEST(SimpleRulebookTest, FindRules) {
  ...
  RoadRulebook::QueryResults reversed = dut.FindRules({kReversedZone},
                                                      kZeroTolerance);
  ...
  const LaneSRange kNearbyRange(
      ...
      {kZone.s_range().s1() + (0.9 * kNonzeroTolerance),
       kZone.s_range().s1() + 50.});
  RoadRulebook::QueryResults nearby = dut.FindRules({kNearbyRange},
                                                    kNonzeroTolerance);
  ...
  const LaneSRange kTooFarRange(
      ...
      {kZone.s_range().s1() + (1.1 * kNonzeroTolerance),
       kZone.s_range().s1() + 50.});
  RoadRulebook::QueryResults toofar = dut.FindRules({kTooFarRange},
                                                    kNonzeroTolerance);

...
/// SimpleRulebook is a simple concrete implementation of the api::RoadRulebook
/// abstract interface.
class SimpleRulebook : public api::rules::RoadRulebook {
 ...
 private:
  virtual api::rules::RoadRulebook::QueryResults DoFindRules(
      const std::vector<api::rules::LaneSRange>& ranges,
      double tolerance) const;
  virtual api::rules::RightOfWayRule DoGetRule(
      const api::rules::RightOfWayRule::Id& id) const;
  virtual api::rules::SpeedLimitRule DoGetRule(
      const api::rules::SpeedLimitRule::Id& id) const;

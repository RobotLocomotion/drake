...
// TODO(maddog@tri.global)  This hash_append implementation will be moved
//                          into drake/common contemporaneously with this PR.
...
QueryResults SimpleRulebook::DoFindRules(
  ...
  for (const LaneSRange& range : ranges) {
    for (IdVariant id : index_->FindRules(range, tolerance)) {
      if (id.r) {

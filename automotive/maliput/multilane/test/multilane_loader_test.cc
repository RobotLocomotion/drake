...
class BuilderFactoryMock : public BuilderFactoryBase {
 public:
  explicit BuilderFactoryMock(std::shared_ptr<BuilderBase> builder_mock)
      : BuilderFactoryBase(), builder_mock_(builder_mock) {
    ON_CALL(*this, MakeShared(_, _, _, _)).WillByDefault(Return(builder_mock_));
  }

  MOCK_CONST_METHOD4(MakeShared,
                     std::shared_ptr<BuilderBase>(double, const api::HBounds&,
                                                  double, double));

 private:
  std::shared_ptr<BuilderBase> builder_mock_;
};
...
GTEST_TEST(MultilaneLoaderTest, MinimalCorrectYaml) {
  ...
  auto builder_mock = std::make_shared<BuilderMock>(
      kLaneWidth, kElevationBounds, kLinearTolerance, kAngularTolerance);
  BuilderFactoryMock builder_factory_mock(builder_mock);
  ...
  EXPECT_CALL(builder_factory_mock,
              MakeShared(kLaneWidth, hbounds_matcher, kLinearTolerance,
                         kAngularTolerance))
      .InSequence(sequence);
...
GTEST_TEST(MultilaneLoaderTest, RoadCircuit) {
  ...
  Sequence conn_seq, g1_seq, g2_seq;
...

  {
    ...
    EXPECT_CALL(*builder_mock,
                Connect("s13", kOneLane, kZeroRRef, kDefaultLeftShoulder,
                        ...
                        endpoint_z_zero_matcher))
        .InSequence(conn_seq, g1_seq, g2_seq);
  }

  // Group expectations.
  {
    EXPECT_CALL(*builder_mock, MakeGroup("g1")).InSequence(g1_seq);
    EXPECT_CALL(*builder_mock, MakeGroup("g2")).InSequence(g2_seq);

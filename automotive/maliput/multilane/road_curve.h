...
class RoadCurve {
 ...
 protected:
  ...
  ///    has a path-length l(p) in range [0, l_max], linearly related to p,
  ///    where l_max is the total path-length of R (in real-world units);
  ///  * the true elevation function E_true(l), parameterized by the
  ///    path-length l of R;
  ///  * the true superelevation function S_true(l), parameterized by the
  ///    path-length l of R;
  ///
  /// then:
  ///  * p = l / l_max;
  ///  * @p elevation is E_scaled = (1 / l_max) *
  ///                               E_true(l_max * p);
  ///  * @p superelevation is  S_scaled = (1 / l_max) *
  ///                                     S_true(l_max * p).

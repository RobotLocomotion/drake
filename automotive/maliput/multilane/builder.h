...
class BuilderFactoryBase {
 public:
  ...
  virtual std::shared_ptr<BuilderBase> MakeShared(
...
class BuilderFactory : public BuilderFactoryBase {
 public:
  ...
  std::shared_ptr<BuilderBase> MakeShared(
      double lane_width, const api::HBounds& elevation_bounds,
      double linear_tolerance, double angular_tolerance) const override {
    return std::make_shared<Builder>(lane_width, elevation_bounds,

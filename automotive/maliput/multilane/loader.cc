...
// @return A std::drake::optional<Endpoint> with the Endpoint or nullopt if it
...
optional<Endpoint> FindEndpointInCatalog(
  ...
  const std::string::size_type forward_pos = ref.find(kForward);
  const std::string::size_type reverse_pos = ref.find(kReverse);
...
drake::optional<Endpoint> ResolveStartReference(
...
// @returns a std::unique_ptr<Endpoint> with the Endpoint or nullopt if it is
// not found inside `xyz_catalog`.
drake::optional<Endpoint> ResolveExplicitEndReference(

...
class RoadNetwork {
 public:
  ...
  RoadNetwork(std::unique_ptr<const RoadGeometry> road_geometry,
              ...
              std::vector<std::unique_ptr<rules::SpeedLimitRule>> speed_limits);
  ...
  const std::vector<std::unique_ptr<rules::SpeedLimitRule>>* speed_limits()
      const {
 ...
 private:
  ...
  std::vector<std::unique_ptr<rules::SpeedLimitRule>> speed_limits_;
};

...
GTEST_TEST(RightOfWayRuleTest, CopyingAndAssignment) {
  const RightOfWayRule source(RightOfWayRule::Id("dut_id"),
                              RightOfWayRule::Type::kStopThenGo);

  const RightOfWayRule dut0(source);
  EXPECT_EQ(dut0.id(), RightOfWayRule::Id("dut_id"));
  EXPECT_EQ(dut0.type(), RightOfWayRule::Type::kStopThenGo);

  RightOfWayRule dut1(RightOfWayRule::Id("other_id"),

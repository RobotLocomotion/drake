...
::testing::AssertionResult IsEqual(const char* a_expression,
  ...
  ADD_RESULT(c, IS_EQUAL(a.zone(), b.zone()));
  ADD_RESULT(c, IS_EQUAL(a.severity(), b.severity()));
  ADD_RESULT(c, IS_EQUAL(a.max(), b.max()));
  ADD_RESULT(c, IS_EQUAL(a.min(), b.min()));
  return c.result();
}
...
GTEST_TEST(SpeedLimitRuleTest, Accessors) {
  ...
  EXPECT_TRUE(IS_EQUAL(dut.zone(), kZone));

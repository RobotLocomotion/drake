...
TYPED_TEST(GeoPositionTest, VectorArithmeticOperators) {
  ...
  GeoPositionT<T> dut1 = gp1 + gp2;
  EXPECT_TRUE(dut1 == GeoPositionT<T>(T(5.), T(7.), T(9.)));
  dut1 = gp1 - gp2;
  EXPECT_TRUE(dut1 == GeoPositionT<T>(T(-3.), T(-3.), T(-3.)));
  dut1 = 2. * gp1;
  EXPECT_TRUE(dut1 == GeoPositionT<T>(T(2.), T(4.), T(6.)));
  dut1 = gp1 * 3.;
  EXPECT_TRUE(dut1 == GeoPositionT<T>(T(3.), T(6.), T(9.)));
}

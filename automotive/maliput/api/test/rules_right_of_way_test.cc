...
::testing::AssertionResult IsEqual(const char* a_expression,
  ...
  ADD_RESULT(c, IS_EQUAL(a.id(), b.id()));
  ADD_RESULT(c, IS_EQUAL(a.controlled_zone(), b.controlled_zone()));
  ADD_RESULT(c, IS_EQUAL(a.type(), b.type()));
  return c.result();
}
...
GTEST_TEST(RightOfWayRuleTest, Accessors) {
  ...
  EXPECT_TRUE(IS_EQUAL(dut.controlled_zone(), kZone));
  EXPECT_TRUE(IS_EQUAL(dut.type(), RightOfWayRule::Type::kStopThenGo));
}
...
GTEST_TEST(RightOfWayRuleTest, Copying) {
  ...
  EXPECT_TRUE(IS_EQUAL(dut, source));
}

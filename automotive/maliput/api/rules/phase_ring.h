...
class PhaseRing final {
 public:
  ...
  /// Returns the possible next phases given the current phase. Aborts if no
  /// phase with an ID of `id` exists.
  const std::vector<NextPhase>& GetNextPhases(const RightOfWayPhase::Id& id)
      const {

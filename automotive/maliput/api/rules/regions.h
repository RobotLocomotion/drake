...
class LaneSRange {
 public:
  ...
  /// Construct a LaneSRange as `s_range` on Lane `lane_id`.
  ...
  /// Test for exact equality.
  bool operator==(const LaneSRange& rhs) const {
    return (lane_id_ == rhs.lane_id_) && (s_range_ == rhs.s_range_);
  }

...
/// Maps BulbColor enums to string representations. Ordering does not matter.
/// A std::map is used instead of a std::unordered_map to ensure Bulb remains
/// hashable.
...
/// Maps BulbType enums to string representations. Ordering does not matter.
/// A std::map is used instead of a std::unordered_map to ensure Bulb remains
/// hashable.
...
class Bulb final {
 public:
  ...
  ///
  /// @param arrow_orientation The orientation of the arrow, which only must be
  /// defined when @p type is BulbType::kArrow. This is the angle along the
  /// bulb's +X axis relative to the bulb's +Y axis. For example, an angle of
  /// zero means the bulb's arrow is pointing along the bulb's +Y axis, which
  /// means it's a right-turn arrow when viewed by an approaching vehicle.
  /// Similarly, an angle of PI/2 points in the bulb's +Z direction (i.e.,
  /// forward from an approaching vehicle's perspective), and an angle of PI
  /// points to in the bulb's -Y direction (i.e., left from an approaching
  ...
  Bulb(const Id& id, const GeoPosition& position_bulb_group,
       ...
       const BulbType& type, optional<double> arrow_orientation = nullopt);
  ...
  const optional<double> arrow_orientation() const {
    return arrow_orientation_;
  }
 ...
 private:
  ...
  optional<double> arrow_orientation_ = nullopt;
};

...
class RightOfWayRule final {
 public:
  ...
  /// may have multiple possible States, thus each State has an Id
  /// which must be unique within the context of the RightOfWayRule.
  ///
...
class RightOfWayStateProvider {
 public:
  ...
  ///
  /// Throws a std::exception if `id` is unrecognized, which should be the
  /// case if no such rule exists or if the rule has only static semantics.
  Result GetState(const RightOfWayRule::Id& id) const {
 ...
 private:
  virtual Result DoGetState(const RightOfWayRule::Id& id) const = 0;
};

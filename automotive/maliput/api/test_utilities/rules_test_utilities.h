...
#define ADD_RESULT(collector, result)                        \
...
inline ::testing::AssertionResult IsEqual(
  ...
  ADD_RESULT(c, IS_EQUAL(a.s0(), b.s0()));
  ADD_RESULT(c, IS_EQUAL(a.s1(), b.s1()));
  return c.result();
}
...
inline ::testing::AssertionResult IsEqual(
  ...
  AssertionResultCollector c;
  ADD_RESULT(c, IS_EQUAL(a.lane_id(), b.lane_id()));
  ADD_RESULT(c, IS_EQUAL(a.s_range(), b.s_range()));
  return c.result();
}
...
inline ::testing::AssertionResult IsEqual(
  ...
  AssertionResultCollector c;
  ADD_RESULT(c, IS_EQUAL(a.size(), b.size()));
  int smallest = std::min(a.size(), b.size());
  for (int i = 0; i < smallest; ++i) {
    ADD_RESULT(c, IS_EQUAL(a[i], b[i]));
  }
  return c.result();
}
...
inline ::testing::AssertionResult IsEqual(
  ...
  AssertionResultCollector c;
  ADD_RESULT(c, IS_EQUAL(a.ranges(), b.ranges()));
  return c.result();
}

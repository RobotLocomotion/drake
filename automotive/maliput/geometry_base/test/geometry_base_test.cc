...
GTEST_TEST(GeometryBaseRoadGeometryTest, BasicConstruction) {
  const double kPositiveDoubleA = 7.0;
  const double kPositiveDoubleB = 99.0;
  // Tolerance values must be positive.
  EXPECT_THROW(MockRoadGeometry(
      api::RoadGeometryId("dut"), kPositiveDoubleA, 0.), std::exception);
  EXPECT_THROW(MockRoadGeometry(
      api::RoadGeometryId("dut"), 0., kPositiveDoubleB), std::exception);

  MockRoadGeometry dut(
      api::RoadGeometryId("dut"), kPositiveDoubleA, kPositiveDoubleB);
  EXPECT_EQ(dut.id(), api::RoadGeometryId("dut"));
  EXPECT_EQ(dut.linear_tolerance(), kPositiveDoubleA);
  EXPECT_EQ(dut.angular_tolerance(), kPositiveDoubleB);
}

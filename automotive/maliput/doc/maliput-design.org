...
corresponding traffic signals.  (Note that a typically a "yellow light"
confers the same right-of-way as a "green light"; the only difference
is that the yellow signals that a transition to red is imminent.)
...
Figure [[fig:RoWR-freeway-merge]] is a scenario with a freeway merge:
  ...
  * Entrance ramp merges onto to freeway from the right (south).

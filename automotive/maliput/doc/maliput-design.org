...
Note that =RoadRulebook= is an abstract API.  =maliput= also provides a
simple concrete implementation of this API, =SimpleRulebook=.
...
#+BEGIN_QUOTE
/Future API's may be developed to express the sensory and symbolic
...
=RightOfWayRule= comprises the static state of such rules:
    * id
    * controlled zone
      * a contiguous longitudinal range potentially spanning multiple
        =LaneId='s
      * /This is directional: the beginning of the range is considered
        the "entry-point" or "stop-line"./
    * type
      * /Proceed/:  Vehicles have right of way, but should avoid stopping
        within the controlled zone (e.g., pedestrian crosswalk).

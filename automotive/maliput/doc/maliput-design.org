...
=RightOfWayRule='s describe which vehicles have right-of-way (also known as
...
=zone=.  If the type is /StopExcluded/, then vehicles are expected to
stop before entering the =zone=, and to leave the =zone= if they lose
the right-of-way.
...
a (possibly empty) list of other rule instances (referenced by Id)
whose right-of-way supersedes this rule.
...
rule/; it's meaning is entirely static and fixed for all time.
...
/next/ =State= of the rule, if a transition is imminent and the next
state is known.  =next.duration_until= is the duration, if known,
until the transition to the known next state.

Following are discussions of possible =RightOfWayRule= configurations
...
Figure [[fig:RoWR-lone-crosswalk]] illustrates a very simple scenario:
  ...
  * The pedestrian traffic route is not modeled in the road network, so only
    one zone (for the vehicular traffic against the crosswalk) is involved.
...
The zone is a =LaneSRoute= spanning from one edge of the crosswalk to
the other, with zone-type /StopExcluded/, which means that stopping
...
always "Go".  However, it does serve to capture the requirement that
...
|---------------+------------------+--------------+---------------------------|
| "WB/Right"    | "Red"            | /StopThenGo/ | "NB/Straight", "NB/Left"  |
|               | "Green"          | /Go/         | ---                       |
|---------------+------------------+--------------+---------------------------|
...
to westbound traffic.  Typically in the US, the protected turn would be
signalled by a solid green arrow, and the permitted turn by a flashing
yellow arrow.

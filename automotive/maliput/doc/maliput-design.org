 ...
 * $(s,r,h)$ are =Lane=-frame coordinates understood in the context
   of =Lane= $L$.
One can construct a map $W: {(L,s,r,h)} \to \mathbb{R}^3$ from the
...
A =Lane= represents a lane of travel in a road network.  As discussed
above, a =Lane=, identified by $L$, defines a map $W_L$ from curvilinear
coordinates to the =World=-frame:
\[
W_L: (s,r,h) \to (x,y,z), \text{ for } s \in [0, s_\text{max}]
\]
...
despite the name).  $W_L$ is required to be $G^1$ continuous, and
...
*** =Lane=
    ...
    * embedding $W_L: (s,r,h) \to (x,y,z)$
    * inverse $W_L^{-1}: (x,y,z) \to (s,r,h)$
...
admitted a single =Lane= per =Segment=.]  is an implementations of the
=maliput= geometry API which synthesizes a road network from planar
geometric primitives, mimicking techniques used in the geometric
design of real roads.  The basic geometry of a =Segment= is derived
from the composition of a planar ($xy$) primitive curve, an elevation
function, and a superelevation function, combined together to define a
ruled surface.  A =Lane= of a =Segment= is defined via a constant
lateral offset along the surface from the $r=0$ reference curve of the
=Segment=.
 ...
 * $(p,r,h)_\text{SEG}$ is a position in a curvilinear reference frame of
   ...
   The parameter $p_\text{SEG} \in [0, 1]$ spans the =Segment= longitudinally.
   $r_\text{SEG}$ is a lateral offset from the =Segment='s reference curve,
   along the =Segment= surface.  $h_\text{SEG}$ is height above the surface.
When describing a single component, parentheses may be dropped for
brevity; e.g., $p_\text{SEG} \equiv (p)_\text{SEG}$.
...
\]
which is itself composed from three fundamental functions, $G_\text{xy}$,
$G_z$, and $\Theta$.

The first fundamental function defines a two dimensional
...
(albeit with a peculiar scale factor):
\[
...
\left(\begin{array}{c}x\\y\\z\end{array}\right) =
W(p)_\text{SEG} =
\left(\begin{array}{c} G_x(p)_\text{SEG}\\
                       G_y(p)_\text{SEG}\\
                       l_\text{max}G_z(p)_\text{SEG} \end{array}\right)
\]
defines the three dimensional /reference curve/ $W(p)_\text{SEG}$
for the =Segment= and establishes the
...
so that:
\begin{eqnarray*}
      z & = & l_\text{max} G_z(p)_\text{SEG}\\
        & = & l_\text{max} G_z\left(\frac{l}{l_\text{max}}\right)\\
\dot{z} & = & \frac{dz}{dl} = \frac{d}{dp_\text{SEG}}G_z(p)_\text{SEG}
\end{eqnarray*}
...
surface:
\[
\Theta: p_\text{SEG} \to \frac{1}{l_\text{max}}\theta
\]
Superelevation $\theta$ is the "twist" in a road, given as the angle
of rotation of the $\hat{r}_\text{SEG}$ basis vector around the
$\hat{p}_\text{SEG}$ basis vector.  Zero superelevation leaves
$\hat{r}_\text{SEG}$ in the $xy$ plane.[fn::Note that superelevation
becomes ambiguous when $\hat{p}_\text{SEG} = \hat{z}$.]  As with
$G_z$, $\Theta$ is scaled so that:
\begin{eqnarray*}
      \theta & = & l_\text{max} \Theta\left(\frac{l}{l_\text{max}}\right)\\
\dot{\theta} & = &
              \frac{d\theta}{dl} = \frac{d}{dp_\text{SEG}}\Theta(p)_\text{SEG}
\end{eqnarray*}

With the three fundamental functions in hand, we can express the orientation
of the $(\hat{p},\hat{r},\hat{h})_\text{SEG}$ frame along the reference curve,
with respect to the =World=-frame, as a roll/pitch/yaw rotation:
\begin{align*}
\mathbf{R}(p)_\text{SEG} &=
...
\beta(p_\text{SEG})  &=
  \mathrm{atan}\negthickspace\left(\frac{dG_z}
                                        {dp_\text{SEG}}\right)
& \text{(pitch)} \\
\alpha(p_\text{SEG}) &= l_\text{max}\Theta(p_\text{SEG}) & \text{(roll)}
\end{align*}
# Note that $\hat{p}_\text{SEG}$ is solely determined by $W(p)_\text{SEG}$,
# and as expected,
# $\hat{p}_\text{SEG} = \frac{W'(p)_\text{SEG}}{\lVert W'(p)_\text{SEG}\rVert}$.

With $\mathbf{R}(p)_\text{SEG}$, we now define the =Segment= /surface/
$W(p,r)_\text{SEG}$ as:
\begin{align*}
\left(\begin{array}{c}x\\y\\z\end{array}\right) = W(p,r)_\text{SEG} = \left(
\begin{array}{c}
   G_{xy}(p)_\text{SEG}\\
   l_\text{max} G_z(p)_\text{SEG}
\end{array} \right) +
\mathbf{R}(p)_\text{SEG}\negthickspace
\begin{pmatrix}
0\\ r_\text{SEG} \\ 0 \end{pmatrix}.
\end{align*}
This is a /ruled surface/.  For any $p_\text{SEG}$,
$W(p,r)_\text{SEG}$ is linear $r_\text{SEG}$ and motion long
$r_\text{SEG}$ is in a straight line.

Now that we have the surface embedding $W(p,r)_\text{SEG}$, we can derive
the basis vectors $(\hat{p}, \hat{r}, \hat{h})_\text{SEG}$ along the surface
and the corresponding orientation $\mathbf{R}(p,r)_\text{SEG}$:
\begin{align*}
\mathbf{R}(p,r)_\text{SEG} &=
                     \begin{pmatrix}\hat{p} & \hat{r} & \hat{h}\end{pmatrix}\\
\hat{p}_\text{SEG} &=
 \frac{\partial_p W(p,r)_\text{SEG}}{\lVert\partial_p W(p,r)_\text{SEG}\rVert}\\
\hat{r}_\text{SEG} &=
 \frac{\partial_r W(p,r)_\text{SEG}}{\lVert\partial_r W(p,r)_\text{SEG}\rVert}\\
\hat{h}_\text{SEG} &= \hat{p} \times \hat{r}
\end{align*}

A few things are worth noting at this point:
 1. $\hat{r}_\text{SEG} = \mathbf{R}(p)_\text{SEG}
    \left(\begin{smallmatrix} 0\\1\\0 \end{smallmatrix}\right)$.  Thus,
    $\hat{r}_\text{SEG}$ is independent of $r_\text{SEG}$.
 1. $\mathbf{R}(p,r)_\text{SEG} = \mathbf{R}(p)_\text{SEG}$ along
    $r_\text{SEG} = 0$ just as it should be; the orientation along the
    =Segment='s reference curve is consistent in both expressions.
 1. $\hat{p}_\text{SEG}$ is /not necessarily/ independent of
    $r_\text{SEG}$.  Consequently, $\mathbf{R}(p,r)_\text{SEG}$ is not
    necessarily equal to $\mathbf{R}(p_\text{SEG})$ for
...
/Finally/, with $\mathbf{R}(p,r)_\text{SEG}$ in hand (and points 1 and
2 above), we can define the complete volumetric world transform
$W(p,r,h)_\text{SEG}$ for a =Segment='s geometry:
\begin{align*}
\begin{pmatrix}x\\y\\z\end{pmatrix} = W(p,r,h)_\text{SEG} = \left(
\begin{array}{c}
   G_x(p_\text{SEG})\\
   G_y(p_\text{SEG})\\
   l_\text{max} G_z(p_\text{SEG})
\end{array} \right) +
\mathbf{R}(p,r)_\text{SEG}\negthickspace
\begin{pmatrix}
0\\ r_\text{SEG} \\ h_\text{SEG} \end{pmatrix}.
\end{align*}
This is simply $W(p,r)_\text{SEG}$ displaced by $h_\text{SEG}$ along
the surface normal $\hat{h}_\text{SEG}$.


*** =Lane= Geometry

A =Lane= derives its geometry from its =Segment=.  In =multilane=, the
centerline of a =Lane= is a parallel curve with a constant lateral
offset $r_0$ from the reference curve (at $r_\text{SEG} = 0$) of the
=Segment=.  We can express this relationship as a transform between
$(s,r,h)_\text{LANE}$ (=Lane=-frame) and $(p,r,h)_\text{SEG}$
(=Segment=-frame):
\begin{align*}
...
\end{align*}
The tricky part here is $P:s_\text{LANE} \to p_\text{SEG}$, which relates
$s_\text{LANE}$ to $p_\text{SEG}$, and involves the
path-length integral over $W(p,r)_\text{SEG}$.

=maliput= defines $s_\text{LANE}$ as the path-length along a =Lane='s
centerline, and in =multilane= a centerline is a curve with constant
$r_\text{SEG} = r_0$.  Thus:
\begin{align*}
s_\text{LANE} = S(p_\text{SEG}) &=
 \left. \int \left\lVert \partial_{p_\text{SEG}}W(p, r)_\text{SEG}
 \right\rVert dp_\text{SEG} \right\rvert_{r_\text{SEG} = r_0}.
\end{align*}
The function $P$ that we are need is the inverse of the path-integral $S$.
...
compute $P(s_\text{LANE})$ and $S(p_\text{SEG})$ analytically if
...
using an arbitrary longitudinal parameter $p_\text{LANE}$ which could
be converted to a distance $s_\text{LANE}$ on demand, instead of the other
way around.]
...
\begin{align*}
\begin{pmatrix} x\\ y \end{pmatrix} = G_\text{xy}(p_\text{SEG}) &=
    \begin{pmatrix}x_0\\ y_0\end{pmatrix} +
\begin{cases} %\left\lbrace\begin{matrix}
 \frac{1}{\kappa}\begin{pmatrix}
\cos(\kappa l_\text{max} p_\text{SEG} + \gamma_0 - \frac{\pi}{2}) - \cos(\gamma_0 - \frac{\pi}{2})\\
\sin(\kappa l_\text{max} p_\text{SEG} + \gamma_0 - \frac{\pi}{2}) - \sin(\gamma_0 - \frac{\pi}{2})
\end{pmatrix} & \text{for }\kappa > 0\\
%
 l_\text{max} p_\text{SEG}
...
\begin{align*}
\begin{split}
\frac{1}{l_\text{max}}z = G_z(p_\text{SEG}) &=
 \frac{z_0}{l_\text{max}} +
 \dot{z_0} p_\text{SEG} +

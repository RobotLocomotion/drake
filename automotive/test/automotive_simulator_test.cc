...
using LoadRobotSubscriber = drake::lcm::Subscriber<lcmt_viewer_load_robot>;
using ViewerDrawSubscriber = drake::lcm::Subscriber<lcmt_viewer_draw>;
...
GTEST_TEST(AutomotiveSimulatorTest, TestLcmOutput) {
  auto simulator = std::make_unique<AutomotiveSimulator<double>>(
      std::make_unique<lcm::DrakeMockLcm>());
  lcm::DrakeLcmInterface* lcm = simulator->get_lcm();
  ASSERT_NE(lcm, nullptr);
  lcm::DrakeMockLcm* mock_lcm = dynamic_cast<lcm::DrakeMockLcm*>(lcm);
  ASSERT_NE(mock_lcm, nullptr);
  LoadRobotSubscriber load_robot_sub(mock_lcm, "DRAKE_VIEWER_LOAD_ROBOT");
  ViewerDrawSubscriber draw_sub(mock_lcm, "DRAKE_VIEWER_DRAW");

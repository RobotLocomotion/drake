...
void CheckResult(const PositionResult<AutoDiffXd>& actual,
  ...
  EXPECT_TRUE(
      CompareMatrices(actual.position[0].derivatives(), exp_pos_derivs, 1e-12));
  EXPECT_TRUE(
      CompareMatrices(actual.position[1].derivatives(), exp_pos_derivs, 1e-12));
  EXPECT_TRUE(
      CompareMatrices(actual.speed.derivatives(), exp_speed_derivs, 1e-12));
}

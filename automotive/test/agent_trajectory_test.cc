...
GTEST_TEST(AgentTrajectoryTest, MismatchedSizes) {
  ...
  const Isometry3d dummy_value(Eigen::Quaternion<double>::Identity());
...
GTEST_TEST(AgentTrajectoryTest, AgentTrajectory) {
  ...
  for (int i{0}; i < static_cast<int>(times.size()); i++) {
    if (i < static_cast<int>(times.size()) - 1) {
      ...
      EXPECT_TRUE(math::IsQuaternionValid(actual.rotation(), kTol));
    }
    ...
    // under linear interpolation and the velocities are as expected given the change in rotations between time steps.

...
AgentTrajectory AgentTrajectory::MakeFohFromWaypoints(
    const std::vector<Eigen::Isometry3d>& waypoints,
    const std::vector<double>& speeds) {
  DRAKE_THROW_UNLESS(speeds.size() == waypoints.size() - 1);
  std::vector<double> times(waypoints.size());
  times[0] = 0.;
  // Populate the segment times given a piecewise-constant speed profile.
  for (int i{0}; i < static_cast<int>(speeds.size()); i++) {
    ...
    const double delta_t = distance / speeds[i];
    times[i + 1] = delta_t + times[i];
  }
  return Make(times, waypoints, InterpolationType::kFirstOrderHold);
}
...
AgentTrajectory AgentTrajectory::MakeFromWaypoints(
    const std::vector<Eigen::Isometry3d>& waypoints, double speed,
    const InterpolationType& interp_type) {
  DRAKE_THROW_UNLESS(speed > 0.);
  if (interp_type == InterpolationType::kFirstOrderHold) {
    std::vector<double> speeds(waypoints.size() - 1);
    std::fill(speeds.begin(), speeds.end(), speed);
    return MakeFohFromWaypoints(waypoints, speeds);
  } else if (interp_type == InterpolationType::kCubic) {

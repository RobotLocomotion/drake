...
void CalcOngoingRoadPosition(const PoseVector<T>& pose,
  ...
  for (const auto& end : {LaneEnd::kStart, LaneEnd::kFinish}) {
    // Check only the relevent lane end.  If s_dot == 0, check both ends
  ...
  // Set RoadPosition to the default if none is found.
  *rp = RoadPosition();
}
...
template void CalcOngoingRoadPosition(
    ...
    const FrameVelocity<AutoDiffXd>& velocity, RoadPosition* rp);
template void CalcOngoingRoadPosition(

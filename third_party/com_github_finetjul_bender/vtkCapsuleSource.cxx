...
namespace {
void InsertPole(vtkPoints* points, vtkFloatArray* normals, double center[3],
...
void ConnectCylinderSide(vtkCellArray* faces, int minusPoleId, int plusPoleId,
...
int vtkCapsuleSource::RequestData(
  ...
  // Set number of faces
  int halfSpheresNumsPolys =
      (this->ThetaResolution - 1) * 2  // top and bottom
  ...
  FillHalfSphere(newPoints, newNormals, this->ThetaResolution,
                 this->PhiResolution, 0.0, 1.0, this->Center, this->Radius,
                 halfHeight);
  this->UpdateProgress(0.255);

  // South pole
  InsertPole(newPoints, newNormals, this->Center, -1 * this->Radius,
             halfHeight);
  ...
  // North pole
  InsertPole(newPoints, newNormals, this->Center, this->Radius,
             -1 * halfHeight);
  this->UpdateProgress(0.305);

  // Create intermediate points
  FillHalfSphere(newPoints, newNormals, this->ThetaResolution,
                 this->PhiResolution, vtkMath::DoubleTwoPi(), -1, this->Center,
  ...
  // first side
  ConnectCylinderSide(newPolys, northPoleMinusId, northPolePlusId, 1, increment,
                      this->LatLongTessellation);

  // second side
  ConnectCylinderSide(newPolys, southPoleMinusId, southPolePlusId, -1,

    ...
    if is_tar_file:
        ...
        with tempfile.TemporaryDirectory(prefix="drake-release-tmp-") as td:
            ...
            version_txt_path = Path(td) / "opt" / "drake" / "share" / "doc" / \
                "drake" / "VERSION.TXT"
            assert version_txt_path.is_file()
...
def _check_urls(*, urls):
    """Check all urls exist using.  Fail if any do not exist with an error
    message indicating which URL(s) cannot be downloaded."""
    assert len(urls) > 0
...
def _download_binaries(*, timestamp, staging, version, deb_only):
    ...
    The `deb_only` is a bool (whether to download just .deb files).
    """
    ...
    if staging:
        ...
        staging_base_url = "https://drake-packages.csail.mit.edu/drake/staging"
        if deb_only:
            binaries[staging_base_url] = deb_filenames
        else:
            # TODO(18145): Download staging .tar.gz once it's ready.
            binaries[staging_base_url] = wheel_filenames + deb_filenames
...
def _download_version(*, timestamp, version, deb_only):
    ...
    filenames = _download_binaries(
        timestamp=timestamp, staging=True, version=version, deb_only=deb_only)
    git_sha = _get_consistent_git_commit_sha(filenames=filenames)
    print(f"The binaries all have the same git commit sha: {git_sha}")

    # Confirm the downloaded .deb files report the correct version.
...
def main():
    ...
    parser.add_argument(
        "--deb-only", action="store_true", help="Only download .deb files.")

...
def _create_tar_gz(*, git_sha, tmp_dir: str, version: str):
    ...
    output_path = f"{tmp_dir}/drake-{version[1:]}-src.tar.gz"
    ...
    _run(f"sha256sum {output_path} > {output_path}.sha256", shell=True)
    _run(f"sha512sum {output_path} > {output_path}.sha512", shell=True)

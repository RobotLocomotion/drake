...
def _run(args):
    ...
    with tarfile.open(args.tgz) as archive:
        ...
        # NOTE: see packages_txt.replace() below, rstrip is required here so
        # so that the last line does *NOT* have a comma (otherwise debian/rules
        # will crash).
        packages_txt = archive.extractfile(
            packages).read().decode('utf-8').rstrip()

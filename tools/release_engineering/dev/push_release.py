...
class _State:
    ...
    def _upload_file_github(self, name: str, local_path: str) -> None:
        ...
        if (local_path.endswith('.tar.gz')):
            ...
            # If we got here, either something is catastrophically wrong, or the
            # script needs to be updated to consider a new case.
            raise RuntimeError('Cannot determine MIME type for file'
                               f' {local_path} with unexpected extension'
                               ' (should be one of: .tar.gz, .sha256, .sha512).')

        if self.options.dry_run:
            print(f'push {name!r} ({content_type}) to {self.release.html_url!r}')
        else:
    ...
    def _download_file(self, name: str, archive_format: str = 'tarball') -> str:
        """

...
def _format_commit(gh, drake, commit):
    ...
    inline_link, _ = _format_pr_link(pr)
...
def _update(args, notes_filename, gh, drake):
    ...
    # .. <begin issue links>
    # .. <end issue links>
    begin = lines.index(_format_comment("<begin issue links>") + "\n")
    end = lines.index(_format_comment("<end issue links>") + "\n")
    assert begin < end
    pr_numbers = set()
    for i, one_line in enumerate(lines):
        if begin < i < end:

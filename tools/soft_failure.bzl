...
# build time whether or not the actual binary exists, and bring it into the
# sandbox only if so.  Bazel notices when glob outcomes change, so will rebuild
# anything the actual binary (dis)appears.  Later at runtime, our wrapper
# sh_binary will see if globbed file exists, and emit a good message if not.

...
clean up the remaining artifacts which are not cleaned up on macOS, i.e., the
Bazel disk and repository caches, run::
...
Note that the Drake build step uses ``${HOME}/.cache/drake-wheel-build/bazel``
for Bazel disk and repository caching. This speeds up iterations for both local

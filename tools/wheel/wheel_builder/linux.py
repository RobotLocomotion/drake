...
targets = {
    ...
    "x86_64": (
        ...
        Target(
            ...
            test_platforms=(
                Platform("amazonlinux", "2023", "AL2023"),
                Platform("ubuntu", "24.04", "noble", uv=True),
                # TODO(tyler-yankee) Add testing for Ubuntu 26.04 (with uv)
                # once it's been released.
            ),
...
def _target_args(target: Target, role: Role, test_index: int | None = None):
    ...
    uv = target.platform(role, test_index).uv
...
def _test_wheel(target, identifier, options):
    ...
    for test_index, test_platform in enumerate(target.test_platforms):
        ...
        install_args = [
            "--wheel", os.path.join(wheelhouse, wheel)
        ]  # fmt: skip
        if test_platform.uv:
            install_args.append("--uv")
        _docker(
            "run", "-t", f"--name={test_container}",
            f"-v{test_dir}:/test",
            f"-v{options.output_dir}:{wheelhouse}",
            base_image, install_script, *install_args,
        )  # fmt: skip

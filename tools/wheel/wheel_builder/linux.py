...
def _target_args(target: Target, role: Role):
    ...
    if role == BUILD and target.python_sha is not None:
        python_args = [
            "--build-arg",
            f"PYTHON=build:{target.python_version_full}",
            "--build-arg",
            f"PYTHON_SHA={target.python_sha}",
        ]
    else:
        python_args = [
            "--build-arg",
...
def _build_image(target, identifier, options):
    ...
    args = [
        "--build-arg",
        f"DRAKE_VERSION={options.version}",
        "--build-arg",
        f"DRAKE_GIT_SHA={_git_sha(resource_root)}",

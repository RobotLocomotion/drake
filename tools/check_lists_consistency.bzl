...
def check_lists_consistency(
    ...
    Args:
        glob (:obj:`str`): expression used to find all the files. The list of
            ...
            lists of files.
        file_lists (:obj:`list` of :obj:`list` of :obj:`str`): List of list of
            file names. This is typically a list of public headers and a list
            of private headers.
    """
    flat_list = [ x for l in file_lists for x in l]
    all_headers = native.glob(glob)
    unknown_headers = [x for x in all_headers if x not in flat_list ]

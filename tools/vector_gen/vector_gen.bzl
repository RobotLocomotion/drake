...
def _vector_gen_outs(srcs, kind):
    """Return the list of output filenames.  The `kind` is either "vector" (foo.h,
    foo.cc) or "lcm" (lcmt_foo_t.lcm).  For compatibility with past practice,
    C++ output will appear under a "gen" folder, but *.lcm output will not.
    """

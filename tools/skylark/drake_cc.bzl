...
def drake_cc_library(
    ...
    Flagging a library as `internal = True` way means that the library is
    internal from the point of view of the the build system, which is an even
    stronger promise that merely placing code inside `namespace internal {}`.
    ...
    deps of _other_ libraries marked asinternal, or as "implementation deps"
    ...
    if internal:
        ...
        if len(kwargs.get("visibility") or []) == 0:
            # If you see this error message, we suggest using either
            #     visibility = ["//visibility:private"],
            # or

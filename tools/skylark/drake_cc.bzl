...
def _raw_drake_cc_library(
    ...
    # code crash if the srcs uses a `select`; that usage is not supported.)
    textual_hdrs = None
...
def drake_cc_library(
    ...
    instead of compiling all scalars within the same translation unit. For
    especially large source files, this reduces build latency.
    """

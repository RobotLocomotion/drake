...
def drake_cc_test(
    ...
    if opt_in_condition == None and opt_out_conditions == None:
        ...
        if build_when_skipped:
            # The test should always be built, but only conditionally run. We'll
            # accomplish that by declaring it both as a test and a binary, but
            # with mutually exclusive conditions for each.

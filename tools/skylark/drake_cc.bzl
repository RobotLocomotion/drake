...
def drake_cc_library_linux_only(
        ...
        visibility = ["//visibility:private"],
        **kwargs):
    """Declares a platform-specific drake_cc_library. When not building on
    Linux, the interface_deps and deps and linkopts are nulled out. The tags
    must have "manual" set so that the library compile is skipped on macOS.
    Note that we do NOT null out srcs; we want the linter to always find them.
    ...
    if "manual" not in (tags or []):
        fail("drake_cc_library_linux_only must have `tags = [\"manual\"]`")
    drake_cc_library(
        name = name,
        srcs = srcs,

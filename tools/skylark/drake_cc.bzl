...
def drake_cc_library(
    ...
    # Because we want to fiddle with include paths, we *must* have all header
    # files listed as hdrs; the include_prefix does not apply to srcs.
    new_srcs, private_hdrs = _prune_private_hdrs(srcs)
    if private_hdrs:
        private_name = "_" + name + "_private_headers_impl"
        _raw_drake_cc_library(
            ...
            copts = new_copts,
            linkstatic = 1,
            # We install private_hdrs by default, because Bazel's visibility
            # denotes whether headers can be *directly* included when using
            # cc_library; it does not precisely relate to which headers should
            # appear in the install tree.  For example, common/symbolic.h is
            # the only public-visibility header for its cc_library, but we also
            # need to install all of its child headers that it includes, such
            # as common/symbolic_expression.h.
            declare_installed_headers = 1,
            **kwargs)
        new_deps = deps + [":" + private_name]
    else:
        new_deps = deps
    _raw_drake_cc_library(
        name = name,
...
def drake_cc_binary(
    ...
    """
    new_copts = _platform_copts(copts, gcc_copts)
    # Because we want to monkey with include paths, we *must* have all header
    # files listed as hdrs; the include_prefix does not apply to srcs.
    new_srcs, private_hdrs = _prune_private_hdrs(srcs)
    if private_hdrs:
        private_name = "_" + name + "_private_headers_impl"

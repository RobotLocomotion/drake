...
def _py_target_isolated(
    ...
    if isolate and not name.startswith(prefix):
        ...
        # N.B. Make sure that a test (visible to both bazel run and bazel test)
        # with the original name redirects to the isolated instantiation so
        # users unfamiliar with isolation that use the "obvious" spelling will

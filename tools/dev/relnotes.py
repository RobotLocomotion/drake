...
def _format_commit(gh, commit):
    ...
    nice_summary = summary
    while nice_summary.endswith("."):
        nice_summary = nice_summary[:-1]
    ...
    reduntant_detail = f"* {summary}"
    if detail.startswith(reduntant_detail):
        detail = detail[len(reduntant_detail):].strip()
    if detail:
        detail = f"  # {detail}"

...
class GlibcMemoryManager final : public MemoryManager {
 public:
  ...
  static void Tare() {
    ...
    g_mallinfo_tare = ::mallinfo();
  }

  static int64_t g_num_allocs;
  static int64_t g_total_allocated_bytes;
  static struct mallinfo g_mallinfo_tare;
};

int64_t GlibcMemoryManager::g_num_allocs = 0;
int64_t GlibcMemoryManager::g_total_allocated_bytes = 0;
struct mallinfo GlibcMemoryManager::g_mallinfo_tare = {};

void GlibcMemoryManager::Stop(Result* result) {
  DRAKE_DEMAND(result != nullptr);

  // TODO(jwnimmer-tri) Use mallinfo2 once we have glibc 2.33.
  struct mallinfo report = ::mallinfo();

  result->num_allocs = g_num_allocs;
  result->max_bytes_used = MemoryManager::TombstoneValue;
...
// XXX We'll need to configure this correctly.

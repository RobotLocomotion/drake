...
class CppGen:
    ...
    def _fill_encode(self):
        ...
        # Format the sequence of preconditions and operations as a C++ short-
        # circuit expression.
        content = " &&\n".join([
            " " * 8 + item
    ...
    def _fill_one_encode(self, field):
        ...
        comma_dims = f", {dims}" if dims else ""
        operations = [
            f"_encode_field({field.name}, _cursor, _end{comma_dims})",
        ]
        return operations
    ...
    def _fill_one_decode(self, field):
        ...
        comma_dims = f", {dims}" if dims else ""
        operations = [
            f"_decode_field(&{field.name}, _cursor, _end{comma_dims})",
        ]

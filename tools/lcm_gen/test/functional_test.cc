...
// With our new lcm-gen, partiall-fixed-size arrays are typed as std::array so
...
GTEST_TEST(RomeoTest, MikeEncode) {
  ...
  std::apply(
      ...
      std::make_tuple(
          ...
          [&](auto* message) {
            message->alpha.india8 = 22;
          },
          [&](auto* message) {
            message->sierra = "sierra";
          },
          [&](auto* message) {
            const int rows = 2;
            const int cols = 3;
          ...
          [&](auto* message) {
            message->xray[1].india8 = 22;
          },
          [&](auto* message) {
            message->yankee[1].india16 = 2222;
          },
          [&](auto* message) {
            message->zulu[1][0].india32 = 22222;
          }));
}

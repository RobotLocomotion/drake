...
#------------------------------------------------------------------------------
def _output_path(input_file, dest, strip_prefix):
    """Compute output path for install action.

    This computes the complete (less install prefix) install path for an input
    file. Its main purpose is a) to determine the path as specified by the user
    when the file was mentioned in a rule, without Bazel's various possible
    extras, and b) to optionally remove prefixes from this path. When removing
    prefixes, the first matching prefix is removed.
    ...
    the prefix ``foo``, giving a final path of ``docs/bar.txt``.

    Args:
        input_file (:obj:`File`): Artifact to be installed.
        dest (:obj:`str`): Install destination.
    ...
    # Determine base path of input file.
    package_root = _join_paths(input_file.owner.workspace_root,
                               input_file.owner.package)
    ...
    for f in file_labels:
        for a in f.files:
            if type(dests) == "dict":
                ...
                dest = dests
            p = _output_path(a, dest, strip_prefix)

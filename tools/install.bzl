...
def _install_java_actions(ctx, target):
    ...
    if target.files_to_run.executable:
        excluded_files = [_output_path(ctx, target.files_to_run.executable, warn_foreign = False)]
    return _install_actions(ctx, [target], dests, strip_prefixes, excluded_files,
                            rename = ctx.attr.rename)
...
def _install_java_launcher_actions(ctx, dest, java_dest, java_strip_prefix, rename, target):
    ...
    for jar in target[MainClassInfo].classpath:
        jar_install = _install_action(ctx, jar, java_dest, java_strip_prefix, rename )
        classpath += "\"" + join_paths("$prefix", jar_install.dst) + "\" "
    ...
    actions.append(struct(src = None, dst = file_dest, classpath = classpath, main_class = main_class))
...
def _java_launcher_code(action):
    return "create_java_launcher(%r, %r, %r)" % (action.dst, action.classpath, action.main_class)
...
def _install_impl(ctx):
    ...
    # Return actions.
    files = ctx.runfiles(files = [a.src for a in actions if not hasattr(a, "main_class")])
    return [

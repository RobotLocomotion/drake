...
def _install_py_actions(ctx, target):
    actions =  _install_actions(ctx, [target], ctx.attr.py_dest,
...
Args:
    ...
    py_dest: Destination for Python targets (default =
        "lib/python2.7/site_packages").
...
#------------------------------------------------------------------------------
def cmake_config(package, script=None, version_file=None):

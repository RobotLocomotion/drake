...
def _drake_toolchain_impl(repository_ctx):
    ...
    repository_ctx.symlink(Label("@local_config_cc//:cc_wrapper.sh"), "cc_wrapper.sh")
    repository_ctx.symlink(Label("@local_config_cc//:CROSSTOOL"), "CROSSTOOL.generic")
    repository_ctx.symlink(Label("//tools:amend_crosstool.sh"), "amend_crosstool.sh")
    res = repository_ctx.execute(["bash", "amend_crosstool.sh", "CROSSTOOL.generic"])

...
def _boost_cc_library(os_name, name, hdrs = None, deps = None):
    ...
    glob_hdrs = [pattern.format(name) for pattern in HDRS_PATTERNS]

    if not deps:
        deps = []

    # The list of headers passed to _boost_cc_library() as an argument are
    # wrapped in a glob() because they may not be present in all versions of
    # Boost.
    content = """
cc_library(
    name = "{}",
    ...
    """.format(name, srcs, hdrs, glob_hdrs, deps)

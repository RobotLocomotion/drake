    ...
    for abs_dirpath, dirs, files in os.walk(workspace_root):
        ...
        for i, one_dir in reversed(list(enumerate(list(dirs)))):
            if one_dir.startswith("."):
                ...
                dirs.pop(i)
    # Fail fast if our paths are not consistent with a Drake source tree.
    if not (1_000 <= len(relpaths) <= 10_000):
        raise RuntimeError(f"Found {len(relpaths)} paths, wanted ~5000")

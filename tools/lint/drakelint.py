...
def _check_header_using_overloaded(filename):
    """Returns 0 if the file is not a header or doesn't include overloaded.h
    Returns 1 otherwise."""
...
def _check_header_doxygen_file_spelling(filename):
    """Returns 1 if the filename is a header and mis-uses Doxygen's `@file`
    markup. (The `@file` should always be followed immediately by a newline.)
    Returns 0 otherwise."""
    message = (
...
def _check_invalid_line_endings(filename):
    """Returns 0 if all of the newlines in @p filename are Unix, and 1
    otherwise.
    """

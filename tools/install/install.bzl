...
def install_test(
    ...
    This wrapper uses `test/install_tests.py` as its main script. It expects to
    receive one argument which is the location of a file containing the list of
    command to run in the test. The current limitation requires each command to
    contain only one command per line. The file containing the list of command
    is typically `install_tests_script` outputted by the `install()` rule.
    """
    if native.package_name():
        fail("This command should be called only once, when the main installation step occurs.")

    src = "//tools/install:test/install_tests.py"

...
defaults to `false`. Prefer `true` only when you control the build and know
it is recent, because an unstamped build could otherwise be arbitrarily old;
see also `DRAKE_VERSION_IS_UNSTAMPED`.
...
@hideinitializer */
#define DRAKE_VERSION_AT_LEAST(major, minor, patch, yyyymmdd, if_unstamped)
#else

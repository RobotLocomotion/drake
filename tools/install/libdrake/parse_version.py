...
def _check_version(version):
    """Checks if a version string conforms to PEP 440."""
    return (
        re.match(
            r"^([1-9][0-9]*!)?(0|[1-9][0-9]*)"
...
def _split_version(version_full):
    """Validates a full version identifier and splits it into four integer
    parts, padding to four. Any pre-release, 'dev', 'post', and/or local
    identifier (the portion following a '+') is discarded. Raises ValueError if
    the version is invalid or carries an (unsupported) epoch."""
...
def main():
    ...
    version_full = _parse_stamp(args.input_path)
    if version_full is None:

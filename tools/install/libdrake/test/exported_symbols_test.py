...
class ExportedSymbolsTest(unittest.TestCase):
    ...
    def test_exported_symbols(self):
        ...
        if ctors:
            ...
            for function_name in ctors:
                ...
                for i, filename in enumerate(ctors[function_name]):
        ...
        if dtors:
            ...
            for function_name in dtors:
                print(f" {function_name}")
                for i, filename in enumerate(dtors[function_name]):
                    print(f"  called by {filename}")
                print()
    ...
    def _global_ctors_dtors(symbols):
        ...
        constructor or destructor to the filenames that call it."""
        ctors = {}
        dtors = {}
        ...
        for symbol in bad_symbols:
            ...
            if not ctor_calls and not dtor_calls:
                # The constructor got inlined (no calls). Be sure to still
                # report a non-specific error.
                ctor_calls = ["unknown constructor"]
            for function_name in ctor_calls:
                known_bad = _is_known_bad_ctor_or_dtor(
                    filename=filename,
    ...
    def _ctor_dtor_calls(*, symbol, objdump_all):
        """Returns (ctor_calls, dtor_calls) lists of function names that are
        called by this `__GLOBAL_sub_...` symbol."""
        ctor_calls = []
        dtor_calls = []
        ...
        # Remove useless instruction padding the end.

...
def _is_known_bad_ctor_or_dtor(*, filename, function_name):
    ...
    if function_name.startswith("__static_initialization_and_destruction_0"):
        ...
        # fixed all of other bad ctors and dtors, we can circle back here to
        # see if this suppression can also be removed.
        return True
    return False
...
class ExportedSymbolsTest(unittest.TestCase):
    def test_everything(self):
        ...
        bad_symbols = sorted(
            bad_symbols,
            key=lambda x: (getattr(x.type, "name", str(x.type)), x.name),
        )
    ...
    def _find_cxa_atexit_callers(binary):
        """Locates __cxa_atexit in relocation tables and byte-scans .text
        to identify functions containing relative calls to its PLT stub,
        filtering out known bad callers via _is_known_bad_ctor_or_dtor.
        """
        ...
        for i, reloc in enumerate(binary.pltgot_relocations):
            if reloc.has_symbol and reloc.symbol.name == "__cxa_atexit":
                # Header offset is 16 bytes for standard PLT.
                atexit_vma = (
                    binary.get_section(".plt").virtual_address + 16 + (i * 16)
                )
                break
        else:
        ...
        # all calls to the __cxa_atexit PLT. The loop variable "i" an index into
        # text_bytes.
        atexit_call_vmas = []
        ...
        while True:
            ...
            i += 1
        # If there was a relocation, it should have been used *somewhere*.
        assert atexit_call_vmas

        # Map addresses of atexit calls back to names of the calling functions.
        # We'll use a table of (vma_start, vma_end, name) for binary searching.
        func_table = [

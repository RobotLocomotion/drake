...
top of any jupyter notebook that allows users to easily provision the Colab
server with drake iff the notebook is being run on Colab, but which fails fast
when run on a local machine.
"""
...
def setup_drake(*, version, build='nightly'):
    ...
    Note: Possible version names vary depending on the build.
        ...
        - Experimental builds use the version name
    ...
    Note: If you already have pydrake installed to the target location, this
        ...
        will force a reload of that module.
    """
    ...
    if spec is not None and spec.origin != path + '/pydrake/__init__.py':
    ...
    # Check that we now have pydrake on the path.

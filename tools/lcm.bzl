...
def _lcm_srcs_to_outs(lcm_package, lcm_srcs, extension):
    """Return the list of filenames (prefixed by the package directory and switched
    to a new extension), based on the lcm_package= and lcm_srcs= parameters
    (see lcm_cc_library below for a description of what those parameters mean).

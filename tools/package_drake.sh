...
(
  ...
  if [ ${#missing[*]} -gt 0 ]; then
    ...
    echo "Fatal: missing license for ${missing%, }"
    exit 1
  fi
)
...
if [ -z "$install_prefix" ] && [ -z "$output_tarfile" ]; then
  echo "No install directory or output tar file specified."
...
if [ -n "$output_tarfile" ]; then
  create_tarfile "$tmpdir" "$(canonicalize "$output_tarfile")"
fi
if [ -n "$install_prefix" ]; then
  mkdir -p "$install_prefix"
  install_tree "$tmpdir" "$(canonicalize "$install_prefix")"
fi

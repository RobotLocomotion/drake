...
Arguments:
    ...
    shallow_since: Optional date, not after the specified commit. Not allowed
        if a branch or tag is specified.
...
def _setup_git(repo_ctx):
    ...
    #
    # Based on https://git.io/fj1fJ#L24-L55.
    ...
    updated_attrs = update_attrs(repo_ctx.attr, _attrs.keys(), attrs_to_update)
...
def _extract_local_archive(repo_ctx, snopt_path):
    ...
    if repo_ctx.attr.patches:
        repo_ctx.report_progress("Patching archive")

        # Based on https://git.io/fj1vp#L102-L117
        bash = repo_ctx.os.environ.get("BAZEL_SH", "bash")
        for patch in repo_ctx.attr.patches:
            command = "{patch_tool} {patch_args} < {patch}".format(
                patch_tool = repo_ctx.attr.patch_tool,
                patch_args = " ".join([
                    "'%s'" % arg
                    for arg in repo_ctx.attr.patch_args
                ]),
                patch = repo_ctx.path(patch),
            )
            result = execute_and_return(repo_ctx, [bash, "-c", command])
            if result.error:
                return result.error

...
def _prepare_venv(repo_ctx, python):
    ...
    pyproject = repo_ctx.path(Label("@drake//:pyproject.toml")).realpath
    repo_ctx.symlink(pyproject, "pyproject.toml")
    repo_ctx.watch(pyproject)

    pylock = repo_ctx.path(Label("@drake//:pdm.lock")).realpath

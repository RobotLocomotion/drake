...
def repository_python_info(repository_ctx):
    ...
    # TODO(jwnimmer-tri) Remove this once our wheel builds using CMake.
    magic_override = "_DRAKE_WHEEL_BUILD_PYTHON_INTERPRETER_PATH"
    if magic_override in repository_ctx.os.environ:
        python = repository_ctx.os.environ[magic_override]
...
python_repository = repository_rule(
    ...
    local = True,
    environ = [
        # This a private var for Drake's wheel build. End users should use
        # `cmake -DPython_EXECUTABLE=/path/to/desired/python3` instead.
        "_DRAKE_WHEEL_BUILD_PYTHON_INTERPRETER_PATH",
    ],
    configure = True,
)

...
def _impl(repo_ctx):
    ...
    # package will not actually be used. In that case, we'll rely on the
    # deferred reporting inside the generated BUILD file to report the error
    # message later on, when the library is actually depended on.

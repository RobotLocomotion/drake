...
def _impl(repo_ctx):
    ...
    if os_result.is_macos:
        # On macOS, we use fmt from homebrew.
        ...
        # we'll recompile from source the same version as is bundled with the
        # spdlog version we're using (1.5.0, per ../spdlog/repository.bzl).
        ...
        repo_ctx.symlink("/usr/include/spdlog/fmt/bundled", "fmt")
        repo_ctx.symlink(
            Label("@drake//tools/workspace/fmt:bundled.BUILD.bazel"),
            "BUILD.bazel",
        )
        error = None
    else:

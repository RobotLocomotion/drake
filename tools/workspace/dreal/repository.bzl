...
def _impl(repo_ctx):
    ...
    if os_result.is_macos:
        ...
        _rename_so(repo_ctx, "opt/libibex/2.6.5/lib", "libibex.so")
        _rename_so(repo_ctx, "opt/dreal/4.18.02.4/lib", "libdreal.so")
        # Our BUILD file declares this dependency with the revised spelling.
        execute_or_fail(repo_ctx, [
            "patchelf", "--remove-needed", "libibex.so",
            "opt/dreal/4.18.02.4/lib/libdrake_dreal.so",
        ])
...
dreal_repository = repository_rule(
    attrs = {
        ...
        "filenames": attr.string_list(
            default = [
                "d/dreal/dreal_4.18.02.4_amd64.deb",
                "libi/libibex-dev/libibex-dev_2.6.5.20180211084215.gitd1419538b4d818ed1cf21a01896bc5eaae5d1d57~16.04_amd64.deb",  # noqa
            ],
        ),

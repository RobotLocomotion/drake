...
class ContactConfigDialog(QtGui.QDialog):
    ...
    def __init__(self, visualizer, parent=None):
        ...
        self.magnitude_mode.addItems((self.get_mode_string(self.kFixedLength),
                                      self.get_mode_string(self.kScaled),
                                      self.get_mode_string(self.kAutoScale)))
        ...
        self.magnitude_mode.setToolTip(
            ...
            '  - {}: force vectors have fixed length equal to global scale\n'
            '  - {}: simply scaled by global scale\n'
            '  - {}: largest force has fixed length and all other '
            'forces with proportional length on a per-message basis.'.format(
                self.get_mode_string(self.kFixedLength),
                self.get_mode_string(self.kScaled),
                self.get_mode_string(self.kAutoScale)))
    ...
    def get_mode_string(mode):
        if mode == ContactConfigDialog.kFixedLength:
            return "Fixed Length"
        elif mode == ContactConfigDialog.kScaled:
            return "Scaled"
        elif mode == ContactConfigDialog.kAutoScale:
            return "Auto-scale"
        else:
...
class ContactVisualizer(object):
    ...
    def set_text(self):
        ...
        my_text = 'Contact vector: {}'.format(
            ContactConfigDialog.get_mode_string(self.magnitude_mode))

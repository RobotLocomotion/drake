...
class VisualModel:
    ...
    tracks them per unique geometry-geometry contact and allows the visualizer
    to update existing components as messages come in rather than rebuilding
    from scratch. This helps preserve GUI-level configurations (like setting
    something visible)."""
    def __init__(self, root_folder_name: str, view):
        ...
        # the unique identifiers of the geometries in contat.
        self._contacts = {}
    ...
    class _Contact:
        ...
        def set_mesh_data(self, mesh_data: vtk.vtkPolyData, item_name: str,
            ...
            """Updates the visual item for the given `item_name` from the
            provided mesh_data.

            If `mesh_data` is None and the item exists, it is removed.
            If `mesh_data` is *not* None, then the existing item is updated or
            a new item is added.

                Args:
                    mesh_data: The mesh data to register.
                    item_name: The name of the entry.
                    add_callback: Functor of the form `f(item: VisualItem)`
                     called when an item is newly added. It can be used to
                     provide custom configuration.
                    update_callback: Functor of the form `f(item: VisualItem)`
                     called when an item is updated. It can be used to provide
                     custom configuration.
                    view: The director view. Required to add new items."""
            if mesh_data is None:
                if item_name in self.items:
                     om.removeFromObjectModel(self.items.pop(item_name).item)
            elif item_name in self.items:
                vis_item = self.items[item_name]
                update_callback(vis_item)
            else:
                ...
                add_callback(vis_item)

        def set_debug_data(self, item_data: DebugData, item_name: str, view):
            """Sets the item to this contact model. If the item has already
            been defined, it gets replaced. If `item_data` is None, any
            previously registered item with the given name is removed.

                Args:
                    item_data: An instance of DebugData.
    ...
    def update_contact_directories(self, message):
        ...
        """
        existing_contacts = set(self._contacts.keys())
        new_contacts = set()
    ...
    def set_contact_debug_data(self, surface, item_data, item_name):
        """For the given surface, sets the item to the contact model. If the
        item has already been defined, it gets replaced.

            Args:
                surface: The contact surface to which this data belongs.
                item_data: The vtkAppendPolyData to add. If the data is None,
                 or no polygonal data has been added, nothing gets added.
                 However, the previous data *will* be cleared.
    ...
    def set_contact_mesh_data(self, surface, mesh_data, item_name,
        ...
        item has already been defined, it gets replaced.
            ...
            Args:
                surface: The contact surface to which this data belongs.
                mesh_data: The PolyDataItem to add.
                item_name: The unique name associated with this data.
                add_callback: Functor of the form `f(item: VisualItem)`
                 called when an item is newly added. It can be used to
                 provide custom configuration.
...
class HydroelasticContactVisualizer:
    def __init__(self):
        ...
        self.dlg.show_traction_vectors.connect("toggled(bool)",
    ...
    def set_color_map(self, new_index):
        ...
        if new_index != self.color_map_mode:
            ...
            if self.visual_model:
                ...
                self.visual_model.update_items('Mesh edges',
                    lambda vis_item:
    ...
    def update_uv_transform(self, xform):
        ...
        # We want to map the interval [min, max] -> [0, 1]. We set a texture
        # coordinate transform that has the origin at min and scales by the
        # size of the domain (max - min). To avoid divide-by-zero errors, we
        # add a small non-zero quantity to the scale factor.
    ...
    def update_max_pressure(self, pressure):
        ...
        if pressure > self.max_pressure_observed:
            ...
            # self.dlg.max_pressure.setText(f"{pressure:.3g}")
            # self.set_max_pressure()
    ...
    def update_visual_data_from_message(self):
        ...
        #  configuration changes. *Any* change triggers a re-evaluation of *all*
        #  visualization data. It's a simple, yet *huge* hammer. Investigate
        #  making the updating more targeted in the future.
        # Set the color map.
        color_map = self.create_color_map()
        ...
        for surface in msg.hydroelastic_contacts:
            ...
            # to update the visual model explicitly saying whether it is present
            # or not. So, we initialize the data to None in all cases and
            # *always* update self.visual_model. If the data is None, the
            ...
            # TODO(SeanCurtis-TRI): Only define this I'm displaying the surface.
            def add(vis_item):
                # Callback for when the pressure mesh is added for the first
                # time. We subvert the standard pipeline that director uses to
                # inject texture coordinate scaling. Scaling the texture
                # coordinates provides a smoother result.
                item = vis_item.item
                xform = vtk.vtkTransformTextureCoords()
                xform.SetInputData(vtk_polydata_tris)
                self.update_uv_transform(xform)
            ...
            def update(vis_item):
                # In updating the pressure, we can't rely on director to connect
                # the mesh data in correctly, we've stashed the texture coord
                # transform into our user data; we'll connect into that.
                vis_item.item.polyData = vtk_polydata_tris
                vis_item.user_data[0].SetInputData(vtk_polydata_tris)
            self.visual_model.set_contact_mesh_data(
                surface, vtk_polydata_tris, 'Contact surface', add, update)
            ...
            self.visual_model.set_contact_mesh_data(
                ...
                lambda vis_item: vis_item.item.actor.GetProperty().SetColor(line_color),
                lambda vis_item: vis_item.item.setPolyData(vtk_polydata_segs))

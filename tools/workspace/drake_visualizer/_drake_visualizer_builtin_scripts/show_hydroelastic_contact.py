...
def create_texture(min_pressure, max_pressure, texture_size, color_map):
    # Create the texture
    pressures = np.linspace(
        min_pressure, max_pressure, texture_size)
    ...
    for i in range(0, texture_size):
        [r, g, b] = color_map.get_color(pressures[i])
...
class HydroelasticContactVisualizer:
    ...
    def configure_via_dialog(self):
        ...
        if dlg.exec_() == QtGui.QDialog.Accepted:
            ...
            # Rescale the texture map.
            self.texture = create_texture(
                self.min_pressure, self.max_pressure, self.texture_size,
    ...
    def calc_uv(self, pressure):
        ...
        return [min(u, 1-1/self.texture_size), 0]
    ...
    def process_triangles(self, surface):
        ...
        max_num_points = surface.num_triangles*3
        pos = [None] * max_num_points
        uvs = [None] * max_num_points
        ...
        # the contact surface.
        normals = [np.zeros(3)] * max_num_points
        offset_scalar = 1e-4
        ...
        for tri in triangles:
            ...
            if norm_normal >= min_area:
                ...
                if va not in vertex_position_to_id:
                    vertex_position_to_id[va] = vertex_id
                    pos[vertex_id] = va_np
                    uvs[vertex_id] = self.calc_uv(tri.pressure_A)
                    self.max_pressure_observed = max(
                        self.max_pressure_observed, tri.pressure_A)
                    vertex_id += 1
                if vb not in vertex_position_to_id:
                    vertex_position_to_id[vb] = vertex_id
                    pos[vertex_id] = vb_np
                    uvs[vertex_id] = self.calc_uv(tri.pressure_B)
                    self.max_pressure_observed = max(
                        self.max_pressure_observed, tri.pressure_B)
                    vertex_id += 1
                if vc not in vertex_position_to_id:
                    vertex_position_to_id[vc] = vertex_id
                    pos[vertex_id] = vc_np
                    uvs[vertex_id] = self.calc_uv(tri.pressure_C)
                    self.max_pressure_observed = max(
                        self.max_pressure_observed, tri.pressure_C)
                    vertex_id += 1
                ...
                normals[va_id] += normal
                normals[vb_id] += normal
                normals[vc_id] += normal
    ...
    def handle_message(self, msg):
        ...
        for surface in msg.hydroelastic_contacts:
            ...
            if self.show_pressure:
                # Move triangle data to vtk objects.
                ...
                # Add the pressure to the view.
                item_name = 'pressure above {}, {}'.format(
                ...
                item_name = 'pressure below {}, {}'.format(
                    surface.body1_name, surface.body2_name)
            ...
            if self.show_contact_edges:
                ...
                item_name = 'contact edges {}, {}'.format(
                    surface.body1_name, surface.body2_name)

...
# When upgrading the location of upstream sources, check the version header
#  within the file `include/pybind11/detail/common.h` in the branch indicated
#  by `_REPOSITORY` and `_COMMIT`, and if it has changed, then update the
#  version number in the two pybind11-*.cmake files in the current directory to
#  match.

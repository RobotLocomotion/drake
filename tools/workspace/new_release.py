...
def _check_for_upgrades(gh, args, metadata):
    for workspace_name, data in sorted(metadata.items()):
        ...
        if rule_type in _GITHUB_RULE_TYPES:
            ...
            # This is just a pointer to elsewhere; never needs an upgrade.
            continue
        elif rule_type == "pkg_config":
            # Drake can't upgrade OS versions.
            continue
            ...
            # crate_universe_repositories_internal().  Upgrades happen via the

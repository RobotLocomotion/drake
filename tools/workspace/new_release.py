...
(2) Use a GitHub API token:
  ...
  bazel build --config python3 //tools/workspace:new_release
  env API_TOKEN=$(cat ~/.config/readonly_github_api_token.txt) \
    bazel-bin/tools/workspace/new_release
...
# We want to pin these indefinitely, not upgrade them.
_IGNORED_REPOSITORIES = [
    ...
    "godotengine",
    # We don't know how to check non-master branches yet.
    "pybind11",
    # The new_release heuristics get confused by the pycodestyle versioning.
    "pycodestyle",
]
...
def main():
    ...
    if args.use_token and not token:
        parser.error("Missing environment variable API_TOKEN")
    elif not args.user:

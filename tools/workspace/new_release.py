...
def _modified_paths(repo, other):
    return [item.b_path for item in repo.index.diff(other)]


def _is_modified(repo, path):
    if path in _modified_paths(repo, None):
        return True
    if path in _modified_paths(repo, 'HEAD'):
        return True
    return False


def _do_upgrade(temp_dir, gh, repo, workspace_name, metadata):
    ...
    if repo is not None:
        if _is_modified(repo, bzl_filename):
    ...
    if commit:
        repo.git.commit('-o', bzl_filename, '-m', message)
...
def main():
    ...
    if args.commit:
        repo = git.Repo(os.path.realpath("."))
    else:
        repo = None
    ...
    if workspaces is not None:
        for workspace in workspaces:
            with TemporaryDirectory(prefix='drake_new_release_') as temp_dir:
                _do_upgrade(temp_dir, gh, repo, workspace, metadata)
    else:

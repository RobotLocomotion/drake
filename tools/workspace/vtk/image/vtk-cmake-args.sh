...
#
# The two different build flavors (linux, mac) share many common arguments, but
# different (platform specific) arguments such as dependencies must change
# depending on the flavor.  This file serves as the single point of coordination
# between the build flavors.
if [[ "${0}" == "${BASH_SOURCE[0]}" ]]; then
    echo 'vtk-cmake-args.sh: this file must be sourced.' >&2
    exit 1
fi
# shellcheck disable=SC2154
if ! [[ "${drake_vtk_build_flavor}" =~ ^(linux|mac)$ ]]; then
    echo "vtk-cmake-args.sh: the 'drake_vtk_build_flavor' variable must be " >&2
    echo "set to one of 'linux', or 'mac'." >&2
...
elif [[ "${drake_vtk_build_flavor}" == "mac" ]]; then
...
if [[ "${drake_vtk_build_flavor}" == "mac" ]]; then

...
def _vtk_cc_module_impl(
    ...
    deps.extend(gen_hdrs_lib)
    deps.extend(module_deps_public)
    deps.extend(module_deps_private)
    deps.extend(deps_extra)
...
def generate_common_core_array_dispatch_array_list():
    ...
    Generates an `*.h` file.
    """
    name = "Common/Core/vtkArrayDispatchArrayList.h"
...
"""
    generate_file(name = name, content = content)
...
def generate_common_core_type_list_macros():
    ...
    Generates an `*.h` file.
    """
    name = "Common/Core/vtkTypeListMacros.h"
    max = 99
    content = [
        "#pragma once",
        "#include \"vtkTypeList.h\"",
    ...
    content.append("")
    generate_file(name = name, content = "\n".join(content))
...
def generate_common_core_array_instantiations():
    ...
    result = []
    for ctype in (
        # TODO(jwnimmer-tri) XXX On arm, do we need `signed char` here?
        "char",
        "unsigned char",
        "short",
        "unsigned short",
...
#pragma once
VTK_ABI_NAMESPACE_BEGIN
constexpr char {constant_name}[] = R"foo(

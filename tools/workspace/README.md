...
Read the documentation in ``//tools/workspace:new_release``.  If you haven't
yet created GitHub API token per those docs, do that now.
...
``status: commits are properly curated``.  Each external being upgraded
should have exactly one commit that does the upgrade, and each commit
should only impact exactly one external.  If we find any problem with
and upgrade, we need to be able to rever the commit for just that one
external upgrade, leaving the other upgrades intact.
...
Mimic an exising example to complet the process, e.g., look at
`//tools/workspace/tinyobjloader` and minic the `repository.bzl` and

...
the macOS build(s) in Jenkins before merging.
...
Ask on slack if the host version is problematic.
...
wider software ecosystem.  Ask on slack if the host version is problematic.

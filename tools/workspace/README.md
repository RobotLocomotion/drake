...
upgrade procedure every month. Instead, open an issue about the need for an
upgrade of that one specific external, assign it to the feature owner associated
with that external (to find out who that is, you can ask for help in the drake
developers slack channel for ``#build``), and omit it from the monthly upgrade
pull request. The main objective of the monthly upgrade is to ensure that we
stay on top of problematic changes from upstream. If we discover such problems,
we want to bring them to the attention of the feature owner; their steering
should provide the most efficient path to resolve the problem.

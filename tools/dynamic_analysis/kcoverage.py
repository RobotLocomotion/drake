#!/usr/bin/python3
"""kcoverage.py -- mimic coverage.py just enough to shim kcov into bazel
coverage builds.
"""
...
def parse_command_line(argv):
    """Absorb the coverage.py command line. We may ignore most of it.  Returns a
    pair of an argparse.Namespace object, and a list of un-parsed arguments to
...
def exec_with_kcov(exe, exe_args):
    """Run exe with exe_args under kcov, getting the kcov command line prefix from
    the KCOV_COMMAND environment variable.
    """
    kcov_command = os.environ.get('KCOV_COMMAND', '')
    ...
    # KCOV_LINK_PATH and handing the real path to kcov.
    os.environ["KCOV_LINK_PATH"] = exe

...
// inlining their calls are within the same shared.  (In other words, the
// shards draw a partition through the call graph that crosses are few
// performance-critical edges as practical.)  When assigning or updating new
// methods to a shard, try to assign the new method to the same shard as the
// methods it calls.  If it does not need to call anything else, assign it to
// the shard that compiles mostly quickly currently.
...
template Eigen::Transform<double    , 3, 1, 0> RigidBodyTree<double    >::relativeTransform<double    >(const KinematicsCache<double    >&, int, int) const;  // NOLINT
template Eigen::Transform<AutoDiffXd, 3, 1, 0> RigidBodyTree<double    >::relativeTransform<AutoDiffXd>(const KinematicsCache<AutoDiffXd>&, int, int) const;  // NOLINT
template Eigen::Transform<AutoDiffXd, 3, 1, 0> RigidBodyTree<AutoDiffXd>::relativeTransform<AutoDiffXd>(const KinematicsCache<AutoDiffXd>&, int, int) const;  // NOLINT
...
template TwistVector<double    > RigidBodyTree<double    >::geometricJacobianDotTimesV<double    >(const KinematicsCache<double    >&, int, int, int) const;  // NOLINT
template TwistVector<AutoDiffXd> RigidBodyTree<double    >::geometricJacobianDotTimesV<AutoDiffXd>(const KinematicsCache<AutoDiffXd>&, int, int, int) const;  // NOLINT
template TwistVector<AutoDiffXd> RigidBodyTree<AutoDiffXd>::geometricJacobianDotTimesV<AutoDiffXd>(const KinematicsCache<AutoDiffXd>&, int, int, int) const;  // NOLINT

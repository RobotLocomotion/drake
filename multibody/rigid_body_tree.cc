...
void RigidBodyTree<T>::CompileCollisionState() {
  ...
  // Update the collision filter groups for the *compiled* elements and notify
  // the collision model.
  for (const std::pair<const RigidBody<double>*, std::pair<bitmask, bitmask>>&
           item : collision_group_manager_.body_groups()) {
    // This const-cast is a short-cut from going from a pointer to const to a
    // non-const reference. This is the RigidBodyTree and *owns* the body, so
    // it can use the pointer to const to look up a non-const reference. The
    // const_cast simplifies the process.
    RigidBody<double>& body = const_cast<RigidBody<double>&>(*item.first);
...
void RigidBodyTree<T>::CreateCollisionCliques(
  ...
  for (auto& pair : body_collision_map_) {
    ...
    // body pair (P, Q) we'll only process them if P.index < Q.index.
    for (int i = 1; i < static_cast<int>(bodies_.size()); ++i) {
      ...
      if (!test_body.CanCollideWith(body)) {
        ...
        // │2│  0  │  0  │ > 0 │ Pending elements adjacent; add clique B, O.
        // │3│  0  │ > 0 │  0  │ Pending elements adjacent; add clique B, O.
        // │4│  0  │ > 0 │ > 0 │ Pending elements adjacent; add clique B, O.
        // │5│░>░0░│░░0░░│░░0░░│ Only new elements for B; no op.
        // │6│░>░0░│░░0░░│░>░0░│ Compiled elements already adjacent; no op.
        // │7│ > 0 │ > 0 │  0  │ Pending elements adjacent; add clique B, O.
        // │8│░>░0░│░>░0░│░>░0░│ Compiled elements already adjacent; no op.
        //
        // Notes:
        //  1. If I already had compiled collision elements on both bodies, then
        //     I already have cliques to handle that and those cliques get
        //     copied in the previous stage. No further action required.
        //     (rows 6 & 8)
        //  2. If there are no collision elements on O, there is no adjacency
        //     that actually needs to be filtered. (rows 1 & 5)
        ...
        //     collision elements.
        //  4. If cliques on a body with compiled elements are changed, that
  ...
  for (auto& pair : body_collision_map_) {
    ...
    if (element.get_num_cliques() == 0) {
      ...
      if (num_B_Q + num_B_C > 1) {
        // This element hasn't been assigned to *any* cliques.
        int clique_id = get_next_clique_id();

...
// flow. However, since CenicIntegrator is strongly coupled to a specific class
// (MultibodyPlant) without any design intention to operate on any other class,
// using the cast to find the plant needle in the diagram haystack is the best
// choice among the available options. See the #plant_rtti comments below.
template <typename T>
class SystemScanner : public SystemVisitor<T> {
 public:
  ...
  static CenicDiagramStructure<T> ScanAndValidateSystem(
    ...
    // #plant_rtti: RTTI-based control flow is our least-bad option.

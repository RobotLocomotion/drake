...
bool CenicIntegrator<T>::DoStep(const T& h) {
  ...
  ComputeNextContinuousState(model_at_x0_, v_guess, scratch_.x_next_full.get());
  ...
  if (this->get_fixed_step_mode()) {
    ...
    x_next.get_mutable_vector().SetFrom(scratch_.x_next_full->get_vector());
    ...
    v_guess += scratch_.x_next_full->get_substate(plant_subsystem_index_)
                   ...
                   .CopyToVector();
    v_guess /= 2.0;
    ComputeNextContinuousState(model_at_x0_, v_guess,
                               scratch_.x_next_half_1.get());
    ...
    x_next.get_mutable_vector().SetFrom(scratch_.x_next_half_1->get_vector());
    ...
    v_guess = scratch_.x_next_full->get_substate(plant_subsystem_index_)
                  .get_generalized_velocity()
                  .CopyToVector();
    ComputeNextContinuousState(model_at_xh_, v_guess,
                               scratch_.x_next_half_2.get());

    // Set the state to the result of the second half-step (since this is more
    // accurate than the full step, and we have it anyway).
    x_next.get_mutable_vector().SetFrom(scratch_.x_next_half_2->get_vector());
    ...
    err.get_mutable_vector().SetFrom(scratch_.x_next_full->get_vector());
    err.get_mutable_vector().PlusEqScaled(-1.0,
                                          scratch_.x_next_half_2->get_vector());
  }

...
system diagram with a `MultibodyPlant` subsystem, or a `MultibodyPlant` system
alone.
...
class CenicIntegrator final : public systems::IntegratorBase<T> {
 public:
  ...
  @param system The overall system to simulate. Must include exactly one
                continuous-time MultibodyPlant. Other (discrete-time) plants
                are allowed in a diagram. This `system` is aliased by this
 ...
 private:
  ...
  struct Scratch {
    ...
    /* State-sized variables, x = [q; v] for the plant only. */
    VectorX<T> v_guess;
    VectorX<T> q;
    ...
    Note that these states include continuous state for the entire system. */
    /* x_{t+h}. */

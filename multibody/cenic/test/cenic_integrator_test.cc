...
TEST_F(DoublePendulum, Stats) {
  ...
  std::vector<NamedStatistic> initial_stats =
      integrator_->GetStatisticsSummary();
  EXPECT_EQ(ssize(initial_stats), 9);
  std::set<std::string> initial_keys;
  for (const auto& [key, value] : initial_stats) {
    SCOPED_TRACE(fmt::format("initial stat for '{}'", key));
  ...
  EXPECT_EQ(initial_keys, expected_keys);

  simulator_->AdvanceTo(1e-2);
  ...
  for (const auto& [key, value] : later_stats) {
    SCOPED_TRACE(fmt::format("later stat for '{}'", key));
    later_keys.insert(key);
    if (key == "integrator_num_steps_taken") {
      EXPECT_EQ(std::get<int64_t>(value), 1);
    } else if (key == "integrator_actual_initial_step_size_taken") {
      EXPECT_EQ(std::get<double>(value), 0.01);
      ...
      EXPECT_EQ(std::get<int64_t>(value), 3);
  ...
  std::vector<NamedStatistic> reset_stats = integrator_->GetStatisticsSummary();
  EXPECT_EQ(ssize(reset_stats), 9);
  std::set<std::string> reset_keys;
  for (const auto& [key, value] : reset_stats) {
    SCOPED_TRACE(fmt::format("reset stat for '{}'", key));
    reset_keys.insert(key);
    if (std::holds_alternative<int64_t>(value)) {
      EXPECT_EQ(std::get<int64_t>(value), 0);
    } else {
      EXPECT_TRUE(std::isnan(std::get<double>(value)));
  ...
  EXPECT_EQ(reset_keys, expected_keys);
}

/* Checks that solver parameter setting is effective. */
...
/* Verifies that energy conservation for an undamped double pendulum is
improved with tightened accuracy. Also checks that error is reduced with
tightened accuracy. */

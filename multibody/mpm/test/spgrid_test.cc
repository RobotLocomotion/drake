...
GTEST_TEST(SpGridTest, SetPadDataAndGetPadData) {
  ...
  /* Allocate memory for a specific offset to ensure that the pads around it are
   active. */
  Offset center_offset = grid.CoordinateToOffset(0, 0, 0);
...
GTEST_TEST(SpGridTest, IterateGridWithOffset) {
  ...
  grid.IterateGridWithOffset([](SpGrid<GridData>::Offset offset,
                                GridData* node_data) {
    node_data->data = Vector4d(
        static_cast<double>(offset * 1.0), static_cast<double>(offset * 2.0),
        static_cast<double>(offset * 3.0), static_cast<double>(offset * 4.0));
  });
  ...
  for (const auto& offset : offsets) {
    ...
    for (int i = 0; i < 3; ++i) {
      for (int j = 0; j < 3; ++j) {
        for (int k = 0; k < 3; ++k) {
          const Vector3<int> coord = grid.OffsetToCoordinate(offset) +
                                     Vector3<int>(i - 1, j - 1, k - 1);
          const Offset adjusted_offset =
              grid.CoordinateToOffset(coord.x(), coord.y(), coord.z());
          Vector4d expected_value(static_cast<double>(adjusted_offset * 1.0),
                                  static_cast<double>(adjusted_offset * 2.0),
                                  static_cast<double>(adjusted_offset * 3.0),
                                  static_cast<double>(adjusted_offset * 4.0));
  ...
  grid.IterateConstGridWithOffset([](SpGrid<GridData>::Offset offset,
                                     const GridData& node_data) {
    Vector4d expected_value(
        static_cast<double>(offset * 1.0), static_cast<double>(offset * 2.0),
        static_cast<double>(offset * 3.0), static_cast<double>(offset * 4.0));
...
GTEST_TEST(SpGridTest, SetFrom) {
  ...
  grid1.IterateGridWithOffset([](SpGrid<GridData>::Offset offset,
                                 GridData* node_data) {
    node_data->data = Eigen::Vector4<double>(
        static_cast<double>(offset * 1.0), static_cast<double>(offset * 2.0),
        static_cast<double>(offset * 3.0), static_cast<double>(offset * 4.0));
  });
  ...
  EXPECT_EQ(grid1.CoordinateToOffset(0, 0, 0),
            grid2.CoordinateToOffset(0, 0, 0));
}

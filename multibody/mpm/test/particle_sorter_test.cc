...
GTEST_TEST(ConvertToRangesTest, ConvertToRanges) {
  std::vector<int> data = {1, 3, 6, 9};
  Ranges ranges;
  ConvertToRanges(data, &ranges);
...
GTEST_TEST(ParticleSorterTest, Sort) {
  ...
  const std::array<Ranges, 8>& colored_ranges = sorter.colored_ranges();
  for (int c = 0; c < 8; ++c) {
    const Ranges& ranges = colored_ranges[c];

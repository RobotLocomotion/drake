 ...
 public:
  ...
  void Sort(const SpGrid& spgrid, double dx,
    ...
    /* We sort particles first based on their base node offsets in the spgrid;
     ...
     with all the data bits being zero (guaranteed by SPGrid). In addition, a
    ...
    for (int p = 0; p < ssize(particle_sorters_); ++p) {
      ...
      /* Right shift to erase the index bits, and then left shifts to recover
       the zero data bits in base node offsets (as required by SPGrid). */

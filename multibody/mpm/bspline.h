...
/* BsplineWeights computes and stores the weights between a point x and the
 ...
 quadratic B-spline kernel. The kernel is defined as:
    ...
    N(d) = 0.75 - d²,           if 0 <= |d| <= 0.5
           0.5 * (1.5 - |d|)²,  if 0.5 < |d| <= 1.5

 where d is the distance between the point and the grid node in the reference
 frame. The kernel is non-zero in the interval [-1.5, 1.5] (in reference frame).
...
class BsplineWeights {
 public:
  ...
  BsplineWeights(const Vector3<T>& x, const T& dx) {
    ...
    Compute(x_reference[0], base_node[0], &data_[0]);
    Compute(x_reference[1], base_node[1], &data_[1]);
    Compute(x_reference[2], base_node[2], &data_[2]);
  }

  /* Returns between the point and the ijk-th grid node in its support.
   @pre 0 <= i, j, k < 3. */
 ...
 private:
  ...
  void Compute(const T& x_reference, int base_node, Vector3<T>* result) const {
    const T d0 = x_reference - static_cast<T>(base_node);
    const T d1 = static_cast<T>(0.5) - d0;
    (*result)(0) = static_cast<T>(0.5) * d1 * d1;
    (*result)(1) = static_cast<T>(0.75) - d0 * d0;
    const T d2 = static_cast<T>(0.5) + d0;
    (*result)(2) = static_cast<T>(0.5) * d2 * d2;
  }

...
class SparseGrid {
 public:
  ...
  /* Iterates through all particles and the grid nodes supported by the
   particles and apply the given kernel. Potentially writes to those grid nodes
   too upon the request of the `write_to_grid` flag.
    @param[in] particle_data  The particle data to iterate over.
    @param[in] kernel         The kernel to apply to the particles and the grid
                              nodes supported by the particles. For each
                              particle the kernel is called with the following
                              arguments:
                              - The pad of grid nodes supported by the particle.
                              - The pad of grid data in the support.
                              - The particle data.
                              - The index of the particle in the particle data.
   @pre Allocate() has been called with the positions of the given
   `particle_data`. */
  ...
  /* Iterates over all particles and the grid nodes supported by them, applying
   ...
   For each particle, it:
     - Retrieves the pad (i.e. block) of grid nodes and grid data that support
       that particle.
  ...
  /* Iterates over all particles and the grid nodes supported by them,
   ...
   This method is similar to Iterate(), but it is intended for cases where the
   particle data is read-only and the grid data is mutable.
   It accepts a const reference to the particle data and calls a kernel
   that receives a const pointer to that data. In this mode, the grid is
   automatically updated with any modifications made to the grid data.

   For each particle, the method:
    - Retrieves the pad (block) of grid nodes and grid data that support that
      particle.
    - Invokes the provided kernel function with:
      - A const reference to the pad of grid nodes (of type Pad<Vector3<T>>).
      - A pointer to the pad of grid data (of type Pad<GridData<T>>*).

...
class ElementCollection final {
 public:
  ...
  /* In support of MbT cloning, imitates the effect of calling Add() at the
  next_index() and then immediately calling Remove() on the added element.
  In practice this means adding a null element at the end, but that is an
 ...
 private:
  ...
  /* Given a (name, index) pair, returns the iterator into `names_` that matches
  those values, or else `names_.end()` when no match is found. */
  std::unordered_multimap<StringViewMapKey, Index>::const_iterator

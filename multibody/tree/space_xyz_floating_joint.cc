...
SpaceXYZFloatingJoint<T>::TemplatedDoCloneToScalar(
  ...
  auto joint_clone = std::make_unique<SpaceXYZFloatingJoint<ToScalar>>(
...
template <typename T>
std::unique_ptr<Joint<double>> SpaceXYZFloatingJoint<T>::DoCloneToScalar(
    const internal::MultibodyTree<double>& tree_clone) const {
  return TemplatedDoCloneToScalar(tree_clone);
}

template <typename T>
std::unique_ptr<Joint<AutoDiffXd>> SpaceXYZFloatingJoint<T>::DoCloneToScalar(
...
SpaceXYZFloatingJoint<T>::DoCloneToScalar(
...
SpaceXYZFloatingJoint<T>::MakeImplementationBlueprint() const {
  auto blue_print = std::make_unique<typename Joint<T>::BluePrint>();
  auto quaternion_floating_mobilizer =
      std::make_unique<internal::SpaceXYZFloatingMobilizer<T>>(
...
template <typename T>
void SpaceXYZFloatingJoint<T>::DoAddInOneForce(const systems::Context<T>&,
                                                 int, const T&,
...
template <typename T>
void SpaceXYZFloatingJoint<T>::DoAddInDamping(

 ...
 public:
  ...
  MobilizerIndex add_mobilizer(
    ...
    if (bodies_[outboard_body].inboard_mobilizer.is_valid()) {
      throw std::runtime_error(
          "When creating a multibody tree, an attempt was made to add two "
          "inboard joints (mobilizers) to the same body; this is not allowed. "
          "One possible cause might be attempting to weld a robot to the world "
          "somewhere other than its base link; see Drake issue #17429 for "
          "discussion and work-arounds, e.g., reversing some joint directions. "
          "Another possible cause might be attempting to form a kinematic loop "
          "using joints; to create a loop, consider using a "

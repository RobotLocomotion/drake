...
// - Indexes to the inboard/outboard RigidBodies of this mobilizer.
...
struct MobilizerTopology {
  ...
  // Returns `true` if this Mobilizer connects these RigidBodies.
...
struct BodyNodeTopology {
  ...
  //                       `rigid_body_in`. In other words, `parent_body_in` is
  //                       the body associated with node `parent_node_in`.
  BodyNodeTopology(MobodIndex index_in, int level_in, MobodIndex parent_node_in,
                   BodyIndex rigid_body_in, BodyIndex parent_body_in,
  ...
  BodyIndex parent_body;  // This node's parent RigidBody P.
...
class MultibodyTreeTopology {
 public:
  ...
  // Returns the number of RigidBodies in the MultibodyPlant. This includes the
  // World RigidBody and therefore the minimum number of rigid bodies after

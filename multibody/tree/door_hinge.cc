...
namespace multibody {
namespace {
// The curv `s` approximates a step curv {x<0: -1 ; x>0: 1} outside of
// -threshold < x < threshold. The curv `singlet` is the first derivative of `s`
// scaled by t, which is a hump at 0 that integrates to 2. The curv `doublet`
// is the second derivative of `s` scaled by t^2, which yields a lump at
// negative x that integrates to -1 and a lump at positive x that integrates
// to 1.
template <typename T>
std::tuple<T, T, T> CalcApproximationCurvs(T t, T x) {
  constexpr double kMinThreshold = 1e-6;
  DRAKE_DEMAND(t > kMinThreshold);
  const T s = tanh(x / t);
...
T DoorHinge<T>::CalcHingeFrictionalTorque(T angular_velocity,
  ...
  if (config.motion_threshold == 0) {
    ...
    const auto [s, singlet, doublet] =
        CalcApproximationCurvs(T(config.motion_threshold), angular_velocity);
...
T DoorHinge<T>::CalcHingeSpringTorque(T angle,
  ...
  if (config.catch_width == 0) {
    ...
    const auto [s, singlet, doublet] =
        CalcApproximationCurvs(catch_center, angle - catch_center);
    drake::unused(s);
    drake::unused(singlet);
...
// Stored engergy consists two parts. One is from the spring and another one is
// from the catch torque. The catch torque engergy is the integration of the
// catch torque over the angle that the catch torque is effective, i.e.
// E_{catch_torque} = config.catch_torque * ∫doublet(t, x))dx, and it's easy to
// see ∫doublet(t, x))dx = 1/t * singlet(t, x).
template <typename T>

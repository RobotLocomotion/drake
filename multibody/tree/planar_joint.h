...
class PlanarJoint final : public Joint<T> {
 public:
  ...
  void set_default_position(const Vector2<double>& p_FoMo_F) {
  ...
  /// @param[in] position The desired default position of the joint
  /// @param[in] angle The desired default angle of the joint
  void set_default_pose(const Vector2<double>& p_FoMo_F, const double& angle) {
    Vector3<double> state(p_FoMo_F[0], p_FoMo_F[1], angle);
    this->set_default_positions(state);
  }

  /// Sets the random distribution that the position and angle of this joint
  /// will be randomly sampled from. See class documentation for details on the
  /// definition of the position and angle.
  void set_random_pose_distribution(
      const Vector2<symbolic::Expression>& p_FoMo_F,
      const symbolic::Expression& angle) {
    Vector3<symbolic::Expression> state(p_FoMo_F[0], p_FoMo_F[1], angle);

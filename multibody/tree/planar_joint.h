...
/// Translations (x, y) are defined to be positive in the direction of their
...
class PlanarJoint final : public Joint<T> {
 public:
  ...
  /// degrees and N⋅m⋅s for the roational degrees. The damping force (in N)
  /// is modeled as `fᵢ = -dampingᵢ⋅vᵢ, i = 1, 2` i.e. opposing motion,
  /// with vᵢ the translation rates along the i-th axis for `this` joint (see
  /// get_translation_rates()) and fᵢ the force on child body B along the same
  /// i-th axis.  The damping torque (in N⋅m) is modeled as `τ = -damping₃⋅ω`
  /// i.e. opposing motion, with ω the angular rate for `this` joint (see
  /// get_angular_rate()) and τ the torque on child body B about the same axis.
  Vector3<double> damping() const { return damping_; }

  /// @name Context-dependent value access
  /// @{

  /// Gets the translations of `this` joint from `context`. See class
  /// documentation for the definition of these translations.
  ///
  /// @param[in] context The context of the model this joint belongs to.
  /// @returns The translations of `this` joint stored in the `context` ordered
  ///          as (x, y).
  Vector2<T> get_translations(const Context<T>& context) const {
    return get_mobilizer()->get_translations(context);
  }

  /// Sets the `context` so that the generalized coordinates corresponding to
  /// the translations of `this` joint equals `translations`.
  /// @param[in] context The context of the model this joint belongs to.
  /// @param[in] translations The desired translations in meters to be stored in
  ///                         `context` ordered as (x, y). See class
  ///                         documentation for details.
  /// @returns a constant reference to `this` joint.
  const PlanarJoint<T>& set_translations(Context<T>* context,
                                         const Vector2<T>& translations) const {
    get_mobilizer()->set_translations(context, translations);
    return *this;
  }
  ...
  /// Gets the rates of change, in meters per second, of `this` joint's
  /// translations (see class documentation) from `context`.
  /// @param[in] context The context of the model this joint belongs to.
  /// @returns The rates of change of `this` joint's translations as stored in
  ///          the `context`.
  Vector2<T> get_translation_rates(const systems::Context<T>& context) const {
    return get_mobilizer()->get_translation_rates(context);
  }

  /// Sets the rates of change, in meters per second, of this `this` joint's
  /// translations (see class documentation) to `translations_dot`. The new
  /// rates of change get stored in `context`.
  /// @param[in] context The context of the model this joint belongs to.
  /// @param[in] translations_dot The desired rates of change of `this` joints's
  ///                             translations in meters per second.
  /// @returns a constant reference to `this` joint.
  const PlanarJoint<T>& set_translation_rates(
      systems::Context<T>* context, const Vector2<T>& translations_dot) const {
    get_mobilizer()->set_translation_rates(context, translations_dot);
    return *this;
  }
  ...
  ///                  See class documentation for details.
  /// @returns a constant reference to `this` mobilizer.
  const PlanarJoint<T>& set_angular_rate(systems::Context<T>* context,
  ...
  /// Gets the default positions for `this` joint. Wrapper for the more
  /// general `Joint::default_positions()`.
  /// @returns The default position of `this` stored in `default_positions_`
  ...
  void set_default_position(const Vector3<double>& position) {
    this->set_default_positions(position);
    if (this->has_implementation()) {
      get_mutable_mobilizer()->set_default_position(this->default_positions());
    }
  }

  /// Sets the random distribution that positions of this joint will be
 ...
 protected:
  ...
  void DoAddInDamping(const systems::Context<T>& context,
    ...
    Eigen::VectorBlock<Eigen::Ref<VectorX<T>>> tau =
        get_mobilizer()->get_mutable_generalized_forces_from_array(
            &forces->mutable_generalized_forces());
    const Vector2<T>& v_translation = get_translation_rates(context);
    const T& v_angular = get_angular_rate(context);

...
/// joint allows frame M to translate with resepct to frame F along the x- and
/// y-axes of frame F and to rotate with repect to frame F about the z-axis of
/// frame F. The translations along the x- and y-axes of F, the rotation about
/// the z-axis of F and their rates, specify the state of the joint.
/// Zero (x, y, θ) corresponds to frames F and M being coincident and aligned.
/// Position (x, y) is defined to be positive in the direction of the respective
/// axes and the rotation θ is defined to be positive according to the
/// right-hand-rule with the thumb aligned in the direction of frame F's z-axis.
///
...
class PlanarJoint final : public Joint<T> {
 public:
  ...
  /// The name for this Joint type.  It resolves to "planar".
  ...
  /// translate and rotate as described in the class's documentation. See class
  ...
  /// degrees and N⋅m⋅s for the roational degree. The damping force (in N) is
  /// modeled as `fᵢ = -dampingᵢ⋅vᵢ, i = 1, 2` i.e. opposing motion, with vᵢ
  /// the translation rates along the i-th axis for `this` joint (see
  /// get_translation_rates()) and fᵢ the force on child body B at Mo and
  ...
  /// rate for `this` joint (see get_angular_rate()) and τ the torque on child
  /// body B expresed in frame F as t_B_F = τ⋅Fz_F.
  Vector3<double> damping() const { return damping_; }

  /// @name Context-dependent value access
  /// @{

  /// Gets the position of `this` joint from `context`. See class documentation
  /// for the definition of these position.
  ///
  /// @param[in] context The context of the model this joint belongs to.
  /// @returns The position of `this` joint stored in the `context` ordered
  ///          as (x, y).
  Vector2<T> get_position(const Context<T>& context) const {
    return get_mobilizer()->get_translations(context);
  }

  /// Sets the `context` so that the generalized coordinates corresponding to
  /// the position of `this` joint equals `p_FoMo_F`.
  /// @param[in] context The context of the model this joint belongs to.
  /// @param[in] p_FoMo_F The desired position in meters to be stored in
  ///                     `context` ordered as (x, y). See class documentation
  ///                     for details.
  /// @returns a constant reference to `this` joint.
  const PlanarJoint<T>& set_position(Context<T>* context,
                                     const Vector2<T>& p_FoMo_F) const {
    get_mobilizer()->set_translations(context, p_FoMo_F);
    return *this;
  }

  /// Gets the angle θ of `this` joint from `context`.  See class documentation
  /// for the definition of this angle.
  ///
  /// @param[in] context The context of the model this joint belongs to.
  /// @returns The angle θ of `this` joint stored in the `context`.
  const T& get_angle(const systems::Context<T>& context) const {
    return get_mobilizer()->get_angle(context);
  }

  /// Sets the `context` so that the generalized coordinate corresponding to
  /// the angle θ of `this` joint equals `angle`.
  ///
  /// @param[in] context The context of the model this joint belongs to.
  /// @param[in] angle The desired angle in radians to be stored in `context`.
  ///                  See class documentation for details.
  /// @returns a constant reference to `this` mobilizer.
  const PlanarJoint<T>& set_angle(systems::Context<T>* context,
                                  const T& angle) const {
  ...
  /// Sets the `context` so that the generalized coordinate corresponding to
  /// the position of `this` joint equals `p_FoMo_F` and those corresponding to
  /// the angle θ of `this` joint equals `angle`.
  ///
  /// @param[in] context The context of the model this joint belongs to.
  /// @param[in] p_FoMo_F The desired position in meters to be stored in
  ...
  /// Gets the velocity v_FoMo_F, in meters per second, of `this` joint's Mo
  /// measured and expressed in frame F from `context`.
  /// @param[in] context The context of the model this joint belongs to.
  /// @returns The velocity v_FoMo_F of `this` joint as stored in the `context`.
  Vector2<T> get_translational_velocity(
      const systems::Context<T>& context) const {
    return get_mobilizer()->get_translation_rates(context);
  }

  /// Sets the velocity, in meters per second, of this `this` joint's Mo
  /// measured and expressed in frame F  to `v_FoMo_F`. The new velocity gets
  /// stored in `context`.
  /// @param[in] context The context of the model this joint belongs to.
  /// @param[in] v_FoMo_F The desired velocity of `this` joint in meters per
  ///                     second.
  /// @returns a constant reference to `this` joint.
  ...
  /// Gets the rate of change, in radians per second, of `this` joint's angle θ
  /// from `context`.  See class documentation for the definition of this angle.
  ///
  /// @param[in] context The context of the model this joint belongs to.
  /// @returns The rate of change of `this` joint's angle θ as stored in the
  ///          `context`.
  const T& get_angular_velocity(const systems::Context<T>& context) const {
    return get_mobilizer()->get_angular_rate(context);
  }

  /// Sets the rate of change, in radians per second, of `this` joint's angle θ
  /// (see class documentation) to `theta_dot`. The new rate of change gets
  /// stored in `context`.
  ///
  /// @param[in] context The context of the model this joint belongs to.
  /// @param[in] angle The desired rates of change of `this` joint's angle in
  ///                  radians per second.
  /// @returns a constant reference to `this` mobilizer.
  ...
  /// Gets the default position for `this` joint. Wrapper for the more
  /// general `Joint::default_positions()`.
  /// @returns The default position of `this` joint.
  Vector2<double> get_default_position() const {
    return this->default_positions().head(2);
  }

  /// Sets the default position of this joint.
  /// @param[in] position The desired default position of the joint
  void set_default_position(const Vector2<double>& position) {
    Vector3<double> state(position(0), position(1),
                          this->default_positions()[2]);
    this->set_default_positions(state);
  }

  /// Gets the default angle for `this` joint.
  /// @returns The default angle of `this` joint.
  double get_default_angle() const { return this->default_positions()[2]; }

  /// Sets the default angle of this joint.
  /// @param[in] angle The desired default angle of the joint
  void set_default_angle(const double& angle) {
  ...
  /// @param[in] position The desired default position of the joint
  /// @param[in] angle The desired default angle of the joint
  void set_default_pose(const Vector2<double>& position, const double& angle) {
    Vector3<double> state(position(0), position(1), angle);
    this->set_default_positions(state);
  }

  // /// Sets the random distribution that the position of this joint will be
  // /// randomly sampled from. See class documentation for details on the
  // /// definition of the position.
  // void set_random_position_distribution(
  //     const Vector2<symbolic::Expression>& position) {
  //   // get_mutable_mobilizer()->set_random_position_distribution(
  //   //     Vector2<symbolic::Expression>{position});
  // }

  // /// Sets the random distribution that the angle of this joint will be
  // /// randomly sampled from. See class documentation for details on the
  // /// definition of the angle.
  // void set_random_angle_distribution(const symbolic::Expression& angle) {
  //   // get_mutable_mobilizer()->set_random_position_distribution(
  //   //     Vector3<symbolic::Expression>{position});
  // }
 ...
 protected:
  ...
  /// translational acceleration and a positve torque causes a positive angular
...
template <typename T>
const char PlanarJoint<T>::kTypeName[] = "planar";

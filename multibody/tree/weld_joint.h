...
class WeldJoint final : public Joint<T> {
 public:
  ...
  /// Constructor for a %WeldJoint between a @p frame_on_parent (P) and a @p
  /// frame_on_child (C) so that their relative pose @p X_PC is fixed as if
  /// they were "welded" together.
  WeldJoint(const std::string& name, const Frame<T>& frame_on_parent,

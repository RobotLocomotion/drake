...
class BodyNode : public MultibodyElement<T> {
 public:
  ...
  // - Called on the _root_ node.
  // - `ac` is nullptr.
  // @param[in] parameters All of the context parameters for MultibodyTree
  //   model.
  // @param[in] all_positions All of the generalized positions q for the
  //   MultibodyTree model.
  // @param[in] all_velocities All of the generalized velocities v for the
  //   MultibodyTree model.
  // @param[in] pc
  //   An already updated position kinematics cache in sync with `context`.
  // @param[in] vc
  ...
  void CalcSpatialAcceleration_BaseToTip(
    ...
    const math::RigidTransform<T> X_MB =
        frame_M.CalcPoseInBodyFrame(parameters).inverse();
    ...
    SpatialAcceleration<T> A_FM =
        get_mobilizer().CalcAcrossMobilizerSpatialAcceleration(
            all_positions, all_velocities, vmdot);
  ...
  void CalcInverseDynamics_TipToBase(
    ...
    const math::RigidTransform<T> X_BM =
        frame_M.CalcPoseInBodyFrame(parameters);
    ...
    for (const BodyNode<T>* child_node : children_) {
      ...
      const math::RigidTransform<T> X_CMc =
          frame_Mc.CalcPoseInBodyFrame(parameters);
    ...
    const math::RotationMatrix<T> R_PF =
        inboard_frame().CalcRotationMatrixInBodyFrame(parameters);
    const math::RotationMatrix<T>& R_WP = get_R_WP(pc);
    ...
    get_mobilizer().ProjectSpatialForce(all_positions, F_BMo_F, tau);
  ...
  void CalcSpatialAccelerationBias(const systems::Context<T>& context,
    ...
    const SpatialAcceleration<T> Ab_FM =
        get_mobilizer().CalcAcrossMobilizerSpatialAcceleration(
            ...
            this->get_parent_tree().get_velocities(context), vmdot_zero);
 ...
 private:
  ...
  void CalcAcrossMobilizerBodyPoses_BaseToTip(
    ...
    X_PB = frame_F.CalcOffsetPoseInBody(context.get_parameters(), X_FB);

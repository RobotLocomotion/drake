...
class BodyNode : public MultibodyElement<BodyNode, T, BodyNodeIndex> {
 public:
  ...
  void CalcSpatialAccelerationBias(const systems::Context<T>& context,
    ...
    // The caller of this method is ultimately trying to compute Ab_WB = Ac_WB +
    // Ab_PB_W, of which Ab_PB (that we're computing here) is a component. See
    // @ref abi_computing_accelerations.
    // Now, Ab_PB_W is the bias term for the acceleration A_PB_W. That is, it is
    // the acceleration A_PB_W when vmdot is zero. We get Ab_PB_W by shifting
    // and re-expressing Ab_FM, the across mobilizer spatial acceleration bias
    // term.
 ...
 private:
  ...
  const SpatialForce<T>& get_Zplus_PB_W(
      const ArticulatedBodyForceCache<T>& aba_force_bias_cache) const {

...
class BodyNode : public MultibodyElement<T> {
 public:
  ...
  BodyNode(const BodyNode<T>* parent_node, const Body<T>* body,
  ...
  void CalcInverseDynamics_TipToBase(
    ...
    // std::cout << "CalcInverseDynamics_TipToBase() for body " << body().name() << std::endl;
    ...
    // std::cout << "  F_BMo_W start= " << F_BMo_W << std::endl << std::endl;
    for (const BodyNode<T>* child_node : children_) {
      ...
      // std::cout << "  F_CMc_W = " << F_CMc_W << std::endl << std::endl;
      ...
      // std::cout << "  F_CMo_W = " << F_CMo_W << std::endl << std::endl;
    ...
    // std::cout << "  F_BMo_W after Fapplied_Bo_W subtraction = " << F_BMo_W << std::endl << std::endl;
    ...
    // std::cout << "  F_BMo_F = " << F_BMo_F << std::endl << std::endl;
    ...
    // std::cout << "  tau = " << fmt::format("{}",fmt_eigen(tau)) << std::endl << std::endl;

    // Include the contribution of applied generalized forces.
    if (tau_applied.size() != 0) tau -= tau_applied;

    // std::cout << "  tau after tau_applied subtraction = " << fmt::format("{}",fmt_eigen(tau)) << std::endl << std::endl;
 ...
 private:
  ...
  Eigen::VectorBlock<const VectorX<T>> get_mobilizer_velocities(
    ...
    return this->get_parent_tree().get_state_segment(
        context, topology_.mobilizer_velocities_start_in_state,
  ...
  void CalcBodySpatialForceGivenItsSpatialAcceleration(
    ...
    // std::cout << "Ftot_BBo_W = " << Ftot_BBo_W << std::endl << std::endl;
    ...
    if (Fb_Bo_W_cache != nullptr) {
      ...
      // std::cout << "Fb_Bo_W = " << Fb_Bo_W << std::endl << std::endl;
      Ftot_BBo_W += Fb_Bo_W;
      // std::cout << "Ftot_BBo_W after Fb_Bo_W addition = " << Ftot_BBo_W << std::endl << std::endl;
    }
  }

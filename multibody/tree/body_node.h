...
class BodyNode : public MultibodyElement<BodyNode, T, BodyNodeIndex> {
 public:
  ...
  void CalcSpatialAccelerationBias(const systems::Context<T>& context,
    ...
    // V_PF = 0. Therefore, DtP(V_PB) = DtF(V_PB). Since M and B are also on the
    // same rigid body, V_MB = 0. We then recognize that
    // V_PB = V_PFb + V_FMb + V_MB = V_FMb. Together, we get that
    // A_PB = DtF(V_FMb) = A_FM.Shift(p_MoBo, w_FM).

    // We first Compute H_FM_bias = Hdot * vm and shift it to Bo to obtain
    // Ab_PB_W.
    // Note, A_FM = H_FM(qm) * vmdot + H_FM_bias(qm, vm).

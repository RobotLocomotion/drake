...
class BodyNode : public MultibodyTreeElement<BodyNode<T>, BodyNodeIndex> {
 public:
  ...
  void CalcSpatialAcceleration_BaseToTip(
    ...
    const math::RigidTransform<T> X_MB = frame_M.CalcPoseInBodyFrame(context).inverse();
  ...
  void CalcInverseDynamics_TipToBase(
    ...
    for (const BodyNode<T>* child_node : children_) {
      ...
      const math::RigidTransform<T> X_CMc = frame_Mc.CalcPoseInBodyFrame(context);
    ...
    const math::RigidTransform<T> X_PF = inboard_frame().CalcPoseInBodyFrame(context);
    const math::RigidTransform<T>& X_WP = get_X_WP(pc);
 ...
 private:
  ...
  const math::RigidTransform<T>& get_X_WB(const PositionKinematicsCache<T>& pc) const {
    return pc.get_X_WB(topology_.index);
  }

  // Mutable version of get_X_WB().
  math::RigidTransform<T>& get_mutable_X_WB(PositionKinematicsCache<T>* pc) const {
    return pc->get_mutable_X_WB(topology_.index);
  }

  // Returns a const reference to the pose of the parent body P measured and
  // expressed in the world frame W.
  const math::RigidTransform<T>& get_X_WP(const PositionKinematicsCache<T>& pc) const {
  ...
  const math::RigidTransform<T>& get_X_FM(const PositionKinematicsCache<T>& pc) const {
    return pc.get_X_FM(topology_.index);
  }

  // Returns a mutable reference to the across-mobilizer pose of the outboard
  // frame M as measured and expressed in the inboard frame F.
  math::RigidTransform<T>& get_mutable_X_FM(PositionKinematicsCache<T>* pc) const {
  ...
  const math::RigidTransform<T>& get_X_PB(const PositionKinematicsCache<T>& pc) const {
    return pc.get_X_PB(topology_.index);
  }

  // Returns a mutable reference to the pose of body B as measured and expressed
  // in the frame of the parent body P.
  math::RigidTransform<T>& get_mutable_X_PB(PositionKinematicsCache<T>* pc) const {
  ...
  void CalcAcrossMobilizerBodyPoses_BaseToTip(
    ...
    const math::RigidTransform<T> X_MB = frame_M.CalcPoseInBodyFrame(context).inverse();
    const math::RigidTransform<T>& X_FM = get_X_FM(*pc);  // mobilizer.Eval_X_FM(ctx)
    const math::RigidTransform<T>& X_WP = get_X_WP(*pc);  // body_P.EvalPoseInWorld(ctx)

    // Output (updating a cache entry):
    // - X_PB(qf_P, qr_B, qf_B)
    // - X_WB(q(W:P), qf_P, qr_B, qf_B)
    math::RigidTransform<T>& X_PB = get_mutable_X_PB(pc);
    math::RigidTransform<T>& X_WB = get_mutable_X_WB(pc);  // body_B.EvalPoseInWorld(ctx)

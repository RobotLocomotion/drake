...
class BodyNode : public MultibodyElement<BodyNode, T, BodyNodeIndex> {
 public:
  ...
  void CalcSpatialAccelerationBias(const systems::Context<T>& context,
    ...
    // Ultimately, We want to compute A_PB = DtP(V_PB).
    // Now, Ab_PB_W (the actual output of this method) is the bias term for the
    ...
    // We first Compute Ab_FM = Hdot * vm and shift it to Bo to obtain Ab_PB_W.
    ...
    // Finally, this is also true when vmdot = 0, and thus for the bias Ab_PB_W.

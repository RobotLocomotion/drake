...
class BodyNode : public MultibodyElement<T> {
 public:
  ...
  // There are six helpers, one for each possible size Rnv of body R(k)'s
  // inboard mobilizer (welds don't contribute here). This allows us to use
  // fixed-size 2d matrices in the implementation as we sweep the inboard
  // bodies. We define a templatized dispatcher class
  // CalcMassMatrixOffDiagonalDispatcher below that body R(k) can use to make
  // the right call.
#define DECLARE_MASS_MATRIX_HELPER(Rnv)                                 \
  virtual void CalcMassMatrixOffDiagonalHelper##Rnv(                    \
...
#undef DECLARE_MASS_MATRIX_HELPER
  ...
  virtual void CalcCompositeBodyInertia_TipToBase(
      ...
      std::vector<SpatialInertia<T>>* Mc_B_W_all) const = 0;
  ...
  class CalcMassMatrixOffDiagonalDispatcher<T, Rnv> {                    \
   public:                                                               \
    static void Dispatch(const BodyNode<T>& body_node, int R_start_in_v, \
      ...
      body_node.CalcMassMatrixOffDiagonalHelper##Rnv(                    \

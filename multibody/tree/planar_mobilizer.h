...
/// outboard frame M that enables tranlation along F's x and y axes and rotation
/// about F's z-axis.
///
/// The generalized coordinates for this mobilizer  (x, y, θ) correspond to
/// translations along the x and y axes of frame F and rotaition about the
/// z-axis of frame F.
/// Zero (x, y, θ) define the "zero configuration" which corresponds to frame F
/// and M being coincident and aligned, see set_zero_state(). The translations
/// (x, y) are defined to be positive in the direction of their respective axes
/// and the rotation θ is defined to be positive according to the
...
class PlanarMobilizer final : public MobilizerImpl<T, 3, 3> {
 public:
  ...
  /// two translational and one rotational degrees of freedom as described in
  ...
  /// (x, y) provided in the input argument `translations`, which stores them
  /// with the format `translations = [x, y]`.
  ///
  /// @param[in] context The context of the model this mobilizer belongs to.
  /// @param[in] translations A vector which must pack values for the
  ///                         translations (x, y) described in this class's
  ///                         documentation, at entries `translations(0)` and
  ///                         `translations(1)`, respectively.
  ...
  /// `this` mobilizer's translations (see get_translations()) to
  /// `translations_dot`.
  /// @param[in] context The context of the model this mobilizer belongs to.
  /// @param[in] translations_dot The desired rate of change of `this`
  ///                             mobilizer's translations, packed as the
  ///                             vector [ẋ, ẏ].
  /// @returns A constant reference to `this` mobilizer.
  ...
  /// `this` mobilizer's angle (see angle()) to `theta_dot`.
  /// @param[in] context The context of the model this mobilizer belongs to.
  ...
  /// The acceleration `A_FM` will be a function of the configuration q and the
  ...
  /// outboard frame M onto the rotational axis, z, and  translational axes,
  /// x and y. Mathematically:
  /// <pre>
  ///    tau = [f_Mo⋅Fx]
  ///          [f_Mo⋅Fy]
  ///          [τ_Mo⋅Fz]
  /// </pre>
  /// Therefore, the result of this method is the vector of torques and forces
  /// for each degree of freedom of `this` mobilizer.

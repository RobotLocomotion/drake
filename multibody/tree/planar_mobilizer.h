...
class PlanarMobilizer final : public MobilizerImpl<T, 3, 3> {
 public:
  ...
  /** Sets in `context` the rate of change, in meters per second, of `this`
   mobilizer's translations (see get_translations()) to `translations_dot`.
   @param[in] context The context of the model this mobilizer belongs to.
   @param[in] translations_dot The desired rate of change of `this` mobilizer's
  ...
  /** Projects the spatial force `F_Mo = [τ_Mo, f_Mo]` on `this` mobilizer's
   outboard frame M onto the rotational axis, z, and  translational axes, s and
   y. Mathematically:
   <pre>
      tau = [f_Mo⋅Fx]
            [f_Mo⋅Fy]

...
parent/child direction, we instead choose M=Jp and an offset from F
    X_JcF = (X_JpJc)⁻¹
so that X_FM = X_FJp = (X_JcF)⁻¹ (X_JpJc)⁻¹ = X_Identity.

If X_JpJc is already X_Identity we just use the original frames for F and M. */
template <typename T>
std::unique_ptr<internal::Mobilizer<T>> WeldJoint<T>::MakeMobilizerForJoint(
  ...
  // this model instance) and the name of the parent or child frame (not
  // necessarily unique).
  auto F_frame_name = [this, tree](const Frame<T>& frame) {
    const std::string F_name =
        fmt::format("{}_{}_F_offset", this->name(), frame.name());
    DRAKE_DEMAND(!tree->HasFrameNamed(F_name, frame.model_instance()));
    return F_name;
  };
  ...
  if (mobod.is_reversed()) {
    M = &Jp;
    if (X_JpJc_is_identity) {
      F = &Jc;
    } else {
      // Create a new offset frame for F with offset X_JcJp (= X_JpJc⁻¹)
      const FixedOffsetFrame<T>& F_frame =
          tree->AddFrame(std::make_unique<FixedOffsetFrame<T>>(
              F_frame_name(Jc), Jc, X_JpJc_.inverse(), Jc.model_instance()));
      F = &F_frame;
    }
  } else {
    M = &Jc;
    if (X_JpJc_is_identity) {
      ...
      // Create a new offset frame for F with offset X_JpJc
      const FixedOffsetFrame<T>& F_frame =
          tree->AddFrame(std::make_unique<FixedOffsetFrame<T>>(
              F_frame_name(Jp), Jp, X_JpJc_, Jp.model_instance()));
      F = &F_frame;
    }
  }

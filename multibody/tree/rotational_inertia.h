...
// (This implementation adapted from Simbody's Rotation::reexpressSymMat33.)
...
// expressed in E (only the frame matters here so we're switching notation to
// emphasize that). We're given a rotation matrix R_AE and would like to
// calculate I' = I_AA = R_AE I_EE R_AEᵀ, writing the result in place. Written
// like that, it would take 90 flops for 3x3 matrices, 75 flops if we're
// careful to calculate only the six elements in final result. Here we'll get it
// done in 57 flops.
//
...
//                                  [ e f c ]
//
// First, split I into I=L+D+vₓ with v=[-f e 0]ᵀ (x means cross product matrix):
//        [a-c   d   0]       [c 0 0]        [ 0  0  e]
//    L = [ d   b-c  0]   D = [0 c 0]   vₓ = [ 0  0  f]
//        [2e   2f   0]       [0 0 c]        [-e -f  0]
// (4 flops to calculate L)
//
// A cross product matrix identity says R vₓ Rᵀ=(Rv)ₓ, so:
//    I' = R I Rᵀ = R L Rᵀ + D + (Rv)ₓ.
...
// index of a matrix to mean "row i", round bracket (j) means "col j".
//
//        [  -   -  0 ]
...
//
// (I looked at the generated assembly code for gcc 7.5 -O3 in Godbolt and
// counted exactly 57 inline flops, no loops or function calls.)
template <typename T>

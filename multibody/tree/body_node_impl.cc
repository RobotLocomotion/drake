...
void BodyNodeImpl<T, ConcreteMobilizer>::CalcPositionKinematicsCache_BaseToTip(
  ...
  X_FM = mobilizer_->calc_X_FM(my_q(positions));
...
//  so that we don't have to make a separate pass.
template <typename T, template <typename> class ConcreteMobilizer>
void BodyNodeImpl<T, ConcreteMobilizer>::
  ...
  if constexpr (kNv > 0) {
    ...
    std::array<T, kNv> v;
    for (int i=0; i < kNv; ++i) v[i] = 0.0;
    Eigen::Matrix<T, 6, kNv>& H_PB_W = my_mutable_H(H_PB_W_cache);
...
void BodyNodeImpl<T, ConcreteMobilizer>::CalcVelocityKinematicsCache_BaseToTip(
  ...
  const std::array<T, kNv>& vm = my_v(velocities);
  ...
  if constexpr (kNv > 0) {
    ...
    const Eigen::Matrix<T, 6, kNv>& H_PB_W = my_H(H_PB_W_cache);
    const Vector<T, kNv> vmx(&vm[0]);  // Alignment prevents direct use of vm.
    V_PB_W.get_coeffs() = H_PB_W * vmx;
  } else {

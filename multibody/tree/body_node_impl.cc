...
void BodyNodeImpl<T, ConcreteMobilizer>::CalcInverseDynamics_TipToBase(
  ...
  const bool no_Fapplied = (ssize(Fapplied_Bo_W_array) == 0);
  const bool no_tau_applied = (ssize(tau_applied_array) == 0);

  // Check sizes.
  DRAKE_ASSERT(Fb_Bo_W_cache == nullptr ||
               ssize(*Fb_Bo_W_cache) == this->get_parent_tree().num_mobods());
  DRAKE_ASSERT(no_Fapplied || ssize(Fapplied_Bo_W_array) ==
                                  this->get_parent_tree().num_mobods());
  DRAKE_ASSERT(no_tau_applied || ssize(tau_applied_array) ==
  ...
  // (Inline version of CalcBodySpatialForcesGivenItsSpatialAcceleration().)
  ...
  // Output generalized forces (same indexing as generalized velocities).

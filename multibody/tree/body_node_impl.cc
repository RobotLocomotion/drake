...
void BodyNodeImpl<T, ConcreteMobilizer>::
  ...
  if constexpr (kNv > 0) {
    ...
    // TODO(sherm1) Special case R_PF = Identity?
    ...
    // N.B. I timed velocity kinematics with and without the following
    // optimization, introduced in PR #22977. With gcc 11.4.0 this accounts for
    // an almost 20% speedup in velocity kinematics.
    const bool is_X_MB_identity =  // this is often the case
        frame_M.is_X_BF_identity(frame_body_pose_cache);  // F==M
    if (is_X_MB_identity) {
      // p_MB is zero so we can avoid a shift.
      for (int imob = 0; imob < kNv; ++imob) {
        v(imob) = 1.0;
        // Compute the imob-th column of H_FM (== H_FB here):
        const SpatialVelocity<T> Himob_FB = mobilizer_->calc_V_FM(q, v.data());
        v(imob) = 0.0;
        // V_PB_W = V_PFb_W + V_FMb_W + V_MB_W = V_FMb_W =
        //        = R_WF * V_FB
        H_PB_W.col(imob) = (R_WF * Himob_FB).get_coeffs();
      }
      return;
    }

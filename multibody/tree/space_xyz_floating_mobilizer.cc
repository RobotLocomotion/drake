...
SpaceXYZFloatingMobilizer<T>::CalcAcrossMobilizerTransform(
  ...
  const auto rpy = this->get_positions(context).template head<3>();
  const auto p_FM = this->get_positions(context).template tail<3>();
...
void SpaceXYZFloatingMobilizer<T>::DoCalcNplusMatrix(
  ...
  Matrix3<T> E_F;
  E_F << cy * cp, -sy, 0.0, sy * cp, cy, 0.0, -sp, 0.0, 1.0;

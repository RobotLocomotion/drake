...
can be exploited for speed. Velocity and acceleration quantities are simplified
also. In robotics, the revolute joint is very common so it is important that we
...
class RevoluteMobilizer : public MobilizerImpl<T, 1, 1> {
 public:
  ...
  // angle (see get_angle()) from `context`. This is the generalized speed v
...
class RevoluteMobilizerAxial final : public RevoluteMobilizer<T> {
 public:
  ...
  math::RigidTransform<T> compose_with_X_FM(
    ...
    X_AF.template ComposeThisWithAxialRotation<axis>(arX_FM, &X_AM);
    return X_AM;
  }
  ...
  math::RigidTransform<T> compose_X_FM_with(
    ...
    X_MB.template ComposeAxialRotationWithThis<axis>(arX_FM, &X_FB);
    return X_FB;
  }

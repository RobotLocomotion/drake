...
std::unique_ptr<Frame<ToScalar>> ShadowFrame<T>::TemplatedDoCloneToScalar(
  ...
  // A shadow frame's model instance is the loop joint's, which need not be the
  // shadow link's, so it has to be copied rather than defaulted.
...
std::unique_ptr<Frame<T>> ShadowFrame<T>::DoShallowClone() const {
  auto new_frame = std::make_unique<ShadowFrame<T>>(
      this->name(), this->link(), source_frame_, this->model_instance());
  new_frame->set_is_ephemeral(this->is_ephemeral());
  return new_frame;
}

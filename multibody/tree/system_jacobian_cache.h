...
template <typename T>
class SystemJacobianCache {
 public:
  DRAKE_DEFAULT_COPY_AND_MOVE_AND_ASSIGN(SystemJacobianCache);

  // Allocates the appropriately-sized data structure for the SystemJacobian,
  // initialized to zero.
  explicit SystemJacobianCache(const SpanningForest& forest);

  const std::vector<Eigen::MatrixX<T>>& block_system_jacobian() const {
    return block_system_jacobian_;
  }

  const Eigen::MatrixX<T>& operator[](TreeIndex i) const {
    return block_system_jacobian_[i];
  }

  Eigen::MatrixX<T>& operator[](TreeIndex i) {
    return block_system_jacobian_[i];
  }
 ...
 private:
  int total_rows_{};  // number of mobods
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(
    class ::drake::multibody::internal::SystemJacobianCache);

...
class JointActuator final
 ...
 public:
  ...
  /// `context`.
  const T& CalcRotorInertia(const systems::Context<T>& context) const {
  ...
  /// `context`.
  const T& CalcGearRatio(const systems::Context<T>& context) const {
  ...
  /// joints. A zero value indicates reflected inertia is not modeled. This
  /// value is used as part of an approximation of a rotor's inertial effects
  /// in a geared motor. It should be noted that the approximation is reasonable
  /// for high gear ratios but less so for small gear ratios
  ...
  T CalcReflectedInertia(const systems::Context<T>& context) const {
    const T& rho = CalcGearRatio(context);
    const T& Ir = CalcRotorInertia(context);
    return rho * rho * Ir;
  }
 ...
 private:
  ...
  double default_gear_ratio_{0.0};

...
class JointActuator final
 ...
 public:
  ...
  /// commonly used in robotics (see [Featherstone, 2008]). The value of
  /// reflected inertia is a function of the gear ratio and the rotor inertia of
  /// the modeled motor. %JointActuator exposes these two values as parameters
  /// (See SetRotorInertia() and SetGearRatio()). Reflected inertia has units of
  /// kg for prismatic joints and units of kg⋅m² for revolute joints. A zero
  /// value indicates reflected inertia is not modeled. This value is used as
  /// part of an approximation of a rotor's inertial effects in a geared motor.
  /// It should be noted that the approximation is reasonable for high gear
  /// ratios but less so for small gear ratios (see [Featherstone, 2008],
  /// Chapter 9.6 on gears).
  ///
  /// <h3> Actuated revolute joints </h4>
  ...
  /// its axis of rotation and from the dimensionless gear ratio ρ. The gear
  /// ratio is defined as the ratio of the rotor angular velocity to the joint's
  /// shaft angular velocity, typically ≫ 1 for robotic systems. For this case
  /// the reflected inertia will be: Iᵣᵢ = ρ²⋅Iᵣ.
  ///
  /// <h3> Actuated prismatic joints </h4>
  ...
  /// rotor Iᵣ, the reflected inertia will be Iᵣᵢ  = ρ²⋅Iᵣ. Notice that in this
  /// case Iᵣᵢ properly has units of mass, i.e. kg.
  ///@{
  ...
  /// See @ref reflected_inertia.
  void SetRotorInertia(systems::Context<T>* context, T rotor_inertia) {
  ...
  /// See @ref reflected_inertia.
  void SetGearRatio(systems::Context<T>* context, T gear_ratio) {
 ...
 private:
  ...
  // The rotor inertia for this actuator.
  double default_rotor_inertia_{0.0};

  // The gear ratio between this actuator's rotor and output shaft.

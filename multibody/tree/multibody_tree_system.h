...
class MultibodyTreeSystem : public systems::LeafSystem<T> {
 public:
  ...
  /** For a body B, this evaluates the articulated body bias `Zb_Bo_W(q, v) =
  Pplus_PB_W(q) * Ab_WB(q, v)`. This computation is particularly expensive when
  performing O(n) forward dynamics with different applied forces but with the
  same multibody state x = [q, v] and therefore it is worth caching. */

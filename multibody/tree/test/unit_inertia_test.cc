...
GTEST_TEST(UnitInertia, SolidCapsule) {
  ...
  // I = diag(Ixx, Iyy, Izz) where
  // Ixx = Iyy = m₁(L²/12 + r²/4) + m₂(2r²/5 + L²/4 + 3Lr/8)
  // Izz = m₁(r²/2) + m₂(2r²/5). m₁ and m₂ are the mass of the cylinder part and
  // the spherical part (sum of the two half-spheres) respectively.
  const double Ixx =

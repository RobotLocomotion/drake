...
// with hinge matrix H to get Pplus. (Pplus is the ABI as seen from the
// inboard side of the inboard joint.) Both inputs should be expressed in World.
template <int dofs>
Matrix6<double> Project(const Matrix6<double>& P,
                        const Eigen::Matrix<double, 6, dofs>& H) {
  const Matrix6<double> projection_matrix =
      (Matrix6<double>::Identity() -
          P * H * (H.transpose() * P * H).inverse() * H.transpose());
  return projection_matrix * P;
}
...
GTEST_TEST(ArticulatedBodyInertiaAlgorithm, ModifiedFeatherstoneExample) {
  ...
  // Now re-express in World
  Matrix6<double> Pplus_C_W_expected_mat =
      R6_WC * M_Ccm_proj * R6_WC.transpose();
  ...
  Vector6d H_M_M; H_M_M << 1, 0, 0, 0, 0, 0;  // rotate about x

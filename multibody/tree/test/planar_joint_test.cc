...
TEST_F(PlanarJointTest, ContextDependentAccess) {
  ...
  // Position access:
  joint_->set_position(context_.get(), translation1);
  EXPECT_EQ(joint_->get_position(*context_), translation1);
  joint_->set_angle(context_.get(), angle1);
  EXPECT_EQ(joint_->get_angle(*context_), angle1);
  joint_->set_pose(context_.get(), translation2, angle2);
  EXPECT_EQ(joint_->get_position(*context_), translation2);
  EXPECT_EQ(joint_->get_angle(*context_), angle2);

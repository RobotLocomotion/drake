...
TEST_F(PlanarJointTest, ContextDependentAccess) {
  ...
  // Position access:
  joint_->set_translations(context_.get(), some_value);
  EXPECT_EQ(joint_->get_translations(*context_), some_value);
  joint_->set_angle(context_.get(), some_value2);
  EXPECT_EQ(joint_->get_angle(*context_), some_value2);

  // Velocity access:
  joint_->set_translation_rates(context_.get(), some_value);
  EXPECT_EQ(joint_->get_translation_rates(*context_), some_value);
  joint_->set_angular_rate(context_.get(), some_value2);
  EXPECT_EQ(joint_->get_angular_rate(*context_), some_value2);
}
...
TEST_F(PlanarJointTest, DefaultAngles) {
  const Vector3d lower_limit_positions =
      Vector3d::Constant(kPositionLowerLimit);
  const Vector3d upper_limit_positions =
      Vector3d::Constant(kPositionUpperLimit);

  const Vector3d default_positions = Vector3d::Zero();

  const Vector3d new_default_positions =
      Vector3d::Constant(kPositionNonZeroDefault);

  const Vector3d out_of_bounds_low_positions =
      lower_limit_positions - Vector3d::Constant(1);
  const Vector3d out_of_bounds_high_positions =
      upper_limit_positions + Vector3d::Constant(1);

  // Constructor should set the default translation to Vector3d::Zero()
  EXPECT_EQ(joint_->get_default_position(), default_positions);

  // Setting a new default translation should propagate so that
  // `get_default_translation()`
  // remains correct.
  mutable_joint_->set_default_position(new_default_positions);
  EXPECT_EQ(joint_->get_default_position(), new_default_positions);

  // Setting the default translation out of the bounds of the position limits
  // should NOT throw an exception.
  EXPECT_NO_THROW(
      mutable_joint_->set_default_position(out_of_bounds_low_positions));
  EXPECT_NO_THROW(
      mutable_joint_->set_default_position(out_of_bounds_high_positions));
}

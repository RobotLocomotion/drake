...
class DoorHingeTester {
 public:
  ...
  double CalcHingeSpringTorque(double angle) const {
    return door_hinge_.CalcHingeSpringTorque(angle, door_hinge_.config());
  }
...
void TestFrictionOnlyEnergyAndPower(const DoorHingeTester& dut,
  ...
  EXPECT_EQ(non_conserv_power,
            dut.CalcHingeFrictionalTorque(angular_rate) * angular_rate);
}
...
void TestSpringTorqueOnlyPower(const DoorHingeTester& dut,
  ...
  const double angular_rate =
      plant.GetJointByName(kRevoluteJointName).GetOneVelocity(context);
  EXPECT_EQ(conserv_power, dut.CalcHingeSpringTorque(angle) * angular_rate);
...
// comparing it against the result from integrating the conservative power (Pc),
// i.e. we should have PE = -∫Pcdt. The `InitialValueProblem` class is used for
...
// be defined as `ẋ[0] = Pc`, `ẋ[1] = v`, `ẋ[2] = 0` and the initial value is
// `x₀ = {0, 0, vₜ}`. Since velocity is constant, the total simulation time
// can be derived as t = qₜ / vₜ. To guarantee the comparison is numerically
// meaningful (not affected by integration error), this function requires the
// value of the potential energy has to be bigger than 1, which is significantly
// bigger than the integration accuracy (1e-9).
void TestPotentialEnergyCalculation(const DoorHingeTester& dut,
  ...
  auto energy_ode = [&dut](const double& t, const VectorX<double> x,
    ...
    ret[0] = -dut.CalcHingeConservativePower(x[1], x[2]);
    ret[1] = x[2];
    ret[2] = 0.0;
    return ret;
  };
  ...
  systems::InitialValueProblem<double> ivp(energy_ode, kDefaultValues);
  ...
  const auto result = ivp.Solve(integration_time);
...
// no error-control or anything on `W(t)`. If chose to use a continuous
...
// If chose to use a discrete system, this error will be more meaningful since
// both terms use the similar integration method. The test here chooses to use
// the discrete system for an apple-to-apple comparison. Therefore, the error
// tolerance of the comparison will purely depend on the maximum time step.
// After running multiple tests with different maximum time steps, the error is
// roughly about 100 times bigger than the time step. Note that, to guarantee
// the test is meaningful, two requirements have to be posted. First, the
// initial total energy should be bigger than the tolerance by several orders of
// magnitude. Here, we choose the number to be 1e3. Second, the change of
// angular rate before and after the simulation has to be more than 20% to make
// sure that the energy has a significant change or shift. There two numbers are
// set by heuristic. Since the main focus of this test is to verify the
// correctness of the energy calculation, the settings are good enough.
void TestEnergyConservative(const MultibodyPlant<double>& plant,
  ...
  // Require the velocity state to change big enough to make the comparison
  // numerically meaningful.
  const double kVelocityChangeRatio = 0.2;
  ...
  DRAKE_DEMAND(velocity_change >
               kVelocityChangeRatio * std::abs(init_state[1]));

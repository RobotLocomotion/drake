...
constexpr double kMaximumTimeStep = 1e-6;
...
void TestPotentialEnergyCalculation(const DoorHingeTester& dut,
  ...
  const double potential_energy = dut.door_hinge().CalcPotentialEnergy(
      context, plant.EvalPositionKinematics(context));
  EXPECT_NEAR(potential_energy, result[0], kIntegrationAccuracy);
}
...
// no error-control or anything on `W(t)`. If the user chose to use a continuous
...
// If chose to use a discrete system, this error will be smaller since both
// terms use the similar integration method. The test here chooses to use the
// discrete system for an apple-to-apple comparison. Therefore, the error
...
// roughly about 100 bigger than the time step.

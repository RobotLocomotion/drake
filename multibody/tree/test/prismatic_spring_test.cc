...
class SpringTester : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    joint_ =
        &model->AddJoint<PrismaticJoint>("joint_AB", *bodyA_, std::nullopt,
                                        *bodyB_, std::nullopt,
                                        Vector3<double>::UnitZ());

    model->AddJoint<PrismaticJoint>("joint_BC", *bodyB_, std::nullopt, *bodyC_,
...
TEST_F(SpringTester, DeltaPosition) {
  ...
  expected_generalized_forces << expected_force_magnitude, 0;

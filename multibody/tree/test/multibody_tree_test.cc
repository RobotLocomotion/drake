...
// Tests the edge case where the system has no q or v state, but does have
// miscellaneous continuous state. We should still successfully compute the
// derivatives of the zs.
...
void CheckZOnlyContinuousStateDerivatives(
    const MultibodyTreeSystem<double>& system) {
...
GTEST_TEST(MultibodyTreeSystem, ZOnlyContinuousStateDerivatives) {
  ZOnlyContinuousStateSystem system(2);
  system.Finalize();

  CheckZOnlyContinuousStateDerivatives(system);
}

// Tests the MultibodyTreeSystem constructor that passes a null tree (so that
// the tree builds its own tree), but a non-zero misc continuous state size.
class ConstructorZOnlyContinuousStateSystem final

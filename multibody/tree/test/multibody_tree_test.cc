...
TEST_F(KukaIiwaModelTests, CalcJacobianTranslationalVelocityA) {
  ...
  const Vector3<double> v_WE_value =
      math::ExtractValue(v_WE_autodiff);
  const MatrixX<double> v_WE_derivs =
      math::ExtractGradient(v_WE_autodiff);
  ...
  const VectorX<AutoDiffXd> q_autodiff =
      ...
      // gradient for math::InitializeAutoDiff().
...
TEST_F(KukaIiwaModelTests, CalcBiasForJacobianTranslationalVelocity) {
  ...
  auto q_autodiff =
      math::InitializeAutoDiff(q, MatrixXd(v));
  auto v_autodiff =
      math::InitializeAutoDiff(v, MatrixXd(vdot));
  ...
  MatrixX<double> Jv_WHp_derivs =
      math::ExtractGradient(Jv_WHp_autodiff);
...
TEST_F(KukaIiwaModelTests, CalcJacobianTranslationalVelocityC) {
  ...
  const Matrix3X<double> p_WPi_value =
      math::ExtractValue(p_WPi_autodiff);
  const MatrixX<double> p_WPi_derivs =
      math::ExtractGradient(p_WPi_autodiff);

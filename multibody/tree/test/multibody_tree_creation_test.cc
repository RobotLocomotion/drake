...
class TreeTopologyTests : public ::testing::Test {
 public:
  ...
  static void TestBodyNode(const MultibodyTreeTopology& topology,
    ...
    EXPECT_EQ(parent_body, topology.get_rigid_body_topology(body).parent_body);
    ...
    // Check if this "node" effectively is a child of "parent_node".
    const std::vector<MobodIndex>& children =
        topology.get_body_node_topology(parent_node).child_nodes;
    EXPECT_TRUE(std::find(children.begin(), children.end(), node) !=
                children.end());
  }
  ...
  static void VerifyTopology(const MultibodyTreeTopology& topology,
    ...
    for (const auto& [body_index, num_outboards] : expected_num_outboards) {
      EXPECT_EQ(node_topology_from_body_index(topology, body_index)
                    .get_num_children(),
                num_outboards);
    }
    ...
    for (const auto& [mobod_index, primary_link_index] :
      ...
      EXPECT_EQ(topology.get_body_node_topology(mobod_index).rigid_body,
                primary_link_index);
    }
...
TEST_F(TreeTopologyTests, SizesAndIndexing) {
  ...
  // The Forest version of the above loop.
  positions_index = velocities_index = 0;

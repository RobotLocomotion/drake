...
GTEST_TEST(SpatialInertia, CastToAutoDiff) {
  ...
  auto G_value = math::ExtractValueMatrixFromAutoDiff(G_autodiff_matrix);
  G_value.resize(3, 3);
  EXPECT_TRUE(G_value.isApprox(G_double.CopyToFullMatrix3(), kEpsilon));
  MatrixXd G_gradient =
      math::ExtractGradientMatrixFromAutoDiff(G_autodiff_matrix);
  ASSERT_EQ(G_gradient.size(), 0);

  // Values and gradients of the unit inertia.
  const auto& com_autodiff = M_autodiff.get_com();
  const Vector3d com_value = math::ExtractValueMatrixFromAutoDiff(com_autodiff);
  EXPECT_TRUE(com_value.isApprox(com_double, kEpsilon));
  MatrixXd com_gradient = math::ExtractGradientMatrixFromAutoDiff(com_autodiff);

...
TEST_F(PlanarMobilizerTest, DefaultPosition) {
  ...
  Vector3d new_defualt(.4, .5, .6);
  mutable_mobilizer->set_default_position(new_defualt);
  mobilizer_->set_default_state(*context_, &context_->get_mutable_state());

  EXPECT_EQ(mobilizer_->get_translations(*context_), new_defualt.head(2));
  EXPECT_EQ(mobilizer_->get_angle(*context_), new_defualt[2]);
}
...
TEST_F(PlanarMobilizerTest, RandomState) {
  ...
  mutable_mobilizer->set_random_position_distribution(
      Vector3<symbolic::Expression>(uniform(generator),
                                    uniform(generator) + 1.0,
                                    uniform(generator) + 2.0));
  mutable_mobilizer->set_random_state(*context_, &context_->get_mutable_state(),
                                      &generator);
  EXPECT_GE(mobilizer_->get_translations(*context_)[0], 0.0);
  EXPECT_LE(mobilizer_->get_translations(*context_)[0], 1.0);
  EXPECT_GE(mobilizer_->get_translations(*context_)[1], 1.0);
  EXPECT_LE(mobilizer_->get_translations(*context_)[1], 2.0);
  EXPECT_GE(mobilizer_->get_angle(*context_), 2.0);
  EXPECT_LE(mobilizer_->get_angle(*context_), 3.0);
  EXPECT_EQ(mobilizer_->get_translation_rates(*context_), Vector2d::Zero());
  EXPECT_EQ(mobilizer_->get_angular_rate(*context_), 0.0);
  ...
  EXPECT_GE(mobilizer_->get_angle(*context_), 2.0);
  EXPECT_LE(mobilizer_->get_angle(*context_), 3.0);
  EXPECT_GE(mobilizer_->get_translation_rates(*context_)[0], -1.0);
  EXPECT_LE(mobilizer_->get_translation_rates(*context_)[0], 0.0);
  EXPECT_GE(mobilizer_->get_translation_rates(*context_)[1], -2.0);
  EXPECT_LE(mobilizer_->get_translation_rates(*context_)[1], -1.0);
  EXPECT_GE(mobilizer_->get_angular_rate(*context_), -3.0);
  EXPECT_LE(mobilizer_->get_angular_rate(*context_), -2.0);

...
// is supported by ElementCollection. To avoid spamming this arbitrary choice
// everywhere, we'll introduce shorthand for it.
template <typename T>
using Element = ModelInstance<T>;
using Index = ModelInstanceIndex;
using Collection = ElementCollection<double, Element, Index>;
...
GTEST_TEST(ElementCollectionTest, AddDense) {
  ...
  EXPECT_EQ(dut.names().size(), 2);
  EXPECT_EQ(dut.names().count(std::string{"zero"}), 1);
  EXPECT_EQ(dut.names().count(std::string{"one"}), 1);

  EXPECT_THAT(dut.indices(), ElementsAre(Index{0}, Index{1}));
...
GTEST_TEST(ElementCollectionTest, DuplicateNames) {
  ...
  EXPECT_EQ(dut.names().size(), 5);
  EXPECT_EQ(dut.names().count(std::string{"foo"}), 3);
  EXPECT_EQ(dut.names().count(std::string{"bar"}), 2);
  std::set<Index> foo_indices;
  auto [lower, upper] = dut.names().equal_range(std::string{"foo"});
  for (auto iter = lower; iter != upper; ++iter) {
    const Index index = iter->second;
    foo_indices.insert(index);
  }
  EXPECT_THAT(foo_indices, ElementsAre(Index{0}, Index{2}, Index{4}));
}
...
GTEST_TEST(ElementCollectionTest, Removals) {
  ...
  dut.Add(std::make_unique<Element<double>>(Index{0}, "zero"));
  dut.AddBorrowed(&one);
  dut.AddRemovedElementClone();  // "two"
  ...
  EXPECT_EQ(dut.names().size(), 3);
  EXPECT_EQ(dut.names().count(std::string{"zero"}), 1);
  EXPECT_EQ(dut.names().count(std::string{"one"}), 1);
  EXPECT_EQ(dut.names().count(std::string{"three"}), 1);

  EXPECT_THAT(dut.indices(), ElementsAre(Index{0}, Index{1}, Index{3}));
  ...
  // Overall we have indices 3',4,5 now.
  dut.Remove(Index{1});
  dut.Remove(Index{0});
  ...
  EXPECT_EQ(dut.names().size(), 3);
  EXPECT_EQ(dut.names().count(std::string{"tres"}), 1);
  EXPECT_EQ(dut.names().count(std::string{"four"}), 1);
  EXPECT_EQ(dut.names().count(std::string{"five"}), 1);

  EXPECT_THAT(dut.indices(), ElementsAre(Index{3}, Index{4}, Index{5}));
  EXPECT_EQ(dut.next_index(), Index{6});
}

// Allocate a range of indices and then fill then in out-of-order.
// This matches how MbT::Clone currently works.
GTEST_TEST(ElementCollectionTest, AllocateThenAdd) {
  ElementCollection<AutoDiffXd, Element, Index> other;
  for (int i = 0; i < 5; ++i) {
    other.AddRemovedElementClone();
  }
  ...
  for (Index i{0}; i < 5; ++i) {
    ...
    EXPECT_EQ(dut.names().count(name), 1);
...
GTEST_TEST(ElementCollectionTest, Autodiff) {
  ElementCollection<AutoDiffXd, Element, Index> autodiff;
}

// A simple sanity check that the class can be instantiated.
GTEST_TEST(ElementCollectionTest, Symbolic) {
  ElementCollection<symbolic::Expression, Element, Index> symbolic;
}

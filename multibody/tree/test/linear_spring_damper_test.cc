...
class SpringDamperTester : public ::testing::Test {
 public:
  void SetUp() override {
    // Create an empty multibody tree.
    ...
    tree->mutable_gravity_field().set_gravity_vector(Eigen::Vector3d::Zero());

    const SpatialInertia<double> point_mass(1., Eigen::Vector3d::Zero(),
                                            UnitInertia<double>(0., 0., 0.));
    bodyA_ = &tree->AddRigidBody("BodyA", point_mass);
    bodyB_ = &tree->AddRigidBody("BodyB", point_mass);

    tree->AddJoint<WeldJoint>("WeldBodyAToWorld", tree->world_body(),
                              std::nullopt, *bodyA_, std::nullopt,
                              math::RigidTransform<double>::Identity());

    // Allow body B to slide along the x axis.
    slider_ = &tree->AddJoint<PrismaticJoint>(
        "Slider", tree->world_body(), std::nullopt, *bodyB_, std::nullopt,
    ...
    forces_ = std::make_unique<MultibodyForces<double>>(*system_);
  }
  ...
  void CalcSpringDamperForces() const {
    ...
    spring_damper_->CalcAndAddForceContribution(
        *context_, system_->EvalPositionKinematics(*context_),
        system_->EvalVelocityKinematics(*context_), forces_.get());
  }
...
GTEST_TEST(LinearSpringDamper, BadParameters) {
  ...
  const auto& bodyA = tree.AddRigidBody("BodyA", SpatialInertia<double>());
  const auto& bodyB = tree.AddRigidBody("BodyB", SpatialInertia<double>());
  ...
  EXPECT_NO_THROW(tree.AddForceElement<LinearSpringDamper>(
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      tree.AddForceElement<LinearSpringDamper>(bodyA, p_AP, bodyB, p_BQ,
                                               -1.0 /* negative rest length */,
                                               stiffness, damping),
      std::exception, ".*condition 'free_length > 0' failed.*");

  DRAKE_EXPECT_THROWS_MESSAGE(tree.AddForceElement<LinearSpringDamper>(
...
TEST_F(SpringDamperTester, LengthLargerThanRestLength) {
  ...
  EXPECT_EQ(system_->EvalPotentialEnergy(*context_), potential_energy);
...
TEST_F(SpringDamperTester, Power) {
  ...
  EXPECT_EQ(system_->EvalConservativePower(*context_), conservative_power);
  EXPECT_EQ(system_->EvalNonConservativePower(*context_),

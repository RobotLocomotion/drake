...
// Rough level of tolerance; equivalent to 2-3 bits of precision loss.
...
class MeshTypeMassPropertiesTest : public testing::Test {};

TYPED_TEST_SUITE_P(MeshTypeMassPropertiesTest);
...
TYPED_TEST_P(MeshTypeMassPropertiesTest, Administrivia) {
...
REGISTER_TYPED_TEST_SUITE_P(MeshTypeMassPropertiesTest, Administrivia);
using MeshTypes = ::testing::Types<geometry::Convex, geometry::Mesh>;
INSTANTIATE_TYPED_TEST_SUITE_P(All, MeshTypeMassPropertiesTest, MeshTypes);
...
GTEST_TEST(TriangleSurfaceMassPropertiesTest, ExactPolyhedron) {
...

  {
    ...
    const geometry::TriangleSurfaceMesh<double> mesh =
        geometry::internal::MakeBoxSurfaceMesh<double>(box,
                                                       std::max({Lx, Ly, Lz}));

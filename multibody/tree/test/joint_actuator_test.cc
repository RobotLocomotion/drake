...
constexpr double kInfEffortLimit = -1;
...
GTEST_TEST(JointActuatorTest, JointAcutatorLimitTest) {
  ...
  const Joint<double>& body2_body1 =
      tree.AddJoint(std::make_unique<PrismaticJoint<double>>(
          ...
          Eigen::Vector3d(0, 0, 1)));
  tree.AddJointActuator("act2", body2_body1, kInfEffortLimit);
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      std::runtime_error, "Zero effort limit is treated as no actuator.");

  const Joint<double>& body4_body3 =
      tree.AddJoint(std::make_unique<PrismaticJoint<double>>(
          "prism4", body3->body_frame(), body4->body_frame(),
          Eigen::Vector3d(0, 0, 1)));
  DRAKE_EXPECT_THROWS_MESSAGE(
      tree.AddJointActuator("act4", body4_body3, kNegativeEffortLimit),
      std::runtime_error,
  ...
  const auto& actuator2 = tree.GetJointActuatorByName("act2");
  EXPECT_EQ(actuator2.effort_limit(), kInf);
}

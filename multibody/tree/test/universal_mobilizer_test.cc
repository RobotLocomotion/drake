...
TEST_F(UniversalMobilizerTest, CalcAcrossMobilizerTransform) {
  ...
  const RotationMatrixd R_x = RotationMatrixd::MakeXRotation(angles[0]);
  const Vector3d axis_M_y(0, 1, 0);
  const AngleAxisd angle_axis_y(angles[1], R_x * axis_M_y);
  const RigidTransformd X_FM_expected(RotationMatrixd(angle_axis_y) * R_x);

...
class LinearBushingRollPitchYawTester : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    // Assign the two bodies that are attached to the bushing.
  ...
  // @param[in] f_Co_C_byHand By-hand calculation of the net force from the
  ...
  void CompareToMotionGenesisResult(
    ...
    //--------- Start of by-hand/MotionGenesis calculations ----------
    ...
    const Vector3<double> t_A_A_expected = Vector3<double>(-Tx, -Ty, -Tz) +
                                        R_AB * p_AoBo_B.cross(f_Ao_B_expected);

    // Calculate the moment on frame C about Co, expressed in A.
    const Vector3<double> p_CoBo_B = -p_AoBo_B;
    const Vector3<double> t_C_C_expected = R_CA * Vector3<double>(Tx, Ty, Tz) +
    ...
    //------------- End of by-hand/MotionGenesis calculations --------

    // Verify F_A_A,, the bushing's spatial force calculation on frame A.
    double torque_epsilon = 32 * kEpsilon * t_A_A_expected.norm();
    double force_epsilon  = 32 * kEpsilon * f_Ao_A_expected.norm();
    ...
    if (t_C_C_byHand != nullptr) {
      EXPECT_TRUE(CompareMatrices(F_C_C.rotational(), *t_C_C_byHand,
                                  torque_epsilon, MatrixCompareType::relative));
    }
    if (f_Co_C_byHand != nullptr) {
      EXPECT_TRUE(CompareMatrices(F_C_C.translational(), *f_Co_C_byHand,
                                  force_epsilon, MatrixCompareType::relative));
    }
...
TEST_F(LinearBushingRollPitchYawTester, VariousOrientationsAtRest) {
  ...
  Vector3<double> f_Co_C_byHand = Vector3<double>(0, 0, 0);
  Vector3<double> t_C_C_byHand = Vector3<double>(0, 0, 0);
  CompareToMotionGenesisResult(rpy, p_zero, w_zero, v_zero,
                               &f_Co_C_byHand, &t_C_C_byHand);

  // Test for non-zero roll only.
  rpy.set(roll_angle, 0, 0);
  t_C_C_byHand = Vector3<double>(k0_roll, 0, 0);
  CompareToMotionGenesisResult(rpy, p_zero, w_zero, v_zero,
                               &f_Co_C_byHand, &t_C_C_byHand);

  // Test for non-zero pitch only.
  rpy.set(0, pitch_angle, 0);
  t_C_C_byHand = Vector3<double>(0, k1_pitch, 0);
  CompareToMotionGenesisResult(rpy, p_zero, w_zero, v_zero,
                               &f_Co_C_byHand, &t_C_C_byHand);

  // Test for non-zero yaw only.
  rpy.set(0, 0, yaw_angle);
  t_C_C_byHand = Vector3<double>(0, 0, k2_yaw);
  CompareToMotionGenesisResult(rpy, p_zero, w_zero, v_zero,
                               &f_Co_C_byHand, &t_C_C_byHand);
...
TEST_F(LinearBushingRollPitchYawTester, VariousPositionsAtRest) {
  ...
  f_Co_C_byHand = Vector3<double>(kx_x, 0, 0);
  t_C_C_byHand = -0.5 * p_AoCo_A.cross(f_Co_C_byHand);
  CompareToMotionGenesisResult(rpy_zero, p_AoCo_A, w_zero, v_zero,
                               &f_Co_C_byHand, &t_C_C_byHand);

  // Test simple translation (y-direction).
  p_AoCo_A = Vector3<double>(0, y, 0);
  f_Co_C_byHand = Vector3<double>(0, ky_y, 0);
  t_C_C_byHand = -0.5 * p_AoCo_A.cross(f_Co_C_byHand);
  CompareToMotionGenesisResult(rpy_zero, p_AoCo_A, w_zero, v_zero,
                               &f_Co_C_byHand, &t_C_C_byHand);
  ...
  f_Co_C_byHand = Vector3<double>(0, 0, kz_z);
  t_C_C_byHand = -0.5 * p_AoCo_A.cross(f_Co_C_byHand);
  CompareToMotionGenesisResult(rpy_zero, p_AoCo_A, w_zero, v_zero,
                               &f_Co_C_byHand, &t_C_C_byHand);
  ...
  f_Co_C_byHand = Vector3<double>(kx_x, ky_y, kz_z);
  t_C_C_byHand = -0.5 * p_AoCo_A.cross(f_Co_C_byHand);
  CompareToMotionGenesisResult(rpy_zero, p_AoCo_A, w_zero, v_zero,
                               &f_Co_C_byHand, &t_C_C_byHand);
}
...
TEST_F(LinearBushingRollPitchYawTester, IdentityPoseWithTranslationalRates) {
  ...
  Vector3<double> f_Co_C_byHand(0, 0, 0);
  Vector3<double> t_C_C_byHand(0, 0, 0);
  CompareToMotionGenesisResult(rpy_zero, p_zero, w_zero, v_ACo_A,
                               &f_Co_C_byHand, &t_C_C_byHand);

  // Test simple translation (x-direction).
  v_ACo_A = Vector3<double>(xDt, 0, 0);
  f_Co_C_byHand = Vector3<double>(bx_xDt, 0, 0);
  CompareToMotionGenesisResult(rpy_zero, p_zero, w_zero, v_ACo_A,
                               &f_Co_C_byHand, &t_C_C_byHand);

  // Test simple translation (y-direction).
  v_ACo_A = Vector3<double>(0, yDt, 0);
  f_Co_C_byHand = Vector3<double>(0, by_yDt, 0);
  CompareToMotionGenesisResult(rpy_zero, p_zero, w_zero, v_ACo_A,
                               &f_Co_C_byHand, &t_C_C_byHand);

  // Test simple translation (z-direction).
  v_ACo_A = Vector3<double>(0, 0, zDt);
  f_Co_C_byHand = Vector3<double>(0, 0, bz_zDt);
  CompareToMotionGenesisResult(rpy_zero, p_zero, w_zero, v_ACo_A,
                               &f_Co_C_byHand, &t_C_C_byHand);

  // Test non-simple translation.
  v_ACo_A = Vector3<double>(xDt, yDt, zDt);
  f_Co_C_byHand = Vector3<double>(bx_xDt, by_yDt, bz_zDt);
  CompareToMotionGenesisResult(rpy_zero, p_zero, w_zero, v_ACo_A,
                               &f_Co_C_byHand, &t_C_C_byHand);
}

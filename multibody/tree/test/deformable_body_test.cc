...
TEST_F(DeformableBodyTest, CalcEffectiveAngularVelocity) {
  ...
  VectorX<double> discrete_state =
      plant_context_->get_discrete_state(body_->discrete_state_index()).value();
  const int num_dofs = body_->num_dofs();
  /* Let q be the concatenation of all vertex positions, measured and expressed
   in the world frame. */
  VectorX<double> q = discrete_state.head(num_dofs);
  /* The position of the sphere center in the world frame. */
  const Vector3d p_WSc = Vector3d(1.0, 0.0, 0.0);
  /* Translate all vertices of the sphere by (1, 0, 0). The resulting effect is
   that now the sphere is centered at (1, 0, 0) in the world frame. */
  for (int i = 0; i < num_dofs / 3; ++i) {
  ...
  /* Set a velocity field that is a uniform angular velocity about axis through
  ...
  for (int i = 0; i < num_dofs / 3; ++i) {
    ...
    const Vector3d p_WScP_W = p_WP - p_WSc;
    v_WP << -p_WScP_W.y(), p_WScP_W.x(), 0.0;
  }
  plant_context_->SetDiscreteState(body_->discrete_state_index(),
                                   discrete_state);
  ...
  /* Now, modify the velocity field to be radially outward from the center of
   the sphere. The resulting effective angular velocity should be zero. */
  v = discrete_state.segment(num_dofs, num_dofs);
  for (int i = 0; i < num_dofs / 3; ++i) {
    const Vector3d p_WP = q.segment<3>(i * 3);
    auto v_WP = v.template segment<3>(i * 3);
    const Vector3d p_WScP_W = p_WP - p_WSc;
  ...
  plant_context_->SetDiscreteState(body_->discrete_state_index(),
                                   discrete_state);
  w_WScm_W = body_->CalcEffectiveAngularVelocity(*plant_context_);

...
class MultibodyTree {
 public:
  ...
  int num_links() const { return ssize(owned_links_); }
  ...
  // Joints' degrees of freedom. Weld joints are ignored. Otherwise this
  // is just the set of Links in the subtrees rooted by these Joints'
  // implementing Mobods.
  std::set<LinkIndex> GetLinksKinematicallyAffectedBy(
      const std::vector<JointIndex>& joint_indexes) const;

  // Returns the set of Links that are on the same Mobod or outboard of the
  // given Links. This is just the set of Links in the subtrees rooted by these
  // Links' implementing Mobods. The given Links are always included.
  ...
  const SpanningForest& BuildSpanningForest() {
    ...
    // link_joint_graph_.model().DumpForest("forest model");
 ...
 private:
  ...
  std::vector<MobodIndex> joint_to_mobilizer_;

...
class MultibodyTree {
 public:
  ...
  2. CalcArticulatedBodyForces(): a second tip to base pass which essentially
     computes the bias terms in the ABA equations. These are a function of the
     full state x = [q; v] and externally applied actuation and forces.
  ...
  <h3> Articulated %Body Inertia and Bias Force </h3>
  ...
  <pre>
    Zjplus_B_W = Zj_B_W + g_B_W * e_B 
  </pre>  
  where notice our Eq. (9) has the additional term Pplus_PB_W * Ab_WB. Finally:
  <pre>
    ej_B = tau_B - H_PB_Wᵀ * Zj_B_W    
  </pre>
  which this is deceivingly the same as our Eq. (10), however the result is
  different given it has Zj_B_W in it.

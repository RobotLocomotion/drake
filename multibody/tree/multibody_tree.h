...
class MultibodyTree {
 public:
  ...
  /// Returns a mutable reference to the kinematic state vector stored in the
  /// given Context as an `Eigen::VectorBlock<VectorX<T>>`. This will
  /// consist only of q and v partitions in that order.
  ...
  /// Returns a mutable reference to the kinematic state vector stored in the
  /// given State as an `Eigen::VectorBlock<VectorX<T>>`. This will
  /// consist only of q and v partitions in that order.
  /// @note This does not cause cache invalidation.
  Eigen::VectorBlock<VectorX<T>> get_mutable_state_vector(
      systems::State<T>* state) const;

  /// Returns a mutable reference to the position state vector q stored in the
  /// given Context as an `Eigen::VectorBlock<VectorX<T>>`.
  ...
  /// Returns a mutable reference to the position state vector q stored in the
  /// given State as an `Eigen::VectorBlock<VectorX<T>>`.
  /// @note This does not cause cache invalidation.
  Eigen::VectorBlock<VectorX<T>> get_mutable_positions(
      systems::State<T>* state) const;

  /// Returns a mutable reference to the velocity state vector v stored in the
  /// given Context as an `Eigen::VectorBlock<VectorX<T>>`.
  ...
  /// Returns a mutable fixed-size Eigen::VectorBlock of `count` elements
  /// referencing a segment in the State with its first element at `start`.
  template <int count>
  Eigen::VectorBlock<VectorX<T>, count> get_mutable_state_segment(

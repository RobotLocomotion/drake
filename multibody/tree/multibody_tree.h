...
class MultibodyTree {
 public:
  ...
  /// with the Jacobian `Jv_WB(q)` which maps generalized velocities v
  ...
  /// (Internal) Computes the cache entry associated with the Jacobian `H_PB_W`
  /// for each node. The Jacobian `H_PB_W` with respect to generalized
  /// velocities relates to the spatial velocity of B in P by

...
class MultibodyTree {
 public:
  ...
  // dynamics.
  /** @name Articulated Body Algorithm Forward Dynamics. The Articulated %Body
  Algorithm (ABA) implements a forward dynamics computation with O(n)
  complexity. The algorithm is implemented in terms of three main passes:
  1. CalcArticulatedBodyInertiaCache(): which computes the
     ArticulatedBodyInertia for each body along with other ABA quantities that
     are configuration dependent only.
  2. CalcArticulatedBodyAlgorithmCache(): which essentially computes the bias
     terms in the ABA equations. These are a function of the full state x = [q;
     v] and externally applied actuation and forces.
  ...
  in a very clear and comptact form in Section 7.2.2 while at the same time
  ...
  Pplus_BCib_W and Zplus_Cib_W can be thought of as the articulated body
  inertia and bias of body B as though it were inertialess, but taken about Ci
  shifted to B. The role of Pplus_BCib_W and Zplus_Cib_W is clearer when
  considering the equation to compute the reaction force at the mobilizer
  constraining the motion of body B (Eq. 7.25 in [Featherstone, 2008]): <pre>
    F_B_W = Pplus_PB_W * Aplus_WB + Zplus_B_W                            (4)
  </pre>
  This equation mirrors Eq. (1) but it is written in terms of the rigidly
  shifted spatial acceleration Aplus_WB.

  The articulated body inertia Plus can be computed once P_B_W is obtained from
  Eq. (2): <pre>
     Pplus_PB_W = P_B_W - P_B_W * H_PB_W * D_B⁻¹ * H_PB_Wᵀ * P_B_W
                = P_B_W - g_B_W * U_B_W                                  (5)
  </pre>
  where D_B = H_PB_Wᵀ * P_B_W * H_PB_Wᵀ ∈ ℝᵐˣᵐ (with m the number of mobilities
  of body B), U_B_W = H_PB_Wᵀ * P_B_W ∈ ℝᵐˣ⁶, and g_B_W = H_PB_Wᵀ * D_B⁻¹ ∈
  ℝ⁶ˣᵐ. g_B_W is a useful configuration dependent quantity that appears several
  times in the ABA. The bias Zplus across the mobilizer is computed once the
  bias Z_B_W is obtained from Eq. (3): <pre>
    Zplus_B_W = Z_B_W + Pplus_PB_W * Ab_WB + g_B_W * e_B                 (6)
    e_B = tau_B - H_PB_Wᵀ * Z_B_W                                        (7)
  </pre>
  where tau_B are the applied generalized forces on body B's mobilizer. Notice
  that, given their definition in Eqs. (3) and (6), the ABA bias terms Z_B_W
  ...
  @note Zplus_B_W in Eq. (6) includes the bias Ab_WB multiplied by Pplus_PB_W.
  ...
  Once ABA inertias and bias terms are computed according to Eqs. (2)-(7),
  ...
  equations: <pre>
    A_WB = Aplus_WB + Ab_WB + H_PB * vdot_B                             (8a)
    F_B = P_B * A_WB + Z_B                                              (8b)
    tau_B = H_PB * F_B                                                  (8c)
  </pre>
  ...
  (8c) projects the reaction force F_B to obtain the externally applied
  generalized forces tau_B.
  We substitute Eqs. (8a) and (8b) into (8c) to obtain: <pre>
    D_B * vdot_B + U_B * (Aplus_WB + Ab_WB) = e_B
  </pre>
  Therefore the last base to tip pass updates generalized accelerations and
  spatial accelerations according to: <pre>
    vdot_B = D_B⁻¹ * e_B - g_Bᵀ * (Aplus_WB + Ab_WB)
    A_WB = Aplus_WB + Ab_WB + H_PB * vdot_B                              (9)
  </pre>

...
(templatized as 0, 1, or 2). The axis vector can be considered axis_F (expressed
in frame F) or axis_M (expressed in frame M) since the components are identical
in either frame.
...
in the direction of the translation axis.
...
// This is the base class for the three available prismatic mobilizers. It
// provides high-level access to the common features of a prismatic mobilizer,
...
class PrismaticMobilizer : public MobilizerImpl<T, 1, 1> {
 public:
  ...
  // inboard frame F.
  const Vector3<double>& translation_axis() const { return axis_FM(); }
 ...
 protected:
  ...
  // some axis common to both frames.
...
class PrismaticMobilizerAxial final : public PrismaticMobilizer<T> {
 public:
  ...
  using PrismaticMobilizer<T>::axis_FM;

  PrismaticMobilizerAxial(const SpanningForest::Mobod& mobod,
                          const Frame<T>& inboard_frame_F,
                          const Frame<T>& outboard_frame_M)
      : PrismaticMobilizer<T>(mobod, inboard_frame_F, outboard_frame_M) {
    Vector3<double> translation_axis = Vector3<double>::Zero();
    translation_axis[axis] = 1;
    this->set_axis_FM(translation_axis);
  }
  ...
  // Mathematically: <pre>
  //    tau = F_Mo_F.translational().dot(axis_FM)
  // </pre>

...
class BodyNodeImpl final : public BodyNode<T> {
 ...
 private:
  ...
  const T* get_q(const T* all_positions) const {
    return &all_positions[mobilizer().position_start_in_q()];
  }

  // Returns this mobilizer's qs as a fixed-size QVector.
  Eigen::Map<const QVector> get_qvector(const T* all_positions) const {
    return Eigen::Map<const QVector>(get_q(all_positions));
  }
  ...
  const T* get_v(const T* all_velocities) const {
    return &all_velocities[mobilizer().velocity_start_in_v()];
  }

  // Returns this mobilizer's vs as a fixed-size VVector.
  Eigen::Map<const VVector> get_vvector(const T* all_velocities) const {
    return Eigen::Map<const VVector>(get_v(all_velocities));
  }

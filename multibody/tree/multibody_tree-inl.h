...
const BodyType<T>& MultibodyTree<T>::AddBody(
  ...
  frame_name_to_index_.emplace(StringViewMapKey(body_frame->name()),
                               body_frame_index);
  frames_.push_back(body_frame);
  // - Register body.
  BodyType<T>* raw_body_ptr = body.get();
  body_name_to_index_.emplace(StringViewMapKey(body->name()), body->index());
...
const JointType<T>& MultibodyTree<T>::AddJoint(
  ...
  joint_name_to_index_.emplace(StringViewMapKey(joint->name()), joint->index());
...
ModelInstanceIndex MultibodyTree<T>::AddModelInstance(const std::string& name) {
  ...
  instance_name_to_index_.emplace(StringViewMapKey(name), index);

 ...
 protected:
  ...
  systems::NumericParameterIndex DeclareNumericParameter(
    ...
    return systems::NumericParameterIndex(
        internal::MultibodyTreeSystemElementAttorney<
  ...
  /// For an example use, see RigidBody::get_mass().
  /// @tparam U The type of the stored parameter. Must be a subclass of
  ///           systems::BasicVector
  ...
  const U<T>& GetNumericParameter(const systems::Context<T>& context,
    ...
    return internal::MultibodyTreeSystemElementAttorney<T>
        ::template GetNumericParameter<U>(GetParentTreeSystem(), context,
                                            index);
  }

  /// To be used by MultibodyElement-derived objects when mutating parameters.
  /// For an example, see RigidBody::SetMass().
  /// @tparam U The type of the stored parameter. Must be a subclass of
  ///           systems::BasicVector
  template <template <typename> class U = systems::BasicVector>
  U<T>& GetMutableNumericParameter(systems::Context<T>* context,
                                   int index) const {
    return internal::MultibodyTreeSystemElementAttorney<T>
        ::template GetMutableNumericParameter<U>(GetParentTreeSystem(),
                                                   context, index);
  }

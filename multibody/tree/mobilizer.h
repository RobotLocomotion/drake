...
class Mobilizer : public MultibodyTreeElement<Mobilizer<T>, MobilizerIndex> {
 public:
  ...
  /// Sets the `state` to a (potentially) random position and velocity.
  /// This default implementation simply sets the state to the _zero_ state,
  /// but derived classes may overload it to draw from a random distribution.
  /// Note that the intended caller of this method is
  /// `MultibodyTree::SetRandomState()` which treats the independent samples
  /// returned from this sample as an initial guess, but may change the value
  /// in order to "project" it onto a constraint manifold.

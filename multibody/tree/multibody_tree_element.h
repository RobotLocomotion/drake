...
class MultibodyElement {
 public:
  ...
  /// (Internal) Checks whether this MultibodyElement has been registered into
  ...
  /// (Internal) Checks whether `this` element has the same parent tree as
  /// `other` and throws an exception if not.
  /// @throws std::logic_error if either or both elements do not
  /// have a parent tree.
  template <template <typename> class OtherElementType,
            typename OtherElementIndexType>
  void HasSameParentTreeOrThrow(
      const MultibodyElement<OtherElementType, T, OtherElementIndexType>& other)
      const {
    this->HasParentTreeOrThrow();
    other.HasParentTreeOrThrow();
    if (parent_tree_ != other.parent_tree_) {
      throw std::logic_error(
          "These two MultibodyElement objects do not belong to "
          "the same MultibodyTree.");
    }
  }

  /// (Internal) Checks whether this MultibodyElement belongs to the provided
 ...
 protected:
  ...
  /// Gives MultibodyElements the opportunity to retrieve their topology

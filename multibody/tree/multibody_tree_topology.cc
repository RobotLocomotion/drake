...
bool MultibodyTreeTopology::operator==(
  ...
  if (body_node_topology_ != other.body_node_topology_) return false;
...
void MultibodyTreeTopology::FinalizeTopology(const LinkJointGraph& graph) {
  ...
  body_node_topology_.reserve(ssize(forest.mobods()));
  for (const auto& mobod : forest.mobods()) {
    ...
    if (node_index != 0) {         // Skip if World.
      ...
      parent_node = rigid_body_topology_[parent_body_index].mobod_index;
      body_node_topology_[parent_node].child_nodes.push_back(node_index);
    }

    // Creates a BodyNodeTopology.
    DRAKE_DEMAND(node_index == ssize(body_node_topology_));
    body_node_topology_.emplace_back(node_index, mobod.level(), parent_node,
                                     rigid_body_index, parent_body_index);

    // Copy coordinate assignments from the MobilizerTopology.
    BodyNodeTopology& node = body_node_topology_.back();
...
void MultibodyTreeTopology::ExtractForestInfo(const LinkJointGraph& graph) {
  ...
  const BodyNodeTopology& root = get_body_node_topology(MobodIndex(0));

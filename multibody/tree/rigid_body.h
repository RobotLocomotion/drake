...
class BodyFrame final : public Frame<T> {
 ...
 private:
  // RigidBody<T> and BodyFrame<T> are natural allies. A BodyFrame object is
  // created every time a RigidBody object is created and they are associated
  // with each other.
  ...
  // of BodyFrame.
  explicit BodyFrame(const RigidBody<T>& body) : Frame<T>(body.name(), body) {}
...
class RigidBodyAttorney {
 private:
  ...
  // namespace.
  static BodyFrame<T>& get_mutable_body_frame(RigidBody<T>* body) {
...
class RigidBody : public MultibodyElement<T> {
 public:
  ...
  /// This method is mostly intended to be called by

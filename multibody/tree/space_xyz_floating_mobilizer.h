...
// Translational motions of M in F are parametrized by the postion `p_FM` of
// frame M in F, measure in frame F.
...
// valid for θᵢ ∈ ℝ. The only operations that are not well defined are those
// related with the kinematic mapping from angular
// velocity `w_FM` to Euler angles' rates, namely MapVelocityToQDot() and
// CalcNMatrix().
//
...
class SpaceXYZFloatingMobilizer final : public MobilizerImpl<T, 6, 6> {
 public:
  ...
  // following order: q = [θ₁, θ₂, θ₃, px_FM, py_FM, pz_FM] that is, rpy angles
  // are stored in the first 3 entries of the configuration vector, followed by
  // the three translational coordinates.
  ...
  // the components of the angular velocity `w_FM` are stored in the first 3

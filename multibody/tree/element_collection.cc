...
Element<T>& ElementCollection<T, Element, Index>::AddImpl(
  ...
  if (index == next_index()) {
    ...
    names_.emplace(std::string(element->name()), index);
    // If the newly-added element is the last non-null element then we can just
    // tack it on the end of the packed lists. Othewise, we'll recalculate the
    // packed lists from scratch. Inserting in the middle (and shifting down)
    // won't be meaningfully cheaper than just re-filling the whole thing.
    if (indices_packed_.empty() || index > indices_packed_.back()) {
      elements_packed_.push_back(element);
      indices_packed_.push_back(index);
    } else {

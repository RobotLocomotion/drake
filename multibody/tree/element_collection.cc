...
void ElementCollection<T, Element, Index>::Remove(Index index) {
  DRAKE_DEMAND(elements_by_index_.at(index) != nullptr);
  elements_packed_.erase(
      std::remove(elements_packed_.begin(), elements_packed_.end(),
                  elements_by_index_[index].get()),
      elements_packed_.end());
  indices_packed_.erase(
      std::remove(indices_packed_.begin(), indices_packed_.end(), index),
      indices_packed_.end());
  const std::string name{elements_by_index_[index]->name()};

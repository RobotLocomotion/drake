...
empty. The namespace name will never start or end with "::".
...
class ScopedName final {
 public:
  ...
  or trailing "::" on the names are automatically removed before joining. After
  joining, the final word after all "::"s is the element name, and everything
  prior is the namespace name. */
  static ScopedName Join(std::string_view name1, std::string_view name2);

  /** Parses the given `scoped_name` string. Any leading or trailing "::" on the
  ...
  typically the model instance name. When empty, it denotes global scope (i.e.,
  the world). */

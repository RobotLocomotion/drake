...
void BodyNodeImpl<T, ConcreteMobilizer>::CalcMassMatrixContribution_TipToBase(
  ...
  if constexpr (kNv != 0) {
    ...
    const auto H_CpC_W = get_H(H_PB_W_cache);
...
#define DEFINE_MASS_MATRIX_HELPER(Rnv)                                         \
  template <typename T, template <typename> class ConcreteMobilizer>           \
  void BodyNodeImpl<T, ConcreteMobilizer>::                                    \
      CalcMassMatrixOffDiagonalHelper##Rnv(                                    \
    ...
    if constexpr (kNv != 0) {                                                  \
      ...
      const int body_start_in_v = this->velocity_start_in_v();                 \
...
DEFINE_MASS_MATRIX_HELPER(1)
DEFINE_MASS_MATRIX_HELPER(2)
DEFINE_MASS_MATRIX_HELPER(3)
DEFINE_MASS_MATRIX_HELPER(4)
DEFINE_MASS_MATRIX_HELPER(5)
DEFINE_MASS_MATRIX_HELPER(6)

#undef DEFINE_MASS_MATRIX_HELPER

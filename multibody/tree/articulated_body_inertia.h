...
class ArticulatedBodyInertia {
 public:
  ...
  /// Subtracts `P_BQ_E` from `this` articulated body inertia. `this` ABI,
  /// `P_BQ_E`, and the result are for the same articulated body B, about the
  /// same point Q and expressed in the same frame E.
  ArticulatedBodyInertia<T>&
  operator-=(const ArticulatedBodyInertia<T>& P_BQ_E) {
    matrix_.template triangularView<Eigen::Lower>() = matrix_ - P_BQ_E.matrix_;
    return *this;

...
class VelocityKinematicsCache {
 ...
 private:
  // Allocates resources for this velocity kinematics cache. Initializes most
  // quantities to NaN, but sets V_FM to zero since many mobilizers have
  // elements that forever remain zero. For example, a weld need not set any
  // part of V_FM, a revolute only needs to set the rotational part, and a
  // prismatic only needs to set the translational part.
  void Allocate();

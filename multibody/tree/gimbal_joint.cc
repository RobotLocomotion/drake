...
std::unique_ptr<Joint<ToScalar>> GimbalJoint<T>::TemplatedDoCloneToScalar(
  ...
  auto joint_clone = std::make_unique<GimbalJoint<ToScalar>>(
      this->name(), frame_on_parent_body_clone, frame_on_child_body_clone,
      this->position_lower_limits(), this->position_upper_limits(),
      this->damping());
...
template <typename T>
std::unique_ptr<Joint<double>> GimbalJoint<T>::DoCloneToScalar(
    const internal::MultibodyTree<double>& tree_clone) const {
  return TemplatedDoCloneToScalar(tree_clone);
}

template <typename T>
std::unique_ptr<Joint<AutoDiffXd>> GimbalJoint<T>::DoCloneToScalar(
    const internal::MultibodyTree<AutoDiffXd>& tree_clone) const {
  return TemplatedDoCloneToScalar(tree_clone);
}

template <typename T>
std::unique_ptr<Joint<symbolic::Expression>> GimbalJoint<T>::DoCloneToScalar(
...
GimbalJoint<T>::MakeImplementationBlueprint() const {

...
const RigidBody<T>& MultibodyTree<T>::AddEphemeralLink(
  ...
  // `M_BBo_B` is the shadow's default (user-facing) share of the split; the
...
void MultibodyTree<T>::Finalize() {
  ...
  /* Add the ephemeral shadow Links. BuildForest() creates a shadow Link for
  each loop it breaks; we must materialize a corresponding RigidBody before
  creating Joint implementations (which reference the shadow Mobods' active
  ...
  for (LinkOrdinal ordinal(graph.num_user_links()); ordinal < graph.num_links();
    ...
    const int num_copies = graph.link_by_index(primary_index).num_shadows() + 1;
    ...
    primary's. A ShadowFrame has no pose parameter of its own; it delegates to
    the user's frame, which stays the single source of truth.
    ...
    const std::vector<LinkIndex>& shadows = primary.shadow_links();
    const auto shadow_iter =
        std::find(shadows.begin(), shadows.end(), graph_link.index());
    DRAKE_DEMAND(shadow_iter != shadows.end());
    ...
    const Frame<T>& shadow_frame =
        AddEphemeralFrame(std::make_unique<ShadowFrame<T>>(
            joint.MakeUniqueOffsetFrameName(source_frame, "shadow"), shadow,
            source_frame, joint.model_instance()));
...
void MultibodyTree<T>::CalcSpatialInertiasInWorld(
  ...
  for (MobodIndex mobod_index(1); mobod_index < num_mobods(); ++mobod_index) {
    ...
    const SpatialInertia<T>& M_BBo_B =
        frame_body_pose_cache.get_M_BBo_B(mobod_index);
    // Re-express body B's spatial inertia in the world frame W.
    SpatialInertia<T>& M_BBo_W = (*M_B_W_all)[mobod_index];
...
void MultibodyTree<T>::CalcFrameBodyPoses(
  ...
  for (const SpanningForest::Mobod& mobod : forest().mobods()) {
    ...
    for (const LinkOrdinal& link_ordinal : mobod.follower_link_ordinals()) {
      ...
      // the number of copies (primary + shadows). Because a shadow's link frame
      // coincides with its primary's (they are welded at identity), the
      // body-frame inertia values are identical, so a shadow and its primary
      // end up with the same split M_LLo_L. A link with no shadows is its own
      // primary with num_copies == 1, i.e. unchanged.

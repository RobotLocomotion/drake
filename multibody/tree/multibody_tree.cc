...
void MultibodyTree<T>::set_default_floating_joint_type(
  ...
  DRAKE_THROW_UNLESS(
      floating_joint_type == QuaternionFloatingJoint<T>::kTypeName ||
...
void MultibodyTree<T>::CreateJointImplementations() {
  ...
  for (BodyIndex body_index(1); body_index < num_bodies(); ++body_index) {
    ...
    if (default_floating_joint_type() == SpaceXYZFloatingJoint<T>::kTypeName) {
      this->AddJoint<SpaceXYZFloatingJoint>(floating_joint_name, world_body(),
                                            {}, body, {});
    }
  }
...
void MultibodyTree<T>::FinalizeInternals() {
  ...
  for (int i = 0; i < num_joints(); ++i) {
    ...
    if (body.is_floating()) {
      ...
      // TODO(amcastro-tri): Consider a FloatingJoint API?
      auto* floating_joint =
          dynamic_cast<Joint<T>*>(joint.get());
      DRAKE_DEMAND(floating_joint != nullptr);
      const auto [quaternion, translation] =
          GetDefaultFreeBodyPoseAsQuaternionVec3Pair(body);
      const RigidTransform<double> pose(quaternion, translation);
      floating_joint->SetDefaultPose(pose);

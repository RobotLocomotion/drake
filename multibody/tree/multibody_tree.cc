...
void MultibodyTree<T>::Finalize() {
  ...
  if (!graph.loop_constraints().empty()) {
    throw std::runtime_error(fmt::format(
        ...
        "in the input and use LinearBushingRollPitchYaw or constraints "
        "to close the loops. See https://drake.mit.edu/getting_help.html "
        "if you need some advice on how to do that."));
  }

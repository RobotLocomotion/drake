...
class Joint : public MultibodyElement<Joint, T, JointIndex> {
 public:
  ...
  /// expected to implement the NVI do_set_default_positions(). It is the
  /// responsibility of the subclass to ensure that their joint implementation,
  /// should they have one, is notified of a change in default
  /// positions.
 ...
 protected:
  ...
  /// details. Implementations must meet the styleguide requirements for
  ...
  /// Implementation to the NVI set_default_posisions(), see
  /// set_default_positions() for details. Implementations must meet the
  /// styleguide requirements for snake_case accessor methods.
  virtual void do_set_default_positions() = 0;

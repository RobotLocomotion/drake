...
void MultibodyTreeSystem<T>::Finalize() {
  ...
  auto& Ab_WB_cache_entry = this->DeclareCacheEntry(
      ...
      [tree = tree_.get()]() {
        std::vector<SpatialAcceleration<T>> cache_model(tree->num_bodies());
        cache_model[world_index()].SetNaN();
        return AbstractValue::Make(cache_model);
      },
      [tree = tree_.get()](const systems::ContextBase& context_base,
                           AbstractValue* cache_value) {
        auto& context = dynamic_cast<const Context<T>&>(context_base);
  ...
  auto& Zb_Bo_W_cache_entry = this->DeclareCacheEntry(
      ...
      [tree = tree_.get()]() {
        std::vector<SpatialForce<T>> cache_model(tree->num_bodies());
        cache_model[world_index()].SetNaN();
        return AbstractValue::Make(cache_model);
      },
      [tree = tree_.get()](const systems::ContextBase& context_base,

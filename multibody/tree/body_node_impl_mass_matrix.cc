...
void BodyNodeImpl<T, ConcreteMobilizer>::
  ...
  if constexpr (kNv != 0) {
    ...
    while (parent_node->mobod_index() != world_mobod_index()) {
      ...
      CalcMassMatrixOffDiagonalInWorldDispatcher<T, kNv>::Dispatch(
...
#define DEFINE_MASS_MATRIX_OFF_DIAGONAL_BLOCK_IN_WORLD(Bnv)                  \
  template <typename T, class ConcreteMobilizer>                             \
  void BodyNodeImpl<T, ConcreteMobilizer>::                                  \
      CalcMassMatrixOffDiagonalBlockInWorld##Bnv(                            \
...
DEFINE_MASS_MATRIX_OFF_DIAGONAL_BLOCK_IN_WORLD(1)
DEFINE_MASS_MATRIX_OFF_DIAGONAL_BLOCK_IN_WORLD(2)
DEFINE_MASS_MATRIX_OFF_DIAGONAL_BLOCK_IN_WORLD(3)
DEFINE_MASS_MATRIX_OFF_DIAGONAL_BLOCK_IN_WORLD(4)
DEFINE_MASS_MATRIX_OFF_DIAGONAL_BLOCK_IN_WORLD(5)
DEFINE_MASS_MATRIX_OFF_DIAGONAL_BLOCK_IN_WORLD(6)

#undef DEFINE_MASS_MATRIX_OFF_DIAGONAL_BLOCK_IN_WORLD
...
void BodyNodeImpl<T, ConcreteMobilizer>::
  ...
  if constexpr (kNv != 0) {
    ...
    while (parent_node->mobod_index() != world_mobod_index()) {
      ...
      CalcMassMatrixOffDiagonalInMDispatcher<T, kNv>::Dispatch(
...
// This is the inner loop of the CalcMassMatrixInM() algorithm. Bnv is the
// size of the outer-loop body node B's mobilizer, kNv is the size of the
// current inboard body P's ConcreteMobilizer encountered on B's inboard sweep.
// Cost depends on the Hᵀ⋅F inline for P's mobilizer (very cheap
// for 1-dof mobilizers).
#define DEFINE_MASS_MATRIX_OFF_DIAGONAL_BLOCK_IN_M(Bnv)                       \
  template <typename T, class ConcreteMobilizer>                              \
  void BodyNodeImpl<T, ConcreteMobilizer>::                                   \
      CalcMassMatrixOffDiagonalBlockInM##Bnv(                                 \
          const T* positions, const PositionKinematicsCacheInM<T>& pcm,       \

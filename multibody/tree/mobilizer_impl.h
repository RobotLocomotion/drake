...
class MobilizerImpl : public Mobilizer<T> {
 public:
  ...
  void SetDefaultPosePair(const Eigen::Quaternion<double> q_FM,
                          const Vector3<double>& p_FM) final {
    default_position_ = ExtractDoubleOrThrow(DoPoseToState(q_FM, p_FM));
  }
  ...
  void SetPosePair(const systems::Context<T>&, const Eigen::Quaternion<T> q_FM,
                   ...
                   systems::State<T>* state) const final {
    get_mutable_positions(&*state) = DoPoseToState(q_FM, p_FM);
  }

  void SetSpatialVelocity(const systems::Context<T>&,
                          const SpatialVelocity<T>& V_FM,
                          systems::State<T>* state) const final {
    get_mutable_velocities(&*state) = DoVelocityToState(V_FM);
  }
 ...
 protected:
  ...
  virtual Vector<T, kNq> DoPoseToState(const Eigen::Quaternion<T> orientation,
                                       const Vector3<T>& translation) const {
    unused(orientation, translation);
    DRAKE_DEMAND(!"DoPoseToState() not implemented by this mobilizer");
    DRAKE_UNREACHABLE();
  }
  ...
  virtual Vector<T, kNv> DoVelocityToState(
      const SpatialVelocity<T>& velocity) const {
    unused(velocity);
    DRAKE_DEMAND(!"DoVelocityToState() not implemented by this mobilizer");
    DRAKE_UNREACHABLE();
  }

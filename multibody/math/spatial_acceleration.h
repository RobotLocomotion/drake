...
/// (Advanced) Spatial accelerations can not be composed by simple addition of
/// their expressed-in components. Refer to ComposeWithMovingFrameAcceleration()
/// for composing accelerations in a physical manner.
/// This only provides a convenience operator that performs the vector
/// addition of the components of two spatial accelerations expressed in a same
/// frame E.
/// To be more explicit: in general, `A_MB_E ≠ A_MA_E + A_AB_E`.
template <typename T>
inline SpatialAcceleration<T> operator+(const SpatialAcceleration<T>& A1_E,
                                        const SpatialAcceleration<T>& A2_E) {
  return SpatialAcceleration<T>(A1_E.get_coeffs() + A2_E.get_coeffs());
}

...
class SpatialForce : public SpatialVector<SpatialForce, T> {
 ...
 public:
  ...
  /// product with the spatial velocity `V_WBp_E` of body B at point P,
  /// measured in an inertial (Newtonian) frame W and expressed in the
  //  same frame E in which the spatial force is expressed.
  ...
  /// @retval The power of `this` spatial force F_Bp_E in frame W.
  /// @note The utility of this method is questionable unless frame W is an
  //   a world frame (i.e., frame W is an inertial/Newtonian frame).
  inline T dot(const SpatialVelocity<T>& V_WBp_E) const;

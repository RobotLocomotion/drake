...
class SpatialForce : public SpatialVector<SpatialForce, T> {
 ...
 public:
  ...
  /// For an arbitrary frame B, calculates the dot-product of F_B_E (frame B's
  /// spatial force expressed-in frame E, stored in `this`) with V_WB_E (frame
  /// B's spatial velocity measured in frame W, expressed-in frame E). The
  /// resulting scalar is the power generated by the spatial force in frame W.
  /// @param[in] V_WB_E frame B's spatial velocity measured in frame W,
  /// expressed in the same frame E as `this` spatial force F_B_E.
  ...
  inline T dot(const SpatialVelocity<T>& V_WB_E) const;

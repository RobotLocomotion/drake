...
/* DeformationGradientData stores per element data that work in tandem with
 ...
 @tparam num_locations Number of locations at which the data are evaluated. */
template <template <typename, int> class DerivedDeformationGradientData,
          typename T, int num_locations>
class DeformationGradientData<
    DerivedDeformationGradientData<T, num_locations>> {
 public:
  using Derived = DerivedDeformationGradientData<T, num_locations>;
  ...
  void UpdateData(const std::array<Matrix3<T>, num_locations>& F) {
    deformation_gradient_ = F;
    static_cast<Derived*>(this)->DoUpdateData(F);
  }

  /* The number of quadrature locations at which the data needs to be evaluated.
   */

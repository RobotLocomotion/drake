...
typename LinearSimplexElement<T, natural_dimension_at_compile_time,
    ...
    template Samples<typename IsoparametricElement<
    ...
    LinearSimplexElement<T, natural_dimension_at_compile_time,
        ...
        CalcGradientInSpatialCoordinates(
            const Eigen::Ref<const Eigen::Matrix<T, spatial_dimension,
                                                 num_nodes>>& xa) const {
  return this->DoCalcGradientInSpatialCoordinates(xa);
}
...
typename LinearSimplexElement<T, natural_dimension_at_compile_time,
    ...
    LinearSimplexElement<T, natural_dimension_at_compile_time,
        ...
        CalcJacobianPseudoinverse(
            const Samples<JacobianMatrix>& jacobian) const {
  return this->DoCalcJacobianPseudoinverse(jacobian);
}
...
typename LinearSimplexElement<T, natural_dimension_at_compile_time,
                              spatial_dimension_at_compile_time,
                              num_sample_locations_at_compile_time>::
    template Samples<typename LinearSimplexElement<
        T, natural_dimension_at_compile_time, spatial_dimension_at_compile_time,
        num_sample_locations_at_compile_time>::ShapeFunctions>
    LinearSimplexElement<
        T, natural_dimension_at_compile_time, spatial_dimension_at_compile_time,
        num_sample_locations_at_compile_time>::GetShapeFunctionsHelper() const {
  Samples<ShapeFunctions> S;
  const Locations& locations = this->locations();
...
const typename LinearSimplexElement<T, natural_dimension_at_compile_time,
  ...
  Samples<GradientInParentCoordinates> dSdxi;
  dSdxi.fill(dSdxi_q);
  return dSdxi;
}

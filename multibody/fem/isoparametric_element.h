...
class IsoparametricElement {
 public:
  ...
  template <typename U>
  using Samples = std::array<U, num_sample_locations>;

  using T = typename DerivedTraits::Scalar;

  /* Alias for std::array of sample locations in the parent domain at which to
   evaluate and/or interpolate numerical quantities from nodes. */
  using Locations = Samples<Vector<double, natural_dimension>>;
  ...
  const Samples<ShapeFunctions>& GetShapeFunctions() const {
  ...
  /* Obtains the gradient of the shape functions in parent coordinates, dS/dξ,
   ...
   node `a`. */
  const Samples<GradientInParentCoordinates>& GetGradientInParentCoordinates()
      const {
    const DerivedElement& derived = static_cast<const DerivedElement&>(*this);
  ...
  Samples<GradientInSpatialCoordinates> CalcGradientInSpatialCoordinates(
  ...
  Samples<JacobianMatrix> CalcJacobian(
      const Eigen::Ref<const Eigen::Matrix<T, spatial_dimension, num_nodes>>&
          xa) const {
    const DerivedElement& derived = static_cast<const DerivedElement&>(*this);
  ...
  /* Computes dξ/dx, the pseudoinverse Jacobian matrix, at each sample location
   ...
   @see CalcJacobian(). */
  Samples<PseudoinverseJacobianMatrix> CalcJacobianPseudoinverse(
  ...
  template <int dim>
  Samples<Vector<T, dim>> InterpolateNodalValues(
      const Eigen::Ref<const Eigen::Matrix<T, dim, num_nodes>>& ua) const {
    const Samples<Vector<T, num_nodes>>& S = GetShapeFunctions();
    Samples<Vector<T, dim>> interpolated_value;
 ...
 protected:
  ...
  explicit IsoparametricElement(Locations locations)
      : locations_(std::move(locations)) {}

  /* Default implementation of CalcGradientInSpatialCoordinates(). */
  Samples<GradientInSpatialCoordinates> DoCalcGradientInSpatialCoordinates(
      const Eigen::Ref<const Eigen::Matrix<T, spatial_dimension, num_nodes>>&
          xa) const {
    Samples<GradientInSpatialCoordinates> dSdX;
  ...
  /* Default implementation of CalcJacobian(). */
  Samples<JacobianMatrix> DoCalcJacobian(
      const Eigen::Ref<const Eigen::Matrix<T, spatial_dimension, num_nodes>>&
          xa) const {
    Samples<JacobianMatrix> dxdxi;
    const Samples<Eigen::Matrix<T, num_nodes, natural_dimension>>& dSdxi =
  ...
  /* Default implementation of CalcJacobianPseudoinverse(). */
  Samples<PseudoinverseJacobianMatrix> DoCalcJacobianPseudoinverse(
      const Samples<JacobianMatrix>& jacobian) const {
    /* Suppose the Jacobian matrix J = dx/dξ is m×n where m >= n. We are looking
     ...
     implies AᵀJᵀ = JA. */
    Samples<PseudoinverseJacobianMatrix> dxidx;
    for (int q = 0; q < num_sample_locations; ++q) {

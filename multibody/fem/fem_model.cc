...
void FemModel<T>::CalcTangentMatrix(
  ...
  if constexpr (std::is_same_v<T, double>) {
    ...
    throw std::logic_error(
        "FemModel::CalcTangentMatrix() does not support AutoDiffXd at the "
        "moment.");
  }
}
...
FemModel<T>::MakePetscSymmetricBlockSparseTangentMatrix() const {
  if constexpr (std::is_same_v<T, double>) {
    ...
    throw std::logic_error(
        "FemModel::MakePetscSymmetricBlockSparseTangentMatrix() does not "
        "support AutoDiffXd at the moment.");
  }
}

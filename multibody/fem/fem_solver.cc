...
void FemSolver<T>::set_linear_solve_tolerance(const T& residual_norm) const {
  ...
  // TODO(xuchenhan-tri): There is a unit mismatch here: relative tolerance has
  // no unit but residual norm has the unit of force. The residual norm should
  // be scaled by a characteristic value.
  constexpr double kLinearToleranceFactor = 0.2;
  const double linear_solve_tolerance =
      std::min({0.5, kLinearToleranceFactor * relative_tolerance_,

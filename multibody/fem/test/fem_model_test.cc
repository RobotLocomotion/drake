...
GTEST_TEST(FemModelTest, Constructor) {
  DummyModel model;
  auto builder = make_unique<DummyModel::DummyBuilder>(&model);
...
GTEST_TEST(FemModelTest, MultipleBuilders) {
  DummyModel model;
  auto builder0 = make_unique<DummyModel::DummyBuilder>(&model);
  builder0->AddElementWithDistinctNodes();
  builder0->Build();
  EXPECT_EQ(model.num_nodes(), DummyElement::Traits::num_nodes);

  auto builder1 = make_unique<DummyModel::DummyBuilder>(&model);
  ...
  builder0->AddElementWithDistinctNodes();
  builder0->Build();
  EXPECT_EQ(model.num_nodes(), 3 * DummyElement::Traits::num_nodes);

  /* Repeatedly calling Build without specifying new elements is No-op. */
  builder0->Build();
  builder0->Build();
  EXPECT_EQ(model.num_nodes(), 3 * DummyElement::Traits::num_nodes);
}

...
GTEST_TEST(FemModelTest, DirichletBoundaryCondition) {
  ...
  DummyModel model_with_out_bc;
  DummyModel::DummyBuilder builder1(&model_with_out_bc);
  builder1.AddElementWithDistinctNodes();
  builder1.Build();
   ...
   is applied from the model or directly from the BC.*/
  {
    ...
    unique_ptr<FemState<T>> state1 = model_with_out_bc.MakeFemState();
    ...
    VectorXd residual1(model_with_out_bc.num_dofs());
    model_with_out_bc.CalcResidual(*state1, &residual1);
   ...
   whether BC is applied from the model or directly from the BC.*/
  {
    ...
    unique_ptr<internal::PetscSymmetricBlockSparseMatrix> tangent_matrix1 =
        model.MakePetscSymmetricBlockSparseTangentMatrix();
    model_with_out_bc.CalcTangentMatrix(*state1, weights,
                                        tangent_matrix1.get());

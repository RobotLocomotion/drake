...
GTEST_TEST(SchurComplementTest, ConstructorWithPrescribedDComplement) {
  Matrix2d D_complement, neg_Dinv_B_transpose;
  // clang-format off
  neg_Dinv_B_transpose << 1, 1,
                         -1, 0;
  D_complement << 4, 1,
                  1, 4;
  // clang-format on
  const SchurComplement<double> s(D_complement, neg_Dinv_B_transpose);
  EXPECT_EQ(s.get_D_complement(), D_complement);
  const Vector2d x(1, 2);
  const VectorXd y = s.SolveForY(x);
  EXPECT_TRUE(CompareMatrices(y, neg_Dinv_B_transpose * x));
}

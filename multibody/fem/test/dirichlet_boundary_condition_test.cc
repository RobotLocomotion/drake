 ...
 protected:
  void SetUp() {
    ...
    fem_state_system_ = make_unique<FemStateSystem<double>>(
        Vector<double, kNumDofs>(0.1, 0.2, 0.3),
        Vector<double, kNumDofs>(0.5, 0.6, 0.7),
        Vector<double, kNumDofs>(0.9, 1.0, 1.1));
  ...
  static VectorXd MakeResidual() {
    return Vector<double, kNumDofs>(1.1, 2.2, 3.3);
  }

  /* Makes an arbitrary SPD tangent matrix with appropriate size in PETSc
   format. The PETSc matrix contains a single block matrix of size 3-by-3.*/
...
TEST_F(DirichletBoundaryConditionTest, ApplyHomogeneousBoundaryCondition) {
  ...
  const Vector<double, kNumDofs> b_expected(0, 2.2, 0);

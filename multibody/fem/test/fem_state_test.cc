  ...
  void SetUp() {
    ...
    std::function<void(const systems::Context<T>&, std::vector<Data<T>>*)>
        calc_element_data = [this](const systems::Context<T>& context,
          ...
          const FemState<T> fem_state(fem_state_manager_.get(), &context);
...
TYPED_TEST(FemStateTest, LiveFemState) {
  using T = TypeParam;
  auto context = this->fem_state_manager_->CreateDefaultContext();
  FemState<T> state(this->fem_state_manager_.get(), context.get());
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(state.SetPositions(-1.23 * q<T>()),
                              "Trying to mutate a live FemState.");
...
TYPED_TEST(FemStateTest, ElementData) {
  ...
  for (FemElementIndex i(0); i < kNumElements; ++i) {
    EXPECT_EQ(
        state.template EvalElementData<Data<T>>(this->cache_index_, i).val,
        positions(i));
  ...
  EXPECT_THROW(state.template EvalElementData<Data<T>>(this->cache_index_,
                                                       invalid_index),
               std::exception);
}

...
class FemSolver {
 public:
  ...
  /* Sets the relative tolerance, unitless. The Newton-Raphson iterations are
   ...
   Newton-Raphson iterations. In other words, the iterations are considered as
   converged if ‖r‖ < εᵣ * ‖r₀‖, where r and r₀ are the residual and the
   initial residual respectively. The default value is 1e-4. */
  ...
  /* Sets the absolute tolerance with unit Newton. The Newton-Raphson iterations
   ...
   absolute tolerance. In other words, the iterations are considered as
   converged if ‖r‖ < εₐ, where r is the residual. The default value is 1e-6. */
 ...
 private:
  ...
  /* Uses a Newton-Raphson solver to solve for the unknown z such that the
   residual is zero, i.e. b(z) = 0, up to the specified tolerances. See
   set_relative_tolerance() and set_absolute_tolerance() for convergence
   criteria. The input FEM state is non-null and is guaranteed to be compatible
   with the FEM model.

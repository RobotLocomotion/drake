...
class FemSolver {
 public:
  ...
  /* The solver is considered as converged if ‖r‖ <= max(εᵣ * ‖r₀‖, εₐ) where r
   and r₀ are `residual_norml` and `initial_residual_norm` respectively, and εᵣ
 ...
 private:
  ...
  /* Updates the relative tolerance for the linear solver used in the
   ...
   tol = min(0.5, k * εᵣ, ‖r‖), where k is a constact scaling factor < 1, εᵣ is
   the relative tolerance for newton iterations (see set_relative_tolerance()),
   and ‖r‖ is `residual_norm`. No-op if the linear solver is direct. */

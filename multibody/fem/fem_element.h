...
class FemElement {
 public:
  ...
  /* Calculates the external force required to keep the element's state given by
   `data` in equilibrium. The required force equals Ma-fₑ(x)-fᵥ(x, v), where M
   is the mass matrix, fₑ(x) is the elastic force, fᵥ(x, v) is the damping
   force. This is equal to the residual contributed by this element excluding
   external forces such as gravity. Notice that the result is "discrete" in

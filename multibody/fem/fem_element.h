...
class FemElement {
 public:
  ...
  /* Calculates the element residual (excluding the contribution of external
   forces) of this element evaluated at the input state. The internal residual
   equals Ma-fₑ(x)-fᵥ(x, v), where M is the mass matrix, fₑ(x) is the elastic
   force, fᵥ(x, v) is the damping force. Notice that the residual is "discrete"
  ...
  void CalcInternalResidual(const Data& data,
                            EigenPtr<Vector<T, num_dofs>> residual) const {
    DRAKE_ASSERT(residual != nullptr);
    residual->setZero();
    static_cast<const DerivedElement*>(this)->DoCalcInternalResidual(data,
                                                                     residual);
  }
  ...
  void AddScaledGravityForce(const Data& data, const T& scale,
    ...
    for (int i = 0; i < num_nodes; ++i) {
      stacked_gravity.template segment<kDim>(kDim * i) = gravity_vector;
    }
 ...
 protected:
  ...
  /* `DerivedElement` must provide an implementation for
   `DoCalcInternalResidual()` to provide the residual that is up to date given
   the `data`. The caller guarantees that `residual` is non-null and contains

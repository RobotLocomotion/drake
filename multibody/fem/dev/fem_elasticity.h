...
class FemElasticity : public FemElement<T, NaturalDim> {
 public:
  ...
  /** Constructs a new FEM elasticity element.
   ...
   element.
   @param[in] param The FemElasticityParameters required for the element
   routine.
 ...
 private:
  ...
  FemElasticityParameters<T> param_;
  ...
  /* The volume evaluated at reference configuration occupied by the quadrature
   ...
   sum f(q)*volume_[q] over all the quadrature points in the element. */

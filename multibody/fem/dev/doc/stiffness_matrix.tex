...
\begin{document}
    ...
    F_{iI}S_{IJ}\frac{\partial N^b}{\partial x_j}F_{jJ}d\XX + \int
    \frac{\partial N^a}{\partial x_p} F_{pI} F_{mK}C_{KIJL}F_{nL}
    ...
    \end{align}
    where the fifth equality uses the identity $\ssigma = \frac{1}{J}\FF \SS
    \FF^T$ and the sixth equality uses the symmetry $c_{ijkl} = c_{ijlk}$ in

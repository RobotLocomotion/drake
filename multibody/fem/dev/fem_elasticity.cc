...
FemElasticity<T, NaturalDim>::FemElasticity(
    ...
    const FemElasticityParameters<T>& param,
  ...
  std::vector<MatrixX<T>> dXdxi =
      shape.CalcJacobian(param_.reference_positions);
...
template <typename T, int NaturalDim>
void FemElasticity<T, NaturalDim>::CalcNegativeElasticForce(

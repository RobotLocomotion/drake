...
class ElasticityModel : public FemModel<T> {
 public:
  ...
  void AddElasticityElementsFromTetMesh(
    ...
    for (int i = 0; i < mesh.num_elements(); ++i) {
      ...
      Matrix3X<T> element_reference_positions(
          3, geometry::VolumeMesh<T>::kVertexPerElement);
      for (int j = 0; j < geometry::VolumeMesh<T>::kVertexPerElement; ++j) {
        const geometry::VolumeVertexIndex vid = e.vertex(j);
        node_indices[j] = NodeIndex(static_cast<int>(vid));
      ...
      AddElasticityElement<IsoparametricElementType, QuadratureType>(
          node_indices, density, std::move(constitutive_model_clone),
 ...
 protected:
  ...
  std::unique_ptr<FemState<T>> DoMakeFemState() const;
 ...
 private:
  ...
  void AddElasticityElement(
    ...
    for (int i = 0; i < static_cast<int>(node_indices.size()); ++i) {
      /* Register new nodes if there is any. */
      this->AddNodeIndex(node_indices[i]);

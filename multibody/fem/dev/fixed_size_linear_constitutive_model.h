...
#include "drake/multibody/fem/dev/fixed_size_constitutive_model.h"
#include "drake/multibody/fem/dev/fixed_size_linear_constitutive_model_cache_entry.h"
...
class FixedSizeLinearConstitutiveModel final
    : public FixedSizeConstitutiveModel<
          FixedSizeLinearConstitutiveModel<T, NumLocations>> {
 public:
  using Base = FixedSizeConstitutiveModel<
      FixedSizeLinearConstitutiveModel<T, NumLocations>>;
  using DeformationGradientCacheEntryType =
      FixedSizeLinearConstitutiveModelCacheEntry<T, NumLocations>;

  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(FixedSizeLinearConstitutiveModel)

  /** Constructs a %FixedSizeLinearConstitutiveModel constitutive model with the
   ...
   0.5. */
  FixedSizeLinearConstitutiveModel(const T& youngs_modulus,
  ...
  ~FixedSizeLinearConstitutiveModel() = default;
 ...
 private:
  ...
  /* Implements the NVI FixedSizeConstitutiveModel::CalcElasticEnergyDensity().
   */
  void DoCalcElasticEnergyDensity(
      const FixedSizeLinearConstitutiveModelCacheEntry<T, NumLocations>&
          cache_entry,
  ...
  /* Implements the NVI FixedSizeConstitutiveModel::CalcFirstPiolaStress().
   */
  void DoCalcFirstPiolaStress(const FixedSizeLinearConstitutiveModelCacheEntry<
                                  T, NumLocations>& cache_entry,
  ...
  /* Implements the NVI
   FixedSizeConstitutiveModel::CalcFirstPiolaStressDerivative(). */
  ...
  /* Creates a FixedSizeLinearElasticityModelCacheEntry that is compatible
   with this FixedSizeLinearConstitutiveModel. */
  FixedSizeLinearConstitutiveModelCacheEntry<T, NumLocations>
  DoMakeDeformationGradientCacheEntry(ElementIndex element_index) const {
    return FixedSizeLinearConstitutiveModelCacheEntry<T, NumLocations>(
        element_index);
  }

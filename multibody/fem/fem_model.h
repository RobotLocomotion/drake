...
class FemModel {
  ...
  class Builder {
   public:
    ...
    /** Adds the FEM elements described by calls to this builder to this
     associated FemModel. `Build` can be called repeatedly on a single %Builder.
    */
    void Build() {
      DoBuild();
   ...
   protected:
    /** Constructs a new builder that builds into the given `model`. The base
     Builder should not be directly constructed. Construct derived builders
     instead.
     @pre model != nullptr.
     @pre The `model` pointer is persisted and the pointed to FemModel must
 ...
 protected:
  ...
  /** Updates the system that manages the states and the cache entries of this
   FEM model. Must be called after each time the FEM model changes (e.g.
   adding new elements). */

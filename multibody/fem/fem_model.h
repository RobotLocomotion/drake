...
class FemModel {
 public:
  ...
  /** Calculates the residual G(x, v, a) (see class doc) evaluated at the
   given FEM state. Degrees of freedom under Dirichlet boundary condition are
   not considered as part of the model when calculating the residual, and their
   correspnding values are set to zero.
  ...
  /** Calculates an approximated tangent matrix evaluated at the given FEM state
   ...
   Degrees of freedom under Dirichlet boundary condition are not considered as
   part of the model when calculating the tangent matrix. The corresponding row
   and column in the tangent matrix is set to zero with the exception of the
   diagonal entry which is set to 1.

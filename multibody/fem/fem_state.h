...
class FemState {
 public:
  ...
  /** Creates an %FemState that allocates and accesses states and cached data
   using the provided `fem_state_manager`. The %FemState created with this
  ...
  /** Creates an %FemState that accesses states and cached data in the given
   `context`. The %FemState created with this constructor
   doesn't own the states and data.*/
  FemState(const internal::FemStateManager<T>* fem_state_manager,

...
/** %FemState provides access to private workspace FEM state and per-element
 state-dependent data. %FemState comes in two flavors, a "owned" version that
...
class FemState {
 public:
  ...
  /** Creates a "owned" version of %FemState that allocates and accesses states
  ...
  const std::vector<Data>& EvalElementData(
    ...
    return manager_->get_cache_entry(cache_index)

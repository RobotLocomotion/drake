...
as the motivating example). γ ≥ 0 and ν ∈ [0,1] are also known as the
"constraint-force mixing" (CFM) and "error reduction parameter" (ERP)
terms, respectively, in documentation for multiple simulation libraries. These
terms are not particularly intuitive on their own; the resulting physical
behavior is better interpreted using β and α instead.

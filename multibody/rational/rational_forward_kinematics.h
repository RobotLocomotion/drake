...
/** For certain robots (whose joint transforms are algebraic functions of joint
 ...
 Currently we support robots with revolute joints.
...
class RationalForwardKinematics {
 public:
  ...
  /** @param plant The plant for which we compute its forward kinematics.
  ...
  /** This is a proximy for math::RigidTransform. It captures the rigid pose of
  ...
  /** Computes the pose X_AB as a multilinear polynomial function. The
   ...
   TODO(hongkai.dai): support computing the pose of any arbitrary frame (instead
   of only the body frames).
   */
  ...
  /** Computes values of s from q_val and q_star_val, while handling the index
   ...
   q(i)). If `clamp_angle` = true and the joint is revolute, then s =
   infinity if q_val ≥ q_star_val + π, and s = -infinity if q_val ≤
   q_star_val - π.
   */
  ...
  */
  ...
  /* Given the pose of the parent frame X_AP measured in a frame A, calculates
   ...
   We use this class in our paper Finding and Optimizing Certified,
   Collision-Free Regions in Configuration Space for Robot Manipulators, where
   we often need to compute the pose of the world body expressed in a robot body
   (rather than computing the pose of a robot body in the world).
   */

...
/**
 * Compute the child link pose given the parent link pose (R_WP, p_WP) and the
 * mobilizer transform.
 */
...
RationalForwardKinematics::CalcLinkPoseAsMultilinearPolynomial(
  ...
  poses[0].expressed_body_index = expressed_body_index;
  for (int i = 1; i < static_cast<int>(path.size()); ++i) {
    CalcChildLinkPoseAsMultilinearPolynomial(q_star, path[i - 1], path[i],
                                             poses[i - 1], &(poses[i]));
    poses[i].expressed_body_index = expressed_body_index;
  }
...
void RationalForwardKinematics::
    CalcRevoluteJointChildLinkPoseAsMultilinearPolynomial(
  ...
  const Matrix3<T>& R_AP = X_AP.R_AB;
  const Vector3<T>& p_AP = X_AP.p_AB;
  Matrix3<T>& R_AC = X_AC->R_AB;
  Vector3<T>& p_AC = X_AC->p_AB;
  CalcChildPose(R_AP, p_AP, X_PF, X_MC, R_FM, p_FM, &R_AC, &p_AC);
  X_AC->expressed_body_index = X_AP.expressed_body_index;
}

template <typename T>
void RationalForwardKinematics::CalcWeldJointChildLinkPose(
...
void RationalForwardKinematics::CalcChildLinkPoseAsMultilinearPolynomial(
  ...
  if (parent_topology.parent_body.is_valid() &&
    ...
    throw std::invalid_argument(
        "CalcChildLinkPoseAsMultilinearPolynomial: parent "
  ...
  if (dynamic_cast<const internal::RevoluteMobilizer<double>*>(mobilizer) !=
    ...
    CalcWeldJointChildLinkPose(X_FM, X_PF, X_MC, X_AP, X_AC);
  } else {
    throw std::runtime_error(

...
void RationalForwardKinematics::ComputeSBounds(
  ...
  for (int i = 0; i < ssize(s_); ++i) {
    ...
    // the mobilizer cannot be a weld joint since weld joints don't introduce
    // variables into s_.
    if (IsRevolute(mobilizer)) {
      ...
      // s_box_upper = tan(min(theta_box_upper - theta_star, pi)/2)
      // If theta_box_upper >= theta_star + pi, then the upper bound on s is
      // inf.
      (*s_box_upper)(i) =
          q_box_upper(q_index) - q_star(q_index) >= M_PI
              ? kInf
              : tan((q_box_upper(q_index) - q_star(q_index)) / 2);
      // s_box_lower = tan(max(theta_box_lower - theta_star, -pi)/2)
      // If theta_box_lower <= theta_star + pi, then the lower bound on s is
      // -inf.
      (*s_box_lower)(i) =

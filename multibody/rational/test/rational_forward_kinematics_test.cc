...
TEST_F(FinalizedIiwaTest, CalcBodyPoses) {
  ...
  EXPECT_EQ(dut.s().rows(), 7);

  // q_val = 0 and q* = 0.
  CheckBodyKinematics(dut, Eigen::VectorXd::Zero(7), Eigen::VectorXd::Zero(7),
                      Eigen::VectorXd::Zero(7), world_);
  // Compute the pose in the iiwa_link[i]'s frame.
  for (int i = 0; i < 8; ++i) {
    CheckBodyKinematics(dut, Eigen::VectorXd::Zero(7), Eigen::VectorXd::Zero(7),
                        Eigen::VectorXd::Zero(7), iiwa_link_[i]);
  }
  ...
  CheckBodyKinematics(dut, q_val, Eigen::VectorXd::Zero(7), s_val, world_);
  // Compute the pose in the iiwa_link[i]'s frame.
  for (int i = 0; i < 8; ++i) {
    CheckBodyKinematics(dut, q_val, Eigen::VectorXd::Zero(7), s_val,
                        iiwa_link_[i]);
  }

...
TEST_F(RigidBodyTreeInverseDynamicsTest, TestSkewSymmetryProperty) {
  ...
  auto qd_to_coriolis_term = [&](const auto& qd_arg) {
    auto q_autodiff = q.cast<AutoDiffXd>().eval();
    auto qdd_autodiff = qdd.cast<AutoDiffXd>().eval();
    auto qd_arg_dynamic = qd_arg.template cast<AutoDiffXd>().eval();
...
TEST_F(RigidBodyTreeInverseDynamicsTest, TestAccelerationJacobianIsMassMatrix) {
  ...
  for (RigidBodyTree<double>* tree : trees_) {
    ...
    auto vd_to_mass_matrix = [&](const auto& vd_arg) {
      auto q_autodiff = q.cast<AutoDiffXd>().eval();
      auto v_autodiff = v.cast<AutoDiffXd>().eval();
      auto vd_arg_dynamic = vd_arg.template cast<AutoDiffXd>().eval();
...
TEST_F(RigidBodyTreeInverseDynamicsTest, TestGeneralizedGravitationalForces) {
  ...
  auto q_to_gravitational_potential_energy = [&](const auto& q_arg) {
    auto gravitational_force_autodiff =
        gravitational_force.cast<AutoDiffXd>().eval();
    auto q_arg_dynamic = q_arg.template cast<AutoDiffXd>().eval();

...
void SapDriver<T>::CalcContactProblemCache(
  ...
  const std::vector<std::vector<int>> unlocked_indices_per_tree =
      joint_locking_data.unlocked_velocity_indices_per_tree;
  problem.ReduceToSelectedDofs(cache->sap_problem_locked.get_mutable(),
                               locked_indices, unlocked_indices_per_tree);
}
  ...
  if constexpr (std::is_same_v<T, double>) {
    if (manager().deformable_driver_ != nullptr) {
      ...
      contact_results->v_next.head(rigid_dofs) =
          ExpandRows(sap_results_locked.v.head(unlocked_indices.size()),
                     rigid_dofs, unlocked_indices);

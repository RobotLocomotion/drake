...
void SapDriver<T>::AddPdControllerConstraints(
  ...
  for (ModelInstanceIndex model_instance_index(0);
    ...
    if (desired_states.is_armed(model_instance_index)) {
      ...
      const std::vector<JointActuatorIndex> instance_actuators =
          plant().GetJointActuatorIndices(model_instance_index);
      const int mum_pd_actuators =
          plant().NumPdControlledActuators(model_instance_index);

      // Sanity check sizes before accessing qd and vd below.
      DRAKE_DEMAND(instance_qd.size() == mum_pd_actuators);
      DRAKE_DEMAND(instance_vd.size() == mum_pd_actuators);

      int a = 0;  // Actuator index local to its model-instance.
      for (JointActuatorIndex actuator_index : instance_actuators) {
        ...
        if (!joint.is_locked(context) && actuator.has_controller()) {

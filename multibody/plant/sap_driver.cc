  ...
  *v_star = v0 + dt * vdot0;
  if constexpr (std::is_same_v<T, double>) {
    if (manager().deformable_driver_ != nullptr) {
      ...
      v_star->conservativeResize(rigid_dofs + deformable_dofs);
...
void SapDriver<T>::PackContactSolverResults(
  ...
  for (int i = 0; i < num_contacts; ++i) {
    const SapConstraint<T>& c = problem.get_constraint(i);
    {
      ...
      AddToGeneralizedContactForce(context, c.first_clique(), Jic, impulse,
                                     &tau_contact);
    }
    ...
    if (c.num_cliques() == 2) {
      ...
      const auto impulse = contact_impulses.template segment<3>(3 * i);
      AddToGeneralizedContactForce(context, c.second_clique(), Jic, impulse,
                                     &tau_contact);
    }
  }
...
template <typename T>
void SapDriver<T>::AddToGeneralizedContactForce(
    const systems::Context<T>& context, int clique,
    const MatrixX<T>& contact_jacobian, const Vector3<T>& impulse,
  ...
  if (clique >= tree_topology().num_trees()) {
    ...
    if constexpr (std::is_same_v<T, double>) {
      ...
      deformable_driver->EvalParticipatingVelocityMultiplexer(context)
          .Demultiplex(tau_contact, body_index) +=
          contact_jacobian.transpose() * impulse;
    } else {
      DRAKE_UNREACHABLE();
    }
  } else {
    ...
    tau_contact->segment(v_start, nv) += contact_jacobian.transpose() * impulse;
  }
}

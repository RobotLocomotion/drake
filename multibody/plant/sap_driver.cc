...
void SapDriver<T>::AddDistanceConstraints(const systems::Context<T>& context,
  ...
  MatrixX<T> Jddot = MatrixX<T>::Zero(1, nv);
  ...
  for (const DistanceConstraintSpecs<T>& specs :
    ...
    if (d0 < 1.0e-3 * specs.distance) {
      PRINT_VAR(p_PQ_W);
      PRINT_VAR(d0);
      throw std::logic_error("Distance constraint became way too small.");
    }
    ...
    PRINT_VAR(dissipation_time_scale);
    PRINT_VAR(specs.stiffness);
    PRINT_VAR(specs.damping);

...
void SapDriver<T>::AddDistanceConstraints(const systems::Context<T>& context,
  ...
  for (const DistanceConstraintSpecs& specs :
    ...
    // There is a limit to how large values of stiffness and dissipation can be
    // in practice before the system becomes untractable due to ill conditioning
    // of the resulting equations. Therefore, with SAP Stiffness and dissipation
    // are set so that the constraint is in the "near-rigid" regime, as
    // described in [Castro et al., 2022].
    const T dissipation_time_scale = specs.stiffness == kInfinity
                                         ? plant().time_step()
                                         : specs.damping / specs.stiffness;

...
class HydroelasticTractionCalculator {
 ...
 private:
  ...
  // Computes the function f(x) = atan(x)/x given x² as input. In general, this
  // should always be preferred over calling:
  //   double x = sqrt(x2);
  //   double f = atan(x)/x;
  //
  ...
  // most likely negligible for most applications. For the full explanation of

...
class HydroelasticTractionCalculator {
 ...
 private:
  ...
  // the dimensionless norm of a vector, which entails a squqre root, the naive
  ...
  // leads to the exact rusult of f(x) and its derivative, modulo machine
  // epsilon (in double precision).
  // N.B. We use SFINAE to provide distinct implementations for numeric and
  // non-numeric scalar types. Numeric scalar types such as double and
  // AutoDiffXd must use the numeric implementation continuous to machine
  // epsilon. Non-numeric types such as symbolic::Expression must use the true
  // expression atan(x)/x.
  template <typename S = T>
  static typename std::enable_if_t<scalar_predicate<S>::is_bool, T>
  CalcAtanXOverXFromXSquared(const T& x_squared) {
    using std::atan;
    using std::sqrt;
  ...
  // SFINAE for when T is not a numeric type.
  template <typename S = T>
  static typename std::enable_if_t<!scalar_predicate<S>::is_bool, T>
  CalcAtanXOverXFromXSquared(const T& x_squared) {
    using std::atan;

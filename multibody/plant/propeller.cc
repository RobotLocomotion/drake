...
void Propeller<T>::CalcSpatialForces(
  ...
  for (int i = 0; i < num_propellers(); i++) {
    ...
    const math::RigidTransform<double> X_PB = prop.X_BP.inverse();
    const Eigen::Vector3d& p_PoBo_P = X_PB.translation();

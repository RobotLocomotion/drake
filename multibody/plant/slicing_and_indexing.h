...
// Makes a new, possibly smaller square matrix from square matrix `M`, by
...
// Writes a new, possibly smaller square matrix into `out` from square matrix
// `M`, by selecting the rows and columns indexed by `indices`.
// @pre indices.size() <= M.rows() (or M.cols() since M is a square matrix).
// @pre indices argument is valid according to DemandIndicesValid().
template <typename T>
...
// Makes a new, possibly smaller matrix from matrix `M`, by selecting the
...
// Makes a new, possibly smaller matrix from matrix `M`, by excluding the
// columns indexed by `indices`.
// @pre indices.size() <= M.cols().
// @pre indices argument is valid according to DemandIndicesValid().
template <typename T>
...
// Makes a new, possibly smaller vector from vector `v`, by selecting the rows
// indexed by `indices`.
...
// Writes a new, possibly smaller vector into `out` from vector `v`, by
// selecting the rows indexed by `indices`.
// @pre indices.size() <= v.size().
// @pre indices argument is valid according to DemandIndicesValid().
template <typename T>
...
// Makes a new, possibly smaller vector from vector `v`, by excluding the rows
// indexed by `indices`.
// @pre indices.size() <= v.size().
// @pre indices argument is valid according to DemandIndicesValid().
template <typename T>
...
// Makes a new, possibly larger vector of size `rows_out`, by copying rows

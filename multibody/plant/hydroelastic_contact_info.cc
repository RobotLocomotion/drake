...
template <typename T>
std::shared_ptr<const geometry::ContactSurface<T>> ConstrctSharedSurface(
    const std::shared_ptr<const geometry::ContactSurface<T>>& other) {
...
HydroelasticContactInfo<T>::HydroelasticContactInfo(
    ...
    : contact_surface_(ConstrctSharedSurface(other.contact_surface_)),

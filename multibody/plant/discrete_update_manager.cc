...
void DiscreteUpdateManager<T>::CalcDiscreteValues(
  ...
  if (memory != nullptr) {
    ...
    // XXX need geometry_contact_data here
    // XXX need discrete_contact_pairs here
...
#if 0  // XXX
  AppendContactResultsForDeformableContact(..., contact_results);
#endif
}
...
void DiscreteUpdateManager<T>::AppendContactResultsForHydroelasticContact(
  ...
  if constexpr (scalar_predicate<T>::is_bool) {
    ...
    for (const HydroelasticContactInfo<T>& info : temp) {
      ...
      // `info` is for the lifetime of the contact results. Also why were
      // we using an intermediate vector instead of blitting directly into
      // the output argument?
      DRAKE_UNREACHABLE();

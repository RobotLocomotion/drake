...
template <typename T>
void DiscreteUpdateManager<T>::AddSurfaceVelocityBias(
  ...
  MultibodyPlantDiscreteUpdateManagerAttorney<T>::AddSurfaceVelocityBias(
      plant(), context, bodyA_index, bodyB_index, nhat_BA_W, v_AcBc_W);
}
...
void DiscreteUpdateManager<T>::AppendDiscreteContactPairsForPointContact(
  ...
  for (int point_pair_index = 0; point_pair_index < num_point_contacts;
    ...
    Vector3<T> v_AcBc_W_ss = Vector3<T>::Zero();
    AddSurfaceVelocityBias(context, body_A.index(), body_B.index(),
                           pair.nhat_BA_W, &v_AcBc_W_ss);
    const Vector3<T> v_AcBc_C_ss = R_WC.transpose() * v_AcBc_W_ss;
  ...
  for (int surface_index = 0; surface_index < num_surfaces; ++surface_index) {
    ...
    for (int face = 0; face < s.num_faces(); ++face) {
      ...
      if (Ae > 1.0e-14) {
        ...
        // C.
        Vector3<T> v_AcBc_W_ss = Vector3<T>::Zero();
        AddSurfaceVelocityBias(context, body_A_index, body_B_index, nhat_BA_W,
                               &v_AcBc_W_ss);
        const Vector3<T> v_AcBc_C_ss = R_WC.transpose() * v_AcBc_W_ss;

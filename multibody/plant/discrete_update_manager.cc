...
void DiscreteUpdateManager<T>::AppendContactResultsForPointContact(
  ...
  for (int icontact = 0; icontact < num_point_contacts; ++icontact) {
    const DiscreteContactPair<T>& discrete_pair = contact_pairs[icontact];
    ...
    const RotationMatrix<T>& R_WC = discrete_pair.R_WC;
...
void DiscreteUpdateManager<T>::CalcDiscreteContactPairs(
  ...
  result->Clear();
  // TODO(xuchenhan-tri): Once we support deformable vs. deformable contact, we
  // can no longer use this as an early exit criterion.

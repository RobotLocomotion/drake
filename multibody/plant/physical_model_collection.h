...
class PhysicalModelCollection : public ScalarConvertibleComponent<T> {
 public:
  ...
  /* Creates a clone of `this` PhysicalModelCollection with the scalar type
   `ScalarType`. The clone should be a deep scalar transmogrification of the
   original PhysicalModelCollection with the exception of back pointer to the
   owning plant. This function should only called from the scalar conversion

...
class MultibodyPlant final : public internal::MultibodyTreeSystem<T> {
 ...
 private:
  ...
  // velocity axis, or if its input bus signal is missing, or there is no
  ...
  void AddSurfaceVelocityBias(const systems::Context<T>& context,
    ...
    DoAddSurfaceVelocityBias(context, bodyA_index, bodyB_index, nhat_BA_W,
                             v_AcBc_W);
  }

  // Helper method to actually add the surface velocity bias into v_AcBc_W.
  void DoAddSurfaceVelocityBias(const systems::Context<T>& context,
                                BodyIndex bodyA_index, BodyIndex bodyB_index,
                                const Vector3<T>& nhat_BA_W,
                                Vector3<T>* v_AcBc_W) const;

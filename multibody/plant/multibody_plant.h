...
class MultibodyPlant : public internal::MultibodyTreeSystem<T> {
 public:
  ...
  /// Positive dimensionless number typically in the range [0.0, 1.0], though
  /// larger values are allowed even if uncommon. This parameter controls the
  /// "near rigid" regime of the SAP solver, see [Castro et al., 2021]. A value
  /// of 1.0 is a conservative choice to avoid ill-conditioning that might lead
  /// to softer than expected contact. If this is your case, consider turning
  /// off this approximation by setting this parameter to zero. For difficult
  /// cases where ill-conditioning is a problem, a small but non-zero number can
  /// be used, e.g. 1.0e-3.
  /// @throws std::exception if near_rigid_parameter is negative.
  /// @throws std::exception if called post-finalize.

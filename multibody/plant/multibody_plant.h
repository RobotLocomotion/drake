...
class MultibodyPlant : public internal::MultibodyTreeSystem<T> {
 public:
  ...
  void SetVelocities(systems::Context<T>* context,
    ...
    ThrowIfNotFinite(v_instance, __func__);
 ...
 private:
  ...
  // input.
  static void ThrowIfNotFinite(const Eigen::Ref<const VectorX<T>>& v,
                               const char* source_method) {
    if (!all_of(v, [](const T& t) {
          using std::isfinite;
          return isfinite(t);
        })) {
      ThrowNonFinite(source_method);
    }
  }

  // Actually throws the non-finite method.
  static void ThrowNonFinite(const char* source_method);

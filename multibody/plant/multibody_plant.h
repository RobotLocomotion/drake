 ...
 public:
  ...
  void SetPositionsAndVelocities(
    ...
    if constexpr (!std::is_same_v<T, symbolic::Expression>) {
      using std::isnan;
      DRAKE_THROW_UNLESS(none_of(q_v, [](const T& t) {
        return isnan(t);
      }));
    }
  ...
  /// is not equal to `num_positions()`, or if `q` contains NaN.
  void SetPositions(systems::Context<T>* context,
    ...
    if constexpr (!std::is_same_v<T, symbolic::Expression>) {
      using std::isnan;
      DRAKE_THROW_UNLESS(none_of(q, [](const T& t) {
        return isnan(t);
      }));
    }
  ...
  /// `num_positions(model_instance)`, or if `q_instance` contains NaN.
  void SetPositions(systems::Context<T>* context,
                    ModelInstanceIndex model_instance,
    ...
    if constexpr (!std::is_same_v<T, symbolic::Expression>) {
      using std::isnan;
      DRAKE_THROW_UNLESS(none_of(q_instance, [](const T& t) {
        return isnan(t);
      }));
    }
  ...
  /// num_positions(), or `q` contains NaN.
  ...
  /// not correspond to the context for a multibody model, if the length of
  /// `v` is not equal to `num_velocities()`, or if `v` contains NaN.
  void SetVelocities(systems::Context<T>* context,
    ...
    if constexpr (!std::is_same_v<T, symbolic::Expression>) {
      using std::isnan;
      DRAKE_THROW_UNLESS(none_of(v, [](const T& t) {
        return isnan(t);
      }));
    }
  ...
  /// `num_velocities(model_instance)`, or if `v_instance` contains NaN.
  void SetVelocities(systems::Context<T>* context,
                     ModelInstanceIndex model_instance,
    ...
    if constexpr (!std::is_same_v<T, symbolic::Expression>) {
      using std::isnan;
      DRAKE_THROW_UNLESS(none_of(v_instance, [](const T& t) {
        return isnan(t);
      }));
    }

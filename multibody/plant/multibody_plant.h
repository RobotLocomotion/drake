...
the net actuation port (get_net_actuation_port()). That is, the net actuation
port reports the total actuation applied by a given actuator.
...
class MultibodyPlant : public internal::MultibodyTreeSystem<T> {
 public:
  ...
  /// @pre Finalize() was already called on `this` plant.
  /// @throws std::exception if called before Finalize().
  const systems::OutputPort<T>& get_net_actuation_port() const;

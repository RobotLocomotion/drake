...
///   Aaccess input and output ports.
...
/// - @ref mbp_state_accessors_and_mutators "State access":
...
///   Perform Context-dependent kinematic and dynamic queries.
...
class MultibodyPlant : public internal::MultibodyTreeSystem<T> {
 public:
  ...
  /// These methods provide access to the Drake System input and output ports
  ...
  /// Various methods for accessing and mutating kinematic state variables
  /// `[q; v]`, where `q` is the vector of generalized positions and `v` is
  /// the vector of generalized velocities, or some portion thereof (e.g.,
  /// only `v`). The state resides in a systems::Context that is supplied
  ...
  /// systems::Context. Methods whose names being with `Eval` return a reference
  ...
  /// Methods in this section compute and return the various matrices that
  ...
  /// matrices. See
  /// @ref mbp_kinematic_and_dynamic_computations "Kinematic and dynamics
  /// computations" for the available computations.
  ///
  /// You can obtain the mass matrix, centrifugal and Coriolis "bias" vector,
  /// and a variety of Jacobian and actuation matrices.
  /// @{

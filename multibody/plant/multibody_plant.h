...
class MultibodyPlant : public internal::MultibodyTreeSystem<T> {
 ...
 private:
  ...
  // Collects up actuator, generalized, and spatial forces from input ports,
  // and contact forces from compliant contact models. Does not include
  // ForceElement forces which are accounted for elsewhere.
  void DoAddInForcesContinuous(const systems::Context<T>& context,

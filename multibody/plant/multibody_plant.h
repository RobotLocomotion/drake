...
class MultibodyPlant final : public internal::MultibodyTreeSystem<T> {
 public:
  ...
  /// body with RigidBody::is_floating_base_body(), and can request a list of
  /// all floating base bodies with GetFloatingBaseBodies(). The relevant joint
  /// coordinate entries q and v in the multibody state vector can be obtained
  /// with RigidBody::floating_positions_start() and
  ...
  ///   which `body.is_floating_base_body()` returns true, not just any free
  ///   body, despite the method name.
  ...
  /// given by `model_instance` (see HasUniqueFloatingBaseBody()), returns that

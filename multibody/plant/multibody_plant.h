...
class MultibodyPlant : public internal::MultibodyTreeSystem<T> {
 public:
  ...
  /// This affects the default state of this %MultibodyPlant if and only if
  /// `body.is_floating()` is true. If called post-finalize, throws if
  /// `body.is_floating()` is false. There's no such restriction pre-finalize
  /// (because there is no way to tell whether is body is floating or not).
  ///
  /// @param[in] body
  ///   Body whose default pose will be set.
  /// @param[in] X_WB
  ///   Default pose of the body.
  /// @throws std::exception if called post-finalize and body.is_floating() is
  /// false.
  void SetDefaultFreeBodyPose(
      const Body<T>& body, const math::RigidTransform<double>& X_WB) {
    DRAKE_THROW_UNLESS(!is_finalized() || body.is_floating());
    this->mutable_tree().SetDefaultFreeBodyPose(body, X_WB);
  }

  /// If called pre-finalize, returns the value set in SetDefaultFreeBodyPose or
  /// the identity if unset. If called post-finalize, throws if
  /// `body.is_floating()` is false and returns the default free body pose
  /// otherwise.
  /// @param[in] body

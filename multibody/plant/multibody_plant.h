...
class MultibodyPlant final : public internal::MultibodyTreeSystem<T> {
 public:
  ...
  /// _free body_. In %MultibodyPlant, that means there is a 6dof ("floating")
  /// joint between the body and its parent. If a body has no user-provided
  /// joint connecting it to any parent body (typical for manipulands, for
  /// example), we give it six degrees of freedom relative to World and call the
  /// resulting free body a _floating base body_. (To be clear on the
  /// terminology: even if there is a user-defined 6dof joint connecting a body
  /// to World it is only a free body, _not_ a floating base body.)
  ///
  /// To implement a floating base body, %MultibodyPlant automatically adds a
  /// floating joint between that body and World at Finalize(). Prior to that
  /// there is no joint so we provide a pre-Finalize() API here to set the
  ...
  /// work with floating base bodies, by accessing the automatically-added
  /// floating joint. Use GetJointByName() with the name of the floating base
  /// body (see RigidBody::name()). (In the rare case that there is already some
  /// unrelated joint with that name, we prepend underscores to the joint's name
  /// until it is unique.)
  ///
  /// Some of the APIs in this group can be used for _any_ free body, not just
  /// floating base bodies (those have "FreeBody" in their names rather than
  /// "FloatingBaseBody"). We recommend against using them that way -- use the
  /// Joint API instead whenever you have a joint to work with.
  /// @{
  ...
  ///
  /// This may be called pre- or post-Finalize(). If `body` is a floating base
  /// body, the recorded pose will affect the initial contents of any
  /// subsequently-created Context. Otherwise the value recorded here is
  /// ignored.
  ///
  ...
  /// For any free body's 6dof joint, gets the pose X_JpJc of the child frame Jc
  ...
  /// For any free body's 6dof joint, sets `context` to store the pose
  ...
  /// For any free body's 6dof joint, sets `context` to store the spatial
  ...
  /// For any free body's 6dof joint, sets the distribution used by

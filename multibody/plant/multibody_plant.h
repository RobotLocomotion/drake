...
class MultibodyPlant final : public internal::MultibodyTreeSystem<T> {
 public:
  ...
  /// System Jacobian against a per-body spatial force, in O(n) without forming
  /// J.
  ///
  ...
  ///   sized num_mobods(), ordered by MobodIndex (entry 0 corresponds to World;
  ///   it is read but has no effect on τ). **This array is destroyed during the
  ///   computation**: the operator sweeps tip-to-base, shifting each body's
  ///   accumulated force into its parent's entry of `F_Bo_W_array` in place to
  ///   avoid allocating a scratch copy.

...
// Attorney-client pattern to provide deformable solver limited "friend" access
// to MultibodyPlant.
template <typename T>
...
class MultibodyPlant : public internal::MultibodyTreeSystem<T> {
 ...
 private:
  ...
  // Declares an discrete state whose model value is populated with the give
  // `q`, `qdot` and `qddot`.
  ...
  /* Copies the vertex positions of each deformable body to the given `output`.

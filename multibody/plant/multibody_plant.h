...
#pragma GCC diagnostic pop
  ...
  ///
  /// Geometries can be associated with bodies via the `Register*Geometry`
  ...
  /// SceneGraph and assign it the proximity role. The `RegisterVisualGeometry`
  /// methods do the same, but assign the illustration role.
  ///
  ...
  ///     plant.get_geometry_query_input_port().Eval<QueryObject<T>>(context);
  /// const GeometryId g_id = id_from_some_query;
  /// const FrameId f_id = query_object.inspector().GetFrameId(g_id);
  ...
  /// `[model instance name]::[link name]`. Searching for a frame with just the
  /// name `link name` will fail.
  /// @{

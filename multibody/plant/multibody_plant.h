...
class MultibodyPlant : public internal::MultibodyTreeSystem<T> {
 ...
 private:
  ...
  std::pair<systems::OutputPortIndex, systems::OutputPortIndex>
  DeclareSampledVectorOutputPort(
      const std::string& name, int size, CalcSampled calc_sampled,
      CalcUnsampled calc_unsampled,
      const std::set<systems::DependencyTicket>& prerequisites_of_calc_sampled,
      const std::set<systems::DependencyTicket>&
  ...
  // Computes the net applied actuation through actuators. For continuous models
  // (thus far) this only inludes values coming from the actuation input port.
  // For discrete models, it includes actuator controllers. We have two flavors
  // of this port (sampled vs unsampled). The caller can optionally choose a
  // specific model instance; otherwise, the actuation is for the entire plant.
  template <bool sampled>
  void CalcNetActuationOutput(
  ...
  // For each body B in the model, evaluates A_WB, B's spatial acceleration in
  // the world frame W, expressed in W (for point Bo, the body's origin) and
  // copies it into A_WB_all, indexed by BodyIndex. We have two flavors of this
  // port (sampled vs unsampled).
  template <bool sampled>
  ...
  systems::OutputPortIndex net_actuation_unsampled_port_;
  ...
  std::vector<systems::OutputPortIndex> instance_net_actuation_unsampled_ports_;
  ...
  systems::OutputPortIndex body_spatial_accelerations_unsampled_port_;

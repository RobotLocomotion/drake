...
class MultibodyPlant final : public internal::MultibodyTreeSystem<T> {
 public:
  ...
  /// @throws std::exception if called after Finalize().
  /// @see GetBaseBodyJointType(), SetFuseWeldedBodies(), Finalize()
  ...
  /// calculations involving individual links (e.g., positions & velocities) can
  /// still be obtained by name or LinkIndex (BodyIndex). However, no results
  /// (e.g., no reaction forces) are available for the weld joints internal to a
  /// fused Mobod.
  ///
  /// The `fuse` flag can be set globally or on a per-model instance basis.
  /// If SetFuseWeldedBodies() is called with a model instance then that
  /// setting is used; otherwise, the global setting is used.
  ///
  ...
  ///
  /// @throws std::exception if called after Finalize().
  /// @see GetFuseWeldedBodies(), SetBaseBodyJointType(), Finalize()
  void SetFuseWeldedBodies(
  ...
  /// @see SetFuseWeldedBodies(), GetBaseBodyJointType(), Finalize()
  bool GetFuseWeldedBodies(
      std::optional<ModelInstanceIndex> model_instance = {}) const;

...
class MultibodyPlant : public internal::MultibodyTreeSystem<T> {
 public:
  ...
  ///
  /// @note This port always has size 2*num_actuators(model_instance), where we
  /// assume 1-DOF actuated joints. This port must provide one desired position
  /// and one desired velocity per joint actuator, packed as xd = [qd, vd], with
  /// positions and velocities in order of increasing JointActuatorIndex. This
  /// is true regardless of the number of PD-controlled actuators. Entries in
  /// the desired state corresponding to actuators without PD control are
  /// ignored. Therefore a model instance can use PD control in only a subset of
  /// its actuators.
  ///

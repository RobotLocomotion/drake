...
class MultibodyPlant final : public internal::MultibodyTreeSystem<T> {
 public:
  ...
  /// finite `double` speed in m/s. If the port is not connected, or a body's
  ...
  /// signal's name is the body's scoped name (i.e., RigidBody::scoped_name()).
  /// The displacement is initialized to zero and integrated from the
  /// `"surface_speeds"` input port.
  /// @pre Finalize() was already called on `this` plant.

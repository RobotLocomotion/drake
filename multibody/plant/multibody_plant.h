...
class MultibodyPlant : public internal::MultibodyTreeSystem<T> {
 ...
 private:
  ...
  // for the given state stored in context, when the model is discrete. If
  // cached contact solver results are not up-to-date with `context`, they'll be
  // recomputed, see EvalTamsiResults(). The solver results are then used to
  // update `contacts_results`.

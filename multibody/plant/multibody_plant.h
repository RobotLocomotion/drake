...
class MultibodyPlant : public internal::MultibodyTreeSystem<T> {
 public:
  ...
  /// on a body B.
  ///
  /// This constraint can be compliant, allowing to model a spring with free
  /// length `distance` and given `stiffness` and `damping` parameters between
  /// points P and Q. If d = ‖p_PQ‖, then a compliant distance constraint models
  /// a spring with force along p_PQ given by:
  ...
  /// @param[in] p_AP Position of point P in body's A frame.
  /// @param[in] p_BP Position of point Q in body's B frame.
  /// @param[in] distance Fixed length of the distance constraint, in meters.
  /// @param[in] stiffness For modeling a spring with free length equal to
  /// `distance`, the stiffness parameter in N/m.
  /// @param[in] damping For modeling a spring with free length equal to
  /// `distance`, damping parameter in N⋅s/m.
  ///
  /// @note To constraint two points to be coincident we need a 3-dof ball

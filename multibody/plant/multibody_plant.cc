...
void AssertIndicesValid(const std::vector<int>& indices, int max_size) {
  DRAKE_ASSERT(static_cast<int>(indices.size()) <= max_size);
  DRAKE_ASSERT(std::is_sorted(indices.begin(), indices.end()));
  DRAKE_ASSERT(indices[0] >= 0);
  DRAKE_ASSERT(indices[indices.size() - 1] < max_size);
}
...
MatrixX<T> SelectRowsCols(const MatrixX<T>& M,
  ...
  DRAKE_ASSERT_VOID(AssertIndicesValid(indices, M.rows()));
...
MatrixX<T> SelectCols(const MatrixX<T>& M,
                      const std::vector<int>& indices) {
  DRAKE_ASSERT_VOID(AssertIndicesValid(indices, M.cols()));
  const int selected_count = indices.size();
...
VectorX<T> ExpandRows(const VectorX<T>& v, int rows_out,
  ...
  DRAKE_ASSERT_VOID(AssertIndicesValid(indices, rows_out));
  if (rows_out == v.rows()) {
    return v;
  }
...
void MultibodyPlant<T>::CalcJointLockingIndices(
  ...
  DRAKE_ASSERT_VOID(AssertIndicesValid(indices, num_velocities()));

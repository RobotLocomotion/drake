...
std::string MultibodyPlant<T>::GetTopologyGraphvizString() const {
  ...
  for (ModelInstanceIndex model_instance_index(0);
    ...
    graphviz += "subgraph cluster" +
                std::to_string(static_cast<int>(model_instance_index)) + " {\n";
    graphviz +=
        "  label=\"" + GetModelInstanceName(model_instance_index) + "\";\n";
    for (const BodyIndex& body_index : GetBodyIndices(model_instance_index)) {
      const Body<T>& body = get_body(body_index);
      graphviz += "  body" + std::to_string(static_cast<int>(body.index())) +
                  " [label=\"" + body.name() + "\"];\n";
    }
    graphviz += "}\n";
  }
  ...
  for (JointIndex joint_index(0); joint_index < num_joints(); ++joint_index) {
    ...
    graphviz +=
        "body" + std::to_string(static_cast<int>(joint.child_body().index())) +
        " -> body" +
        std::to_string(static_cast<int>(joint.parent_body().index())) +
        " [label=\"" + joint.name() + " [" + joint.type_name() + "]\"];\n";
  }

...
template <typename T>
void MultibodyPlant<T>::CalcLockedAndUnlockedVelocityIndices(
    const systems::Context<T>& context,
  ...
  DRAKE_DEMAND(ssize(unlocked) == unlocked_cursor);
  DRAKE_DEMAND(ssize(locked) == locked_cursor);
...
void MultibodyPlant<T>::DeclareCacheEntries() {
  ...
  const auto& joint_locking_data_cache_entry = this->DeclareCacheEntry(
      "Locked and unlocked Velocity Indices.",
      internal::JointLockingCacheData<T>{},
      &MultibodyPlant::CalcLockedAndUnlockedVelocityIndices,
      {this->all_parameters_ticket()});

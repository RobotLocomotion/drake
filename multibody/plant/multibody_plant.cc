...
MultibodyConstraintId MultibodyPlant<T>::AddWeldConstraint(
    const RigidBody<T>& body_A, const RigidTransform<double>& X_AP,
    const RigidBody<T>& body_B, const RigidTransform<double>& X_BQ) {
  // N.B. The manager is set up at Finalize() and therefore we must require
  // constraints to be added pre-finalize.
  DRAKE_MBP_THROW_IF_FINALIZED();
...
const WeldJoint<T>& MultibodyPlant<T>::WeldFrames(
    ...
    const RigidTransform<double>& X_FM) {
...
geometry::GeometryId MultibodyPlant<T>::RegisterVisualGeometry(
    const RigidBody<T>& body, const RigidTransform<double>& X_BG,
...
void MultibodyPlant<T>::SetFloatingBaseBodyPoseInWorldFrame(
    ...
    const RigidTransform<T>& X_WB) const {
  DRAKE_MBP_THROW_IF_NOT_FINALIZED();
...
void MultibodyPlant<T>::SetFloatingBaseBodyPoseInAnchoredFrame(
    ...
    const RigidBody<T>& body, const RigidTransform<T>& X_FB) const {
  DRAKE_MBP_THROW_IF_NOT_FINALIZED();
  DRAKE_THROW_UNLESS(body.is_floating_base_body());
  this->ValidateContext(context);
...
void MultibodyPlant<T>::AddEphemeralLoopConstraints() {
  ...
  for (const internal::LinkJointGraph::LoopConstraint& loop_constraint :
    ...
    internal::WeldConstraintSpec spec{
        ...
        .X_AP = RigidTransform<double>::Identity(),
        .body_B = loop_constraint.shadow_link(),
        .X_BQ = RigidTransform<double>::Identity(),
        .id = id,
        .is_ephemeral = true};
  ...
  const auto is_irrelevant_geometry =
      [&geometry_id_to_body_index, &forest,
       &per_tree_unlocked_indices](GeometryId geometry_id) {
        ...
        if (!tree_index.is_valid()) return true;
...
void MultibodyPlant<T>::ThrowIfUnsupportedContinuousTimeDynamics(
  ...
  if (num_constraints() > 0) {
    ...
    const char* sc = (num_constraints() == 1 ? "" : "s");
    const char* sl = (num_loops == 1 ? "" : "s");
    std::string constraints;
    if (num_loops == 0) {
      constraints = fmt::format("{} constraint{}", num_constraints(), sc);
    } else if (num_loops == num_constraints()) {
      constraints = fmt::format(
          "{} constraint{}, which Finalize() added in order to close the "
          "kinematic loop{} in this model (see SetEnableLoopTopology())",
          num_loops, sc, sl);
    } else {
      constraints = fmt::format(
          "{} constraint{}, {} of which Finalize() added in order to close the "
          "kinematic loop{} in this model (see SetEnableLoopTopology())",
          num_constraints(), sc, num_loops, sl);
    }
    throw std::logic_error(fmt::format(
...
void MultibodyPlant<T>::DeclareOutputPorts() {
  ...
  output_port_indices_.body_poses =
      this->DeclareAbstractOutputPort(
              "body_poses", std::vector<RigidTransform<T>>(num_bodies()),
...
void MultibodyPlant<T>::CalcBodyPosesOutput(
    const Context<T>& context, std::vector<RigidTransform<T>>* output) const {
  DRAKE_MBP_THROW_IF_NOT_FINALIZED();
  this->ValidateContext(context);

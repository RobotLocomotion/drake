      ...
      for (const auto& s : surfaces) {
        ...
        for (int face = 0; face < s.num_faces(); ++face) {
          ...
          if (Ae > 1.0e-14) {
            ...
            // Prohibit rigid-rigid contact. One of them must be compliant.
            ...
            const T gM = M_is_soft ? s.EvaluateGradE_M_W(face).dot(nhat_W)
                                   : T(std::numeric_limits<double>::infinity());
            const T gN = N_is_soft ? s.EvaluateGradE_N_W(face).dot(nhat_W)
                                   : T(std::numeric_limits<double>::infinity());
            // Effective pressure gradient g (Pascal/meter) from both bodies.
            T g = 0;
            using std::abs;
            if (gM == T(std::numeric_limits<double>::infinity())) {
              g = -gN;
            } else if (gN == T(std::numeric_limits<double>::infinity())) {
              g = gM;

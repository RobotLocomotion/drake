...
void MultibodyPlant<T>::CalcNormalAndTangentContactJacobians(
  ...
  for (int icontact = 0; icontact < num_contacts; ++icontact) {
    ...
    const BodyIndex bodyA_index = FindContactBody(geometryA_id);
    const BodyIndex bodyB_index = FindContactBody(geometryB_id);
...
template <typename T>
BodyIndex MultibodyPlant<T>::FindContactBody(GeometryId geometry_id) const {
...
void MultibodyPlant<T>::CalcHydroelasticContactForces(
  ...
  for (const ContactSurface<T>& surface : all_surfaces) {
    ...
    // A and B.
    const BodyIndex bodyA_index = FindContactBody(geometryM_id);
    const BodyIndex bodyB_index = FindContactBody(geometryN_id);
    const Body<T>& bodyA = get_body(bodyA_index);
    const Body<T>& bodyB = get_body(bodyB_index);

...
void MultibodyPlant<T>::ThrowIfUnsupportedContinuousTimeDynamics(
  ...
  // Reject PD control.
  for (int k = 0; k < num_model_instances(); ++k) {
    // The logic here is an approximation of what AssembleDesiredStateInput()
    // does. Here we skip the NaN checking and the collection of actual values.
    ModelInstanceIndex index{k};
    if (num_actuated_dofs(index) > 0 &&
        this->get_desired_state_input_port(index).HasValue(context)) {
      throw std::logic_error(

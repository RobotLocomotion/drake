...
template <typename T>
std::vector<BodyIndex> MultibodyPlant<T>::GetBodiesAffectedBy(
  ...
  const std::set<BodyIndex> bodies =
      multibody_graph_.FindBodiesOutBoardOfJoints(joint_indexes);
  return std::vector<BodyIndex>(bodies.begin(), bodies.end());
}

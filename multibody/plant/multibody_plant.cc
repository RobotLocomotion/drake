...
template <typename T>
void MultibodyPlant<T>::SetFuseWeldedBodies(
    bool fuse, std::optional<ModelInstanceIndex> model_instance) {
  mutable_tree().SetFuseWeldedBodies(fuse, model_instance);
}
...
template <typename T>
bool MultibodyPlant<T>::GetFuseWeldedBodies(
    std::optional<ModelInstanceIndex> model_instance) const {
  return internal_tree().GetFuseWeldedBodies(model_instance);
}

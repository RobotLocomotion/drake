...
template <typename T>
int MultibodyPlant<T>::NumPdControlledActuators(
    ModelInstanceIndex model_instance) const {
  int num_actuators = 0;
  for (JointActuatorIndex a : GetJointActuatorIndices()) {
    const JointActuator<T>& actuator = get_joint_actuator(a);
...
internal::DesiredStateInput<T> MultibodyPlant<T>::AssembleDesiredStateInput(
  ...
  for (ModelInstanceIndex model_instance_index(0);
    ...
    const int num_pd_controlled_actuators =
        NumPdControlledActuators(model_instance_index);
    ...
    if (xd_input_port.HasValue(context)) {
      ...
      const auto qd = xd_instance.head(num_pd_controlled_actuators);
      const auto vd = xd_instance.tail(num_pd_controlled_actuators);
...
void MultibodyPlant<T>::DeclareInputPorts() {
  ...
  for (ModelInstanceIndex i(0); i < num_model_instances(); ++i) {
    ...
    const int instance_num_xd = 2 * NumPdControlledActuators(i);

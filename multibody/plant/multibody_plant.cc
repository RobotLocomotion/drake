...
MultibodyPlant<symbolic::Expression>::RegisterAsSourceForSceneGraph(
    SceneGraph<symbolic::Expression>* scene_graph) {
...
struct MultibodyPlant<T>::SceneGraphStub {
  ...
  void AssignRole(Args...) const {
    Throw("AssignRole");
  }

  template <typename... Args>
  FrameId RegisterFrame(Args...) const {
    Throw("RegisterFrame");
    return {};
  }

  template <typename... Args>
  GeometryId RegisterGeometry(Args...) const {
    Throw("RegisterGeometry");
    return {};
  }

  template <typename... Args>
  void ExcludeCollisionsBetween(Args...) const {
    Throw("ExcludeCollisionsBetween");
  }

  template <typename... Args>
  void ExcludeCollisionsWithin(Args...) const {
    Throw("ExcludeCollisionsWithin");
  }
};
...
void MultibodyPlant<symbolic::Expression>::DeclareSceneGraphPorts() {
  // SceneGraph doesn't support symbolic::Expression, but we'd like our port
  // indices to remain consistent.
  geometry_query_port_ = this->DeclareAbstractInputPort(
      "geometry_query", systems::Value<SymbolicGeometryValue>{}).get_index();
  geometry_pose_port_ = this->DeclareAbstractOutputPort(
      "geometry_pose",
      []() { return std::make_unique<Value<SymbolicGeometryValue>>(); },
      [](const auto&, auto*) { /* no-op calc */ }).get_index();
}

        ...
        for (geometry::SurfaceFaceIndex face(0); face < mesh_W.num_faces();
          ...
          if (Ae > 1.0e-14) {
            ...
            for (int qp = 0; qp < num_quad_points; ++qp) {
              ...
              // "elastic" force on the body according to:
              //   f₀ₚ = ωₚ Aₑ pₚ n̂ₚ
              ...
              // hydroelastics stiffness as:
              //   k := ∂f₀ₚ/∂n̂ ≈ ωₚ⋅Aₑ⋅∇pₚ⋅n̂ₚ
              // that is, the variation of the normal force experiments if the
              ...
              // of the normal. IsFaceNormalInNormalDirection() in
              // contact_surface_utility.cc excludes triangles for which the
              // angle between the presure gradient times the normal is larger
              // than 5/8π = 112.5ᵒ. Therefore there are cases for which the
              // definition above of k might lead to negative values. We
              // observed that this condition happens sparsely at some of the
              // boundary triangles of the contact surface, while the positive
              // values in inner triangles dominates the overall compliance. In
              // practice we did not observe this to cause stability issues.
              // Since a negative value of k is correct, we decided to keep
              // these contributions.

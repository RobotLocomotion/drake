...
void MultibodyPlant<T>::Finalize() {
  ...
  if (geometry_source_is_registered()) {
    // TODO(huihua) This change yields the functions GetBodyFrameIdIfExists()
    // and GetBodyFrameIdOrThrow() not accurate anymore since it guarantees that
    // every body has at least one registered geometry frame. Deprecate these
    // two functions and have a new function GetBodyFrameId() to replace them.

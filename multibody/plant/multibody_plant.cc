...
void MultibodyPlant<T>::CalcReactionForces(
  ...
  for (JointIndex joint_index : GetJointIndices()) {
    ...
    //  - Jp == M and Jc == F (mobilizer was reversed from joint)
    ...
    if (!is_reversed) {
      ...
      // Find the position vector p_JcJp_Jc.
      const RigidTransform<T> X_JcJp = frame_Jp.CalcPose(context, frame_Jc);
      const Vector3<T>& p_JcJp_Jc = X_JcJp.translation();

      // Perform  the Jp->Jc shift.
      F_CJc_Jc = F_CJp_Jc.Shift(-p_JcJp_Jc);
    }
  }
}

...
void MultibodyPlant<T>::DeclareStateCacheAndPorts() {
  ...
  actuation_output_port_ =
      this->DeclareVectorOutputPort(
              "actuation_output", num_actuated_dofs(),
...
template <typename T>
const systems::OutputPort<T>& MultibodyPlant<T>::get_actuation_output_port()
    const {
  DRAKE_MBP_THROW_IF_NOT_FINALIZED();
  return systems::System<T>::get_output_port(actuation_output_port_);
}

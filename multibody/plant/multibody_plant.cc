...
template <typename T>
ConstraintIndex MultibodyPlant<T>::AddWeldConstraint(
    const Body<T>& body_A, const math::RigidTransform<double>& X_AP, const Body<T>& body_B,
  ...
  internal::WeldConstraintSpecs spec{body_A.index(), X_AP, body_B.index(),
                                     X_BQ};
  if (!spec.IsValid()) {
    const std::string msg = fmt::format(
        "Invalid set of parameters for constraint between bodies '{}' and "
  ...
  weld_constraints_specs_.push_back(spec);

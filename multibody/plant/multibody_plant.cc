...
MultibodyPlant<T>::CalcCombinedFrictionCoefficients(
  ...
  for (const auto& pair : point_pairs) {
    ...
    DRAKE_THROW_UNLESS(propA->HasProperty(property));
    DRAKE_THROW_UNLESS(propB->HasProperty(property));

    const auto& geometryA_friction =
        propA->Get<CoulombFriction<double>>(property);
    const auto& geometryB_friction =
        propB->Get<CoulombFriction<double>>(property);

...
void MultibodyPlant<T>::Finalize() {
  ...
  if (geometry_source_is_registered()) {
    // Assues every body has at least one geometry frame registered. This change
    // yields the functions GetBodyFrameIdIfExists() and GetBodyFrameIdOrThrow()
    // not accurate anymore.
    // TODO(huihua) Deprecate these two functions and have a new function
    // GetBodyFrameId() to replace them since every BodyIndex should always
    // map to a FrameId

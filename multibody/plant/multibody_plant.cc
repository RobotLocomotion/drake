...
void MultibodyPlant<T>::CalcNetActuationOutput(const Context<T>& context,
  ...
  if (is_discrete()) {
    ...
    DRAKE_DEMAND(!sampled);
    ThrowIfUnsupportedPdControl(context);
...
template <typename T>
void MultibodyPlant<T>::ThrowIfUnsupportedPdControl(
    const systems::Context<T>& context) const {
  // Reject PD control.  The logic here is an approximation of what
  // AssembleDesiredStateInput() does. Here we skip the NaN checking and the
  // collection of actual values.
  for (ModelInstanceIndex k{0}; k < num_model_instances(); ++k) {
    if (num_actuated_dofs(k) > 0 &&
        this->get_desired_state_input_port(k).HasValue(context)) {
      for (JointActuatorIndex actuator_index : GetJointActuatorIndices(k)) {
        const JointActuator<T>& actuator = get_joint_actuator(actuator_index);
        const bool is_locked = actuator.joint().is_locked(context);
        if (actuator.has_controller() && !is_locked) {
          throw std::logic_error(

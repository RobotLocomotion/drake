    ...
    if (HasSurfaceVelocityBias(bodyA_index, bodyB_index)) {
      data.add_surface_velocity_bias =
          [this, &context, bodyA = bodyA_index, bodyB = bodyB_index](
            ...
            // The hydro calculator uses v_BcAc. The method is documented the
            // other way around, so we reverse the A/B semantics here.
...
template <typename T>
void MultibodyPlant<T>::DoAddSurfaceVelocityBias(

...
// gains defined.
template <typename T>
bool ModelInstanceIsPdControlled(const MultibodyPlant<T>& plant,
...
template <typename T>
int NumberOfPdControlledActuators(const MultibodyPlant<T>& plant,
                                  ModelInstanceIndex model_instance) {
  int num_actuators = 0;
  for (JointActuatorIndex actuator_index :
       plant.GetJointActuatorIndices(model_instance)) {
  ...
  if (actuation_port.HasValue(context)) {
    ...
    for (ModelInstanceIndex model_instance_index(first_non_world_index);
      ...
      if (!input_port.HasValue(context)) {
        if (!ModelInstanceIsPdControlled(*this, model_instance_index)) {
          throw std::logic_error(
...
VectorX<T> MultibodyPlant<T>::AssembleDesiredStateInput(
  ...
  for (ModelInstanceIndex model_instance_index(first_non_world_index);
    ...
    const int num_pd_controlled_actuators =
        NumberOfPdControlledActuators(*this, model_instance_index);

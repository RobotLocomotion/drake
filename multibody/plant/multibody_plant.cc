...
MultibodyPlant<T>::MultibodyPlant(
  ...
  // Add the world link to the graph.
  multibody_graph_.AddLink(world_body().name(), world_body().model_instance());
}
...
std::vector<const Body<T>*> MultibodyPlant<T>::GetBodiesWeldedTo(
  ...
  // Assume one-to-one correspondence of bodies to links.
  const std::set<internal::LinkIndex> island =
      multibody_graph_.FindLinksWeldedTo(to_link_index(body.index()));

  // Map link indices to pointers.
  std::vector<const Body<T>*> sub_graph_bodies;
  for (internal::LinkIndex link_index : island) {
    const BodyIndex body_index(link_index.operator int());

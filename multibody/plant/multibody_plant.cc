...
MultibodyPlant<T>::MultibodyPlant(const MultibodyPlant<U>& other)
  ...
  // Here we stepped through every member field one by one, in the exact order
  ...
  // a mention.
  // For each field, this function would either:
  ...
  // (3) Place a disclaimer comment why that field does not need to be copied
  {
    ...
    coupler_constraints_specs_.clear();

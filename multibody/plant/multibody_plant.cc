...
MultibodyPlant<T>::MultibodyPlant(const MultibodyPlant<U>& other)
    : internal::MultibodyTreeSystem<T>(
          ...
          other.is_discrete(), std::ssize(other.surface_velocity_bodies_)) {
...
void MultibodyPlant<T>::SetSurfaceVelocityAxis(
  ...
  surface_velocity_bodies_.insert_or_assign(
      body.index(), SurfaceVelocityEntry{body.scoped_name().to_string(),
...
void MultibodyPlant<T>::Finalize() {
  // If we have any miscellaneous continuous state, we need to declare it before
  // we finalize the tree system.
  if (!is_discrete()) {
    this->DeclareMiscContinuousState(std::ssize(surface_velocity_bodies_));
  }
...
{  // NOLINT(whitespace/braces)
  ...
  for (const ContactSurface<T>& surface : all_surfaces) {
    ...
    read_surface_velocity(bodyA_index, &data.surface_speed_A, &data.a_ss_A);
    read_surface_velocity(bodyB_index, &data.surface_speed_B, &data.a_ss_B);

...
std::vector<std::set<LinkIndex>> MultibodyGraph::FindIslandsOfWeldedLinks()
  ...
  for (const auto& link : links_) {
    if (!visited[link.index()]) {
      // If `link` was not visited yet, we create an island for it.
      ...
      // Thus far `link` forms its own island. Find if other bodies belong to
...
std::set<LinkIndex> MultibodyGraph::FindLinksWeldedTo(
  ...
  // to FindLinksWeldedTo(). Consider storing this for subsequent calls if it
  // becomes a performance bottleneck.
  const std::vector<std::set<LinkIndex>> islands = FindIslandsOfWeldedLinks();

  // Find subgraph that contains this link_index.
  // TODO(amcastro-tri): Consider storing within Link the island it belongs to

...
template <typename T>
ContactResultsToMeshcatSystem<T>::ContactResultsToMeshcatSystem(
    std::shared_ptr<Meshcat> meshcat, ContactResultsToMeshcatParams params)
    : systems::LeafSystem<T>(
          systems::SystemTypeTag<ContactResultsToMeshcatSystem>{}),
  ...
  this->DeclarePeriodicPublishEvent(
      params_.publish_period, 0.0,
      &ContactResultsToMeshcatSystem<T>::UpdateMeshcat);
  this->DeclareForcedPublishEvent(
      &ContactResultsToMeshcatSystem<T>::UpdateMeshcat);

  if (params_.delete_on_initialization_event) {
    this->DeclareInitializationPublishEvent(
        &ContactResultsToMeshcatSystem<T>::OnInitialization);
  }
...
ContactResultsToMeshcatSystem<T>::ContactResultsToMeshcatSystem(
    ...
    : ContactResultsToMeshcatSystem(other.meshcat_, other.params_) {}

template <typename T>
void ContactResultsToMeshcatSystem<T>::Delete() const {
  meshcat_->Delete(params_.prefix);
  contacts_.clear();
}

template <typename T>
const ContactResultsToMeshcatSystem<T>&
ContactResultsToMeshcatSystem<T>::AddToBuilder(
...
ContactResultsToMeshcatSystem<T>::AddToBuilder(
    ...
    std::shared_ptr<Meshcat> meshcat, ContactResultsToMeshcatParams params) {
  auto& visualizer =
      *builder->template AddSystem<ContactResultsToMeshcatSystem<T>>(
          std::move(meshcat), std::move(params));
...
template <typename T>
systems::EventStatus ContactResultsToMeshcatSystem<T>::UpdateMeshcat(
    const systems::Context<T>& context) const {
  const auto& contact_results =
      contact_results_input_port().template Eval<ContactResults<T>>(context);

  std::map<SortedPair<GeometryId>, bool> contacts_to_process =
      contacts_;
...
template <typename T>
systems::EventStatus ContactResultsToMeshcatSystem<T>::OnInitialization(

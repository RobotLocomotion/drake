...
using multibody::hydroelastic_contact::HydroelasticTraction;
...
class MultibodyPlantHydroelasticTractionTests : public ::testing::Test {
 public:
  ...
  const HydroelasticTraction<double>& traction_calculator() const {
 ...
 private:
  void SetUp() override {
    ...
    traction_calculator_ = std::make_unique<HydroelasticTraction<double>>(
        plant_);
  }
  ...
  std::unique_ptr<HydroelasticTraction<double>> traction_calculator_;
};
...
TEST_F(MultibodyPlantHydroelasticTractionTests, TractionWithDissipation) {
  ...
  // constant at each point will be 1.5 * field value * dissipation coefficient.

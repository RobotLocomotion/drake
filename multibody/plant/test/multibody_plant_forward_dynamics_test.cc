...
// propogates through all forward dynamics ABA computations. It is expected that
...
// joints match given an aribitrary state.
GTEST_TEST(JointLocking, PendulumAccelerationTest) {
  ...
  const double lAy = 1.0, lBy = 1.0,
               lCy = 1.0;  // Length along x of links A, B, C.
  const double lAz = 1.0, lBz = 1.0,
               lCz = 1.0;  // Length along x of links A, B, C.
  ...
  // Lock the elbow at the 0 state.
  elbow_AB.set_angle(context.get(), 0.0);
  elbow_AB.set_angular_rate(context.get(), 0.0);
  ...
  // Verify the v_dot for both shoulder joints are equal.
  EXPECT_EQ(v_dot[shoulder_AB.velocity_start()],
            v_dot[shoulder_C.velocity_start()]);
}

// The purpose of this test is to verify that the status of locked joints
// propogates through all forward dynamics ABA computations, and that cached
// quantities have the correct dependencies. It is expected that locked joints
// behave as welded at the configuration given in the context with zero
...
GTEST_TEST(JointLocking, FreebodyAccelerationTest) {
  ...
  // Calculate locked then unlocked, same context.
  {
    std::unique_ptr<systems::Context<double>> context =
        plant.CreateDefaultContext();
    ...
    bodyB.Unlock(context.get());

    {
      const VectorX<double> v_dot =
          MultibodyPlantTester::CalcGeneralizedAccelerations(plant, *context);
      const Vector3<double> g = plant.gravity_field().gravity_vector();
      VectorX<double> v_dot_expected(6);

...
class JointLockingTest : public ::testing::TestWithParam<int> {
 ...
 private:
  void AddFloatingPendulum() {
    ...
    plant_->AddRigidBody("body1", SpatialInertia<double>::MakeSolidBox());
    plant_->AddRigidBody("body2", SpatialInertia<double>::MakeSolidBox());
  ...
  void AddDoublePendulum() {
    // To avoid unnecessary warnings/errors, use a non-zero spatial inertia.
    plant_->AddRigidBody("body3", SpatialInertia<double>::MakeSolidBox());
    plant_->AddRigidBody("body4", SpatialInertia<double>::MakeSolidBox());

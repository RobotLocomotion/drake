...
// their dofs locked. However, geometry queries for collision are not filtered.
// Previously contact results assumed that each point pair collision data mapped
// to a corresponding `DiscreteContactPair` and subsequently a corresponding
// `PointPairContactInfo` all sharing the same index. When multibody contact
// data and results are filtered, this is no longer true. We also test that
// point contact results keep a correct reference to its corresponding geometry
// collision data.
TEST_P(FilteredContactResultsTest, VerifyLockedResults) {
  ...
  // All spheres unlocked, expect a two contact results.
  {
  ...
  // One body locked, still expect a two contact results.
  {
  ...
  // Twop spheres locked, expect only one contact result.
  {
    plant_->GetBodyByName("ball_A").Lock(&context);

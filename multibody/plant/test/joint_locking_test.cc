...
class JointLockingLadderTest
 ...
 public:
  ...
  void AddModel() {
    ...
    geometry::GeometryId G_gid = plant_->RegisterCollisionGeometry(
        plant_->world_body(), RigidTransformd(Vector3d{0, 0, -0.5}), G_shape,
    ...
    geometry::GeometryId L_gid =
        plant_->RegisterCollisionGeometry(L_lower, RigidTransformd::Identity(),
                                          L_shape, "L_lower_collision", L_mu);
    plant_->RegisterCollisionGeometry(L_upper, RigidTransformd::Identity(),
                                      L_shape, "L_upper_collision", L_mu);
    ...
    // Frame P on L_lower and frame Q on L_upper, coincident for the joint
    // between them.
    RigidTransformd X_LlowerP(Vector3d(0, 0, kLLength / 2));
    RigidTransformd X_LupperQ = X_LlowerP;
    // Relative transform between frame P and Q
    RigidTransformd X_PQ(RotationMatrix<double>::MakeYRotation(kLq));

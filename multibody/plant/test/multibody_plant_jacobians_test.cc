...
void CalcJacobianViaPartialDerivativesOfPositionWithRespectToQ(
  ...
  math::InitializeAutoDiffFromValueMatrix(q_double, &q);
  ...
  *p_WoEi_W_deriv_wrt_q = math::ExtractGradientMatrixFromAutoDiff(p_WoEi_W);
...
TEST_F(KukaIiwaModelTests, CalcJacobianSpatialVelocity) {
  ...
  VectorX<AutoDiffXd> qdot_autodiff =
      math::InitializeAutoDiffFromValueMatrix(VectorX<double>::Zero(
          kNumPositions));
  ...
  MatrixX<double> Jq_V_WEp_autodiff =
      math::ExtractGradientMatrixFromAutoDiff(V_WEp_autodiff.get_coeffs());
...
TEST_F(KukaIiwaModelTests, CalcBiasSpatialAcceleration) {
  ...
  auto q_autodiff =
      math::InitializeAutoDiffFromValueAndGradientMatrix(q, MatrixXd(qdot));
  auto v_autodiff =
  ...
  auto Dt_Jv_V_WEp = math::ExtractGradientMatrixFromAutoDiff(Jv_V_WEp_autodiff);
...
TEST_F(KukaIiwaModelTests, CalcBiasTranslationalAcceleration) {
  ...
  auto Dt_Jv_v_WEi = math::ExtractGradientMatrixFromAutoDiff(Jv_v_WEi_autodiff);

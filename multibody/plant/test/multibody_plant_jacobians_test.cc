...
GTEST_TEST(SystemJacobianTransposeTimesF, MultipleTrees) {
  ...
  // Build the reference full System Jacobian J (size 6*num_mobods × nv) and a
  // matching applied-force stack F_stack of size 6*num_mobods. F_stack[6k:6k+6]
  // = [τ_k; f_k] in the same convention as SpatialForce::get_coeffs().
  const MatrixX<double> J = plant.CalcFullSystemJacobian(*context);
  ASSERT_EQ(J.rows(), 6 * num_mobods);
  ASSERT_EQ(J.cols(), nv);

  // Deterministic pseudo-random per-body spatial forces (no <random> dep).
  std::vector<SpatialForce<double>> F_Bo_W_array(num_mobods);
  VectorX<double> F_stack(6 * num_mobods);
  for (int k = 0; k < num_mobods; ++k) {
    Vector6<double> c;
    for (int j = 0; j < 6; ++j) {
      c[j] = std::sin(0.123 * k + 0.789 * j + 0.456);
    }
    F_Bo_W_array[k] = SpatialForce<double>(c);
    F_stack.segment<6>(6 * k) = c;
  }
  ...
  // Edge case: zero applied forces ⇒ zero generalized forces.
  ...
  // Edge case: a single non-zero force on one mobod must equal that mobod's
  ...
  // AutoDiff smoke test: instantiates and produces a finite result. Numerical
  // accuracy is covered by the double precision checks above.

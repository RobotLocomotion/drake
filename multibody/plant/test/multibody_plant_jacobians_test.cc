...
TEST_F(KukaIiwaModelTests, CalcJacobianSpatialVelocity) {
  ...
  MatrixX<double> Jv_V_WB1(6 * plant_->num_bodies(), plant_->num_velocities());
  Jv_V_WB1.setZero();
  ...
  EXPECT_EQ(Jv_V_WB2.rows(), 6 * plant_->num_bodies());
...
GTEST_TEST(BlockSystemJacobian, MultipleTrees) {
  ...
  // Calculate the System Jacobian Jv_V_WB_W two ways and compare.
  // TODO(sherm1) Should be num_mobods.
  MatrixX<double> Jv_V_WB1(6 * plant.num_bodies(), plant.num_velocities());
  Jv_V_WB1.setZero();
  ...
  EXPECT_EQ(Jv_V_WB2.rows(), 6 * plant.num_bodies());

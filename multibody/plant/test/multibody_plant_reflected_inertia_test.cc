...
TEST_F(MultibodyPlantReflectedInertiaTests, CalcMassMatrix) {
  ...
  gear_ratios << 1, 2, 3, 4, 5, 6, 7, 8, 9;

  VectorX<double> reflected_inertia =
...
// This test veifies that the effects of reflected inertia propagate into the
// kinetic energy calculation by comparing it with the kinetic energy of the
// system computed using the mass matrix.
TEST_F(MultibodyPlantReflectedInertiaTests, CalcKineticEnergy) {
  ...
  // Compute kinetic energy of the sytstem with 0.5 * v^T M v.
  MatrixX<double> M_ri(plant_ri_.num_velocities(), plant_ri_.num_velocities());
  plant_ri_.CalcMassMatrix(*context_ri_, &M_ri);

  const auto& v_ri = plant_ri_.GetVelocities(*context_ri_);

  double energy_mass_matrix = 0.5 * v_ri.transpose() * M_ri * v_ri;

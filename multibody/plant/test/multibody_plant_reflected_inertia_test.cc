...
TEST_F(MultibodyPlantReflectedInertiaTests, CalcMassMatrix) {
  ...
  MatrixX<double> Mdiagonal = reflected_inertia.asDiagonal();
...
TEST_F(MultibodyPlantReflectedInertiaTests, CalcInverseDynamics) {
  ...
  EXPECT_TRUE(CompareMatrices(tau_diff, reflected_inertia, kTolerance,
                              MatrixCompareType::relative));
}
...
TEST_F(MultibodyPlantReflectedInertiaTests, CalcForwardAndInverseDynamics) {
  ...
  VectorX<double> vdot_aba = MultibodyPlantTester::CalcGeneralizedAccelerations(
      plant_ri_, *context_ri_);

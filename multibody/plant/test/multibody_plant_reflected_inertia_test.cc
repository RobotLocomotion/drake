...
TEST_F(MultibodyPlantReflectedInertiaTests, CalcMassMatrix) {
  ...
  const VectorX<double> reflected_inertia_velocity_coordinates =
      B * reflected_inertia;
  ...
  const MatrixX<double> Mdiagonal =
      reflected_inertia_velocity_coordinates.asDiagonal();
...
// This test verifies that reflected_inertia properly propagates to
...
TEST_F(MultibodyPlantReflectedInertiaTests, CalcInverseDynamics) {
  ...
  EXPECT_TRUE(CompareMatrices(tau_diff, reflected_inertia_velocity_coordinates,
...
TEST_F(MultibodyPlantReflectedInertiaTests, CalcForwardDynamics) {
  ...
  VectorX<double> vdot = mass_matrix.llt().solve(-tau_id);
  ...
  // matrix.
  const double kTolerance = (1.0 / mass_matrix.llt().rcond()) *

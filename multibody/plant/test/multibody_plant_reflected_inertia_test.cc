...
TEST_F(MultibodyPlantReflectedInertiaTests, CalcInverseDynamics) {
  ...
  // M(q) v̇ + C(q, v) v - tau_applied = τ M is mass matrix for plant_.
  // M̃(q) v̇ + C(q, v) v - tau_applied = τ̃ M̃ is the mass matrix for plant_ri,
  // where τ can be calculated by plant_.CalcInverseDynamics()
  // and τ̃ can be calculated by plant_ri_.CalcInverseDynamics().
  // Since reflected inertia should not affect either C(q,v) or tau_applied, the
  // 2nd equation subtracted from the 1st equation gives (M̃ - M) v̇ = τ - τ̃.
  // Setting v̇ to 𝟏 leads to τ - τ̃ = (M̃ - M) [𝟏] , which are exactly the
  // values in reflected_inertia_with_generalized_velocity_indices.

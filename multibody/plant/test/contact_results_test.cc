...
TEST_F(ContactResultsTest, SetAccessClear) {
  ...
  EXPECT_EQ(
      contact_results.hydroelastic_contact_info(0).contact_surface().id_M(),
  ...
  EXPECT_EQ(
      copy.hydroelastic_contact_info(0).contact_surface().id_M(),
      my_hydroelastic_contact_info_->contact_surface().id_M());
...
TEST_F(ContactResultsTest, AssignHydroelasticContactInfo) {
  ...
  // LHS will have an empty vector of alias pointers to hydroelastic contacts.
  {
    ContactResults<double> lhs;
    lhs.set_plant(&plant_);
    lhs.AddContactInfo(point_pair_info_);
    lhs.AddContactInfo(my_hydroelastic_contact_info_.get());
    ASSERT_EQ(lhs.num_hydroelastic_contacts(), 1);
    ASSERT_EQ(lhs.num_point_pair_contacts(), 1);
    ASSERT_EQ(lhs.plant(), &plant_);

    const ContactResults<double> rhs_empty;
    ASSERT_EQ(rhs_empty.num_hydroelastic_contacts(), 0);
    ASSERT_EQ(rhs_empty.num_point_pair_contacts(), 0);
    ASSERT_EQ(rhs_empty.plant(), nullptr);
  ...
  {
    ContactResults<double> lhs;
    ASSERT_EQ(lhs.num_hydroelastic_contacts(), 0);
    ASSERT_EQ(lhs.num_point_pair_contacts(), 0);
    ASSERT_EQ(lhs.plant(), nullptr);
    ...
    ASSERT_EQ(rhs_non_empty.num_hydroelastic_contacts(), 1);
    ASSERT_EQ(rhs_non_empty.num_point_pair_contacts(), 1);
    ASSERT_EQ(rhs_non_empty.plant(), &plant_);
...
// The copy constructor uses the assignment operator. This is just a coverage
// test.
TEST_F(ContactResultsTest, CopyConstructor) {
  const ContactResults<double> from;
  const ContactResults<double> to(from);
}

TEST_F(ContactResultsTest, ViewHydroelastic) {
  ContactResults<double> contact_results;
  contact_results.set_plant(&plant_);
  contact_results.AddContactInfo(point_pair_info_);
  contact_results.AddContactInfo(my_hydroelastic_contact_info_.get());

  const ContactResults<double> no_contacts = contact_results.ViewHydroelastic(
      [](const HydroelasticContactInfo<double>&) -> bool {
        return false;
      });
  EXPECT_EQ(no_contacts.num_point_pair_contacts(), 0);
  EXPECT_EQ(no_contacts.num_hydroelastic_contacts(), 0);

  const ContactResults<double> one_contact = contact_results.ViewHydroelastic(
      [](const HydroelasticContactInfo<double>& contact) -> bool {
        return true;
      });
  EXPECT_EQ(one_contact.num_point_pair_contacts(), 0);
  EXPECT_EQ(one_contact.num_hydroelastic_contacts(), 1);

...
class DeformableDriverTest : public ::testing::Test {
 protected:
  ...
  /* Forwarding calls private member function in DeformableDriver with the same
   name.
   @{ */
  ...
  const fem::FemState<double>& EvalFreeMotionFemState(
      const systems::Context<double>& context,
      DeformableBodyIndex index) const {
    return driver_->EvalFreeMotionFemState(context, index);
  }

  const fem::FemState<double>& EvalNextFemState(
      const systems::Context<double>& context,
      DeformableBodyIndex index) const {

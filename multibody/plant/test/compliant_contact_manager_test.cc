...
class SpheresStack : public ::testing::Test {
 public:
  ...
  void VerifyDiscreteContactPairs(
    ...
    // ContactParameters::dissipation_timescale to indicate we want to build
    ...
    if (*sphere1_point_params.dissipation_timescale < 0 ||
      ...
      DRAKE_EXPECT_THROWS_MESSAGE(
          EvalDiscreteContactPairs(*plant_context_),
          "Dissipation time scale must be positive and dissipation_timescale "
          "= .* was provided. For geometry .* on body .*.");
      return;
    }
...
TEST_F(SpheresStack, DissipationTimeScaleMustBePositive) {
  ContactParameters soft_point_contact{
      1.0e3, std::nullopt, -1.0 /* Negative dissipation time scale */, 1.0};
  ContactParameters hard_point_contact{1.0e40, std::nullopt, 0.0, 1.0};

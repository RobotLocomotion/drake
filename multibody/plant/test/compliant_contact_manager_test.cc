 ...
 public:
  ...
  void MakeDiagram(bool with_algebraic_loop) {
    ...
    if (with_algebraic_loop) {
      ...
      // plant straight into the controller, creating an algebraic loop. A more
      // realistic model would include a force estimator, that most likely would
      // introduce state and break the algebraic loop. Another option would be
      // to introduce a delay between the force output ant the controller,
      // effectively modeling a delay in the measured signal. This last strategy
      // also introduces state and also breaks the algebraic loop.
      auto* feedback = builder.AddSystem<PassThrough>(plant_->num_velocities());
...
TEST_F(AlgebraicLoopDetection, VerifyNoFalsePositivesWhenCachingIsDisabled) {
  ...
  EXPECT_NO_THROW(
      plant_
          ->get_generalized_contact_forces_output_port(default_model_instance())
          .Eval(*plant_context_));
  // However the workaround for #12786 can mistake successive evaluations of
  // contact forces with an algebraic loop when caching is disabled. We verify
  // we at least get a proper error message.
  DRAKE_EXPECT_THROWS_MESSAGE(
      plant_
          ->get_generalized_contact_forces_output_port(default_model_instance())
          .Eval(*plant_context_),
      "Algebraic loop detected with caching disabled.*");
}

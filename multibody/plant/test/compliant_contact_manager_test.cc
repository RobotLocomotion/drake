...
// Due to #12786 we cannot properly declare the dependence of contact forces on
// the plant's inputs. Therefore CompliantContactManager implements an algebraic
// loop detection that we test here.
GTEST_TEST(CompliantContactManager, AlgebraicLoopDetection) {
  systems::DiagramBuilder<double> builder;
  MultibodyPlant<double>& plant = AddMultibodyPlantSceneGraph(&builder, 1.0e-3);
  ...
  // We intentionally create an algebraic loop by placing a pass through system
  // between the contact forces output and the input forces.
  auto* feedback = builder.AddSystem<PassThrough>(plant.num_velocities());
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      plant.get_generalized_contact_forces_output_port(default_model_instance())
          .Eval(plant_context),

...
class SpheresStackTest : public SpheresStack, public ::testing::Test {
 public:
  ...
  void VerifyDiscreteContactPairs(
    ...
    const GeometryContactSummary<double>& geometry_contact_summary =
        EvalGeometryContactSummary(*plant_context_);
    const std::vector<PenetrationAsPointPair<double>>& point_pairs =
        geometry_contact_summary.point_pairs;
  ...
  const GeometryContactSummary<double>& EvalGeometryContactSummary(
      const Context<double>& context) const {
    return CompliantContactManagerTester::EvalGeometryContactSummary(
...
TEST_F(SpheresStackTest,
  ...
  const std::vector<PenetrationAsPointPair<double>>& point_pairs =
      EvalGeometryContactSummary(*plant_context_).point_pairs;
  const int num_point_pairs = point_pairs.size();
  ...
  const std::vector<geometry::ContactSurface<double>>& surfaces =
      EvalGeometryContactSummary(*plant_context_).surfaces;

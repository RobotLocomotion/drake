...
class SphereOnGround : public ::testing::TestWithParam<TestConfig> {
 ...
 protected:
  ...
  // Helper to retrieve the TamsiDriver under test.
  const TamsiDriver<double>& tamsi_driver() {
    return CompliantContactManagerTester::tamsi_driver(*manager_);
  }
...
TEST_P(SphereOnGround, VerifyEquilibriumConfiguration) {
  ...
  tamsi_driver().CalcContactSolverResults(*plant_context_, &results);

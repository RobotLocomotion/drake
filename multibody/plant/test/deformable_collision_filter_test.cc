...
class DeformableCollisionFilterTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    /* Apply a few collision filters that only affect rigid geometries. We test
     below that they do not affect deformable geometries. */
...
/* Tests that the default collision filters applied in MbP, as well as the
 filters applied in MbP::ExcludeCollisionGeometriesWithCollisionFilterGroupPair,
 do not affect deformable bodies. That is, the deformable body is in contact
 with both rigid geometries. */
TEST_F(DeformableCollisionFilterTest, MbPFilterIgnoresDeformable) {

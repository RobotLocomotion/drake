...
int VerifyModelInstanceIsATreeAndReturnTreeIndex(
  ...
  // For the purpose of these tests, verify the entire model instance has
...
void VerifyDofsWhenModelInstanceIsATree(
  ...
  if (is_floating) {
    ...
    for (int i = 0; i < 6; ++i) {
      ...
      // N.B. Since we use a "reverse DFT order, the six dofs of a floating base
      // will always be at the end of a tree's dofs.
...
GTEST_TEST(DofsPermutation, VelocitiesPermutation) {
  ...
  // We expect four threes in the forest.

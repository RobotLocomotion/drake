...
class TwoBodiesTest : public ::testing::TestWithParam<TestConfig> {
 ...
 protected:
  ...
  const RigidTransformd X_AP_{RotationMatrixd::MakeZRotation(0.01 * M_PI),
                              p_AP_};
  const RigidTransformd X_BQ_{RotationMatrixd::MakeZRotation(-0.01 * M_PI),
                              p_BQ_};
...
TEST_P(TwoBodiesTest, ConfirmConstraintProperties) {
  ...
  // R_WQ.
  //
  // a_PQ_N = θ⋅λ̂ is the axis-angle representation of of the relative
  ...
  const Vector3d a_PQ_N(0.5 * (R(2, 1) - R(1, 2)), 0.5 * (R(0, 2) - R(2, 0)),
                        0.5 * (R(1, 0) - R(0, 1)));
  const Vector6d g_expected = (Vector6d() << a_PQ_N, p_PQ_N).finished();
  ...
  //        = (w_WB_N, v_WB_N + w_WB_N x p_BQ_N) - (w_WA_N, v_WA_N + w_WA_N x
  //        p_AP_N) = (w_WB_N, v_WB_N - p_BQ_N x w_WB_N) - (w_WA_N, v_WA_N -
  //        p_AP_N x w_WA_N) = [   [R_NW]       0 ]            [   [R_NW] 0 ]

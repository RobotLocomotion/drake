...
TEST_P(TwoBodiesTest, ConfirmConstraintProperties) {
  ...
  // We know by construction that the 6 generalized coordinates for a floating
  // body A are laid out in the same order as V_WA, we can express the Jacobian
  // of the constraint with respect to each body's spatial velocity.:
  //
  // g = p_PQ_W
  //
  // d(g)/dt = v_PQ_W

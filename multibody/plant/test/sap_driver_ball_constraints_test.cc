...
GTEST_TEST(BallConstraintsTests, VerifyIdMapping) {
  ...
  ConstraintId ball_id = plant.AddBallConstraint(bodyA, p_AP, bodyB, p_BQ);
  const BallConstraintSpecs& ball_specs =
      plant.get_ball_constraint_specs(ball_id);
  EXPECT_EQ(ball_specs.id, ball_id);
  EXPECT_EQ(ball_specs.body_A, bodyA.index());
  EXPECT_EQ(ball_specs.body_B, bodyB.index());
  EXPECT_EQ(ball_specs.p_AP, p_AP);
  EXPECT_EQ(ball_specs.p_BQ, p_BQ);

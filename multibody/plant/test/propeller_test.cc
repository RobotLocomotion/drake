...
GTEST_TEST(PropellerTest, SinglePropTest) {
  ...
  auto prop = builder.AddSystem<Propeller<double>>(
      body.index(), math::RigidTransform<double>::Identity(), thrust_ratio,
      moment_ratio);
...
GTEST_TEST(PropellerTest, BiRotorTest) {
  ...
  PropellerInfo prop1(body.index(), T, thrust_ratio, moment_ratio);
  T.set_translation(Eigen::Vector3d(-arm_length, 0, 0));
  PropellerInfo prop2(body.index(), T, thrust_ratio, moment_ratio);

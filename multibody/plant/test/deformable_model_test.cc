...
TEST_F(DeformableModelTest, GetBodyIdFromBodyIndex) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      deformable_model_ptr_->GetBodyIdOrThrow(DeformableBodyIndex(0)),
      ".*GetBodyIdOrThrow.*before system resources have been declared.*");
  plant_->Finalize();
  EXPECT_EQ(deformable_model_ptr_->GetBodyIdOrThrow(DeformableBodyIndex(0)),
            body_id);
  // Throws for invalid indexes.
  EXPECT_THROW(deformable_model_ptr_->GetBodyIdOrThrow(DeformableBodyIndex(1)),
               std::exception);
  EXPECT_THROW(deformable_model_ptr_->GetBodyIdOrThrow(DeformableBodyIndex()),
               std::exception);
}
...
TEST_F(DeformableModelTest, GetGeometryId) {
  ...
  geometry::GeometryId geometry_id =
      deformable_model_ptr_->GetGeometryIdOrThrow(body_id);
  const SceneGraphInspector<double>& inspector =
      scene_graph_->model_inspector();
  EXPECT_TRUE(inspector.IsDeformableGeometry(geometry_id));
  DeformableBodyId fake_id = DeformableBodyId::get_new_id();
  DRAKE_EXPECT_THROWS_MESSAGE(
      deformable_model_ptr_->GetGeometryIdOrThrow(fake_id),
      "GetGeometryIdOrThrow.*No deformable body with id.*");
}

TEST_F(DeformableModelTest, ToModelPointerVariant) {
  ModelPointerVariant<double> variant = deformable_model_ptr_->ToModelPointerVariant();
  EXPECT_TRUE(

...
// The same mesh as returned by CreateSurfaceMesh() except rather than two
// triangles, it is a single quad.
std::unique_ptr<PolygonSurfaceMesh<double>> CreatePolygonalMesh() {
  // Create the vertices, all of which are offset vectors defined in the
  // halfspace body frame.
  std::vector<Vector3<double>> vertices = {
      {0.5, 0.5, -0.5},
  ...
  // We define a single squared polygon.

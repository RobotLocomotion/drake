...
GTEST_TEST(HydroelasticTractionCalculatorTest,
  ...
  RigidTransform<AutoDiffXd> X_WA(
      math::InitializeAutoDiffFromValueMatrix(Vector3<double>(0, 0, 1)));
  ...
  EXPECT_TRUE(CompareMatrices(math::ExtractGradientMatrixFromAutoDiff(nhat_W),
                              zeros, 1e-15));
  ...
  const Matrix3<double> grad_traction_Aq_W =
      math::ExtractGradientMatrixFromAutoDiff(point_data.traction_Aq_W);
  ...
  const Vector3<double> dtdz =
      0.5 * E / h * math::ExtractValueMatrixFromAutoDiff(nhat_W);

...
GTEST_TEST(HydroelasticTractionCalculatorTest,
  ...
  vertices.emplace_back(p_WC + Vector3<AutoDiffXd>(-0.5, 0, 0));
  vertices.emplace_back(p_WC + Vector3<AutoDiffXd>(0, -0.5, 0));
  ...
  // direction of the normal to be in the +z axis, the normal traction on body A
  // will point into the -z direction. This is important to understand the sign
  // of the resulting derivative below.
  ...
  // For this simple case the expression for the z component tz of the traction
  // traction_Aq_W is:
  //  tz = -0.5 * p_WA[2] * E / H
  // where the negative sign stems from the direction of the normal and the
  // convention that it points "into" body A (see note above).
  // Therefore the derivative of tz along p_WA[2] is:
  //   tz_derivative = -0.5 * E / H = -5.0e5.
  // The remaining 8 components of the gradient are trivially zero.
  const double tz_derivative = -0.5 * E / H;
  Matrix3<double> expected_grad_traction_Aq_W = Matrix3<double>::Zero();
  expected_grad_traction_Aq_W(2, 2) = tz_derivative;

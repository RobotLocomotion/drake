...
/* Friend class to expose the internal of ContactResultsToLcmSystem. */
...
/* Transforms Vector3<T> --> Vector3<double>.  */
template <typename T>
Vector3<double> Vector3DoubleOrThrow(const Vector3<T>& v) {
  return Vector3<double>(ExtractDoubleOrThrow(v[0]), ExtractDoubleOrThrow(v[1]),
                         ExtractDoubleOrThrow(v[2]));
}

/* Retrieves a set of fixed geometry ids -- always the same N values. */
template <int N>
 ...
 protected:
  void SetUp() {
    if constexpr (std::is_same_v<T, symbolic::Expression>) {
      /* SceneGraph<symbolic::Exppression> is *not* supported. */
  ...
  static void AddFakePointPairContact(ContactResultsToLcmSystem<U>* lcm_system,
    ...
    static const PointPairContactInfo<U> pair1(
    ...
    results->AddContactInfo(pair1);
    static const PointPairContactInfo<U> pair2(
  ...
  /* Adds fake hydro contact results to the given set of contact `results`.
   ...
   nulltr). This doesn't affect the simple body_names vector. */
  template <typename U = T>
...
TYPED_TEST(ContactResultsToLcmTest, EmptyContactResults) {
  ...
  auto confirm_empty = [&lcm, &context = *context, kTime]() {
    const auto& msg = lcm.get_lcm_message_output_port()
                          .template Eval<lcmt_contact_results_for_viz>(context);

    // Message time is an integer number of microseconds.
    const double kTimeMicroSec =
        static_cast<int64_t>(ExtractDoubleOrThrow(kTime) * 1e6);
    EXPECT_EQ(msg.timestamp, kTimeMicroSec);
    EXPECT_EQ(msg.num_point_pair_contacts, 0);
    EXPECT_EQ(msg.point_pair_contact_info.size(), 0);
    EXPECT_EQ(msg.num_hydroelastic_contacts, 0);
    EXPECT_EQ(msg.hydroelastic_contacts.size(), 0);
  };
...
/* Tests the case where ContactResults contains *only* point-pair data. This
 ...
 There are *three* things that this system does in the process:
   ...
   - Writes various Vector3 quantities (same type different semantics).
...
TYPED_TEST(ContactResultsToLcmTest, PointPairContactOnly) {
  ...
  EXPECT_EQ(msg.num_point_pair_contacts, 2);
  EXPECT_EQ(msg.point_pair_contact_info.size(), 2);
  EXPECT_EQ(msg.num_hydroelastic_contacts, 0);
  EXPECT_EQ(msg.hydroelastic_contacts.size(), 0);
  ...
  auto confirm_message = [kTime, &body_names](
                             ...
                             const ContactResults<T>& contacts, int i) {
    // Message time is an integer number of microseconds.
    const double kTimeMicroSec =
        static_cast<int64_t>(ExtractDoubleOrThrow(kTime) * 1e6);
    const auto& pair_msg = msg.point_pair_contact_info[i];
    const auto& pair_data = contacts.point_pair_contact_info(i);

    EXPECT_EQ(pair_msg.timestamp, kTimeMicroSec);
    EXPECT_EQ(pair_msg.body1_name, body_names[pair_data.bodyA_index()]);
    EXPECT_EQ(pair_msg.body2_name, body_names[pair_data.bodyB_index()]);
    // clang-format off
    EXPECT_TRUE(CompareMatrices(
        Vector3<double>(pair_msg.contact_point),
        Vector3DoubleOrThrow(pair_data.contact_point())));
    EXPECT_TRUE(CompareMatrices(
        Vector3<double>(pair_msg.contact_force),
        Vector3DoubleOrThrow(pair_data.contact_force())));
    EXPECT_TRUE(CompareMatrices(
        Vector3<double>(pair_msg.normal),
        Vector3DoubleOrThrow(pair_data.point_pair().nhat_BA_W)));
    // clang-format on
    /* None of the rest of the pair data makes it to the message. */
  };

  for (int i = 0; i < 2; ++i) {
    SCOPED_TRACE(fmt::format("Penetration point pair {} out of 2", (i + 1)));
    confirm_message(msg, contacts, i);
  }
}
...
TYPED_TEST(ContactResultsToLcmTest, HydroContactOnly) {
  ...
  EXPECT_EQ(msg.num_point_pair_contacts, 0);
  EXPECT_EQ(msg.point_pair_contact_info.size(), 0);
  EXPECT_EQ(msg.num_hydroelastic_contacts, 2);
  EXPECT_EQ(msg.hydroelastic_contacts.size(), 2);
  ...
  auto confirm_message = [&geo_to_body_map](
                             const lcmt_contact_results_for_viz& msg,
                             const ContactResults<T>& contacts, int i) {
    const auto& pair_msg = msg.hydroelastic_contacts[i];
    const auto& pair_data = contacts.hydroelastic_contact_info(i);
    const auto& surface = pair_data.contact_surface();
    const auto& mesh = surface.mesh_W();

    EXPECT_EQ(pair_msg.body1_name, geo_to_body_map.at(surface.id_M()));
    EXPECT_EQ(pair_msg.body2_name, geo_to_body_map.at(surface.id_N()));

    /* Mesh aggregate results: centroid, force, moment. */
    // clang-format off
    EXPECT_TRUE(CompareMatrices(
        Vector3<double>(pair_msg.centroid_W),
        Vector3DoubleOrThrow(mesh.centroid())));
    EXPECT_TRUE(CompareMatrices(
        Vector3<double>(pair_msg.force_C_W),
        Vector3DoubleOrThrow(pair_data.F_Ac_W().translational())));
    EXPECT_TRUE(CompareMatrices(
        Vector3<double>(pair_msg.moment_C_W),
        Vector3DoubleOrThrow(pair_data.F_Ac_W().rotational())));
    // clang-format on

    /* Compare meshes and pressure fields. */
    EXPECT_EQ(pair_msg.num_triangles, mesh.num_faces());
    EXPECT_EQ(pair_msg.triangles.size(), mesh.num_faces());
    const auto& field = surface.e_MN();
    for (SurfaceFaceIndex f(0); f < mesh.num_faces(); ++f) {
      const auto& tri_msg = pair_msg.triangles[f];
      const auto& tri_data = mesh.element(f);
      // clang-format off
      EXPECT_TRUE(CompareMatrices(
          Vector3<double>(tri_msg.p_WA),
          Vector3DoubleOrThrow(mesh.vertex(tri_data.vertex(0)).r_MV())));
      EXPECT_TRUE(CompareMatrices(
          Vector3<double>(tri_msg.p_WB),
          Vector3DoubleOrThrow(mesh.vertex(tri_data.vertex(1)).r_MV())));
      EXPECT_TRUE(CompareMatrices(
          Vector3<double>(tri_msg.p_WC),
          Vector3DoubleOrThrow(mesh.vertex(tri_data.vertex(2)).r_MV())));
      // clang-format on
      EXPECT_EQ(
          tri_msg.pressure_A,
    ...
    const auto& msg_quads = pair_msg.quadrature_point_data;
    EXPECT_EQ(pair_msg.num_quadrature_points, data_quads.size());
    EXPECT_EQ(msg_quads.size(), data_quads.size());
    EXPECT_EQ(pair_msg.num_quadrature_points,
              mesh.num_faces() * kNumPointPerTri);
    for (int q = 0; q < static_cast<int>(data_quads.size()); ++q) {
      // clang-format off
      EXPECT_TRUE(CompareMatrices(
        Vector3<double>(msg_quads[q].p_WQ),
        Vector3DoubleOrThrow(data_quads[q].p_WQ)));
      EXPECT_TRUE(CompareMatrices(
        Vector3<double>(msg_quads[q].vt_BqAq_W),
        Vector3DoubleOrThrow(data_quads[q].vt_BqAq_W)));
      EXPECT_TRUE(CompareMatrices(
        Vector3<double>(msg_quads[q].traction_Aq_W),
        Vector3DoubleOrThrow(data_quads[q].traction_Aq_W)));
      // clang-format on
    }
  };

  for (int i = 0; i < 2; ++i) {
    SCOPED_TRACE(fmt::format("Hydro contact {} out of 2", (i + 1)));
    confirm_message(msg, contacts, i);
  }
}
...
TYPED_TEST(ContactResultsToLcmTest, Transmogrifcation) {
  ...
  /* We don't have to test transmogrification from double to double. */
  if constexpr (!std::is_same_v<T, double>) {
    ...
    ContactResultsToLcmSystem<T> lcm_T(lcm_double);

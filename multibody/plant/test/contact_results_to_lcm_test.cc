...
/* ContactResultsToLcmSystem basically does two things:
 ...
 ContactResults contains various vectors of data (e.g., vertex positions, etc.)
 The test is *assuming* that the order of the data in the input is the order of
 the data on the output. It simplifies the test and only fails if the
 serialization perturbs the order. */
template <typename T>
...
TYPED_TEST(NonsymbolicContactResultsToLcmTest, HydroContactOnly) {
  ...
  auto confirm_message = [&geo_to_body_map](
    ...
    /* Quadrature data is always empty these days. */
    EXPECT_EQ(pair_message.num_quadrature_points, 0);
    EXPECT_TRUE(pair_message.quadrature_point_data.empty());
  };

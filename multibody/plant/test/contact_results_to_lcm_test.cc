...
/* For the purpose of thie test, enable writing FullBodyName to string so that
...
class ContactResultsToLcmTest : public ::testing::Test {
 protected:
  ...
  /* Adds a body with the given `name` to the given `plant` as part of the given
   model instance. In order to test the resulting LCM message data fields, it
   places the expected strings representing the body's name, model instance
   name, and geometry name in the given `id_to_body_map` and `body_names`
   tables.

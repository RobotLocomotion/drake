...
// joints.
class ContactResultsToMeshcatSystemTest : public ::testing::Test {
 protected:
  ContactResultsToMeshcatSystemTest() : meshcat_(std::make_shared<Meshcat>()) {}
  ...
  void SetUpDiagram(const ContactResultsToMeshcatParams& cparams =
    ...
    visualizer_ = &ContactResultsToMeshcatSystemd::AddToBuilder(
        &builder, plant, meshcat_, cparams);
  ...
  const ContactResultsToMeshcatSystem<double>* visualizer_{};
  std::unique_ptr<systems::Diagram<double>> diagram_{};
  std::unique_ptr<systems::Context<double>> context_{};
};

TEST_F(ContactResultsToMeshcatSystemTest, Publish) {
  SetUpDiagram();
...
TEST_F(ContactResultsToMeshcatSystemTest, Parameters) {
...
TEST_F(ContactResultsToMeshcatSystemTest, Delete) {
  SetUpDiagram();
...
GTEST_TEST(ContactResultsToMeshcatSystem, PortTest) {
  auto meshcat = std::make_shared<Meshcat>();
  // Also tests the default constructor.
  ContactResultsToMeshcatSystem<double> visualizer(meshcat);
...
// plant).
GTEST_TEST(ContactResultsToMeshcatSystem, AddToBuilderVariant) {
  auto meshcat = std::make_shared<Meshcat>();
  systems::DiagramBuilder<double> builder;
  auto [plant, scene_graph] =
      multibody::AddMultibodyPlantSceneGraph(&builder, 0.001);
  ...
  ContactResultsToMeshcatSystemd::AddToBuilder(
...
TEST_F(ContactResultsToMeshcatSystemTest, ScalarConversion) {
  SetUpDiagram();

...
/* Friend class giving the tests below access to the SAP contact problem that
the driver hands to the solver -- the only place we can see for certain which
constraints the solver actually got. */
class SapDriverTest {
 public:
...
class FourBar {
 public:
  ...
  /* Builds the diagram that owns the plant and its SceneGraph. Only the tests
  that actually step the dynamics need this; a Context taken from the diagram
  has the plant's geometry query input port connected, which any contact query
  (and hence any discrete update) requires. Call after Finalize(); `plant()`
  stays valid, since the diagram takes ownership of the same objects. */
  std::unique_ptr<systems::Diagram<double>> Build() { return builder_.Build(); }
...
GTEST_TEST(ClosedTopologyTest, LoopJointIsRetargetedToTheShadowLink) {
  ...
  EXPECT_EQ(shadow_frame->source_frame().index(),
            loop_joint.frame_on_parent().index());
...
/* A shadow frame belongs to the model instance of the loop joint whose frame it
substitutes for, which is not necessarily the model instance of the shadow link
it is fixed to: a joint takes its model instance from its child link, while a
shadow link inherits its primary's. Those two disagree whenever the loop is
closed by a joint that spans model instances and it is that joint's parent end
that gets retargeted. A Frame has two clone paths and both are checked here.

The four-bar built here is the one described at the top of this file, minus the
poses and inertias that don't affect which link gets split, but with the rocker
in a model instance of its own. That puts the retargeted joint (coupler_rocker)
in the rocker's model instance while the coupler's shadow is in the coupler's
model instance. */
GTEST_TEST(ClosedTopologyTest, ShadowFrameKeepsItsModelInstanceWhenCloned) {
  MultibodyPlant<double> plant(0.0 /* continuous */);
  plant.SetEnableLoopTopology(true);
  const ModelInstanceIndex linkage_instance = plant.AddModelInstance("linkage");
  const ModelInstanceIndex rocker_instance = plant.AddModelInstance("rocker");

  const SpatialInertia<double> M = SpatialInertia<double>::SolidCubeWithMass(
      1.0 /* mass */, 0.1 /* length */);
  const Link<double>& driver =
      plant.AddRigidBody("driver", linkage_instance, M);
  const Link<double>& coupler =
      plant.AddRigidBody("coupler", linkage_instance, M);
  const Link<double>& rocker = plant.AddRigidBody("rocker", rocker_instance, M);

  const Vector3<double> axis = Vector3<double>::UnitY();
  plant.AddJoint<RevoluteJoint>("world_driver", plant.world_body(), {}, driver,
                                {}, axis);
  plant.AddJoint<RevoluteJoint>("world_rocker", plant.world_body(), {}, rocker,
                                {}, axis);
  plant.AddJoint<RevoluteJoint>("driver_coupler", driver, {}, coupler, {},
                                axis);
  plant.AddJoint<RevoluteJoint>("coupler_rocker", coupler, {}, rocker, {},
                                axis);
  plant.Finalize();
  ...
  ASSERT_NE(retargeted.joint, nullptr);
  const Joint<double>& loop_joint = *retargeted.joint;
  ASSERT_EQ(loop_joint.name(), "coupler_rocker");
  // It is the coupler (parent) end that moved onto the coupler's shadow, so
  // the joint and the shadow link really do live in different model instances
  // -- otherwise the checks below would pass vacuously.
  ASSERT_TRUE(retargeted.moved_parent);
  ASSERT_EQ(shadow.model_instance(), linkage_instance);
  ASSERT_EQ(loop_joint.model_instance(), rocker_instance);

  const Frame<double>& shadow_frame = loop_joint.effective_frame_on_parent();
  ASSERT_EQ(shadow_frame.body().index(), shadow.index());
  EXPECT_EQ(shadow_frame.model_instance(), rocker_instance);

  // A shallow clone keeps the frame's properties, including the model instance.
  const std::unique_ptr<Frame<double>> shallow_clone =
      shadow_frame.ShallowClone();
  EXPECT_EQ(shallow_clone->name(), shadow_frame.name());
  EXPECT_EQ(shallow_clone->model_instance(), rocker_instance);
  EXPECT_TRUE(shallow_clone->is_ephemeral());

  // Scalar conversion also clones the shadow frame, and the clone must be
  // reachable by the same instance-scoped name lookup as the original.
  std::unique_ptr<MultibodyPlant<AutoDiffXd>> plant_ad =
      systems::System<double>::ToAutoDiffXd(plant);
  const Joint<AutoDiffXd>& loop_joint_ad =
      plant_ad->GetJointByName("coupler_rocker");
  const Frame<AutoDiffXd>& shadow_frame_ad =
      loop_joint_ad.effective_frame_on_parent();
  ASSERT_NE(&shadow_frame_ad, &loop_joint_ad.frame_on_parent());
  EXPECT_EQ(shadow_frame_ad.model_instance(), rocker_instance);
  EXPECT_EQ(
      plant_ad->GetFrameByName(shadow_frame.name(), rocker_instance).index(),
      shadow_frame.index());
}
...
const internal::WeldConstraintSpec& GetSoleLoopConstraintSpec(
  ...
  static const never_destroyed<internal::WeldConstraintSpec> kEmpty;
  ...
  EXPECT_EQ(found.size(), 1);
  return found.empty() ? kEmpty.access() : *found.front();
}

/* Breaking the loop leaves the coupler and its shadow as two disconnected
partitions of the same link; a weld constraint between them is what actually
closes the loop. The user didn't ask for that constraint, so it is marked
ephemeral. The shadow is constructed expecting its link frame to be made
coincident with the primary's link frame, so the constraint welds the two link
frames directly, with no pose offset. */
...
except that the user didn't add it, so it has to coexist with -- but be
...
/* The loop is closed exactly when the shadow link's frame coincides with its
primary's, so X_PS (the return value, pose of the shadow measured in the
primary) is the loop closure error: the identity when closed, and anything
else when not. */
math::RigidTransformd CalcLoopClosureError(
    const MultibodyPlant<double>& plant,
    const systems::Context<double>& context) {
  const internal::WeldConstraintSpec& spec = GetSoleLoopConstraintSpec(plant);
  const math::RigidTransformd& X_WP =
      plant.EvalBodyPoseInWorld(context, plant.get_body(spec.body_A));
  const math::RigidTransformd& X_WS =
      plant.EvalBodyPoseInWorld(context, plant.get_body(spec.body_B));
  return X_WP.InvertAndCompose(X_WS);
}
...
Here we look directly into the SapContactProblem that SapDriver builds for the
discrete update, since that problem _is_ what the solver is handed. When the
constraint is active the problem contains a weld constraint that is
demonstrably the loop's: it couples two cliques (the loop's two branches) and
its constraint function is exactly the primary-to-shadow error we independently
measure from the plant. When the constraint is deactivated the problem contains
no constraints at all. */
GTEST_TEST(ClosedTopologyTest, LoopConstraintReachesTheDiscreteSolver) {
  FourBar four_bar(0.01 /* discrete, SAP */);
  MultibodyPlant<double>& plant = four_bar.plant();
  plant.Finalize();
  ASSERT_EQ(plant.get_discrete_contact_solver(), DiscreteContactSolver::kSap);

  // Swap in our own manager (the same class Finalize() installs by default) so
  // that we can reach its SapDriver.
  auto owned_manager =
      std::make_unique<internal::CompliantContactManager<double>>();
  const internal::CompliantContactManager<double>& manager = *owned_manager;
  plant.SetDiscreteUpdateManager(std::move(owned_manager));
  const internal::SapDriver<double>& sap_driver =
      internal::CompliantContactManagerTester::sap_driver(manager);

  // The SAP problem includes contact, so we need a Context in which the
  // plant's geometry query port is connected; only the diagram provides that.
  std::unique_ptr<systems::Diagram<double>> diagram = four_bar.Build();
  std::unique_ptr<systems::Context<double>> diagram_context =
      diagram->CreateDefaultContext();
  systems::Context<double>& context =
      plant.GetMyMutableContextFromRoot(diagram_context.get());

  // Move just the driver, leaving the rocker (and with it the shadow) behind,
  // so the loop is open by a large, easily measured amount.
  plant.GetJointByName<RevoluteJoint>("world_driver").set_angle(&context, 0.25);
  const math::RigidTransformd X_PS = CalcLoopClosureError(plant, context);
  ASSERT_GT(X_PS.translation().norm(), 0.1);

  const MultibodyConstraintId id = GetSoleLoopConstraintSpec(plant).id;
  ASSERT_TRUE(plant.GetConstraintActiveStatus(context, id));

  {
    const contact_solvers::internal::SapContactProblem<double>& problem =
        *internal::SapDriverTest::EvalContactProblemCache(sap_driver, context)
             .sap_problem;

    // The loop weld is the model's only constraint (there is no contact),
    // and a weld contributes six equations.
    ASSERT_EQ(problem.num_constraints(), 1);
    EXPECT_EQ(problem.num_constraint_equations(), 6);
    const auto* weld = dynamic_cast<
        const contact_solvers::internal::SapWeldConstraint<double>*>(
        &problem.get_constraint(0));
    ASSERT_NE(weld, nullptr);

    // Breaking the loop left two branches, each its own clique; the weld
    // is what couples them.
    EXPECT_EQ(weld->num_cliques(), 2);
    EXPECT_NE(weld->first_clique(), weld->second_clique());

    // This is the loop's weld, not some other one: SAP's constraint function
    // g = (a_PS_W, p_PS_W) is the loop closure error we measured above.
    // (Both frame offsets are the identity; see
    // LoopIsClosedByAnEphemeralWeldConstraint.)
    const Vector6<double>& g = weld->constraint_function();
    const math::RigidTransformd& X_WP =
        plant.EvalBodyPoseInWorld(context, plant.GetBodyByName("coupler"));
    const Eigen::AngleAxisd a_PS = X_PS.rotation().ToAngleAxis();
    EXPECT_TRUE(CompareMatrices(
        g.head<3>(), X_WP.rotation() * (a_PS.angle() * a_PS.axis()), 1e-14));
    EXPECT_TRUE(CompareMatrices(g.tail<3>(),
                                X_WP.rotation() * X_PS.translation(), 1e-14));
  }

  // Deactivating it takes it back out of the solver's problem entirely; there
  // is then nothing left for SAP to enforce.
  plant.SetConstraintActiveStatus(&context, id, false);
  ASSERT_FALSE(plant.GetConstraintActiveStatus(context, id));
  const contact_solvers::internal::SapContactProblem<double>& problem =
      *internal::SapDriverTest::EvalContactProblemCache(sap_driver, context)
           .sap_problem;
...
/* The previous test shows the constraint is in (or out of) the problem SAP is
handed. This one shows that it matters dynamically: the solver actually holds
the loop closed while the constraint is active, and the mechanism comes apart
at the split link when it isn't.

Gravity is tipped into +x so that the linkage moves at all -- its default
configuration, with both cranks vertical, is an equilibrium under plain -z
gravity. */
GTEST_TEST(ClosedTopologyTest, SapEnforcesTheLoopConstraint) {
  constexpr double kTimeStep = 1.0e-3;
  constexpr int kNumSteps = 100;  // 0.1 seconds of simulated time.

  // Simulates kNumSteps discrete updates from the default (assembled, at rest)
  // state and returns how far the loop drifted open, in meters.
  auto simulate_and_measure_opening = [&](bool constraint_active) {
    FourBar four_bar(kTimeStep /* discrete, SAP */);
    MultibodyPlant<double>& plant = four_bar.plant();
    plant.mutable_gravity_field().set_gravity_vector({5.0, 0.0, -9.81});
    plant.Finalize();
    std::unique_ptr<systems::Diagram<double>> diagram = four_bar.Build();
    ...
    plant.SetConstraintActiveStatus(
        &context, GetSoleLoopConstraintSpec(plant).id, constraint_active);

    // The model starts out assembled, so there is nothing to hold closed yet.
    EXPECT_LT(CalcLoopClosureError(plant, context).translation().norm(), 1e-14);

    simulator.AdvanceTo(kNumSteps * kTimeStep);

    // Whatever else happened, the linkage really did swing; otherwise the
    // "constraint active" case below would pass for want of any motion at all.
    EXPECT_GT(std::abs(plant.GetJointByName<RevoluteJoint>("world_driver")
                           .get_angle(context)),
              0.01);
    return CalcLoopClosureError(plant, context).translation().norm();
  };

  // Active: SAP holds the primary and shadow together, so the linkage still
  // moves as a closed four-bar after 0.1 s of swinging.
  EXPECT_LT(simulate_and_measure_opening(true), 1e-6);

  // Inactive: the two branches swing independently -- the driver carries half
  // the coupler's mass and the rocker carries the other half -- and the split
  // link visibly comes apart.
  EXPECT_GT(simulate_and_measure_opening(false), 1e-2);
}
...
GTEST_TEST(ClosedTopologyTest, ContinuousDynamicsSaysWhereLoopConstraintsCame) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      "Finalize\\(\\) added in order to close the kinematic loop in this "
      "model \\(see SetEnableLoopTopology\\(\\)\\).*");
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      "order to close the kinematic loop in this model "
      "\\(see SetEnableLoopTopology\\(\\)\\).*");
}
...
link is already named as a child by one of the loop's joints.

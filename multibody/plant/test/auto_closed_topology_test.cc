...
are intended to coincide). The user-facing per-link (default) mass is unchanged
for the coupler (its full declared 1 kg) while the shadow link's default
reflects its half share; the effective (split) inertia checked here is what
drives the dynamics. Also ensure the unsplit links (driver and rocker) are
unaffected. */
GTEST_TEST(ClosedTopologyTest, ShadowMassIsSplitEvenly) {
  ...
  // equal rotational inertia (their link frames are intended to coincide).
...
artifact intended to coincide with its primary link, but it must still have an
(empty) entry in the plant's per-body geometry arrays, which are indexed by
BodyIndex and so must stay dense over num_bodies(). Shadow links are created
inside MultibodyTree::Finalize() rather than by MultibodyPlant::AddRigidBody()
(which is what normally extends those arrays), so Finalize() has to extend
them. */
...
one of the two joints attached to the coupler must be re-targeted to the shadow.
...
substitution is numerically a no-op: a shadow's link frame is intended to
coincide with its primary's, and neither the coupler nor its shadow is fused
into a welded composite, so the substituted frame's pose in its link frame is
identical to the pose of the user's frame on the primary link. Only the
structure -- which link the mobilizer's frame is fixed to -- distinguishes a
retargeted joint from one that was left pointing at the primary. */
...
GTEST_TEST(ClosedTopologyTest, MovingThePrimaryFrameMovesTheShadowFrame) {
  ...
  // The shadow-side frame starts out with the same pose as the user's frame.
  // Each pose is measured in its own link frame, and those two link frames are
  // intended to coincide (when the weld constraint is satisfied) so the two
  // local poses will be the same.
...
GTEST_TEST(ClosedTopologyTest, LoopIsClosedByAnEphemeralWeldConstraint) {
  ...
  // frame is intended to coincide with its primary link's frame, so both

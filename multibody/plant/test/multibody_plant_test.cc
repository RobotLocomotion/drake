...
GTEST_TEST(MultibodyPlantTest, RigidBodyParameters) {
  ...
  const SpatialInertia<double> sphere_inertia_in_context =
      plant.GetBodySpatialInertia(*context, sphere);

  const SpatialInertia<double> cube_inertia_in_context =
      plant.GetBodySpatialInertia(*context, cube);
  ...
  plant.SetBodySpatialInertia(context.get(), sphere, new_sphere_params);
  plant.SetBodySpatialInertia(context.get(), cube, new_cube_params);
...
GTEST_TEST(MultibodyPlantTest, AutoDiffAcrobotParameters) {
  ...
  const double G1 = (1.0 / 12.0) * l1 * l1;
  const double G2 = (1.0 / 12.0) * l2 * l2;
  const double Ic1 = m1 * G1;
  const double Ic2 = m2 * G2;
  ...
  const AutoDiffXd G2_ad = (1.0 / 12.0) * l2_ad * l2_ad;
  ...
  UnitInertia<AutoDiffXd> G1_Bcm =
      UnitInertia<AutoDiffXd>::StraightLine(G1, Vector3d::UnitZ());
  SpatialInertia<AutoDiffXd> M1_L1o =
      SpatialInertia<AutoDiffXd>::MakeFromCentralInertia(m1_ad, p_L1L1cm,
                                                         G1_Bcm * m1_ad);

  UnitInertia<AutoDiffXd> G2_Bcm =
      UnitInertia<AutoDiffXd>::StraightLine(G2_ad, Vector3d::UnitZ());
  SpatialInertia<AutoDiffXd> M2_L2o =
      SpatialInertia<AutoDiffXd>::MakeFromCentralInertia(m2_ad, p_L2L2cm,
                                                         G2_Bcm * m2_ad);

  plant_autodiff->SetBodySpatialInertia(
      context_autodiff.get(),
      plant_autodiff->GetRigidBodyByName(params.link1_name()), M1_L1o);

  plant_autodiff->SetBodySpatialInertia(
      context_autodiff.get(),
      plant_autodiff->GetRigidBodyByName(params.link2_name()), M2_L2o);

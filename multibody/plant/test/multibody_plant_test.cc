...
TEST_F(AcrobotPlantTests, EvalStateAndAccelerationOutputPorts) {
  ...
  // results.
  const ModelInstanceIndex default_instance(1);
  const auto& accel_default_instance =
      plant_->get_generalized_acceleration_output_port(default_instance)
          .Eval<BasicVector<double>>(context);
  EXPECT_EQ(accel_default_instance.size(), 0);

  const ModelInstanceIndex world_instance(0);
  const auto& state_world_instance =
      plant_->get_state_output_port(world_instance)
          .Eval<BasicVector<double>>(context);

...
GTEST_TEST(MultibodyPlantTest, ReversedWeldJoint) {
  ...
  // reverse case. See schematic_for_reverse_weld.png in this test directory for
  // the 2D schematic for this test. The 2D schematic has (from left-to-right)
  // parent body frame P, joint frame Jp, joint frame Jc, child body frame C.
  // Each of the frame origins are 1 meter apart along a horizontal line that is
  // parallel to the unit vector Px (fixed in frame P). Hence, the position
  // vectors locating the frame origins are p_PJp = Px, p_JpJc = Px, p_JcC = Px.
  // The orientation of frame Jp in frame P is characterized by a right-hand
  // rotation of π/4 about Py (fixed in frame P). Similarly frame Jc in Jp is
  // π/4 about Py, and frame C in Jc is π/2 about Py. Hence the "y" basis vector
  // of each frame is in the same direction (Py = Jpy = Jcy = Cy). Note: Frames
  // translate in the Px direction whereas frames rotate about Py to simplify
  // by-hand calculations, but not to so simple (e.g., with both translation and
  // rotation in the Px direction) so as to hide frame problems. The 2D
  // schematic shows the unit vector Pz (fixed in frame P) as vertically upward,
  // which is opposite gravity when P=World (typical case). However, Pz is in
  // the gravity direction when P=child (reverse case).
  ...
  torque_expected = Vector3d(0, -g, 0);  force_expected = Vector3d(-g, 0, 0);
  EXPECT_TRUE(CompareMatrices(
  ...
  torque_expected = Vector3d(0, 2 * g, 0);  force_expected = Vector3d(-g, 0, 0);
  EXPECT_TRUE(CompareMatrices(

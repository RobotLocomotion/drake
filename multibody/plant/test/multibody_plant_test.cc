...
// Reaction forces are the trickiest part since we are required to report them
// in the _joint's_ child frame (in this case a frame on World) but internally
// they are calculated on the outboard body of the mobilizer.
GTEST_TEST(MultibodyPlantTest, ReversedWeldJoint) {
  ...
  // re-expressed and shifted for the reverse case. We're only rotating the
  // frames about x to make hand calculation easier, but be sure that the
  // shift is not also along x -- that could hide a frame problem since the
  // shift vector would be the same after rotation about x.
  ...
  EXPECT_TRUE(reaction_forces[forward_weld.ordinal()].IsApprox(
      SpatialForce<double>(Vector3d(g, 0, 0), Vector3d(0, g, 0)), 1e-14));
  ...
  EXPECT_TRUE(reaction_forces[reverse_weld.ordinal()].IsApprox(
      SpatialForce<double>(Vector3d(-2 * g, 0, 0), Vector3d(0, g, 0)), 1e-14));
}

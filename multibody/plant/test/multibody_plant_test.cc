...
GTEST_TEST(MultibodyPlantTest, DefaultFreeBodyPosePostFinalize) {
  MultibodyPlant<double> plant(0.0);

  const auto& free_body =
      plant.AddRigidBody("free body", SpatialInertia<double>::MakeUnitary());
  ...
  // It's always ok to call Set/GetDefaultFreeBodyPose() before Finalize.
  const RigidTransformd X_WB(RollPitchYawd(1.0, 2.0, 3.0),
                             Vector3d(11, 22, 33));
  EXPECT_NO_THROW(plant.SetDefaultFreeBodyPose(free_body, X_WB));
  EXPECT_NO_THROW(plant.SetDefaultFreeBodyPose(welded_body, X_WB));
  EXPECT_TRUE(
      CompareMatrices(plant.GetDefaultFreeBodyPose(free_body).GetAsMatrix4(),
                      X_WB.GetAsMatrix4()));
  EXPECT_TRUE(
  ...
  plant.Finalize();
  // However, only floating bodies are allowed for Set/GetDefaultFreeBodyPose()
  // post-finalize.
  const RigidTransformd new_X_WB(RollPitchYawd(1.1, 2.2, 3.3),
                                 Vector3d(1, 2, 3));
  EXPECT_NO_THROW(plant.SetDefaultFreeBodyPose(free_body, new_X_WB));
  const double kTolerance = 4.0 * std::numeric_limits<double>::epsilon();
  EXPECT_TRUE(
      CompareMatrices(plant.GetDefaultFreeBodyPose(free_body).GetAsMatrix4(),
                      new_X_WB.GetAsMatrix4(), kTolerance));
  EXPECT_THROW(plant.SetDefaultFreeBodyPose(welded_body, new_X_WB),
               std::exception);
  EXPECT_THROW(plant.GetDefaultFreeBodyPose(welded_body), std::exception);
}
...
GTEST_TEST(SetRandomTest, SetDefaultWhenNoDistributionSpecified) {
  ...
  const RigidTransformd default_free_body_pose(RollPitchYawd(0.3, 0.4, 0.5),
                                               Vector3d(1.0, 2.0, 3.0));
  plant.SetDefaultFreeBodyPose(body, default_free_body_pose);

...
GTEST_TEST(MultibodyPlantTest, AutoDiffAcrobotParameters) {
  ...
  // variables to differentiate. We must update multibody components that
  // depend on these parameters so that their derivatives will propagate.
  // For this example the multibody elements that require parameter updates
  // are the bodies' mass and inertia, and the joints' offset frames.
  ...
  // Differentiable parameters for the acrobots mass/inertia parameters.
...
GTEST_TEST(MultibodyPlantTests, FixedOffsetFrameParameters) {
  ...
  // Create a default context.override
  auto context = plant.CreateDefaultContext();

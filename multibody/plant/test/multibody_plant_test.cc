...
GTEST_TEST(KukaModel, ActuationMatrix) {
  ...
  const Eigen::SparseMatrix<double> B_inv = plant.MakeActuationMatrixInverse();
...
GTEST_TEST(StateSelection, KukaWithSimpleGripper) {
  ...
  ASSERT_EQ(B.cols(), nu);
  // B is not invertible.
  DRAKE_EXPECT_THROWS_MESSAGE(plant.MakeActuationMatrixInverse(),
                              ".*num_actuated_dofs.*num_velocities.*");

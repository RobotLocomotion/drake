...
GTEST_TEST(MultibodyPlantTests, ConstraintActiveStatus) {
  ...
  EXPECT_TRUE(plant.get_constraint_active_status(*context, coupler_id));
  EXPECT_TRUE(plant.get_constraint_active_status(*context, distance_id));
  EXPECT_TRUE(plant.get_constraint_active_status(*context, ball_id));

  // Set all constraints to inactive.
  plant.set_constraint_active_status(context.get(), coupler_id, false);
  plant.set_constraint_active_status(context.get(), distance_id, false);
  plant.set_constraint_active_status(context.get(), ball_id, false);

  // Verify all constraints are inactive in the context.
  EXPECT_FALSE(plant.get_constraint_active_status(*context, coupler_id));
  EXPECT_FALSE(plant.get_constraint_active_status(*context, distance_id));
  EXPECT_FALSE(plant.get_constraint_active_status(*context, ball_id));

  // Set all constraints to back to active.
  plant.set_constraint_active_status(context.get(), coupler_id, true);
  plant.set_constraint_active_status(context.get(), distance_id, true);
  plant.set_constraint_active_status(context.get(), ball_id, true);

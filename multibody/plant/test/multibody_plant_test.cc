...
class AcrobotPlantTests : public ::testing::Test {
 public:
  ...
  void VerifyCalcGravityGeneralizedForces(double theta1, double theta2) {
    ...
    // N.B. For this particular model we know `forces` includes generalized
    // forces due to dissipation at the joints and spatial forces due to
    // gravity. In general this is not true and we should use ForceElement's API
    // to retrieve the specific component we are interested in.

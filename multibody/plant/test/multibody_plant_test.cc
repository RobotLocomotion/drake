...
GTEST_TEST(SetRandomTest, FloatingBodies) {
  // Create a model that simply contains a box.
  MultibodyPlant<double> plant;
  const Body<double>& box = plant.AddRigidBody("Box", SpatialInertia<double>());
  plant.Finalize();
  ...
  plant.SetFreeBodyRandomPositionDistribution(box, xyz);
  plant.SetFreeBodyRandomRotationDistributionToUniform(box);

  auto context = plant.CreateDefaultContext();

  const math::RigidTransform<double> X_WB_default =
      plant.GetFreeBodyPose(*context, box);

...
// joints to be welds instead of hinges. This will reduce the number of
// collisions, thanks to automatic filtering of welded subgraphs.
...
// Ensure world-welded bodies are collision filtered; see also #1116.
...
GTEST_TEST(MultibodyPlantTest, FilterWeldedSubgraphs) {
  ...
  const std::set<std::pair<GeometryId, GeometryId>>& expected_pairs =
      scenario.unfiltered_collisions();
  constexpr int filtered_in_first_subgraph = 3;  // 3 choose 2.
  constexpr int filtered_in_second_subgraph = 6;  // 4 choose 2.

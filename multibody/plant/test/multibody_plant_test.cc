...
GTEST_TEST(MultibodyPlant, EmptyWorldElements) {
  ...
  EXPECT_EQ(plant.num_model_instances(), 2);
  EXPECT_EQ(
      plant.GetModelInstanceName(world_model_instance()),
      "WorldModelInstance");
  EXPECT_EQ(
      plant.GetModelInstanceName(default_model_instance()),
      "DefaultModelInstance");
  // Bodies.
  EXPECT_EQ(plant.num_bodies(), 1);
  const BodyIndex world_body_index{0};
  const Body<double>& world_body = plant.get_body(world_body_index);
  EXPECT_EQ(world_body.name(), "WorldBody");
  EXPECT_EQ(world_body.model_instance(), world_model_instance());
  // Frames.
  EXPECT_EQ(plant.num_frames(), 1);
  const FrameIndex world_frame_index{0};
  const Frame<double>& world_frame = plant.get_frame(world_frame_index);
  EXPECT_EQ(&world_body.body_frame(), &world_frame);
  EXPECT_EQ(world_frame.name(), "WorldBody");
  // Remaining elements.

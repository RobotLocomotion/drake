...
GTEST_TEST(DistanceConstraintsTests, VerifyIdMapping) {
  ...
  const DistanceConstraintSpecs& distance_specs =
      plant.get_ball_constraint_specs(distance_id);
  EXPECT_EQ(distance_specs.id, distance_id);
  EXPECT_EQ(distance_specs.body_A, bodyA.index());
  EXPECT_EQ(distance_specs.body_B, bodyB.index());
  EXPECT_EQ(distance_specs.p_AP, p_AP);
  EXPECT_EQ(distance_specs.p_BQ, p_BQ);
  EXPECT_EQ(distance_specs.distance, distance);

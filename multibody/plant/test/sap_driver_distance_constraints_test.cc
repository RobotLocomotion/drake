...
TEST_F(TwoBodiesTest, SingleClique) {
  ...
  // Translational velocity aligned with p_PQ.
  const double gdot_expected = 1.2;
  const SpatialVelocity<double> V_WB(Vector3d::Zero(),
                                     Vector3d::UnitX() * gdot_expected);
  plant_.SetFreeBodySpatialVelocity(context_.get(), *bodyB_, V_WB);
  const VectorXd& v = plant_.GetVelocities(*context_);
  const VectorXd gdot = J * v;
  EXPECT_TRUE(CompareMatrices(gdot, Vector1d(gdot_expected)));
}

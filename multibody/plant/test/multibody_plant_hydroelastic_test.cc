 ...
 protected:
  ...
  std::vector<SpatialForce<double>> SpatialForceFromContactResults(
    ...
    for (int i = 0; i < contacts.num_hydroelastic_contacts(); ++i) {
      ...
      // The force applied to body A at the centroid point C.
      const SpatialForce<double>& F_C_W = contact_info.F_Ac_W();
      F_BBo_W_array[body_A.index()] += F_C_W.Shift(p_CAo_W);
      F_BBo_W_array[body_B.index()] -= F_C_W.Shift(p_CBo_W);
    }

...
class WeldedBoxesTest : public ::testing::Test {
 protected:
  ...
  void VerifyBodyReactionForces() {
    ...
    // Given the problem setup, this will be the force on A applied at it's
    // center of mass Acm, expressed in the world frame.
    ...
    EXPECT_TRUE(
        CompareMatrices(F_Acm_W.translational(), f_Acm_W_expected, kTolerance));
    // Box B hangs from box A, and therefore the reaciton on weld1 must balance
    // the torque due to gravity on box B applied on box A.
    const Vector3d t_Acm_W_expected =
        -kCubeSize * box_weight * Vector3d::UnitY();
    EXPECT_TRUE(
        CompareMatrices(F_Acm_W.rotational(), t_Acm_W_expected, kTolerance));

    // Given the problem setup, this will be the force on B applied at it's
    // center of mass Bcm, expressed in the world frame.
    ...
    EXPECT_TRUE(
        CompareMatrices(F_Bcm_W.translational(), f_Bcm_W_expected, kTolerance));
    EXPECT_TRUE(
        CompareMatrices(F_Bcm_W.rotational(), t_Bcm_W_expected, kTolerance));
  }

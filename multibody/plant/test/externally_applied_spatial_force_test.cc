...
TEST_F(ExternallyAppliedForcesTest, CalcGeneralizedForcesDueToMultibodyForces) {
  ...
  const Context<double>& compensator_context =
      acrobot_gravity_compensator_->GetMyContextFromRoot(*context_);
  ...
  // N.B. CalcGeneralizedForces() resizes `generalized_forces` appropriately.
  VectorX<double> generalized_forces;

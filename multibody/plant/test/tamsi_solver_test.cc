...
class PizzaSaver : public ::testing::Test {
 public:
  ...
  MatrixX<double> ComputeTangentialJacobian(double theta) {
    ...
    // Point A
    Jt.block(0, 0, 2, nv_) << 1, 0, -p_BoA_W.y(), 0, 1, p_BoA_W.x();

    // Point B
    Jt.block(2, 0, 2, nv_) << 1, 0, -p_BoB_W.y(), 0, 1, p_BoB_W.x();

    // Point C
    Jt.block(4, 0, 2, nv_) << 1, 0, -p_BoC_W.y(), 0, 1, p_BoC_W.x();

  ...
  */
  ...
  for (int i = 0; i < num_vertices; ++i) {
    ...
    if (n_W_.dot(reference_position - p_WQ_) <= 0.0) {
      /* The z-coordinate of the vertex in the world frame should stay
       unmoved if the the reference position is above z=0 and thus subject to
       the boundary condition. */
      EXPECT_EQ(current_position(2), reference_position(2));
    } else {
      /* Otherwise, it would fall to be smaller than the reference position z
       coordinate under gravity but still stay close to the boundary due to
       internal forces. */
      EXPECT_LT(current_position(2), reference_position(2));
      EXPECT_GT(current_position(2), -1.5 * kRadius);
    }
  }
}

...
class DeformableIntegrationTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    deformable_model->SetWallBoundaryCondition(body_id_, Vector3d(0, 0, 0),
                                               Vector3d(0, 0, -1));
    ...
    const RigidTransformd X_WG(Vector3d(0, 0, 0.2));
    const Box box(0.5, 0.5, 0.4);
...
TEST_F(DeformableIntegrationTest, SteadyState) {
  ...
  /* Verify that position of the ball is still close to wall. */
  const VectorXd& q = fem_state.GetPositions();
  for (int i = 2; i < q.size(); i += 3) {
    /* Get the z-coordinate of the vertex in the world frame, it should be close
     to 0, where the wall bc is. We set the bound to be 150% of the diameter. */
    EXPECT_LT(q(i), 1.5 * kRadius);
    EXPECT_GT(q(i), -1.5 * kRadius);
  }
}

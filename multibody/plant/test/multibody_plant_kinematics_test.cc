...
class TwoDOFPlanarPendulumTest : public ::testing::Test {
 public:
  ...
  Vector2<double> CalcVdotForNoAppliedForces() {
    ...
    const double wADt = 0.5*z3*(z2*wAzsq+z4*wBz_*(wBz_+2*wAz_))/(z4*z1-z2sq);
    const double wBDt = -0.5*z3*(z1*wAzsq+z2*wBz_*(wBz_+2*wAz_))/(z4*z1-z2sq);
...
TEST_F(TwoDOFPlanarPendulumTest, CalcCenterOfMassAcceleration) {
  ...
  EXPECT_TRUE(CompareMatrices(a_WAcm_W,
                              A_WAcm_W_expected.translational(), kTolerance));
  ...
  EXPECT_TRUE(CompareMatrices(a_WBcm_W,
                              A_WBcm_W_expected.translational(), kTolerance));
}

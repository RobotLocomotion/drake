...
TEST_F(MultibodyPlantRemodeling, AddPdControllerConstraints) {
  ...
  const std::vector<int> dof_to_actuator_index{0, -1, 1};
  ...
  for (int i = 0; i < problem_cache.num_pd_controller_constraints; ++i) {
    ...
    const int actuator_index = dof_to_actuator_index[configuration.clique_dof];
    EXPECT_EQ(configuration.qd, x_desired(actuator_index));
    EXPECT_EQ(configuration.vd,
              x_desired(actuator_index + plant_->num_actuated_dofs()));
    EXPECT_EQ(configuration.u0, u_feed_forward(actuator_index));
  }
}

...
class ActuatedModelsTest : public ::testing::TestWithParam<TestParam> {
 public:
  ...
  struct Descriptor {
    ...
    JointIndex joint;
    bool is_actuated{};
    JointActuatorIndex joint_actuator;
    bool has_controller{};
    bool is_locked{};
    std::optional<double> actuation_input{};
    std::optional<Vector2d> desired_state_input{};
  };
...
TEST_P(ActuatedModelsTest, PortAndDofSizes) {
  ...
  // PD gains are only supported for a discrete-time plant.
  if (param_.time_step > 0.0) {
    SetControllerGains(gripper_model_);
  }
...
TEST_P(ActuatedModelsTest,
  ...
  if (!(param_.time_step == 0.0)) {
    // Only run this test on continuous-time plants.
    return;
  }

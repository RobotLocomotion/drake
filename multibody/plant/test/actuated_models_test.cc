...
TEST_F(ActuatedIiwaArmTest, GetDesiredStatePort) {
  ...
  const int num_gripper_pds = plant_->NumPdControlledActuators(gripper_model_);
  const int num_arm_pds = plant_->NumPdControlledActuators(arm_model_);
  EXPECT_EQ(num_gripper_pds, 2);
  EXPECT_EQ(num_arm_pds, 2);

  // All other models have no PD control.
  EXPECT_EQ(plant_->NumPdControlledActuators(box_model_), 0);
  EXPECT_EQ(plant_->NumPdControlledActuators(acrobot_model_), 0);

  // Partially controlled arm with only two actuators with PD control. Therefore
  // its state is of size 4 (= 2 positions + 2 velocities).
  EXPECT_EQ(plant_->get_desired_state_input_port(arm_model_).size(),
            2 * num_gripper_pds);

  // Gripper had PD control in all of its actuators. Therefore its state is of
  // size 4 (= 2 positions + 2 velocities).
  ...
  EXPECT_EQ(plant_->get_desired_state_input_port(acrobot_model_).size(), 0);
  EXPECT_EQ(plant_->get_desired_state_input_port(box_model_).size(), 0);
...
TEST_F(ActuatedIiwaArmTest,
  ...
  // Only two arm actuators have PD control.
  const int num_arm_pds = plant_->NumPdControlledActuators(arm_model_);
  EXPECT_EQ(num_arm_pds, 2);
  const VectorXd arm_xd = (VectorXd(4) << 1.0, 2.0, 3.0, 4.0).finished();
  plant_->get_desired_state_input_port(arm_model_)
      .FixValue(context_.get(), arm_xd);

  // Gripper is fully PD-controlled.
  ...
  EXPECT_EQ(input.positions(arm_model_), arm_xd.head(2));
  EXPECT_EQ(input.velocities(arm_model_), arm_xd.tail(2));
}

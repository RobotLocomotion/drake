...
/* Test fixure to test DeformableDriver::AppendContactKinematics.
 ...
 world to the contact frame and contact velocities to expected values. Recall
 that the basis for the contact frame C is defined such that Cz is aligned with
 the contact normal and Cx and Cy are arbitrarily defined to form a right-handed
 basis. */
class DeformableDriverContactKinematicsTest : public ::testing::Test {
 protected:
  ...
  /* Sets up the scene described in the class documentation. The rigid body is
   ...
   of the bodies are set so that the contact normals are all parallel to the
   z-axis of an arbitrarily chosen frame, F. Velocities of the body are set so
   that the contact velocity in the C frame is (0, 0, -1). */
  void MakeScene(bool dynamic_rigid_body) {
    ...
    /* Set up the deformable body velocity and the rigid body velocity so that
     the relative velocity at the contact points are (0, 0, 1) in the F frame.
     Because we set up the geometries such that Cz = -Fz at all contact points,
     we can expect the contact velocities in the C frames are (0, 0, -1). */
    if (dynamic_rigid_body) {
  ...
  void ValidateContactKinematics(bool dynamic_rigid_body) {
    ...
    /* The expected contact velocity in the C frame at each contact point. */

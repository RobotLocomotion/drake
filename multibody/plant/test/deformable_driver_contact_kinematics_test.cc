...
class DeformableDriverContactKinematicsTest : public ::testing::Test {
 protected:
  ...
  /* Sets up the scene described in the class documentation. The rigid body is
   ...
   of the bodies are set so that the contact normal in the world frame is
   (0, 0, 1). Velocities of the body are set so that the contact velocity in the
   contact frame is (0, 0, 1). */
  void MakeScene(bool dynamic_rigid_body) {
    ...
    const RigidTransformd X_WB(Vector3<double>(0, 0, -0.75));
  ...
  void ValidateContactKinematics(bool dynamic_rigid_body) {
    ...
    /* The contact surfaces are parallel to the xy-plane and the contact normal
     points from the deformable body into the rigid body. */
    const Vector3d nhat_W(0, 0, -1);
 ...
 private:
  ...
  void SetVelocity(DeformableBodyId id, const Vector3d& v_WB) {
    ...
    for (int i = 0; i < num_dofs; i += 3) {
      velocities.segment<3>(i) = v_WB;
    }

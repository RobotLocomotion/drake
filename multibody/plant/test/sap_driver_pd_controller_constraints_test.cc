...
class ActuatedIiiwaArmTest : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    // N.B. Unfortunately plant_->GetJointActuatorIndices(arm_model_) must be
    // called post-finalize, set_controller_gains() must-be called pre-finalize.
    // This is a workaround until this is resolved.
 ...
 protected:
  ...
  // Effort values defined in iiwa14_no_collision.sdf
  const double kArmEffortLimit1_{320.0};
  const double kArmEffortLimit3_{176.0};
...
TEST_F(ActuatedIiiwaArmTest, VerifyConstraints) {
  ...
  // Plant desired positions, arm (7) + gripper (2).
  // We only use entries for joints with PD control.
  const VectorXd plant_qd = VectorXd::LinSpaced(9, 1.0, 4.0);

  // Plant desired velocities, arm (2) + gripper (2).
  const VectorXd plant_vd = VectorXd::LinSpaced(9, -1.0, 2.0);
  ...
  const int arm_num_controllers = plant_->NumPdControlledActuators(arm_model_);
  const VectorXd arm_xd = (VectorXd(2 * arm_num_controllers) << plant_qd(1),
                           plant_qd(3), plant_vd(1), plant_vd(3))
                              .finished();
    ...
    if (constraint != nullptr) {
      ...
      if (c.clique_dof >= 7) {  // Gripper
        ...
        const double expected_limit =
            c.clique_dof == 1 ? kArmEffortLimit1_ : kArmEffortLimit3_;

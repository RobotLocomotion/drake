...
TEST_F(ActuatedIiiwaArmTest, VerifyConstraints) {
  ...
  // N.B. We make vectors of desired positions and velocities for all joints of
  // the arm, even though only a subset of the arm's actuators have PD control.
  // This simplifies the indexing of dofs below when checking for constraints.
  // Values corresponding to joints with no PD control not used.
  ...
  // Desired state for the arm. Notice states are not consecutive, though
  // provided in increasing order of JointActuatorIndex, as required.

...
modeled with explicit weld joints or fused into a single composite
mobilized body. The test builds two identical models that differ only in whether
SetFuseWeldedBodies() is enabled. */
...
// Holds one version of the test model (either explicit welds or with welded
// links fused onto a single mobilized body) along with its context, ready
// for kinematics queries.
struct TestModel {
...
TestModel MakeModel(bool fuse_welded_bodies) {
  ...
  m.plant->SetFuseWeldedBodies(fuse_welded_bodies);
...
/* Ensure the fused mobilized body's composite spatial inertia for Link123
...
- Pass 2: poses X_BL, where mobod (mobilized body) frame B and link frame L
  ...
  fused body's "active" link, frame B is frame L and X_BL is identity. In
...
fusing enabled and one without -- then verify that the mass matrix and
...
GTEST_TEST(FusedTest, CalcFrameBodyPosesAllPaths) {
  ...
  auto make_model = [&](bool fuse) {
    ...
    plant->SetFuseWeldedBodies(fuse);
  ...
  ASSERT_EQ(tree_f.num_mobods(),
            2);  // World + fused body (links A, B, C).

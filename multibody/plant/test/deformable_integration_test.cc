...
const CoulombFriction<double> kFriction{0.4, 0.4};  // tan(π/12) ≈ 0.27 < 0.4.

/* Sets up a deformable simulation with a deformable octrahedron centered at the
 origin of the world frame and a rigid slope weld to the world body. The setup
...
TEST_F(DeformableIntegrationTest, SteadyState) {
  ...
  EXPECT_TRUE(
      CompareMatrices(a, VectorXd::Zero(a.size()), kVelocityThreshold / kDt));
  ...
  const VectorXd& tau_contact = contact_solver_results.tau_contact;
  Vector3d total_contact_force = Vector3d::Zero();
  for (int i = 0; i < tau_contact.size(); i += 3) {
    total_contact_force += tau_contact.segment<3>(i);
  }
  ...
  EXPECT_TRUE(CompareMatrices(expected_contact_force, total_contact_force,
                              kForceThreshold));
}

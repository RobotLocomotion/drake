...
class DeformableDriverContactTest : public ::testing::Test {
 protected:
  ...
  void AppendDiscreteContactPairs(
      const systems::Context<double>& context,
      std::vector<DiscreteContactPair<double>>* result) const {
    driver_->AppendDiscreteContactPairs(context, result);
  }
...
TEST_F(DeformableDriverContactTest, AppendDiscreteContactPairs) {
  ...
  AppendDiscreteContactPairs(plant_context, &contact_pairs);
  ...
  /* tau for deformable body is set to 0.1 and is unset for rigid body (which
   then assumes the default value, dt). */

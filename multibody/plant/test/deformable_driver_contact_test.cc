...
TEST_F(DeformableDriverContactTest, EvalFreeMotionTangentMatrix) {
  ...
  std::unique_ptr<PetscSymmetricBlockSparseMatrix> calculated_tangent_matrix =
      EvalFreeMotionTangentMatrix(plant_context, body_index).Clone();
  /* Make the matrix mutable so that we can assemble the Petsc matrix. */
  calculated_tangent_matrix->AssembleIfNecessary();
...
TEST_F(DeformableDriverContactTest,
  ...
  const Context<double>& plant_context =
      plant_->GetMyContextFromRoot(*context_);
  std::unique_ptr<PetscSymmetricBlockSparseMatrix> tangent_matrix =
      EvalFreeMotionTangentMatrix(plant_context, body_index).Clone();
  tangent_matrix->
      AssembleIfNecessary();

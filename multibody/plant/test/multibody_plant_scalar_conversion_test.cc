...
TEST_P(DiscretePlantTestExpression, ForcedUpdate) {
  ...
  // We do not support discrete updates when T = Expression. Depending on the
  // plant configuration, the throw will happen from various different places
  // but one way or another it will throw.
  DRAKE_EXPECT_THROWS_MESSAGE(diagram.CalcForcedDiscreteVariableUpdate(
                                  model_->context(), updates.get()),
                              ".*not support.* T = [^ ]*Expression");
}

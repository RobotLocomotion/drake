...
class MultibodyPlantTester {
 public:
  template <typename T>
  static const std::vector<internal::CouplerConstraintSpecs<T>>& get_specs(
...
// Verify that constraint specs survive scalar conversions.
GTEST_TEST(ScalarConversionTest, CouplerConstraintSpecs) {
  ...
  ASSERT_EQ(plant_double_to_autodiff->num_constraints(),
            plant_double.num_constraints());
  const internal::CouplerConstraintSpecs<AutoDiffXd>& spec_ad =
      MultibodyPlantTester::get_specs(*plant_double_to_autodiff)[0];
  EXPECT_EQ(spec_ad.joint0_index, reference_spec.joint0_index);
  EXPECT_EQ(spec_ad.joint1_index, reference_spec.joint1_index);
  EXPECT_EQ(ExtractDoubleOrThrow(spec_ad.gear_ratio),
            reference_spec.gear_ratio);
  EXPECT_EQ(ExtractDoubleOrThrow(spec_ad.offset), reference_spec.offset);
  // AutoDiffXd -> double.
  ...
  ASSERT_EQ(plant_autodiff_to_double->num_constraints(),
            plant_double.num_constraints());
  const internal::CouplerConstraintSpecs<double>& spec_double =
      MultibodyPlantTester::get_specs(*plant_autodiff_to_double)[0];
  EXPECT_EQ(spec_double.joint0_index, reference_spec.joint0_index);
  EXPECT_EQ(spec_double.joint1_index, reference_spec.joint1_index);
  EXPECT_EQ(spec_double.gear_ratio, reference_spec.gear_ratio);
  EXPECT_EQ(spec_double.offset, reference_spec.offset);
}

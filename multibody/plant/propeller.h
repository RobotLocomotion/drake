...
Note: Set moment_ratio to zero if you want a simple thruster which applies only
a force (no moment) in the Propeller coordinates.

Note: Due to the current lack of a better design pattern that supports scalar
conversion (#13131), propeller forces can currently only be applied to a
MultibodyPlant that registers with a SceneGraph.  The SceneGraph need not appear
in the diagram with the Propeller; registering it with a SceneGraph is
...
class Propeller final : public systems::LeafSystem<T> {
 public:
  ...
  /** Returns a reference to the geometry_poses input port.  It is anticipated
  that this port will be connected the geometry_poses output port of a
  MultibodyPlant. */

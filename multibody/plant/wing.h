...
simplified dynamics of an airfoil (or hydrofoil).
...
- It is expected that the body_poses input should be connected to the
  ...
  output port" abd that body_spatial_velocities input should be connected to
...
class Wing final : public systems::LeafSystem<T> {
 public:
  ...
  /** Constructs a system describing a single wing using flat-plate
   ...
   whose origin at the aerodynamic center of the wing, the positive x-axis
  ...
  */
  ...
  /** Returns a reference to the optional fluid_density input port, which
   accepts scalar vector in units kg/m^3. This port is provided to support for
  ...
  /** Returns a 3-element position of the aerodynamic center of the wing in
   world coordinates. This input port does not depend on the optional wind

...
void DeformableDriver<T>::AppendDiscreteContactPairs(
  ...
  // name here.
  const std::string body_A_name("deformable_body");
  for (const DeformableContactSurface<double>& surface :
    ...
    const T k = GetCombinedPointContactStiffness(
        surface.id_A(), surface.id_B(), manager_->default_contact_stiffness(),
        inspector);
    ...
    const double mu =
        GetCombinedCoulombFriction(surface.id_A(), surface.id_B(), inspector)
            .dynamic_friction();

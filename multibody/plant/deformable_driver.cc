...
void DeformableDriver<T>::AppendContactKinematics(
  ...
  for (const auto& surface : contact_surfaces) {
    ...
    for (int i = 0; i < surface.num_contact_points(); ++i) {
      ...
      /* Calculate the jacobian block for the rigid body B if it's not
       * static.
       */
...
void DeformableDriver<T>::CalcFreeMotionFemState(
  ...
  // TODO(xuchenhan-tri): We should expose an API to set the solver
  // tolerance here.
...
void DeformableDriver<T>::CalcNextFemState(const systems::Context<T>& context,
  ...
  // TODO(xuchenhan-tri): Update this implementation to include the effect
  // of contact and constraints.
...
void DeformableDriver<T>::CalcFreeMotionTangentMatrix(
  ...
  /* Multiply by dt because we need the tangent matrix of the momentum
   balance instead of the force balance. */

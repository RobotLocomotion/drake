...
void DeformableDriver<T>::AppendDiscreteContactPairs(
  ...
  for (const DeformableContactSurface<double>& surface :
    ...
    /* We use an arbitrarily large stiffness as the default dissipation constant
     so that the contact is in near-rigid regime and the compliance is only used
     as stabilization. */
    const T k = GetCombinedPointContactStiffness(surface.id_A(), surface.id_B(),
                                                 1.0e12, inspector);

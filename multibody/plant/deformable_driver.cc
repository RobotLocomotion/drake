...
void DeformableDriver<T>::AppendContactKinematics(
  ...
  /* Since v_AcBc_W = v_WBc - v_WAc the relative velocity Jacobian will be:
   ...
   Finally J_AcBc_C = R_WC.transpose() * J_AcBc_W.
   ...
   deformable bodies and rigid bodies are mutually exclusive, and Jv_WAc_W = 0
   for rigid dofs and Jv_WBc_W = 0 for deformable dofs. As a result, we know the
   size of Jv_WBc_W up front. */
    ...
    for (int i = 0; i < surface.num_contact_points(); ++i) {
      ...
      for (int v = 0; v < 4; ++v) {
        ...
        /* v_WAc = (b₀ * v₀ + b₁ * v₁ + b₂ * v₂ + b₃ * v₃) where v₀, v₁, v₂,
         ...
         barycentric weights. */
        Jv_WAc_W.template middleCols<3>(3 * participating_vertices(v)) =
            b(v) * Matrix3<T>::Identity();
      }
      jacobian_blocks.emplace_back(clique_index_A,
                                   -R_WC.transpose().matrix() * Jv_WAc_W);
      ...
      if (tree_index.is_valid()) {
        ...
        Matrix3X<T> J =
            R_WC.transpose().matrix() *
            Jv_WBc_W.middleCols(tree_topology.tree_velocities_start(tree_index),

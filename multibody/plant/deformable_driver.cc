...
void DeformableDriver<T>::DeclareCacheEntries(
  ...
  for (DeformableBodyIndex i(0); i < deformable_model_->num_bodies(); ++i) {
    const DeformableBodyId id = deformable_model_->GetBodyIdOrThrow(i);
...
void DeformableDriver<T>::CalcFemState(const systems::Context<T>& context,
  ...
  const DeformableBodyId id = deformable_model_->GetBodyIdOrThrow(index);
  ...
  const VectorX<T>& discrete_value = discrete_state.get_value();

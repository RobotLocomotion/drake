...
void DeformableDriver<T>::AppendDiscreteContactPairs(
  ...
  for (int surface_index = 0; surface_index < ssize(contact_surfaces);
    ...
    for (int i = 0; i < surface.num_contact_points(); ++i) {
      ...
      /* We set a large stiffness to approximate rigid contact. We choose 1e8
       [Pa/m] from emprirical experience.
       We then scale the stiffness by the area of the contact polygon to compute
       an effective stiffness that has unit N/m. */

...
void DeformableDriver<T>::CalcNextFemState(const systems::Context<T>& context,
  ...
  if (participation.num_vertices_in_contact() == 0) {
    ...
    const VectorX<T>& stacked_v_star =
    ...
    const Eigen::Ref<const VectorX<T>> participating_v_star =
        mux.Demultiplex(stacked_v_star, index);
    const Eigen::Ref<const VectorX<T>> participating_v_next =
        mux.Demultiplex(stacked_v_next, index);
    const VectorX<T> participating_dv =
        participating_v_next - participating_v_star;
    /* Compute the value of the post-constraint non-participating
     velocities using Schur complement. */
    const SchurComplement<T>& schur_complement =
        EvalFreeMotionTangentMatrixSchurComplement(context, index);
    const VectorX<T> nonparticipating_dv =
        schur_complement.SolveForY(participating_dv);
    ...
    VectorX<T> v_next = EvalFreeMotionFemState(context, index).GetVelocities();
    v_next += dv;
    const FemState<T>& fem_state = EvalFemState(context, index);

 ...
 private:
  ...
  struct ContactData {
    /* The world frame position of the relative-to point for reporting the
     ...
     DiscreteContactPair::p_BqC_W. `p_WG` will be used as the value for `p_WAp`
     and `p_WBq` respectively. */
    Vector3<T> p_WG;
    /* Contact Jacobians for the kinematic tree corresponding to the object
     ...
     the i-th contact point. This is empty if the geometry is welded to World.
    */

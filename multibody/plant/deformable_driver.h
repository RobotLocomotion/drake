...
class DeformableDriver : public ScalarConvertibleComponent<T> {
 public:
  ...
  /* Evaluates the velocities of all participating dofs. The dofs are ordered
   in the increasing order of deformable body indexes. That is, the
 ...
 private:
  ...
  /* This class acts both as a multiplexer and a demultiplexer -- it combines
   multiple eigen vectors into a single stacked vector and it also splits a
   eigen vector into multiple vectors. */
  class Multiplexer {
   public:
    ...
    int num_channels() const { return sizes_.size(); }
    ...
    /* Splits the given vector into multiple vectors and return the one with the
     given `index`.
     @throws std::exception if the size of `input` is not the sum of sizes
     provided at construction.
     @throws std::exception if index is not in [0, num_channels). */

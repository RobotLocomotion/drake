...
MultibodyConstraintId DeformableModel<T>::AddFixedConstraint(
  ...
  auto instance =
      std::make_unique<GeometryInstance>(X_BG, shape.Clone(), "rigid shape");
  ...
  DRAKE_DEMAND(mesh_A != nullptr);
  /* Pose the mesh in B's body frame. */
  geometry::VolumeMesh<T> mesh_B = *mesh_A;
  mesh_B.TransformVertices(X_BA);
  int vertex_index = 0;
  for (const Vector3<T>& p_BPi : mesh_B.vertices()) {
    ...
    const std::vector<geometry::SignedDistanceToPoint<T>> signed_distances =
        query.ComputeSignedDistanceToPoint(p_BPi);

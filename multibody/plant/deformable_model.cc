...
void DeformableModel<T>::SetWallBoundaryCondition(DeformableBodyId id,
  ...
  const int kDim = 3;
  auto is_inside_wall = [&p_WQ, &n_hatW](const Vector3<T>& p_WV) {
    T distance_to_wall = (p_WV - p_WQ).dot(n_hatW);
    return distance_to_wall <= 0.0;
  };

  const VectorX<T>& p_WVs = GetReferencePositions(id);
  fem::internal::DirichletBoundaryCondition<T> bc;
  for (int n = 0; n < num_nodes; ++n) {
    const Vector3<T>& p_WV = p_WVs.template segment<kDim>(n * kDim);

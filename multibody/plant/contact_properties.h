...
/* Returns the hydroelastic contact modulus stored in group
 geometry::internal::kHydroGroup with property geometry::internal::kElastic for
 the specified geometry.

 This method returns std::numeric_limits<double>::infinity() for geometries with
 value geometry::internal::HydroelasticType::kRigid for the property
 geometry::internal::kComplianceType.

 If not defined rigid and if the stiffness property is absent, it returns the
...
/* Returns the combined Hunt & Crossley dissipation of geometries A and B.
 With k₁ and k₂ `stiffness_A` and `stiffness_B` respectively and, d₁ and d₂ the
 Hunt & Crossley dissipation for geometries A and B respectively, this method
 computes the combined dissipation according to the rule:
   d = k₂/(k₁+k₂)⋅d₁ + k₁/(k₁+k₂)⋅d₂
 This methods requires both values of stiffness coefficients to be positive
 (zero is allowed) and it allows one, but only one value, to be infinite. That

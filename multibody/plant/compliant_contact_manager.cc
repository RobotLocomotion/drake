          ...
          std::function<void(const systems::Context<T>&,
              ...
              [this](const systems::Context<T>& context,
                ...
                if (last_time_evaluated == context.get_time()) {
                  if (value.is_cache_entry_disabled()) {
                    // When caching is disabled, this mechanism might lead to a
                    // false positive if this entry is evaluated successively on
                    // the same context. It is uncommon for users to disable
                    // caching and most often only developers do for debugging
                    // and/or testing purposes. Therefore here we limit
                    // ourselves to throwing an exception explaining this
                    // situation.
                    const char* msg =
                        "Algebraic loop detected with caching disabled. This "
                        "detection was placed in CompliantContactManager as a "
                        "workaround to issue #12786 but it can lead to false "
                        "positives. Unless you have a good reason, please "
                        "enable caching. Otherwise contact the developers for "
                        "a solution.";
                    throw std::runtime_error(msg);
                  } else {
                    const char* msg =
                        "Algebraic loop detected. This situation is caused "
                        "when connecting the input of your MultibodyPlant to "
                        "the output of a feedback system which is an algebraic "
                        "function of a feedthrough output of the plant. Ways "
                        "to remedy this: 1. Revisit the model for your "
                        "feedback system. Consider if its output can be "
                        "written in terms of other inputs. 2. Break the "
                        "algebraic loop by adding state to the controller, "
                        "typically to 'remember' a previous input. 3. Break "
                        "the algebraic loop by adding a zero-order hold system "
                        "between the output of the plant and your feedback "
                        "system. This  effectively delays the input signal to "
                        "the controller.";
                    throw std::runtime_error(msg);
                  }
                }

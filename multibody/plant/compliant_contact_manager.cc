...
bool CompliantContactManager<T>::is_cloneable_to_symbolic() const {
  // We require that SetOwningMultibodyPlant() was already called, and therefore
  // model info along with specific solver drivers is available.
  DRAKE_DEMAND(sap_driver_ != nullptr || dense_solver_driver_ != nullptr);
  // SAP does not support scalar conversion to symbolic.
  if (sap_driver_ != nullptr) return false;
  return true;
}
...
void CompliantContactManager<T>::DeclareCacheEntries() {
  ...
  if (dense_solver_driver_ != nullptr)
    dense_solver_driver_->DeclareCacheEntries(this);
}
...
void CompliantContactManager<T>::DoCalcContactSolverResults(
  ...
  if (plant().get_discrete_contact_solver() == DiscreteContactSolver::kTamsi)
    DRAKE_DEMAND(dense_solver_driver_ != nullptr);
  if constexpr (!std::is_same_v<T, symbolic::Expression>) {
    if (sap_driver_ != nullptr)
      sap_driver_->CalcContactSolverResults(context, contact_results);
  }
  if (dense_solver_driver_ != nullptr)
    dense_solver_driver_->CalcContactSolverResults(context, contact_results);
}
...
CompliantContactManager<T>::CloneToSymbolic() const {
  if (plant().get_discrete_contact_solver() == DiscreteContactSolver::kSap) {
    // Since SAP does not support symbolic::Expression, cloning is not possible.
    throw std::logic_error(
...
void CompliantContactManager<T>::ExtractModelInfo() {
  ...
  switch (plant().get_discrete_contact_solver()) {
    ...
    case DiscreteContactSolver::kTamsi:
      dense_solver_driver_ = std::make_unique<DenseSolverDriver<T>>(this);
      break;
  }

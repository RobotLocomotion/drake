...
CompliantContactManager<T>::CompliantContactManager() {
  ...
  sap_parameters_.ls_alpha_max = 1.0 / sap_parameters_.ls_rho;
}
...
void CompliantContactManager<T>::CalcAccelerationsDueToNonContactForcesCache(
  ...
  //   M⋅v̇ = k(x)
  // where k(x) includes continues forces of the state x not from constraints
  ...
  // is the positive diagonal matrix for damping. Using this split, we
  ...
  //   M⋅(v-v₀)/dt = k₁(x₀) - D⋅v
  // We can rewrite this by adding and substracting -D⋅v₀ on the right hand
  // side:
  //   M⋅(v-v₀)/dt = k₁(x₀) - D⋅(v-v₀) - D⋅v₀
  // which can be rearranged as:
  //   (M + dt⋅D)⋅(v-v₀)/dt = k₁(x₀) - D⋅(v-v₀) - D⋅v₀ = k(x₀)

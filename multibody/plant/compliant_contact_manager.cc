...
void CompliantContactManager<T>::AppendDiscreteContactPairsForPointContact(
  ...
  for (const PenetrationAsPointPair<T>& pair : point_pairs) {
    ...
    const T tau = GetCombinedDissipationTimeConstant(
        pair.id_A, pair.id_B, 0.1, body_A.name(), body_B.name(), inspector);
    const T mu = GetCombinedCoulombFriction(pair.id_A, pair.id_B, inspector)
                     .dynamic_friction();
...
void CompliantContactManager<T>::
  ...
  for (const auto& s : surfaces) {
    ...
    const T tau = GetCombinedDissipationTimeConstant(
        s.id_M(), s.id_N(), 0.1, body_M.name(), body_N.name(), inspector);

    // Combine friction coefficients.
    const T mu = GetCombinedCoulombFriction(s.id_M(), s.id_N(), inspector)
                     .dynamic_friction();

...
template <typename T>
using ModelPointerVariant = std::variant<const DeformableModel<T>*>;
...
class PhysicalModel : public ScalarConvertibleComponent<T> {
 public:
  ...
  ModelPointerVariant<T> ToModelPointerVariant() const {
    return DoToModelPointerVariant();
  }

 protected:
  /* Derived classes must override this function to return their specific model
   variant. */
  virtual ModelPointerVariant<T> DoToModelPointerVariant() const = 0;

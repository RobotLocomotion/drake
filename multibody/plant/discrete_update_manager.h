...
class DiscreteUpdateManager : public ScalarConvertibleComponent<T> {
 public:
  ...
  /* Evaluate the actuation applied through actuators during the discrete
   ...
   The output vector is indexed by JointActuatorIndex. */
 ...
 protected:
  ...
  /* Concrete managers must implement this method to compute the total actuation
   ...
   The default implementation sets actuation = AssembleActuationInput(context).
   For instance, managers that implement implicit controllers will want to
   overwrite this method to include these terms. */

...
HydroelasticTractionCalculator<T>::CalcTractionAtQHelper(
  ...
  if (data.speed_axis_A.has_value()) {
    const Vector3<T> n_A = data.X_WA.rotation().inverse() * (-nhat_W);
    v_BqAq_W +=
        data.X_WA.rotation() * data.speed_axis_A->template cast<T>().cross(n_A);
  }
  if (data.speed_axis_B.has_value()) {
    const Vector3<T> n_B = data.X_WB.rotation().inverse() * nhat_W;
    v_BqAq_W -=
        data.X_WB.rotation() * data.speed_axis_B->template cast<T>().cross(n_B);
  }

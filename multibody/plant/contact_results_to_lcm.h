...
/** A System that encodes ContactResults into a lcmt_contact_results_for_viz
 ...
 Generally, you shouldn't construct a %ContactResultsToLcmSystem instace
 ...
 These triples will help distinguish contacts which might otherwise be ambiguous
 ...
 geometries, those default names may not be helpful. Instead,
 %ContactResultsToLcmSystem can use the names associated with the id in a
 geometry::SceneGraph instance. The *preferred* method for doing this is
...
class ContactResultsToLcmSystem final : public systems::LeafSystem<T> {
 public:
  ...
  // /** (Advanced) Constructs an instance with SceneGraph-defined geometry names.
  //  Please use one of the @ref contact_result_vis_creation
  //  "ConnectContactResultsToDrakeVisualizer()" methods instead. */
  // ContactResultsToLcmSystem(const MultibodyPlant<double>& plant,
...
/** @name Visualizing contact results
 ...
 @pre `multibody_plant` is contained within the supplied `builder`.

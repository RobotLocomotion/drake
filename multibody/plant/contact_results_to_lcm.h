...
// Stores the "full" name of a body *for contact*: its model instance name,
// body name, and geometry name. This assumes that `model_name` is guaranteed
// to be unique within MBP. So, for two bodies which may be identically named
// (body name), they *must* differ by model name. For a body that has multiple
// collision geometries, we rely on the fact that every collision geometry
// for a single frame must be uniquely named.
struct FullBodyName {
  std::string model;
  std::string body;
  std::string geometry;
...
class ContactResultsToLcmSystem final : public systems::LeafSystem<T> {
 public:
  ...
  /** Constructor.
   ...
   otherwise be ambiguous (i.e., contact with two bodes, both called "box" but
   part of different model instances).

   The name of each collision geometry is determined by the
   `geometry_name_lookup` parameter. If omitted, each geometry will have a named
   ...
   @throws std::exception if a GeometryId found in `plant` is not recognized by
                          the provided `geometry_namer` and it chooses to throw.

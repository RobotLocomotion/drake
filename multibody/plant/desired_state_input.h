...
/* This struct stores the desired state inputs for all model instances.
 ...
 controller is disabled because its joint is locked, then this class will not
...
struct DesiredStateInput {
 public:
  ...
  struct Item {
    ...
    // TODO(jwnimmer-tri) The JointActuator should have a getter than returns
    // this index (it is widely used). Then we could remove this field.
    int actuation_index_within_model_instance{};
  };

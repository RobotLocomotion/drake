...
/** ContactResultsToMeshcatSystem is a system that publishes a ContactResults to
geometry::Meshcat; drawing double-sided arrows at the location of the contact
...
port of a MultibodyPlant.
 ...
 @system
 name: ContactResultsToMeshcatSystem
 input_ports:
 - contact_results
 @endsystem
...
template <typename T>
class ContactResultsToMeshcatSystem final : public systems::LeafSystem<T> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(ContactResultsToMeshcatSystem)

  /** Creates an instance of %ContactResultsToMeshcatSystem */
  explicit ContactResultsToMeshcatSystem(
  ...
  template <typename U>
  explicit ContactResultsToMeshcatSystem(
      const ContactResultsToMeshcatSystem<U>& other);

  /** Calls geometry::Meschat::Delete(std::string path), with the path set to
  `prefix`. Since this visualizer will only ever add geometry under this prefix,
  this will remove all geometry/transforms added by the visualizer, or by a
  previous instance of this visualizer using the same prefix.  Use the
  `delete_on_initialization_event` in the constructor to determine whether this
  ...
  /** Adds a ContactResultsToMeshcatSystem and connects it to the given
  MultibodyPlant's ContactResults-valued output port. */
  static const ContactResultsToMeshcatSystem<T>& AddToBuilder(
 ...
 private:
  /* ContactResultsToMeshcatSystem of different scalar types can all access each
  other's data. */
  template <typename>
  friend class ContactResultsToMeshcatSystem;
  ...
  mutable std::map<SortedPair<geometry::GeometryId>, bool>
      contacts_{};
...
/** A convenient alias for the ContactResultsToMeshcatSystem class when using
the `double` scalar type. */
using ContactResultsToMeshcatSystemd = ContactResultsToMeshcatSystem<double>;
...
template <>
struct Traits<multibody::ContactResultsToMeshcatSystem>
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(
    class ::drake::multibody::ContactResultsToMeshcatSystem)

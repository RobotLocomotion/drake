...
void TamsiSolver<T>::CalcNormalForces(
  ...
  // We simply use the chain rule to compute the derivative of fₙ as a function
  // of vₙ (on a per contact basis, that's why the use of .array() operations
  // below).
  // dfₙ/dvₙ = -d⋅H(1 − d vₙ)⋅(fₙ₀ − h k vₙ)₊  - h⋅k⋅(1 − d vₙ)₊⋅H(fₙ₀ − h k vₙ)
  // Note that dfₙ/dvₙ < 0 always.

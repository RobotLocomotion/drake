...
// There's an anoymous helper function in multibody_plant.cc ("GetScopedName")
// that rewrites geometry names. Unwind that here so that we can report the
// actual names that the user specified.
template <typename T>
...
void ResetHelper(
  ...
  for (BodyIndex i{0}; i < num_bodies; ++i) {
    ...
    prototype.geometry_is_unique_within_body =
        (geometry_ids.size() == 1);

...
/* This class is meant to perform all calculations needed to advance state for a
 ...
 In addition, managers have the chance to declare additional state, cache and/or
 ports. This allows developers to incorporate additional discrete models coupled
 with the rigid body dynamics. For instance simulation of deformable objects
 requires additional state and ports to interact with externals systems such as
 visualization.

 We can think of ContactComputationManager as an extension of MultibodyPlant but
 in a different compilation unit, in that we are still allowed to access
 MultibodyPlant's private members and methods as needed.

 A manager can get access to private methods and/or members through the
 internal::MultibodyPlantAccess class, refer to MultibodyPlantAccess class's

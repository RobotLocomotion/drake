...
class GeometryContactData {
 public:
  ...
  /* Returns a shared_ptr to the data. Note that this is alias to the *current*
 ...
 private:
  /* Returns a forever-valid pointer (to static storage) that is empty. */
  static const NestedGeometryContactData<T>* get_empty_ptr();

  std::shared_ptr<const NestedGeometryContactData<T>> data{nullptr};
};

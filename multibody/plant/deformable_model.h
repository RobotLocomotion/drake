 ...
 public:
  ...
  /** Defines a fixed constraint between a deformable body A a rigid body B.
   ...
   @param[in] shape        The geometry shape of the rigid body B. If a vertex
                           Pᵢ of A, in its reference configuration, is inside
                           (or on the surface of) the given `shape`, then it is
                           subject to the fixed constraint. The vertex Pᵢ is
                           fixed to the point Qᵢ, where Qᵢ is the point rigidly
                           affixed to B that is coincident with Pᵢ when A is in
                           its reference configuration and B is in pose X_WB.

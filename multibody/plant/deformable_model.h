...
class DeformableModel final : public multibody::internal::PhysicalModel<T> {
 public:
  ...
  /* Construct a DeformableModel to be owned by the given MultibodyPlant. We
   currently only support deformable models that are discrete in time.
   @pre plant != nullptr.
  ...
  //  simulated with MbP that involves contact. Also, move resolution_hint
  // into the properties of the instance.
  /* Registers a deformable body in `this` DeformableModel with the given
   GeometryInstance. The body is simulated with FEM with linear elements and
   first order quadrature rule that integrates linear functions exactly.

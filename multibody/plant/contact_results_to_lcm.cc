...
void ContactResultsToLcmSystem<T>::CalcLcmContactOutput(
  ...
  for (int i = 0; i < contact_results.num_point_pair_contacts(); ++i) {
    ...
    assign(info_msg.contact_point, contact_info.contact_point());
    assign(info_msg.contact_force, contact_info.contact_force());
    assign(info_msg.normal, contact_info.point_pair().nhat_BA_W);
  }
  ...
  for (int i = 0; i < contact_results.num_hydroelastic_contacts(); ++i) {
    ...
    assign(surface_message.centroid_W, contact_surface.centroid());
    assign(surface_message.force_C_W,
           hydroelastic_contact_info.F_Ac_W().translational());
    assign(surface_message.moment_C_W,
    ...
    for (int j = 0; j < surface_message.num_quadrature_points; ++j) {
      ...
      assign(quad_data_message.p_WQ, quadrature_point_data[j].p_WQ);
      assign(quad_data_message.vt_BqAq_W, quadrature_point_data[j].vt_BqAq_W);
      assign(quad_data_message.traction_Aq_W,

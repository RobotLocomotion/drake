...
// updates using a contact solver that works with dense algebra. A const manager
// is provided at construction so that the driver has access to the const model
// and computation services agnostic to the solver type, such as geometry
// queries and/or kinematics. Mutable access to the manager is provided during
// DeclareCacheEntries() to allow the declaration of system-level cache entries.

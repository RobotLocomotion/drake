 ...
 *
 * We will use q_star to denote the nominal posture. For a revolute joint, that
 * q_star is the variable θ*. Note that the stereographic projection doesn't
 * work when θ = θ* ± π, as tan(Δθ/2) is infinity.
 */
class RationalForwardKinematics {
 ...
 private:
   ...
   * polynomial, with indeterminates being s_q and c_q, where s_q represents
   * sin(q - q_star) and c_q represents cos(q - q_star).
   */

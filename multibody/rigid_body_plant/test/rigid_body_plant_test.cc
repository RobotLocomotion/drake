...
GTEST_TEST(RigidBodyPlantTest, LinearizePendulumTest) {
  ...
  auto linearized_pendulum = systems::Linearize(pendulum, *context);
  ...
  EXPECT_TRUE(CompareMatrices(linearized_pendulum->A(), A, 1e-8));
  EXPECT_TRUE(CompareMatrices(linearized_pendulum->B(), B, 1e-8));
  EXPECT_TRUE(CompareMatrices(linearized_pendulum->C(),
                              Eigen::Matrix2d::Identity(), 1e-8));

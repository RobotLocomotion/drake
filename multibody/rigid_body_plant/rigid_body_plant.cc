...
void RigidBodyPlant<T>::DoCalcTimeDerivatives(
  ...
  if (tree_->getNumPositionConstraints()) {
    ...
    // implemented as
    //   J*vdot = -(Jdotv + 2 * alpha * J * v + alpha * alpha * phi)

    // Solve [H,-J^T] * [vdot] = [  right_hand_side  ]
    //       [J, 0  ]   [ f  ]   [- Jdotv ... (above)].
    MatrixX<T> A(H_and_neg_JT.rows() + J.rows(),
                 H_and_neg_JT.cols() + J.rows());
    VectorX<T> b(H_and_neg_JT.rows() + J.rows());
    // clang-format off
    A << H_and_neg_JT, -J.transpose(),
         J, MatrixX<T>::Zero(J.rows(), H_and_neg_JT.cols()+J.rows()-J.cols());
    b << right_hand_side,
    ...
    // Solve [H,-J^T] * [vdot;f] = -C.
    const VectorX<T> vdot_f =
        H_and_neg_JT.jacobiSvd(Eigen::ComputeThinU | Eigen::ComputeThinV)
...
RigidBodyPlant<T>::DoCalcDiscreteVariableUpdatesImpl(
  ...
  if (!std::is_same<T, double>::value && contacts.size() > 0) {
    DRAKE_ABORT_MSG("Contact calculations only support T=double.");
  }

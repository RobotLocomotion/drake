...
implication of that is collision filter groups defined in sdf file contain
...
@ref /drake/multibody/collision/test/multi_model_groups.sdf for an example.
...
legacy API means that the bodies parsed from a file cannot be programmatically
altered.

There is a _second_ parsing interface that allows for suppressing the automatic
tree compilation. If there is a need to augment or modify the collision filter
groups declared in a parsed file, pass `false` into this alternate interface.

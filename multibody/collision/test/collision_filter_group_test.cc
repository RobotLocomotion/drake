...
// Helper function to create dummy rigid bodies. The body is joined to the world
// via either a fixed or floating joint, depending ont he value of `is_fixed`.
...
// The body will be owned by the given tree but a reference to that body will be
// returned.
RigidBody<double>& MakeDummyBody(const std::string& name, bool is_fixed,
  ...
  if (!is_fixed) {
    // Garbage non-zero inertia to prevent the body from being welded to world.
...
GTEST_TEST(CollisionFilterGroupRBT, CollisionElementSetFilters) {
  ...
  Element element1(DrakeShapes::Sphere(1.0), Isometry3d::Identity(), &body1);
  Element element2(DrakeShapes::Sphere(1.0), Isometry3d::Identity(), &body2);
  ...
  tree.addCollisionElement(element1, body1, collision_group);
  tree.addCollisionElement(element2, body2, collision_group);
...
GTEST_TEST(CollisionFilterGroupRBT, AddElementPostCompileNoFilter) {
  ...
  Element element(DrakeShapes::Sphere(1.0), Isometry3d::Identity(), &body);
...
GTEST_TEST(CollisionFilterGroupRBT, AddedElementInheritsFilter) {
  ...
  tree.addCollisionElement(Element(DrakeShapes::Sphere(1.0),
                                   Isometry3d::Identity(), &bodyA), bodyA, "");
  RigidBody<double>& bodyB = MakeDummyBody("bodyB", is_fixed, &tree);
  tree.addCollisionElement(Element(DrakeShapes::Sphere(1.0),
                                   Isometry3d::Identity(), &bodyB), bodyB, "");
  RigidBody<double>& bodyC = MakeDummyBody("bodyC", is_fixed, &tree);
  tree.addCollisionElement(Element(DrakeShapes::Sphere(1.0),
                                   Isometry3d::Identity(), &bodyC), bodyC, "");
  ...
  tree.addCollisionElement(Element(DrakeShapes::Sphere(2.0),
                                   Isometry3d::Identity(), &bodyB), bodyB, "");
  tree.compile();
  pairs = CollideAtZero(&tree);
...
//
//   - State 1: Body has no adjacent bodies
//    - State 1a: Body had no previous elements.
//    - Result 1a: added element should have *no* collision cliques.
//    - State 1b: Body had *one* previous elements.
//    - Result 1b: both elements should have a single common collision clique.
//    - State 1c: Body had *multiple* previous elements.
//    - Result 1c: New element has same, single clique as previous elements.
//
//   - State 2: Body has an adjacent body with a compiled collision element
//    - State 2a: Body has no collision elements
//    - Result 2a: Collision element has single clique, that clique is added to
//                 adjacent collision element.
//    - State 2b: Body has one (or more) compiled elements
//    - Result 2b: Prior collision elements are unchanged, new collision element
//                 has clique identical to compiled element
//
//   - State 3: Add collision elements to two adjacent bodies.
//    - State 3a: Neither body has collision elements.
//    - Result 3a: Both collision elements have a single, common clique.
//    - State 3b: One body has one collision element.
//    - Result 3b: All three collision elements have a single, common clique.
//    - State 3c: One body has multiple collision elements.
//    - Result 3c: Collision element for body with *no* elements has single
//                 clique. Elements for other body all have 2 cliques (including
//                 the same clique for the other body).
//    - State 3d: Each body has one (or more) collision element.
//    - Result 3d: Each new element gets a copy of the original elements.
//
//  - Action 2: Attaching a new body (with a single collision element) to a
//    compiled body (e.g., putting a gripper on an arm.)
//    - State 4: Compiled body has no collision element
//    - Result 4: No collision cliques on new body's element.
//    - State 5: Compiled body *has* one or more collision elements.
//    - Result 5: Element on new body has collision clique; all elements in
//                compiled body include that clique.
//
...
GTEST_TEST(PostCompileCliqueRBT, AddElementToIsolatedBody) {
  ...
  Element& element = **bodyA.collision_elements_begin();
  EXPECT_EQ(element.get_num_cliques(), 0);
...
GTEST_TEST(PostCompileCliqueRBT, AddElementToNeighborBody) {
  ...
  tree.addCollisionElement(
      Element(DrakeShapes::Sphere(1.0), Isometry3d::Identity(), &adjacent_body),
      adjacent_body, "");
  ...
  // State 2a
  tree.addCollisionElement(
      Element(DrakeShapes::Sphere(1.0), Isometry3d::Identity(), &target_body),
      target_body, "");
  tree.compile();
...
GTEST_TEST(PostCompileCliqueRBT, AddElementsToEmptyNeighborBodies) {
  ...
  // State 3a
  tree.addCollisionElement(
      Element(DrakeShapes::Sphere(1.0), Isometry3d::Identity(), &body1), body1,
      "");
  tree.addCollisionElement(
      Element(DrakeShapes::Sphere(1.0), Isometry3d::Identity(), &body2), body2,
      "");
  tree.compile();
...
GTEST_TEST(PostCompileCliqueRBT, AddElementsToMultiNeighborBodies) {
  ...
  for (auto element : {&element1a, &element1b, &element1c}) {
    ASSERT_NE(element->collision_cliques().front(), common_clique);
    ASSERT_EQ(element->collision_cliques().back(), common_clique);
  }
}
...
GTEST_TEST(PostCompileCliqueRBT, AddElementsToPopulatedNeighborBodies) {
  ...
  // These tests rely on the fact that clique ids only get larger and that the
  // cliques are stored in increasing order.
  for (auto element : {&element1a, &element1b, &element2a, &element2b}) {
    EXPECT_EQ(element->collision_cliques().front(), original_clique);
  }
}
...
GTEST_TEST(PostCompileCliqueRBT, AddNeighorToEmptyBody) {
  ...
  tree.addCollisionElement(
      Element(DrakeShapes::Sphere(1.0), Isometry3d::Identity(), &body3), body3,
      "");
  tree.compile();
  ASSERT_EQ(body1.get_num_collision_elements(), 0);
  ASSERT_EQ(body2.get_num_collision_elements(), 1);

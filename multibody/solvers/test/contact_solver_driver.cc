...
void ContactSolverDriver::Initialize() {
...
void ContactSolverDriver::SetPointContactParameters(const Body<double>& body,
  ...
  const auto& inspector = GetInspectorToMutableState();
...
const geometry::SceneGraphInspector<double>& ContactSolverDriver::GetInspector()
    const {
...
const geometry::SceneGraphInspector<double>&
ContactSolverDriver::GetInspectorToMutableState() {
  const geometry::SceneGraphInspector<double>* inspector{nullptr};
  if (initialized_) {
    const auto& query_object =
        plant_->get_geometry_query_input_port()

...
GTEST_TEST(TestDiffferentiableNorm, test_autodiff) {
  ...
  x_grad << 1, 2, 3;
  Vector3<AutoDiffXd> x = math::InitializeAutoDiffFromValueAndGradientMatrix(
  ...
  // Test when x is zero.
  x = math::InitializeAutoDiffFromValueAndGradientMatrix(
      Eigen::Vector3d::Zero(), x_grad);
...
void TestEval(const QuaternionEulerIntegrationConstraint& dut,
  ...
  const AutoDiffVecXd x_ad =
      math::InitializeAutoDiffFromValueAndGradientMatrix(Eigen::VectorXd(x),
                                                         x_grad);
  AutoDiffVecXd y_ad;
  dut.Eval(x_ad, &y_ad);
  ...
  EXPECT_TRUE(CompareMatrices(
      math::ExtractValueMatrixFromAutoDiff(y_ad),
      math::ExtractValueMatrixFromAutoDiff(y_ad_expected), 10 * kEps));
  EXPECT_TRUE(CompareMatrices(
      math::ExtractGradientMatrixFromAutoDiff(y_ad),
      math::ExtractGradientMatrixFromAutoDiff(y_ad_expected), 10 * kEps));
}

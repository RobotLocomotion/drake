...
TEST_F(ContactWrenchEvaluatorTest,
  ...
  const Vector3<AutoDiffXd> lambda_val =
      math::InitializeAutoDiffFromValueMatrix(Eigen::Vector3d(1, 2, 3));
  const VectorX<AutoDiffXd> x_value =
      evaluator.ComposeVariableValues(*plant_context, lambda_val);
  AutoDiffVecXd y;
  evaluator.Eval(x_value, &y);
  EXPECT_EQ(math::ExtractValueMatrixFromAutoDiff(y.head<3>()),
            Eigen::Vector3d::Zero());
  EXPECT_EQ(y.tail<3>(), lambda_val);
  EXPECT_TRUE(
      CompareMatrices(math::ExtractGradientMatrixFromAutoDiff(y.tail<3>()),
                      math::ExtractGradientMatrixFromAutoDiff(lambda_val)));
}
}  // namespace
}  // namespace multibody
}  // namespace drake

 ...
 public:
  ...
  void CheckStaticEquilibriumConstraintEval(
    ...
    EXPECT_TRUE(CompareMatrices(
        math::ExtractValueMatrixFromAutoDiff(y_autodiff),
        math::ExtractValueMatrixFromAutoDiff(y_autodiff_expected), 100 * kEps));
    EXPECT_TRUE(CompareMatrices(
        math::ExtractGradientMatrixFromAutoDiff(y_autodiff),
        math::ExtractGradientMatrixFromAutoDiff(y_autodiff_expected),
        100 * kEps));
    ...
    const auto J = math::ComputeNumericalGradient(
        eval_fun, math::ExtractValueMatrixFromAutoDiff(x_autodiff));
    EXPECT_TRUE(CompareMatrices(
        math::ExtractGradientMatrixFromAutoDiff(y_autodiff), J, 1e-5));
  }
...
TEST_F(TwoFreeSpheresTest, Eval) {
  ...
  auto x_autodiff = math::InitializeAutoDiffFromValueMatrix(x_val);
  ...
  x_val << q_val, lambda_val;
  x_autodiff = math::InitializeAutoDiffFromValueMatrix(x_val);
  CheckStaticEquilibriumConstraintEval(
      static_equilibrium_binding, x_autodiff.head<14>(), x_autodiff.tail<9>());

...
TEST_F(IiwaKinematicConstraintTest, SpatialVelocityConstraint) {
  ...
  w_AQ_bounds.magnitude_upper = 2.0;
  ...
  expected_lower << v_AQ_lower, std::pow(w_AQ_bounds.magnitude_lower, 2),
      std::cos(w_AQ_bounds.theta_bound);
  expected_upper << v_AQ_upper, std::pow(w_AQ_bounds.magnitude_upper, 2), 1;
  ...
  EXPECT_NEAR(y[3], V_AQ_A.rotational().squaredNorm(), 1e-14);
  EXPECT_NEAR(y[4],

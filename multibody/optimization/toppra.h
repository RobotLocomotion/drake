...
class Toppra {
 public:
   ...
   * </pre>
   * is less than `max_err`. Gridpoints are selected by adding the midpoint
   * between two gridpoints whenever the distance between them is too large or
   * the estimated error is too high. This results in more points in parts of
   ...
   * constrain the generalized torques applied to the plant and does not reason
  ...
  /**
   * Adds a limit on the spatial velocity of the given frame, measured and
   * and expressed in the world frame.  The limits should be given as [ω_WF,
   * v_WF], where ω_WF is the frame's angular velocity and v_WF is the frame's
  ...
  /**
   * Adds a limit on the spatial acceleration of the given frame, measured and
   * and expressed in the world frame.  The limits should be given as [α_WF,
   * a_WF], where α_WF is the frame's angular acceleration and v_WF is the
   * frame's translational acceleration.

...
template <typename T>
Acrobot<T>::Acrobot(const Vector3<T>& normal, const Vector3<T>& up, double m1,
                    double m2, double l1, double l2, double lc1, double lc2,
                    double Ic1, double Ic2, double b1, double b2, double g)
    : m1_(m1),
      m2_(m2),
      l1_(l1),
      l2_(l2),
  ...
  DRAKE_ASSERT(std::abs(std::abs(ExtractDoubleOrThrow(normal.dot(up))) - 1.0) >
...
template <typename T>
Vector2<T> Acrobot<T>::CalcCoriolisVector(const T&, const T& theta2,
                                          const T& theta1dot,
...
template <typename T>
Vector2<T> Acrobot<T>::CalcGravityVector(const T& theta1,
...
template <typename T>
RigidTransform<T> Acrobot<T>::CalcLink2PoseInWorldFrame(const T& theta1,
                                                        const T& theta2) const {
  using std::cos;
  using std::sin;
...
Vector6<T> Acrobot<T>::CalcLink2SpatialVelocityInWorldFrame(
    const T& theta1, const T& theta2, const T& theta1dot,
    const T& theta2dot) const {
  using std::cos;
  using std::sin;
...
Vector6<T> Acrobot<T>::CalcLink1SpatialAccelerationInWorldFrame(
  ...
  const Vector3<T> acm1_D =
      lc1_ * Vector3<T>(-sin(theta1), cos(theta1), 0.0) * theta1dot *
          theta1dot +

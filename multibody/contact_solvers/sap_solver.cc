...
void SapSolver<T>::CalcDelassusDiagonalApproximation(
    int nc, const std::vector<MatrixX<T>>& At,
...
void SapSolver<T>::PreProcessData(const T& time_step,
  ...
  dynamics_data.get_A().AssembleMatrix(&data.Ablock);
  contact_data.get_Jc().AssembleMatrix(&data.Jblock);

  // Extract momentum matrix's per-tree diagonal blocks. Compute diagonal
  // scaling inv_sqrt_A.
  data.At.clear();
  data.At.reserve(data.Ablock.num_blocks());
  for (const auto& block : data.Ablock.get_blocks()) {
    ...
    data.At.push_back(Mij);

    const int start = data.Ablock.row_start(t1);
    DRAKE_DEMAND(start == data.Ablock.col_start(t2));
    inv_sqrt_A.segment(start, nt) = Mij.diagonal().cwiseInverse().cwiseSqrt();
  }
  ...
  CalcDelassusDiagonalApproximation(nc, data.At, data.Jblock, &data.Wdiag);
  ...
  data.mu = mu;
  data.Ablock.Multiply(data.v_star, &data.p_star);
}
...
void SapSolver<T>::CalcStoppingCriteriaResidual(const State& state,
  ...
  const VectorX<T> j_tilde = inv_sqrt_A.asDiagonal() * j;
...
ContactSolverStatus SapSolver<double>::DoSolveWithGuess(
  ...
  for (; k < parameters().max_iterations; ++k) {
    ...
    if (momentum_residual <= parameters().rel_tolerance * momentum_scale) {
      ...
      // iteration it is needed. If the stopping criteria is satisfied at k = 0
      // (good guess), then we skip the expensive instantiation of the solver.
    }
...
void SapSolver<T>::UpdateMomentumCache(const State& state, Cache* cache) const {
  ...
  data().Ablock.Multiply(state.v(), &momentum_cache.p);  // p = A⋅v.
...
void SapSolver<T>::UpdateSearchDirectionCache(const State& state,
  ...
  data().Ablock.Multiply(search_direction_cache.dv, &search_direction_cache.dp);

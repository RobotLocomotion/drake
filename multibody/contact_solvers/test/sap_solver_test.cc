...
class ParticlesStackProblem {
 public:
  ...
  // Contact Jacobian reprsenting the configuration in which the particles form
...
GTEST_TEST(SapSolver, PreProcessedData) {
  ...
  EXPECT_EQ(data.Ablock.MakeDenseMatrix(), problem_data->M);
  ...
  EXPECT_EQ(data.inv_sqrt_A, expected_inv_sqrt_A);
  EXPECT_EQ(data.v_star, problem_data->v_star);
  EXPECT_EQ(data.p_star, problem_data->M * problem_data->v_star);
  ...
  EXPECT_TRUE(CompareMatrices(data.Wdiag, Wdiag_expected,
                              std::numeric_limits<double>::epsilon(),

...
class SapSolverTester {
 public:
  typedef SapSolver<double>::PreProcessedData PreProcessedData;
...
GTEST_TEST(SapSolver, PackContactResults) {
  ...
  const int nv = 6;
  const int nc = 3;
  SapSolverTester::PreProcessedData data(nv, nc);
  data.time_step = 1.0e-3;
  ...
  EXPECT_EQ(results.tau_contact, tauc_expected);
}

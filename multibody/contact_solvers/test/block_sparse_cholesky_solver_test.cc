...
GTEST_TEST(BlockSparseCholeskySolverTest, FailureDueToNonSpdness) {
  ...
  /* Calling CalcSchurComplementAndFactor() also fails. */
  {
    for (const std::unordered_set<int>& eliminated_blocks :
      ...
      const std::optional<MatrixXd> schur_complement =
          solver.CalcSchurComplementAndFactor(A, eliminated_blocks);
...
GTEST_TEST(BlockSparseCholeskySolverTest, CalcSchurComplementAndFactor) {
  ...
  /* All blocks are eliminated. */
  {
    ...
    const std::optional<MatrixXd> schur_complement =
        solver.CalcSchurComplementAndFactor(
  ...
  {
    const std::optional<MatrixXd> schur_complement =
        solver.CalcSchurComplementAndFactor(M, std::unordered_set<int>());
    EXPECT_TRUE(schur_complement.has_value());
  ...
  /* Some of the blocks are eliminated. */
  {
    ...
    const std::optional<MatrixXd> schur_complement =
        solver.CalcSchurComplementAndFactor(M, eliminated_blocks);
    EXPECT_TRUE(schur_complement.has_value());

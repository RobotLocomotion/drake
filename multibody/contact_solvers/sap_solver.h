...
class SapSolver final : public ContactSolver<T> {
 ...
 private:
  ...
  // [Castro et al., 2021] as best as we can, though with ASCII and Unicod
  // symbols.
  struct PreProcessedData {
    ...
    T time_step;      // Discrete time step used by the solver.
    ...
    BlockSparseMatrix<T> Jblock;  // Jacobian as block-sparse matrix.
    BlockSparseMatrix<T> Ablock;  // Momentum matrix as block-sparse matrix.
    ...
    // diag(A)^{-1/2}. This matrix is used to compute a dimensionless residual

...
/* Implements a "fixed" tendon constraint for the SAP solver, [Castro et al.,
 ...
 Compliant impulse:
  ...
  constraint when g₀ < 0. This is the strategy we employ internally, see #22664
  for more details.
  ...
  class Parameters {
   public:
    ...
    /* Constructs a valid set of parameters.
     @param lower_limit Lower limit of the tendond length in [m] or [rad].
     @param upper_limit Upper limit of the tendond length in [m] or [rad].
    ...
    int num_equations() const { return n_eq_; }
   ...
   private:
    ...
    T damping_;
    double beta_{0.1};
    // Stores the number of constraint equations that should be generated
    // given the values of lower_limit and upper_limit. Either 1 when
    // only one of the limits is finite, and two otherwise.
    int n_eq_;
  };

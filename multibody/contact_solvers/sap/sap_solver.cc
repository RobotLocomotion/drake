...
void SapSolver<T>::CallDenseSolver(const Context<T>& context,
  ...
  // though the computation of the dense H (and in paricular of the Jᵀ⋅G⋅J term)
  // is very costly. Therefore below we decided to trade off speed for stability
  // when choosing to use an LDLT decomposition instead of a slightly faster,
  // though less stable, LLT decomposition.

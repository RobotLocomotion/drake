...
/* Implements a SAP (compliant) distance constraint between two points. With
 ...
 With p̂ the unit vector from P to Q, the force vector on B applied at Q is:
   f_Bq = γ⋅p̂
 and the reaction for on A applied at P is:
   f_Ap = γ⋅p̂

 @tparam_nonsymbolic_scalar */
template <typename T>
class SapDistanceConstraint : public SapHolonomicConstraint<T> {
 public:
  ...
  /* Parameters that define the constraint's compliance. */
  class Parameters {
   public:
    DRAKE_DEFAULT_COPY_AND_MOVE_AND_ASSIGN(Parameters);

    /* Stiffness k and damping c. */
    Parameters(T stiffness, T damping);
  ...
  SapDistanceConstraint(Kinematics kinematics, Parameters parameters);
 ...
 private:
  ...
  MakeSapHolonomicConstraintParameters(const Parameters& p);

  // Makes the constraint function and Jacobian needed to intialize the base
  // class SapHolonomicConstraint. Initializes p_hat as needed by this
  // constraint.

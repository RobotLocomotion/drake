...
struct SapFixedTendonConstraintData {
  ...
  T cost_;              // Cost ℓ(v)
  VectorX<T> gamma_;    // Impulse γ(v) = −∂ℓ(v)/∂v.
...
/* Implements a "fixed" tendon constraint for the SAP solver, [Castro et al.,
 ...
 This class imposes a set of bi-lateral constraints such that this length stays
 ...
 Compliant impulse:
  ...
  N⋅m⋅rad/s), (x)₊ = max(0, x) component-wise, and H(x) the heaviside step
  function. Dissipation is parameterized as d = tau_d⋅k, where tau_d is the
  "dissipation time scale". Notice that these impulses are strictly positive
  (force is only applied to bring the length within limits) when the constraint
  is violated and are zero otherwise.
  ...
  Notice that ℓ(v) = (R/2)⋅γ(v)² + C is a valid cost (i.e. γ(v) = −∂ℓ(v)/∂v) for
  any constant C. We simply choose the constant C = -(R/2)⋅γ(ṽ)² such that the
  cost is continuous and is exactly zero when the constraint is inactive.
...
class SapFixedTendonConstraint final : public SapConstraint<T> {
 ...
 private:
  ...
  /* Computes the constraint function g(q0) as a function of the configuration
   given in `kinematics`. */

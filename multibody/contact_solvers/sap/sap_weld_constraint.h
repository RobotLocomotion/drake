...
/* Implements a SAP (compliant) weld constraint between two bodies.
 ...
 Let a = a_PQ_N = θλ be a first-order approximation to the axis-angle product
 representation of the relative rotation R_PQ, Q's orientation in P expressed in
 frame N. The rotational component of the constraint impulse γᵣ ∈ ℝ³ is modeled
 as:
   γᵣ = −(kᵣ⋅a + dᵣ⋅ȧ)
 where k and dᵣ are the stiffness and damping coefficients respectively.

 N.B. a, expressed in either P, Q, or N is exactly the same triplet of numbers.
 We choose to say a_PQ_N for symmetry reasons in the notation.

 Let p = p_PoQo_N, Qo's position relative to Po expressed in frame N. The
 translational component of the constraint impulse γₜ ∈ ℝ³ is modeled as:
   γₜ = −(kₜ⋅p + dₜ⋅ṗ)
 where kₜ and dₜ are, likewise, the stiffness and damping coefficients.

 Thus the full constraint function is:
   g = (a_PQ_N, p_PoQo_N)
 Essentially penalizing non-zero position p_PoQo and non-zero angle-axis a_PQ.
 Letting k = (kᵣ, kₜ), d = (dᵣ, dₜ) we can write the complete impulse as:
   γ = -(k⋅g + d⋅ġ)

 This results in a constraint impulse on B applied at No expressed in frame N:
   F_BNo_N = (γᵣ, γₜ)
 and a impulse on A applied at No expressed in frame N:
   F_ANo_N = -(γᵣ, γₜ)

 DoAccumulateSpatialImpulses() shifts these impulses to the body frame origins
 and re-expresses them in the world frame, reporting F_BBo_W and F_AAo_W,
 respectively.
  ...
  class Kinematics {
   public:
    ...
    /* @param[in] objectA
       ...
       @param[in] Jv_PQ_W Jacobian for the relative spatial velocity V_PQ_W.

       @note Thus far only dense Jacobian blocks are supported, i.e. rigid body
       applications in mind.
       @throws std::exception If Jv_PQ_W.blocks_are_dense() == false.
    */

...
struct SapSolverParameters {
  ...
  struct ExactLineSearchParameters {
    ...
    // switches to bisection when Newon convergence is slow, this means that the
    // step size paramter will be computed with precision 0.5^max_iterations.
    // We very seldom need to change this parameter.
...
class SapSolver {
 ...
 private:
  ...
  // @param vec_scratch A scratchpad needed when computing d2ell_dalpha2. Must
  //   not be nullptr if d2ell_dalpha2 != nullptr.

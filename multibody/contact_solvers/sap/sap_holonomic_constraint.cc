...
void SapHolonomicConstraint<T>::Project(const Eigen::Ref<const VectorX<T>>& y,
  ...
  *gamma = y.array().max(gl.array()).min(gu.array());
  if (dPdy != nullptr) {
    ...
    // Eigen resizes the object on the left-hand side automatically so that it
    // matches the size of the object on the right-hand side. Resizing is no-op
    // if already the proper size.

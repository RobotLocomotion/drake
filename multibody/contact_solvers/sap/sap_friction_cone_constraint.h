...
class SapFrictionConeConstraint final : public SapConstraint<T> {
 public:
  ...
  /* Constructor for a SAP contact constraint between to objects A and B. The
 ...
 private:
  ...
  /* Accumulates generalized forces applied by this constraint on the o-th
   object.
   @param[in] o Object index. As defined at construction o = 0 corresponds to
   object A and o = 1 corresponds to object B.
   @param[in] gamma Impulses for this constraint, of size
   num_constraint_equations().
   @pre F is not nullptr.
   @throws std::logic_error if `object` does not correspond to this constraint.
  */

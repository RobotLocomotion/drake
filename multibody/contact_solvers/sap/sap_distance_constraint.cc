...
SapDistanceConstraint<T>::SapDistanceConstraint(const Parameters& p, int clique,
                                                const MatrixX<T>& J,
                                                const T& d0)
    : SapConstraint<T>(clique, Vector1<T>::Constant(d0), J),
      parameters_(p),
      d0_(d0) {
  DRAKE_DEMAND(this->first_clique_jacobian().rows() == 1);
}
...
SapDistanceConstraint<T>::SapDistanceConstraint(const Parameters& p,
                                                ...
                                                const MatrixX<T>& J1,
                                                const T& d0)
    : SapConstraint<T>(clique0, clique1, Vector1<T>::Constant(d0), J0, J1),
      parameters_(p),
      d0_(d0) {
  DRAKE_DEMAND(this->first_clique_jacobian().rows() == 1);
  DRAKE_DEMAND(this->second_clique_jacobian().rows() == 1);
}

...
void SapContactProblem<T>::ExpandContactSolverResults(
  ...
  for (int i = 0; i < num_constraints(); ++i) {
    ...
    Eigen::VectorBlock<VectorX<T>> vc_segment = results->vc.segment(
        constraint_velocity_start(i), c.num_constraint_equations());
...
int SapContactProblem<T>::AddConstraint(std::unique_ptr<SapConstraint<T>> c) {
  ...
  if (constraint_velocity_start_.size() == 0) {
    constraint_velocity_start_.push_back(0);
  } else {
    constraint_velocity_start_.push_back(constraint_velocity_start_.back() +

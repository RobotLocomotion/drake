...
std::unique_ptr<SapContactProblem<T>> SapContactProblem<T>::MakeReduced(
  ...
  const int num_cliques = ssize(per_clique_known_free_motion_dofs);
  const int num_constraints = this->num_constraints();
  ...
  for (int i = 0; i < num_cliques; ++i) {
    ...
    if (ssize(per_clique_known_free_motion_dofs[i]) < A_[i].rows()) {

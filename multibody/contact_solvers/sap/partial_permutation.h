...
// That is, given S = {s₁, s₂, ⋯ ,s'ᵢ, ⋯, sₙ}, this class represents the
...
// to elements s'ᵢ in S whenever there exist i ∈ [0,n) such that ip = P(i).
// Elements s'ᵢ for which no ip is defined are left unchanged.
class PartialPermutation {
 public:
  ...
  // n) is given by ip = permutation[i] in [0, m). The permutation is
  // a full permutation whenever m = n and all indexes in [0, m) are present in
  ...
  // For both partial and full permutations, all indexes in [0, m) must be
  // present or an exception is thrown.
  //
  // Example: permutation = {-1, 0, 2, 1, -1, 3} indicates defines P(1) = 0,
  // P(2) = 2, P(3) = 1 and P(5) = 3. Since their entries are negative, elements
  // indexed with 0 and 4 in the domain do not participate in the permutation.
  // For this case domain_size() = 6 and permuted_domain_size() = 4. Notice that
  // all indexes in [0, 4) are present in the provided array `permutation`.
  //
  ...
  // @throws if ip = P(i) for i ∈ [0,n) does not span all indexes in [0,m), i.e.
  // there are holes in the permuted domain.

...
/* This class represents the graph for a SapContactProblem where each node in
 ...
 "constraint cluster", see ConstraintCluster. A constraint cluster is the set of
 all constraints acting on the same clique, or on the same pair of cliques. This
 concept is described in detail with an example in [Castro et al., 2021].
...
class ContactProblemGraph {
  ...
  class ConstraintCluster {
   public:
    ...
    /* Constructor for a cluster of constraints between the cliques specified by
     `cliques_pair`. Indexes in `cliques_pair` must be non-negative. or an
     exception is thrown.*/
    ...
    const drake::SortedPair<int>& cliques() const { return cliques_; }
 ...
 private:
  ...
  int AddConstraint(SortedPair<int> cliques_pair, int num_constrained_dofs);

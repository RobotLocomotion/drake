...
typename SapBallConstraint<T>::Kinematics MakeArbitraryKinematics(
  ...
  Vector3<T> p_WA(1., 2., 3.);
  Vector3<T> p_WP(4., 5., 6.);
  const int objectB = 5;
  Vector3<T> p_WB(7., 8., 9.);
  Vector3<T> p_WQ(10., 11., 12.);
  const int clique0 = 3;
  ...
  return typename SapBallConstraint<T>::Kinematics{
      objectA, p_WA, p_WP, objectB, p_WB, p_WQ, J_PQ_W};
}

...
typename SapCouplerConstraint<T>::ComplianceParameters
MakeArbitraryParameters() {
  const T stiffness = 1.0e5;
  const T dissipation_time_scale = 1.0e3;
  return typename SapCouplerConstraint<T>::ComplianceParameters{
...
GTEST_TEST(SapCouplerConstraint, SingleCliqueConstraint) {
  ...
  const SapCouplerConstraint<double>::ComplianceParameters parameters =
      MakeArbitraryParameters();
  const SapCouplerConstraint<double>::Kinematics kinematics =
      MakeArbitraryKinematics(num_cliques);
  SapCouplerConstraint<double> c(kinematics, parameters);
  ...
  EXPECT_EQ(c.compliance_parameters().dissipation_time_scale(),
...
void ValidateProjection(
    const SapCouplerConstraint<double>::ComplianceParameters& p,
    const Vector1d& vc) {
  SapCouplerConstraint<AutoDiffXd>::ComplianceParameters p_ad(
      p.stiffness(), p.dissipation_time_scale());
  ...
  SapCouplerConstraint<AutoDiffXd> c(kin_ad, p_ad);
...
GTEST_TEST(SapCouplerConstraint, Gradients) {
  ...
  SapCouplerConstraint<double>::ComplianceParameters p(1.0e3, 1.0e2);
...
GTEST_TEST(SapCouplerConstraint, SingleCliqueConstraintClone) {
  ...
  EXPECT_EQ(clone->compliance_parameters().dissipation_time_scale(),
            parameters.dissipation_time_scale());
}
...
GTEST_TEST(SapCouplerConstraint, SingleCliqueAccumulateGeneralizedImpulses) {
  // Make a distance constraint with an arbitrary kinamtics state and set of
  ...
  tau_clique1(kinematics.clique_dof1) = -gamma(0);

...
class DummyModel {
 public:
  ...
  MatrixXd MakeModelJacobian() const {
    ...
    J.block(0, 0, 3, 2) = MakeJacobian(3, 2);

    // Constraint 1. Between clique 1 and 2, cluster 1.
    J.block(3, 2, 5, 3) = MakeJacobian(5, 3);
    J.block(3, 5, 5, 4) = MakeJacobian(5, 4);

    // Constraint 2. Within clique 0, cluster 0.
    J.block(8, 0, 3, 2) = MakeJacobian(3, 2);
...
TEST_F(DummyModelTest, ImpulsesPermutation) {
  ...
  sap_model_->GetMutableVelocities(context_.get()) = v_permuted;
  ...
  // We generated the expected result with a call to EvalConstraintVelocities().
  ...
  VectorXd vc_permuted = vc;
  EXPECT_FALSE(CompareMatrices(vc_permuted, vc_expected, kEpsilon,
                              MatrixCompareType::relative));

  // We now verify the resulf of applying the permutation.

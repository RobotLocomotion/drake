...
class DummyModelTest : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    EXPECT_EQ(sap_problem_->num_constraints(), 4);
    EXPECT_EQ(sap_problem_->num_constraint_equations(), 16);
    sap_model_ = std::make_unique<SapModel<double>>(sap_problem_.get());
    context_ = sap_model_->MakeContext();
...
TEST_F(DummyModelTest, CostGradients) {
  ...
  EXPECT_EQ(sap_problem_ad->num_constraints(), 4);
  EXPECT_EQ(sap_problem_ad->num_constraint_equations(), 16);
  auto sap_model_ad =

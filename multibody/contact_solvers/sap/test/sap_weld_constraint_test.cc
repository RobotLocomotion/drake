...
GTEST_TEST(SapWeldConstraint, SingleCliqueConstraintClone) {
  ...
  EXPECT_EQ(clone->parameters().impulse_upper_limits(),
            kInf * Vector6d::Ones());
  EXPECT_EQ(clone->parameters().stiffnesses(), kInf * Vector6d::Ones());
  EXPECT_EQ(clone->parameters().relaxation_times(), Vector6d::Zero());
  // This value is hard-coded in the source. This test serves as a brake to
  // prevent the value changing without notification. Changing this value
  // would lead to a behavior change and shouldn't happen silently.
  EXPECT_EQ(clone->parameters().beta(), 0.1);
  EXPECT_EQ(clone->parameters().num_constraint_equations(), 6);
}
...
GTEST_TEST(SapWeldConstraint, AccumulateSpatialImpulses) {
  ...
  const SpatialForce<double> gamma_BNo_N(gamma.template head<3>(),
                                         gamma.template tail<3>());

  const RotationMatrix<double>& R_WN = c.kinematics().X_WN().rotation();

  // Expected spatial impulse on B.
  const Vector3d& p_NoBo_N = c.kinematics().X_NB().translation();
  const SpatialForce<double> gamma_BBo_W = R_WN * gamma_BNo_N.Shift(p_NoBo_N);

  // Expected spatial impulse on A.
  const Vector3d& p_NoAo_N = c.kinematics().X_NA().translation();
  SpatialForce<double> gamma_AAo_W = -(R_WN * gamma_BNo_N.Shift(p_NoAo_N));

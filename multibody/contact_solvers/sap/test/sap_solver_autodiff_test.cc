...
GTEST_TEST(SapAutoDiffTest, SolveWithGuess) {
  ...
  SapContactProblem<AutoDiffXd> contact_problem_without_constraint(
      kTimeStep, std::move(A), std::move(v_star));
  const VectorX<AutoDiffXd> v_guess = VectorX<AutoDiffXd>::Zero(kNumDofs);
  ...
  constexpr int kNumConstraints = 3;
  ...
  MatrixX<AutoDiffXd> J = MatrixX<AutoDiffXd>::Ones(kNumConstraints, kNumDofs);

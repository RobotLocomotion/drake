...
typename SapDistanceConstraint<T>::Parameters MakeArbitraryParameters() {
  const T stiffness = 1.0e5;
  const T damping = 1.0e3;
  return typename SapDistanceConstraint<T>::Parameters{stiffness, damping};
}
...
GTEST_TEST(SapDistanceConstraint, SingleCliqueConstraint) {
  ...
  const SapDistanceConstraint<double>::Parameters parameters =
  ...
  EXPECT_EQ(c.parameters().damping(), parameters.damping());
}
...
void ValidateProjection(const SapDistanceConstraint<double>::Parameters& p,
                        const Vector1d& vc) {
  SapDistanceConstraint<AutoDiffXd>::Parameters p_ad(p.stiffness(),
                                                     p.damping());
...
GTEST_TEST(SapDistanceConstraint, Gradients) {
  ...
  SapDistanceConstraint<double>::Parameters p(1.0e3, 1.0e2);
...
GTEST_TEST(SapDistanceConstraint, SingleCliqueConstraintClone) {
  ...
  EXPECT_EQ(clone->parameters().damping(), parameters.damping());
}

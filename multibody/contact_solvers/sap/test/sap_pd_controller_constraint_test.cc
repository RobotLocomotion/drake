...
/* SapPdControllerConstraint models (without considering effort limits) the
 ...
 achieve that effort, as:
   v = (u₀ − u − Kp⋅(q₀-qd)) / (δt⋅Kp + Kd)
 This value of velocity might exceed effort limits. */
...
bool AreConfigurationsEqual(const Configuration& a, const Configuration& b) {
...
bool AreParametersEqual(const Parameters& a, const Parameters& b) {
...
GTEST_TEST(SapPdControllerConstraint, Construction) {
  ...
  EXPECT_TRUE(AreConfigurationsEqual(c.configuration(), configuration));

  // Parameters at construction.
  EXPECT_TRUE(AreParametersEqual(c.parameters(), parameters));
  ...
  const MatrixX<double> J_expected =
      VectorXd::Unit(configuration.clique_nv, configuration.clique_dof);
...
// regime.
GTEST_TEST(SapPdControllerConstraint, MakeData_OutsideTheNearRigidRegime) {
  const double dt = 0.015;
  const Parameters p = MakeArbitraryParameters();
  // The configuration is irrelevant for this test.
  const Configuration k = MakeArbitraryConfiguration();
  const SapPdControllerConstraint<double> c(k, p);
  // Zero Delassus, will always lead to be outside the near-rigid regime.
  ...
  EXPECT_EQ(data.Kp(), p.Kp());
  EXPECT_EQ(data.Kd(), p.Kd());
  EXPECT_EQ(data.time_step(), dt);
}

// Unit test SapPdControllerConstraint::MakeData() in the near-rigid regime.
GTEST_TEST(SapPdControllerConstraint, MakeData_NearRigidRegime) {
  ...
  const Configuration k = MakeArbitraryConfiguration();
  const SapPdControllerConstraint<double> c(k, p);
  // Non-zero Delassus. Given the large values of the gains, the constraint will
  // be in the near-rigid regime.
  const VectorXd w = Vector1d::Constant(1.6);
  ...
  const auto& data =
      abstract_data->get_value<SapPdControllerConstraintData<double>>();
  EXPECT_NEAR(data.Kp(), Kp_nr, kEps);
  EXPECT_NEAR(data.Kd(), Kd_nr, kEps);
...
GTEST_TEST(SapPdControllerConstraint, Clone) {
  ...
  EXPECT_TRUE(AreConfigurationsEqual(clone->configuration(), configuration));
  EXPECT_TRUE(AreParametersEqual(clone->parameters(), parameters));
}

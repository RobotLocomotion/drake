...
eliminated clique now become single clique constraints.
  ...
  0[2](2)└──└┬─┬┘     └───┘
         ...
         0[1, 3](6,5)
*/
GTEST_TEST(ContactProblem, Reduce) {
  const double time_step = 0.01;
  const std::vector<MatrixXd> A{S22, S33, S44, S22};
  const VectorXd v_star = VectorXd::LinSpaced(11, 1.0, 11.0);
  SapContactProblem<double> problem(time_step, std::move(A), std::move(v_star));
  ...
  ReducedMapping mapping =
      problem.Reduce(locked_indices, clique_locked_indices);

  EXPECT_EQ(problem.num_cliques(), 2);
  EXPECT_EQ(problem.num_constraints(), 4);
  EXPECT_EQ(problem.num_constraint_equations(), 14);
  // Verify some expected data.
  EXPECT_EQ(problem.v_star().size(), 5);
  // Two cliques eliminated, so two remain.
  EXPECT_EQ(problem.dynamics_matrix().size(), 2);
  // Original clique 2 (reduced clique 0) is unaffected.
  EXPECT_EQ(problem.dynamics_matrix()[0], S44);
  // Original clique 3 (reduced clique 1) has only 1 dof now.
  EXPECT_EQ(problem.dynamics_matrix()[1].size(), 1);
  EXPECT_EQ(problem.dynamics_matrix()[1](0, 0), 2);
  ...
  {
    const SapConstraint<double>& c = problem.get_constraint(0);
   ...
   */
  {
    const SapConstraint<double>& c = problem.get_constraint(1);
    EXPECT_EQ(c.num_cliques(), 1);
    EXPECT_EQ(c.first_clique(), mapping.clique_permutation.permuted_index(3));
  ...
  */
  {
    const SapConstraint<double>& c = problem.get_constraint(2);
    EXPECT_EQ(c.num_cliques(), 1);
    EXPECT_EQ(c.first_clique(), mapping.clique_permutation.permuted_index(3));
  ...
  */
  {
    const SapConstraint<double>& c = problem.get_constraint(3);
    EXPECT_EQ(c.num_cliques(), 1);
    EXPECT_EQ(c.first_clique(), mapping.clique_permutation.permuted_index(3));

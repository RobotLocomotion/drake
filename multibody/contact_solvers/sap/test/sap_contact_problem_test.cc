...
GTEST_TEST(ContactProblem, AddConstraints) {
  ...
  // Expected constraint velocity indicies.
  const std::vector<int> constraint_velocities_start_expected{0, 1, 4, 10, 12};
  for (int i = 0; i < problem.num_constraints(); ++i) {
    EXPECT_EQ(problem.constraint_velocities_start(i),
              constraint_velocities_start_expected[i]);
  }
...
GTEST_TEST(ContactProblem, ExpandContactSolverResults) {
  ...
  for (int i = 0; i < problem.num_constraints(); ++i) {
    ...
    vc_expected.segment(problem.constraint_velocities_start(i),

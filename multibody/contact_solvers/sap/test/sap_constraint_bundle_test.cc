...
// projection function such that as gamma = -param * vc.
template <typename T>
class TestConstraint final : public SapConstraint<T> {
 public:
  ...
  std::unique_ptr<SapConstraint<T>> Clone() const final {
    return std::make_unique<TestConstraint<T>>(*this);
  }
...
TEST_F(SapConstraintBundleTest, VerifyGradients) {
  ...
  // Extract Hessian computed by automatic differentation of gamma.

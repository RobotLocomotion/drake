...
class PizzaSaverTest
 ...
 public:
  ...
  // the saver will start sliding. For this set up the transition occurs at
...
TEST_P(PizzaSaverTest, StictionAtTightOptimalityTolerance) {
  ...
  // method since we specified aboves zero tolerances.
...
TEST_P(SapNewtonIterationTest, GuessWithinLimits) {
  ...
  if (params.line_search_type ==
    ...
    // (m=0..max_iterations), we set ls_alpha_max = 1.0/ls_rho so that alpha_1 =

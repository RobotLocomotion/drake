...
SapWeldConstraint<T>::MakeSapHolonomicConstraintKinematics(
  ...
  // Consider the cosntraint velocity ġ = (ȧ_PQ, ṗ_PoQo_N). The translational
  // component:
  ...
  // conditioning, etc. We choose M = W, the world frame, for convenience.
  // The angular component:
  //   ȧ_PQ = d(θk)/dt = dθ/dt⋅k + θ⋅dk/dt
  // From [Mitiguy 2022, §9.2.2]:
  //   θ⋅ᴮdk/dt = θ/2⋅[(cos(θ/2)/sin(θ/2))⋅(w_AB − dθ/dt⋅k) + k×w_AB)]
  // We make a small angle assumption θ ≈ 0 to get:
  //   θ⋅ᴮdk/dt ≈ w_AB − dθ/dt⋅k + θ/2⋅k×w_AB
  // Thus:
  //   ȧ_PQ ≈ dθ/dt⋅k + w_AB − dθ/dt⋅k + θ/2⋅k×w_AB
  //          = w_AB + θ/2⋅k×w_AB
  ...
  // small angle assumption the frame dependent terms drop out.
  // Then:
  //   ġ = (ȧ_PQ, ṗ_PoQo_N) = (w_AB, v_W_PoQo_N) = V_W_PQ_N
  //
  // Thus the constraint Jacobian is the relative spatial velocity jacobian
  // between P and Q expressed in frame N:
  //  ġ = V_PQ_N = R_NW ⋅ V_W_PQ = (R_NW ⋅ J_W_PQ) ⋅ v = J ⋅ v
  //

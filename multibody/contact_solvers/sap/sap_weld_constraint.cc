...
SapWeldConstraint<T>::Kinematics::Kinematics(
  ...
  // Rodrigues formula states that the roation matrix is:
  //   R = I + sin(θ)⋅K + (1−cos(θ))⋅K²
  // where K = λ× is the skew-symmetric cross product matrix
  // (such that λ×⋅v = λ×v), and where θ and λ are the angle and axis
  // representation of of rotation matrix R.
  //
  // For small rotation angles, we can approximate sin(θ)≈θ and 1−cos(θ)≈θ²/2.
  // Neglecting second order terms in θ we then write R = I + a×, where we
  // defined a = θλ. Using this approximation, we then can obtain the components
  // of a× from the antisymmetric part of R as a× = (R-Rᵀ)/2. The non-zero
  // entries in a× correspond to the components of a.

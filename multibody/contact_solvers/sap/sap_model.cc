...
SapModel<T>::SapModel(const SapContactProblem<T>* problem) : problem_(problem) {
  ...
  VectorX<T> vhat(sap_problem().num_constraint_equations());
  VectorX<T> R(sap_problem().num_constraint_equations());
...
BlockSparseMatrix<T> SapModel<T>::MakeConstraintsBundleJacobian(
  ...
  std::vector<int> group_dofs(graph.num_clusters(), 0);
  for (int e = 0; e < graph.num_clusters(); ++e) {
    const auto& edge = graph.get_cluster(e);
    for (int k : edge.constraint_index()) {
      const SapConstraint<T>& c = problem.get_constraint(k);
      group_dofs[e] += c.num_constraint_equations();
    }
  }
  ...
  for (int block_row = 0; block_row < graph.num_clusters(); ++block_row) {
    const auto& constraint_group = graph.get_cluster(block_row);
    ...
    int row_start = 0;
    for (int i : constraint_group.constraint_index()) {
      ...
      J0.middleRows(row_start, ni) =
            c0 == c.first_clique() ? c.first_clique_jacobian() : c.second_clique_jacobian();
      if (c1 != c0) {
        J1.middleRows(row_start, ni) =
            c1 == c.first_clique() ? c.first_clique_jacobian() : c.second_clique_jacobian();
      }

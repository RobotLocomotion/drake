...
void SapModel<T>::DeclareCacheEntries() {
  ...
  const auto& hessian_cache_entry = system_->DeclareCacheEntry(
      ...
      systems::ValueProducer(
          this, &SapModel<T>::CalcHessianCacheAlongWithImpulsesCache),
      {system_->cache_entry_ticket(
          system_->cache_indexes().constraint_velocities)});
...
template <typename T>
void SapModel<T>::CalcHessianCacheAlongWithImpulsesCache(
    const systems::Context<T>& context, std::vector<MatrixX<T>>* G) const {
  system_->ValidateContext(context);

  // This method also updates the ImpulsesCache in the process. We use the same
  // memory and mark it up-to-date below.
  const auto& cache_entry =
      system_->get_cache_entry(system_->cache_indexes().impulses);
  auto& value = cache_entry.get_mutable_cache_entry_value(context);
  // This is required by GetMutableValueOrThrow() whenever the value is already
  // up-to-date. Essentially when the entry is up-to-date we simply recompute it
  // if the gradient G is requested.
  value.mark_out_of_date();
  auto& impulses_cache =
      value.template GetMutableValueOrThrow<ImpulsesCache<T>>();

  impulses_cache.Resize(num_constraint_equations());
  G->resize(num_constraints());
  const VectorX<T>& vc = EvalConstraintVelocities(context);
  constraints_bundle().CalcUnprojectedImpulses(vc, &impulses_cache.y);
  constraints_bundle().ProjectImpulsesAndCalcConstraintsHessian(
      impulses_cache.y, &impulses_cache.gamma, G);

  // We mark the impulses cache up-to-date.
  value.mark_up_to_date();
  // We also propagate the change to its dependents.
  const systems::DependencyTracker& tracker =
      context.get_tracker(cache_entry.ticket());

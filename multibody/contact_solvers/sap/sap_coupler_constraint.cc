...
template <typename T>
SapCouplerConstraint<T>::ComplianceParameters::ComplianceParameters(
    T stiffness, T dissipation_time_scale)
    : stiffness_(std::move(stiffness)),
      dissipation_time_scale_(std::move(dissipation_time_scale)) {
  DRAKE_DEMAND(stiffness_ > 0.0);
  DRAKE_DEMAND(dissipation_time_scale_ >= 0.0);
}
...
SapCouplerConstraint<T>::MakeSapHolonomicConstraintParameters(
  ...
  return typename SapHolonomicConstraint<T>::Parameters{
      ...
      Vector1<T>(p.dissipation_time_scale()), kBeta};
}
...
void SapCouplerConstraint<T>::DoAccumulateGeneralizedImpulses(
  ...
  // The sign convention of gamma is important here.
  if (c == 0) {
    (*tau)(kinematics().clique_dof0) += gamma(0);
  }

  if (this->num_cliques() == 1 || c == 1) {
    (*tau)(kinematics().clique_dof1) -= gamma(0);
  }
}

...
class SapPdControllerConstraintData {
 public:
  ...
  /* Constructs data for a SapPdControllerConstraintData.
    Refer to SapPdControllerConstraintData's documentation for further details.
    @param Kp Proportional gain.
    @param Kd Derivative gain.
    @param time_step Discrete simulation time step. */
  SapPdControllerConstraintData(T Kp, T Kd, T time_step)
      : parameters_{Kp, Kd, time_step} {}

  // Const data after construction.
  const T& Kp() const { return parameters_.Kp; }
  const T& Kd() const { return parameters_.Kd; }
...
/* Implements a SAP constraint to model a PD controller with effort limits.
 ...
 function such that it models the actuation force u from a PD controller
 according to:
...
class SapPdControllerConstraint final : public SapConstraint<T> {
 public:
  ...
  struct Configuration {
    /* Clique index. */
    int clique;
    /* PD controlled dof within the given clique. */
    int clique_dof;
 ...
 private:
  ...
  /* Computes the antiderivative of the Clamp() function. */

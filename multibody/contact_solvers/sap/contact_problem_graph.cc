...
int ContactProblemGraph::AddConstraint(SortedPair<int> cliques_pair,
                                       int num_constraint_equations) {
  auto [iterator, new_cluster] = pair_to_cluster_index_.insert(
      std::make_pair(cliques_pair, clusters_.size()));
  if (new_cluster) clusters_.emplace_back(std::move(cliques_pair));
...
int ContactProblemGraph::AddConstraint(int clique,
  ...
  return AddConstraint(std::move(drake::MakeSortedPair(clique, clique)),
...
int ContactProblemGraph::AddConstraint(int first_clique, int second_clique,
  ...
  DRAKE_THROW_UNLESS(num_constraint_equations >= 0);
  return AddConstraint(std::move(MakeSortedPair(first_clique, second_clique)),
                       num_constraint_equations);
}

...
class SapContactProblem {
 public:
  ...
  /* Makes a "reduced" contact problem given the DOFs specified in
    ...
    @pre known_free_motion_dofs is a sorted subset of
         [0, ..., num_velocities()-1].
  ...
  */
  ...
  vc.segment(constraint_velocity_start(c), num_constraint_equations(c))
  corresponds to the vector of constraint velocities for the c-th constraint.*/
  int constraint_velocity_start(int constraint_index) const {
    DRAKE_THROW_UNLESS(0 <= constraint_index &&
                       constraint_index < num_constraints());
    return constraint_velocity_start_[constraint_index];
  }
 ...
 private:
  ...
  std::vector<int> constraint_velocity_start_;

...
class SapConstraint {
  ...
  */
  ...
  /* Creates a "reduced" clone of this constraint by removing known DoFs from
   ...
   where participates(c) = clique_permutation.participates(c) and
   permuted(x) = clique_permutation.permuted_index(c). For convenience
   c.first_clique = i  c.second_clique = j and cᵣ.first_clique = iᵣ
   cᵣ.second_clique = jᵣ:

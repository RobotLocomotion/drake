 ...
 public:
  ...
  /* Constructor for a constraint among DOFs within a single `clique`.
   ...
   @param[in] g
     ...
     at the previous time step state of the mechanical system. g.size() will
     correspond to the number of constraint equations, see
  ...
  /* Computes the projection γ = P(y) onto the convex set specific to a
   ...
   @param[in,out] gamma On output, the projected impulse y. On input it must be
   of size num_constraint_equations().
   @param[in,out] Not used if nullptr. Otherwise it must be a squared matrix of
   size num_constraint_equations() to store the Jacobian dP/dy on output. */

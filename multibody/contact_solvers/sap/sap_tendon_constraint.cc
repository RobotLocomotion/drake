...
SapTendonConstraint<T>::Parameters::Parameters(const T& lower_limit,
  ...
  n_finite_ = lower_limit > -kInf && upper_limit < kInf ? 2 : 1;
}
...
SapConstraintJacobian<T> SapTendonConstraint<T>::CalcConstraintJacobian(
  ...
  if (ll > -kInf) J0.topRows(1) += kinematics.a0().transpose();
  if (ul < kInf) J0.bottomRows(1) -= kinematics.a0().transpose();

  if (kinematics.num_cliques() > 1) {
    MatrixX<T> J1 = MatrixX<T>::Zero(parameters.num_finite_limits(),
                                     kinematics.clique1_nv());

    if (ll > -kInf) J1.topRows(1) += kinematics.a1().transpose();
    if (ul < kInf) J1.bottomRows(1) -= kinematics.a1().transpose();

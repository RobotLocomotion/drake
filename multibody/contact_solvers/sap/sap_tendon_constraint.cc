...
SapTendonConstraint<T>::Parameters::Parameters(const T& lower_limit,
  ...
  n_eq_ = lower_limit > -kInf && upper_limit < kInf ? 2 : 1;
}

template <typename T>
SapTendonConstraint<T>::Kinematics::Kinematics(int clique0, int clique1,
                                               int clique0_nv, int clique1_nv,
...
template <typename T>
SapTendonConstraint<T>::Kinematics::Kinematics(int clique0, int clique0_nv,
                                               VectorX<T> q0, VectorX<T> a0,
                                               T offset)
    : clique0_(clique0),
      clique0_nv_(clique0_nv),
      q0_(std::move(q0)),
...
template <typename T>
bool SapTendonConstraint<T>::Kinematics::operator==(
    const Kinematics& other) const {
  if (other.num_cliques() != num_cliques()) return false;

  if (other.clique0_ != clique0_ || other.clique0_nv_ != clique0_nv_ ||
      other.q0_ != q0_ || other.a0_ != a0_ || other.offset_ != offset_) {
    return false;
  }

  if (num_cliques() == 2) {
    if (*other.clique1_ != *clique1_ || other.clique1_nv_ != clique1_nv_ ||
        other.q1_ != q1_ || other.a1_ != a1_) {
      return false;
    }
  }
  return true;
...
VectorX<T> SapTendonConstraint<T>::CalcConstraintFunction(
  ...
  VectorX<T> g0(parameters.num_equations());
...
SapConstraintJacobian<T> SapTendonConstraint<T>::CalcConstraintJacobian(
  ...
  MatrixX<T> J0 =
      MatrixX<T>::Zero(parameters.num_equations(), kinematics.clique0_nv());
  ...
  if (kinematics.num_cliques() > 1) {
    MatrixX<T> J1 =
        MatrixX<T>::Zero(parameters.num_equations(), kinematics.clique1_nv());
...
std::unique_ptr<AbstractValue> SapTendonConstraint<T>::DoMakeData(
  ...
  T k = parameters_.stiffness();
  T taud = parameters_.damping() / k;

  // Compliant regularization.
  const T R_compliant = 1.0 / (dt * k * (dt + taud));

  // "Near-rigid" regularization, [Castro et al., 2021].
  VectorX<T> R_near_rigid = beta_factor * delassus_estimation;

  // Effective regularization.
  VectorX<T> R_effective = (R_near_rigid).cwiseMax(R_compliant);
...
std::unique_ptr<SapConstraint<double>> SapTendonConstraint<T>::DoToDouble()
  ...
  if (k.num_cliques() > 1) {
    ...
    SapTendonConstraint<double>::Kinematics k_to_double(
        k.clique0(), k.clique0_nv(), math::DiscardGradient(k.q0()),

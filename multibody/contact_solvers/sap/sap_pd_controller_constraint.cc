...
std::unique_ptr<AbstractValue> SapPdControllerConstraint<T>::DoMakeData(
  ...
  //
  // Therefore, to minimize round-off errors, the criteria we use to choose from
  // (1*) or (2*) is to use the expression that makes Kp* (or Kd*) farthest from
  // zero. That is, we use (1*) whenever τ* < 1 and (2*) when τ* < 1. This
  ...
  // first gain using (1) (for Kp) when δt⋅Kp > Kd and (2) (for Kd) when δt⋅Kp <
  // Kd. The second gain is obtained from the condition to respect the user
  // provided Kp to Kd ratio.

...
/* N.B. This method makes a SapPdControllerConstraintData witt "effective"
 ...
 the solver decides clamp stiff values to condition the problem numerics better,
 without sacrificing accuracy (since the given time step size cannot resolve
 this fast dynamics anyway). When the specified time step size can resolve the
 dynamics of the controller, these effective values will equal the user provided
 Kp and Kd values. */
template <typename T>

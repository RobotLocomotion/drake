...
class BlockSparseCholeskySolver {
 public:
  ...
  /* Computes the block sparse Cholesky factorization of the given matrix `M`.
   ...
   We then define M̂ = P⋅M⋅Pᵀ, where P is the permutation matrix representation
   of p (i.e. Pᵢⱼ = 1 if j = p(i) and Pᵢⱼ = 0 otherwise). In other words, M̂ is
   permuted from M so that blocks with indices in `eliminated_blocks` appear on
   the top left corner of the matrix M̂ and all other blocks appear on the bottom
   right corner of the matrix. The matrix M̂ can be written in block form as M̂ =
   [D, B; Bᵀ, C] where D corresponds to the blocks with indices in E and C
   corresponds to the blocks with indices outside of E.

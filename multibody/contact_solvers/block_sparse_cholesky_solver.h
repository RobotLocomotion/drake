...
class BlockSparseCholeskySolver {
 public:
  ...
  /* Computes the block sparse Cholesky factorization of the given matrix `M`.
   Returns std::nullopt if the factorization fails. Failure is triggered by an
   internal failure of Eigen::LLT. This can fail if, for instance, the input
   matrix `M` is not positive definite. If failure is encountered, the user
   ...
   indices of `M` such that p(i) < p(j) iff

    (1) i ∈ E and j ∉ E or
    (2) i ∈ E and j ∈ E and i < j or
    (3) i ∉ E and j ∉ E and i < j.

   We then define M̂ = P⋅M⋅Pᵀ. In other words, M̂ is permuted from M so that
   blocks with indices in `eliminated_blocks` appear on the top left corner of
   the matrix M̂ and all other blocks appear on the bottom right corner of the
   matrix. The matrix M̂ can be written in block form as M̂ = [D, B; Bᵀ, A] where
   D corresponds to the blocks with indices in E and A corresponds to the blocks
   with indices outside of E.

   If the fatorization of M is successful, returns the Schur complement
   S = A - BᵀD⁻¹B.

   @pre `eliminated_blocks` has all its entries in [0, M.block_cols()).
   @post solver_mode() is SolverMode::kFactored if factorization is successful
   and is SolverMode::kEmpty otherwise. */
  [[nodiscard]] std::optional<MatrixX<double>> CalcSchurComplementAndFactor(
 ...
 private:
  ...
  /* Helper for Factor() and CalcSchurComplementAndFactor() to factorize part of
   the matrix. Does not set the solver mode. Unless
   [starting_col_block, ending_col_block) is equal to [0, L.block_cols()), this
   may leave L in an intermediate state. Call this function with care.

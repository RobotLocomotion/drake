...
class BlockSparseLowerTriangularOrSymmetricMatrix {
 public:
  ...
  /* If this matrix is symmetric, zeros out all row and column blocks whose
   block indices are included in `indices` and sets their diagonal blocks to be
   diagonal submatrices. This function doesn't change the sparsity pattern as
   it's forbidden by this class (see class documentation).

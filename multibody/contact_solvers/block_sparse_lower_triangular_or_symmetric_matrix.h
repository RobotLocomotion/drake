...
/* This class provides a representation for sparse matrices with a structure
 ...
 @tparam MatrixType   The Eigen matrix type of each block in block sparse
                      matrix, e.g. Matrix3<double> or MatrixX<double>. We
...
class BlockSparseLowerTriangularOrSymmetricMatrix {
 public:
  ...
  /* Adds Aij to the ij-th block of this matrix. If `this` matrix is symmetric,
   Aijᵀ is added to the ji-th block of `this` matrix.
   @pre Aij = Aij.transpose() if i==j and `this` matrix is symmetric. This
  ...
  /* Overwrites the ij-th block with `Aij`. If `this` matrix is symmetric,
   Aijᵀ is overwritten to the ji-th block of `this` matrix.
  ...
  /* Returns the ij-th block.
   @pre i >= j and HasBlock(i,j) == true.
   @note In the case of i < j, if the matrix is lower triangular, then the ij-th
   block is not non-zero; if the matrix is symmetric, one can obtain the Aij via
   Ajiᵀ. */
 ...
 private:
  /* Checks if the input block row and column indices and the corresponding
   matrix block is complies with the requirement of the class; throws an
   exception if not. Invocations provide a string explaining the origin of the
   violation. These checks are expensive and are not suitable for inner loop

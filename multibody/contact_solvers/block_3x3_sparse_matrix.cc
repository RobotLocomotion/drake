...
void Block3x3SparseMatrix<T>::TransposeAndMultiplyAndAddTo(
  ...
  for (int k = 0; k < block_rows_; ++k) {
    for (const Triplet& m : row_data_[k]) {
      const int i = std::get<1>(m);  // column index of M block
      const Matrix3<T>& Mb = std::get<2>(m);
      for (const Triplet& a : A.row_data_[k]) {
        const int j = std::get<1>(a);  // column index of A block
        const Matrix3<T>& Ab = std::get<2>(a);
        /* Notice the dummy index k in Mₖᵢ * Aₖⱼ is on the row index of M. Hence
         the M block needs to be transposed. */
        y->template block<3, 3>(3 * i, 3 * j) += Mb.transpose() * Ab;
      }
    }
  }
}

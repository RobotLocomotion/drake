...
void BlockSparseCholeskySolver<BlockType>::SetMatrix(const SymmetricMatrix& A) {
  ...
  for (int block_col = 0; block_col < ssize(A_block_pattern.neighbors());
       ++block_col) {
    DRAKE_THROW_UNLESS(!A_block_pattern.neighbors()[block_col].empty());
    DRAKE_THROW_UNLESS(A_block_pattern.neighbors()[block_col][0] == block_col);
  }
...
BlockSparseCholeskySolver<BlockType>::FactorAndCalcSchurComplement(
  ...
  SetMatrixImpl(M, elimination_ordering,
                SymbolicFactor(M, elimination_ordering));

  /* Reset solver mode and exit if factorization of the eliminated blocks fails.
   */
  if (!FactorImpl(0, num_eliminated_blocks)) {
  ...
  /* Finish the factorization. If the factorization fails, set the solver mode
   and exit. */
  bool success = FactorImpl(num_eliminated_blocks, L_->block_cols());
  solver_mode_ = success ? SolverMode::kFactored : SolverMode::kEmpty;
  if (!success) {
    return std::nullopt;
  }

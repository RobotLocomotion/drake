...
void DistanceConstraintsPool<T>::Set(int index, int bodyA, int bodyB,
  ...
  // Constraint function g₀ = d₀ − ℓ ∈ ℝ. Unlike the weld/ball, the distance
  // constraint carries the user's stiffness and damping.
...
Vector1<T> DistanceConstraintsPool<T>::CalcConstraintVelocity(
  ...
  // vc = ḋ = p̂ᵀ⋅(v_W_Bq − v_W_Ap), the rate of change of distance. The point
  // velocities are formed with one cross product each; the impulse acts at the
  // constraint points P and Q directly (no midpoint).
...
void DistanceConstraintsPool<T>::CalcSpatialImpulses(
  ...
  // The scalar impulse γ acts as the pure force γ⋅p̂ along the line PQ, applied
...
void DistanceConstraintsPool<T>::CalcHessianBlocks(int k, const T& R_inv,
  ...
  // The scalar impulse acts as the force (0, γ⋅p̂), so the effective 3×3
  // translational cost Hessian is the rank-1 Gt = R⁻¹⋅p̂⋅p̂ᵀ. Each body-space
  // block is G_Xp = Φ(p_X)ᵀ⋅diag(0, Gt)⋅Φ(p_X), but the anchor is the point X
  // (P or Q) itself — not a midpoint — since the collinear force conserves
  // angular momentum.
  const Vector3<T>& p_hat_W = p_hat_W_[k];
  ...
  if (G_Ap != nullptr) {
    ...
    // G_cross = −Φ(p_BQ)ᵀ⋅diag(0, Gt)⋅Φ(p_AP).
...
void DistanceConstraintsPool<T>::ReduceGeometryInto(
  ...
  // g₀ = d − ℓ is symmetric under swapping P and Q (handled by the base), but
  // the unit direction p̂ (from P to Q) negates. The anchor points swap.
  if (flip) {

...
void IcfModel<T>::ReduceInto(IcfModel<T>* reduced_model,
  ...
  for (int k = 0; k < num_bodies(); ++k) {
    ...
    if (is_anchored(k) ||
      ...
      // This body will not participate in the reduced problem, but we
      // allocate a dummy column in the Jacobian, following the pattern for
      // anchored bodies used in IcfBuilder::UpdateModel.
      reduced_params->body_to_clique[k] = -1;
      reduced_params->J_WB.Add(from.rows(), 1);
    } else {

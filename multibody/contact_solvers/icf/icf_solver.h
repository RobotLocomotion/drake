...
as per [Hairer and Wanner, 1996]. Norms are scaled by D = diag(M)⁻¹/², so that
...
class IcfSolver {
 ...
 private:
  ...
  @pre The equation in question must have one real root in [0, 1]. */
  ...
  bool hessian_factorization_is_fresh_{true};

...
void IcfData<T>::Resize(int num_bodies, int num_velocities, int max_clique_size,
  ...
  DRAKE_DEMAND(num_islands > 0 || num_velocities == 0);
  island_cost_.resize(num_islands);
  island_scratch_.reserve(num_islands);
  ...
  for (int i = 0; i < num_islands; ++i) {
    island_scratch_[i]->Resize(num_bodies, num_velocities, max_clique_size,
                               num_ball_constraints, num_couplers,
                               num_distance_constraints, num_welds, gain_sizes,

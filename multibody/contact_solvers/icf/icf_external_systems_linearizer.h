...
class IcfExternalSystemsLinearizer {
 ...
 private:
  ...
  struct Scratch {
    ...
    VectorX<T> tau_u_tilde0;  // Actuation at gu(x̃₀) at x̃₀ = x̃(v₀).
    VectorX<T> tau_e_tilde0;  // External forces ge(x̃₀) at x̃₀ = x̃(v₀).

    VectorX<T> x_prime;      // Perturbed plant state x′ for finite differences.
    VectorX<T> tau_u_prime;  // Perturbed actuation gu(x′) = B u(x′).
    VectorX<T> tau_e_prime;  // Perturbed external forces ge(x′) = τ_ext(x′).
  };

...
std::tuple<bool, bool> IcfExternalSystemsLinearizer<T>::LinearizeExternalSystem(
  ...
  const bool has_actuation_input =
      plant_.get_actuation_input_port().HasValue(plant_context) ||
      std::ranges::any_of(std::views::iota(0, plant_.num_model_instances()),
                          [this, &plant_context](int i) {
                            return plant_
                                .get_actuation_input_port(ModelInstanceIndex{i})
                                .HasValue(plant_context);
                          });
  const bool has_desired_state_input = std::ranges::any_of(
      std::views::iota(0, plant_.num_model_instances()),
      [this, &plant_context](int i) {
        return plant_.get_desired_state_input_port(ModelInstanceIndex{i})
            .HasValue(plant_context);
      });
  ...
  if (has_desired_state_input) {
    ...
    for (ModelInstanceIndex model_instance_index(0);
         model_instance_index < plant_.num_model_instances();
      ...
      if (desired_state.is_armed(model_instance_index)) {
        ...
        for (JointActuatorIndex actuator_index :
             plant_.GetJointActuatorIndices(model_instance_index)) {
          const JointActuator<T>& actuator =
              plant_.get_joint_actuator(actuator_index);
          const Joint<T>& joint = actuator.joint();
          if (actuator.has_controller() && !joint.is_locked(plant_context)) {
            ...
            bu[joint.velocity_start()] += -Kp * (q - qd) - Kd * vd;
          }
          ++a;
        }
      }

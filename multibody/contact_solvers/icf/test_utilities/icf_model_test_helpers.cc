...
void MakeModelReducible(IcfModel<T>* model,
  ...
  for (int v = 0; v < nv; ++v) {
    ...
    if ((remove_cursor >= ssize(dofs_to_remove)) ||
      ...
      r->per_clique_known_free_motion_dofs[clique_cursor].push_back(clique_v);
      ++remove_cursor;
    } else {
      DRAKE_UNREACHABLE();
    }

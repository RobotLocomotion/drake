...
optimization,
```math
  v_{n+1} = \min_v \ell(v;q_n, v_n, \delta t),
```
...
ensure that $v_{n+1}$ respects the discrete momentum balance
```math
...
Key components and how they talk to each other:
   ...
   - `IcfSolver`: Solves the convex problem itself, e.g., does Newton
     iterations.
...
Other components:
   ...
   - Constraint pools (`CouplerConstraintsPool`, `GainConstraintsPool`,
     `LimitConstraintsPool`, `PatchConstraintsPool`) are part of `IcfModel`, and
     hold constraints of various types.

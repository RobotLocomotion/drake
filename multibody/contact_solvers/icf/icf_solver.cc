...
double IslandWeightedNorm(const IcfModel<double>& model, int island,
  ...
  for (int c : model.partition().island_cliques(island)) {
    ...
    if (use_inverse) {
      // TODO(rpoyner-tri): use_inverse is always false; just delete the
      // branch, parameter, and doc?
      DRAKE_DEMAND(false);
...
bool IcfSolver::SolveWithGuess(const IcfModel<double>& model,
  ...
  // Solve each island as an independent convex subproblem. Each island owns its
  // IslandSolverState and writes only disjoint regions of `data`,
  // decision_variables_, and search_direction_, so the per-island solves do not
  // interfere.
  ...
  // Aggregate stats in island order: total iterations are the max over islands,
  // factorizations sum, the per-iteration vectors are concatenated, and
  // convergence is the AND over all islands. For a single island this is

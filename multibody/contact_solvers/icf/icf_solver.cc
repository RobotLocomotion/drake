...
bool IcfSolver::SolveWithGuess(const IcfModel<double>& model,
  ...
  for (int k = 0; k < parameters_.max_iterations; ++k) {
    ...
    if (k > 1) {
      ...
      // N.B. unlike in the Newton-Raphson steps discussed in [Hairer and
      // Wanner, 1996], it is possible that θ ≥ 1 without divergence, thanks to
      // linesearch.
      ...
      if (anticipated_residual > scaled_tolerance || theta >= 1.0) {
        ...
        hessian_factorization_is_fresh_ = false;
      }
    }
    ...
    bool reuse_hessian = parameters_.enable_hessian_reuse &&
                         ...
                         hessian_factorization_is_fresh_;
...
void IcfSolver::ComputeSearchDirection(const IcfModel<double>& model,
  ...
  if (parameters_.use_dense_algebra) {
    if (!reuse_factorization) {
      ...
      hessian_factorization_is_fresh_ = true;
    }

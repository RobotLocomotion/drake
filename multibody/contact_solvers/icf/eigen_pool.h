...
struct DynamicSizeStorage {
  ...
  @pre If either or both of RowsAtCompileTime or ColsAtCompileTime are not
  Eigen::Dynamic, then the respecive `rows` or `cols` argument must match the
  compile-time size. */
  void Resize(int num_elements, int rows, int cols) {
    Clear();
  ...
  @pre If either or both of RowsAtCompileTime or ColsAtCompileTime are not
  Eigen::Dynamic, then the respecive `rows` or `cols` argument must be empty. */
...
class EigenPool {
 public:
  ...
  respecive `rows` or `cols` argument must match the compile-time size. */
  void Resize(int num_elements, int rows, int cols) {
  ...
  of RowsAtCompileTime or ColsAtCompileTime are not Eigen::Dynamic, then the
  respecive `rows` or `cols` argument must be empty. */
  void Resize(int num_elements, std::span<const int> rows,
              std::span<const int> cols) {
    static_assert(EigenType::SizeAtCompileTime == Eigen::Dynamic);
    DRAKE_DEMAND(EigenType::RowsAtCompileTime == Eigen::Dynamic ||

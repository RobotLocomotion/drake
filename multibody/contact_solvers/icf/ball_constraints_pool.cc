...
Vector3<T> BallConstraintsPool<T>::CalcConstraintVelocity(
  ...
  // The stored constraint function g₀ is the relative translation p_PQ.
  const Vector3<T>& p_PQ = this->g0()[k];
  const Vector3<T> p_BoBm_W = p_BQ_W_[k] - 0.5 * p_PQ;
  ...
  if (V_WA != nullptr) {
    const Vector3<T> p_AoAm_W = p_AP_W_[k] + 0.5 * p_PQ;
...
void BallConstraintsPool<T>::ReduceGeometryInto(BallConstraintsPool<T>* reduced,
  ...
  // g₀ = p_PQ is handled (negated on flip) by the base; only the anchor points
  // swap here.
  if (flip) {

...
IcfBuilder<T>::IcfBuilder(const MultibodyPlant<T>* plant)
  ...
  // The number of bodies is known now, but masses must be computed from the
  // Context.
  body_mass_.resize(plant_.num_bodies());
}
...
void IcfBuilder<T>::UpdateModel(
  ...
  params->clique_sizes = plant_facts_.clique_sizes_;
  params->clique_start.resize(plant_facts_.clique_sizes_.size() + 1);
  params->clique_start[0] = 0;
  for (size_t c = 0; c < plant_facts_.clique_sizes_.size(); ++c) {
    params->clique_start[c + 1] =
        params->clique_start[c] + plant_facts_.clique_sizes_[c];
  }

  // Body mass is affected by context parameters, so needs to be refreshed
  // every step. TODO(rpoyner-tri): consider giving MultibodyPlant a cache
  // entry to track masses.
  for (int b = 0; b < plant_.num_bodies(); ++b) {
    ...
    // composite's masses.
    body_mass_[b] = body.get_mass(context);
    if (body_mass_[b] == 0.0) {
      DRAKE_DEMAND(false);  // XXX untested
      ...
      if (composite_mass == 0.0) {
        DRAKE_DEMAND(false);  // XXX untested
        // TODO(rpoyner-tri): write a unit test to reach this branch.
  ...
  // current state (q0, v0).
  params->body_to_clique = plant_facts_.body_to_clique_;
  params->body_is_floating = plant_facts_.body_is_floating_;
  ...
  J_WB.Resize(plant_.num_bodies(), 6, plant_facts_.body_jacobian_cols_);
...
void IcfBuilder<T>::UpdateFeedbackGains(
  ...
  DRAKE_DEMAND(gain_constraints.num_constraints() ==
               (model->num_cliques() * has_external_forces) +
                   (plant_facts_.num_actuation_constraints_ * has_actuation));
...
void IcfBuilder<T>::AllocateLimitConstraints(IcfModel<T>* model) const {
  ...
  limits.Resize(plant_facts_.limited_clique_sizes_);
}
  ...
  for (JointIndex joint_index : plant_.GetJointIndices()) {
    ...
    if (joint.num_positions() == 1 && joint.num_velocities() == 1) {
      ...
      if (!isinf(ql) || !isinf(qu)) {
        ...
        const int index = plant_facts_.clique_to_limit_constraint_[clique];
...
void IcfBuilder<T>::SetPatchConstraintsForHydroelasticContact(
  ...
  constexpr double kDefaultDissipation =
...
void IcfBuilder<T>::AllocateGainConstraints(IcfModel<T>* model,
  ...
  if (has_actuation) {
    for (int c = 0; c < model->num_cliques(); ++c) {
      if (plant_facts_.clique_nu_[c] > 0) {
        gain_constraint_sizes[constraint_index] = model->clique_size(c);
        ++constraint_index;
      }
    }
...
void IcfBuilder<T>::SetActuationGainConstraints(const VectorX<T>& Ku,
  ...
  for (int c = 0; c < model->num_cliques(); ++c) {
    ...
    const auto e_c = model->clique_segment(c, plant_facts_.effort_limits_);
...
IcfBuilder<T>::PlantFacts::PlantFacts(const MultibodyPlant<T>& plant) {
  ...
  for (int b = 0; b < plant.num_bodies(); ++b) {
    ...
    if (plant.IsAnchored(body)) {
      ...
      body_to_clique_[b] = tree_to_clique_[t];
    }
  ...
  effort_limits_.setConstant(kInf);
  ...
  for (JointIndex joint_index : plant.GetJointIndices()) {
    ...
    const int clique = tree_to_clique_[tree_index];
    const int clique_nv = clique_sizes_[clique];
    limited_clique_sizes_.push_back(clique_nv);
    clique_to_limit_constraint_[clique] = ssize(limited_clique_sizes_) - 1;
  }
}

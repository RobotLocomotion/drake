...
void IcfBuilder<T>::UpdateModel(
  ...
  if (params->body_mass.empty()) {
    ...
    params->clique_sizes = plant_facts_.clique_nvs;
    ...
    DRAKE_DEMAND(params->clique_sizes.size() == plant_facts_.clique_nvs.size());
  ...
  for (int dof : locking.unlocked_velocity_indices) {
    const int clique = plant_facts_.velocity_index_to_clique[dof];
    params->reduction.per_clique_unlocked_dofs[clique].push_back(
        dof - plant_facts_.clique_start_in_v[clique]);
  }
  ...
  for (int b = 0; b < plant_.num_bodies(); ++b) {
    ...
    if (!plant_.IsAnchored(body)) {
      ...
      const int clique_v_start = plant_facts_.clique_start_in_v[clique];
      const int clique_nv = plant_facts_.clique_nvs[clique];
...
void IcfBuilder<T>::SetCouplerConstraints(const systems::Context<T>& context,
  ...
  for (const auto& [id, spec] : specs_map) {
    ...
    const int clique0 = plant_facts_.velocity_index_to_clique[dof0];
    const int clique1 = plant_facts_.velocity_index_to_clique[dof1];
    ...
    const int clique_dof0 = dof0 - plant_facts_.clique_start_in_v[clique0];
    const int clique_dof1 = dof1 - plant_facts_.clique_start_in_v[clique1];
...
void IcfBuilder<T>::SetLimitConstraints(const systems::Context<T>& context,
  ...
  for (JointIndex joint_index : plant_.GetJointIndices()) {
    ...
    if (joint.num_positions() == 1 && joint.num_velocities() == 1) {
      ...
      const int clique = plant_facts_.velocity_index_to_clique[velocity_start];
      DRAKE_ASSERT(clique >= 0);
      const int clique_dof =
          velocity_start - plant_facts_.clique_start_in_v[clique];
...
IcfBuilder<T>::PlantFacts::PlantFacts(const MultibodyPlant<T>& plant) {
  ...
  // component of the movable mobods.
  //
  ...
  // movable mobod inherits its inboard mobod's clique. Anchored mobods carry
  // no velocities and belong to no clique.  Because mobods are numbered
  // depth-first, the velocity DOFs of a clique form a contiguous range,
  // matching ICF's per-clique contiguous-segment storage.
  ...
  velocity_index_to_clique.assign(nv, -1);
  ...
  for (int m = 1; m < num_mobods; ++m) {
    ...
    if (inboard.is_anchored()) {
      clique = ssize(clique_nvs);
      clique_nvs.push_back(0);
    } else {
      clique = mobod_to_clique[inboard.index()];
      DRAKE_DEMAND(clique >= 0);
    }
    mobod_to_clique[m] = clique;
    clique_nvs[clique] += mobod.nv();
    for (int k = 0; k < mobod.nv(); ++k) {
      velocity_index_to_clique[mobod.v_start() + k] = clique;
    }
  }
  ...
  // ascending v order; velocity_index_to_clique is non-decreasing exactly when
  // that holds, which we assert here.
  clique_start_in_v.assign(clique_nvs.size() + 1, 0);
  std::partial_sum(clique_nvs.begin(), clique_nvs.end(),
                   clique_start_in_v.begin() + 1);
  for (int v = 0; v < nv; ++v) {
    DRAKE_DEMAND(velocity_index_to_clique[v] >= 0);
    if (v > 0)
      DRAKE_DEMAND(velocity_index_to_clique[v] >=
                   velocity_index_to_clique[v - 1]);
    DRAKE_DEMAND(clique_start_in_v[velocity_index_to_clique[v]] <= v &&
                 v < clique_start_in_v[velocity_index_to_clique[v] + 1]);
  }
  ...
  for (int b = 0; b < plant.num_bodies(); ++b) {
    ...
    if (plant.IsAnchored(body)) {
      ...
      DRAKE_DEMAND(clique >= 0);
      body_jacobian_cols[b] = clique_nvs[clique];
  ...
  clique_nu.assign(clique_nvs.size(), 0);
  for (JointActuatorIndex actuator_index : plant.GetJointActuatorIndices()) {
    ...
    const Joint<T>& joint = actuator.joint();
    const int c = velocity_index_to_clique[joint.velocity_start()];
    DRAKE_ASSERT(c >= 0);
    ++clique_nu[c];
  }
  ...
  clique_to_limit_constraint.assign(clique_nvs.size(), -1);
  for (JointIndex joint_index : plant.GetJointIndices()) {
    ...
    const int clique = velocity_index_to_clique[joint.velocity_start()];
    const int clique_nv = clique_nvs[clique];

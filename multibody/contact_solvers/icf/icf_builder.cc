...
void IcfBuilder<T>::SetPatchConstraintsForPointContact(
  ...
  for (int point_pair_index = 0; point_pair_index < num_point_contacts;
    ...
    BodiesSortedByAnchorage sorted = SortBodiesByAnchorage(bodyM, bodyN);
...
template <typename T>
typename IcfBuilder<T>::BodiesSortedByAnchorage
IcfBuilder<T>::SortBodiesByAnchorage(const RigidBody<T>* bodyM,
                                     const RigidBody<T>* bodyN) const {
  IcfBuilder<T>::BodiesSortedByAnchorage result;
  const bool M_anchored = plant_.IsAnchored(*bodyM);
  const bool N_anchored = plant_.IsAnchored(*bodyN);

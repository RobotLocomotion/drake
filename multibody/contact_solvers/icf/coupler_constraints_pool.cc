...
void CouplerConstraintsPool<T>::ReduceInto(
  ...
  for (int k = 0; k < num_constraints(); ++k) {
    ...
    const bool have_i = dof_permutation.participates(i);
    const bool have_j = dof_permutation.participates(j);
    if (!have_i && !have_j) {
    ...
    // above.
    const int r_i = have_i ? dof_permutation.permuted_index(i) : -1;
    const int r_j = have_j ? dof_permutation.permuted_index(j) : -1;

    // Fill in the reduced constraint.
    reduced_pool->constraint_to_clique_[r_n] = r_c;
    reduced_pool->dofs_[r_n] = std::make_pair(r_i, r_j);
    reduced_pool->gear_ratio_[r_n] = gear_ratio_[k];

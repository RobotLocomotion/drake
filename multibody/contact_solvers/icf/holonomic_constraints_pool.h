...
position p_AB of B relative to A, returns the equivalent spatial impulse J_A at
A:
  Γ_A = Φ(p_AB)⋅Γ_B,
...
template <typename T>
Vector6<T> ShiftSpatialImpulse(const Vector6<T>& Gamma_B, const Vector3<T>& p_AB) {
  Vector6<T>  Gamma_A;

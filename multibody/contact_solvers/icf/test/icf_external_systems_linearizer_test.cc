...
// MJCF model of a floating ball (no joints).
constexpr char kFloatingBodyXml[] = R"""(
<?xml version="1.0"?>
<mujoco model="ball">
  <worldbody>
    <body>
...
TEST_F(IcfExternalSystemsLinearizerTest, ExternalSpatialForce) {
  Parser(builder_.get()).AddModelsFromString(kFloatingBodyXml, "xml");
  plant_.Finalize();
  // std::vector<ExternallyAppliedSpatialForce>

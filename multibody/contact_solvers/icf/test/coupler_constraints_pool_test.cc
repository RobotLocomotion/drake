...
GTEST_TEST(CouplerConstraintsPool, Reduce) {
  ...
  auto check_reduced = [&](const std::vector<int>& locked_dofs) {
    ...
    model.ReduceInto(&reduced_model, &mapping);
    bool have_i = mapping.velocity_subsequence.participates(7);
    bool have_j = mapping.velocity_subsequence.participates(9);
    ...
    if (have_i && have_j) {
      ...
      ASSERT_NEAR(reduced_data_pool.gamma(0),
                  (v_hat - vc) / reduced_pool.R()[0], 1e-11);
      auto clique_v =

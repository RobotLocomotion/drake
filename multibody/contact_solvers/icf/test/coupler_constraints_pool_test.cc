...
GTEST_TEST(CouplerConstraintsPool, Reduce) {
  ...
  auto check_reduced = [&](const std::vector<int>& locked_dofs) {
    ...
    model.ReduceInto(&reduced_model, &mapping);
    bool have_i = mapping.velocity_permutation.participates(7);
    bool have_j = mapping.velocity_permutation.participates(9);
    int expected_constraints = (have_i || have_j);
    EXPECT_EQ(reduced_model.num_coupler_constraints(), expected_constraints);
    ...
    int full_clique = full_pool.constraint_to_clique()[0];
    int reduced_clique = reduced_pool.constraint_to_clique()[0];
    EXPECT_EQ(mapping.clique_permutation.permuted_index(full_clique),
              reduced_clique);
    int f_i = full_pool.dofs()[0].first;
    int f_j = full_pool.dofs()[0].second;
    int r_i = reduced_pool.dofs()[0].first;
    int r_j = reduced_pool.dofs()[0].second;
    const auto& clique_mapping = mapping.clique_dof_permutations[full_clique];
    if (have_i) {
    ...
    auto reduced_velocity =
        SelectRows(v, mapping.velocity_permutation.inverse_permutation());
    ...
    if (have_i && have_j) {
      ...
      // data from the full problem.
      EXPECT_NE(reduced_data_pool.gamma(0), full_data_pool.gamma(0));
      EXPECT_NE(reduced_data_pool.cost(), full_data_pool.cost());

      // To verify values, reverse the implementation algebra to examine
      // dof-dependent terms.
      auto v_hat = reduced_pool.g_hat()[0] / reduced_model.time_step();
      auto vc =
          reduced_data_pool.cost() * -2.0 / reduced_data_pool.gamma(0) + v_hat;
      // Sanity: this check does not depend on presence or absence of dofs.
      ASSERT_EQ(reduced_data_pool.gamma(0), (v_hat - vc) / reduced_pool.R()[0]);
      auto clique_v =
          reduced_model.clique_segment(reduced_clique, reduced_velocity);
      if (have_i) {
        EXPECT_NEAR(vc, clique_v(r_i), 1e-14);
      } else if (have_j) {
        EXPECT_NEAR(vc, -reduced_pool.gear_ratio()[0] * clique_v(r_j), 1e-14);
      }
    }
    ...
    const auto& reduced_w =
        SelectRows(w, mapping.velocity_permutation.inverse_permutation());
    double reduced_dcost{}, reduced_d2cost{};
    reduced_pool.CalcCostAlongLine(reduced_data_pool, reduced_w, &reduced_dcost,
                                   &reduced_d2cost);
    if (have_i && have_j) {
      EXPECT_NEAR(dcost, reduced_dcost, 1e-22);
      EXPECT_NEAR(d2cost, reduced_d2cost, 1e-21);
    } else {
      // Partially participating reduced constraint data does *not* match the
      // data from the full problem.
      EXPECT_NE(dcost, reduced_dcost);
      EXPECT_NE(d2cost, reduced_d2cost);

      // To verify values, reverse the implementation algebra to examine
      // dof-dependent terms.
      const double vw = reduced_dcost / -reduced_data_pool.gamma(0);
      // Sanity: this check does not depend on presence or absence of dofs.
      ASSERT_EQ(vw * vw, reduced_d2cost * reduced_pool.R()[0]);
      const auto clique_w =
          reduced_model.clique_segment(reduced_clique, reduced_w);
      if (have_i) {
        EXPECT_NEAR(vw, clique_w(r_i), 1e-14);
      } else if (have_j) {
        EXPECT_NEAR(vw, -reduced_pool.gear_ratio()[0] * clique_w(r_j), 1e-14);
      }
    }
  };

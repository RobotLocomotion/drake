...
GTEST_TEST(IcfFeedbackTest, Basic) {
  ...
  const IcfFeedback<double> dut(&plant);
...
// dynamics controller only changes slowly with q. Lacking such a test is
// probably a big reason we're running into this whole linearization bug /
// confusion snafu.

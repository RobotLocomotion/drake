...
GTEST_TEST(IcfPartitionTest, TwoDisjointPairs) {
  ...
  const auto c2i = p.clique_to_island();
  EXPECT_EQ(std::vector<int>(c2i.begin(), c2i.end()),
...
GTEST_TEST(IcfPartitionTest, RecomputeReusesStorageGrowOnly) {
  ...
  // A subsequent Compute() on a smaller problem must not allocate.
  const BlockSparsityPattern smaller_pattern = MakePattern(4, {{0, 1}});
  {
    drake::test::LimitMalloc guard;
    p.Compute(smaller_pattern);
  }
}
...
GTEST_TEST(IslandItemMapTest, NoIslands) {
  IslandItemMap map;
  // Empty build does not complain.
  map.Build(0, {});
  EXPECT_EQ(map.num_islands(), 0);
  EXPECT_EQ(map.num_items(), 0);
}

GTEST_TEST(IslandItemMapTest, AllAnchored) {
  IslandItemMap map;
  // A not-very-interesting problem.
  map.Build(3, std::vector<int>{-1, -1, -1});
  EXPECT_EQ(map.num_islands(), 3);
  EXPECT_EQ(map.num_items(), 0);
  EXPECT_EQ(map.items(0).size(), 0);
  EXPECT_EQ(map.items(1).size(), 0);
  EXPECT_EQ(map.items(2).size(), 0);
}

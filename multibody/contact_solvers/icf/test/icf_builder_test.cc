...
// Unit test that the builder checks the active status of each constraint and
// produces an IcfModel with only the active constraints.
GTEST_TEST(IcfBuilder, ConstraintActiveStatus) {
  ...
  // active status is forwarded through to the model. So bodies, joints, and
  ...
  // Add one ball constraint, one coupler constraint, one distance constraint
  // and one weld constraint.
  ...
  {
    builder.UpdateModel(*context, time_step, nullptr, nullptr, &model);
    ...
    EXPECT_EQ(model.ball_constraints_pool().num_constraints(), 1);
    EXPECT_EQ(model.coupler_constraints_pool().num_constraints(), 1);
    EXPECT_EQ(model.distance_constraints_pool().num_constraints(), 1);
    EXPECT_EQ(model.weld_constraints_pool().num_constraints(), 1);
  }

  // Set the distance and ball constraints to inactive and verify that only the
  // coupler and weld constraints show up in the model.
  {
    ...
    EXPECT_EQ(model.ball_constraints_pool().num_constraints(), 0);
    EXPECT_EQ(model.coupler_constraints_pool().num_constraints(), 1);
    EXPECT_EQ(model.distance_constraints_pool().num_constraints(), 0);
    EXPECT_EQ(model.weld_constraints_pool().num_constraints(), 1);
  }

  // Now disable the coupler and weld constraints and verify that the model
  // has 0 constraints.
  {
    ...
    EXPECT_EQ(model.ball_constraints_pool().num_constraints(), 0);
    EXPECT_EQ(model.coupler_constraints_pool().num_constraints(), 0);
    EXPECT_EQ(model.distance_constraints_pool().num_constraints(), 0);
    EXPECT_EQ(model.weld_constraints_pool().num_constraints(), 0);
  }

  // Reactivate all constraints and verify they show up in the model.
  {
    plant.SetConstraintActiveStatus(context.get(), distance_constraint_id,
                                    true);

...
class IcfModel {
  ...
  */
  ...
  const IslandItemMap& island_balls() const { return island_balls_; }
  const IslandItemMap& island_couplers() const { return island_couplers_; }
  const IslandItemMap& island_distances() const { return island_distances_; }
  ...
  @pre Sparsity pattern must be valid. */
  ...
  */
  ...
  Note that the `search_direction` must be the island-specific data.
  */
  T CalcCostAlongLine(const T& alpha, const IcfData<T>& data, int island,
                      const IcfSearchDirectionData<T>& search_direction,
                      T* dcost_dalpha, T* d2cost_dalpha2) const;
 ...
 private:
  ...
  IslandItemMap island_balls_;
  IslandItemMap island_couplers_;
  IslandItemMap island_distances_;

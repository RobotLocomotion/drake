...
class IcfModel {
 public:
  ...
  bool is_reducible() const {
    return params().r.is_valid && params().r.is_smaller;
  }
  ...
  */
  ...
  parameters at `params().r`. The mapping from full problem to reduced problem
  is summarized in `mapping`.
  ...
  parameters are both valid and describe a smaller problem.

  @pre reduced_model != nullptr
  @pre mapping != nullptr
  @pre params().r.is_valid == true
  */

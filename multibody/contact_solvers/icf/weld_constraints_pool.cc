...
void WeldConstraintsPool<T>::Set(int index, int bodyA, int bodyB,
  ...
  // Weld constraints always function as near-rigid, so we set infinite
  // stiffness so that HolonomicConstraintsPool treats it as near-rigid.
  Vector6<T> g0;
  g0 << a_PQ_W, p_PoQo_W;
  constexpr double kInf = std::numeric_limits<double>::infinity();
  this->SetCommon(index, bodyA, bodyB, g0, T(kInf), T(0.0));
}

template <typename T>
Vector6<T> WeldConstraintsPool<T>::CalcConstraintVelocity(
...
template <typename T>
void WeldConstraintsPool<T>::CalcSpatialImpulses(int k, const Vector6<T>& gamma,
                                                 Vector6<T>* Gamma_Bo,
  ...
  const Vector3<T> p_BoBm_W = p_BQ_W_[k] - 0.5 * p_PoQo;
  *Gamma_Bo = ShiftSpatialImpulse<T>(gamma, p_BoBm_W);

  // For body A: the spatial impulse at Am is −γ, shifted to Ao.
  // p_AoAm_W = p_AP_W + 0.5*p_PoQo_W
  if (Gamma_Ao != nullptr) {
    const Vector3<T> p_AoAm_W = p_AP_W_[k] + 0.5 * p_PoQo;
    *Gamma_Ao = ShiftSpatialImpulse<T>(Vector6<T>(-gamma), p_AoAm_W);
  }
}

template <typename T>
void WeldConstraintsPool<T>::CalcHessianBlocks(int k, const T& R_inv,
                                               Matrix6<T>* G_Bp,
                                               Matrix6<T>* G_Ap,
                                               Matrix6<T>* G_cross) const {
  // G = diag(R⁻¹) ∈ ℝ⁶ˣ⁶, with rotational and translational blocks Gr and Gt.
  const Matrix3<T> Gr = R_inv * Matrix3<T>::Identity();
  const Matrix3<T>& Gt = Gr;
  const Vector3<T>& p_PoQo = p_PoQo_W_[k];
  const Vector3<T> p_BoBm_W = p_BQ_W_[k] - 0.5 * p_PoQo;
...
template <typename T>
void WeldConstraintsPool<T>::ResizeGeometry(int num_constraints) {
...
template <typename T>
void WeldConstraintsPool<T>::ReduceGeometryInto(WeldConstraintsPool<T>* reduced,
                                                int k, bool flip) const {
  if (flip) {
    reduced->p_AP_W_.Add(3, 1) = p_BQ_W_[k];
    reduced->p_BQ_W_.Add(3, 1) = p_AP_W_[k];
    reduced->p_PoQo_W_.Add(3, 1) = -p_PoQo_W_[k];
  } else {

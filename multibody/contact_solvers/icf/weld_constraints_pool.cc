...
void WeldConstraintsPool<T>::ReduceInto(
  ...
  int reduced_size{0};
  for (int k = 0; k < num_constraints(); ++k) {
    // Determine whether constraint should be removed or modified, by counting
    // the relevant cliques that survive in the reduced problem. A body's
    // relevance is determined by the participation of its clique. A body whose
    // clique has been removed is effectively anchored to the world; any joint
    // velocities that could have affected it are prescribed to be 0.

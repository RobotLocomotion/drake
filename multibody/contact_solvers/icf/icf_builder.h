...
class IcfBuilder {
 ...
 private:
  ...
  between bodyA and bodyB, following the sign convention v_b_W = v_B_ss - v_A_ss
  used by PatchConstraintsPool. Each body's surface velocity is evaluated with
  its own outward normal (pointing away from that body into the other) and
  rotated from the body frame into the world frame. Returns the zero vector when
  neither body declares a surface velocity.

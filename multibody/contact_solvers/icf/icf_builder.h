...
class IcfBuilder {
  ...
  */
  ...
  @param actuation_feedback linearization data (Kᵤ, bᵤ) for
         actuation forces τ = clamp(-Kᵤ⋅v + b, e). May be nullptr.
  @param external_feedback linearization data (Kₑ, bₑ) for external
         forces τ = -Kₑ⋅v + bₑ. May be nullptr.
  @param model The IcfModel to update.

  Note that the presence/absence of actuation_feedback and external_feedback
  affects the allocation of gain constraints. In particular, sending nullptr for
  either or both erases any prior constraints related to those forces.
 ...
 private:
  ...
  /* Throws if the context provided to UpdateModel is not compatible with ICF.
   */
  ...
  int tree_to_clique(int tree_index) const {
    DRAKE_ASSERT(tree_index >= 0 &&
                 tree_index < ssize(plant_facts_.tree_to_clique_));
    return plant_facts_.tree_to_clique_[tree_index];
  }
  ...
  // Model properties that do not change unless the plant changes.
  // TODO(rpoyner-tri): either enforce or document const-ness.
  struct PlantFacts {
    ...
    std::vector<int> body_is_floating_;    // 1 if body is floating, else 0.
  ...
  // Mass is a function of the plant Context.

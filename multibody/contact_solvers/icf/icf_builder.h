...
class IcfBuilder {
 ...
 private:
  ...
  struct PlantFacts {
    ...
    std::vector<int> velocity_index_to_clique;  // clique of each velocity DOF.
    std::vector<int> clique_nvs;                // nv for each clique.
    std::vector<int> clique_start_in_v;  // Starting index in v for each clique.

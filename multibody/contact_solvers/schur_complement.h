...
/* Given a symmetric linear system of equations Az = b in block form as:
 ...
 matrix A. Since A is positive definite, so is the S.

 Given the symmetric matrix A and information on how to decompose M into
 submatrices B, C, and D, this class computes the Schur complement of the block
 D of A. The matrix is factorized in the process and consequently, one can solve
 the system Az = b efficiently once the Schur complement is computed. */
class SchurComplement {
 public:
  ...
  3*N-by-3*N consisting of blocks of size 3x3. `D_indices` and `C_indices`
  determine which block rows/columns make up the D and C diagonal blocks.
  ...
  and D_indices = {0, 2} and C_indices = {1}, then submatrix D is given by

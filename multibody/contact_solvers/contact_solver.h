...
class ContactSolver {
 public:
  ...
  // @param dynamics_data Data describing the dynamics of the system.
  // @param contact_data Data describing the discrete contact set.
  ...
  virtual ContactSolverInfo SolveWithGuess(const T& time_step,
                                           SystemDynamicsData<T> dynamics_data,
                                           PointContactData<T> contact_data,
 ...
 protected:
  ...
  void FormDelassusOperatorMatrix(const LinearOperator<T>& G,
    ...
    // N.B. ei.makeCompressed() is not available for SparseVector.

...
class Block3x3SparseMatrix {
 public:
  ...
  /* Fills `this` matrix with the given vector of triplets. The input vector of
   ...
   same block row and column index) are summed. The contents in `this` matrix
   prior to the call to this function is destroyed.
 ...
 private:
  /* We store the non-zero blocks in the matrix in a row-major fashion. Within
   ...
   row_data_.size() is always equal to block_rows(). If row_data_[i] is empty,
   then it means that there's no non-zero block in that block row. */

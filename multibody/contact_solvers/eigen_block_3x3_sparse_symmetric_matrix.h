...
class EigenBlock3x3SparseSymmetricMatrix
 ...
 private:
  ...
  /* The connectivity pattern of the blocks in A. More specifically,
   `neighbors()[i][k]` gives the block column index of the k-th non-zero block
   in the i-th block row. `neighbors()[i]` is sorted for each block row i and
   each entry in `neighbors()[i]` is less than or equal to i. In other words,
   only the lower triangular part of the sparsity pattern is specified. As a
   result, we have `neighbors[i].back() = i` for each i because all diagonal

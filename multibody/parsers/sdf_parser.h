 ...
 * after a successful parse and one compilation depends on an input parameter,
 * `do_compile`. If `do_compile` is false, it is the responsibility of the
 * caller to ensure RigdBodyTree::compile() is invoked appropriately subsequent
 * to the successful parse.
 *

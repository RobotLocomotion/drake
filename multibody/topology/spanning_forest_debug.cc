...
void SpanningForest::SanityCheckForest() const {
  ...
  /* Check each composite to make sure:
     - the mobods it contains agree that they are contained in that composite
     - mobods welded to World know they are "anchored"
     - except for the World composite, the first entry in each composite
       Mobod has a non-weld inboard joint and all the others are Welds.
     - the first entry in each composite mobod should map to the link (the
       "active link") that is listed first in its composite link. */
  for (CompositeMobodIndex composite_index(0);
    ...
    for (MobodIndex index : composite) {
      const Mobod& mobod = mobods()[index];
      const bool is_active_mobod = index == composite[0];
      const bool should_be_weld =  // World or an interior body.
          composite_index == 0 || !is_active_mobod;
      ...
      if (is_active_mobod) {
        const LinkIndex active_link = mobod.link();
        const CompositeLinkIndex composite_link =
            links(active_link).composite();
        DRAKE_DEMAND(composite_link.is_valid());
        DRAKE_DEMAND(graph().composite_links(composite_link)[0] == active_link);
      }
    }
  }
}
...
void SpanningForest::DumpForest(std::string title) const {
  ...
  for (const auto& tree : trees()) {
    ...
    for (LinkIndex i : graph().FindPathFromWorld(tree.back().link()))
      std::cout << i << " ";
    std::cout << "\n";

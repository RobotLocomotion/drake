...
void SpanningForest::SanityCheckForest() const {
  ...
  DRAKE_DEMAND(!world_mobod.joint_ordinal().is_valid());
  ...
  DRAKE_DEMAND(world_mobod.welded_mobods_group().has_value());
  ...
  DRAKE_DEMAND(world_link.composite().has_value());
  DRAKE_DEMAND(world_link.composite() == LinkCompositeIndex(0));
  DRAKE_DEMAND(graph().link_composites()[0].links[0] == BodyIndex(0));

  for (MobodIndex index(1); index < ssize(mobods()); ++index) {
    const Mobod& mobod = mobods(index);
    DRAKE_DEMAND(mobod.index() == index);
    DRAKE_DEMAND(links(mobod.link_ordinal()).mobod_index() == index);
    DRAKE_DEMAND(joints(mobod.joint_ordinal()).mobod_index() == index);
  ...
  /* Check that ephemeral Links and Joints have assigned Mobods that point
  back to them. */
  for (LinkOrdinal link_ordinal(graph().num_user_links());
       link_ordinal < ssize(links()); ++link_ordinal) {
    const Link& link = links(link_ordinal);
    DRAKE_DEMAND(mobods(link.mobod_index()).HasFollower(link_ordinal));
  }
  for (JointOrdinal joint_ordinal(graph().num_user_joints());
       joint_ordinal < ssize(joints()); ++joint_ordinal) {
    const Joint& joint = joints(joint_ordinal);
    if (!joint.mobod_index().is_valid()) continue;  // Not modeled with Mobod
    DRAKE_DEMAND(mobods(joint.mobod_index()).joint_ordinal() == joint_ordinal);
  }

  /* Check that _all_ the Links and Joints match their assigned Mobods.
  (Repeating for the ephemeral ones.) */
  ...
  for (WeldedMobodsIndex welded_mobods_index(0);
    ...
    for (MobodIndex index : welded_mobods_group) {
      ...
      DRAKE_DEMAND(mobod.welded_mobods_group() == welded_mobods_index);
      ...
      if (is_active_mobod) {
        ...
        DRAKE_DEMAND(link_composite.has_value());
        DRAKE_DEMAND(graph().link_composites(*link_composite).links[0] ==
                     active_link.index());
      }
    }
  }
...
std::ostream& operator<<(std::ostream& o, const std::vector<LinkOrdinal>& v) {
...
std::ostream& operator<<(std::ostream& o, ForestBuildingOptions options) {
  if (options == ForestBuildingOptions::kDefault) {
    ...
    if ((options & ForestBuildingOptions::kUseRpyFloatingJoints) ==
      ...
      o << " UseRpy";
    if ((options & ForestBuildingOptions::kMergeLinkComposites) ==
...
void SpanningForest::DumpForest(std::string title) const {
  ...
  const auto& inst_options =
      graph().data_.model_instance_forest_building_options;
  for (ModelInstanceIndex inst{0}; inst < ssize(inst_options); ++inst) {
    if (!inst_options[inst].has_value()) continue;
    std::cout << " mi" << inst << ": " << options(inst);
  }
  std::cout << "\n  height=" << height() << "\n";
  std::cout << "\nWorld " << world_mobod().index()
            << (world_mobod().is_anchored() ? "A" : " ");
  std::cout << " L" << world_mobod().follower_link_ordinals() << "\n";
  for (const auto& tree : trees()) {
    ...
    const Link& last_link = links(tree.back().link_ordinal());
    for (BodyIndex i : graph().FindPathFromWorld(last_link.index()))
      std::cout << i << " ";
    std::cout << "\n";
    DumpForestImpl(tree.base_mobod(), 2);
  }

  std::cout << "ephemeral joint -> mobod:\n";
  for (JointIndex joint_index(graph().num_user_joints());
    ...
    std::cout << graph().joint_traits(joint.traits_index()).name;
    std::cout << " " << joint.name() << " -> ";
  ...
  if (!loop_constraints().empty()) {
    std::cout << "\n(ephemeral) loop constraints:\n";
    for (LoopConstraintIndex i(0); i < ssize(loop_constraints()); ++i) {
      const LoopConstraint& constraint = loop_constraints()[i];
      std::cout << fmt::format("{} mobods: {} {}  index: {}\n", i,
                               constraint.primary_mobod(),
                               constraint.shadow_mobod(), constraint.index());
    }
  }
...
void SpanningForest::DumpForestImpl(MobodIndex index, int level) const {
  ...
  std::cout << " L" << mobod.follower_link_ordinals();
  if (mobod.joint_ordinal().is_valid()) {
    std::cout << " J" << mobod.joint_ordinal();

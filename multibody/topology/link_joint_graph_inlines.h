...
inline auto LinkJointGraph::links(LinkOrdinal link_ordinal) const
    -> const Link& {
  DRAKE_ASSERT(link_ordinal < ssize(links()));
  return data_.links[link_ordinal];
}

inline auto LinkJointGraph::link_by_index(BodyIndex link_index) const
    -> const Link& {
  const std::optional<LinkOrdinal>& ordinal =
      data_.link_index_to_ordinal.at(link_index);
  if (!ordinal.has_value()) ThrowLinkWasRemoved(link_index);
  DRAKE_ASSERT(ordinal < ssize(links()));
  return links(*ordinal);
}

inline auto LinkJointGraph::mutable_link(LinkOrdinal link_ordinal) -> Link& {
  DRAKE_ASSERT(link_ordinal < ssize(links()));
  return data_.links[link_ordinal];
}
...
inline auto LinkJointGraph::joints(JointOrdinal joint_ordinal) const
    -> const Joint& {
  DRAKE_ASSERT(joint_ordinal < ssize(joints()));
  return data_.joints[joint_ordinal];
}

inline auto LinkJointGraph::joint_by_index(JointIndex joint_index) const
    -> const Joint& {
  const std::optional<JointOrdinal>& ordinal =
      data_.joint_index_to_ordinal.at(joint_index);
  if (!ordinal.has_value()) ThrowJointWasRemoved(joint_index);
  DRAKE_ASSERT(ordinal < ssize(joints()));
  return joints(*ordinal);
}

inline auto LinkJointGraph::mutable_joint(JointOrdinal joint_ordinal)
    -> Joint& {
  DRAKE_ASSERT(joint_ordinal < ssize(joints()));
  return data_.joints[joint_ordinal];
}

...
class SpanningForest {
 public:
  ...
  ephemeral Links.
  This is the same as `ssize(links())`. Links that have been removed are not
  counted. */
  ...
  [[nodiscard]] int num_mobods() const { return ssize(mobods()); }
  ...
  `ssize(trees())`. */
  [[nodiscard]] int num_trees() const { return std::ssize(trees()); }

...
collision computations and can (optionally) be optimized by modeling with a
single mobilized body.
...
may be eliminated by creating WeldedLinksAssemblies that require only a single
mobilizer for a group of Links.

The SpanningForest contains a Mobod node corresponding to each Link or
WeldedLinksAssembly. A mobilizer connects each Mobod to an inboard Mobod (except
for World). An additional "shadow" Link and its Mobod is created whenever a loop
is broken by cutting a Link, and a LoopConstraint is added to reconnect the
primary Link to its shadow. Every moving Joint will map to a Mobod and there
will be additional floating or weld joints added as needed to connect tree root
nodes (a.k.a. "base bodies") to World.
...
Things we get for free (O(1)) here as a side effect of building the Forest:
  ...
  - find out which Mobod (if any) represents a given Joint (welds internal
      to WeldedLinksAssemblies are not modeled)
  - find all the Links that follow a particular Mobod (for
...
class SpanningForest {
 public:
  ...
  into single bodies, then each WeldedLinksAssembly gets only a single Mobod and
  hence there won't be any WeldedMobods groups here (except for World which is
  always considered to be in a WeldedMobods group even if nothing is welded to
  it). Use mobod_to_links() to find all the Links following a single Mobod.
  ...
  That moving Mobod (the "active Mobod") is necessarily the lowest numbered
  (most inboard) Mobod of the WeldedMobods group. The remaining Mobods are in no
  particular order.
  ...
  /* Returns the indices of all quaternions within the generalized position

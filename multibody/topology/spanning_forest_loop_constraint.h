...
class SpanningForest::LoopConstraint {
 public:
  /** (Internal use only) Created in BuildForest when loops are cut. */
  LoopConstraint(LoopConstraintIndex loop_constraint_index,
    ...
    DRAKE_DEMAND(loop_constraint_index.is_valid() &&
                 parent_mobod_index.is_valid() && child_mobod_index.is_valid());
    DRAKE_DEMAND(parent_mobod_index != child_mobod_index);
  }

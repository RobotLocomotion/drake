...
void SpanningForest::HandleLoopClosure(JointIndex loop_joint_index) {
  ...
  a massless body. */
  if (parent_is_massless && child_is_massless && !data_.no_dynamics) {
    data_.no_dynamics = true;
  ...
  /* Prefer to split the child link unless the parent is a better choice. */
  const bool split_parent = child_is_massless && !parent_is_massless;

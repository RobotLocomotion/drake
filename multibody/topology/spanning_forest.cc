...
void SpanningForest::ChooseForestTopology() {
  ...
  World by the user, (b) Static Links for which we just added weld joints, and
  (c) MustBeBaseBody Links for which we just added floating joints. */
...
/* Grows each Tree by one level with two exceptions:
   - if we're optimizing composites, keep growing LinkComposites as far as
     possible since they don't contribute to numerics, and
   - if we encounter a "can't be terminal" Link (likely massless) we keep
     growing that Tree until we can end on something massful.
Note that a LinkComposite can be terminal if _any_ of its Links are massful. */

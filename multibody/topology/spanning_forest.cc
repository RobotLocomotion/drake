...
void SpanningForest::HandleLoopClosure(JointOrdinal loop_joint_ordinal) {
  ...
  if (split_link_is_massless && data_.dynamics_ok) {
    ...
    if (parent_is_world || child_is_world) {
      data_.why_no_dynamics = fmt::format(
          "Loop first identified between World and massless link {}, with "
          "joint {} needing to be retargeted to a shadow link. Since World "
          "can't be split we had to split {} instead, so its shadow is a "
          "terminal, massless body in the tree which will produce a singular "
          "mass matrix. Hence this model cannot be used for dynamics. "
          "Likely this is due to a loop composed only of joints that "
          "connect massless links and World; consider revising the model.\n",
          split_link.name(), loop_joint.name(), split_link.name());
    } else {
      data_.why_no_dynamics = fmt::format(
          "Loop first identified between two massless links {} and {}, with "
          "joint {} needing to be retargeted to a shadow link. Whichever one "
          "we split, its shadow is a terminal, massless body in the tree "
          "which will produce a singular mass matrix. Hence this model "
          "cannot be used for dynamics. Likely this is due to a loop composed "
          "only of joints that connect massless links; consider revising the "
          "model.\n",

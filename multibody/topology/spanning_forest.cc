...
void SpanningForest::FixupForestToUseNewNumbering(
  ...
  is an O(n) algorithm, and usually num_swaps << n, with a worst case of n-1
  swaps. (Hand-wavy proof: each iteration of the inner loop moves one Mobod to
  its final location from which it never leaves.) Also World is already in
  the right place so "n" here is the number of non-World Mobods. */
  const int max_swaps = std::max(0, num_mobods - 2);
  int num_swaps = 0;
  for (MobodIndex new_index(1);  // World is always right already.
       new_index < num_mobods && num_swaps < max_swaps; ++new_index) {

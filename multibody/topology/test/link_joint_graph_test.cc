...
GTEST_TEST(LinkJointGraph, FlagsAndOptions) {
  ...
  const auto use_fixed_base = ModelingOptions::kUseFixedBase;
  const auto combine_links = ModelingOptions::kCombineLinkComposites;
  auto modeling_options = use_fixed_base | combine_links;
  static_assert(std::is_same_v<decltype(modeling_options), ModelingOptions>);
  EXPECT_EQ(modeling_options & use_fixed_base, ModelingOptions::kUseFixedBase);
  EXPECT_EQ(modeling_options & combine_links,
            ModelingOptions::kCombineLinkComposites);
  EXPECT_FALSE(static_cast<bool>(modeling_options &
                                 ModelingOptions::kUseRpyFloatingJoints));
  EXPECT_EQ(modeling_options & ModelingOptions::kUseRpyFloatingJoints,
            ModelingOptions::kDefault);
...
GTEST_TEST(LinkJointGraph, CopyMoveAssignTest) {
  ...
  const SpanningForest* original_graph_forest_ptr = &graph.BuildForest();
  ...
  EXPECT_NE(graph_copy_forest, original_graph_forest_ptr);
...
GTEST_TEST(LinkJointGraph, SerialChain) {
  ...
  const SpanningForest& forest =
      graph.BuildForest(ModelingOptions::kDefault,
                        {{static_model_instance, ModelingOptions::kStatic}});
  ...
  graph.BuildForest(ModelingOptions::kCombineLinkComposites,
                    {{model_instance, ModelingOptions::kCombineLinkComposites},
                     {static_model_instance, ModelingOptions::kStatic}});
...
GTEST_TEST(LinkJointGraph, MultipleBranches) {
  ...
  EXPECT_EQ(forest.options(), ModelingOptions::kDefault);
  EXPECT_EQ(forest.options(model_instance), ModelingOptions::kDefault);
...
GTEST_TEST(LinkJointGraph, SimpleTrees) {
  ...
  graph.BuildForest(ModelingOptions::kUseRpyFloatingJoints);

...
GTEST_TEST(LinkJointGraph, SpecifyForestBuildingOptions) {
  ...
  EXPECT_EQ(graph.get_forest_building_options(default_model_instance()),
            default_options);
  EXPECT_EQ(graph.get_forest_building_options(ModelInstanceIndex(73)),
            default_options);

  // Global options should be used for any unrecognized model instance but
  // should be overridden by instance-specific options.
  graph.set_global_forest_building_options(two_options);
  graph.set_forest_building_options(ModelInstanceIndex(9),
                                    ForestBuildingOptions::kStatic);
  EXPECT_EQ(graph.get_global_forest_building_options(), two_options);
  EXPECT_EQ(graph.get_forest_building_options(ModelInstanceIndex(3)),
            two_options);  // Nothing set for this instance.
  EXPECT_EQ(graph.get_forest_building_options(ModelInstanceIndex(9)),
            ForestBuildingOptions::kStatic);  // Overridden.

  // Reset should put everything back as it was.
  graph.reset_forest_building_options();
  EXPECT_EQ(graph.get_global_forest_building_options(), default_options);
  EXPECT_EQ(graph.get_forest_building_options(ModelInstanceIndex(9)),
            default_options);  // Overridden.
}
...
GTEST_TEST(LinkJointGraph, CopyMoveAssignTest) {
  ...
  const SpanningForest* original_graph_forest_ptr = &graph.GetValidForest();
  ...
  EXPECT_TRUE(graph.forest().is_valid());
  graph.AddLink("link1", ModelInstanceIndex(19));  // Should invalidate forest.
  EXPECT_FALSE(graph.forest_is_valid());
  EXPECT_FALSE(graph.forest().is_valid());
  graph.GetValidForest();  // Update the forest.
  ...
  graph.GetValidForest();  // Mark forest valid again.
  EXPECT_TRUE(graph.forest_is_valid());
  EXPECT_EQ(ssize(graph.links()), 2);  // World + link1
  ...
  EXPECT_NE(&graph.GetValidForest(), original_graph_forest_ptr);
  ...
  EXPECT_NE(&graph_copy.GetValidForest(), graph_copy_forest_ptr);
}
...
GTEST_TEST(LinkJointGraph, EmptyTest) {
  ...
  const SpanningForest& forest = graph.GetValidForest();
  ...
  const SpanningForest& same_forest = graph.GetValidForest();

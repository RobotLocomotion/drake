...
GTEST_TEST(LinkJointGraph, SerialChain) {
  ...
  EXPECT_EQ(graph.links(LinkIndex(0)).model_instance(), world_model_instance());
  ...
  const SpanningForest::Mobod& mobod_for_link3 =
      forest.mobods(graph.links(LinkIndex(3)).mobod_index());
  EXPECT_EQ(mobod_for_link3.link(), LinkIndex(3));
  ...
  const SpanningForest::Mobod& mobod_for_base11 =
      forest.mobods(graph.links(LinkIndex(11)).mobod_index());
...
GTEST_TEST(LinkJointGraph, Weldedsubgraphs) {
  ...
  std::vector<std::set<LinkIndex>> expected_before_subgraphs;
  ...
  expected_before_subgraphs.push_back(std::set{LinkIndex(2)});
  ...
  EXPECT_EQ(forest.composite_mobods(CompositeMobodIndex(0))[0], MobodIndex(0));
}
...
GTEST_TEST(LinkJointGraph, SimpleTrees) {
  ...
  graph.AddJoint("world_joint_7", model_instance, kRevoluteType, LinkIndex(0),
                 LinkIndex(1));
  graph.AddJoint("world_joint_8", model_instance, kRevoluteType, LinkIndex(0),
                 LinkIndex(7));
  graph.BuildForest();
...
GTEST_TEST(LinkJointGraph, MasslessBodiesShareSplitLink) {
  ...
  graph.AddJoint("prismatic_1", model_instance, kPrismaticType, world_index(),
                 LinkIndex(2));
  graph.AddJoint("revolute_2", model_instance, kRevoluteType, LinkIndex(1),
                 LinkIndex(3));
  graph.AddJoint("revolute_3", model_instance, kRevoluteType, LinkIndex(2),
                 LinkIndex(3));

...
GTEST_TEST(MultibodyGraph, BodiesAffectedByJoints) {
  ...
  // Add joints.
  int j = 0;
  graph.AddJoint("joint" + std::to_string(j++), world_model_instance(),
                 graph.weld_type_name(), graph.world_body().index(),
                 BodyIndex(1));
  graph.AddJoint("joint" + std::to_string(j++), model_instance,
                 graph.weld_type_name(), BodyIndex(1), BodyIndex(2));
  graph.AddJoint("joint" + std::to_string(j++), model_instance,
                 graph.weld_type_name(), BodyIndex(2), BodyIndex(3));
  graph.AddJoint("joint" + std::to_string(j++), world_model_instance(),
                 graph.weld_type_name(), graph.world_body().index(),
                 BodyIndex(4));
  graph.AddJoint("joint" + std::to_string(j++), model_instance,
                 graph.weld_type_name(), BodyIndex(4), BodyIndex(5));
  graph.AddJoint("joint" + std::to_string(j++), model_instance,
                 graph.weld_type_name(), BodyIndex(5), BodyIndex(6));
  graph.AddJoint("joint" + std::to_string(j++), model_instance,
                 graph.weld_type_name(), BodyIndex(6), BodyIndex(4));

...
GTEST_TEST(SpanningForest, WeldedSubgraphs) {
  ...
  auto check_shadows = [&graph](LinkIndex primary_index, LinkIndex shadow_index,
    ...
    EXPECT_TRUE(primary.has_joint(joint_index));  // it's still there, but ...
    EXPECT_TRUE(primary.joint_has_moved_to_shadow_link(joint_index));
    EXPECT_TRUE(shadow.is_shadow());
    EXPECT_EQ(shadow.primary_link(), primary_index);
    EXPECT_TRUE(shadow.has_joint(joint_index));  // retargeted
    EXPECT_FALSE(shadow.joint_has_moved_to_shadow_link(joint_index));
  };
...
GTEST_TEST(SpanningForest, DoubleLoop) {
  ...
  EXPECT_FALSE(primary_link.joint_has_moved_to_shadow_link(JointIndex(5)));
  EXPECT_TRUE(primary_link.joint_has_moved_to_shadow_link(JointIndex(6)));
  EXPECT_TRUE(primary_link.joint_has_moved_to_shadow_link(JointIndex(7)));

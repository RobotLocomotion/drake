...
links (an "un-fused assembly"). In the latter case we also
...
are formed entirely of welds. When modeling in the un-fused mode where every
...
Therefore we expect the following un-fused WeldedLinksAssemblies, with the

...
class SpanningForest::Mobod {
 public:
  ...
  bool is_anchored() const {
    return composite_mobod_index_.is_valid() &&
           composite_mobod_index_ == CompositeMobodIndex(0);
  }

  /** Returns true if there is no Mobod that claims this one as its
  inboard Mobod. Even if this Mobod is not a leaf of its tree, it could still
  ...
  parent/child relation in the Joint that this Mobod is modeling. */
  ...
  /** Returns the index of this Mobod withing the SpanningForest. World has
  ...
  invalid if and only if this is the World Mobod. The inboard Mobod's level()
  ...
  Link, this is the "active" Link, the one whose (non-Weld) Joint connects the
  ...
  %Mobod. The first one is the "active" Link as returned by link(). There is
  always at least one Link. */
  ...
  a CompositeLink (which has internal Weld Joints), the Joint returned here
  is the "active" Joint that connects the Composite's active Link to its
  inboard Mobod. */
  ...
  /** Returns the index of the Composite Mobod of which this Mobod is a part,
  if any. If there are no Mobods welded to this one, then the returned index
  is invalid. */
  ...
  level 0, the root of each Tree is level 1, Mobods connected to the root are
  level 2, etc. */
  int level() const { return level_; }

  /** Starting offset within the contiguous q vector. */
  int q_start() const { return q_start_; }
  /** The number of position coordinates q used by this mobilizer. */
  int nq() const { return nq_; }

  /** Start offset within the contiguous v vector. */
  int v_start() const { return v_start_; }
  /** The number of velocity coordinates v used by this mobilizer. */

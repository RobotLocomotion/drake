...
class LinkJointGraph::Joint {
 public:
  ...
  /** (Internal use only) Don't construct these directly. */
  Joint(JointIndex index, const std::string& name,
    ...
    DRAKE_DEMAND(index.is_valid() && !name.empty() &&
                 model_instance_.is_valid() && joint_type_index.is_valid() &&
                 parent_link_index_.is_valid() && child_link_index_.is_valid());
  ...
  /** Returns `true` if this %Joint was added with JointFlags::MustBeModeled. */
  ...
  /** (Internal use only) If `to_set` is JointFlags::Default sets the flags to
 ...
 private:
  ...
  std::variant<std::monostate, MobodIndex, ConstraintIndex, LinkCompositeIndex,
               IgnoredLoopJoint>
      how_modeled_;
};

...
class SpanningForest::Tree {
 public:
  ...
  /** The level-1 Mobod that is the base (root) body of this %Tree. */
  ...
  int nv() const {
    ...
    // would be if they had v's.
    const int last_v_plus_1 = back().v_start() + back().nv();
    return last_v_plus_1 - v_start();
  }

  /** (Internal use only) Trees are constructed during BuildForest(). */

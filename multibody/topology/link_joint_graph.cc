...
void LinkJointGraph::InvalidateForest() {
  ...
  // Remove any ephemeral elements from the graph. The corresponding indexes
  // will appear contiguously at the end of the index_to_ordinal vectors.

  if (ssize(data_.links) > num_user_links()) {
    const BodyIndex last_user_link_index =
        data_.links[num_user_links() - 1].index();
    data_.link_index_to_ordinal.erase(
        data_.link_index_to_ordinal.begin() + (last_user_link_index + 1),
  ...
  if (ssize(data_.joints) > num_user_joints()) {
    const JointIndex last_user_joint_index =
        data_.joints[num_user_joints() - 1].index();
    data_.joint_index_to_ordinal.erase(
        data_.joint_index_to_ordinal.begin() + (last_user_joint_index + 1),
...
std::vector<std::set<BodyIndex>> LinkJointGraph::GetSubgraphsOfWeldedLinks()
  ...
  for (const std::vector<BodyIndex>& composite : link_composites()) {
    subgraphs.emplace_back(
        std::set<BodyIndex>(composite.cbegin(), composite.cend()));
  }
...
std::set<BodyIndex> LinkJointGraph::GetLinksWeldedTo(
  ...
  const std::vector<BodyIndex>& welded_links =
      link_composites(*composite_index);
...
void LinkJointGraph::ThrowLinkWasRemoved(BodyIndex link_index) const {
  throw std::logic_error(fmt::format(
      "links(): An attempt was made to access a link with index {} but that "
      "link was removed.",
      link_index));
}

void LinkJointGraph::ThrowJointWasRemoved(JointIndex joint_index) const {
  throw std::logic_error(fmt::format(
      "joints(): An attempt was made to access a joint with index {} "

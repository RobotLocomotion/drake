...
void LinkJointGraph::ClearForest(bool keep_modeling_additions) {
...
CompositeLinkIndex LinkJointGraph::AddToCompositeLink(
  ...
  if (!existing_composite.is_valid()) {
    // We're starting a new Composite Link. This must be the "active link"
...
std::vector<LinkIndex> LinkJointGraph::FindPathFromWorld(
  ...
  while (mobod->inboard().is_valid()) {
    path[mobod->level()] = mobod->link();  // Active Link if composite.
...
std::vector<std::set<LinkIndex>> LinkJointGraph::CalcSubgraphsOfWeldedLinks()
  ...
  for (LinkIndex index : subgraphs.back()) visited[index] = true;
  ...
  for (LinkIndex link_index(1); link_index < num_user_links(); ++link_index) {
    if (visited[link_index]) continue;
    std::set<LinkIndex> welded_links = CalcLinksWeldedTo(link_index);
    for (LinkIndex index : welded_links) visited[index] = true;
...
// If the Link isn't part of a CompositeLink just return the Link. Otherwise,
// return all the Links in its CompositeLink.
std::set<LinkIndex> LinkJointGraph::GetLinksWeldedTo(
  ...
  const CompositeLinkIndex composite_index = link.composite();
  if (!composite_index.is_valid()) return std::set<LinkIndex>{link_index};
  const std::vector<LinkIndex> welded_links = composite_links(composite_index);
  return std::set<LinkIndex>(welded_links.cbegin(), welded_links.cend());
}

// Without a Forest we don't have CompositeLinks available so recursively
...
void LinkJointGraph::AppendLinksWeldedTo(LinkIndex link_index,
  ...
  for (auto joint_index : link.joints()) {
    const Joint& joint = joints(joint_index);
    if (joint.type_index() != weld_type_index()) continue;

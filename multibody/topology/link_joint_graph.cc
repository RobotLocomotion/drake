...
BodyIndex LinkJointGraph::AddShadowLink(BodyIndex primary_link_index,
  ...
  // Name should be <primary_name>$<shadow_num> (unique within primary's
  // model instance). In the unlikely event that a user has names like this,
  // we'll keep adding $'s to the end until this one is unique. Nothing much
  // depends on the details of this name.

...
class LinkJointGraph {
 public:
  ...
  the current forest.
  @see set_forest_building_options() */
  void set_global_forest_building_options(
  ...
  has yet been made to set_global_forest_building_options(), returns
  ...
  Invalidates the current forest.
  @see set_global_forest_building_options() */
  void set_forest_building_options(ModelInstanceIndex model_instance,
  ...
  `model_instance`, if any. Otherwise, returns the global forest building
  options as provided in the most recent set_global_forest_building_options()
  call, or ForestBuildingOptions::kDefault if no options have been set. */
  ForestBuildingOptions get_forest_building_options(
  ...
  void reset_forest_building_options() {
    data_.global_forest_building_options = ForestBuildingOptions::kDefault;
    data_.model_instance_forest_building_options.clear();
  }

  /** Returns a reference to this LinkJointGraph's SpanningForest model,
  first rebuilding the forest if it is not already valid. The currently-set
  forest building options are used. The returned reference is
  always to the same SpanningForest object; we do not create a new one when
  rebuilding.
  @warning if you retain the returned reference be aware that changes to the
    graph will invalidate its contents. You can guard against accidental
    misuse with the forest's `is_valid()` function.
  @see forest_is_valid() to check whether this function will rebuild.
  @see forest() for access to the forest object regardless of its current
                validity with respect to the graph.
  @see InvalidateForest() */
  const SpanningForest& GetValidForest();
  ...
  subsequent call to GetValidForest() will rebuild the forest. */
  void InvalidateForest();

  /** Has the SpanningForest been rebuilt since the most recent change to this
  LinkJointGraph? If this returns `false` then the next call to GetValidForest()
  will rebuild. */
  bool forest_is_valid() const { return data_.forest_is_valid; }

  /** (Advanced) Returns a reference to this LinkJointGraph's SpanningForest
  object without rebuilding. This is the same reference as is returned by
  GetValidForest(). You can call the graph's `forest_is_valid()` or the
  forest's `is_valid()` function to check whether the forest is current valid
  with respect to the graph's contents. When invalid, the forest object will
  be empty; it will not contain an out-of-date earlier model. Use
  GetValidForest() to bring the forest up to date if necessary. */
 ...
 private:
  ...
  // default-constructed condition. The forest object (but not its value) is
  // preserved if there is one.
  void Reinitialize();

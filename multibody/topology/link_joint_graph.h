...
limited set of _modeling_ Links, Joints, and Constraints to the graph. Those are
...
class LinkJointGraph {
 public:
  ...
  /** Restores this graph to its default-constructed state. The existing
  SpanningForest is preserved but now invalid. */
  void Clear();
  ...
  const SpanningForest& BuildForest(
      ModelingOptions global_options = ModelingOptions::kDefault,
  ...
  It is possible to specify that an entire ModelInstance is "static", meaning
  that it specifies only Links that are fixed to World. Only Links that have
  been explicitly flagged are returned here; those that are static implicitly
  due to their ModelInstance are not. */
  const std::vector<Constraint>& constraints() const {
    return data_.constraints;
  }

  /** Returns a reference to a particular Constraint. */
  ...
  /** Non-static Links that were flagged LinkFlags::kMustBeBaseBody in
  AddLink(). */

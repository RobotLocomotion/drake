...
a SpanningForest, one of which is owned by the graph.
...
class LinkJointGraph {
 public:
  ...
  "active link", the one whose (non-weld) Joint moves the whole Composite Link
  due to its modeling in the Spanning Forest. The rest of the Links in the
  composite are listed in no particular order.

  The 0th Composite Link is always present and its first entry is World
  (Link 0), even if nothing else is welded to World. Otherwise, composites are
  present here only for groups of two or more welded Links. Composite Links are
  discovered as a side effect of Forest-building; there is no cost to accessing
  them here. */
  ...
  for each Composite Link (Links connected by Weld joints), we only report the
  "active Link" for each Composite Link so that there is only one Link returned
  ...
  part of such a Composite the final entry will be L's composite's active link,
  which might not be L. Cost is O(ℓ) where ℓ is Link L's level in the
  SpanningForest.
  @throws std::exception if called prior to modeling
  @see composite_links(), SpanningForest::FindPathFromWorld() */
  std::vector<LinkIndex> FindPathFromWorld(LinkIndex link_index) const;

  /** After the Forest is built, finds the first Link common to the paths to
  World from each of two Links to World in the SpanningForest. Returns World
  ...
  there is a path between them which consists only of Weld joints (see
  weld_type_name()). Each subgraph of welded Links is represented as a set of
  Link indexes. By definition, these subgraphs will be disconnected by any
  non-Weld joint between two Links. A few more notes:
  ...
  CompositeLink the given `link_index` is part of (if any).
 ...
 private:
  ...
  struct Data {
    ...
    // active Link. If there is anything welded to World, that is the 0th
    // Composite and World (listed first) is its "active" Link.

...
(Mobods). A single Mobod may represents multiple Links (e.g., when multiple
...
class LinkJointGraph {
 public:
  ...
  /** Links that were explicitly flagged LinkFlags::Static in AddLink().
  Links that are part of a Static ModelInstance are not included here unless
  they were also explicitly flagged that way. */
  const std::vector<BodyIndex>& static_links() const {
    return data_.static_links;
  }

  /** Non-static Links that were flagged LinkFlags::MustBeBaseBody in
  AddLink(). */
 ...
 private:
  ...
  // default-constructed condition. The forest is preserved if there is one.

...
class LinkJointGraph {
 public:
  ...
  post-processing computations for links that follow fused (composite) Mobods.
  ...
  links are ordered such that kinematics of all links within a fused body can
  ...
  (1) If assembly fusing is disabled, then each link is modeled by
  ...
  (2) If assembly fusing is enabled and no weld joint is marked for
  ...
  fused Mobod) to be efficiently calculated by traversing the links in order.
  (Precalculating those transforms allows for efficient post-processing to
  get link kinematics from the calculated fused Mobod kinematics.) Although
  we don't expect to report reaction forces for unmodeled joints interior to
  a fused Mobod, this ordering can also facilitate computing those forces
  ...
  unmodeled joints interior to a fused body. Processed that way, each link

...
Terminology note: for clarity we use "Link" here to mean what
MultibodyPlant calls a "RigidBody" (or just "Body"), that is, what a
user inputs as an sdf or urdf "link" or adds using the AddRigidBody() call in
...
class LinkJointGraph {
 public:
  ...
  /** Returns the name reserved to identify weld joints (always "weld"). */
  ...
  inline const Link& links(BodyIndex link_index) const;  // Defined below.

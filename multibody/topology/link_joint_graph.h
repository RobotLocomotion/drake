...
easily restore the graph to its original condition. These additions are limited
to those needed for breaking loops (by splitting a Link) and ensuring that every
Link follows a Mobod whose mobility path leads to World (by adding floating or
weld joints).
...
correspondence between elements of the graph and the forest:
  ...
  - A primary Link and its shadows must be welded together by Weld Constraints
    ...
    the graph.
  - An assembly (links welded together) may be represented using one or more
    composite Mobods so many Links may follow one Mobod.
...
class LinkJointGraph {
 public:
  ...
  there are no loops, it is also possible to post-process in reverse order to
  calculate reaction forces for unmodeled joints interior to a composite body.

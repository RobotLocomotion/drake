...
class LinkJointGraph::Link {
 public:
  ...
  /** Returns `true` if this %Link was added with LinkFlags::Static. */
  bool is_static() const {
    return static_cast<bool>(flags_ & LinkFlags::Static);
  }

  /** Returns `true` if this %Link was added with LinkFlags::MustBeBaseBody. */
  bool must_be_base_body() const {
    return static_cast<bool>(flags_ & LinkFlags::MustBeBaseBody);
  }

  /** Returns `true` if this %Link was added with LinkFlags::TreatAsMassless. */
  ...
  /** (Internal use only) If `to_set` is LinkFlags::Default sets the flags to

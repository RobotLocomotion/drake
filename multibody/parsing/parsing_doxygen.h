...
- Syntax: Two attributes `p` (proportional gain) and `d` (derivative gain) 
          each of which contain a non-negative floating point value.

@subsection tag_drake_controller_gains_semantics Semantics

If present, this element provides proportional and derivative gains for a low level PD controller
for the drake::multibody::JointActuator associated with the drake::multibody::Joint the element
is defined under. It is stored in a drake::multibody::PdControllerGains object in the
drake::multibody::JointActuator class.

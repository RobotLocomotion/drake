...
@subsection urdf_nonstandard URDF Standard Tags Not Supported by Drake
TODO(rpoyner-tri): reference section covering urdf unsupported

@subsection urdf_community_extensions Commonly Used Non-standard URDF Tags Supported by Drake

@subsubsection urdf_capsule capsule
...
URDF note: drake supports a non-standard `capsule` tag with the same
...
drake::multibody::PlanarJoint. See that class for discussion of units and
detailed semantics.
...
@ref render_engine_vtk_properties
...
Constructs a drake::geometry::Ellipsoid with the given axis parameters, and
adds it to either the visual or collision geometry of the model.
...
Applies the indicated gear ratio value to the appropriate JointActuator object.

@see drake::multibody::JointActuator, @ref tag_drake_rotor_inertia,
@ref reflected_inertia
...
If present, this element provides a value for the Hunt-Crossley dissipation
model. It is stored in a ProximityProperties object under
`(material, hunt_crossley_dissipation)`.

@see drake::geometry::ProximityProperties, @ref mbp_hydroelastic_materials_properties
...
If present, this element provides a value for the hydroelastic modulus. It is
stored in a ProximityProperties object under
`(hydroelastic, hydroelastic_modulus)`.

@see drake::geometry::ProximityProperties,
@ref mbp_hydroelastic_materials_properties, @ref MODULE_NOT_WRITTEN_YET
...
- URDF path: `/robot/drake:collision_filter_group/drake:ignored_collision_filter_group`
- Syntax: String.
...
The provided value helps select the resolution of meshes generated from
geometric primitives (sphere, cylinder, capsule, etc.). The exact semantics
depend on the geometry being generated.
...
The provided value sets the dynamic friction parameter for CoulombFriction.
...
The provided value sets the static friction parameter for CoulombFriction.
...
The string names a frame (defined elsewhere in the model) that is associated
the parent link of the joint being defined.
...
If present, this element provides a stiffness value for point contact
calculations. It is stored in a ProximityProperties object under
...
If present, this element sets the compliance type of the element being defined
to be rigid, as opposed to compliant.
...
Applies the indicated rotor inertia value to the appropriate JointActuator object.

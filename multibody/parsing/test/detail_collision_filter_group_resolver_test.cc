...
TEST_F(CollisionFilterGroupResolverTest, LinkScoped) {
  ...
  EXPECT_FALSE(resolver_.groups().empty());

  resolver_.AddPair(diagnostic_policy_, "a", "a", r1);
  EXPECT_FALSE(resolver_.pairs().empty());

  resolver_.Resolve(diagnostic_policy_);

  // Resolve() clears internal state.
  EXPECT_TRUE(resolver_.groups().empty());
  EXPECT_TRUE(resolver_.pairs().empty());

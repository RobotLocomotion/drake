...
// and capsule. The request to auto-compute the inertia should work with these
// geometry types as well.
...
<model name='inertia_from_box'>
  ...
  const double density = 25;
  auto M_BBo_B_expected =
      SpatialInertia<double>::SolidEllipsoidWithDensity(density, 0.5, 0.6, 0.7)
          .Shift(-Vector3d(1, 2, 3));
  M_BBo_B_expected += SpatialInertia<double>::SolidCapsuleWithDensity(
                          density, 0.5, 1.2, Vector3d::UnitZ())
                          .Shift(-Vector3d(-2, -4, -6));

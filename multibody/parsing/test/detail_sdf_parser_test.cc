...
GTEST_TEST(MultibodyPlantSdfParserTest, JointActuatorParsingTest) {
  ...
  // Read in the SDF file.
  // Test the joint actuator with the effort limit sets to negative value,
  // This negative value does not equal to -1.
  DRAKE_EXPECT_THROWS_MESSAGE(
      AddModelFromSdfFile(full_name, "", package_map, &plant, &scene_graph),
      std::runtime_error,
  ...
  // Test the joint actuator with the effort limit sets to 0, which means no
  // actuation.
  DRAKE_EXPECT_THROWS_MESSAGE(

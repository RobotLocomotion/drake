...
std::unique_ptr<Parser> make_parser(
  ...
  auto parser = scene_graph
      ? std::make_unique<Parser>(plant, scene_graph)
      : std::make_unique<Parser>(plant);
...
GTEST_TEST(ProcessModelDirectivesTest, FlattenCollisionGroups) {
  ...
  // Collision filter groups are not directly observable from outside of the
  // plant.  However because the test dmd.yaml file contains collision filter
  // groups that reference one another by scoped name, we know that the
  // groups are in fact present and addressable via those scoped names.
}

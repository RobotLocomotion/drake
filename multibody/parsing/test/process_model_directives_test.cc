...
GTEST_TEST(ProcessModelDirectivesTest, AddScopedSmokeTest) {
  ...
  // - Checking simple_model_origin frames that have model namespaces
  for (const std::string model_namespace : {"left", "right", "mid::nested"}) {
    check_frame(model_namespace, "simple_model_origin");
  }
  // - Checking for simple_model_origin that was added without a model namespace
  // This frame was added without a model namespace, but ties itself to the
  // model namespace of its base frame instead of the world. See next test
  // AddFrameWithoutScope, for a more concrete example.
  check_frame("simple_model", "simple_model_origin");
}
...
GTEST_TEST(ProcessModelDirectivesTest, AddFrameWithoutScope) {
  ...
  ASSERT_EQ(
      &GetScopedFrameByName(plant, "world_as_base_frame"),
      &plant.GetFrameByName("world_as_base_frame", world_model_instance()));
  ...
  ASSERT_EQ(
      &GetScopedFrameByName(plant, "simple_model::included_as_base_frame"),
      &plant.GetFrameByName("included_as_base_frame", simple_model_instance));
}

...
GTEST_TEST(ProcessModelDirectivesTest, Flatten) {
  ...
  std::vector<ModelInstanceInfo> reflattened_models;
  {
    ...
    FlattenModelDirectives(
      flat_directives, parser->package_map(), &reflattened_directives);
    reflattened_models = ProcessModelDirectives(reflattened_directives,
                                                parser.get());
  }
  ...
  for (const auto& info : deep_models) { deep_names.insert(info.model_name); }
  std::set<std::string> flat_names;
  for (const auto& info : flat_models) { flat_names.insert(info.model_name); }

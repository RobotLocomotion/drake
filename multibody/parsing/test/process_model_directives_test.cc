...
GTEST_TEST(ProcessModelDirectivesTest, AddFrameWithoutScope) {
  ...
  // base_frame's model instance, instead of under the world model instance.

  // Frame added with world as base frame. It is scoped under the world
  // model instance.
  EXPECT_TRUE(
      plant.HasFrameNamed("world_as_base_frame", world_model_instance()));

  // Frame added with included model as base frame. It is then scoped under the
  // included model instance, instead of the world model instance.

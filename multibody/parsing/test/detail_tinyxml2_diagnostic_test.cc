...
<stuff>
  ...
  const XMLElement& GetElement(const char* name) {
...
TEST_F(TinyXml2DiagnosticContentsTest, PolicyForNode) {
  ...
  auto error_policy = diagnostic_.MakePolicyForNode(&GetElement("error"));
  error_policy.Error("bad");
  EXPECT_EQ(TakeError(), "<literal-string>.stuff:4: error: bad");

  auto warning_policy = diagnostic_.MakePolicyForNode(&GetElement("warning"));

...
GTEST_TEST(SchemaCheckerTest, ThreadedTest) {
  std::string schema_str = slurp(testdir / schema);
  ...
  auto thread_action = [&](int index) {
    ...
    int entry_point = index % 3;
    int doc_index = index % docs.size();
    bool result{};
    switch (entry_point) {
      case 0:
        result = CheckDocumentAgainstRncSchema(
            policy, testdir / schema, testdir / docs[doc_index]);
        break;
      case 1:
        result = CheckDocumentAgainstRncSchema(
            policy, schema_str, testdir / schema, testdir / docs[doc_index]);
        break;
      case 2:
        result = CheckDocumentAgainstRncSchema(
            policy, schema_str, testdir / schema,

...
GTEST_TEST(SceneGraphParserDetail, MakeProximityPropertiesForCollision) {
  // Generic string that we can shove arbitrary collision parameters into (into
  // the `{}` after the </geometry> tag is the fmt::format place holder.
...
<collision name="some_geo">
  ...
  auto assert_friction = [](const ProximityProperties& properties,
    ...
    ASSERT_TRUE(
        properties.HasProperty(geometry::kMaterialGroup, geometry::kFriction));
  ...
  <drake:drake>
    ...
    assert_single_property(properties, geometry::kHydroGroup,
                           geometry::kRezHint, 2.5);
  ...
  <drake:drake>
    ...
    ProximityProperties properties =
        MakeProximityPropertiesForCollision(*sdf_collision);
    assert_single_property(properties, geometry::kMaterialGroup,
                           geometry::kElastic, 3.5);
    assert_friction(properties, default_friction());
  }

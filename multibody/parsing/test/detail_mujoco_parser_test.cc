...
TEST_F(MujocoParserTest, MeshFileDirect) {
  // Absolute path, referencing the obj directly.
  std::string mesh_asset =
      fmt::format(R"""(<mesh name="box" file="{}"/>)""", box_obj_);
  TestMesh(box_obj_, mesh_asset);
}

TEST_F(MujocoParserTest, MeshFileStlOrObj) {
...
TEST_F(MujocoParserTest, MeshFileRelativePath) {
...
TEST_F(MujocoParserTest, MeshFileAbsolutePathCompiler) {
  ...
  TestMesh(box_obj_, mesh_asset, compiler);
}

TEST_F(MujocoParserTest, MeshFileRelativePathCompiler) {

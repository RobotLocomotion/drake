...
<mujoco model="test">
  ...
  AddModelFromMujocoXml({.file_contents = &xml}, "test", std::nullopt, &plant);
...
<mujoco model="test">
  ...
  AddModelFromMujocoXml({.file_contents = &xml}, "test", std::nullopt, &plant);
  AddModelFromMujocoXml({.file_contents = &radians_xml}, "radians_test",
                        std::nullopt, &plant);
  AddModelFromMujocoXml({.file_contents = &degrees_xml}, "degrees_test",
                        std::nullopt, &plant);
  ...
  CheckPose("axisangle_rad",
            RigidTransformd(
                Eigen::AngleAxis<double>(0.5, Vector3d{4, 5, 6}), p));
  CheckPose("euler_rad", RigidTransformd(
                         RollPitchYawd{0.5, 0.7, 1.05}, p));
  CheckPose("axisangle_deg",
            RigidTransformd(
                Eigen::AngleAxis<double>(M_PI / 6.0, Vector3d{4, 5, 6}), p));
  CheckPose("euler_deg", RigidTransformd(
                         RollPitchYawd{M_PI / 6.0, M_PI / 4.0, M_PI / 3.0}, p));
}

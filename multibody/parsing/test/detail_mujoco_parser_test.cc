...
<mujoco model="test_false">
  ...
  auto CheckBody = [&plant, &context, &inspector](
                       const std::string& body_name,
                       const UnitInertia<double>& unit_M_BB_B) {
    const Body<double>& body = plant.GetBodyByName(body_name);
    EXPECT_TRUE(CompareMatrices(
        body.CalcSpatialInertiaInBodyFrame(*context).CopyToFullMatrix6(),
        SpatialInertia<double>(2.53, Vector3d::Zero(), unit_M_BB_B)
            .CopyToFullMatrix6(),
        1e-14));
  };

  auto CheckBodySpatial = [&plant, &context, &inspector](
                       const std::string& body_name,
                       const SpatialInertia<double>& M_BB_B) {
    const Body<double>& body = plant.GetBodyByName(body_name);
    EXPECT_TRUE(CompareMatrices(
        body.CalcSpatialInertiaInBodyFrame(*context).CopyToFullMatrix6(),
        M_BB_B.CopyToFullMatrix6(), 1e-14));
  };
  ...
  SpatialInertia<double> M_BB_B = SpatialInertia<double>(2.53, Vector3d::Zero(),
                             UnitInertia<double>::SolidSphere(0.1));
  M_BB_B += SpatialInertia<double>(2.53, Vector3d::Zero(),
                              UnitInertia<double>::SolidSphere(0.2));
  CheckBodySpatial("two_spheres", M_BB_B);
  // Use the equation for the moment of inertia of a cylinder about one end.
  M_BB_B = SpatialInertia<double>(
      2.53,                                    // mass
      Vector3d{2.0, 0.0, 0.0},                 // Center of mass
      UnitInertia<double>(0.16 / 2.0,          // 1/2 r²
                          .04 + (16.0 / 3.0),  // 1/4 r^2+ 1/3 L^2)
                          .04 + (16.0 / 3.0)));
  CheckBodySpatial("offset_cylinder", M_BB_B);
  ...
  RotationalInertia<double> I_BF_F(2, 2, 2, -.75, -.75, -.75);
  CheckBodySpatial("full_inertia",
                   SpatialInertia<double>::MakeFromCentralInertia(
                       1.23, Vector3d::Zero(), I_BF_F));
  CheckBodySpatial("full_inertia_w_pose",
                   SpatialInertia<double>::MakeFromCentralInertia(
                       1.23, Vector3d{1, 2, 3},
                       I_BF_F.ReExpress(RotationMatrixd(
                           Eigen::Quaternion<double>{.2, .4, .3, .1}))));
}

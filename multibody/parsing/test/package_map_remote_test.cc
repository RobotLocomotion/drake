...
using RemotePackageParams = PackageMap::RemotePackageParams;
...
// When the cache directory already containts the content-addressable {sha256}
...
TEST_F(PackageMapRemoteTest, GetPathPrepopulated) {
  ...
  const std::string sha256(size_t{64}, '0');
  ...
  dut.AddRemotePackage(package_name, params);

  // Only a call to GetPath kicks off the resolution.
...
RemotePackageParams MakeGoodParams() {
  RemotePackageParams params;
...
TEST_F(PackageMapRemoteTest, ActuallyFetch) {
  ...
  dut.AddRemotePackage(package_name, MakeGoodParams());

  // Resolve the package, to force a "download" and extract.
  const std::string resolved = dut.GetPath(package_name);

  // Check that the extracted data lives where it should.
...
TEST_F(PackageMapRemoteTest, RejectDuplicates) {
  ...
  dut.Add("foo", "local_dir");

  DRAKE_EXPECT_THROWS_MESSAGE(
      dut.AddRemotePackage("foo", RemotePackageParams{}),
      ".*foo.*already.*registered.*");
}
...
TEST_F(PackageMapRemoteTest, InputSanitization) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      ".*at least one URL.*");
  DRAKE_EXPECT_THROWS_MESSAGE(
      dut.AddRemotePackage("foo",
                           RemotePackageParams{
                               .urls = {"mailfrom:rico@drake"},
                           }),
      ".*unsupported URL.*");
  DRAKE_EXPECT_THROWS_MESSAGE(
      dut.AddRemotePackage("foo",
...
TEST_F(PackageMapRemoteTest, CouldNotCreateCache) {
  ...
  PackageMap dut = PackageMap::MakeEmpty();
  dut.AddRemotePackage("foo", MakeGoodParams());
...
TEST_F(PackageMapRemoteTest, AddMapMergeUnfetched) {
  ...
  other.AddRemotePackage("compressed", MakeGoodParams());
...
TEST_F(PackageMapRemoteTest, AddMapDeduplicate) {
  PackageMap dut = PackageMap::MakeEmpty();
  dut.AddRemotePackage("compressed", MakeGoodParams());
...
TEST_F(PackageMapRemoteTest, AddMapMismatch) {
  RemotePackageParams params1 = MakeGoodParams();
  RemotePackageParams params2 = params1;
  params2.urls.push_back(params1.urls.back());

  PackageMap dut = PackageMap::MakeEmpty();
  dut.AddRemotePackage("compressed", params1);
  PackageMap other = PackageMap::MakeEmpty();
  other.AddRemotePackage("compressed", params2);

  DRAKE_EXPECT_THROWS_MESSAGE(dut.AddMap(other), ".*already.*registered.*");
}

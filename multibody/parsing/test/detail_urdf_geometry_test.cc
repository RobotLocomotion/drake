...
GTEST_TEST(DetailUrdfGeometryTest, AddMaterialToMaterialMap) {
  ...
  {
    const AddMaterialResult  result1 = AddMaterialToMaterialMap(
        empty_mat_name, empty_mat, abort_if_name_clash, &materials);
    EXPECT_EQ(std::get<UrdfMaterial>(result1), default_mat);
    const AddMaterialResult result2 = AddMaterialToMaterialMap(
        empty_mat_name, empty_mat, !abort_if_name_clash, &materials);
    EXPECT_EQ(std::get<UrdfMaterial>(result2), default_mat);
  }
  // Case: Adding a unique material with only rgba.
  {
    const AddMaterialResult result1 = AddMaterialToMaterialMap(
        rgba_mat_name, rgba_mat, abort_if_name_clash, &materials);
    EXPECT_EQ(std::get<UrdfMaterial>(result1), rgba_mat);
    const AddMaterialResult result2 = AddMaterialToMaterialMap(
        rgba_mat_name, rgba_mat, !abort_if_name_clash, &materials);
    EXPECT_EQ(std::get<UrdfMaterial>(result2), rgba_mat);
  }
  ...
  // Case: Adding a unique material with only diffuse map - get default rgba.
  {
    ...
    EXPECT_EQ(std::get<UrdfMaterial>(result1), expected);
    const AddMaterialResult result2 = AddMaterialToMaterialMap(
        diffuse_mat_name, diffuse_mat, !abort_if_name_clash, &materials);
    EXPECT_EQ(std::get<UrdfMaterial>(result2), expected);
  }
  ...
  // Case: adding a unique material with both diffuse map and rgba.
  {
    ...
    EXPECT_EQ(std::get<UrdfMaterial>(result1), full_mat);
    const AddMaterialResult result2 = AddMaterialToMaterialMap(
        full_mat_name, full_mat, !abort_if_name_clash, &materials);
    EXPECT_EQ(std::get<UrdfMaterial>(result2), full_mat);
  }
  ...
  {
    const AddMaterialResult result = AddMaterialToMaterialMap(
  ...
  // Case: Cache only has diffuse map and rgba defined; input material is empty.
  {
    ...
    EXPECT_EQ(std::get<UrdfMaterial>(result), full_mat);
  }
  ...
  // parameter.
  {
    ...
    const AddMaterialResult result = AddMaterialToMaterialMap(
        empty_mat_name, empty_mat, !abort_if_name_clash, &materials);
    EXPECT_EQ(std::get<UrdfMaterial>(result), default_mat);

    // Redundant adding with name clash *is* an error.
    EXPECT_THAT(
        std::get<std::string>(
            AddMaterialToMaterialMap(empty_mat_name, empty_mat,
                                     abort_if_name_clash, &materials)),
        testing::MatchesRegex("Material '.+' was previously defined.+"));
  }
 ...
 public:
  ...
  void ParseUrdfGeometry(const DataSource& source) {
    ...
    for (const XMLElement* material_node = node->FirstChildElement("material");
      ...
      ParseMaterial(material_node, true, source, workspace_, &materials_);
    }

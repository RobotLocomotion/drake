...
TEST_F(UrdfGeometryTest, TestDuplicateVisualName) {
  ...
  EXPECT_EQ(visual_instances_[1].name(), "hello2");
  EXPECT_THAT(TakeWarning(), ContainsRegex("visual.*hello.*renam.*hello2"));
}
...
TEST_F(UrdfGeometryTest, TestDuplicateCollisionName) {
  ...
  EXPECT_EQ(collision_instances_[1].name(), "hello2");
  EXPECT_THAT(TakeWarning(), ContainsRegex("collision.*hello.*renam.*hello2"));
}

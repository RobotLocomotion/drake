  ...
  void DoParseUrdfGeometry() {
    ...
    for (const XMLElement* link_node = node->FirstChildElement("link");
      ...
      for (const XMLElement* visual_node =
        ...
        UrdfVisualProperties visual_props = ParseVisual(
            *diag_, body_name, package_map_, root_dir_, visual_node,
            &materials_, &geometry_names_, numeric_name_suffix_limit_);
        if (visual_props.geometry) {
          // Set properties on the geometry instance.
          ...
          visual_instances_.push_back(*visual_props.geometry);
        }
      }

...
TEST_F(UrdfParserTest, NoName) {
  ...
  EXPECT_EQ(FormatError(0),
            "<literal-string>.urdf:1: error: Your robot must have a name"
            " attribute or a model name must be specified.");
  EXPECT_TRUE(warning_records_.empty());
}

TEST_F(UrdfParserTest, ObsoleteLoopJoint) {
  EXPECT_EQ(AddModelFromUrdfString("<robot name='a'><loop_joint/></robot>", ""),
            std::nullopt);
  EXPECT_EQ(FormatError(0),
            "<literal-string>.urdf:1: error: loop joints are not supported in"
            " MultibodyPlant");
  EXPECT_TRUE(warning_records_.empty());
}

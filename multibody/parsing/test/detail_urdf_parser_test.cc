...
class UrdfParserTestFixture : public ::testing::Test {
 ...
 protected:
  ...
  ParsingWorkspace w{package_map, diagnostic, &plant, &scene_graph};
};

// Verifies that the URDF loader can leverage a specified package map.
TEST_F(UrdfParserTestFixture, PackageMapSpecified) {
...
TEST_F(UrdfParserTestFixture, DoublePendulum) {
...
TEST_F(UrdfParserTestFixture, TestAtlasMinimalContact) {
  std::string full_name = FindResourceOrThrow(
...
TEST_F(UrdfParserTestFixture, TestAddWithQuaternionFloatingDof) {
...
TEST_F(UrdfParserTestFixture, TestOptionalSceneGraphYes) {
...
TEST_F(UrdfParserTestFixture, TestOptionalSceneGraphNo) {
  const std::string full_name = FindResourceOrThrow(
      "drake/examples/atlas/urdf/atlas_minimal_contact.urdf");
  // Test implicitly specifying.
  plant.RegisterAsSourceForSceneGraph(&scene_graph);
  ParsingWorkspace no_scene_graph{package_map, diagnostic, &plant, nullptr};
  AddModelFromUrdfFile(full_name, "", &no_scene_graph);
  plant.Finalize();
  EXPECT_NE(plant.num_visual_geometries(), 0);
}

TEST_F(UrdfParserTestFixture, JointParsingTest) {
  const std::string full_name = FindResourceOrThrow(
      "drake/multibody/parsing/test/urdf_parser_test/"
...
TEST_F(UrdfParserTestFixture, JointParsingTagMismatchTest) {
...
<robot xmlns:xacro="http://ros.org/wiki/xacro" name="joint_parsing_test">
  ...
  MultibodyPlant<double> plant(0.0);
  SceneGraph<double> scene_graph;
  ParsingWorkspace w{{}, {}, &plant, &scene_graph};
  AddModelFromUrdf(source, "urdf", {}, &w);
...
class UrdfParsedGeometryFixture : public UrdfParserTestFixture {
 public:
...
TEST_F(UrdfParsedGeometryFixture, CollisionGeometryParsing) {
...
TEST_F(UrdfParsedGeometryFixture, VisualGeometryParsing) {
  TestForParsedGeometry(
      "drake/multibody/parsing/test/urdf_parser_test/"
...
void ParseTestString(const std::string& inner,
  ...
  AddModelFromUrdf({ .file_contents = &contents }, model_name, {}, &w);
}
...
TEST_F(UrdfParserTestFixture, CollisionFilterGroupParsingTest) {
  const std::string full_name = FindResourceOrThrow(
      "drake/multibody/parsing/test/urdf_parser_test/"

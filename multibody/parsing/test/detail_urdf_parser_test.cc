...
ModelInstanceIndex AddModelFromUrdfFile(
    ...
    ParsingWorkspace* w) {
  return AddModelFromUrdf({ .file_name = &file_name }, model_name, {}, w);
}
...
GTEST_TEST(MultibodyPlantUrdfParserTest, PackageMapSpecified) {
  ...
  ParsingWorkspace w{package_map, {}, &plant, &scene_graph};
  AddModelFromUrdfFile(full_urdf_filename, "", &w);
  plant.Finalize();
...
GTEST_TEST(MultibodyPlantUrdfParserTest, DoublePendulum) {
  ...
  PackageMap package_map;
  ParsingWorkspace w{package_map, {}, &plant, &scene_graph};
  AddModelFromUrdfFile(full_name, "", &w);
  plant.Finalize();
...
GTEST_TEST(MultibodyPlantUrdfParserTest, TestAddWithQuaternionFloatingDof) {
  ...
  SceneGraph<double> scene_graph;
  ParsingWorkspace w{package_map, {}, &plant, &scene_graph};
  AddModelFromUrdfFile(model_file, "", &w);
  plant.Finalize();
...
GTEST_TEST(MultibodyPlantUrdfParserTest, JointParsingTagMismatchTest) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      AddModelFromUrdfFile(full_name_mismatch_1, "", &w),
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      AddModelFromUrdfFile(full_name_mismatch_2, "", &w),
...
void ParseTestString(const std::string& inner,
  ...
  AddModelFromUrdfFile(filename, model_name, &w);
}
...
GTEST_TEST(MultibodyPlantUrdfParserTest, CollisionFilterGroupParsingTest) {
  ...
  AddModelFromUrdfFile(full_name, "model2", &w);
}

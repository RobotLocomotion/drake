...
class ControllerGainsTest : public UrdfParserTest {
 public:
  void VerifyParameters(const std::string& controller_gains_text, double p,
                        double d) {
    ...
    EXPECT_EQ(actuator.get_controller_gains().p, p);
    EXPECT_EQ(actuator.get_controller_gains().d, d);
  }

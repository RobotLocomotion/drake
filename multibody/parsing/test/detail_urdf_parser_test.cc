...
class UrdfParserTest : public ::testing::Test {
 public:
  ...
  std::string FormatWarning(int index) {
    return warning_records_.at(index).FormatWarning();
  }
...
TEST_F(UrdfParserTest, BadFilename) {
  ...
  EXPECT_THAT(FormatError(0), MatchesRegex(
...
TEST_F(UrdfParserTest, BadXmlString) {
  ...
  EXPECT_EQ(FormatError(0), "<literal-string>.urdf:1: error: Failed to parse"
...
TEST_F(UrdfParserTest, DrakeFrictionWarning) {
  ...
  EXPECT_TRUE(error_records_.empty());
  EXPECT_THAT(FormatWarning(0), MatchesRegex(
                  ".*A joint has specified a non-zero value for the 'friction'"
                  " attribute of a joint/dynamics tag. MultibodyPlant does not"
                  " currently support non-zero joint friction."));
}
...
TEST_F(UrdfParserTest, DrakeCoulombWarning) {
  ...
  EXPECT_TRUE(error_records_.empty());
  EXPECT_THAT(FormatWarning(0), MatchesRegex(
                  ".*A joint has specified a value for the 'coulomb_window'"
                  " attribute of a <joint> tag. Drake no longer makes use of"
                  " that attribute; all instances will be ignored."));
}
...
TEST_F(UrdfParserTest, FrameNameBroken) {
  EXPECT_NE(AddModelFromUrdfString(R"""(
    <robot name="a">
        <frame nem="broken" link="A" rpy="0 0 0" xyz="0 0 0"/>
    </robot>)""", ""), std::nullopt);
  EXPECT_THAT(FormatError(0), MatchesRegex(".*parsing frame name."));
  EXPECT_TRUE(warning_records_.empty());
}
...
TEST_F(UrdfParserTest, FrameLinkBroken) {
  EXPECT_NE(AddModelFromUrdfString(R"""(
    <robot name="a">
        <frame name="frameA" yink="broken" rpy="0 0 0" xyz="0 0 0"/>
    </robot>)""", ""), std::nullopt);
  EXPECT_THAT(FormatError(0), MatchesRegex(
...
TEST_F(UrdfParserTest, TransmissionTypeUnknown) {
  ...
  EXPECT_TRUE(error_records_.empty());
  EXPECT_THAT(FormatWarning(0), MatchesRegex(warning_pattern));
  ...
  EXPECT_TRUE(error_records_.empty());
  EXPECT_THAT(FormatWarning(1), MatchesRegex(warning_pattern));
}
...
TEST_F(UrdfParserTest, TransmissionJointNotExist) {
  ...
  EXPECT_THAT(FormatError(0), MatchesRegex(".*Transmission specifies joint"
...
TEST_F(UrdfParserTest, JointParsingTagMismatchTest) {
  ...
  EXPECT_THAT(FormatError(1), MatchesRegex(
  ...
  EXPECT_NE(AddModelFromUrdfString(R"""(
    <robot name="bushing_test">
        ...
        <drake:linear_bushing_rpy>
            ...
            <drake:bushing_frameC nem="frameC"/>
            <drake:bushing_torque_stiffness value="1 2 3"/>
            <drake:bushing_torque_damping   value="4 5 6"/>
            <drake:bushing_force_stiffness  value="7 8 9"/>
            <drake:bushing_force_damping    value="10 11 12"/>
 ...
 public:
  ...
  void ProvokeError(const std::string& rotor_inertia_text,
    ...
    std::string text =
        fmt::format(test_string_, rotor_inertia_text, gear_ratio_text);
    EXPECT_NE(AddModelFromUrdfString(text, ""), std::nullopt);
    EXPECT_THAT(FormatError(0), MatchesRegex(error_pattern));
    EXPECT_TRUE(warning_records_.empty());
  }

...
class UrdfParserTest : public ::testing::Test {
  ...
  UrdfParserTest() {
    ...
    diagnostic_.SetActionForErrors(
        [this](const DiagnosticDetail& detail) {
          error_record_indices_.push_back(records_.size());
          records_.push_back(detail);
        });
    diagnostic_.SetActionForWarnings(
        [this](const DiagnosticDetail& detail) {
          warning_record_indices_.push_back(records_.size());
          records_.push_back(detail);
        });
  }
 ...
 protected:
  std::vector<DiagnosticDetail> records_;
  std::vector<int> error_record_indices_;
  std::vector<int> warning_record_indices_;
...
TEST_F(UrdfParserTest, BadFilename) {
  ...
  ASSERT_EQ(error_record_indices_.size(), 1);
  int error_index = error_record_indices_[0];
  std::string full_message = records_[error_index].FormatError();

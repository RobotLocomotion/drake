...
GTEST_TEST(MultibodyPlantUrdfParserTest, CollisionFilterGroupParsingTest) {
  ...
  // The URDF has been configured such that the unfiltered pairs are
  // a) geometry 2 and 4 by using ignore, and b) geometry 2 and 1 by
  // not applying any filter. The filterd pairs are done via two ways as
  // listed in the following.
  // Test collision filter inside the same group.
  EXPECT_TRUE(
      inspector.CollisionFiltered(geometry_id_link1, geometry_id_link4));

  // Test collision filter between two different groups.
  EXPECT_TRUE(

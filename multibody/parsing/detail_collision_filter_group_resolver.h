...
class CollisionFilterGroupResolver {
 public:
  ...
  // @returns a reference to the map of defined groups.
  const std::map<std::string, geometry::GeometrySet>& groups() const {
    return groups_;
  }

  // @returns a reference to the set of pairs of groups.
  const std::set<SortedPair<std::string>>& pairs() const { return pairs_; }

...
UrdfMaterial AddMaterialToMaterialMap(const std::string& material_name,
  ...
  if (material_iter != materials->end()) {
    ...
    //   Cached  |  Input  | Result
    //  ---------|---------|--------
    //   nullopt | nullopt | No error, They match
    //   nullopt |  val1   | ERROR
    //    val1   | nullopt | No error, not defining a value is compatible with a
    //           |         |   predefined value.
    //    val1   |  val1   | No error, they match
    //    val1   |  val2   | ERROR, they don't match

    bool error = abort_if_name_clash;
    if (!error) {
      // Evaluate the truth table for diffuse_map.
    ...
    if (!error) {
      // Evaluate the truth table for rgba, but for color values that are equal
      // to within a threshold. W

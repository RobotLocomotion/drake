...
geometry::GeometryInstance ParseCollision(
  ...
  if (drake_element) {
    ...
    // TODO(DamrongGuoy): Remove this ad-hoc input sanitization when we
    //  resolve issue 16229 "Diagnostics for unsupported SDFormat and URDF
    //  stanzas."
    ...
    if (no_longer_supported) {
      throw std::runtime_error(fmt::format(
          ...
          "<drake:soft_hydroelastic> on line {}, "

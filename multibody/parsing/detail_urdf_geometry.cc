...
geometry::GeometryInstance ParseCollision(
  ...
  if (drake_element) {
    ...
    if (rez_hint) {
      props.AddProperty(geometry::kHydroGroup, geometry::kRezHint, *rez_hint);
    }
    std::optional<double> elastic_modulus =
        read_double("drake:elastic_modulus");
    if (elastic_modulus) {
      props.AddProperty(geometry::kMaterialGroup, geometry::kElastic,
                        *elastic_modulus);
    }
    std::optional<double> dissipation = read_double("drake:dissipation");
    if (dissipation) {
      props.AddProperty(geometry::kMaterialGroup, geometry::kHCDissipation,
                        *dissipation);
    }

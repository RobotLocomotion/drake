...
UrdfMaterial AddMaterialToMaterialMap(const std::string& material_name,
  ...
  if (material_iter != materials->end()) {
    ...
    // values must match. This compound expression tests all ways of matching
    // *except* matching rgba values for when both are defined.
    bool error =
        abort_if_name_clash ||
        (cached_material.rgba.has_value() != material.rgba.has_value()) ||
        (cached_material.diffuse_map != material.diffuse_map);
    if (!error && cached_material.rgba.has_value()) {

...
geometry::GeometryInstance ParseCollision(
  ...
  if (!props.HasProperty(geometry::internal::kMaterialGroup,
    ...
    if (node->FirstChildElement("drake_compliance")) {
      static const logging::Warn log_once(
          "At least one of your URDF files has specified the the collision "
          "properties using both <drake:proximity_properties> and "
          "<drake_compliance> tags. The <drake_compliance> tag is ignored in "
          "that case and is generally dispreferred. Consider removing all "
          "instances of <drake_compliance> in favor of "

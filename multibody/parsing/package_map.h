...
from the internet on an as-needed basis via AddRemote(). */
class PackageMap final {
 public:
  ...
  struct RemotePackageParams {
    ...
    /** Equality operator. */
    friend bool operator==(const RemotePackageParams&,
                           const RemotePackageParams&);
  ...
  void AddRemote(std::string package_name, RemotePackageParams params);

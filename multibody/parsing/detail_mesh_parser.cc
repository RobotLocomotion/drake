...
// Attempts to parse a single mesh otu of the input stream returning the
...
NamedMesh DoGetObjMesh(const DiagnosticPolicy& diagnostic,
  ...
  auto on_warning = [&diagnostic](std::string_view message) {
    diagnostic.Warning(std::string(message));
  };
  auto on_error = [&diagnostic](std::string_view message) {
...
std::optional<ModelInstanceIndex> AddModelFromMesh(
  ...
  const SpatialInertia<double> M_BBo_B =
      CalcSpatialInertia(*named_mesh.mesh, 1.0 /* kg/m3 */);
...
std::vector<ModelInstanceIndex> MeshParserWrapper::AddAllModels(
    ...
    const std::optional<std::string>& parent_model_name,
    const ParsingWorkspace& workspace) {
  return AddModelsFromMesh(data_source, parent_model_name, workspace);
}

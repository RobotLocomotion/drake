...
class UrdfParser {
 public:
  ...
  void Warning(const XMLNode& location, const std::string& message) const {
    diagnostic_.Warning(location, message);
  }

  void Error(const XMLNode& location, const std::string& message) const {
    diagnostic_.Error(location, message);
    ++errors_;
  }
...
void UrdfParser::ParseJoint(
  ...
  if (type.compare("revolute") == 0 || type.compare("continuous") == 0) {
    ...
    Error(*node, "Joint " + name + " has unrecognized type: " + type);
    return;
  }
    ...
    if (value_node != nullptr) {
      ...
      if (ParseVectorAttribute(value_node, "value", &value)) {
        ...
        Error(*node, fmt::format(
                  "Unable to read the 'value' attribute for the <{}> tag",
                  element_name));
        return {};
      }
    } else {
  ...
  auto read_frame = [node, this](const char* element_name)
    ...
    if (value_node != nullptr) {
      ...
      if (ParseStringAttribute(value_node, "name", &frame_name)) {
        ...
        Error(*value_node, fmt::format(
                  "Unable to read the 'name' attribute for the <{}> tag",
                  element_name));
        return {};
      }
...
std::optional<ModelInstanceIndex> UrdfParser::Parse() {
  ...
  for (XMLElement* material_node = node->FirstChildElement("material");
    ...
    ParseMaterial(material_node, true /* name_required */, *data_source_, w_,
                  &materials);
  }

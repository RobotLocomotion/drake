...
void UrdfParser::ParseJoint(
  ...
  // The rigid transform from parent to child. See the Joint class
  // documentation.
  ...
  if (type.compare("revolute") == 0 || type.compare("continuous") == 0) {
    ...
    // Frame F = M = B. See Joint class documentation.
    ...
    // with the joint axis. So in general, we don't have F = M = B as we do in
    // in, e.g., revolute joint.  Here, we still set F = M, but they are not
    // necessarily equal to B. Instead, we let Mz_B to be specified by the
    // parsed axis, and we let M and B have the same origin.

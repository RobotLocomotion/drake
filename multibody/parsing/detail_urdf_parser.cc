  ...
  if (dynamics_node) {
    ...
    if (ParseScalarAttribute(dynamics_node, "friction", &coulomb_friction) &&
      ...
      static const logging::Warn log_once(
          ...
          "the 'friction' attribute of a <joint> tag. Drake doesn't make use "
          "of that attribute and all instances will be ignored.");
    }
    if (ParseScalarAttribute(dynamics_node, "coulomb_window",
      ...
      static const logging::Warn log_once(
          "At least one of your URDF files has specified a non-zero value for "
          "the 'coulomb_window' attribute of a <joint> tag. Drake doesn't make "
...
void ParseTransmission(
  ...
  if (type.find("SimpleTransmission") == std::string::npos) {
    static const logging::Warn log_once(
        "At least one of your URDF files has <transmission> type of "
        "'SimpleTransmission'. Drake doesn't make use of that tag and all "

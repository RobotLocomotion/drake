...
void ParseJoint(ModelInstanceIndex model_instance,
  ...
  auto throw_on_custom_joint = [node, name, type](bool is_custom_joint) {
    const std::string node_name(node->Name());
    const bool want_custom_joint = node_name == "drake:joint";
    if (is_custom_joint && !want_custom_joint) {
      throw std::runtime_error(
          "ERROR: Joint " + name + " of type " + type +
          " is a custom joint type, and should be a <drake:joint>");
    } else if (!is_custom_joint && want_custom_joint) {
      throw std::runtime_error(
          "ERROR: Joint " + name + " of type " + type +

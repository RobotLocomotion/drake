...
SpatialInertia<double> ExtractSpatialInertiaAboutBoExpressedInB(
    XMLElement* node, ParsingWorkspace* w) {
  DRAKE_THROW_UNLESS(w != nullptr);
  [[maybe_unused]] auto& [package_map, diagnostic, plant, scene_graph] = *w;
...
void ParseBody(const std::string& root_dir,
  ...
  if (body_name == kWorldName) {
    ...
    if (!inertial_node) {
      ...
      M_BBo_B = ExtractSpatialInertiaAboutBoExpressedInB(inertial_node, w);
    }
...
void ParseJoint(ModelInstanceIndex model_instance,
  ...
  ParseJointKeyParams(node, &name, &type, &parent_name, &child_name, w);

  const Body<double>& parent_body = GetBodyForElement(
      name, parent_name, model_instance, w);
  const Body<double>& child_body = GetBodyForElement(
      name, child_name, model_instance, w);
...
void ParseFrame(ModelInstanceIndex model_instance,
  ...
  const Body<double>& body = GetBodyForElement(
      name, body_name, model_instance, w);

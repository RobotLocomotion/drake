 ...
 public:
  UrdfParser(
      ...
      : data_source_(data_source), model_name_(model_name),
        ...
        root_dir_(data_source->GetRootDir()),
        xml_doc_(xml_doc), w_(w), diagnostic_(&w.diagnostic, data_source) {
  ...
  void ParseBody(XMLElement* node,
                 MaterialMap* materials);
  SpatialInertia<double> ExtractSpatialInertiaAboutBoExpressedInB(
      XMLElement* node);

  template <typename... Args>
  void Warning(const XMLNode& location, const char* format,
               const Args&... args) const {
    diagnostic_.Warning(location, format, args...);
  }

  template <typename... Args>
  void Error(const XMLNode& location, const char* format,
             const Args&... args) const {
    diagnostic_.Error(location, format, args...);
    ++errors_;
  }
...
void UrdfParser::ParseBody(XMLElement* node,
                           MaterialMap* materials) {
  ...
  if (body_name == kWorldName) {
    ...
    body_pointer = &plant->AddRigidBody(body_name, model_instance_, M_BBo_B);
  }
...
void UrdfParser::ParseCollisionFilterGroup(XMLElement* node) {
  ...
  auto get_string_attribute = [this](const ElementNode& data_element,
    ...
    if (!ParseStringAttribute(anode, attribute_name, &attribute_value)) {
      Error(*anode, "The tag <{}> does not specify the required attribute"
...
void UrdfParser::ParseJointKeyParams(XMLElement* node,
  ...
  if (!ParseStringAttribute(node, "type", type)) {
    Error(*node, "joint {} is missing type attribute", *name);
    return;
  }
  ...
  if (!parent_node) {
    Error(*node, "joint {} doesn't have a parent node!", *name);
    return;
  }
  if (!ParseStringAttribute(parent_node, "link", parent_link_name)) {
    Error(*parent_node, "joint {}'s parent does not have a link attribute!",
          *name);
    return;
  }
  ...
  if (!child_node) {
    Error(*node, "joint {} doesn't have a child node", *name);
    return;
  }
  if (!ParseStringAttribute(child_node, "link", child_link_name)) {
    Error(*child_node, "joint {}'s child does not have a link attribute!",
          *name);
    return;
  }
}
  ...
  *damping = 0.0;
  ...
  if (dynamics_node) {
    ...
    if (ParseScalarAttribute(dynamics_node, "friction", &coulomb_friction) &&
      ...
      Warning(*dynamics_node, "At least one of your URDF files has specified a"
              " non-zero value for the 'friction' attribute of a"
              " joint/dynamics tag. MultibodyPlant does not currently support"
              " non-zero joint friction.");
    }
    if (ParseScalarAttribute(dynamics_node, "coulomb_window",
                             &coulomb_window) &&
        coulomb_window != std::numeric_limits<double>::epsilon()) {
      Warning(*dynamics_node, "At least one of your URDF files has specified a"
              " value for  the 'coulomb_window' attribute of a <joint> tag."
              " Drake no longer makes use of that attribute and all instances"
              " will be ignored.");
    }
  }
}
...
const Body<double>* UrdfParser::GetBodyForElement(
  ...
  if (!plant->HasBodyNamed(link_name, model_instance_)) {
    Error(*xml_doc_, "Could not find link named\"{}\" with model instance ID"
          " {} for element {}.", link_name, std::to_string(model_instance_),
          element_name);
    return nullptr;
  }
...
void UrdfParser::ParseJoint(
  ...
  if (axis_node && type.compare("fixed") != 0 &&
    ...
    if (axis.norm() < 1e-8) {
      Error(*axis_node, "Joint {} axis is zero.  Don't do that.", name);
    }
    axis.normalize();
  }
  ...
  auto throw_on_custom_joint =
    ...
    if (want_custom_joint && !is_custom_joint) {
      Error(*node, "Joint {} of type {} is a custom joint type, and should be"
            " a <drake:joint>", name, type);
      return;
    } else if (!want_custom_joint && is_custom_joint) {
      Error(*node, "Joint {} of type {} is a standard joint type, and should"
            " be a <joint>", name, type);
      return;
    }
  };
  ...
  if (type.compare("revolute") == 0 || type.compare("continuous") == 0) {
    ...
    throw_on_custom_joint(false);
    Warning(*node, "Joint {} specified as type floating which is not supported"
            " by MultibodyPlant.  Leaving {} as a free body.",
            name, child_name);
    ...
    Error(*node, "Joint {} has unrecognized type: {}", name, type);
    return;
  }
...
void UrdfParser::ParseTransmission(
  ...
  if (type.find("SimpleTransmission") == std::string::npos) {
    Warning(*type_node, "At least one of your URDF files has <transmission>"
            " type that isn't 'SimpleTransmission'. Drake only supports"
            " 'SimpleTransmission'; all other transmission types will be"
            " ignored.");
    return;
  }
  ...
  if (!plant->HasJointNamed(joint_name, model_instance_)) {
    Error(*joint_node, "Transmission specifies joint {} which does not exist.",
          joint_name);
    return;
  }
  ...
  if (effort_iter->second < 0) {
    Error(*joint_node, "Transmission specifies joint {} which has a negative"
          " effort limit.", joint_name);
    return;
  }

  if (effort_iter->second <= 0) {
    Warning(*joint_node, "Skipping transmission since it's attached to "
            "joint \"{}\" which has a zero effort limit {}.",
  ...
  if (rotor_inertia_node) {
    ...
    if (!ParseScalarAttribute(rotor_inertia_node, "value", &rotor_inertia)) {
      Error(*rotor_inertia_node, "joint actuator {}'s drake:rotor_inertia does"
            " not have a \"value\" attribute!", actuator_name);
      return;
    }
  ...
  if (gear_ratio_node) {
    ...
    if (!ParseScalarAttribute(gear_ratio_node, "value", &gear_ratio)) {
      Error(*gear_ratio_node, "joint actuator {}'s drake:gear_ratio does not"
            " have a \"value\" attribute!", actuator_name);
      return;
    }
    plant->get_mutable_joint_actuator(actuator.index())
  ...
  auto read_vector = [node, this](const char* element_name) -> Eigen::Vector3d {
    ...
    if (value_node != nullptr) {
      ...
      if (ParseVectorAttribute(value_node, "value", &value)) {
        ...
        Error(*node, "Unable to read the 'value' attribute for the <{}> tag",
              element_name);
        return {};
      }
    } else {
      Error(*node, "Unable to find the <{}> tag", element_name);
      return {};
    }
  };
    ...
    if (value_node != nullptr) {
      ...
      if (ParseStringAttribute(value_node, "name", &frame_name)) {
        if (!plant->HasFrameNamed(frame_name, model_instance_)) {
          Error(*value_node, "Frame: {} specified for <{}> does not exist in"
                " the model.", frame_name, element_name);
          return {};
        }
        return &plant->GetFrameByName(frame_name, model_instance_);

      } else {
        Error(*value_node, "Unable to read the 'name' attribute for the"
              " <{}> tag", element_name);
        return {};
      }
...
std::optional<ModelInstanceIndex> UrdfParser::Parse() {
  ...
  for (XMLElement* transmission_node = node->FirstChildElement("transmission");
    ...
    ParseTransmission(joint_effort_limits,
                      transmission_node);
  }
...
std::optional<ModelInstanceIndex> AddModelFromUrdf(
  ...
  if (data_source.IsFilename()) {
    ...
    if (xml_doc.ErrorID()) {
      diag.Error(xml_doc, "Failed to parse XML file: {}", xml_doc.ErrorName());
      return std::nullopt;
    }
  } else {
    xml_doc.Parse(data_source.contents().c_str());
    if (xml_doc.ErrorID()) {
      diag.Error(xml_doc, "Failed to parse XML string: {}",
                 xml_doc.ErrorName());
      return std::nullopt;
    }
  }

  UrdfParser parser(&data_source, model_name_in, parent_model_name, &xml_doc,
                    workspace);

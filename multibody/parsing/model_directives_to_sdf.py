...
class ModelDirectivesToSdf:
    ...
    #                 weld['child'][len(model_ns)+len(self.SCOPE_DELIMITER):]
    ...
    def add_model(self, root: ET, directive: Dict[str, Any]) -> bool:
        ...
        if merge_include:
            model_root.set('placement_frame', weld['child'][len(model_name)+len(self.SCOPE_DELIMITER):])
            pose_elem = ET.SubElement(model_root, 'pose', relative_to=self.resolve_implicit_frame_scope(weld['parent']))
        else:
            placement_frame_elem = ET.SubElement(include_elem, 'placement_frame')
            placement_frame_elem.text = \
                weld['child'][len(model_name)+len(self.SCOPE_DELIMITER):]
            pose_elem = ET.SubElement(include_elem, 'pose', relative_to=self.resolve_implicit_frame_scope(weld['parent']))
        return True
        ...
        for directive in self.directives:
            if 'add_frame' in directive:
                if frame_name == directive['add_frame']['name']:
                    if len(scope) == 0:
                        scope = directive['add_frame']['X_PF']['base_frame'].split(self.SCOPE_DELIMITER)[:-1]
                    else:
                        print(f"Failed to convert add_weld directive: unable to identify the parent frame for weld. Two frames contain the same name: {weld['parent']}")
                        return None
        if len(scope) == 0 or '::' in frame_name:
            return frame_name
        else:
            return '::'.join(scope) + '::' + frame_name
    ...
    def add_frame(self, root: ET, directive: Dict[str, Any]) -> bool:
        ...
        split_frame_name = scoped_frame_name.split(self.SCOPE_DELIMITER)

        x_pf = directive['X_PF']
        scoped_base_frame = x_pf['base_frame']
        if scoped_base_frame == self.WORLD_FRAME:
            print(f'Workflows where base frame "{self.WORLD_FRAME}" exist is not '
                'supported to be converted using this script.')
            return False
        split_base_frame_name = scoped_base_frame.split(self.SCOPE_DELIMITER)
        ...
        scoped_base_frame = self.SCOPE_DELIMITER.join(split_relative_base_frame)
        ...
        for scope in split_frame_name[:-1]:
            if current_model_scope is None:
                ...
                current_model_scope += self.SCOPE_DELIMITER + scope
    ...
    def add_weld_fixed_joint(self, root: ET, directive: Dict[str, Any]) -> bool:
        ...
        joint_name = \
            f'{parent_name.replace(self.SCOPE_DELIMITER, "__")}___to___' \
            f'{child_name.replace(self.SCOPE_DELIMITER, "__")}___weld_joint'
    ...
    def convert_directive(self, input_path: str) -> str:
        ...
        root = ET.Element('sdf', version=self.SDF_VERSION)
        ...
        for directive in self.directives:
            if 'add_model_instance' in directive:
                ...
                weld_child_scopes = weld_child.split(self.SCOPE_DELIMITER)
                #Check for conflicts on weld parent
                weld_parent_scope = []
                explicit_parent = self.resolve_implicit_frame_scope(weld['parent'])
                ...
                self.CHILD_FRAME_MODEL_NAME_TO_WELD_MAP[weld_child_scopes[0]] = weld
        ...
        for directive in directives:
            ...
            if 'add_model' in directive:
                success = self.add_model(root_model_elem, directive['add_model'])
...
def main() -> None:
    ...
    sdf_result = model_directives_to_sdf.convert_directive(args.model_directives)

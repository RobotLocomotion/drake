...
//
// @returns false if the attribute is not present
// @tparam _Rows is indicates the size of the vector `val`.
//
// @throws std::exception if the attribute doesn't contain `_Rows` numeric
// values.
template <int _Rows>
bool ParseVectorAttribute(const tinyxml2::XMLElement* node,
                          const char* attribute_name,
                          Eigen::Matrix<double, _Rows, 1>* val) {
  const char* attr = node->Attribute(attribute_name);
  if (attr) {
    std::vector<double> vals = ConvertToDoubles(attr);
    if (vals.size() != _Rows) {
      throw std::invalid_argument(
          fmt::format("Expected {} values for attribute {} got {}", _Rows,
                      attribute_name, attr));
    }
    *val = Eigen::Matrix<double, _Rows, 1>(vals.data());
    return true;
  }
  return false;
}

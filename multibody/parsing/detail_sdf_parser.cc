  ...
  for (uint64_t model_index = 0; model_index < model.ModelCount();
    ...
    std::vector<ModelInstanceIndex> nested_model_instances =
        AddModelsFromSpecification(
            ...
            plant, package_map, root_dir, subgraph_merged_model_instances);
...
sdf::InterfaceModelPtr ParseNestedInterfaceModel(
  ...
  if (is_urdf) {
    ...
    main_model_instance =
        AddModelsFromSpecification(
            diagnostic, model,
            sdf::JoinName(include.AbsoluteParentName(), model_name), {}, plant,
            package_map, data_source.GetRootDir(), nullptr)
            .front();
  }

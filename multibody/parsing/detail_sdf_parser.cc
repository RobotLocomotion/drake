...
sdf::InterfaceModelPtr ParseNestedInterfaceModel(
  ...
  const bool is_urdf =
      EndsWithCaseInsensitive(include.ResolvedFileName(), kExtUrdf);
  if (!is_urdf) {
  ...
  int cur_num_models = workspace.plant->num_model_instances();
  ...
  for (ModelInstanceIndex model_instance(cur_num_models);
    ...
    sdf::RepostureFunction reposture_model =
        [&workspace, model_instance, errors](
            const sdf::InterfaceModelPoseGraph &graph) {
      auto& w = workspace;
      // N.B. This should also posture the model appropriately.
      for (auto interface_link_ind : w.plant->GetBodyIndices(model_instance)) {
        const auto& interface_link = w.plant->get_body(interface_link_ind);

        gz::math::Pose3d X_WL;
        sdf::Errors inner_errors = graph.ResolveNestedFramePose(
            X_WL, interface_link.name());
        PropagateErrors(std::move(inner_errors), errors);
        w.plant->SetDefaultFreeBodyPose(interface_link, ToRigidTransform(X_WL));
      }
    };

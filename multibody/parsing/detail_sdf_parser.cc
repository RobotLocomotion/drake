...
bool IsError(const sdf::Error& report) {
  switch (report.Code()) {
    case sdf::ErrorCode::ELEMENT_INCORRECT_TYPE: {
      // TODO(#15018): Change unrecognized elements to become an error on or
      // after 2022-02-01. There is no error code dedicated solely to such
      // elements, so we'll need to match the UnrecognizedElementsPolicy
      // message output for its enforceConfigurablePolicyCondition.

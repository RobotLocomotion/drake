...
Eigen::Vector3d ParseVector(const sdf::ElementPtr node,
  ...
  if (!successful) {
    throw std::runtime_error(
        fmt::format("Unable to read the value of the <{}> tag.", element_name));
  }
...
const Frame<double>& ParseFrame(const sdf::ElementPtr node,
  ...
  if (!plant->HasFrameNamed(frame_name)) {
    throw std::runtime_error(
        fmt::format("Frame: {} specified for <{}> does not exist in the model.",
...
void AddDrakeJointFromSpecification(const sdf::ElementPtr node,
  ...
  if (!node->HasAttribute("type")) {
    throw std::runtime_error("drake:joint: Unable to find the type tag.");
  }
  const std::string joint_type = node->Get<std::string>("type");
  if (!node->HasAttribute("name")) {
    throw std::runtime_error("drake:joint: Unable to find the name tag.");
  }
  ...
  if (node->HasElement("pose")) {
    throw std::runtime_error("drake:joint does not yet support the pose tag.");
  }

  const Frame<double>& parent_frame =
      ParseFrame(node, plant, "drake:parent_frame");
  const Frame<double>& child_frame =
      ParseFrame(node, plant, "drake:child_frame");

  if (joint_type.compare("planar") == 0) {
    Vector3d damping = ParseVector(node, "drake:damping");
    plant->AddJoint(std::make_unique<PlanarJoint<double>>(
        joint_name, parent_frame, child_frame, damping));
  } else {
    throw std::runtime_error("ERROR: drake:joint " + joint_name +
...
const LinearBushingRollPitchYaw<double>& AddBushingFromSpecification(
  ...
  auto read_vector = [node](const char* element_name) -> Eigen::Vector3d {
    return ParseVector(node, element_name);
  };

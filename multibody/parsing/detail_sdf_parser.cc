...
// object. We need to split declaration & definition to add the C++ attribute.
sdf::Errors LoadSdf(sdf::Root*, const DataSource&, const sdf::ParserConfig&)
  ...
  if (model.Element()->HasElement("drake:joint")) {
    for (sdf::ElementPtr joint_node =
      ...
      AddDrakeJointFromSpecification(
          joint_node, model_instance, plant);
    }
  }
...
sdf::InterfaceModelPtr ParseNestedInterfaceModel(
  ...
  if (is_urdf) {
    ...
    if (!inner_errors.empty()) {
      for (auto& item : inner_errors) {
...
std::optional<ModelInstanceIndex> AddModelFromSdf(
  ...
  if (!errors.empty()) {
    PropagateErrors(errors, workspace.diagnostic);
    return std::nullopt;
  }
...
std::vector<ModelInstanceIndex> AddModelsFromSdf(
  ...
  // TODO(jwnimmer-tri) Actually use the workspace.diagnostic_policy,
  // thoughout this function and its helpers.

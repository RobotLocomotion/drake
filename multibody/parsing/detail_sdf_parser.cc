...
void AddDrakeJointFromSpecification(const sdf::ElementPtr node,
  ...
  if (!node->HasAttribute("type")) {
    throw std::runtime_error("Unable to find the type tag.");
  }
  const std::string joint_type = node->Get<std::string>("type");
  if (!node->HasAttribute("name")) {
    throw std::runtime_error("Unable to find the name tag.");
  }
  const std::string joint_name = node->Get<std::string>("name");

  const Frame<double>& parent_frame =
      ParseFrame(node, plant, "drake:joint_parent_frame");
  const Frame<double>& child_frame =
      ParseFrame(node, plant, "drake:joint_child_frame");

  if (joint_type.compare("planar") == 0) {
    Vector3d damping = ParseVector(node, "drake:joint_damping");

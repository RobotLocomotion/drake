...
sdf::ParserConfig MakeSdfParserConfig(
    const ParsingWorkspace&, const std::string& /* root_dir */,
    const SDFormatDiagnostic& /* sdf_diagnostic */);
...
sdf::InterfaceModelPtr ParseNestedInterfaceModel(
    ...
    const SDFormatDiagnostic& sdf_diagnostic) {
  const sdf::ParserConfig parser_config =
      MakeSdfParserConfig(workspace, root_dir, sdf_diagnostic);
...
// TODO(14837) We won't need this when drake:ellipsoid and drake:capsule are no
// more.
// The callback for SDFormat to auto compute inertia for geometry types it
// doesn't recognize (such as drake:capsule and drake:ellipsoid).
std::pair<bool, std::optional<gz::math::Inertiald>> CalculateGeometryInertia(
    double density, const std::optional<sdf::Geometry>& geometry,
    const SDFormatDiagnostic& sdf_diagnostic) {
  if (!geometry.has_value()) {
    return {false, std::nullopt};
  }
  // This should only be called on types that SDFormat didn't recognize.
  DRAKE_DEMAND(geometry->Type() == sdf::GeometryType::EMPTY);

  if (!(geometry->Element()->HasElement("drake:capsule") ||
        geometry->Element()->HasElement("drake:ellipsoid"))) {
    // Not a drake extension type; it's not supported.
    return {false, std::nullopt};
  }

  // Use our logic to extract a shape *we* recognize.
  std::unique_ptr<geometry::Shape> shape = MakeShapeFromSdfGeometry(
      sdf_diagnostic, *geometry, /* resolve_filename= */ [](auto, auto) {
        // We shouldn't have to resolve filenames.
        return "";
      });
  if (!shape) return {true, std::nullopt};

  const SpatialInertia<double> M_GGo_G = CalcSpatialInertia(*shape, density);

  return {true, MakeGzInertia(M_GGo_G)};
}
...
sdf::ParserConfig MakeSdfParserConfig(
    const ParsingWorkspace& workspace, const std::string& root_dir,
    const SDFormatDiagnostic& sdf_diagnostic) {
  ...
  parser_config.RegisterCustomModelParser(
      [&workspace, &root_dir, &sdf_diagnostic](
  ...
  parser_config.RegisterCustomGeometryInertiaCalc(
      [&sdf_diagnostic](sdf::Errors&,
                        const sdf::CustomInertiaCalcProperties& properties) {
        return CalculateGeometryInertia(properties.Density(),
                                        properties.Geometry(), sdf_diagnostic);
      });
...
std::optional<ModelInstanceIndex> AddModelFromSdf(
  ...
  sdf::ParserConfig parser_config =
      MakeSdfParserConfig(workspace, data_source.GetRootDir(), diagnostic);

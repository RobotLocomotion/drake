...
def _run(*, temp_dir: Path, package_name: str, urls: List[str], sha256: str,
    ...
    output_dir_temp = tempfile.mkdtemp(
        dir=output_dir.parent,
        prefix=f"temp.{output_dir.name}.")
    ...
    try:
        ...
        try:
            ...
            # This can happen if the output_dir was just created by some other
            # downloader process while we were downloading. Just below (after
            # some cleanup), we'll confirm that the output_dir actually exists.
            pass
    finally:
        try:

...
std::string NamespaceJoin(std::string prefix, std::string name) {
  return ScopedName::Join(prefix, name).to_string();
}

AddWeld ApplyDirectiveNamespace(const AddWeld& orig, std::string prefix) {
...
void FlattenModelDirectivesInternal(
  ...
  for (auto& directive : directives.directives) {
    if (directive.add_directives) {
      ...
      result.add_frame =
        ApplyDirectiveNamespace(*directive.add_frame, prefix);
      if (scoped_name.get_namespace().empty()) {
        // `name` was bare, so infer the namespace of the X_PF base frame.
        std::string base_frame_prefix = std::string(
          ScopedName::Parse(
            *result.add_frame->X_PF.base_frame).get_namespace());
      ...
      result.add_collision_filter_group =
        ApplyDirectiveNamespace(*directive.add_collision_filter_group, prefix);
      out->directives.push_back(result);
    } else {
      DRAKE_UNREACHABLE();
    }

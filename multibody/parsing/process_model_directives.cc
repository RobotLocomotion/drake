...
std::string NamespaceJoin(std::string model_namespace, std::string name) {
...
AddFrame ApplyDirectiveNamespace(const AddFrame& orig,
  ...
  const ScopedName scoped_name = ScopedName::Parse(orig.name);
  // Handle the general case of prepending the model namespace.
  if (!model_namespace.empty()) {
    result.name = NamespaceJoin(model_namespace, orig.name);
    result.X_PF.base_frame =
      NamespaceJoin(model_namespace, *orig.X_PF.base_frame);
  }
  ...
  if (scoped_name.get_namespace().empty()) {
    ...
    const std::string base_frame_namespace(
      ScopedName::Parse(*orig.X_PF.base_frame).get_namespace());
    result.name =
      NamespaceJoin(model_namespace,
        NamespaceJoin(base_frame_namespace,
                      std::string(scoped_name.get_element())));
  }
  return result;
}
...
void FlattenModelDirectivesInternal(
  ...
  for (auto& directive : directives.directives) {
    if (directive.add_directives) {
      ...
      result.add_weld = ApplyDirectiveNamespace(*directive.add_weld,
                                                model_namespace);
      ...
      result.add_model_instance =
        ApplyDirectiveNamespace(*directive.add_model_instance, model_namespace);
      out->directives.push_back(result);
      ...
      ModelDirective result;
      result.add_collision_filter_group =
        ApplyDirectiveNamespace(*directive.add_collision_filter_group,
                                model_namespace);
      out->directives.push_back(result);
    } else {
      DRAKE_UNREACHABLE();

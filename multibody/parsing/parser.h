...
class Parser final {
 public:
  ...
  /// scene_graph). Is specified, the resulting parser will apply
  /// `model_name_prefix` to the names of any models parsed.
  ///
  /// @pre Either the given `builder` contains a MultibodyPlant system named
  /// "plant" or else the provided `plant` is non-null.
  /// @pre If both `builder` and `plant` are specified, then the plant must be
  /// one of the systems in the builder.

...
/// Parses model description input texts into a MultibodyPlant and (optionally)
/// a SceneGraph. A variety of input formats are supported, and are recognized
/// by filename suffixes:
///
...
class Parser final {
 public:
  ...
  /// to @p plant. It is an error to call this using an SDFormat file with more
  /// than one root-level `<model>` element, or a model directives file that
  /// adds more than one model instance.
  ///
  ...
  /// nesting in SDFormat.
  /// TODO(rpoyner-tri): documentation of nesting in model directives?

...
class Parser final {
 public:
  ...
  /// Parses the SDF or URDF file named in @p file_name and adds its model(s)
  /// to @p plant.
  ///
  ...
  ///   tag will be used.  If an SDF file contains multiple `<model>` tags,
  ///   this argument must be empty.
  ...
  /// one `<model>` element.
  ///
  /// This method is a version of AddModelsFromFile where exactly one model is
  /// added.  See that method for more details.

...
//   name of the parsed model will be prefixed with the parent_model_name, using
//   the scope delimiter "::". The body name does not get prefixed.
// @param workspace
...
// format and contain only a _single_ object specification.
//
// See AddModelFromMesh for the details on what model is added to the plant.
//
// @throws std::exception if the file is not in accordance with the OBJ
// specification containing a message with a list of errors encountered while
// parsing the file.
...
// @param workspace
//   The ParsingWorkspace.
// @returns The singleton set of the one model instance index for the newly
//   added model. This set might be empty if there were parsing errors reported
//   through the workspace.diagnostic policy.
std::vector<ModelInstanceIndex> AddModelsFromMesh(

...
void CollisionFilterGroupResolver::AddGroup(
  ...
  const std::optional<ScopedName> full_group_name = FullyQualify(
      model_instance, group_name);
  if (!full_group_name) {
  ...
  for (const auto& body_name : body_names) {
    ...
    ScopedName scoped_name = FullyQualify(body_name, model_instance);
    if (!CheckLegalName(diagnostic, body_name, "body name")) { continue; }

    const Body<double>* body{};
    if (plant_->HasModelInstanceNamed(scoped_name.get_namespace())) {
      ModelInstanceIndex body_model =
          plant_->GetModelInstanceByName(scoped_name.get_namespace());
      ...
      body = FindBody(scoped_name.get_element(), body_model);
    }
    if (!body) {
...
std::optional<ScopedName> CollisionFilterGroupResolver::FullyQualify(
    std::optional<ModelInstanceIndex> model_instance,
    const std::string& name) const {
  return ScopedName::join(
      model_instance.has_value() ? plant_->GetModelInstanceName(*model_instance)
                                 : {},
      name);
}

...
std::string PackageMap::RemotePackageParams::ToJson() const {
  return yaml::SaveJsonString(*this);
}

bool operator==(const PackageMap::RemotePackageParams& left,
                const PackageMap::RemotePackageParams& right) {
...
class NeedsFetch {
 public:
  ...
  /* Returns a lock_guard for our owns mutex. */
...
class PackageData {
 public:
  ...
  static PackageData MakeRemote(PackageMap::RemotePackageParams params) {
    PackageData result;
    result.remote_ = std::move(params);
  ...
  bool is_local() const { return !remote_.has_value(); }

  /* Returns true iff this package was added using AddRemote(). */
  bool is_remote() const { return remote_.has_value(); }

  /* Returns the remote params; throws if this not is_remote(). */
  const PackageMap::RemotePackageParams& remote_params() const {
  ...
  /* For local packages, returns the local path; for remote packages, returns
  the first URL. */
  const std::string& display_path() const {
    return remote_.has_value() ? remote_.value().urls.front() : path_;
  }
  ...
  or remote) must match, but the deprecation or fetch status can differ. In
 ...
 private:
  ...
  /* When a package is added as an AddRemote(), this starts out 'true'.
  After the downloader finishes successfully, it changes to 'false'. */
  NeedsFetch needs_fetch_;

  /* Directory in which the manifest resides. When is_remote() is true and the
  package is unfetched, this string will be empty. */
  std::string path_;

  /* If this was added as an AddRemote(), this will contain the details, even
  ...
  std::optional<PackageMap::RemotePackageParams> remote_;
...
void PackageMap::AddRemote(std::string package_name,

  ...
  void ParseAsset(XMLElement* node) {
    ...
    for (XMLElement* mesh_node = node->FirstChildElement("mesh"); mesh_node;
      ...
      if (ParseStringAttribute(mesh_node, "file", &file)) {
        ...
        // The file not existing at this stage is not an error. Absorb errors
        // from canonical().
        std::error_code ec;
        filename = std::filesystem::canonical(filename, ec);
        // TODO(rpoyner-tri): deal with errors not dealt with below?

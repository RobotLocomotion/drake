...
class MujocoParser {
 public:
  ...
  void ProcessIncludeTags(tinyxml2::XMLElement* node,
    ...
    if (std::string(node->Value()) == "include") {
      ...
      location is relative to the directory of the main MJCF file. If the file
      is not in the same directory, it should be prefixed with a relative
      path.*/
      ...
      tinyxml2::XMLDocument* xml_doc = node->GetDocument();
      tinyxml2::XMLElement* parent = node->Parent()->ToElement();
      tinyxml2::XMLElement* node_in_parent = node;
    ...
    while (child) {
      ProcessIncludeTags(child, parent_mjcf_path);
      tinyxml2::XMLElement* to_delete =
          (std::string(node->Value()) == "include") ? child : nullptr;
      child = child->NextSiblingElement();
  ...
  std::pair<std::optional<ModelInstanceIndex>, std::string> Parse(
    ...
    ProcessIncludeTags(xml_doc->RootElement(), main_mjcf_path_);

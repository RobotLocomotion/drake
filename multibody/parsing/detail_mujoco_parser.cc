...
class MujocoParser {
 public:
  ...
  SpatialInertia<double> ParseInertial(XMLElement* node) {
    RigidTransformd X_BF_B = ParseTransform(node);
    double mass;
    ...
    RotationalInertia<double> I_BF_B = I_BF_F.ReExpress(X_BF_B.rotation());
    SpatialInertia<double> M_BBo_B =
        SpatialInertia<double>::MakeFromCentralInertia(
            mass, X_BF_B.translation(), I_BF_B);
    return M_BBo_B;
  }

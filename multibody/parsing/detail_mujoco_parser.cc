...
class MujocoParser {
 public:
  ...
  MujocoGeometry ParseGeometry(XMLElement* node, int num_geom) {
    ...
    if (type == "plane") {
      // We interpret the MuJoCo infinite place as a half-space.
    ...
    if (ParseVectorAttribute(node, "friction", &friction)) {
      ...
      if (friction[1] != 0.0 || friction[2] != 0.0) {
        drake::log()->warn(
            ...
            "attribution of {} are unsupported and will be ignored.",
            node->Name());
      }
    }
  ...
  void ParseCompiler(XMLElement* node) {
    ...
    if (ParseStringAttribute(node, "angle", &angle)) {
      if (angle == "degree") {
        ...
        drake::log()->warn(
            "Unknown value {} for the angle attribute will be ignored.", angle);
      }
    }

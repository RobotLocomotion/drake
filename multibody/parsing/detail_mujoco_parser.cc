...
class MujocoParser {
 public:
  ...
  MujocoGeometry ParseGeometry(XMLElement* node, int num_geom,
    ...
    // rgba takes precidence over materials, so must be parsed after.
      ...
      if (ParseStringAttribute(mesh_node, "file", &file)) {
        ...
        if (std::filesystem::exists(filename)) {
          ...
          log()->warn(
              ...
              "load {}, but that file does not exist. If you have "
              "built Drake from source, running\n\n bazel run "

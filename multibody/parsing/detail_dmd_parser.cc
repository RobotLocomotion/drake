...
std::vector<ModelInstanceInfo> ProcessModelDirectives(
    ...
    const ParsingWorkspace& workspace) {
  std::vector<ModelInstanceInfo> added_models;
  ProcessModelDirectivesImpl(directives, parent_model_name.value_or(""),
...
std::optional<ModelInstanceIndex> DmdParserWrapper::AddModel(
  ...
  if (!model_name.empty()) {
    workspace.diagnostic.Warning(fmt::format(
        "The provided model name '{}' will be ignored; the name provided in"
        " the model directives data will be used instead.", model_name));
  }
  auto indices = AddAllModels(data_source, parent_model_name, workspace);
...
std::vector<ModelInstanceIndex> DmdParserWrapper::AddAllModels(
  ...
  // TODO(rpoyner-tri): diagnostic policy?
  auto infos = ProcessModelDirectives(directives, parent_model_name, workspace);

...
class MeshcatMouseSpring final : public systems::LeafSystem<double> {
 ...
 private:
  ...
  // reading frame names from `scene_graph` and mapping frames to bodies via the
  // plant.
...
path segment matches `path`, or an invalid BodyIndex if none matches.
`path_to_body` maps each body's scoped path segment (e.g. "model/body", or just
"body" for the default and world model instances) to its index. Exposed for
testing; see the .cc for the exact matching rules. */

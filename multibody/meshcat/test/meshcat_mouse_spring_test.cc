...
// Directly exercises the path-segment-to-body matching that powers dragging,
// independent of any MultibodyPlant / SceneGraph wiring.
GTEST_TEST(FindBodyForPathTest, BoundedSegmentMatch) {
  const BodyIndex ball(1);
  const BodyIndex link(2);
  const std::map<std::string, BodyIndex> path_to_body = {
      {"ball", ball},        // A default-instance body.
      {"robot/link", link},  // A model-instance-scoped body.
  };

  // A segment matches when bounded by '/' on the left and by '/' or end-of-path
  // on the right, regardless of the visualization-layer prefix...
  EXPECT_EQ(internal::FindBodyForPath(path_to_body, "/drake/visualizer/ball"),
            ball);
  EXPECT_EQ(
      internal::FindBodyForPath(path_to_body, "/drake/visualizer/ball/<geom>"),
      ball);
  EXPECT_EQ(internal::FindBodyForPath(
                path_to_body, "/drake/illustration/robot/link/some_geometry"),
            link);

  // ...but a segment must not match as a substring of a larger path segment.
  EXPECT_FALSE(
      internal::FindBodyForPath(path_to_body, "/drake/some_ball").is_valid());
  EXPECT_FALSE(
      internal::FindBodyForPath(path_to_body, "/drake/ballistic").is_valid());
  EXPECT_FALSE(internal::FindBodyForPath(path_to_body, "/drake/robot/link_2")
                   .is_valid());

  // A leading segment boundary is required (so "ball" alone can't match "ball"
  // with no preceding '/'), and unrelated paths don't match.
  EXPECT_FALSE(internal::FindBodyForPath(path_to_body, "ball").is_valid());
  EXPECT_FALSE(
      internal::FindBodyForPath(path_to_body, "/drake/visualizer").is_valid());
  EXPECT_FALSE(internal::FindBodyForPath(path_to_body, "").is_valid());
}

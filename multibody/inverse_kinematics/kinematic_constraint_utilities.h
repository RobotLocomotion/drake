 ...
 * same as @p q_v.
 * If they are not the same, then reset @p context's generalized positions and
 * velocities to @p q_v. Otherwise, leave @p context unchanged. The intention is
 * to avoid dirtying the computation cache, given it is ticket-based rather than
 * hash-based.
 */

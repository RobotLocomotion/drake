...
namespace {
void InitializeY(const Eigen::Ref<const Eigen::VectorXd>&, Eigen::VectorXd* y, double y_value) {
  (*y)(0) = y_value;
}

void InitializeY(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y, double y_value) {
...
void Penalty(const MultibodyPlant<double>&, const systems::Context<double>&,
  ...
  double penalty;
  const double x = ScaleDistance(distance, minimum_distance, threshold_distance);

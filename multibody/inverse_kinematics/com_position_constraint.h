...
namespace multibody {
/**
 * Impose the constraint CenterOfMass(q) - p_EC = 0, where CenterOfMass(q)
 * computes the center-of-mass (COM) position from robot generalized position q,
 * expressed in a frame E. p_EC ∈ ℝ³ is the variable representing robot CoM (C)
 * position expressed in frame E. The evaluated variables are [q;r], where q is
 * the generalized position vector of the entire plant.
 */

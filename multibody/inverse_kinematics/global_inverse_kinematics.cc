...
GlobalInverseKinematics::AddWorldPositionConstraint(
  ...
  Vector3<Expression> p_WQ = p_WBo_[body_idx] + R_WB_[body_idx] * p_BQ;
...
GlobalInverseKinematics::AddWorldRelativePositionConstraint(
    ...
    const Eigen::Vector3d& box_lb_F, const Eigen::Vector3d& box_ub_F,
    const RigidTransformd& X_WF) {
  Vector3<Expression> p_WQ = p_WBo_[body_idx_B] + R_WB_[body_idx_B] * p_BQ;
  Vector3<Expression> p_WP = p_WBo_[body_idx_A] + R_WB_[body_idx_A] * p_AP;
  return prog_.AddLinearConstraint(
      X_WF.inverse().cast<Expression>() * (p_WQ - p_WP), box_lb_F, box_ub_F);
}

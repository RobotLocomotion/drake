...
GlobalInverseKinematics::GlobalInverseKinematics(
  ...
  for (JointIndex joint_index{0}; joint_index < plant_.num_joints();
       ++joint_index) {
    body_to_joint_map.emplace(
        plant_.get_joint(joint_index).child_body().index(), joint_index);
  }
  ...
  const std::vector<const Body<double>*> weld_to_world_bodies =
      plant_.GetBodiesWeldedTo(plant_.world_body());
  std::unordered_set<BodyIndex> weld_to_world_body_index_set;
  for (const auto body : weld_to_world_bodies) {
    weld_to_world_body_index_set.insert(body->index());
  }
...
void GlobalInverseKinematics::ReconstructGeneralizedPositionSolutionForBody(
  ...
  const Joint<double>& joint =
      plant_.get_joint(body_to_joint_map.at(BodyIndex{body_idx}));
  const Body<double>& parent = joint.parent_body();
  if (weld_to_world_body_index_set.count(BodyIndex{body_idx}) == 0) {
...
Eigen::VectorXd GlobalInverseKinematics::ReconstructGeneralizedPositionSolution(
  ...
  while (num_link_visited < plant_.num_bodies()) {
    if (!is_link_visited[body_idx]) {
      ...
      int parent_idx = -1;
      if (plant_.get_body(BodyIndex{body_idx}).is_floating()) {
        parent_idx = plant_.world_body().index();
      } else {
        parent_idx = plant_.get_joint(body_to_joint_map.at(BodyIndex{body_idx}))
                         .parent_body()
                         .index();
      }
...
void GlobalInverseKinematics::AddJointLimitConstraint(
  ...
  if (plant_.get_body(BodyIndex{body_index}).is_floating()) {
    throw std::runtime_error(
  ...
  for (JointIndex joint_index{0}; joint_index < plant_.num_joints();
       ++joint_index) {
    if (plant_.get_joint(joint_index).child_body().index() ==

...
ComPositionConstraint::ComPositionConstraint(
  ...
  if (model_instances.has_value()) {
    throw std::runtime_error(
        "ComPositionConstraint: currently we only accept std::nullptr as "
        "model_instances");
  }
  if (model_instances.has_value()) {
    model_instances_ = model_instances.value();
  } else {
    model_instances_.reserve(plant->num_model_instances() - 1);
    for (ModelInstanceIndex model_instance_index(1);
         model_instance_index < plant->num_model_instances();
         ++model_instance_index) {
      model_instances_.push_back(model_instance_index);
    }
  }
  if (plant_context == nullptr)
    throw std::invalid_argument("plant_context is nullptr.");
  this->set_description(plant->GetSystemName() + " CoM constraint");
}
...
// T can be either double or AutoDiffXd
template <typename T>
void EvalConstraintGradient(const systems::Context<T>&,
                            const MultibodyPlant<T>&,
                            const std::vector<ModelInstanceIndex>&,
                            const Frame<T>&, const Vector3<T>& p_EC,
                            const Eigen::Ref<const VectorX<T>>& x,
                            VectorX<T>* y) {
  *y = p_EC - x.template tail<3>();
}

// We can evaluate the gradient of the constraint with MultibodyPlant<double>,
// using the Jacobian function in MBP<double>.
...
void DoEvalGeneric(const MultibodyPlant<T>& plant, systems::Context<T>* context,
                   const std::vector<ModelInstanceIndex>& model_instances,
                   FrameIndex expressed_frame_index,

...
void TestComInPolyhedronConstraint(
  ...
  const Vector3<T> p_EC =
      plant->CalcRelativeTransform(*plant_context,
                                   plant->get_frame(expressed_frame_index),
                                   plant->world_frame()) *
      p_WC;
  const VectorX<T> y_expected = A * p_EC;
...
TEST_F(IiwaKinematicConstraintTest,
       ComInPolyhedronConstraint_empty_model_instance) {
...
GTEST_TEST(DualIiwaTest, ComInPolyhedronConstraint_model_instance) {

...
void TestComInPolyhedronConstraint(
  ...
  AutoDiffVecXd q_autodiff = math::InitializeAutoDiffFromValueMatrix(q_val);
  ...
  EXPECT_TRUE(CompareMatrices(
      y, math::ExtractValueMatrixFromAutoDiff(y_autodiff), tol));
  ...
  q_autodiff =
      math::InitializeAutoDiffFromValueAndGradientMatrix(q_val, q_grad);
  plant_autodiff->GetMutablePositions(plant_context_autodiff) = q_autodiff;
  constraint.Eval(q_autodiff, &y_autodiff);

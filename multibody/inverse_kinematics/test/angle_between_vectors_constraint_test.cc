...
TEST_F(IiwaKinematicConstraintTest, AngleBetweenVectorsConstraint) {
  ...
  const FrameIndex frameA_index = iiwa_link_frame_indices_[3];
  const FrameIndex frameB_index = iiwa_link_frame_indices_[6];
...
TEST_F(TwoFreeBodiesConstraintTest, AngleBetweenVectorsConstraint) {
  ...
  AngleBetweenVectorsConstraint constraint_satisfied(
      *two_bodies_autodiff_, body1_index_, n_A, body2_index_, n_B, angle - 0.01,
      angle + 0.01,
      dynamic_cast<MultibodyTreeContext<AutoDiffXd>*>(context_autodiff_.get()));
  ...
  AngleBetweenVectorsConstraint constraint_unsatisfied(
      *two_bodies_autodiff_, body1_index_, n_A, body2_index_, n_B, angle - 0.02,
      angle - 0.01,
      dynamic_cast<MultibodyTreeContext<AutoDiffXd>*>(context_autodiff_.get()));
...
TEST_F(IiwaKinematicConstraintTest,
  ...
  EXPECT_THROW(
      AngleBetweenVectorsConstraint(
          *iiwa_autodiff_, iiwa_link_frame_indices_[2], Eigen::Vector3d::Zero(),
          iiwa_link_frame_indices_[3], Eigen::Vector3d::Ones(), 0.1, 0.2,
  ...
  EXPECT_THROW(
      AngleBetweenVectorsConstraint(
          *iiwa_autodiff_, iiwa_link_frame_indices_[2], Eigen::Vector3d::Ones(),
          iiwa_link_frame_indices_[3], Eigen::Vector3d::Zero(), 0.1, 0.2,
          dynamic_cast<MultibodyTreeContext<AutoDiffXd>*>(
              context_autodiff_.get())),
      std::logic_error);
  // angle_lower < 0
  ...
  EXPECT_THROW(
      AngleBetweenVectorsConstraint(
          *iiwa_autodiff_, iiwa_link_frame_indices_[2], Eigen::Vector3d::Ones(),
          iiwa_link_frame_indices_[3], Eigen::Vector3d::Zero(), 0.1, 0.09,
          dynamic_cast<MultibodyTreeContext<AutoDiffXd>*>(
              context_autodiff_.get())),
      std::logic_error);
  // angle_upper > pi
  EXPECT_THROW(
      AngleBetweenVectorsConstraint(
          *iiwa_autodiff_, iiwa_link_frame_indices_[2], Eigen::Vector3d::Ones(),
          iiwa_link_frame_indices_[3], Eigen::Vector3d::Zero(), 0.1, 1.1 * M_PI,
          dynamic_cast<MultibodyTreeContext<AutoDiffXd>*>(
              context_autodiff_.get())),
      std::logic_error);
}

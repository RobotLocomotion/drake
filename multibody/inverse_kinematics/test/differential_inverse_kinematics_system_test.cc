...
TEST_F(DifferentialInverseKinematicsTest, Structure) {
  ...
  const Recipe& owned_recipe = dut.recipe();
  EXPECT_EQ(owned_recipe.num_ingredients(), 4);
  EXPECT_EQ(NiceTypeName::Get(owned_recipe.ingredient(0)),
            NiceTypeName::Get<DiffIk::LeastSquaresCost>());
  EXPECT_EQ(NiceTypeName::Get(owned_recipe.ingredient(1)),
            NiceTypeName::Get<DiffIk::CollisionConstraint>());
  EXPECT_EQ(NiceTypeName::Get(owned_recipe.ingredient(2)),
            NiceTypeName::Get<DiffIk::JointCenteringCost>());
  EXPECT_EQ(NiceTypeName::Get(owned_recipe.ingredient(3)),

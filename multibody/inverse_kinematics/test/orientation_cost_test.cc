...
GTEST_TEST(OrientationCostTest, TwoFreeBodies) {
  ...
  auto CheckCases = [](const OrientationCost& orientation_cost,
    ...
    orientation_cost_ad.Eval(q, &y);
    EXPECT_NEAR(y[0], y_expected, 1e-12);

    // MultibodyPlant double, Eval AutoDiffXd
    const VectorX<AutoDiffXd> q_ad = InitializeAutoDiff(q);
    VectorX<AutoDiffXd> y_ad(1);
    orientation_cost.Eval(q_ad, &y_ad);
    EXPECT_NEAR(y_ad[0].value(), y_expected, 1e-12);
    ...
    EXPECT_TRUE(CompareMatrices(y_ad[0].derivatives(), y_ad_ad[0].derivatives(),
                                1e-11));
  };
  ...
  double theta = math::wrap_to(R_AB.ToAngleAxis().angle(), -M_PI/2.0, M_PI/2.0);
  CheckCases(*cost, *cost_ad, q, 1.0 - cos(theta));

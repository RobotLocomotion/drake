...
void TestComPositionConstraint(
  ...
  if constexpr (std::is_same_v<T, double>) {
    ...
    EXPECT_TRUE(CompareMatrices(
        y, math::ExtractValueMatrixFromAutoDiff(y_expected), tol));
  }

  AutoDiffVecXd x_autodiff = math::InitializeAutoDiffFromValueMatrix(x);
  ...
  x_autodiff = math::InitializeAutoDiffFromValueAndGradientMatrix(x, x_grad);
  plant_autodiff->GetMutablePositions(plant_context_autodiff) =
      x_autodiff.head(plant->num_positions());
  constraint.Eval(x_autodiff, &y_autodiff);

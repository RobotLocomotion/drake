...
InverseKinematics::InverseKinematics(
  ...
  // Obey joint locking.
  const Eigen::VectorXd q = plant.GetPositions(*context_);
  ...
  for (JointIndex i{0}; i < plant.num_joints(); ++i) {
    ...
    if (joint.is_locked(*context_)) {
      const int start = joint.position_start();
      const int size = joint.num_positions();
      lb.segment(start, size) = q.segment(start, size);
      ub.segment(start, size) = q.segment(start, size);

...
// TODO(russt): Move this to a more central location.
// Makes the matrix Nplus(q) by calling MapQDotToVelocity repeatedly.
Eigen::MatrixXd MakeQDotToVelocityMap(const MultibodyPlant<double>& plant,
                                      const systems::Context<double>& context) {
  Eigen::MatrixXd Nplus(plant.num_velocities(), plant.num_positions());
...
DifferentialInverseKinematicsResult DoDifferentialInverseKinematics(
  ...
  if (parameters.get_joint_position_limits()) {
    if (Nplus) {
      ...
      const auto SNplus = Nplus->sparseView();
...
DifferentialInverseKinematicsResult DoDifferentialInverseKinematics(
  ...
  const Eigen::MatrixXd Nplus = MakeQDotToVelocityMap(plant, context);

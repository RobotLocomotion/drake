...
class DifferentialInverseKinematicsParameters {
 public:
   ...
   * N * v_next = K * (q_nominal - q_current).
 ...
 *                         + |P * (N*v_next - K*(q_nominal - q_current))|^2
 *
 *   s.t.
 ...
 * MultibodyPlant::MapVelocityToQDot() for more details (you can even obtain
 * N by calling that method multiple times). By default, it is taken to be
 * the identity matrix. If dim(q) != dim(v), then you *must* provide N.

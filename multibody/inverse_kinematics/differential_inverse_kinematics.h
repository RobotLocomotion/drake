...
class DifferentialInverseKinematicsParameters {
 public:
   ...
   * v_next = N⁺ * K * (q_nominal - q_current).
 ...
 *                         + |P * (v_next - N⁺*K*(q_nominal - q_current))|^2
 *
 *   s.t.
 ...
 * constants.
 * @param Nplus (optional) matrix which maps v = N⁺(q)⋅q̇. See
 * MultibodyPlant::MapQDotToVelocity() for more details (you can even obtain
 * Nplus by calling that method multiple times). By default, it is taken to be
 * the identity matrix. If dim(q) != dim(v), then you *must* provide Nplus.

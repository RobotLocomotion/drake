...
class HydroelasticEngine<T>::Impl final : public geometry::ShapeReifier {
 public:
  ...
  const HydroelasticGeometry<T>* get_model(GeometryId id) const {
  ...
  optional<ContactSurface<T>> CalcContactSurface(
    ...
    auto grad_level_set_R =
        std::make_unique<geometry::SurfaceMeshFieldLinear<Vector3<T>, T>>(
            "grad_h_MN_M", std::move(grad_level_set_R_surface),
            surface_R.get());
    ...
    // TODO(amcastro-tri): Update this when SeanCurtis-TRI introduces
    // ContactSurface::SwapMAndN(). Notice that this will require re-expressing
    // `surface` and `grad_level_set` in the S frame, according to
    // ContactSurface's contract.
...
template <typename T>
HydroelasticEngine<T>::HydroelasticEngine() : impl_(new Impl()) {}

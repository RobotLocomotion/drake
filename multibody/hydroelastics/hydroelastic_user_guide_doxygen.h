 ...
 */
 ...
 /**  @defgroup hydroelastic_user_guide Hydroelastic User Guide
      @ingroup multibody
...
Hydroelastic Contact is another compliant contact formulation. It was originally introduced by Elandt et al in A pressure field model for fast, robust approximation of net contact force and moment between nominally rigid objects. In Drake, we refer to this model as the “hydroelastic” model. It differs from point contact in how it characterizes the contact. Rather than a single point, it imagines an entire contact surface. This surface is an approximation of the contact surface as visualized in Figure 2(b).
...
Compliant objects are represented by tetrahedral meshes (although, in this guide, they’ll typically be illustrated as 2D triangle meshes). The Drake data type is VolumeMesh. The pressure fields on those meshes are piecewise linear approximations of the pressure field. That means the pressure values are continuous inside the mesh, but not smooth; the spatial gradient of the pressure field is known to be discontinuous at the tetrahedron boundaries. The maximum pressure of the field is typically located on (or near) the geometry’s medial axis. That maximum pressure value is finite.

The resulting contact surface is likewise a discrete mesh. This mesh may be built of nothing but triangles or polygons. The choice of representation has various implications on the computation of contact forces (see below). The pressure on the contact surface is likewise a piecewise linear function -- the pressure across each face of the contact surface mesh is a linear function. As with the pressure field defined on the inside of a VolumeMesh, the pressure field on the ContactSurface is continuous but not smooth.

...
@subsubsection hug_hydroelastication Automatic Hydroelastication

The automatic hydroelastication feature offers a quick-start path to using
...
With automatic hydroelastication, the results will be approximate, but using
...
To use automatic hydroelastication, all that is needed is to set a
...
controlled by drake::geometry::HydroelasticationConfig, and can be changed for
each application. The threshold size for geometries to ignore can also be
changed via that configuration structure.

Having used hydroelastication to get up and running, it may still be useful to
...
the hydroelastication defaults for the annotated geometries.

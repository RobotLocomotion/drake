...
/** %FemElement is the base class for spatially discretized FEM elements.
 ...
 eliminate all heap allocations. Derived FEM elements must inherit from
 this base class and implement the interface this class provides. The derived
 FEM elements must also be accompanied by a corresponding traits class that
 declares the compile time quantities and type declarations that this base class
 requires.
...
class FemElement {
 public:
  ...
  /** The type of ElementCacheEntry that is compatible with this %FemElement. */
  ...
  /** The dimension of the co-domain of the solution to the PDE that the FEM
   element is solving for. */
  ...
  /** Number of quadrature points in this element at which element quantities
   are evaluated. */
  ...
  /** Number of degrees of freedom associated with this element. This is equal
   to the number of generalized positions that this element has. */
 ...
 protected:
  /* Constructs a new FEM element. The constructor is made protected because
   ...
   @pre Entries in node_indices must be valid.*/

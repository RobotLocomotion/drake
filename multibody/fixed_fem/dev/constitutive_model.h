...
class ConstitutiveModel {
 public:
  ...
  /* Calculates the energy density in reference configuration, in unit J/m³,
   given the `data` on deformation gradient related quantities.
   @pre `Psi != nullptr`. */
  ...
  /* Calculates the First Piola stress, in unit Pa, given the `data` on
   deformation gradient related quantities.
   @pre `P != nullptr`. */
  ...
  /* Calculates the derivative of first Piola stress with respect to the
   deformation gradient, given the `data` on deformation gradient related
  quantities. The stress derivative dPᵢⱼ/dFₖₗ is a 4-th order tensor that is
  flattened to a 9-by-9 matrix. The 9-by-9 matrix is organized into 3-by-3
  blocks of 3-by-3 submatrices. The ik-th entry in the jl-th block corresponds
  to the value dPᵢⱼ/dFₖₗ. Let A denote the fourth order tensor dP/dF, then A is
  flattened to a 9-by-9 matrix in the following way:

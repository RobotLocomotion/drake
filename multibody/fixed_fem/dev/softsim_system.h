...
/** A minimum drake system (see systems::System) for simulating the dynamics of
 ...
 z-positions (measured and expressed in the world frame) of the jth vertex are
 ...
 positions of the vertices of the body.
...
class SoftsimSystem final : public systems::LeafSystem<T> {
 public:
  ...
  /* Construct a SoftsimSystem with the prescribed discrete time step.
   @pre dt > 0. */
  explicit SoftsimSystem(double dt);

  // TODO(xuchenhan-tri): Identify deformable bodies with actual identifiers,
  //  which would allow deleting deformable bodies.
  /** Adds a deformable body modeled with linear simplex element, linear
   quadrature rule and the given DeformableBodyConfig. Returns the index of
   the newly added deformable body.

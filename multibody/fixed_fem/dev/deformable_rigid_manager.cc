...
void DeformableRigidManager<T>::DeclareCacheEntries() {
  ...
  /* The free motion rigid velocities depend on discrete states and all external
   forces applied through input ports. */
  std::set<systems::DependencyTicket>
      free_motion_rigid_velocities_dependency_tickets{
          systems::System<T>::xd_ticket(),
          this->plant().input_port_ticket(
              this->plant().get_applied_spatial_force_input_port().get_index()),
          this->plant().input_port_ticket(
              this->plant()
                  .get_applied_generalized_force_input_port()
                  .get_index())};
  for (ModelInstanceIndex model_instance_index(0);
       model_instance_index < this->plant().num_model_instances();
       ++model_instance_index) {
    free_motion_rigid_velocities_dependency_tickets.emplace(
        this->plant().input_port_ticket(
            this->plant()
                .get_actuation_input_port(model_instance_index)
                .get_index()));
  }

  /* Calculates the free motion rigid velocities. */
  const auto& free_motion_rigid_velocities_cache_entry =
      this->DeclareCacheEntry(
          ...
          std::move(free_motion_rigid_velocities_dependency_tickets));
  free_motion_rigid_velocities_cache_index_ =
      free_motion_rigid_velocities_cache_entry.cache_index();

  /* Calculates the participating free motion velocities for contact. */
...
void DeformableRigidManager<T>::CalcFreeMotionRigidVelocities(
  ...
  /* Notice we are using an explicit Euler scheme here since all forces are
   evaluated at the start of the time step. */

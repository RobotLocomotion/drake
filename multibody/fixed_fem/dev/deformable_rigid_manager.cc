...
void DeformableRigidManager<T>::DeclareCacheEntries() {
  ...
  const auto& velocities_cache_entry = this->DeclareCacheEntry(
      "Velocities for all dofs at previous time step",
...
void DeformableRigidManager<T>::CalcVelocities(
    const systems::Context<T>& context, VectorX<T>* v0) const {
  DRAKE_DEMAND(v0 != nullptr);
  const int num_rigid_velocities = this->plant().num_velocities();
  const int num_deformable_velocities = deformable_model_->num_dofs();
  v0->resize(num_rigid_velocities + num_deformable_velocities);
  v0->head(num_rigid_velocities) = this->plant().GetVelocities(context);
  int dof_offset = num_rigid_velocities;
  for (DeformableBodyIndex deformable_index(0);
       deformable_index < deformable_model_->num_bodies(); ++deformable_index) {
    const VectorX<T>& deformable_v0 =
        EvalFemStateBase(context, deformable_index).qdot();
    v0->segment(dof_offset, deformable_v0.size()) = deformable_v0;
    dof_offset += deformable_v0.size();
  }
  /* Sanity check that all entries in `v0` have been filled. */
  DRAKE_DEMAND(dof_offset == v0->size());
}

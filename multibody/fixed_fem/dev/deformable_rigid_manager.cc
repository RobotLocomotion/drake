...
DeformableRigidManager<T>::CalcDeformableRigidContactPair(
  ...
  std::cout << collision_objects_.pose_in_world(rigid_id) << std::endl;
...
BlockSparseMatrix<T> DeformableRigidManager<T>::CalcContactTangentMatrix(
  ...
  std::cout << "M.rows() = " << M.rows() << ", M.cols() = " << M.cols()
            << std::endl;
  ...
  for (SoftBodyIndex i(0); i < deformable_model_->num_bodies(); ++i) {
    ...
    std::cout << deformable_contact_data[i].num_contact_points() << std::endl;
    ...
    const auto& DC = tangent_matrix_schur_complement.get_D_complement();
    std::cout << "DC.rows() = " << DC.rows() << ", DC.cols() = " << DC.cols()
              << std::endl;
    ++block_index;
  }
  return builder.Build();

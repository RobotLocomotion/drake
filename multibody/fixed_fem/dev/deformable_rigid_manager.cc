...
void DeformableRigidManager<T>::DoCalcDiscreteValues(
  ...
  for (DeformableBodyIndex body(0); body < discrete_state_indexes.size();
    ...
    const auto& body_contact_data = deformable_contact_data[body];
    /* If the deformable body is not in contact, then the free motion state
     is the state at the next time step. Otherwise, evaluate the state at the
     next time step. Note that calling EvalNextFemStateBase() for *all*
     deformable bodies would also be correct but may incur unnecessary copies.
    */
    if (body_contact_data.num_contact_points() == 0) {
      const FemStateBase<T>& free_motion_state =
          EvalFreeMotionFemStateBase(context, body);
      next_discrete_value.head(body_num_dofs) = free_motion_state.q();
      next_discrete_value.segment(body_num_dofs, body_num_dofs) =
          free_motion_state.qdot();
      next_discrete_value.tail(body_num_dofs) = free_motion_state.qddot();
    } else {

          ...
          systems::ValueProducer(
              std::function<void(const systems::Context<T>&, VectorX<T>*)>{
                  [this](const systems::Context<T>& context,
                         VectorX<T>* v_star) {
                    this->CalcContactParticipatingVelocities(context, v_star,
                                                             true);
                  }}),
          {systems::System<T>::xd_ticket(),
      ...
      systems::ValueProducer(
          std::function<void(const systems::Context<T>&, VectorX<T>*)>{
              [this](const systems::Context<T>& context, VectorX<T>* v0) {
                this->CalcContactParticipatingVelocities(context, v0, false);
              }}),
      {systems::System<T>::xd_ticket()});
...
void DeformableRigidManager<T>::CalcTwoWayCoupledContactSolverResults(
  ...
  const VectorX<T>& v_star = EvalContactParticipatingVelocities(context, true);
  ...
  const VectorX<T>& v0 = EvalContactParticipatingVelocities(context, false);
...
template <typename T>
void DeformableRigidManager<T>::CalcContactParticipatingVelocities(
    const systems::Context<T>& context, VectorX<T>* v, bool free_motion) const {
  DRAKE_DEMAND(v != nullptr);
  const std::vector<internal::DeformableContactData<T>>&
      deformable_contact_data = EvalDeformableRigidContact(context);
  ...
  if (free_motion) {
    v->head(num_rigid_velocities) = EvalFreeMotionRigidVelocities(context);
  } else {
  ...
  for (DeformableBodyIndex deformable_index(0);
    ...
    const FemStateBase<T>& fem_state =
        free_motion ? EvalFreeMotionFemStateBase(context, deformable_index)
                    : EvalFemStateBase(context, deformable_index);
    const VectorX<T>& v_deformable = fem_state.qdot();
    ...
    for (int vertex = 0;
      ...
      /* For each participating deformable vertex, look up its (free motion)
       ...
       index. */
      v->template segment<3>(v_offset) = v_deformable.template segment<3>(
          3 * permuted_to_original_indexes[vertex]);
      v_offset += 3;
    }
  }
  /* Sanity check that all entries in `v` has been filled. */

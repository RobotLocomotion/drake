...
void DeformableRigidManager<T>::DeclareCacheEntries() {
  ...
  for (SoftBodyIndex deformable_body_id(0);
        ...
        systems::ValueProducer(
            *model_fem_state,
            std::function<void(const systems::Context<T>&, FemStateBase<T>*)>(
                std::bind(&DeformableRigidManager<T>::CopyToFemState, this,
                          deformable_body_id, std::placeholders::_1,
                          std::placeholders::_2))),
    ...
    const auto& free_motion_cache_entry = this->DeclareCacheEntry(
        ...
        systems::ValueProducer(
            *model_fem_state,
            std::function<void(const systems::Context<T>&, FemStateBase<T>*)>(
                std::bind(&DeformableRigidManager<T>::CalcFemStateStar, this,
                          deformable_body_id, std::placeholders::_1,
                          std::placeholders::_2))),
  ...
  const auto& deformable_contact_data_cache_entry = this->DeclareCacheEntry(
      ...
      systems::ValueProducer(this, &DeformableRigidManager<T>::CalcContactData),
      {systems::System<T>::xd_ticket()});
...
template <typename T>
void DeformableRigidManager<T>::CopyToFemState(
    SoftBodyIndex deformable_body_id, const systems::Context<T>& context,
...
template <typename T>
void DeformableRigidManager<T>::CalcFemStateStar(
    SoftBodyIndex deformable_body_id, const systems::Context<T>& context,
...
template <typename T>
void DeformableRigidManager<T>::CalcContactData(
  ...
  for (SoftBodyIndex i(0); i < num_bodies; ++i) {
    result->emplace_back(this->CalcDeformableContactData(i));
  }
}

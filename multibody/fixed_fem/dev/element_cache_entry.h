...
/** %ElementCacheEntry stores the per element state-dependent quantities used in
 ...
 The type of FemElement and %ElementCacheEntry in correspondence
 ...
 StaticElasticityElement) declares its compatible %ElementCacheEntry through an
 alias declaration `ElementCacheEntryType`. FemElement can also build a
 compatible %ElementCacheEntry via FemElement::MakeElementCacheEntry() by
 calling the constructor of the compatible %ElementCacheEntry that takes the
 element index as the only argument. Therefore, each concrete %ElementCacheEntry
 should provide a constructor that takes an element index and sets all cached
 quantities to their default values. */

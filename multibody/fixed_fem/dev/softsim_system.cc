...
void SoftsimSystem<T>::AdvanceOneTimeStep(
  ...
  for (int i = 0; i < num_bodies(); ++i) {
    ...
    const int size = discrete_value.size();
    const auto& q = discrete_value.head(size / 3);
    const auto& qdot = discrete_value.segment(size / 3, size / 3);
    const auto& qddot = discrete_value.tail(size / 3);
    /* Set up FemState and AdvanceOneTimeStep. */
    ...
    next_discrete_value.head(size / 3) = next_fem_state.q();
    next_discrete_value.segment(size / 3, size / 3) = next_fem_state.qdot();
    next_discrete_value.tail(size / 3) = next_fem_state.qddot();
  }
}

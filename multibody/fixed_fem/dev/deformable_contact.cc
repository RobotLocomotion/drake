  ...
  DeformableContactSurface<T> Intersect(const VolumeMesh<T>& tet_mesh_D,
    ...
    for (VolumeElementIndex tet_index(0); tet_index < tet_mesh_D.num_elements();
      ...
      for (SurfaceFaceIndex tri_index(0); tri_index < surface_R.num_faces();
        ...
        // Computes the centroid of the polygon in both Cartesian and
        // Barycentric coordinates given the centroids and areas of the fan of
        // triangles making up the polygon along with the area of the polygon.
        auto calc_polygon_centroid =
            [](const vector<IntersectionVertex<T>>& tri_centroids,
               const vector<T>& tri_areas,
               const T& poly_area) -> IntersectionVertex<T> {
          DRAKE_DEMAND(tri_areas.size() == tri_centroids.size());
          DRAKE_DEMAND(poly_area > 0);
          const int num_tris = tri_centroids.size();
          Vector3<T> poly_centroid_D = Vector3<T>::Zero();
          Vector4<T> b_poly_centroid = Vector4<T>::Zero();
          for (int i = 0; i < num_tris; ++i) {
            poly_centroid_D +=
                tri_areas[i] / poly_area * tri_centroids[i].cartesian;
            b_poly_centroid += tri_areas[i] / poly_area * tri_centroids[i].bary;
          }
          return {poly_centroid_D, b_poly_centroid};
        };
        ...
        // i ∈ [1, N - 2]. For each triangle, compute its area and centroid.
        vector<T> tri_areas;
        vector<IntersectionVertex<T>> tri_centroids;
        for (int i = 1; i < poly_vertex_count - 1; ++i) {
          tri_areas.emplace_back(calc_area(0, i, i + 1, poly_vertices_D));
          tri_centroids.emplace_back(
              calc_triangle_centroid(0, i, i + 1, poly_vertices_D));
        }
        T poly_area = std::accumulate(tri_areas.begin(), tri_areas.end(), T(0));
        // Use the areas and the centroids of the triangle fan to find the
        // centroid of the polygon.
        const IntersectionVertex<T> poly_centroid =
            calc_polygon_centroid(tri_centroids, tri_areas, poly_area);
        out_poly_data.push_back({poly_area, nhat_D, poly_centroid.cartesian,
                                 poly_centroid.bary, tet_index});
      }
    }

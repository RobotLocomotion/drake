...
conditions. The bar has initial length $L$ in the $y$-direction and the center
of the bar has initial (reference) position $(0,0,0)$. We assume linear
elasticity with Young's modulus $E$ and Poisson ratio $\nu$. We apply Dirichlet
boundary condition at $y = \frac{L}{2}$ and $y = -\frac{L}{2}$ so that the bar
is stretched by a factor of $s$ in the $y$-direction. In other words, the
boundary condition applied on the displacement $\uu = (u_1(x,y,z), u_2(x,y,z),
	u_3(x,y,z))^T$ takes the form
\begin{align*}
	u_2(x,-L,z) = -(s-1)L \\
	u_2(x,L,z) = (s-1)L.
\end{align*}
...
\begin{align*}
	...
	u_3(x,y,0) = 0,
\end{align*}
because otherwise the solution would not be unique. (For a given solution, any
rigid transform in the xz-plane would provide another solution.) All other
boundaries assume zero Neumann (no stress) boundary conditions. We assume zero
...
\begin{align}
	\epsilon_{22} & = \frac{c}{E}\label{eq:e22},                       \\
	\epsilon_{11} & = \epsilon_{33} = \frac{-\nu c}{E}. \label{eq:e11}
\end{align}
Integrating equations \eqref{eq:e22} and \eqref{eq:e11}, we get
\begin{align}
	u_1(x,y,z) & = \frac{-\nu c}{E}x + h_1(y,z), \label{eq:u1} \\
	u_2(x,y,z) & = \frac{c}{E}y + h_2(x,z), \label{eq:u2}      \\
	u_3(x,y,z) & = \frac{-\nu c}{E}z + h_3(x,y), \label{eq:u3}
\end{align}
where $h_1, h_2, h_3$ are arbitrary functions resulting from integration.
Using boundary conditions $u_2(x,L,z) = (s-1)L$ and $u_2(x,-L,z) = -(s-1)L$, we
get
\begin{align*}
	h_2(x,z) = 0, \\
	c = (s-1)E.

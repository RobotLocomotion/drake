...
/* StateUpdater provides the interface to update FemStateBase in FemModelBase.
 ...
 The mapping from `z` to the states usually takes the affine form
...
class StateUpdater {
 public:
  ...
  /* Returns (αₚ, αᵥ, αₐ) (See class documentation). These weights can be
   ...
   the tangent matrix (see FemModelBase). If q̇ or q̈ are not a part of the state,
   their derivatives are set to 0. */

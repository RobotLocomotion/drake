...
class DeformableRigidManager final
 ...
 private:
  ...
  /* Evaluates the Schur complement of the tangent matrix of the deformable body
   ...
   deformable body takes the form M = [A, B; Bᵀ, D] where block A corresponds to
   dofs in contact and block D corresponds to dofs not in contact. The returned
   Schur complement provides information on M/D = A - BD⁻¹Bᵀ, which is
   guaranteed to be symmetric positive definite. */
  ...
  */
  ...
  /* Calculates the tangent matrix used by the contact solver. The number and
   the order of the columns of the calculated tangent matrix is the same as
   the contact Jacobian matrix (see CalcContactJacobian()). The result is
   guaranteed to be symmetric positive definite. */

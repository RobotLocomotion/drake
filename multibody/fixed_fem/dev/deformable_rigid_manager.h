...
class DeformableRigidManager final
 ...
 public:
  ...
  /** Registers all the rigid collision geometries from the owning
   ...
   @pre The owning MultibodyPlant is registered as a source at the given
   `scene_graph`. */
 ...
 private:
  ...
  /* Updates the vertex positions for all deformable deformable meshes */
  ...
  /* Output port temporarily held by DeformableRigidManager to update the
   deformable vertices. In the future, there should be an input port in
   SceneGraph that connects to this output port. */
  const systems::OutputPort<T>* vertex_positions_output_port_;
};
}  // namespace fixed_fem
}  // namespace multibody
}  // namespace drake

...
class DeformableRigidManager final
  ...
  */
  ...
  /* Extracts the velocities of the participating dofs used by the contact
   ...
   columns of the contact tangent matrix (see CalcContactTangentMatrix()). */
  ...
  /* Calculates the velocities of all rigid and deformable dofs at the previous
   time step, with the rigid velocities coming before deformable velocities. */
  void CalcVelocities(const systems::Context<T>& context, VectorX<T>* v0) const;

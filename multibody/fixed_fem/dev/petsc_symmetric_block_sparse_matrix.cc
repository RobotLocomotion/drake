...
class PetscSymmetricBlockSparseMatrix::Impl {
 public:
  ...
  void SolveInPlace(SolverType solver_type,
      ...
      case SolverType::kDirect:
        ...
        if (preconditioner_type != PreconditionerType::kCholesky) {
          throw std::logic_error(
              "Direct solver can only be paired with Cholesky "
              "preconditioner.");
        }
        break;
    ...
    // TODO(xuchenhan-tri): Currently we don't repeatedly solve the same
    // system with multiple right hand sides, so we always reset the
    // operators. Split the factorization and the solve into two distinct
    // phases to reuse the factorization if we need that in the future.
    /* Set matrix. */
  ...
  void ThrowIfNotAssembled(const char* func) const {
    if (!is_assembled(owned_matrix_)) {
      throw std::runtime_error(
          ...
          "(): matrix is not yet assembled. Call AssembleIfNecessary() "
          "first.");
    }
  }

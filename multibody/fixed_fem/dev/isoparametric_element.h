...
class IsoparametricElement {
 ...
 protected:
  ...
  /* Default implementations of "Calc" methods that should be used unless
   derived concrete elements can provide an optimization.
   @{ */
  ArrayNumSamples<GradientInSpatialCoordinates>
  ...
  ArrayNumSamples<JacobianMatrix> DoCalcJacobian(
      const Eigen::Ref<const Eigen::Matrix<T, spatial_dimension, num_nodes>>&
          xa) const {
  ...
  ArrayNumSamples<PseudoinverseJacobianMatrix> DoCalcJacobianPseudoinverse(
      const ArrayNumSamples<JacobianMatrix>& jacobian) const {

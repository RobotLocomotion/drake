...
class IsoparametricElement {
 public:
  ...
  /* Returns the std::array of sample locations in the parent domain at which
   ...
   construction. */
  const ArrayOfSamples<Vector<double, natural_dimension>>& locations() const {
    return locations_;
  }
  ...
  const ArrayOfSamples<ShapeFunctions>& GetShapeFunctions() const { return S_; }
  ...
  /* Obtains the gradient of the shape functions in parent coordinates, dS/dξ,
   ...
   node `a`. */
  const ArrayOfSamples<GradientInParentCoordinates>&
  ...
  ArrayOfSamples<GradientInSpatialCoordinates> CalcGradientInSpatialCoordinates(
  ...
  ArrayOfSamples<JacobianMatrix> CalcJacobian(
      const Eigen::Ref<const Eigen::Matrix<T, spatial_dimension, num_nodes>>&
          xa) const {
  ...
  /* Computes dξ/dx, the pseudoinverse Jacobian matrix, at each sample location
   ...
   @see CalcJacobian(). */
  ArrayOfSamples<PseudoinverseJacobianMatrix> CalcJacobianPseudoinverse(
      const ArrayOfSamples<JacobianMatrix>& jacobian) const {
  ...
  template <int dim>
  ArrayOfSamples<Vector<T, dim>> InterpolateNodalValues(
      const Eigen::Ref<const Eigen::Matrix<T, dim, num_nodes>>& ua) const {
    const ArrayOfSamples<Vector<T, num_nodes>>& S = GetShapeFunctions();
    ArrayOfSamples<Vector<T, dim>> interpolated_value;
 ...
 protected:
  ...
  IsoparametricElement(
      ArrayOfSamples<Vector<double, natural_dimension>> locations,
      ArrayOfSamples<Vector<T, num_nodes>> S,
      ArrayOfSamples<GradientInParentCoordinates> dSdxi)
      : locations_(std::move(locations)),
        S_(std::move(S)),
        dSdxi_(std::move(dSdxi)) {}

  /* Default implementation of CalcGradientInSpatialCoordinates() that should be
   used unless derived concrete elements can provide an optimization. */
  ArrayOfSamples<GradientInSpatialCoordinates>
  DoCalcGradientInSpatialCoordinates(
      const Eigen::Ref<const Eigen::Matrix<T, spatial_dimension, num_nodes>>&
          xa) const {
    ArrayOfSamples<GradientInSpatialCoordinates> dSdX;
  ...
  /* Default implementation of CalcJacobian() that should be used unless derived
   concrete elements can provide an optimization. */
  ArrayOfSamples<JacobianMatrix> DoCalcJacobian(
      const Eigen::Ref<const Eigen::Matrix<T, spatial_dimension, num_nodes>>&
          xa) const {
    ArrayOfSamples<JacobianMatrix> dxdxi;
    const ArrayOfSamples<Eigen::Matrix<T, num_nodes, natural_dimension>>&
  ...
  /* Default implementation of CalcJacobianPseudoinverse() that should be used
   unless derived concrete elements can provide an optimization. */
  ArrayOfSamples<PseudoinverseJacobianMatrix> DoCalcJacobianPseudoinverse(
      const ArrayOfSamples<JacobianMatrix>& jacobian) const {
    /* Suppose the Jacobian matrix J = dx/dξ is m×n where m >= n. We are looking
     ...
     implies AᵀJᵀ = JA. */
    ArrayOfSamples<PseudoinverseJacobianMatrix> dxidx;
    for (int q = 0; q < num_sample_locations; ++q) {
 ...
 private:
  ...
  ArrayOfSamples<Vector<double, natural_dimension>> locations_;
  /* Shape functions evaluated at points specified at construction. */
  ArrayOfSamples<Vector<T, num_nodes>> S_;
  /* Shape function derivatives evaluated at points specified at construction.
   */
  ArrayOfSamples<GradientInParentCoordinates> dSdxi_;
};

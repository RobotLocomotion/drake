...
/* In general, the stiffness matrix calculated by DynamicElasticityElement is
 only a first-order approximation of the stiffness matrix as it does not
 include the contribution of -∂fᵥ(x, v)/∂x = β * ∂²fₑ(x)/∂x² * v, where β is
 the stiffness damping parameter and fₑ is the elastic force. For
 LinearConstitutiveModel, however, fₑ(x) is linear in x, so this particular
 term vanishes. */

...
constexpr double kYoungsModulus = 1;
constexpr double kPoissonRatio = 0.3;
constexpr double kDensity = 1;
constexpr double kMassDamping = 0.001;
constexpr double kStiffnessDamping = 0.002;
/* Time step. */
...
class DeformableRigidContactSolverTest : public ::testing::Test {
 protected:
  /* Set up a scene with a deforamble octahedron and one rigid cube. */
  void SetUp() override {
    ...
    pgs_solver->set_parameters({kRelaxationFactor, kTol, kTol, kIterations});
...
TEST_F(DeformableRigidContactSolverTest, InContact) {
  ...
  const VectorXd v_star = EvalParticipatingFreeMotionVelocities(plant_context);
  const VectorXd dv = results.v_next - v_star;
  EXPECT_TRUE(CompareMatrices(A * dv, results.tau_contact,
                              std::numeric_limits<double>::epsilon()));
  ...
  for (int i = 0; i < nc; ++i) {
    EXPECT_GE(results.vn(i), -kTol);
    EXPECT_GE(results.fn(i), 0);
  }
  EXPECT_TRUE(CompareMatrices(results.vn.cwiseProduct(results.fn),
                              VectorXd::Zero(nc), kTol * kTol));
}

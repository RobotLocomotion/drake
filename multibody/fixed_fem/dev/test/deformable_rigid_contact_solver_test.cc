...
TEST_F(DeformableRigidContactSolverTest, InContact) {
  Context<double>& plant_context =
      plant_->GetMyMutableContextFromRoot(diagram_context_.get());
  /* Transform the rigid cube so that it is in contact with the deformable
   octahedron but not the deformable cube. */
   ...
   -Y-----●---+---●---+---●----+Y
            \ |   |   |  ⁄
              ...
              ----|---- rigid cube
                 ...
                 -Z                    */
  /* Transform the rigid cube so that it is in contact with the deformable
   octahedron but not the deformable cube. By doing so, the test covers the
   post-contact deformable results for both bodies that are in contact and those
   that aren't. */
  const Vector3d p_WB(0, 0, -1.25);
  const math::RigidTransformd X_WB(p_WB);
  plant_->SetFreeBodyPose(&plant_context, plant_->get_body(rigid_index_), X_WB);

...
void TestUndeformedState() {
  ...
  const std::array<Matrix3d, num_locations> F{Matrix3d::Identity()};
  data.UpdateData(F);
  /* At the undeformed state, the energy density should be zero. */
  const std::array<double, num_locations> analytic_energy_density{0};
  /* At the undeformed state, the stress should be zero. */
  const std::array<Matrix3d, num_locations> analytic_stress{Matrix3d::Zero()};
  std::array<double, num_locations> energy_density;
  model.CalcElasticEnergyDensity(data, &energy_density);
  EXPECT_EQ(energy_density, analytic_energy_density);
  std::array<Matrix3d, num_locations> stress;

...
GTEST_TEST(PetscSymmetricBlockSparseMatrixTest, Solve) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      A->Solve(b), std::exception,
      "PetscSymmetricBlockSparseMatrix::Solve.*: matrix is not yet factored.*");
  A->Factor(PetscSymmetricBlockSparseMatrix::SolverType::kMINRES,
            PetscSymmetricBlockSparseMatrix::PreconditionerType::kNone);
  const VectorXd x = A->Solve(b);
  EXPECT_TRUE(CompareMatrices(x, x_expected, b.norm() * kEps));
  VectorXd x_in_place = b;
  A->SolveInPlace(&x_in_place);
...
GTEST_TEST(PetscSymmetricBlockSparseMatrixTest, Clone) {
  ...
  A->Factor(PetscSymmetricBlockSparseMatrix::SolverType::kMINRES,
            PetscSymmetricBlockSparseMatrix::PreconditionerType::kNone);
  const VectorXd x = A->Solve(b);
  A_clone->SetRelativeTolerance(kEps);
  A_clone->Factor(PetscSymmetricBlockSparseMatrix::SolverType::kMINRES,
                  PetscSymmetricBlockSparseMatrix::PreconditionerType::kNone);
  const VectorXd x_clone = A_clone->Solve(b);
...
GTEST_TEST(PetscSymmetricBlockSparseMatrixTest, MultiThreadTest) {
  ...
  auto run_solver = [&A](const VectorXd& b, VectorXd* x) { *x = A->Solve(b); };

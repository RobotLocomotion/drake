...
class DeformableRigidContactSolverTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    // TODO(xuchenhan-tri): Alternatively, we can set both the deformable box
    //  and the ground to be axis-aligned and change gravity to *simulate* a
    //  tilted plane. That setup, without the falling transient, could
    //  potentially save us some time on the test. */
    /* Set the position of the ground so that it's slightly tilted. */
    const RigidTransformd X_WG(Eigen::AngleAxisd(0.2, Vector3d(1, 0, 0)),
                               Vector3d(0, 0, -0.35));

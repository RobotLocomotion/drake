...
class DeformableRigidContactSolverTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    const RigidTransformd X_WG(Eigen::AngleAxisd(0.05, Vector3d(1, 0, 0)),
                               Vector3d(0, 0, -kRigidSize));
...
TEST_F(DeformableRigidContactSolverTest, SteadyState) {
  ...
  constexpr double kSteadyStateThreshold = 5e-5;  // unit: m/s.
  ...
  EXPECT_TRUE(
      CompareMatrices(expected_contact_impulse, contact_impulse, 5e-6));
}

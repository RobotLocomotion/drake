...
TEST_F(DeformableRigidManagerTest, CalcDeformableRigidContactPair) {
  ...
  EXPECT_EQ(contact_pair.num_contacts(),
  ...
  /* Verifies the calculated rotation matrices are indeed rotation matrices and
   that they map contact normals from world frame to contact frame ({0,0,1}). */
  constexpr double kTol = std::numeric_limits<double>::epsilon();
  EXPECT_EQ(contact_pair.R_CWs.size(), num_contacts);
  for (int i = 0; i < num_contacts; ++i) {
    EXPECT_TRUE(math::RotationMatrix<double>::IsOrthonormal(
        contact_pair.R_CWs[i], kTol));

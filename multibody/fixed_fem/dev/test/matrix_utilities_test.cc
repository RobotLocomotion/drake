...
MatrixXd MakeArbitraryMatrix(int rows, int cols) {
...
MatrixX<AutoDiffXd> MakeAutoDiffMatrix(int rows, int cols) {
  const MatrixXd A = MakeArbitraryMatrix(rows, cols);
...
GTEST_TEST(MatrixUtilitiesTest, PolarDecompose) {
  const Matrix3<double> A = MakeArbitraryMatrix(3, 3);
  Matrix3<double> R, S;
...
GTEST_TEST(PermuteBlockSparseMatrix, AnalyticTest) {
  ...
  const MatrixXd A = MakeArbitraryMatrix(kSize, kSize);

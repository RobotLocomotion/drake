...
const double kDensity = 1e-22;  // 0.789;
...
const double kStiffnessDamping = 0.0;
...
class DynamicElasticityModelTest : public ::testing::Test {
 protected:
  ...
  FemState<ElementType> MakeDeformedState() const {
    FemState<ElementType> reference_state = model_.MakeFemState();
    ...
    /* It's important to set up the deformed state with `state_updater_` so that
     the derivatives such as dq/dqddot are set up. */

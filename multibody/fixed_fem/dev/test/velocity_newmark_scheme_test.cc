...
GTEST_TEST(VelocityNewmarkSchemeTest, AdvanceOneTimeStep) {
  ...
  const Vector4d qdot = MakeQdot();
  const FemState<DummyElement<2>> state_0(q, qdot, Vector4d::Zero());
  ...
  EXPECT_TRUE(CompareMatrices(state_n.qdot(), state_0.qdot()));
  EXPECT_TRUE(CompareMatrices(state_n.q(),

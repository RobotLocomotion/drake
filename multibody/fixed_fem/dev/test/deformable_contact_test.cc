...
/* Returns true if
 ...
 @pre Entries in B are such that maxₙ|B[i](n)-B[j](n)| >> kEps if i != j, and
      therefore for each i, there exists at most one j such that
      maxₙ|A[i](n)-B[j](n)| < kEps.  */
template <typename T>
bool CompareSetOfVector3s(const std::vector<Vector3<T>>& A,
  ...
  vector<bool> matched(B.size(), false);
  for (int i = 0; i < static_cast<int>(A.size()); ++i) {
    for (int j = 0; j < static_cast<int>(B.size()); ++j) {
      if (!matched[j] && CompareMatrices(A[i], B[j], kEps)) {
        matched[j] = true;
        break;
      }
    }
  }
  return all_of(matched.begin(), matched.end(), [](bool v) { return v; });
}
...
void TestComputeTetMeshTriMeshContact() {
  ...
  EXPECT_EQ(contact_D.size(), kNumPolys);
  ...
  const double epxected_area = 0.125;
  const Vector3<T> expected_nhat_D(0, 0, -1);
  for (int i = 0; i < kNumPolys; ++i) {
    if constexpr (std::is_same_v<T, double>)
      EXPECT_DOUBLE_EQ(contact_data[i].area, epxected_area);
    else
      EXPECT_DOUBLE_EQ(contact_data[i].area.value(), epxected_area);
    EXPECT_TRUE(CompareMatrices(contact_data[i].nhat_D, expected_nhat_D, kEps));
  }

...
void VerifySizes(const LinearConstitutiveModelData<double, kNumQuads>& data) {
  ASSERT_EQ(data.deformation_gradient().size(), kNumQuads);
  ASSERT_EQ(data.strain().size(), kNumQuads);
  ASSERT_EQ(data.trace_strain().size(), kNumQuads);
}

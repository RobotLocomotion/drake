...
void AddMaterialToMaterialMap(const std::string& material_name,
  ...
  if (material_iter != materials->end()) {
    ...
    if (abort_if_name_clash || (color_rgba != existing_color)) {
      ...
      error_buff << std::string(__FILE__) + ": " + __func__ + ": ERROR: "
...
void ParseMaterial(XMLElement* node, MaterialMap* materials) {
  ...
  if (color_node) {
    ...
    if (materials->find(name) == materials->end()) {
      ...
      if (texture_node) {
        std::stringstream error_buff;
        error_buff << std::string(__FILE__) + ": " + __func__
                   << ": WARNING: Material \""
...
std::unique_ptr<geometry::Shape> ParseBox(XMLElement* shape_node) {
  double x = 0, y = 0, z = 0;
  const char* attr = shape_node->Attribute("size");
  ...
  if (attr) {
    ...
    throw std::runtime_error(
        std::string(__FILE__) + ": " + __func__ + ": ERROR parsing box "
        "element size");
  }
  return std::make_unique<geometry::Box>(x, y, z);
}

std::unique_ptr<geometry::Shape> ParseSphere(XMLElement* shape_node) {
  double r = 0;
  const char* attr = shape_node->Attribute("radius");
  ...
  if (attr) {
    ...
    throw std::runtime_error(
        std::string(__FILE__) + ": " + __func__ + ": ERROR parsing sphere "
        "element radius");
  }
...
std::unique_ptr<geometry::Shape> ParseCylinder(XMLElement* shape_node) {
  double r = 0, l = 0;
  const char* attr = shape_node->Attribute("radius");
  ...
  if (attr) {
    ...
    throw std::runtime_error(
        std::string(__FILE__) + ": " + __func__ + ": ERROR parsing "
        "cylinder element length");
  }
  return std::make_unique<geometry::Cylinder>(r, l);
}

std::unique_ptr<geometry::Shape> ParseMesh(
    XMLElement* shape_node, const PackageMap& package_map,
    const std::string& root_dir) {
  const char* attr = shape_node->Attribute("filename");
  if (!attr) {
    throw std::runtime_error(
        std::string(__FILE__) + ": " + __func__ + ": ERROR Mesh element has "
        "no filename tag");
  }
  const std::string filename(attr);
...
std::unique_ptr<geometry::Shape> ParseGeometry(
  ...
  throw std::runtime_error(
      std::string(__FILE__) + ": " + __func__ + ": Warning: geometry element "
...
geometry::GeometryInstance ParseVisual(
  ...
  if (!geometry_node) {
    throw std::runtime_error(
        std::string(__FILE__) + ": " + __func__ + ": ERROR: Link " +
        element_name + " has a visual element without geometry.");
  }
  ...
  if (material_node) {
    ...
    if (color_node) {
      if (!ParseVectorAttribute(color_node, "rgba", &rgba)) {
        throw std::runtime_error(
            std::string(__FILE__) + ": " + __func__ + ": ERROR: "
            "Failed to parse color of material for link " + element_name);
      }
  ...
  if (attr) {
    ...
    geometry_name = MakeGeometryName(element_name + "_Visual", node);
  }
...
geometry::GeometryInstance ParseCollision(
  ...
  if (!geometry_node) {
    throw std::runtime_error(
        std::string(__FILE__) + ": " + __func__ + ": ERROR: Link " +
        element_name + " has a collision element without geometry.");
  }
  ...
  *friction = default_friction();
  ...
  if (compliant_node) {
    ...
    if (friction_node) {
      ...
      if (friction_node->QueryDoubleText(&static_friction)) {
        throw std::runtime_error(
            std::string(__FILE__) + ": " + __func__ + ": ERROR: Unable " +
            "to parse static_friction for link " + element_name);
      }
    }
    ...
    if (friction_node) {
      ...
      if (friction_node->QueryDoubleText(&dynamic_friction)) {
        throw std::runtime_error(
            std::string(__FILE__) + ": " + __func__ + ": ERROR: Unable " +
            "to parse dynamic_friction for link " + element_name);
      }
    }
    ...
    if (static_friction_present != dynamic_friction_present) {
        throw std::runtime_error(
            std::string(__FILE__) + ": " + __func__ + ": ERROR: Link " +
            element_name + ": When specifying coefficient of friction, "
    ...
    if (compliant_node->FirstChildElement("youngs_modulus")) {
      drake::log()->warn("Ignoring youngs_modulus for link " + element_name);
    }

    if (compliant_node->FirstChildElement("dissipation")) {
      drake::log()->warn("Ignoring dissipation for link " + element_name);
    }
  }
  ...
  if (attr) {
    ...
    geometry_name = MakeGeometryName(element_name + "_Collision", node);
  }

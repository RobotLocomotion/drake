...
TEST_F(UrdfGeometryTests, TestWrongElementType) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      detail::ParseVisual("link_name", package_map_, root_dir_, material_node,
                          &materials_), std::runtime_error,
      "In link link_name expected visual element, got material");

  multibody_plant::CoulombFriction<double> friction;
  DRAKE_EXPECT_THROWS_MESSAGE(
      detail::ParseCollision("link_name", package_map_, root_dir_,
                             material_node, &friction), std::runtime_error,
      "In link link_name expected collision element, got material");
}

...
TEST_F(MultibodyPlantSdfParser, ModelInstanceTest) {
  ...
  // isn't specified.
  EXPECT_THROW(plant_.HasBodyNamed("Link1"), std::logic_error);
  ...
  EXPECT_THROW(plant_.GetBodyByName("Link1"), std::logic_error);

  EXPECT_THROW(plant_.HasJointNamed("ShoulderJoint"), std::logic_error);
  ...
  EXPECT_THROW(plant_.GetJointByName("ShoulderJoint"), std::logic_error);

  EXPECT_THROW(plant_.HasJointActuatorNamed("ElbowJoint"), std::logic_error);
  ...
  EXPECT_THROW(plant_.GetJointActuatorByName("ElbowJoint"), std::logic_error);
}

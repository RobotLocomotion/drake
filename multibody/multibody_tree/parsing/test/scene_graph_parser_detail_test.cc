...
GTEST_TEST(SceneGraphParserDetail, ParseVisualMaterialDiffuse) {
  ...
  {
    VisualMaterial expected_material{{0.25, 0.5, 0.75, 1.0}};
    using std::to_string;
    const std::string diffuse_str = to_string(expected_material.diffuse()(0)) +
        " " + to_string(expected_material.diffuse()(1)) +
        " " + to_string(expected_material.diffuse()(2)) +
        " " + to_string(expected_material.diffuse()(3));
    unique_ptr<sdf::Visual> sdf_visual = MakeSdfVisualFromString(
        "<visual name='some_link_visual'>"
        "  <pose>0 0 0 0 0 0</pose>"
        "  <geometry>"
        ...
        "</visual>");
    VisualMaterial material = MakeVisualMaterialFromSdfVisual(*sdf_visual);
    EXPECT_TRUE(CompareMatrices(material.diffuse(), expected_material.diffuse(),
                                0.0, MatrixCompareType::absolute));
  }
  ...
  // Case: Too many channel values -- truncate.
  {
    unique_ptr<sdf::Visual> sdf_visual = MakeSdfVisualFromString(
        ...
        "</visual>");
    VisualMaterial material = MakeVisualMaterialFromSdfVisual(*sdf_visual);
    EXPECT_TRUE(CompareMatrices(material.diffuse(),
                                Vector4<double>{0.25, 1, 0.5, 0.25},
                                0.0, MatrixCompareType::absolute));
  }
  ...
  // Case: Too few channel values -- fill in with 0 for b and 1 for alpha.
  {
    unique_ptr<sdf::Visual> sdf_visual = MakeSdfVisualFromString(
        ...
        "</visual>");
    VisualMaterial material = MakeVisualMaterialFromSdfVisual(*sdf_visual);
    EXPECT_TRUE(CompareMatrices(material.diffuse(), Vector4<double>{0, 1, 0, 1},
                                0.0, MatrixCompareType::absolute));
  }

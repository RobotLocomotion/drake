...
template <typename T>
class QuaternionMobilizer final : public MobilizerImpl<T, 4, 3> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(QuaternionMobilizer)

  /// Constructor for a %QuaternionMobilizer between the inboard frame F
  ...
  /// vector.
  QuaternionMobilizer(const Frame<T>& inboard_frame_F,
  ...
  const QuaternionMobilizer<T>& set_quaternion(systems::Context<T>* context,
                                         const Quaternion<T>& q_FM) const;

  const QuaternionMobilizer<T>& set_quaternion(
  ...
  const QuaternionMobilizer<T>& SetFromRotationMatrix(

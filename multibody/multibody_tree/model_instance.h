...
///
/// A MultiBody tree is composed of a number of MultibodyTreeElement
/// items.  In more complex trees, these items will be loaded from
/// multiple models (e.g. robot arm, attached gripper, free bodies
/// being manipulated).  Each model may have a different controller or
/// observer, so the ability to view the state or command actuation
/// for each model independently can be useful.  For this reason,
/// elements are associated with a ModelInstanceIndex to determine
/// which model an element belongs to.
///
/// There are two special ModelElementIndex values.  The world body is
/// always ModelInstanceIndex 0, and ModelInstanceIndex 1 is reserved
/// for all elements with no explicit model instance.  This is
/// generally only relevant for elements created programmatically, as
/// model parsers should handle creating model elements as needed.
///
...
/// * Mobilizer: 1. Same as the joint (if created from a joint)
///              2. Same as the body (for free floating bodies)
///              3. Use caution when creating mobilizers for other reasons
...
class ModelInstance :
 ...
 public:
  ...
  /// Given the actuation values @p u_a for all actuators in `this`
  /// model instance, this method sets the portion of the actuation
  /// vector @p u corresponding to the actuators for this model
  /// instance.

...
GTEST_TEST(RollPitchYawTest, UsingSpaceXYZBall) {
...
// TODO: make with_quaternion a unit test parameter.
GTEST_TEST(RollPitchYawTest, UsingQuaternionBall) {
  const double kEpsilon = std::numeric_limits<double>::epsilon();
  const double kAccuracy = 1.0e-5;  // The integrator's desired accuracy.
  // The numerical tolerance accepted for these tests.
  const double kTolerance = 1.0e-5;
  ...
  // Instantiate the model for the free body in space.
  FreeRotatingBodyPlant<double> free_body_plant(
      benchmark_.get_I(), benchmark_.get_J(),
      true /* Using QuaternionMobilizer */);

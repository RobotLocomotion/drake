...
class SpringDamperTester : public ::testing::Test {
 public:
  void SetUp() override {
    bodyA_ = &mutable_tree().AddRigidBody(
        "BodyA", SpatialInertia<double>());
    bodyB_ = &mutable_tree().AddRigidBody(
        "BodyB", SpatialInertia<double>());

    mutable_tree().AddJoint<WeldJoint>(
        "WeldBodyAToWorld", tree().world_body(), {}, *bodyA_, {},
        Isometry3<double>::Identity());

    // Allow body B to slide along the x axis.
    slider_ = &mutable_tree().AddJoint<PrismaticJoint>(
        "Slider", mutable_tree().world_body(), {}, *bodyB_, {},
        Vector3<double>::UnitX());

    spring_damper_ = &mutable_tree().AddForceElement<LinearSpringDamper>(
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
        mutable_tree().AddForceElement<LinearSpringDamper>(
 ...
 private:
  MultibodyTree<double>& mutable_tree() const { return system_.mutable_tree(); }
};

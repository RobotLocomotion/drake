...
TEST_F(KukaIiwaModelTests, CalcJacobianSpatialVelocity) {
  ...
  tree().CalcJacobianSpatialVelocity(
      *context_, JacobianWithRespectTo::kGeneralizedVelocities,
      link7.body_frame(), p_L7Q, link3.body_frame(), link5.body_frame(),
      &Jv_L3L7q_L5);
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      tree().CalcJacobianSpatialVelocity(
          *context_, JacobianWithRespectTo::kGeneralizedVelocities,
          link7.body_frame(), p_L7Q, link3.body_frame(), link5.body_frame(),
          &Jv_wrong_size),
      std::exception, ".*'J_ABp_E->cols\\(\\) == num_columns'.*");
}

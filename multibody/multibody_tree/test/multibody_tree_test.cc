...
class BadDerivedMBSystem : public MultibodyTreeSystem<double> {
 public:
  BadDerivedMBSystem(bool double_finalize) : MultibodyTreeSystem<double>() {
...
GTEST_TEST(MultibodyTreeSystem, CatchBadBehavior) {
  EXPECT_NO_THROW(BadDerivedMBSystem(false));
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(MultibodyTreeSystem<double>(nullptr),
      std::logic_error, ".*MultibodyTreeSystem.*MultibodyTree was null.*");
}

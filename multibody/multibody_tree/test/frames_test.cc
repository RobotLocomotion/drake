...
class FrameTests : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    bodyB_ = &mutable_tree().AddBody<RigidBody>(M_Bo_B);
    ...
    mutable_tree().AddMobilizer<RevoluteMobilizer>(
        tree().world_frame(), bodyB_->body_frame(),
    ...
    frameP_ =
        &mutable_tree().AddFrame<FixedOffsetFrame>(bodyB_->body_frame(), X_BP_);
    ...
    frameQ_ =
        &mutable_tree().AddFrame<FixedOffsetFrame>(*frameP_, X_PQ_);

    // Frame R is arbitrary, but named.
    frameR_ = &mutable_tree().AddFrame<FixedOffsetFrame>(

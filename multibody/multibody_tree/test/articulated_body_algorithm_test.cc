...
GTEST_TEST(ArticulatedBodyInertiaAlgorithm, FeatherstoneExample) {
  ...
  MultibodyTreeSystem<double> system;

  // Add box body and SpaceXYZ mobilizer.
  const RigidBody<double>& box_link =
      system.mutable_tree().AddBody<RigidBody>(M_Bcm);
  const Frame<double>& world_frame = system.tree().world_frame();
  const Frame<double>& box_frame = box_link.body_frame();
  system.mutable_tree().AddMobilizer<SpaceXYZMobilizer>(world_frame, box_frame);

  // Add cylinder body and Featherstone mobilizer.
  const RigidBody<double>& cylinder_link =
      system.mutable_tree().AddBody<RigidBody>(M_Ccm);
  const Frame<double>& cylinder_frame = cylinder_link.body_frame();
  system.mutable_tree().AddMobilizer<FeatherstoneMobilizer>(box_frame,
                                                            cylinder_frame);

  system.FinalizeMultibodyTreeSystem();
...
GTEST_TEST(ArticulatedBodyInertiaAlgorithm, ModifiedFeatherstoneExample) {
  ...
  const SpaceXYZMobilizer<double>& WB_mobilizer =
      system.mutable_tree().AddMobilizer<SpaceXYZMobilizer>(world_frame,
                                                            box_frame);

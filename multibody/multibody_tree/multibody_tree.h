...
class MultibodyTree {
 public:
  ...
  /// model instance is not explicitly specified, the returned reference may
  /// be in any model instance which has an element by that name.
  ///
  /// @{
  ...
  const Joint<T>& GetJointByName(const std::string& name) const {
    ...
    return get_joint(range.first->second);
  }
  ...
  const JointActuator<T>& GetJointActuatorByName(
    ...
    return get_joint_actuator(range.first->second);
  }

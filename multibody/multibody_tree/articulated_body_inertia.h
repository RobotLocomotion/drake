...
class ArticulatedBodyInertia {
 public:
  ...
  ArticulatedBodyInertia<T>& ShiftInPlace(const Vector3<T>& p_QR_E) {
    ...
    // |J + [p]Fᵀ - Fp[p]  Fp|
    // |Fpᵀ                 M|
    // where [p] is used to denote p_QR_E× and Fp = F + [p]M.

    // Some aliases definitions.
    const Vector3<T>& p = -p_QR_E;
    ...
    Fp = F + M.colwise().cross(-p);

    // Update J according to J + [p]Fᵀ - Fp[p] = J - ((F[p])ᵀ + Fp[p]).
    // Costs 68 flops (54 for cross product and 12 for triangular addition).
    J -= (F.rowwise().cross(p).transpose() + Fp.rowwise().cross(p));
  ...
  ProjectAcrossMobilizer(const MatrixUpTo6<T>& H_RQ_E) const {
    ...
    const MatrixUpTo6<T> D_RQ_E = H_RQ_E.transpose() * *this * H_RQ_E;

    // Compute Dinv_RQ_E = D_RQ_E⁻¹.
    const MatrixUpTo6<T> Dinv_RQ_E = D_RQ_E.inverse();

...
GTEST_TEST(MultibodyPlantTest, VisualGeometryRegistration) {
  ...
  EXPECT_TRUE(CompareMatrices(
      (*query_object.GetVisualMaterial(ground_id)).get().diffuse(),
      VisualMaterial().diffuse(), 0.0, MatrixCompareType::absolute));
  EXPECT_TRUE(CompareMatrices(
      (*query_object.GetVisualMaterial(sphere1_id)).get().diffuse(),
      sphere1_diffuse, 0.0, MatrixCompareType::absolute));
  EXPECT_TRUE(CompareMatrices(
      (*query_object.GetVisualMaterial(sphere2_id)).get().diffuse(),
      sphere2_diffuse, 0.0, MatrixCompareType::absolute));
}

...
class MultibodyPlantTester {
 public:
  ...
  template <typename T>
  static FrameId RegisterFrameForBody(const RigidBody<T>& body,
                                      MultibodyPlant<double>* plant,
                                      SceneGraph<double>* scene_graph) {
    EXPECT_FALSE(plant->body_has_registered_frame(body));
    FrameId frame_id = scene_graph->RegisterFrame(
        plant->source_id_.value(),
        geometry::GeometryFrame(
            body.name(),
            /* Initial pose: Not really used by GS. Will get removed. */
            Isometry3<double>::Identity()));
    plant->body_index_to_frame_id_[body.index()] = frame_id;
    return frame_id;
  }
};
...
//  1. It tests that a world frame gets properly recorded when MBP becomes a
//     geometry source, and
//  2. it tests the `GetBodyFrameId*()` methods.
GTEST_TEST(MultibodyPlantTest, WorldFrameId) {
  // Case: Plant is *not* a geometry source --> no world frame id.
  {
    MultibodyPlant<double> plant;
    plant.Finalize();
    DRAKE_EXPECT_THROWS_MESSAGE(
    ...
    auto id = plant.GetBodyFrameIdIfExists(plant.world_body().index());
    EXPECT_EQ(id, nullopt);
  }

  // Case: Plant *is* a geometry source -- world frame id record.
  {
    MultibodyPlant<double> plant;
    SceneGraph<double> scene_graph;
    ...
    FrameId world_frame_id =
        plant.GetBodyFrameIdOrThrow(plant.world_body().index());
    EXPECT_EQ(world_frame_id, scene_graph.world_frame_id());
    auto optional_id = plant.GetBodyFrameIdIfExists(plant.world_body().index());
    ASSERT_TRUE(optional_id);
    ASSERT_EQ(optional_id.value(), scene_graph.world_frame_id());
  }
}
...
class SphereChainScenario {
 public:
  SphereChainScenario(
    ...
    world_frame_id_ = scene_graph_->world_frame_id();
    ...
    ground_id_ = plant_->RegisterCollisionGeometry(
        ...
        geometry::HalfSpace(), ground_friction, scene_graph_);
    ...
    for (int i = 0; i < sphere_count - 1; ++i) {
      plant_->AddJoint<RevoluteJoint>("hinge" + to_string(i) + to_string(i + 1),
                                      *spheres_[i], {}, *spheres_[i + 1], {},
                                      Vector3d::UnitY());
    }

    // Body with registered frame.
    frame_only_body_ = &plant_->AddRigidBody("FrameOnly",
                                             SpatialInertia<double>());
    frame_only_id_ = MultibodyPlantTester::RegisterFrameForBody(
        *frame_only_body_, plant_, scene_graph_);
  ...
  const RigidBody<double>& frame_only_body() const { return *frame_only_body_; }
 ...
 private:
  ...
  const RigidBody<double>* frame_only_body_;
  FrameId frame_only_id_{};
...
// is sufficient to capture one or more geometries. So, the contests of the
// set will be *only* FrameIds -- specifically, the FrameId associated with
// each body -- even if no *geometries* are registered.
GTEST_TEST(MultibodyPlantTest, CollectRegisteredGeometries) {
  ...
  // Case: Body with frame but no geometry; it is still included.
  {
    GeometrySet set =
        plant.CollectRegisteredGeometries({&scenario.frame_only_body()});
    EXPECT_EQ(set.num_geometries(), 0);
    EXPECT_EQ(set.num_frames(), 1);
  }
  ...
  // Case: World body
  {
    ...
    EXPECT_EQ(set.num_geometries(), 0);
    EXPECT_EQ(set.num_frames(), 1);
    EXPECT_TRUE(set.contains(scenario.world_frame_id()));
  }
}

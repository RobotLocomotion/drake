...
TEST_F(AcrobotPlantTests, VisualGeometryRegistration) {
  ...
  for (BodyIndex body_index(1);
    ...
    const optional<FrameId> optional_id =
        plant_->GetBodyFrameIdMaybe(body_index);
    EXPECT_TRUE(optional_id.has_value());
  ...
  // Similarly, the "maybe" variant should return a null opt.
  optional<FrameId> undefined_id = plant_->GetBodyFrameIdMaybe(world_index());

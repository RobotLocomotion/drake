...
GTEST_TEST(MultibodyGraphMaker, SerialChain) {
  ...
  maker.AddLink("world", 0);
  maker.RegisterJointType("pin", 1);

  maker.AddLink("link1", kNonzeroMass);
  maker.AddLink("link2", kNonzeroMass);
  maker.AddLink("link3", kNonzeroMass);
  maker.AddLink("link4", kNonzeroMass);
  maker.AddLink("link5", kNonzeroMass);
  ...
  EXPECT_EQ(maker.GetJointTypeNum("weld"), 0);
  EXPECT_EQ(maker.GetJointTypeNum("free"), 1);
  EXPECT_EQ(maker.GetJointTypeNum("pin"), 2);  // 3rd joint.
  EXPECT_EQ(maker.get_world_body_name(), "world");
  EXPECT_EQ(maker.num_bodies(), 6);
  EXPECT_EQ(maker.FindBodyNum("link3"), 3);  // world is 0th body.
  EXPECT_EQ(maker.num_joints(), 5);
  EXPECT_EQ(maker.FindJointNum("pin4"), 3);
  EXPECT_EQ(maker.num_loop_constraints(), 0);

  EXPECT_EQ(maker.num_mobilizers(), 5);
  for (int i = 0; i < maker.num_mobilizers(); ++i) {
    const auto& mobilizer = maker.get_mobilizer(i);
    ...
    EXPECT_EQ(mobilizer.inboard_body_num(), i);
...
GTEST_TEST(MultibodyGraphMaker, HasLoop) {
  ...
  maker.AddLink("world", 0);
  for (int i = 1; i <= 3; ++i)
    maker.AddLink("link" + std::to_string(i), kNonzeroMass);
  ...
  EXPECT_EQ(maker.GetJointTypeNum("pin"), 2);  // 3rd joint.
  EXPECT_EQ(maker.get_world_body_name(), "world");
  EXPECT_EQ(maker.num_bodies(), 5);  // link3 is split into two bodies.
  EXPECT_EQ(maker.get_body(4).name(), "#link3_slave_1");
  ...
  EXPECT_EQ(maker.num_loop_constraints(), 1);
  EXPECT_EQ(maker.num_mobilizers(), 4);
  EXPECT_EQ(maker.FindBaseBodies(), std::vector<int>({1, 3}));
  ...
  EXPECT_EQ(maker.get_mobilizer(0).level(), 1);
  EXPECT_EQ(maker.get_mobilizer(1).level(), 1);
  EXPECT_EQ(maker.get_mobilizer(2).level(), 2);
  EXPECT_EQ(maker.get_mobilizer(3).level(), 3);
  for (int i = 0; i < maker.num_mobilizers(); ++i) {
    const auto& mobilizer = maker.get_mobilizer(i);
    EXPECT_FALSE(mobilizer.is_added_base_mobilizer());
  ...
  const auto& constraint = maker.get_loop_constraint(0);
  EXPECT_TRUE(constraint.is_master_slave_weld());
  EXPECT_EQ(constraint.get_constraint_type_name(), "weld");
  EXPECT_EQ(constraint.parent_body_num(), 3);  // link3
  EXPECT_EQ(constraint.child_body_num(), 4);   // #link3_slave
  EXPECT_EQ(constraint.joint_num(), -1);       // No associated joint.
}
...
GTEST_TEST(MultibodyGraphMaker, HasLoopWithMasslessBody) {
  ...
  EXPECT_EQ(maker.num_bodies(), 5);  // link2 is split into two bodies.
  EXPECT_EQ(maker.get_body(4).name(), "#link2_slave_1");
  EXPECT_EQ(maker.num_joints(), 4);
  // Should have one loop constraint; the loop is broken by splitting a body and
  // adding a weld constraint.
  EXPECT_EQ(maker.num_loop_constraints(), 1);
  ...
  EXPECT_EQ(maker.get_mobilizer(1).level(), 1);
  EXPECT_EQ(maker.get_mobilizer(2).level(), 2);
  EXPECT_EQ(maker.get_mobilizer(3).level(), 2);
  for (int i = 0; i < maker.num_mobilizers(); ++i) {
    const auto& mobilizer = maker.get_mobilizer(i);
    EXPECT_FALSE(mobilizer.is_added_base_mobilizer());
    // The 4th mobilizer is a slave mobilizer (child body is a slave).
  ...
  maker.ChangeLinkMass("link3", kNonzeroMass);
  maker.ChangeLinkMass("link2", kMassless);
  EXPECT_FALSE(maker.graph_has_been_generated());
  ...
  EXPECT_EQ(maker.get_mobilizer(0).level(), 1);
  EXPECT_EQ(maker.get_mobilizer(1).level(), 2);
  EXPECT_EQ(maker.get_mobilizer(2).level(), 3);
  EXPECT_EQ(maker.get_mobilizer(3).level(), 1);
  ...
  // Check the loop weld constraint.
  const auto& constraint2 = maker.get_loop_constraint(0);
  EXPECT_TRUE(constraint2.is_master_slave_weld());
  EXPECT_EQ(constraint2.get_constraint_type_name(), "weld");
  EXPECT_EQ(constraint2.parent_body_num(), 3);  // link3
  EXPECT_EQ(constraint2.child_body_num(), 4);   // #link3_slave
  EXPECT_EQ(constraint2.joint_num(), -1);       // No associated joint.
}
...
GTEST_TEST(MultibodyGraphMaker, ChooseBaseBody) {
  MultibodyGraphMaker maker(false);  // Don't use Drake defaults.
  maker.RegisterJointType("pin", 1);
  maker.AddLink("ground", 0, false);  // Instead of "world".
  ...
  EXPECT_EQ(maker.get_mobilizer(0).get_joint_type_name(), "free");
  EXPECT_EQ(maker.get_mobilizer(0).inboard_body_num(),
            maker.FindBodyNum("ground"));  // Should be 0.
  EXPECT_EQ(maker.get_mobilizer(0).outboard_body_num(), 1);

  EXPECT_EQ(maker.get_mobilizer(1).get_joint_type_name(), "free");
  EXPECT_EQ(maker.get_mobilizer(1).inboard_body_num(), 0);
  EXPECT_EQ(maker.get_mobilizer(1).outboard_body_num(), 3);
  ...
  EXPECT_EQ(maker.num_mobilizers(), 8);
  EXPECT_EQ(maker.num_bodies(), 9);  // Added one slave.
  EXPECT_EQ(maker.num_loop_constraints(), 1);

  EXPECT_EQ(maker.get_mobilizer(0).get_joint_type_name(), "free");
  EXPECT_EQ(maker.get_mobilizer(0).inboard_body_num(), 0);
  EXPECT_EQ(maker.get_mobilizer(0).outboard_body_num(), 1);

  EXPECT_EQ(maker.get_mobilizer(1).get_joint_type_name(), "free");
  EXPECT_EQ(maker.get_mobilizer(1).inboard_body_num(), 0);
  EXPECT_EQ(maker.get_mobilizer(1).outboard_body_num(), 4);
}
...
GTEST_TEST(MultibodyGraphMaker, MultipleSlaves) {
  ...
  // into four pieces:
  //
  //                  +--> link2 --> link3_slave1          Weld constraints
  //                  |                                        ## slave1
  //  world --> link1 +--> link3                         link3 ## slave2
  //                  |                                        ## slave3
  //                  +--> link4 --> link3_slave2
  //                             --> link5 --> link3_slave3
  //
  ...
  for (int i = 1; i <= 5; ++i) {
    maker.AddLink("link" + std::to_string(i), 1.);
  }
  ...
  EXPECT_EQ(maker.num_mobilizers(), 0);  // Graph hasn't been generated yet.
  EXPECT_EQ(maker.num_loop_constraints(), 0);

  maker.GenerateGraph();
  maker.DumpGraph(std::cout);

  const int link3_num = maker.FindBodyNum("link3");
  const MultibodyGraphMaker::Body& link3 = maker.get_body(link3_num);
  EXPECT_EQ(link3.num_children(), 0);  // As input.
  EXPECT_EQ(link3.num_parents(), 4);

  EXPECT_TRUE(link3.is_master());
  EXPECT_FALSE(link3.is_slave());
  EXPECT_EQ(link3.num_fragments(), 4);
  EXPECT_EQ(link3.num_slaves(), 3);

  EXPECT_EQ(maker.num_bodies(), 9);  // Added three slave bodies.
  EXPECT_EQ(maker.get_body(6).name(), "#link3_slave_1");
  EXPECT_EQ(maker.get_body(7).name(), "#link3_slave_2");
  EXPECT_EQ(maker.get_body(8).name(), "#link3_slave_3");

  EXPECT_EQ(maker.num_joints(), 8);  // Added free joint to World.
  EXPECT_EQ(maker.get_joint(7).name(), "#world_link1");

  EXPECT_EQ(maker.num_mobilizers(), 8);
  EXPECT_EQ(maker.num_loop_constraints(), 3);

  for (int i = 0; i < maker.num_loop_constraints(); ++i) {
    const auto& loop_constraint = maker.get_loop_constraint(i);
    EXPECT_TRUE(loop_constraint.is_master_slave_weld());
    EXPECT_EQ(loop_constraint.get_constraint_type_name(),
              maker.get_weld_joint_type_name());
    EXPECT_EQ(loop_constraint.joint_num(), -1);  // No associated joint.
    EXPECT_EQ(loop_constraint.parent_body_num(), maker.FindBodyNum("link3"));
    EXPECT_EQ(loop_constraint.child_body_num(), 6 + i);  // One of the slaves.
  }

  EXPECT_EQ(maker.get_mobilizer(0).get_joint_type_name(), "free");
  EXPECT_EQ(maker.get_mobilizer(0).inboard_body_num(),
            maker.FindBodyNum("world"));
  EXPECT_EQ(maker.get_mobilizer(0).outboard_body_num(),
            maker.FindBodyNum("link1"));
}
...
GTEST_TEST(MultibodyGraphMaker, MultipleSubtrees) {
  ...
  EXPECT_EQ(maker.FindBaseBodies(), std::vector<int>({6, 1, 7, 9}));

  EXPECT_EQ(maker.FindBaseBody(0), -1);  // World has no base body.
  EXPECT_EQ(maker.FindBaseBody(3), 1);   // See above.
  EXPECT_EQ(maker.FindBaseBody(4), 6);
  EXPECT_EQ(maker.FindBaseBody(5), 6);
  EXPECT_EQ(maker.FindBaseBody(7), 7);  // Already a base body.
  EXPECT_EQ(maker.FindBaseBody(8), 7);
  EXPECT_EQ(maker.FindBaseBody(9), 9);

  EXPECT_EQ(maker.FindPathToWorld(3), std::vector<int>({3, 2, 1}));
  EXPECT_EQ(maker.FindPathToWorld(5), std::vector<int>({5, 4, 6}));
  EXPECT_EQ(maker.FindPathToWorld(9), std::vector<int>({9}));
  EXPECT_EQ(maker.FindPathToWorld(0), std::vector<int>());
}
...
GTEST_TEST(MultibodyGraphMaker, DeleteBodiesAndClear) {
  MultibodyGraphMaker maker;
  for (int i = 1; i <= 3; ++i) maker.AddLink("link" + std::to_string(i), 1.);
  maker.AddJoint("joint0", "prismatic", "link1", "link2");
  maker.AddJoint("joint1", "revolute", "link2", "link3");

  EXPECT_EQ(maker.num_bodies(), 4);
  EXPECT_EQ(maker.num_joints(), 2);
  EXPECT_EQ(maker.FindBodyNum("world"), 0);
  EXPECT_EQ(maker.FindBodyNum("link1"), 1);
  EXPECT_EQ(maker.FindBodyNum("link2"), 2);
  EXPECT_EQ(maker.FindBodyNum("link3"), 3);
  EXPECT_EQ(maker.FindJointNum("joint0"), 0);
  EXPECT_EQ(maker.FindJointNum("joint1"), 1);

  maker.GenerateGraph();
  maker.FindBaseBodies(), std::vector<int>({1});  // link1

  // Make sure ClearGraph() nukes mobilizers, but leaves input bodies
  // and joints.
  EXPECT_EQ(maker.num_mobilizers(), 3);
  maker.ClearGraph();
  EXPECT_EQ(maker.num_mobilizers(), 0);
  EXPECT_EQ(maker.num_bodies(), 4);
  EXPECT_EQ(maker.num_joints(), 2);
  ...
  EXPECT_EQ(maker.FindBodyNum("world"), 0);
  EXPECT_EQ(maker.FindBodyNum("link1"), -1);
  EXPECT_EQ(maker.FindBodyNum("link2"), 1);
  EXPECT_EQ(maker.FindBodyNum("link3"), 2);
  EXPECT_EQ(maker.FindJointNum("joint0"), -1);
  EXPECT_EQ(maker.FindJointNum("joint1"), 0);

  maker.GenerateGraph();
  maker.FindBaseBodies(), std::vector<int>({1});  // link2
  ...
  maker.GenerateGraph();
  maker.FindBaseBodies(), std::vector<int>({1, 2});
}
...
GTEST_TEST(MultibodyGraphMaker, ReplaceJoint) {
  MultibodyGraphMaker maker;
  for (int i = 1; i <= 5; ++i) maker.AddLink("link" + std::to_string(i), 1.);
  ...
  EXPECT_EQ(maker.FindBaseBody(1), 3);  // link1's base body is link3.
  EXPECT_EQ(maker.FindBaseBodies(), std::vector<int>({3}));

  // Now replace link3's free joint with a weld.
  maker.AddJoint("joint5", "weld", "world", maker.get_body(3).name());
  maker.GenerateGraph();
  maker.DumpGraph(std::cout);
  ...
  for (int i = 0; i < maker.num_mobilizers(); ++i) {
    const MultibodyGraphMaker::Mobilizer& mobilizer = maker.get_mobilizer(i);

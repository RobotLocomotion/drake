...
void MultibodyPlant<T>::DeclareGeometrySystemPorts() {
  ...
  // Compute once, and for all, a vector of ids to be used in CalcFrameIdOutput.
...
FramePoseVector<T> MultibodyPlant<T>::AllocateFramePoseOutput(
  ...
  const int num_bodies_with_geometry =
      static_cast<int>(body_index_to_frame_id_.size());
  return FramePoseVector<T> (source_id_.value(), num_bodies_with_geometry);
}
...
void MultibodyPlant<T>::CalcFramePoseOutput(
  ...
  for (const auto it : body_index_to_frame_id_) {
    ...
    // NOTE: The body frames were all registered as children of the world, so
    // report poses in the world frame.

...
void ContactResultsToLcmSystem<T>::CalcLcmContactOutput(
  ...
  for (int i = 0; i < contact_results.num_contacts(); ++i) {
    ...
    auto write_double3 = [](double* dest, const Vector3<T>& src) {
      dest[0] = ExtractDoubleOrThrow(src(0));
      dest[1] = ExtractDoubleOrThrow(src(1));
      dest[2] = ExtractDoubleOrThrow(src(2));
    };
    write_double3(info_msg.contact_point, contact_info.contact_point());
    write_double3(info_msg.contact_force, contact_info.contact_force());
    write_double3(info_msg.normal, contact_info.point_pair().nhat_BA_W);
  }
}

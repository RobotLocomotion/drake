...
class MultibodyPlant : public systems::LeafSystem<T> {
 ...
 private:
  ...
  // If the plant has been registered as a source to a scene graph instance, it
  // will also include a mapping from world_body().index() to the scene graph's
  // world_frame_id() value.

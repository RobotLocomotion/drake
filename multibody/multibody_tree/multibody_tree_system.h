...
%MultibodyTreeSystem. */
template <typename T>
class MultibodyTreeSystem : public LeafSystem<T> {
 public:
  ...
  explicit MultibodyTreeSystem(bool is_discrete = false)
      : MultibodyTreeSystem(nullptr, is_discrete) {}

  /** If given a non-null `tree`, takes ownership, finalizes it if necessary,
  and then allocates the resources it needs. You cannot modify the tree after
  that, and you should not call FinalizeMultibodyTreeSystem(). If the `tree`
  is null, we allocate an empty one and leave it unfinalized. */
  ...
  void FinalizeMultibodyTreeSystem();

...
class SpatialKinematicsPVA {
 public:
  ...
  const Isometry3<T>& transform() const { return X_NB_; }
  ...
  const Matrix3<T>& rotation_matrix() const { return R_NB_ = X_NB_.linear(); }
  ...
  const Vector3<T>& position_vector() const {
    return p_NoBo_N_ = X_NB_.translation();
  }
 ...
 private:
  ...
  // To avoid changing the interface to this class, additional member data is
  // added to class to compensate for the Isometry3 member methods linear() and
  // translation() which do not return references.
  // TODO(Mitiguy) Replace Isometry3 class with Transform class.
  mutable Matrix3<T> R_NB_;
  mutable Vector3<T> p_NoBo_N_;
};

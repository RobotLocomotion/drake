...
SpatialAcceleration<double> CalcSpatialAccelerationViaAutomaticDifferentiation(
  ...
  auto q_autodiff = math::InitializeAutoDiffFromValueAndGradientMatrix(
      q, Eigen::MatrixXd(qdot));
  auto v_autodiff = math::InitializeAutoDiffFromValueAndGradientMatrix(
      v, Eigen::MatrixXd(vdot));
  ...
  const SpatialAcceleration<double> A_ABq_A(
      math::ExtractGradientMatrixFromAutoDiff(V_ABq_A_autodiff.get_coeffs()));

...
class IrisNp2Options {
 public:
  ...
  /** Add a hyperplane at a particle in collision if the nonlinear solve (initialized at that point) fails. Generally leads to regions with more faces, but helpful for getting the algorithm unstuck if most nonlinear solves are failing. */

...
/* Computes the gradient ∇f = (∂f/∂q)ᵀ of an unspecifed distance function "f".
 The value of "f" is based on a weighted combination of CollisionChecker's
 RobotClearance data, and so its gradient points `q` into a "clearer" state,
 reducing the likelihood of collision.

 The gradient reconciles multiple simultaneous potential collisions via a
 penalty method. The closer to penetration, the more weight that potential
 collision has. No measurements greater than `max_clearance` contribute to the

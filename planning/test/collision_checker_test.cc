...
GTEST_TEST(CollisionCheckerTest, CollisionCheckerEmpty) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(dut.plant_context(), ".*not allocated context.*");
  DRAKE_EXPECT_THROWS_MESSAGE(dut.model_context(), ".*not allocated context.*");
  DRAKE_EXPECT_THROWS_MESSAGE(dut.Clone(), ".*not allocated context.*");
  DRAKE_EXPECT_THROWS_MESSAGE(dut.MakeStandaloneModelContext(),
                              ".*not allocated context.*");

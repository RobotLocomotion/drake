...
GTEST_TEST(VisibilityGraphTest, BoxesInCorners) {
  ...
  EXPECT_FALSE(checker->CheckConfigCollisionFree(0.5 * points.col(1) +
                                                 0.5 * points.col(2)));
  ...
  EXPECT_TRUE(CompareMatrices(A.toDense().cast<int>(), A_expected.cast<int>()));

  // Now with a (too) large edge step size, it returns a fully connected graph.

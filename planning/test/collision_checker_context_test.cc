...
/* Simply confirms that providing *only* an (empty) model populates all the
 contexts (and the attendant query object). We configure the for "live" mode,
 to opt-out of the extra non-kinematic state that would be used the plant in
 "sampled" mode. This would be a typical choice for a collision checker,

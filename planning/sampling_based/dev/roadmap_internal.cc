...
template <>
uint64_t SerializeStateType(
    const drake::math::RigidTransformd& state,
    // NOLINTNEXTLINE(runtime/references)
    std::vector<uint8_t>& buffer) {
...
template <>
uint64_t SerializeStateType(
    const ControlPlanningState<Eigen::Vector2d>& state,
    // NOLINTNEXTLINE(runtime/references)
    std::vector<uint8_t>& buffer) {
...
template <>
uint64_t SerializeStateType(
    const ControlPlanningState<Eigen::Vector3d>& state,
    // NOLINTNEXTLINE(runtime/references)
    std::vector<uint8_t>& buffer) {
...
template <>
uint64_t SerializeStateType(
    const ControlPlanningState<Eigen::VectorXd>& state,
    // NOLINTNEXTLINE(runtime/references)
    std::vector<uint8_t>& buffer) {
...
template <typename StateType>
Deserialized<StateType> DeserializeStateType(
...
Deserialized<ControlPlanningState<Eigen::Vector2d>> DeserializeStateType(
  ...
  return MakeDeserialized(
      ControlPlanningState<Eigen::Vector2d>(
          deserialized_state.Value(), deserialized_control.Value()),
      bytes_read);
}
...
Deserialized<ControlPlanningState<Eigen::Vector3d>> DeserializeStateType(
  ...
  const uint64_t bytes_read = current_offset - starting_offset;
  return MakeDeserialized(
      ControlPlanningState<Eigen::Vector3d>(
          deserialized_state.Value(), deserialized_control.Value()),
      bytes_read);
}
...
Deserialized<ControlPlanningState<Eigen::VectorXd>> DeserializeStateType(
  ...
  const uint64_t bytes_read = current_offset - starting_offset;
  return MakeDeserialized(
      ControlPlanningState<Eigen::VectorXd>(
          deserialized_state.Value(), deserialized_control.Value()),
      bytes_read);
}
}  // namespace

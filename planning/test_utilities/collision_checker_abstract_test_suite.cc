...
void CollisionCheckerAbstractTestSuite::TestParallelizeableDiscreteQueries(
  ...
  auto& checker = *params.checker;
  // Check single queries Queries we know are collision-free
  EXPECT_TRUE(checker.CheckConfigCollisionFree(qs_.q1));
...
void CollisionCheckerAbstractTestSuite::TestParallelizeableGradientQueries(
  ...
  while (current_iteration < max_iterations &&
    ...
    const Eigen::VectorXd delta_qtest =
    ...
    log()->info(
        ...
        common_robotics_utilities::print::Print(scaled_delta_qtest),
        common_robotics_utilities::print::Print(qtest));
    current_iteration++;
  }

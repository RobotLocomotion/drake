...
// new contact is added in addition to an existing contact.
GTEST_TEST(HybridMultibodyCollocationTest, DropAndRollIntoCorner) {
  const std::string xml = R"""(
<mujoco model="test">
<worldbody>
    <geom name="ramp1" type="box" size="50 50 50" pos="-12.94 0 -48.29"
        euler="0 15 0"/>
    <geom name="ramp2" type="box" size="50 50 50" pos="12.94 0 -48.29"
        euler="0 -15 0"/>
    <body name="body">
    <joint type="slide" name="x" axis="1 0 0" />
    <joint type="slide" name="z" axis="0 0 1" />
    <joint type="hinge" name="theta" axis="0 1 0" />
    <geom name="ball" type="sphere" size="1" mass="1"  pos="3 0 0"/>
    </body>
</worldbody>
</mujoco>)""";
  ...
  auto result = Solve(prog);
  unused(ramp1_phase, corner_phase);

  /* TODO(russt): Need to finish this test.
    EXPECT_TRUE(result.is_success());
    // Ball is rolling at the final state.
    EXPECT_GE(result.GetSolution(ground_phase->final_state()[5]), 0.1);
    // Ball has rolled a non-trivial amount.

...
bool SnoptSolverUnavailableDuringMemoryCheck() {
  return (std::getenv("VALGRIND_OPTS") != nullptr) &&
         !(solvers::SnoptSolver::is_available() &&
...
GTEST_TEST(GcsTrajectoryOptimizationTest, DerivativeBoundsOnEdges) {
  ...
  */
  ...
  // wants to race, but the only availbe car is a tow truck 🛻!
  ...
  if (SnoptSolverUnavailableDuringMemoryCheck()) return;

...
GTEST_TEST(GcsTrajectoryOptimizationTest, VelocityBoundsOnEdges) {
  ...
  his car allow through the finish line (50 m/s). There is only one problem,
  ...
  */
  ...
  auto& race_track_2 = gcs.AddRegions(
      MakeConvexSets(HPolyhedron::MakeBox(Vector2d(0, -5), Vector2d(305, 5))),
      3);
  // Bob's car can only drive straight. So he can drive at 50 m/s in x,
  // but not in y.
  race_track_1.AddVelocityBounds(Vector2d(-kMaxSpeed, 0),
                                 Vector2d(kMaxSpeed, 0));
  race_track_2.AddVelocityBounds(Vector2d(-kMaxSpeed, 0),
                                 Vector2d(kMaxSpeed, 0));

...
TEST_F(SimpleEnv2D, GlobalContinuityConstraints) {
  ...
  auto descaled_traj = ReconstructGcsPathTrajectory(traj);
  EXPECT_TRUE(CompareMatrices(descaled_traj.value(descaled_traj.start_time()),
                              start, 1e-6));
  EXPECT_TRUE(CompareMatrices(descaled_traj.value(descaled_traj.end_time()),
                              goal, 1e-6));
  EXPECT_EQ(descaled_traj.get_number_of_segments(),
            traj.get_number_of_segments());
  EXPECT_EQ(descaled_traj.start_time(), traj.start_time());
  // Since each segment will be one second long, we expect the number of
  // segments to match the duration of the descaled trajectory.
  EXPECT_EQ(descaled_traj.get_number_of_segments(),
            descaled_traj.end_time() - descaled_traj.start_time());

  // Check for velocity and acceleration continuity.
  for (int i = 0; i < descaled_traj.get_number_of_segments() - 1; ++i) {
    auto descaled_segment_vel = descaled_traj.segment(i).MakeDerivative();
    auto descaled_next_segment_vel =
        descaled_traj.segment(i + 1).MakeDerivative();
    EXPECT_TRUE(CompareMatrices(
        descaled_segment_vel->value(descaled_segment_vel->end_time()),
        descaled_next_segment_vel->value(
            descaled_next_segment_vel->start_time()),
        1e-6));

    auto descaled_segment_acc = descaled_traj.segment(i).MakeDerivative(2);
    auto descaled_next_segment_acc =
        descaled_traj.segment(i + 1).MakeDerivative(2);
    EXPECT_TRUE(CompareMatrices(
        descaled_segment_acc->value(descaled_segment_acc->end_time()),
        descaled_next_segment_acc->value(
            descaled_next_segment_acc->start_time()),
        1e-6));
  }
}

...
GTEST_TEST(GcsTrajectoryOptimizationTest, PathLengthCost) {
  ...
  const GraphOfConvexSets& gcs = trajopt.gcs();
...
GTEST_TEST(GcsTrajectoryOptimizationTest, VelocityBounds) {
  ...
  // dimension 1 lower bound. The velocity in the first half of the trajectory is ((1 - 4) / 0.5) / 2 = -3. That exceeds the lower bound of -2.
  ...
  Vector2d start(-0.5, -0.5), goal(0.5, 0.5);
  auto& source = trajopt.AddRegions(MakeConvexSets(Point(start)), 0);
  auto& target = trajopt.AddRegions(MakeConvexSets(Point(goal)), 0);
  trajopt.AddEdges(source, regions);
  trajopt.AddEdges(regions, target);

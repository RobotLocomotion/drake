...
class ImpactConstraint : public solvers::Constraint {
 public:
  ...
  ImpactConstraint(const RobotDiagram<AutoDiffXd>* robot_diagram,
      ...
      : Constraint(
            robot_diagram->plant().num_velocities() + 3 * in_contact.size(),
            robot_diagram->plant().num_positions() +
                2 * robot_diagram->plant().num_velocities() +
                3 * in_contact.size()),
        robot_diagram_(robot_diagram),
        in_contact_(in_contact),
 ...
 private:
  ...
  void DoEval(const Eigen::Ref<const AutoDiffVecXd>& x,
    ...
    DRAKE_ASSERT(index + 3 * num_contacts_ == num_vars());
    ...
    const auto& query_object =
        plant.get_geometry_query_input_port()
            .template Eval<geometry::QueryObject<AutoDiffXd>>(*plant_context);
    auto& inspector = query_object.inspector();
    MatrixX<AutoDiffXd> J(3 * num_contacts_, plant.num_velocities());

    int i = 0;
    for (const auto& c : in_contact_) {
      geometry::SignedDistancePair<AutoDiffXd> distance_pair =
      ...
      const Frame<AutoDiffXd>& A =
          plant.GetBodyFromFrameId(inspector.GetFrameId(c.first()))
              ->body_frame();
      const Frame<AutoDiffXd>& B =
      ...
      plant.CalcJacobianTranslationalVelocity(
          *plant_context, JacobianWrtVariable::kV, A, p_ACa,
          plant.world_frame(), plant.world_frame(), &J_WCa);
      plant.CalcJacobianTranslationalVelocity(
          *plant_context, JacobianWrtVariable::kV, B, p_BCb,
          plant.world_frame(), plant.world_frame(), &J_WCb);
      J.middleRows<3>(3 * i) = J_WCb - J_WCa;
      ++i;
    }
    ...
    y->tail(3 * num_contacts_) = J * vplus;
    for (i = 0; i < num_contacts_; ++i) {
      (*y)[plant.num_velocities() + 3 * i] -=
...
HybridMultibodyCollocation::AddModeWithInelasticImpact(
  ...
  prog_.AddConstraint(
      std::make_shared<internal::ImpactConstraint>(
          robot_diagram_ad_.get(), in_contact, coefficient_of_restitution,
  ...
  for (const auto& c : in_contact) {
    auto& contact_wrench_evaluator = contact_wrench_evaluators_.emplace_back(
        std::make_unique<
            multibody::ContactWrenchFromForceInWorldFrameEvaluator>(
            &plant, plant_context, c));
    prog_
        .AddConstraint(
            std::make_shared<multibody::StaticFrictionConeConstraint>(
                contact_wrench_evaluator.get()),
            {d->initial_state().head(plant.num_positions()),
             Lambda.segment<3>(3 * i)})
        .evaluator()
        ->set_description(fmt::format("static friction cone for impact between "
                                      "geometries {} and {} from_mode {} to {}",
                                      c.first(), c.second(), dircon_.size() - 2,
                                      dircon_.size() - 1));
    ++i;
  }

...
/** Implements hybrid trajectory optimization using a MultibodyPlant, where the
 ...
 are added as explicit decision variables. Each mode is defined by the active
 set of the SceneGraph collision candidates, the contact model is rigid contact
 (no penetration at optimality), and collisions can be inelastic or elastic.
 ...
 For example, to generate an optimization that starts in an "aerial phase" (with
 no contacts), and then transitions to a "ground phase" with the first contact
 active, one could write e.g.
 @code
  HybridMultibodyCollocation hybrid(diagram, context, kMinTimeStep,
                                    kMaxTimeStep)
  auto* aerial_phase = hybrid.AddMode("aerial", kNumTimeSteps, {});
  auto first_contact_pair = *hybrid.GetContactPairCandidates().begin();
  auto* ground_phase = hybrid.AddModeWithInelasticImpact(
      "ground", kNumTimeSteps, first_contact_pair);
 @endcode
 ...
 public:
  ...
  class ConstrainedDirectCollocation : public MultipleShooting {
   public:
    ...
    constraints at the @p index sample. */

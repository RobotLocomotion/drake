...
class GCSTrajectoryOptimization {
  ...
  */
  ...
  @param d_max is the maximum duration to spend in a region (seconds) if that
  region is visited on the optimal path. Some solvers struggle numerically with
  large values.
  ...
  */
  ...
  added, including the position constraint on the connecting control points.
  Only subspaces of type point or HPolyhedron are supported. Otherwise create a
  subgraph of zero order with the subspace as the region and connect it between
  the two subgraphs.
  */
  ...
  defined for Bezier curves that have two or more control points, this cost will

...
The ith piece of the composite trajectory is defined as q(t) = r((t - t_i) /
d_i). r : [0, 1] → ℝⁿ is a the path, parametrized as a Bézier curve with order
n. tᵢ and dᵢ are the initial time and duration of the ith sub-trajectory.
  ...
  */
  ...
  class Subgraph final {
    ...
    */
    ...
    /** Adds multiple L2Norm Costs on the upper bound of the path length.
    We upper bound the trajectory lengthy by the sum of the distances between
    control points. For Bézier curves, this is equivalent to the sum
    of the L2Norm of the derivative control points of the curve divided by the
    order.

...
/* Implements a constraint of the form
  ...
  duration = x[0]
  control_points = x[1:num_control_points].reshaped(num_positions,
  num_control_points) q = control_points * a_pos v = control_points * a_vel
*/
...
// For MatrixXDecisionVariable X and VectorXd a, returns SparseMatrix A and
// VectorXDecisionVariable x such that Ax == Xa.

...
KinematicTrajectoryOptimization::AddVelocityConstraintAtNormalizedTime(
  ...
  std::vector<std::optional<int>> var_to_qdot(binding.variables().size());
  ...
  for (int i = 0; i < binding.variables().size(); ++i) {
    ...
    var_to_qdot[i] = FindVariableIndex(var, placeholder_qdot_vars_);
    if (!var_to_qdot[i].has_value()) {
    ...
    P_triplets.emplace_back(i, var_to_qdot[i].value(), 1);
  }

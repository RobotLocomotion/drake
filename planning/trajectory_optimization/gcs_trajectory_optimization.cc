...
void Subgraph::AddPathLengthCost(const MatrixXd& weight_matrix) {
  ...
  */
  ...
  for (Vertex* v : vertices_) {
    ...
    for (int i = 1; i < control_points.cols(); ++i) {
...
void Subgraph::AddVelocityBounds(const Eigen::Ref<const VectorXd>& lb,
  ...
  // rdot_control.row(i) = r_control.row(i) * M, so we loop over each dimension
  // and the bound for that element of all the control points.
  ...
  H.leftCols(order_ + 1) =
      r_trajectory_.DerivativeControlPointCoefficients(1).transpose();
  for (int i = 0; i < r_trajectory_.rows(); ++i) {
    // Lower bound.
    H.rightCols<1>() = VectorXd::Constant(order_, -lb[i]).sparseView();
    const auto lb_constraint =
        std::make_shared<LinearConstraint>(H, kVecZero, kVecInf);
    for (Vertex* v : vertices_) {
      vars << GetControlPoints(*v).row(i).transpose(), GetTimeScaling(*v);
      v->AddConstraint(Binding<LinearConstraint>(lb_constraint, vars));
    }
    // Upper bound.
    H.rightCols<1>() = VectorXd::Constant(order_, -ub[i]).sparseView();
    const auto ub_constraint =
        std::make_shared<LinearConstraint>(H, -kVecInf, kVecZero);
    for (Vertex* v : vertices_) {
      vars << GetControlPoints(*v).row(i).transpose(), GetTimeScaling(*v);

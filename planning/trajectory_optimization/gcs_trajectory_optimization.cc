...
/* Implements a constraint of the form
  ...
  N := derivative order,
  h = x[num_ctrl_pts],
  dᴺr(s) / dsᴺ = M * x[0:num_ctrl_pts-1].

  This constraint is enforced along one dimension of the Bézier curve, hance
  mussed be called for each dimension separately.
*/
...
void Subgraph::AddNonlinearDerivativeBounds(
  ...
  // reasonable heuristc, e.g. based on the maximum length of the convex set.
  const double h0 = 1.0;
  ...
  for (int i = 0; i < num_positions(); ++i) {
    ...
    H.block(0, order_ + 1, rdot_control_points, 1) =
        VectorXd::Constant(rdot_control_points,
                           -pow(h0, derivative_order - 1) * lb[i])
            .sparseView();
    H.block(rdot_control_points, order_ + 1, rdot_control_points, 1) =
        VectorXd::Constant(rdot_control_points,
                           pow(h0, derivative_order - 1) * ub[i])
            .sparseView();
...
void EdgesBetweenSubgraphs::AddNonlinearDerivativeBounds(
  ...
  if (from_subgraph_.order() >= derivative_order) {
    ...
    for (int i = 0; i < num_positions(); ++i) {
      ...
      H(0, from_subgraph_.order() + 1) = -pow(h0, derivative_order - 1) * lb[i];
      H(1, from_subgraph_.order() + 1) = pow(h0, derivative_order - 1) * ub[i];
  ...
  if (to_subgraph_.order() >= derivative_order) {
    ...
    for (int i = 0; i < num_positions(); ++i) {
      ...
      H(0, to_subgraph_.order() + 1) = -pow(h0, derivative_order - 1) * lb[i];
      H(1, to_subgraph_.order() + 1) = pow(h0, derivative_order - 1) * ub[i];
      const auto convex_derivative_constraint =
          std::make_shared<LinearConstraint>(H.sparseView(), kVecZero, kVecInf);

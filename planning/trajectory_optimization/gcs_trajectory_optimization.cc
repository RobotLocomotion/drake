...
std::tuple<std::vector<MatrixXd>, VectorXb> CondenseToNonzeroColumns(
  ...
  for (const MatrixXd& matrix : matrices) {
    ...
    int condensed_col_idx = 0;
    for (int orig_col_idx = 0; orig_col_idx < matrix.cols(); ++orig_col_idx) {
      if (nonzero_cols_mask(orig_col_idx)) {
        condensed_matrix.col(condensed_col_idx) = matrix.col(orig_col_idx);
        condensed_col_idx++;
      }
    }
  }

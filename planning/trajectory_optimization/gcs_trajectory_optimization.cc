...
trajectories::CompositeTrajectory<double> DescaleGcsTrajectory(
    const trajectories::CompositeTrajectory<double>& gcs_trajectory) {
  std::vector<copyable_unique_ptr<Trajectory<double>>> unscaled_bezier_curves;
  ...
  for (int i = 0; i < gcs_trajectory.get_number_of_segments(); ++i) {
    ...
    if (const BezierCurve<double>* gcs_segment =
            dynamic_cast<const BezierCurve<double>*>(
                &gcs_trajectory.segment(i))) {
      unscaled_bezier_curves.emplace_back(std::make_unique<BezierCurve<double>>(
          start_time, start_time + 1.0, gcs_segment->control_points()));
      start_time += 1.0;
    } else {
      throw std::runtime_error(
  ...
  return CompositeTrajectory<double>(unscaled_bezier_curves);
}

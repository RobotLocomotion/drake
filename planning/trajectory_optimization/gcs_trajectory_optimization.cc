...
// values along the dimenesions corresponding to those revolute joints for two
// convex sets, compute the translation that should be applied to the first
// convex set to align it with the second, such that can directly be checked
// without considering the 2π wraparound that may occur with continuous revolute
// joints.
VectorXd ComputeOffset(
    const int dimension, const std::vector<int>& continuous_revolute_joints,
  ...
  for (int kk = 0; kk < ssize(continuous_revolute_joints); ++kk) {
    ...
    if (continuous_bbox_A.at(kk).first < continuous_bbox_B.at(kk).first) {
      ...
      offset(k) = 2 * M_PI *
                  std::floor((continuous_bbox_B.at(kk).second -
                              continuous_bbox_A.at(kk).first) /
                             (2 * M_PI));
    } else {
      ...
      // translation.
      offset(k) = -2 * M_PI *
                  std::floor((continuous_bbox_A.at(kk).second -
                              continuous_bbox_B.at(kk).first) /
                             (2 * M_PI));
    }
  }
  return offset;
...
PairwiseIntersectionsContinuousJoints(
  ...
  for (int i = 0; i < ssize(convex_sets_A); ++i) {
    region_minimum_and_maximum_values_A.emplace_back(
        GetMinimumAndMaximumValueAlongDimension(*convex_sets_A[i],
  ...
  if (compute_intersections_within_A) {
    ...
    for (int i = 0; i < ssize(convex_sets_B); ++i) {
      maybe_region_minimum_and_maximum_values_B.emplace_back(
          GetMinimumAndMaximumValueAlongDimension(*convex_sets_B[i],
                                                  continuous_revolute_joints));
    }
  }
  ...
  for (int i = 0; i < ssize(convex_sets_A); ++i) {
    for (int j = 0; j < ssize(convex_sets_B); ++j) {
      ...
      offset = ComputeOffset(dimension, continuous_revolute_joints,
                             region_minimum_and_maximum_values_A.at(i),
...
EdgesBetweenSubgraphs::EdgesBetweenSubgraphs(
  ...
  if (subspace != nullptr) {
    std::vector<std::pair<double, double>> subspace_revolute_joints_bbox =
        GetMinimumAndMaximumValueAlongDimension(*subspace,
                                                continuous_revolute_joints());
    for (const auto& convex_set_ptr : from_subgraph.regions()) {
      std::vector<std::pair<double, double>> set_revolute_joints_bbox =
          GetMinimumAndMaximumValueAlongDimension(*convex_set_ptr,
                                                  continuous_revolute_joints());
      sets_A_subspace_offset.push_back(ComputeOffset(
  ...
  for (const auto& edge : edge_data) {
    ...
    if (subspace != nullptr) {
      ...
      const int u_last_start = (from_subgraph.order() - 1) * num_positions();
      const auto vars = u->x().segment(u_last_start, num_positions());
      if (typeid(*subspace) == typeid(Point)) {
        const auto subgraph_constraint =
            std::make_shared<LinearEqualityConstraint>(
                ...
                subspace->MaybeGetPoint().value() + sets_A_subspace_offset[i]);
        uv_edge->AddConstraint(Binding<Constraint>(subgraph_constraint, vars));
      } else if (typeid(*subspace) == typeid(HPolyhedron)) {
        const HPolyhedron* hpoly_ptr =
            dynamic_cast<const HPolyhedron*>(subspace);
        const Eigen::MatrixXd& hpoly_A = hpoly_ptr->A();
        const Eigen::VectorXd& hpoly_b = hpoly_ptr->b();

...
// Internal driver of the adaptive interval certifier. Nothing here is part of
// the public API; it exists so that continuous_collision_checker.cc and
// certifier.cc can share one set of per-call data structures.
...
/* Everything one run needs; assembled by the facade. All pointers are aliased
...
The search returns the earliest-in-time violation and stops as soon as that
violation is proven earliest.
...
contexts, one atomic earliest-violation bound, an atomic node counter, and a
findings sink under a mutex.
...
child keeps each worker's own descent left-first, which is what makes the
earliest-violation bound tighten early.
...
points and its inherited active set, so the *reported witness* and the verdict
are identical serially and at any thread count; Result::num_nodes is not,
because the bound prunes a timing-dependent set of nodes that start at or after
a witness already found. One exception: on a degenerate segment with t_start ==
t_end every node maps to the same time, so the bound prunes on a tie and the
reported configuration (not its time) may differ.

/// @file
/// T8 — the two per-call parallel *scaling* claims, split out of
/// concurrency_test.cc because they are wall-clock claims and it is not.
///
/// Every other T8 case is an equality and runs under every build flavor. A
/// duration, by contrast, means nothing under an instrumented build: Valgrind
/// serializes threads outright, so `parallel < serial` inverts and the case
/// fails for a reason that has nothing to do with the driver. Hence the
...
/// and a sanitizer runtime linked in from elsewhere is equally invisible, so
/// the environment is consulted too: the tools that make a duration
/// meaningless all announce themselves through an options variable. That is
/// the same test limit_malloc.cc uses to disarm itself, and the same
/// VALGRIND_OPTS check gcs_trajectory_optimization_test.cc uses.

...
// The fixed benchmark worlds and trajectories. Every world is built from the
// cached `drake_models` iiwa14 dense-sphere collision model plus programmatic
// anchored boxes, so a run is reproducible from this file alone.
...
// The dense-sphere iiwa14 collision variant: 46 collision spheres over links
// 0-7, i.e. realistic proximity-pair counts rather than the 4-primitive model.
extern const char* const kIiwaUrl;

// Nominal shelf geometry, in metres: the fixed dimensions of the bookcase that
// MakeShelfWorld builds.
...
// `shelf_scale` s translates the whole bookcase by +s in x *and* opens the
// reached-into bay by s on each side, so the swept clearance of the fixed
// benchmark trajectory is monotone non-decreasing in s over the useful range.
// This one scalar is what the tier bisection turns.
...
// Times of the dual-arm waypoints (0, 1, ..., 4): 4 quintic Bézier segments.

...
std::unique_ptr<CollisionChecker> CollisionChecker::Clone() const {
  if (IsInitialSetup()) {
    throw std::logic_error(
      "Cannot clone; CollisionChecker has not allocated contexts yet");
  }
  return DoClone();
  }
...
const CollisionCheckerContext& CollisionChecker::model_context() const {
  if (IsInitialSetup()) {
    throw std::logic_error(
        "Cannot access implicit context; CollisionChecker has not allocated "
        "contexts yet");
  }
...
CollisionChecker::MakeStandaloneModelContext() const {
  if (IsInitialSetup()) {
    throw std::logic_error(
        "Cannot make stand alone context; CollisionChecker has not allocated "
        "contexts yet");
  }
...
void CollisionChecker::PerformOperationAgainstAllModelContexts(
  ...
  if (IsInitialSetup()) {
    throw std::logic_error(
        "Cannot apply operation; CollisionChecker has not allocated contexts "
        "yet");
  }

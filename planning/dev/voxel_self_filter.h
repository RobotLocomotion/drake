...
void SetSphereCells(
  ...
  for (int64_t x_index = min_corner_index.X();
       x_index <= max_corner_index.X();
       ++x_index) {
    for (int64_t y_index = min_corner_index.Y();
         y_index <= max_corner_index.Y();
         ++y_index) {
      for (int64_t z_index = min_corner_index.Z();
           z_index <= max_corner_index.Z();
           ++z_index) {
...
void SelfFilter(
  ...
  for (const auto& filter_sphere : filter_spheres) {
    SetSphereCells(environment, filter_sphere, padding, empty_cell_value);
  }

...
SampledClearance SampleClearance(
    const CertifiedContinuousCollisionChecker& checker,
    const PiecewiseBezierPath& path, int samples_per_segment,
    double threshold) {
...
GTEST_TEST(CertifierTest, PaddingSemantics) {
  ...
  // environment padding.
  {
    ...
    const CertifiedContinuousCollisionChecker checker(params);
    const CertificationResult result = checker.CheckTrajectory(trajectory);
    ASSERT_EQ(result.verdict, Verdict::kViolationFound);
  ...
  // A mis-sized matrix is a clear throw.
  {
    ...
    EXPECT_THROW(CertifiedContinuousCollisionChecker{params}, std::exception);
  }
}
...
class CertificateFixture : public ::testing::Test {
 protected:
  ...
  CertifiedContinuousCollisionChecker checker_;

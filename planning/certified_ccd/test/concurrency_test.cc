...
///
///     bazel test --config=tsan //planning/certified_ccd:concurrency_test
///
...
/// certified_ccd frames. It sees all of the
...
/// rooted in a
/// certified_ccd frame is a real bug.
...
const std::vector<std::unique_ptr<Case>>& Corpus() {
  static const std::vector<std::unique_ptr<Case>>* corpus = [] {
    ...
    for (uint64_t seed = 1; seed <= 200; ++seed) {
      ...
      entry->checker =
          std::make_unique<CertifiedContinuousCollisionChecker>(params);

/// @file
/// The `certified_ccd` performance benchmark suite (the performance
...
#include "drake/planning/certified_ccd/benchmark/benchmark_util.h"
#include "drake/planning/certified_ccd/benchmark/scenario_worlds.h"
...
struct World {
  ...
  std::unique_ptr<CertifiedContinuousCollisionChecker> certified;
...
World MakeWorld(std::shared_ptr<RobotDiagram<double>> diagram,
  ...
  CertifiedContinuousCollisionChecker::Params cparams;
  cparams.model = world.diagram;
  world.certified =
      std::make_unique<CertifiedContinuousCollisionChecker>(cparams);
...
CertRun MeasureCertify(const CertifiedContinuousCollisionChecker& checker,
...
CertRun MeasureCertifyEdge(const CertifiedContinuousCollisionChecker& checker,
...
double ProbeCost(const CertifiedContinuousCollisionChecker& checker,
...
int Main(int argc, char** argv) {
  ...
  std::printf("certified_ccd benchmark suite\n");
...
int main(int argc, char** argv) {
  try {
    return drake::planning::certified_ccd::benchmark::Main(argc, argv);

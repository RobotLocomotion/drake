...
KinematicsEngine::KinematicsEngine(
  ...
  if (!plant_->is_finalized()) {
    throw std::runtime_error(
        "certified_ccd: KinematicsEngine requires a finalized "
...
void KinematicsEngine::BuildTopology() {
  ...
  for (JointIndex ji : joint_indices) {
    ...
    try {
      ...
      throw std::runtime_error(fmt::format(
          "certified_ccd: joint '{}' ({}) is mounted on a frame whose pose in "
          "its body is not fixed, so its chain contribution to the reach "
          "bound cannot be computed at construction time. Mount joints on "
          "body frames or FixedOffsetFrames. Underlying error: {}",
  ...
  for (int b = 0; b < num_bodies_; ++b) {
    if (!visited[b]) {
      throw std::runtime_error(fmt::format(
          "certified_ccd: body '{}' is not connected to the world through the "
          "plant's joints; the kinematics module requires the single "
  ...
  for (const JointRecord& rec : joints_) {
    if (!rec.outboard.is_valid()) {
      throw std::runtime_error(fmt::format(
          "certified_ccd: joint '{}' ({}) closes a kinematic loop (both of its "
          "bodies are already reachable from the world without it). Loop "
  ...
  for (int k = 0; k < static_cast<int>(joints_.size()); ++k) {
    ...
    if (rec.outboard != joint.child_body().index()) {
      throw std::runtime_error(fmt::format(
          "certified_ccd: joint '{}' ({}) is reversed — its declared parent "
    ...
    if (rec.subtree != tree_subtree[k]) {
      throw std::runtime_error(fmt::format(
          "certified_ccd: the plant's kinematically-affected set for joint "
  ...
  for (int b = 0; b < num_bodies_; ++b) {
    ...
    for (GeometryId gid :
      ...
      try {
        ...
        throw std::runtime_error(fmt::format(
            "certified_ccd: proximity geometry '{}' on body '{}' cannot be "
            "bounded. {}",
...
void KinematicsEngine::CheckHalfSpaceRule() const {
  ...
  for (const auto& [ga, gb] : inspector.GetCollisionCandidates()) {
    ...
    for (int k : positioned_order_) {
      ...
      throw std::runtime_error(fmt::format(
          "certified_ccd: HalfSpace geometry '{}' (body '{}') rotates relative "
          "to its unfiltered partner geometry '{}' (body '{}') through joint "
          "'{}' ({}). A half space has unbounded reach, so no finite motion "
          "bound λ exists for that pair (the geometry-support scope). Fix the "
          "model by anchoring "
          "the half space, filtering the pair, or replacing the half space "
          "with a large Box.",
...
double KinematicsEngine::Reach(int joint_ord, BodyIndex body,
  ...
  throw std::runtime_error(
      "certified_ccd: internal error — reach chain walk did not reach the "
...
MotionBoundTable KinematicsEngine::ComputeMotionBoundTable(
  ...
  if (path.num_positions() != num_positions_) {
    throw std::runtime_error(fmt::format(
        "certified_ccd: the path has {} positions but the plant has {}.",
...
MotionBoundTable KinematicsEngine::ComputeMotionBoundTable(
  ...
  if (lower.size() != num_positions_ || upper.size() != num_positions_ ||
    ...
    throw std::runtime_error(fmt::format(
        "certified_ccd: control-box size mismatch — got lower={}, upper={}, "
  ...
  for (int c = 0; c < num_positions_; ++c) {
    if (!std::isfinite(lower[c]) || !std::isfinite(upper[c]) ||
        lower[c] > upper[c]) {
      throw std::runtime_error(fmt::format(
          "certified_ccd: the trajectory's global control box is invalid at "
          "coordinate {}: [{}, {}].",
          c, lower[c], upper[c]));
    }
  }
      ...
      case JointKind::kUnsupported: {
        for (int c = ps; c < ps + rec.num_positions; ++c) {
          if (!constant_coordinates[c]) {
            throw std::runtime_error(fmt::format(
                "certified_ccd: this trajectory moves coordinate {} of joint "
                "'{}', whose type '{}' is excluded in v1 (the joint-support "
                "scope). Quaternion "
                ...
                "is accepted via the constant-coordinate carve-out. See "
                "the white paper's future extensions for the "
                "manifold-curve extension.",
  ...
  for (const PairId& pair : pairs) {
    ...
    if (!a.is_valid() || !b.is_valid() || a >= num_bodies_ ||
        b >= num_bodies_) {
      throw std::runtime_error(fmt::format(
          "certified_ccd: pair references body indices ({}, {}) outside the "
          "plant's {} bodies.",
    ...
    for (int k : positioned_order_) {
      ...
      for (int c = ps; c < ps + rec.num_positions; ++c) {
        ...
        switch (rec.kind) {
          ...
          case JointKind::kUnsupported:
            throw std::runtime_error(fmt::format(
                "certified_ccd: internal error — joint '{}' ({}) reached the "
                "λ assembly with an unsupported kind.",
                rec.name, rec.type_name));
        }
...
const BoundingSphere& KinematicsEngine::geometry_sphere(GeometryId id) const {
  ...
  if (it == geometry_spheres_.end()) {
    throw std::runtime_error(fmt::format(
        "certified_ccd: geometry {} has no bounding sphere; it is either not a "
        "proximity geometry of this model or it is a HalfSpace.",
        id));
  }
  return it->second;
}

...
{
  if(nrhs != 4){
    mexErrMsgIdAndTxt("Drake:cylindrical2cartesianmex:IncorrectInputs", "Usage [x,dx] = cylindrical2cartesian(cylinder_axis,cylinder_x_dir,cylinder_origin,x_cylinder)");
  }
  ...
  {
    mexErrMsgIdAndTxt("Drake:cylindrical2cartesianmex:InvalidInput","cylinder_x_dir and cylinder_axis should be perpendicular to each othter");
  }
  Matrix3d R;
  ...
  T = T.rotate(R);  
  auto x_output = cylindrical2cartesian(T,x_input);

...
for i = 1 : 100
  ...
  [dT, dTInv, dAdT, dAdT_transpose, x_norm, dx_norm, ddx_norm] = testGeometryGradientsmex(T, S, qdot_to_v, X, dX, x);
  
  check_dT = dHomogTrans(T, S, qdot_to_v);
  check_dTInv = dHomogTransInv(T, dT);
  check_dAdT = dTransformAdjoint(T, X, dT, dX);
  check_dAdT_transpose = dTransformAdjointTranspose(T, X, dT, dX);
  ...
  valuecheck(check_dAdT_transpose, dAdT_transpose);

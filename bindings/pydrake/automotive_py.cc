...
PYBIND11_MODULE(automotive, m) {
  ...
  py::class_<RoadOdometry<T>>(m, "RoadOdometry")
      ...
      .def(py::init<const maliput::api::RoadPosition&,
           ...
           // Keep alive, reference: `self` keeps `RoadPosition` alive.
           py::keep_alive<1, 2>())
      .def(py::init<const maliput::api::Lane*,
           ...
           py::keep_alive<1, 2>())
      .def_readwrite("lane", &RoadOdometry<T>::lane, py_reference_internal)

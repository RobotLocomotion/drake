...
class JointSliders(VectorSystem):
    ...
    def __init__(self, robot, lower_limit=-10., upper_limit=10.,
        ...
        for i in range(0, robot.num_joints()):
            ...
            for j in range(0, joint.num_positions()):
                self.__slider.append(tk.Scale(self.window,
                                              from_=max(low[j],
                                                        lower_limit[k]),
    ...
    def set(self, q):
        ...
        """
        assert(len(q) == len(self.__slider))
        for i in range(len(self.__slider)):
            self.__slider[i].set(q[i])
    ...
    def _DoCalcVectorOutput(self, context, unused, unused2, output):
        for i in range(0, len(self.__slider)):
            output[i] = self.__slider[i].get()
...
class SchunkWsgButtons(LeafSystem):
    ...
    def __init__(self, window=None, open_position=0.055,
                 closed_position=0.008, force_limit=40, update_period_sec=0.1):
        """"
        Args:
        ...
        self.__open_button = tk.Button(self.window, text="Open Gripper",
                                       state=tk.DISABLED,
                                       command=self.open)
        self.__open_button.pack()
        self.__close_button = tk.Button(self.window, text="Close Gripper",
                                        command=self.close)
        self.__close_button.pack()

        self.__open_state = True

        self.__open_position = open_position
        self.__closed_position = closed_position
        self.__force_limit = force_limit
    ...
    def open(self):
        ...
        """
        self.__open_state = True
        self.__open_button.configure(state=tk.DISABLED)
    ...
    def close(self):
        ...
        self.__open_button.configure(state=tk.NORMAL)
        self.__close_button.configure(state=tk.DISABLED)

...
class TestCollisionChecker(unittest.TestCase):
    ...
    def test_robot_collision_type(self):
        ...
        # Check the setters.
        self.assertEqual(
            dut.kSelfCollision.SetInSelfCollision(
                in_self_collision=False),
            dut.kNoCollision)
        self.assertEqual(
            dut.kEnvironmentCollision.SetInSelfCollision(
                in_self_collision=True),
            dut.kEnvironmentAndSelfCollision)
        self.assertEqual(
            dut.kEnvironmentCollision.SetInEnvironmentCollision(
                in_environment_collision=False),
            dut.kNoCollision)
        self.assertEqual(
            dut.kSelfCollision.SetInEnvironmentCollision(
    ...
    def _make_scene_graph_collision_checker(self):
        robot, index = self._make_robot_diagram()
        params = mut.CollisionCheckerParams()
        params.model = robot
        params.robot_model_instances = [index]
        params.configuration_distance_function = self._configuration_distance
        params.edge_step_size = 0.125
        return mut.SceneGraphCollisionChecker(params=params)

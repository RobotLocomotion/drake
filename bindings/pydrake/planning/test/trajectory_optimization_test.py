...
from pydrake.trajectories import PiecewisePolynomial, BsplineTrajectory, CompositeTrajectory
...
class TestTrajectoryOptimization(unittest.TestCase):
    ...
    def test_gcs_trajectory_optimization_2d(self):
        ...
        We have two possible starts, S1 and S2, and two possible goals, G1 and G2.
        The goal is to find a the shortest path from either of the starts to
        either of the goals while avoiding the obstacles. Further we constraint
        the path to go through either the intermediate point I or the subspace.
        ...
        kDimension = 2
        gcs = GCSTrajectoryOptimization(positions=kDimension)
        ...
        source = gcs.AddRegions([Point(start1), Point(start2)], order=0)
        ...
        # in the x direction only for the main2 subgraph.
        ...
        if not (
            (mp.MosekSolver().available() and mp.MosekSolver().enabled()) or

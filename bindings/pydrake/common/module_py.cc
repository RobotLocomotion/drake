...
PYBIND11_MODULE(_module_py, m) {
  ...
  // function, so that it can be triggered once `pydrake.common` is fully
  // loaded for the `pydrake` root module (which is necessary so we can
  // effectively import `pydrake.common.deprecation`.
  m.def("_deferred_deprecations_due_to_stupid_dependency_cyle",
      // N.B. We need `mutable` so that we can (of course) mutate the relevant
      // binding types from pybind11 while keeping the relevant data alive
      // (which is OK to copy in a shallow fashion via this lambda).

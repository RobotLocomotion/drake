...
void MaybeRedirectPythonLogging() {
  ...
  if (log_sinks.size() != 1) {
    drake::log()->debug(
        "Will not redirect C++ logging to Python (root sinks != 1)");
    return;
  }
  ...
  if (dist_sink == nullptr) {
    drake::log()->debug(
        "Will not redirect C++ logging to Python (no dist sink)");
    return;
  }
  ...
  if (dist_sinks.size() != 1) {
    drake::log()->debug(
        "Will not redirect C++ logging to Python (dist sinks != 1)");
    return;
  }

...
void MaybeRedirectPythonLogging() {
  ...
  const std::vector<std::shared_ptr<spdlog::sinks::sink> >& log_sinks =
      drake::log()->sinks();
  if (log_sinks.size() != 1) {
    drake::log()->debug("Cannot redirect C++ logs to Python (sinks != 1)");
    return;
  }
  spdlog::sinks::sink* const log_sink = log_sinks.front().get();
  auto* dist_sink = dynamic_cast<spdlog::sinks::dist_sink_mt*>(log_sink);
  if (dist_sink == nullptr) {
    drake::log()->debug("Cannot redirect C++ logs to Python (not a dist sink)");
    return;
  }
  const std::vector<std::shared_ptr<spdlog::sinks::sink> >& dist_sinks =
      dist_sink->sinks();
  if (dist_sinks.size() != 1) {
    drake::log()->debug("Cannot redirect C++ logs to Python (dist sinks != 1)");
    return;
  }
  spdlog::sinks::sink* const only_sink = dist_sinks.front().get();
  if (dynamic_cast<spdlog::sinks::stderr_sink_mt*>(only_sink) == nullptr) {
    drake::log()->debug("Cannot redirect C++ logs to Python (not stderr sink)");
    return;
  }

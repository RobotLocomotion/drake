...
class TestTextLoggingExample(unittest.TestCase,
    ...
    def test_example(self, *, use_nice_format, root_level, drake_level):
        ...
        # Confirm that all messages above that level were output.
        for some_level_token in expected_messages:
            level = token_to_level[spdlog_level_token]
            if level >= effective_level:
                self.assertIn(spdlog_level_token, observed_spdlog_tokens)
            else:
                self.assertNotIn(spdlog_level_token, observed_spdlog_tokens)

        # Compute what message level we should expect to have seen.
        if root_level < 0:
            # We did not directly set the root level; what was it's default?
            if use_nice_format:
                ...
                root_level = WARNING
        if drake_level < 0:
            # We did not directly set the Drake level; what was it's default?
            if use_nice_format:

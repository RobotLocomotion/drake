import pydrake.common.eigen_geometry as eigen_geometry
...
import pydrake.common.test.eigen_geometry_test_util as eigen_geometry_test_util
...
class TestEigenGeometry(unittest.TestCase):
    ...
    def check_quaternion(self, T):
        # Simple API.
        Quaternion = eigen_geometry.Quaternion_[T]
        cast = np.vectorize(T)
        ...
        # Test `type_caster`s.
        if T == float:
            value = eigen_geometry_test_util.create_quaternion()
            self.assertTrue(isinstance(value, eigen_geometry.Quaternion))
            eigen_geometry_test_util.check_quaternion(value)

    def test_isometry3(self):
        self.check_types(self.check_isometry3)

    def check_isometry3(self, T):
        Isometry3 = eigen_geometry.Isometry3_[T]
        # - Default constructor
        transform = Isometry3()
        ...
        # Test `type_caster`s.
        if T == float:
            value = eigen_geometry_test_util.create_isometry()
            self.assertTrue(isinstance(value, eigen_geometry.Isometry3))
            eigen_geometry_test_util.check_isometry(value)
        # Operations.
        transform = Isometry3(rotation=R, translation=p)
    ...
    def test_translation(self):
        # Test `type_caster`s.
        value = eigen_geometry_test_util.create_translation()
        self.assertEqual(value.shape, (3,))
        eigen_geometry_test_util.check_translation(value)

    def test_angle_axis(self):
        self.check_types(self.check_angle_axis)

    def check_angle_axis(self, T):
        AngleAxis = eigen_geometry.AngleAxis_[T]

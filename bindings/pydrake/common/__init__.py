...
def _wrap_to_match_input_shape(f):
    # See dosctring for `WrapToMatchInputShape` in `eigen_pybind.h` for more
    # details.
    # N.B. We cannot use `inspect.ismethod` on pybind11 methods currently, nor
    # can we use `inspect.Signature` due to the fact that pybind11's instance
    # method is not insepctable for overloads.
    assert callable(f), f

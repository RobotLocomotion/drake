...
def _enumerate_field_types(obj):
    ...
    raise NotImplementedError(
        f"Schema objects of type {type(obj)} are not yet supported")
...
def _merge_yaml_dict_item_into_target(*, options, name, yaml_value,
    ...
    if nested_optional_type is not None:
        # In the yaml was null, the Python field will be None.
...
def yaml_load_typed(*, schema,
    ...
    If the optional `child_name` is provided, loads data from given-named child
    of the document's root instead of the root itself.

    If set to true, when parsing a Mapping the loader will merge the YAML data
    into the destination, instead of replacing the dict contents entirely. In
    other words, a Mapping field in a schema can have default values that are

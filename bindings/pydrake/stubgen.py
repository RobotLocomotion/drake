...
def _pydrake_modules():
    ...
    # it as a function-local import because don't want `_wrapper_main` to pay
    # the expense of importing this.
...
def _pyi_generated(directory: Path):
    """Returns a list[Path] of all *.pyi files in the given directory."""
    result = []
...
def _copy_pyi(pyi_generated, output_root, pyi_outputs):
    ...
    if missing_pyi:
        raise RuntimeError(
            ...
            "Possibly PYI_FILES should not have listed those items.")
...
def _actual_main():
    # Our arguments are the list of *.pyi outputs wanted from stubgen.bzl. They
    # are probably relative paths, which we'll convert to an absolute root path
    # for the parent of "pydrake" lives, and then a list of relative paths
    # under that.
    output_root = Path(sys.argv[1].rsplit("/pydrake/")[0]).absolute()
    pyi_outputs = sorted([
    ...
    for name in list(native_modules):

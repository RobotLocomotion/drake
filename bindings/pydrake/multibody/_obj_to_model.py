...
<package format="2">
  <name>{name}</name>
  <description>
    Package generated by obj_to_model from OBJ file: {obj_name}.
  </description>
</package>
"""
_VISUAL_TEMPLATE = """
...
def _resolve_package_prefix(package_spec: str, obj_path: str):
    """
    ...
    Args:
        package_spec: Can be one of "none", "auto", or a path to a package.xml
            file.
        obj_path: The path to the obj file being processed. Used if
            package_spec is "auto". A package.xml file in that directory will
            be used or created.

    Returns:
        A string that can be prefixed to the obj file name to be used as the
        mesh URI.
    """
    ...
    if package_spec == "auto":
        ...
        candidate_package_path = os.path.join(obj_dir, "package.xml")
        if os.path.exists(candidate_package_path):
            package_name = _read_package_name(candidate_package_path)
        else:
            # The name of the *package* is the name of the OBJ's directory.
            package_name = os.path.split(obj_dir)[-1]
...
def obj_to_model(obj_path: str, scale: float, model_name: str,
                 mass_spec: ObjMassSpec, frame_pose: ObjFramePose,
                 no_collision: bool, no_visual: bool,
                 encoded_package: str):
    """
    Creates an SDF file based on the geometry defined in a Wavefront OBJ file.
    """
    mesh_G = ReadObjToTriangleSurfaceMesh(filename=obj_path, scale=scale)
    ...
    package_prefix = _resolve_package_prefix(encoded_package, obj_path)
    obj_file_name = os.path.basename(obj_path)
    mesh_uri = package_prefix + obj_file_name

    # Possible model name.
    obj_stem = os.path.splitext(obj_file_name)[0]
    ...
    subs = dict()
    # model_name is None *or* model_name == "" prefers obj_stem.
    subs["name"] = model_name or obj_stem

...
class FormatDriver(object, metaclass=abc.ABCMeta):
    ...
    def format_inertia(self, input_bytes: bytes, inertial_facts: ElementFacts,
                       ...
                       products: list) -> bytes:
        """Return formatted XML bytes for the repaired inertial tag.
        ...
        Args:
            ...
            pose: Pose of the center-of-mass frame relative to the body frame,
                  as a 6-element list: [x, y, z, r, p, y].
...
{d(0)}</inertial>{coda()}""".encode('utf-8')
        ...
        for facts in inertial_facts.children:
            ...
            if facts.name == "mass":
                output += f'<mass value="{mass}"/>{coda()}'.encode('utf-8')
...
{d(0)}</inertia>{coda()}""".encode('utf-8')
            if facts.name == "mass":
                output += f"<mass>{mass}</mass>{coda()}".encode('utf-8')
            if facts.name == "pose":
...
class XmlInertiaMapper:
    ...
    def build_output(self, new_inertias_mapping: dict) -> str:
        """Build and return the complete output text.
...
class InertiaFixer:
    ...
    def fix_inertia(self):
        """Execute the inertia fixing processing and write the output as

...
{d(0)}</inertia>{coda()}"""
            ...
            if facts.name == "pose":
                output += f"<pose>0 0 0 0 0 0</pose>{coda()}"
            index = adjusted_element_end_index(input_text, facts)
        end = adjusted_element_end_index(input_text, inertial_facts)
        output += input_text[index:end]
        return output
...
class InertiaProcessor:
    ...
    def _maybe_fix_inertia(
        ...
        # TODO: shift the summed-up inertia back to Bcm, rotate to zero out
        # products of inertia, and return both inertia matrix and pose.

"""
Utility for converting a Wavefront OBJ file into an SDF file containing a
single model.
...
the OBJ's mesh. Finally, the mesh is (optionally) registered as visual and
collision geometries for the body.
...
assuming the sdf file and OBJ files are in the same directory. There are two
alternatives: create/use a package at the OBJ location or specify a particular
package.
    ...
    Default behavior::
        ...
        When calling
            ...
            obj_to_model path/to/my_models/mesh.obj
    ...
    Specify package::
        ...
        When calling
            ...
            obj_to_model --package path/to/my_package/package.xml \
    ...
    Create/use package at OBJ location::

        When calling

            obj_to_model --package auto path/to/my_models/mesh.obj
        ...
        The package.xml file will not be fully compliant (missing some of the
        minimally required tags -- see
...
Running::
    ...
    From a Drake source build, run this as::
        ...
        bazel run //tools:obj_to_model -- --help

    From a Drake binary release (including pip releases), run this as::

        python3 -m pydrake.multibody.obj_to_model path/to/obj
"""
...
from pydrake.multibody._obj_to_model import (
    obj_to_model as _obj_to_model,
    ObjMassSpec as _ObjMassSpec,
    ObjFramePose as _ObjFramePose,
)
...
def _main():
    ...
    parser.add_argument(
        ...
        help=("Specify the package semantics of the resulting sdf. Options "
              "are: 'none' (the obj will be referenced using a relative path "
              "in the same directory), 'auto' (current directory is treated "
              "as a package root and package.xml will be added if not already "
              "present), or path to a package.xml file (the mesh will be "
              "referenced as being in the root of that package)."))

    parser.add_argument(
        "--no-collision", action="store_true", default=False,
        help="If specified, the mesh will *not* be used as collision geometry")
    parser.add_argument(
        "--no-visual", action="store_true", default=False,
        help="If specified, the mesh will *not* be used as visual geometry")

    parser.add_argument(
        "obj", nargs=1, default=None,
        help="The path to the obj file to process")

    args = parser.parse_args()

    mass_spec = _ObjMassSpec(mass=args.mass, density=args.density)

    frame_pose = _ObjFramePose(at_com=args.origin_at_com,
                               p_GoBo=args.body_origin)

    print(_obj_to_model(obj_path=args.obj[0], scale=args.scale,

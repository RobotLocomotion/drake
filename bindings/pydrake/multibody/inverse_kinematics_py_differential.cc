...
template <typename DerivedIngredient, bool bind_config_ctor = true,
    typename ClassDoc>
PyClassIngredient<DerivedIngredient> BindIngredient(const char* class_name,
    const ClassDoc& cls_doc,
    PyClassDifferentialInverseKinematicsSystem* diff_ik_cls) {
  using Config = typename DerivedIngredient::Config;

  PyClassIngredient<DerivedIngredient> cls(
      *diff_ik_cls, class_name, cls_doc.doc);
  ...
  cls  // BR
      .def("GetConfig", &DerivedIngredient::GetConfig,
          py_rvp::reference_internal, cls_doc.GetConfig.doc)
      .def("SetConfig", &DerivedIngredient::SetConfig, py::arg("config"),
...
void DefineDifferentialIkSystem(py::module m) {
...

  {
    ...
    PyClassIngredient<NestedClass> nested_cls =
        BindIngredient<NestedClass, false>(

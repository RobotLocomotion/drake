...
class MeshModelMaker:
    ...
    def make_model(self):
        ...
        subs["visual"] = \
            _VISUAL_TEMPLATE.format(**subs) if self.visual else ""
    ...
    def _read_package_name(package_path: str):
        ...
        value of the name tag. Returns None if no name tag could be found.
        """
        with open(package_path) as f:
            text = "".join(f.readlines())
        match = re.search("<name>(.+?)</name>", text)
        if not match:
            _logger.error(f"Given package.xml file is not properly formed: "
                          f"'{package_path}'")
            return None

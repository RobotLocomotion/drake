...
class ForceDensityFieldPublic : public ForceDensityField<T> {
 ...
 protected:
  ForceDensityFieldPublic(ForceDensityType density_type)
...
class DelegatedForceDensityField final : public ForceDensityField<T> {
 public:
  DelegatedForceDensityField(std::shared_ptr<ForceDensityField<T>> impl)
...
class PyForceDensityField : public ForceDensityFieldPublic<T> {
 public:
  PyForceDensityField(ForceDensityType density_type)
  ...
  Vector3<T> DoEvaluateAt(const systems::Context<T>& context,
    ...
    PYBIND11_OVERRIDE_PURE(
        Vector3<T>, ForceDensityField<T>, DoEvaluateAt, context, p_WQ);
  }
  ...
  std::unique_ptr<ForceDensityFieldBase<T>> DoClone() const override {
    ...
    if (!override) {
      throw std::logic_error(
          "Derived Python class must override DoClone() and return a clone of "
          "itself.");
    }

    py::object cloned = override();  // Call Python-side DoClone
    auto impl = cloned.cast<std::shared_ptr<ForceDensityField<T>>>();
    if (impl.get() == this) {
      throw std::logic_error(
          "The method DoClone() in Python must return a clone and not itself.");
    }
    return std::make_unique<DelegatedForceDensityField<T>>(std::move(impl));
  }

 ...
 public:
  ...
  Vector3<T> DoEvaluateAt(const systems::Context<T>& context,
    ...
    if (!override) {
      throw std::logic_error(
          "Derived Python class must override DoEvaluateAt().");
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Cannot cast the return value of DoEvaluateAt() to Vector3<T>.");
    }
  }
  ...
  std::unique_ptr<ForceDensityFieldBase<T>> DoClone() const override {
    ...
    try {
      ...
      throw std::runtime_error("DoClone() must return a clone of itself.");
    }
    if (cloned.get() == this) {
      throw std::logic_error("DoClone() must return a clone and not itself.");
    }

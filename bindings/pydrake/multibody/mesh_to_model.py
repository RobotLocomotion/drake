...
Properties of the resulting SDF:
  ...
  - The mesh will be used as both <collision> and <visual> geometries (unless
    you opt out -- see below).
...
units to meters. For example, if your mesh is in centimeters, use 0.01 for the
scale.

Configuring mass::

The body's mass can be configured through one of two ways: specifying the
_density_ of a homogeneous material which completely fills the enclosed volume
of the mesh, or specifying the total _mass_ of the body. If neither is
specified, it is assumed to have the density of water (~1000kg/m³). Only one of
density and mass can be specified.
...
package.
    ...
    Default behavior::
        ...
        When calling
            ...
            mesh_to_model path/to/my_models/mesh.obj path/to/my_models/mesh.sdf
        ...
        Drake parser extension. It is your responsibility to place the mesh in
        the same folder as the sdf.
    ...
    Specify package::
        ...
        When calling
            ...
            mesh_to_model --package=path/to/package.xml \
                  path/to/my_models/mesh.obj path/to/my_models/mesh.sdf
        ...
        is "my_package". Errors in reading the file (or the package name
        from the manifest) will cause the program to fail. The mesh file msut
        live somewhere underneath the package.xml directory.
...
# this transforms the error message from:
#    argument --body-origin: invalid _CommaSeparatedXYZ value: [value]
# to
#    argument --body-origin: invalid comma-separated triple value: [value]
...
def _main():
    ...
    # N.B. Don't print the under-the-hood defaults because the parameter name
    # is the _opposite_ of what is stored internally, inviting confusion.
    parser.add_argument(
        "--no-collision", dest="collision", action="store_false",
        help="If specified, the model will have no collision geometry.")
    parser.add_argument(
        "--no-visual", dest="visual", action="store_false",

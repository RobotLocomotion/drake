...
from pydrake.multibody._mesh_to_model import (
    mesh_to_model as _mesh_to_model,
...
class _MyFormatter(argparse.ArgumentDefaultsHelpFormatter,
    ...
    """
    A formatter that leaves the description formatted as documented above *and*
    prints the default values for variables.
    """
    pass
...
def _main():
    ...
    parser.add_argument(
        "mesh", nargs=1, default=None,
        help="The path to the mesh file to process.")

    args = parser.parse_args()

    mass_spec = _MeshMassSpec(mass=args.mass, density=args.density)

    frame_pose = _MeshFramePose(at_com=args.origin_at_com,
                                p_GoBo=args.body_origin)

    print(_mesh_to_model(mesh_path=args.mesh[0], scale=args.scale,
                         model_name=args.model_name,
                         mass_spec=mass_spec,
                         frame_pose=frame_pose,
                         no_collision=args.no_collision,
                         no_visual=args.no_visual,
                         encoded_package=args.package))

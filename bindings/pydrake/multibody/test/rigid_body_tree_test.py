...
class TestRigidBodyTree(unittest.TestCase):
    def test_kinematics_api(self):
        ...
        tree = RigidBodyTree(FindResourceOrThrow(
            "drake/examples/pendulum/Pendulum.urdf"), floating_base_type=FloatingBaseType.kQuaternion)
        num_q = 8
        num_v = 7
        self.assertEqual(tree.number_of_positions(), num_q)
        self.assertEqual(tree.number_of_velocities(), num_v)

        np.set_printoptions(linewidth=120, formatter={'float_kind':lambda x: "% .2f" % x})
        ...
        J_not_in_terms_of_q_dot, v_indices_not_in_terms_of_qdot = tree.geometricJacobian(kinsol, 0, 2, 0, False)
        ...
        J_in_terms_of_q_dot, v_indices_in_terms_of_qdot = tree.geometricJacobian(kinsol, 0, 2, 0, True)

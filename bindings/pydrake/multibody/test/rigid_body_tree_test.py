...
class TestRigidBodyTree(unittest.TestCase):
    def test_kinematics_api(self):
        ...
        tree = RigidBodyTree(FindResourceOrThrow(os.path.join(
    ...
    def test_dynamics_api(self):
        urdf_path = FindResourceOrThrow(os.path.join(
            "drake", "examples", "pendulum", "Pendulum.urdf"))
        tree = RigidBodyTree(
    ...
    def test_atlas_parsing(self):
        ...
        pm = PackageMap()
        model = FindResourceOrThrow(os.path.join(
    ...
    def test_joint_limits(self):
        ...
        # Check extremal values against values taken from URDF-file.
        self.assertAlmostEqual(np.min(tree.joint_limit_min), -3.011)
        self.assertAlmostEqual(np.max(tree.joint_limit_max), 3.14159)

...
class TestDiffIkSystems(unittest.TestCase):
    """Tests the DifferentialInverseKinematicsSystem API including all nested
    classes.
    """
    ...
    def _config_matches_params(self, config, params):
        ...
        for key in params.keys():
            ...
            if isinstance(value, dict):
                ...
                # cartesian mask and skipping it; they're a pain to
                # compare. If all other attributes pass, we'll assume the
                # masks worked as well.
                pass
            elif isinstance(value, np.ndarray):
                np.testing.assert_array_equal(value, params[key])
                # self.assertTrue((value == params[key]).all())
            else:
                self.assertEqual(getattr(config, key), params[key])

    def _test_ingredient_config(self, dut_type, params):
    ...
    def _test_common_ingredient(self, dut_type, params1, params2, ctor=None):
        """This tests ingredients which are fully specified by a single, nested
        Config struct.
        """
        config = self._test_ingredient_config(dut_type.Config, params1)
    ...
    def test_least_squares_cost(self):
        ...
        self._test_common_ingredient(Ingredient, params,
    ...
    def test_collision_constraint(self):
        ...
        dut = self._test_common_ingredient(
    ...
    def test_diff_ik_system(self):
        """Build a simple Diff IK system and evaluate it."""
        ...
        command = dut.get_output_port_commanded_velocity().Eval(context)
        ...
        # Note: setting poses here in addition to the velocities set earlier
        # will cause an evaluation error.
        desired_poses = BusValue()
        desired_poses.Set("robot::ball", Value(RigidTransform([1, 2, 0])))
        dut.get_input_port_desired_cartesian_poses().FixValue(context,
                                                              desired_poses)
        with self.assertRaisesRegex(
                SystemExit,
                r".*condition 'has_cartesian_positions_input != "
                r"has_cartesian_velocities_input' failed.*") as cm:
    ...
    def test_diff_ik_controller(self):
        ...
        dut.SetDefaultState(context=context, state=state)
        dut.SetRandomState(context=context, state=state,

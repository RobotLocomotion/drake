...
class TestModelMaker(unittest.TestCase):
    ...
    def test_simple_valid_invocation(self):
        ...
        self.assertRegex(_file_contents(sdf_result), "<?xml")
    ...
    def test_scale(self):
        ...
        self.assertRegex('\n'.join([r.getMessage() for r in cm.records]),
                         "2.0 x 2.0 x 2.0.")
        # Density is 1.0.
        self.assertRegex(_file_contents(sdf_result), "<mass>8.0</mass>")
        self.assertRegex(_file_contents(sdf_result), "<scale>1 1 1</scale>")
        ...
        self.assertRegex('\n'.join([r.getMessage() for r in cm.records]),
                         "4.0 x 4.0 x 4.0")
        self.assertRegex(_file_contents(sdf_result), "<mass>64.0</mass>")
        self.assertRegex(_file_contents(sdf_result), "<scale>2 2 2</scale>")
    ...
    def test_mass(self):
        ...
        with self.assertRaisesRegex(ValueError, ".*Neither.*provided.*"):
            MeshModelMaker(mesh_path='', output_path='', density=None,
                           mass=None)
    ...
    def test_body_origin(self):
        ...
        with self.assertRaisesRegex(ValueError,
            ...
            MeshModelMaker(mesh_path='', output_path='', at_com=True,
                           p_GoBo=(1, 2, 3))

    def test_package_none(self):
        sdf_result = self._temp_dir / "with_none_package.sdf"
        ...
        dut.make_model()
        _parse_model_no_throw(sdf_result)
        self.assertRegex(_file_contents(sdf_result), "<uri>box.obj</uri>")
    ...
    def test_package_auto(self):
        ...
        sdf_result = package_dir / "with_auto_package.sdf"
    ...
    def test_package_errors(self):
        sdf_result = self._temp_dir / "bad_package.sdf"
...
class TestMeshToModelProcess(unittest.TestCase):
    ...
    to ModelMeshMaker and compare the files.
    """
    @classmethod
    def setUpClass(cls):
        cls._temp_dir = Path(temp_directory())
    ...
    def test_non_default_parameters(self):
        ...
        # passed through provides sufficient evidence.

        reference_sdf = self._temp_dir / "default_ref.sdf"
        maker = MeshModelMaker(mesh_path=self._obj_path,
                               output_path=reference_sdf,
                               scale=1.5).make_model()

        dut_sdf = self._temp_dir / "default_dut.sdf"
        subprocess.check_call(
            [self._dut, '--scale', '1.5', self._obj_path, dut_sdf])

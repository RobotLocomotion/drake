...
class TestStaticEquilibriumProblem(unittest.TestCase):
    @unittest.skipUnless(
        mp.SnoptSolver().available() and mp.SnoptSolver().enabled(),
        "Requires SNOPT",
    )
    def test_one_box(self):
        ...
        dut.get_mutable_prog().SetInitialGuess(box_xyz, np.array([0, 0, 0.3]))
        ...
        self.assertAlmostEqual(np.linalg.norm(box_quat_sol), 1.0)
...
class TestSpatialVelocityConstraint(unittest.TestCase):
    def test(self):
        ...
        np.testing.assert_array_almost_equal(
            dut.lower_bound(), [1, 2, 3, 0.04, np.cos(np.pi / 4)]
        )
        np.testing.assert_array_almost_equal(
...
class TestToppra(unittest.TestCase):
    ...
    def test_all_constraints(self):
        ...
        plant.WeldFrames(
            plant.world_frame(), plant.GetFrameByName("iiwa_link_0", iiwa_id)
        )
        plant.Finalize()
        ...
        knots = np.array(
            [
                ...
                [-0.8, -0.6, 0.5, 0.1, -0.5, -0.1, -1],
            ]
        ).T
        path = PiecewisePolynomial.CubicWithContinuousSecondDerivatives(
            [0.0, 1.0], knots, np.zeros(7), np.zeros(7)
        )
        ...
        constraint = toppra.AddJointVelocityLimit(
        ...
        constraint = toppra.AddFrameVelocityLimit(
        ...
        constraint = toppra.AddFrameTranslationalSpeedLimit(
            constraint_frame=frame, upper_limit=1.0
        )
        self.assertIsInstance(constraint, mp.Binding[mp.BoundingBoxConstraint])
        ...
        upper_traj = PiecewisePolynomial.FirstOrderHold(breaks, np.ones((6, 2)))
        lower_traj = PiecewisePolynomial.FirstOrderHold(
            breaks, -np.ones((6, 2))
        )
        backward_con, forward_con = toppra.AddFrameAccelerationLimit(
            constraint_frame=frame,
            lower_limit=lower_traj,
    ...
    def test_solve(self):
        ...
        path = PiecewisePolynomial.CubicWithContinuousSecondDerivatives(
            [0.0, 1.0], np.array([[0.0, 1.0]]), [0.0], [0.0]
        )
        gridpoints = Toppra.CalcGridPoints(path, CalcGridPointsOptions())
        toppra = Toppra(path=path, plant=plant, gridpoints=gridpoints)

        toppra.AddJointAccelerationLimit([-1.0], [1.0])

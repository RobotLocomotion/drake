...
class TestSpatialVelocityConstraint(unittest.TestCase):
    def test(self):
        ...
        w_AQ_bounds = SpatialVelocityConstraint.AngularVelocityBounds()
        w_AQ_bounds.magnitude_lower = 0.2
        w_AQ_bounds.magnitude_upper = 0.4
        w_AQ_bounds.nominal_direction = [1, 2, 3]
        w_AQ_bounds.theta_bound = np.pi / 4
        dut = SpatialVelocityConstraint(
            ...
            w_AQ_bounds=w_AQ_bounds)
        self.assertIsInstance(dut, mp.Constraint)
        np.testing.assert_equal(dut.lower_bound(), [1, 2, 3, 0.2, 0])
        np.testing.assert_equal(dut.upper_bound(), [4, 5, 6, 0.4, np.pi / 4])

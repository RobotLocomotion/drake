...
class TestPlant(unittest.TestCase):
    ...
    def test_collision_filter(self):
        ...
        # TODO(SeanCurtis-TRI): When GeometrySet's full API is bound and the
        # GeometrySet can be arbitrarily constructed, move the tests of these
        # functions into geometry_test.py.
        diagram = builder_f.Build()
        context = diagram.CreateDefaultContext()
        sg_context = diagram.GetMutableSubsystemContext(scene_graph_f, context)

        sg = scene_graph_f
        for func in [sg.ExcludeCollisionsWithin,
                     lambda x: sg.ExcludeCollisionsWithin(sg_context, x),
                     lambda x: sg.ExcludeCollisionsBetween(x, x),
                     lambda x: sg.ExcludeCollisionsBetween(sg_context, x, x)]:
            func(geometries)

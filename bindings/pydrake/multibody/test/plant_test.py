...
class TestPlant(unittest.TestCase):
    ...
    def test_model_instance_state_access(self, T):
        ...
        plant_f.WeldFrames(
            frame_on_parent=plant_f.world_frame(),
            frame_on_child=plant_f.GetFrameByName("iiwa_link_0", iiwa_model))
        # Perform the second weld without named arguments to ensure that the
        # non-deprecated binding gets invoked.
        plant_f.WeldFrames(
    ...
    def test_model_instance_state_access_by_array(self, T):
        ...
        plant_f.WeldFrames(
            ...
            frame_on_child=plant_f.GetFrameByName("iiwa_link_0", iiwa_model))
        plant_f.WeldFrames(
            frame_on_parent=plant_f.GetFrameByName("iiwa_link_7", iiwa_model),
            frame_on_child=plant_f.GetFrameByName("body", gripper_model),
            X_PC=X_EeGripper)
        plant_f.Finalize()
        plant = to_type(plant_f, T)

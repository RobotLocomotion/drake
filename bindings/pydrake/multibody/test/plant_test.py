...
class TestPlant(unittest.TestCase):
    ...
    def test_collision_filter_between(self):
        builder_f = DiagramBuilder_[float]()
        # N.B. `Parser` only supports `MultibodyPlant_[float]`.
        ...
        set_a = plant_f.CollectRegisteredGeometries([body_a])

        body_b = plant_f.GetBodyByName("body2")
        set_b = plant_f.CollectRegisteredGeometries([body_b])
        scene_graph_f.ExcludeCollisionsBetween(set_a, set_b)

        self.assertIsInstance(set_a, GeometrySet)
        self.assertIsInstance(set_b, GeometrySet)

    def test_collision_filter_within(self):
        builder_f = DiagramBuilder_[float]()
        # N.B. `Parser` only supports `MultibodyPlant_[float]`.
        plant_f, scene_graph_f = AddMultibodyPlantSceneGraph(builder_f, 0.0)
        parser = Parser(plant=plant_f, scene_graph=scene_graph_f)
        ...
        scene_graph_f.ExcludeCollisionsWithin(geometries)

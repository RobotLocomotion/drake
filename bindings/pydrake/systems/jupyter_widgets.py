...
class PoseSliders(LeafSystem):
    ...
    Visible.__new__.__defaults__ = [True] * 6
    MinRange = namedtuple("Visible", ("roll", "pitch", "yaw", "x", "y", "z"))
    MinRange.__new__.__defaults__ = [-np.pi, -np.pi, -np.pi, -1.0, -1.0, -1.0]
    MaxRange = namedtuple("Visible", ("roll", "pitch", "yaw", "x", "y", "z"))
    MaxRange.__new__.__defaults__ = [np.pi, np.pi, np.pi, 1.0, 1.0, 1.0]
    Value = namedtuple("Visible", ("roll", "pitch", "yaw", "x", "y", "z"))
    Value.__new__.__defaults__ = [0.0] * 6
    ...
    def __init__(self, visible=Visible(), min_range=MinRange(),        
        ...
        self.roll = FloatSlider(min=min_range.roll,
                                max=max_range.roll,
                                value=value.roll,
                                step=0.01,
        ...
        self.pitch = FloatSlider(min=min_range.pitch,
                                 max=max_range.pitch,
                                 value=value.pitch,
                                 step=0.01,
                                 continuous_update=True,
                                 description="pitch",
                                 layout=Layout(width='90%'))
        self.yaw = FloatSlider(min=min_range.yaw,
                               max=max_range.yaw,
                               value=value.yaw,
                               step=0.01,
                               continuous_update=True,
                               description="yaw",
                               layout=Layout(width='90%'))
        self.x = FloatSlider(min=min_range.x,
                             max=max_range.x,
                             value=value.x,
                             step=0.01,
                             continuous_update=True,
                             ...
                             orient='horizontal',
                             layout=Layout(width='90%'))
        self.y = FloatSlider(min=min_range.y,
                             max=max_range.y,
                             value=value.y,
                             step=0.01,
                             continuous_update=True,
                             description="y",
                             layout=Layout(width='90%'))
        self.z = FloatSlider(min=min_range.z,
                             max=max_range.z,
                             value=value.z,
                             step=0.01,
                             continuous_update=True,

...
class PlanarSceneGraphVisualizer(PyPlotVisualizer):
    ...
    def __init__(self,
        ...
        self._T_VW = RigidTransform(T_VW)
        ...
        for full_name in self._view_patches.keys():
            view_patches, view_colors = self._get_view_patches(full_name,
                                                               X_WB_initial)
            self._body_fill_dict[full_name] = []
            for patch_W, color in zip(view_patches, view_colors):
    ...
    def _build_view_patches(self, use_random_colors):
        ...
        for i in range(load_robot_msg.num_links):
            ...
            for j in range(link.num_geom):
                ...
                # Compute pose in world.
                patch_W = X_WG @ patch_G

                # Close path if not closed.
                if (patch_W[:, -1] != patch_W[:, 0]).any():
                    patch_W = np.hstack((patch_W, patch_W[:, 0][np.newaxis].T))

                this_body_patches.append(patch_W)
    ...
    def _get_view_patches(self, full_name, X_WB):
        ...
        """
        patches_W = [X_WB @ patch for patch in self._view_patches[full_name]]
        colors = self._view_patch_colors[full_name]
        return (patches_W, colors)

    def draw(self, context):
        """Overrides base with the implementation."""

        pose_bundle = self._pose_bundle_port.Eval(context)
        T_VW_matrix = self._T_VW.matrix()
        view_dir = np.cross(T_VW_matrix[0, :3], T_VW_matrix[1, :3])
        for frame_i in range(pose_bundle.get_num_poses()):
            ...
            pose = RigidTransform(pose_bundle.get_pose(frame_i))
            view_patches, _ = self._get_view_patches(full_name, pose)
            for i, patch_W in enumerate(view_patches):
                ...
                body_fill.zorder = pose.translation() @ view_dir

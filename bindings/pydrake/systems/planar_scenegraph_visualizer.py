...
class PlanarSceneGraphVisualizer(PyPlotVisualizer):
    ...
    def __init__(self,
        ...
        for full_name in self._view_patches.keys():
            view_patches, view_colors = self._get_view_patches(full_name, T_I)
            ...
            for patch, color in zip(view_patches, view_colors):
                ...
                # vertex set.
                patch_proj = np.dot(self._T_VW, patch)
    ...
    def _build_view_patches(self, use_random_colors):
        ...
        for i in range(load_robot_msg.num_links):
            ...
            for j in range(link.num_geom):
                ...
                patch = np.dot(T_LE.GetAsMatrix4(), patch)
        ...
        for frame_i in range(pose_bundle.get_num_poses()):
            ...
            for i, patch in enumerate(view_patches):
                ...
                if patch_proj.shape[1] > 3:
                    hull = spatial.ConvexHull(patch_proj.T)
                    patch_proj = np.vstack(
                        [patch_proj[:, v] for v in hull.vertices]).T
                ...
                patch_proj = np.pad(
                    ...
                    mode="edge")
                body_fill.get_path().vertices[:, :] = patch_proj.T
                body_fill.zorder = np.dot(pose[:3, 3], view_dir)

...
PYBIND11_MODULE(rendering, m) {
  ...
  frame_velocity
    .def(py::init())
    .def("get_velocity",
         [](const FrameVelocity<T>* self) {
           multibody::SpatialVelocity<T> result = self->get_velocity();
           return std::make_pair(result.rotational(), result.translational());
         })
    .def("set_velocity",
         [](FrameVelocity<T>* self, const Vector3<T>& w, const Vector3<T>& v) {
           self->set_velocity(multibody::SpatialVelocity<T>(w, v));
         }, py::arg("w"), py::arg("v"));

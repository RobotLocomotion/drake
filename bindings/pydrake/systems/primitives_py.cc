...
PYBIND11_MODULE(primitives, m) {
  ...
  auto bind_common_scalar_types = [&m, &doc](auto dummy) {
    ...
    DefineTemplateClassWithDefault<MultilayerPerceptron<T>, LeafSystem<T>>(m,
        ...
        .def("Backpropagation",
            WrapCallbacks([](const MultilayerPerceptron<T>* self,
                              const Context<T>& context,
                              const Eigen::Ref<const Eigen::MatrixXd>& X,
                              std::function<double(
                                  const Eigen::Ref<const Eigen::MatrixXd>& Y,
                                  Eigen::Ref<Eigen::MatrixXd> dloss_dY)>
                                  loss,
                              Eigen::Ref<Eigen::VectorXd> dloss_dparams) {
              auto new_loss = [loss](const Eigen::Ref<const Eigen::MatrixXd>& Y,
                                  EigenPtr<Eigen::MatrixXd> dloss_dY) {
                return loss(Y, *dloss_dY);
              };
              self->Backpropagation(context, X, new_loss, &dloss_dparams);
            }),
        ...
        .def("BackpropagationMeanSquaredError",
            WrapCallbacks(
                [](const MultilayerPerceptron<T>* self,
                    ...
                    Eigen::Ref<Eigen::VectorXd> dloss_dparams) {
                  self->BackpropagationMeanSquaredError(

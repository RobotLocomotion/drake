...
PYBIND11_MODULE(lcm, m) {
...

  {
    ...
    // interface will call the trampolin implementation methods above.
    cls  // BR
  ...
  // LCM types should be bound on an as-needed basis, in the module that uses
  // those LCM types. If one LCM type is used in two disparate modules, then
  // the definition should be hoisted this module.
  // TODO(eric.cousineau): Consider just auto-binding all LCM types.

...
PYBIND11_MODULE(sensors, m) {
...

  {
    ...
    config_cls  // BR
        ...
        .def("focal_y", &CameraConfig::focal_x, config_cls_doc.focal_x.doc)
    ...
    // N.B. We can't use serialization for attributes or __repr__ because a
    // default-constructed FovDegrees is invalid and throws during
    // serialization.
    ...
    fov_class  // BR
        ...
        .def_readwrite("x", &CameraConfig::FovDegrees::x)
        .def_readwrite("y", &CameraConfig::FovDegrees::y)
        .def("__repr__", [](const CameraConfig::FovDegrees& self) {
          return py::str("CameraConfig.FovDegrees(x={}, y={})")
              .format(self.x, self.y);
        });
    DefCopyAndDeepCopy(&fov_class);

    // N.B. We can't use serialization for attributes or __repr__ because a
    // default-constructed FocalLength is invalid and throws during
    // serialization.
    ...
    focal_class  // BR
        ...
        .def_readwrite("x", &CameraConfig::FocalLength::x)
        .def_readwrite("y", &CameraConfig::FocalLength::y)
        .def("__repr__", [](const CameraConfig::FocalLength& self) {
          return py::str("CameraConfig.FocalLength(x={}, y={})")
              .format(self.x, self.y);
        });

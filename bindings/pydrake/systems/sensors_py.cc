...
PYBIND11_MODULE(sensors, m) {
...

  {
    ...
    auto instantiation_visitor = [&](auto param) {
      ...
      auto check_coord = [](const ImageT* self, int x, int y, int c) {
        // Since Image<>::at(...) uses DRAKE_ASSERT for performance reasons,
        // rewrite the checks here using DRAKE_DEMAND so that it will not
        // segfault in Python.
        DRAKE_DEMAND(x >= 0 && x < self->width());
        DRAKE_DEMAND(y >= 0 && y < self->height());
        DRAKE_DEMAND(c >= 0 && c < int{ImageTraitsT::kNumChannels});
      };
      ...
      image
          ...
          .def("at", [=](const ImageT* self, int x, int y, int c) {
                check_coord(self, x, y, c);
                return self->at(x, y)[c];
              })
          // Non-C++ properties. Make them Pythonic.
          .def("set", [=](ImageT* self, int x, int y, int c, T value) {
                check_coord(self, x, y, c);
                self->at(x, y)[c] = value;
              })
          .def_property_readonly("shape", get_shape)
          .def_property_readonly("data", get_data, py_iref)
          .def_property_readonly("mutable_data", get_mutable_data, py_iref);
      // Constants.

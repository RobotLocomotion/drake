...
PYBIND11_MODULE(sensors, m) {
...

  {
    ...
    auto instantiation_visitor = [&](auto param) {
      ...
      image
          .def(py::init<int, int>(), doc.Image.doc)
          .def(py::init<int, int, T>(), doc.Image.doc_2)
  ...
  py::class_<CameraInfo>(m, "CameraInfo", doc.CameraInfo.doc)
    .def(py::init<int, int, double>(),
         ...
         py::arg("fov_y"), doc.CameraInfo.ctor.doc)
    .def(py::init<int, int, double, double, double, double>(),
         py::arg("width"), py::arg("height"),
         py::arg("focal_x"), py::arg("focal_y"),
         py::arg("center_x"), py::arg("center_y"), doc.CameraInfo.ctor.doc_2)
  ...
  rgbd_camera
    .def(
      ...
      py::keep_alive<1, 3>(), doc.RgbdCamera.ctor.doc)

...
PYBIND11_MODULE(sensors, m) {
  ...
  {
    // For nested structs, we need to declare the outer struct with no bindings,
    // then bind its inner structs, then bind the outer struct. Otherwise, the
    // attribute bindings on the outer struct will refer to inner structs that

...
void DefineFrameworkPySemantics(py::module m) {
  ...
  abstract_values
    .def(py::init<>(), doc.AbstractValues.ctor.doc)
    .def(py::init<AbstractValuePtrList>(), doc.AbstractValues.ctor.doc)
  ...
  auto bind_common_scalar_types = [m](auto dummy) {
    ...
    DefineTemplateClassWithDefault<Context<T>>(
        m, "Context", GetPyParam<T>(), doc_lambda.Context.doc)
      .def("get_num_input_ports", &Context<T>::get_num_input_ports,
        doc_lambda.ContextBase.get_num_input_ports.doc)
      .def("FixInputPort",
           py::overload_cast<int, const BasicVector<T>&>(
               &Context<T>::FixInputPort),
           py::arg("index"), py::arg("vec"),
           py_reference_internal, doc_lambda.Context.FixInputPort.doc)
      .def("FixInputPort",
           ...
           py::keep_alive<3, 1>(), doc_lambda.Context.FixInputPort.doc_2)
      .def("FixInputPort",
           py::overload_cast<int, const Eigen::Ref<const VectorX<T>>&>(
               &Context<T>::FixInputPort),
           py::arg("index"), py::arg("data"),
           py_reference_internal, doc_lambda.Context.FixInputPort.doc_3)
      .def("get_time", &Context<T>::get_time, doc_lambda.Context.get_time.doc)
      .def("set_time", &Context<T>::set_time, doc_lambda.Context.set_time.doc)
      .def("set_accuracy", &Context<T>::set_accuracy,
           doc_lambda.Context.set_accuracy.doc)
      .def("get_accuracy", &Context<T>::get_accuracy,
           doc_lambda.Context.get_accuracy.doc)
      .def("Clone", &Context<T>::Clone, doc_lambda.Context.Clone.doc)
    ...
    DefineTemplateClassWithDefault<DiagramBuilder<T>>(
      ...
      .def(
          ...
          py::keep_alive<2, 1>(), doc_lambda.DiagramBuilder.AddSystem.doc)
      .def("Connect",
           py::overload_cast<
               const OutputPort<T>&, const InputPort<T>&>(
               &DiagramBuilder<T>::Connect),
           doc_lambda.DiagramBuilder.Connect.doc)
      .def("ExportInput", &DiagramBuilder<T>::ExportInput, py::arg("input"),
           py::arg("name") = kUseDefaultName, py_reference_internal,
           doc_lambda.DiagramBuilder.ExportInput.doc)
      .def("ExportOutput", &DiagramBuilder<T>::ExportOutput, py::arg("output"),
           py::arg("name") = kUseDefaultName, py_reference_internal,
           doc_lambda.DiagramBuilder.ExportOutput.doc)
      .def("Build", &DiagramBuilder<T>::Build,
           // Keep alive, transitive: `return` keeps `self` alive.
           py::keep_alive<1, 0>(), doc_lambda.DiagramBuilder.Build.doc)
      .def("BuildInto", &DiagramBuilder<T>::BuildInto,
           // Keep alive, transitive: `Diagram` keeps `self` alive.
           py::keep_alive<2, 1>(), doc_lambda.DiagramBuilder.BuildInto.doc);

    DefineTemplateClassWithDefault<OutputPort<T>>(
        m, "OutputPort", GetPyParam<T>(), doc_lambda.OutputPort.doc)
      .def("size", &OutputPort<T>::size, doc_lambda.OutputPortBase.size.doc)
      .def("get_index", &OutputPort<T>::get_index,
           doc_lambda.OutputPortBase.get_index.doc);

    auto system_output = DefineTemplateClassWithDefault<SystemOutput<T>>(
        m, "SystemOutput", GetPyParam<T>(), doc_lambda.SystemOutput.doc);
    system_output
      .def("get_num_ports", &SystemOutput<T>::get_num_ports,
           doc_lambda.SystemOutput.get_num_ports.doc)
      .def("get_data", &SystemOutput<T>::get_data,
           py_reference_internal, doc_lambda.SystemOutput.get_data.doc)
      .def("get_vector_data", &SystemOutput<T>::get_vector_data,
           py_reference_internal, doc_lambda.SystemOutput.get_vector_data.doc);

    DefineTemplateClassWithDefault<InputPort<T>>(
        m, "InputPort", GetPyParam<T>(), doc_lambda.InputPort.doc)
      .def("size", &InputPort<T>::size, doc_lambda.InputPortBase.size.doc)
      .def("get_data_type", &InputPort<T>::get_data_type,
           doc_lambda.InputPortBase.get_data_type.doc)
      .def("get_index", &InputPort<T>::get_index,
           doc_lambda.InputPortBase.get_index.doc);
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
  ...
  #pragma GCC diagnostic pop  // pop -Wdeprecated-declarations
    ...
    auto parameters = DefineTemplateClassWithDefault<Parameters<T>>(
        m, "Parameters", GetPyParam<T>(), doc_lambda.Parameters.doc);
    DefClone(&parameters);
    using BasicVectorPtrList = vector<unique_ptr<BasicVector<T>>>;
    parameters
      .def(py::init<>(), doc_lambda.Parameters.ctor.doc)

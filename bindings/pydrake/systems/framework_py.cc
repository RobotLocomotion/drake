...
PYBIND11_MODULE(framework, m) {
  ...
  // alive).
  // - `ownership` implies that one argument is owned directly by one of the
  // arguments (which includes `self`).
  // - `reference` implies a reference that is lifetime-sensitive (not
  // ownership).
  // - `transitive` implies a transfer of ownership of owned objects from one
  // container to another. (e.g. transfering all `System`s from `DiagramBuilder`
  // to `Diagram` when calling `DiagramBuilder.Build()`.)
  // TODO(eric.cousineau): Move these notes to `pydrake/pydrake_pybind.h`, to
  // be used with other utilities.
  ...
  py::class_<DiagramBuilder<T>>(m, "DiagramBuilder")
    ...
    .def("BuildInto", &DiagramBuilder<T>::BuildInto,
         // Keep alive, transitive: `System` keeps `self` alive.
         py::keep_alive<1, 2>());

...
class TestAnalysis(unittest.TestCase):
    ...
    def test_batch_eval(self, T):
        ...
        dt_contexts = []
        for i in range(2):
            dt_contexts.append(dt_system.CreateDefaultContext())
        ...
        np.testing.assert_arrays_equal(A @ states + B @ inputs, next_state)

        ct_system = LinearSystem_[T](A, B)
        ct_contexts = []
        for i in range(2):
            ct_contexts.append(ct_system.CreateDefaultContext())
        derivatives = BatchEvalTimeDerivatives(
            system=ct_system,
            context_pool=ct_contexts,
            times=times,
            states=states,
            inputs=inputs,
        ...
        np.testing.assert_arrays_equal(A @ states + B @ inputs, derivatives)

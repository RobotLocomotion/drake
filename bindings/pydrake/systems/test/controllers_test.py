...
class TestControllers(unittest.TestCase):
    ...
    def test_inverse_dynamics(self):
        ...
        def test_torque_example(controller, q, v, v_dot_desired=None):
            if (controller.is_pure_gravity_compensation()):
                v_dot_desired = np.zeros(Nv)
            ...
            print(expected_torque)
            print(output.get_vector_data(0).CopyToVector())
            print(q)
            print(v)
            print(v_dot_desired)
            self.assertTrue(
        ...
        v = np.zeros(Nv)
        test_torque_example(controller, q, v)
        ...
        test_torque_example(controller, q, v, v_dot_desired)

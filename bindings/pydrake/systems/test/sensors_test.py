...
class TestSensors(unittest.TestCase):
    ...
    def test_camera_config(self):
        mut.CameraConfig()
        config = mut.CameraConfig(width=124,
                                  focal=mut.CameraConfig.FocalLength(x=10))
        self.assertEqual(config.width, 124)
        self.assertIn("width", repr(config))
        copy.copy(config)
        self.assertEqual(config.focal_x(), 10)
        self.assertEqual(config.focal_y(), 10)
        config.principal_point()

        # Keyword construction shows that fields have been bound.

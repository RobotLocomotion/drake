...
class TestGeneral(unittest.TestCase):
    ...
    def test_vector_log_sink(self, T):
        ...
        loggers = []
        if T == float:
            loggers.append(builder.AddSystem(VectorLogSink(kSize)))
            loggers.append(builder.AddSystem(VectorLogSink(input_size=kSize)))
            loggers.append(builder.AddSystem(VectorLogSink(kSize, 0.125)))
            loggers.append(builder.AddSystem(
                VectorLogSink(input_size=kSize, publish_period=0.125)))
            loggers.append(builder.AddSystem(
                VectorLogSink(kSize, {TriggerType.kForced})))
            loggers.append(builder.AddSystem(
                VectorLogSink(input_size=kSize,
                              publish_triggers={TriggerType.kForced})))
            loggers.append(builder.AddSystem(
                VectorLogSink(kSize, {TriggerType.kPeriodic}, 0.125)))
            loggers.append(builder.AddSystem(
                VectorLogSink(input_size=kSize,
                              publish_triggers={TriggerType.kPeriodic},
                              publish_period=0.125)))

        loggers.append(builder.AddSystem(VectorLogSink_[T](kSize)))
        loggers.append(builder.AddSystem(VectorLogSink_[T](input_size=kSize)))
        loggers.append(builder.AddSystem(VectorLogSink_[T](kSize, 0.125)))
        loggers.append(builder.AddSystem(
            VectorLogSink_[T](input_size=kSize, publish_period=0.125)))
        loggers.append(builder.AddSystem(
            VectorLogSink_[T](kSize, {TriggerType.kForced})))
        loggers.append(builder.AddSystem(
            VectorLogSink_[T](input_size=kSize,
                              publish_triggers={TriggerType.kForced})))
        loggers.append(builder.AddSystem(
            VectorLogSink_[T](kSize, {TriggerType.kPeriodic}, 0.125)))
        loggers.append(builder.AddSystem(
            VectorLogSink_[T](input_size=kSize,
                              publish_triggers={TriggerType.kPeriodic},
                              publish_period=0.125)))
        ...
        self.assertTrue(
            all(x[0].GetLog(x[1]) == x[0].GetMutableLog(x[1])
                for x in loggers_and_contexts))
        # GetLog and FindLog find the same object, given the proper contexts.
        self.assertTrue(
            all(x[0].GetLog(x[1]) == x[0].FindLog(context)
                for x in loggers_and_contexts))

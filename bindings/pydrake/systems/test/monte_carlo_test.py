...
class TestMonteCarlo(unittest.TestCase):
    ...
    def test_pendulum(self):
        ...
        theta_expression = Variable(
            name="theta",
            type=Variable.Type.RANDOM_GAUSSIAN)*0.25 + upright_theta
        ...
        torque_limiter = builder.AddSystem(
            Saturation(min_value=np.array([-1.0]),
                       max_value=np.array([1.0])))

        builder.Connect(controller.get_output_port(0), torque_limiter.get_input_port(0))
        builder.Connect(torque_limiter.get_output_port(0), pendulum.get_actuation_input_port())
        ...
        def calc_output(system, context):
            ...
            return (state.GetAtIndex(0) - upright_theta + np.pi) % (2*np.pi) - np.pi

        results = MonteCarloSimulation(
            make_simulator=make_simulator, output=calc_output,
            final_time=1.0, num_samples=100, generator=RandomGenerator())

        num_passing = sum([abs(res.output) < 0.05 for res in results])
        passing_ratio = float(num_passing) / len(results)
        # Empirically...
        self.assertTrue(0.3 <= passing_ratio <= 0.8)
        print("Fraction of cases that converged: %f" % passing_ratio)

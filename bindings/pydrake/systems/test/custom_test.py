...
class CustomPortsLifetimeHazardSystem(LeafSystem):
    ...
    def __init__(self):
        ...
        # results in self.
        stuff = set()
        stuff.add(self.DeclareInputPort(kUseDefaultName,
                                        PortDataType.kVectorValued, 1))
        stuff.add(self.DeclareAbstractInputPort("Ain", Value[str]()))
        stuff.add(self.DeclareAbstractOutputPort("Aout",
                                                 lambda: Value(""),
                                                 lambda: Value("")))
        stuff.add(self.DeclareVectorInputPort("Vin", 2))
        stuff.add(self.DeclareVectorInputPort("Vin.rand", 2, random_type=None))
        stuff.add(self.DeclareVectorOutputPort("Vout.size", 2, lambda: [1, 2]))
        stuff.add(self.DeclareVectorOutputPort("Vout.model", BasicVector(2),
                                               lambda: [1, 2]))
        stuff.add(self.DeclareStateOutputPort(kUseDefaultName,

...
void DefineFrameworkPyValues(py::module m) {
  ...
  auto bind_common_scalar_types = [m](auto dummy) {
    ...
    DefineTemplateClassWithDefault<VectorBase<T>>(
      m, "VectorBase", GetPyParam<T>(), doc_lambda.VectorBase.doc)
      .def("CopyToVector", &VectorBase<T>::CopyToVector,
           doc_lambda.VectorBase.CopyToVector.doc)
      .def("SetAtIndex", &VectorBase<T>::SetAtIndex,
           doc_lambda.VectorBase.SetAtIndex.doc)
      .def("SetFromVector", &VectorBase<T>::SetFromVector,
           doc_lambda.VectorBase.SetFromVector.doc)
      .def("size", &VectorBase<T>::size, doc_lambda.VectorBase.size.doc);

    // TODO(eric.cousineau): Make a helper function for the Eigen::Ref<>
    // patterns.
    auto basic_vector =
        DefineTemplateClassWithDefault<BasicVector<T>, VectorBase<T>>(
            m, "BasicVector", GetPyParam<T>(), doc_lambda.BasicVector.doc);
    ...
    basic_vector
      ...
      // permitted).
      .def(py::init<VectorX<T>>(), doc_lambda.BasicVector.ctor.doc)
      .def(py::init<int>(), doc_lambda.BasicVector.ctor.doc_5)
      .def("get_value",
          [](const BasicVector<T>* self) -> Eigen::Ref<const VectorX<T>> {
            return self->get_value();
          }, py_reference_internal, doc_lambda.BasicVector.get_value.doc)
      ...
      .def("get_mutable_value",
          [](BasicVector<T>* self) -> Eigen::Ref<VectorX<T>> {
           ...
           doc_lambda.BasicVector.get_mutable_value.doc)
       .def("GetAtIndex", [](BasicVector<T>* self, int index) -> T& {
         return self->GetAtIndex(index);
       }, py_reference_internal, doc_lambda.BasicVector.GetAtIndex.doc);

    DefineTemplateClassWithDefault<Supervector<T>, VectorBase<T>>(
        m, "Supervector", GetPyParam<T>(), doc_lambda.Supervector.doc);

    DefineTemplateClassWithDefault<Subvector<T>, VectorBase<T>>(
        m, "Subvector", GetPyParam<T>(), doc_lambda.Subvector.doc);
  };

...
PYBIND11_MODULE(analysis, m) {
  ...
  py::class_<Simulator<T>>(m, "Simulator")
    .def(py::init<const System<T>&>(), py::keep_alive<1, 2>())
    .def(py::init<const System<T>&, unique_ptr<Context<T>>>(),
         // This refers to System, and thus must keep the system alive.
         py::keep_alive<1, 2>(),
         // This owns `Context`, and thus must be kept alive by Context.
         py::keep_alive<3, 1>())

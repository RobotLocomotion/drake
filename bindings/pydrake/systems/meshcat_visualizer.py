...
from pydrake.geometry import (
    ...
    DispatchLoadMessage, FrameId, QueryObject, Role, SceneGraph
)
...
class MeshcatVisualizer(LeafSystem):
    ...
    def __init__(self,
        ...
        self._lcm_visualization_input_port = self.DeclareAbstractInputPort(
            "lcm_visualization", AbstractValue.Make(PoseBundle(0))).get_index()
        self._using_deprecated_pose_bundle_api = False
        # This variable provides a "back door", to improve the
        # ConnectMeshcatVisualizer deprecation workflow.
        self._override_using_deprecated_pose_bundle_api = None

        self._geometry_query_input_port = self.DeclareAbstractInputPort(
            "geometry_query", AbstractValue.Make(QueryObject())).get_index()
    ...
    def get_geometry_query_input_port(self):
        return self.get_input_port(self._geometry_query_input_port)
    ...
    def load(self, context=None):
        ...
        Note on deprecation of the pose_bundle API.:
            If context=None, or if the context is passed in without the
            geometry_query_input_port connected, then we must assume that the
            old API is in use.  Experts can override this by setting,
            `_override_using_deprecated_pose_bundle_api`.
        """
        if self._delete_prefix_on_load:
            self.vis[self.prefix].delete()

        # Frustratingly complex deprecation logic!!
        self._using_deprecated_pose_bundle_api = (
            context is None
            or not self.get_geometry_query_input_port().HasValue(context)
        )
        if self._override_using_deprecated_pose_bundle_api is not None:
            self._using_deprecated_pose_bundle_api = self._override_using_deprecated_pose_bundle_api  # noqa

        if self._using_deprecated_pose_bundle_api:
            # Deprecated workflow, using DispatchLoadMessage.
            warnings.warn(
                "The scene_graph argument, and the lcm_visualization input "
                "port are deprecated and will be removed after 2021-02-01. "
                "Please connect the geometry_query input port of "
                "MeshcatVisualizer to the query port of SceneGraph "
                "instead, e.g. with:\n"
                "  builder.Connect(\n"
            ...
            for i in range(load_robot_msg.num_links):
                ...
                for j in range(link.num_geom):
                    ...
                    meshcat_geom, material, element_local_tf = _convert_mesh(
                        geom)
                    if meshcat_geom is not None:
                        ...
                        # multiple (identical) geometries may have the same
                        # UUID, but testing suggests that meshcat + three.js
                        # are ok with it.
                        if isinstance(meshcat_geom,
                                      meshcat.geometry.MeshGeometry):
                            meshcat_geom.uuid = str(uuid.uuid5(
                                uuid.NAMESPACE_X500, meshcat_geom.contents))
                            material.uuid = str(uuid.uuid5(
                                uuid.NAMESPACE_X500, meshcat_geom.contents
                                + "material"))
                            mesh = meshcat.geometry.Mesh(meshcat_geom,
                                                         material)
        ...
        if context is None and self._scene_graph:
            # This is tricky, but important for a smooth deprecation period, I
            # need to allow `load()`` with no Context to still work, but switch
            # `ConnectMeshcatVisualizer` to the new API. My claim is that using
            # a default context here will obtain the same geometry information
            # that I get from the `DispatchLoadMessage(scene_graph,...)` call
            # in the old workflow.
            scene_graph_context = self._scene_graph.CreateDefaultContext()
            query_object = self._scene_graph.get_query_output_port().Eval(
                scene_graph_context)
        else:
            query_object = self.get_geometry_query_input_port().Eval(
                context)

        # Continue new workflow, using SceneGraph's QueryObject.
        inspector = query_object.inspector()

        # Collect dynamic frames that actually have geometries to display.
        ...
        for frame_id in inspector.all_frame_ids():
            ...
            for g_id in inspector.GetGeometries(frame_id, Role.kIllustration):
                ...
                if isinstance(shape, Box):
                    ...
                    continue
                geometry_vis = frame_vis[str(g_id)]
    ...
    def DoPublish(self, context, event):
        ...
        if self._using_deprecated_pose_bundle_api:
            ...
            pose_bundle = self.EvalAbstractInput(
                context, self._lcm_visualization_input_port).get_value()
            ...
            for frame_i in range(pose_bundle.get_num_poses()):
                ...
                if self._is_recording:
                    with self._animation.at_frame(
                            cur_vis, self._recording_frame_num) as f:
...
def ConnectMeshcatVisualizer(builder, scene_graph=None, output_port=None,
    ...
    Args:
        ...
        scene_graph: (optional) The SceneGraph in builder containing the
            geometry to be visualized.  One of either scene_graph OR
            output_port must be valid (not None).
        output_port: (optional) If not None, then output_port will be connected
            to the visualizer input port instead of the
            scene_graph.get_pose_bundle_output_port().  This is required, for
            instance, when the SceneGraph is inside a Diagram, and we must
            connect the exposed port to the visualizer instead of the original
            SceneGraph port.
    ...
    visualizer._override_using_deprecated_pose_bundle_api = False
    if output_port is None:
        assert scene_graph, "Either output_port or scene_graph must be valid."

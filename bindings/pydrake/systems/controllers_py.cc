...
PYBIND11_MODULE(controllers, m) {
  ...
  {
    using Class = DynamicProgrammingOptions::PeriodicBoundaryCondition;
    constexpr auto& cls_doc =
        doc.DynamicProgrammingOptions.PeriodicBoundaryCondition;
    py::object scope = m.attr("DynamicsProgrammingOptions");
    py::class_<Class> cls(scope, "PeriodicBoundaryCondition", cls_doc.doc);
    cls  // BR
        .def(py::init<int, double, double>(), py::arg("state_index"),
            py::arg("low"), py::arg("high"), cls_doc.ctor.doc)
        .def_readwrite(
            "state_index", &Class::static_index, cls_doc.state_index.doc)
        .def_readwrite("low", &Class::low, cls_doc.low.doc)
        .def_readwrite("high", &Class::high, cls_doc.high.doc);
    // TODO(eric.cousineau): Deprecate module-scope alias.
    m.attr("PeriodicBoundaryCondition") = cls;
  }
  ...
  {
    using Class = InverseDynamics<double>::InverseDynamicsMode;
    constexpr auto& cls_doc = doc.InverseDynamics.InverseDynamicsMode;
    py::object scope = m.attr("InverseDynamics");
    py::enum_<Class>(scope, "InverseDynamicsMode")
        .value("kInverseDynamics", Class::kInverseDynamics, cls_doc.doc)
        .value("kGravityCompensation", Class::kGravityCompensation,
            cls_doc.kGravityCompensation.doc)
        .export_values();
  }
...

  {
    ...
    constexpr auto& cls_doc =
        doc.PidControlledSysteml py::class_<Class, Diagram<double>>(
               m, "PidControlledSystem", cls_doc.doc)
            .def(py::init<std::unique_ptr<System<double>>, double, double,
                     double, int, int>(),
            ...
            .def(py::init<std::unique_ptr<System<double>>,
            ...
            .def(
                py::init<std::unique_ptr<System<double>>,
                    const MatrixX<double>&, double, double, double, int, int>(),
            ...
            .def(
                py::init<std::unique_ptr<System<double>>,
                    const MatrixX<double>&, const VectorX<double>&,

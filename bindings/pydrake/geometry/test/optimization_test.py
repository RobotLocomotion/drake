...
class TestGeometryOptimization(unittest.TestCase):
    ...
    def test_h_polyhedron(self):
        ...
        new_vars_and_constraints = hpoly.AddPointInSetConstraints(
            self.prog, self.x)
        self.assertEqual(new_vars_and_constraints[0].size, 0)
    ...
    def test_hyper_ellipsoid(self):
        ...
        new_vars_and_constraints = ellipsoid.AddPointInSetConstraints(
            self.prog, self.x)
        self.assertEqual(new_vars_and_constraints[0].size, 0)
        self.assertGreater(len(new_vars_and_constraints[1]), 0)
    ...
    def test_v_polytope(self):
        ...
        new_vars_and_constraints = vpoly.AddPointInSetConstraints(
            self.prog, self.x[0:2])
        self.assertEqual(new_vars_and_constraints[0].size, vertices.shape[1])

...
class TestGeometryCore(unittest.TestCase):
    ...
    def test_aabb_overlap(self):
        ...
        half_width2 = np.array([0.8, 0.8, 0.8])
        aabb2 = mut.Aabb(p_HoBo=center2, half_width=half_width2)

        identity_transform = RigidTransform()
        self.assertTrue(mut.Aabb.HasOverlap(aabb1, aabb2, identity_transform))

        # Non-overlapping box.
        center3 = np.array([3.0, 0.0, 0.0])
        aabb3 = mut.Aabb(p_HoBo=center3, half_width=half_width1)
        self.assertFalse(mut.Aabb.HasOverlap(aabb1, aabb3,
                                             identity_transform))

        # Test with transform.
        transform = RigidTransform(p=[-2.0, 0.0, 0.0])
        self.assertTrue(mut.Aabb.HasOverlap(aabb1, aabb3, transform))
    ...
    def test_obb_overlap(self):
        ...
        # Overlapping Obb.
        pose2 = RigidTransform(p=[0.5, 0.5, 0.5])
        half_width2 = np.array([0.8, 0.8, 0.8])
        obb2 = mut.Obb(X_HB=pose2, half_width=half_width2)

        identity_transform = RigidTransform()
        self.assertTrue(mut.Obb.HasOverlap(obb1, obb2, identity_transform))

        # Non-overlapping Obb.
        pose3 = RigidTransform(p=[3.0, 0.0, 0.0])
        obb3 = mut.Obb(X_HB=pose3, half_width=half_width1)
        self.assertFalse(mut.Obb.HasOverlap(obb1, obb3, identity_transform))

        # Test Obb-Aabb overlap.
        center = np.array([0.5, 0.5, 0.5])
        half_width = np.array([0.8, 0.8, 0.8])
        aabb = mut.Aabb(p_HoBo=center, half_width=half_width)
        self.assertTrue(mut.Obb.HasOverlap(obb1, aabb, identity_transform))
    ...
    def test_compute_bounding_boxes_for_triangle_mesh(self):
        ...
        self.assertTrue(all(hw >= 0 for hw in obb.half_width()))

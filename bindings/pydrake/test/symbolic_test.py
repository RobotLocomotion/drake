...
class TestSymbolicExpression(unittest.TestCase):
    ...
    def test_get_variable_vector(self):
        vars = sym.GetVariableVector([e_x, e_y])
        self.assertEqual(len(vars), 2)
        if vars[0].get_id() == x.get_id():
            self.assertEqual(vars[1].get_id(), y.get_id())
        else:
            self.assertEqual(vars[0].get_id(), y.get_id())
            self.assertEqual(vars[1].get_id(), x.get_id())

...
class TestSymbolicMonomial(unittest.TestCase):
    ...
    def testMultiplicationAssignment1(self):
        m = sym.Monomial(x, 2)
        m *= sym.Monomial(y, 3)
        self.assertEqual(m.degree(x), 2)
        self.assertEqual(m.degree(y), 3)

    def testMultiplicationAssignment2(self):
        m = sym.Monomial(x, 2)
        m *= y
        self.assertEqual(m.degree(x), 2)

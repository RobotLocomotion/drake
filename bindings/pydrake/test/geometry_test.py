...
class TestGeometry(unittest.TestCase):
    ...
    def test_scene_graph_api(self, T):
        ...
        perception.Add(PropertyName("label", "id"), mut.render.RenderLabel(0))
    ...
    def test_geometry_properties_api(self):
        ...
        phong_diffuse = mut.PropertyName(group="phong", property="diffuse")
        self.assertEqual(phong_diffuse.group(), "phong")
        self.assertEqual(phong_diffuse.property(), "diffuse")
        self.assertEqual(str(phong_diffuse), "phong/diffuse")
        ...
        default_test = mut.PropertyName(default_group, "test")
        prop.Add(property=default_test, value=3)
        self.assertTrue(prop.HasProperty(default_test))
        self.assertEqual(prop.Get(property=default_test), 3)
        self.assertEqual(
            prop.GetPropertyOrDefault(
                property=mut.PropertyName(default_group,
                                          "empty"),
                default_value=5),
            5)
        ...
        # Remove the property.
        self.assertTrue(prop.Remove(property=default_test))
        self.assertFalse(prop.HasProperty(property=default_test))
        # Update a property.
        default_to_update = mut.PropertyName(default_group, "to_update")
        prop.Add(property=default_to_update, value=17)
        self.assertTrue(prop.HasProperty(property=default_to_update))
        self.assertEqual(prop.Get(property=default_to_update), 17)

        prop.Update(property=default_to_update, value=20)
        self.assertTrue(prop.HasProperty(property=default_to_update))
        self.assertEqual(prop.Get(property=default_to_update), 20)
    ...
    def test_geometry_properties_old_api(self):
        ...
        with catch_drake_warnings(expected_count=15):
            # TODO(SeanCurtis-TRI): This whole test should be deprecated when
            #  the old API gets deprecated.
            # Test perception/ illustration properties (specifically Rgba).
            test_vector = [0., 0., 1., 1.]
            test_color = mut.Rgba(0., 0., 1., 1.)
            phong_props = mut.MakePhongIllustrationProperties(test_vector)

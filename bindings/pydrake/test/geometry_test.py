...
class TestGeometry(unittest.TestCase):
    ...
    def check_scene_graph_api(self, T):
        ...
        # TODO(m-chaturvedi): Add other types to `ConnectDrakeVisualizer` in
        # `geometry_visualization.cc`.
        if T == float:
            # Test visualization API.
            # Use a mockable so that we can make a smoke test without sidez
            # effects.
            lcm = DrakeMockLcm()
            for i in range(2):
                builder = DiagramBuilder()
                tmp_scene_graph = SceneGraph()
                scene_graph = builder.AddSystem(tmp_scene_graph)
                if i == 1:
                    ConnectDrakeVisualizer = mut.ConnectDrakeVisualizer

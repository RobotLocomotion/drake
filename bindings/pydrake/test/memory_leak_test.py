...
class _DutRepeater:
    ...
    def dut_trivial_simulator(self):
        ...
        return {self.make_sentinel(builder, "trivial builder"),
                self.make_sentinel(got, "trivial source"),
                self.make_sentinel(got2, "trivial source2"),
                self.make_sentinel(diagram, "trivial diagram"),
                self.make_sentinel(simulator, "trivial simulator")}
    ...
    def dut_mixed_language_simulator(self):
        ...
        return {self.make_sentinel(simulator, "mixed simulator"),
                self.make_sentinel(builder, "mixed builder"),
                self.make_sentinel(diagram, "mixed diagram")}
    ...
    def dut_full_example(self):
        ...
        return {self.make_sentinel(simulator, "full simulator"),
                self.make_sentinel(builder, "full builder"),
                self.make_sentinel(diagram, "full diagram")}

    def repeat(self, *, dut: callable, count: int) -> list[RepetitionDetail]:
...
def _main():
    ...
    repeater = _DutRepeater(verbose=args.verbose)
    all_duts = dict([
        (dut.__name__[4:], dut)
        for dut in [
            repeater.dut_simple_source,
            repeater.dut_trivial_simulator,
            repeater.dut_mixed_language_simulator,
            repeater.dut_full_example,
        ]
    ])
    if args.list:
        print(f"available DUTs: {[k for k in all_duts.keys()]}")
        return
    if args.dut:
    ...
    for name, dut in run_duts.items():
        details = repeater.repeat(dut=dut, count=args.count)
        print(f"RUNNING: {name}")
        for x in details:
            print(x)
    leaks = repeater.leaks()
    print(f"leaks: {leaks}")
    return leaks

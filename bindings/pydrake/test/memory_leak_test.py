...
class Sentinel:
    """Sentinel uses `weakref.finalize` to spy on the end of an object's
...
def _report_sentinels(sentinels, message: str):
    """Print extensive debug information for a sequence of Sentinels.
...
def _counts_for_cycle_parts(o, name):
    ...
    participates in a cycle created by the proposed (but not yet in master)
    drake::pydrake::internal::ref_cycle.

    See: https://github.com/RobotLocomotion/drake/pull/22022

    Object of interest include the object itself, its attribute dictionary
    `__dict__`, and the set contained in `o._pydrake_ref_cycle_peers`.
    """
...
class _DutRepeater:
    """_DutRepeater holds the DUT methods (examples of code that may leak), and
    additional code to exercise, instrument and debug them.
    """
    ...
    def leaks(self):
        """Returns the number of memory leaks detected during any repeat()
        calls on this instance."""
        return self._leaks

    def make_sentinel(self, obj, name):
        """Make a Sentinel for `obj` using `name` for debugging messages.  If
        this instance was created with `verbose=False` (the default), no
        messages will be printed, but the Sentinel will still track the object.
        """
        if self._verbose:
    ...
    def make_sentinels_from_locals(self, dut_name, locals_dict):
        """Make Sentinels for all local variables of interest."""
    ...
    def repeat(self, *, dut: callable, count: int):
        ...
        collection before and after each call. Tracks memory leaks of
        interests, which can be read out by the `leaks()` method. If this
        instance was created with `verbose=True`, additional debug information
        will be printed.
        """
...
class TestMemoryLeaks(unittest.TestCase):
    ...
    def do_test(self, *, dut, count, leaks_allowed=0):
        """Run the requested `dut` (a `self._repeater` bound method named
        'dut_*') for `count` iterations. Check that leaks detected <= leaks
        allowed. In addition, complain if the leaks allowed threshold is higher
        than the actual leaks measured. In that case, the test can likely be

...
// Example class for `DefCopyAndDeepCopy`.
...
GTEST_TEST(PydrakePybindTest, DefCopyAndDeepCopy) {
  ...
  // Ensure that the class instance is copied properly.
  // See `check_copy` in `_pydrake_pybind_test_extra.py` for more information.
...
// See comments in `DefCopyAndDeepCopy` test.
...
GTEST_TEST(PydrakePybindTest, ParamInit) {
  ...
  // Ensure that the values are initialized properly when using the constructor
  // defined by `ParamInit`.

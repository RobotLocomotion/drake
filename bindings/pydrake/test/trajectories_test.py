...
class TestTrajectories(unittest.TestCase):
    ...
    def test_discrete_time_trajectory(self, T):
        ...
        dut1 = DiscreteTimeTrajectory_[T](
            times=times, values=traj.vector_values(times))  # Use Eigen ctor.

        values = [[0.0, 0.0], [1.0, 1.0], [2.0, 2.0]]
        dut2 = DiscreteTimeTrajectory_[T](
            times=times, values=values)  # Use Eigen ctor.
        dut2 = DiscreteTimeTrajectory_[T](
            times=times, values=np.array(values)[..., np.newaxis])  # Use stdvector ctor.

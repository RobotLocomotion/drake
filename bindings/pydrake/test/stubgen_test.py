...
class TestStubgen(unittest.TestCase):
    ...
    def _expected_pyi_files(self):
        ...
        a list of relative paths to all of them.
        """
    ...
    def _check_one_valid_syntax(self, stubs_dir, path):
        ...
        with open(f"{stubs_dir}/{path}", "r") as f:
            source = f.read()
        # Check that the parser doesn't raise any exceptions simply by calling
        # it. For compilation errors, we'll display an improved error message
        # via unittest that shows the problematic line.

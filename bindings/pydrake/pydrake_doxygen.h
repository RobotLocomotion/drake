...
- Module bindings must always be split into parts (to avoid large files that
  would bloat our compile times). The easiest way to understand this
  pattern is probably to look at an existing module that follows the convention
  (e.g., `pydrake/geometry/**`), but here's a summary of the rules:
  - foo_py.h: provides function signatures for the various "Define..." helper
    functions that comprise the module.
  - foo_py.cc: uses PYBIND11_MODULE to define the module, by importing other
    dependent modules and then calling the "Define..." helper functions.
    Must not itself add bindings; it must always call helpers that add them.

...
def plot_sublevelset_quadratic(ax, A, b=[0, 0], c=0, vertices=51, **kwargs):
    ...
    assert(np.size(A, 0) == 2 and np.size(A, 1) == 2), "A must be 2x2"
    ...
    H = .5*(A+np.transpose(A))
    xmin = np.linalg.solve(-2*H, np.reshape(b, (2, 1)))
    fmin = -np.transpose(xmin).dot(H).dot(xmin) + c  # since b = -2*H*xmin
    assert(fmin <= 1), "The minimum value is > 1; there is no sub-level " \
                       "set to plot"

    # f = (x-xmin)'H(x-xmin) + fmin = 0
    # Make a circle of values y, such that: y'y = 1-fmin
    th = np.linspace(0, 2*np.pi, vertices)
    Y = np.sqrt(1-fmin)*np.vstack([np.sin(th), np.cos(th)])
    # then choose L'*(x - xmin) = y, where H = LL'
    L = np.linalg.cholesky(H)
    X = np.tile(xmin, vertices) + \
        np.linalg.inv(np.transpose(L)).dot(Y)

...
class TestRimlessWheel(unittest.TestCase):
    ...
    def test_params(self):
        ...
        params.set_mass(1.0)
        params.set_length(2.0)
        params.set_gravity(4.0)
        params.set_number_of_spokes(7)
        params.set_slope(0.15)
        self.assertEqual(params.mass(), 1.0)
        self.assertEqual(params.length(), 2.0)
        self.assertEqual(params.gravity(), 4.0)
    ...
    def test_state(self):
        ...
        state.set_theta(1.0)
        state.set_thetadot(2.0)
        self.assertEqual(state.theta(), 1.0)
        self.assertEqual(state.thetadot(), 2.0)
    ...
    def test_geometry_with_params(self):
        ...
        geom = RimlessWheelGeometry.AddToBuilder(
            builder=builder,
            floating_base_state_port=plant.get_floating_base_state_output_port(),  # noqa
            rimless_wheel_params=RimlessWheelParams(),
            scene_graph=scene_graph,
        )
        # Confirming that the resulting diagram builds.
        builder.Build()

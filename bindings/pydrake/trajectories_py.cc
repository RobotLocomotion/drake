...
struct Impl {
  ...
  static void DoScalarDependentDefinitions(py::module m) {
...

    {
      ...
      cls  // BR
          .def(py::init([](const Eigen::Ref<const VectorX<T>>& times,
                            const Eigen::Ref<const MatrixX<T>>& values,
                            double time_comparison_tolerance) {
            if (times.size() != values.cols() &&
                times.size() == values.rows()) {
              return DiscreteTimeTrajectory<T>(
                  times, values.transpose(), time_comparison_tolerance);
            } else {
              return DiscreteTimeTrajectory<T>(
                  times, values, time_comparison_tolerance);
            }
          }),
              py::arg("times"), py::arg("values"),
              py::arg("time_comparison_tolerance") =
                  std::numeric_limits<double>::epsilon(),
              cls_doc.ctor.doc_Eigen)

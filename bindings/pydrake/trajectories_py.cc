...
void BindPiecewisePolynomialSerialize(PyClass* cls) {
  ...
  using Polynomials = std::vector<Eigen::MatrixX<Eigen::VectorXd>>;
  // An adaptor to get and/or set the serialized fields.
  struct Archive {
  ...
  cls->def_property_readonly_static("__fields__", [](py::object /* self */) {
  ...
  // and bind them below with that name.
  ...
  cls->def_property(
      "_polynomials", [](const Class&) { return nullptr; },

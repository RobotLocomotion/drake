...
PYBIND11_MODULE(trajectories, m) {
  ...
  py::class_<BsplineTrajectory<T>, Trajectory<T>>(
      ...
      .def("value", &BsplineTrajectory<T>::value, py::arg("time"),
          doc.BsplineTrajectory.value.doc)
      .def("rows", &BsplineTrajectory<T>::rows, doc.BsplineTrajectory.rows.doc)
      .def("cols", &BsplineTrajectory<T>::cols, doc.BsplineTrajectory.cols.doc)
      .def("start_time", &BsplineTrajectory<T>::start_time,
          doc.BsplineTrajectory.start_time.doc)
      .def("end_time", &BsplineTrajectory<T>::end_time,
          doc.BsplineTrajectory.end_time.doc)

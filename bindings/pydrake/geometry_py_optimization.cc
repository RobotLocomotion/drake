...
void DefineGeometryOptimization(py::module m) {
  ...
  // GraphOfConvexSetsOptions
  {
    ...
    py::class_<GraphOfConvexSetsOptions>(
        ...
        .def_readwrite("preprocessing", &SppOptions::preprocessing,
            cls_doc.preprocessing.doc)
        .def_property("solver",
            py::cpp_function(
                [](GraphOfConvexSetsOptions& self) { return self.solver; },
                py_rvp::reference_internal),
            py::cpp_function(
                [](GraphOfConvexSetsOptions& self,
                    solvers::SolverInterface* solver) { self.solver = solver; },
                // Keep alive, reference: `self` keeps `solver` alive.
                py::keep_alive<1, 2>()),  // BR
            cls_doc.solver.doc)
        .def_readwrite("solver_options",
            &GraphOfConvexSetsOptions::solver_options,

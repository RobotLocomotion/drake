...
void DoScalarIndependentDefinitions(py::module m) {
  ...
  m.def(
      ...
      [](std::function<Eigen::VectorXd(const Eigen::VectorXd&)> calc_fun,
          const Eigen::VectorXd& x, const NumericalGradientOption& option) {
        std::function<void(const Eigen::VectorXd&, Eigen::VectorXd*)>
            calc_fun_no_return =
                [&calc_fun](const Eigen::VectorXd& x_val, Eigen::VectorXd* y) {
                  *y = calc_fun(x_val);
                };
        return ComputeNumericalGradient(calc_fun_no_return, x, option);
      },
      py::arg("calc_fun"), py::arg("x"),
      py::arg("option") =

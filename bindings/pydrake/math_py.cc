...
PYBIND11_MODULE(math, m) {
  ...
  // either using __array_ufunc__ or user dtypes. N.B. The ordering in which
  // the overloads are resolved will change based on when modules are
  // loaded. However, there should not be ambiguous implicit conversions
  // between autodiff and symbolic, and double overloads should always occur
  // first, so it shouldn't be a problem. See `math_overloads_test`, which

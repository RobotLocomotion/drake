...
class _GeometryFileHasher:
    ...
    Each "on_..." method incorporates the all external files cited by the given
    argument (of a specific type) into the current hash. Files that are named
    but cannot be opened are sliently skipped.
    """
    ...
    def _read_file(self, path: Path):
        ...
        If the file is missing, sliently returns an empty ``bytes``.
        """
        try:
    ...
    def on_mesh(self, path: Path):
        ...
        for mtl_name in re.findall(rb"^\s*mtllib\s+(.*?)\s*$", content,
                                   re.MULTILINE):
            self.on_mtl(path.parent / mtl_name.decode("utf-8"))

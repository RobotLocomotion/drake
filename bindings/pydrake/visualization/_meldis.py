...
class _DrawFrameApplet:
    ...
    def on_frame_update(self, channel, message):
        ...
        if set(self._channel_link_map.get(channel)) != set(message.link_name):
            self._meshcat.Delete(path=self._channel_to_meshcat_path(channel))
            self._channel_link_map[channel] = message.link_name

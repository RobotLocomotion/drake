...
class _ViewerApplet:
    ...
    def __init__(self, *, meshcat, path, alpha_slider_name,
                 link_filter=None, start_visible=True):
        """Constructs an applet.

        If link_filter is given, only links where `link_filter(link.name)` is
        true will be displayed. (N.B. This filter only applies to rigid bodies;
        it does not apply to deformable geometries, which do not have a concept
        of a "link".)
        """
        self._meshcat = meshcat
        self._path = path
        ...
        if link_filter is not None:
            ...
            self._link_filter = lambda _: True
    ...
    def _build_links(self):
        ...
        for link in message.link:
            if not self._link_filter(link.name):
                continue
    ...
    def on_viewer_draw(self, message):
        ...
        for i in range(message.num_links):
            if not self._link_filter(message.link_name[i]):
                continue
...
class Meldis:
    ...
    def __init__(self, *, meshcat_host=None, meshcat_port=None):
        ...
        default_viewer = _ViewerApplet(meshcat=self.meshcat,
                                       ...
                                       link_filter=is_not_inertia_link)

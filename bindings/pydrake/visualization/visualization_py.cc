...
void DoScalarDependentDefinitions(py::module m, T) {
  ...
  // MeshcatPoseInput
  {
    using Class = MeshcatPoseInput<T>;
    constexpr auto& cls_doc = doc.MeshcatPoseInput;
    DefineTemplateClassWithDefault<MeshcatPoseInput<T>, systems::LeafSystem<T>>(
        m, "MeshcatPoseInput", param, doc.MeshcatPoseInput.doc)
        .def(
            ...
            py::arg("decrement_keycodes") = std::vector<std::string>(
                {"KeyQ", "KeyW", "KeyA", "KeyJ", "KeyI", "KeyO"}),
            py::arg("increment_keycodes") = std::vector<std::string>(
                {"KeyE", "KeyS", "KeyD", "KeyL", "KeyK", "KeyU"}),
            py::arg("prefix") = "",

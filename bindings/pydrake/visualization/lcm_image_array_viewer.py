...
class _ImageServer(Flask):
    ...
    def _response_generator(self):
        for image_data, mine_type in self._image_generator():
            yield (
                b"--frame\r\nContent-Type: "
                + mine_type.encode("utf-8")
                + b"\r\n\r\n"
                + image_data
                + b"\r\n"
            )
...
class LcmImageArrayViewer:
    ...
    def _process_message(self):
        ...
        np_concatenated_image = LcmImageArrayViewer._concatenate_images(
            rgba_images, 1, len(rgba_images)
        )
    ...
    @staticmethod
    def _concatenate_images(images, row, col):
        """Helper function to concatenate multiple images. It is assumed that
        `images` to be a list of systems::sensros::Image with the same size.
        """

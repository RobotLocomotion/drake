...
class TestModelVisualizerCamera(unittest.TestCase):
    ...
    def test_rgbd_renderer_configuration(self):
        ...
        with patch.object(mut_private, "kHasRenderEngineGl", True):
    ...
    def test_rgbd_renderer_availability_messaging(self):
        ...
        with patch.object(mut_private, "kHasRenderEngineGl", True):
            self.assertEqual(
                mut_private._get_supported_rgbd_renderers(), ("vtk", "gl")
            )
            dut = mut.ModelVisualizer(rgbd_renderer="gl")
            self.assertIsInstance(
                dut._make_rgbd_renderer_class(show_window=False),
                RenderEngineGlParams,
            )

        with patch.object(mut_private, "kHasRenderEngineGl", False):
            self.assertEqual(
                mut_private._get_supported_rgbd_renderers(), ("vtk",)
            )
            with self.assertRaisesRegex(ValueError, "kHasRenderEngineGl"):
                mut.ModelVisualizer(rgbd_renderer="gl")
    ...
    def test_rgbd_renderer_cli_help(self):
        ...
        self.assertIn(
            "--rgbd_renderer {vtk,gl}", get_help(has_render_engine_gl=True)
        )

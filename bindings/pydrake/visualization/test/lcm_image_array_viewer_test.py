...
class TestLcmImageArrayViewer(unittest.TestCase):
    ...
    def test_image_processing(self):
        ...
        returned image, and verifies the pixel values for the RGBA images
        remain the same.
        """
        ...
        # Check the RGBA images, i.e., the first and the last images. The pixel
        # values for the colorized depth and label are not tested here.
        np_image_data = np.array(pil_image)
        image_width = 3
        np.testing.assert_equal(np_image_data[:, 0:image_width], 0)
        np.testing.assert_equal(np_image_data[:, -image_width:], 0)

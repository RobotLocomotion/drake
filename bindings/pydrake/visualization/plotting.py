...
def plot_sublevelset_expression(ax, e, vertices=51, **kwargs):
    ...
    for theta in np.linspace(0, np.pi, vertices):
        ...
        env = {x[0]: r*np.sin(theta), x[1]: r*np.cos(theta)}
        ...
        Xplus[0, i] = rplus*np.sin(theta)
        Xplus[1, i] = rplus*np.cos(theta)
        ...
        Xminus[0, i] = rminus*np.sin(theta)
        Xminus[1, i] = rminus*np.cos(theta)
        i = i + 1

...
class ModelVisualizer:
    ...
    def Run(self, position=None, loop_once=False):
        ...
        Returns:
          A ModelVisualizer.RunResult code indicating why the method stopped.
        """
        if self._builder:
        ...
        loop_result = self.RunResult.KEEP_GOING
        try:
    ...
    def RunWithReload(self, position=None):
        ...
        if position:
            # Finalize the plant so we can validate positions.
            self._plant.Finalize()
            self._raise_if_invalid_positions(position)
        ...
        while True:
            ...
            if result != self.RunResult.RELOAD:
                return

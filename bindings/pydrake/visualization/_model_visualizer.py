...
class ModelVisualizer:
    ...
    def Run(self, position=None, loop_once=False, add_stop_button=False):
        """
        ...
        Args:
          ...
          add_stop_button: if True, a 'Stop JointSliders' button will be
            added along with the joint sliders themselves. Useful inside
            Python notebooks when re-using a meshcat instance or wherever
            the publication loop can't be quit from the terminal.
        """
        if self._builder:
        ...
        button_name = "Stop JointSliders"
        if not loop_once:
            if add_stop_button:
                extra_text = f" or click '{button_name}'"
            else:
                extra_text = ""
            print(f"Use Ctrl-C{extra_text} to quit")
        ...
        try:
            ...
            while True:
                ...
                if loop_once:
                    return
                if (add_stop_button
                        and self._meshcat.GetButtonClicks(button_name) > 0):
                    return

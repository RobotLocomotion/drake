...
class ModelVisualizer:
    ...
    def Finalize(self, position=None):
        ...
        """
        already_final_msg = "Finalize has already been called."
        assert self._builder is not None, already_final_msg
        assert self._plant is not None, already_final_msg
        assert self._scene_graph is not None, already_final_msg
        assert self._parser is not None, already_final_msg

        assert self._diagram is None, already_final_msg
        assert self._sliders is None, already_final_msg
        assert self._context is None, already_final_msg
        assert self._plant_context is None, already_final_msg
    ...
    def Run(self, position=None, loop_once=False):
        ...
        invalid_finalize_msg = "Incorrect state after Finalize."
        assert self._builder is None, invalid_finalize_msg
        assert self._scene_graph is None, invalid_finalize_msg
        assert self._parser is None, invalid_finalize_msg

        assert self._diagram is not None, invalid_finalize_msg
        assert self._sliders is not None, invalid_finalize_msg
        assert self._context is not None, invalid_finalize_msg
        assert self._plant_context is not None, invalid_finalize_msg

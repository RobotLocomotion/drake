...
class ModelVisualizer:
    ...
    def Finalize(self, position=None):
        ...
        if self._show_rgbd_sensor:
            ...
            ApplyCameraConfig(
                ...
                builder=self._builder.builder())
            self.camera_sensor = self._builder.builder().GetSubsystemByName(
                "rgbd_sensor_preview")
            ...
            self._builder.builder().ExportOutput(
                self.camera_sensor.GetOutputPort("color_image"),
                "preview_image")
        ...
        if time.time() > (self.last_camera_time + 0.0625):
            ...
            if X_WC is not None:
                ...
                if (self.last_X_WC is None
                    ...
                    self.camera_sensor.SetPoseInParent(
                        self.camera_sensor
    ...
    def Run(self, position=None, loop_once=False):
        ...
        if self._show_rgbd_sensor:
            # State necessary for self.render_if_necessary().

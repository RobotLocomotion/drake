...
class ModelVisualizer:
    ...
    def Finalize(self, position=None):
        ...
        if self._model_filenames:
            model_suffix = "s"
            if self._model_filenames and len(self._model_filenames) == 1:
                model_suffix = ""
            self._reload_button_name = f"Reload Model File{model_suffix}"
    ...
    def _get_default_positions(self):
        ...
        positions = self._diagram.plant().GetPositions(default_context)
    ...
    def _get_position_to_name_map(self):
        ...
        for joint_index in range(self._diagram.plant().num_joints()):
            ...
            for joint_pos in range(joint.num_positions()):
                ...
                instance = int(joint.model_instance())
                result[index] = f"{name}_{suffix}/{instance}"
        return result

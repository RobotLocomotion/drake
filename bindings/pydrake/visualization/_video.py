...
class ColorizeDepthImage(LeafSystem):
    ...
    across is entirety. The ability to set a fixed palette is future work.
...
class VideoWriter(LeafSystem):
    ...
    For companion systems, see also ColorizeDepthImage, ColorizeLabelImage, and
    ConcatenateImages. The static method VideoWriter.AddToBuilder makes it easy
    to record color and/or depth and/or labels all at once.
    ...
    def AddToBuilder(*, filename, builder, sensor_pose, fps=16.0,
        ...
        For simplicity, we recommend using this static factory method instead
        of the VideoWriter constructor.
        ...
        Warning:
            To output the video, this class uses either Pillow (``import PIL``)
            or OpenCV (``import cv2``) and will fail at construction time if
            the module cannot be imported. Use the ``backend`` argument to
            determine which module will be used.
        """
        assert len(kinds) > 0
        if not set(kinds).issubset(set(["color", "label", "depth"])):
            raise RuntimeError("kinds must be a sequence of 'color', 'label', "
                               f"and/or 'depth'; not {kinds!r}")

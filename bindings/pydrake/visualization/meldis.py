...
In many cases, pasing ``-w`` (i.e., ``--open-window``) to the program will be
convenient.
...
class _ViewerApplet:
    ...
    def on_viewer_load(self, message):
        ...
        # Make ourselves invisible so that the viewer_load geometry does not
        # clutter up the scene until we are given its poses in a viewer_draw
        # message.

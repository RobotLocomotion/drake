...
class _ViewerApplet:
    ...
    def __init__(self, *, meshcat, path, alpha_slider_name=None,
                 alpha_slider_value=1.0):
        self._meshcat = meshcat
        self._path = path
        self._load_message = None
        self._alpha_slider_name = alpha_slider_name or self._path[1:11] + ". α"
        self._alpha_slider_value = alpha_slider_value
        self._alpha_slider_exists = False
        self._geom_paths = []
        self._geom_colors = []
    ...
    def on_viewer_draw(self, message):
        ...
        if self._waiting_for_first_draw_message:
            ...
            self._build_links()
            if not self._alpha_slider_exists:
                self._alpha_slider_exists = True
                self._meshcat.AddSlider(self._alpha_slider_name, 0.02, 1.0,
                                        0.02, self._alpha_slider_value)

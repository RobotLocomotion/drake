...
// this should (a) test references for use in `pybind`, and (b) clang-tidy does
// a bad job of parsing `std::function<>` signatures.
...
WTEST(Lambda) {
  ...
  std::function<void (int)> func_1_func = func_1_lambda;  // NOLINT
...
void ChangeCallbackNested(
    const std::function<Callback (const Callback&)>&) {}  // NOLINT

WTEST(ChangeCallbackNested) {
  ...
  using check_expected =
      check_signature<
          ...
          std::function<CallbackWrapped (CallbackWrapped)>>;  // NOLINT
  check_expected::run(wrapped);
}

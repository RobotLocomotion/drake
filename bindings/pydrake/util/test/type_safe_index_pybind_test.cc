...
GTEST_TEST(TypeSafeIndexTest, CheckCasting) {
  ...
  // TODO(eric.cousineau): Should this work?
  // CheckValue("pass_thru_int(10)", Index{10});
  ...
  // TODO(eric.cousineau): Should this work?
  // CheckValue("pass_thru_index(10)", 10);
  ...
  ASSERT_THROW(
      CheckValue("pass_thru_index(OtherIndex(10))", OtherIndex{10}),
      std::runtime_error);
}

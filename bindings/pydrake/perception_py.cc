...
void init_pc_flags(py::module m) {
  ...
  m.attr("BaseFieldT") = GetPyParam<BaseFieldT>()[0];

  {
    using Class = BaseField;
    constexpr auto& cls_doc = doc.BaseField;
    py::enum_<Class>(m, "BaseField", cls_doc.doc, py::arithmetic())
        .value("kNone", Class::kNone, cls_doc.kNone.doc)
        .value("kInherit", Class::kInherit, cls_doc.kInherit.doc)
        .value("kXYZs", Class::kXYZs, cls_doc.kXYZs.doc);
  }

  py::class_<DescriptorType>(m, "DescriptorType", doc.DescriptorType.doc);

  {
    using Class = Fields;
    constexpr auto& cls_doc = doc.Fields;
    py::class_<Class>(m, "Fields")
        ...
        .def("has_descriptor", &Class::has_descriptor,
             cls_doc.has_descriptor.doc)
...
void init_perception(py::module m) {
...

  {
    ...
    cls
        ...
        .def("resize", [](PointCloud* self, int new_size) {
        ...
        .def("has_descriptors", [](const PointCloud* self) {
               return self->has_descriptors();
             }, cls_doc.has_descriptors.doc)
        .def("SetFrom", [](PointCloud* self, const PointCloud& other) {
               self->SetFrom(other);
             },

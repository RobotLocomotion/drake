...
PYBIND11_MODULE(mixed_integer_rotation_constraint, m) {
...

  {
    ...
    py::class_<Struct>(cls, "ReturnType", struct_doc.doc)
        // Massage these types to return copies to appease numpy.
        // If we don't do this, numpy complains on accessing either field:
        //   "dtype=object arrays must be copied, and cannot be referenced."
        .def_property_readonly(
            "B_",
            [](Struct* self)
                -> const std::array<std::array<VectorXDecisionVariable, 3>, 3> {
              return self->B_;
            },
            struct_doc.B_.doc)
        .def_property_readonly(

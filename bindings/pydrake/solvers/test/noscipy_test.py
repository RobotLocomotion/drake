...
class TestMathematicalProgram(unittest.TestCase):
    ...
    def test_linear_constraint(self):
        ...
        # for use outside of Drake. In that case, we should warn them that
        # the test results might not be sound.
        try:
            import scipy
            self.assertNotIn(
                "/scipy_stub/", scipy.__file__,
                "The BUILD dependencies for this test MUST NOT deps on the"
                " :scipy_stub_py, either directly or transitively.")
            warnings.warn(
                f"Your Python runtime has scipy installed at {scipy.__file__};"
                " this test will not be effective at catching bugs.")

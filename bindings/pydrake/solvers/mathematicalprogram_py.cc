  ...
  py::class_<MathematicalProgramResult>(
      ...
      .def("get_solver_details",
          [](const MathematicalProgramResult& self) {
            const auto& abstract = self.get_solver_details();
            // By convention, C++ methods that return a non-Eigen AbstractValue
            // are bound in pydrake to automatically unwrap it for the caller.

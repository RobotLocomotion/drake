...
auto RegisterBinding(py::handle* pscope,
                     ...
                     const string& name, const char* doc_string = "") {
  ...
  prog_cls.def(
      ...
      doc.MathematicalProgram.EvalBindingAtSolution.doc, doc_string);
  return binding_cls;
}
...
PYBIND11_MODULE(_mathematicalprogram_py, m) {
  ...
  RegisterBinding<LinearEqualityConstraint>(&m, &prog_cls,
    ...
    doc.LinearEqualityConstraint.ctor.doc);
  ...
  RegisterBinding<Cost>(&m, &prog_cls, "Cost", doc.Cost.doc);
  RegisterBinding<LinearCost>(&m, &prog_cls, "LinearCost", doc.LinearCost.doc);
  RegisterBinding<QuadraticCost>(&m, &prog_cls, "QuadraticCost",
    doc.QuadraticCost.doc);

  py::class_<VisualizationCallback, EvaluatorBase,
             std::shared_ptr<VisualizationCallback>>(m,
                                                     "VisualizationCallback",
    doc.VisualizationCallback.ctor.doc);

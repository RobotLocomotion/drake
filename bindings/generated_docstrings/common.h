...
the matrix ``m``. An empty matrix returns false.)""";
    ...
    // Symbol: drake::common
    struct /* common */ {
      // Symbol: drake::common::LapTimer
      struct /* LapTimer */ {
        // Source: drake/common/profiler.h
        const char* doc =
R"""(Lap timer. A timer which supports the idea of "laps", a sequence of
timer milestones in an otherwise uninterrupted window of time.

times t₀ t₁ t₂ t₃ t₄ t₅ t₆
┄┄┾━━━━━━━━━━┿━━━━━━━┿━━━━━━━━┿━━━━━━━━━━┿━━━━━━━━━━━━━┽┄┄┄┄┄┄┤ f()
s() l() l() l() l() l() a() laps 1 2 3 4 5

The timeline above shows a series of LapTimer method invocations:
``s()``, `l()``, and `a()``, representing start(), lap(), and
average(), respectively. At each call of lap(), the duration elapsed
between that call and the previous invocation is reported, but the
clock never stops. Finally, the call to average() reports to the total
time spanned by the invocation to start() and the *last* call to lap()
and divides it by the number of calls to lap() (i.e., ``(t₅ - t₀) /
5``).

This class does not store the times for the individual laps; if that
granularity of information is needed, the caller should restore the
returned lap times.

The lap timer can also be used to measure the average duration of
intervals by alternating calls to start() and lap().

times t₀ t₁ t₂ t₃ t₄ t₅ t₆
┄┄┾━━━━━━━━━━┽┄┄┄┄┄┄┄┾━━━━━━━━┽┄┄┄┄┄┄┄┄┄┄┾━━━━━━━━━━━━━┽┄┄┄┄┄┄┤ f()
s() l() s() l() s() l() a() laps 1 2 3

The timeline above shows the measurement of three, non-contiguous
intervals, each bound by a pair of calls to start() and lap(). In this
case, average() will report the average interval length as ``(t₅ - t₄
+ t₃ - t₂ + t₁ + t₀) / 3``.)""";
        // Symbol: drake::common::LapTimer::LapTimer
        struct /* ctor */ {
          // Source: drake/common/profiler.h
          const char* doc = R"""()""";
        } ctor;
        // Symbol: drake::common::LapTimer::Units
        struct /* Units */ {
          // Source: drake/common/profiler.h
          const char* doc = R"""()""";
        } Units;
        // Symbol: drake::common::LapTimer::average
        struct /* average */ {
          // Source: drake/common/profiler.h
          const char* doc =
R"""(Reports the average lap time across all recorded laps.

Template parameter ``Units``:
    the units in which the elapsed time will be reported.)""";
        } average;
        // Symbol: drake::common::LapTimer::lap
        struct /* lap */ {
          // Source: drake/common/profiler.h
          const char* doc =
R"""(Reports the time elapsed from the previous call to lap() or start() to
this call.

Template parameter ``Units``:
    the units in which the elapsed time will be reported.)""";
        } lap;
        // Symbol: drake::common::LapTimer::lapCount_
        struct /* lapCount_ */ {
          // Source: drake/common/profiler.h
          const char* doc = R"""()""";
        } lapCount_;
        // Symbol: drake::common::LapTimer::laps
        struct /* laps */ {
          // Source: drake/common/profiler.h
          const char* doc = R"""(Reports the number of calls to laps.)""";
        } laps;
        // Symbol: drake::common::LapTimer::total
        struct /* total */ {
          // Source: drake/common/profiler.h
          const char* doc =
R"""(Reports the total time measured.

Template parameter ``Units``:
    the units in which the elapsed time will be reported.)""";
        } total;
        // Symbol: drake::common::LapTimer::total_
        struct /* total_ */ {
          // Source: drake/common/profiler.h
          const char* doc = R"""()""";
        } total_;
      } LapTimer;
      // Symbol: drake::common::Timer
      struct /* Timer */ {
        // Source: drake/common/profiler.h
        const char* doc =
R"""(Basic timer. A timer that can start and report the amount of time
elapsed since calling start().

times t₀ t₁ t₂ t₃ t₄ t₅
┄┄┾━━━━━━━━━━┿━━━━━━━┿━━━━━━━━┿━━━━━━━━━━┿━━━━━━━━━━━━━┽┄┄┄┄ f() s()
e() e() e() e() e()

The timeline above shows a series of Timer method invocations: ``s()``
and ``e()``, representing start() and elapsed(), respectively. At each
call of elapsed, the duration elapsed between that time and the time
that start() was called is returned (i.e., ``tᵢ - t₀``, for ``i >
0``).)""";
        // Symbol: drake::common::Timer::Timer
        struct /* ctor */ {
          // Source: drake/common/profiler.h
          const char* doc = R"""()""";
        } ctor;
        // Symbol: drake::common::Timer::elapsed
        struct /* elapsed */ {
          // Source: drake/common/profiler.h
          const char* doc =
R"""(Reports the time elapsed between this call and the last invocation of
start() in the units specified.

Template parameter ``Units``:
    the units in which the elapsed time will be reported.)""";
        } elapsed;
        // Symbol: drake::common::Timer::start
        struct /* start */ {
          // Source: drake/common/profiler.h
          const char* doc = R"""(Starts the timer running.)""";
        } start;
        // Symbol: drake::common::Timer::start_
        struct /* start_ */ {
          // Source: drake/common/profiler.h
          const char* doc =
R"""(The time (in clock cycles) at which the timer was started.)""";
        } start_;
      } Timer;
      // Symbol: drake::common::TimerIndex
      struct /* TimerIndex */ {
        // Source: drake/common/profiler.h
        const char* doc =
R"""(Index used to identify a registered timer.)""";
      } TimerIndex;
    } common;
    // Symbol: drake::cond
    struct /* cond */ {

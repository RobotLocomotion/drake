...
Warning:
    CENIC's error control implementation is only sensitive to the
    generalized positions of the plant() used to formulate the convex
    optimization problem.

Warning:
    For 0-sized integration steps, CENIC executes a special case that
    does no integration or state updates, but does reset the error
    estimate to all 0, and always succeeds. This is in contrast to
    other integrator implementations in Drake. See DoStep() in the
    implementation.

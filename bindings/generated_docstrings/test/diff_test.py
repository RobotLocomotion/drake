...
def _read_resource(resource_path):
...
def main():
    if sys.version_info[:2] == (3, 10):
        # The libclan on jammy gives different docstring output.
        print("WARNING: skipping test on Ubuntu 22.04 Jammy")
        return 0
    filenames = sys.argv[1:]
    assert len(filenames) > 0
    encoding = dict(encoding='utf-8')
    expected_dir = Path("_main/bindings/generated_docstrings/gen")
    actual_dir = Path("_main/bindings/generated_docstrings")

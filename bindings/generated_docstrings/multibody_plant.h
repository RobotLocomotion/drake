...
displacement (in meters) for each body registered via
SetSurfaceVelocityAxis(). Each signal's name is the body's scoped name
(i.e., RigidBody∷scoped_name()). The displacement is initialized to
...
name is the body's scoped name (i.e., RigidBody∷scoped_name()) and its
value is a finite ``double`` speed in m/s. If the port is not
connected, or a body's signal is absent, that body's speed is treated
as zero.

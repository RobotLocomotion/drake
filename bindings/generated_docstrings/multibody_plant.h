...
Raises:
    ...
    ModelInstanceIndex.)""";
        ...
        struct /* CalcSystemJacobianTransposeTimesF */ {
          // Source: drake/multibody/plant/multibody_plant.h
          const char* doc =
R"""(Computes τ = Jᵀ⋅F, the transpose-times-vector product of the System
Jacobian against a per-body spatial force, in O(n) without forming J.

Here J is the System Jacobian — the block-row-stacked Jacobian
Jv_V_WB_W whose B-th block is ``∂V_WB_W/∂v``, the Jacobian of
mobilized body B's spatial velocity in World (about Bo, expressed in
W) with respect to the generalized velocities v. See
...
the generalized forces equivalent to the applied spatial forces. This
is the O(n) operator companion to CalcInverseDynamics(): with zero
generalized accelerations and zero velocities, inverse dynamics
returns −τ (from Eq. (1)), but this method computes τ directly without
forming inertia or bias terms.

Parameter ``context``:
    The state of the multibody system.

Parameter ``F_Bo_W_array``:
    Per-body applied spatial forces, sized num_mobods(), ordered by
    MobodIndex (entry 0 corresponds to World; it is read but has no
    effect on τ). **This array is destroyed during the computation**:
    the operator sweeps tip-to-base, shifting each body's accumulated
    force into its parent's entry of ``F_Bo_W_array`` in place to
    avoid allocating a scratch copy. Callers that need to preserve
    their input should pass a copy.
...
Raises:
    RuntimeError if ``F_Bo_W_array`` is nullptr or wrong size, or if
    ``tau`` is nullptr or wrong size.

...
apply in a continuous, period manner. Failure to do this can cause
...
(which handles argument and the connection for you).)""";
          // Source: drake/geometry/meshcat_visualizer.h
...
documentation.)""";
        ...
        struct /* ctor */ {
          // Source: drake/geometry/meshcat_visualizer.h
          const char* doc =
R"""((Advanced) Creates an instance of MeshcatVisualizer.

While MeshcatVisualizer can be explicitly constructed, proper use
requires proper construction of the parameters below and subsequent
connections between MeshcatVisualizer's input ports to the appropriate
sources. This process is greatly simplified by using the preferred
...
Parameter ``surface_data``:
    Configuration for visualizing surface displacement. If a body in
    MultibodyPlant has registered surface velocity but its
...
must be connected to actually have any effect. The preferred mechanism
is to use visualization∷ApplyVisualizationConfig (which handles the

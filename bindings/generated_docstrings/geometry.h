...
A SceneGraph instance contains the set of geometries with assigned
proximity properties ``G = R ⋃ V ⋃ A = {g₀, g₁, ..., gₙ}``, where R is
the set of dynamic rigid geometries, V is the set of deformable
geometries, and A is the set of anchored rigid geometries. These three
sets are pairwise disjoint. Many proximity queries operate on pairs of
geometries (e.g., (gᵢ, gⱼ)). Those queries operate on a theoretical
set of *candidate pairs*, ``C ⊂ G × G``. C lacks many geometry pairs
that have been excluded from consideration. CollisionFilterManager
excludes many pairs intrinsically and provides mechanisms for the
users to exclude even more pairs.
...
In addition to excluding specific geometry pairs, a geometry can be
declared inactive* (see Deactivate() and Activate()). An inactive
geometry is essentially excluded from G. It still exists as a known
geometry. So, adding or removing filters on that geometry is still
valid. However, those declarations will have no apparent effect until
the geometry is reactivated. An inactive geometry will simply never
appear in any pair in the candidate set C.

We can define ``N`` as the set of inactive geometries and ``Gₐ = G -
N`` as the set of active geometries.

Candidate Geometry Pairs

Therefore, the set of geometry pair candidates C for proximity queries
is defined as:
...
Warning:
    Generally, the effect of applying a declaration is based on the
    state of SceneGraph's geometry data at the time of application.
    The geometry's active status is the one exception (see below).
    More concretely: - For a particular FrameId in a GeometrySet
    instance, only those geometries attached to the identified frame
    with the proximity role assigned at the time of the call will be
    included in the filter. If geometries are subsequently added or
    assigned the proximity role, they will not be retroactively added
    to the user-declared filter. - In general, adding collision
    geometries and assigning proximity roles should happen prior to
    collision filter configuration. - Declarations of user-defined
    exclusions and allowances can only be made on "known" geometries
    (geometries with assigned the proximity role). Declarations on
    unknown geometries will be rejected with an error. - Declaring a
    set of geometries active/inactive is the exception. These add or
    remove the geometries from the set Gₐ. We are not adding/deleting
    filtered pairs at the time of declaration, but simply changing the
    active status of the geometries. A geometry's active status

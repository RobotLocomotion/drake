...
template <typename T>
VectorX<T> NeuralNetwork<T>::evaluateLayer(
...
template <typename T>
std::unique_ptr<MatrixX<T>> NeuralNetwork<T>::getWeightMatrix(
...
template <typename T>
std::unique_ptr<VectorX<T>> NeuralNetwork<T>::getBiasVector(
    int index, const Context<T>& context) const {
  DRAKE_THROW_UNLESS((0 <= index) &&
...
template <typename T>
std::unique_ptr<BasicVector<T>> NeuralNetwork<T>::encode(
...
template <typename T>
std::unique_ptr<MatrixX<T>> NeuralNetwork<T>::decode(
...
template <typename T>
void NeuralNetwork<T>::writeOutput(const VectorX<T> value,
                                   BasicVector<T>* output) const {

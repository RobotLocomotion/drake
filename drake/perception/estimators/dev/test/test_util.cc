...
AssertionResult ExpectSO3(const Eigen::Matrix3d& R,
                                     double tolerance) {
...
AssertionResult CompareTransforms(
  ...
  AssertionResult check_R_expected =
      ExpectSO3(X_expected.rotation(), tolerance);
  if (!check_R_expected) {
    return check_R_expected << "(X_expected)";
  }
  AssertionResult check_R_actual =
      ExpectSO3(X_actual.rotation(), tolerance);
  if (!check_R_actual) {
...
AssertionResult CompareTransformWithoutAxisSign(
  ...
  return CompareSO3WithoutAxisSign(X_expected.rotation(),

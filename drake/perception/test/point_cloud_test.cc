...
GTEST_TEST(PointCloudTest, Basic) {
  ...
  // Extras (Curvature).
  Eigen::RowVectorXf extras_expected(count);
  extras_expected <<
    1, 2, 3, 4, 5;
  CheckFields(extras_expected, pc_flags::kExtras, pc_flags::kExtraCurvature,
...
GTEST_TEST(PointCloudTest, Fields) {
  ...
  EXPECT_EQ(
      "(kXYZs | kExtras::Curvature)",
            ToString(pc_flags::kXYZs | pc_flags::kExtras,
  ...
  // Check basic requirements.
  {
    ...
    EXPECT_FALSE(cloud.HasFields(pc_flags::kExtras));
    EXPECT_THROW(cloud.RequireFields(pc_flags::kExtras),
                 std::runtime_error);
  }

  // Check with exact fields.
  {
    PointCloud cloud(1, pc_flags::kXYZs | pc_flags::kExtras,
                     pc_flags::kExtraCurvature);
    EXPECT_TRUE(cloud.HasExactFields(
        pc_flags::kXYZs | pc_flags::kExtras, pc_flags::kExtraCurvature));
  ...
  // Check with extras.
  {
    ...
    EXPECT_FALSE(cloud.has_extras(pc_flags::kExtraFPFH));

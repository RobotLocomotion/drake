...
class GeometryStateTest : public ::testing::Test {
 protected:
  void SetUp() {
    frame_ = make_unique<GeometryFrame<double>>();
  ...
  SourceId SetUpSingleSourceTree() {
    ...
    frames_.push_back(geometry_state_.RegisterFrame(
        source_id_, GeometryFrame<double>()));  // f0.

    // Create f1.
    frames_.push_back(geometry_state_.RegisterFrame(
        source_id_, GeometryFrame<double>()));  // f1.

    // Create f2.
    frames_.push_back(geometry_state_.RegisterFrame(
        source_id_, frames_[1], GeometryFrame<double>()));  // f2

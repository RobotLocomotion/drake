...
int main(int argc, char **argv) {
  ...
  valuecheck(derivative.scalarValue(result.getEndTime(num_segments - 1)), xdf,
             tol);
  ...
  for (int i = 0; i < num_knots; i++) {
    ...
    valuecheck(result.scalarValue(t_knot - eps),
               result.scalarValue(t_knot + eps), 1e-8);
    valuecheck(derivative.scalarValue(t_knot - eps),
               derivative.scalarValue(t_knot + eps), 1e-8);
    valuecheck(second_derivative.scalarValue(t_knot - eps),
  ...
  int ntests = 1000;
  cout << "time: "
       << measure<chrono::microseconds>::execution(randomSpeedTest, ntests) /
              (double)ntests << " microseconds." << endl;
#endif

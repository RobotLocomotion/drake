...
int SplineInformation::getSegmentPolynomialDegree(int segment_number, Eigen::Index row, Eigen::Index cols) const {
  checkScalarValued();
  return segment_polynomial_degrees[segment_number];
}

Eigen::Index SplineInformation::rows() const {
  return 1;
}

Eigen::Index SplineInformation::cols() const {
  return 1;
}

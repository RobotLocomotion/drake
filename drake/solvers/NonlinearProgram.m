      ...
      if(any(empty_grad_row))
        ...
        if(~isempty(empty_grad_cin))
          warning('Drake:NonlinearProgram:EmptyGradient',...
            sprintf('The decision variables in nonlinear inequality constraint(s) %s are all fixed (due to equality bounding box constraints on the decision variables). Consider either removing this constraint, or relaxing the bounds on the decision variables.\n',...
              show_names(empty_grad_cin, obj.cin_name)));
        end
        if(~isempty(empty_grad_ceq))
          warning('Drake:NonlinearProgram:EmptyGradient',...
            sprintf('The decision variables in nonlinear equality constraint(s) %s are all fixed (due to equality bounding box constraints on the decision variables). Consider either removing this constraint, or relaxing the bounds on the decision variables.\n',...
              show_names(empty_grad_ceq, obj.ceq_name)));
        end
        if(~isempty(empty_grad_Ain))
          warning('Drake:NonlinearProgram:EmptyGradient',...
            sprintf('The decision variables in linear inequality constraint(s) %s are all fixed (due to equality bounding box constraints on the decision variables). Consider either removing this constraint, or relaxing the bounds on the decision variables.\n',...
              show_names(empty_grad_Ain, obj.Ain_name)));
        end
        if(~isempty(empty_grad_Aeq))
          warning('Drake:NonlinearProgram:EmptyGradient',...
            sprintf('The decision variables in linear equality constraint(s) %s are all fixed (due to equality bounding box constraints on the decision variables). Consider either removing this constraint, or relaxing the bounds on the decision variables.\n',...

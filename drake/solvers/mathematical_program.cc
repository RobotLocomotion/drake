...
Binding<LinearConstraint> MathematicalProgram::AddLinearConstraint(
  ...
  unordered_map<symbolic::Variable, int, hash_value<symbolic::Variable>>
      map_var_to_index;
  vector<symbolic::Variable> var_vec;
  int idx{0};
  for (int i{0}; i < v.size(); ++i) {
    for (const symbolic::Variable& var : v(i).GetVariables()) {
      if (map_var_to_index.find(var) == map_var_to_index.end()) {
        map_var_to_index.emplace(var, idx++);
  ...
  for (int i{0}; i < v.size(); ++i) {
    ...
    if (is_addition(e_i)) {
      // Decompose a linear combination e_i = c0 + c1 * v1 + ... cn * vn into
      // the followings:
      //    constant term      : c0
      //    coefficient vector : [c1, ..., cn]
      //    variable vector    : [v1, ..., vn]
      const double constant_term{get_constant_term_in_addition(e_i)};
      const map<symbolic::Expression, double>& term_to_coeff_map{
          get_terms_in_addition(e_i)};
      for (const pair<symbolic::Expression, double>& p : term_to_coeff_map) {
        if (is_variable(p.first)) {
          ...
          A(i, map_var_to_index[var]) = coeff;
        } else {

...
pair<double, Eigen::RowVectorXd> MathematicalProgram::DecomposeLinearExpression(
    const symbolic::Expression& e,
    const unordered_map<size_t, int>& map_var_to_index) {
  const double constant_term{get_constant_term_in_addition(e)};
  Eigen::RowVectorXd coeffs{Eigen::RowVectorXd::Zero(map_var_to_index.size())};
  ...
  for (const pair<symbolic::Expression, double>& p : term_to_coeff_map) {
    if (is_variable(p.first)) {
      ...
      coeffs(map_var_to_index.at(var.get_id())) = coeff;
    } else {
      throw symbolic_error(e, "non-linear");
    }
  }
  return make_pair(constant_term, coeffs);
}
...
Binding<LinearConstraint> MathematicalProgram::AddLinearConstraint(
  ...
  Eigen::MatrixXd A{v.size(), vars.size()};
  ...
  for (int i{0}; i < v.size(); ++i) {
    ...
    if (is_addition(e_i)) {
      const pair<double, Eigen::RowVectorXd> decomposition{
          DecomposeLinearExpression(e_i, map_var_to_index)};
      const double constant_term{decomposition.first};
      A.row(i) = decomposition.second;

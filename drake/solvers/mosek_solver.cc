...
MSKrescodee AddLinearConstraintsFromBindings(MSKtask_t &task, const std::list<_Binding> &constraint_list, bool is_equality_constraint) {
  for(const auto &binding : constraint_list) {
    ...
    for (int i = 0; i < A.rows(); ++i) {
      ...
      if(is_equality_constraint) {
        rescode = MSK_putconbound(task, constraint_idx + i, MSK_BK_FX, lb(i), ub(i));
        if(rescode != MSK_RES_OK) {
          return rescode;
        }
      }
        ...
        if(isinf(lb(i)) && isinf(ub(i))) {
          ...
          rescode = MSK_putconbound(task, constraint_idx + i, MSK_BK_UP, -MSK_INFINITY, ub(i));
        }
        else if(!isinf(lb(i)) && isinf(ub(i))) {
          rescode = MSK_putconbound(task, constraint_idx + i, MSK_BK_LO, lb(i), MSK_INFINITY);
        }
        else {
          rescode = MSK_putconbound(task, constraint_idx + i, MSK_BK_RA, lb(i), ub(i));
        }
        if(rescode != MSK_RES_OK) {
          return rescode;
        }
      ...
      rescode = MSK_putarow(task, constraint_idx + i, A_nnz_val.size(), A_nnz_col_idx.data(), A_nnz_val.data());
      if(rescode != MSK_RES_OK) {
        return rescode;
      }
    }
...
MSKrescodee AddLinearConstraints(MSKenv_t &env, MSKtask_t &task, const MathematicalProgram &prog) {
  MSKrescodee rescode = AddLinearConstraintsFromBindings(task, prog.linear_equality_constraints(), true);
  if(rescode != MSK_RES_OK) {
    return rescode;
  }
  rescode = AddLinearConstraintsFromBindings(task, prog.linear_constraints(), false);
  if(rescode != MSK_RES_OK) {
    return rescode;
  }
}

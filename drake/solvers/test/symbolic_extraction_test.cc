...
GTEST_TEST(SymbolicExtraction, DecomposeQuadraticExpression) {
  ...
  VectorDecisionVariable<3> vars_expected(x, y, z);
  Matrix3d Q_in(num_variables, num_variables);
  Q_in <<
    3, 2, 1,
    4, 6, 5,
    7, 8, 9;
  Vector3d b_expected(10, 11, 12);
...
GTEST_TEST(SymbolicExtraction, DecomposeLinearExpression) {
  ...
  VectorXDecisionVariable vars_expected(num_variables);
  vars_expected << x, y, z;

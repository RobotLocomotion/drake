...
class SosUtilTest : public ::testing::Test {
 public:
  SosUtilTest()
      : exprs_({2, x_, 5 * x_, y_, x_ * y_, 2 * x_ * x_, 6 * x_ * y_,
        ...
        // TODO(FischerGundlach) why doesnt it catch this one?
        // support_1{&data_1[0]},
 ...
 protected:
  ...
  // TODO(FischerGundlach) Solve the riddle: why does the initialization with
  // other members works here but NOT with the initilization list?
  // work around in order to intialize Eigen::Matrix
  /*support_0 is empty*/
  const Eigen::Matrix<int, 0, 0> support_0;
  ...
  const Eigen::Matrix<int, 1, 0> polytope_0;
  const Eigen::Matrix<int, 1, 0> polytope_1;
  const Eigen::Matrix<int, 1, 0> polytope_2;
  const Eigen::Matrix<int, 1, 0> polytope_3;
...
TEST_F(SosUtilTest, CrossTerms) {
  ...
  for (int i = 0; i < static_cast<int>(base_matrices_.size()); ++i) {
    EXPECT_EQ(RowIntersect(CrossTerms(base_matrices_[i]), cross_matrices_[i]),
              cross_matrices_[i]);
    //    EXPECT_EQ(RowIntersect(CrossTerms(base_matrices_[i]),
...
TEST_F(SosUtilTest, DiagConsistentTest) {
  ...
  // tested within DiagConsistent
  EXPECT_EQ(DiagConsistent(support_9, sos_support_9), support_9 / 2);
}
...
TEST_F(SosUtilTest, ExponentBoundPolytopeTest) {
  ...
  for (auto& p : polys_) {
    ...
    EXPECT_EQ(ExponentBoundPolytope(pair_.first), polytopes_[i]);
    ++i;
  }
}
...
TEST_F(SosUtilTest, RandomPruneTest) {
  ...
  for (auto& p : polys_) {
    ...
    Eigen::MatrixXi mpow{ExponentBoundPolytope(pow)};

    if (mpow.cols() > 0) {
      // performe the test 100 times, as the pruning is a random operation.
      for (int testruns = 0; testruns < 100; ++testruns) {
        mpow = RandomPrune(pow, mpow, 1);
...
TEST_F(SosUtilTest, RemoveRowsTest) {
  array<int, 1> data_remove_0{{0}};
  set<int> set_remove_0{data_remove_0.begin(), data_remove_0.end()};
  array<int, 1> data_remove_1{{0}};
  set<int> set_remove_1{data_remove_1.begin(), data_remove_1.end()};
  array<int, 1> data_remove_2{{0}};
  set<int> set_remove_2{data_remove_2.begin(), data_remove_2.end()};
  array<int, 1> data_remove_3{{0}};
  set<int> set_remove_3{data_remove_3.begin(), data_remove_3.end()};

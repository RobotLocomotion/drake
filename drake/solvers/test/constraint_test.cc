...
void TestRotatedLorentzConeEval(const VectorXd &x_test, bool is_in_cone) {
  ...
  y_expected(2) = pow(x_test(0)*x_test(1), 2) - x_test.tail(x_test.size()-2).squaredNorm();
  EXPECT_TRUE(CompareMatrices(y, y_expected, 1E-10, MatrixCompareType::absolute));

  bool is_in_cone_expected = (x_test(0)>=0)&(x_test(1)>=0)&(x_test(0)*x_test(1)>=x_test.tail(x_test.size()-2).norm());
  EXPECT_TRUE(is_in_cone==is_in_cone_expected);
...
GTEST_TEST(testConstraint, testRotatedLorentzConeConstraint) {
  ...
  auto lb = cnstr.lower_bound();
  auto ub = cnstr.upper_bound();
  EXPECT_TRUE(CompareMatrices(Eigen::Vector3d::Zero(), lb, 1E-10, MatrixCompareType::absolute));
  EXPECT_TRUE(CompareMatrices(Eigen::Vector3d::Constant(std::numeric_limits<double>::infinity()), ub, 1e-10, MatrixCompareType::absolute));

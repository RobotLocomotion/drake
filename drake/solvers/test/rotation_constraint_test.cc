...
/*
void RotationMatrixCheck(const Eigen::Ref<const Matrix3d>& R)
{
...
GTEST_TEST(RotationTest, TestMcCormick) {
  ...
  for (int num_bins = 1; num_bins < 3; num_bins++) {
    ...
    auto IsFeasible = [&](Matrix3d R) -> bool {
      return IsFeasibleCheck(&prog, feasibility_constraint, R);
    };
    ...
    // This one caught a bug during random testing.

...
GTEST_TEST(testConstraint, appendLinearConstraint){
	...
	Vector4d ub_new;
	ub_new<<ub,ub_append;
	EXPECT_TRUE(CompareMatrices(A_new,lin_con.A(),1e-10,MatrixCompareType::absolute));
	EXPECT_TRUE(CompareMatrices(lb_new,lin_con.lower_bound(),1e-10,MatrixCompareType::absolute));
	EXPECT_TRUE(CompareMatrices(ub_new,lin_con.upper_bound(),1e-10,MatrixCompareType::absolute));
}
		}
	}
}
...
class GurobiSolverResult : public MathematicalProgramSolverResult {
 public:
  GurobiSolverResult(SolutionSummary summary, int gurobi_error, int status) :
      MathematicalProgramSolverResult(summary), gurobi_error_(gurobi_error), status_(status) {}
 ...
 private:
  int gurobi_error_; // The error encountered by calling Gurobi functions, refer
  ...
  int status_; // The status after the optimize call has returned.
               // Refer to http://www.gurobi.com/documentation/6.5/refman/optimization_status_codes.html
               // for more details.
};

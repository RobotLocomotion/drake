...
class MathematicalProgram {
 public:
  ...
  /** Returns a pair of a SOS polynomial p = xᵀQx and a PSD constraint for the
   * coefficients matrix Q, where x is a monomial basis over @p indeterminates
   * of @p degree. For example, `NewSosPolynomial({x}, 2)` returns a pair of a
   * polynomial p = Q₍₀,₀₎x⁴ + 2Q₍₁,₀₎ x³ + (2Q₍₂,₀₎ + Q₍₁,₁₎)x² + 2Q₍₂,₁₎x +
   * Q₍₂,₂₎ and a PSD constraint over Q.
   */
   ...
   *  - The coefficient matrix Q is PSD (positive semidefinite).
   *  - The coefficient matching conditions in linear equality constraint.
   */

...
class MathematicalProgram {
 public:
  ...
  VectorDecisionVariable<rows> NewContinuousVariables(
    ...
    int offset = (name.compare("x") == 0) ? decision_variables_.rows() : 0;
    for (int i = 0; i < rows; ++i) {
  ...
  VectorDecisionVariable<rows> NewBinaryVariables(
      const std::string& name = "b") {
    std::array<std::string, rows> names;
    int offset = (name.compare("b") == 0) ? decision_variables_.rows() : 0;
    for (int i = 0; i < rows; ++i) {
      names[i] = name + "(" + std::to_string(offset + i) + ")";
    }
    return NewBinaryVariables<rows, 1>(names);
  ...
  void SetInitialGuessForAllVariables(const Eigen::MatrixBase<Derived>& x0) {
    DRAKE_ASSERT(x0.rows() == static_cast<int>(decision_variables_.rows()) &&
                 x0.cols() == 1);
    x_initial_guess_ = x0;
  }
  ...
  void PrintSolution() {
    for (int i = 0; i < static_cast<int>(decision_variables_.rows()); ++i) {
 ...
 private:
  ...
  void NewVariables_impl(
    ...
    decision_variables_.conservativeResize(
        decision_variables_.rows() + num_new_vars, Eigen::NoChange);
    x_values_.resize(decision_variables_.rows() + num_new_vars, NAN);
    int row_index = 0;
    int col_index = 0;
    for (int i = 0; i < num_new_vars; ++i) {
      decision_variables_(decision_variables_.rows() - num_new_vars + i) =
          symbolic::Variable(names[i], type);
      const int new_var_index = decision_variables_.rows() - num_new_vars + i;
    ...
    x_initial_guess_.conservativeResize(decision_variables_.rows());

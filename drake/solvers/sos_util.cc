...
VectorX<Monomial> GramBasis(const symbolic::Polynomial& p) {
  if (p.TotalDegree() % 2 == 1) {  // catches off polynomials
    // TODO(FischerGundlach) Instead of returning an empty Vector, rather throw
    // an error, that the p is odd.
    // TODO(FischerGundlach) Catch all obvious non-SOS polynomial, i.e.
    // x_0^3*x_1^5 is clearly not SOS.
    return VectorX<Monomial>();
    ...
    MatrixXi mpow{build_gram_basis::ExponentBoundPolytope(pow)};
...
MatrixXi CrossTerms(const MatrixXi& mpow) {
...
MatrixXi DiagConsistent(const MatrixXi& pow, MatrixXi mpow) {
  ...
  // These cases should be catched in GramBasis already.
...
MatrixXi RandomPrune(const MatrixXi& pow, MatrixXi mpow,
  ...
  // Seed 150588 and range [0,99] are chosen arbitrarily.
  std::mt19937 mt(150588);

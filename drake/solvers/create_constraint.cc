...
//        -∞ <= -∞     OK
//
//  and throw an exception for the infeasible case and set e := e1' - e2' and
...
void FindBound(const Expression& e1, const Expression& e2, Expression* const e,
  ...
  if (c1 == numeric_limits<double>::infinity() &&
    ...
    ostringstream oss;
    oss << "FindBound() detects infeasible constraint: " << e1 << " <= " << e2
        << ".";
    throw runtime_error{oss.str()};
  }
  *c = numeric_limits<double>::infinity();  // Sets the upper-bound to be ∞.
}
}  // namespace

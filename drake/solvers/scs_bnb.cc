...
namespace drake {
namespace solvers {
ScsNode::ScsNode(const AMatrix& A, const scs_float* const b,
    ...
    : A_{static_cast<AMatrix*>(malloc(sizeof(AMatrix)))},
      b_{static_cast<scs_float*>(
          scs_calloc(A.m + 2 * binary_var_indices.size(), sizeof(scs_float)))},
  ...
  for (int j = 1; j <= A.n; ++j) {
    ...
    if (is_binary_column) {
      ...
      A_->i[A_->p[j - 1] + column_nonzero_index + 1] =
    ...
    for (; column_nonzero_index < A.p[j] - A.p[j - 1]; ++column_nonzero_index) {
      A_->x[A_->p[j - 1] + column_nonzero_index + num_added_constraints] =
          A.x[A.p[j - 1] + column_nonzero_index];
      A_->i[A_->p[j - 1] + column_nonzero_index + num_added_constraints] =
          A.i[A.p[j - 1] + column_nonzero_index] +
...
void ScsNode::Branch(int binary_var_index) {
  ...
  left_child_->A_->p = static_cast<scs_int*>(scs_calloc(left_child_->A_->n + 1,
sizeof(scs_int)));
...
sizeof(scs_float)));
  // We fix the binary variable z to 0 in the left child, and to 1 in the right
child.
...
sizeof(scs_float)));
  ...
  right_child_ = new ScsNode(left_child_->A_, left_child_->b_, left_child_->c_,

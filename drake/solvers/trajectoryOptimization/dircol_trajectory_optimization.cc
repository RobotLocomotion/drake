...
void DircolTrajectoryOptimization::AddRunningCost(
  ...
  opt_problem()->AddCost(
      std::make_shared<RunningCostEndWrapper>(constraint),
      {h_vars().head(1), x_vars().head(num_states()),
            u_vars().head(num_inputs())});

  for (int i = 1; i < N() - 1; i++) {
    opt_problem()->AddCost(
        std::make_shared<RunningCostMidWrapper>(constraint),
        {h_vars().segment(i - 1, 2),  // Emacs likes this indentation...

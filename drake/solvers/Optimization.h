...
class DifferentiableFunctionConstraint : public Constraint {
 public:
  DifferentiableFunctionConstraint(
      const std::shared_ptr<DifferentiableFunction>& func,
      size_t num_constraints)
      : Constraint(num_constraints), func(func) {}
...
class DRAKEOPTIMIZATION_EXPORT OptimizationProblem {
 ...
 public:
  ...
  std::shared_ptr<DifferentiableFunctionConstraint> addCost(
      const std::shared_ptr<DifferentiableFunction>& obj,
      const VariableList& vars) {
    std::shared_ptr<DifferentiableFunctionConstraint> objective(
        new DifferentiableFunctionConstraint(obj, 1));

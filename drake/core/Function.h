...
class Function {
 public:
  ...
  virtual const InputOutputRelation& getInputOutputRelation() const {
    return relation;
  };
  // note: for multiple argument functions, this can be
  // getInputOutputRelation(input_num,output_num)
...
class DifferentiableFunction : public Function {
 public:
  DifferentiableFunction()
      : Function(InputOutputRelation::Form::DIFFERENTIABLE){};
...
class TemplatedDifferentiableFunction : public DifferentiableFunction {
 public:
  TemplatedDifferentiableFunction(Derived& derived)
      : DifferentiableFunction(), derived(derived){};
  virtual ~TemplatedDifferentiableFunction(){};

  virtual void eval(const Eigen::Ref<const Eigen::VectorXd>& x,
                    Eigen::VectorXd& y) const override {
    derived.evalImpl(x, y);
  }
  virtual void eval(const Eigen::Ref<const TaylorVecXd>& x,
...
// idea: avoid dynamic allocation by having derived classes which template on
// input/output size?
// hopefully the use of Ref above will mean they can still derive from Function

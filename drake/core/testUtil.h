#ifndef DRAKE_CORE_TESTUTIL_H_
#define DRAKE_CORE_TESTUTIL_H_
...
/*
 * +++tk:
 * Note: can't call this function 'valuecheck' because it conflicts with our
 *valuecheck template for general types above on GCC.
 * (yes, even though the number of template arguments and the number of function
 *arguments is different!)
 * It does compile on MSVC 2010 and Clang.
 * What I believe happens is that during argument-dependent name lookup, GCC
 *instantiates Eigen::MatrixBase<int>, which causes errors deep in
 *Eigen::internal code
...
void valuecheckMatrix(const Eigen::MatrixBase<DerivedA>& a,
  ...
  if (a.rows() != b.rows() || a.cols() != b.cols()) {
    throw std::runtime_error(
        "Drake:ValueCheck ERROR:" + error_msg + "size mismatch: (" +
        std::to_string(static_cast<unsigned long long>(a.rows())) + " by " +
        std::to_string(static_cast<unsigned long long>(a.cols())) + ") and (" +
      ...
      for (int i = 0; i < a.rows(); i++)
        for (int j = 0; j < a.cols(); j++) {
          bool both_positive_infinity =
              ...
              b(i, j) == std::numeric_limits<double>::infinity();
          bool both_negative_infinity =
              a(i, j) == -std::numeric_limits<double>::infinity() &&
              b(i, j) == -std::numeric_limits<double>::infinity();
          bool both_nan = std::isnan(a(i, j)) && std::isnan(b(i, j));
          ok = ok && (both_positive_infinity || both_negative_infinity ||
                      (both_nan) || (std::abs(a(i, j) - b(i, j)) < tol));
        }
      if (ok) return;
    }
    error_msg += "\nA:\n" + to_string(a) + "\nB:\n" + to_string(b) +

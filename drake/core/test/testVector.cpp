...
int main(int argc, char* argv[]) {
  try {
    ...
    }
    {
      ...
      // original type
      {
        ...
        if (!is_same<PendulumState<double>, decltype(test)>::value)
          throw std::runtime_error("combined vector builder returned " +
    ...
    static_assert(Eigen::Matrix<double, 2, 1>::RowsAtCompileTime == 2,
    ...
    */
    ...
    if (!InputOutputRelation::isA(InputOutputRelation::Form::ZERO,
                                  InputOutputRelation::Form::ARBITRARY))
      throw runtime_error("linear is arbitrary");

    if (InputOutputRelation::leastCommonAncestor(
...

    {
      ...
      if (InputOutputRelation::composeWith(g, f).form !=
          InputOutputRelation::Form::POLYNOMIAL)
        throw runtime_error("should be poly");
      if (InputOutputRelation::composeWith(f, g).form !=
          InputOutputRelation::Form::POLYNOMIAL)
        throw runtime_error("should be poly");
      if (InputOutputRelation::combine(g, f).form !=
          InputOutputRelation::Form::POLYNOMIAL)
        throw runtime_error("should be poly");
    }

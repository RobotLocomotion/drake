...
class DRAKECORE_EXPORT FunctionalForm {
 public:
  ...
  class DRAKECORE_EXPORT Variables {
   ...
   private:
    Variables(std::vector<Variable>&& vars);
 ...
 private:
  enum class Class;
  FunctionalForm(Class c, Variables&& v);
...
#if 0
  // Enable these when we encounter a need for them.
  static inline Real epsilmn() { return FunctionalForm(); }
  static inline FunctionalForm highest() { return FunctionalForm(); }
  static inline FunctionalForm lowest()  { return FunctionalForm(); }
#endif
};

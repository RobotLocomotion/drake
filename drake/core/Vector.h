...
namespace Drake {
   ...
   */
  template<typename VecType>
  std::size_t size(const VecType &vec) { return internal::SizeDispatch<VecType>::eval(vec); }
  ...
  */
  ...
  public:
    ...
    CombinedVector(const Eigen::MatrixBase <Derived> &x) : vec1(x.topRows(Vector1<ScalarType>::RowsAtCompileTime)),
                                                           vec2(x.bottomRows(Vector2<ScalarType>::RowsAtCompileTime)) {
      static_assert(RowsAtCompileTime != Eigen::Dynamic,
                    "Cannot determine sizes of subvectors because sizes are not known at compile time."); // TODO: could handle cases where only one of the subvectors has dynamic size
    };

    template<typename Derived1, typename Derived2>
    CombinedVector(const Eigen::MatrixBase <Derived1> &x1, const Eigen::MatrixBase <Derived2> &x2) : vec1(x1),
                                                                                                     vec2(x2) { };
    ...
    enum {
      RowsAtCompileTime = (Vector1<ScalarType>::RowsAtCompileTime == Eigen::Dynamic ||
                           Vector2<ScalarType>::RowsAtCompileTime == Eigen::Dynamic) ? Eigen::Dynamic :
                          Vector1<ScalarType>::RowsAtCompileTime + Vector2<ScalarType>::RowsAtCompileTime
    };
  ...
  */
  ...
  namespace internal {
    template<template<typename> class Vector1,
            template<typename> class Vector2, bool Vec2IsNull = false>
    struct CombinedVectorHelper {
      ...
      static const Vector1<ScalarType> &first(
              const CombinedVector<ScalarType, Vector1, Vector2> &vec) { return vec.first(); }

      template<typename ScalarType>
      static const Vector2<ScalarType> &second(
              const CombinedVector<ScalarType, Vector1, Vector2> &vec) { return vec.second(); }
    ...
    struct CombinedVectorHelper<Vector1, Vector2, true> {
      ...
      template<typename ScalarType>
      static const Vector1<ScalarType> &first(const Vector1<ScalarType> &vec) { return vec; }

      template<typename ScalarType>
      static NullVector <ScalarType> second(const Vector1<ScalarType> &vec) { return NullVector<ScalarType>(); }

      template<typename ScalarType>
      static Vector1<ScalarType> combine(const Vector1<ScalarType> &vec1,
                                         const Vector2<ScalarType> &vec2) { return vec1; };
    };
  }
   ...
   */
  template<template<typename> class Vector1,
          template<typename> class Vector2, typename Vec1IsNull = void>
  struct CombinedVectorUtil {
    // Vec1 is non-empty (Vec2 could be either)
    typedef typename internal::template CombinedVectorHelper<Vector1, Vector2,
            Vector2<double>::RowsAtCompileTime == 0> helper;
    template<typename ScalarType> using type = typename helper::template type<ScalarType>;

    template<typename ScalarType>
    static Vector1<ScalarType> first(const typename helper::template type<ScalarType> &vec) { return helper::first(vec); }
    ...
    template<typename ScalarType>
    static typename helper::template type<ScalarType> combine(const Vector1<ScalarType> &vec1,
                                                              const Vector2<ScalarType> &vec2) {
  ...
  struct CombinedVectorUtil<Vector1, Vector2, typename std::enable_if<Vector1<double>::RowsAtCompileTime == 0>::type> {
    ...
    template<typename ScalarType>
    static NullVector <ScalarType> first(const Vector2<ScalarType> &vec) { return NullVector<ScalarType>(); }

    template<typename ScalarType>
    static const Vector2<ScalarType> &second(const Vector2<ScalarType> &vec) { return vec; }

    template<typename ScalarType>
    static Vector2<ScalarType> combine(const Vector1<ScalarType> &vec1, const Vector2<ScalarType> &vec2) { return vec2; };
  };

...
def _get_required_helpers(scope_locals):
    ...
    _exec_check = _ExecCheck()
    push_exec_check = _exec_check.push
    pop_exec_check = _exec_check.pop
...
class CallPythonClient(object):
    ...
    def run(self):
        ...
        # Check `_exec_check`.
        exec_check_count = self.scope_globals['_exec_check'].count
        if not self._had_error and exec_check_count != 0:
            self._had_error = True
            sys.stderr.write(
                "ERROR: 'pop_exec_check' called insufficient number of " +
                "times: {}\n".format(exec_check_count))

...
class PiecewisePolynomial : public PiecewisePolynomialBase {
 ...
 public:
   ...
   * derivatives.
   *
   * If @p zero_end_point_derivatives is true, @p breaks and @p knots can have
   * only 2 elements. The result is equivalent to computing a cubic polynomial
   * whose values are given by @p knots, and derivatives set to zero.
   *
   * @throws std::runtime_error if
   *    `breaks` and `knots` have different length,
   *    `breaks` is not strictly increasing,

...
GTEST_TEST(FindResourceTest, AlternativeDirectory) {
  ...
  // an empty file in a scratch directory. Bazel tests are run in a
  // scratch directory, so we don't need to remove anything manually.
  ...
  EXPECT_EQ(GetResourceSearchPaths()[0].value(), test_directory);
  EXPECT_NO_THROW(drake::FindResourceOrThrow(candidate_filename));
  // Check that `ClearResourceSearchPath()` empties the persistent variable.
  ClearResourceSearchPaths();
  EXPECT_THROW(drake::FindResourceOrThrow(candidate_filename), std::exception);
}

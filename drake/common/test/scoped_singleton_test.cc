 ...
 */
GTEST_TEST(SingletonScopeTest, NestedAndSpecializedTest) {
  weak_ptr<TrivialSingleton> wref;
  EXPECT_EQ(0, wref.use_count());
  EXPECT_EQ(0, TrivialSingleton::instance_count());
  {
    auto ref_1 = GetScopedSingleton<TrivialSingleton>();
    wref = ref_1;
    EXPECT_EQ(1, wref.use_count());
    EXPECT_EQ(1, TrivialSingleton::instance_count());
    {
      auto ref_2 = GetScopedSingleton<TrivialSingleton>();
      EXPECT_EQ(2, wref.use_count());
      EXPECT_EQ(1, TrivialSingleton::instance_count());
      // Use specialized version.
      auto ref_a = GetScopedSingleton<TrivialSingleton, Specialized>();
      EXPECT_EQ(1, ref_a.use_count());
      // Original singleton's reference count should not change.
      EXPECT_EQ(2, wref.use_count());
      // We should have two unique TrivialSingleton instances.

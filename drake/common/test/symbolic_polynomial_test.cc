...
TEST_F(SymbolicPolynomialTest, DifferentiateJacobian) {
  // p = 2a²bx² + 3bc²x + 7ac.
  const Polynomial p{
      2 * a_ * a_ * b_ * x_ * x_ + 3 * b_ * c_ * c_ * x_ + 7 * a_ * c_,
      {var_a_, var_b_, var_c_}};

  // d/dx p = 4a²bx + 3bc²
  const Polynomial p_x{4 * a_ * a_ * b_ * x_ + 3 * b_ * c_ * c_,
                       {var_a_, var_b_, var_c_}};
  ...
  // d/da p = 4abx² + 7c
  const Polynomial p_a{4 * a_ * b_ * x_ * x_ + 7 * c_,
                       {var_a_, var_b_, var_c_}};
  EXPECT_PRED2(PolyEqual, p.Differentiate(var_a_), p_a);

  // d/db p = 2a²x² + 3c²x
  const Polynomial p_b{2 * a_ * a_ * x_ * x_ + 3 * c_ * c_ * x_,
                       {var_a_, var_b_, var_c_}};
  ...
  const Eigen::Matrix<Polynomial, 1, 2> J_xy{p.Jacobian(vars_xy)};

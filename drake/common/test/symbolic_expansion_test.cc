...
TEST_F(SymbolicExpansionTest, ExpressionAlreadyExpandedPolynomial) {
  ...
  EXPECT_TRUE(CheckExpandIsFixpoint(0));
  EXPECT_TRUE(CheckExpandIsFixpoint(1));
  EXPECT_TRUE(CheckExpandIsFixpoint(-1));
  EXPECT_TRUE(CheckExpandIsFixpoint(42));
  EXPECT_TRUE(CheckExpandIsFixpoint(-5));
  EXPECT_TRUE(CheckExpandIsFixpoint(x_));
  EXPECT_TRUE(CheckExpandIsFixpoint(-x_));
  EXPECT_TRUE(CheckExpandIsFixpoint(3 * x_));
  EXPECT_TRUE(CheckExpandIsFixpoint(-2 * x_));
  EXPECT_TRUE(CheckExpandIsFixpoint(3 * x_ * y_));               // 3xy
  EXPECT_TRUE(CheckExpandIsFixpoint(3 * pow(x_, 2) * y_));       // 3x^2y
  EXPECT_TRUE(CheckExpandIsFixpoint(3 * pow(x_, 2) * y_ / 10));  // 3x^2y / 10
  EXPECT_TRUE(CheckExpandIsFixpoint(-7 + x_ + y_));              // -7 + x + y
  EXPECT_TRUE(CheckExpandIsFixpoint(1 + 3 * x_ - 4 * y_));       // 1 + 3x -4y
}

TEST_F(SymbolicExpansionTest, ExpressionAlreadyExpandedPow) {
  // The followings are all already expanded.
  EXPECT_TRUE(CheckExpandIsFixpoint(pow(x_, y_)));            // x^y
  EXPECT_TRUE(CheckExpandIsFixpoint(pow(x_, -1)));            // x^(-1)
  EXPECT_TRUE(CheckExpandIsFixpoint(pow(x_, -1)));            // x^(-1)
  EXPECT_TRUE(CheckExpandIsFixpoint(pow(x_ + y_, -1)));       // (x + y)^(-1)
  EXPECT_TRUE(CheckExpandIsFixpoint(pow(x_ + y_, 0.5)));      // (x + y)^(0.5)
  EXPECT_TRUE(CheckExpandIsFixpoint(pow(x_ + y_, 2.5)));      // (x + y)^(2.5)
  EXPECT_TRUE(CheckExpandIsFixpoint(pow(x_ + y_, x_ - y_)));  // (x + y)^(x - y)
}

...
void CheckTraits(bool is_vector) {
  std::cout << NiceTypeName::Get<Derived>() << std::endl;
  EXPECT_TRUE(detail::is_eigen_type<Derived>::value);
  EXPECT_TRUE((detail::is_eigen_scalar_same<Derived, double>::value));
  EXPECT_EQ(is_vector, (detail::is_eigen_vector_of<Derived, double>::value));
  EXPECT_EQ(!is_vector, (detail::is_eigen_matrix_of<Derived, double>::value));
}
...
template <typename T,
          typename Cond = typename std::enable_if<
              detail::is_eigen_type<T>::value>::type>
...
template <typename T,
          typename Cond = typename std::enable_if<
              detail::is_eigen_scalar_same<T, double>::value>::type>
...
GTEST_TEST(EigenTypesTest, TraitsSFINAE) {
  ...
  // EXPECT_FALSE((detail::is_eigen_vector_of<std::string, double>::value));
}

...
void CheckTraits(bool is_vector) {
  ...
  EXPECT_TRUE(detail::is_eigen_type<Derived>::value) << info;
  EXPECT_TRUE((detail::is_eigen_scalar_same<Derived, double>::value)) << info;
  EXPECT_EQ(is_vector, (detail::is_eigen_vector_of<Derived, double>::value))
      << info;
  EXPECT_EQ(!is_vector, (detail::is_eigen_nonvector_of<Derived, double>::value))
      << info;
}

...
TEST_F(SymbolicMixingScalarTypesTest, MatrixAdditionExprVar) {
  const auto M1{M_expr_fixed_ + M_var_fixed_};
  const auto M2{M_expr_fixed_ + M_var_dyn_};
  const auto M3{M_expr_dyn_ + M_var_fixed_};
  const auto M4{M_expr_dyn_ + M_var_dyn_};
...
TEST_F(SymbolicMixingScalarTypesTest, MatrixAdditionExprDouble) {
  const auto M1{M_expr_fixed_ + M_double_fixed_};
  const auto M2{M_expr_fixed_ + M_double_dyn_};
  const auto M3{M_expr_dyn_ + M_double_fixed_};
  const auto M4{M_expr_dyn_ + M_double_dyn_};
  const string expected{
      "      (1 + x)       (2 + y)\n      (3 + z)       (4 + w)"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, MatrixAdditionVarExpr) {
  const auto M1{M_var_fixed_ + M_expr_fixed_};
  const auto M2{M_var_fixed_ + M_expr_dyn_};
  const auto M3{M_var_dyn_ + M_expr_fixed_};
  const auto M4{M_var_dyn_ + M_expr_dyn_};
  const string expected{
      "      (2 * x)       (2 * y)\n      (2 * z)       (2 * w)"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, MatrixAdditionVarDouble) {
  const auto M1{M_var_fixed_ + M_double_fixed_};
  const auto M2{M_var_fixed_ + M_double_dyn_};
  const auto M3{M_var_dyn_ + M_double_fixed_};
  const auto M4{M_var_dyn_ + M_double_dyn_};
  const string expected{
      "      (1 + x)       (2 + y)\n      (3 + z)       (4 + w)"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, MatrixAdditionDoubleExpr) {
  const auto M1{M_double_fixed_ + M_expr_fixed_};
  const auto M2{M_double_fixed_ + M_expr_dyn_};
  const auto M3{M_double_dyn_ + M_expr_fixed_};
  const auto M4{M_double_dyn_ + M_expr_dyn_};
  const string expected{
      "      (1 + x)       (2 + y)\n      (3 + z)       (4 + w)"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, MatrixAdditionDoubleVar) {
  const auto M1{M_double_fixed_ + M_var_fixed_};
  const auto M2{M_double_fixed_ + M_var_dyn_};
  const auto M3{M_double_dyn_ + M_var_fixed_};
  const auto M4{M_double_dyn_ + M_var_dyn_};
  const string expected{
      "      (1 + x)       (2 + y)\n      (3 + z)       (4 + w)"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, MatrixVectorMultiplicationExprVar) {
  const auto M1{M_expr_fixed_ * V_var_fixed_};
  const auto M2{M_expr_fixed_ * V_var_dyn_};
  const auto M3{M_expr_dyn_ * V_var_fixed_};
  const auto M4{M_expr_dyn_ * V_var_dyn_};
  const string expected{
      "                      (pow(x, 2) + pow(y, 2))\n"
      "                      ((x * z) + (y * w))"};
  EXPECT_EQ(to_string(M1), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, VectorMatrixMultiplicationExprVar) {
  const auto M1{V_expr_fixed_.transpose() * M_var_fixed_};
  const auto M2{V_expr_fixed_.transpose() * M_var_dyn_};
  const auto M3{V_expr_dyn_.transpose() * M_var_fixed_};
  const auto M4{V_expr_dyn_.transpose() * M_var_dyn_};
  const string expected{
      "                    ((y * z) + pow(x, 2))"
      "                     ((x * y) + (y * w))"};
  EXPECT_EQ(to_string(M1), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, VectorVectorMultiplicationExprVar) {
  const auto M1{V_expr_fixed_.transpose() * V_var_fixed_};
  const auto M2{V_expr_fixed_.transpose() * V_var_dyn_};
  const auto M3{V_expr_dyn_.transpose() * V_var_fixed_};
  const auto M4{V_expr_dyn_.transpose() * V_var_dyn_};
...
TEST_F(SymbolicMixingScalarTypesTest, MatrixVectorMultiplicationVarExpr) {
  const auto M1{M_var_fixed_ * V_expr_fixed_};
  const auto M2{M_var_fixed_ * V_expr_dyn_};
  const auto M3{M_var_dyn_ * V_expr_fixed_};
  const auto M4{M_var_dyn_ * V_expr_dyn_};
  const string expected{
      "                      (pow(x, 2) + pow(y, 2))\n"
      "                      ((x * z) + (y * w))"};
  EXPECT_EQ(to_string(M1), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, VectorMatrixMultiplicationVarExpr) {
  const auto M1{V_var_fixed_.transpose() * M_expr_fixed_};
  const auto M2{V_var_fixed_.transpose() * M_expr_dyn_};
  const auto M3{V_var_dyn_.transpose() * M_expr_fixed_};
  const auto M4{V_var_dyn_.transpose() * M_expr_dyn_};
  const string expected{
      "                    ((y * z) + pow(x, 2))"
      "                     ((x * y) + (y * w))"};
  EXPECT_EQ(to_string(M1), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, VectorVectorMultiplicationVarExpr) {
  const auto M1{V_var_fixed_.transpose() * V_expr_fixed_};
  const auto M2{V_var_fixed_.transpose() * V_expr_dyn_};
  const auto M3{V_var_dyn_.transpose() * V_expr_fixed_};
  const auto M4{V_var_dyn_.transpose() * V_expr_dyn_};
  const string expected{"                      (pow(x, 2) + pow(y, 2))"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
  EXPECT_EQ(to_string(M3), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, MatrixVectorMultiplicationExprDouble) {
  const auto M1{M_expr_fixed_ * V_double_fixed_};
  const auto M2{M_expr_fixed_ * V_double_dyn_};
  const auto M3{M_expr_dyn_ * V_double_fixed_};
  const auto M4{M_expr_dyn_ * V_double_dyn_};
...
TEST_F(SymbolicMixingScalarTypesTest, VectorMatrixMultiplicationExprDouble) {
  const auto M1{V_expr_fixed_.transpose() * M_double_fixed_};
  const auto M2{V_expr_fixed_.transpose() * M_double_dyn_};
  const auto M3{V_expr_dyn_.transpose() * M_double_fixed_};
  const auto M4{V_expr_dyn_.transpose() * M_double_dyn_};
  const string expected{
      "              (x + 3 * y)               (2 * x + 4 * y)"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, VectorVectorMultiplicationExprDouble) {
  const auto M1{V_expr_fixed_.transpose() * V_double_fixed_};
  const auto M2{V_expr_fixed_.transpose() * V_double_dyn_};
  const auto M3{V_expr_dyn_.transpose() * V_double_fixed_};
  const auto M4{V_expr_dyn_.transpose() * V_double_dyn_};
  const string expected{"          (x + 2 * y)"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
  EXPECT_EQ(to_string(M3), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, MatrixVectorMultiplicationDoubleExpr) {
  const auto M1{M_double_fixed_ * V_expr_fixed_};
  const auto M2{M_double_fixed_ * V_expr_dyn_};
  const auto M3{M_double_dyn_ * V_expr_fixed_};
  const auto M4{M_double_dyn_ * V_expr_dyn_};
  const string expected{
      "              (x + 2 * y)\n              (3 * x + 4 * y)"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, VectorMatrixMultiplicationDoubleExpr) {
  const auto M1{V_double_fixed_.transpose() * M_expr_fixed_};
  const auto M2{V_double_fixed_.transpose() * M_expr_dyn_};
  const auto M3{V_double_dyn_.transpose() * M_expr_fixed_};
  const auto M4{V_double_dyn_.transpose() * M_expr_dyn_};
...
TEST_F(SymbolicMixingScalarTypesTest, VectorVectorMultiplicationDoubleExpr) {
  const auto M1{V_double_fixed_.transpose() * V_expr_fixed_};
  const auto M2{V_double_fixed_.transpose() * V_expr_dyn_};
  const auto M3{V_double_dyn_.transpose() * V_expr_fixed_};
  const auto M4{V_double_dyn_.transpose() * V_expr_dyn_};
  const string expected{"          (x + 2 * y)"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
  EXPECT_EQ(to_string(M3), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, MatrixVectorMultiplicationVarDouble) {
  const auto M1{M_var_fixed_ * V_double_fixed_};
  const auto M2{M_var_fixed_ * V_double_dyn_};
  const auto M3{M_var_dyn_ * V_double_fixed_};
  const auto M4{M_var_dyn_ * V_double_dyn_};
  const string expected{"          (x + 2 * y)\n          (z + 2 * w)"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
  EXPECT_EQ(to_string(M3), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, VectorMatrixMultiplicationVarDouble) {
  const auto M1{V_var_fixed_.transpose() * M_double_fixed_};
  const auto M2{V_var_fixed_.transpose() * M_double_dyn_};
  const auto M3{V_var_dyn_.transpose() * M_double_fixed_};
  const auto M4{V_var_dyn_.transpose() * M_double_dyn_};
  const string expected{
      "              (x + 3 * y)               (2 * x + 4 * y)"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, VectorVectorMultiplicationVarDouble) {
  const auto M1{V_var_fixed_.transpose() * V_double_fixed_};
  const auto M2{V_var_fixed_.transpose() * V_double_dyn_};
  const auto M3{V_var_dyn_.transpose() * V_double_fixed_};
  const auto M4{V_var_dyn_.transpose() * V_double_dyn_};
  const string expected{"          (x + 2 * y)"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
  EXPECT_EQ(to_string(M3), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, MatrixVectorMultiplicationDoubleVar) {
  const auto M1{M_double_fixed_ * V_var_fixed_};
  const auto M2{M_double_fixed_ * V_var_dyn_};
  const auto M3{M_double_dyn_ * V_var_fixed_};
  const auto M4{M_double_dyn_ * V_var_dyn_};
  const string expected{
      "              (x + 2 * y)\n              (3 * x + 4 * y)"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, VectorMatrixMultiplicationDoubleVar) {
  const auto M1{V_double_fixed_.transpose() * M_var_fixed_};
  const auto M2{V_double_fixed_.transpose() * M_var_dyn_};
  const auto M3{V_double_dyn_.transpose() * M_var_fixed_};
  const auto M4{V_double_dyn_.transpose() * M_var_dyn_};
  const string expected{"          (x + 2 * z)           (y + 2 * w)"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
  EXPECT_EQ(to_string(M3), expected);
...
TEST_F(SymbolicMixingScalarTypesTest, VectorVectorMultiplicationDoubleVar) {
  const auto M1{V_double_fixed_.transpose() * V_var_fixed_};
  const auto M2{V_double_fixed_.transpose() * V_var_dyn_};
  const auto M3{V_double_dyn_.transpose() * V_var_fixed_};
  const auto M4{V_double_dyn_.transpose() * V_var_dyn_};
  const string expected{"          (x + 2 * y)"};
  EXPECT_EQ(to_string(M1), expected);
  EXPECT_EQ(to_string(M2), expected);
  EXPECT_EQ(to_string(M3), expected);

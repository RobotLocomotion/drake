...
TEST_F(SymbolicDifferentiationTest, SymbolicBaseCases) {
  ...
  // d/dx (2 + x + y + z) = 1
  check_list.emplace_back(2 + x, var_x_, 1);
  // d/dx (2 + 3*x) = 3
  ...
  // d/dx pow(x, 5) = 5 * pow(x, 4)
  check_list.emplace_back(pow(x, 5), var_x_, 5 * pow(x, 4));
  // d/dx pow(5, x) = pow(5, x) * log(5)
...
TEST_F(SymbolicDifferentiationTest, NumericalTests) {
  const Expression& x{var_x_};
  const Expression& y{var_y_};
  const Expression& z{var_z_};
  ...
  fns.push_back(cos(x));
  fns.push_back(tan(x));
  fns.push_back(asin(x));
  fns.push_back(acos(x));
  ...
  fns.push_back(log(sin(x) + cos(x)));
  fns.push_back(exp(cos(x) / 10 + tan(y) / 10));
  fns.push_back(sqrt(tan(x)));
    ...
    for (const Environment& env : envs) {
      for (const Variable& var : vars) {
        try {
          ...
          // ignore domain_error/runtime_error
          continue;
        }
      }
    }

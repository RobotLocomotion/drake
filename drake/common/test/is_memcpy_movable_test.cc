...
class MemcpyMovable {
 public:
  MemcpyMovable(int id, string name) : id_{id}, name_{new string{move(name)}} {}
...
GTEST_TEST(IsMemcpyMovableTest, MemcpyMovableTestClass) {
  ...
  EXPECT_FALSE(IsMemcpyMovable(v, MemcpyMovable::EqualTo()));
  EXPECT_FALSE(IsMemcpyMovable(move(v), MemcpyMovable::EqualTo()));
}

 ...
 * m. If @p is_lower is true, it's using the lower-triangular part of @p m to
 * construct a positive-semidefinite formula. Otherwise, the upper-triangular
 * part of @p m is used. See the following code snippet.
 *
 * @code
 ...
 *      3.0, 4.0;
 *
 * const Formula psd_l{positive_semidefinite_from_triangular(m, true)};
 * // psd_l includes [1.0 3.0]
 * //                [3.0 4.0].
 *
 * const Formula psd_u{positive_semidefinite_from_triangular(m, false)};
 * // psd_u includes [1.0 2.0]
 * //                [2.0 4.0].
 * @endcode
 */
Formula positive_semidefinite_from_triangular(const MatrixX<Expression>& m,
                                              bool is_lower);
...
positive_semidefinite(const Eigen::TriangularView<Derived, Eigen::Lower>& l) {
  return positive_semidefinite_from_triangular(l, true);
}
...
positive_semidefinite(const Eigen::TriangularView<Derived, Eigen::Upper>& u) {
  return positive_semidefinite_from_triangular(u, false);
}

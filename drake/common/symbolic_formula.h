...
/// +--------------------------------------------------------------+
/// |      ==        | EA<Expression> | EA<Variable> | EA<double>  |
/// -----------------+----------------+--------------+--------------
/// | EA<Expression> | EA<Formula>    | EA<Formula>  | EA<Formula> |
/// -----------------+----------------+--------------+--------------
/// | EA<Variable>   | EA<Formula>    | EA<bool>     | EA<Formula> |
/// -----------------+----------------+--------------+--------------
...
/// following two cases. They return `Eigen::Array<bool>` and are provided by
/// Eigen.
///
///    - Eigen::Array<Variable> == Eigen::Array<Variable>
...
typename std::enable_if<
    std::is_base_of<Eigen::ArrayBase<DerivedA>, DerivedA>::value &&
        ...
        // Case: EA<Expr> == EA<{Expr, Var, double}>
...
/// Returns a symbolic formula checking if two matrices @p
/// m1 and @p m2 are equal.
///
/// The following table describes the return type of @p m1 == @p m2.
/// +-------------------------------------------------------------+
/// |      ==        | EM<Expression> | EM<Variable> | EM<double> |
/// -----------------+----------------+--------------+-------------
/// | EM<Expression> | Formula        | Formula      | Formula    |
/// -----------------+----------------+--------------+-------------
/// | EM<Variable>   | Formula        | bool         | Formula    |
/// -----------------+----------------+--------------+-------------
...
///
/// Note that this function does *not* provide operator overloading for the
/// following two cases. They return `bool` and are provided by Eigen.
///
///    - Eigen::Matrix<Variable> == Eigen::Matrix<Variable>
...
typename std::enable_if<
    std::is_base_of<Eigen::MatrixBase<DerivedA>, DerivedA>::value &&
        ...
        // Case: EM<Expr> == EM<{Expr, Var, double}>
        ((std::is_same<typename DerivedA::Scalar, Expression>::value &&
          (std::is_same<typename DerivedB::Scalar, Expression>::value ||
           std::is_same<typename DerivedB::Scalar, Variable>::value ||
           std::is_same<typename DerivedB::Scalar, double>::value)) ||
         // Case: EM<{Expr, Var, double}> == EM<Expr>
         ((std::is_same<typename DerivedA::Scalar, Expression>::value ||
           std::is_same<typename DerivedA::Scalar, Variable>::value ||
           std::is_same<typename DerivedA::Scalar, double>::value) &&
          std::is_same<typename DerivedB::Scalar, Expression>::value) ||
         // Case: EM<Var> == EM<double>
         (std::is_same<typename DerivedA::Scalar, Variable>::value &&
          std::is_same<typename DerivedB::Scalar, double>::value) ||
         // Case: EM<double> == EM<Var>
         (std::is_same<typename DerivedA::Scalar, double>::value &&
          std::is_same<typename DerivedB::Scalar, Variable>::value)),
    Formula>::type
operator==(const DerivedA& m1, const DerivedB& m2) {

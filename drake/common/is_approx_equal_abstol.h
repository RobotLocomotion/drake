...
/// Returns true if and only if the two matrices are equal to within a certain
/// absolute elementwise @p tolerance, given a permutation of the columns, e.g.
/// there exists a P such that
/// <pre>
///    forall i,j,  |m1*P - m2|_{i,j} <= tolerance
...
bool IsApproxEqualAbsTolWithPermutedColumns(
  ...
  for (int i = 0; i < m1.cols(); i++) {
    ...
    if (!found_unique_match) return false;
  }
  return true;
}

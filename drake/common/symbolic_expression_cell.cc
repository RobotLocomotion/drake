...
Expression DifferentiatePow(const Expression& f, const Expression& g,
  ...
  if (is_constant(g)) {
    ...
    //     d/dx pow(n, g) = log(n) * pow(n, g) * d/dx g
...
Expression ExpressionMul::Differentiate(const Variable& x) const {
  ...
  //= c * [expr * (d/dx f_1^g_1) / f_1^g_1 +
  //       expr * (d/dx f_2^g_2) / f_2^g_2 +
  //                      ...              +
  //       expr * (d/dx f_n^g_n) / f_n^g_n]
  //
  // where expr = (f_1^g_1 * f_2^g_2 * ... * f_n^g_n).
  const map<Expression, Expression>& m{base_to_expnt_map_};
  Expression ret{Expression::Zero()};
  const Expression expr{
      ExpressionMulFactory{1.0, base_to_expnt_map_}.GetExpression()};
  for (auto it1 = m.begin(); it1 != m.end(); ++it1) {
    const Expression& base{it1->first};
    const Expression& expnt{it1->second};
    ret += expr * DifferentiatePow(base, expnt, x) * pow(base, -expnt);
  }

...
/** Smart pointer with deep copy semantics.
 ...
 uses a default deleter. Second, there is no array version, Third, to allow
 future copy-on-write optimizations, there is a distinction between writable and
 const access, the get() method is modified to return only a const pointer,
 with get_mutable() added to return a writable pointer. There are also
 corresponding get_ref() and get_mutable_ref() with the same semantics.
 ...
 @subsection Usage without polymorphism

 In many typical use cases, the specialized type of pointer will match the type
 of object it references, e.g.:
 @code
 copyable_unique_ptr<Foo> ptr = make_unique<Foo>(...);
 @endcode
 In this case, as long a `Foo` is deemed compatible, the behavior will be as
 expected.

 @subsection Usage _with_ polymorphism

 There will be cases, where the specialized type of pointer will be different
 ...
 `Base` and `Foo` in order for this to be compilable.
   ...
   1. If the `Base` class has a _public_ copy constructor, that copy constructor
      ...
      an instance of `Base`.
   2. As noted in @ef is_cloneable_doc "is_cloneable", both the `Base` and
 ...
 %copyable_unique_ptr in a polymorphic context:
  ...
  struct Derived {
    ...
    // This declaration hides Base::Clone().

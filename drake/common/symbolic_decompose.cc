...
void DecomposeLinearExpressions(
  ...
  DRAKE_DEMAND(M1.rows() == expressions.rows() && M1.cols() == vars1.rows());
  DRAKE_DEMAND(M2.rows() == expressions.rows() && M2.cols() == vars2.rows());
  for (int i = 0; i < expressions.size(); ++i) {
    const Expression& e{expressions(i)};
    if (!e.is_polynomial()) {
      ThrowError("non-polynomial", e.to_string());  // e should be a polynomial.
    }
    const Variables indeterminates{Variables{vars1} + Variables{vars2}};
    DRAKE_DEMAND(indeterminates.size() ==
                 static_cast<size_t>(vars1.size() + vars2.size()));
    const Polynomial p{e, indeterminates};
    if (p.TotalDegree() > 1) {
    ...
    for (int j = 0; j < vars1.size(); ++j) {
      FindCoefficientAndFill(map, Monomial{vars1.coeff(j)}, j, M1.row(i));
    }
    // Fill M2(i, j).
    for (int j = 0; j < vars2.size(); ++j) {
      FindCoefficientAndFill(map, Monomial{vars2.coeff(j)}, j, M2.row(i));
    }
  }
}

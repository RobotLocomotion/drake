...
Expression if_then_else(const Formula& f_cond, const Expression& e_then,
                        const Expression& e_else);
/** Constructs an uninterpreted-function expression with @p name and @p vars. An
 * uninterpreted function is identified by its name and the set of variables
 * attached to it. */
 ...
 */
const std::string& get_name(const Expression& e);
/** Returns the variables in an uninterpreted-function expression @p e.
 *  \pre{@p e is an uninterpreted-function expression.}
 */
const std::vector<Variable>& get_arguments(const Expression& e);

...
class unique_ptr_ref {
 ...
 public:
  ...
  /** Move construction from a compatible `%unique_ptr_ref`. Type `U*` must be
  implicitly convertible to type `T*`. Ownership is transferred from the source
  source to the new `%unique_ptr_ref`. If the source was empty this one will be
  empty also. No heap activity occurs. **/
  template <class U>
  ...
  unique_ptr_ref& operator=(const T& x) {
    reset(cloneOrNull(&x));
    return *this;
  }
  ...
  unique_ptr_ref& operator=(T* p) noexcept {
    reset(p);
    return *this;
  }
  /**@}**/
  ...
  /** Return a pointer to the contained object if any, or `nullptr`. **/
  ...
  object. Behavior is undefined if the container is empty. **/
  ...
  be empty. **/
  std::unique_ptr<T> move() { return std::unique_ptr<T>(release()); }

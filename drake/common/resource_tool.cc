...
DEFINE_string(
    set_root_directory, "",
    "Set root directory in which the resources live. This directory will be"
    "search after the environment variable but before the directory in which"
...
int main(int argc, char* argv[]) {
  ...
  const int num_commands =
      ...
      (!FLAGS_set_root_directory.empty()
      && FLAGS_print_resource_root_environment_variable_name ? 1 : 0);
  if (num_commands != 1) {
  ...
  if (!FLAGS_set_root_directory.empty()) {
    SetRootDirectory(FLAGS_set_root_directory);
  }

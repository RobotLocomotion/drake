...
inline AutoDiffScalar<NewDerType> MakeAutoDiffScalar(
 ...
 *
 * \param _DerType the vector type used to store/represent the derivatives. The
 * base scalar type as well as the number of derivatives to compute are
 * determined from this type. Typical choices include, e.g., \c
 * Vector4f for 4 derivatives, or \c VectorXf if the number of
 * derivatives is not known at compile time, and/or, the number
 *                 of derivatives is large.
 *                 Note that _DerType can also be a reference (e.g., \c
 * VectorXf&) to wrap a existing vector into an AutoDiffScalar. Finally,
 * _DerType can also be any Eigen compatible expression.
 *
 * This class represents a scalar value while tracking its respective
 * derivatives using Eigen's expression template mechanism.
 *
 * It supports the following list of global math function:
 *  - std::abs, std::sqrt, std::pow, std::exp, std::log, std::sin, std::cos,
 *  - internal::abs, internal::sqrt, numext::pow, internal::exp, internal::log,
 * internal::sin, internal::cos,
 *  - internal::conj, internal::real, internal::imag, numext::abs2.
 *
 * AutoDiffScalar can be used as the scalar type of an Eigen::Matrix object.
 * However, in that case, the expression template mechanism only occurs at the
 * top Matrix level, while derivatives are computed right away.
 *
 */
...
class AutoDiffScalar
    : public internal::auto_diff_special_op<
          ...
          !internal::is_same<
              typename internal::traits<
                  typename internal::remove_all<_DerType>::type>::Scalar,
 ...
 public:
  typedef internal::auto_diff_special_op<
      ...
      !internal::is_same<
          typename internal::traits<
              typename internal::remove_all<_DerType>::type>::Scalar,
          typename NumTraits<typename internal::traits<
              typename internal::remove_all<_DerType>::type>::Scalar>::Real>::
          value>
      Base;
  ...
  /** Constructs an active scalar from its \a value,
      and initializes the \a nbDer derivatives such that it corresponds to the
     \a derNumber -th variable */
  ...
  //     inline const AutoDiffScalar<DerType&> operator+(const Real& other)
  //     const
  //     {
  //       return AutoDiffScalar<DerType&>(m_value + other, m_derivatives);
  //     }
  ...
  operator+(const AutoDiffScalar<OtherDerType>& other) const {
    ...
    return AutoDiffScalar<CwiseBinaryOp<
        internal::scalar_sum_op<Scalar>, const DerType,
        const typename internal::remove_all<OtherDerType>::type> >(
        m_value + other.value(), m_derivatives + other.derivatives());
  }
  ...
  operator-(const AutoDiffScalar<OtherDerType>& other) const {
    internal::make_coherent(m_derivatives, other.derivatives());
    return AutoDiffScalar<CwiseBinaryOp<
        internal::scalar_difference_op<Scalar>, const DerType,
        const typename internal::remove_all<OtherDerType>::type> >(
        m_value - other.value(), m_derivatives - other.derivatives());
  }
  ...
  //     inline const AutoDiffScalar<typename
  //     CwiseUnaryOp<internal::scalar_multiple_op<Real>, DerType>::Type >
  //     operator*(const Real& other) const
  //     {
  //       return AutoDiffScalar<typename
  //       CwiseUnaryOp<internal::scalar_multiple_op<Real>, DerType>::Type >(
  //         m_value * other,
  //         (m_derivatives * other));
  //     }
  ...
  inline const AutoDiffScalar<EIGEN_EXPR_BINARYOP_SCALAR_RETURN_TYPE(
      CwiseBinaryOp<
          internal::scalar_difference_op<Scalar> EIGEN_COMMA const
              EIGEN_EXPR_BINARYOP_SCALAR_RETURN_TYPE(DerType, Scalar, product)
  ...
  template <typename OtherDerType>
  inline const AutoDiffScalar<CwiseBinaryOp<
...
//                            is_same<Scalar,typename
//                            NumTraits<Scalar>::Real>::value>
{
  ...
  //                            is_same<Scalar,typename
  //                            NumTraits<Scalar>::Real>::value> Base;
  ...
  inline const AutoDiffScalar<DerType&> operator+(const Real& other) const {
    return AutoDiffScalar<DerType&>(derived().value() + other,
                                    derived().derivatives());
  }
  ...
  operator*(const Real& other) const {
    return AutoDiffScalar<typename CwiseUnaryOp<
        bind2nd_op<scalar_product_op<Scalar, Real> >, DerType>::Type>(
...
template <typename A_Scalar, int A_Rows, int A_Cols, int A_Options,
          int A_MaxRows, int A_MaxCols, typename B>
struct make_coherent_impl<
    Matrix<A_Scalar, A_Rows, A_Cols, A_Options, A_MaxRows, A_MaxCols>, B> {
...
template <typename A, typename B_Scalar, int B_Rows, int B_Cols, int B_Options,
          int B_MaxRows, int B_MaxCols>
struct make_coherent_impl<
...
template <typename A_Scalar, int A_Rows, int A_Cols, int A_Options,
          int A_MaxRows, int A_MaxCols, typename B_Scalar, int B_Rows,
          int B_Cols, int B_Options, int B_MaxRows, int B_MaxCols>
struct make_coherent_impl<
...
// The following is an attempt to let Eigen's known about expression template,
// but that's more tricky!

// template<typename DerType, typename BinOp>
// struct ScalarBinaryOpTraits<AutoDiffScalar<DerType>,AutoDiffScalar<DerType>,
// BinOp>
// {
//   enum { Defined = 1 };
...
// struct
// ScalarBinaryOpTraits<AutoDiffScalar<DerType1>,AutoDiffScalar<DerType2>,
// BinOp>
// {
//   enum { Defined = 1 };//internal::is_same<typename DerType1::Scalar,typename
//   DerType2::Scalar>::value }; typedef AutoDiffScalar<typename
//   DerType1::PlainObject> ReturnType;
// };
...
pow(const Eigen::AutoDiffScalar<DerType>& x,
    const typename internal::traits<
        typename internal::remove_all<DerType>::type>::Scalar& y) {
  using namespace Eigen;
  using std::pow;
...
inline const AutoDiffScalar<
    Matrix<typename internal::traits<
               typename internal::remove_all<DerTypeA>::type>::Scalar,
           Dynamic, 1> >
atan2(const AutoDiffScalar<DerTypeA>& a, const AutoDiffScalar<DerTypeB>& b) {
  using std::atan2;
  typedef typename internal::traits<
      typename internal::remove_all<DerTypeA>::type>::Scalar Scalar;

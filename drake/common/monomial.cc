 ...
 public:
  ...
  int operator()(const shared_ptr<ExpressionAdd>& e,
    ...
    for (const auto& p : e->get_expr_to_coeff_map()) {
      degree = std::max(degree, Degree(p.first, vars));
    }
    return degree;
  }
  ...
  int operator()(const shared_ptr<ExpressionDiv>& e,
                 const Variables& vars) const {
    return Degree(e->get_first_argument(), vars) -
           Degree(e->get_second_argument(), vars);
  }
  ...
  int operator()(const shared_ptr<ExpressionPow>& e,
    ...
    return Degree(e->get_first_argument(), vars) * exponent;
  }
};

int Degree(const Expression& e, const Variables& vars) {
  return visit_polynomial<int>(DegreeVisitor(), e, vars);
}

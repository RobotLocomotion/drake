...
GTEST_TEST(DrakeMockLcmTest, DecodeLastPublishedMessageAsTest) {
  ...
  vector<uint8_t> original_message_bytes;
  original_message_bytes.resize(message_size);
  original_message.encode(original_message_bytes.data(), 0,
                          original_message_bytes.size());

  // Instantiates the Device Under Test (DUT).
  DrakeMockLcm dut;
  dut.StartReceiveThread();
  dut.Publish(channel_name, &original_message_bytes[0], message_size);

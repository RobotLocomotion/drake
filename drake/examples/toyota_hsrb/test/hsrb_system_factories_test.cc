...
GTEST_TEST(ToyotaHsrbTests, TestDiagramFactories) {
  ...
  unique_ptr<Diagram<double>> dut2;
  {
    ...
    ASSERT_EQ(dut1->get_connected(visualizer_input_port_id),
              plant_output_port_zero);

    // Verifies that the Diagram has one input port and that it's connected to
    // the plant's input port.
    ASSERT_EQ(dut1->get_num_input_ports(), 1);
    ASSERT_TRUE(dut1->has_input(plant_input_port));

    // Verifies that the plant's output port is connected to the Diagram's
    // output port.
    ASSERT_EQ(dut1->get_num_output_ports(), 1);
    ASSERT_TRUE(dut1->has_output(plant_output_port_zero));
  ...
  ASSERT_EQ(dut2->get_output_port(0).get_size(), kNumStates);
  ASSERT_EQ(plant->get_num_states(), kNumStates);
}

 ...
 *  This class inherits general functionality and storage from
 *  DrakeShapes::HeightMapTerrain
 */
class SinusoidalTerrain : public DrakeShapes::HeightMapTerrain {
 public:
  /** \brief SinusoidalTerrain constructor
   *  Notice the call to HeightMapTerrain::finalize_loading_data the end of the
   *  constructor. This call MUST be performed.
   *  \sa DrakeShapes::HeightMapTerrain
   *  \param name This is only used to generate a file name where the height map
   *  is written as a triangular mesh for director to render.
   *  The file name will be <name>.obj.
   *  \param ncells Two dimensional vector of integers with the number of cells
   *  used to discretize the map in "local" x and y directions respectively.
   *  The number of cells MUST be a power of 2.
   *  \param size The provided array will be scaled so that the terrain is
   *  size(0) x size(1) in size.
  ...
  SinusoidalTerrain(const std::string& name, const Eigen::Vector2i& ncells,
    ...
    for (int i = 0; i < nnodes(0); ++i) {
      ...
      for (int j = 0; j < nnodes(1); ++j) {
        // double y = j*delta_ell(1); //y is not used and triggers a warning by
        // the compiler.
...
int main(int argc, char* argv[]) {
  ...
  // NOTE:
  // The next line ouptuts a dot file wchih allows to visualize the robot's
  // connectivity as a schematic.
  // The *.dot file can be converted to a *.png image file with: dot -Tpng
  // graphiviz_test.dot -o graphiviz_test.png

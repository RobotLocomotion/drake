...
VanDerPolOscillator<T>::VanDerPolOscillator() : systems::LeafSystem<T>() {
  ...
  systems::SystemConstraint<T>::CalcCallback mu = [](
      const systems::Context<T>& context, Eigen::VectorXd* value) {
    // Extract μ from the parameters.
    *value = Vector1d(context.get_numeric_parameter(0)->GetAtIndex(0));
  };
  this->DeclareInequalityConstraint(mu, 1, "mu ≥ 0"), 0);
}

...
template<typename T>
PoseTree<T>::PoseTree(const std::string& root_frame)
...
void PoseTree<T>::Update(
  ...
  rooted_map_[source_frame] = (
      rooted_map_.at(target_frame) * pose);
}

template<typename T>
Isometry3<T> PoseTree<T>::Transform(
    const std::string& target_frame,
...
template class PoseTree<double>;

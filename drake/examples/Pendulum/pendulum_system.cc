...
template <typename T>
std::unique_ptr<systems::SystemOutput<T>> PendulumSystem<T>::AllocateOutput(
    const systems::Context<T>& context) const {
  std::unique_ptr<systems::LeafSystemOutput<T>> output(
      new systems::LeafSystemOutput<T>);
  {
    std::unique_ptr<systems::BasicVector<T>> data(new PendulumStateVector<T>());
    std::unique_ptr<systems::OutputPort> port(
        new systems::OutputPort(std::move(data)));
    output->get_mutable_ports()->push_back(std::move(port));
  }
  return std::unique_ptr<systems::SystemOutput<T>>(output.release());
}
...
void PendulumSystem<T>::EvalOutput(const systems::Context<T>& context,
  ...
  PendulumStateVector<T>* output_vector = get_mutable_output(output);
  output_vector->set_theta(state.get_theta());
  output_vector->set_thetadot(state.get_thetadot());
}

...
class PendulumStateVector : public systems::BasicVector<T> {
 ...
 private:
  ...
  explicit PendulumStateVector(const systems::BasicVector<T>& other) = delete;
  PendulumStateVector& operator=(const systems::BasicVector<T>& other) = delete;
  explicit PendulumStateVector(systems::BasicVector<T>&& other) = delete;
  PendulumStateVector& operator=(systems::BasicVector<T>&& other) = delete;
};

/// A model of a one-dimensional pendulum systemm.
///
/// @tparam T The vector element type, which must be a valid Eigen scalar.
///
/// Instantiated templates for the following kinds of T's are provided:
...
class PendulumSystem : public systems::LeafSystem<T> {
 ...
 private:
  ...
  explicit PendulumSystem(const systems::LeafSystem<T>& other) = delete;
  PendulumSystem& operator=(const systems::LeafSystem<T>& other) = delete;
  explicit PendulumSystem(systems::LeafSystem<T>&& other) = delete;
  PendulumSystem& operator=(systems::LeafSystem<T>&& other) = delete;

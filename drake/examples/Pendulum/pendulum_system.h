...
/// - AutoDiffXd
template <typename T>
class DRAKEPENDULUMSYSTEM_EXPORT PendulumStateVector
    : public systems::BasicVector<T> {
 public:
...
/// - AutoDiffXd
template <typename T>
class DRAKEPENDULUMSYSTEM_EXPORT PendulumSystem
 ...
 public:
  ...
  T get_theta(const MyContext& context) const {
    return get_state(context).get_theta();
  }
  ...
  T get_thetadot(const MyContext& context) const {
    return get_state(context).get_thetadot();
  }
  ...
  // System<T> overrides.
  /// Allocates a single output port of type PendulumStateVector<T>.
 ...
 private:
  ...
  static const PendulumStateVector<T>& get_output(const MyOutput& output) {
    return dynamic_cast<const PendulumStateVector<T>&>(

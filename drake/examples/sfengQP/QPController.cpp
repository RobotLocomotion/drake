...
int QPController::control(
  ...
  VectorXd ci_u =
    VectorXd::Constant(11*nContacts, std::numeric_limits<double>::infinity());
  VectorXd ci_l =
    VectorXd::Constant(11*nContacts, -std::numeric_limits<double>::infinity());
  ...
  prog.AddQuadraticCost(
      ...
      input.w_pelv * (rs.pelv.Jdv-input.pelvdd_d).transpose()*rs.pelv.J, {qdd});
  ...
  for (auto cost_b : costs) {
    ...
    std::cout << "cost term 0.5 x^T * H * x + h0 * x: "
  ...
  for (int i = 0; i < nContacts; i++) {
    output.trq -=
      rs.foot[i]->J.block(0, 6, 6, nTrq).transpose() * output.foot_wrench_w[i];
  }
...
double QPOutput::computeCost(
    const HumanoidState &rs,
    const QPInput &input) const {
  VectorXd c, tot;
  c = 0.5*qdd.transpose()*input.w_com*rs.J_com.transpose()*rs.J_com*qdd
    + input.w_com*(rs.Jdv_com-input.comdd_d).transpose()*rs.J_com*qdd;
  tot = c;
  std::cout << "com cost: " << c << std::endl;

  c = 0.5*qdd.transpose()*input.w_pelv*rs.pelv.J.transpose()*rs.pelv.J*qdd
    + input.w_pelv*(rs.pelv.Jdv-input.pelvdd_d).transpose()*rs.pelv.J*qdd;
  tot += c;
  ...
  for (int i = 0; i < 2; i++) {
    c = 0.5*foot_wrench_w[i].transpose()*input.w_wrench_reg*foot_wrench_w[i]
      + input.w_wrench_reg*(-input.wrench_d[i]).transpose()*foot_wrench_w[i];
    tot += c;

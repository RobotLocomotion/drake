 ...
 * acceleration objective, \f$ \dot{v}_d \f$, for the qp inverse dynamics
 * controller. A way to generate \f$ \dot{v}_d \f$ is through a simple PD law
 * around a nominal setpoint \f$ (q^*, v^*, \dot{v}^*) \f$. This setpoint is
 * typically generated by some motion planner or simple heuristics.
 * More specifically:
 * \f[
 * \dot{v}_d = Kp (q^* - q) + Kd (v^* - v) + \dot{v}^*.
 * \f]
 * \f$ \dot{v}_d \f$ is then used in the quadratic cost function in the qp
 * inverse
 * dynamics controller. The cost function typically looks like:
 * \f[
 * min_{\dot{v}} (\dot{v}_d - \dot{v})^T W (\dot{v}_d - \dot{v}),
 * \f]
 * where \f$ W \f$ is a diagonal weighting matrix.
 * \f$ \dot{v}_d \f$ can also be treated as an equality constraint in the
 * optimization.
 * If weight(i) is positive, \f$ \dot{v}_d(i) \f$ is used in the cost function.
 * If weight(i) is negative, \f$ \dot{v}_d(i) \f$ is used in the equality
 * constraint. If weight(i) is zero, \f$ \dot{v}_d(i) \f$ is ignored.
 *
 * This struct holds Kp, Kd, and weight.
 */
 ...
 * ContactInformation which is used in the qp inverse dynamics controller.
 */
struct ContactParam {
  std::string name;
 ...
 * ContactInformation, DesiredBodyMotion and DesiredDoFMotion from the stored
...
class ParamSet {
 public:
   ...
   * so that these parameters can be decoupled from specific RigidBodyTree.
  ...
  /**
   * Makes a map from body names to ContactInformation, where the body names
   * belongs to the body group specified by @p group_name in @p alias_group.
   * For each body name, if it has no corresponding ContactParam, a
   * ContactInformation will be constructed using the default ContactParam.
   * If @p alias_group does not contain @p group_name, an empty map will be
   * returned.
   *
  ...
  /**
   * Makes a map from body names to DesiredBodyMotions, where the body names
   * belongs to the body group specified by @p group_name in @p alias_group.
   * This method only sets the weights and constraint_types fields of
   * DesiredBodyMotion. The values field need to be set separately.
   * For each body name, if it has no corresponding DesiredMotionParam,
   * a DesiredBodyMotion will be constructed using the default ContactParam.
   * If @p alias_group does not contain @p group_name, an empty map will be
   * returned.
   *
   * @param group_name Name of the body group of interest.
  ...
  /**
   * Makes a single ContactInformation for RigidBody @p body. If @p body has no
   ...
   * @return ContactInformation constructed from the ContactParam associated
   * with @p body.
   */
  ...
  /**
   * Makes a single DesiredBodyMotion for RigidBody @p body. This method only
   * sets the weights and constraint_types fields of DesiredBodyMotion. The
   * values field need to be set separately. If @p body has no corresponding
   * DesiredMotionParam, a DesiredBodyMotion constructed with the default
   * DesiredMotionParam will be returned.
   *
   * @param body RigidBody reference.
   * @return DesiredBodyMotion constructed from the DesiredMotionParam
   * associated with @p body.
   */
  DesiredBodyMotion MakeDesiredBodyMotion(const RigidBody<double>& body) const;

  /**
   * Finds the Kp and Kd gains for all the bodies in body group specified by
   * @p group_name. For each body in body group @p group_name, if it has no
  ...
  /**
   * Finds the Kp and Kd gains for RigidBody @p body. If it has no corresponding
   ...
   * @p Kp and @p Kd will be resized to math the number of DoFs of the
 ...
 private:
  ...
  double basis_regularization_weight_;

  // map from body_name (or "default") to ContactParam.
  std::unordered_map<std::string, ContactParam> contact_params_;
  // map from body_name (or "default") to DesiredMotionParam.
  ...
  ContactInformation MakeContactInformation(const RigidBody<double>& body,
                                            const ContactParam& param) const;

  DesiredBodyMotion MakeDesiredBodyMotion(

...
class System2PlanEval : public LeafSystem<double> {
 public:
  ...
  void EvalOutput(const Context<double> &context, SystemOutput<double>* output) const override {
    ...
    const example::qp_inverse_dynamics::HumanoidStatus *robot_status = EvalInputValue<example::qp_inverse_dynamics::HumanoidStatus>(context, input_port_num_humanoid_status_);

    // output: qp input
    example::qp_inverse_dynamics::QPInput& input = output->GetMutableData(output_port_num_qp_input_)->GetMutableValue<example::qp_inverse_dynamics::QPInput>();

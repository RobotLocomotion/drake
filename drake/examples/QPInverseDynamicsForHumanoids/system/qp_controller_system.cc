...
void QPControllerSystem::DoCalcOutput(
  ...
  qp_output = context.get_abstract_state<QpOutput>(kAbsStateIdxQpOutput);
...
void QPControllerSystem::DoCalcUnrestrictedUpdate(
  ...
  QpOutput& qp_output =
      get_mutable_value<QpOutput>(state, kAbsStateIdxQpOutput);
  ...
  lcmt_inverse_dynamics_debug_info& debug =
      get_mutable_value<lcmt_inverse_dynamics_debug_info>(state,
                                                          kAbsStateIdxDebug);
...
QPControllerSystem::AllocateAbstractState() const {
  ...
  abstract_vals[kAbsStateIdxQpOutput] = std::unique_ptr<systems::AbstractValue>(

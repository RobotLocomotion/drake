...
void KukaPlanEvalSystem::SetDesiredTrajectory(
  ...
  // Setup cartesian
  const RigidBody<double>* flange =
      alias_groups_.get_body_group("flange").front();
  VectorX<double> q;
  ...
  for (size_t i = 0; i < times.size(); ++i) {
    ...
    Isometry3<double> flange_pose =
        robot_.CalcBodyPoseInWorldFrame(cache, *flange);
    flange_pos[i] = flange_pose.translation();
    flange_rot[i] = flange_pose.linear();
  }
  plan.flange_pos_n_traj =
      PiecewisePolynomial<double>::FirstOrderHold(times, flange_pos);
...
void KukaPlanEvalSystem::DoCalcOutput(
    const systems::Context<double>& context,
    systems::SystemOutput<double>* output) const {
  // Gets the plan from abstract state.
  const KukaPlan& plan =
      context.get_abstract_state<KukaPlan>(abstract_state_plan_index_);
  ...
  // TODO(siyuan): this copying won't be necessary once this untire block is
  // moved to DoCalcUnrestrictedUpdate.
  ...
  qp_input.mutable_desired_dof_motions().mutable_values() =
      servo.ComputeTargetAcceleration(robot_status->position(),
  ...
  // Measured
  Isometry3<double> flange_pose =
      robot_.CalcBodyPoseInWorldFrame(robot_status->cache(), *flange);
  Vector6<double> flange_vel = robot_.CalcBodySpatialVelocityInWorldFrame(
      robot_status->cache(), *flange);
  // Desired
  flange_servo.mutable_desired_pose().translation() =
      plan.flange_pos_n_traj.value(time);
  flange_servo.mutable_desired_velocity().tail<3>() =
      plan.flange_pos_n_traj.derivative().value(time);
  flange_servo.mutable_desired_pose().linear() =
      Matrix3<double>(plan.flange_quat_n_traj.orientation(time));
  flange_servo.mutable_desired_velocity().head<3>() =
      plan.flange_quat_n_traj.angular_velocity(time);
...
void KukaPlanEvalSystem::DoCalcUnrestrictedUpdate(
  ...
  // TODO(siyuan): move stuff in DoCalcOutput to here once we have
  // CalcUnrestrictedUpdate in diagram.
}

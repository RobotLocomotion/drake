...
JointLevelControllerSystem::JointLevelControllerSystem(const RigidBodyTree<double>& robot) : robot_(robot) {
  input_port_index_qp_output_ = DeclareAbstractInputPort().get_index();
  output_port_index_torque_ = DeclareOutputPort(systems::kVectorValued,
      robot_.get_num_actuators()).get_index();
}

void JointLevelControllerSystem::DoCalcOutput(const systems::Context<double>& context,

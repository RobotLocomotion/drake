...
RobotStatusWrapper::RobotStatusWrapper(const RigidBodyTree<double>& robot) : robot_(robot) {
...
void RobotStatusWrapper::DoCalcOutput(const systems::Context<double>& context,
  ...
  hum_status.Update(context.get_time(), x.head(robot_.get_num_positions()), x.tail(robot_.get_num_velocities()), VectorX<double>::Zero(robot_.actuators.size()), Vector6<double>::Zero(), Vector6<double>::Zero());
}

std::unique_ptr<systems::SystemOutput<double>> RobotStatusWrapper::AllocateOutput(

...
// HumanoidPlanEvalSystem be zero desired accelereations, and the acceleration
...
class HumanoidPlanEvalAndQpInverseDyanmicsTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    auto plan_eval = builder.AddSystem<HumanoidPlanEvalSystem>(
        *robot, kAliasGroupsPath, kControllConfigPath, kControlDt);
...
TEST_F(HumanoidPlanEvalAndQpInverseDyanmicsTest, InputShouldBeZero) {
...
TEST_F(HumanoidPlanEvalAndQpInverseDyanmicsTest, DofAcceleration) {
  const QpInput& qp_input =
      output_->get_data(qp_input_index_)->template GetValue<QpInput>();
...
TEST_F(HumanoidPlanEvalAndQpInverseDyanmicsTest, ContactAcceleration) {
  const QpOutput& qp_output =
      output_->get_data(qp_output_index_)->template GetValue<QpOutput>();
...
TEST_F(HumanoidPlanEvalAndQpInverseDyanmicsTest, BodyAcceleration) {
  const QpInput& qp_input =
      output_->get_data(qp_input_index_)->template GetValue<QpInput>();
  const QpOutput& qp_output =
      output_->get_data(qp_output_index_)->template GetValue<QpOutput>();
...
// well.
TEST_F(HumanoidPlanEvalAndQpInverseDyanmicsTest, CentroidalMomentum) {
  const QpInput& qp_input =
      output_->get_data(qp_input_index_)->template GetValue<QpInput>();
  const QpOutput& qp_output =
      output_->get_data(qp_output_index_)->template GetValue<QpOutput>();

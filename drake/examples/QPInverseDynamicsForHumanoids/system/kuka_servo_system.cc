...
KukaServoSystem::KukaServoSystem(
    ...
    : DiscreteTimePlanEvalSystem(robot, alias_groups_file_name, param_file_name,
                                 dt) {
  DRAKE_DEMAND(get_robot().get_num_velocities() == get_robot().get_num_positions());
  input_port_index_desired_state_and_acceleration_ = DeclareInputPort(systems::kVectorValued, get_robot().get_num_positions() + 2 * get_robot().get_num_velocities()).get_index();
  set_name("kuka_servo_plan_eval");
}

void KukaServoSystem::Initialize(systems::State<double>* state) {
  VectorSetpoint<double>& plan = get_mutable_plan<VectorSetpoint<double>>(state);
...
void KukaServoSystem::DoCalcUnrestrictedUpdate(
  ...
  const systems::BasicVector<double>* desired = EvalVectorInput(context, input_port_index_desired_state_and_acceleration_);
  ...
  for (int i = 0; i < get_robot().get_num_positions(); i++) {
    ...
    plan.mutable_desired_velocity()[i] = desired->GetAtIndex(i + get_robot().get_num_positions());
    plan.mutable_desired_acceleration()[i] = desired->GetAtIndex(i + get_robot().get_num_positions() + get_robot().get_num_velocities());
  }

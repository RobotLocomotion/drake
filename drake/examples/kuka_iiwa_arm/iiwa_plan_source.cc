...
constexpr double kPlanUpdateInterval = 0.1;
...
std::unique_ptr<systems::AbstractValues> IiwaPlanSource::AllocateAbstractState()
  ...
  abstract_vals.push_back(systems::AbstractValue::Make<PlanData>(default_plan));
  abstract_vals.push_back(systems::AbstractValue::Make<bool>(false));
...
void IiwaPlanSource::Initialize(const double plan_start_time,
...
void IiwaPlanSource::DoCalcUnrestrictedUpdate(
  ...
  if (plan_input.num_states && encoded_msg != plan.encoded_msg) {
    ...
    if (knots.size() >= 3) {
      ...
      plan.pp = PiecewisePolynomial<double>::Cubic(input_time, knots, knot_dot,
                                                   knot_dot);
    } else {
      plan.pp = PiecewisePolynomial<double>::FirstOrderHold(input_time, knots);
    }
    plan.pp_deriv = plan.pp.derivative();
  }
}

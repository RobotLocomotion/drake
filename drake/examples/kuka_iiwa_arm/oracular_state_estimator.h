 ...
 * in `W`), where `W` is the world frame, and `B` is the first non-world rigid
 * body's body frame in the given rigid body tree. Similarly,
 * `message.twist` = V_WB is the spatial velocity of `B` relative to `W`
 * expressed in `W`. If the first non-world rigid body is rigidly attached to
 * the world, V_WB = 0. The joint section of `message` excludes all degrees of
 * freedom that correspond to the floating base (if one exists), and will be in
 * the order of the rigid body tree's generalized position. Note that the
 * `message.joint_effort` will be set to zero as this class only encodes

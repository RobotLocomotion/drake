...
int main(int argc, const char* argv[]) {
  ...
  plan.plan.resize(kNumTimesteps);
  plan.plan_info.resize(kNumTimesteps);
  for (int i = 0; i < kNumTimesteps; i++) {
    bot_core::robot_state_t& step = plan.plan[i];
    step.utime = t[i] * 1e6;
    memset(&step.pose, 0 , sizeof(step.pose));
    memset(&step.twist, 0 , sizeof(step.twist));
    step.num_joints = 7;
    ...
    memset(&step.force_torque, 0, sizeof(step.force_torque));

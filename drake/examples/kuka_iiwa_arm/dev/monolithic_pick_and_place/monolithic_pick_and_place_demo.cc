  ...
  *wsg_instance = tree_builder->get_model_info_for_instance(wsg_id);
  ...
  if (camera) {
    ...
    int fixture_id = tree_builder->AddModelInstanceToFrame(
        "xtion_wsg_fixture",
        tree_builder->tree().findFrame("body_frame", wsg_id),
        drake::multibody::joints::kFixed);
...
int DoMain(void) {
  ...
  Xtion* camera = nullptr;  // This instance will be owned by the diagram.
  if (FLAGS_with_camera) {
    camera = new Xtion("xtion");
  }
  ...
  // TODO(eric.cousineau): Use `simulator.set_publish_every_time_step(false)`
  // and have the visualizer publish on every major time step, rather than
  // forcing the entire diagram to update (which causes the LCM publishers to
  // publish every step as well, which is costly given that these are images).
  // (#5629).
  ...
  if (camera) {
    camera->BuildDiagram(&lcm, FLAGS_with_camera_lcm, false);

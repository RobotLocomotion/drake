...
#include "drake/examples/kuka_iiwa_arm/iiwa_xtion.h"
...
std::unique_ptr<systems::RigidBodyPlant<double>> BuildCombinedPlant(
  ...
  *wsg_instance = tree_builder->get_model_info_for_instance(wsg_id);
  ...
  if (pcamera) {
    *pcamera = new IiwaXtion(tree_builder.get(), wsg_id);
  }
...
int DoMain(void) {
  ...
  if (camera) {
    ...
    builder.AddSystem(std::unique_ptr<IiwaXtion>(camera));
    builder.Connect(

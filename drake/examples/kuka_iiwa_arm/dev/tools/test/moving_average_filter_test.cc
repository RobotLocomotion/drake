...
class MovingAverageDoubleTest : public ::testing::Test {
 protected:
  void SetUp() override {
    filter_ = std::make_unique<MovingAverageFilter<double>>(kWindowSize);
  }
  std::unique_ptr<MovingAverageFilter<double>> filter_;
};

TEST_F(MovingAverageDoubleTest, Compute) {
...
class MovingAverageVectorTest : public ::testing::Test {
 protected:
  void SetUp() override {
    filter_ =
        std::make_unique<MovingAverageFilter<Eigen::Vector3d>>(kWindowSize);
  }
  std::unique_ptr<MovingAverageFilter<Eigen::Vector3d>> filter_;
};

TEST_F(MovingAverageVectorTest, compute_vector_test) {

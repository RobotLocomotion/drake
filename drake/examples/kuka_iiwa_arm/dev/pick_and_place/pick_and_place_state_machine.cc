...
bool PlanMoveStraight(const VectorX<double>& q_current,
  ...
  PiecewiseQuaternionSlerp<double> rot_traj({0, 1}, quats);
  PiecewisePolynomial<double> pos_traj =
      PiecewisePolynomial<double>::FirstOrderHold({0, 1}, pos);

  std::vector<IiwaIkPlanner::IkCartesianWaypoint> waypoints(num_via_points + 1);
  const double dt = 1. / (num_via_points + 1);
  double time = 0;
  times->clear();

...
class SimulatedKuka : public systems::Diagram<T> {
 public:
  SimulatedKuka() {
    ...
    const double kd = 0.0;  // derivative constant.  Unused, see below!
    ...
    // desired value for the arm, so this simulation doesn't take
    ...
    // set the D term to zero above and feed in zero inputs.

...
std::unique_ptr<RigidBodyPlant<T>> BuildCombinedPlant(
  ...
  tree_builder->StoreModel(
      "iiwa", "/examples/kuka_iiwa_arm/urdf/iiwa14_simplified_collision.urdf");
  tree_builder->StoreModel(
      "table",
      // NOLINTNEXTLINE(whitespace/line_length)
      "/examples/kuka_iiwa_arm/models/table/extra_heavy_duty_table_surface_only_collision.sdf");
  tree_builder->StoreModel(
      "cylinder",
      "/examples/kuka_iiwa_arm/models/objects/simple_cylinder.urdf");
  tree_builder->StoreModel(
      "schunk", "/examples/schunk_wsg/models/schunk_wsg_50.sdf");
  ...
  const Eigen::Vector3d kCylinderBase(1 + -0.43, -0.65, kTableTopZInWorld + 0.15);
  ...
  *wsg_instance_id = tree_builder->AddModelInstanceToFrame(
      "schunk", Eigen::Vector3d::Zero(),  Eigen::Vector3d::Zero(),
...
class SimulatedIiwaWithWsg : public systems::Diagram<T> {
 public:
  SimulatedIiwaWithWsg() {
    ...
    // estimation of the model constants such as friction in the
    // joints.
    ...
    auto iiwa_control_port = iiwa_ports.first;
    auto iiwa_state_port = iiwa_ports.second;
    ...
    const T wsg_ki = 0.0;
    const T wsg_kd = 5.0;
    const VectorX<T> wsg_v = VectorX<T>::Ones(wsg_input_port.get_size());
    auto wsg_ports = PidControlledSystem<T>::ConnectController(
        ...
        &builder);
    auto wsg_control_port = wsg_ports.first;
    auto wsg_state_port = wsg_ports.second;

    // Connect the iiwa's gravity compensation.
  ...
  const SystemPortDescriptor<T>& get_iiwa_input_port() const {
    return this->get_input_port(0);
  }

  const SystemPortDescriptor<T>& get_iiwa_state_port() const {
    return this->get_output_port(0);
  }

  const SystemPortDescriptor<T>& get_wsg_input_port() const {
    return this->get_input_port(1);
  }

  const SystemPortDescriptor<T>& get_wsg_state_port() const {
    return this->get_output_port(1);
  }

  const SystemPortDescriptor<T>& get_plant_output_port() const {
...
int DoMain() {
  ...
  auto wsg_trajectory_generator =
      builder.AddSystem<SchunkWsgTrajectoryGenerator>(
          model->get_wsg_state_port().get_size(), 0);
  ...
  auto wsg_status_sender = builder.AddSystem<SchunkWsgStatusSender>(
      model->get_wsg_state_port().get_size(), 0, 1);

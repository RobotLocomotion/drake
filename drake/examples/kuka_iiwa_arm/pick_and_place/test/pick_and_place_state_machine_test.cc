...
GTEST_TEST(PickAndPlaceStateMachineTest, StateMachineTest) {
  ...
  steps.push_back(TestStep{0, 1, OPEN_GRIPPER});
  steps.push_back(TestStep{0, 1, APPROACH_PICK_PREGRASP});
  steps.push_back(TestStep{1, 1, APPROACH_PICK_PREGRASP});
  steps.push_back(TestStep{1, 1, APPROACH_PICK});
  steps.push_back(TestStep{2, 1, APPROACH_PICK});
  steps.push_back(TestStep{3, 1, GRASP});
  steps.push_back(TestStep{3, 2, GRASP});
  steps.push_back(TestStep{3, 2, GRASP});
  steps.push_back(TestStep{3, 2, LIFT_FROM_PICK});
  steps.push_back(TestStep{4, 2, LIFT_FROM_PICK});
  steps.push_back(TestStep{4, 2, APPROACH_PLACE_PREGRASP});
  steps.push_back(TestStep{5, 2, APPROACH_PLACE_PREGRASP});
  steps.push_back(TestStep{5, 2, APPROACH_PLACE});
  steps.push_back(TestStep{6, 2, APPROACH_PLACE});
  steps.push_back(TestStep{7, 2, PLACE});
  steps.push_back(TestStep{7, 3, PLACE});
  steps.push_back(TestStep{7, 3, PLACE});
  steps.push_back(TestStep{7, 3, LIFT_FROM_PLACE});
  steps.push_back(TestStep{8, 3, LIFT_FROM_PLACE});
  steps.push_back(TestStep{9, 3, DONE});
  steps.push_back(TestStep{9, 3, DONE});
  ...
  for (const TestStep& step : steps) {
    ...
    if (step.state_expected == APPROACH_PLACE_PREGRASP) {
      object_msg.pose.translation.y = 0.36;
      world_state.HandleObjectStatus(object_msg);
    }
    ...
    EXPECT_EQ(iiwa_plan_count, step.iiwa_expected);
    EXPECT_EQ(wsg_command_count, step.wsg_expected);

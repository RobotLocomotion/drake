...
void RobotPlanInterpolator::DoCalcUnrestrictedUpdate(
  ...
  if (encoded_msg != plan.encoded_msg) {
    ...
    if (plan_input.num_states == 0) {
      ...
      if (knots.size() == 2) {
        // Insert an artifical midpoint so that we can do cubic
        // interpolation.
        std::vector<Eigen::MatrixXd> new_knots;
        std::vector<double> new_input_time;
        new_knots.push_back(knots.front());
        new_input_time.push_back(input_time.front());
        new_knots.push_back((knots.back() - knots.front()) * 0.5);
        new_input_time.push_back(
            (input_time.back() - input_time.front()) * 0.5);
        new_knots.push_back(knots.back());
        new_input_time.push_back(input_time.back());
        knots.swap(new_knots);
        input_time.swap(new_input_time);
      }

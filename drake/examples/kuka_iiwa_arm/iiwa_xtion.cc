...
bool IiwaXtion::is_movable() const {
  return &fixture_frame_->get_rigid_body() == &tree_->world();
}

IiwaXtion::IiwaXtion(TreeBuilder* tree_builder, RigidBody<double>* parent,
                     const Eigen::Isometry3d& X_PX)
    : tree_(&tree_builder->tree()) {
  std::string urdf_resource =
      "drake/examples/kuka_iiwa_arm/models/cameras/xtion.urdf";
  ...
  // Add sensor frame, `B` for RgbdCamera.
  Eigen::Isometry3d X_XB = xtion_frames::X_XB();
...
IiwaXtion::IiwaXtion(IiwaXtion::TreeBuilder* tree_builder, int wsg_id)
    : IiwaXtion(tree_builder,
                tree_builder->mutable_tree().FindBody("body", "", wsg_id),
                xtion_frames::X_GX()) {}

void IiwaXtion::Build(lcm::DrakeLcm* lcm, bool do_lcm_publish,

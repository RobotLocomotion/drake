 ...
 * @endcode
 */
class IiwaXtion : public systems::Diagram<double> {
 public:
  typedef manipulation::util::WorldSimTreeBuilder<double> TreeBuilder;

  /// Attach camera to a given body, with a given transform.
  IiwaXtion(TreeBuilder* tree_builder, RigidBody<double>* parent,
            const Eigen::Isometry3d& X_PX);

  /// Convenience for WSG.
  explicit IiwaXtion(TreeBuilder* tree_builder, int wsg_id = -1);

  // Convenience for world-frame.
  IiwaXtion(TreeBuilder* tree_builder, const Eigen::Isometry3d& X_WX)
      : IiwaXtion(tree_builder, &tree_builder->mutable_tree().world(), X_WX) {}

  void Build(lcm::DrakeLcm* lcm, bool do_lcm_publish = true,

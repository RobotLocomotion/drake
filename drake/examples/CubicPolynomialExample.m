...
%      pause;
      ...
      legend('xdot','Vdot','lambda','Vdot+lambda*(1-V)');
        ...
        function genericSOS(x0)
        % If the problem seems primal infeasible, try increasing the order
        % of monomials, m=monomials(x,0:n)
        ...
        % display the polynomial representation of V that results
        display(V_estimate.getPoly);
        
        % Define a new symbolic variable
        x=msspoly('x');
        
        % Define the dynamics and Lyapunov function
        f = getPolyDynamics(p,0); % (sys, t)

function [p,v,xtraj,utraj,ltraj,z,F,info,traj_opt] = test3D2ModeHybridConstrainedDircol(z0,xtraj,utraj,ltraj)
...
to_options.mode_options{2}.active_inds = [1;2;3;4;6;9];
...
% MONDAY TODO CHECK THESE CONSTRAINTS, AND CONTACT INDICES
% AGAINST TEST3MODEPASSIVEHYBRID
...
if length(N) > 1
  ...
  traj_init.mode{2}.x = PPTrajectory(foh(t_init{2},repmat(x0,1,N(2))));
  traj_init.mode{2}.u = PPTrajectory(foh(t_init{2},randn(15,N(2))));
  traj_init.mode{2}.l = PPTrajectory(foh(t_init{2},repmat([l0],1,N(2))));

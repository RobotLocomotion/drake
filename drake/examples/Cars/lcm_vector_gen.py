...
class DRAKECARS_EXPORT %(camel)sTranslator
 ...
 public:
  ...
  void TranslateLcmToVectorBase(
      const ::lcm::ReceiveBuffer*, systems::VectorBase<double>*)
      const override;
  void PublishVectorBaseToLCM(
      const systems::VectorBase<double>&,
      const std::string& channel, ::lcm::LCM*)
      const override;
};
"""
...
void %(camel)sTranslator::PublishVectorBaseToLCM(
    const systems::VectorBase<double>& vector_base,
    const std::string& channel, ::lcm::LCM* lcm) const {
...
void %(camel)sTranslator::TranslateLcmToVectorBase(
  ...
  auto* const vector = dynamic_cast<%(camel)s<double>*>(vector_base);
  DRAKE_ABORT_UNLESS(vector != nullptr);
...
"""
VECTOR_TO_LCM_FIELD = """  vector->set_%(field)s(message.%(field)s);"""

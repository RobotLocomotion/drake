...
int do_main(int argc, char* argv[]) {
  ...
  auto acrobot = dynamic_cast<const AcrobotPlant<double>*>(
      acrobot_w_encoder->GetSubsystem("Acrobot"));
  ...
  auto observer_acrobot = AcrobotWEncoder<double>();
  ...
  {  // Set context to upright fixed point.
    auto x0 = dynamic_cast<AcrobotStateVector<double>*>(
        dynamic_cast<systems::DiagramContext<double>*>(observer_context.get())
  ...
  AcrobotPlant<double> simple_acrobot;
  auto controller = builder.AddSystem(BalancingLQRController(&simple_acrobot));

function [p,v,xtraj,utraj,z,ltraj,F,info,traj_opt] = testConstrainedDircol_v1(N)
...
xf = [0; 0; 0; 0; 0; 0];
xf_min = [-1e-3; -2*pi; -2*pi; -1e-3; -1e-3; -1e-3];
xf_max = -[-1e-3; -2*pi; -2*pi; -1e-3; -1e-3; -1e-3];

to_options.lambda_bound = 26;

traj_opt = ConstrainedDircolTrajectoryOptimization(p,N,[T T],to_options);

traj_opt = traj_opt.addStateConstraint(BoundingBoxConstraint(x0-1e-3,x0+1e-3),1); % at the first time step constrain the state

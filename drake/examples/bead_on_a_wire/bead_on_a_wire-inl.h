...
template <typename T>
void BeadOnAWire<T>::HandleImpact(const systems::Context<T>& context,
...
//          x-axis).
template <class T>
Vector2<T> BeadOnAWire<T>::CalcFConeImpactImpulse(
...
//          x-axis).
template <class T>
Vector2<T> BeadOnAWire<T>::CalcStickingImpactImpulse(
    const systems::Context<T>& context) const {
  // Get the necessary parts of the state.
  const systems::VectorBase<T>& state = context.get_continuous_state_vector();
  const T x = state.GetAtIndex(0);
...
template <class T>
void BeadOnAWire<T>::SetVelocityDerivatives(const systems::Context<T>& context,
...
template <class T>
Vector2<T> BeadOnAWire<T>::CalcStickingContactForces(
    const systems::Context<T>& context) const {
...
template <class T>
void BeadOnAWire<T>::DoCalcTimeDerivativesOneContactNoSliding(
...
template <class T>
void BeadOnAWire<T>::DoCalcTimeDerivativesTwoContact(
    const systems::Context<T>& context,
    systems::ContinuousState<T>* derivatives) const {
  using std::abs;
...
template <class T>
bool BeadOnAWire<T>::IsImpacting(const systems::Context<T>& context) const {
  using std::sin;
  using std::cos;

...
def generate_default_ctor(context, fields):
    _ = fields
    header = context["header"]
    print >> header, DEFAULT_CTOR_TEXT % context,
...
def generate_initial_state_ctor(context, fields):
    header = context["header"]
    print >> header, INITIAL_STATE_CTOR_BEGIN % context,
    # pylint: disable=unused-variable
    for k, field in enumerate(fields):
        if k > 0:
            print >> header, INITIAL_STATE_CTOR_JOIN,
        print >> header, INITIAL_STATE_CTOR_FIELD % locals(),
    print >> header, INITIAL_STATE_CTOR_END % context
...
def generate_eigen_assignment(context, fields):
    header = context["header"]
    print >> header, EIGEN_ASSIGNMENT_BEGIN % context,
    # pylint: disable=unused-variable
    for k, field in enumerate(fields):
        print >> header, EIGEN_ASSIGNMENT_FIELD % locals(),
    print >> header, EIGEN_ASSIGNMENT_END % context
...
def generate_to_eigen(context, fields):
    header = context["header"]
    print >> header, TO_EIGEN_BEGIN % context
    print >> header, "    result << %s;" % ", ".join(
        ["vec.%s" % f for f in fields])
    print >> header, TO_EIGEN_END % context
...
def generate_coord_namer(context, fields):
    header = context["header"]
    print >> header, COORD_NAMER_BEGIN % context,
    # pylint: disable=unused-variable
    for k, field in enumerate(fields):
        print >> header, COORD_NAMER_FIELD % locals()
    print >> header, COORD_NAMER_END % context


def generate_members(context, fields):
    header = context["header"]
    for field in fields:
        print >> header, "  ScalarType %s = 0;" % field
...
def generate_encode(context, fields):
    header = context["header"]
    print >> header, ENCODE_BEGIN % context,
    # pylint: disable=unused-variable
    for k, field in enumerate(fields):
        print >> header, ENCODE_FIELD % locals()
    print >> header, ENCODE_END % context
...
def generate_decode(context, fields):
    header = context["header"]
    print >> header, DECODE_BEGIN % context,
    # pylint: disable=unused-variable
    for k, field in enumerate(fields):
        print >> header, DECODE_FIELD % locals()
    print >> header, DECODE_END % context
...
def generate_code(args):
    ...
    print >> header, HEADER_PREAMBLE % locals()
    print >> header, CLASS_BEGIN % locals()
    ...
    print >> header, CLASS_END % locals()
    generate_encode(locals(), args.fields)
    generate_decode(locals(), args.fields)
    print >> header, HEADER_POSTAMBLE % locals()

    print >> lcmtype, LCMTYPE_PREAMBLE % locals()
    for field in args.fields:
        print >> lcmtype, "  double %s;" % field
    print >> lcmtype, LCMTYPE_POSTAMBLE % locals()

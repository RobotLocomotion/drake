...
#include "drake/examples/SimpleCar/floating_joint_state.h"
...
int main(int argc, const char* argv[]) {
  ...
  D <<
      ...
      0, 0,  0, 0, 0,  // pitch
      0, 0, -1, 0, 0;  // yaw
  auto adapter = std::make_shared<Drake::AffineSystem<
      Drake::NullVector, Drake::SimpleCarState, Drake::FloatingJointState> >(
  ...
  auto viz = std::make_shared<
    Drake::BotVisualizer<Drake::FloatingJointState> >(lcm, tree);

  // Make some taps to publish intermediate states to LCM.
  auto car_tap = std::make_shared<LCMTap<Drake::SimpleCarState> >(lcm);
  auto adapter_tap = std::make_shared<LCMTap<Drake::FloatingJointState> >(lcm);

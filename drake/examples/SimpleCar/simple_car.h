...
class SimpleCar {
 public:
  ...
  SimpleCar(const drake::lcmt_simple_car_config_t& config = kDefaultConfig)
      : config_(config) {}
  ...
  StateVector<ScalarType> dynamics(const ScalarType& time,
    ...
    ScalarType sane_steering_angle =
        std::max(std::min(std::remainder(input.steering_angle, 2 * kPi),
    ...
    delta.steering_angle =
        std::remainder(sane_steering_angle - state.steering_angle, 2 * kPi);
    delta.x = velocity * std::sin(state.heading);
    delta.y = velocity * std::cos(state.heading);

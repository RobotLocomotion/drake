...
TEST(SimpleCarTest, ZerosIn) {
  ...
  EXPECT_TRUE(CompareMatrices(toEigen(default_state), toEigen(new_state),
...
TEST(SimpleCarTest, Accelerating) {
  ...
  EXPECT_NEAR(history_system->states_[max_time].y,
              Drake::SimpleCar::kDefaultConfig.max_velocity * end_time, 3e2);
...
TEST(SimpleCarTest, Steering) {
  Drake::DrivingCommand<double> left(Eigen::Vector3d(-M_PI, 0., 0.));
  ...
  double predicted_heading = -std::remainder(speed * max_time - start_time,
                                             2 * M_PI * min_turn_radius) /
                             min_turn_radius;
  double end_heading =

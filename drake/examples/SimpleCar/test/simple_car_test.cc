...
class ConstantInputSystem {
 public:
  ...
  ConstantInputSystem(const OutputVector<double>& constant)
      : output_(constant) {}
...
class HistorySystem {
 public:
  ...
  HistorySystem(const InputVector<double> initial) :initial_(initial) {}
...
TEST(SimpleCarTest, ZerosIn) {
  Drake::SimpleCar dut;
  Drake::SimpleCarState<double> default_state;
  Drake::DrivingCommand<double> default_input;

  Drake::SimpleCarState<double> new_state =
      dut.dynamics(0., default_state, default_input);

  const double tolerance = 1e-8;
  std::string error_msg;
  EXPECT_TRUE(
      CompareMatrices(toEigen(default_state), toEigen(new_state),
                      tolerance, MatrixCompareType::absolute, &error_msg))
      << error_msg;
}
...
TEST(SimpleCarTest, Accelerating) {
  ...
  auto lead_foot = Drake::cascade(
      Drake::cascade(
          std::make_shared<
          ConstantInputSystem<Drake::DrivingCommand>>(max_throttle),
          car),
      history_system);
  ...
  EXPECT_EQ(history_system->states_[start_time].steering_angle, 0.);
  ...
  // value, but rather that we got somewhere.
  EXPECT_NEAR(history_system->states_[max_time].y, 45 * 100, 3e2);
  EXPECT_EQ(history_system->states_[max_time].heading, 0.);
  EXPECT_EQ(history_system->states_[max_time].steering_angle, 0.);
...
TEST(SimpleCarTest, Braking) {
  ...
  auto panic_stop = Drake::cascade(
      Drake::cascade(
          std::make_shared<
          ConstantInputSystem<Drake::DrivingCommand>>(max_brake),
          car),
      history_system);
  ...
  EXPECT_LT(history_system->states_[max_time].y, 10.);
  EXPECT_EQ(history_system->states_[max_time].heading, 0.);
  EXPECT_EQ(history_system->states_[max_time].steering_angle, 0.);
...
TEST(SimpleCarTest, Steering) {
  Drake::DrivingCommand<double> left(Eigen::Vector3d(-kPi, 0., 0.));
  ...
  auto brickyard = Drake::cascade(
      Drake::cascade(
          std::make_shared<
          ConstantInputSystem<Drake::DrivingCommand>>(left),
          car),
      history_system);
  ...
  double endx = history_system->states_[max_time].x;
  double endy = history_system->states_[max_time].y;

  // Our drive circle should fit in a small box.
  EXPECT_LT(endx, turn_epsilon);
  EXPECT_GT(endx, -(2 * min_turn_radius + turn_epsilon));
  EXPECT_LT(endy, min_turn_radius + turn_epsilon);
  EXPECT_GT(endy, -(min_turn_radius + turn_epsilon));
  // Our drive state should be near the predicted circle.
  EXPECT_NEAR(std::hypot(endx + min_turn_radius, endy), min_turn_radius, 1e-3);

  // Predict our final heading.
  double predicted_heading =
      -std::remainder(speed * max_time - start_time,
                      2 * kPi * min_turn_radius) /
      min_turn_radius;
  double end_heading =
      std::remainder(history_system->states_[max_time].heading, 2 * kPi);
  EXPECT_NEAR(end_heading, predicted_heading, 1e-5);

  EXPECT_NEAR(history_system->states_[max_time].steering_angle,
              -Drake::SimpleCar::kDefaultConfig.max_abs_steering_angle, 1e-5);

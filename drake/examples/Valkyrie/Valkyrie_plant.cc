...
ValkyriePlant::ValkyriePlant() {
  ...
  sys_->AddModelInstanceFromFile(
      GetDrakePath() +
          "/examples/Valkyrie/val_description/urdf/valkyrie_sim_drake.urdf",
...
const std::shared_ptr<RigidBodyTree>& ValkyriePlant::get_rigid_body_tree()
    const {
  return sys_->getRigidBodyTree();
}

bool ValkyriePlant::isTimeVarying() const { return sys_->isTimeVarying(); }

size_t ValkyriePlant::getNumInputs() const { return sys_->getNumInputs(); }

ValkyriePlant::StateVector<double> ValkyriePlant::output(
    const double& t, const StateVector<double>& x,
    const InputVector<double>& u) const {
  return sys_->output(t, x, u);
}

ValkyriePlant::StateVector<double> ValkyriePlant::dynamics(
    const double& t, const StateVector<double>& x,
    const InputVector<double>& u) const {

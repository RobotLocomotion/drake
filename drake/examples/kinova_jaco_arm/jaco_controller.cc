...
int DoMain() {
  ...
  // Specify a zero initial velocity.
  VectorX<double> v0 = VectorX<double>::Zero(num_joints + num_fingers);
  for (int i = 0; i < first_status.num_joints; ++i) {
    v0(i) = first_status.joint_position[i];
  }

  for (int i = 0; i < first_status.num_fingers; ++i) {
    v0(i + num_joints) = first_status.finger_position[i];
  }
  ...
  plan_source->Initialize(msg_time, v0,

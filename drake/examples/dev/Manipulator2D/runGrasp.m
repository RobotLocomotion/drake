function [xtraj,utraj,ltraj,v,p,info,trans_info]=runGrasp(xtraj_init, utraj_init, ltraj_init, trans_info)
clear mex
% p = AcrobotPlant();
p = Gripper2D_gcs;
N = 30;
...
% xf = [[0;.1;0;1.2;-1.15;-.2;-.3;.4];zeros(8,1)];

qf = [0;.1;0;1.2;-1.15;-.2;-.3;.4];
xf = [qf;zeros(8,1)];

xf_err = 0*[0;0;0;.02*ones(5,1);1*ones(8,1)];
xfmax = xf + xf_err;
xfmin = xf - xf_err;
...
%restrict the ball frommoving
xmin = [0;.1;-inf;1;-inf; -pi/4; -pi/2;-pi/4;-2*ones(8,1)];
xmax = [0;.1;inf; pi; inf; pi/4; pi/4;pi/2;2*ones(8,1)];
...
con.clambdai.ub.clambdai = repmat([inf(7,1); .25; inf(6,1)],1,N);
...
options.MajorIterationsLimit = 200;
options.IterationsLimit = 200000;

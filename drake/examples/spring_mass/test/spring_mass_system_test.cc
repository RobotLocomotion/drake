...
class SpringMassSystemTest : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    output_ = dynamic_cast<SpringMassStateVector*>(
...
TEST_F(SpringMassSystemTest, Output) {
  ...
  // Check the output through the StateVectorInterface API.
  ASSERT_EQ(2, output_->size());
  EXPECT_NEAR(0.1, output_->GetAtIndex(0), 1e-8);
  EXPECT_EQ(0.0, output_->GetAtIndex(1));
}

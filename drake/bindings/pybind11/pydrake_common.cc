...
PYBIND11_PLUGIN(_pydrake_common) {
  ...
  m.def("GetResourceSearchPaths", &drake::GetResourceSearchPaths,
        "Gets the list of paths set programmatically in which resource files"
        "are searched.");
  // Convenient wrapper to clear the list of resource search paths.
  m.def("ClearResourceSearchPaths", &drake::ClearResourceSearchPaths,
        "Clears the persistent variable containing the list of paths set"
        "programmatically in which resource files are searched.");

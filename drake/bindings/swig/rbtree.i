...
%eigen_typemaps(Eigen::Matrix<double, drake::math::SPACE_DIMENSION, 1>)
%eigen_typemaps(Eigen::Matrix3Xd)
%eigen_typemaps(Eigen::Matrix<double, drake::math::SPACE_DIMENSION, Eigen::Dynamic>)
...
%template(AutoDiff3XDynamic) AutoDiffWrapper<Eigen::VectorXd, drake::math::SPACE_DIMENSION, Eigen::Dynamic>;
%template(AutoDiff3XMax73) AutoDiffWrapper<Eigen::Matrix<double, Eigen::Dynamic, 1, 0, 73>, drake::math::SPACE_DIMENSION, Eigen::Dynamic>;
...
%extend RigidBodyTree {
  ...
  Eigen::Matrix<double, drake::math::SPACE_DIMENSION, Eigen::Dynamic> transformPoints(
      const KinematicsCache<double> &cache, const Eigen::Matrix<double, drake::math::SPACE_DIMENSION, Eigen::Dynamic> &points, int current_body_or_frame_ind, int new_body_or_frame_ind) const
  {
    return $self->transformPoints(cache, points, current_body_or_frame_ind, new_body_or_frame_ind);
  }

  AutoDiffWrapper<Eigen::VectorXd, drake::math::SPACE_DIMENSION, Eigen::Dynamic> transformPoints(
      const KinematicsCache<Eigen::AutoDiffScalar<Eigen::VectorXd> > &cache, const Eigen::Matrix<double, drake::math::SPACE_DIMENSION, Eigen::Dynamic> &points, int current_body_or_frame_ind, int new_body_or_frame_ind) const
  {
    return $self->transformPoints(cache, points, current_body_or_frame_ind, new_body_or_frame_ind);
  }

  AutoDiffWrapper<Eigen::Matrix<double, Eigen::Dynamic, 1, 0, 73>, drake::math::SPACE_DIMENSION, Eigen::Dynamic> transformPoints(
      const KinematicsCache<Eigen::AutoDiffScalar<Eigen::Matrix<double, Eigen::Dynamic, 1, 0, 73> > > &cache, const Eigen::Matrix<double, drake::math::SPACE_DIMENSION, Eigen::Dynamic> &points, int current_body_or_frame_ind, int new_body_or_frame_ind) const
  {
    return $self->transformPoints(cache, points, current_body_or_frame_ind, new_body_or_frame_ind);
  }

  Eigen::Matrix<double, drake::math::SPACE_DIMENSION, 1> centerOfMass(KinematicsCache<double> &cache, const std::set<int> &model_instance_id = default_model_instance_id_set) const {
    return $self->centerOfMass(cache, model_instance_id);
  }

  Eigen::Matrix<double, drake::math::SPACE_DIMENSION, Eigen::Dynamic> centerOfMassJacobian(KinematicsCache<double>& cache, const std::set<int>& model_instance_ids = default_model_instance_id_set, bool in_terms_of_qdot = false) const {

...
#include "drake/multibody/rigid_body_tree.h"
  ...
  // The following statement supresses the following compiler warning:
  //
...
%extend RigidBodyTree {
  ...
  Eigen::Matrix<double, drake::kSpaceDimension, 1> centerOfMass(KinematicsCache<double> &cache, const std::set<int> &model_instance_id) const {
    return $self->centerOfMass(cache, model_instance_id);
  }

  Eigen::Matrix<double, drake::kSpaceDimension, Eigen::Dynamic> centerOfMassJacobian(KinematicsCache<double>& cache, const std::set<int>& model_instance_ids, bool in_terms_of_qdot = false) const {

 ...
 *
 * Overloads for isConvertibleFromMex and fromMexUnsafe should be provided for each argument type.
 * isConvertibleFromMex and fromMexUnsafe functions should be of the form
 *
 *  bool isConvertibleFromMex(const mxArray* source, TriggerType*, std::ostream* log) NOEXCEPT
 ...
 * isConvertibleFromMex functions should do minimal work when the log argument is a nullptr. If it is not null, they should provide a helpful error message.
 *
 ...
 * Converts arguments and calls function *without* checking whether arguments are convertible first.
 */
...
void mexCallFunctionUnsafe(std::function<R(Arg0, Args...)> func, int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  auto partially_applied = [&](Args&& ...tail) {
    return std::move(func)(fromMexUnsafe(prhs[0], static_cast<FromMexType<Arg0>*>(nullptr)), std::forward<Args>(tail)...);
  };

  // recursively call mexCallFunctionUnsafe with partially applied function
  return mexCallFunctionUnsafe(std::function<R(Args...)>{partially_applied}, nlhs, plhs, nrhs - 1, &prhs[1]);
};

/**
 * areArgumentsConvertibleFromMex
 * Checks whether the Matlab input arguments match the function signature
 * When the log argument is not a nullptr, writes diagnostic message to log upon failure.
 */
// base case
...
bool areArgumentsConvertibleFromMex(int nrhs, const mxArray *prhs[], int arg_num, std::ostream* log, Arg0*, Args*... tail) NOEXCEPT {
  if (arg_num == 0) {
    ...
    if (nrhs != expected_num_args) {
      if (log)
        *log << "Expected " << expected_num_args << " arguments, but got " << nrhs << ".";
      return false;
    }
  }
  ...
  if (!isConvertibleFromMex(prhs[0], (FromMexType<Arg0>*) nullptr, log)) {
    if (log)
      *log << std::endl << "Error occurred while checking argument " << arg_num << " of " << nrhs << ".";
    return false;
  }

  return areArgumentsConvertibleFromMex(nrhs, &prhs[1], arg_num + 1, log, tail...);
}

/**
 * mexCallFunction
 * checks whether arguments are convertible, and then calls mexCallFunctionUnsafe.
...
bool mexCallFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[], bool throw_on_error, std::function<R(Args...)> func) {
  if (areArgumentsConvertibleFromMex(nrhs, prhs, 0, nullptr, (static_cast<FromMexType<Args>*>(nullptr))...)) {
    ...
    areArgumentsConvertibleFromMex(nrhs, prhs, 0, &log, (static_cast<FromMexType<Args> *>(nullptr))...);
 ...
 * write diagnostic information to log; called after mexTryToCallFunctions has exhausted all options
 */
// base case
...
void collectMexTryCallFunctionsErrorDiagnostics(int nrhs, const mxArray *prhs[], std::ostream& log, int function_num, std::function<R0(Args0...)> function0, Funcs... functions) {
  log << std::endl << std::endl << "Trying function number " << function_num << std::endl;
  areArgumentsConvertibleFromMex(nrhs, prhs, 0, &log, (static_cast<FromMexType<Args0>*>(nullptr))...);
  collectMexTryCallFunctionsErrorDiagnostics(nrhs, prhs, log, function_num + 1, functions...);
}
...
{
  bool success = mexCallFunction(nlhs, plhs, nrhs, prhs, false, function0); // try the first one
  if (!success) { // recurse to try the other options
    success = mexTryToCallFunctions(nlhs, plhs, nrhs, prhs, false, functions...); // don't throw immediately, wait until all options exhausted
    if (throw_on_error && !success) {
      std::ostringstream log;
      log << "mexTryToCallFunctions: failed to find a function to call after trying " << sizeof...(Funcs) + 1 << " options. Errors:";
      collectMexTryCallFunctionsErrorDiagnostics(nrhs, prhs, log, 1, function0, functions...);
      throw std::runtime_error(log.str());
    }
  }
  return success;

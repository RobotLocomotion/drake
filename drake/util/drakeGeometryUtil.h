...
typename drake::math::Gradient<
    Eigen::Matrix<typename DerivedR::Scalar, drake::kSpaceDimension, 1>,
...
drotmat2rpy(const Eigen::MatrixBase<DerivedR>& R,
  ...
  ReturnType drpy(drake::kSpaceDimension, nq);
...
void angularvel2rpydotMatrix(
  ...
  phi.resize(drake::kSpaceDimension, drake::kSpaceDimension);
  ...
  phi << cy / cp, sy / cp, Scalar(0), -sy, cy, Scalar(0), cy * tp, tp * sy,
      Scalar(1);
  if (dphi) {
    dphi->resize(phi.size(), drake::kSpaceDimension);
    Scalar sp2 = sp * sp;
    Scalar cp2 = cp * cp;
    ...
    if (ddphi) {
      ddphi->resize(dphi->size(), drake::kSpaceDimension);
      Scalar cp3 = cp2 * cp;
...
void rpydot2angularvelMatrix(
    ...
    typename drake::math::Gradient<DerivedE, drake::kSpaceDimension, 1>::type*
        dE = nullptr) {
...
void rpydot2angularvel(
    ...
    typename drake::math::Gradient<DerivedOMEGA, drake::kSpaceDimension,

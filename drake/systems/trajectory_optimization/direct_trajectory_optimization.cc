...
solvers::SolutionResult DirectTrajectoryOptimization::SolveTraj(
  ...
  if (timesteps_are_decision_variables_) {
    DRAKE_DEMAND(!traj_init_u.empty() || !traj_init_x.empty());
    if (traj_init_u.empty()) {
      start_time = traj_init_x.getStartTime();
      end_time = traj_init_x.getEndTime();
    } else if (traj_init_x.empty()) {
      start_time = traj_init_u.getStartTime();
      end_time = traj_init_u.getEndTime();
    } else {
      start_time =
          std::max(traj_init_u.getStartTime(), traj_init_x.getStartTime());
      end_time = std::min(traj_init_u.getEndTime(), traj_init_x.getEndTime());
    }
    DRAKE_DEMAND(start_time <= end_time);
    SetInitialGuess(h_vars_, VectorXd::Constant(h_vars_.size(),
                                                (end_time - start_time) / N_));
  }
  const double timestep = (end_time - start_time) / N_;
  ...
  if (traj_init_u.empty()) {
    ...
    for (int i = 0; i < N_; ++i) {
      guess_u.segment(num_inputs_ * i, num_inputs_) =
          traj_init_u.value(start_time + i * timestep);
    }
  }
  ...
  if (traj_init_x.empty()) {
    ...
    for (int i = 0; i < N_; ++i) {
      guess_x.segment(num_states_ * i, num_states_) =
          traj_init_x.value(start_time + i * timestep);
    }
  }

...
DirectTrajectoryOptimization::SubstitutePlaceholderVariables(
  ...
  symbolic::Expression time =
      Eigen::RowVectorXd::Ones(interval_index) * h_vars_.head(interval_index);

  sub.insert(std::pair<symbolic::Variable, symbolic::Expression>(
      placeholder_t_var_(0), time));
  for (int i = 0; i < num_states_; i++)
    sub.insert(std::pair<symbolic::Variable, symbolic::Expression>(
        placeholder_x_vars_(i), x_vars_(interval_index * num_states_ + i)));
  for (int i = 0; i < num_inputs_; i++)
    sub.insert(std::pair<symbolic::Variable, symbolic::Expression>(
        placeholder_u_vars_(i), u_vars_(interval_index * num_inputs_ + i)));

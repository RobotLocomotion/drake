...
class ImplicitEulerIntegrator : public IntegratorBase<T> {
 public:
  ...
  /// Enables/disables error estimation. Error estimation causes the integrator
  /// to run 200% slower. When error estimation is disabled, calling
  /// IntegratorBase::get_error_estimate() after integrating will return a
  /// vector of NaN values. Error estimation is enabled by default.
  void set_error_estimation_enabled(bool flag) { err_est_enabled_ = flag; }

  /// Gets whether error estimation is enabled or disabled.
  bool is_error_estimation_enabled() const { return err_est_enabled_; }
 ...
 private:
  ...
  // Whether error estimation is enabled: requires 200% more computation
  // currently!
  bool err_est_enabled_{true};

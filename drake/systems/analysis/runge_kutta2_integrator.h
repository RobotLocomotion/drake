...
namespace drake {
    namespace systems {
        ...
        public:
            ...
            RungeKutta2Integrator(const System <T>& system, double step_size, Context<T>* context = NULL) : IntegratorBase<T>(system, context) {
          ...
          virtual void set_accuracy(double accuracy) { throw std::runtime_error("Accuracy setting not available"
                                                                                    " for RungeKutta2"
                                                                                    " integrator"); }

          /// No accuracy setting for RK2 integrator
          virtual double get_target_accuracy() const { throw std::runtime_error("Accuracy setting not available"
                                                                                    " for RungeKutta2"
                                                                                    " integrator"); }
        ...
        template <class T>
        typename IntegratorBase<T>::StepResult RungeKutta2Integrator<T>::Step(const T& dt) {
            DRAKE_THROW_UNLESS(dt >= 0.0);
            DRAKE_THROW_UNLESS(IntegratorBase<T>::initialization_done_);

            // Find the continuous state xc within the Context, just once.
            VectorBase<T>* xc =
                    IntegratorBase<T>::context_->get_mutable_state()->continuous_state->get_mutable_state();
            ...
            IntegratorBase<T>::system_.EvalTimeDerivatives(*IntegratorBase<T>::context_, derivs0_.get());
            ...
            IntegratorBase<T>::context_->set_time(IntegratorBase<T>::context_->get_time()+dt);

            // use derivative at t+dt
            IntegratorBase<T>::system_.EvalTimeDerivatives(*IntegratorBase<T>::context_, derivs1_.get());
            ...
            if (++IntegratorBase<T>::num_steps_taken_ == 1) {  // The first step.
                ...
                if (dt < IntegratorBase<T>::smallest_step_size_taken_) IntegratorBase<T>::smallest_step_size_taken_ = dt;
                if (dt > IntegratorBase<T>::largest_step_size_taken_) IntegratorBase<T>::largest_step_size_taken_ = dt;
            }

...
class Simulator {
 public:
  ...
  /** Sets whetheer the simulation should invoke Publish on the System under
   * simulation at the end of every timestep.
   */
  void set_publish_every_timestep(bool publish) {
    publish_every_timestep_ = publish;
  }

  /** Returns true if the simulation should invoke Publish on the System under
   * simulation at the end of every timestep.  By default, returns true.
   */
  // TODO(sherm1, edrumwri): Consider making this false by default.
  bool publish_every_timestep() const { return publish_every_timestep_; }
 ...
 private:
  ...
  bool publish_every_timestep_{true};
...
void Simulator<T>::StepTo(const T& boundary_time) {
  ...
  while (context_->get_time() <= boundary_time) {
    ...
    const T next_sampling_time =
        system_.CalcNextUpdateTime(*context_, &update_actions);
    DRAKE_ASSERT(next_sampling_time >= step_start_time);
    ...
    for (const DiscreteEvent<T>& event : update_actions.events) {
      if (event.action == DiscreteEvent<T>::kUpdateAction) {
        next_update_dt = next_sampling_time - step_start_time;
      }
      if (event.action == DiscreteEvent<T>::kPublishAction) {
        next_publish_dt = next_sampling_time - step_start_time;
      }
    }
    ...
    switch (result) {
      ...
      case IntegratorBase<T>::kReachedPublishTime:
        sampling_time_hit = true;
        break;

      case IntegratorBase<T>::kTimeHasAdvanced:
        sampling_time_hit = false;
        break;

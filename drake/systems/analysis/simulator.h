...
class Simulator {
 public:
   ...
   * after discrete updates and before continuous integration.
   */
  ...
  bool publish_every_time_step() const { return publish_every_time_step_; }
...
void Simulator<T>::StepTo(const T& boundary_time) {
  ...
  while (context_->get_time() <= boundary_time) {
    ...
    const T next_system_update_time =
        system_.CalcNextUpdateTime(*context_, &update_actions);
    DRAKE_ASSERT(next_system_update_time >= step_start_time);
    ...
    for (const DiscreteEvent<T>& event : update_actions.events) {
      if (event.action == DiscreteEvent<T>::kUpdateAction) {
        next_update_dt = next_system_update_time - step_start_time;
      }
      if (event.action == DiscreteEvent<T>::kPublishAction) {
        next_publish_dt = next_system_update_time - step_start_time;
      }
    }

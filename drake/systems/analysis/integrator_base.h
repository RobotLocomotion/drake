...
typename IntegratorBase<T>::StepResult IntegratorBase<T>::StepOnceAtMost(
  ...
  if (!step_size_was_dt) return IntegratorBase<T>::kTimeHasAdvanced;
  else if (dt == &publish_dt) return IntegratorBase<T>::kReachedPublishTime;
  else if (dt == &update_dt) return IntegratorBase<T>::kReachedUpdateTime;
  else return IntegratorBase<T>::kTimeHasAdvanced;
}

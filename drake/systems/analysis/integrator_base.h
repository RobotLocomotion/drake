...
typename IntegratorBase<T>::StepResult IntegratorBase<T>::StepOnceAtMost(
  ...
  // By default, the active constraint is the next discrete update event.
  StepResult active_constraint = IntegratorBase<T>::kReachedUpdateTime;
  T dt = update_dt;

  // If the next discrete publish event is sooner than the next discrete update
  // event, the publish event becomes the active constraint.
  if (publish_dt < update_dt) {
    active_constraint = IntegratorBase<T>::kReachedPublishTime;
    dt = publish_dt;
  }
  ...
  if (max_dt * kMaxStretch < dt) {
    active_constraint = IntegratorBase<T>::kTimeHasAdvanced;
    dt = max_dt;
  }

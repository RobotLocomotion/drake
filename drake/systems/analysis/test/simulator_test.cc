  ...
  class PerStepActionTestSystem : public LeafSystem<double> {
   public:
    ...
    void AddPerStepPublishEvent() {
      PublishEvent<double> event(Trigger::TriggerType::kPerStep);
      this->DeclarePerStepEvent(event);
    }

    void AddPerStepDiscreteUpdateEvent() {
      DiscreteUpdateEvent<double> event(Trigger::TriggerType::kPerStep);
      this->DeclarePerStepEvent(event);
    }

    void AddPerStepUnrestrictedUpdateEvent() {
      UnrestrictedUpdateEvent<double> event(Trigger::TriggerType::kPerStep);
      this->DeclarePerStepEvent(event);
    }

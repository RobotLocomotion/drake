...
class LeafSystem : public System<T> {
 public:
  ...
  std::unique_ptr<SystemOutput<T>> AllocateOutput(
    ...
    for (const auto& descriptor : this->get_output_topology()) {
 ...
 private:
  ...
  void ReserveInputs(Context<T>* context) const {
    context->SetNumInputPorts(this->get_input_topology().size());
  }
  ...
  LeafSystem(const System<T>& other) = delete;
  LeafSystem& operator=(const System<T>& other) = delete;
  LeafSystem(System<T>&& other) = delete;

...
class LeafSystem : public System<T> {
 public:
  ...
  void SetDefaults(Context<T>* context) const final {
    ...
    // First, set the defaults that apply to all Systems.
    System<T>::SetDefaults(context);

    // Then, set the defaults that apply only to LeafSystems.
    ...
    // Implementations of SetDefaultParameters must not change the number of
    // parameters.

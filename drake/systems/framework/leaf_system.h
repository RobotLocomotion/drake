...
class LeafSystem : public System<T> {
 public:
  ...
  std::multimap<int, int> GetDirectFeedthroughs() const final {
    // This predicate answers a feedthrough query using symbolic form, and
    // returns "true" if symbolic form is unavailable.  It is lazy, in that it
    // will not create the symbolic form until the first query is attempted.
    optional<std::unique_ptr<SystemSymbolicInspector>> inspector;
    auto has_symbolic_feedthrough = [this, &inspector](int u, int v) {
    ...
    // Iterate all input-output pairs, populating the map with "true" terms.
    std::multimap<int, int> pairs;
    for (int u = 0; u < this->get_num_input_ports(); ++u) {
      for (int v = 0; v < this->get_num_output_ports(); ++v) {
        // Ask our subclass whether it wants to directly declare the sparsity.
        optional<bool> leaf_sparsity = DoHasDirectFeedthrough(u, v);
        // If our subclass doesn't declare an answer, use symbolic form instead.
        if (leaf_sparsity == nullopt) {
          leaf_sparsity = has_symbolic_feedthrough(u, v);
        }
        // Record the answer.
        if (leaf_sparsity.value()) {
          pairs.emplace(u, v);
        }
      }
    }
 ...
 protected:
  ...
  ///   is recommended when DoToSymbolic has not been implemented, or when its

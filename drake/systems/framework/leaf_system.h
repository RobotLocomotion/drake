...
class LeafSystem : public System<T> {
 ...
 protected:
  ...
  /// enable AutoDiff support, either use the constructor that takes a
  /// SystemTypeTag or call SetSystemScalarConverter() directly.
  LeafSystem() {
  ...
  /// The scalar-type conversion support is per SetSystemScalarConverter<S>()
  /// for the `S` tag passed to this constructor.  Refer to that method's
  /// documentation for additional details.
  template <template <typename> class S>
  explicit LeafSystem(SystemTypeTag<S>) : LeafSystem() {
    SetSystemScalarConverter<S>();
  }

  /// Declares scalar-type conversion support (for AutoDiff, etc.) based the
  /// specified System subclass S and its scalar_conversion::Traits<S>.  The
  /// scalar-type conversion support will use `S` as the system type to

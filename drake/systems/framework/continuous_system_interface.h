...
class ContinuousSystemInterface : public SystemInterface<T> {
 public:
  ...
  virtual void EvalTimeDerivatives(const AbstractContext<T>& context,
  ...
  /// return 0.
  virtual T EvalConservativePower(const AbstractContext<T>& context) const {
    return T(0);
  }
  ...
  virtual T EvalNonConservativePower(const AbstractContext<T>& context) const {
    return T(0);
  }

...
class Context : public ContextBase<T> {
 public:
  ...
  /// depend upon State.
  ///
  /// To prevent cache corruption, callers MUST NOT write on the returned
  /// pointer after writing to the cache or passing this Context to a System
  /// that might write on the cache.
 ...
 private:
  /// Allocates @p n new input ports, not connected to anything. This function

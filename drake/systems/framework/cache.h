...
/// Context.
///
/// The cache is not thread-safe. It is copyable, assignable, and movable.
class DRAKESYSTEMFRAMEWORK_EXPORT Cache {
 public:
  ...
  /// This pointer will be deleted whenever Set or Swap is next called for the
  /// same @p ticket, or when the Cache itself is destroyed. Invalidates all
  /// entries that depend on the given @p ticket.
  ...
  /// initialized with a different type.
  ///
  ...
  /// Returns the cached item for the given @p ticket, or nullptr if the item
  /// has been invalidated. Replaces it with @p value. Invalidates all entries
  /// that depend on the given @p ticket.

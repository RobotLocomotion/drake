...
class InputPortDescriptor {
 public:
  ...
  // Default move-able.
  InputPortDescriptor(InputPortDescriptor&&) = default;
  ...
  // This class aliases a pointer to the system that contains it and also
  // captures its own index within that system's port vector, so we must be
  // careful not to allow C++ copying to extend the lifetime of the system
  // alias or duplicate our claim to the port index.  Thus, this class is
  // `MoveConstructible` but neither copyable nor assignable: it supports move
  // semantics to populate a vector, but is non-copyable in order remain the
  // "one true descriptor" after construction and non-assignable in order to
  // remain const.  Code that wishes to refer to this descriptor after
  // insertion into the vector should use a reference (not copy) of this
  // descriptor.
...
class OutputPortDescriptor {
 public:
  ...
  // Non-copyable, non-assignable; see documentation in InputPortDescriptor.

...
class VectorSystem : public LeafSystem<T> {
 ...
 protected:
  ...
  void CalcVectorOutput(const Context<T>& context,
    ...
    // Short-circuit when there's no work to do.
    if (this->get_num_output_ports() == 0) {
      return;
    }
  ...
  /// time.
  ///
  /// By default, this function does nothing if the @p outputs are empty,
  ...
  ///
  /// By default, this function does nothing if the @p next_state are

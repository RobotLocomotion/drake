...
template <typename T>
const T& UnpackValue(const AbstractValue& value) {
  const Value<T>* unpacked = dynamic_cast<const Value<T>*>(&value);
  DRAKE_DEMAND(unpacked != nullptr);
  return unpacked->get_value();
}

/// Extracts mutable data of type T from the given mutable @p value, or aborts
/// if the @p value does not contain type T.
template <typename T>
T& UnpackMutableValue(AbstractValue* value) {
  Value<T>* unpacked = dynamic_cast<Value<T>*>(value);
  DRAKE_DEMAND(unpacked != nullptr);

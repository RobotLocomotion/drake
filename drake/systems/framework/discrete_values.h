...
class DiscreteValues {
 public:
  ...
  /// Constructs a discrete values that does not own the underlying @p data.
  /// The data must outlive this DiscreteValues.
  explicit DiscreteValues(const std::vector<BasicVector<T>*>& data)
      : data_(data) {}

  /// Constructs a discrete values that owns the underlying @p data.
  ...
  /// Constructs a discrete values that owns a single @p datum vector.
  ...
  BasicVector<T>* get_mutable_vector() {
    DRAKE_ASSERT(num_groups() == 1);
    return get_mutable_value(0);
  }

  //@}


  const BasicVector<T>* get_value(int index) const {
    DRAKE_ASSERT(index >= 0 && index < num_groups());
    return data_[index];
  }

  BasicVector<T>* get_mutable_value(int index) {
    DRAKE_ASSERT(index >= 0 && index < num_groups());
    return data_[index];
  }
 ...
 private:
  ...
  void SetFromGeneric(const DiscreteValues<U>& other) {
    ...
    for (int i = 0; i < num_groups(); i++) {
      DRAKE_ASSERT(other.get_value(i) != nullptr);
      DRAKE_ASSERT(data_[i] != nullptr);
      data_[i]->set_value(
          other.get_value(i)->get_value().template cast<T>());
    }
  }
};

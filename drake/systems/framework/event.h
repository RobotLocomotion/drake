 ...
 * main pieces of information: a reason for the event occurreing (i.e., a
 * "trigger") and additional data that can be passed from the System to its
 * event handler (to mitigate or eliminate recomputation). Derived classes
 * should contain a function pointer to an optional callback function for
 * handling the event.
 */
template <typename T>
class Event {
...
class PublishEvent : public Event<T> {
 public:
   ...
   * the deep copy into @p events's collection of publish events. Aborts if
...
class DiscreteUpdateEvent : public Event<T> {
 public:
   ...
   * events. Aborts if these assumptions are not met.
   */
  void add_to_composite(CompositeEventCollection<T>* events) const override {
    Event<T>* clone = this->Clone().release();

...
template <typename T>
ConstantSource<T>::ConstantSource(const VectorX<T>& source_value) :
    source_value_(source_value) {
  // Input source must have at least one entry.
  DRAKE_ABORT_UNLESS(source_value.size() == 0);
  // TODO(amcastro-tri):
  // The constructor for a simple system like this should read:
  // add_output_port(make_unique<OutputPort>("source", 1 /* length */));
}
...
std::unique_ptr<ContextBase<T>> ConstantSource<T>::CreateDefaultContext() const {
  ...
  // A ConstantSource block only has one input port.
...
std::unique_ptr<SystemOutput<T>> ConstantSource<T>::AllocateOutput(
  ...
  // A ConstantSource has just one output port, a BasicVector of the size specified

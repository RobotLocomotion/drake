...
TEST_F(AdderTest, Topology) {
  ASSERT_EQ(2u, adder_->get_input_topology().size());
  for (const auto& descriptor : adder_->get_input_topology()) {
    EXPECT_EQ(kVectorValued, descriptor.get_type());
    EXPECT_EQ(kInputPort, descriptor.get_face());
    EXPECT_EQ(3, descriptor.get_size());
    EXPECT_EQ(kAutoSample, descriptor.get_sample_time_sec());
  }

  ASSERT_EQ(1u, adder_->get_output_topology().size());
  for (const auto& descriptor : adder_->get_output_topology()) {
    EXPECT_EQ(kVectorValued, descriptor.get_type());

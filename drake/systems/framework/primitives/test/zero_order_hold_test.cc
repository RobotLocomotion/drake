...
class ZeroOrderHoldTest : public ::testing::Test {
 protected:
  void SetUp() override {
    hold_ = make_unique<ZeroOrderHold<double>>(kTenHertz, kLength);
    context_ = hold_->CreateDefaultContext();
    output_ = hold_->AllocateOutput(*context_);
    input_ = make_unique<BasicVector<double>>(kLength);
  }
...
TEST_F(ZeroOrderHoldTest, ReservesState) {
  const VectorBase<double>* xd = context_->get_difference_state();
...
TEST_F(ZeroOrderHoldTest, Output) {
  BasicVector<double>* xd = dynamic_cast<BasicVector<double>*>(
      context_->get_mutable_difference_state());
...
// Tests that when the current time is exactly on the sampling period, a sample
// is requested in the future.
TEST_F(ZeroOrderHoldTest, NextUpdateTimeIsCurrentTime) {
  ...
  // Check that the action is to sample.
...
// Tests that when the current time is between samples, a sample is requested
...
TEST_F(ZeroOrderHoldTest, Sample) {
  auto input = MakeInput(BasicVector<double>::Make({1.0, 1.0, 3.0}));
  context_->SetInputPort(0, std::move(input));

  // Fire off a sample event.
  DiscreteEvent<double> sample_event;
  sample_event.recipient = hold_.get();
  sample_event.action = DiscreteEvent<double>::kUpdateAction;

  hold_->HandleDiscreteEvents({sample_event}, context_.get());

...
TEST_F(IntegratorTest, Topology) {
  ASSERT_EQ(1u, integrator_->get_input_topology().size());
  const auto& input_descriptor = integrator_->get_input_topology()[0];
  EXPECT_EQ(kVectorValued, input_descriptor.get_type());
  EXPECT_EQ(kInputPort, input_descriptor.get_face());
  EXPECT_EQ(kLength, input_descriptor.get_size());
  EXPECT_EQ(kContinuousSample, input_descriptor.get_sample_time_sec());

  ASSERT_EQ(1u, integrator_->get_output_topology().size());
  const auto& output_descriptor = integrator_->get_output_topology()[0];
  EXPECT_EQ(kVectorValued, output_descriptor.get_type());
  EXPECT_EQ(kOutputPort, output_descriptor.get_face());
  EXPECT_EQ(kLength, output_descriptor.get_size());
  EXPECT_EQ(kContinuousSample, output_descriptor.get_sample_time_sec());
}

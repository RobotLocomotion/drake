...
/// to
/// implement the shifting/scaling, the system itself contains all of the
...
class RandomSource : public LeafSystem<double> {
 public:
  ...
  explicit RandomSource(int num_outputs, double sampling_interval_sec) {
  ...
  /// Compute a random number and store it in the discrete state.
  void DoEvalDifferenceUpdates(
      const drake::systems::Context<double>& context,
      drake::systems::DifferenceState<double>* updates) const override {
    int N = updates->get_difference_state(0)->size();
    for (int i = 0; i < N; i++) {

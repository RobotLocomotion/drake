...
PidController<T>::PidController(
  ...
  pass_through_ = builder.AddSystem(std::make_unique<PassThrough<T>>(length));
  proportional_gain_ = builder.AddSystem(std::make_unique<Gain<T>>(Kp, length));
  integral_gain_ = builder.AddSystem(std::make_unique<Gain<T>>(Ki, length));
  derivative_gain_ = builder.AddSystem(std::make_unique<Gain<T>>(Kd, length));
  integrator_ = builder.AddSystem(std::make_unique<Integrator<T>>(length));
  adder_ = builder.AddSystem(std::make_unique<Adder<T>>(3 /* inputs */,
                                                        length));

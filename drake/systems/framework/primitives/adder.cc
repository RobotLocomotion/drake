...
template <typename T>
std::unique_ptr<AbstractContext<T>> Adder<T>::CreateDefaultContext() const {
...
template <typename T>
std::unique_ptr<SystemOutputInterface<T>> Adder<T>::AllocateOutput(
    const AbstractContext<T>& context) const {
...
template <typename T>
void Adder<T>::EvalOutput(const AbstractContext<T>& context,
                          SystemOutputInterface<T>* output) const {

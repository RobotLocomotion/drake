...
Adder<T>::Adder(int num_inputs, int length) {
  for (int i = 0; i < num_inputs; i++) {
    this->declare_continuous_input_port(length);
  }
  this->declare_continuous_output_port(length);
}
...
void Adder<T>::EvalOutput(const ContextBase<T>& context,
  ...
  const int n = output_port->get_value().rows();
  output_port->get_mutable_value() = VectorX<T>::Zero(n);
  ...
  for (int i = 0; i < context.get_num_input_ports(); i++) {
    ...
    DRAKE_ASSERT(input->get_value().rows() == n);

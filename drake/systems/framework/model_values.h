...
class ModelValues {
 public:
  ...
  /// Returns zero iff all values are nullptr; otherwise, returns one greater
  /// than the largest non-nullptr index.  (This is akin to a vector, where
  /// size is the exclusive upper bound on meaningful indices).
  int size() const;

  /// Sets @p model_value to be the model value for @p index.
  ///
  /// @pre index >= size()
  /// @pre model_value != nullptr
  void AddModel(int index, std::unique_ptr<AbstractValue> model_value);

  /// Wraps @p model_vector in a Value<BasicVector<T>*> and sets that to be the
  /// model value for @p index.
  ///
  /// @pre index >= size()
  /// @pre model_vector != nullptr
  template <typename T>
 ...
 private:
  ...
  std::vector<std::unique_ptr<AbstractValue>> values_;
};

template <typename T>
void ModelValues::AddVectorModel(
    int index, std::unique_ptr<BasicVector<T>> model_vector) {
  DRAKE_DEMAND(model_vector.get() != nullptr);

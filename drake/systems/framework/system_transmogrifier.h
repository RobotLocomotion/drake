...
template <template <typename> class S> struct SystemLabel {};
...
class SystemTransmogrifier {
 public:
  ...
  explicit SystemTransmogrifier(SystemLabel<S>) : SystemTransmogrifier() {
  ...
  /// CopyConstructible.
  SystemTransmogrifier(const SystemTransmogrifier&);

  /// CopyAssignable.
  SystemTransmogrifier& operator=(const SystemTransmogrifier&);
 ...
 private:
  ...
  void Insert(const std::type_info&, const std::type_info&, const Func&) const;
...
SystemTransmogrifier MakeDefaultSystemTransmogrifier() {
  return SystemTransmogrifier(SystemLabel<S>{});
}

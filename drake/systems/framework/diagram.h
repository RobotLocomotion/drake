 ...
 private:
  ...
  bool HasDirectFeedthroughFromAnyInput(
    ...
    for (int i = 0; i < system->get_num_input_ports(); i++) {
      ...
      // there is direct-feedthrough to output_port_id.
    ...
    // If none of the system's input ports create direct-feedthrough path back
    // to an input of the Diagram, there is no direct-feedthrough to
    // output_port_id.
    return false;
  }

...
struct DiagramOutput : public SystemOutput<T> {
 ...
 protected:
  // Returns a clone that includes a shallow copy of the pointers to the
  // constituent system outputs.
...
class Diagram : public ContinuousSystemInterface<T> {
 public:
  ...
  void Finalize() {
    ThrowIfFinal();
    if (systems_.size() == 0) {
      throw std::runtime_error("Cannot finalize an empty Diagram.");
    }
  ...
  std::unique_ptr<ContextBase<T>> CreateDefaultContext() const override {
    ...
    for (const PortIdentifier& id : input_port_ids_) {
      context->AddInput(ConvertToContextPortIdentifier(id));
    }
 ...
 private:
  ...
  std::vector<const SystemInterface<T>*> SortSystems() const {
    ...
    if (output.size() != systems_.size()) {
      ...
      throw std::runtime_error("Cycle detected in Diagram.");
    }
    return output;
  }

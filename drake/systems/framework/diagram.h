...
class Diagram : public System<T> {
 public:
  ...
  Diagram(const std::map<PortIdentifier, PortIdentifier>& dependency_graph,
    ...
    // Every system must appear in the sort order exactly once, and the sort
    // order must square with the dependency_graph_.
 ...
 private:
  ...
  bool SortOrderIsCorrect() const {
    std::map<const System<T>*, const System<T>*> prerequisites;

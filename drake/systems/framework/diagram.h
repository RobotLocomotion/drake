...
class DiagramDifferenceVariables : public DifferenceState<T> {
 public:
  explicit DiagramDifferenceVariables(
      std::vector<std::unique_ptr<DifferenceState<T>>>&& substates)
      : DifferenceState<T>(Flatten(Unpack(substates))),
        substates_(std::move(substates)) {}
  ...
  DifferenceState<T>* get_mutable_subdifference(int index) {
    DRAKE_DEMAND(index >= 0 && index < static_cast<int>(substates_.size()));
 ...
 private:
  ...
  std::vector<std::unique_ptr<DifferenceState<T>>> substates_;
};
...
class Diagram : public System<T>,
 ...
 private:
  ...
  void HandlePublish(
      const Context<T>& context,
      std::vector<std::pair<int, UpdateActions<T>>> sub_actions) const {

...
class System {
 public:
  ...
  /// handlers. The Simulator calls this method at the start of each continuous
  ...
  /// This method is the public entry point for dispatching all discrete variable
  ...
  /// This method forces an unrestricted update, which is able to change *any*
  /// state variables in the @p context by outputting the results to @p state.
  /// @sa CalcUnrestrictedUpdate(const Context<T>&, const EventCollection<UnrestrictedUpdateEvent<T>>*, State<T>* state)
  ...
  /// step, the simulator will merge @p event_info with the other EventCollection
 ...
 protected:
  ...
  virtual void DoCalcNextUpdateTime(const Context<T>& context,
      CompositeEventCollection<T>* event_info, T* time) const {

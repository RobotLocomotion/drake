...
struct DiscreteEvent {
  ...
  std::function<void(const Context<T>&, DiscreteState<T>*)> do_update = nullptr;
};
...
class System {
 public:
  ...
  /// because the given @p event has arrived.  Dispatches to DoUpdate by
  /// default, or to `event.do_update` if provided.
  void Update(const Context<T>& context, const DiscreteEvent<T>& event,
              DiscreteState<T>* discrete_state) const {
    DRAKE_ASSERT_VOID(CheckValidContext(context));
    DRAKE_DEMAND(event.action == DiscreteEvent<T>::kUpdateAction);
    if (event.do_update == nullptr) {
      DoUpdate(context, discrete_state);
    } else {
  ...
  /// should override.
  virtual std::unique_ptr<DiscreteState<T>> AllocateDiscreteUpdate() const {
    return nullptr;
  }

...
class System {
 public:
  ...
  static std::unique_ptr<S<AutoDiffXd>> ToAutoDiffXd(const S<T>& from) {
    ...
    std::unique_ptr<System<U>> clone(from_system.DoToAutoDiffXd());
    if (!clone) {
      std::stringstream ss;
    ...
    std::unique_ptr<S<U>> result(&dynamic_cast<S<U>&>(*clone));
    clone.release();
  ...
  /// scalar type. Returns nullptr if this System does not support the symbolic
  /// scalar type.
  ...
  /// type. Returns nullptr if this System does not support the symbolic scalar
  /// type.
  ///
  /// Usage: @code
  ///   MySystem<double> plant;
  ...
  static std::unique_ptr<S<symbolic::Expression>> ToSymbolic(const S<T>& from) {
    ...
    const System<T>& from_system = from;  // Upcast to unlock protected methods.
    std::unique_ptr<System<U>> clone(from_system.DoToSymbolic());
    if (!clone) {
      return nullptr;
    }

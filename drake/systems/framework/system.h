...
class System {
 ...
 protected:
  ...
  /// Adds a port with the specified @p type, @p size, and @p sample_time_sec
  /// to the input topology.
  void declare_input_port(PortDataType type, int size, double sample_time_sec) {
    input_topology_.emplace_back(this, kInputPort, input_topology_.size(),
                                 kVectorValued, size, sample_time_sec);
  }
  ...
  void declare_output_port(PortDataType type, int size,

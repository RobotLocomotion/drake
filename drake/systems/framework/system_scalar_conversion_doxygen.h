...
class MyDiagram : public Diagram<double> {
 public:
  MyDiagram() {
    ...
    const auto* integrator = builder.template AddSystem<Integrator<double>>(1);
...
code, `MyDiagram<double>::ToAutoDiffXd()` will return a object of runtime type
`Diagram<AutoDiffXd>`, not type `MyDiagram<AutoDiffXd>`.
...
namespace sample {
template <typename T>
class MyDiagram<T> : public Diagram<T> {
 public:
  MyDiagram() : Diagram<T>(SystemTypeTag<sample::MyDiagram>{}) {
    // ... same body as prior example (only the above line changes) ...
  }
};
@endcode

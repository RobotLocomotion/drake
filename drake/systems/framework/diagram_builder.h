...
class DiagramBuilder {
 public:
  ...
  ///   builder.AddSystem(std::make_unique<Foo<T>>());
  ///
  ...
  ///   builder.AddSystem<Foo<double>>("name", 3.14);
  ///
  /// Note that for dependent names you must use the template keyword:
  ///   DiagramBuilder<T> builder;
  ///   builder.template AddSystem<Foo<T>>("name", 3.14);
  ...
  ///
  /// DiagramBuilder<double> builder;
  /// builder.AddSystem<Foo>("name", 3.14);  // Foo must be a template.
  ///
  /// Note that for dependent names you must use the template keyword:
  ///   DiagramBuilder<T> builder;
  ///   builder.template AddSystem<Foo>("name", 3.14);
  /// You may prefer the `unique_ptr` variant instead.
  ///

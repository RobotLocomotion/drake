...
class DiagramBuilder {
 ...
 private:
  ...
  static bool HasCycleRecurse(
    ...
    auto IsInStack = [](const auto& vector, const auto& value) {
      for (const auto& test_value : vector) {
        if (test_value == value) return true;
      }
      return false;
    };

...
// A System that transmogrify only *from* double form.
template <typename T>
class SystemC : public MagicSystem<T> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(SystemC);

  // User constructor.
  explicit SystemC(int magic) : MagicSystem<T>(magic) {
    this->template SetConcreteSubclass<systems::SystemC>();
  }

  // Transmogrification constructor.
  SystemC(const TransmogrifierTag&, const SystemC<double>& other)
      : SystemC(other.magic()) {}
};

}  // namespace
namespace transmogrifier {
template <> struct Traits<SystemC> : public FromDoubleTraits {};
}
namespace {
...
GTEST_TEST(SystemTransmogrifierTest, TestSystemC) {
  ...
  const auto* system_c_autodiffxd =
      dynamic_cast<const SystemC<AutoDiffXd>*>(system_autodiffxd.get());

...
class DiagramContextTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    xd->get_mutable_value(0)->SetAtIndex(0, 44.0);
...
void VerifyClonedState(const State<double>& clone) {
  ...
  EXPECT_EQ(44.0, xd->get_value(0)->GetAtIndex(0));
  // - Abstract
...
TEST_F(DiagramContextTest, State) {
  ...
  EXPECT_EQ(1, xd->get_value(0)->size());
  ...
  EXPECT_EQ(44.0, hold_xd->get_value(0)->GetAtIndex(0));
  ...
  // - Discrete
  hold_xd->get_mutable_value(0)->SetAtIndex(0, 1001.0);
  EXPECT_EQ(1001.0, xd->get_value(0)->GetAtIndex(0));
}

...
class DiagramContextTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    context_->AddInput({0 /* sys0_ */, 1 /* port 1 */});
    context_->AddInput({1 /* sys1_ */, 0 /* port 0 */});
  }
...
TEST_F(DiagramContextTest, AddAndRetrieveConstituents) {
  ...
  EXPECT_NE(nullptr, context_->GetSubsystemOutput(1));
  EXPECT_NE(nullptr, context_->GetSubsystemContext(1));
}

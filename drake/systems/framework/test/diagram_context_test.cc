...
class DiagramContextTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    std::vector<PortIdentifier<double>> input_port_ids{{sys0_.get(), 1},
                                                       {sys1_.get(), 0}};

    context_.reset(new DiagramContext<double>(input_port_ids));
...
TEST_F(DiagramContextTest, AddAndRetrieveConstituents) {
  EXPECT_EQ(nullptr, context_->GetSubsystemOutput(sys2_.get()));
  EXPECT_EQ(nullptr, context_->GetSubsystemContext(sys2_.get()));

  context_->AddConstituentSystem(sys2_.get());

  EXPECT_NE(nullptr, context_->GetSubsystemOutput(sys1_.get()));
  EXPECT_NE(nullptr, context_->GetSubsystemContext(sys1_.get()));
}

// Tests that no exception is thrown when connecting a valid source
// and destination port.
TEST_F(DiagramContextTest, ConnectValid) {
  EXPECT_NO_THROW(context_->Connect({sys0_.get(), 0}, {sys1_.get(), 1}));
}

// Tests that an exception is thrown when connecting from a source port that
// does not exist.
TEST_F(DiagramContextTest, ConnectInvalidSrcPort) {
  EXPECT_THROW(context_->Connect({sys0_.get(), 1}, {sys1_.get(), 1}),
               std::out_of_range);
}
...
TEST_F(DiagramContextTest, ConnectInvalidDestPort) {
  EXPECT_THROW(context_->Connect({sys0_.get(), 0}, {sys1_.get(), 2}),
               std::out_of_range);
}
...
TEST_F(DiagramContextTest, Clone) {
  context_->Connect({sys0_.get(), 0}, {sys1_.get(), 1});
  AttachInputPorts();
  ...
  // sys1 input port 1.
  EXPECT_EQ(clone->GetSubsystemContext(sys1_.get())->get_vector_input(1),
            clone->GetSubsystemOutput(sys0_.get())->get_port(0).get_vector_data());
}

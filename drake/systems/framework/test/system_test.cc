...
class TestSystem : public System<double> {
 ...
 protected:
  ...
  void DoCalcNextUpdateTime(const Context<double>& context,
    ...
    if (context.get_time() < 10.0) {
      ...
      event.action = DiscreteEvent<double>::kUpdateAction;
      event.do_update = std::bind(&TestSystem::DoUpdateNumber, this,
                                  std::placeholders::_1 /* context */,
 ...
 private:
  ...
  void DoUpdateNumber(const Context<double>& context,
                      DiscreteState<double>* discrete_state, int num) const {
...
// Tests that the default DoUpdate is invoked when no other handler is
// registered in DoCalcNextUpdateTime.
TEST_F(SystemTest, DiscreteUpdate) {
  ...
  std::unique_ptr<DiscreteState<double>> update =
      system_.AllocateDiscreteUpdate();
  system_.Update(context_, actions.events[0], update.get());

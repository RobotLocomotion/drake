...
// Subclasses that violate the Vector premise fail-fast.
TEST_F(VectorSystemTest, TopologyFailFast) {
  ...
  { // Abstract state.
    ...
    // consistent with the Vector contract.  However, if we widened the Vector
    ...
    // doesn't seem worth it yet, but if we ever want abstract state in Vector,
...
TEST_F(VectorSystemTest, TimeDerivatives) {
  ...
  // There's no state declared yet, so the VectorSystem base shouldn't call our DUT.
...
TEST_F(VectorSystemTest, NoInputSystemTest) {
...
class NoInputNoOutputDiscreteTimeSystem : public VectorSystem<double> {
 public:
  NoInputNoOutputDiscreteTimeSystem()
      : VectorSystem<double>(0, 0) {  // Zero inputs, zero outputs.

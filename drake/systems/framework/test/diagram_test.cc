...
TEST_F(DiagramTest, Topology) {
  ASSERT_EQ(3u, diagram_->get_input_topology().size());
  for (const auto& descriptor : diagram_->get_input_topology()) {
  ...
  ASSERT_EQ(2u, diagram_->get_output_topology().size());
  for (const auto& descriptor : diagram_->get_output_topology()) {
    EXPECT_EQ(kVectorValued, descriptor.get_type());

...
class AbstractStateDiagram : public Diagram<double> {
 public:
  ...
  AbstractStateSystem* get_mutable_sys(int id) {
    DRAKE_DEMAND(id == 0 || id == 1);
    if (id) return sys1_;
    return sys0_;
  }
...
class AbstractStateDiagramTest : public ::testing::Test {
 protected:
  ...
  double get_abstract_data_as_double(int id) {
    const AbstractStateSystem* sys = diagram_.get_mutable_sys(id);

    const Context<double>& sys_context =
        diagram_.GetSubsystemContext(*context_, sys);
...
TEST_F(AbstractStateDiagramTest, CalcUnrestrictedUpdate) {
  ...
  double abs_data0 = get_abstract_data_as_double(0);
  double abs_data1 = get_abstract_data_as_double(1);

  // The abstract data should be initialized to their ids.
  EXPECT_EQ(abs_data0, 0);
  EXPECT_EQ(abs_data1, 1);
  ...
  abs_data0 = get_abstract_data_as_double(0);
  abs_data1 = get_abstract_data_as_double(1);
  EXPECT_EQ(abs_data0, 0);
  EXPECT_EQ(abs_data1, 1);

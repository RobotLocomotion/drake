...
class DiagramTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    diagram_->AddInput(adder0_.get(), 0);
    diagram_->AddInput(adder0_.get(), 1);
    diagram_->AddInput(adder1_.get(), 1);
    diagram_->AddOutput(adder1_.get(), 0);
    diagram_->AddOutput(adder2_.get(), 0);
...
TEST_F(DiagramTest, Cycle) {
  ...
  EXPECT_THROW(diagram_->Finalize(), std::runtime_error);
}
...
TEST_F(DiagramTest, Unfinalized) {
  diagram_.reset(new Diagram<double>("Unfinalized diagram!"));
  auto adder = std::make_unique<Adder<double>>(1 /* inputs */, 1 /* length */);
  diagram_->AddInput(adder.get(), 0);

  // Before the diagram is finalized, CreateDefaultContext should throw.
  EXPECT_THROW(diagram_->CreateDefaultContext(), std::runtime_error);

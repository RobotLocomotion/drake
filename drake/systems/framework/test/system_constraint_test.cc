...
GTEST_TEST(SystemConstraintTest, FunctionHandleTest) {
  ...
  EXPECT_EQ(dut.get_constraint(SystemConstraintIndex(0)).description(), "x1");
}
...
GTEST_TEST(SystemConstraintTest, DiagramTest) {
  ...
  Eigen::VectorXd value;
  diagram->get_constraint(SystemConstraintIndex(0)).Calc(*context, &value);
  EXPECT_EQ(value.size(), 1);
  EXPECT_EQ(value[0], 11.0);
  EXPECT_EQ(diagram->get_constraint(SystemConstraintIndex(0)).lower_bound()[0],
            0.0);
  EXPECT_EQ(diagram->get_constraint(SystemConstraintIndex(0)).upper_bound()[0],
            0.0);

  diagram->get_constraint(SystemConstraintIndex(1)).Calc(*context, &value);
  EXPECT_EQ(value.size(), 2);
  EXPECT_EQ(value[0], 11.0);
  EXPECT_EQ(value[1], 12.0);
  EXPECT_EQ(diagram->get_constraint(SystemConstraintIndex(1)).lower_bound()[1],
            0.0);
  EXPECT_EQ(diagram->get_constraint(SystemConstraintIndex(1)).upper_bound()[1],
            10.0);
}

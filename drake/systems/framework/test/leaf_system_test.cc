...
class TestSystem : public LeafSystem<double> {
 public:
  ...
  void AddPeriodicUpdate() {
    ...
    const double phase = 5.0;
    this->DeclarePeriodicUpdate(period, phase);
  }
...
// Tests that if the current time is smaller than the phase shift, the next
// update time is the phase shift.
TEST_F(LeafSystemTest, PhaseHasNotArrivedYet) {
...
// Tests that if the current time is exactly than the phase shift, the next
// update time is in the future.
TEST_F(LeafSystemTest, ExactlyAtPhase) {
...
// Tests that if the current time is larger than the phase shift, the next
// update time is determined by the period.
TEST_F(LeafSystemTest, PhaseIsInThePast) {

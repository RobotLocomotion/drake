...
GTEST_TEST(AutoDiffTest, ScalarConverterAutoDiff) {
  ...
  EXPECT_EQ(TestSystem<double>{}.ToAutoDiffXdMaybe(), nullptr);
...
GTEST_TEST(AutoDiffTest, ScalarConverterSymbolic) {
  ...
  // Conversion is not supported.
  EXPECT_EQ(TestSystem<double>{}.ToSymbolicMaybe(), nullptr);
  EXPECT_THROW(TestSystem<double>{}.ToSymbolic(), std::exception);
}

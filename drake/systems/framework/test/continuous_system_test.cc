...
class TestContinuousSystem : public ContinuousSystem<double> {
 public:
  ...
  std::unique_ptr<AbstractContext<double>> CreateDefaultContext()
      const override {
    return nullptr;
  }

  std::unique_ptr<SystemOutputInterface<double>> AllocateOutput(
      const AbstractContext<double>& context) const override {
    return nullptr;
  }

  void EvalOutput(const AbstractContext<double>& context,
                  SystemOutputInterface<double>* output) const override {}

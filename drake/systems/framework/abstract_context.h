...
/// AbstractContext is an abstract base class template that represents all
/// the inputs to a System: time, state, and input vectors. The framework
/// provides two concrete subclasses of AbstractContext: Context (for
...
class AbstractContext {
 public:
  ...
  /// Returns a deep copy of this AbstractContext. The clone's input ports will
  /// hold deep copies of the data that appears on this context's input ports
  /// at the time the clone is created.
  std::unique_ptr<AbstractContext<T>> Clone() const {
    return std::unique_ptr<AbstractContext<T>>(DoClone());
  }

 protected:
  /// Contains the return-type-covariant implementation of Clone().
  virtual AbstractContext<T>* DoClone() const = 0;

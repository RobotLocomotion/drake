...
/// port, and only a single output port.
///
...
class SisoVectorSystem : public LeafSystem<T> {
 public:
  ...
  void DoCalcTimeDerivatives(const Context<T>& context,
    ...
    Eigen::VectorBlock<const VectorX<T>> state_block =
  ...
  void DoCalcDiscreteVariableUpdates(
    ...
    // Obtain the block form of xd[n+1].
 ...
 protected:
  ...
  /// Performs the same logical operation as DoCalcOutput, but on VectorBlocks
  /// that have already been consistency-checked with the sizes declared at
  /// construction-time.
  ///
  /// The @p state will be either empty, the continuous state, or the discrete
  /// state, depending on which (or none) was declared at construction-time.
  ...
  /// Performs the same logical operation as DoCalcTimeDerivatives, but on
  /// VectorBlocks that have already been consistency-checked with the sizes
  /// declared at construction-time.
  ///
  /// The @p state will be either empty or the continuous state, depending on
  /// whether continuous state was declared at construction-time.
  ///
  ...
  /// Performs the same logical operation as DoCalcDiscreteVariableUpdates, but
  /// on VectorBlocks that have already been consistency-checked with the sizes
  /// declared at construction-time.
  ///
  /// The @p state will be either empty or the discrete state, depending on
  /// whether discrete state was declared at construction-time.
  ///

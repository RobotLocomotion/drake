...
class DiagramContext : public ContextBase<T> {
 public:
  ...
  void AddSystem(SystemIndex sys, std::unique_ptr<ContextBase<T>> context,
    ...
    DRAKE_ABORT_UNLESS(contexts_[sys] == nullptr);
    DRAKE_ABORT_UNLESS(outputs_[sys] == nullptr);
  ...
  /// Returns the output structure for a given constituent system @p sys.
  /// Aborts if @p sys is out of bounds, or if no system has been added to the
  /// DiagramContext at that index.
  SystemOutput<T>* GetSubsystemOutput(SystemIndex sys) const {
    DRAKE_ABORT_UNLESS(sys >= 0 && sys < static_cast<int>(outputs_.size()));
    DRAKE_ABORT_UNLESS(outputs_[sys] != nullptr);
    return outputs_[sys].get();
  }

  /// Returns the context structure for a given constituent system @p sys.
  /// Aborts if @p sys is out of bounds, or if no system has been added to the
  /// DiagramContext at that index.
  const ContextBase<T>* GetSubsystemContext(SystemIndex sys) const {
    DRAKE_ABORT_UNLESS(sys >= 0 && sys < static_cast<int>(contexts_.size()));
    DRAKE_ABORT_UNLESS(contexts_[sys] != nullptr);
    return contexts_[sys].get();
  }

  /// Returns the context structure for a given subsystem @p sys.
  /// Aborts if @p sys is out of bounds, or if no system has been added to the
  /// DiagramContext at that index.
 ...
 private:
  ...
  // Declaration order is critical: Output ports must outlive the
  // DependentInputPorts that depend on them, because the input ports
  // unregister themselves from output port change notifications at destruction
  // time. Thus, outputs_ must be declared before contexts_, so that contexts_
  // is destroyed before outputs_.

...
class OutputPort {
 public:
  ...
  explicit OutputPort(std::unique_ptr<V<T>> vec)
      : data_(new VectorValue<T>(
            std::unique_ptr<VectorInterface<T>>(vec.release()))) {}
  ...
  const VectorInterface<T>* get_vector_data() const {
    return data_->GetValue<VectorInterface<T>*>();
  }
 ...
 private:
  ...
  // The port data, if the port is abstract-valued.
...
class SystemOutput {
 public:
  ...
  const VectorInterface<T>* get_vector_data(int index) const {
    DRAKE_ASSERT(index >= 0 && index < get_num_ports());
  ...
  ///
  /// Throws std::bad_cast if this is not a vector-valued port.
  VectorInterface<T>* GetMutableVectorData(int index) {
    DRAKE_ASSERT(index >= 0 && index < get_num_ports());

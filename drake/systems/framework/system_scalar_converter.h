...
///
/// Because it is not templated on any a System subclass, this class can be
...
class SystemScalarConverter {
 public:
  ...
  /// scalar_conversion::Traits to disclaim support for some of all of these
 ...
 private:
  ...
  // added yet, returns a std::function that yields nullptr.
  ...
  // Adds converter for an S<U> into an S<T>, iff Traits says its supported.
...
std::unique_ptr<System<T>> SystemScalarConverter::Convert(
  ...
  // match, it will return an converter that yields null, _not_ one that lacks
  // a target (i.e., it will never return a null-ish ErasedConverterFunc).

...
class Eval {
 ...
 public:
  Eval(const VectorXd& breaks, const MatrixXd& K, const MatrixXd& A,
       const MatrixXd& alpha, const MatrixXd& gamma)
      : m_breaks(breaks),
        m_K(K),
        m_A(A),
        m_alpha(alpha),
        m_gamma(gamma),
        m_n(static_cast<int>(alpha.cols())),
        m_d(static_cast<int>(K.rows())),
        m_p(static_cast<int>(gamma.cols())) {}
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
  if (nrhs == 5 && nlhs == 1) {
    ...
    if (sizeof(eval) == 4)
      cid = mxUINT32_CLASS;
    else if (sizeof(eval) == 8)
      cid = mxUINT64_CLASS;
    else
      mexErrMsgIdAndTxt("Drake:ExpPlusPPTmex:PointerSize",
    ...
    if (nrhs == 0 || !mxIsNumeric(prhs[0]) ||
      ...
      mexErrMsgIdAndTxt("Drake:ExpPlusPPTmex:BadInputs",

...
void ThrowNonAffineError(const string& msg) {
  throw runtime_error(
      "DecomposeAffineSystem detects that an non-affine expression: " + msg +
      ".");
}

// A helper function for DecomposeAffineSystem. It finds the coefficient of the
// monomial `m` in the `map` and fill `M(i)` with the value. If the monomial `m`
// does not appear in `map`, it uses `0.0` instead. If the coefficient is not a
// constant, it throws a runtime_error.

...
class ConstantVectorSourceTest : public ::testing::Test {
 protected:
  void SetUpEigenModel() {
    ...
    output_ = source_->get_output_port(0).AllocateVector(*context_);
  }
...
TEST_F(ConstantVectorSourceTest, EigenModel) {
  ...
  source_->get_output_port(0).CalcVector(*context_, output_.get());

  const BasicVector<double>* output_vector = output_.get();
...
TEST_F(ConstantVectorSourceTest, BasicVectorModel) {
  ...
  auto output_vector =
      dynamic_cast<const MyVector<3, double>*>(output_.get());
  ASSERT_NE(nullptr, output_vector);

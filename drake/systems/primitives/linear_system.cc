...
double GetTimePeriodIfDiscreteUpdatesArePeriodic(
  ...
  // TODO(jadecastro) Clean up this implementation upon resolution of #6878.
  DRAKE_DEMAND(event_info->HasDiscreteUpdateEvents());
  ...
  DRAKE_DEMAND(discrete_events.size() == 1);
  DRAKE_DEMAND(discrete_events.front()->get_trigger_type() ==
...
Eigen::MatrixXd MakeStateAndInputMatrices(
  ...
  if (!is_approx_equal_abstol(math::autoDiffToValueMatrix(autodiff_x1_vec),

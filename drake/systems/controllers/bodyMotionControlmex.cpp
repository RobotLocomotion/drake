...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  if (nrhs < 1)
    mexErrMsgTxt(
        "usage: ptr = bodyMotionControlmex(0,robot_obj,Kp,Kd,body_index); "
        "y=bodyMotionControlmex(ptr,x,body_pose_des,body_v_des,body_vdot_des)");
  ...
  if (mxGetScalar(prhs[0]) == 0) {  // then construct the data object and return
    ...
    if (!mxIsNumeric(prhs[1]) || mxGetNumberOfElements(prhs[1]) != 1)
      mexErrMsgIdAndTxt("DRC:bodyMotionControlmex:BadInputs",
    ...
    if (sizeof(pdata) == 4)
      ...
      mexErrMsgIdAndTxt("Drake:bodyMotionControlmex:PointerSize",
  ...
  KinematicsCache<double> cache = pdata->r->doKinematics(
      q, qd);  // FIXME: pass this into the mex function instead
  ...
  pose_rpy = rotmat2rpy(transform.linear());
  ...
  J.topRows<3>() = pdata->r->transformPointsJacobian(
      cache, Vector3d::Zero().eval(), pdata->body_index, 0, true);
  J.bottomRows<3>() =
      pdata->r->relativeRollPitchYawJacobian(cache, pdata->body_index, 0, true);

  Vector6d body_vdot =
      (pdata->Kp.array() * body_error.array()).matrix() +
      (pdata->Kd.array() * (body_v_des - J * qd).array()).matrix() +
      body_vdot_des;

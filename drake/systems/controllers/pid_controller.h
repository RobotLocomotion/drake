 ...
 * <pre>
 * y = P_y * (kp * (q_d - q) + kd * (v_d - v) + ki * integral(q_d - q,
 * dt)),
 * </pre>
 ...
 * output port `y`. `x = P_x * x_input`, where `P_x` is the input
 ...
 * `P_x` and `P_yput` is to select a subset of state for feedback.
 *
...
class PidController : public StateFeedbackControllerInterface<T>,
 ...
 public:
   ...
   * @param state_projection The input projection matrix `P_x`.
   * @param kp P gain.
   * @param ki I gain.
   * @param kd D gain.
   *

...
void checkAndCopy(const mxArray *source, const int idx, const char *fieldname,
                  double *destination) {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  if (nrhs < 1 || nrhs > 2) {
    mexErrMsgTxt(
        "usage: msg_str=encodeQPInputLCMMex(qp_input) OR "
        "msg_str=encodeQPInputLCMMex(qp_input, publish_flag)");
  }

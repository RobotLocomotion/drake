...
drakeControlUtilEXPORT int contactPhi(RigidBodyManipulator* r, SupportStateElement& supp, VectorXd &phi, double terrain_height);
drakeControlUtilEXPORT int contactConstraintsBV(RigidBodyManipulator *r, int nc, std::vector<double> support_mus, std::vector<SupportStateElement,Eigen::aligned_allocator<SupportStateElement>>& supp, MatrixXd &B, MatrixXd &JB, MatrixXd &Jp, VectorXd &Jpdotv, MatrixXd &normals, double terrain_height);

...
PIDOutput wholeBodyPID(NewQPControllerData* pdata, double t,
                       ...
                       const Ref<const VectorXd>& qd,
                       const Ref<const VectorXd>& q_des,
  ...
  pdata->state.q_integrator_state =
      (1 - params->integrator.eta) * pdata->state.q_integrator_state +
      params->integrator.gains.cwiseProduct(q_des - q) * dt;
  pdata->state.q_integrator_state = pdata->state.q_integrator_state.array().max(
      -params->integrator.clamps.array());
  pdata->state.q_integrator_state = pdata->state.q_integrator_state.array().min(
      params->integrator.clamps.array());
  out.q_ref = q_des + pdata->state.q_integrator_state;
  out.q_ref = out.q_ref.array().max(
      (pdata->r->joint_limit_min - params->integrator.clamps).array());
  out.q_ref = out.q_ref.array().min(
      (pdata->r->joint_limit_max + params->integrator.clamps).array());
...
VectorXd velocityReference(NewQPControllerData* pdata, double t,
                           const Ref<VectorXd>& q, const Ref<VectorXd>& qd,
                           const Ref<VectorXd>& qdd, bool foot_contact[2],
  ...
  for (i = 0; i < rpc->position_indices.at("r_leg").size(); i++) {
    ...
    if (q(pos_ind) <= pdata->r->joint_limit_min(pos_ind) +
                          LEG_INTEGRATOR_DEACTIVATION_MARGIN) {
      qdd_limited(pos_ind) = std::max(qdd(pos_ind), 0.0);
    } else if (q(pos_ind) >= pdata->r->joint_limit_max(pos_ind) -
                                 LEG_INTEGRATOR_DEACTIVATION_MARGIN) {
  ...
  pdata->state.vref_integrator_state =
      (1 - params->eta) * pdata->state.vref_integrator_state +
  ...
  if (params->zero_ankles_on_contact && foot_contact[0] == 1) {
    for (i = 0; i < rpc->position_indices.at("l_leg_ak").size(); i++) {
      pdata->state.vref_integrator_state(
          rpc->position_indices.at("l_leg_ak")(i)) = 0;
    }
  }
  if (params->zero_ankles_on_contact && foot_contact[1] == 1) {
    for (i = 0; i < rpc->position_indices.at("r_leg_ak").size(); i++) {
      pdata->state.vref_integrator_state(
          rpc->position_indices.at("r_leg_ak")(i)) = 0;
    }
  }
  if (pdata->state.foot_contact_prev[0] != foot_contact[0]) {
    // contact state changed, reset integrated velocities
    for (i = 0; i < rpc->position_indices.at("l_leg").size(); i++) {
      pdata->state.vref_integrator_state(rpc->position_indices.at("l_leg")(i)) =
          qd(rpc->position_indices.at("l_leg")(i));
    }
  }
  if (pdata->state.foot_contact_prev[1] != foot_contact[1]) {
    // contact state changed, reset integrated velocities
    for (i = 0; i < rpc->position_indices.at("r_leg").size(); i++) {
      pdata->state.vref_integrator_state(rpc->position_indices.at("r_leg")(i)) =
          qd(rpc->position_indices.at("r_leg")(i));
    }
  }
...
void addJointSoftLimits(
  ...
  for (std::vector<drake::lcmt_joint_pd_override>::iterator it =
  ...
  for (auto support_it = active_supports.begin();
    ...
    for (auto contact_position_it = support.contact_pts.begin();
      ...
      contact_positions_world.col(col++) =
          r->transformPoints(cache, contact_point, support.body_idx, 0);
    }
  }
...
Vector2d computeCoP(RigidBodyTree* r, const KinematicsCache<double>& cache,
  ...
  std::pair<Eigen::Vector3d, double> cop_and_normal_torque =
      r->resolveCenterOfPressure(cache, force_torque_measurements, normal,
...
void estimateCoMBasedOnMeasuredZMP(
    NewQPControllerData* pdata, QPControllerParams* params,
    std::vector<SupportStateElement,
                Eigen::aligned_allocator<SupportStateElement>>& active_supports,
  ...
  double average_contact_point_height = averageContactPointHeight(
      pdata->r, pdata->cache, active_supports, num_contact_points);
  ...
  Vector2d zmp_from_force_sensors =
      computeCoP(pdata->r, pdata->cache, foot_force_torque_measurements,
                 point_on_contact_plane, Vector3d::UnitZ().eval());
  if (pdata->state.t_prev == 0) {
    pdata->state.center_of_mass_observer_state.topRows<2>() = xcom.topRows<2>();
    pdata->state.center_of_mass_observer_state.bottomRows<2>() =
        xcomdot.topRows<2>();
  }
  ...
  if (PUBLISH_ZMP_COM_OBSERVER_STATE) {
    ...
    if (lcm->good()) {
      ...
      eigenVectorToCArray(pdata->state.center_of_mass_observer_state.head<2>(),
                          zmp_com_observer_state_msg.com);
      eigenVectorToCArray(pdata->state.center_of_mass_observer_state.tail<2>(),
...
void checkCentroidalMomentumMatchesTotalWrench(
    RigidBodyTree* r, KinematicsCache<double>& cache, const VectorXd& qdd,
  ...
  for (size_t j = 0; j < active_supports.size(); j++) {
    ...
    Matrix3d R_world_to_body =
        r->relativeTransform(cache, active_support.body_idx, 0).linear();
    ...
    Isometry3d transform_to_world =
        r->relativeTransform(cache, 0, active_support.body_idx);
  ...
  auto world_momentum_matrix = r->worldMomentumMatrix(cache);
  auto world_momentum_matrix_dot_times_v =
      r->worldMomentumMatrixDotTimesV(cache);
...
int setupAndSolveQP(NewQPControllerData* pdata,
                    std::shared_ptr<drake::lcmt_qp_controller_input> qp_input,
                    DrakeRobotState& robot_state,
                    const Ref<Matrix<bool, Dynamic, 1>>& b_contact_force,
                    const std::map<Side, ForceTorqueMeasurement>&
                        foot_force_torque_measurements,
  ...
  std::vector<SupportStateElement,
              Eigen::aligned_allocator<SupportStateElement>> active_supports =
      getActiveSupports(pdata->r, robot_state.q, robot_state.qd,
                        available_supports, b_contact_force,
  ...
  Map<VectorXi> condof(qp_input->whole_body_data.constrained_dofs.data(),
                       qp_input->whole_body_data.num_constrained_dofs);

  PIDOutput pid_out = wholeBodyPID(pdata, robot_state.t, robot_state.q,
                                   robot_state.qd, q_des, &params->whole_body);
  VectorXd w_qdd = params->whole_body.w_qdd;

  addJointSoftLimits(params->joint_soft_limits, robot_state, q_des,
                     active_supports, qp_input->joint_pd_override);
  applyJointPDOverride(qp_input->joint_pd_override, robot_state, pid_out,
                       w_qdd);
  ...
  for (int i = 0; i < qp_input->num_tracked_bodies; i++) {
    ...
    Map<Vector4d> quat_task_to_world(
        qp_input->body_motion_data[i].quat_task_to_world);
    Map<Vector3d> translation_task_to_world(
    ...
    Map<Vector3d> xyz_kp_multiplier(
        qp_input->body_motion_data[i].xyz_kp_multiplier);
    Map<Vector3d> xyz_damping_ratio_multiplier(
    ...
    desired_body_accelerations[i].body_path = pdata->r->findKinematicPath(
    ...
    desired_body_accelerations[i].body_vdot = bodySpatialMotionPD(
        pdata->r, robot_state, body_or_frame_id0, body_pose_des, body_v_des,
  ...
  for (int i = 0; i < active_supports.size(); ++i) {
    ...
    if ((body_id == pdata->rpc.body_ids.l_foot ||
         body_id == pdata->rpc.body_ids.r_foot) &&
        !b_contact_force(active_supports[i].body_idx)) {
  ...
  // std::cout << std::endl;
  int nc = contactConstraintsBV(pdata->r, pdata->cache, num_active_contact_pts,
                                adjusted_mus, active_supports, B, JB, Jp,
                                Jpdotv, normals);
  int neps = nc * dim;

  if (params->use_center_of_mass_observer &&
      foot_force_torque_measurements.size() > 0) {
    estimateCoMBasedOnMeasuredZMP(pdata, params, active_supports, nc,
  ...
  VectorXd f(nparams);
  {
    if (nc > 0) {
      ...
      pdata->fqp -=
          (w_qdd.array() * pid_out.qddot_des.array()).matrix().transpose();
      if (include_angular_momentum) {
        pdata->fqp +=
            pdata->Akdot_times_v.transpose() * params->W_kdot * pdata->Ak;
  ...
  for (int i = 0; i < desired_body_accelerations.size(); i++) {
    if (desired_body_accelerations[i].weight <
      ...
      int body_id0 = pdata->r->parseBodyOrFrameID(
  ...
  if (pdata->use_fast_qp > 0 && !include_angular_momentum &&
    ...
    for (std::set<int>::iterator it = pdata->state.active.begin();
         it != pdata->state.active.end(); it++) {
    ...
    for (int i = 0; i < desired_body_accelerations.size(); i++) {
      if (desired_body_accelerations[i].weight > 0) {
        ...
        if (desired_body_accelerations[i].control_pose_when_in_contact ||
          ...
          for (int j = 0; j < 6; j++) {
            if (!std::isnan(desired_body_accelerations[i].body_vdot[j])) {
              pdata->Hqp += desired_body_accelerations[i].weight *
    ...
    if (pdata->use_fast_qp > 0) {  // set up and call fastqp
      info = fastQP(QBlkDiag, f, Aeq, beq, Ain_lb_ub, bin_lb_ub,
                    pdata->state.active, alpha);
      // if (info<0)    mexPrintf("fastQP info=%d... calling Gurobi.\n", info);
    } else {
      // use gurobi active set
      model = gurobiActiveSetQP(pdata->env, QBlkDiag, f, Aeq, beq, Ain, bin, lb,
                                ub, pdata->state.vbasis,
                                pdata->state.vbasis_len, pdata->state.cbasis,
                                pdata->state.cbasis_len, alpha);
      CGE(GRBgetintattr(model, "NumVars", &(pdata->state.vbasis_len)),
          pdata->env);
      CGE(GRBgetintattr(model, "NumConstrs", &(pdata->state.cbasis_len)),
          pdata->env);
      info = 66;
      // info = -1;
    }
    ...
    if (info < 0) {
      model = gurobiQP(pdata->env, QBlkDiag, f, Aeq, beq, Ain, bin, lb, ub,
                       pdata->state.active, alpha);
      int status;
      CGE(GRBgetintattr(model, "Status", &status), pdata->env);
  ...
  if (CHECK_CENTROIDAL_MOMENTUM_RATE_MATCHES_TOTAL_WRENCH) {
    checkCentroidalMomentumMatchesTotalWrench(pdata->r, cache, qp_output->qdd,
                                              active_supports, B, beta);
  }

  // use transpose because B_act is orthogonal
  qp_output->u = pdata->B_act.transpose() *
                 (pdata->H_act * qp_output->qdd + pdata->C_act - D_act * beta);
  for (int i = 0; i < qp_output->u.size(); i++) {
    if (std::isnan(qp_output->u(i))) qp_output->u(i) = 0;
  }
  // y =
  // pdata->B_act.jacobiSvd(ComputeThinU|ComputeThinV).solve(pdata->H_act*qdd +
  // pdata->C_act - Jz_act.transpose()*lambda - D_act*beta);

  bool foot_contact[2];
  foot_contact[0] = b_contact_force(pdata->rpc.body_ids.l_foot) == 1;
  foot_contact[1] = b_contact_force(pdata->rpc.body_ids.r_foot) == 1;
  qp_output->qd_ref = velocityReference(
      pdata, robot_state.t, robot_state.q, robot_state.qd, qp_output->qdd,
      foot_contact, &(params->vref_integrator), &(pdata->rpc));

...
int setupAndSolveQP(
  ...
  pdata->r->doKinematics(pdata->cache, false);
  ...
  // std::cout << std::endl;
  int nc = contactConstraintsBV(pdata->r, pdata->cache, num_active_contact_pts, adjusted_mus, active_supports, pdata->map_ptr, B, JB, Jp, Jpdotv, normals, pdata->default_terrain_height);
  int neps = nc*dim;

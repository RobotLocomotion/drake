...
void InstantaneousQPController::initialize() {
  ...
  qdd_lb = Eigen::VectorXd::Zero(nq).array() - std::numeric_limits<double>::infinity();
  qdd_ub = Eigen::VectorXd::Zero(nq).array() + std::numeric_limits<double>::infinity();
  ...
  controller_state.vref_integrator_state = Eigen::VectorXd::Zero(robot->num_velocities);
  controller_state.q_integrator_state = Eigen::VectorXd::Zero(robot->num_positions);
...
void InstantaneousQPController::loadConfigurationFromYAML(const std::string& control_config_filename) {
...
PIDOutput InstantaneousQPController::wholeBodyPID(double t, const Ref<const VectorXd> &q, const Ref<const VectorXd> &qd, const Ref<const VectorXd> &q_des, const WholeBodyParams& params) {
  ...
  controller_state.q_integrator_state = (1-params.integrator.eta) * controller_state.q_integrator_state + params.integrator.gains.cwiseProduct(q_des - q) * dt;
  controller_state.q_integrator_state = controller_state.q_integrator_state.array().max(-params.integrator.clamps.array());
  controller_state.q_integrator_state = controller_state.q_integrator_state.array().min(params.integrator.clamps.array());
  out.q_ref = q_des + controller_state.q_integrator_state;
  out.q_ref = out.q_ref.array().max((robot->joint_limit_min - params.integrator.clamps).array());
  out.q_ref = out.q_ref.array().min((robot->joint_limit_max + params.integrator.clamps).array());
...
VectorXd InstantaneousQPController::velocityReference(double t, const Ref<const VectorXd> &q, const Ref<const VectorXd> &qd, const Ref<const VectorXd> &qdd, bool foot_contact[2], const VRefIntegratorParams& params) {
  ...
  for (i=0; i < rpc.position_indices.legs.at(Side::RIGHT).size(); i++) {
    ...
    if (q(pos_ind) <= robot->joint_limit_min(pos_ind) + LEG_INTEGRATOR_DEACTIVATION_MARGIN) {
      qdd_limited(pos_ind) = std::max(qdd(pos_ind), 0.0);
    } else if (q(pos_ind) >= robot->joint_limit_max(pos_ind) - LEG_INTEGRATOR_DEACTIVATION_MARGIN) {
  ...
  controller_state.vref_integrator_state = (1-params.eta)*controller_state.vref_integrator_state + params.eta*qd + qdd_limited*dt;

  if (params.zero_ankles_on_contact && foot_contact[0] == 1) {
    for (i=0; i < rpc.position_indices.ankles.at(Side::LEFT).size(); i++) {
      controller_state.vref_integrator_state(rpc.position_indices.ankles.at(Side::LEFT)[i]) = 0;
    }
  }
  if (params.zero_ankles_on_contact && foot_contact[1] == 1) {
    for (i=0; i < rpc.position_indices.ankles.at(Side::RIGHT).size(); i++) {
      controller_state.vref_integrator_state(rpc.position_indices.ankles.at(Side::RIGHT)[i]) = 0;
    }
  }
  if (controller_state.foot_contact_prev[0] != foot_contact[0]) {
    // contact state changed, reset integrated velocities
    for (i=0; i < rpc.position_indices.legs.at(Side::LEFT).size(); i++) {
      controller_state.vref_integrator_state(rpc.position_indices.legs.at(Side::LEFT)[i]) = qd(rpc.position_indices.legs.at(Side::LEFT)[i]);
    }
  }
  if (controller_state.foot_contact_prev[1] != foot_contact[1]) {
    // contact state changed, reset integrated velocities
    for (i=0; i < rpc.position_indices.legs.at(Side::RIGHT).size(); i++) {
      controller_state.vref_integrator_state(rpc.position_indices.legs.at(Side::RIGHT)[i]) = qd(rpc.position_indices.legs.at(Side::RIGHT)[i]);
    }
  }
...
std::vector<SupportStateElement,Eigen::aligned_allocator<SupportStateElement>> InstantaneousQPController::loadAvailableSupports(const drake::lcmt_qp_controller_input& qp_input) {
  ...
  for (int i=0; i < qp_input.num_support_data; i++) {
    available_supports[i].body_idx = body_or_frame_name_to_id.at(qp_input.support_data[i].body_name);
    for (int j=0; j < 4; j++) {
...
double averageContactPointHeight(const RigidBodyTree& robot, const KinematicsCache<double>& cache, std::vector<SupportStateElement, Eigen::aligned_allocator<SupportStateElement> >& active_supports, int nc)
{
  ...
  for (auto support_it = active_supports.begin(); support_it != active_supports.end(); ++support_it) {
    ...
    for (auto contact_position_it = support.contact_pts.begin(); contact_position_it != support.contact_pts.end(); ++contact_position_it) {
      ...
      contact_positions_world.col(col++) = robot.transformPoints(cache, contact_point, support.body_idx, 0);
    }
  }
...
Vector2d computeCoP(const RigidBodyTree& robot, const KinematicsCache<double>& cache, const std::map<Side, ForceTorqueMeasurement>& foot_force_torque_measurements, Vector3d point_on_contact_plane, Eigen::Vector3d normal)
{
  ...
  std::pair<Eigen::Vector3d, double> cop_and_normal_torque = robot.resolveCenterOfPressure(cache, force_torque_measurements, normal, point_on_contact_plane);
  Vector2d zmp_from_force_sensors = cop_and_normal_torque.first.head<2>();
  return zmp_from_force_sensors;
}

void InstantaneousQPController::estimateCoMBasedOnMeasuredZMP(const QPControllerParams& params, std::vector<SupportStateElement, Eigen::aligned_allocator<SupportStateElement> >& active_supports, int num_contact_points, const std::map<Side, ForceTorqueMeasurement>& foot_force_torque_measurements, double dt,
    Vector3d& xcom, Vector3d& xcomdot)
{
  ...
  double average_contact_point_height = averageContactPointHeight(*robot, cache, active_supports, num_contact_points);
  Vector3d point_on_contact_plane;
  point_on_contact_plane << 0.0, 0.0, average_contact_point_height;
  Vector2d zmp_from_force_sensors = computeCoP(*robot, cache, foot_force_torque_measurements, point_on_contact_plane, Vector3d::UnitZ().eval());
  if (controller_state.t_prev == 0) {
    controller_state.center_of_mass_observer_state.topRows<2>() = xcom.topRows<2>();
    controller_state.center_of_mass_observer_state.bottomRows<2>() = xcomdot.topRows<2>();
  }
  ...
  xcom.topRows<2>() = controller_state.center_of_mass_observer_state.topRows<2>();
  // xcomdot.topRows<2>() = controller_state.center_of_mass_observer_state.bottomRows<2>();
  ...
  if (PUBLISH_ZMP_COM_OBSERVER_STATE) {
    ...
    if (lcm->good()) {
      ...
      eigenVectorToCArray(controller_state.center_of_mass_observer_state.head<2>(), zmp_com_observer_state_msg.com);
      eigenVectorToCArray(controller_state.center_of_mass_observer_state.tail<2>(), zmp_com_observer_state_msg.comd);
...
void checkCentroidalMomentumMatchesTotalWrench(const RigidBodyTree& robot, KinematicsCache<double>& cache, const VectorXd& qdd, const std::vector<SupportStateElement, Eigen::aligned_allocator<SupportStateElement> >& active_supports, const MatrixXd& B, const VectorXd& beta)
{
  ...
  for (size_t j = 0; j < active_supports.size(); j++) {
    ...
    Matrix3d R_world_to_body = robot.relativeTransform(cache, active_support.body_idx, 0).linear();
    ...
    Isometry3d transform_to_world = robot.relativeTransform(cache, 0, active_support.body_idx);
  ...
  auto world_momentum_matrix_dot_times_v = robot.worldMomentumMatrixDotTimesV(cache);
...
int InstantaneousQPController::setupAndSolveQP(const drake::lcmt_qp_controller_input& qp_input, const DrakeRobotState &robot_state, const Ref<const Matrix<bool, Dynamic, 1> > &contact_detected, const std::map<Side, ForceTorqueMeasurement>& foot_force_torque_measurements, QPControllerOutput& qp_output, QPControllerDebugData* debug) {
  ...
  std::vector<SupportStateElement,Eigen::aligned_allocator<SupportStateElement>> active_supports = getActiveSupports(*robot, robot_state.q, robot_state.qd, available_supports, contact_detected, params.contact_threshold);
  ...
  for (int i=0; i < qp_input.num_tracked_bodies; i++) {
    int body_or_frame_id0 = body_or_frame_name_to_id.at(qp_input.body_motion_data[i].body_or_frame_name);
    ...
    Map<const Vector4d> quat_task_to_world(qp_input.body_motion_data[i].quat_task_to_world);
    Map<const Vector3d> translation_task_to_world(qp_input.body_motion_data[i].translation_task_to_world);
    ...
    Map<const Vector3d> xyz_kp_multiplier(qp_input.body_motion_data[i].xyz_kp_multiplier);
    Map<const Vector3d> xyz_damping_ratio_multiplier(qp_input.body_motion_data[i].xyz_damping_ratio_multiplier);
    ...
    desired_body_accelerations[i].body_path = robot->findKinematicPath(0, desired_body_accelerations[i].body_or_frame_id0);
    ...
    desired_body_accelerations[i].body_vdot = bodySpatialMotionPD(*robot, robot_state, body_or_frame_id0, body_pose_des, body_v_des, body_vdot_des, body_Kp, body_Kd,desired_body_accelerations[i].T_task_to_world);
  ...
  for (int i=0; i < active_supports.size(); ++i) {
    ...
    if ((body_id == rpc.foot_ids.at(Side::RIGHT) || body_id == rpc.foot_ids.at(Side::LEFT)) && (contact_detected(active_supports[i].body_idx) == 0)) {
  ...
  // std::cout << std::endl;
  int nc = contactConstraintsBV(*robot, cache, num_active_contact_pts, adjusted_mus, active_supports, B, JB, Jp, Jpdotv, normals);
  int neps = nc*dim;

  if (params.use_center_of_mass_observer && foot_force_torque_measurements.size() > 0) {
    estimateCoMBasedOnMeasuredZMP(params, active_supports, nc, foot_force_torque_measurements, dt, xcom, xcomdot);
  }
  ...
  for (int i=0; i<desired_body_accelerations.size(); i++) {
    if (desired_body_accelerations[i].weight < 0) { // negative implies constraint
      int body_id0 = robot->parseBodyOrFrameID(desired_body_accelerations[i].body_or_frame_id0);
      if (desired_body_accelerations[i].control_pose_when_in_contact || !inSupport(active_supports,body_id0)) {
        Matrix<double,6,Dynamic> Jb_compact = robot->geometricJacobian(cache, 0,desired_body_accelerations[i].body_or_frame_id0, desired_body_accelerations[i].body_or_frame_id0, true);
        Jb = robot->compactToFull<Matrix<double,6,Dynamic>>(Jb_compact,desired_body_accelerations[i].body_path.joint_path,true);
        Jbdotv = robot->geometricJacobianDotTimesV(cache, 0,desired_body_accelerations[i].body_or_frame_id0,desired_body_accelerations[i].body_or_frame_id0);
  ...
  { 
    ...
    if (nc>0) {
      ...
      if (R_DQyD_ls.trace()>1e-15) { // R_DQyD_ls is not zero
        Hqp = Wi - Wi*Jcom.transpose()*(R_DQyD_ls.inverse() + Jcom*Wi*Jcom.transpose()).inverse()*Jcom*Wi;
      }
    } 
    else {
    ...
    for (std::set<int>::iterator it = controller_state.active.begin(); it != controller_state.active.end(); it++) {
    ...
    info = fastQPThatTakesQinv(QBlkDiag, f, Aeq, beq, Ain_lb_ub, bin_lb_ub, controller_state.active, alpha);
    ...
    for (int i=0; i<desired_body_accelerations.size(); i++) {
      if (desired_body_accelerations[i].weight > 0) {
        ...
        if (desired_body_accelerations[i].control_pose_when_in_contact || !inSupport(active_supports,body_id0)) {
          ...
          for (int j=0; j<6; j++) {
            if (!std::isnan(desired_body_accelerations[i].body_vdot[j])) {
              Hqp += desired_body_accelerations[i].weight*desired_body_accelerations[i].weight_multiplier(j)*(Jb.row(j)).transpose()*Jb.row(j);
    ...
    { // set up and call fastqp
      info = fastQP(QBlkDiag, f, Aeq, beq, Ain_lb_ub, bin_lb_ub, controller_state.active, alpha);
      //if (info<0)    mexPrintf("fastQP info=%d... calling Gurobi.\n", info);
    }
    else {
      // use gurobi active set 
      model = gurobiActiveSetQP(env,QBlkDiag,f,Aeq,beq,Ain,bin,lb,ub,controller_state.vbasis,controller_state.vbasis_len,controller_state.cbasis,controller_state.cbasis_len,alpha);
      CGE(GRBgetintattr(model,"NumVars",&(controller_state.vbasis_len)), env);
      CGE(GRBgetintattr(model,"NumConstrs",&(controller_state.cbasis_len)), env);
      info=66;
      //info = -1;
    }

    if (info<0) {
      model = gurobiQP(env,QBlkDiag,f,Aeq,beq,Ain,bin,lb,ub,controller_state.active,alpha);
      int status; CGE(GRBgetintattr(model, "Status", &status), env);
  ...
  if (CHECK_CENTROIDAL_MOMENTUM_RATE_MATCHES_TOTAL_WRENCH) {
    checkCentroidalMomentumMatchesTotalWrench(*robot, cache, qp_output.qdd, active_supports, B, beta);
  }

  // use transpose because B_act is orthogonal
  qp_output.u = B_act.transpose()*(H_act*qp_output.qdd + C_act - D_act*beta);
  for (int i=0; i < qp_output.u.size(); i++) {
      if (std::isnan(qp_output.u(i))) qp_output.u(i) = 0;
  }
  //y = B_act.jacobiSvd(ComputeThinU|ComputeThinV).solve(H_act*qdd + C_act - Jz_act.transpose()*lambda - D_act*beta);

  bool foot_contact[2];
  foot_contact[0] = contact_detected(rpc.foot_ids.at(Side::LEFT)) == 1;
  foot_contact[1] = contact_detected(rpc.foot_ids.at(Side::RIGHT)) == 1;
  qp_output.qd_ref = velocityReference(robot_state.t, robot_state.q, robot_state.qd, qp_output.qdd, foot_contact, params.vref_integrator);

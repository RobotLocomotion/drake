...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  NewQPControllerData *pdata =
      (NewQPControllerData *)getDrakeMexPointer(prhs[0]);
  ...
  // qp_input
  shared_ptr<drake::lcmt_qp_controller_input> qp_input(
  ...
  Map<VectorXd> contact_force_detected(mxGetPrSafe(pobj),
                                       mxGetNumberOfElements(pobj), 1);
  Matrix<bool, Dynamic, 1> b_contact_force =
      Matrix<bool, Dynamic, 1>::Zero(contact_force_detected.size());
  ...
  if (nrhs > 5) {
    ...
    if (!mxIsEmpty(mex_foot_force_torque_measurements)) {
      ...
      foot_force_torque_measurements[Side::RIGHT].frame_idx =
          pdata->r->findLinkId("r_foot");
      foot_force_torque_measurements[Side::RIGHT].wrench =
          matlabToEigenMap<TWIST_SIZE, 1>(
              mxGetFieldSafe(mex_foot_force_torque_measurements, "right"));
    }
  }
  int info = setupAndSolveQP(pdata, qp_input, robot_state, b_contact_force,
                             foot_force_torque_measurements, &qp_output, debug);
  ...
  if (nlhs > narg) {
    ...
    for (vector<SupportStateElement,
             ...
             debug->active_supports.begin();
         iter != debug->active_supports.end(); iter++) {
  ...
  if (nlhs > narg) {
    ...
    if (debug->nc > 0)
      ...
      // term here
      Vdot =
          ((2 * debug->x_bar.transpose() * debug->S + debug->s1.transpose()) *
               (debug->A_ls * debug->x_bar +
                debug->B_ls * (debug->Jcomdotv + debug->Jcom * qp_output.qdd)) +
           debug->s1dot.transpose() * debug->x_bar)(0) +
          debug->s2dot;
    else
      Vdot = 0;
  ...
  if (nlhs > narg) {
    ...
    VectorXd individual_cops =
        individualSupportCOPs(r, pdata->cache, debug->active_supports,
                              debug->normals, debug->B, debug->beta);

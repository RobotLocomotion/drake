...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (nrhs < 2) mexErrMsgTxt("usage: ptr = constructQPDataPointerMex(urdf_filename, control_config_filename, urdf_modifications_filename="");");
  ...
  if (nrhs >= 3) {
    ...
    std::cout << "urdf_modifications_filename: " << urdf_modifications_filename << std::endl;
    if (urdf_modifications_filename.size() > 0) {
      applyURDFModifications(robot, urdf_modifications_filename);
    }
  }
  InstantaneousQPController* controller = new InstantaneousQPController(std::move(robot), control_config_filename);

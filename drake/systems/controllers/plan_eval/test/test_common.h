...
class GenericPlanTest : public ::testing::Test {
 protected:
  ...
  void AllocateResources(const std::string& robot_path,
    ...
    alias_groups_ = RigidBodyTreeAliasGroupsLoadFromFile(robot_.get(),
                                                         alias_groups_path);

    paramset_ = qp_inverse_dynamics::ParamSetLoadFromFile(control_param_path,
                                                          *alias_groups_);
  ...
  Vector6<double> ComputeExpectedBodyAcceleration(
    ...
    paramset_->LookupDesiredBodyMotionGains(*body, &kp, &kd);
  ...
  std::unique_ptr<qp_inverse_dynamics::ParamSet> paramset_{nullptr};
  std::unique_ptr<
      systems::controllers::qp_inverse_dynamics::RobotKinematicState<double>>

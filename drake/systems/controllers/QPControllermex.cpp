...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
  ...
  if (nrhs < 1)
    mexErrMsgTxt(
        "usage: ptr = QPControllermex(0,control_obj,robot_obj,...); "
        "alpha=QPControllermex(ptr,...,...)");
  ...
  if (mxGetScalar(prhs[0]) == 0) {  // then construct the data object and return
    ...
    memcpy(pdata->W_kdot.data(), mxGetPrSafe(pm),
           sizeof(double) * mxGetM(pm) * mxGetN(pm));
    ...
    for (int i = 0; i < pdata->n_body_accel_bounds; i++) {
      pdata->accel_bound_body_idx.push_back(
          (int)mxGetScalar(mxGetField(body_accel_bounds, i, "body_idx")) - 1);
    ...
    memcpy(pdata->body_accel_input_weights.data(), mxGetPrSafe(pm),
           sizeof(double) * pdata->n_body_accel_inputs);
    ...
    if (!mxIsNumeric(prhs[2]) || mxGetNumberOfElements(prhs[2]) != 1)
      mexErrMsgIdAndTxt("Drake:QPControllermex:BadInputs",
                        "the third argument should be the robot mex ptr");
    memcpy(&(pdata->r), mxGetData(prhs[2]), sizeof(pdata->r));

    pdata->B.resize(mxGetM(prhs[3]), mxGetN(prhs[3]));
    memcpy(pdata->B.data(), mxGetPrSafe(prhs[3]),
           sizeof(double) * mxGetM(prhs[3]) * mxGetN(prhs[3]));
    ...
    if (!mxIsNumeric(prhs[6]) || mxGetNumberOfElements(prhs[6]) != 1)
      mexErrMsgIdAndTxt("Drake:QPControllermex:BadInputs",
                        "the seventh argument should be the map ptr");
    ...
    if (sizeof(pdata) == 4)
      ...
      mexErrMsgIdAndTxt("Drake:constructModelmex:PointerSize",
  ...
  //    pdata->r->bodies[i].linkname.c_str(),
  //    pdata->r->bodies[i].contact_pts.cols());

  int nu = pdata->B.cols(), nq = pdata->r->num_positions,
      nv = pdata->r->num_velocities;
  const int dim = 3,  // 3D
      nd = 2 *
  ...
  if (!mxIsEmpty(prhs[narg])) {
    ...
    memcpy(condof.data(), mxGetPrSafe(prhs[narg++]),
           sizeof(double) * num_condof);
  } else {
    num_condof = 0;
  ...
  Map<MatrixXd> A_ls(mxGetPrSafe(prhs[narg]), mxGetM(prhs[narg]),
                     mxGetN(prhs[narg]));
  narg++;
  Map<MatrixXd> B_ls(mxGetPrSafe(prhs[narg]), mxGetM(prhs[narg]),
                     mxGetN(prhs[narg]));
  narg++;
  Map<MatrixXd> Qy(mxGetPrSafe(prhs[narg]), mxGetM(prhs[narg]),
                   mxGetN(prhs[narg]));
  narg++;
  Map<MatrixXd> R_ls(mxGetPrSafe(prhs[narg]), mxGetM(prhs[narg]),
                     mxGetN(prhs[narg]));
  narg++;
  Map<MatrixXd> C_ls(mxGetPrSafe(prhs[narg]), mxGetM(prhs[narg]),
                     mxGetN(prhs[narg]));
  narg++;
  Map<MatrixXd> D_ls(mxGetPrSafe(prhs[narg]), mxGetM(prhs[narg]),
                     mxGetN(prhs[narg]));
  narg++;
  Map<MatrixXd> S(mxGetPrSafe(prhs[narg]), mxGetM(prhs[narg]),
                  mxGetN(prhs[narg]));
  narg++;

  Map<VectorXd> s1(mxGetPrSafe(prhs[narg]), mxGetNumberOfElements(prhs[narg]));
  narg++;
  Map<VectorXd> s1dot(mxGetPrSafe(prhs[narg]),
                      mxGetNumberOfElements(prhs[narg]));
  narg++;
  double s2dot = mxGetScalar(prhs[narg++]);
  Map<VectorXd> x0(mxGetPrSafe(prhs[narg]), mxGetNumberOfElements(prhs[narg]));
  narg++;
  Map<VectorXd> u0(mxGetPrSafe(prhs[narg]), mxGetNumberOfElements(prhs[narg]));
  narg++;
  Map<VectorXd> y0(mxGetPrSafe(prhs[narg]), mxGetNumberOfElements(prhs[narg]));
  narg++;
  Map<VectorXd> qdd_lb(mxGetPrSafe(prhs[narg]),
                       mxGetNumberOfElements(prhs[narg]));
  narg++;
  Map<VectorXd> qdd_ub(mxGetPrSafe(prhs[narg]),
                       mxGetNumberOfElements(prhs[narg]));
  narg++;
  ...
  if (!mxIsEmpty(prhs[desired_support_argid])) {
    ...
    mxArray* mxContactPts =
        myGetField(prhs[desired_support_argid], "contact_pts");
    ...
    for (i = 0; i < mxGetNumberOfElements(mxBodies); i++) {
      ...
      all_body_contact_pts.resize(mxGetM(mxBodyContactPts),
                                  mxGetN(mxBodyContactPts));
      pr = mxGetPrSafe(mxBodyContactPts);
      memcpy(
          all_body_contact_pts.data(), pr,
          sizeof(double) * mxGetM(mxBodyContactPts) * mxGetN(mxBodyContactPts));
  ...
  int nc =
      contactConstraintsBV(pdata->r, cache, num_active_contact_pts, support_mus,
  ...
  VectorXd f(nparams);
  {
    if (nc > 0) {
      ...
      pdata->fqp -=
          (pdata->w_qdd.array() * qddot_des.array()).matrix().transpose();
      if (include_angular_momentum) {
        pdata->fqp +=
            pdata->Akdot_times_v.transpose() * pdata->W_kdot * pdata->Ak;
  ...
  for (int i = 0; i < pdata->n_body_accel_inputs; i++) {
    if (pdata->body_accel_input_weights(i) < 0) {
      ...
      if (!inSupport(active_supports, body_idx)) {
        ...
        Jb.topRows<3>() = pdata->r->transformPointsJacobian(cache, origin,
                                                            body_idx, 0, false);
        Jb.bottomRows<3>() =
            pdata->r->relativeRollPitchYawJacobian(cache, body_idx, 0, false);

        Matrix<double, 6, 1> Jbdot_times_v;
        Jbdot_times_v.topRows<3>() = pdata->r->transformPointsJacobianDotTimesV(
            cache, origin, body_idx, 0);
        Jbdot_times_v.bottomRows<3>() =
            pdata->r->relativeRollPitchYawJacobianDotTimesV(cache, body_idx, 0);
  ...
  for (int i = 0; i < pdata->n_body_accel_bounds; i++) {
    ...
    Matrix<double, 6, Dynamic> Jb(6, pdata->r->num_velocities);
    Jb.topRows<3>() =
        pdata->r->transformPointsJacobian(cache, origin, body_index, 0, false);
    Jb.bottomRows<3>() =
        pdata->r->relativeRollPitchYawJacobian(cache, body_index, 0, false);

    Matrix<double, 6, 1> Jbdot_times_v;
    Jbdot_times_v.topRows<3>() = pdata->r->transformPointsJacobianDotTimesV(
        cache, origin, body_index, 0);
    Jbdot_times_v.bottomRows<3>() =
        pdata->r->relativeRollPitchYawJacobianDotTimesV(cache, body_index, 0);

    Ain.block(constraint_start_index, 0, 6, pdata->r->num_positions) = Jb;
    bin.segment(constraint_start_index, 6) =
        -Jbdot_times_v + pdata->max_body_acceleration[i];
    constraint_start_index += 6;
    Ain.block(constraint_start_index, 0, 6, pdata->r->num_positions) = -Jb;
    bin.segment(constraint_start_index, 6) =
        Jbdot_times_v - pdata->min_body_acceleration[i];
    constraint_start_index += 6;
  }
  ...
  bool include_body_accel_cost_terms =
      pdata->n_body_accel_inputs > 0 &&
      pdata->body_accel_input_weights.array().maxCoeff() > 1e-10;
  if (use_fast_qp > 0 && !include_angular_momentum &&
    ...
    for (int i = 0; i < pdata->n_body_accel_inputs; i++) {
      ...
      if (w_i > 0) {
        ...
        if (!inSupport(active_supports, body_idx)) {
          // TODO: should use geometric Jacobians instead
          Matrix<double, 6, Dynamic> Jb(6, pdata->r->num_velocities);
          Jb.topRows<3>() = pdata->r->transformPointsJacobian(
              cache, origin, body_idx, 0, false);
          Jb.bottomRows<3>() =
              pdata->r->relativeRollPitchYawJacobian(cache, body_idx, 0, false);

          Matrix<double, 6, 1> Jbdot_times_v;
          Jbdot_times_v.topRows<3>() =
              pdata->r->transformPointsJacobianDotTimesV(cache, origin,
                                                         body_idx, 0);
          Jbdot_times_v.bottomRows<3>() =
              pdata->r->relativeRollPitchYawJacobianDotTimesV(cache, body_idx,
                                                              0);

          for (int j = 0; j < 6; j++) {
            if (!std::isnan(body_vdot[j])) {
              pdata->Hqp += w_i * (Jb.row(j)).transpose() * Jb.row(j);
              f.head(nq) += w_i * (Jbdot_times_v(j) - body_vdot[j]) *
                            Jb.row(j).transpose();
            }
          }
        }
    ...
    if (use_fast_qp > 0) {  // set up and call fastqp
      info = fastQP(QBlkDiag, f, Aeq, beq, Ain_lb_ub, bin_lb_ub, pdata->active,
                    alpha);
      // if (info<0)    mexPrintf("fastQP info=%d... calling Gurobi.\n", info);
    } else {
      // use gurobi active set
      model = gurobiActiveSetQP(pdata->env, QBlkDiag, f, Aeq, beq, Ain, bin, lb,
                                ub, pdata->vbasis, pdata->vbasis_len,
                                pdata->cbasis, pdata->cbasis_len, alpha);
  ...
  y = pdata->B_act.transpose() *
      (pdata->H_act * qdd + pdata->C_act - D_act * beta);
  // y =
  ...
  if (nlhs > 13) {
    double Vdot;
    if (nc > 0)
      // note: Sdot is 0 for ZMP/double integrator dynamics, so we omit that
      // term here
      Vdot = ((2 * x_bar.transpose() * S + s1.transpose()) *
                  (A_ls * x_bar + B_ls * (Jcomdotv + Jcom * qdd)) +
              s1dot.transpose() * x_bar)(0) +
             s2dot;
    else
      Vdot = 0;
  ...
  if (nlhs > 14) {
    ...
    VectorXd individual_cops =
        individualSupportCOPs(r, cache, active_supports, normals, B, beta);

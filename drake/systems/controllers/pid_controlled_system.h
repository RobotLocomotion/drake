...
class PidControlledSystem : public Diagram<T> {
 public:
  ...
  /// Connect a PidController to an existing plant and DiagramBuilder,
  /// adding additional systems as needed.  Returns a pair of port
  /// descriptors.  The first is the feed forward control input, the
  /// second is the feedback state input.  @p controller will be
  /// populated with a pointer to the newly created PidController.

...
GTEST_TEST(RigidBodySystemTest, TestLoadURDFWorld) {
  ...
  drake::parsers::urdf::AddModelInstanceFromUrdfFile(
      ...
      DrakeJoint::FIXED, nullptr /* weld to frame */, tree_ptr.get());
...
GTEST_TEST(RigidBodySystemTest, MapVelocityToConfigurationDerivatives) {
  ...
  tree->AddFloatingJoint(DrakeJoint::QUATERNION, {body->get_body_index()});
  tree->compile();
  ...
  // Sets fee_body to have zero translation and zero rotation.
  ...
  auto generalized_velocities_vec =
      make_unique<BasicVector<double>>(plant.get_num_velocities());
  auto positions_derivatives_vec =
      make_unique<BasicVector<double>>(plant.get_num_positions());

  Vector3d v0(1, 2, 3);    // Linear velocity in body's frame.
  Vector3d w0(-1, 2, -3);  // Angular velocity in body's frame.
  generalized_velocities_vec->get_mutable_value() << w0, v0;
  BasicStateVector<double> generalized_velocities(
      std::move(generalized_velocities_vec));
  BasicStateVector<double> positions_derivatives(
      std::move(positions_derivatives_vec));
 ...
 protected:
  void SetUp() override {
    ...
    drake::parsers::urdf::AddModelInstanceFromUrdfFile(
        ...
        DrakeJoint::FIXED, nullptr /* weld to frame */, tree.get());

...
class DRAKE_RBP_EXPORT RigidBodyPlant : public LeafSystem<T> {
 public:
  ...
  void SetZeroConfiguration(ContextBase<T> *context) const {
  ...
  void MapVelocityToConfigurationDerivatives(
      ...
      StateVector<T>* positions_derivative) const override;
  ...
  T EvalPotentialEnergy(const ContextBase<T>& context) const override {
    return T(0);
  }

  T EvalKineticEnergy(const ContextBase<T>& context) const override {
    return T(0);
  }

...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  if (nrhs < 3) {
    mexErrMsgIdAndTxt("Drake:allCollisions:NotEnoughInputs",
                      "Usage allCollisionsmex(model_ptr, cache_ptr, "
  ...
  // DEBUG
  // cout << "collisionDetectmex: Num fields in active_collision_options" <<
  ...
  if (body_idx != NULL) {
    ...
    // DEBUG
    // cout << "collisionDetectmex: n_active_bodies = " << n_active_bodies <<
    // endl;
    // END_DEBUG

...
public:
  ...
  std::string getPositionName(int index) const { throw std::runtime_error("bad index"); }
  ...
  template<typename Scalar>
  void motionSubspace(const Eigen::Ref< const Eigen::Matrix<Scalar, Eigen::Dynamic, 1> >& q, MotionSubspaceType& motion_subspace,
  ...
  void qdot2v(const Eigen::Ref< const Eigen::Matrix<Scalar, Eigen::Dynamic, 1> >& q,
    Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic> & qdot_to_v, Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>* dqdot_to_v) const {

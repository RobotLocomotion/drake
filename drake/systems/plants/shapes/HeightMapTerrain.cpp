...
HeightMapTerrain::~HeightMapTerrain() { delete m_rawHeightfieldData; }
...
HeightMapTerrain *HeightMapTerrain::clone() const {
  ...
  return new HeightMapTerrain(
      *this);  // unreachable. avoid compiler warning for non return.
}
...
void HeightMapTerrain::computeMinMaxHeights() {
  ...
  // This next code ensure the terrain bounding box is centered around z=0
  // (local z)
  // Bullet takes what we pass as min/max heights and does not compute them.
  // The bounding box is computed based on these values. It is then useful to
  // have minHeight=-maxHeight
  // so that what the user thinks as z=0 actually is z=0.
  // Another option would be to have an offset if keeping a tighter AABB is
  // desired.
...
void HeightMapTerrain::initialize() { assert(!"Implement me!!"); }
...
double HeightMapTerrain::cellValue(int i, int j) const {
  ...
  return *((double *)(m_rawHeightfieldData +
                      (i + j * nnodes(1)) * bytesPerElement));
}
...
void FlatTerrain::initialize() { assert(!"Implement me!!"); }

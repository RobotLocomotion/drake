...
/// the RigidBodyTree.
/// Each RigidBodyActuator maps to a single DOF joint (currently actuation
/// cannot be applied to multiple DOF's joints). The units of the actuation
/// are the same as the units of the generalized force on the joint.
/// In addition, actuators allow for a gear box reduction factor and for
...
/// </pre>
/// where `q` are the generalized coordinates (or positions), `v` are the
/// generalized velocities, `C` includes the velocity-dependent Coriolis and
/// gyroscopic forces, `tau_actuators` is the vector of externally applied
/// generalized forces and finally `tau_constraints` is the vector of
...
///   qdot = N(q) * v
/// </pre>
/// where `N(q)` a transformation matrix only dependent on the positions.
///

...
if kinsol.mex
  if nargout > 2
    [J, dJ] = centerOfMassJacobianmex(model.mex_model_ptr, kinsol.mex_ptr, robotnum, in_terms_of_qdot);
    dJ = reshape(dJ, size(J, 1), []); % convert to strange second derivative output format
  elseif nargout > 1
    J = centerOfMassJacobianmex(model.mex_model_ptr, kinsol.mex_ptr, robotnum, in_terms_of_qdot);
  end

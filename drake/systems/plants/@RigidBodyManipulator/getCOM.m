...
if kinsol.mex
  if nargout > 2
    [com,J,dJ] = centerOfMassmex(model.mex_model_ptr, kinsol.mex_model_ptr, robotnum, in_terms_of_qdot);
    dJ = reshape(dJ, size(J, 1), []); % convert to strange second derivative output format
  elseif nargout > 1
    [com,J] = centerOfMassmex(model.mex_model_ptr, kinsol.mex_model_ptr, robotnum, in_terms_of_qdot);
  else
    com = centerOfMassmex(model.mex_model_ptr, kinsol.mex_model_ptr, robotnum, in_terms_of_qdot);
  end
else
  m = 0;
  com = zeros(3,1);

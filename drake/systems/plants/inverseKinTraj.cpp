...
drakeIK_DLLEXPORT void inverseKinTraj(RigidBodyTree * model, const int nT, const double* t, const MatrixBase<DerivedA> &qdot0_seed, const MatrixBase<DerivedB> &q_seed, const MatrixBase<DerivedC> &q_nom, const int num_constraints, RigidBodyConstraint** const constraint_array, MatrixBase<DerivedD> &q_sol, MatrixBase<DerivedE> &qdot_sol, MatrixBase<DerivedF> &qddot_sol, int &INFO, vector<string> &infeasible_constraint, IKoptions ikoptions)
{

...
// Distance works with negative depth, but closest point is wrond when in collision.
...
void testBoxFCLDistance(float tx, float ty, float tz)
{
  ...
  cout << "     (" << res.nearest_points[0][0] << ", " <<res.nearest_points[0][1] << ", " <<res.nearest_points[0][2] << "), ";
  cout << "(" << res.nearest_points[1][0] << ", " <<res.nearest_points[1][1] << ", " <<res.nearest_points[1][2] << ")\n";
}      
			   ...
			   SplitMethodType split_method)
{
  ...
  if(res.numContacts() > 0) {
    ...
    for (int i = 0; i < res.numContacts(); ++i) {
      ...
      std::cout << "n=(" << c.normal[0] << "," << c.normal[1] << "," << c.normal[2] << ")";

...
namespace DrakeCollision
{
  ...
  BVHModel<OBBRSS>* FCLModel::newFCLSphereShape(const DrakeShapes::Sphere& geometry,
						bool use_margins)
  {
    // NOTE:  Ignore margins for now.
    use_margins = use_margins;
  ...
  BVHModel<OBBRSS>* FCLModel::newFCLCylinderShape(const DrakeShapes::Cylinder& geometry,
						  bool use_margins)
  {
    // NOTE:  Ignore margins for now.
    use_margins = use_margins;
  ...
  BVHModel<OBBRSS>* FCLModel::newFCLCapsuleShape(const DrakeShapes::Capsule& geometry,
						bool use_margins)
  {
    // NOTE:  Ignore margins for now.
    use_margins = use_margins;
  ...
  ElementId FCLModel::addElement(const Element& element)
  {
    ...
    if (id != 0) {
      ...
      if (bvh_shape) {
	this->fclElements.insert(make_pair(id,unique_ptr<BVHModel<OBBRSS>>(bvh_shape)));
      }
    }
    return id;
  }
  ...
  vector<PointPair> FCLModel::potentialCollisionPoints(bool use_margins)
  {
    ...
    cerr << "Warning: FCLModel::potentialCollisionPoints not implemented." << endl;
    return c.getResults();
  }
  ...
  {
    // TODO: Not Implemented
    cerr << "Warning: FCLModel::collisionPointsCheckOnly not implemented." << endl;
    //if (c.isInCollision()) {
    //  return true;
    //}
  ...
  {
    // Since the transform is applide during collision, this method may not need to be
    // implemented.
    cerr << "Warning: FCLModel::updateElementWorldTransform not implemented." << endl;
					      ...
					      std::unique_ptr<ResultCollector>& c)
  {
    ...
    if (dist > 0) {
      ...
      fcl::collide(fclElements[idA].get(), tfa, fclElements[idB].get(), tfb, cRequest, cRes);
	...
	Vector3d ptB(fclPt[0], fclPt[1], fclPt[2]);
	Vector3d normal(contact.normal[0],
			contact.normal[1],
			contact.normal[2]);
					   ...
					   std::vector<PointPair>& closest_points)
  {
    ...
    cerr << "Warning: FCLModel::collisionDetectFromPoints not implemented." << endl;    
    //...

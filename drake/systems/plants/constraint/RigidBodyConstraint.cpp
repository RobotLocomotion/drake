...
void QuasiStaticConstraint::eval(const double *t,
                                 ...
                                 MatrixXd &dc) const {
  if (this->isTimeValid(t)) {
    int nq = this->robot->number_of_positions();
...
SingleTimeLinearPostureConstraint::SingleTimeLinearPostureConstraint(
  ...
  if (jAvar.minCoeff() < 0 ||
      jAvar.maxCoeff() > this->robot->number_of_positions() - 1) {
    std::cerr << "Drake:RigidBodyConstraint:SingleTimeLinearPostureConstraint: "
  ...
  this->jAvar = jAvar;
  this->A = A;
  this->A_mat.resize(this->num_constraint, this->robot->number_of_positions());
...
void PositionConstraint::eval(const double *t, KinematicsCache<double> &cache,
  ...
  if (this->isTimeValid(t)) {
    ...
    MatrixXd J(3 * this->n_pts, this->robot->number_of_positions());
    this->evalPositions(cache, pos, J);
    c.resize(this->getNumConstraint(t), 1);
    dc.resize(this->getNumConstraint(t), this->robot->number_of_positions());
    int valid_row_idx = 0;
    int i = 0;
...
void QuatConstraint::eval(const double *t, KinematicsCache<double> &cache,
  ...
  dc.resize(num_constraint, this->robot->number_of_positions());
  if (this->isTimeValid(t)) {
    double prod;
    MatrixXd dprod(1, this->robot->number_of_positions());
...
RelativeQuatConstraint::RelativeQuatConstraint(RigidBodyTree *robot,
  ...
  this->bodyA_idx = bodyA_idx;
  this->bodyB_idx = bodyB_idx;
  this->bodyA_name = this->robot->bodies[bodyA_idx]->name_;
  this->bodyB_name = this->robot->bodies[bodyB_idx]->name_;
...
void EulerConstraint::eval(const double *t, KinematicsCache<double> &cache,
  ...
  if (this->isTimeValid(t)) {
    Vector3d rpy;
    MatrixXd drpy(3, this->robot->number_of_positions());
    this->evalrpy(cache, rpy, drpy);
    c.resize(n_constraint);
    dc.resize(n_constraint, this->robot->number_of_positions());
    int valid_row_idx = 0;
    int i = 0;
...
RelativeGazeTargetConstraint::RelativeGazeTargetConstraint(
  ...
  this->bodyA_idx = bodyA_idx;
  this->bodyB_idx = bodyB_idx;
  this->bodyA_name = this->robot->bodies[this->bodyA_idx]->name_;
  this->bodyB_name = this->robot->bodies[this->bodyB_idx]->name_;
...
void Point2PointDistanceConstraint::eval(const double *t,
  ...
  if (this->isTimeValid(t)) {
    ...
    MatrixXd dposA(3 * this->ptA.cols(), this->robot->number_of_positions());
    if (this->bodyA != 0) {
    ...
    c.resize(num_cnst, 1);
    c = tmp2.transpose();
    dc.resize(num_cnst, this->robot->number_of_positions());
    for (int i = 0; i < num_cnst; i++) {
      dc.row(i) = 2 * d.col(i).transpose() *
                  dd.block(3 * i, 0, 3, this->robot->number_of_positions());
    }
  } else {
    c.resize(0);
    dc.resize(0, 0);
...
void Point2PointDistanceConstraint::name(
  ...
  if (this->isTimeValid(t)) {
    ...
    for (int i = 0; i < num_cnst; i++) {
      ...
      if (this->bodyA != 0) {
        bodyA_name = this->robot->bodies[bodyA]->name_;
      } else {
        bodyA_name = std::string(RigidBodyTree::kWorldLinkName);
      }
      std::string bodyB_name;
      if (this->bodyB != 0) {
        bodyB_name = this->robot->bodies[bodyB]->name_;
      } else {
...
void Point2LineSegDistConstraint::name(
  ...
  if (this->isTimeValid(t)) {
    std::string time_str = this->getTimeString(t);
    name_str.push_back(
        "Distance from " + this->robot->bodies[this->pt_body]->name_ +
        " pt to a line on " + this->robot->bodies[this->line_body]->name_ +
        time_str);
...
void AllBodiesClosestDistanceConstraint::eval(const double *t,
  ...
  if (this->isTimeValid(t)) {
    ...
    dc = MatrixXd::Zero(num_pts, robot->number_of_positions());
    MatrixXd JA = MatrixXd::Zero(3, robot->number_of_positions());
    MatrixXd JB = MatrixXd::Zero(3, robot->number_of_positions());
...
void GravityCompensationTorqueConstraint::eval(const double *t,
  ...
  c.resize(num_constraint);
  dc.resize(num_constraint, robot->number_of_positions());

  for (int i = 0; i < num_constraint; ++i) {
    c(i) = G(joint_indices(i));
    dc.row(i) = dG.block(joint_indices(i), 0, 1, robot->number_of_positions());
  }
}

...
void mexFunction( int nlhs, mxArray *plhs[],int nrhs, const mxArray *prhs[] ) {
  ...
  for (int trial=0; trial<1; trial++) {
  	...
  	VectorXd matlab_q(matlab_model->num_positions), cpp_q(cpp_model->num_positions);
		...
		// run kinematics
		KinematicsCache<double> matlab_cache = matlab_model->doKinematics(matlab_q, matlab_v, 1, true);
		KinematicsCache<double> cpp_cache = cpp_model->doKinematics(cpp_q, cpp_v, 1, true);

...
drakeIK_DLLEXPORT void inverseKinPointwise(RigidBodyTree * model, const int nT, const double* t, const MatrixBase<DerivedA> &q_seed, const MatrixBase<DerivedB> &q_nom, const int num_constraints, RigidBodyConstraint** const constraint_array, MatrixBase<DerivedC> &q_sol, int* INFO, vector<string> &infeasible_constraint, const IKoptions &ikoptions)
{

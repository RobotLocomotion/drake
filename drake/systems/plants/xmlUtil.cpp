...
#ifdef _WIN32
    bool dirIsRelative = !(normalized_root_dir.size() >= 2 && std::isalpha(normalized_root_dir[0]) && normalized_root_dir[1] == ':');
#else
    bool dirIsRelative = !(normalized_root_dir.size() >= 1 && normalized_root_dir[0] == '/');
#endif
    if (dirIsRelative) {

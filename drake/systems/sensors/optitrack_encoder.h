 ...
 * @param id_ The rigid body ID as defined in the Optitrack software
...
struct TrackedBody {
  ...
  bool operator < (const TrackedBody& obj) const {
    return (id_ < obj.id_);
  }
};
 ...
 * described by its Optitrack ID, it's name, and it's pose w.r.t. the world. No
...
class OptitrackEncoder : public systems::LeafSystem<double> {
 public:
  ...
  const systems::OutputPort<double>& get_optitrack_output_port() const {
    return this->get_output_port(optitrack_output_port_index_);
  }

  int get_optitrack_output_port_index() const {
    return this->optitrack_output_port_index_;
  }
 ...
 private:
  ...
  std::vector<TrackedBody> MakeOutputStatus() const;

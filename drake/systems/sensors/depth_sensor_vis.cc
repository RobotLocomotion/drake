...
DepthSensorVis::DepthSensorVis(const DepthSensorSpecification& spec)
    : spec_(spec) {
  input_port_index_ =
...
DepthSensorVis::pointcloud_message_output_port() const {
  return System<double>::get_output_port(output_port_index_);
}

void DepthSensorVis::DoCalcOutput(const systems::Context<double>& context,

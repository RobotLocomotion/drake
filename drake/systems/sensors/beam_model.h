...
/// occurred (true + noise, short return, max return, or random return), and one
/// each for modeling the distribution of short true but noisy returns, short
/// returns, and random returns).
///
/// We deviate from the textbook model in one respect: in both models, the
/// distribution over short returns and the distribution over getting a noisy
/// version of the true return (aka a "hit") are truncated.  The short returns
/// are from an exponential distribution but truncated to be less than the
/// input depth, and "hits" are drawn from a Gaussian centered at the input
/// depth but truncated at the maximum range of the sensor.  In the book, these
/// distributions are normalized so that the total probability of getting a
/// short return and/or hit stays constant (independent of the input depth).
...
class BeamModel final : public LeafSystem<T> {
 public:
  ...
  const InputPortDescriptor<T>& get_gaussian_random_input_port() const {
    return this->get_input_port(2);
  }
  const InputPortDescriptor<T>& get_exponential_random_input_port() const {

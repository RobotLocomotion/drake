...
///
/// Let:
///  - `G` be the gyroscope's frame with origin point `Go`.
///  - `W` be the world frame with origin point `Wo`.
///  - `ω_WGo_W` be `Go`'s rotational velocity in `W` expressed in `W`.
///  - `R_GW` be the rotation matrix from `W` to `G`.
///  - `ω_WGo_G` be `Go`'s rotational velocity in `W` expressed in `G`. This is
///     the desired output of the gyroscope.
///
/// The math implemented by this sensor is as follows:
///
/// <pre>
/// ω_WGo_G = R_GW * ω_WGo_W
/// </pre>
///
...
/// accessor:
///
///  - get_input_port(): Contains the state (i.e., postion and velocity)
...
class Gyroscope : public systems::LeafSystem<double> {
 public:
  ...
  /// Gyroscope is neither copyable nor moveable.
  ///@{
  explicit Gyroscope(const Gyroscope&) = delete;

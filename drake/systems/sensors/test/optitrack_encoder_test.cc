...
//    int model_id = mtable.begin()->second;
    ...
    pose_bundle_->set_pose(0, Eigen::Isometry3d(Eigen::AngleAxis<double>(0.1, axis)));
    pose_bundle_->set_name(1, "iiwa_frame_1");
    pose_bundle_->set_pose(1, Eigen::Isometry3d(Eigen::AngleAxis<double>(0.2, axis)));
    pose_bundle_->set_name(2, "iiwa_frame_2");
    pose_bundle_->set_pose(2, Eigen::Isometry3d(Eigen::AngleAxis<double>(0.3, axis)));

    /// The OptitrackEncoder class constructor takes a map of unique frame names
    /// (from the PoseBundle input object) to unique Optitrack IDs.
    ...
    /// optional. If not provided, rigid body names will match the frame
    /// names contained in the PoseBundle input. These names are not necessarily
    /// unique (as in the case for the Optitrack application software, e.g., Motive).
  ...
  std::vector<TrackedObject> UpdateInputCalcOutput(
    ...
    auto output_value =
        output->get_data(dut.get_tracked_objects_output_port_index());
...
TEST_F(OptitrackEncoderTest, WithoutRigidBodyNamesTest) {
  ...
  auto tracked_objects = UpdateInputCalcOutput(dut);
   ...
   for (auto it = tracked_objects.begin(); it != tracked_objects.end(); ++it) {
     EXPECT_EQ((*it).id_, frame_name_to_id_map_[(*it).body_name_]);
   }
}
...
// the parameters passed in at object construction. We ensure each TrackedObject
...
TEST_F(OptitrackEncoderTest, WithRigidBodyNamesTest) {
  ...
  while (fr_map_iter != frame_name_to_id_map_.end() &&
    ...
    auto obj_it = std::find_if(tracked_objects.begin(), tracked_objects.end(),
                               [&fr_map_iter](const TrackedObject& tobj) {
                                 return tobj.id_ == fr_map_iter->second;
                               });
    EXPECT_EQ(*rb_name_iter, (*obj_it).body_name_);
    fr_map_iter++;
    rb_name_iter++;
  }
}

#include "drake/systems/sensors/depth_sensor_to_point_cloud_message.h"
...
class TestDepthSensorToPointCloudMessage : public ::testing::Test {
 protected:
  ...
  const bot_core::pointcloud_t& InitializeAndOutputMessage() {
    ...
    DepthSensorToPointCloudMessage dut(spec_);
...
TEST_F(TestDepthSensorToPointCloudMessage, Octant1Test) {
...
TEST_F(TestDepthSensorToPointCloudMessage, XyPlanarTest) {
...
// The points within the point cloud are hard-coded to be halfway between the
// minimum and maximum depth sensing range.
TEST_F(TestDepthSensorToPointCloudMessage, XzPlanarTest) {
...
TEST_F(TestDepthSensorToPointCloudMessage, XyzSphericalTest) {
...
// range.
TEST_F(TestDepthSensorToPointCloudMessage, XLinearTest) {

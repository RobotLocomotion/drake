...
const char* const kSensorName = "gyro sensor";

// Tests Gyroscope's various accessor and streaming to-string methods.
GTEST_TEST(TestGyroscope, AccessorsAndToStringTest) {
  auto tree = make_unique<RigidBodyTree<double>>();
  RigidBodyFrame<double> frame("gyro frame", &tree->world(),
                               Eigen::Isometry3d::Identity());

  // Defines the Device Under Test (DUT).
  Gyroscope dut(kSensorName, frame, *tree);

  stringstream string_buffer;
  string_buffer << dut;
  const string dut_string = string_buffer.str();

  EXPECT_NE(dut_string.find("Gyroscope:"), string::npos);
...
// space measures zero rotational acceleration.
GTEST_TEST(TestGyroscope, TestFreeFall) {
  auto tree = make_unique<RigidBodyTree<double>>();
  ...
  const Vector3d expected_acceleration(0, 0, 0);

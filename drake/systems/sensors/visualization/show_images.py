...
"""
This is a simple script to convert from robotlocomotion LCM image types to
bot_core, so that we may leverage existing director code for visualization.

To run this, use `directorPython`:

    directorPython ./show_images.py --bot-config ./bot_frames.cfg

Note that if you use a non-default --channel, then you should use the
appropriate --bot_config file.

At present, you cannot run this as part of drake-visualizer. If you do, you
will get an error such as:
    ValueError: Trying to call 'isVisible' on a destroyed ddQVTKWidgetView
    object
...
# TODO(eric.cousineau): See if this could be run directly in drake-visualizer.
...
class DrakeImageViewer(object):
    ...
    def setup(self):
        ...
        # Redirect bot_core duplictes to a separate channel.
        channel = args.channel + "_BOT_CORE"
        # Add LCM translator.
        self._translators.append(addTranslator(args.channel, channel))
...
def addTranslator(rl_channel, bc_channel):
    """ Translate from robotlocomotion to bot_core image type """
    def callback(rl_msg):
...
def depth32f_to_depthmm(rl_msg, bc_msg):
    """ Convert Depth32F to bot_core depth image type. """
    assert(rl_msg.row_stride == 4 * rl_msg.width)
    num_pixels = rl_msg.width * rl_msg.height
...
def main():
    # TODO(eric.cousineau): Enable this to run in drake-visualizer, possibly as
    # a docked widget.

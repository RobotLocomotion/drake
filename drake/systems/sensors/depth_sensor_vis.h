...
/// A DepthSensorVis takes as input a DepthSensorOutput and outputs an
/// AbstractValue containing a `Value<bot_core::pointcloud_t>` LCM message. This
/// message can then be sent to `drake-visualizer` using LcmPublisherSystem for
/// visualizing the depth readings contained within the DepthSensorOutput.
class DepthSensorVis : public systems::LeafSystem<double> {
 public:
  ...
  /// visualized.
  explicit DepthSensorVis(const DepthSensorSpecification& spec);

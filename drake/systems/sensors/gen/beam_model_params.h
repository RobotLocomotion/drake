...
struct BeamModelParamsIndices {
  ...
  static const int kProbabilityRandom = 4;
...
class BeamModelParams : public systems::BasicVector<T> {
 public:
  ...
  /// @arg @c probability_random defaults to 0.0 dimensionless.
  BeamModelParams() : systems::BasicVector<T>(K::kNumCoordinates) {
    ...
    this->set_probability_random(0.0);
  }
  ...
  /// The probability of ignoring the input depth and simple returning a random
  /// value between 0 and the max range of the sensor
  /// @note @c probability_random is expressed in units of dimensionless.
  /// @note @c probability_random has a limited domain of [0.0, 1.0].
  const T& probability_random() const {
    return this->GetAtIndex(K::kProbabilityRandom);
  }
  void set_probability_random(const T& probability_random) {
    this->SetAtIndex(K::kProbabilityRandom, probability_random);
  }
  //@}
  ...
  decltype(T() < T()) IsValid() const {
    ...
    result = result && !isnan(probability_random());
    result = result && (probability_random() >= T(0.0));
    result = result && (probability_random() <= T(1.0));
    return result;
  }
  ...
  void CalcInequalityConstraint(VectorX<T>* value) const override {
    ...
    (*value)[6] = probability_random() - T(0.0);
    (*value)[7] = T(1.0) - probability_random();
  }
};

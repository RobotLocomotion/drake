...
void DepthSensor::DoCalcOutput(const systems::Context<double>& context,
  ...
  BasicVector<double>* output_data =
      output->GetMutableVectorData(depth_output_port_index_);
  DRAKE_ASSERT(output_data != nullptr);
  output_data->SetFromVector(distances);


  // Evaluates theh output port containing X_WF.
  const drake::Isometry3<double> X_WF =
      tree_.CalcFramePoseInWorldFrame(kinematics_cache, frame_);
  PoseVector<double>* output_x_ws =
      dynamic_cast<PoseVector<double>*>(
          output->GetMutableVectorData(pose_output_port_index_));
  DRAKE_ASSERT(output_x_ws != nullptr);
  output_x_ws->set_translation(
      Eigen::Translation<double, 3>(X_WF.translation()));
  output_x_ws->set_rotation(Eigen::Quaternion<double>(X_WF.rotation()));
}

...
std::shared_ptr<CascadeSystem<
    AffineSystem<
      EigenVector<NumStates1>::template type,
      EigenVector<NumInputs1>::template type,
      EigenVector<NumOutputs1>::template type >,
    AffineSystem<
      EigenVector<NumStates2>::template type,
      EigenVector<NumOutputs1>::template type,
      EigenVector<NumOutputs2>::template type >>>
MakeCascadeOfAffineSystems(size_t num_states_1, size_t num_inputs_1, size_t num_outputs_1, size_t num_states_2, size_t num_outputs_2) {
  ...
  auto sys2_ptr = system_test::CreateRandomAffineSystem<NumStates2, NumOutputs1, NumOutputs2>(num_states_2, num_outputs_1, num_outputs_2);
...
TEST(CascadeSystemTest, DynamicSizes) {
	auto combined = MakeCascadeOfAffineSystems<Dynamic, Dynamic, Dynamic, Dynamic, Dynamic>(3, 4, 5, 6, 7);
  EXPECT_EQ(3 + 6, getNumStates(*combined));
  EXPECT_EQ(3 + 6, createStateVector<double>(*combined).size());
  EXPECT_EQ(4, getNumInputs(*combined));
  EXPECT_EQ(7, getNumOutputs(*combined));

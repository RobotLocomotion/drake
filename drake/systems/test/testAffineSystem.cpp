...
void testCascade(size_t num_states_1, size_t num_inputs_1, size_t num_outputs_1,
  ...
  auto sys1_ptr = createRandomAffineSystem<NumStates1, NumInputs1, NumOutputs1>(
      num_states_1, num_inputs_1, num_outputs_1);
  auto sys2_ptr =
      createRandomAffineSystem<NumStates2, NumOutputs1, NumOutputs2>(
          num_states_2, num_outputs_1, num_outputs_2);

  auto combined = cascade(sys1_ptr, sys2_ptr);

  valuecheck(num_states_1 + num_states_2, getNumStates(*combined),
             "Wrong number of states");
  valuecheck(num_inputs_1, getNumInputs(*combined), "Wrong number of inputs");
  valuecheck(num_outputs_2, getNumOutputs(*combined),
             "Wrong number of outputs");

  auto x = createStateVector<double>(*combined);
  valuecheck(getNumStates(*combined), static_cast<size_t>(x.size()),
             "State vector size wrong");
}

template <int NumStates1, int NumInputs1, int NumOutputs1, int NumStates2>
void testFeedback(size_t num_states_1, size_t num_inputs_1,
                  size_t num_outputs_1, size_t num_states_2) {
  auto sys1_ptr = createRandomAffineSystem<NumStates1, NumInputs1, NumOutputs1>(
      num_states_1, num_inputs_1, num_outputs_1);
  auto sys2_ptr = createRandomAffineSystem<NumStates2, NumOutputs1, NumInputs1>(
  ...
  valuecheck(num_states_1 + num_states_2, getNumStates(*combined),
             "Wrong number of states");
  valuecheck(num_inputs_1, getNumInputs(*combined), "Wrong number of inputs");
  valuecheck(num_outputs_1, getNumOutputs(*combined),
             "Wrong number of outputs");

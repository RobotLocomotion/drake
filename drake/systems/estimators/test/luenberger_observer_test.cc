...
GTEST_TEST(TestLuenberger, ErrorDynamics) {
  ...
  // TODO(russt): Replace with a better drake random number utility.
  srand(42);
  double tol = 1e-10;
  for (int i = 0; i < 10; i++) {
    ...
    Eigen::Vector3d xhat = Eigen::Vector3d::Random();
    Vector1d u = Vector1d::Random();  // TODO(all): Should we consider putting
                                      // the eigen_types in the Eigen namespace?
    Eigen::Vector2d y = Eigen::Vector2d::Random();

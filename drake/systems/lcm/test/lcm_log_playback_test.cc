...
void SetInput(double time, const std::string& name, double val,
  ...
  msg.timestamp = drake::lcm::DrakeLcmLog::second_to_timestamp(time);
...
class DummySys : public LeafSystem<double> {
 ...
 private:
  void DoPublish(const Context<double>& context,
    ...
    if (received_msgs_.empty() && msg->timestamp != 0)
      is_new_msg = true;
    if (!received_msgs_.empty() && (msg->timestamp != received_msgs_.back().timestamp)) {
      is_new_msg = true;
    }
...
void CheckLog(const std::vector<double>& expected_times,
  ...
  for (size_t i = 0; i < expected_times.size(); i++) {
    ...
    EXPECT_EQ(msg.timestamp, expected_times[i] * 1e6);

    // Note: this needs to match the how we schedule event processing
    // (DoCalcNextUpdateTime) in LcmSubscriberSystem.
    EXPECT_EQ(times[i], expected_times[i] + 0.0001);
  }
}

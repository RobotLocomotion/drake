...
void GenerateLog() {
  // Imitating a log generated by some hardware with a timer that has some
  // arbitrary shift. An alternative is to use start_time = 0 here, and shift
  // the log accordingly during playback.
  const double start_time = 12345;

  drake::lcm::DrakeLcmLog w_log("test.log", true, start_time);
  ...
  SetInput(start_time + 0.1, "Ch0", 1, context0.get());
  pub0->Publish(*context0);

  SetInput(start_time + 0.22, "Ch1", 2, context1.get());
  ...
  SetInput(start_time + 0.3, "Ch0", 3, context0.get());
  pub0->Publish(*context0);
  SetInput(start_time + 0.3, "Ch0", 4, context0.get());
  pub0->Publish(*context0);
  SetInput(start_time + 0.3, "Ch0", 5, context0.get());
  pub0->Publish(*context0);

  // Testing sending a message to a different channel at the same time.
  SetInput(start_time + 0.3, "Ch1", 6, context1.get());
  pub1->Publish(*context1);

  SetInput(start_time + 0.4, "Ch1", 7, context1.get());
  pub1->Publish(*context1);
}

...
GTEST_TEST(PoseBundleToDrawMessageTest, Conversion) {
  ...
  bar_pose *= Eigen::AngleAxisd(-M_PI_2, Eigen::Vector3d::UnitX());
  ...
  EXPECT_NEAR(std::cos(-M_PI_2 / 2), message.quaternion[1][0], 1.0e-6);  // w
  EXPECT_NEAR(std::cos(-M_PI_2 / 2), message.quaternion[1][0], 1.0e-6);  // x
  EXPECT_EQ(0, message.quaternion[0][2]);  // y
  EXPECT_EQ(0, message.quaternion[0][3]);  // z
}

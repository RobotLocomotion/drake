...
const int kNumRigidBodyPoses = 2;
...
class PoseAggregatorTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    aggregator_.AddRigidBodyInput(tree_);
    // Allocate another input for two generic poses.
    aggregator_.AddGenericInput("generic", kNumGenericPoses);
...
TEST_F(PoseAggregatorTest, HeterogeneousAggregation) {
  // Set the rigid body state input so that the revolute joint is pointing
  // toward pi/2.
  ...
  *generic_input.get_mutable_pose(0) = translation_0;
  generic_input.set_name(1, "Mycroft");
  *generic_input.get_mutable_pose(1) = translation_1;
  ...
  EXPECT_EQ("world0::world", bundle.get_name(0));
  const Isometry3d& world_pose = bundle.get_pose(0);
  EXPECT_EQ("robot0::link1", bundle.get_name(1));
  const Isometry3d& link1_pose = bundle.get_pose(1);

  EXPECT_TRUE(CompareMatrices(Isometry3d::Identity().matrix(),
                              world_pose.matrix()));
  ...
  EXPECT_EQ("generic::Sherlock", bundle.get_name(2));
  const Isometry3d& generic_pose_0 = bundle.get_pose(2);
  EXPECT_EQ("generic::Mycroft", bundle.get_name(3));
  const Isometry3d& generic_pose_1 = bundle.get_pose(3);

...
class PoseAggregatorTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    const int kRandomModelId = 42;
    aggregator_.AddSingleInput("vector", kRandomModelId);
...
TEST_F(PoseAggregatorTest, HeterogeneousAggregation) {
  ...
  const int kSherlockModelId = 17;
  generic_input.set_name(0, "Sherlock");
  generic_input.set_pose(0, Isometry3d(translation_0));
  generic_input.set_model_instance_id(0, kSherlockModelId);

  const int kMycroftModelId = 13;
  generic_input.set_name(1, "Mycroft");
  generic_input.set_pose(1, Isometry3d(translation_1));
  generic_input.set_model_instance_id(1, kMycroftModelId);
  ...
  EXPECT_EQ(kSherlockModelId, bundle.get_model_instance_id(1));
  const Isometry3d& generic_pose_0 = bundle.get_pose(1);
  EXPECT_EQ("bundle::Mycroft", bundle.get_name(2));
  EXPECT_EQ(kMycroftModelId, bundle.get_model_instance_id(2));

...
/// Tests that the PoseVector is initialized to the identity.
...
GTEST_TEST(PoseVector, Rotation) {
  ...
  Isometry3<double> X_WA = vec.get_isometry();
  Eigen::Vector3d p_in = {1.0, 0.0, 0.0};
  Eigen::Vector3d p_out = X_WA * p_in;
...
GTEST_TEST(PoseVector, Translation) {
  ...
  Isometry3<double> X_WA = vec.get_isometry();
  Eigen::Vector3d p_in = {1.0, 2.0, 3.0};
  Eigen::Vector3d p_out = X_WA * p_in;
...
GTEST_TEST(PoseVector, Symbolic) {
  ...
  Isometry3<symbolic::Expression> X_WA = vec.get_isometry();
...
GTEST_TEST(PoseVector, Autodiff) {
  ...
  // Spot-check some terms from the output.
  Isometry3<AutoDiffXd> X_WA = vec.get_isometry();
  auto matrix = X_WA.matrix();

...
class AutodiffTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    // with respect to vec_0 is initially 0: [1.0, 0.0].
  ...
  // derivatives.
  static VectorX<Scalar> DoMath(const VectorX<Scalar>& vec_) {
    VectorX<Scalar> output(2);
    // y1 = sin(v0) + v1
    output[1] = sin(vec_[0]) + vec_[1];
    // y0 = (sin(v0) + v1) * (cos(v0) / v1)
    //    = cos(v0) + (sin(v0) * cos(v0) / v1)
    output[0] = cos(vec_[0]) / vec_[1];
    output[0] *= output[1];
    return output;
  }

...
TEST_F(RotationConversionTest, QuatAxis) {
  ...
  EXPECT_NEAR(acos(std::abs(q_.transpose() * q_back)), 0.0, 1e-14);
}
...
TEST_F(RotationConversionTest, RotmatAxis) {
  ...
  EXPECT_TRUE(CompareMatrices(R_, R_back, 1e-14, MatrixCompareType::absolute));
}
...
TEST_F(RotationConversionTest, AxisRPY) {
  ...
  EXPECT_TRUE(
      CompareMatrices(rpy, rpy_back, 1e-14, MatrixCompareType::absolute));
}
...
TEST_F(RotationConversionTest, QuatEigenQuaternion) {
  ...
  EXPECT_TRUE(
      CompareMatrices(R_expected, R_eigen, 1e-14, MatrixCompareType::absolute));
}

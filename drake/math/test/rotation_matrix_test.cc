...
GTEST_TEST(RotationMatrixTest, TestXYZ) {
  ...
  EXPECT_TRUE(CompareMatrices(XRotation(2 * M_PI + 0.3), XRotation(0.3), tol));
  EXPECT_TRUE(CompareMatrices(YRotation(2 * M_PI + 0.3), YRotation(0.3), tol));
  EXPECT_TRUE(CompareMatrices(ZRotation(2 * M_PI + 0.3), ZRotation(0.3), tol));
}
...
GTEST_TEST(RotationMatrixTest, TestProjection) {
  ...
  // Non-rotation matrix in gets a rotation matrix out.
  Matrix3d M2;
  M2 << 1, 2, 3, 4, 5, 6, 7, 8, 9;
  Matrix3d R2 = ProjectMatToRotMat(M2);
  EXPECT_TRUE(CompareMatrices(R2.transpose() * R2.inverse(),
                              Matrix3d::Identity(), tol));

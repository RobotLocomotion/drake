...
void reorder_eigen(Eigen::Ref<Eigen::MatrixXd> S, 
            Eigen::Ref<Eigen::MatrixXd> T,
...
Eigen::MatrixXd DiscreteAlgebraicRiccatiEquation(
    ...
    if (R_cholesky.info() != Eigen::Success) throw std::runtime_error("R must be positive definite");
 ...
 * Reorder the eigenvalues of (S,T) such that the top-left n by n matrix has stable eigenvalues
 ...
 * p points to the current block (1x1 or 2x2) and q points to the block with the stable eigenvalue(s).
 ...
 * "A generalized eigenvalue approach for solving Riccati equations" by P. Van Dooren, 1981.
 */
void swap_block(
...
void reorder_eigen(Eigen::Ref<Eigen::MatrixXd> S, 
    ...
    while(p<n && q<n2){
        ...
        if(q>=n2) throw std::runtime_error("fail to find enough stable eigenvalues");
    ...
    if (p<n && q>=n2) throw std::runtime_error("fail to find enough stable eigenvalues");
}

/**
 * Functionality of "swap_block" function:
 * swap the 1x1 or 2x2 blocks pointed by p and q.
 * There are four cases: swaping 1x1 and 1x1 matrices, swaping 2x2 and 1x1 matrices,
...
void swap_block(
    ...
    while(p_tmp-- > p){
        ...
        switch(p_block_size*10 + q_block_size){
            case 11: swap_block_11(S,T,Z_tmp,p_tmp);
                     break;
            case 21: swap_block_21(S,T,Z_tmp,p_tmp);
                     break;
            case 12: swap_block_12(S,T,Z_tmp,p_tmp);
                     break;
            case 22: swap_block_22(S,T,Z_tmp,p_tmp);
                     break;
        }
        Z = (Z * Z_tmp).eval();
    }
...
void swap_block_21(
    ...
    Givens_rotation(H(1,2),H(1,1),Z1.block<2,2>(p+1,p+1));//compute Z1
    H = (H * Z1.block<3,3>(p,p)).eval();
    Givens_rotation(H(0,1),H(0,0),Z2.block<2,2>(p,p));//compute Z2
    S = (S * Z1).eval(), T = (T * Z1).eval(), Z = (Z * Z1 * Z2).eval();
    Givens_rotation(T(p+1,p+1),T(p+2,p+1),Q1.block<2,2>(p+1,p+1));//compute Q1
...
void swap_block_12(
    ...
    Givens_rotation(H(1,2),H(2,2),Q2.block<2,2>(p+1,p+1));//compute Q2
    S = (Q1 * S).eval(), T = (Q1 * T).eval();
    Givens_rotation(S(p+1,p+1),S(p+1,p),Z1.block<2,2>(p,p));//compute Z1
    S = (Q2 * S * Z1).eval(), T = (Q2 * T * Z1).eval();
    Givens_rotation(S(p+2,p+2),S(p+2,p+1),Z2.block<2,2>(p+1,p+1));//compute Z2
    ...
    S(p+2,p) = S(p+2,p+1) = T(p+1,p) = T(p+2,p) = T(p+2,p+1) = 0;
}  
...
void swap_block_22(
    ...
    // "On Swapping Diagonal Blocks in Real Schur Form" by Zhaojun Bai and James W. Demmelt;
    // "Numerical Methods for General and Structured Eigenvalue Problems" by Daniel Kressner, p108-111.
    int n2 = S.rows();
    ...
    C(1,1) = A(0,0), C(1,3) = A(0,1), C(1,4) = -A(2,3), C(1,5) = -A(3,3),
    C(2,0) = A(1,0), C(2,2) = A(1,1), C(2,6) = -A(2,2), C(2,7) = -A(3,2),
    C(3,1) = A(1,0), C(3,3) = A(1,1), C(3,6) = -A(2,3), C(3,7) = -A(3,3),
    C(4,0) = B(0,0), C(4,2) = B(0,1), C(4,4) = -B(2,2), C(4,5) = -B(3,2),
    C(5,1) = B(0,0), C(5,3) = B(0,1), C(5,4) = -B(2,3), C(5,5) = -B(3,3),
    C(6,0) = B(1,0), C(6,2) = B(1,1), C(6,6) = -B(2,2), C(6,7) = -B(3,2),
    C(7,1) = B(1,0), C(7,3) = B(1,1), C(7,6) = -B(2,3), C(7,7) = -B(3,3);    
    ...
    S(p+2,p) = S(p+2,p+1) = S(p+3,p) = S(p+3,p+1) = T(p+1,p)
        = T(p+2,p) = T(p+2,p+1) = T(p+3,p) = T(p+3,p+1) = T(p+3,p+2) = 0;
}

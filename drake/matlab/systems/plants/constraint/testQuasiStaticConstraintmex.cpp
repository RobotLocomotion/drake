 ...
 * angles (floating base)
 * @param weights                         A num_weightsx1 double vector, the
 * weight at each contact point
 * @param t                               An optional argument. The time to
 * evaluate the constraint.
 * @retval active_flag                    A boolean flag, true if the
 * QuasiStaticConstraint is active
 * @retval num_weights                    The number of contact points
 * @retval constraint                     The value of the constraint
 * @retval dconstraint                    The gradient of the constraint w.r.t
 * [q;weights]
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
  ...
  RigidBodyTreed* model = qsc->getRobotPointer();

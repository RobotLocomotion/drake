...
PiecewisePolynomial<double> matlabPPTrajectoryOrMatrixToPiecewisePolynomial(
    const mxArray* array) {
  if (mxIsNumeric(array)) {
    return PiecewisePolynomial<double>(
        matlabToEigenMap<Dynamic, Dynamic>(array));
  } else {
    return matlabPPFormToPiecewisePolynomial(mxGetPropertySafe(array, "pp"));
  }
}

PiecewisePolynomial<double> matlabCoefsAndBreaksToPiecewisePolynomial(
    const mxArray* mex_coefs, const mxArray* mex_breaks,
  ...
  std::vector<PiecewisePolynomial<double>::PolynomialMatrix>
      polynomial_matrices;
  ...
  for (mwSize segment_index = 0; segment_index < dims[1]; segment_index++) {
    ...
    for (mwSize row = 0; row < dims[0]; row++) {
      ...
      for (mwSize coefficient_index = 0; coefficient_index < dims[2];
           coefficient_index++) {
        mwSize third_dimension_index = flip_last_dimension
                                           ? dims[2] - coefficient_index - 1
                                           : coefficient_index;
  ...
  PiecewisePolynomial<double> piecewise_polynomial_part =
...
matlabExpPlusPPToExponentialPlusPiecewisePolynomial(const mxArray* array) {
  auto K =
      matlabToEigenMap<Dynamic, Dynamic>(mxGetFieldOrPropertySafe(array, "K"));
  auto A =
      matlabToEigenMap<Dynamic, Dynamic>(mxGetFieldOrPropertySafe(array, "A"));
  auto alpha = matlabToEigenMap<Dynamic, Dynamic>(
      mxGetFieldOrPropertySafe(array, "alpha"));
  ...
  PiecewisePolynomial<double> piecewise_polynomial_part =
      matlabCoefsAndBreaksToPiecewisePolynomial(mex_coefs, mex_breaks, false);
  return ExponentialPlusPiecewisePolynomial<double>(K, A, alpha,
                                                    piecewise_polynomial_part);
}

ExponentialPlusPiecewisePolynomial<double>
matlabExpPlusPPOrPPTrajectoryOrVectorToExponentialPlusPiecewisePolynomial(
    const mxArray* array) {
  ...
  for (int support_num = 0; support_num < num_supports; support_num++) {
    ...
    for (size_t i = 0; i < body_ids.size(); ++i) {
      ...
      support_state_element.contact_points = matlabToEigenMap<3, Dynamic>(
          mxGetCell(mxGetFieldOrPropertySafe(mex_supports, support_num,
                                             "contact_pts"),
                    i));
      support_state_element.support_surface = matlabToEigenMap<4, 1>(
          mxGetCell(mxGetFieldOrPropertySafe(mex_supports, support_num,
                                             "support_surface"),
                    i));
...
setUpContactGroups(RigidBodyTreed* robot, const mxArray* mex_contact_groups) {
  ...
  for (size_t body_id = 0; body_id < robot->bodies.size(); body_id++) {
    ...
    for (int field_number = 0;
         field_number < mxGetNumberOfFields(mex_contact_group);
         field_number++) {
      contact_group[mxGetFieldNameByNumber(mex_contact_group, field_number)] =
          matlabToEigenMap<3, Dynamic>(
              mxGetFieldByNumber(mex_contact_group, 0, field_number));
    }
...
std::vector<BodyMotionData> setUpBodyMotions(const mxArray* mex_body_motions) {
  ...
  for (int i = 0; i < num_body_motions; ++i) {
    ...
    body_motion_data.body_or_frame_id =
        static_cast<int>(
            mxGetPrSafe(mxGetPropertySafe(mex_body_motions, i, "body_id"))[0]) -
        1;  // base 1 to base 0
    body_motion_data.trajectory = matlabCoefsAndBreaksToPiecewisePolynomial(
        mxGetPropertySafe(mex_body_motions, i, "coefs"),
        mxGetPropertySafe(mex_body_motions, i, "ts"), true);
    body_motion_data.toe_off_allowed = matlabToStdVector<bool>(
        mxGetPropertySafe(mex_body_motions, i, "toe_off_allowed"));
    body_motion_data.in_floating_base_nullspace = matlabToStdVector<bool>(
        mxGetPropertySafe(mex_body_motions, i, "in_floating_base_nullspace"));
    body_motion_data.control_pose_when_in_contact = matlabToStdVector<bool>(
        mxGetPropertySafe(mex_body_motions, i, "control_pose_when_in_contact"));
    ...
    body_motion_data.xyz_proportional_gain_multiplier = matlabToEigenMap<3, 1>(
        mxGetPropertySafe(mex_body_motions, i, "xyz_kp_multiplier"));
    body_motion_data.xyz_damping_ratio_multiplier = matlabToEigenMap<3, 1>(
        mxGetPropertySafe(mex_body_motions, i, "xyz_damping_ratio_multiplier"));
    body_motion_data.exponential_map_proportional_gain_multiplier = mxGetPrSafe(
        mxGetPropertySafe(mex_body_motions, i, "expmap_kp_multiplier"))[0];
    body_motion_data.exponential_map_damping_ratio_multiplier =
        mxGetPrSafe(mxGetPropertySafe(mex_body_motions, i,
                                      "expmap_damping_ratio_multiplier"))[0];
    body_motion_data.weight_multiplier = matlabToEigenMap<6, 1>(
        mxGetPropertySafe(mex_body_motions, i, "weight_multiplier"));
  }
  return ret;
}
...
TVLQRData setUpZMPData(const mxArray* mex_zmp_data) {
  ...
  TVLQRData ret;
  ret.A = matlabToEigenMap<NUM_STATES, NUM_STATES>(
      mxGetFieldSafe(mex_zmp_data, "A"));
  ret.B = matlabToEigenMap<NUM_STATES, NUM_INPUTS>(
      mxGetFieldSafe(mex_zmp_data, "B"));
  ret.C = matlabToEigenMap<NUM_OUTPUTS, NUM_STATES>(
      mxGetFieldSafe(mex_zmp_data, "C"));
  ret.D = matlabToEigenMap<NUM_OUTPUTS, NUM_INPUTS>(
      mxGetFieldSafe(mex_zmp_data, "D"));
  ret.u0 = matlabToEigenMap<NUM_INPUTS, 1>(mxGetFieldSafe(mex_zmp_data, "u0"));
  ret.R = matlabToEigenMap<NUM_INPUTS, NUM_INPUTS>(
      mxGetFieldSafe(mex_zmp_data, "R"));
  ret.Qy = matlabToEigenMap<NUM_OUTPUTS, NUM_OUTPUTS>(
      mxGetFieldSafe(mex_zmp_data, "Qy"));
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
  string usage =
      ...
      "qp_locomotion_settings, lcm_channel);";
  if (nrhs < 1 || nrhs > 3) mexErrMsgTxt(usage.c_str());
  ...
  if (nrhs == 1) {
    ...
    if (isa(prhs[0], "DrakeMexPointer")) {
      ...
      mexErrMsgIdAndTxt("Drake:constructQPLocomotionPlanmex:BadInputs",
  ...
  // robot
  RigidBodyTreed* robot = (RigidBodyTreed*)getDrakeMexPointer(mex_model);

  // settings
  QPLocomotionPlanSettings settings;
  settings.duration = mxGetScalar(mxGetPropertySafe(mex_settings, "duration"));
  settings.supports =
      setUpSupports(mxGetPropertySafe(mex_settings, "supports"));
  settings.support_times = matlabToStdVector<double>(
      mxGetPropertySafe(mex_settings, "support_times"));
  settings.contact_groups = setUpContactGroups(
      robot, mxGetPropertySafe(mex_settings, "contact_groups"));
  settings.planned_support_command = matlabToStdVector<bool>(
      mxGetPropertySafe(mex_settings, "planned_support_command"));
  settings.early_contact_allowed_fraction = mxGetScalar(
      mxGetPropertySafe(mex_settings, "early_contact_allowed_fraction"));
  settings.body_motions =
      setUpBodyMotions(mxGetPropertySafe(mex_settings, "body_motions"));
  settings.zmp_trajectory = matlabPPTrajectoryOrMatrixToPiecewisePolynomial(
      mxGetPropertySafe(mex_settings, "zmptraj"));
  settings.zmp_data = setUpZMPData(mxGetPropertySafe(mex_settings, "zmp_data"));
  settings.D_control =
      matlabToEigenMap<2, 2>(mxGetPropertySafe(mex_settings, "D_control"));
  settings.V = setUpLyapunovFunction(mxGetPropertySafe(mex_settings, "V"));
  settings.q_traj = matlabPPTrajectoryOrMatrixToPiecewisePolynomial(
      mxGetPropertySafe(mex_settings, "qtraj"));
  settings.com_traj =
      matlabExpPlusPPOrPPTrajectoryOrVectorToExponentialPlusPiecewisePolynomial(
          mxGetPropertySafe(mex_settings, "comtraj"));
  settings.gain_set =
      mxGetStdString(mxGetPropertySafe(mex_settings, "gain_set"));
  settings.mu = mxGetScalar(mxGetPropertySafe(mex_settings, "mu"));
  settings.use_plan_shift = static_cast<bool>(
      mxGetScalar(mxGetPropertySafe(mex_settings, "use_plan_shift")));
  ...
  settings.knee_settings.min_knee_angle =
      mxGetScalar(mxGetPropertySafe(mex_settings, "min_knee_angle"));
  settings.ankle_limits_tolerance =
      mxGetScalar(mxGetPropertySafe(mex_settings, "ankle_limits_tolerance"));
  settings.knee_settings.knee_kp =
      mxGetScalar(mxGetPropertySafe(mex_settings, "knee_kp"));
  settings.knee_settings.knee_kd =
      mxGetScalar(mxGetPropertySafe(mex_settings, "knee_kd"));
  settings.knee_settings.knee_weight =
      mxGetScalar(mxGetPropertySafe(mex_settings, "knee_weight"));
  settings.zmp_safety_margin =
      mxGetScalar(mxGetPropertySafe(mex_settings, "zmp_safety_margin"));
  settings.pelvis_name = mxGetStdString(mxGetPropertySafe(
      mxGetPropertySafe(mex_settings, "robot"), "pelvis_name"));
  settings.foot_names[Side::LEFT] = mxGetStdString(mxGetPropertySafe(
      mxGetPropertySafe(mex_settings, "robot"), "l_foot_name"));
  settings.foot_names[Side::RIGHT] = mxGetStdString(mxGetPropertySafe(
      mxGetPropertySafe(mex_settings, "robot"), "r_foot_name"));
  settings.knee_names[Side::LEFT] = mxGetStdString(mxGetPropertySafe(
      mxGetPropertySafe(mex_settings, "robot"), "l_knee_name"));
  settings.knee_names[Side::RIGHT] = mxGetStdString(mxGetPropertySafe(
      mxGetPropertySafe(mex_settings, "robot"), "r_knee_name"));
  settings.akx_names[Side::LEFT] = mxGetStdString(mxGetPropertySafe(
      mxGetPropertySafe(mex_settings, "robot"), "l_akx_name"));
  settings.akx_names[Side::RIGHT] = mxGetStdString(mxGetPropertySafe(
      mxGetPropertySafe(mex_settings, "robot"), "r_akx_name"));
  settings.aky_names[Side::LEFT] = mxGetStdString(mxGetPropertySafe(
      mxGetPropertySafe(mex_settings, "robot"), "l_aky_name"));
  settings.aky_names[Side::RIGHT] = mxGetStdString(mxGetPropertySafe(
      mxGetPropertySafe(mex_settings, "robot"), "r_aky_name"));
  settings.constrained_position_indices = matlabToStdVector<int>(
      mxGetPropertySafe(mex_settings, "constrained_dofs"));
  addOffset(settings.constrained_position_indices, -1);  // base 1 to base 0
  settings.untracked_position_indices = matlabToStdVector<int>(
      mxGetPropertySafe(mex_settings, "untracked_joint_inds"));

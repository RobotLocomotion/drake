...
void constraints(mxArray* c_out, mxArray* ceq_out, mxArray* dc_out,
                 mxArray* dceq_out, const Eigen::MatrixXd& x, size_t nc,
                 size_t nceq, size_t nv, size_t nsteps) {
  ...
  for (j = 2; j <= static_cast<int>(nsteps); j++) {
    ...
    dx = rel_steps(0, j - 1);
    dy = rel_steps(1, j - 1);
    dceq.block(x1_ndx + 5, con_ndx, 1, con_dndx) << dx * si + dy * co,
        -dx * co + dy * si;
  ...
  memcpy(mxGetPr(ceq_out), ceq.data(),
         sizeof(double) * ceq.rows() * ceq.cols());
  memcpy(mxGetPr(dceq_out), dceq.data(),
         sizeof(double) * dceq.rows() * dceq.cols());

classdef (InferiorClasses = {?DrakeSystem}) RobustDrakeSystem < DrakeSystem
...
% with (band-limited) white noise input
  ...
  methods % Derived classes should implement one or more of these
    function xcdot = robustDynamics(obj,t,x,u,w)
      % Defines the dynamics xcdot = fc(t,x,u,w), where w is a bounded noise input
      error('systems with continuous states must implement dynamics');
    end
    
    function xdn = robustUpdate(obj,t,x,u,w)
      % Defines the update xd[n+1] = fd(t,x,u,w), where w is a bounded noise input
      error('systems with continuous states must implement update');
    end
    
    function y = robustOutput(obj,t,x,u,w)
      % Defines the output y = g(t,x,u,w), where w is a bounded noise input
      error('systems with outputs must implement the output function');
    end
  end

  methods
    function obj = RobustDrakeSystem(num_xc,num_xd,num_u,num_y,direct_feedthrough_flag,time_invariant_flag,num_w,ts_w)
      % Construct a RobustDrakeSystem
      % 
      ...
      %   possible.
      % @param num_w number of bounded noise inputs

...
class FormatterBase(object):
    """A base class for formatting-related tools, with the ability to load a file,
    add / remove / modify lines, clang-format selected regions, and compare the
    changes to the original file contents.
    ...
    def __init__(self, filename, readlines=None):
        """Create a new FormatterBase.  The required filename parameter refers to the
        workspace-relative full path, e.g. drake/common/drake_assert.h.
    ...
    def is_permutation_of_original(self):
        ...
        file contents read in by (or passed to) the constructor, modulo blank
        lines.
    ...
    def get_format_indices(self):
        """Return the sorted list of all indices that pass the self.should_format
        predicate.
...
class IncludeFormatter(FormatterBase):
    ...
    def __init__(self, filename, *args, **kwargs):
        ...
        # For an inl-h file, clang will not realize the .h is "related".
    ...
    def format_includes(self):
        ...
        # We want to preserve the comment line lalyours near include
        # statements, so here only _completely_ empty ranges are removed.

...
2. ``Workspace`` is your existing ``drake-distro`` directory.
3. Import your project view from your workspace: ``drake-distro/.bazelproject``
4. (Advanced) You can make custom edits to your ``.bazelproject`` if you only
   wish to develop a subset of Drake. Most users should leave it as-is.
5. CLion will begin ingesting the Drake source and building symbols. This will
...
To build or run a specific target to to ``Run > Edit Configurations``. Click
``+`` to create a new Bazel command.  Specify the configuration name and Bazel
options, then hit OK.  Launch the configuration from the ``Run`` menu.

...
Drake includes support for some externals which we are not able to redistribute
directly (SNOPT, SEDUMI, ...).  In order to have drake locally install these for
you, you must have access to the corresponding private github repositories, and
must set your machine up with SSH keys.
...
Gurobi's website, <http://www.gurobi.com/>. Download Gurobi 6.0.5, and set
the ``GUROBI_DISTRO`` environment variable to the path to the downloaded file.
Consult Gurobi's documentation to activate your license; exact procedures
depend on license type. Once activated, place your Gurobi license file
``gurobi.lic`` in your home directory.

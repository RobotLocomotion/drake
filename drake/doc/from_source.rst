...
Ninja can rebuild Drake from within the `/drake` subdirectory without
...
``cmake`` options can be configured via ``ccmake`` or at the command line.
For instance, to set the build type:

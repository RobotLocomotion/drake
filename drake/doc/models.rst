...
There are three ways in which ``drake-visualizer`` applies textures to meshes:

  1. Via VTK's native VTM/VTP texture files.
  2. Via OBJ's native MTL material files.
  3. Via URDF's and SDF's link color specification.

Priority will be given in the order shown above, i.e., VTM/VTP will be used
if they exist followed by MTL and finally SDF and URDF link color
specifications. Note that VTM/VTP files will only be used if they (1) exist in
the same directory as the OBJ file and (2) share the same base file name as the
OBJ file.

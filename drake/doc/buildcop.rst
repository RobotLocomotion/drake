...
If you believe a CI script change is the culprit, contact the author, and also
alert ``@RobotLocomotion/kitware`` via GitHub issue. If they are not responsive,
revert the commit yourself and see what happens on the next continuous build.
There are no pre-merge builds you can run that exercise changes to the CI
scripts themselves.

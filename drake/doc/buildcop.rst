...
Build Cop Playbook
------------------
This section is a quick-reference manual for the oncall build-cop.

Monitor the Build
^^^^^^^^^^^^^^^^^
Check either your CI failure notification e-mail list, or the
`post-submit <https://drake-jenkins.csail.mit.edu/view/Continuous/>`_ build
dashboard in Jenkins, at least once an hour during oncall hours. If any builds
are yellow or red, you need to act.
...
In almost any build breakage, thehe first information-gathering step is to
...
* The platform or test case only runs post-submit.
* A administrator performed an override-merge of the culprit PR,
  circumventing pre-submit checks.
...
by clicking "Revert" in the GitHub UI. Use the template message above to
communicate with the author.

If the failing build passes on the rollback PR, the odds are good that you have
found the culprit. Proceed as specified in the template message.
...
GitHub UI. Use the template message above to communicate with the author, and
...
the `drake-ci <https://github.com/RobotLocomotion/drake-ci>`_ repo as well as
Drake proper. Consider whether those changes are possible culprits.  If you
believe so, contact the author, and also alert Kitware via GitHub issue. If they
are not responsive, revert the commit yourself and see what happens on the next
continuous build.  (CI script changes cannot be tested before merge.)
...
* Broken connection to a Mac build VM.
* Broken connection to a Windows build VM.
...
increased, alert Kitware via GitHub issue.
...
Attempt to figure out which piece of infrastructure collapsed. If it's under
our control, alert Kitware via GitHub issue. If it's under a vendor's control,
spread the news as widely as you can and simply wait it out.
...
build dashboard in Jenkins at least once an hour during oncall hours. If any
...
Manually schedule the failing build as an experimental build on the rollback
PR. If it passes, the odds are good that you have found the culprit. Proceed
as specified in the template message.
...
believe so, contact the author, and also alert ``@RobotLocomotion/kitware`` via
GitHub issue. If they are not responsive, revert the commit yourself and see
what happens on the next continuous build.  (CI script changes cannot be tested
before merge.)

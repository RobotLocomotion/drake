...
.. _dynamic_analysis_tools_sanitizers:
...
and above. ::
    ...
    cmake -DCMAKE_BUILD_TYPE:STRING=Debug -DUSE_SANITIZER:BOOL=Address ..
    cmake --build .
    cd drake
...
is a fast memory leak detector that is part of AddressSanitizer. ::
    ...
    mkdir build && cd build
    cmake -DCMAKE_BUILD_TYPE:STRING=Debug -DUSE_SANITIZER:BOOL=Leak ..
    cmake --build .
    cd drake
    ctest --extra-verbose --test-action MemCheck .
...
above. ::
    ...
    mkdir build && cd build
    cmake -DCMAKE_BUILD_TYPE:STRING=Debug -DUSE_SANITIZER:BOOL=Memory ..
    cmake --build .
    cd drake
    ctest --extra-verbose --test-action MemCheck .
...
above. ::
    ...
    mkdir build && cd build
    cmake -DCMAKE_BUILD_TYPE:STRING=Debug -DUSE_SANITIZER:BOOL=Thread ..
    cmake --build .
    cd drake
    ctest --extra-verbose --test-action MemCheck .
...
GCC 4.9 and above. ::
    ...
    mkdir build && cd build
    cmake -DCMAKE_BUILD_TYPE:STRING=Debug -DUSE_SANITIZER:BOOL=Undefined ..
    cmake --build .
    cd drake
    ctest --extra-verbose .

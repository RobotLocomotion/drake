...
* The `std::hash <https://google.github.io/styleguide/cppguide.html#std_hash>`_
  ...
  * Unless there is a specific and well-documented reason to the contrary,
    *all* hash functions should be implemented as either a direct call to a
    built-in C++ hash function, or a composition of standard built-in hash
    functions using the machinery in ``drake/common/hash.h``.
  ...
  * All hash functions should be given the same level of scrutiny, no matter
    what names they are given or where they are defined.
  * Rationale:

    * Hash functions are necessity of modern C++ programming.

...
* Never use the `assert()` macro from `<cassert>`; instead, use `DRAKE_ASSERT`.
  Never use `DRAKE_ASSERT(false)`; instead, use `DRAKE_ABORT()`. For discussion,
  see Drake `#1935 <https://github.com/RobotLocomotion/drake/issues/1935>`_.

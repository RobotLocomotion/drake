...
* After including ``<cstddef>``, assume that ``size_t`` and ``ptrdiff_t``
  are defined in the global namespace. Do not preface them
  with ``std::`` and do not write ``using std::size_t``. There is a
  hypothetical possibility that this won't work on some compiler
  someday but we deem the risk acceptable in trade for allowing this
  common, clutter-reducing practice. For discussion, see

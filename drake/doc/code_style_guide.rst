...
* For the `Copyable and Movable Types <https://google.github.io/styleguide/cppguide.html#Copyable_Movable_Types>`_,
  we clarify that the copy and move operations on a class must always be noted
  specifically for each class (e.g., not simply inheriting the disposition of a
  base class).  Prefer to use ``drake/common/drake_copyable.h`` macros (either
  ``DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN`` or
  ``DRAKE_DEFAULT_COPY_AND_MOVE_AND_ASSIGN`` whenever practical.  When not using
  the macros, the copy constructor and copy-assignment operator at least must
  be either deleted, default, or implemented.  For discussion, see

...
* Always use `in-class member initialization <http://www.stroustrup.com/C++11FAQ.html#member-init>`_ for data members that would otherwise be uninitialized, such as ints and pointers. The syntax ``int i{};`` (called "value initialization") ensures that numeric and pointers types are zero-initialized rather than left as garbage.

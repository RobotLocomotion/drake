...
IdealDepthSensorSpecification::IdealDepthSensorSpecification(
...
void IdealDepthSensorSpecification::set_min_theta(double min_theta) {
  min_theta_ = min_theta;
  update_theta_increment();
}

void IdealDepthSensorSpecification::set_max_theta(double max_theta) {
  max_theta_ = max_theta;
  update_theta_increment();
}

void IdealDepthSensorSpecification::set_num_theta_values(
    double num_theta_values) {
  num_theta_values_ = num_theta_values;
  update_num_depth_readings();
  update_theta_increment();
}

void IdealDepthSensorSpecification::set_num_phi_values(double num_phi_values) {
  num_phi_values_ = num_phi_values;
  update_num_depth_readings();
  update_phi_increment();
}

void IdealDepthSensorSpecification::update_num_depth_readings() {
  num_depth_readings_ = num_theta_values_ * num_phi_values_;
}

void IdealDepthSensorSpecification::update_theta_increment() {
  theta_increment_ =
      CalculateIncrementSize(max_theta_, min_theta_, num_theta_values_);
}

void IdealDepthSensorSpecification::update_phi_increment() {

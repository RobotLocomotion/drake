...
/// of IdealDepthSensor::kInvalidDepthMeasurement is provided.
///
/// @ingroup sensor_systems
///
class IdealDepthSensor : public RigidBodyTreeSensor {
 public:
  /// The depth reading when either the sensed object is less than the minimum
  /// range or greater than the maximum range.
  static const int kInvalidDepthMeasurement = -1;

  /// A constructor that initializes an IdealDepthSensor.
  ///
  ...
  ///
  IdealDepthSensor(const RigidBodyTree<double>& tree,
  ...
  ///@{
  explicit IdealDepthSensor(const IdealDepthSensor&) = delete;
  IdealDepthSensor& operator=(const IdealDepthSensor&) = delete;
  ///@}

  double get_min_theta() const { return min_theta_; }

  double get_max_theta() const { return max_theta_; }

  double get_min_phi() const { return min_phi_; }

  double get_max_phi() const { return max_phi_; }

  double get_num_theta_values() const { return num_theta_values_; }

  double get_num_phi_values() const { return num_phi_values_; }
  ...
  /// constructor.
  int num_pixel_rows() const { return num_phi_values_; }
  ...
  /// constructor.
  int num_pixel_cols() const { return num_theta_values_; }

  /// Returns the size of the system's output, which equals the product of
  /// num_pixel_rows() and num_pixel_cols().
  int get_num_depth_readings() const { return num_depth_readings_; }

  /// Debug code
  // const Eigen::Matrix3Xd& get_endpoints() const;
  ...
  /// Allocates the output port. See this class's description for details of
  /// this port.
  std::unique_ptr<SystemOutput<double>> AllocateOutput(
      const Context<double>& context) const override;

  friend std::ostream& operator<<(std::ostream& out,
                                  const IdealDepthSensor& depth_sensor);
 ...
 private:
  ...
  // sensor's frame (as defined by member variable frame_). Its end, in the
  // sensor's frame, is computed by this method and stored in member variable
  // raycast_endpoints_ at the time of construction. raycast_endpoints_ is only
  // computed once at construction since the end points are constant in the
  // frame of the sensor.
  //
  ...
  const double min_theta_{};
  const double max_theta_{};
  const double min_phi_{};
  const double max_phi_{};
  const int num_theta_values_{};
  const int num_phi_values_{};
  const int num_depth_readings_{};
  const double min_range_{0};
  const double max_range_{std::numeric_limits<double>::max()};

  const double theta_increment_{};
  const double phi_increment_{};

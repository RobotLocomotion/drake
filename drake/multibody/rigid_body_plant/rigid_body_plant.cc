...
RigidBodyPlant<T>::RigidBodyPlant(std::unique_ptr<const RigidBodyTree<T>> tree)
  ...
  const std::pair<int, int> default_entry = std::pair<int, int>(-1, 0);
  ...
  if (num_instances != 0) {
    ...
    for (int actuator_index = 0; actuator_index < tree_->get_num_actuators();
      ...
      if (actuator_map_[instance_id].first == -1) {
    ...
    for (int i = 0; i < num_instances; ++i) {
      if (actuator_map_[i].first == -1) { continue; }
    ...
    for (const auto& body : tree_->bodies) {
      ...
      if (num_positions) {
        if (position_map_[instance_id].first == -1) {
...
void RigidBodyPlant<T>::EvalOutput(const Context<T>& context,
  ...
  const auto output_values = output_vector->get_value();
...
void RigidBodyPlant<T>::EvalTimeDerivatives(
  ...
  if (input != nullptr) {
    ...
    for (int instance_id = 0;
      ...
      DRAKE_ASSERT(instance_actuators.first != -1);

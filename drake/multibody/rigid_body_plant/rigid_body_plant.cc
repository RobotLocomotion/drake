...
RigidBodyPlant<T>::RigidBodyPlant(std::unique_ptr<const RigidBodyTree<T>> tree)
  ...
  for (const auto& actuator : actuator_map_) {
    input_map_[actuator.first] = this->DeclareInputPort(
        kVectorValued, actuator.second.size()).get_index();
  }
  ...
  for (auto& position : position_map_) {
    ...
    // Sort the vectors in the position and velocity map so that the
    // order corresponds to what they would have been in the rigid
    // body tree.
    std::sort(position.second.begin(), position.second.end());
    std::sort(velocity_map_[instance_id].begin(),
              velocity_map_[instance_id].end());
  }
}
...
template <typename T>
int RigidBodyPlant<T>::FindInstancePositionIndex(
    int model_instance_id, int position_index) {
  DRAKE_ASSERT(position_map_.count(model_instance_id) > 0);
  for (int i = 0;
       i < static_cast<int>(position_map_[model_instance_id].size()); ++i) {
    const int position = position_map_[model_instance_id][i];
    if (position_index == position) { return i; }
  }
  return -1;
}

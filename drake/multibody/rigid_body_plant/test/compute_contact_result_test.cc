...
TEST_F(ContactResultTest, SingleCollision) {
  ...
  // NOTE: the *effective* stiffness of the contact is half of the material
  // stiffness.
  const double effective_stiffness = kContactStiffness * 0.5;
  ...
  // the offset.
  double force = effective_stiffness * offset * 2;

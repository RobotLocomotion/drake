...
GTEST_TEST(RigidBodyPlantMuxTest, InstancePortTest) {
  auto tree_ptr = make_unique<RigidBodyTree<double>>();
  drake::parsers::urdf::AddModelInstanceFromUrdfFile(
      drake::GetDrakePath() +
  ...
  EXPECT_EQ(plant.get_num_actuators(0), 4);
  EXPECT_EQ(plant.get_num_positions(0), 4);
  EXPECT_EQ(plant.get_num_velocities(0), 4);
  EXPECT_EQ(plant.get_num_states(0), 8);
  EXPECT_EQ(plant.get_num_actuators(1), 3);
  EXPECT_EQ(plant.get_num_positions(1), 3);
  EXPECT_EQ(plant.get_num_velocities(1), 3);
  EXPECT_EQ(plant.get_num_states(1), 6);
};

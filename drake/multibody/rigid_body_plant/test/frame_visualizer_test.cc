...
GTEST_TEST(FrameVisualizerTests, TestMessageGeneration) {
  ...
  // Visualize the 7th body' frame.
  std::vector<FrameVisualizer::FrameData> local_transforms(1);
  local_transforms[0].body = tree.FindBody("iiwa_link_ee");
  local_transforms[0].name = "iiwa_link_ee";
  local_transforms[0].X_BF.linear() =
      AngleAxis<double>(0.3, Vector3<double>::UnitZ()).toRotationMatrix();
  local_transforms[0].X_BF.translation() = Vector3<double>(1, 2, 3);
  ...
  const Isometry3<double> X_WF = tree.CalcFramePoseInWorldFrame(
      cache, *local_transforms[0].body, local_transforms[0].X_BF);
  const Quaternion<double> quat_tmp(X_WF.linear());
  // Casts to float, becase the message is in float.

...
template <typename T>
void CompliantContactModel<T>::set_parameters(
...
VectorX<T> CompliantContactModel<T>::ComputeContactForce(
  ...
  for (const auto& pair : pairs) {
    if (pair.distance < 0.0) {  // There is contact.
      ...
      // the contact point converges to that point.
      // The use of s_a *may* seem counter-intuitive. I.e., if *all* compression
      // is on element A, we are fully taking the position on element B. The
      // point *on* B is in fact the deepest penetrating point on A, which
      // represents the desired contact point. So, the apparent backwardness
      // is, in fact, correct.
...
double CompliantContactModel<T>::CalcContactParameters(
  ...
  // Relative "squish" s_a and s_b; the portion of the compression that is
  // experienced by elements A and B, respectively, based on their
  // relative stiffness.
  ...
  parameters->set_friction(
      2 * paramA.static_friction() * paramB.static_friction() /
          (paramA.static_friction() + paramB.static_friction()),
      2 * paramA.dynamic_friction() * paramB.dynamic_friction() /

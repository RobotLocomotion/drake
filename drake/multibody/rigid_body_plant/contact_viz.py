...
class ContactVisualizer(object):
    def __init__(self):
        ...
        lcmUtils.addSubscriber('CONTACT_RESULTS', messageClass=lcmdrakemsg.lcmt_contact_results_for_viz, callback=self.handleMessage)
    ...
    def handleMessage(self, msg):
        ...
        for contact in msg.contact_info:
            point = np.array([contact.contact_point[0], contact.contact_point[1], contact.contact_point[2]])
            force = np.array([contact.contact_force[0], contact.contact_force[1], contact.contact_force[2]])
        ...
        for key, list_of_forces in collision_pair_to_forces.iteritems():
            ...
            for force_pair in list_of_forces:
                d.addArrow(start=force_pair[0], end=force_pair[1], tubeRadius=0.005, headRadius=0.01)
...
def initVisualizer():
    ...
    # add to tools menu
    applogic.MenuActionToggleHelper('Tools', myVisualizer.name, myVisualizer.isEnabled, myVisualizer.setEnabled)
    return myVisualizer

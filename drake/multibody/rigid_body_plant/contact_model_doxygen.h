...
/** @defgroup contact_model Computing Contact Forces
 ...
 we can consider a decomposition of `f(q) = A(q)·σ(q)` where `q` is the
 contact configuration which depends on how the contact is characterized,
 `A(q)` is a function that captures the area of the contact and
 ...
 fₙ(q) = k·A(q)·σ(q) is a reasonable characterization of compliant response.
...
/** @defgroup contact_engineering Working with Contacts in Drake
 ...
 - ** Stiffness and model limitations **
   ...
   by assuming `A(x) = 1 m²` and `σ(x) = x / 1 m`. The implication is that the

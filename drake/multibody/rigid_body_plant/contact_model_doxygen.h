...
/** @defgroup per_object_contact Per-object contact material
 ...
 as the default values change. The implication is the order of operations
...
/** @defgroup contact_engineering Working with Contacts in Drake
 ...
 - ** Stiffness and model limitations **
   ...
   The current implementation models contact with a single representative point.
   And a single scalar measure of the depth of penetration of that point.
   However, we defined stiffness's units to be stress/strain which is N/m² and
   strain is a dimensionless ratio. So, when we multiple stiffness by
   penetration depth (as indicated above), we don't properly get force, but
   force per length. However, this is the value that is reported as the normal
   force. There is an inaccurate implicit assumption of another length dimension
   of unit length. This assumption is erroneous. Ideally, there should be some
   additional value associated with the contact point which would allow us to
   capture the not just the depth of the penetration, but the area of the
   contact manifold. A contact that affects a large cross section to a fixed
   depth should obviously create a greater force than one of the same depth
   but smaller contact manifold.

   The implication is that the effect of a single stiffness value will be
   affected by the number of contact points reported. Selecting a stiffness
   value from a reference table may not produce the desired result. The
   simulated material may appear significantly softer. As the number of contact
   points increase, we sample the contact manifold at a higher rate and get
   a truer representation and, therefore, a more meaningful contact force. This
   is particularly significant when compared to the next point.

...
/** @defgroup contact_model Computing Contact Forces
 ...
 More generally, we can define `fₙ = kf(x)` (omitting the unitless dissipation
 effects for simplicity). `f(x)` must have units of m²·strain. To that end,
 we can consider a decomposition of `f(q) = A(q)·σ(q)` where `q` is the state
 of the contact,`A(q)` is a function that captures the area of the contact and
 `σ(q)` captures the strain (guaranteeing our units are correct). In Drake's
 ...
 function makes a similar unit assumption as `σ(x) = x /1 m`. Therefore,
 `f(x) = A(q)·σ(q) = 1 m²·x /1 m = x` (with units m²·strain). These simplifying
 assumptions lead to artifacts. See @ref contact_engineering for details.

 This is a meaningful mathematical model for compliant contact. Future
 implementations will replace the overly simplistic assumptions built into the
...
/** @defgroup contact_engineering Working with Contacts in Drake
 ...
 - ** Stiffness and model limitations **
   ...
   As indicated earlier, the contact model generates a contact force by assuming
   `A(x) = 1 m²` and `q(x) = x / 1 m`. The implication is that the force acting
   on bodies in contact are not purely a function of stiffness and degree of
   penetration but also to the _number_ of contact points reported.
   Selecting a stiffness value from a reference table may not produce
   ...
   higher rate and get a truer representation of the area of contact and,

...
bool CollisionCallback(fcl::CollisionObjectd* fcl_object_A,
  ...
  if (element_A && element_B && element_A->CanCollideWith(element_B)) {
    ...
    for (auto contact : contacts) {
      // Let P and Q be the contact points on bodies A and B respectively.
...
void FclModel::DoAddElement(const Element& element) {
  ...
  if (id != 0) {
    ...
    if (fcl_geometry != nullptr) {
      ...
      // Store a pointer to the Element in the fcl collision object. This will
      // be used for collision filtering inside callback functions.
      ...
      // Take ownership of FCL collision object, as its lifetime must extend to
      // at least that of the broadphase manager.
...
bool FclModel::ComputeMaximumDepthCollisionPoints(
  ...
  // The use_margins parameter only applies to Bullet.

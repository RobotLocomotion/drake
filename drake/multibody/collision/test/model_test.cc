...
TEST_P(ShapeVsShapeTest, ComputeMaximumDepthCollisionPoints) {
  ...
  Vector3d n_BcAc_W_expected = solution_[point.elementB].normal;
  // Remainder of test assumes unit normal
  ASSERT_DOUBLE_EQ(n_BcAc_W_expected.norm(), 1);
  Vector3d p_BcAc_W_expected = p_WAc_expected - p_WBc_expected;
  double distance_expected{p_BcAc_W_expected.dot(n_BcAc_W_expected)};
  ...
  EXPECT_TRUE(CompareMatrices(point.normal, n_BcAc_W_expected, tolerance_,

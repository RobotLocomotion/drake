...
class Model {
 public:
  ...
  /** Changes the element-to-world transform of a specified collision element.
  ...
  /** Computes the points of closest approach between all eligible pairs of
   ...
   @param use_margins flag indicating whether or not to use the version of this
   model with collision margins

   @param[out] closest_points reference to a vector of PointPair objects that
  ...
  /** Computes closest distance from each point to any surface in the collision
   model utilizing Bullet's collision detection code.

   @param points a matrix of points from which to computing distances

   @param use_margins flag indicating whether or not to use the version of this
   model with collision margins

   @param[out] closest_points a vector of PointPair objects containing the
   signed distances **/
  ...
  indices of those points within a specified distance of any collision geometry
  in the model.

  In other words, this method tests if a sphere of radius `collision_threshold`
  located at `input_points[i]` collides with any part of the model. The result
  is returned as a vector of indexes in `input_points` that do collide with the
  model. Points are not checked against one another but only against the
  existing model.
  ...
  /** Tests if any of the points supplied in `input_points` collides with any
  part of the model within a given `collision_threshold`.
  ...
  @return `true` if any of the points positively checks for collision. `false`
  otherwise. **/
  virtual bool collidingPointsCheckOnly(
      const std::vector<Eigen::Vector3d>& input_points,
      double collision_threshold) = 0;

  /** Performs raycast collision detection (like a LIDAR / laser range finder).
   ...
   3 x 1, the same origin is used for all raycasts

   @param ray_endpoint a 3 x N matrix in which each column specifies a second
   point on the corresponding ray

   @param use_margins a flag indicating whether or not to use the version of
   this model with collision margins

   @param[out] distance to the first collision, or -1 if no collision occurs

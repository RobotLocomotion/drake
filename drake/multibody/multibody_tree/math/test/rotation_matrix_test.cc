...
GTEST_TEST(RotationMatrix, DefaultRotationMatrixIsIdentity) {
  ...
  Matrix3d zero_matrix = R.get_as_Matrix3() - Eigen::Matrix3<T>::Identity();
  EXPECT_TRUE((zero_matrix.array() == 0).all());
}

// Test constructor for an Eigen::Matrix3.
GTEST_TEST(RotationMatrix, EigenMatrix3Constructor) {
  ...
  RotationMatrix<double> R = RotationMatrix::MakeUnchecked(m);
...
GTEST_TEST(RotationMatrix, Transpose) {
  ...
  RotationMatrix<double> R = RotationMatrix::MakeUnchecked(m);
  RotationMatrix<double> R_tranpose = R.transpose();
...
GTEST_TEST(RotationMatrix, Inverse) {
  ...
  EXPECT_TRUE(should_be_identity.IsNearlyEqualTo(identity_matrix,9*kEpsilon));
}
...
GTEST_TEST(RotationMatrix, Symmetry) {
...
GTEST_TEST(RotationMatrix, IsNearlyEqualTo) {
...
GTEST_TEST(RotationMatrix, ReExpressInAnotherFrameA) {
...
// TestB: Rotational inertia expressed in frame R then re-expressed in frame E.
GTEST_TEST(RotationMatrix, ReExpressInAnotherFrameB) {
  // Rotate rigid-body B from frame A by a Euler body-fixed sequence
  // characterized by BodyXYZ q1, q2, q3.  Calculations from MotionGenesis.
...
GTEST_TEST(RotationMatrix, ShiftFromCenterOfMass) {
...
GTEST_TEST(RotationMatrix, ShiftToCenterOfMass) {
...
GTEST_TEST(RotationMatrix, ShiftToThenAwayFromCenterOfMass) {
...
GTEST_TEST(RotationMatrix, CouldBePhysicallyValidA) {
...
GTEST_TEST(RotationMatrix, CouldBePhysicallyValidB) {
...
GTEST_TEST(RotationMatrix, CouldBePhysicallyValidC) {
...
GTEST_TEST(RotationMatrix, CouldBePhysicallyValidD) {
...
GTEST_TEST(RotationMatrix, CouldBePhysicallyValidE) {
...
GTEST_TEST(RotationMatrix, PrincipalMomentsOfInertia) {
  const double mass = 1.0;
  const double Lx = 3.0;
  const double Ly = 1.0;
  const double Lz = 5.0;
...
GTEST_TEST(RotationMatrix, PrincipalMomentsOfInertiaLaplacianTest) {
...
GTEST_TEST(RotationMatrix, MultiplicationWithScalarFromTheLeft) {
  const Vector3d m(2.0,  2.3, 2.4);  // m for moments.
  const Vector3d p(0.1, -0.1, 0.2);  // p for products.
  const RotationalInertia<double> I(m(0), m(1), m(2),  /* moments of inertia */
                                    p(0), p(1), p(2)); /* products of inertia */
...
//  - operator*=(const T&)
//  - operator/=(const T&)
GTEST_TEST(RotationMatrix, OperatorPlusEqual) {
  const Vector3d m(2.0,  2.3, 2.4);  // m for moments.
  const Vector3d p(0.1, -0.1, 0.2);  // p for products.
...
GTEST_TEST(RotationMatrix, ShiftOperator) {
...
GTEST_TEST(RotationMatrix, CastToAutoDiff) {
...
GTEST_TEST(RotationMatrix, AutoDiff) {
  typedef Eigen::AutoDiffScalar<Vector1<double>> AutoDiff1d;

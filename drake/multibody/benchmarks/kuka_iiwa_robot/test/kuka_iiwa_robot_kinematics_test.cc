...
class KukaIIwaRobotTestKinematics {
 public:
  ...
  KukaIIwaRobotTestKinematics() {
    ...
    model_->Finalize();
    EXPECT_TRUE(model_->topology_is_valid());
  ...
  CalcEndEffectorPositionVelocity(const double q[7], const double v[7]) {
...
GTEST_TEST(KukaIIwaRobotKinematics, KinematicsTestA) {
  ...
  const double qA = 0 * q60, qADt = 0.0;
  const double qB = 0 * q30, qBDt = 0.0;
  const double qC = 0 * q60, qCDt = 0.0;
  const double qD = 0 * q30, qDDt = 0.0;
  const double qE = 0 * q60, qEDt = 0.0;
  const double qF = 0 * q30, qFDt = 0.0;
  const double qG = 0 * q60, qGDt = 0.0;
  ...
  SpatialVelocity<double> V_NG_N;
  std::tie(X_NG, V_NG_N) =
      drake_kuka_robot.CalcEndEffectorPositionVelocity(q.data(), qDt.data());
  ...
  KukaIIwaRobot<double> motion_genesis_kukaIIwaRobot;
  // R_NG       | Rotation matrix relating Nx, Ny, Nz to Gx, Gy, Gz.
  // p_NoGo_N   | Go's position from No, expressed in N.
  // w_NG_N     | G's angular velocity in N, expressed in N.
  // v_NGo_N    | Go's velocity in N, expressed in N.
  ...
  std::tie(R_NG_true, p_NoGo_N_true, w_NG_N_true, v_NGo_N_true) =
      motion_genesis_kukaIIwaRobot.CalcForwardKinematicsEndEffector(q, qDt);

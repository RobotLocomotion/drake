...
/// revolute motor.  The motor stator is at point Nao (the origin of a frame Na
/// that is fixed on N) and the motor's rotor is at the origin Ao of rigid link
/// (frame) A.  Point Nao is coincident with Ao (these points are at the same
/// location -- but fixed to different objects (Nao is fixed to N and Ao to A).

...
class GlobalInverseKinematics : public solvers::MathematicalProgram {
 public:
   ...
   * pose for each body (position/orientation). This function loops through each
   * body inside the robot kinematics tree, add the constraint on the body pose,
   * so that the adjacent bodies are welded correctly by the joint.
   ...
   * the specified index. This is the orientation of body i's frame in the world
   ...
   * specified index. This is body i's frame origin in the world frame,
 ...
 private:
  ...
  // p_WB_[i] is the position for body i's origin in the world frame, expressed
  // in the world frame.

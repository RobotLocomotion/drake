...
unique_ptr<RigidBodyTree<double>> RigidBodyTree<double>::Clone() const {
  ...
  for (const auto& original_body : bodies) {
    ...
    DRAKE_DEMAND(original_body_index == original_body->get_body_index());
    if (original_body_index == RigidBodyTreeConstants::kWorldBodyIndex) {
      continue;
    }
    ...
    DRAKE_DEMAND(cloned_body_index == cloned_body->get_body_index());

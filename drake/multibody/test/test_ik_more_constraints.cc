...
GTEST_TEST(testIKMoreConstraints, IKMoreConstraints) {
  ...
  // 2 Knees Constraint
  PostureConstraint kc_posture_knees(model.get(), tspan);
  ...
  PostureConstraint kc_posture_larm(model.get(), tspan);
  ...
  Vector3d lfoot_pos0(0, 0.13, 0.08);
  ...
  WorldPositionConstraint kc_rfoot_pos(model.get(), r_foot, r_foot_pt,
                                       rfoot_pos_lb, rfoot_pos_ub, tspan);
  WorldQuatConstraint kc_rfoot_quat(model.get(), r_foot, quat_des, tol, tspan);
  ...
  Vector3d rhand_pos0(0.2, -0.5, 0.4);
  // Vector3d rhand_pos0 =
  //     model->forwardKin(r_hand_pt, r_hand, 0, 0, 0).value();
  ...
  rhand_pos_ub(2) += 0.05;
  WorldPositionConstraint kc_rhand(model.get(), r_hand, r_hand_pt, rhand_pos_lb,
                                   rhand_pos_ub, tspan);

  // 7 QuasiStatic Constraints
  QuasiStaticConstraint kc_quasi(model.get(), tspan);
  ...
  inverseKin(model.get(), qstar, qstar, constraint_array.size(),

...
RigidBody<double>* MakeBody(int id, RigidBody<double>* parent) {
  auto rb1 = new RigidBody<double>();
...
GTEST_TEST(RigidBodyTreeCompleteness, FullyConnected) {
  ...
  unique_ptr<RigidBody<double>> rb1(MakeBody(1, &world));
  unique_ptr<RigidBody<double>> rb2(MakeBody(2, &world));
  unique_ptr<RigidBody<double>> rb3(MakeBody(3, rb1.get()));
...
GTEST_TEST(RigidBodyTreeCompleteness, MissingParent) {
  ...
  unique_ptr<RigidBody<double>> rb2(MakeBody(2, &world));
  unique_ptr<RigidBody<double>> rb3(MakeBody(3, rb1.get()));
  unique_ptr<RigidBody<double>> rb4(MakeBody(4, nullptr));

...
TEST_F(RigidBodyTreeCloneTest, CloneTwoDofRobot) {
  ...
  AddModelInstanceFromUrdfFileWithRpyJointToWorld(filename,
                                                  tree_.get());
  unique_ptr<RigidBodyTree<double>> cloned_tree = tree_->Clone();
  EXPECT_TRUE(tree_->CompareToClone(*cloned_tree));
}

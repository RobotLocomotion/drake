...
bool CompareToClone(const RigidBodyTree<double>& tree) {
  ...
  for (int i = 0; i < tree.get_num_actuators(); ++i) {
    const RigidBodyActuator* this_actuator = &tree.actuators.at(i);
    const RigidBodyActuator* other_actuator = &clone->actuators.at(i);
    if (!this_actuator->CompareToClone(*other_actuator)) {
      drake::log()->debug(
          "CompareToClone(RigidBodyTree): actuators mismatch at index {}.",
          i);
      return false;
    }
  }

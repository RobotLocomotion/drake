...
std::unique_ptr<AutomotiveSimulator<double>>
MakeWithIdmCarAndDecoy(bool disable_lcm) {
  auto simulator = std::make_unique<AutomotiveSimulator<double>>(
      std::make_unique<lcm::DrakeMockLcm>(), disable_lcm);
  const maliput::api::RoadGeometry* road{};
  EXPECT_NO_THROW(
  ...
  const maliput::api::GeoPosition start_position = start_lane->ToGeoPosition(
      {start_s_position, 0., 0.});
...
GTEST_TEST(AutomotiveSimulatorTest, TestIdmControlledSimpleCar) {
  auto simulator = MakeWithIdmCarAndDecoy(false /* enable LCM */);
...
GTEST_TEST(AutomotiveSimulatorTest, TestIdmControlledSimpleCarLcmDisabled) {
  auto simulator = MakeWithIdmCarAndDecoy(true /* disable LCM */);

...
TYPED_TEST(TrajectoryCarTest, ConstantSpeedTest) {
  ...
  for (const auto& it : cases) {
    ...
    SetDerivatives(&speed, Vector1d{0.});
...
TYPED_TEST(TrajectoryCarTest, AccelerationInputTest) {
  ...
  SetDerivatives(&kInitialPathPosition, Vector1d{0.});
  SetDerivatives(&kInitialSpeed, Vector1d{0.});
  ...
  // Recover the AutoDiff derivative at the speed derivative.
...
TYPED_TEST(TrajectoryCarTest, SaturatingSpeedTest) {
  ...
  SetDerivatives(&kOverSpeed, Vector1d{0.});
  ...
  T kUnderSpeed = -5.;
  SetDerivatives(&kUnderSpeed, Vector1d{0.});

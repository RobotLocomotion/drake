...
GTEST_TEST(BoxCarVisTest, BasicTest) {
  ...
  PoseBundle<double> pose_bundle(1 /* num poses */);
  EXPECT_THROW(dut.AddPoses(root_pose.get_isometry(), &pose_bundle),
      std::runtime_error);

  lcmt_viewer_load_robot load_message;
  load_message.num_links = 0;
  load_message.link.resize(0);

  dut.AddVisualizationElements(&load_message);
  EXPECT_THROW(dut.AddVisualizationElements(&load_message), std::runtime_error);
  EXPECT_EQ(load_message.num_links, 1);

  const lcmt_viewer_link_data& link_data = load_message.link.at(0);
  ...
  EXPECT_NO_THROW(dut.AddPoses(root_pose.get_isometry(), &pose_bundle));
  ...
  EXPECT_EQ(pose_bundle.get_model_instance_id(0), kModelInstanceId);
  EXPECT_EQ(pose_bundle.get_name(0), kName);
}

class CarVisApplicatorTest : public ::testing::Test {
 protected:
  void SetUp() override {
    dut_.reset(new CarVisApplicator<double>(1 /* num_car_poses */,
                                            1 /* num_vis_poses */));
...
TEST_F(CarVisApplicatorTest, Configuration) {
  ...
  // Verifies that an exception is thrown if more than one CarVis is provided
  ...
  EXPECT_EQ(dut_->num_model_instances(), 1);
  EXPECT_EQ(dut_->num_car_poses(), 1);

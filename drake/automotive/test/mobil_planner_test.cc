...
class MobilPlannerTest : public ::testing::Test {
 protected:
  ...
  void SetDefaultMultiLanePoses(const LaneDirection& initial_lane_direction,
    ...
    // Mock up an command acceleration for the ego car.
    std::unique_ptr<systems::BasicVector<double>> acceleration_command(
        new systems::BasicVector<double>(1));
    (*acceleration_command)[0] = kEgoAccelerationCommand;
    context_->FixInputPort(ego_acceleration_input_index_,
                           std::move(acceleration_command));

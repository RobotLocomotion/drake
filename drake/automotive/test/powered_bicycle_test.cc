...
class PoweredBicycleTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    kSteeringIndex = dut_->get_steering_input_port().get_index();
    kThrottleIndex = dut_->get_throttle_input_port().get_index();
    kPowertrainIndex = dut_->get_powertrain_output_port().get_index();
    kBikeStateIndex = dut_->get_state_output_port().get_index();
  }
...
TEST_F(PoweredBicycleTest, Topology) {
  ...
  const auto& powertrain_descriptor = dut_->get_output_port(kPowertrainIndex);
  EXPECT_EQ(systems::kVectorValued, powertrain_descriptor.get_data_type());
  EXPECT_EQ(1, powertrain_descriptor.size());

  const auto& state_descriptor = dut_->get_output_port(kBikeStateIndex);

...
TEST_F(SimpleCarTest, Derivatives) {
  ...
  // When velocity is near zero, the deceleration is diminished via tanh.  For
  // our specific configuration, 7% power reduction at 0.1 m/s.  (We can remove
  // this test once the tanh in the implementation goes away, to be replaced
  // with zero-crossing events.)
  const double creeping = 0.1;
  continuous_state()->set_velocity(creeping);
  SetInputValue(0.0, 0.0, 1.0);
  dut_->CalcTimeDerivatives(*context_, derivatives_.get());
  EXPECT_EQ(0.1, result->x());
  EXPECT_EQ(0.0, result->y());
  EXPECT_EQ(0.0, result->heading());
  EXPECT_NEAR(-0.9293 * max_acceleration, result->velocity(), 0.001);
  // ... but not when accelerating.

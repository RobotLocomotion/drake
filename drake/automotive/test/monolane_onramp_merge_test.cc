...
const api::Lane* GetLaneById(const api::RoadGeometry& rg,
                              const std::string& lane_id) {
  for (int i = 0; i < rg.num_junctions(); ++i) {
    if (rg.junction(i)->id().id == lane_id) {
...
GTEST_TEST(MonolaneOnrampMergeTest, TestDefaultAndNonDefaultAttributes) {
  ...
  const api::Lane* onramp1 = GetLaneById(*rg, "j:onramp1");
  ...
  const api::Lane* pre0 = GetLaneById(*rg, "j:pre0");

...
// x_axis_aligned is true, then the heading is set to be in alignment with the
// x-axis; otherwise it has opposite alignment.
static void SetPoses(double s_offset, double r_offset,
                     PoseVector<double>* ego_pose,
                     PoseBundle<double>* traffic_poses,
                     bool x_axis_aligned = true) {
  ...
  ego_pose->set_translation(Eigen::Translation3d(
      kEgoSPosition /* s */, kEgoRPosition /* r */, 0. /* h */));
  const double yaw = x_axis_aligned ? 0. : 2. * M_PI;  // x_axis_aligned
...
TEST_F(PoseSelectorDragwayTest, TwoLaneDragway) {
  MakeDragway(2 /* num lanes */, kLaneLength);
...
TEST_F(PoseSelectorDragwayTest, EgoOrientation) {
  ...
  for (double yaw = -M_PI; yaw <= M_PI; yaw += 0.1) {
    ...
    EXPECT_EQ(kLeadingSPosition, closest_ahead.odometry.pos.s());
    EXPECT_EQ(kTrailingSPosition, closest_behind.odometry.pos.s());
  }
}
...
TEST_F(PoseSelectorDragwayTest, TestGetSigmaVelocity) {
  MakeDragway(1 /* num lanes */, kLaneLength);
...
GTEST_TEST(PoseSelectorTest, MultiSegmentRoad) {
  ...
  for (const auto& road : roads) {
    ...
    for (double s_offset = 8.;
      ...
      // Situate the ego car within the 0th lane, facing along the x-axis.
      ...
      // Situate the ego car within the 0th lane, facing against the x-axis.
      SetPoses(s_offset, 0. /* r_offset */, &ego_pose, &traffic_poses,
               false /* x_axis_aligned */);

  ...
  EXPECT_EQ(kJustAheadSPosition - kEgoSPosition, closest_pose.distance);
  {
    ...
    const std::map<AheadOrBehind, const ClosestPose<double>> closest_poses =
        PoseSelector<double>::FindClosestPair(
            ...
            scan_ahead_distance, false /* skip branches */);
...
TEST_F(PoseSelectorDragwayTest, TwoLaneDragwayCheckBranches) {
  ...
  const double scan_ahead_distance = kDragwayLaneLength / 2.;
  {
    const std::map<AheadOrBehind, const ClosestPose<double>> closest_poses =
        PoseSelector<double>::FindClosestPair(
            get_lane(ego_pose, *road_), ego_pose, traffic_poses,
            scan_ahead_distance, false /* check branches */);
...
TEST_F(PoseSelectorDragwayTest, IdenticalSValues) {
...

  {
    ...
    const std::map<AheadOrBehind, const ClosestPose<double>> closest_poses =
        PoseSelector<double>::FindClosestPair(
            get_lane(ego_pose, *road_)->to_left(), ego_pose, traffic_poses,
            1000. /* scan_ahead_distance */, false /* skip branches */);
...
static void SetDefaultOnrampPoses(const Lane* ego_lane,
  ...
  // N.B. This rotation only works when roll and pitch is zero (the case here).
  isometry.rotate(RollPitchYawToQuaternion(rpy));

  traffic_poses->set_pose(0, isometry);
...
void CheckOnrampPosesInBranches(const maliput::api::RoadGeometry& road,
                                ...
                                LanePolarity ego_polarity, Cases& ego_cases) {
  ...
  ClosestPose<double> closest_pose_leading =
      PoseSelector<double>::FindSingleClosestPose(
          ...
          1000. /* scan_ahead_distance */, ego_cases[ego_polarity].first,
          true /* check branches */);
  ...
  const std::map<AheadOrBehind, const ClosestPose<double>> closest_poses =
      PoseSelector<double>::FindClosestPair(
          ego_position.lane, ego_pose, traffic_poses,
          1000. /* scan_ahead_distance */, true /* check branches */);

  // Verifies that the kAhead closest pose agrees with closest_pose_leading.
  EXPECT_EQ(closest_pose_leading.odometry.lane->id(),
            closest_poses.at(ego_cases[ego_polarity].first).odometry.lane->id());
  EXPECT_EQ(closest_pose_leading.odometry.pos.s(),
            closest_poses.at(ego_cases[ego_polarity].first).odometry.pos.s());
  EXPECT_EQ(closest_pose_leading.distance,
            closest_poses.at(ego_cases[ego_polarity].first).distance);
  // Verifies that the kBehind closest pose is infinity in the ego car's lane.
  EXPECT_EQ(
      ego_position.lane->id(),
      closest_poses.at(ego_cases[ego_polarity].second).odometry.lane->id());
  EXPECT_EQ(infty,
            closest_poses.at(ego_cases[ego_polarity].second).odometry.pos.s());
  EXPECT_EQ(infty, closest_poses.at(ego_cases[ego_polarity].second).distance);
...
GTEST_TEST(PoseSelectorOnrampTest, CheckBranches) {
  ...
  const std::vector<TestCase> test_cases{
    {"j:b0_lane0", "j:lane6", LanePolarity::kWithS, 252., "l:lane6"},
    {"j:b0_lane0", "j:lane0", LanePolarity::kWithS, infty, "l:b0_lane0"},
    {"j:b1_lane0", "j:lane2", LanePolarity::kAgainstS, 12., "l:lane2"},
    {"j:b1_lane0", "j:lane2", LanePolarity::kWithS, infty, "l:b1_lane0"},
    {"j:b2_lane0", "j:lane4", LanePolarity::kAgainstS, 12., "l:lane4"},
    {"j:b2_lane0", "j:lane4", LanePolarity::kWithS, infty, "l:b2_lane0"},
    {"j:lane0", "j:b0_lane0", LanePolarity::kAgainstS, infty, "l:lane0"},
    {"j:lane0", "j:b1_lane0", LanePolarity::kAgainstS, infty, "l:lane0"},
    {"j:lane0", "j:b1_lane1", LanePolarity::kAgainstS, 32., "l:b1_lane1"},
    {"j:lane0", "j:b2_lane0", LanePolarity::kAgainstS, 12., "l:b2_lane0"},
    {"j:lane0", "j:b2_lane1", LanePolarity::kAgainstS, 112., "l:b2_lane1"},
    {"j:lane1", "j:b2_lane1", LanePolarity::kAgainstS, 72., "l:b2_lane1"},
  };
  ...
  for (const auto& it : test_cases) {
    const Lane* traffic_lane = GetLaneByJunctionId(*road, it.traffic_lane);
    for (const auto ego_polarity :
         {LanePolarity::kWithS, LanePolarity::kAgainstS}) {
      SetDefaultOnrampPoses(GetLaneByJunctionId(*road, it.ego_lane),
      ...
      CheckOnrampPosesInBranches(*road, ego_pose, traffic_poses,
                                 ...
                                 traffic_lane->length() - 1. /* exp. s-pos */,

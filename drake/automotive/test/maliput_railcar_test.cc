...
TEST_F(MaliputRailcarTest, DecreasingSMonolane) {
  ...
  continuous_state()->set_speed(kInitialSpeed);
  dut_->CalcOutput(*context_, output_.get());
  const PoseVector<double>* pose2 = pose_output();

  Eigen::Translation<double, 3> with_s_translation = pose2->get_translation();
  Eigen::Quaternion<double> with_s_orientation = pose2->get_rotation();

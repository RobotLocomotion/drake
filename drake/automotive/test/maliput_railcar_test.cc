...
TEST_F(MaliputRailcarTest, DecreasingSMonolane) {
  ...
  // To double-check that with_s_orientation is opposite of
  // against_s_orientation, we create a quaternion called flip_yaw that is a
  // rotation about the yaw (z) axis by -180 degrees. with_s_orientation is then
  // multiplied by this rotation and the resulting quaternion is verified to
  // equal against_s_orientation.
  const Vector3<double> z_axis{0.0, 0.0, 1.0};
  const Eigen::AngleAxis<double> rotation(-M_PI, z_axis);
  const Eigen::Quaternion<double> flip_yaw(rotation);
  EXPECT_TRUE((with_s_orientation * flip_yaw).isApprox(against_s_orientation));
}

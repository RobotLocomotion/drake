...
TEST_F(MaliputRailcarTest, NonZeroStartTime) {
  ...
  // Verifies that the vehicle has non-zero derivatives at the start time.
  ...
  // Verifies that the vehicle has non-zero derivatives after the start time.
  context_->set_time(kStartTime + 1e-8);
  ASSERT_NO_FATAL_FAILURE(
      dut_->CalcTimeDerivatives(*context_, derivatives_.get()));
  EXPECT_DOUBLE_EQ(result->s(), kSpeed);

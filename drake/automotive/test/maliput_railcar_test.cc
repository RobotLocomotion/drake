...
TEST_F(MaliputRailcarTest, TestStopConditions) {
  ...
  // Verifies that the car does not stop when it is on the straight lane
  // regardless of whether it is traveling with s if it is (1) just past the
  // lane's start, (2) in the middle of the lane, or (3) just before the lane's
  // end, or (4) just after the lane's end.
  ...
  // Verifies that the car does not stop when it is on the curved lane
  // regardless of whether it is traveling with s if it is (1) just before the
  // lane's start, (2) just after the lane's start, (3) in the middle of the
  // lane, or (4) just before the lane's end.
  for (const auto s : std::list<double>{-1e-10,
                                        1e-10,

...
GTEST_TEST(PriusVisTest, BasicTest) {
  ...
  // more details.
  const double kVisOffset{1.40948};
  ...
  EXPECT_DOUBLE_EQ(
      ...
      kVisOffset * cos(M_PI_4));
  EXPECT_DOUBLE_EQ(
      floor_pose_down_hill.translation().z(),
      -kVisOffset * cos(M_PI_4) + floor_pose_identity.translation().z());

  // Tests the visualization's pose when the model is rotated 45 degrees about
  // its +X axis. In other words, the vehicle is leaning to its left side due to
  // the lane being severely cambered.
  ...
  EXPECT_DOUBLE_EQ(floor_pose_severe_camber.translation().x(), kVisOffset);
}

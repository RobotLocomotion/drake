...
GTEST_TEST(PriusVisTest, BasicTest) {
  ...
  systems::rendering::PoseBundle<double> origin_vis_poses =
      dut.CalcPoses(X_WM_W_origin);
  ...
  for (const auto& translation : translation_offsets) {
    ...
    systems::rendering::PoseBundle<double> offset_vis_poses =
        dut.CalcPoses(X_WM_W_offset);

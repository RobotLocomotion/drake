...
GTEST_TEST(TrajectoryOptimizationTest, SimpleCarDircolTest) {
  ...
  const SimpleCarParams<double>* params = dynamic_cast<const SimpleCarParams<double>*>(context->get_numeric_parameter(0));
  DRAKE_DEMAND(params != nullptr);
  const double kConstraintLimitScaling{0.8};  // Impose limits that are inside the true command limits.

  // TODO(russt): Provide a more elegant and efficient way to write the bounding box constraints below
  // (this generates MANY small bounding box constraints for the MathematicalProgram).
  ...
  prog.AddConstraintToAllKnotPoints(input.steering_angle() <= kConstraintLimitScaling*params->max_abs_steering_angle());
  prog.AddConstraintToAllKnotPoints(-kConstraintLimitScaling*params->max_abs_steering_angle() <= input.steering_angle());
  prog.AddConstraintToAllKnotPoints(input.acceleration() <= kConstraintLimitScaling*params->max_acceleration());
  prog.AddConstraintToAllKnotPoints(-kConstraintLimitScaling*params->max_acceleration() <= input.acceleration());
  ...
  prog.AddConstraintToAllKnotPoints(state.velocity() <= kConstraintLimitScaling*params->max_velocity());

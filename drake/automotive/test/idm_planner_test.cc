...
class IdmPlannerTest : public ::testing::Test {
 protected:
  ...
  double get_free_dadv() { return -delta() * a() / v_ref(); }
  ...
  // by IDM.
  double get_dadv() { return -2 * a() * s_0() * time_headway(); }

  // Returns ∂/∂d(result) for the special case where v = d_dot = 0 and d = 1,
  // where `v` is ego_velocity, `d` is target_distance, `d_dot` is
  // target_distance_dot, and `result` is the longitudinal acceleration returned
  // by IDM.
  double get_dadd() { return 2 * a() * s_0(); }
  ...
  void SetAllDerivatives(T* ego_velocity, T* target_distance,
    ...
    SetDerivatives(ego_velocity, Eigen::Vector3d{1., 0., 0.});
    SetDerivatives(target_distance, Eigen::Vector3d{0., 1., 0.});
    SetDerivatives(target_distance_dot, Eigen::Vector3d{0., 0., 1.});
  }

  double delta() { return ExtractDoubleOrThrow(this->params_.delta()); }
  double a() { return ExtractDoubleOrThrow(this->params_.a()); }
  double v_ref() { return ExtractDoubleOrThrow(this->params_.v_ref()); }
  double s_0() { return ExtractDoubleOrThrow(this->params_.s_0()); }
  double time_headway() {

...
GTEST_TEST(IdmControllerTest, ConstructOpposingSegments) {
  ...
  const double straight_length{path.get_path().getStartTime(
      std::ceil(kStraightRoadLength / kStepSize))};
...
GTEST_TEST(IdmControllerTest, ConstructConfluentSegments) {
  ...
  const double straight_length{path.get_path().getEndTime(
      std::ceil(kStraightRoadLength / kStepSize))};
  const double curved_length{total_length - straight_length};
  expected_value << 10., 0., 0.;

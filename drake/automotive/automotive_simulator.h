...
class AutomotiveSimulator {
 public:
  ...
  /// @pre Start() has NOT been called.
  ///
  /// @param model_name The car's name, which must be unique among all
  /// cars. Otherwise a std::runtime_error will be thrown.
  ///
  ...
  int AddMobilControlledSimpleCar(
      const std::string& model_name, bool initial_with_s,
      const SimpleCarState<T>& initial_state = SimpleCarState<T>());

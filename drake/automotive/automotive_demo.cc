...
void AddVehicles(RoadNetworkType road_network_type,
  ...
  // This must match the distance between the visualization's rear axle and the
  // visualization's origin.
  static const double kP_MoVo{1.40948};
  static const VehicleVisualization visualization(
  ...
  if (FLAGS_simple_car_names.empty()) {
    ...
    simulator->AddSimpleCarFromSdf(visualization, name, channel_name);
  } else {
  ...
  if (road_network_type == RoadNetworkType::dragway) {
    ...
    for (int i = 0; i < FLAGS_num_trajectory_car; ++i) {
      ...
      simulator->AddTrajectoryCarFromSdf(visualization,
                                         std::get<0>(params),
                                         std::get<1>(params),
                                         std::get<2>(params));
    }

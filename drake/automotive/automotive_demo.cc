...
DEFINE_double(dragway_base_speed, 4.0,
              ...
              "dragway.");
DEFINE_double(dragway_lane_speed_delta, 0,
              ...
              "dragway_base_speed + 2 * dragway_lane_speed_delta m/s. This "
              "parameter is by default zero meaning vehicles on all lanes will "
              "travel at the same speed.");
DEFINE_double(dragway_vehicle_temporal_spacing, 3,
              "The temporal spacing between consecutive vehicles on a lane.");
  ...
  if (road_network_type == RoadNetworkType::dragway) {
    ...
    for (int i = 0; i < FLAGS_num_trajectory_car; ++i) {
      ...
      const double start_time = i / FLAGS_num_dragway_lanes *
           FLAGS_dragway_vehicle_temporal_spacing;

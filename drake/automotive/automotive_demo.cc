...
// Road network parameter.
DEFINE_int32(road_network, 0,
    "Specifies which road network to include. Valid options include:\n"
    "  - 0: flat terrain\n"
    "  - 1: maliput monolane\n"
    "  - 2: maliput dragway");
...
// `SimpleCar` vehicles and `TrajectoryCar` vehicles. If command line parameter
// `road_network` equals `kDragway`, the provided `road_geometry` must not be
// `nullptr`.
void AddVehicles(AutomotiveSimulator<double>* simulator,
    const maliput::api::RoadGeometry* road_geometry) {
  ...
  if (FLAGS_road_network == kFlatTerrain) {
    ...
    for (int i = 0; i < FLAGS_num_trajectory_car; ++i) {
      ...
      drake::log()->error("AddVehicles(): Unsupported road network type: {}.",
          std::to_string(FLAGS_road_network));
      DRAKE_ABORT();
  }
}
...
const maliput::api::RoadGeometry*
    AddDragway(AutomotiveSimulator<double>* simulator) {
  std::unique_ptr<const maliput::api::RoadGeometry> road_geometry_temp
      = std::make_unique<const maliput::dragway::RoadGeometry>(
          maliput::api::RoadGeometryId({"MyDragway"}),
          FLAGS_num_lanes,
          FLAGS_length,
          FLAGS_lane_width,
          FLAGS_shoulder_width);
  return simulator->SetRoadGeometry(&road_geometry_temp);
}
...
const maliput::api::RoadGeometry*
  ...
  if (FLAGS_road_file.empty()) {
    drake::log()->error("AddMonolane(): No monolane road file specified.");
    DRAKE_ABORT();
  }
...
const maliput::api::RoadGeometry* AddTerrain(
  ...
  switch (FLAGS_road_network) {
    ...
    break;
    default:
      drake::log()->error("AddTerrain(): Unsupported road network type: {}.",
          std::to_string(FLAGS_road_network));
      DRAKE_ABORT();
  }
...
int main(int argc, char* argv[]) {
  ...
  const maliput::api::RoadGeometry* road_geometry = AddTerrain(simulator.get());
  AddVehicles(simulator.get(), road_geometry);

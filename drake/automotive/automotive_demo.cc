...
DEFINE_string(road_path, "",
              "A string defining a circuit through the monolane road geometry, "
...
// be `nullptr`.
void AddVehicles(int road_network_type, AutomotiveSimulator<double>* simulator,
    const maliput::api::RoadGeometry* road_geometry) {
  ...
  if (road_network_type == kFlatTerrain) {
  ...
  } else if (road_network_type == kMonolane) {
    ...
    AddMonolaneVehicles(ego_car_names, road_geometry, sdf_file, simulator);
      ...
      drake::log()->error("AddVehicles(): Unsupported road network type: {}.",
          std::to_string(road_network_type));
      DRAKE_ABORT();
  }
}
...
int main(int argc, char* argv[]) {
  ...
  const int road_network_type = DetermineRoadNetworkType();

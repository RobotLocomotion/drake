...
// The distance between consecutive rows of railcars on a dragway.
...
// Adds a MaliputRailcar to the simulation involving a dragway.
//
...
void AddMaliputRailcar(int num_cars, bool idm_controlled, int initial_s_offset,
  ...
  for (int i = 0; i < num_cars; ++i) {
    ...
    // empirically chosen.
    state.set_s(initial_s_offset + kRailcarRowSpacing * row);

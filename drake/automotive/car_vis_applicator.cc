...
void CarVisApplicator<T>::DoCalcOutput(
  ...
  for (int i = 0; i < vehicle_poses.get_num_poses(); ++i) {
    ...
    for (int j = 0; j < model_vis_poses.get_num_poses(); ++j) {
      ...
      // TODO(liang.fok) The following two statements are actually constant with
      // respect to a single CarVis instance and thus should only need to be
      // done once at the start of the simulation. Either place them into a
      // Finalize() method that must be called after all of the CarVis objects
      // are added, but before the simulation starts, or wait until the system
      // framework supports an "initialization" phase.

...
void AutomotiveSimulator<T>::AddSimpleCar() {
  ...
  auto simple_car = builder_->template AddSystem<SimpleCar<double>>();
  auto coord_transform =
      builder_->template AddSystem<SimpleCarToEulerFloatingJoint<double>>();
...
template <typename T>
void AutomotiveSimulator<T>::AddPublisher(const SimpleCar<double>& system,
                                          int vehicle_number) {
  DRAKE_DEMAND(!started_);
...
void AutomotiveSimulator<T>::AddPublisher(
    const SimpleCarToEulerFloatingJoint<double>& system,
    int vehicle_number) {
  DRAKE_DEMAND(!started_);
...
systems::System<T>& AutomotiveSimulator<T>::GetBuilderSystemByName(
  ...
  for (systems::System<T>* system : builder_->GetSystems()) {

...
GTEST_TEST(MonolaneLanesTest, Rot3) {
  ...
  EXPECT_TRUE(
      CompareMatrices(rpy90.apply({1., 0., 0.}), V3(0., 0., -1.), kVeryExact));
  EXPECT_TRUE(
      CompareMatrices(rpy90.apply({0., 1., 0.}), V3(0., 1., 0.), kVeryExact));
  EXPECT_TRUE(
      CompareMatrices(rpy90.apply({0., 0., 1.}), V3(1., 0., 0.), kVeryExact));
}
...
GTEST_TEST(MonolaneLanesTest, FlatLineLane) {
  ...
  Lane* l1 =
      s1->NewLineLane({"l1"}, {100., -75.}, {100., 50.},
  ...
  EXPECT_TRUE(api::CompareRBounds(l1->lane_bounds(0.), api::RBounds(-5., 5.),
                                  kVeryExact));
  EXPECT_TRUE(api::CompareRBounds(l1->driveable_bounds(0.),
                                  api::RBounds(-10., 10.), kVeryExact));
  EXPECT_TRUE(api::CompareHBounds(l1->elevation_bounds(0., 0.),
                                  api::HBounds(0., 5.), kVeryExact));

  EXPECT_TRUE(api::CompareGeoPositions(l1->ToGeoPosition({0., 0., 0.}),
  ...
  Lane* l1_with_z =
      s2->NewLineLane({"l1_with_z"}, {100., -75.}, {100., 50.}, {-5., 5.},
...
GTEST_TEST(MonolaneLanesTest, FlatArcLane) {
  ...
  Segment* s1 = rg.NewJunction({"j1"})->NewSegment({"s1"});
  Lane* l2 =
      s1->NewArcLane({"l2"}, center, radius, theta0, d_theta,
                     // lane/driveable/elevation bounds
                     {-5., 5.}, {-kHalfWidth, kHalfWidth}, {0., kMaxHeight},
                     // Zero elevation, zero superelevation == flat.
                     zp, zp);
  ...
  EXPECT_TRUE(api::CompareRBounds(l2->lane_bounds(0.), api::RBounds(-5., 5.),
                                  kVeryExact));
  EXPECT_TRUE(api::CompareRBounds(l2->driveable_bounds(0.),
                                  api::RBounds(-10., 10.),
                                  kVeryExact));  // kHalfWidth
  EXPECT_TRUE(api::CompareHBounds(l2->elevation_bounds(0., 0.),
                                  api::HBounds(0., 5.), kVeryExact));
  ...
  Lane* l2_with_z = s2->NewArcLane(
      {"l2_with_z"}, center, radius, theta0, d_theta, {-5., 5.},
      {-kHalfWidth, kHalfWidth}, {0., kMaxHeight},
  ...
  Lane* l2_overlapping = s3->NewArcLane(
      {"l2_overlapping"}, center, radius, theta0, d_theta_overlap, {-5., 5.},
      {-kHalfWidth, kHalfWidth}, {0., kMaxHeight},
      // Zero elevation, zero superelevation == flat.
      zp, zp);
  EXPECT_TRUE(api::CompareLanePositions(
  ...
  Lane* l2_wrap =
      s4->NewArcLane({"l2_wrap"}, center, radius, theta0_wrap, d_theta_wrap,
                     {-5., 5.}, {-kHalfWidth, kHalfWidth}, {0., kMaxHeight},
                     // Zero elevation, zero superelevation == flat.
                     zp, zp);
  ...
  EXPECT_TRUE(api::CompareRotations(l2->GetOrientation({l2->length(), 0., 0.}),
...
GTEST_TEST(MonolaneLanesTest, ArcLaneWithConstantSuperelevation) {
  ...
  RoadGeometry rg({"apple"}, kLinearTolerance, kAngularTolerance);
  Segment* s1 = rg.NewJunction({"j1"})->NewSegment({"s1"});
  Lane* l2 = s1->NewArcLane({"l2"}, {100., -75.}, 100., 0.25 * M_PI, 1.5 * M_PI,
                            {-5., 5.}, {-10., 10.},
                            {0., 5.},  // lane/driveable/elevation bounds
                            zp, {(kTheta) / (100. * 1.5 * M_PI), 0., 0., 0.});
  ...
  EXPECT_TRUE(
      api::CompareRotations(l2->GetOrientation({l2->length(), 0., 0.}),
...
GTEST_TEST(MonolaneLanesTest, HillIntegration) {
  ...
  const CubicPolynomial kHillPolynomial(z0 / p_scale, 0.,
                                        ...
                                        (-2. * (z1 - z0) / p_scale));
  Lane* l1 =
      s1->NewArcLane({"l2"}, {-100., -100.}, 100., theta0, d_theta, {-5., 5.},
                     {-10., 10.}, {0., 5.},  // lane/driveable/elevation bounds
                     kHillPolynomial, zp);
  ...
  api::LanePosition lp_final_a =
      IntegrateTrivially(l1, kLpInitialA, kVelocity, kTimeStep, kStepsForZeroR);
  EXPECT_TRUE(api::CompareGeoPositions(
  ...
  api::LanePosition lp_final_b =
      IntegrateTrivially(l1, kLpInitialB, kVelocity, kTimeStep, kStepsForR10);
  EXPECT_TRUE(api::CompareGeoPositions(

...
api::RoadPosition RoadGeometry::DoToRoadPosition(
  ...
  if (hint != nullptr) {
    ...
    if (min_distance != 0.) {
      ...
      for (int iter = api::LaneEnd::kStart; iter <= api::LaneEnd::kFinish;
           ++iter) {
        const api::LaneEndSet* ends = hint->lane->GetOngoingBranches(
            static_cast<api::LaneEnd::Which>(iter));
        if (ends == nullptr) continue;

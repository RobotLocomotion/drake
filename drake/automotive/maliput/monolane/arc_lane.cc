...
//
// This function deals with the following two cases separately:
//
// 1) If θ_min, θ_max are such that there exists n ∈ ℤ such that θ_min + 2πn,
// θ_max + 2πn ∈ [-π, +π], i.e. the ordering is preserved and the wrapped
// interval does not cross ±π, then the saturated result is done in the standard
// way.
//
// 2) On the other hand, if θ_min, θ_max are such that, for some n ∈ ℤ such that
// θ_min + 2πn, θ_max + 2π(n ± 1) ∈ [-π, +π], i.e the wrapped interval straddles
// ±π, then the function does not saturate if θ is either within [θ_max + 2πn,
// +π] or [-π, θ_max + 2πn].  Otherwise, it saturates to the closest bounding
// value in the cyclic coordinate system.

...
// Implements the saturate function for bounding angles that are wrapped to the
// interval range [-π, π].  Specifically, given an angle θ ∈ [-π, π], and lower
// and upper bounds θ_min, θ_max, this function does two things.  First, it
// wraps θ_min and θ_max to the range [-π, π] (preserving their variable names).
// Then, it saturates θ with respect to this wrapped range.  If θ_max > 2π +
// θ_min, then no saturation occurs.
//
// N.B. θ_min, θ_max can take on any value, provided that θ_min <= θ_max.
double saturate_on_wrapped_bounds(double theta, double theta_min,
  ...
  // First, deal with the case where [θ_min, θ_max] does not cross the ±π
  // wrap-point (i.e. θ₀ ≤ θ₁).
  if (theta_0 <= theta && theta <= theta_1) return theta;
  // Next, deal with the case where [θ_min, θ_max] straddles ±π (i.e. θ₁ < θ₀).

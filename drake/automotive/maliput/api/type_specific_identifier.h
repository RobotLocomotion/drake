...
/// TypeSpecificIdentifier's constructed from equal strings are considered to
/// be equal.  There is currently no other semantic value attributed to the
/// contents of the string.
///
/// Construction from empty strings are not allowed; there is no notion of
...
struct hash<drake::maliput::api::TypeSpecificIdentifier<T>> {
  ...
  result_type operator()(const argument_type& id) const {
    /// TODO(maddog@tri.global)  Per the GSG, our current style guide strictly
    ///                          prohibits creating new specializations of
    ///                          std::hash on the grounds that it is in general
    ///                          difficult to do that correctly.  However,
    ///                          since this implementation is merely a wrapper
    ///                          around std::string with stricter type checking
    ///                          and since it merely invokes a C++ standard
    ///                          hash approved by the style guide, it has been
    ///                          granted an exception.  If this implementation
    ///                          changes, the exception must be reevaluated.
    ///                          Conversely, if the (arguably maladaptive)
    ///                          prohibition is removed from our style guide,

...
class Lane {
 ...
 private:
  ...
  // derivatives as those appearing in geo_position.  If either @p nearest_point
  // or @p distance are non-null, they will also contain geo_position's partial
  // derivatives.
  //
  // Note: As`distance` approaches zero, its partial derivative with respect to
  // an element of `geo_pos` will be nearly-singular, when distance = 0 at the
  // boundary of the lane, it is undefined (NaN) (see dragway/lane.cc for
  // technical details).  Since distance = 0 independent of the position within
  // the lane, the distance.derivatives() are zero within the lane's interior.
  //

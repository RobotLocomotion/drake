...
//     const {
//   const Lane* lane = dynamic_cast<const
//   Lane*>(junction_.segment(0)->lane(0));
...
//       (geo_pos.y > 0 && geo_pos.y < length &&
//       geo_pos.x < max_r && geo_pos.x > min_r)) {
//     return true;
//   } else {
//     drake::log()->trace(
//         "crossroad::RoadGeometry::IsGeoPositionOnCrossroad(): The provided
//         geo_pos "
...
// TODO(shensquared) re-write the method possibly to account for
// // multiple segments design
...
//     for (int i = 0; !lane_found && i < junction_.segment(j)->num_lanes();
//     ++i) {
//       const Lane* lane = dynamic_cast<const
//       Lane*>(junction_.segment(j)->lane(i));
//       DRAKE_ASSERT(lane != nullptr);
...
// }
//   if (!lane_found) {
//     throw std::runtime_error("crossroad::RoadGeometry::GetLaneIndex: Failed
//     to "
...
api::RoadPosition RoadGeometry::DoToRoadPosition(
  ...
  // closest_position.x = clamp(geo_pos.x, min_s, max_s);
  // closest_position.y = clamp(geo_pos.y, min_r, max_r);
  ...
  //     dynamic_cast<const
  //     Lane*>(junction_.segment(0)->lane(closest_lane_index));

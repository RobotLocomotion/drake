...
static AutoDiffXd PadScalarWithZeroedPartials(const AutoDiffXd& x,
  ...
  Eigen::internal::make_coherent(x.derivatives(), model_scalar.derivatives());
  return x;
}
...
api::LanePositionT<AutoDiffXd> Lane::DoToLanePosition(
...
api::LanePositionT<T> Lane::ImplDoToLanePositionT(
  ...
  if (distance != nullptr) {
    ...
    if (distance_unsat > T(0.)) {  // ∂/∂x(distance) = 0 when distance = 0.
      *distance = distance_unsat;
    } else {
      *distance = PadScalarWithZeroedPartials(T(0.), distance_unsat);
    }
  }

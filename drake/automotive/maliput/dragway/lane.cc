...
api::LanePositionT<T> Lane::ImplDoToLanePositionT(
  ...
  if (distance != nullptr) {
    ...
    // respect to position is undefined (i.e. NaN).  This follows since distance
    // = norm(x) and hence ∂/∂x(distance) = ∂/∂x(norm(x)) = x / norm(x) = x /
    // distance is only defined for distance > 0.

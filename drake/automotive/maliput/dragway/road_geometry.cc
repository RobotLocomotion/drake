...
api::RoadPosition RoadGeometry::DoToRoadPosition(
  ...
  if (IsGeoPositionOnDragway(geo_pos)) {
    ...
    // Ensures that the returned result is at `h = 0`. This is true since the
    // entire dragway is at `h = 0`.
    DRAKE_ASSERT(result_lane_position.h == 0);

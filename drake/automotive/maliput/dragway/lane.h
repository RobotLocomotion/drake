...
/**
 ...
 The following lane is implemented:
  ...
  the latitudinal traversal at a particular `s`. Coordinate `h` specifies the
  height of the lane at a particular `s` and `r`. Since Dragway lanes are flat
  and level, `h = 0` for all values of `s` and `r`. The origin of the lane's
  frame is defined by the `o` along the above-shown `s = 0` line.
**/
class Lane : public api::Lane {
 public:
  ...
  /// lane's `s = 0` and `r = 0`. This value can be negative if the lane's
  /// `s = 0` and `r = 0` is to the left of the world frame's origin.
  ///

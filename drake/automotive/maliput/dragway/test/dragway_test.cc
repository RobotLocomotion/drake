...
// Tests dragway::RoadGeometry::ToRoadPosition() using a two-lane dragway.
TEST_F(MaliputDragwayLaneTest, TestToRoadPositionOnRoad) {
  ...
  // Spot checks geographic positions on lane 0 and the right shoulder.
  for (double x = 0; x <= length_; x += 0.5) {
    for (double y = -lane_width_ - shoulder_width_; y <= 0; y += 0.1) {
      for (double z = 0; z <= 10; z += 2.5) {
        ...
        EXPECT_DOUBLE_EQ(nearest_position.z, 0);
        ...
        EXPECT_EQ(road_position.pos.h, 0);
      }
    }
  }

  // Spot checks geographic positions on lane 1 and the left shoulder.
  for (double x = 0; x <= length_; x += 0.5) {
    for (double y = 0; y <= lane_width_ + shoulder_width_; y += 0.1) {
      for (double z = 0; z <= 10; z += 2.5) {
        api::GeoPosition nearest_position;
        double distance;
        const api::RoadPosition road_position = road_geometry.ToRoadPosition(

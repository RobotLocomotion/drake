...
// Tests dragway::Lane::ToLanePosition() using geographic positions that whose
...
 
  ...
  */
  ...
  // the driveable area are evaluated.
  const std::vector<double> x_test_cases{
    min_x - 10, min_x - 1e-10, min_x, max_x, max_x + 1e10, max_x + 10};
  const std::vector<double> y_test_cases{
    min_y - 10, min_y - 1e-10, min_y, max_y, max_y + 1e10, max_y + 10};

  // Spot checks geographic positions on the lane's driveable region with a
  // focus on edge cases.
  for (const auto x : x_test_cases) {
    for (const auto y : y_test_cases) {

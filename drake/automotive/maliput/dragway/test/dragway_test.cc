...
class MaliputDragwayLaneTest : public ::testing::Test {
 public:
  ...
  void VerifyLaneCorrectness(const Lane* lane, int num_lanes) {
    const int lane_index = lane->index();
    const double expected_y_offset =
        (kLaneWidth_ * num_lanes / 2) - kLaneWidth_ / 2 -
            kLaneWidth_ * lane_index;
    EXPECT_EQ(lane->y_offset(), expected_y_offset);
    ...
    const double driveable_r_min =
        (lane_index == 0 ? -kLaneWidth_ / 2 - kShoulderWidth_ :
            -kLaneWidth_ / 2);
    const double driveable_r_max =
        (lane_index == num_lanes - 1 ? kLaneWidth_ / 2 + kShoulderWidth_ :
            kLaneWidth_ / 2);
    ...
    for (double s = 0; s < kLength_; s += kLength_ / 100) {
      for (double r = driveable_r_min; r < driveable_r_max;
        ...
        for (double h = 0; h < 1; h += 0.1) {
          ...
          EXPECT_DOUBLE_EQ(geo_position.y, expected_y_offset + r);
    ...
    for (double s = 0; s < kLength_; s += kLength_ / 100) {
      for (double r = driveable_r_min; r < driveable_r_max;
        ...
        for (double h = 0; h < 1; h += 0.2) {
          ...
          // The following rotational velocities can be any value. We just want
          // to verify that the same values are returned from
...
TEST_F(MaliputDragwayLaneTest, SingleLane) {
  ...
  const Lane* lane = dynamic_cast<const Lane*>(api_lane);

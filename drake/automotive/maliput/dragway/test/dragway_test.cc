...
class MaliputDragwayLaneTest : public ::testing::Test {
 public:
  ...
  void VerifyLaneCorrectness(const api::Lane* lane, int num_lanes) {
    ...
    // The following blocks of test code each evaluate a particular method.
    // Ideally, we want to check that the method-under-test is correct for all
    // combinations of (s, r, h). This, unfortunately, it not possible since
    // there are too many combinations . Instead we pick points in a grid that
    // spans the (s, r, h) state space and only check those points in the hopes
    // that they are representative of the entire state space.

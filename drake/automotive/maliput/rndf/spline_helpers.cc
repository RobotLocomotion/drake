...
// The InverseFunctionInterpolator holds a graph structure where each node is a
...
// two numbers are related with the error_boundary set to
// InverseFunctionInterpolator's constructor in the sense that we can get
// smaller error bounds given high values of kFunctionPartitionTreeDegree and /
// or kFunctionPartitionTreeDegree.
...
InverseFunctionInterpolator::InverseFunctionInterpolator(
    ...
    : function_(function), error_boundary_(error_boundary),
...
double InverseFunctionInterpolator::InterpolateMthDerivative(int mth,
    double y) {
  // Make sure that the derivative order is non-negative.
  DRAKE_THROW_UNLESS(mth >= 0);
  ...
  const double iy = std::min(std::max(y,
                                      root->segment.ymin),
                                      root->segment.ymax);
  ...
  while (true) {
    if (root->partitions.empty()) {
      ...
      DRAKE_THROW_UNLESS(depth <= partition_tree_max_depth_);

...
/// A partition of a single-variable function graph, as a segment plus a set of
/// sub partitions.
...
/// Helpful for arc-length parameterization with ignition::math::Splines.
class InverseFunctionInterpolator {
 public:
  ...
  /// @param[in] _error_boundary a positive constrain on the maximum error
  ...
  /// satisfied.
  double InterpolateMthDerivative(const int _mth, const double _y) const;
...
class ArcLengthParameterizedSpline {
 public:
  ...
  /// @throws whenever arguments constrains are not satisfied.
  ignition::math::Vector3d InterpolateMthDerivative(const int _mth,
  ...
  /// Function to find the closest point on the spline to a point in space.
  ///
  /// It will iterate over the range of s of the spline which is 0 to 1 in terms
  /// of ignition::math::Spline's parameter at constant arc length @p step
  /// increments. On each iteration we get the distance and save the the arc
  /// length coordinate that gives the minimum distance.
  ///
  /// @param point is the point in space from which we want to get the s arc
  /// length value that has an image with minimum distance to it.
  /// @param step is the arc length step to iterate over the complete spline arc
  /// length.
  /// @return the arc length value that applied to the internal
  /// ignition::math::Spline has a minimum distance to @p point.
 ...
 private:
  ...
  std::unique_ptr<InverseFunctionInterpolator>
      t_s_;  ///< Inverse arc length function t(s).
};

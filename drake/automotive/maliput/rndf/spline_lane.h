...
/// superelevation so all the Lanes will depict a flat path just over the
/// ground.
class SplineLane : public Lane {
 public:
  ...
  /// where the interpolated curve will pass. They must be at least two. The
  /// tuple will be interpreted as with the first value as the knot and the
  /// second as the tangent.
  ///
 ...
 private:
  ...
  // It will provide the angle of the tangent vector evaluated at an arc length
  ...
  // components of ignition::math::Spline are independent from each other and a

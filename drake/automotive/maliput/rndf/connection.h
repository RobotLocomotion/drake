...
/// DirectedWaypoints objects (based on RNDF waypoints that belong to the same
/// RNDF lane).
///
/// A Connection object derives one or multiple Lanes objects. Consequently the
...
/// sense of flow. To define the direction, one of the Connections in the group
/// will be flagged as the reference, and an interpolated spline is computed
/// from the the DirectedWaypoints within the reference Connection to obtain the
/// tangent information. An arbitrary point outside the bounding box of the road
/// map is selected to be center of rotation and the sum of all the
/// DirectedWaypoint momentums (with normalized tangents) is computed. It must
/// be said that momentum equation is used to derive lane direction, but it has
/// no physical meaning. Since all the waypoints lay in the z=0 plane,
/// the momentum vector will only contain a non-zero z coordinate value.
/// The sign of the momentum's z coordinate will be taken as a reference
...
/// index based algorithm, a pair of consecutive waypoints in a Connection that
/// are valid are used to create a SplineLane. At the same time, we should

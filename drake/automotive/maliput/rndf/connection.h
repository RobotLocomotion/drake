...
///
/// So, mapping those waypoints to SplineLanes and Segments will be done by the
/// Builder. In the end, we end up with something like:
///
/// |        |1.1.1                  |1.1.2     |
...
/// At the building stage, we can now match on an index based algorithm, a pair
/// of consecutive waypoints in a Connection that are valid or not and create a
/// SplineLane from them. At the same time, we should create those Lanes inside
/// the same segment, which is not difficult to do it from the index approach.
/// To sum up, Builder will create the following:
///
/// - Segment 1_0:
///   - Lane 1_2_1-1_2_3
/// - Segment 1_1:

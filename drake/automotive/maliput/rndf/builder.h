...
/// flow. To define the flow direction, one of the Connections in the group will
/// be flagged as the reference, and an interpolated spline is computed from the
/// the DirectedWaypoints to obtain the tangent information. An arbitrary point,
/// outside the bounding box of all the road map, is selected to be the center
/// of rotation and the sum of all the DirectedWaypoint momentums (with
/// normalized tangents) is computed. As all the waypoints lay in the z=0 plane,
/// the momentum vector will only contain a z coordinate value different from
/// zero. The sign of the momentum's z coordinate will be taken as a reference
/// to define the sense of flow. For all other connections, the momemtum is
/// computed and compared against the sign of the reference. This way, grouping
/// is achieved.
class Connection {
 public:

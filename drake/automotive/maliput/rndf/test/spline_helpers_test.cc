...
// We use this tolerance to match the interpolated positions given by the arc
// length interpolator wrappers.
const double kLinearTolerance = 1e-4;
// We use this step to sample the spline at this rate, to get the closest arc
...
// This is a wrapper to get easily create an ignition::math::Spline .
...
GTEST_TEST(RNDFSplineHelperTest, StraightLine) {
  ...
  for (double l = 0.0; l < length; l += 1.0) {
    ...
    EXPECT_IGN_VECTOR_NEAR(
        arc_lenght_param_spline->InterpolateMthDerivative(0, l), p,
        kLinearTolerance);
  }
  EXPECT_IGN_VECTOR_NEAR(
      arc_lenght_param_spline->InterpolateMthDerivative(0, length),
      ignition::math::Vector3d(20.0, 0.0, 0.0), kLinearTolerance);
}

// Tests a set of points and the result of the arc length distance that returns
// an Spline interpolated point closest to it.

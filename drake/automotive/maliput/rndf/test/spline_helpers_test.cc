...
GTEST_TEST(RNDFSplineToBezierTest, ConversionTest) {
  ...
  // First case where the points and tangents shape a 90° figure. Tangents are
...
GTEST_TEST(RNDFBezierToBezierTest, ConversionTest) {
  ...
  // First case where the points control points shape a 90° figure. Control
  // points are set to get critical tangents in Hermite base.
  p0.Set(0.0, 10.0, 0.0);
  p1.Set(0.0, 0.0, 0.0);
  p2.Set(0.0, 0.0, 0.0);
  p3.Set(10.0, 0.0, 0.0);
...
GTEST_TEST(RNDFAdjustBezierGeometryTest, ExceptionCases) {
  ...
  EXPECT_THROW(AdjustBezierGeometry({}, 1.0), std::runtime_error);
  EXPECT_THROW(AdjustBezierGeometry(input_bezier_points, -1.0),
               std::runtime_error);
  EXPECT_THROW(AdjustBezierGeometry(input_bezier_points, 2.0),
               std::runtime_error);
}
...
//                   |
//                   v
GTEST_TEST(RNDFAdjustBezierGeometryTest, Case90DegreeConnection) {
  const ignition::math::Vector3d p0(0.0, 10.0, 0.0);
  const ignition::math::Vector3d p1(0.0, 5.0, 0.0);
  const ignition::math::Vector3d p2(5.0, 0.0, 0.0);
  const ignition::math::Vector3d p3(10.0, 0.0, 0.0);
  ...
  const std::vector<ignition::math::Vector3d> output_bezier_points =
      AdjustBezierGeometry(input_bezier_points, 1.0);
...
//      XXXXXX
// XXXXXX-------------->
GTEST_TEST(RNDFAdjustBezierGeometryTest, CaseParallelNonColinearConnection) {
...
// Desired geometry from points and tangents in Hermite base.
// +--------------->XXXXXXXX XXXXX+------------->
GTEST_TEST(RNDFAdjustBezierGeometryTest, CaseParallelColinearConnection) {
  const ignition::math::Vector3d p0(0.0, 0.0, 0.0);
  const ignition::math::Vector3d p1(20.0, 0.0, 0.0);
...
//                 \X
//                  >
GTEST_TEST(RNDFAdjustBezierGeometryTest, CaseObliqueConnection) {
  const ignition::math::Vector3d p0(0.0, 10.0, 0.0);
...
// XX
// XXXXX+------>
GTEST_TEST(RNDFAdjustBezierGeometryTest, CaseObliqueNonConvexConnection) {
  const ignition::math::Vector3d p0(0.0, 10.0, 0.0);
  ...
  const std::vector<ignition::math::Vector3d> input_bezier_points = {p0, p1, p2,
                                                                     p3};
  const std::vector<ignition::math::Vector3d> output_bezier_points =
      AdjustBezierGeometry(input_bezier_points, 0.5);
  EXPECT_TRUE(test::IsIgnitionVector3dClose(output_bezier_points[0],
                                            p0,
                                            kLinearTolerance));
  EXPECT_TRUE(test::IsIgnitionVector3dClose(output_bezier_points[1],
...
GTEST_TEST(RNDFCreatePChipBasedSplineTest, CaseExceptions) {
  ...
  // Checks that it will abort if two consecutive points are the same.
  points.push_back(ignition::math::Vector3d(20.0, 0.0, 0.0));
  EXPECT_DEATH(CreatePChipBasedSpline(points), "(.*?)");
}

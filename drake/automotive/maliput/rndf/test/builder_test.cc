...
GTEST_TEST(RNDFBuilderTest, ConnectionTest) {
  ...
  EXPECT_EQ(connection.start().id().X(), 1);
  EXPECT_EQ(connection.start().id().Y(), 1);
  EXPECT_EQ(connection.start().id().Z(), 1);
  EXPECT_EQ(connection.end().id().X(), 1);
  EXPECT_EQ(connection.end().id().Y(), 1);
  EXPECT_EQ(connection.end().id().Z(), 2);
  ...
  EXPECT_NE(connection.inverse_direction(), kInverseDirection);

...
// tuple's first value is the knot point while its second value is the tangent
// at the knot point.
...
GTEST_TEST(RNDFSplineLanesTest, FlatLineLane) {
  ...
  // Check bounds.
  EXPECT_NEAR(l1->lane_bounds(0.).r_min, -kWidth / 2., kVeryExact);
  EXPECT_NEAR(l1->lane_bounds(0.).r_max, kWidth / 2., kVeryExact);
  EXPECT_NEAR(l1->driveable_bounds(0.).r_min, -kWidth / 2., kVeryExact);
  EXPECT_NEAR(l1->driveable_bounds(0.).r_max, kWidth / 2., kVeryExact);

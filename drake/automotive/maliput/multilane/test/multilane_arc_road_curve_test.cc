...
TEST_F(MultilaneArcRoadCurveTest, Orientation) {
  ...
  for (double p : p_vector) {
    for (double r : r_vector) {
      for (double h : h_vector) {
        ...
        EXPECT_NEAR(wrap(rotation.yaw()), wrap(flat_dut.heading_of_p(p)),
                    kVeryExact);
      }
    }
  }
  ...
  for (double p : p_vector) {
    for (double r : r_vector) {
      for (double h : h_vector) {
        const Rot3 Rabg = elevated_dut.Rabg_of_p(p);
        const Vector3<double> w_prime =
            elevated_dut
                .W_prime_of_prh(p, r, h, Rabg, linear_elevation.f_dot_p(p))
                .normalized();
        const Rot3 rotation = elevated_dut.Orientation(p, r, h);
        const Vector3<double> r_versor = Rabg.apply({0., 1., 0.});
        const double pitch = std::atan2(
            -w_prime[2],
            std::sqrt(w_prime[0] * w_prime[0] + w_prime[1] * w_prime[1]));
        EXPECT_NEAR(wrap(rotation.pitch()), wrap(pitch), kVeryExact);
        EXPECT_NEAR(rotation.roll(), std::atan2(r_versor[2] / std::cos(pitch),
                                                ((r_versor[1] * w_prime[0]) -
                                                 (r_versor[0] * w_prime[1])) /
                                                    std::cos(pitch)),
                    kVeryExact);
        EXPECT_NEAR(wrap(rotation.yaw()),
                    wrap(std::atan2(w_prime[1], w_prime[0])), kVeryExact);
      }
    }
  }
  ...
  for (double p : p_vector) {
    for (double r : r_vector) {
      for (double h : h_vector) {
        const Rot3 Rabg = superelevated_dut.Rabg_of_p(p);
        const Vector3<double> w_prime =
            superelevated_dut.W_prime_of_prh(p, r, h, Rabg, 0.).normalized();

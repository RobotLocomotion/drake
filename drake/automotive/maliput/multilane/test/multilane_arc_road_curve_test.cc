...
TEST_F(MultilaneArcRoadCurveTest, OffsetTest) {
  ...
  // lateral offset.
  EXPECT_THROW(dut.p_scale_offset_factor(1., kRadius), std::runtime_error);
  EXPECT_THROW(
      dut.p_scale_offset_factor(1., 2.0 * kRadius), std::runtime_error);
  ...
  for (double r : r_vector) {
    for (double p : p_vector) {
      EXPECT_DOUBLE_EQ(
          dut.p_scale_offset_factor(p, r), kRadius / (kRadius - r));
    }
  }

...
TEST_F(MultilaneBuilderCrossTest, LaneToLaneMultilaneCross) {
  ...
  Builder b(kLaneWidth, kElevationBounds, kLinearTolerance, kAngularTolerance);
  // Creates connections.
  auto c1 = b.Connect("c1", kThreeLanes, kLeftShoulder, kRightShoulder,
                      kFirstLaneId, endpoint_a, 50., kFirstLaneId, kLowFlatZ);
  auto c2 =
      b.Connect("c2", kTwoLanes, kLeftShoulder, kRightShoulder, kFirstLaneId,
                c1->LaneEnd(kSecondLaneId), 20., kSecondLaneId, kLowFlatZ);
  b.Connect("c3", kTwoLanes, kLeftShoulder, kRightShoulder, kSecondLaneId,
            c2->LaneEnd(kSecondLaneId), 30., kFirstLaneId, kLowFlatZ);
  auto c4 = b.Connect("c4", kThreeLanes, kLeftShoulder, kRightShoulder,
                      kSecondLaneId, c1->LaneEnd(kSecondLaneId),
                      ArcOffset(10., -M_PI / 2.), kFirstLaneId, kLowFlatZ);
  auto c5 = b.Connect("c5", kTwoLanes, kLeftShoulder, kRightShoulder,
                      kFirstLaneId, endpoint_h, 36., kFirstLaneId, kLowFlatZ);
  auto c6 = b.Connect("c6", kTwoLanes, kLeftShoulder, kRightShoulder,
                      kFirstLaneId, c5->LaneEnd(kFirstLaneId),
                      ArcOffset(10., M_PI / 2.), kFirstLaneId, kLowFlatZ);
  auto c7 =
      b.Connect("c7", kTwoLanes, kLeftShoulder, kRightShoulder, kSecondLaneId,
                c6->LaneStart(kSecondLaneId), 20., kSecondLaneId, kLowFlatZ);
  b.Connect("c8", kThreeLanes, kLeftShoulder, kRightShoulder, kThirdLaneId,
            c7->LaneEnd(kSecondLaneId), 44., kSecondLaneId, kLowFlatZ);
  // Creates the crossing junction.
  std::vector<const Connection*> connections{c2, c4, c6, c7};
  b.MakeGroup("cross", connections);

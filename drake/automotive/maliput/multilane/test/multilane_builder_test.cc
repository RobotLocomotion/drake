...
GTEST_TEST(MultilaneBuilderTest, QuadRing) {
  ...
  auto left1 = b.Connect("left1", kOneLane, kZeroR0, kLeftShoulder,
                         kRightShoulder, northbound.reverse(),
                         kLargeClockwiseLoop, kFlatZ);
  // This heads +y, loops to -x, counterclockwise, back to origin.
  auto left0 = b.Connect("left0", kOneLane, kZeroR0, kLeftShoulder,
                         kRightShoulder, northbound, kSmallCounterClockwiseLoop,
                         kFlatZ);
  // This heads +y, loops to +x, clockwise, back to origin.
  auto right0 = b.Connect("right0", kOneLane, kZeroR0, kLeftShoulder,
                          kRightShoulder, northbound, kSmallClockwiseLoop,
                          kFlatZ);
  // This heads -y, loops to +x, counterclockwise, back to origin.
  auto right1 = b.Connect("right1", kOneLane, kZeroR0, kLeftShoulder,
                          kRightShoulder, northbound.reverse(),
...
TEST_F(MultilaneBuilderPrimitivesTest, MultilaneLineSegment) {
  ...
  const Vector3<double> r_versor(
      -std::sin(kStartHeading), std::cos(kStartHeading), 0.);
  ...
  for (int i = 0; i < kNumLanes; i++) {
    ...
    // applied.
    const Vector3<double> lane_start_geo = start_reference_curve +

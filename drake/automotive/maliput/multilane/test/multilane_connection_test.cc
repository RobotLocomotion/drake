...
TEST_F(MultilaneConnectionTest, LaneToLaneArcRoadCurveValidation) {
  ...
  EXPECT_TRUE(CompareMatrices(
      Vector3<double>(21.7677669529664, 31.7677669529664, kLowFlatZ.z()),
      flat_origin, kVeryExact));
  const Vector3<double> flat_end = road_curve->W_of_prh(1., 0., 0.);
  EXPECT_TRUE(CompareMatrices(
      Vector3<double>(flat_dut.end().xy().x(), flat_dut.end().xy().y(),
      ...
      flat_end, kVeryExact));
  EXPECT_TRUE(CompareMatrices(
      Vector3<double>(41.7677669529664, 31.7677669529664, kLowFlatZ.z()),
      flat_end, kVeryExact));
  ...
  EXPECT_TRUE(CompareMatrices(
      ...
      complex_end, kVeryExact));
  EXPECT_TRUE(CompareMatrices(
      Vector3<double>(41.7677669529664, 31.7677669529664,
  ...
  EXPECT_NEAR(complex_road_curve->elevation().a(), 0., kVeryExact);
  EXPECT_NEAR(complex_road_curve->elevation().b(), 0., kVeryExact);
  EXPECT_NEAR(complex_road_curve->elevation().c(), -0.321619522811069,
              kVeryExact);
  EXPECT_NEAR(complex_road_curve->elevation().d(), 0.557952555802309,
              kVeryExact);
  EXPECT_NEAR(complex_road_curve->superelevation().a(), 0., kVeryExact);
  EXPECT_NEAR(complex_road_curve->superelevation().b(), 0., kVeryExact);
  EXPECT_NEAR(complex_road_curve->superelevation().c(), -0.959907943666135,
              kVeryExact);
  EXPECT_NEAR(complex_road_curve->superelevation().d(), 0.983478169705686,
              kVeryExact);
}
...
TEST_F(MultilaneConnectionTest, LaneToLaneLineRoadCurveValidation) {
  ...
  EXPECT_TRUE(CompareMatrices(
      ...
      flat_end, kVeryExact));
  EXPECT_TRUE(CompareMatrices(
      Vector3<double>(51.76776695296637, 1.76776695296637, kLowFlatZ.z()),
      flat_end, kVeryExact));
  ...
  EXPECT_TRUE(CompareMatrices(
      ...
      complex_end, kVeryExact));
  EXPECT_TRUE(CompareMatrices(
      Vector3<double>(51.76776695296637, 1.76776695296637,
                      kEndElevatedEndpointZ.z() +
                          kRRef * std::sin(kEndElevatedEndpointZ.theta())),
      complex_end, kVeryExact));
  ...
  EXPECT_NEAR(complex_road_curve->elevation().a(), 0., kVeryExact);
  EXPECT_NEAR(complex_road_curve->elevation().b(), 0., kVeryExact);
  EXPECT_NEAR(complex_road_curve->elevation().c(), -0.628768939877063,
              kVeryExact);
  EXPECT_NEAR(complex_road_curve->elevation().d(), 0.752512626584708,
              kVeryExact);
  EXPECT_NEAR(complex_road_curve->superelevation().a(), 0., kVeryExact);
  EXPECT_NEAR(complex_road_curve->superelevation().b(), 0., kVeryExact);
  EXPECT_NEAR(complex_road_curve->superelevation().c(), -0.962975975515347,

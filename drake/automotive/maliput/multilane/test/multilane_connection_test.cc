  ...
  for (int i = 0; i < num_lanes; i++) {
    ...
    const Endpoint lane_start{
        {kCenterX + effective_radius * std::cos(kTheta0),
         ...
         wrap(kTheta0 + M_PI / 2.)},
        {start_z.z(), start_z.z_dot() * kRadius / effective_radius,
         start_z.theta(), start_z.theta_dot() * kRadius / effective_radius}};
    EXPECT_TRUE(
    ...
    const Endpoint lane_end{
        ...
        {end_z.z(), end_z.z_dot() * kRadius / effective_radius,
         end_z.theta(), end_z.theta_dot() * kRadius / effective_radius}};

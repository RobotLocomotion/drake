...
double ArcRoadCurve::p_from_s(double s, double r) const {
  ...
  DRAKE_DEMAND(r == 0. || (superelevation().a() == 0. &&
               ...
               superelevation().d() == 0.));
  // TODO(maddog-tri)  This code only supports up to linear elevation
  //                   polynomials. Generalization to upper grade functions is
  //                   needed.
  DRAKE_DEMAND(r == 0 || (elevation().c() == 0. && elevation().d() == 0.));
...
Vector3<double> ArcRoadCurve::ToCurveFrame(
  ...
  const double p_unsaturated = d_theta_nearest_unwrapped / std::abs(d_theta_);

...
// computed. `road_curve` is used to compute z derivative with respect to p at
// the start or end of the `lane` respectively.
Endpoint ComputeEndpointForLane(const RoadCurve* road_curve, const Lane* lane,
  ...
  return Endpoint(EndpointXy(position.x(), position.y(), rotation.yaw()),
                  EndpointZ(position.z(), w_prime.z(), zpoint.theta(),
                            zpoint.theta_dot()));
}
}  // namespace
...
std::vector<Lane*> Builder::BuildConnection(
  ...
  for (int i = 0; i < conn->num_lanes(); i++) {
    ...
    const Endpoint start_endpoint = ComputeEndpointForLane(
        segment->road_curve(), lane, api::LaneEnd::kStart, conn->start().z());
    const Endpoint finish_endpoint = ComputeEndpointForLane(
        segment->road_curve(), lane, api::LaneEnd::kFinish, conn->end().z());

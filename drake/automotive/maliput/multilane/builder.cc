...
const Connection* Builder::Connect(const std::string& id, int num_lanes,
  ...
  connections_.push_back(std::make_unique<Connection>(
      id, start, start_lane_index, z_end, end_lane_index, num_lanes, 0.,
      start_lane_index, lane_width_, left_shoulder, right_shoulder, length));
  return connections_.back().get();
}
...
const Connection* Builder::Connect(const std::string& id, int num_lanes,
  ...
  connections_.push_back(std::make_unique<Connection>(
      id, start, start_lane_index, z_end, end_lane_index, num_lanes, 0.,
      start_lane_index, lane_width_, left_shoulder, right_shoulder, arc));
  return connections_.back().get();
}

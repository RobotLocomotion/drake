...
double Lane::do_length() const { return road_curve_->trajectory_length(r0_); }
...
api::LanePosition Lane::DoEvalMotionDerivatives(
  ...
  const double ds_dsigma = road_curve_->p_scale() /
                           road_curve_->p_scale_offset_factor(p, r0_) *
...
api::LanePosition Lane::DoToLanePosition(
  ...
  const api::LanePosition lane_position = api::LanePosition(
      lane_position_in_segment_curve_frame[0] /
          road_curve_->p_scale_offset_factor(
              road_curve_->p_from_s(lane_position_in_segment_curve_frame[0],
                                    0.),
              r0_),

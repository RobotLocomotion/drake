...
Endpoint Connection::LaneStart(int lane_index) const {
  ...
  const double theta_dot = start_.z().theta_dot() / planar_length;
  return Endpoint(EndpointXy(position[0], position[1], rotation.yaw()),
                  EndpointZ(position[2], z_dot, start_.z().theta(), theta_dot));
}
...
Endpoint Connection::LaneEnd(int lane_index) const {
  ...
  // Given that ∂p/∂t = 1 / planar_length.
  const double z_dot = w_prime.z() / planar_length;
  const double theta_dot = end_.z().theta_dot() / planar_length;
  return Endpoint(EndpointXy(position[0], position[1], rotation.yaw()),
                  EndpointZ(position[2], z_dot, end_.z().theta(), theta_dot));
}

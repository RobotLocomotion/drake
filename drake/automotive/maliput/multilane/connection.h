...
class Connection {
 public:
  ...
  ///
  /// Segment's reference curve begins at `start` and extends on the plane
  /// z=0 `line_length` distance with `start.xy().heading()` heading angle. The
  /// reference curve will be coincident with `start_lane_index` lane. `end_z`
  /// is the end EndpointZ information of `end_lane_index` lane. It will be
  /// translated to `start_lane_index` lane and then used to build elevation and
  /// superelevation information of the road.
  ///
  /// `line_length` must be non negative.
  ///
  /// Segments will contain `num_lanes` lanes, which must be greater than zero.
  /// First Lane centerline will be placed at `start_lane_index` times
  /// `lane_width` distance from the reference curve. Each lane's width will be
  /// `lane_width`, which should be greater or equal to zero.
  ///
  ...
  ///
  /// Segment's reference curve begins at `start` and extends on the plane z=0
  /// with `arc_offset.radius()` and angle span of `arc_offset.d_theta()`.The
  /// reference curve will be coincident with `start_lane_index` lane. `end_z`
  /// is the end EndpointZ information of `end_lane_index` lane. It will be
  /// translated to `start_lane_index` lane and then used to build elevation and
  /// superelevation information of the road.

...
using multibody::CreateLoadMessage;
...
template <typename T> const int PriusVis<T>::kNumPositionDofs{13};
template <typename T> const int PriusVis<T>::kNumVelocityDofs{13};
...
PriusVis<T>::PriusVis(int id, const std::string& name)
  ...
  lcmt_viewer_load_robot load_message =
      multibody::CreateLoadMessage<T>(*tree_);

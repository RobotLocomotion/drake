...
Bicycle<T>::get_steering_input_port() const {
  return systems::System<T>::get_input_port(0);
}

template <typename T>
const systems::InputPortDescriptor<T>& Bicycle<T>::get_pt_input_port()
    const {
...
template <typename T>
void Bicycle<T>::SetDefaultParameters(
    const systems::LeafContext<T>& context,
    systems::Parameters<T>* params) const {
  // Parameters are from Althoff & Dolan, 2014
  auto p = dynamic_cast<BicycleParameters<T>*>(
      params->get_mutable_numeric_parameter(0));
  DRAKE_DEMAND(p != nullptr);
  p->set_mass(2278.);  // Mass [kg].
  p->set_lf(1.292);  // Distance from CG to front axle [m].
  p->set_lr(1.515);  // Distance from CG to rear axle [m].
  p->set_Iz(3210.);  // Moment of inertia about the yaw-axis [kg m^2].
  p->set_Cf(10.8e4);  // Cornering stiffness (front) [N / rad].
  p->set_Cr(10.8e4);  // Cornering stiffness (rear) [N / rad].
}

// These instantiations must match the API documentation in bicycle.h.
template class Bicycle<double>;
template class Bicycle<drake::TaylorVarXd>;

...
ClosestPose<T> PoseSelector<T>::FindSingleClosestPose(
  ...
  result.odometry = make_infinite_odometry(lane_direction);
...
T PoseSelector<T>::GetSigmaVelocity(const RoadOdometry<T>& road_odometry) {
  DRAKE_DEMAND(is_within_lane(road_odometry.pos, road_odometry.lane));
  const LanePosition position{road_odometry.pos.s(), road_odometry.pos.r(),
                              road_odometry.pos.h()};
  const maliput::api::Rotation rot =
      road_odometry.lane->GetOrientation(position);
...
template <typename T>
bool PoseSelector<T>::is_within_driveable(const LanePosition& lane_position,
                                          const Lane* lane) {
...
template <typename T>
bool PoseSelector<T>::is_within_lane(const LanePosition& lane_position,
                                     const Lane* lane) {
  if (is_within_driveable(lane_position, lane)) {
...
double PoseSelector<T>::calc_lane_progress(const LaneDirection& lane_direction,
  ...
  DRAKE_DEMAND(is_within_driveable(lane_position, lane_direction.lane));
  const double new_s =

...
ClosestPose<T> PoseSelector<T>::FindSingleClosestPose(
  ...
  // vehicle.
  const Lane* lane = road.ToRoadPosition(
      MakeGeoPosition(ego_pose.get_isometry()), nullptr, nullptr, nullptr).lane;
  const ClosestPose<T> result_in_path = FindSingleClosestPose(
...
ClosestPose<T> PoseSelector<T>::FindSingleClosestInBranches(
  ...
  const Lane* const ego_lane =
      road.ToRoadPosition(MakeGeoPosition(ego_pose.get_isometry()),
  ...
  LaneDirection ego_lane_direction =
      CalcLaneDirection(ego_lane, ego_lane_position, ego_pose.get_rotation(),
                        side);
  ClosestPose<T> result;
  ...
  for (int i = 0; i < traffic_poses.get_num_poses(); ++i) {
    ...
    const Lane* const lane =
        road.ToRoadPosition(MakeGeoPosition(traffic_isometry),
                            nullptr, nullptr, nullptr).lane;
    ...
    // is occupying.
    const T lane_sigma_v = GetSigmaVelocity(
...
PoseSelector<T>::FindConfluentBranches(
    const RoadGeometry& road, const PoseVector<T>& ego_pose,

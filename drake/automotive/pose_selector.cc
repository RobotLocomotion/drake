...
// lane_direction to match the `lane` and `with_s` of that branch.  If there is
// no default branch, the ongoing branch with index = 0 is selected.  Otherwise,
// returns `nullopt` and sets `lane_direction->lane` to `nullptr`.
optional<LaneEnd> GetDefaultOrFirstOngoingLane(LaneDirection* lane_direction) {
  ...
  // The LaneEnd corresponds to the traversal end; need to reverse this to get
  // the opposite end (i.e. the one connected to the branch point).
  branch->end =
...
optional<LaneEnd> GetTargetLaneEnd(const LaneDirection& lane_direction) {
  const Lane* const lane{lane_direction.lane};
  const bool with_s{lane_direction.with_s};
    ...
    for (auto branch_distance : branches) {
      ...
      while (distance_scanned < distance_to_scan) {
        ...
        if (0. < effective_headway && effective_headway < result.distance) {
          ...
          result.odometry =
              RoadOdometry<T>(traffic_lane, lane_position,
                              traffic_poses.get_velocity(i));
          break;
        }
  ...
  for (auto side : {AheadOrBehind::kAhead, AheadOrBehind::kBehind}) {
    result.insert(std::make_pair(
        side, FindSingleClosestPose(lane, ego_pose, traffic_poses,
                                    scan_distance, side, check_branches)));
  }
  return result;
}
...
ClosestPose<T> PoseSelector<T>::FindSingleClosestPose(
    const Lane* lane, const PoseVector<T>& ego_pose,
    const PoseBundle<T>& traffic_poses, const T& scan_distance,
    const AheadOrBehind side, bool check_branches) {
  ...
  if (!check_branches) return result_in_path;

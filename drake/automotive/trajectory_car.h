...
class TrajectoryCar : public systems::LeafSystem<T> {
 ...
 private:
  ...
  const PositionHeading CalcRawPose(const TrajectoryCarState<T>& state) const {
    ...
    // TODO(jadecastro): Now that the curve is a function of position, we are no
    // longer using a `trajectory` anymore.  Rename this System to
    // PathFollowingCar or something similar.

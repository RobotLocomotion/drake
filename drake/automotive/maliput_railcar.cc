...
template <typename T>
constexpr double MaliputRailcar<T>::kDefaultMaxAcceleration;
template <typename T>
...
MaliputRailcar<T>::MaliputRailcar(const Lane& lane, double start_time)
  ...
  command_input_port_index_ =
      this->DeclareInputPort(systems::kVectorValued,
          RailcarCommandIndices::kNumCoordinates).get_index();
...
void MaliputRailcar<T>::DoCalcTimeDerivatives(
  ...
  // Obtains the input.
  const RailcarCommand<T>* const input =
      this->template EvalVectorInput<RailcarCommand>(context,
          command_input_port_index_);
  DRAKE_ASSERT(input);
...
void MaliputRailcar<T>::SetDefaultParameters(MaliputRailcarConfig<T>* config) {
  ...
  config->set_max_acceleration(kDefaultMaxAcceleration);

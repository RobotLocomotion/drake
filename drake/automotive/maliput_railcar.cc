...
void MaliputRailcar<T>::ImplCalcVelocity(const MaliputRailcarParams<T>& params,
  ...
  //  - Vt is the translational component of the spatial velocity.
  //  - L is the lane frame.
  //  - W is the world frame.
  //  - R is a rotation matrix.

  const Vector3<T> Vt_L(lane_direction.with_s ? state.speed() : -state.speed(),
                       0 /* r_dot */, 0 /* h_dot */);
  const Rotation rotation =

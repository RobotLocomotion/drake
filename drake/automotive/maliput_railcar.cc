...
MaliputRailcar<T>::MaliputRailcar(const LaneDirection& initial_lane_direction,
    ...
    : start_time_(start_time),
      initial_lane_direction_(initial_lane_direction.lane,
                              initial_lane_direction.with_s) {
...
void MaliputRailcar<T>::ImplCalcLaneOutput(const LaneDirection& lane_direction,
  ...
  output->lane = lane_direction.lane;
  output->with_s = lane_direction.with_s;
}
...
MaliputRailcar<T>::AllocateOutputAbstract(
    const systems::OutputPortDescriptor<double>& descriptor) const {
  auto output = std::make_unique<systems::LeafSystemOutput<double>>();
  LaneDirection msg;
  return std::make_unique<systems::Value<LaneDirection>>(msg);
}
...
void MaliputRailcar<T>::SetDefaultState(const Context<T>& context,
  ...
  LaneDirection& lane_direction =
      state->get_mutable_abstract_state()->get_mutable_abstract_state(0).
          template GetMutableValue<LaneDirection>();
  lane_direction.lane = initial_lane_direction_.lane;
  lane_direction.with_s = initial_lane_direction_.with_s;
}

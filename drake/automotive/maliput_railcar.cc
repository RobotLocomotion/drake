...
template <typename T>
constexpr T MaliputRailcar<T>::kDefaultInitialS;
template <typename T>
constexpr T MaliputRailcar<T>::kDefaultInitialSpeed;
template <typename T>
...
void MaliputRailcar<T>::DoCalcOutput(const Context<T>& context,
  ...
  PoseVector<T>* const pose_vector = dynamic_cast<PoseVector<T>*>(
  ...
  FrameVelocity<T>* const frame_velocity = dynamic_cast<FrameVelocity<T>*>(
...
void MaliputRailcar<T>::ImplCalcPose(const MaliputRailcarParams<T>& params,
  ...
  const Rotation adjusted_rotation =
      (lane_direction.with_s
           ? rotation
           : Rotation(-rotation.roll, -rotation.pitch,
                      atan2(-sin(rotation.yaw), -cos(rotation.yaw))));
  pose->set_translation(Eigen::Translation<T, 3>(geo_position.xyz()));
  pose->set_rotation(RollPitchYawToQuaternion(Vector3<T>(
      adjusted_rotation.roll, adjusted_rotation.pitch, adjusted_rotation.yaw)));
}

template <typename T>
void MaliputRailcar<T>::ImplCalcVelocity(

...
template <typename T>
CrossIdmPlanner<T>::CrossIdmPlanner(const T& v_ref) : v_ref_(v_ref) {
...
template <typename T>
CrossIdmPlanner<T>::~CrossIdmPlanner() {}

template <typename T>
const systems::InputPortDescriptor<T>& CrossIdmPlanner<T>::get_ego_port() const {
  return systems::System<T>::get_input_port(0);
}

template <typename T>
const systems::InputPortDescriptor<T>& CrossIdmPlanner<T>::get_agent_port() const {
  return systems::System<T>::get_input_port(1);
}

template <typename T>
void CrossIdmPlanner<T>::DoCalcOutput(const systems::Context<T>& context,
...
template <typename T>
std::unique_ptr<systems::Parameters<T>> CrossIdmPlanner<T>::AllocateParameters()
    const {
...
template <typename T>
void CrossIdmPlanner<T>::SetDefaultParameters(const systems::LeafContext<T>& context,
...
// idm_planner.h.
template class CrossIdmPlanner<double>;
template class CrossIdmPlanner<drake::TaylorVarXd>;
template class CrossIdmPlanner<drake::symbolic::Expression>;

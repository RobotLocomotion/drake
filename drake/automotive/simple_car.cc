...
void SimpleCar<T>::ImplCalcTimeDerivatives(const SimpleCarConfig<T>& config,
  ...
  DRAKE_DEMAND(abs(input.steering_angle()) < T(M_PI));
  DRAKE_DEMAND(input.throttle() >= T(0));
  DRAKE_DEMAND(input.brake() >= T(0));
  ...
  const T smoothing_factor =
      pow(tanh(20.0 * (state.velocity() - relevant_limit)), T(2));
  const T smooth_acceleration = damped_acceleration * smoothing_factor;

  // Determine steering.
  const T clamped_steering_angle = clamp(
      input.steering_angle(),
      T(-config.max_abs_steering_angle()),
      T(config.max_abs_steering_angle()));
  const T curvature = tan(clamped_steering_angle) / config.wheelbase();

  // Don't allow small negative velocities to affect position or heading.
  const T nonneg_velocity = std::max(T(0), state.velocity());

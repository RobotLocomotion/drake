...
class MaliputRailcar : public systems::LeafSystem<T> {
 public:
  ...
  /// always greater than the current time. Despite the spatial window provided
  /// by kLaneEndEpsilon, it is still possible for the vehicle to end up
  /// precisely at the end of its current lane (e.g., it could be initialized to
  /// be in this state). In this scenario, the next update time will be equal to
  /// the current time. The integrator, however, requires that the next update
  /// time be strictly after the current time, which is when this constant is
  /// used. The primary constraint on the selection of this constant is the
  /// application's sensitivity to a MaliputRailcar being "late" in its

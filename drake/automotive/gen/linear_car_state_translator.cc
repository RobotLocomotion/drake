#include "drake/automotive/gen/linear_car_state_translator.h"
...
LinearCarStateTranslator::AllocateOutputVector() const {
  return std::make_unique<LinearCarState<double>>();
}

void LinearCarStateTranslator::Serialize(
    double time, const systems::VectorBase<double>& vector_base,
    std::vector<uint8_t>* lcm_message_bytes) const {
  const auto* const vector =
      dynamic_cast<const LinearCarState<double>*>(&vector_base);
...
void LinearCarStateTranslator::Deserialize(
    const void* lcm_message_bytes, int lcm_message_length,
    systems::VectorBase<double>* vector_base) const {
  DRAKE_DEMAND(vector_base != nullptr);
  auto* const my_vector = dynamic_cast<LinearCarState<double>*>(vector_base);

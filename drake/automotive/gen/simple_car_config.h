...
class SimpleCarConfig : public systems::BasicVector<T> {
 public:
  ...
  //@{
    ...
    const T max_abs_steering_angle() const { return this->GetAtIndex(K::kMaxAbsSteeringAngle); }
    ...
    const T max_acceleration() const { return this->GetAtIndex(K::kMaxAcceleration); }
    void set_max_acceleration(const T& max_acceleration) {
      this->SetAtIndex(K::kMaxAcceleration, max_acceleration);
    }
    const T velocity_lookahead_time() const { return this->GetAtIndex(K::kVelocityLookaheadTime); }

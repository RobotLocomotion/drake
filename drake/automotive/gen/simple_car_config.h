...
class SimpleCarConfig : public systems::BasicVector<T> {
 public:
  ...
  /// A limit on the driving_command.steering angle input (the desired steering
  /// angle of a virtual center wheel), in radians; this element must be
  /// positive, and is applied symmetrically to both left- and right-turn
  /// limits.
  ...
  /// A limit on the car's forward velocity, in meters per second; this element
  /// must be positive.
  ...
  /// A limit on the car's acceleration and deceleration, in meters per second
  ...
  /// Smoothing constant for velocity changes, in seconds; this element must be
  ...
  /// Smoothing constant for velocity changes, in Hz; this element must be
  /// positive.  This is deprecated and will eventually be removed.

...
class PoseSelector {
 public:
  ...
  /// `s`-coordinate of the ego vehicle's lane.  In this function, only poses in
  /// the same path as the ego (ahead or behind) are tracked.
  ///
  ...
  /// Same as PoseSelector::FindClosestPair(), except poses in lanes that
  ...
  /// default path.
  static std::map<AheadOrBehind, const ClosestPose<T>>
  FindClosestPairCheckBranches(
      const maliput::api::Lane* lane,
      const systems::rendering::PoseVector<T>& ego_pose,
      const systems::rendering::PoseBundle<T>& traffic_poses,
      const T& scan_distance);
  ...
  /// (AheadOrBehind::kBehind).  In this function, only poses in the same path
  /// as the ego (ahead or behind) are tracked.
  ///
  ...
  /// Same as PoseSelector::FindSingleClosestPose(), except poses in lanes that
  /// eventually merge (converge to the same branch point that is on the default
  /// path of @p lane) are considered in addition to those along the ego car's
  /// default path.
  static ClosestPose<T> FindSingleClosestPoseCheckBranches(
      const maliput::api::Lane* lane,
      const systems::rendering::PoseVector<T>& ego_pose,
      const systems::rendering::PoseBundle<T>& traffic_poses,
      const T& scan_distance, const AheadOrBehind side);

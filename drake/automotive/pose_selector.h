...
class PoseSelector {
 public:
  ...
  /// `s`-coordinate of the ego vehicle's lane.  If @p check_branches is false,
  /// only poses in the same path as the ego (ahead or behind) are tracked; if
  /// true, poses in lanes that eventually merge (converge to the same branch
  /// point that is on the default path of @p lane) are considered in addition
  /// to those along the ego car's default path.
  ///
  ...
  static std::map<AheadOrBehind, const ClosestPose<T>> FindClosestPair(
      ...
      const T& scan_distance, bool check_branches);
  ...
  static ClosestPose<T> FindSingleClosestPose(
      ...
      const systems::rendering::PoseVector<T>& ego_pose,
      const systems::rendering::PoseBundle<T>& traffic_poses,
      const T& scan_distance, const AheadOrBehind side, bool check_branches);

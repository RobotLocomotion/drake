...
class PoseSelector {
 public:
  ...
  /// Returns the leading and trailing vehicles that are closest to an ego
  /// vehicle in a given @p lane as measured along the `s`-coordinate of the
  /// lane.  The ego vehicle may be in any lane, provided it is within the
  /// `driveable_bounds` of @p lane (i.e. the road is contiguous with @p lane
  /// along the `r`-direction`).  This function is used, for instance, as logic
  /// for lane-change planners (e.g. MOBIL).  The ego car's pose @p ego_pose and
  /// the poses of the traffic cars (@p traffic_poses) are provided.  The
  /// parameter @p scan_distance determines the distance along the sequence of
  /// lanes to scan before declaring the car is at infinite distance ahead
  /// (resp. behind).  If no leading/trailing vehicles are seen within @p
  /// traffic_lane, `s`-positions are taken to be at infinite distances away
  /// from the ego car.
  ///
  ...
  /// Returns `true` if and only id @p geo_position is within the longitudinal
  ...
  /// (i.e. within `lane->lane_bounds()` and `lane->elevation_bounds()`).
  static bool is_within_driveable(
  ...
  static bool is_within_lane(const maliput::api::LanePosition& lane_position,
                             const maliput::api::Lane* lane);
 ...
 private:
  ...
  // Returns a RoadOdometry that contains infinite positions. If @p
  // lane_direction contains `with_s == True`, A RoadOdometry containing an
  // s-position at positive infinity is returned; otherwise a negative-intinite
  // position is returned.
  static RoadOdometry<T> make_infinite_odometry(
      const LaneDirection& lane_direction);

  // Returns the distance from an end of a lane to a @p lane_position in that
  // lane, where the end is determined by the `with_s` of the provided
  // `lane_direction`.  Both `lane` and `with_s` are specified in @p
  // lane_direction.  Throws if any element of @p lane_position is not within
  // the respective bounds of `lane_direction.lane`.

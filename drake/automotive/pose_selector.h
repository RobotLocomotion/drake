...
class PoseSelector {
 public:
  ...
  ///
  /// If @p check_branches = true, then poses in lanes that eventually merge
  /// (converge to the same branch point) are considered in addition to those
  /// along the ego car's path.  Otherwise, only poses in the same path as the
  /// ego (ahead or behind) are tracked.
  ///
  ...
  /// the same RoadPosition as the ego car) are discarded.  If no
  /// leading/trailing vehicles are seen within scan-distance of the ego car,
  /// `s`-positions are taken to be at infinite distances away from the ego car.
  /// Note also that traffic vehicles having exactly the same s-position as the
  /// ego vehicle but with different `r`-value or are directly perpendicular in
  /// a Lane::to_left(), Lane::to_right() lane are taken to be *behind* the ego
  /// vehicle.
  ///
  ...
  static std::map<AheadOrBehind, const ClosestPose<T>> FindClosestPair(
      ...
      const T& scan_distance, bool check_branches = false);
  ...
  /// addition to those along the ego car's path.  Otherwise, only poses in the
  /// same path as the ego (ahead or behind) are tracked.  Cars in other lanes
  /// are only tracked if they are in confluent lanes to a given branch point
  /// within the `scan_distance`; i.e. cars in two side-by-side lanes that never
  /// enter a branch point will not be selected.  This assumes that the ego car
  /// has knowledge of the traffic cars' default lane.
  ///

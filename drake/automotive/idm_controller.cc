...
void IdmController<T>::ImplDoCalcOutput(
  ...
  const T& s_dot_ego = GetSVelocity(RoadOdometry<double>(
      ego_position, ego_velocity));
      // ego_velocity.get_velocity().translational().x();
...
template <typename T>
const double IdmController<T>::GetSVelocity(const RoadOdometry<T>& road_odom)
    const {

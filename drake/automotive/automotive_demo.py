...
class Launcher(object):
    ...
    def launch(self, command, label=None):
        ...
        if label is None:
            label = os.path.basename(command_words[0])
...
def bot_spy_that_actually_works():
    ...
    theLauncher.launch("java -cp %s lcm.spy.Spy" % classpath, label="bot-spy")
...
def wait_for_lcm_message_on_channel(channel):
    ...
    sub = lc.subscribe(channel, receive)
    try:
        while True:
            rlist, _, _ = select.select([lc], [], [])
            if lc in rlist:
                lc.handle()
...
def main():
    ...
    # theLauncher.launch(os.path.join(_DRAKE_INSTALL_BIN, "lcm-logger"))
    theLauncher.launch(
        os.path.join(DRAKE_DIST_BUILD_DIR,
                     "externals", "lcm", "lcm-logger", "lcm-logger"))

    # TODO(#3231) Use this shell script once it works again.
    # theLauncher.launch(os.path.join(_DRAKE_INSTALL_BIN, "bot-spy")
    bot_spy_that_actually_works()

    theLauncher.launch(os.path.join(DRAKE_INSTALL_BIN_DIR, "drake-visualizer"))

    # Wait, to be sure drake-visualizer sees the load_robot message.
    wait_for_lcm_message_on_channel('DRAKE_VIEWER_STATUS')

    theLauncher.launch(demo_path + " ".join([''] + tail))
    theLauncher.launch(os.path.join(_THIS_DIR, "steering_command_driver.py"))

    try:
        theLauncher.wait(args.duration)
    finally:
        theLauncher.kill()

    sys.exit(theLauncher.returncode)

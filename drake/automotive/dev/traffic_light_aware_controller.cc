...
template <typename T>
const systems::InputPortDescriptor<T>& TrafficLightAwareController<T>::car_state() const {
 return System<T>::get_input_port( car_state_input_index_ );
}

template <typename T>
const systems::InputPortDescriptor<T>& TrafficLightAwareController<T>::other_controller_acceleration() const {
 return System<T>::get_input_port( acceleration_input_index_ );
}

template <typename T>
const systems::InputPortDescriptor<T>& TrafficLightAwareController<T>::traffic_light_input() const {
 return System<T>::get_input_port( traffic_light_input_index_ );
}

template <typename T>
const systems::OutputPort<T>& TrafficLightAwareController<T>::acceleration_output() const {
 return System<T>::get_output_port( output_index_ );
}

template <typename T>
void TrafficLightAwareController<T>::DoCalcOutput( const systems::Context<T>& context,
  ...
  VectorX<T> traffic_light_signal = ReadInput( context, traffic_light_input_index_ );
...
template <typename T>
const VectorX<T> TrafficLightAwareController<T>::ReadInput( const Context<T>& context,

...
void Xtion::Build(lcm::DrakeLcm* lcm, bool add_lcm_publisher,
  ...
  // Wrap constants with `double{}` to avoid need for ODR (one definition rule).
  auto* camera = builder.AddSystem<RgbdCameraDiscrete>(
      std::make_unique<RgbdCamera>(
          name_, *tree_, *sensor_frame_,
          double{kDepthRangeNear}, double{kDepthRangeFar},
          double{kFovY}),
      period_);
  camera->set_name(name_ + "_rgbd_camera");

  // Use `PassThrough` to connect imported input to multiple inputs within this
  // diagram.

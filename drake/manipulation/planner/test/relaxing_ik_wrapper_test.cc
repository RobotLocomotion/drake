...
// kinematics poses.
GTEST_TEST(RelaxingIkWrapperTest, SolveIkFromFk) {
  ...
  IKResults ik_res;
  RelaxingIkWrapper ik_planner(kModelPath, kEndEffectorLinkName,
                               Isometry3<double>::Identity());
  RelaxingIkWrapper::IkCartesianWaypoint wp;
  wp.pos_tol = Vector3<double>(0.001, 0.001, 0.001);
  wp.rot_tol = 0.005;
  wp.constrain_orientation = true;
  std::vector<RelaxingIkWrapper::IkCartesianWaypoint> waypoints(1, wp);

 ...
 protected:
  ...
  void RunEncodeDecode() {
    ...
    for (int n = 0; n < kNumTests; ++n) {
      ...
      Isometry3<double> X_WB, X_WB_expected;
      Vector6<double> V_WB, V_WB_expected;

      X_WB = DecodePose(message_.pose);
      V_WB = DecodeTwist(message_.twist);
      X_WB_expected = robot.CalcBodyPoseInWorldFrame(cache, base);
      V_WB_expected = robot.CalcBodySpatialVelocityInWorldFrame(cache, base);

...
class TestRunSpongControlledAcrobot(unittest.TestCase):
    ...
    def test_stochastic_scenario(self):
        ...
        output = os.path.join(os.environ["TEST_TMPDIR"], "output.yaml")
        dump_file = os.path.join(os.environ["TEST_TMPDIR"], "scenario_out.yaml")
        subprocess.check_call(
            [
                self.dut,
                "--scenario",
        ...
        self.assertTrue(
            all(type(x) is float for x in dump["controller_params"])
        )
        self.assertEqual(len(dump["initial_state"]), 4)
        self.assertTrue(all(type(x) is float for x in dump["initial_state"]))

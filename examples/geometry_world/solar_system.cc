...
void SolarSystem<T>::AllocateGeometry(GeometrySystem<T>* geometry_system) {
  ...
  X_EG.translation() << kEarthOrbitRadius, 0, -kEarthBottom;
  geometry_system->RegisterGeometry(
      source_id_, planet_id,
      make_unique<GeometryInstance>(X_EG, make_unique<Sphere>(0.25f)));
  // Earth's orrery arm.
  MakeArm(source_id_, planet_id, kEarthOrbitRadius, -kEarthBottom, pipe_radius,
          geometry_system);

  // Luna - Luna's frame X_EL is at the center of the Earth's geometry.
  const Isometry3<double>& X_EL = X_EG;  // X_EL and X_EG are coincident.
  ...
  Isometry3<double> X_LG{Translation3<double>{luna_position}};
  geometry_system->RegisterGeometry(
      source_id_, luna_id,
      make_unique<GeometryInstance>(X_LG, make_unique<Sphere>(0.075f)));
  ...
  // Mars moon - Phobos's frame is centered on Mars's geometry. The plane's axis
  // is negated so it orbits in the opposite direction.
  const Isometry3<double>& X_MP = X_MGm;  // X_MP and X_MGm are coincident.
  FrameId phobos_id = geometry_system->RegisterFrame(
      source_id_, planet_id, GeometryFrame("phobos", X_MP)); // X_MGm = X_MP
  ...
  Isometry3<double> X_PG{Translation3<double>{kPhobosOrbitRadius, 0, 0}};
  geometry_system->RegisterGeometry(
      source_id_, phobos_id,
      make_unique<GeometryInstance>(X_PG, make_unique<Sphere>(0.06f)));

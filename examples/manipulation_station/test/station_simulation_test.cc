...
GTEST_TEST(SimulationStationTest, CheckStateFromPosition) {
  ...
  // If I add the wsg to the plant, but not the controller, the l∞ error is
  // < 4.3.  When I add the wsg back to the controller (given my current
  // implementation), l∞ error is < 1.1.
  const double kTolerance = 1.1;  // rad/sec^2.

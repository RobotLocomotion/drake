...
class EndEffectorTeleop(LeafSystem):
    ...
    _ROLL_DEFAULT = SliderDefault("Roll", 0.0)
    _PITCH_DEFAULT = SliderDefault("Pitch", 0.0)
    _YAW_DEFAULT = SliderDefault("Yaw", 1.57)
    _X_DEFAULT = SliderDefault("X", 0.0)
    _Y_DEFAULT = SliderDefault("Y", 0.0)
    _Z_DEFAULT = SliderDefault("Z", 0.0)
    ...
    def __init__(self, meshcat, planar=False):
        ...
        self.meshcat.AddSlider(
            name=self._ROLL_DEFAULT.name, min=-2.0 * np.pi, max=2.0 * np.pi,
            step=0.01, value=self._ROLL_DEFAULT.default)
        if not self.planar:
            self.meshcat.AddSlider(
                name=self._PITCH_DEFAULT.name, min=-2.0 * np.pi,
                max=2.0 * np.pi, step=0.01, value=self._PITCH_DEFAULT.default)
            self.meshcat.AddSlider(
                name=self._YAW_DEFAULT.name, min=-2.0 * np.pi, max=2.0 * np.pi,
                step=0.01, value=self._YAW_DEFAULT.default)

        # Position control sliders.
        self.meshcat.AddSlider(
            name=self._X_DEFAULT.name, min=-0.6, max=0.8, step=0.01,
            value=self._X_DEFAULT.default)
        if not self.planar:
            self.meshcat.AddSlider(
                name=self._Y_DEFAULT.name, min=-0.8, max=0.3, step=0.01,
                value=self._Y_DEFAULT.default)
        self.meshcat.AddSlider(
            name=self._Z_DEFAULT.name, min=0.0, max=1.1, step=0.01,
            value=self._Z_DEFAULT.default)

        # If the pose is set, these values get overriden and are returned in
        # DoCalcOutput.  This is to account for the fact that the meshcat
        # sliders for pitch, yaw, and Y-axis are *NOT* added to the UI in the
        # planar configuration, but if a SetPose is issued we do not want to
        # return the initial default configuration, we want to return their
        # requested pose.
        self._roll = self._ROLL_DEFAULT.default
        self._pitch = self._PITCH_DEFAULT.default
        self._yaw = self._YAW_DEFAULT.default
        self._x = self._X_DEFAULT.default
        self._y = self._Y_DEFAULT.default
        self._z = self._Z_DEFAULT.default
    ...
    def SetRPY(self, rpy):
        ...
        """
        self._roll = rpy.roll_angle()
        self._pitch = rpy.pitch_angle()
        self._yaw = rpy.yaw_angle()

        self.meshcat.SetSliderValue(self._ROLL_DEFAULT.name, self._roll)
        if not self.planar:
            self.meshcat.SetSliderValue(self._PITCH_DEFAULT.name, self._pitch)
            self.meshcat.SetSliderValue(self._YAW_DEFAULT.name, self._yaw)
    ...
    def SetXYZ(self, xyz):
        ...
        self.meshcat.SetSliderValue(self._X_DEFAULT.name, self._x)
        if not self.planar:
            self.meshcat.SetSliderValue(self._Y_DEFAULT.name, self._y)
        self.meshcat.SetSliderValue(self._Z_DEFAULT.name, self._z)

    def DoCalcOutput(self, context, output):
        # Update any internal state for sliders that were added depending on
        # self.planar by querying the meshcat UI.
        self._roll = self.meshcat.GetSliderValue(self._ROLL_DEFAULT.name)
        if not self.planar:
            self._pitch = self.meshcat.GetSliderValue(self._PITCH_DEFAULT.name)
            self._yaw = self.meshcat.GetSliderValue(self._YAW_DEFAULT.name)
        self._x = self.meshcat.GetSliderValue(self._X_DEFAULT.name)
        if not self.planar:
            self._y = self.meshcat.GetSliderValue(self._Y_DEFAULT.name)
        self._z = self.meshcat.GetSliderValue(self._Z_DEFAULT.name)

        # Set the output with the updated values.
        output.SetAtIndex(0, self._roll)
        output.SetAtIndex(1, self._pitch)
        output.SetAtIndex(2, self._yaw)
        output.SetAtIndex(3, self._x)
        output.SetAtIndex(4, self._y)
        output.SetAtIndex(5, self._z)
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            "Visualize the robot using the meshcat visualizer.  Note that "
            "the slider controls always show up in meshcat, this option "
            "additonally visualizes the scene in the same viewer."))
    parser.add_argument(

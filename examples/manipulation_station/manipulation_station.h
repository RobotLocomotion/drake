...
class ManipulationStation : public systems::Diagram<T> {
 public:
  ...
  /// manipulation station uses. The render engine instances will be removed
  /// from the given map. Calling this method with a nullptr with a pointer to
  /// an empty map is equivalent to calling Finalize(). See Finalize() for more
  /// details.
 ...
 private:
  ...
  static constexpr const char* default_renderer_name_ = "manip_station_renderer";

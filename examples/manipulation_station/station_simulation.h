...
/// @defgroup manipulation_station_systems Intelligent Robot Manipulation
/// Systems
/// @{
/// @brief Systems related to the Intelligent Robot Manpulation class examples.
...
/// to contact (note that they should also include the missing contributions
/// from the gripper fingers, but we do not have those yet).
...
class StationSimulation : public systems::Diagram<T> {
 public:
  ...
  /// MultibodyPlant<T>.
  StationSimulation(double timestep = 0.0002);
  ...
  /// Return a reference to the main plant responsible for the dynamics of the
  /// robot and the environment.  This can be used to, e.g., add additional
  /// elements into the world before calling Finalize().
  multibody::multibody_plant::MultibodyPlant<T>& get_multibody_plant(
      void) const {
    return *plant_;
  }

  /// Return a reference to the SceneGraph responsible for all of the
  ...
  geometry::SceneGraph<T>& get_scene_graph(void) const { return *scene_graph_; }
 ...
 private:
  ...
  std::unique_ptr<multibody::multibody_plant::MultibodyPlant<T>> plant_ptr_;
  std::unique_ptr<geometry::SceneGraph<T>> scene_graph_ptr_;

  // These are valid for the lifetime of this system.
  std::unique_ptr<multibody::multibody_plant::MultibodyPlant<T>>
      controller_plant_ptr_;

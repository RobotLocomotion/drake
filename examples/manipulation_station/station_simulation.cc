...
StationSimulation<T>::StationSimulation(double timestep)
    : plant_ptr_(std::make_unique<MultibodyPlant<T>>(timestep)),
      scene_graph_ptr_(std::make_unique<SceneGraph<T>>()),
      controller_plant_ptr_(std::make_unique<MultibodyPlant<T>>()) {
  ...
  plant_ = plant_ptr_.get();
  scene_graph_ = scene_graph_ptr_.get();
  ...
  const auto controller_iiwa_model =
      AddModelFromSdfFile(iiwa_sdf_path, "iiwa", controller_plant_ptr_.get());
  controller_plant_ptr_->WeldFrames(controller_plant_ptr_->world_frame(),
                                    controller_plant_ptr_->GetFrameByName(
  ...
  const multibody::RigidBody<T>& wsg_equivalent =
      controller_plant_ptr_->AddRigidBody("wsg_equivalent",
                                          controller_iiwa_model,
                                          wsg_body.default_spatial_inertia());
  controller_plant_ptr_->WeldFrames(controller_plant_ptr_->GetFrameByName(
...
void StationSimulation<T>::Finalize() {
  ...
  builder.AddSystem(std::move(plant_ptr_));
  builder.AddSystem(std::move(scene_graph_ptr_));
  ...
  {
    controller_plant_ptr_->Finalize();
    ...
    auto iiwa_controller = builder.template AddSystem<
        ...
        *controller_plant_ptr_, iiwa_kp, iiwa_ki, iiwa_kd, false);

...
void WeldGripperFrames(MultibodyPlant<T>* plant) {
  ...
  const double kFinger2Angle = M_PI;
  const double kFinger3Angle = -M_PI / 3.0;
...
std::pair<MatrixX<double>, std::map<std::string, int>> ParseKeyframes(
  ...
  // Set the brick's initial pose in the world frame W. Here we assume the
  // gripper frame G is aligned with the world frame W, e.g., as given by
  // calling WeldGripperFrames().
  // Check here to make sure this assumption holds.
  DRAKE_DEMAND(

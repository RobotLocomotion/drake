...
void WeldGripperFrames(MultibodyPlant<T>* plant) {
  ...
  const RigidTransformd X_WF2 =
      RigidTransformd(RollPitchYawd(2 * M_PI / 3, 0, 0),
                      Vector3d::Zero()) *
      X_WF1;
...
MatrixX<double> MakeKeyframes(
    MatrixX<double> all_keyframes, std::vector<std::string> joint_ordering,
  ...
  for (const auto& header_name : joint_ordering) {
    ...
    joint_name_to_col_index_map[header_name] =
        match_it - headers.begin();
  }
  // Now create the keyframe matrix.
  const int kKeyframeCount = all_keyframes.rows();
  const int kNumFingerJoints = joint_ordering.size();
  MatrixX<double> joint_keyframes(kKeyframeCount, kNumFingerJoints);
  for (int i = 0; i < kNumFingerJoints; ++i) {
    const std::string& joint_name = joint_ordering[i];
    const int all_keyframe_col_index =
        joint_name_to_col_index_map[joint_name];
    joint_keyframes.block(0, i, kKeyframeCount, 1) =
        all_keyframes.block(0, all_keyframe_col_index, kKeyframeCount, 1);
  }
...
std::pair<MatrixX<double>, std::map<std::string, int>> ParseKeyframes(
  ...
  if (line.size() == 0) {
    throw std::runtime_error(
        "Error reading keyframe input file. Cannot end in emtpy line(s).");
  } else if (file.eof()) {
    line_count++;  // Count the last real line, which contains the EOF.
  } else {
    // Did not find the EOF.
    throw std::runtime_error(
        "Error reading keyframe input file. EOF not found.");
  }
  // The keyframe count does not count the header line at the beginning of the
  // file.
  const int kKeyframeCount = line_count - 1;
  drake::log()->info("Found {} keyframes", kKeyframeCount);
  ...
  MatrixX<double> all_keyframes(kKeyframeCount, headers.size());
  int header_count;
  for (int i = 0; i < all_keyframes.rows(); ++i) {
    for (header_count = 0; header_count < all_keyframes.cols();
         ++header_count) {
      file >> all_keyframes(i, header_count);
    }
    DRAKE_DEMAND(header_count == kNumHeaders);
  }

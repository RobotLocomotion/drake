...
///
/// Users have the option to orient the gravity vector to point along the
/// -Z-axis, i.e., simulating the case when the planar-gripper is stood up
/// vertically, or have gravity point along the -X-axis, i.e., simulating the
/// case when the planar-gripper is laid down flat on the floor.
///
/// When simulating the vertical case (gravity acting along -Z) the position of
/// the brick along the X-axis is fixed and can be specified via the flag
/// `brick_floor_penetration`, which determines the amount by which the brick
/// geometry penetrates the floor, and hence, determines the amount of friction
/// introduced via brick/floor contact. To simulate zero contact between the
/// brick and the floor, set this flag to zero.
///
/// When simulating the horizontal case (gravity acting along -X) the brick is
/// additionally free to move along the X-axis and is subject to gravitational
/// as well as floor contact forces.
///
...
int DoMain() {
  ...
  // lies in the world Y-Z plane. Therefore, gravity can either point along the
  // -Z axis (vertical case), or -X-axis (horizontal case).
  ...
  Vector3<double> brick_initial_2D_pose;  // (y, z, theta) in the gripper frame.
  std::tie(keyframes, finger_joint_name_to_col_index_map) =
      ParseKeyframes(keyframe_path, &brick_initial_2D_pose);
  ...
  y_translate.set_translation(&plant_context, brick_initial_2D_pose(0));
  z_translate.set_translation(&plant_context, brick_initial_2D_pose(1));
  x_revolute.set_angle(&plant_context, brick_initial_2D_pose(2));

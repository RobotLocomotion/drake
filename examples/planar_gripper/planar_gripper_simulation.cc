...
int DoMain() {
  ...
  const ModelInstanceIndex gripper_index =
      Parser(&plant).AddAllModelsFromFile(full_name).at(0);
  ...
  const ModelInstanceIndex brick_index =
      Parser(&plant).AddAllModelsFromFile(brick_file_name).at(0);
  ...
  Parser(&control_plant).AddAllModelsFromFile(full_name);

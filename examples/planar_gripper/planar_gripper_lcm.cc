...
void GripperCommandEncoder::OutputCommand(
  ...
  std::vector<lcmt_planar_gripper_finger_command>& fcommand =
      command->finger_command;

  for (int i = 0; i < num_fingers_; ++i) {
    auto st_index = 2 * i;
    fcommand[i].joint_position[0] =
        state_input->GetAtIndex(st_index);
    fcommand[i].joint_position[1] =
        state_input->GetAtIndex(st_index + 1);
    fcommand[i].joint_velocity[0] =
        state_input->GetAtIndex(num_joints_ + st_index);
    fcommand[i].joint_velocity[1] =
        state_input->GetAtIndex(num_joints_ + st_index + 1);
    fcommand[i].joint_torque[0] =
        torque_input->GetAtIndex(st_index);
    fcommand[i].joint_torque[1] =
        torque_input->GetAtIndex(st_index + 1);
  }
}
...
systems::EventStatus GripperStatusDecoder::UpdateDiscreteState(
  ...
  const std::vector<lcmt_planar_gripper_finger_status>& fstatus =
      status.finger_status;
  for (int i = 0; i < status.num_fingers; ++i) {
    const int st_index = 2 * i;
    positions(st_index) = fstatus[i].joint_position[0];
    positions(st_index + 1) = fstatus[i].joint_position[1];
    velocities(st_index) = fstatus[i].joint_velocity[0];
    velocities(st_index + 1) = fstatus[i].joint_velocity[1];
    tip_forces(st_index) = fstatus[i].fingertip_force.fy;
    tip_forces(st_index + 1) = fstatus[i].fingertip_force.fz;
  }
...
GripperStatusEncoder::GripperStatusEncoder(int num_fingers)
  ...
  force_input_port_ = &this->DeclareInputPort(
      "fingertip_force", systems::kVectorValued, num_fingers_ * 2);
...
void GripperStatusEncoder::OutputStatus(
  ...
  std::vector<lcmt_planar_gripper_finger_status>& fstatus =
      status->finger_status;
  for (int i = 0; i < num_fingers_; ++i) {
    const int st_index = 2 * i;
    fstatus[i].joint_position[0] = state_value(st_index);
    fstatus[i].joint_position[1] = state_value(st_index + 1);
    fstatus[i].joint_velocity[0] =
        state_value(num_joints_ + st_index);
    fstatus[i].joint_velocity[1] =
        state_value(num_joints_ + st_index + 1);
    fstatus[i].fingertip_force.timestamp =
        static_cast<int64_t>(context.get_time() * 1e3);
    fstatus[i].fingertip_force.fy = force_value(st_index);
    fstatus[i].fingertip_force.fz = force_value(st_index + 1);

    // For the planar gripper, these are all zero.
    fstatus[i].fingertip_force.fx = 0;
    fstatus[i].fingertip_force.tx = 0;
    fstatus[i].fingertip_force.ty = 0;
    fstatus[i].fingertip_force.tz = 0;
  }
}

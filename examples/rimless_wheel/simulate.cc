...
DEFINE_bool(gtest_color, false, "only so CLion will debug");
...
/// Note: Currently the simulation results in the wheel falling through the
/// ground as it approaches the standing fixed point to numerical precision.
/// (the basin of attraction of that fixed point is given explicitly in
/// http://underactuated.csail.mit.edu/underactuated.html?chapter=simple_legs).
/// This is being discussed in #4447.
int DoMain() {
  ...
  // any collisions).  This test fails at the standing fixed point (#4447).

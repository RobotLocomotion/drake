...
void RimlessWheel<T>::StepForwardReset(
  ...
  // small).  So post state should be just above the ground.  This is
  // important so that the witness functions get evaluated without penetration
  // at least once.
  ...
  std::cout << next_state.thetadot() << std::endl;
  if (next_state.thetadot() < 0.02) {
    bool& double_support = get_mutable_double_support(state);
    double_support = true;
    next_state.set_thetadot(0.0);
    std::cout << "double support (forward)" << std::endl;
  }
}
...
void RimlessWheel<T>::StepBackwardReset(
  ...
  if (next_state.thetadot() > -0.02) {
    ...
    double_support = true;
    next_state.set_thetadot(0.0);
    std::cout << "double support (backward)" << std::endl;
  }
}

...
void RimlessWheel<T>::StepForwardReset(
  ...
  // small).  The post-impact state needs to be above the ground, so that a
  // least one witness function evaluation after the impact evaluates as
  // positive (because we have declared the WitnessFunction with

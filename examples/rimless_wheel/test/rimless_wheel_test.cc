...
GTEST_TEST(RimlessWheelTest, FixedPointTest) {
  const RimlessWheel<double> rw;
  auto context = rw.CreateDefaultContext();
  auto derivatives = rw.AllocateTimeDerivatives();

  RimlessWheelState<double>& state = rw.get_mutable_state(context.get());
  const RimlessWheelParams<double>& params = rw.get_parameters(*context);
  const RimlessWheelState<double>& rw_derivatives = rw.get_state(*derivatives);
  const double alpha = rw.calc_alpha(params);

  // Front foot down.
  state.set_theta(params.slope() + alpha);
  state.set_thetadot(0.0);
  rw.CalcTimeDerivatives(*context, derivatives.get());
  EXPECT_EQ(rw_derivatives.theta(), 0.0);
  EXPECT_EQ(rw_derivatives.thetadot(), 0.0);

...
class RimlessWheelGeometry final : public systems::LeafSystem<double> {
 public:
  ...
  /// `builder`.  Both the `rimless_wheel_floating_base_state_port.get_system()`
  ...
  static const RimlessWheelGeometry* AddToBuilder(
      ...
      const systems::OutputPort<double>&
          floating_base_state_port,
      const RimlessWheelParams<double>& rimless_wheel_params,
      geometry::SceneGraph<double>* scene_graph);

  /// Creates, adds, and connects an RimlessWheelGeometry system into the given
  /// `builder`.  Both the `rimless_wheel_state_port.get_system()` and

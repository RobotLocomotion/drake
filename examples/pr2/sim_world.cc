...
SimWorld<T>::SimWorld(const std::vector<std::string>& robot_names)
    : robot_names_(robot_names),
      owned_plant_(std::make_unique<MultibodyPlant<T>>(
          pr2::parameters::sim_flags().time_step)),
...
template <typename T>
const pr2::parameters::RobotParameters SimWorld<T>::LoadRobotParameters(
    const std::string& robot_name) const {
  pr2::parameters::RobotParameters robot_parameters;
  ...
  DRAKE_DEMAND(pr2::parameters::ReadParametersFromFile(
...
void SimWorld<T>::SetupSimWorld() {
  const auto& sim_flags = pr2::parameters::sim_flags();
  // Add a ground plane.
  {
...
void SimWorld<T>::MakeRobotControlPlants() {
  ...
  for (const auto& [robot_name, robot_parameters] : robots_parameters_) {
    OwnedRobotControllerPlant owned_plants(
        pr2::parameters::sim_flags().time_step);
    const pr2::common::ModelInstanceInfo model_info =

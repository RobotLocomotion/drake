...
class SimWorld : public systems::Diagram<T> {
 ...
 private:
  ...
  // does not exists.
  const pr2::parameters::RobotParameters LoadRobotParameters(
      const std::string& robot_name) const;
  ...
  std::map<std::string, pr2::parameters::RobotParameters> robots_parameters_;

  std::map<std::string, pr2::common::ModelInstanceInfo> items_instance_info_;

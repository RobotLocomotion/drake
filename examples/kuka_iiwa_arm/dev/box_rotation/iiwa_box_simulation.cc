...
int DoMain() {
  ...
  const std::map<std::string, geometry::FrameId>& frame_name_to_id_map =
      pose_tracker->get_tracked_frame_ids();
  frame_map[frame_name_to_id_map.at("left_iiwa_base")] =
      std::pair<std::string, int>("left_iiwa_base", 1);
  frame_map[frame_name_to_id_map.at("right_iiwa_base")] =
      std::pair<std::string, int>("right_iiwa_base", 2);
  frame_map[frame_name_to_id_map.at("box")] =
      std::pair<std::string, int>("box", 3);

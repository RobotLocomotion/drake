...
void KukaTorqueController<T>::SetUp(const VectorX<double>& kp,
  ...
  */
  ...
  auto vel_desired = builder.template AddSystem<ConstantVectorSource<T>>(
  ...
  builder.Connect(vel_desired->get_output_port(), mux->get_input_port(1));
  ...
  // builder.Connect(vel_desired->get_output_port(), adder->get_input_port(0));

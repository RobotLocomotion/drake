...
class StateDependentDamper : public LeafSystem<T> {
 public:
  StateDependentDamper(const RigidBodyTree<T>& tree, const VectorX<double>& kp,
                       const VectorX<double>& damping_ratio)
      : tree_(tree), kp_(kp), damping_ratio_(damping_ratio) {
    const int num_q = tree_.get_num_positions();
    const int num_v = tree_.get_num_velocities();
    const int num_x = num_q + num_v;

    DRAKE_DEMAND(kp.size() == num_v);
 ...
 private:
  ...
  void CalcTorque(const Context<T>& context, BasicVector<T>* torque) const {
    ...
    // // Compute critical damping gains and scale by damping ratio.
    auto cache = tree_.doKinematics(q);
    Eigen::ArrayXd temp =
        tree_.massMatrix(cache).diagonal().array() * kp_.array();
...
void KukaTorqueController<T>::SetUp(const VectorX<double>& kp,
  ...
  */
  ...
  // Connects the gravity comp and spring and damper torques to the adder.

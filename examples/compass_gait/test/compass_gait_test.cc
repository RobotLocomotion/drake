...
GTEST_TEST(CompassGaitTest, TestEnergyConservedInSwing) {
  ...
  const Expression energy_dot =
      energy.Jacobian(get_variable_vector(state.CopyToVector())) *
  ...
  EXPECT_NEAR(energy_dot.Evaluate(e), 0.0, 1e-12);
}
...
double CalcAngularMomentum(const CompassGait<double>& cg,
  ...
  const auto my_cross = [](const Eigen::Vector2d& a, const Eigen::Vector2d&
  b) {

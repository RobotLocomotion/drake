...
T CompassGait<T>::FootCollision(const systems::Context<T>& context) const {
  ...
  // (the true height scaled by 1/length_leg).
  const T swing_foot_height = cos(cg_state.stance() - params.slope()) -
                              cos(cg_state.swing() - params.slope());
  ...
  using std::max;
  return max(swing_foot_height, cg_state.swing() - cg_state.stance());
}

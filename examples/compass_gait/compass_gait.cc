...
T CompassGait<T>::FootCollision(const systems::Context<T>& context) const {
  ...
  // Want swing - slope = - (stance - slope), crossing from stance > swing.
  // (Geometrically, the biped forms an isosceles triangle with the ramp).
  const T collision =
...
void CompassGait<T>::CollisionDynamics(
  ...
  // The collision update is performed by projecting up to the (planar)
  // floating-base coordinates (with the origin at the pre-collision stance
  // toe), implementing the collision, and projecting back to to the minimal
  // coordinates. Mathematica derivation is here:

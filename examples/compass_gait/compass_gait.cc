...
T CompassGait<T>::DoCalcKineticEnergy(
  ...
  return .5 * (mh * l * l + m * a * a) * vst * vst +
         ...
         m * l * b * vst * vsw * cos(cg_state.stance() - cg_state.swing());
}
...
T CompassGait<T>::FootCollision(const systems::Context<T>& context) const {
  ...
  // Want swing - slope = - (stance - slope).
  // Crossing from stance' > swing'
  const T collision =
      2. * params.slope() - cg_state.swing() - cg_state.stance();

  // Only trigger when swing < stance (e.g. forward step), by taking max with
  // swing()-stance().
  using std::max;
...
void CompassGait<T>::CollisionDynamics(
  ...
  const T alpha = cg_state.stance() - cg_state.swing();
  const T c = cos(alpha);
  ...
  // Confirm that the post impact (pre-)swing toe velocity is actually zero.
  DRAKE_ASSERT((J * v_post).isZero(1e-12));
  ...
  // toe += step length.
  toe += 2. * params.length_leg() * sin(alpha / 2.);

  // Switch stance foot from left to right (or back).
  bool& left_stance = get_mutable_left_stance(state);
...
void CompassGait<T>::FloatingBaseStateOut(
  ...
  const bool left_stance = get_left_stance(context);
...
Matrix2<T> CompassGait<T>::MassMatrix(
  ...
  const T b = params.center_of_mass_leg();
  const T l = params.length_leg();
  const T c = cos(cg_state.stance() - cg_state.swing());
...
void CompassGait<T>::DoCalcTimeDerivatives(
  ...
  Vector4<T> xdot;
  xdot << cg_state.stancedot(), cg_state.swingdot(),
      -M.inverse() * bias;

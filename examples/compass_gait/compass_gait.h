...
/// Discrete States: stance toe position, left support indicator.
/// Outputs:
///   0) continuous (minimal coordinates) state output
...
class CompassGait final : public systems::LeafSystem<T> {
 public:
  ...
  /// Return reference to the output port that publishes only
  /// [theta_stance, theta_swing, thetatdot_stance, thetadot_swing].
  const systems::OutputPort<T>& get_minimal_state_output_port() {
    return this->get_output_port(0);
  }

  /// Returns reference to the output port that provides a the state output
  /// for CompassGait.urdf (as instantiated with
  /// FloatingBaseType::kRollPitchYaw).
  const systems::OutputPort<T>& get_floating_base_state_output_port() {
    return this->get_output_port(1);
  }

  /// Access the CompassGaitContinuousState.
  ...
  /// Access the mutable CompassGaitContinuousState.
  ...
  static bool get_left_stance(const systems::Context<T>& context) {
    return context.template get_abstract_state<bool>(0);
  }

  static bool& get_mutable_left_stance(systems::State<T>* state) {
  ...
  ///@{
  /// Manipulator equation of CompassGait: M(q)q̈ + bias(q,q̇̇) = B*u.
  /// - M[2x2] is the mass matrix.
  /// - bias[2x1] includes the Coriolis term and gravity term,
  ///     i.e. bias[2x1] = C(q,v)*v - τ_g(q).
 ...
 private:
  ...
  // Check when the foot hits the ramp (with a sufficiently large step length).
  T FootCollision(const systems::Context<T>& context) const;

  // Handles the impact dynamics, including reseting the stance and swing legs.

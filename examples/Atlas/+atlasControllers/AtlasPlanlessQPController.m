classdef AtlasPlanlessQPController < MIMODrakeSystem
  ...
  function varargout=mimoOutput(obj,t,~,varargin)
    ...
    supp = ctrl_data.support;
    support_mask = ([supp.bodies] ~= r.foot_body_id.right) | fc(2);
    support_mask = support_mask & (([supp.bodies] ~= r.foot_body_id.left) | fc(1));
    active_supports = supp(support_mask);
    ...
    if (obj.use_mex==0 || obj.use_mex==2)
      ...
      num_active_contacts = sum([supp.num_contact_pts]);
      ...
      if ~isempty(supp)
        ...
        for j=1:length(active_supports)
          [~,~,JB] = contactConstraintsBV(r,kinsol,false,struct('terrain_only',~obj.use_bullet,...
            'body_idx',[1,active_supports(j).bodies],'collision_groups',active_supports(j).collision_groups));
        ...
        terrain_pts = getTerrainContactPoints(r,active_supports,{active_supports.contact_groups});

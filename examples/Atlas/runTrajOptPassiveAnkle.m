function [p,xtraj,utraj,ltraj,ljltraj,z,F,info,traj_opt] = runTrajOptPassiveAnkle(xtraj,utraj,ltraj,ljltraj,scale)
...
if nargin < 2
  ...
  traj_init.u = PPTrajectory(foh(t_init,randn(5,N)));
...
lz_bound = BoundingBoxConstraint(.3*ones(2*(N - 2*d),1),inf(2*(N - 2*d),1));

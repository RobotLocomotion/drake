...
DEFINE_double(
    ...
    "system, and the value specifies the fixed-time step period (in seconds) "
    "of discrete updates for the plant. If this value is zero, the plant is "
    "modeled as a continuous system. [s].");
...
int do_main() {
  ...
  if (plant.is_discrete()) {
    // If the plant used discrete steps, output the step size and count.
    fmt::print("Discrete time stepping with dt = {} s\n", plant.time_step());

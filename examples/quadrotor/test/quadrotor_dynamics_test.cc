...
//
// Both types also provide GetState.
...
class GenericQuadrotor: public Diagram<double> {
 public:
  ...
  RigidTransformd GetState(const Context<double>& context) const {
...
class MultibodyQuadrotor: public Diagram<double> {
 public:
  ...
  void SetState(Context<double>* context, const VectorXd& x) const {
    ...
    const RigidTransformd X_WB(RollPitchYawd(rpy), xyz);
...
void TestPassiveBehavior(const VectorXd& x0) {
  ...
  const RigidTransformd ge_pose = ge_model.GetState(ge_context);
  const RigidTransformd mb_pose = mb_model.GetState(mb_context);
  EXPECT_TRUE(CompareMatrices(ge_pose.GetAsMatrix34(),
                              mb_pose.GetAsMatrix34(),
                              1e-10 /* tolerance */,

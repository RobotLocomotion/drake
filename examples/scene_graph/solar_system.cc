...
void SolarSystem<T>::AllocateGeometry(SceneGraph<T>* scene_graph) {
  ...
  scene_graph->RegisterGeometry(
      ...
      make_unique<GeometryInstance>(X_OeE, make_unique<Sphere>(0.25f), "Earth",
  ...
  // The geometry is displaced from Luna's orbital frame so that it orbits.
  ...
  scene_graph->RegisterGeometry(
      ...
      make_unique<GeometryInstance>(
          X_OlL, make_unique<Box>(0.15, 0.15, 0.15),
          "boxsat", VisualMaterial(Vector4d(1, 0, 1, 1))));

  // Mars's oribtal frame Om lies directly *below* the sun (to account for the
  // orrery arm).
  ...
  GeometryId mars_geometry_id = scene_graph->RegisterGeometry(
      source_id_, planet_id, make_unique<GeometryInstance>(
                                 X_OmM, make_unique<Sphere>(kMarsSize), "Mars",
  ...
  scene_graph->RegisterGeometry(
      ...
      make_unique<GeometryInstance>(
          X_MR, make_unique<Mesh>(rings_absolute_path, kMarsSize), "MarsRings",
  ...
  // So, X_OmOp = X_OmM. The normal of the plane is negated so it orbits in th
  // opposite direction.

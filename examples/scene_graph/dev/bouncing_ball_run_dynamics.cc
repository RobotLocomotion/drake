...
DEFINE_bool(disable_camera, false, "When defined, cameras are disabled");
DEFINE_bool(no_color, false, "When defined no color image is rendered");
DEFINE_bool(no_depth, false, "When defined no depth image is rendered");
DEFINE_bool(no_label, false, "When defined no label image is rendered");
...
int do_main() {
  ...
  if (!FLAGS_disable_camera) {
    ...
    if (!(FLAGS_no_color && FLAGS_no_depth && FLAGS_no_label)) {
      ...
      std::cout << "Rendering at " << FLAGS_render_fps << " fps\n";

...
DEFINE_bool(real_time, false, "Set to true to run no faster than realtime");
...
class MovingBall final : public LeafSystem<double> {
 ...
 private:
  ...
  void CalcFramePoseOutput(
    ...
    auto pos_z = context.get_continuous_state().get_vector()[0];
 ...
 private:
  void CalcContactResults(const Context<double>& context,
    ...
    for (int i = 0; i < num_contacts; ++i) {
      ...
      surface_msg.body1_name = "Id " + to_string(contacts[i].id_M());
      surface_msg.body2_name = "Id " + to_string(contacts[i].id_N());

...
def monitor_large_energy_delta(simulator, plant, max_energy_gain=10000.0):
    # Without regulating translation, the instantaneous rotational velocity
    # seems "hard" to compute as zero for articulated bodies. Do a hacky check
    # to ensure we don't "explode" on energy in a general way.
    ...
    def monitor(diagram_context):
        ...
        if energy_delta > max_energy_gain:
            t = context.get_time()
            raise RuntimeError(f"Too much energy gained at t={t}")
...
class Test(unittest.TestCase):
    def run_floating(
        self, time_step, *, kd, solver=None, max_energy_gain=0.0,
    ):
        ...
        if max_energy_gain is not None:
            monitor_large_energy_delta(simulator, ns.plant, max_energy_gain)
    ...
    def run_articulated(
        self, time_step, *, kd, solver=None, max_energy_gain=0.01,
    ):
        print((time_step, kd, solver))
        ns = SimpleNamespace()
        add_basic_simulation_components(ns, time_step, solver)
    ...
    def test_floating(self):
        ...
        self.run_floating(time_step_unstable, kd=kd_stable, solver="sap")
        self.run_floating(time_step_unstable, kd=kd_stable, solver="tamsi")
        self.run_floating(time_step_continuous, kd=kd_stable)
        ...
        self.run_floating(time_step_stable, kd=kd_unstable, solver="sap")
        self.run_floating(time_step_stable, kd=kd_unstable, solver="tamsi")
        # - Continuous is good (but "stiff").
        self.run_floating(time_step_continuous, kd=kd_unstable)
    ...
    def test_articulated(self):
        ...
        # (kd_stable, time_step_unstable): Things are happy.
        self.run_articulated(time_step_unstable, kd=kd_stable, solver="sap")
        self.run_articulated(time_step_unstable, kd=kd_stable, solver="tamsi")
        self.run_articulated(time_step_continuous, kd=kd_stable)
        ...
        with self.assert_raises_message(["Too much energy gained"]):
            ...
            self.run_articulated(
                time_step_unstable, kd=kd_unstable, solver="sap",
            )
        with self.assert_raises_message(
        ...
        self.run_articulated(time_step_continuous, kd=kd_unstable)

        # Using same gain but decreasing time step makes it stable.
        self.run_articulated(time_step_stable, kd=kd_unstable, solver="sap")
        self.run_articulated(time_step_stable, kd=kd_unstable, solver="tamsi")

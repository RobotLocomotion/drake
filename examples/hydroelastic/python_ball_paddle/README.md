...
![ball_paddle_near_edge_gif](https://media.giphy.com/media/m0Is4ANaiIOASOuKYv/giphy-downsized-large.gif)

## Drop the ball near the paddle's corner

We specify even lower `target_realtime_rate` so that we can see it easier.
The initial height of the ball is also lower,
so it takes less time to make contact at a corner of the paddle.

```
bazel run //examples/hydroelastic/python_ball_paddle:contact_sim_demo_py -- \
--ball_initial_position 0.102 0.102 0.05 \
--target_realtime_rate=0.01 \
--simulation_time=0.18
```

![ball_paddle_corner](images/ball_paddle_corner.jpg)
![ball_paddle_corner_gif](https://media.giphy.com/media/ctP9EsmMqrSoP3WtrF/giphy.gif)

...
int do_main() {
  ...
  // The simulation can be (depending on parameter settings) very fast and
  // short. We want to see every time step in the simulation, so we set the
  // publishing period to be the same as the size of time step.
  const double publish_period = FLAGS_mbp_dt;
  ConnectContactResultsToDrakeVisualizer(&builder, plant, scene_graph,
                                         /* lcm */ nullptr, publish_period);

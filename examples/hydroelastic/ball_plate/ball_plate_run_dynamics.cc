...
DEFINE_double(hydroelastic_modulus, 3.0e4,
              "Hydroelastic modulus of both the ball and the floor, [Pa].");
...
DEFINE_double(resolution_hint_factor, 0.3, "resolution hint.");
...
int do_main() {
  ...
  if (FLAGS_hydro_rep == "tri") {
    ...
    throw std::runtime_error(
        "Invalid contact-surface representation in "
        "hydroelastics: `" +
        FLAGS_hydro_rep + "'.");
  }

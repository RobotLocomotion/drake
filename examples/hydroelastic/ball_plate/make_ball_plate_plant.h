...
/// plate. MultibodyPlant models the contact of the ball with the floor as an
...
///   This factory method will register the return multibody plant to be a
///   source for the geometry system of `scene_graph`.
///
/// @note The MultibodyPlant model is not finalized. You must call Finalize() on
/// the new model once you are done creating it.
///
/// @pre `scene_graph` is not null.
///
/// See also
...
std::unique_ptr<drake::multibody::MultibodyPlant<double>> MakeBallPlatePlant(
    ...
    const Vector3<double>& gravity_W, double resolution_hint_factor,

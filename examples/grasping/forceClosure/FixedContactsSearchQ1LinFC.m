classdef FixedContactsSearchQ1LinFC < FixedContactsSearchQ1Base
  ...
  properties(SetAccess = protected)
    ...
    monomial1 % The monomial vector of the Lagrangian multiplier, for constraint a^T*Vij+b >=0
    monomial2 % The monomial vector of the Lagrangian multiplier, for constraint a'*inv(Qw)*a= s^2
    
    L1_mat % A num_contacts x num_fc_edges cell, L1_mat{i,j} is the monomial matrix of the Lagrangian multiplier, for constraint a^T*V_ij+b>=0
    L2_mat % The monomial matrix of the Lagrangian multiplier, for constraint a'*inv(Qw)*a = s^2
  ...
  methods
    function obj = FixedContactsSearchQ1LinFC(disturbance_pos,num_contacts,num_fc_edges,Qw)
      ...
      obj.L1_mat = cell(obj.num_contacts,obj.num_fc_edges);
      [obj, obj.L2_mat] = obj.newSym(length(obj.monomial2));
      for i = 1:obj.num_contacts
        for j = 1:obj.num_fc_edges
          [obj, obj.L1_mat{i,j}] = obj.newSym(length(obj.monomial1));
          obj = obj.withPSD(obj.L1_mat{i,j});
        end
      end
      [obj,obj.radius] = obj.newFree(1,1);
      obj = obj.withPos(obj.radius);
...
%           radius_lb = max(radius_lb,radius_min);
    ...
    function sol = retrieveSolution(obj,solver_sol)
      ...
      sol.L1_mat = cell(obj.num_contacts,obj.num_fc_edges);
      sol.L2_mat = double(solver_sol.eval(obj.L2_mat));
      for i = 1:obj.num_contacts
        for j = 1:obj.num_fc_edges
          sol.L1_mat{i,j} = double(solver_sol.eval(obj.L1_mat{i,j}));
        end
      end
    end
  end
  ...
  methods(Access = protected)
    function V = sosQ1condition(obj,m_L0_mat,friction_cones,radius)
      ...
      V = (obj.monomial0'*m_L0_mat*obj.monomial0)*(obj.b_indet - radius) - obj.monomial2'*obj.L2_mat*obj.monomial2*(1-obj.a_indet'*inv(obj.Qw)*obj.a_indet);
      ...
      for i = 1:obj.num_contacts
        ...
        for j = 1:obj.num_fc_edges
          V = V - obj.monomial1'*obj.L1_mat{i,j}*obj.monomial1*(Vi(:,j)'*obj.a_indet + obj.b_indet);
        end
      end
    end
  end

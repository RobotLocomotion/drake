...
DEFINE_bool(with_reflected_inertia, true,
            "If true, it models the effect of reflected inertia due to the very"
            "high gear ratio at the actuator of each finger.");
DEFINE_double(
    pid_frequency, 10.0,
...
void DoMain() {
  ...
  const double rotor_inertia = FLAGS_with_reflected_inertia ? 1.0e-6 : 0.0;
  // Gear ratio, dimensionless.
  const double gear_ratio = FLAGS_with_reflected_inertia ? 369 : 1.0;

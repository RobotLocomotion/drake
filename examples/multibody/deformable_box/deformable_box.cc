...
DEFINE_double(beta, 0.005,
              "Stiffness damping coeffcient for the deformable body [1/s].");
...
namespace {
/* We create a leaf system that uses PD control to output force signal to
 gripper to follow a close-lift-open motion sequence. The signal is
 2-dimensional with the first corresponding to the wrist degree of freedom and
 the second corresponding to the left finger degree of freedom. This control is
 a time-based state machine, where forces change based on the context time. This
 is strictly for demo purposes and is not intended to generalize to other cases.
 ...
 private:
  void SetAppliedForce(const Context<double>& context,
    ...
    if (t < fingers_closed_time_) {
      const double T = fingers_closed_time_;
      const double theta = t / T;
      desired_positions =
          theta * closed_state_ + (1.0 - theta) * initial_state_;
    } else if (t < gripper_lifted_time_) {
      const double T = gripper_lifted_time_ - fingers_closed_time_;
      const double theta = (t - fingers_closed_time_) / T;
      desired_positions = theta * lifted_state_ + (1.0 - theta) * closed_state_;
    } else if (t < fingers_open_time_) {
      const double T = fingers_open_time_ - gripper_lifted_time_;
      const double theta = (t - gripper_lifted_time_) / T;
...
int do_main() {
  ...
  /* Minimumly required proximity properties for rigid bodies to interact with
   deformable bodies.
   1. A valid Coulomb friction coefficient, and
   2. A resolution hint. */
  ProximityProperties rigid_proximity_props;
  const CoulombFriction<double> surface_friction(1.5, 1.5);
  ...
  /* Load the geometry and scale it up by 20%. */
  ...
  builder.Connect(
      deformable_model_ptr->vertex_positions_port(),

...
Vector3<double> PointSourceForceField::DoEvaluateAt(
  ...
  const double max_value =
      maximum_force_input_port().HasValue(context)
          ? maximum_force_input_port().Eval<systems::BasicVector<double>>(
                context)[0]
          : 0.0;
...
void PointSourceForceField::DoDeclareInputPorts(MultibodyPlant<double>* plant) {
  maximum_force_port_index_ =
      this->DeclareVectorInputPort(plant, "maximum force magnitude in Newtons",

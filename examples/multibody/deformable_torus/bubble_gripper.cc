...
DEFINE_double(time_step, 2.5e-2,
...
int do_main() {
  ...
  /* Add a renderer to render the inside dot pattern of the bubble gripper.
   Currently, deformable rendering is only supported by RenderEngineGl. */
  ...
  /* Avoid rendering the ground as it clutters the background.
   Currently, all visual geometries added through MultibodyPlant is automatcally
   assigned perception properties. When that automatic assignment is no longer
   done, we can remove this and simply not assign a perception property. */

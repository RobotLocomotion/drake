...
closed kinematic loops. It shows:
  - How to define a four bar linkage the obvious way -- as a loop formed by four
    revolute joints connecting the links. The model is built using the C++ API.
  - How to opt in to automatic loop modeling with
    `MultibodyPlant::SetEnableLoopTopology()`.
  - How to use the existing solvers to perform assembly analysis.
  - How to visualize the underlying model Drake builds to represent the
    linkage (for pedagogical purposes, not needed normally).

Closing the loop takes a constraint, so this model requires a solver that
supports constraints. It runs in discrete mode under SAP by default, and
continuous under CENIC if requested:
    --time_step=0 --simulator_integration_scheme=cenic

Compare four_bar.cc, which reads the linkage from an
SDFormat file already assembled, and four_bar_with_bushing.cc, which
models the same kind of mechanism with one joint left out and replaced by a
LinearBushingRollPitchYaw force element.
...
the shadow copy's link frame to this one). All these frames are aligned
with World as drawn.
...
// those points. Everything is drawn in the y = 0 plane the mechanism moves in,
// so links do overlap on screen where they are joined; the bars are drawn a
// little short of their pivots to keep that overlap small.
...
// coupler's link frame at Co and its shadow's matching link frame. They are
// drawn identically so that they stand out against the blues around them
// and so they read as a single frame once the weld has brought them together.
...
std::pair<RigidTransformd, Box> MakeBar(const Vector3d& p_LStart, double length,
  ...
  // slimmed bar still runs exactly as far as the one it is split from.
...
// stay together at all times, even in the unassembled configuration: no joint
// is ever broken in a joint coordinate multibody code like Drake. Breaking a
// loop splits a LINK instead, dividing the joints that link carries between the
// primary and its shadow; see DrawShadowLink().
...
// Draws `shadow` as a faint copy of the coupler's own blue: a slightly
// slimmer bar of `length` running along `direction`. A split link's joints
// are divided up between the primary and shadow links -- here
// the coupler keeps the joint at its Cd end, the driver_coupler, while the
// joint at its Cr end, the coupler_rocker, moved over to the shadow.
//
// MultibodyPlant::RegisterVisualGeometry() can't do this job because it is a
...
// that Finalize() registered for the shadow. Note that we can reuse the
// primary's geometry poses verbatim, because a shadow link's geometry (just
// frame poses) mimics its primary's by construction -- when assembled, the
// shadow's link frame will be coincident with its primary's link frame.
void DrawShadowLink(const MultibodyPlant<double>& plant,
  ...
  // The bar, and then the one joint the shadow carries: the parent end at the
  ...
  // Finally a triad at the shadow's own origin, the mate of the one drawn at
  // the coupler's Co. Unlike the bar, this is NOT slimmed and does not take the
...
void BuildFourBarLinkage(MultibodyPlant<double>* plant) {
  ...
  // Co at the middle rather than at either end. Note the matching change of
  // inertia -- a rod about its center of mass, not about one of its ends.
  ...
  // this loop. This is the joint that ends up on the shadow, so its parent pin
  // is drawn there instead -- see DrawShadowLink(). The coupler keeps the joint
  // at its Cd end, the driver_coupler above.
  DrawChildPin(Rc, yellow, "coupler_rocker", plant);

  // Draw Co, the coupler's own link frame, at the middle of the bar.
  DrawFrame(Co, WeldFrameColor(), "Co", plant, 2.0);

  // Draw Cr as well, on the coupler.
  DrawFrame(Cr, blue, "Cr", plant);
}

// Returns the loop closure error, in meters. The two frames we named for the
// now-retargeted joint are not held together by the joint any more, and the
// distance between them is the loop closure error: large in the unassembled
// starting configuration, and driven to (nearly) zero as the model assembles.
...
// Records one configuration the linkage passed through on its way to being
// assembled, along with the loop closure error there. Assemble() records these
// so that ReplayAssembly() can show them; see the comment there for why they
// have to be recorded and replayed rather than just watched as they happen.
struct AssemblyStep {
  VectorXd q;
  double error{};
};
...
//
// Note that nothing here solves the loop closure equations. The solver pulls
// the two halves of the split link together over the first few steps, and all
// this function does is step until they have arrived. The driver link is
// locked. Every unlocked joint moves as the loop closes, and the driver would
// otherwise arrive at whatever angle the transient left it at.
//
...
double Assemble(const systems::Diagram<double>& diagram,
  ...
  // would otherwise miss the starting configuration.
  ...
  // Release the driver link now that the linkage is assembled, so that the
  // whole mechanism is free to move.
  ...
  // and with the clock a few milliseconds in. Zero both.
  four_bar.SetVelocities(four_bar_context,
                         VectorXd::Zero(four_bar.num_velocities()));
  simulator->get_mutable_context().SetTime(0.0);

  // Initialize() is required after changing the time.
...
// configuration. Does nothing if `duration` is not positive. We have to slow
// this down to illustrate the assembly since the solvers assemble in just a few
// milliseconds. The playback timeline is openly cosmetic, and built so as not
// to depend on how many steps the solver took or how they were spaced: frames
// are placed by how much loop closure error is left rather than by step, so the
// two halves of the split link close on each other steadily and the replay
// looks the same whichever solver produced it.
...
int do_main() {
  ...
  // below would throw: this model's joints form a loop.
  ...
  // We are done defining the model. Splitting a link, retargeting a joint,
  // and adding a weld happens here.
  four_bar.Finalize();
  ...
  // since it did not exist until Finalize(). The modeler's "equal branch
  // lengths" heuristic means the central link (the coupler) is the one that
  // gets split in this model, so it is the coupler's geometry we copy.
  ...
  // the figure at the top of this file.
  ...
  ShowAndWait(*diagram, simulator.get(),
              "The linkage is unassembled: the coupler and its pale shadow "
  ...
  ShowAndWait(*diagram, simulator.get(),
              ...
              "coupler it was split from, together reconstituting the original "
              "link's properties.",
              "simulate");
  ...
  std::cout << fmt::format(
      ...
      CalcLoopClosureError(
          four_bar, four_bar.GetMyContextFromRoot(simulator->get_context())));

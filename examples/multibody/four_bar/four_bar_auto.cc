...
void DrawShadowLink(const MultibodyPlant<double>& plant,
  ...
  // Finally a triad at the shadow's own origin, the twin of the one drawn at
  // the coupler's Co. The pose is identity because a shadow's link frame
  // coincides with its primary's by construction. Unlike the bar, this is NOT
  // slimmed and does not take the shadow's pale blue: it is drawn exactly like
  // the coupler's Co triad, so that when the weld is satisfied the two become
  // one orange frame.
  for (int axis = 0; axis < 3; ++axis) {

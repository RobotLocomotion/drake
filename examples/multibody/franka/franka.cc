...
// TODO(jwnimmer-tri) Really, this should be part of the systems/analysis
// library so it can be used by all examples, etc.
//
/* Implicit euler integrator config.
Only used when integration_scheme is "implicit_euler". */
struct ImplicitEulerConfig {
  template <typename Archive>
  void Serialize(Archive* a) {
    a->Visit(DRAKE_NVP(jacobian_scheme));
    a->Visit(DRAKE_NVP(full_newton));
    a->Visit(DRAKE_NVP(trapezoid));
  }

  std::string jacobian_scheme{"forward"};
  bool full_newton{false};
...
struct Config {
  ...
  void Serialize(Archive* a) {
    ...
    a->Visit(DRAKE_NVP(implicit_euler_config));
  ...
  ImplicitEulerConfig implicit_euler_config;
...
int do_main() {
  ...
  if (config.simulator_config.integration_scheme == "implicit_euler") {
    ...
    if (config.implicit_euler_config.jacobian_scheme == "forward") {
      ie.set_jacobian_computation_scheme(
          JacobianComputationScheme::kForwardDifference);
    } else if (config.implicit_euler_config.jacobian_scheme == "central") {
      ie.set_jacobian_computation_scheme(
          JacobianComputationScheme::kCentralDifference);
    } else if (config.implicit_euler_config.jacobian_scheme == "automatic") {
      ie.set_jacobian_computation_scheme(JacobianComputationScheme::kAutomatic);
    } else {
      throw std::logic_error("Invalid jacobian scheme");
    }
    ie.set_use_full_newton(config.implicit_euler_config.full_newton);
    ie.set_use_implicit_trapezoid_error_estimation(
        config.implicit_euler_config.trapezoid);
  }

...
welded to the world, with a deformable ball initially falling onto a single
segment. Although the belts never move -- they are rigidly welded to the world
and their configuration is constant for the whole simulation -- the ball is
...
the MultibodyPlant documentation for the full model. It works with rigid contact
and deformable contact alike; this example uses a squishy ball because it's fun.
...
```
bazel run //examples/multibody/conveyor_belt:conveyor_belt
```
...
```
bazel run //examples/multibody/conveyor_belt:conveyor_belt -- --belt_speed=-0.5
```

...
namespace std {
// This is obviously not 32-bit compatible.
template <>
struct hash<pair<int, int>> {
  size_t operator()(const pair<int, int>& vertex) const {
    static_assert(sizeof(size_t) == sizeof(pair<int, int>));
    size_t hashval = 0;
    memcpy(&hashval, &vertex, sizeof(size_t));
    return hashval;
  }
};
}  // namespace std
...
using geometry::internal::MakeBoxVolumeMesh;
using geometry::internal::MakeCylinderVolumeMesh;
using geometry::internal::MakeEllipsoidVolumeMesh;
using geometry::internal::MakeSphereVolumeMesh;
...
struct PaintedPatch {
  unique_ptr<TriangleSurfaceMesh<double>> mesh;
  vector<Vector2d> uvs;
};

// TODO(calderpg): This mesh intersection code came from drake:
//  mesh_intersection.h. It has recently been modified to run more efficiently
//  (particularly reducing the ridiculous amounts of memory allocation). When
//  that code is done being optimized, steal the optimizations for this.
 ...
 */
template <typename T>
vector<int> ComputeMinimumPolygon(const std::vector<Vector3<T>>& polygon) {
  ...
  const Vector3<T>* last_value = &polygon[v_count - 1];
...
pair<vector<Vector3<T>>, vector<Vector2<T>>> ClipPolygonByHalfSpace(
  ...
  int prev_index = (size > 1) ? size - 1 : 0;
  const Vector3<T>* previous_ptr =
      (size > 0) ? &polygon_vertices_F[prev_index] : nullptr;
  bool previous_contained =
      (size > 0) ? H_F.CalcSignedDistance(*previous_ptr) <= 0 : false;

  for (int i = 0; i < size; ++i) {
    const Vector3<T>& current = polygon_vertices_F[i];
    const Vector3<T>& previous = *previous_ptr;
    ...
    previous_contained = current_contained;
  }
...
vector<Vector2d> ClipTriangleByTetrahedronForUvPatch(
    ...
    const vector<Vector2d>& surface_uvs, const RigidTransformd& X_MN) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    const Vector3d& p_NV = surface_N.vertex(v);
  ...
  Vector3d p_MVs[4];
  for (int i = 0; i < 4; ++i) {
    const int v = volume_M.element(element).vertex(i);
...
PaintedPatch CalculatePaintedPatch(const TexturedCanvasMesh& canvas_C,
  ...
  vector<SurfaceTriangle> surface_faces;
  vector<Vector3<double>> surface_vertices_P;
  ...
  auto callback = [& volume_P = *painter_P.mesh, &surface_C = *canvas_C.mesh,
                   ...
                   &surface_uvs,
                   &surface_vertices_P](int tet_index, int tri_index) {
    ...
    const int v0 = static_cast<int>(surface_uvs.size());
  ...
  auto mesh_P = make_unique<TriangleSurfaceMesh<double>>(
      move(surface_faces), move(surface_vertices_P));
 ...
 public:
  /* Returns a volume mesh for the shape indicated by `id`. The `id` must be
   "valid" in the sense:
     ...
     2. It must be a supported geometry (see the set of ImplementGeometry below
        for supported geometries).

   @throws For "invalid" `id`. */
  static unique_ptr<PainterMesh> MakePainterMesh(
      GeometryId id, const SceneGraph<double>& scene_graph) {
    PainterReifier reifier;
    const SceneGraphInspector<double>& inspector =
        scene_graph.model_inspector();
    const Shape& painter_shape = inspector.GetShape(id);
    painter_shape.Reify(&reifier, nullptr);
    return move(reifier.mesh_);
  }

 private:
  // Currently, we're implementing only a subset of shapes.
  // (See ThrowUnsupportedGeometry for the list.)
  ...
  void ImplementGeometry(const Box& box, void*) final {
    ...
    auto mesh = make_unique<VolumeMesh<double>>(
        MakeBoxVolumeMesh<double>(box, resolution_hint));
    auto bvh = make_unique<Bvh<VolumeMesh<double>>>(*mesh);
  ...
  void ImplementGeometry(const Cylinder& cylinder, void*) final {
    ...
    auto mesh = make_unique<VolumeMesh<double>>(
        MakeCylinderVolumeMesh<double>(cylinder, resolution_hint));
    auto bvh = make_unique<Bvh<VolumeMesh<double>>>(*mesh);
    mesh_ = make_unique<PainterMesh>(move(mesh), move(bvh));
  }
  ...
  void ImplementGeometry(const Ellipsoid& ellipsoid, void*) final {
    ...
    auto mesh = make_unique<VolumeMesh<double>>(MakeEllipsoidVolumeMesh<double>(
...
class CanvasReifier final : public ShapeReifier {
 public:
   ...
   */
  static unique_ptr<TexturedCanvasMesh> MakeCanvasMesh(
      GeometryId id, const SceneGraph<double>& scene_graph) {
    CanvasReifier reifier;
    const SceneGraphInspector<double>& inspector =
        scene_graph.model_inspector();
    const Shape& painter_shape = inspector.GetShape(id);
    painter_shape.Reify(&reifier, nullptr);
 ...
 private:
  ...
  void ImplementGeometry(const geometry::Mesh& mesh_spec, void*) final {
    ...
    if (!warn.empty()) {
      log()->warn(
          "Warning parsing Wavefront obj file ({}): {}", mesh_spec.filename(),
          warn);
    }
    ...
    // The parsed product needs to be further processed. The TriangleSurfaceMesh
    // and SurfaceMeshLinear assumes one-to-one correspondence between vertex
    ...
    // constructs a face. We don't have that luxury in the TriangleSurfaceMesh
    ...
    //     can build the faces in the final TriangleSurfaceMesh by mapping the
    //     (p, uv)  pair in the OBJ face specification to the final
    //     TriangleSurfaceMesh vertex index i.
    ...
    // The map from (p, uv) --> i.
    unordered_map<pair<int, int>, int> obj_vertex_to_new_vertex;
    ...
    auto surface_mesh =
        make_unique<TriangleSurfaceMesh<double>>(move(faces), move(positions));
    auto bvh = make_unique<Bvh<TriangleSurfaceMesh<double>>>(*surface_mesh);
...
void RasterizeTriangle(const Vector2d& a, const Vector2d& b, const Vector2d& c,
  ...
  */
  ...
  // Casting double to int truncates -- so we are rounding down (assuming
  // non-negative values).
...
MeshPainterSystem::MeshPainterSystem(GeometryId canvas_id,
                                     GeometryId painter_id,
                                     const ImageRgba8U& image,
                                     const SceneGraph<double>& scene_graph,
                                     double period_sec,
                                     const bool parallelize_raster)
    : LeafSystem<double>(),
      ...
      parallelize_raster_(parallelize_raster) {
  ...
  texture_state_index_ = this->DeclareAbstractState(Value(image));
  this->DeclarePeriodicUnrestrictedUpdateEvent(
      period_sec, 0., &MeshPainterSystem::UpdateTextureImage);

  // Initialize the output texture image with the input image. We'll paint
  // opaque black onto the texture.
  texture_output_port_ = &this->DeclareAbstractOutputPort(
      "texture", image, &MeshPainterSystem::OutputTextureImage);
}
...
MeshPainterSystem::MeshPainterSystem(GeometryId canvas_id,
                                     ...
                                     double period_sec,
                                     const bool parallelize_raster)
    : MeshPainterSystem(canvas_id, painter_id, ImageRgba8U(width, height, 255),
                        scene_graph, period_sec, parallelize_raster) {}

const ImageRgba8U& MeshPainterSystem::get_texture_state(
    const Context<double>& context) const {
  return context.get_abstract_state<ImageRgba8U>(texture_state_index_);
}

ImageRgba8U& MeshPainterSystem::get_mutable_texture_state(
    Context<double>* context) const {
  return context->get_mutable_abstract_state<ImageRgba8U>(texture_state_index_);
}

void MeshPainterSystem::OutputTextureImage(const Context<double>& context,
                                           ImageRgba8U* texture) const {
  *texture = context.get_abstract_state<ImageRgba8U>(texture_state_index_);
}

void MeshPainterSystem::UpdateTextureImage(const Context<double>& context,
                                           State<double>* state) const {
  ImageRgba8U& texture =
      state->get_mutable_abstract_state<ImageRgba8U>(texture_state_index_);
  ...
  // respectively.
  const RigidTransformd& X_WC = query_object.GetPoseInWorld(canvas_id_);
  const RigidTransformd& X_WP = query_object.GetPoseInWorld(painter_id_);
  ...
  const TriangleSurfaceMesh<double>& mesh = *patch.mesh;
  const vector<Vector2d>& uvs = patch.uvs;

  if (parallelize_raster_) {
    const auto& triangles = mesh.triangles();
#if defined(_OPENMP)
#pragma omp parallel for
#endif
    for (size_t idx = 0; idx < triangles.size(); idx++) {
      const auto& face = triangles[idx];
      RasterizeTriangle(uvs[face.vertex(0)], uvs[face.vertex(1)],
                        uvs[face.vertex(2)], &texture);
    }
  } else {
    for (const SurfaceTriangle& face : mesh.triangles()) {
      RasterizeTriangle(uvs[face.vertex(0)], uvs[face.vertex(1)],
                        uvs[face.vertex(2)], &texture);
    }
  }

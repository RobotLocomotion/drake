...
class MovingRod final : public LeafSystem<double> {
 public:
  ...
  explicit MovingRod(SceneGraph<double>* scene_graph) {
    ...
    illus_props.AddProperty("phong", "diffuse", Rgba(0.2, 0.2, 0.8, 1));
    scene_graph->AssignRole(source_id_, geometry_id_, illus_props);
    PerceptionProperties percep_props;
    percep_props.AddProperty("phong", "diffuse", Rgba{0.2, 0.2, 0.8, 1.0});
  ...
  const OutputPort<double>& geometry_pose_output_port() const {
...
int do_main() {
  ...
  illustration_box.AddProperty("phong", "diffuse",
                               Rgba{0.5, 0.5, 0.45, 1.0});
  scene_graph.AssignRole(source_id, ground_id, illustration_box);
  PerceptionProperties percep_props;
  percep_props.AddProperty("phong", "diffuse", Rgba{0.2, 0.5, 0.25, 1.0});
  percep_props.AddProperty("label", "id",
  ...
  auto& image_to_lcm_image_array = *builder.template AddSystem<
  ...
  auto render_engine =
      MakeRenderEngineVtkWithMaskImages(RenderEngineVtkParams());
  scene_graph.AddRenderer(render_name, move(render_engine));
  DepthRenderCamera camera_prop(
      RenderCameraCore(
          render_name, CameraInfo(640, 480, M_PI_4), ClippingRange(0.01, 100.0),
          RigidTransformd()),
      DepthRange(0.1, 2.0));

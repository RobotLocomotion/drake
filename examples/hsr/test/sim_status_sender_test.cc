...
GTEST_TEST(SimStatusSenderTest, MessageAssmeblyTest) {
  SimLcmTestConfig test_config{kRobotModelUrdf};

  const auto& robot_plant = test_config.robot_plant();
  SimStatusSender dut(&robot_plant);

  std::unique_ptr<systems::Context<double>> context =
      dut.CreateDefaultContext();

  const Eigen::VectorXd& input_value = test_config.estimated_state_vector();
  dut.get_estimated_state_input_port().FixValue(context.get(), input_value);

  const lcmt_hsr_sim_status& output =
      dut.get_sim_status_output_port().Eval<lcmt_hsr_sim_status>(*context);

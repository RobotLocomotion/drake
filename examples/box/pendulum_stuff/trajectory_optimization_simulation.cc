...
DEFINE_double(torque_limit, 3.0,
              "Torque limit (N-m) ");

DEFINE_double(x_cost, 1.0,
              "Multiplicatioin factor of the cost on x ");              
...
int DoMain() {
  ...
  const double kTorqueLimit = FLAGS_torque_limit;  // N*m.
  ...
  const auto& xf = final_state.get_value();
  const auto& xd = x - xf;
  dircol.AddRunningCost(((R * u) * u)(0) + FLAGS_x_cost * xd.cast<symbolic::Expression>().dot(xd));

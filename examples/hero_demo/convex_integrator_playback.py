...
# Importing this registers CenicIntegrator so get_mutable_integrator() can
# downcast to it (giving access to {get,Set}SolverParameters).
from pydrake.multibody import cenic  # noqa: F401
...
class JointTargetSource(LeafSystem):
    ...
    def __init__(self):
        ...
        keyframes_file = FindResourceOrThrow(
            "drake/examples/hero_demo/keyframes.txt.gz"
        )
        with gzip.open(keyframes_file, "rt", encoding="utf8") as f:
            lines = f.readlines()
...
remote_params = PackageMap.RemoteParams(
    urls=[
        "https://github.com/ToyotaResearchInstitute/lbm_eval/releases/download/1.1.0/lbm_eval_models-1.1.0-py3-none-any.whl"  # noqa
    ],

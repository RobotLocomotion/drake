...
p = PlanarRigidBodyManipulator('spring_flamingo_passive_ankle.urdf',options);
...
if nargin < 2
  ...
  traj_init.u = PPTrajectory(foh(linspace(0,tf0,N),randn(4,N)));
...
xf_min = [-inf(18,1)];
xf_max = [-.4;inf(17,1)];  %walking negative x direction
...
lz_bound = BoundingBoxConstraint(zeros(2*(N - 2*d),1),zeros(2*(N - 2*d),1));

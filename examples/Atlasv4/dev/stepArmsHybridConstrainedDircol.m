function [p,v,xtraj,utraj,ltraj,z,F,info,traj_opt] = stepArmsHybridConstrainedDircol(z0,xtraj,utraj,ltraj)
step_height = .3;
...
p = PlanarRigidBodyManipulator('../urdf/atlas_planar_two_arms.urdf',options);
...
N = [6,6,5,5,6];
N = [5,4,4,4,5];
duration = {[.2 .7],[.1 1.5] [.1 1.5], [.1 1] [.1 .7]};
modes = {[1;2],[1;2;5;6],[2;3;4;5;6], [3;4;5;6],[3;4]};
% 
% N = N(1:4);
...
to_options.mode_options{2}.active_inds = [1;2;4;5;6;7;8];
to_options.mode_options{3}.active_inds = [1;2;3;4;6;7;8;9;10];
to_options.mode_options{4}.active_inds = [1;2;4;5;6;7;8];
to_options.mode_options{5}.active_inds = [1;2;4];

contact_q0 = zeros(14,1);
contact_q0(10) = -1;  %right hip
contact_q0(4) = .5; %back
contact_q0(5) = -1.5; % shoulder
contact_q0(9) = -1.5; % shoulder
...
traj_opt = traj_opt.addModeStateConstraint(1,BoundingBoxConstraint(x0 - [0;.0*ones(13,1);.1*ones(14,1)],x0+[0;.0*ones(13,1);.1*ones(14,1)]),1);
if length(N) == 5
...
if length(N) > 1
  traj_opt = traj_opt.addConstraint(BoundingBoxConstraint([-.33;-.33],inf(2,1)),traj_opt.mode_opt{2}.cstrval_inds(2:3) + traj_opt.var_offset(2));
end
...
if length(N) == 5
  ...
  R_periodic(4,4) = 1; % back joint
  R_periodic(5:8,9:12) = eye(4); %leg and usy joints w/symmetry
  R_periodic(9:12,5:8) = eye(4); %leg ans usy joints w/symmetry
  R_periodic(13,14) = -1; %elx
  R_periodic(14,13) = -1; %elx
  
  
  
  R_periodic(15:17,15:17) = eye(3); %x,z,pitch velocities
  R_periodic(18,18) = 1; % back joint
  R_periodic(19:22,23:26) = eye(4); %leg and usy joints w/symmetry
  R_periodic(23:26,19:22) = eye(4); %leg ans usy joints w/symmetry
  R_periodic(27,28) = -1; %elx
  R_periodic(28,27) = -1; %elx

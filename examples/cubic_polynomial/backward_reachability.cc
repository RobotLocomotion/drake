...
Expression MakeVolumetricCost(const Polynomial& p, BoundingBox b) {
  ...
  for (const auto& monomial : map) {
    std::map<Variable, int> powers = monomial.first.get_powers();
    Expression moment{1.};
...
/// solve the dual relaxation over the space of nonegative functions in order to
/// find a tight over-approximattion to the reachable set.
///
...
void ComputeBackwardReachableSet() {
  ...
  // Define the (symmeric) domain bounds on x: ‖x‖ ≤ x_bound (X = {x | gx ≥ 0}).
  ...
  // Define the ground-truth BRS (for plotting only): ‖x(0)‖ < 0.5 (a.k.a. X₀).
  ...
  for (const auto& it : w.decision_variables()) {
    w_env.insert(it, prog.GetSolution(it));
  }
  ...
  CallPython("plt.ylabel", "w (blue), I_B (orange)");
}

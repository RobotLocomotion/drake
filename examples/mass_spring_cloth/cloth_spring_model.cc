...
ClothSpringModel<T>::ClothSpringModel(int nx, int ny, T h, double dt)
  ...
  if (dt > 0) {
    ...
    this->DeclareVectorOutputPort("states",
                                  systems::BasicVector<T>(6 * num_points_),
                                  &ClothSpringModel::CopyDiscreteStateOut);
    this->DeclarePeriodicDiscreteUpdateEvent(
        dt_, 0., &ClothSpringModel::UpdateDiscretePositionAndVelocity);
  } else {
    // Continuous system.
    // Adding 3*N positions and 3*N velocities.
  ...
  param_offset_ = this->DeclareNumericParameter(ClothSpringModelParams<T>());
  ...
  if (dt > 0) {
    ...
    for (const Spring& s : springs_) {
      for (int d = 0; d < 3; ++d) {
        num_nonzero_entries[3 * s.node0 + d] += 3;
        num_nonzero_entries[3 * s.node1 + d] += 3;
      }
    }
...
void ClothSpringModel<T>::InitializePositionAndVelocity(
  ...
  if (dt_ > 0) {
    ...
    InitializePositionAndVelocity(nx_, ny_, h_, &x, &v);
  } else {
    // Extract mutable position and velocity states and initialize.
...
template <typename T>
void ClothSpringModel<T>::BuildRectanglePatch(bool use_shearing_springs) {
...
template <typename T>
void ClothSpringModel<T>::UpdateDiscretePositionAndVelocity(
    const systems::Context<T>& context,
...
template <typename T>
void ClothSpringModel<T>::CalcSpringForce(

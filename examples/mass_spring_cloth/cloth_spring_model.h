...
/** A system that models a rectangular piece of cloth using as a model a
 ...
 (0, Ly, 0), where Ly is the side length of the cloth in the y-direction and is
 equal to (ny_-1)*h. Shearing springs are added to penalize shearing modes. The
...
class ClothSpringModel final : public systems::LeafSystem<T> {
  ...
  */
  ...
  /** Initialize the positions of the particles to be in a rectangular grid of
    nx-by-ny points with neighboring points separated by h. The velocities are
    initialized to zero. */
 ...
 private:
  ...
  void InitializePositionAndVelocity(VectorType* x, VectorType* v) const {
    DRAKE_DEMAND(x->size() == 3 * num_points_ && v->size() == 3 * num_points_);
    for (int i = 0; i < nx_; ++i) {
      for (int j = 0; j < ny_; ++j) {

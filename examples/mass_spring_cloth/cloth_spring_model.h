...
/** A system that models a rectangular piece of cloth using as a model a
 ...
 world frame unless otherwise noted. The system consists of nx-by-ny mass points
 with neighboring points separated by a distance of h meters. The rectangular
 grid initially lies in the z=0 plane and is fixed at two corners at (0,0,0) and
 (0, (ny-1)*h, 0). Shearing springs are added to penalize shearing modes. The
 model is described in detail in [Provot 1995].
 ...
 points connected by the spring.
 - __gravity__: the gravitational constant, a (usually negative) scalar. The
 gravity experienced by the mass points is (0,0,gravity).
...
class ClothSpringModel final : public systems::LeafSystem<T> {
 public:
  ...
  /** Creates a rectangular cloth with a grid of nx by ny points.

   @param[in] nx   The number of points in the x-direction.
   @param[in] ny   The number of points in the y-direction.
   @param[in] h    The spacing (in meters) between neighboring points.
 ...
 private:
  struct Spring {
    ...
    // don't actually index into any collection of points.
    T rest_length;
  };
  ...
  // The distance between neighboring mass points.
  const T h_{};
  ...
  Vector3<T> particle_state(int particle_index, const VectorType& vec) const {
    int p_index = particle_index * 3;
  ...
  template <class VectorType>
  void set_particle_state(int particle_index, const Vector3<T>& state,
  ...
  // Similar to set_particle_set, but add state into the corresponding position
  // in vec without zeroing out the old value.
  template <class VectorType>
  void accumulate_particle_state(int particle_index, const Vector3<T>& state,
                                 VectorType* vec) const {
    int p_index = particle_index * 3;
    (*vec)[p_index] += state(0);
    (*vec)[++p_index] += state(1);
  ...
  void InitializePositionAndVelocity(VectorType* x, VectorType* v) const {
    DRAKE_DEMAND(x->size() == 3 * nx_ * ny_ && v->size() == 3 * nx_ * ny_);
    for (int i = 0; i < nx_; ++i) {
      for (int j = 0; j < ny_; ++j) {
        ...
        |   ...........................................................
        |   |               |               |                         |
        |   |(nx_-1)*ny_    |(nx_-1)*ny_+1  |(nx_-1)*ny_+2            | nx_*ny_-1
        |   x ------------- x ------------- x ------- ..... --------- x
        |
        V
        x-axis
  ...
  void AccumulateElasticForce(const ClothSpringModelParams<T>& param,
    ...
    for (const Spring& s : springs_) {
      ...
      const Vector3<T> x_WP0 = particle_state(p0, x);
      const Vector3<T> x_WP1 = particle_state(p1, x);
      const Vector3<T> p_P0P1_W = x_WP1 - x_WP0;

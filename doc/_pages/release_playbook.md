...
1. Choose the git sha that will be tagged as the release:
   1. Don't choose a sha whose tests fail. To that end, the nominal choice of
      sha would use <https://github.com/RobotLocomotion/drake/commits/master/>
      as a reference to find the last commit from the prior day, while making
      sure that <https://drake-jenkins.csail.mit.edu/view/Production/> and
      <https://drake-jenkins.csail.mit.edu/view/Nightly%20Production/> are both
      clean (modulo the ``*-coverage`` builds, which we can ignore).

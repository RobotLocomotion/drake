...
To install on Ubuntu run `sudo apt-get install libegl1 -y`. To install on Amazon
Linux run `sudo dnf install libglvnd-egl -y`.

On Linux, if you are using either `RenderEngineVtk` under the (non-default)
setting `backend = "GLX"` or `RenderEngineGl`, then you must ensure that prior
to using Drake the `$DISPLAY` environment variable is set to an available X11
display server (e.g., `":1"`). If you are running as a desktop user (not over

...
[release notes](/release_notes.html)) . Any element of Drake that is not
explicitly documented to be part of the Stable API is deemed "unstable" and is
...
* Excluding the "all" modules such as `pydrake.all`, `pydrake.systems.all`,
  ...
  are not Stable.
  * The existence of the `pydrake.all` module is part of the Stable API (we
    will not remove it without notice), but the specific list of items
...
* The `add_default_...` macros defined in `@drake//tools/workspace:default.bzl`
  ...
  * For any Bazel externals loaded by these functions (e.g., `"@eigen"`), we
...
the calculated results. There is no guarantee that Drake's output is bitwise-
identical from one version to the next.
...
* In C++, do not take the address of any Drake function.
  * We may add new, defaulted arguments to functions without any prior notice.
...
for each stable release. For C++ and Python API changes, for convenience we
will also use the language mechanism to highlight the change when possible
(`[[deprecated]]` for C++; `warnings.warn` for Python). However, those
mechanisms will not always trigger in every case, and some deprecations will
reach beyond what those tools can denote. Therefore, we still recommend that
you monitor the release notes as the final arbiter of deprecation
announcements.

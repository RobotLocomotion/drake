...
<!-- Reminder to follow-up from #16503#issuecomment-1029449308 to explain the "experimental" idea in our documentation. -->

# Stable API

We define a large portion of Drake as our "Stable API" that you can rely
on. For the Stable API, we aim to give at least 3 months of notice (via
deprecation announcements) ahead of any disruption. Any element of Drake that
is not explicitly documented to be part of the Stable API is deemed "unstable"
or "experimental" and is subject to change or removal without prior notice.
...
the Stable API in its local documentation.

## C++

The C++ Stable API covers all C++ library code within `namespace drake` whose
header files are distributed in a Drake pre-compiled binary image:
...
* Excluding code documented with an "experimental" warning box.

## Python

The Python Stable API covers all Python library code under `import drake` or
`import pydrake` that is distributed in a Drake PyPI wheel:

* Excluding the `pydrake.examples` module tree.
* Excluding the "all" modules such as `pydrake.all`, `pydrake.systems.all`,
  etc. These are intended to be shortcuts for local hacking only, and so are
  not Stable.
  * The mere existence of the `pydrake.all` module is part of the Stable API
    (we will not remove it without notice), but the specific list of items
    contained within it is not Stable.
* Except for "private" names that begin with a leading underscore.
* Excluding code documented as "internal use only".
* Excluding code documented with an "experimental" warning box.
...
* This **includes** depending on those labels at build-time (e.g.,
  `deps = ["@drake//common"]`).
* This **excludes** loading Drake's bzl macros at analysis-time (e.g.,
  `load("@drake//foo:bar.bzl", "bar_macro")`.
 
For Drake's dependencies:

* All macros defined in `@drake//tools/workspace:default.bzl` are part of the
  Stable API.
* Similarly, all of the `add_default_foo` macros that are called by
  `@drake//tools/workspace:default.bzl` are included in the Stable API.  For any
  bazel externals loaded by these functions (e.g., `"@eigen"`), we will
  deprecate it prior to removing the definition of the dependency.
  * Excluding any items documented as "internal use only".
  * Excluding any items  documented as "experimental".
...
TBD write something about framework port names, or general about behavioral
changes that don't change the API signatures.
...
where the Stable guarantees apply.) However, we will make a best effort to
adhere to these guidelines as we resolve any issues with the new API, even in
the first release.
...
* Include What You Use. Drake's `#include` graph will change over time; always
  include the header for each class you use.
  * Relatedly, do not include headers that you don't use. Our C++ or Python
    deprecation warnings typically only trigger on _use_ of the API, not mere
    inclusion or importing.
* For Bazel users, `deps` what you use. Drake's `deps` graph will change over
  time; always list out your Drake library dependencies (to match what you
  `#include` or `import`); do not rely on transitive dependencies to bring
  them in.
* In C++, do not forward-declare anything from Drake.
* In C++, do not take the address of any Drake function. We may add new,
  defaulted arguments to functions without any prior notice.
...
SemVer. Instead, we use a rolling deprecation window. In each release, new
deprecations might be announced, and then those changes will be finalized 3+
minor versions later, without changing the major version number.

Deprecation announcements appear in the release notes for each stable
release. For C++ and Python API changes, for convenience we will also use the
...
Drake does not offer any stable ABI. We expect any C++ code linked against
Drake within the same program to use the identical source revision and build
flags of Drake as any other code within that program.

...
5. Unzip it. We suggest that you use ``/Library/gurobi1301`` to simplify
   working with Drake installations.
6. If you unzipped into a location other than ``/Library/gurobi1301``, then
   call ``export GUROBI_HOME=GUROBI_UNZIP_PATH/macos_universal2`` to set the
   path you used, where in ``GUROBI_HOME`` folder you can find ``bin`` folder.

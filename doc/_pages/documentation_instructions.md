...
``@tparam`` is a regular Doxygen command to describe a template argument. In
Drake, for code templated on the "scalar type", writing out the description of
the template argument would be onerous boilerplate. Instead, we have a few
custom abbreviations for the common cases:

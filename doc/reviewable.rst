...
curated``, which removes the multi-commit merge prevention.  PRs with this
label should be merged to master using the "Create a merge commit" option, not
...
the reviewer should apply the label ``"status: single reviewer ok"`` to note
this condition, which removes the single-assignee merge prevention.

...
# These tutorials cannot be executed during the documenation build.
_NO_EXECUTE = (
    # Requires deepnote.
...
def _build(*, out_dir, temp_dir, modules):
    ...
    templates_dir = Path(
        manifest.Rlocation("drake/doc/tutorials/templates/drake_style/conf.json")
    ).parent
    ...
    for module in modules_to_document:
        ...
        check_call(
            [
                ...
                "--to=html",
                f"--TemplateExporter.extra_template_basedirs={templates_dir.parent}",

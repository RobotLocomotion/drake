...
def _build(*, out_dir, temp_dir, modules):
    ...
    # Process the command-line request for which modules to document, but never
    # omit the index page.
    if not modules:
        modules_to_document = _get_pydrake_modules()
    else:
        # TODO(jeremy.nimmer) Parents should imply their children
        # (as doxygen_cxx/build.py does).
        modules_to_document = set([
            x for x in _get_pydrake_modules()
            if x in modules or x == "pydrake"
        ])
        # TODO(jeremy.nimmer) Error on bad values.

    # Generate tables of contents.
    for name in modules_to_document:
        if name == "pydrake":
            rst_name = "index.rst"
        else:
            rst_name = name + ".rst"

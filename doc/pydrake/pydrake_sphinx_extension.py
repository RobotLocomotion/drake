...
def autodoc_process_bases(app, name, obj, options, bases):
    """Hides `pybind11_object`, the implementation base class that pybind11
    injects into every bound class, from the list of documented bases.
    """
    bases[:] = [
        b for b in bases if getattr(b, "__name__", None) != "pybind11_object"
    ]
...
def patch_sort_members(original, self, documenters, order):
    ...
    using the result of the `autodoc-member-order-custom-function` event.
    """
    ...
    documenters.sort(key=custom_key)
    return documenters

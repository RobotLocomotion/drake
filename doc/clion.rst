...
box appears, paste the contents of the clipboard there. In "Bazel command",
select either "run" (for an arbitrary program), or "test" (for a Bazel test
target). Everything else can be left at default values. Click OK to finish.

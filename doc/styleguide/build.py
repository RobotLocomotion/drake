...
def _add_title(*, temp_dir, filename, title, new_filename=None):
    ...
    If a new_filename is not given, the original file is replaced.
    If a new_filename is given, the original file is removed and new_filename
      is added.
    """
    with open(join(temp_dir, filename), "r", encoding="utf-8") as f:
        data = f.read()
    os.unlink(join(temp_dir, filename))

    if new_filename is None:
        new_filename = filename
    with open(join(temp_dir, new_filename), "w", encoding="utf-8") as f:

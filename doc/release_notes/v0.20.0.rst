...
* Allow changing point contact parameters on a per-geometry basis via SceneGraph; these become Systems Framework parameters that live in the Context (`#13630`_)
...
* Fix and/or cleanup OpenGL dependencies (`#13649`_)  # We need either libOpenGL.so, libOpenGL.so.0, libGLX.so, libGLX.0, GL/gl.h, and GL/glx.h (modern, vendor neutral) or libGL.so, libGL.so.1, GL/gl.h, and GL/glx.h (legacy). Vendor neutral is preferable, for NVIDIA users and others, so we choose libOpenGL.so and libGLX.so rather than libGL.so. Also rebuild VTK and drake-visualizer to ensure they are correctly linked to libOpenGL.so and libGLX.so rather than libGL.so.
* wsg_50_description: Deprecate the URDF and meshes (`#13693`_)
* added parsing of point_contact_stiffness (`#13686`_)

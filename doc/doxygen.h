...
/**
  @defgroup conventions Drake Conventions for Terminology & Notation
...
/** @addtogroup accuracy_and_tolerance
 ...
 under user control. We need standard, unambiguous terminology in Drake
 for requesting and reporting solution quality. The terms "tolerance",
 "accuracy", and "precision" are commonly used in this context but often
 imprecisely. Here is how these terms are used in Drake:
 ...
 numerical packages.) Ideally you can think of `accuracy` as specifying the
 number of correct significant digits in the desired result. For example,
 specifying `α = 10⁻³` should yield a result correct to three digits while
 `α = 10⁻⁶` should give six. This is a Platonic ideal that cannot always be
 achieved perfectly in practice, but Drake algorithms should attempt to respond
 to a user's `accuracy` request in approximately this way.

 We use the unambiguous terms "tight/tighter accuracy" and "loose/looser
 accuracy" in preference to "high", "more", "low", or "less accuracy" which are
 prone to being misunderstood due to the fact that numerically smaller values
 correspond to more (tighter) accuracy.
 ...
 many numerical packages.) So "constraint tolerance" is the amount by which a
 user is willing to have a constraint violated, in units appropriate to that
 constraint. Thus a tolerance ε=10⁻⁴ for a position constraint means that an
 error of 0.1mm is acceptable.

 ### precision

 We do not use this term in Drake in this context, and you shouldn't either.
 Instead we reserve it for the Machine Learning term-of-art as in "precision
 and recall".
 ...
 The above discussion follows [Sherman, 2011]. See the discussion there for
 more information.

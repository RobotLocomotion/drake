...
* [fix] TBD [geometry,workspace] Add poisson disk sampling ([#22243][_#22243])
* [fix] TBD [mujoco parser] Don't compute inertia of a HalfSpace ([#22290][_#22290])  # The InertialCalculator returns NaNs, which would be fine, but then the SpatialInertial constructor threw an exception immediately after. This PR avoids that particular failure (by not computing the inertia of a HalfSpace), and also improves the error message so that if we _do_ calculate an invalid inertia, we also print the geom name.
* [fix] TBD Fixes bug with geometries in marginal contact ([#22153][_#22153])
...
* Work around linker error with clang 16+ ([#22172][_#22172])
...
* [fix] TBD [doc,tools] [setup] Upgrade Bazel to latest release 8.0.0 ([#22285][_#22285])
...
* [fix] TBD [workspace,third_party,setup] [workspace] Upgrade bazelisk to latest release 1.25.0 ([#22292][_#22292])
* [fix] TBD [workspace] Upgrade gz_utils_internal, gz_math_internal, sdformat_internal to latest ([#22259][_#22259])  # Co-Authored-By: Rick Poyner <rick.poyner@tri.global>

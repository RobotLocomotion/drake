...
* TBD Add python binding for SceneGraph::RegisterGeometry with context ([#17342][_#17342])  # * Bind RegisterGeometry with context
New features
...
* TBD [workspace] Use stable URLs for GitHub downloads where possible ([#17344][_#17344])
* TBD [tools,solvers] [workspace] Build NLopt from source ([#17288][_#17288])  # Compared to the Ubuntu and macOS packages, this removes the LGPL'd "luksan" solver from the build, which means we can link the library statically (and eventually, hidden). This might mean that NLopt is no longer able to solve certain kinds of programs, but that is a fair trade-off given all of the problems caused by LGPL linking. The package.BUILD.bazel is modeled on nlopt.BUILD.bazel in a10ebb7. Deprecate the now-unused current (host OS) nlopt external.
* TBD [tools,manipulation,examples,multibody,geometry] [workspace] Withdraw stable API support for several externals ([#17324][_#17324])  # These are the python- and models-only repositories that should not be public. The models already have Drake-public filegroup wrappers where appropriate. The python libraries are either maintainer-only code (github3, template), or else brittle interior dependencies (clang) of private tools (mkdoc).

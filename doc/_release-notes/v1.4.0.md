...
* TBD [systems,common,multibody,geometry] DRAKE_EXPECT_THROWS_MESSAGE now requires an expression ([#17341][_#17341])  # The documentation required an expression, but the implementation accidentally allowed for a block instead. Now it fails-fast. Also suppresses "unused result" warnings for the expressions in question. This is helpful when a function is marked "nodiscard".
New features

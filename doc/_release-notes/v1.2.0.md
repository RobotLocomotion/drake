...
* TBD [solvers,geometry,systems] Deprecate LinearConstraint::A() with GetDenseA() ([#16976][_#16976])
* TBD MathematicalProgram::NewSosPolynomial takes degree=0 ([#16983][_#16983])
New features
...
* TBD [geometry] Updates hydroelastic user guide for compliant convex feature ([#16995][_#16995])  # * update user guide for compliant convex mesh
New features
...
* TBD Add Monomial::Evaluate() for a batch of data ([#16984][_#16984])
...
* TBD "Fix" install RPATH for MOSEK ([#16985][_#16985])  # Modify how we adjust RPATHs in installed libraries to rewrite two types of paths: - Paths starting with `@loader_path`. - Paths into Bazel's private build ares (`/private/var/tmp/_bazel_$USER`). Previously, we only rewrote the former. However, for reasons which are unclear, MOSEK gets linked using the latter, which was resulting in the installed library containing a reference to the absolute path of MOSEK as found in the build tree (which is clearly not useful).

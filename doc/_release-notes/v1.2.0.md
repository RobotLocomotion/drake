...
* TBD LinearConstraint stores a sparse matrix A ([#16894][_#16894])  # It can optionally also store a dense matrix A. BoundingBoxConstraint stores the sparse identity matrix.
New features
...
* TBD [tools,doc] [workspace] Remove bionic doxygen build ([#16946][_#16946])
* TBD Enable MOSEK in wheel builds ([#16927][_#16927])
* TBD [tools,setup] [workspace] Disable drake-visualizer on macOS ([#16945][_#16945])
* TBD [workspace] Fix "disable drake-visualizer on macOS" for Ubuntu ([#16962][_#16962])  # The installed flavor of the Ubuntu launcher script got lost in the shuffle.
* TBD Add clang-12 on Focal, macOS ([#16943][_#16943])  # Modify setup scripts to also install clang-12. Ultimately, we want to get rid of clang-9, but this will allow us to switch CI builds to 12 without needing to lock-step changes in the main repo and CI.
* TBD [tools,examples] [workspace] Remove bionic pygame, deprecate @pygame_py repository ([#16959][_#16959])  # - Bionic support removed, eliminating the need for the PyPI wheel to be obtained. - As a result, the @pygame_py repository is no longer useful and is now deprecated.
* TBD [wheel] pygame is not a dependency, python 3.8+ only ([#16963][_#16963])
* TBD [workspace] Fix "disable drake-visualizer on macOS" for Ubuntu again ([#16964][_#16964])
* None

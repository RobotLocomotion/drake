...
* TBD [common] Improve symbolic polynomial performance via is_expanded ([#16980][_#16980])  # In cases where we are operating on affine expressions over variables, notice cases where we know that is_expanded == true upon construction so that we don't need to re-expand it later. This yields a ~2x speedup in BenchmarkSosProgram1. As of this change, all calls from benchmark program to Expression::Expand are no-ops; the expression is always already known to be expanded.
...
* TBD [workspace] Remove bionic support for drake_visualizer ([#16972][_#16972])  # - Any non-focal distribution uses the fail-fast stub. - Remove references to bionic artifacts. - Rebuild and deploy new focal drake_visualizer tarball.

...
* [fix] TBD Fix ConvexSet move operations ([#19311][_#19311])
* [feature] TBD Make RenderEngineGl intelligently threadsafe ([#19426][_#19426])  # When a RenderEngineGl is cloned, instead of *sharing* an instance of OpenGlContext with its clones, each engine clone has a unique clone of the original engine's OpenGlContext. The cloned contexts are unique in that they can be bound to different threads, but they share the same underlying OpenGl objects, stored on the GPU. Because RenderEngineGl no longer shares the exact same OpenGl context with the original, some additional work needs to be done to the clone to make sure it is fully prepared: - Its OpenGl state needs to be configured (back face culling, etc.) - Vertex array objects need to be recreated from the vertex buffers shared in the OpenGl context -- by design, vertex array objects don't get shared. - Additional infrastructure has been added so the recreation of array objects from buffers is possible. - Each cloned instance has to have its own unique namespace for shade program uniforms. This is accomplished by recreating the OpenGl program for instance (using the same compiled shaders). This means, the private copy constructor for RenderEngineGl is, itself, insufficient to create a working copy. Only the public Clone() method should be used. A new test has been added to test the thread safety invariants of both OpenGlContext and RenderEngineGl.
* [feature] TBD SpatialInertia to equivalent dimensions and pose for a solid uniform-density ellipsoid and box ([#19414][_#19414])  # Co-Authored-By: mitiguy <paul.mitiguy@tri.global>
New features

* Add SceneGraphInspector::GetAllSourceIds ([#19525][_#19525])
* Implement MeshcatPoseSliders ([#19513][_#19513])
...
* [fix] TBD [examples,multibody,systems,geometry,manipulation,common,workspace,planning,tools,lcm,math,perception,solvers,visualization] [tools] Default clang_format_lint to be enabled in new packages ([#19547][_#19547])
...
* [fix] TBD Updated URDFs for Franka with the reflected rotor inertia values ([#19500][_#19500])
* [feature] TBD [visualization] Add basic inertia visualizer ([#19210][_#19210])  # Add the InertiaVisualizer and update ApplyVisualizationConfig() likewise (off by default).  Inertia geometry is fully transparent for non-Meshcat visualizers (which do not offer alpha sliders).
* None
...
<!--
  ...
  Current newest_commit 344dabc283ede0992c0e28eb2f7f3c804e4ba29b (inclusive).
-->

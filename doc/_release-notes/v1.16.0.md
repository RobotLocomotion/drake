...
* Update RenderEngineVtk to use RenderMaterial for appearance data ([#19383][_#19383])
  * While the "if foo.png exists for foo.obj, apply it" behavior isn't *gone*, it *has* been supplanted in the case where foo.obj actually has a material library (mtl). So, if models out there had let the mtl file rot when they catered to Drake's former idiosyncracies, this will cause the appearance to shift back towards the actual material specification.

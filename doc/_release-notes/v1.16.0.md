...
* [feature] TBD Implements ExpressionCost ([#19405][_#19405])  # Most importantly, now AddCost(Expression) will succeed, even if it is passed in an Expression that is non-Polynomial. Resolves #17897.
New features
...
* [feature] TBD Calculate principal moments of inertia and their axes ([#19281][_#19281])  # Co-Authored-By: mitiguy <paul.mitiguy@tri.global>
* [fix] TBD [geometry] Map white color to kDontCare in render_gltf_client ([#19397][_#19397])  # When a render server returns a white pixel value, i.e., RGB=(255, 255, 255), RenderEngineGltfClient should convert it to render::RenderLabel::kDontCare as it could contain the background and server-only geometries.
* [fix] TBD [multibody] Improve error message for !(Delta > 0) ([#19407][_#19407])
New features
...
* [fix] TBD [planning] Improve performance of SceneGraphCollisionChecker through better collision filtering ([#19387][_#19387])  # * Allow CollisionChecker implementations to perform additional filter work via UpdateCollisionFilters(). * SceneGraphCollisionChecker uses UpdateCollisionFilters to apply filters in SceneGraph for improved performance. * Rename DoRemoveAddedGeometries to RemoveAddedGeometries as it is not NVI. * Add missing overload of SetCollisionFilteredWithAllBodies.
New features
...
* [fix] TBD [workspace] Upgrade stable_baselines3_internal to latest release v1.8.0 ([#19187][_#19187])  # * Update repository.bzl connection.patch no longer needed with this version (was fixed upstream) * connection.patch no longer needed with this version (was fixed upstream) * Update no_torch.patch lines and context Co-Authored-By: ggould-tri <ggould@tri.global>
...
<!--
  ...
  Current newest_commit 870bdc0494bb60301978f2057a630c252dc04287 (inclusive).
-->

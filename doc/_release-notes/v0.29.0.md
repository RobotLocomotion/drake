...
* TBD sensors: add documentation about the camera frame ([#14127][_#14127])  # Elaborate the documentation in `camera_info.h` and add cross reference and extended discussion in rgbd_sensor.h.
* TBD systems: Avoid redundantly computing subsystem feedthrough maps ([#14788][_#14788])  # The key observation is that HasDirectFeedthrough was always computing the full GetDirectFeedthroughs and then throwing away all but the one map entry. We can compute the map once per system and reuse it as we search, instead of once per possible input.  This removes some super-linear growth factor in diagram size that ends up being problematic.
* TBD DirectTranscription class supports continuous-time MultibodyPlant ([#14840][_#14840])  # It turns out that it actually DID support it already, but there were some old DRAKE_DEMANDs that were erroneously blocking the execution.
* TBD direct_transcript_test: Warn when test is skipped b/c of no SNOPT ([#14847][_#14847])
* TBD CI fix: Remove test from asan until its timeout issue is addressed ([#14854][_#14854])
* TBD direct_transcription: Parallelize test cases ([#14859][_#14859])  # This commit replaces the first-aid fix of #14854, which addressed test timeouts introduced at #14840. Run the test in 2 shards. Most of the cases are vanishingly fast, but the discrete- and continuous-mode SNOPT solves of Multibody*Test are quite long under slow tooling (ASAN). This reshuffle should bring the test latency down enough to avoid timeouts. Also, a minor tweak: remove or re-spell some uses of DRAKE_DEMAND, to make the test more idiomatic.

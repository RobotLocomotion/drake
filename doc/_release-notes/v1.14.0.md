...
* [feature] TBD [solvers]: QuadraticConstraint stores Hessian type and checks convexity ([#18965][_#18965])  # Indicate whether the Hessian is positive/negative semidefinite or indefinite. This will be useful for checking the convexity of the constraint.
New features
...
* [feature] TBD [parsing] make sdf errors use diagnostic mechanism ([#18462][_#18462])
* [feature] TBD [meshcat] Implement Meshcat::PlotSurface() ([#18924][_#18924])  # This is a handy plotting helper, providing an interface like matplotlib's plot_surface. I've been using a python version of it for years in my course notes, but now I find myself wanting it in C++.
* [feature] TBD Add SpatialInertia::ThinRodWithMassAboutEnd() ([#18950][_#18950])  # Co-Authored-By: mitiguy <paul.mitiguy@tri.global>
* [fix] TBD ModelVisualizer: re-pose joints by name ([#18949][_#18949])  # Makes the reload feature of ModelVisualizer more robust by setting joint positions in the reloaded model to the prior values of joints with the same names, rather than requiring an exact match of the number of joints before and after. Adds a new method to Meshcat to fetch all slider names to enable the above behavior. Also fixes docstrings relating to accessing the visualizer's parser.
New features
...
* [fix] TBD [py manipulation] Condense into a single module ([#18973][_#18973])

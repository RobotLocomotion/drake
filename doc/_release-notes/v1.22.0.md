...
* [fix] TBD [snopt] Nerf gfortran argument-mismatch errors into warnings ([#20330][_#20330])  # SNOPT 7.6 has broken code that gfortran previously accepted without complaint but is now an error as of gfortran 10 or newer. Since the code apparently works anyway, nerf the error down to a warning so that users can still build with SNOPT 7.6. (Drake does not officially support building with 7.6, but since the code for 7.4 is so much more difficult to obtain than 7.6 we try to accommodate 7.6 on a best-effort basis.)
* [newly deprecated fix] TBD [workspace,tools,setup] [workspace] Build libtiff_internal from source ([#20331][_#20331])  # Deprecate libtiff from the host OS and remove it from setup prereqs.
...
<!--
  ...
  Current newest_commit 47cd611a79e91ea4a0d3cf01a2b0d7d51aeb5167 (inclusive).
-->

...
* [newly deprecated] TBD [framework] Deprecate LeafSystem update dispatcher overrides ([#20094][_#20094])  # Publish events, discrete update events, and unrestricted update events must be declared with handler callbacks; overriding the dispatcher to handle events is no longer supported and will be removed after the deprecation window.
New features
...
* [fix] TBD [setup,tools] [setup] Add nasm to source prereqs ([#20353][_#20353])  # This will be required to build libjpeg-turbo from source.
* [newly deprecated] TBD [setup,workspace] [workspace] Deprecate the expat external ([#20351][_#20351])  # As of df39f120, Drake no longer needs this at all.
...
* [fix] TBD [workspace,tools] [workspace] Build VTK-vendored third-party code with hidden symbols ([#20346][_#20346])  # - vtkdoubleconversion - vtkglew - vtkpugixml Instead of patching vtkutf8 to be hidden, remove it from the build entirely. As it turns out, we never actually needed it.
...
<!--
  ...
  Current newest_commit 3cfe0b0710866715c243094c24d0d41adff0bbdf (inclusive).
-->

...
* [feature] TBD [parsing] Add model directives support to Parser ([#17623][_#17623])  # Move the bulk of the model directives implementation into a new internal module, and allow both the Parser APIs and the standalone model directives APIs to use it. Update documentation, and add some minimal tests of model directives via Parser. A future effort could migrate some tests onto the internal module directly.
New features
...
* [feature] TBD [manipulation,workspace] Add joint sliders and dual geometry capability to show_model ([#18044][_#18044])  # Extend the show_model script to cover all of the capabilities of geometry_inspector and remove superfluous code from the latter script: - show_model now provides joint sliders in meshcat. - show_model now always provides a meshcat connection. - show_model now visualizes both visual and collision geometry. - show_model now supports --position for setting sliders. - show_model now includes a --test option. This change also removes the --meshcat and --meshcat_role options, as they are no longer required.
...
* [fix] TBD [workspace] Update PETSc to latest release v3.18.0 ([#18038][_#18038])

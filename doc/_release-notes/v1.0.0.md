...
* TBD Allow start_new_change_event() on const Context ([#16619][_#16619])
New features
...
* TBD [geometry,multibody] [geometry] Add in place transform for volume mesh ([#16603][_#16603])
New features
...
* TBD [pydrake] Overhaul logging configuration ([#16577][_#16577])  # Add a new function pydrake.common.configure_logging() that will set up a nice formatter style, somewhat like Drake C++ offers by default. Don't perform metadata formatting (e.g., times and severities) in C++ when logging via Python. Allow the Python user's configuration to handle that. Rewrite the unit test to be comprehensive. Update meldis to use the new infrastructure. When pydrake's native code is being initialized, only re-route logging to Python if the C++ logging configuration is unchanged vs its defaults. If the user has customized C++ logging already, then don't change anything. This should only make any difference in the case of embedding pydrake as a scripting language.
...
<!--
  ...
  Current newest_commit 207faaf25a43396a02396f0e22e8bfab1c1865c6 (inclusive).
-->

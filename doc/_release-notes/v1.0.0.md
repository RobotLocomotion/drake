...
* Hydroelastic Contact model is now released!
  ...
  * Rigid-compliant and compliant-compliant contact both supported.
    (Rigid object can be any mesh; compliant currently limited to primitive
    shapes.)
...
* TBD [solvers,multibody] Fix noncompliant name kInfeasible_Or_Unbounded ([#16688][_#16688])  # Also bind MathematicalProgramResult.set_solution_result. The old name is deprecated.  Uses of the old name are rectified. One test is added to ensure old code still compiles. The bindings are updated likewise.  However we also bind set_solution_result in order to test these.
* TBD [solvers,geometry] [solvers] Add PerspectiveQuadraticCost and include in GraphOfConvexSets cost options ([#16679][_#16679])
New features
...
* TBD [geometry] Add cleanup logic for unexpected websocket thread exit ([#16670][_#16670])  # This is a complex feature; the pull request discussion contains additional details.
* TBD [geometry/optimization] Enable linear costs in GraphOfConvexSets ([#16680][_#16680])
New features
...
* yaml::YamlReadArchive constructors that accept a YAML::Node ([#16057][_#16057], [#16673][_#16673])
...
<!--
  ...
  Current newest_commit 290b929d3a9a20af2a625b04cb06ad3d1d358f0b (inclusive).
-->

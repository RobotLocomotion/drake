...
* [feature] TBD Fix SemiExplicitEulerIntegrator to support Expression scalar type ([#22751][_#22751])
* [fix] TBD Avoid accessing nullptr in SemiExplicitEulerIntegrator constructor ([#22743][_#22743])
New features
...
* [feature] TBD [multibody] Add PackageMap overloads for filesystem and pathlib ([#22731][_#22731])
* [feature] TBD [multibody] Remove all temporal setup limitations on PD gains ([#22715][_#22715])
New features
...
* [fix] TBD Make IrisZo better handle the case where the ellipsoid center is invalid (in-collision or violates user-specified constraints) ([#22737][_#22737])  # We throw an error if the center of the user-provided ellipsoid is invalid. If the center of the ellipsoid is moved to an invalid configuration after an iteration of the algorithm, we terminate early, and return the current polytope.
* [fix] TBD [planning,workspace] [workspace,planning/dev] Update CRU & VGT, re-enable TSAN on voxel self-filter ([#22484][_#22484])  # * (workspace) Update common_robotics_utilities to latest * (workspace) Update voxelized_geometry_tools to latest * (planning/dev) Update voxel_self_filter_test and remove tsan disable
New features
...
* [fix] TBD [workspace,tools,solvers] [workspace] Upgrade crate_universe, clarabel_cpp_internal to latest ([#22732][_#22732])  # Upgrade clarabel_cpp_internal to latest release v0.10.0. Upgrade crate_universe to latest packages. Co-authored-by: Jeremy Nimmer <jeremy.nimmer@tri.global>
...
<!--
  ...
  Current newest_commit bd2d8336dbc70790cfcde2d0d4151fb12be60a5a (inclusive).
-->

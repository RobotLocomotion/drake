...
* [feature] TBD Add AffineSubspace::OrthogonalComplementBasis() ([#19975][_#19975])
New features
...
* Create `AffineSubspace` subclass of `ConvexSet`. ([#19828][_#19828], [#19850][_#19850], [#19851][_#19851], [#19864][_#19864])
...
* [fix] TBD [geometry] Increase range of Meshcat's search for an open port ([#19960][_#19960])  # (Some people want to have more than 100 Meshcat servers per host.) Support port == 0 to let the kernel automatically select an arbitrary port.  This should be a lot faster than linear probing.
* [feature] TBD [geometry,multibody] Support collision filtering for deformable geometries ([#19897][_#19897])  # So far, deformable geometries are ignored by the collision filtering mechanism. With this commit, SceneGraph users can now opt into collision filtering for deformable geometries through CollisionFilterDeclaration with the appropriate CollisionFilterScope. In particular, MultibodyPlant default collision filters still exclude deformable geometries by using the `kOmitDeformable` scope. As a result, there is no behavior change unless a collision filter for deformable geometries is explicitly introduced.
New features

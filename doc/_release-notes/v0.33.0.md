...
* TBD [systems/framework] LeafSystem::DeclareVector*Port(name, size, ...) ([#15538][_#15538])  # Adds sugar to create ports based on an integer size rather than forcing users to pass a BasicVector<T>(size). Resolves #10751.
* TBD [systems,manipulation,examples,tutorials,multibody] [systems/framework] Use LeafSystem::DeclareVector*Port(name, size) ([#15538][_#15538])  # Note: there are also many instances of DeclareInputPort(name, kVectorValued, size) that could be spelled DeclareVectorInputPort(name, size).  I've not changed those, but could.
* TBD Adapt uses of reset_after_move vs Eigen 3.4 prerelease ([#15556][_#15556])  # In Eigen 3.4, implicit index conversion from reset_after_move may fail. This is like f0cdd44674c6c4c4f05ae1b3a4e4bfc935ba0a31, but for new code written after that fix was made.
* TBD primitives: Introduce VectorLogSink ([#15523][_#15523])  # Relevant to: #10228 This is step 2 of 4 to deprecate and replace SignalLogger and SignalLog with a logging system that avoids threading hazards (SignalLogger) and slow memory management (SignalLog). VectorLogSink will eventually replace SignalLogger.  The chief improvement is relocation of log storage from member data in a System(!) to storage per-context via a scratch cache entry. The API for triggering log records is modernized.  The direct-to-log accessors and mutators are removed, but access is clarified and still convenient via {Get,Find}[Mutable]Log methods.
New features
...
* TBD GurobiSolver respects CommonSolverOptions ([#15497][_#15497])
New features
...
* TBD SDF parsing uses set unrecognized elements policy to warning ([#15317][_#15317])
New features
...
* TBD Gives SortedPair compatibility with structured bindings ([#15551][_#15551])  # We can now do: SortedPair<Foo> pair{Foo(1), Foo(2)}; const auto& [a, b] = pair;
...
<!--
  ...
  Current newest_commit 54b30fceceede1f8c8dc1dc716ba4949bf9e193a (inclusive).
-->

...
* TBD [systems/framework] Sugar for setting/getting Eigen::VectorX data from DiscreteValues ([#15539][_#15539])  # Related to #9171 Removes one level of indirection required in writing a discrete variable update method.  Now one can set/get the Eigen::VectorX directly, without having to use `discrete_values->get_vector()->set_value(...)`, etc. Importantly, this also makes it possible to do more things without having to ever think about `BasicVector`.
* TBD [systems,examples,multibody,manipulation] [systems/framework] Use DiscreteValues Eigen::VectorX sugar ([#15539][_#15539])  # This second commit applies the sugar in appropriate places throughout the codebase.
New features
...
* TBD [geometry] Deprecate SceneGraphInspector::all_frame_ids() ([#15525][_#15525])  # * Deprecate SceneGraphInspector::all_frame_ids() The old API was leaking internal:: classes into the public API. The `GeometryState::FrameIdRange` is an alias for internal::MapKeyRange which, itself, is templated on internal::InternalFrame -- so doubly sinful. And when we push GeometryState into internal:: it'll be three sins. This deprecates the API and makes it match GetAllGeometryids (in terms of behavior and performance). This required changes to the MapKeyRange itself in order to instantiate vector(range.begin(), range.end()) -- it supports the std::iterator_traits directly. Also deprecates the bindings, binds the new API an updates calls to the old API to the new API.
* TBD [parsing] Require exactly one model or world ([#15540][_#15540])  # The previous behavior of allowing for multiple models was deprecated, and is now removed.
* TBD [geometry] Finish deprecation of "is gradient unique" from signed distance queries ([#15532][_#15532])  # This removes the deprecated member indicating whether the gradient of a signed distance query is "unique".
New features
...
* TBD [common] Deprecate DRAKE_THROW_UNLESS on a bare pointer ([#15513][_#15513])  # For a raw pointer (e.g., 'foo' in 'void DoStuff(Foo* foo);'), when checking for null we'd like to insist on an explicit mention of nullptr: Correct: DRAKE_THROW_UNLESS(foo != nullptr); Incorrect: DRAKE_THROW_UNLESS(foo); Because the macro is intended to provide a useful exception message to users, we should err on the side of extra detail about the failure condition.  The meaning of "foo" isolated within error message text does not make it clear that a null pointer is the proximate cause of the problem.
...
* TBD [workspace] Upgrade styleguide latest commit ([#15547][_#15547])
* TBD [tools,multibody,manipulation] [workspace] Upgrade gtest to latest release 1.11.0 ([#15544][_#15544])  # Patch for upstream filed at google/googletest#3472. This re-applies 92bb2a6, which was previously reverted. Add printers for value-parameterized tests, to avoid printing uninitialized memory.
* TBD [workspace] Upgrade rules_pkg to latest release 0.5.0 ([#15548][_#15548])
...
* Remove lcmtypes_bot2_core ([#15515][_#15515])
* Remove lcmtypes_robotlocomotion ([#15515][_#15515])
...
<!--
  ...
  Current newest_commit fc40d614a062380c0758dd5a2cf43db345539399 (inclusive).
-->

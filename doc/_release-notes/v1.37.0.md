...
* [feature] TBD [workspace,tools,cmake] [build] Use eigen, fmt, spdlog from bzlmod ([#22432][_#22432])  # As Drake's public dependencies, it's important that we depend on eigen, fmt, and spdlog as modules instead of module extensions. This allows Bazel's MVS algorithm to coordinate our versions to be compatible with anything else in the downstream user's project. When Drake is consumed as a bzlmod external, now by default Drake will use eigen, fmt, and spdlog as modules from the Bazel Central Registry. Bazel users may use our new @drake//tools/flags options to opt-in back to using pkg-config instead of modules. (Users can also use bzlmod's standard MODULE.bazel override functions and/or command line flags to replace modules.) When Drake is consumed as a CMake external, the find_package() default behavior remains intact (not using bzlmod). We also now offer a flag in @drake//tools/flags to opt-out of spdlog entirely. Users who wish to (in their own code) depend on the exact libraries that Drake is using may continue to use the drake_dep_repositories module extension in their MODULE.bazel file, or may instead use new aliases that do not require any extra text in MODULE.bazel: - @drake//tools/workspace/eigen - @drake//tools/workspace/fmt - @drake//tools/workspace/spdlog Upgrade rules_cc to 0.1.0 (for MODULE only, not WORKSPACE). The DRAKE_OS environment variable no longer has any effect. Details: We delete our custom package.BUILD.bazel rules for these modules (used only by our wheel builds) in lieu of the Bazel Central Registry rules. As a side effect, our (compressed) wheels are now about 1-2% smaller because spdlog is part of libdrake.so instead of libdrake_spdlog.so. Because we build these from source as modules now, we'll use the rules_license machinery to install the license notices. However, that means for CMake find_package() we don't know what to install so we create a stub license file instead.
...
<!--
  ...
  Current newest_commit 95e9e34e5deb97ac5daccb2c939da25adfb00a1f (inclusive).
-->

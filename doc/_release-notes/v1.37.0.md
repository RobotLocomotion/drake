...
* SAP is now the default contact solver for discrete MultibodyPlant. This may
  cause different simulation results for users who rely on the default contact
  solver/approximation ([#21294][_#21294]).
...
* [feature] TBD Adds MathematicalProgram::AddQuadraticCost w*|x-x_d|^2 ([#22424][_#22424])  # Adds one more entry point to make the most common case cleaner. Mostly this will improve code readability.
* [newly deprecated] TBD [solvers] Fix MobyLCPSolver to use Drake logging not cerr ([#22359][_#22359])  # Deprecate its bespoke SetLoggingEnabled.
* [fix] TBD [solvers,planning] Relax the tolerance in IpoptSolver ([#22426][_#22426])  # Previously we set the tolerance to 1.05E-10, this tolerance is too tight and causing numeric problems. We switch back to IPOPT default tolerance.
New features
...
* [feature] TBD [geometry,solvers] Add functionality to create Convex shapes from vertices ([#22390][_#22390])  # Enable geometry::VPolytope to use it to create its corresponding Convex shape.
* [feature] TBD Use AddMultibodyPlantConstraints in InverseKinematics ([#22361][_#22361])  # Also moves the joint limits / joint locking logic from the IK constructor to AddMultibodyPlantConstraints. Towards #18917.
* [feature] TBD [mujoco parser] implement "ref" for joints ([#22446][_#22446])  # The Cassie model in the menagerie sets the default positions of some joints using this element. The default positions define the loop joints. The loop joints were visibly incorrect without this implementation.
New features
...
* [feature] TBD [common,doc] [common] Enable AVX512 when available ([#22423][_#22423])  # Offer environment variables to opt-out.
...
* [fix] TBD Add bindings for based on Convex from points ([#22442][_#22442])  # For release engineers, this includes: - Convex::Convex(points) - VPolytope::ToShapeConvex()
New features
...
* [fix] TBD [build] CMake prefers find_package over pkg-config ([#22421][_#22421])  # In other words, change the default value for CMake's WITH_USER_EIGEN and WITH_USER_FMT and WITH_USER_SPDLOG from OFF to ON. This means the CMake's find_package logic is used instead of our Bazel logic that wraps pkg-config.
...
* [fix] TBD [workspace] Upgrade drake_models to latest commit ([#22408][_#22408])  # Bringing in https://github.com/RobotLocomotion/models/pull/73. This adds textures to the Atlas model.
* Require Bazel >= 7.4 ([#22305][_#22305])
* Use `MODULE.bazel` for all dependencies ([#22338][_#22338])
* Provide new Bazel flags to console solver dependencies ([#22393][_#22393])
...
<!--
  ...
  Current newest_commit d958145386302bac6524fec7b04dc3ae4638d9a1 (inclusive).
-->

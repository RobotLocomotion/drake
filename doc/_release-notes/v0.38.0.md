...
* TBD [primitives] Deprecate RandomSourced alias ([#16359][_#16359])  # Users should spell out RandomSource<double> instead
New features
...
* TBD [multibody,examples] Updates to unrecognized sdf elements EnforcementPolicy ([#16347][_#16347])  # The target date for setting a warning to error has been shifted back (2022-02-01). This includes more information in SDFormat error messages (on par with the information shown in warnings). Some of these fixes involved removing unparsed elements. No function This also fixes models that were issuing warnings, at least as far as they're exercised in unittests. This involved removing elements that were unparsed which should have no functional effect. Co-authored-by: Eric Cousineau <eric.cousineau@tri.global>
* TBD [geometry] Implements `CalcVolume` for basic `Shape`s ([#16354][_#16354])
* TBD VPolytope implements convex geometry ([#16351][_#16351])
...
* TBD [pydrake] Bind SystemVisitor and Diagram introspection methods ([#16352][_#16352])  # This should enable us to explore solutions to #13874 completely in python.
New features

...
* [feature] TBD [systems,common,multibody] [controllers] InverseDynamics doxygen and scalar support ([#18271][_#18271])
* [newly deprecated feature] TBD [systems,multibody] [framework] Add the ability to deprecate input and output ports ([#18274][_#18274])
* [feature] TBD [systems,multibody] [controllers] Implements JointStiffnessController ([#18272][_#18272])  # Towards #11955. This is almost certainly a better model for the iiwa controller than the InverseDynamicsController model that we've been using forever. Also adds transmission elements to multibody/benchmarks/acrobot/double_pendulum to enable testing the controllers.
New features
...
* [fix] TBD MosekSolver aggregates duplicated variables in constraints ([#18280][_#18280])
* [fix] TBD Gurobi handles duplicated variables in its linear constraints ([#18291][_#18291])
New features
...
* [newly deprecated fix] TBD Deprecate FixedOffsetFrame::SetPoseInBodyFrame() for SetPoseInParentFrame(). Add GetPoseInBodyFrame()(#18198) ([#18198][_#18198])  # This resolve issue #15638. Also added more tests to check situations in which the parent frame differs from the body frame. Co-Authored-By: mitiguy <paul.mitiguy@tri.global>
* [fix] TBD Improve MultibodyPlant detection/feedback for invalid mass/inertia properties ([#18153][_#18153])  # While running forward dynamics in continuous mode, throw an exception if invalid mass/inertia properties are detected in Articulated Body Algorithm. Make progress towards a more general solution for detecting near-singular mass/inertia properties during dynamic simulation (to be addressed in a future PR).  PR# 18153 is progress towards resolution of issue #17081. Co-Authored-By: mitiguy <paul.mitiguy@tri.global> Co-Authored-By: Sean Curtis <sean.curtis@tri.global>
* [feature] TBD [meshcat] Add support for gamepads in Meshcat ([#18243][_#18243])  # The meshcat javascript reads the gamepad info and sends it to the meshcat server whenever a change is detected.  The c++ meshcat interface provides access to the most recent gamepad status. This feature is provided primarily to support applications where Drake is running on a remote machine (e.g. in the cloud), and the Meshcat javascript in the browser is the only code running on the local machine which has access to the gamepad.
* [feature] TBD [render_gltf_client] Add 16U PNG and TIFF depth support ([#18205][_#18205])  # Co-authored-by: Zach Fang <zach.fang@tri.global>
New features
...
* Add KinematicTrajectoryOptimization ([#18190][_#18190])
...
* [feature] TBD Parallelize pointcloud operations with OpenMP and use CRU DSHVG for voxel filter ([#18216][_#18216])  # * Parallelize EstimateNormals and VoxelizedDownSample using OpenMP * Use CRU DSHVG to implement VoxelizedDownSample
* [feature] TBD [trajectories] Add StackedTrajectory ([#18192][_#18192])
New features
...
* [feature] TBD py systems: Bind SetStateAndParametersFrom ([#18267][_#18267])
* [feature] TBD Add a Stop Running button and Meshcat reuse to ModelVisualizer ([#18284][_#18284])
* [fix] TBD [visualization] Do not show stack trace for meldis on sigint ([#18293][_#18293])
* [fix] TBD [pydrake] Remove spurous keep-alive annotations ([#18301][_#18301])  # The function takes its arguments by reference and internally copies them. There's no need to any keep_alive annotations.
New features
...
* [fix] TBD [setup] Remove errors on Ubuntu arm64 ([#18261][_#18261])
* [fix] TBD [doc,tools,examples,workspace,.github] [workspace] Drop macOS Big Sur support ([#18299][_#18299])  # - Minimum supported macOS is now: Monterey. - Minimum supported Xcode is now: 14.0.1. - CMake will warn (rather than error) on unsupported macOS. - Advertise existing macOS arm64 wheels (monterey and later).
New features
...
* [feature] TBD [workspace,geometry] [meshcat] Update Meshcat sha (now using msgpack-javascript) ([#18269][_#18269])  # This update includes my recent meshcat PR https://github.com/rdeits/meshcat/pull/132, which switches meshcat to using msgpack-javascript instead of msgpack-lite.  Resolves #17861. Importantly, I've also exported the msgpack requirement via meshcat.js, so that we don't need to build it again ourselves here. msgpack-lite included the msgpack.min.js in its github distribution, but msgpack-javascript does not.  I would have needed to pull it from some CDN or NPM server, or build it locally (which would require node as a dependency).
* [fix] TBD [workspace] Upgrade usockets and uwebsockets to latest commit ([#18259][_#18259])
* [fix] TBD Revert "[workspace] Upgrade usockets and uwebsockets to latest commit" ([#18289][_#18289])  # This reverts commit 285452ad162100886322324e594dbb7fc83cdb91.
* [fix] TBD [workspace] Upgrade curl_internal to latest release 7.86.0 ([#18285][_#18285])
* Fully evict pydrake from drake-visualizer ([#18144][_#18144])
...
[_#18259]: https://github.com/RobotLocomotion/drake/pull/18259
...
[_#18289]: https://github.com/RobotLocomotion/drake/pull/18289
...
<!--
  ...
  Current newest_commit 36817f86b7e1fae897bdfd7fa4930159e0461c90 (inclusive).
-->

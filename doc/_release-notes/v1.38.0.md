...
* [fix] TBD SceneGraph protects itself from non-finite values on its inputs ([#22592][_#22592])  # The various kinematics vectors (pose and deformable configuration) can now detect if they have been populated with any non-finite values. SceneGraph uses that functionality to detect if its input has non-fininte values throws exceptions attributing the bad input to the appropriate input port.
New features
...
* [feature] TBD Complete GetContinuousRevoluteJointIndices, adding support for the last two compatible joints (UniveralJoint and BallRpyJoint) ([#22585][_#22585])
New features
...
* [feature] TBD Make ModelVisualizer show a .gltf file directly ([#22596][_#22596])  # Co-authored-by: Jeremy Nimmer <jeremy.nimmer@tri.global>
New features
...
* [fix] TBD [tools,workspace,cmake,systems,doc] [build] Optionally build zlib from source ([#22588][_#22588])  # Switch the wheel build to use the source build. Even though zlib is part of the required set of dynamic libraries wheels are allowed to universally rely on, it's easier to just remain consistent and set all of the options the same way. This is the last of the tools/wheel/image/dependencies, so we also remove all of that infrastructure as well.
...
<!--
  ...
  Current newest_commit c06a99bb8f188251596226a627f13ffad24a6fa5 (inclusive).
-->

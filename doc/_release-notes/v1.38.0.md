...
* [breaking change] TBD Convert kintrajopt to use AsLinearInControlPoints ([#22552][_#22552])  # Resolves #22533. This includes a breaking change. The vector<vector<Binding>> return types for KinematicTrajectoryOptimization AddAccelerationBounds and AddJerkBounds are now vector<Bindings>.
New features
...
* [fix] TBD [setup] Upgrade Python venv (mac) to latest ([#22570][_#22570])
* [fix] TBD [build] Document the new "spdlog disabled" flag as experimental ([#22577][_#22577])
* [fix] TBD [setup] Only prefetch bazel when its been installed ([#22575][_#22575])
...
* [fix] TBD [workspace] Upgrade abseil_cpp_internal to latest commit ([#22545][_#22545])
* [fix] TBD [workspace] Upgrade mujoco_menagerie_internal to latest commit ([#22545][_#22545])
* [fix] TBD [workspace] Upgrade nanoflann_internal to latest release v1.6.3 ([#22545][_#22545])
* [fix] TBD [workspace] Upgrade tinygltf_internal to latest release v2.9.5 ([#22545][_#22545])
* [fix] TBD [workspace] Upgrade libpng_internal to latest release v1.6.46 ([#22545][_#22545])
* [fix] TBD [workspace] Upgrade drake_models to latest commit ([#22545][_#22545])
* [fix] TBD [workspace] Upgrade ros_xacro_internal to latest release 2.0.12 ([#22545][_#22545])
* [fix] TBD [workspace,tools] [workspace] Upgrade crate_universe to latest ([#22545][_#22545])
* [fix] TBD [workspace] Upgrade buildifier to latest release v8.0.2 ([#22545][_#22545])
* [breaking change newly deprecated feature] TBD [workspace,tools,geometry] [tools] Switch to Python reimplementation of lcm_gen ([#20761][_#20761])  # This was developed from scratch within Drake to modernize our codegen. Features of Drake's new lcm_gen: - Portable and easy to modify: - Python 3. - High level language. - No compilation step. - Single file. - Depends only on the Python 3 standard library. - Improved C++ output: - C++20 features; faster encode/decode. - Generated code has no dependencies other than the standard library. - Uses std::array<> instead of raw C arrays. - Multi-dimensional arrays with a mix of fixed and variable sizes use std::array<> for the fixed-sized element, not std::vector. This version only supports C++ codegen, because the upstream codegen is so bad. We could imagine adding Python and/or Java codegen later, if we need it. Breaking change: Drake's generated C++ LCM message headers now use std::array. The class drake::EigenMapView might help with porting. Important change: Consuming Drake as a CMake external will no longer find and depend on LCM. Newly deprecated: After 2025-05-01, Drake will no longer install LCM.
* [newly deprecated feature] TBD [workspace,tools,cmake,doc] [build] Optionally build blas and lapack from source ([#22529][_#22529])  # New CMake options WITH_USER_BLAS and WITH_USER_LAPACK are available (except on macOS); they default to ON to match existing behavior. New Bazel flags private_runtime_repo_default, blas_repo, lapack_repo are now available. For Drake Developers their defaults match existing behavior, but downstream projects now default to building from source. Wheel builds now use our own source build of BLAS and LAPACK (and so, no longer copy the Ubuntu libraries into our wheel). Deprecate the libblas and liblapack repositories.
...
<!--
  ...
  Current newest_commit 1c496a395ea95e6cc9d8d2ddccd4fa098d8323bd (inclusive).
-->

...
* [feature] TBD Introduces MinCliqueCoverSolver and MinCliqueCoverSolverViaGreedy ([#21892][_#21892])  # Introduces an interface for solving the minimum clique cover problem as well as provide an implementation of the solver which approximately solves the problem by iteratively removing the largest clique.
* [feature] TBD Adjust GcsTrajectoryOptimization::AddEdges and AddRegions to automatically compute edge offsets if they're not provided ([#21946][_#21946])
...
* [fix] TBD [py examples] Fix signatures to use Python types (not C++) ([#21918][_#21918])  # Adjust bindings to follow topological order, so that function signatures can look up their requisite python types.

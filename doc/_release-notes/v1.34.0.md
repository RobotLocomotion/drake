...
* [feature] TBD Python binding for Body::CalcCenterOfMassTranslationalVelocityInWorld() ([#21926][_#21926])  # Co-Authored-By: mitiguy <paul.mitiguy@tri.global>
...
* [fix] TBD Fix a bug in SchunkGripper ([#21925][_#21925])  # Update trajectory when trajectory_ is empty, so that the gripper can close its fingers at the beginning of the simulation.
* TBD
...
* [fix] TBD [py math] Fix signatures to use Python types (not C++) ([#21908][_#21908])
* [fix] TBD [pydrake] Fix signatures to use Python types (not C++) ([#21911][_#21911])  # Add missing imports to bindings, so that function signatures can look up their requisite python types. (This set of changes is the set of modules whose sole problem was missing imports.)
* [fix] TBD [py optimization] Fix nested class declaration order ([#21913][_#21913])  # Nested classes must always be declared inbetween their outer class's declaration and outer class's definition. Otherwise, functions of the outer class will not be able to look up the types. This commit shuffles some code around, with no other changes.
* [fix] TBD [py geometry] Fix signatures to use Python types (not C++) ([#21907][_#21907])  # Adjust bindings to follow topological order, so that function signatures can look up their requisite python types. This is not a 100% fix -- dependencies on other modules (perception, multibody) still show up as dangling due to module cycles.
* [fix] TBD [py planning] Fix signatures to use Python types (not C++) ([#21916][_#21916])  # Adjust bindings to follow topological order, so that function signatures can look up their requisite python types.
* [fix] TBD [py solvers] Fix signatures to use Python types (not C++) ([#21909][_#21909])  # Adjust bindings to follow topological order, so that function signatures can look up their requisite python types.
* [fix] TBD [py optimization] Fix signatures to use Python types (not C++) ([#21919][_#21919])  # Adjust bindings to follow topological order, so that function signatures can look up their requisite python types.
* [fix] TBD [py rational] Fix signatures to use Python types (not C++) ([#21917][_#21917])  # Adjust bindings to follow topological order, so that function signatures can look up their requisite python types.
* [fix] TBD Add two missing bindings for MbP ([#21933][_#21933])
* [fix] TBD [py framework] Fix signatures to use Python types (not C++) ([#21921][_#21921])  # Adjust bindings to follow topological order, so that function signatures can look up their requisite python types.
* [fix] TBD [py common] Improve a yaml_dump error message ([#21935][_#21935])
* [fix] TBD [py schema] Add support for yaml_dump_typed on Transform ([#21934][_#21934])
...
* [fix] TBD [workspace] Re-fetch when the is_wheel_build setting changes ([#21938][_#21938])  # Repository rules that use is_wheel_build to influence the fetching should declare the dependency on the related environment variable.
* TBD
...
<!--
  ...
  Current newest_commit d96a2e0c89bf7a45d76da7bed2956c1ddc0734a8 (inclusive).
-->

...
* TBD Adds suffix strings to give unique names to each position/velocity el… ([#14971][_#14971])  # * [multibody] Provide suffix strings for each position/velocity in MultibodyPlant This PR plumbs through the mobilizer/joint class hierachy in order to accomplish a simple goal: I want to have string names for each element of the multibody context.  We already have names for bodies and joint, but (floating) bodies and joints can have more than one position/velocity. Here we introduce a "suffix" for each position/velocity element which can be combined with the body/joint name to obtain a meaningful position / velocity name.
* TBD [hydro] Enable polygon contact surface in mesh-half space intersection ([#16164][_#16164])  # * Enable polygon contact surface in mesh-half space intersection This reframes mesh-half space intersection to use the new MeshBuilder so it can create either polygon or triangle mesh representations. The APIs have been modified to communicate the desired representation and the tests have been updated accordingly. This entailed changes to the organization of the code so it more closely resembled mesh-plane intersection. Various related call sites have been updated to *explicitly* request triangle meshes (this was implicit before).
New features
...
<!--
  ...
  Current newest_commit 966b2c264753530b78506441f28397ab95df1cf1 (inclusive).
-->

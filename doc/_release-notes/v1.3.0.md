...
* TBD [control] Improve error message for planning and control input port selections ([#17191][_#17191])  # If I had a nickel for every time somebody called, e.g. DirectCollocation with a MultibodyPlant and did not understand the error message... Previously ``` from pydrake.all import MultibodyPlant, DirectCollocation, Parser, FindResourceOrThrow plant = MultibodyPlant(0.0) Parser(plant).AddModelFromFile( FindResourceOrThrow('drake/examples/multibody/cart_pole/cart_pole.sdf')) plant.Finalize() context = plant.CreateDefaultContext() DirectCollocation(plant, context, 10, 0.1, 1.0) ``` would result in ``` Failure at systems/framework/system.cc:40 in AllocateInputVector(): condition 'input_port.get_data_type() == kVectorValued' failed. ``` Now DirectCollocation and the related methods provide a more helpful error message string.
New features
...
* TBD [geometry/optimization] Minor cleanup of `GraphOfConvexSets::GetGraphvizString` ([#17190][_#17190])  # Previously, when one called `GraphOfConvexSets::GetGraphvizString` with `result=None`, they would see a spurious `]` in the vertex labels. The parenthesis is now placed properly inside the conditional. Also adds a missing default for `result` in the python binding.
* TBD [meshcat] Remove console spam in Run with timeout ([#17192][_#17192])
* TBD [geometry/optimization] Make `IrisOptions.enable_ibex = false` by default ([#17198][_#17198])  # Previously the default was to have Ibex enabled, but this has fallen out of favor -- we find that Ibex cannot solve many problems of interest. In addition, we allow the test that requires Ibex to pass quietly if IbexSolver is not available + enabled.  This is required for tests to pass on mac arm64.
...
* TBD [manipulation] add a visualization of frames in show_model.py ([#17141][_#17141])
* None
...
* TBD Add python binding for Parser::SetStrictParsing ([#17181][_#17181])  # Relevant to: #17107
* TBD [bindings] Add bindings for MathematicalProgram variable scaling ([#17200][_#17200])
...
* TBD Remove -lquadmath when building on mac arm64 ([#17196][_#17196])  # Towards #17009, where jwnimmer-tri commented "I just peeked at https://packages.ubuntu.com/focal/libgfortran5 and can confirm that quadmath is [amd64, i386] only. When we're building on ARM64, we should neither try to find -lquadmath, nor link to it."
...
[_#17141]: https://github.com/RobotLocomotion/drake/pull/17141
...
[_#17196]: https://github.com/RobotLocomotion/drake/pull/17196
...
<!--
  ...
  Current newest_commit 2f6d0ddaf7c929a73fdc0a888a2e9286e002aa00 (inclusive).
-->

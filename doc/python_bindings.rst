...
.. warning::
   Drake is currently incompatible with the Python environment supplied by
   Anaconda. Please uninstall Anaconda or remove the Anaconda ``bin`` directory
   from the ``PATH`` before building or using the Drake Python bindings.

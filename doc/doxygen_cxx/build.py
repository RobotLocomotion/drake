...
def _generate_doxyfile(*, drake_workspace, out_dir, temp_dir, dot):
    """Creates Doxyfile_CXX from Doxyfile_CXX.in."""
    # TODO(jwnimmer-tri) Use Rlocation instead of drake_workspace.
    input_filename = join(drake_workspace, "doc/doxygen_cxx/Doxyfile_CXX.in")
    ...
    check_call([
        sys.executable,
        join(drake_workspace, "tools/workspace/cmake_configure_file.py"),
...
def _build(*, out_dir, temp_dir, quick, modules):
    ...
    drake_workspace = os.path.dirname(r.Rlocation("drake/.bazelproject"))
    assert os.path.exists(drake_workspace), drake_workspace

    # Find doxygen.
    doxygen = r.Rlocation("doxygen/doxygen")

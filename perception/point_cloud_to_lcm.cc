...
//   it as part of this class, with at cross-check vs the cloud size.
//
...
void Calc(double time, const std::string& frame_name, const PointCloud& cloud,
  ...
  const int num_fields = 3 + (has_rgbs ? 1 : 0) + (has_normals ? 3 : 0);
  message->num_fields = num_fields;
  message->fields.resize(num_fields, lcmt_point_cloud_field{});
  {
    int current_field = 0;
  ...
  message->data.clear();
  {
    const int current_align = message->getEncodedSize() % 16;
    const int filler_size = (16 - current_align) % 16;
  ...
  message->data.resize(num_points * point_step);
  ...
  const int data_size = cursor - message->data.data();
  DRAKE_DEMAND(data_size == (count * point_step));

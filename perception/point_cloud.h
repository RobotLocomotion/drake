...
class PointCloud final {
 public:
  ...
  /// @returns true iff all points were assigned normals at least three nearest
  /// neighbors.
  ///
  /// @pre @p radius > 0 and @p max_nearest_neighbors >= 3.
  /// @throws std::exception if has_xyzs() is false.
  bool EstimateNormals(double radius, int max_nearest_neighbors);

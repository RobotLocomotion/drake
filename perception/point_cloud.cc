...
// Finds different fields introduced by `new_fields` w.r.t. `old_fields`.
pc_flags::Fields FindDifferentFields(pc_flags::Fields old_fields,
  ...
  if (old_fields.descriptor_type() != new_fields.descriptor_type()) {
...
void PointCloud::SetFields(pc_flags::Fields new_fields, bool skip_initialize) {
  ...
  if (!skip_initialize) {
    ...
    pc_flags::Fields added_fields = FindDifferentFields(old_fields, new_fields);
    if (added_fields.contains(pc_flags::kXYZs))
      mutable_xyzs().middleCols(0, size()).setConstant(kDefaultValue);
    if (added_fields.contains(pc_flags::kNormals))
      mutable_normals().middleCols(0, size()).setConstant(kDefaultValue);
    if (added_fields.contains(pc_flags::kRGBs))
      mutable_rgbs().middleCols(0, size()).setConstant(kDefaultColor);
    if (added_fields.has_descriptor())
      mutable_descriptors().middleCols(0, size()).setConstant(kDefaultValue);
  }
}

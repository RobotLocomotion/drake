...
class PointCloud::Storage {
 public:
  ...
  void UpdateFields(pc_flags::Fields f) {
    ConservativeResizeCols(&xyzs_, f.contains(pc_flags::kXYZs) ? size_ : 0);
    ConservativeResizeCols(&normals_,
                           f.contains(pc_flags::kNormals) ? size_ : 0);
    ConservativeResizeCols(&rgbs_, f.contains(pc_flags::kRGBs) ? size_ : 0);
    if (f.descriptor_type() != fields_.descriptor_type()) {

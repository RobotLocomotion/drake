...
DepthImageToPointCloud::DepthImageToPointCloud(const systems::sensors::CameraInfo& camera_info) : camera_info_(camera_info) {
  ...
  this->DeclareAbstractOutputPort(&DepthImageToPointCloud::MakeOutputPointCloud,
...
void DepthImageToPointCloud::ConvertDepthImageToPointCloud(const systems::Context<double>& context, PointCloud* output) const {
  ...
  const auto& input_image = input_depth_image_ptr->GetValue<systems::sensors::ImageDepth32F>();

...
void DoConvert(
    ...
    const float scale,
    PointCloud* output) {
  // Reset the `output` size and channels, if necessary.
  const int cloud_size = depth_image.size();
  const bool skip_initialize = true;
  if (output->fields() != pc_flags::kXYZs) {
    *output = PointCloud(cloud_size, pc_flags::kXYZs, skip_initialize);
  } else if (output->size() != cloud_size) {
    output->resize(cloud_size, skip_initialize);
  }
  if (depth_image.size() == 0) { return; }
...
void DepthImageToPointCloud::Convert(
      ...
      PointCloud* output) {
  DoConvert(camera_info, camera_pose ? &*camera_pose : nullptr,
...
void DepthImageToPointCloud::CalcOutput32F(
  ...
  DoConvert(camera_info_, pose_or_null, *image, scale_, output);
}

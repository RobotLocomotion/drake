...
DepthImageToPointCloud::DepthImageToPointCloud(
  ...
  camera_pose_input_port_ = this->DeclareAbstractInputPort("camera_pose",
      systems::Value<Eigen::Isometry3d>{}).get_index();
...
void DepthImageToPointCloud::ConvertDepthImageToPointCloud(
  ...
  Eigen::Isometry3f image_to_parent = Eigen::Isometry3f::Identity();
  if (camera_pose_ptr) {
    image_to_parent =
        camera_pose_ptr->GetValue<Eigen::Isometry3d>().inverse().cast<float>();
  }

  const int kNumCols = point_cloud.cols();
  output->resize(kNumCols);
  output->mutable_xyzs() = image_to_parent * point_cloud;

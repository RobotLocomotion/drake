...
#  $failcondition = "1";   # missing dependency => failure
  ...
  print $ctestfile "ADD_TEST($testname \"matlab\" \"-nosplash\" \"-nodisplay\" \"-r\" \"addpath('$CMAKE_INSTALL_PREFIX/matlab'); addpath_$POD_NAME; rng('shuffle'); try, feval('$test'); catch ex, disp(getReport(ex,'extended')); force_close_system; exit($failcondition); end; force_close_system; exit(0)\")\n";

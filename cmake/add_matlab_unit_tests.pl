...
while (<$in>) {
  ...
  print $ctestfile "ADD_TEST($testname \"matlab\" \"-nosplash\" \"-nodisplay\" \"-r\" \"addpath('$CMAKE_INSTALL_PREFIX/matlab'); addpath_drake; try, feval('$test'); catch(ex), disp(getReport(ex,'extended')); force_close_system; exit(-1); end; force_close_system; exit(0)\")\n";

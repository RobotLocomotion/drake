...
# supports the flag.
#------------------------------------------------------------------------------
macro(drake_add_compiler_flag_if_supported FLAG NAME)
...
# flag.
#------------------------------------------------------------------------------
macro(drake_add_compiler_and_linker_flag FLAG NAME)
  set(SAFE_CMAKE_REQUIRED_LIBRARIES ${CMAKE_REQUIRED_LIBRARIES})
  set(CMAKE_REQUIRED_LIBRARIES ${FLAG})
  drake_add_compiler_flag_if_supported("${FLAG}" ${NAME})
...
macro(drake_add_common_memcheck_flags)
  drake_add_compiler_flag_if_supported("-fno-omit-frame-pointer"
    FNO_OMIT_FRAME_POINTER)

  if(NOT CMAKE_CONFIGURATION_TYPES)
    if(CMAKE_BUILD_TYPE STREQUAL "Debug")
      drake_add_compiler_flag_if_supported("-O1" O1)
    endif()

    if(CMAKE_BUILD_TYPE STREQUAL "MinSizeRel" OR CMAKE_BUILD_TYPE STREQUAL "Release")
      drake_add_compiler_flag_if_supported("-gline-tables-only"
        GLINE_TABLES_ONLY)
    endif()
  endif()
  ...
  if(CMAKE_GENERATOR STREQUAL "Ninja" OR CMAKE_GENERATOR STREQUAL "Unix Makefiles")
    ...
    if(INCLUDE_WHAT_YOU_USE_EXECUTABLE)
      ...
      if(USE_INCLUDE_WHAT_YOU_USE)
        set(CMAKE_C_INCLUDE_WHAT_YOU_USE "${INCLUDE_WHAT_YOU_USE_EXECUTABLE}")
...
macro(drake_setup_sanitizers)
  ...
  if(USE_SANITIZER)
    ...
    if(USE_SANITIZER STREQUAL "Address")
      drake_add_compiler_and_linker_flag("-fsanitize=address" FSANITIZE_ADDRESS)
      set(ENV{ASAN_OPTIONS}
        "${SANITIZER_COMMON_OPTIONS}:suppressions=${_tools_dir}/asan.supp:$ENV{ASAN_OPTIONS}")
      set(MEMORYCHECK_TYPE AddressSanitizer)
    elseif(USE_SANITIZER STREQUAL "Leak")
      drake_add_compiler_and_linker_flag("-fsanitize=leak" FSANITIZE_LEAK)
    ...
    elseif(USE_SANITIZER MATCHES "Memory(WithOrigins)?")
      drake_add_compiler_and_linker_flag("-fsanitize=memory" FSANITIZE_MEMORY)

      if(USE_SANITIZER STREQUAL "MemoryWithOrigins")
        drake_add_compiler_and_linker_flag("-fsanitize-memory-track-origins"
    ...
    elseif(USE_SANITIZER STREQUAL "Thread")
      drake_add_compiler_and_linker_flag("-fsanitize=thread" FSANITIZE_THREAD)
    ...
    elseif(USE_SANITIZER STREQUAL "Undefined")
      drake_add_compiler_and_linker_flag("-fsanitize=undefined"
        FSANITIZE_UNDEFINED)
  ...
  if(VALGRIND_EXECUTABLE)
    ...
    if(USE_VALGRIND)
      ...
      if(_use_valgrind_lower MATCHES "(cachegrind|callgrind|drd|hellgrind|massif|memcheck)")
        ...
        set(_trace_children_skip "/bin/*,/usr/bin/*,/usr/local/bin/*")

        if(Matlab_FOUND AND NOT DISABLE_MATLAB)
          if(APPLE)
            set(_trace_children_skip "${_trace_children_skip},${Matlab_ROOT_DIR}/*")
          else()
            set(_trace_children_skip "${_trace_children_skip},${Matlab_ROOT_DIR}/*")
          endif()
        endif()
        ...
        set(VALGRIND_COMMAND_OPTIONS
          "--error-exitcode=${MEMORYCHECK_ERROR_EXITCODE} --tool=${_use_valgrind_lower} --trace-children=yes --trace-children-skip=${_trace_children_skip}")
        ...
        if(PROJECT_NAME STREQUAL "drake")

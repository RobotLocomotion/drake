...
// 3. must include at least one of @p
...
bool IsNLP(const MathematicalProgram& prog) {
  ...
  const bool has_nonconvex_quadratic_cost =
      std::any_of(prog.quadratic_costs().begin(), prog.quadratic_costs().end(),
  ...
  if (no_binary_variable && (has_generic_cost || has_nonconvex_quadratic_cost ||
...
ProgramType GetProgramType(const MathematicalProgram& prog) {
  if (GetRequirementsLP().SatisfiesProgramType(prog.required_capabilities())) {
    return ProgramType::kLP;
  } else if (GetRequirementsQP().SatisfiesProgramType(
                 prog.required_capabilities()) &&
             AllQuadraticCostsConvex(prog.quadratic_costs())) {
    return ProgramType::kQP;
  } else if (GetRequirementsSOCP().SatisfiesProgramType(
                 prog.required_capabilities())) {
    return ProgramType::kSOCP;
  } else if (GetRequirementsSDP().SatisfiesProgramType(
                 prog.required_capabilities())) {
    return ProgramType::kSDP;
  } else if (GetRequirementsGP().SatisfiesProgramType(
                 prog.required_capabilities())) {
    // TODO(hongkai.dai): support more general type of geometric programming,
    // with constraints on posynomials.
    return ProgramType::kGP;
  } else if (GetRequirementsCGP().SatisfiesProgramType(
                 prog.required_capabilities())) {
    return ProgramType::kCGP;
  } else if (GetRequirementsMILP().SatisfiesProgramType(
                 prog.required_capabilities())) {
    return ProgramType::kMILP;
  } else if (GetRequirementsMIQP().SatisfiesProgramType(
                 prog.required_capabilities()) &&
             AllQuadraticCostsConvex(prog.quadratic_costs())) {
    return ProgramType::kMIQP;
  } else if (GetRequirementsMISOCP().SatisfiesProgramType(
                 prog.required_capabilities())) {
    return ProgramType::kMISOCP;
  } else if (GetRequirementsMISDP().SatisfiesProgramType(
                 prog.required_capabilities())) {
    return ProgramType::kMISDP;
  } else if (GetRequirementsQuadraticCostConicProgram().SatisfiesProgramType(
                 prog.required_capabilities()) &&
             AllQuadraticCostsConvex(prog.quadratic_costs())) {
    return ProgramType::kQuadraticCostConicConstraint;
  } else if (GetRequirementsLCP().SatisfiesProgramType(
                 prog.required_capabilities())) {

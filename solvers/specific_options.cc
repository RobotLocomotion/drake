...
/* Extracts one common (Drake) option from `common`. Does nothing if not set,
or if set to the wrong type. */
...
/* Extracts common (Drake) options from `solver_options`. */
CommonSolverOptionValues GetCommonOptions(const SolverOptions* all_options) {
  ...
  if (const auto common_iter = all_options->options.find("Drake");
...
std::optional<Result> SpecificOptions::Pop(std::string_view key) {
  ...
  if (auto iter = respelled_.find(key); iter != respelled_.end()) {
    ...
    if (std::holds_alternative<Result>(value)) {
      std::optional<Result> result = std::get<Result>(value);

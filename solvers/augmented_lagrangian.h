...
class NonsmoothAugmentedLagrangian {
 public:
   ...
   * append the Lagrangian multipler for the lower and upper bounds (where the
   ...
   */
  template <typename T>
  T Eval(const Eigen::Ref<const VectorX<T>>& x, const Eigen::VectorXd& lambda,
         double mu, VectorX<T>* constraint_residue, T* cost) const;

  const MathematicalProgram& prog() const { return *prog_; }

  /**
   * Whether the bounding box constraint x_lo <= x <= x_up is included in the
   * augmented Lagrangian L(x, λ, μ)
   */
  ...
  /**
   * Whether each constraint is equality or not. The order of the constraint is
  ...
  /** Getter for all the lower bounds of x .*/
  const Eigen::VectorXd& x_lo() const { return x_lo_; }

  /** Getter for all the upper bounds of x .*/
  const Eigen::VectorXd& x_up() const { return x_up_; }

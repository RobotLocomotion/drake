...
struct IbexOptions {
  ...
  // http://www.ibex-lib.org/doc/optim.html?highlight=eps#return-status for
  // details.
  bool rigor{false};
  ...
  // Timeout (time in seconds). 0.0 indicates +oo.
  double timeout{0.0}; /* sec */
};
...
IbexOptions ParseOptions(const SolverOptions& merged_options) {
  ...
  for (const auto& it : merged_options.GetOptionsDouble(IbexSolver::id())) {
    ...
    throw std::runtime_error{
        fmt::format("IbexSolver: unsupported option '{option}'.",
                    fmt::arg("option", it.first))};
  }

...
*/
void RemoveDiagonallyInconsistentExponents(
              const ExponentList & exponents_of_p,
              ExponentList * exponents_of_basis) {
  while (1) {
    ExponentList exponents_to_keep(exponents_of_basis->rows(),
                                exponents_of_basis->cols());
    int cnt = 0;
    ExponentList valid_squares = VerticalStack(
        PairwiseSums(*exponents_of_basis), exponents_of_p);
...
Hyperplanes RandomSupportingHyperplanes(ExponentList exponents_of_p,
  ...
  H.normal_vectors = Eigen::MatrixXi::Random(num_hyperplanes,
                                  exponents_of_p.cols())/scale;
  Eigen::MatrixXi dot_products = H.normal_vectors
                    * exponents_of_p.transpose();
...
ExponentList ConstructMonomialBasis(const ExponentList & exponents_of_p) {
  ...
  unsigned int random_seed = 1; std::srand(random_seed);

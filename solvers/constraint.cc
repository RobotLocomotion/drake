...
PositiveSemidefiniteConstraint::PositiveSemidefiniteConstraint(int rows)
  ...
  if (matrix_rows_ == 1) {
    drake::log()->warn(
        ...
        "re-formulate this as a linear inequality constraint for better "
        "speed/numerics.");
  } else if (matrix_rows_ == 2) {
    drake::log()->warn(
        "PositiveSemidefiniteConstraint: rows==2, please consider to "
        "re-formulate this as a rotated Lorentz cone constraint for better "
        "speed/numerics.");
  }
}
...
LinearMatrixInequalityConstraint::LinearMatrixInequalityConstraint(
  ...
  if (matrix_rows_ == 1) {
    drake::log()->warn(
        ...
        "impose it as a linear inequality constraint for better "
        "speed/numerics.");
  } else if (matrix_rows_ == 2) {
    drake::log()->warn(
        "LinearMatrixInequalityConstraint: the matrix has size 2. Please "
        "impose it as a rotated Lorentz cone constraint for better "
        "speed/numerics.");
  }

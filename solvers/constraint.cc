...
void LorentzConeConstraintEvalConvex2Autodiff(
  ...
  const Eigen::VectorXd x_val = math::ExtractValueMatrixFromAutoDiff(x);
  ...
  Eigen::RowVectorXd dy =
      dy_dz * A_dense * math::ExtractGradientMatrixFromAutoDiff(x);
  (*y) = math::InitializeAutoDiffFromValueAndGradientMatrix(y_val, dy);
}
}  // namespace

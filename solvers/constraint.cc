...
LorentzConeConstraint::LorentzConeConstraint(
    ...
    : Constraint(eval_type == LorentzConeConstraint::EvalType::kType3 ? 2 : 1,
                 A.cols(),
                 eval_type == LorentzConeConstraint::EvalType::kType3
...
void LorentzConeConstraintEvalType2Autodiff(
...
void LorentzConeConstraint::DoEvalGeneric(const Eigen::MatrixBase<DerivedX>& x,
  ...
  y->resize(num_constraints());
  switch (eval_type_) {
    case EvalType::kType1: {
      (*y)(0) = z(0) - z.tail(z.rows() - 1).norm();
      break;
    }
    case EvalType::kType2: {
      if constexpr (std::is_same<ScalarY, AutoDiffXd>::value) {
        LorentzConeConstraintEvalType2Autodiff(A_dense_, b_, x, y);
      } else {
        (*y)(0) = z(0) - z.tail(z.rows() - 1).norm();
      }
      break;
    }
    case EvalType::kType3: {
      (*y)(0) = z(0);

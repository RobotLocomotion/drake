...
// Currently clarabel's solution doesn't contain the objective value, see
// https://github.com/oxfordcontrol/Clarabel.cpp/issues/22. When Clarabel fixes
// this problem in the upstream code, remove get_obj_val function and call
// solution.obj_val directly.
double get_obj_val(const clarabel::DefaultSolution<double>& solution,
                   const Eigen::SparseMatrix<double>& P_upper,
                   const Eigen::Map<Eigen::VectorXd>& q, double cost_constant) {
  double cost =
      0.5 * solution.x.dot(
                (P_upper + P_upper.triangularView<Eigen::StrictlyUpper>()) *
                solution.x) +
      q.dot(solution.x) + cost_constant;
  return cost;
}
}  // namespace
...
void ClarabelSolver::DoSolve(const MathematicalProgram& prog,
  ...
  if (solution.status == clarabel::SolverStatus::Solved ||
    ...
    result->set_optimal_cost(get_obj_val(solution, P, q_vec, cost_constant));

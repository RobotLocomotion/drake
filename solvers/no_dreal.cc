...
static optional<DrealSolver::IntervalBox> DrealSolver::CheckSatisfiability(
...
static optional<IntervalBox> DrealSolver::Minimize(
    const symbolic::Expression&,
    const symbolic::Formula&,
    double, DrealSolver::LocalOptimization) {
  throw std::runtime_error(
      "The dReal bindings were not compiled.  You'll need to use a different "
      "solver.");
}

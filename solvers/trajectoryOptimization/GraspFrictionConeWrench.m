classdef GraspFrictionConeWrench < RigidBodyContactWrench
  ...
  methods
    function obj = GraspFrictionConeWrench(robot,grasp_object_idx, finger_idx, finger_grasp_pt,grasp_object_FC_axis,FC_mu,force_normalize_factor)
      ...
      axis_size = size(grasp_objectFC_axis);
      ...
      obj.wrench_iCfun = repmat([reshape(bsxfun(@times,(1:obj.num_pts)',ones(1,nq)),[],1);reshape(bsxfun(@times,ones(3,1),1:obj.num_pts),[],1)],2,1);
    ...
    function [c,dc] = evalWrenchConstraint(obj,kinsol,F,slack)
      [grasp_object_quat,dgrasp_object_quat] = forwardKin(obj.robot,kinsol,obj.grasp_object_idx,2);

...
/**
template <typename T>
Eigen::Map<const VectorX<T>, 0, Eigen::OuterStride<>> Flatten(
    const Eigen::Ref<const MatrixX<T>>& mat) {
  return Eigen::Map<const VectorX<T>, 0, Eigen::OuterStride<>>(
      mat.data(), mat.size(), Eigen::OuterStride<>(mat.outerStride()));
}
*/

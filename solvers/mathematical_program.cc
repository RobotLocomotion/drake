...
MathematicalProgram::AddL1NormCostUsingSlackVariables(
    const Eigen::Ref<const Eigen::MatrixXd>& A,
    const Eigen::Ref<const Eigen::VectorXd>& b,
    const Eigen::Ref<const VectorXDecisionVariable>& vars) {
  auto s = this->NewContinuousVariables(A.rows(), "slack");
  // We want to encode s >= Ax+b and s >= -(Ax+b), with a cost on the sum of the
  // entries of s. This can be written as: A_full = [-I  A]
  //          [-I -A]
  // lb = [-b]
  //      [ b]
  // A_full * vars >= lb

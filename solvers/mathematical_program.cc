...
namespace {
template <typename T>
Eigen::Map<const VectorX<T>> Flatten(const Eigen::Ref<const MatrixX<T>>& mat) {
  return Eigen::Map<const VectorX<T>>(mat.data(), mat.size());
}

template <typename T>
Eigen::Map<const VectorX<T>> Flatten(const MatrixX<T>& mat) {
  return Eigen::Map<const VectorX<T>>(mat.data(), mat.size());
}
}  // namespace
...
void MathematicalProgram::AddDecisionVariables(
  ...
  for (int i = 0; i < decision_variables.rows(); ++i) {
    for (int j = 0; j < decision_variables.cols(); ++j) {
      if (decision_variables(i, j).is_dummy()) {
        throw std::runtime_error(fmt::format(
            "decision_variables({}, {}) should not be a dummy variable", i, j));
      }
      if (decision_variable_index_.find(decision_variables(i, j).get_id()) !=
          decision_variable_index_.end()) {
        throw std::runtime_error(fmt::format(
            "{} is already a decision variable.", decision_variables(i, j)));
      }
      if (indeterminates_index_.find(decision_variables(i, j).get_id()) !=
          indeterminates_index_.end()) {
        throw std::runtime_error(fmt::format("{} is already an indeterminate.",
                                             decision_variables(i, j)));
      }
      CheckVariableType(decision_variables(i, j).get_type());
      decision_variable_index_.insert(std::make_pair(
          decision_variables(i, j).get_id(),
  ...
  decision_variables_.tail(decision_variables.size()) =
      Flatten(decision_variables);
...
void MathematicalProgram::AddIndeterminates(
  ...
  for (int i = 0; i < new_indeterminates.rows(); ++i) {
    for (int j = 0; j < new_indeterminates.cols(); ++j) {
      if (new_indeterminates(i, j).is_dummy()) {
        throw std::runtime_error(fmt::format(
            "new_indeterminates({},{}) should not be a dummy variable.", i, j));
      }
      if (indeterminates_index_.find(new_indeterminates(i, j).get_id()) !=
              indeterminates_index_.end() ||
          decision_variable_index_.find(new_indeterminates(i, j).get_id()) !=
              decision_variable_index_.end()) {
        throw std::runtime_error(
            fmt::format("{} already exists in the optimization program.",
                        new_indeterminates(i, j)));
      }
      if (new_indeterminates(i, j).get_type() !=
  ...
  indeterminates_.tail(new_indeterminates.size()) = Flatten(new_indeterminates);
}

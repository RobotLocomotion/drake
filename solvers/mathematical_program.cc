...
void MathematicalProgram::RemoveLinearCost(const Binding<LinearCost>& cost) {
  RemoveCostOrConstraintImpl(cost, ProgramAttribute::kLinearCost,
                             &(this->linear_costs_));
}

void MathematicalProgram::RemoveQuadraticCost(
    const Binding<QuadraticCost>& cost) {
  RemoveCostOrConstraintImpl(cost, ProgramAttribute::kQuadraticCost,
                             &(this->quadratic_costs_));
}
...
void MathematicalProgram::RemoveCost(const Binding<Cost>& cost) {
  ...
  if (dynamic_cast<QuadraticCost*>(cost_evaluator)) {
    return RemoveQuadraticCost(
        internal::BindingDynamicCast<QuadraticCost>(cost));
  } else if (dynamic_cast<LinearCost*>(cost_evaluator)) {
    return RemoveLinearCost(internal::BindingDynamicCast<LinearCost>(cost));
  } else {

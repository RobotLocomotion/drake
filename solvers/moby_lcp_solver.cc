...
bool MobyLCPSolver<T>::SolveLcpFast(const MatrixX<T>& M,
  ...
  DRAKE_LOGGER_TRACE("MobyLCPSolver::SolveLcpFast() entered");

  // look for trivial solution
  if (N == 0) {
    DRAKE_LOGGER_TRACE("MobyLCPSolver::SolveLcpFast() - empty problem");
    z->resize(0);
    return true;
  }
  ...
  if (z->size() == q.size()) {
    DRAKE_LOGGER_TRACE(
        "MobyLCPSolver::SolveLcpFast() - warm starting activated");
    ...
    DRAKE_LOGGER_TRACE(" -- non-basic indices: {}", fmt::join(nonbas_, " "));
  } else {
    ...
    if (minw_val > -mod_zero_tol) {
      DRAKE_LOGGER_TRACE(
          "MobyLCPSolver::SolveLcpFast() - trivial solution found");
      z->resize(N);
      z->fill(0);
      return true;
    }
  ...
  for (pivots_ = 1; pivots_ <= MAX_PIV; pivots_++) {
    ...
    if (minw == UINF || w[minw] > -mod_zero_tol) {
      ...
      if (minz < UINF && zz[minz] < -mod_zero_tol) {
        ...
        DRAKE_LOGGER_TRACE("MobyLCPSolver::SolveLcpFast() - solution found!");
        return true;
      }
    } else {
      DRAKE_LOGGER_TRACE("(minimum w too negative)");
...
bool MobyLCPSolver<T>::SolveLcpFastRegularized(const MatrixX<T>& M,
                                               ...
                                               const T& zero_tol) const {
  DRAKE_LOGGER_TRACE("MobyLCPSolver::SolveLcpFastRegularized() entered");
  ...
  DRAKE_LOGGER_TRACE(" zero tolerance: {}", mod_zero_tol);
    ...
    if (z->minCoeff() >= -mod_zero_tol) {
      ...
      if (wx.minCoeff() >= -mod_zero_tol) {
        ...
        if (wx_min >= -mod_zero_tol && wx_max < mod_zero_tol) {
          DRAKE_LOGGER_TRACE("  solved with no regularization necessary!");
          DRAKE_LOGGER_TRACE("  pivots / total pivots: {} {}", pivots_,
                             total_piv);
          DRAKE_LOGGER_TRACE("MobyLCPSolver::SolveLcpFastRegularized() exited");
          return true;
        } else {
          DRAKE_LOGGER_TRACE(
              "MobyLCPSolver::SolveLcpFastRegularized() - '<w, z>' not within "
              "tolerance(min value: {} max value: {})",
              wx_min, wx_max);
        }
      } else {
        DRAKE_LOGGER_TRACE(
            "  MobyLCPSolver::SolveLcpFastRegularized() - 'w' not solved to "
            "desired tolerance");
        DRAKE_LOGGER_TRACE("  minimum w: {}", wx.minCoeff());
      }
    } else {
      DRAKE_LOGGER_TRACE(
          "  MobyLCPSolver::SolveLcpFastRegularized() - 'z' not solved to "
          "desired tolerance");
      DRAKE_LOGGER_TRACE("  minimum z: {}", z->minCoeff());
    }
  } else {
    DRAKE_LOGGER_TRACE(
        "  MobyLCPSolver::SolveLcpFastRegularized() - solver failed with zero "
        "regularization");
  }
  ...
  while (rf < max_exp) {
    ...
    DRAKE_LOGGER_TRACE("  trying to solve LCP with regularization factor: {}",
                       lambda);
    ...
    if (result) {
      ...
      if (z->minCoeff() > -mod_zero_tol) {
        ...
        if (wx.minCoeff() > -mod_zero_tol) {
          ...
          if (wx_min > -mod_zero_tol && wx_max < mod_zero_tol) {
            DRAKE_LOGGER_TRACE("  solved with regularization factor: {}",
                               lambda);
            DRAKE_LOGGER_TRACE("  pivots / total pivots: {} {}", pivots_,
                               total_piv);
            DRAKE_LOGGER_TRACE(
                "MobyLCPSolver::SolveLcpFastRegularized() exited");
            pivots_ = total_piv;
            return true;
          } else {
            DRAKE_LOGGER_TRACE(
  ...
  DRAKE_LOGGER_TRACE("  unable to solve given any regularization!");
  DRAKE_LOGGER_TRACE("MobyLCPSolver::SolveLcpFastRegularized() exited");
...
void MobyLCPSolver<T>::FinishLemkeSolution(const MatrixType& M,
  ...
  VectorX<T> wl = (M * (*z)) + q;
  DRAKE_LOGGER_TRACE("  z: {}", *z);
  DRAKE_LOGGER_TRACE("  w: {}", wl);
  DRAKE_LOGGER_TRACE("  minimum w: {}", wl.minCoeff());
  DRAKE_LOGGER_TRACE("  w'z: {}", abs(wl.dot(*z)));
}
...
bool MobyLCPSolver<T>::SolveLcpLemke(const MatrixX<T>& M,
  ...
  DRAKE_LOGGER_TRACE("MobyLCPSolver::SolveLcpLemke() entered");
  DRAKE_LOGGER_TRACE("  M: {}", M);
  DRAKE_LOGGER_TRACE("  q: {}", q);
  ...
  if (CheckLemkeTrivial(n, mod_zero_tol, q, z)) {
    DRAKE_LOGGER_TRACE(" -- trivial solution found");
    DRAKE_LOGGER_TRACE("MobyLCPSolver::SolveLcpLemke() exited");
    return true;
  }
  ...
  if (!bas_.empty()) {
    DRAKE_LOGGER_TRACE("-- initial basis not empty (warmstarting)");
    ...
    DRAKE_LOGGER_TRACE("-- using basis of -1 (no warmstarting)");
  ...
  if (x.minCoeff() >= 0.0) {
    DRAKE_LOGGER_TRACE(" -- initial basis provides a solution!");
  ...
  leaving = *iiter;
  DRAKE_LOGGER_TRACE(" -- x: {}", x);
  DRAKE_LOGGER_TRACE(
      " -- first pivot: leaving index={} entering index={} minimum value: {}",
  ...
  *iiter = t;  // replace w var with _z0 in basic indices
  ...
  x[lvindex] = tval;
  Bl.col(lvindex) = Be;
  DRAKE_LOGGER_TRACE("  new q: {}", x);

  // main iterations begin here
  for (pivots_ = 0; pivots_ < max_iter; pivots_++) {
    DRAKE_LOGGER_TRACE("basic variables: {}", fmt::join(bas_, " "));
    DRAKE_LOGGER_TRACE("leaving: {} t: {}", leaving, t);

    // check whether done; if not, get new entering variable
    if (leaving == t) {
      DRAKE_LOGGER_TRACE("-- solved LCP successfully!");
      FinishLemkeSolution(M, q, x, z);
      DRAKE_LOGGER_TRACE("MobyLCPSolver::SolveLcpLemke() exited");
      return true;
    } else if (leaving < n) {
    ...
    if (j_.empty()) {
      ...
      DRAKE_LOGGER_TRACE("MobyLCPSolver::SolveLcpLemke() exiting");
      z->setZero(n);
      return false;
    }

    DRAKE_LOGGER_TRACE("d: {}", dl);
    DRAKE_LOGGER_TRACE("j (before min ratio): {}", fmt::join(j_, " "));
    ...
    DRAKE_LOGGER_TRACE("j (after min ratio): {}", fmt::join(j_, " "));

    // if j is empty, then likely the zero tolerance is too low
    if (j_.empty()) {
      DRAKE_LOGGER_TRACE("zero tolerance too low?");
      DRAKE_LOGGER_TRACE("MobyLCPSolver::SolveLcpLemke() exited");
      z->setZero(n);
      return false;
    }
    ...
    Bl.col(lvindex) = Be;
    *iiter = entering;
    DRAKE_LOGGER_TRACE(" -- pivoting: leaving index={} entering index={}",
                       lvindex, entering);
  }

  DRAKE_LOGGER_TRACE(" -- maximum number of iterations exceeded (n={}, max={})",
                     n, max_iter);
  DRAKE_LOGGER_TRACE("MobyLCPSolver::SolveLcpLemke() exited");
  z->setZero(n);
  return false;
...
bool MobyLCPSolver<T>::SolveLcpLemkeRegularized(const MatrixX<T>& M,
  ...
  DRAKE_LOGGER_TRACE("MobyLCPSolver::SolveLcpLemkeRegularized() entered");
    ...
    if (z->minCoeff() >= -mod_zero_tol) {
      ...
      if (wx.minCoeff() >= -mod_zero_tol) {
        ...
        if (wx_min >= -mod_zero_tol && wx_max < mod_zero_tol) {
          DRAKE_LOGGER_TRACE("  solved with no regularization necessary!");
          DRAKE_LOGGER_TRACE(
              "MobyLCPSolver::SolveLcpLemkeRegularized() exited");
          return true;
        } else {
          DRAKE_LOGGER_TRACE(
              "MobyLCPSolver::SolveLcpLemke() - '<w, z> not within "
              "tolerance(min value: {} max value: {})",
              wx_min, wx_max);
        }
      } else {
  ...
  DRAKE_LOGGER_TRACE("  unable to solve given any regularization!");
  DRAKE_LOGGER_TRACE("MobyLCPSolver::SolveLcpLemkeRegularized() exited");

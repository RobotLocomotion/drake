...
For examples of use, refer to all of the existing Drake solver wrappers. */
class SpecificOptions {
 public:
  ...
  provided for those names (in our constructor). It is OK for the callback to
  have side-effects; it will be invoked exactly once and is not retained.
  As such, this can also serve as a way to inject default values (by returning
  then from the respell callback here).
  ...
  specific -- `all_options` remain unchanged; instead, the `key` is memorized
  and future calls to either of the `CopyTo...` functions will skip over it.)
  If the option was not set, returns nullopt. When checking if an option was
  set, this checks `all_options` for our `id` as well as any options added
  during a Respell().
  ...
  `Output` struct must obey Drake's Serialize() protocol, for us to interrogate
  the option names.
  @param [in,out] result The solver's specific options struct; any options not
  mentioned in `solver_options` will remain unchanged. */
  template <typename Result>

...
struct KnownOptions {
  ...
  // NLopt's own defaults leave the objective-value criteria disabled, so these
  // three values reproduce the behavior of never calling their setters at all.
  double ftol_rel{0.0};
  double ftol_abs{0.0};
  ...
  // place. The four NaN tolerances below are sentinels meaning "inherit the
  ...
  // zero is a meaningful NLopt value (the criterion is disabled). There is
  ...
  double local_optimizer_ftol_rel{std::numeric_limits<double>::quiet_NaN()};
  double local_optimizer_ftol_abs{std::numeric_limits<double>::quiet_NaN()};
...
void NloptSolver::DoSolve2(const MathematicalProgram& prog,
  ...
  if (!parsed_options.local_optimizer_algorithm.empty()) {
    ...
    local_opt.set_ftol_rel(std::isnan(parsed_options.local_optimizer_ftol_rel)
                               ? parsed_options.ftol_rel
                               : parsed_options.local_optimizer_ftol_rel);
    local_opt.set_ftol_abs(std::isnan(parsed_options.local_optimizer_ftol_abs)
                               ? parsed_options.ftol_abs
                               : parsed_options.local_optimizer_ftol_abs);
  ...
  try {
    ...
    switch (nlopt_result) {
      ...
      case nlopt::XTOL_REACHED: {
        // All three can stop before the solver has converged, so re-check
        // the constraints. STOPVAL_REACHED is grouped here defensively: every
        // constraint-aware NLopt algorithm already gates its stopval test on
        // feasibility (e.g. "feasible && *minf < stop->minf_max" in
        // src/algs/slsqp/slsqp.c, and likewise in mma.c, ccsa_quadratic.c and
        // isres.c), so it is not known to arrive here with a violated
        // constraint -- but the check is shared with the cases below and so
        // costs nothing.

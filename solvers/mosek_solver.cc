...
MSKrescodee AddQuadraticCostAsLinearCost(
  ...
  // a linear cost. To do so, we introduce a slack variable s, with the rotated
  // lorentz cone constraint 2s >= | L * quadratic_vars|², where L'L = Q. We
  // then minimize s.
  // In Mosek, we add new variables z, such that
  // z(0) = 1
  // z(1) = s
...
MSKrescodee AddQuadraticCost(
    ...
    MSKtask_t* task) {
  MSKrescodee rescode = MSK_RES_OK;
  // Add the quadratic cost 0.5 * quadratic_vars' * Q * quadratic_vars to Mosek.
  // Q_lower_triplets correspond to the matrix that multiplies with all the
  // non-matrix decision variables in Mosek, not with `quadratic_vars`. We
  // need to map each variable in `quadratic_vars` to its index in Mosek
  ...
  for (int j = 0; j < Q_lower.outerSize(); ++j) {
    for (Eigen::SparseMatrix<double>::InnerIterator it(Q_lower, j); it; ++it) {

  ...
  for (const auto& psd_constraint : prog.positive_semidefinite_constraints()) {
    ...
    if (it == psd_barvar_indices.end()) {
      throw std::runtime_error(
          ...
          "semidefinite constraint has not"
          "registered in Mosek as a matrix variable. This should not happen, "
          "please report a bug on drake github.");
    }
    ...
    // Copy barsj to dual_lower. I don't use barsj directly since MSKrealt might
    // be a different data type from double.

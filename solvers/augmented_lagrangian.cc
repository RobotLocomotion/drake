...
T EvalAugmentedLagrangian(const MathematicalProgram& prog,
  ...
  *cost = T{0};
  constraint_residue->resize(lambda.rows());
  for (const auto& cost_binding : prog.GetAllCosts()) {
    *cost += prog.EvalBinding(cost_binding, x)(0);
  }
  T al = *cost;
  ...
  if (include_x_bounds) {
    ...
    for (int i = 0; i < prog.num_vars(); ++i) {
      if (x_lo(i) == x_up(i)) {
        ...
        (*constraint_residue)(lagrangian_count) = x(i) - x_lo(i);
        lagrangian_count++;
      } else {
        if (!std::isinf(x_lo(i))) {
        ...
        if (!std::isinf(x_up(i))) {
          ...
          (*constraint_residue)(lagrangian_count) = x_up(i) - x(i);
          lagrangian_count++;
        }
      }
    }
...
int GetLagrangianSizeForAl(const MathematicalProgram& prog,
...
std::vector<bool> IsEqualityForAl(const MathematicalProgram& prog,
                                  bool include_x_bounds) {
  const int constraint_size = GetLagrangianSizeForAl(prog, include_x_bounds);
  std::vector<bool> is_equality(constraint_size);

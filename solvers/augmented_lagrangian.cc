...
NonsmoothAugmentedLagrangian::NonsmoothAugmentedLagrangian(
  ...
  // I loop through all the constraints again instead of combining this loop
  // with the previous loop when we find lagrangian_size_. The reason is that I
...
T NonsmoothAugmentedLagrangian::Eval(const Eigen::Ref<const VectorX<T>>& x,
  ...
  DRAKE_DEMAND(lambda.rows() == lagrangian_size_);
  DRAKE_DEMAND(mu > 0);
  ...
  *cost = T{0};
  ...
  if (include_x_bounds_) {
    for (int i = 0; i < prog_->num_vars(); ++i) {
      if (x_lo_(i) == x_up_(i)) {
        al += -lambda(lagrangian_count) * (x(i) - x_lo_(i)) +
              1. / (2 * mu) * (x(i) - x_lo_(i)) * (x(i) - x_lo_(i));
        (*constraint_residue)(lagrangian_count) = x(i) - x_lo_(i);
        lagrangian_count++;
      } else {
        if (!std::isinf(x_lo_(i))) {
          al += psi(x(i) - x_lo_(i), lambda(lagrangian_count), mu);
          (*constraint_residue)(lagrangian_count) = x(i) - x_lo_(i);
          lagrangian_count++;
        }
        if (!std::isinf(x_up_(i))) {
          al += psi(x_up_(i) - x(i), lambda(lagrangian_count), mu);

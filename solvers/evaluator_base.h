...
class FunctionEvaluator : public EvaluatorBase {
 public:
  ...
  template <typename T>
  using Calc =
      std::function<void(const Eigen::Ref<const VectorX<T>>&, VectorX<T>*)>;

  FunctionEvaluator(int num_outputs, int num_vars, Calc<double> calc_double,
                    Calc<AutoDiffXd> calc_autodiffxd = {},
                    Calc<symbolic::Expression> calc_expression = {},
                    const std::string& description = "")
 ...
 private:
  ...
  Calc<symbolic::Expression> calc_expression_;
};

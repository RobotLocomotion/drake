 ...
 */
class Callback : public EvaluatorBase {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(Callback)

  typedef std::function<void(const Eigen::Ref<const
  Eigen::VectorXd>&)> CallbackFnc;

  Callback(int num_inputs, const CallbackFnc& callback,
           const std::string& description = "")

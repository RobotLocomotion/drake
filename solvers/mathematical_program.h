...
class MathematicalProgram {
 public:
   ...
   */
  Binding<Callback> AddCallback(
      const Callback::CallbackFnc& callback,
   ...
   * @param vars the decision variables that should be passed to the callback.
   */
  Binding<Callback> AddCallback(const Callback::CallbackFnc& callback,
                                const VariableRefList& vars) {
    return AddCallback(callback, ConcatenateVariableRefList((vars)));
  }
   ...
   */
  const std::vector<Binding<Callback>>& callbacks() const {
    return callbacks_;
  ...
  /** Evaluates all callbacks registered with the MathematicalProgram.
   *
   * @param prog_var_vals The value of all the decision variables in this
   * program. @throw a logic error if the size does not match.
   **/
  void EvalCallbacks(const Eigen::Ref<const Eigen::VectorXd>& prog_var_vals)
  const {
    if (prog_var_vals.rows() != num_vars()) {
      std::ostringstream oss;
      oss << "The input binding variable is not in the right size. Expects "
    ...
    for (auto const& binding : callbacks_) {
 ...
 private:
  ...
  std::vector<Binding<Callback>> callbacks_;

...
class MathematicalProgram {
 public:
   ...
   * @returns Vector in the following order: generic, linear, quadratic.
   */
   ...
   * @returns Vector in the following order: generic, linear inequality,
   * linear equality, bbox, lorentz cone, rotated lorentz code,
   * linear matrix inequality, linear complementarity.
   */
  ...
  EvalBindings(
      ...
      const Eigen::MatrixBase<DerivedX>& prog_dec_vars) const {
    ...
    int offset_y{};
    for (auto& binding : bindings) {
      VectorX<Scalar> binding_y = EvalBinding(binding, prog_dec_vars);

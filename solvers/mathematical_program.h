...
class MathematicalProgram {
 public:
  ...
  /** Returns a pair of a SOS polynomial p = mᵀQm of degree @p degree and a PSD
   * constraint for the coefficients matrix Q, where m is the @p monomial basis.
   ...
   * and a PSD constraint for the coefficients matrix Q, where m(x) is a
   * monomial basis over @p indeterminates, x, of degree `@p degree / 2`. For
   * example, `NewSosPolynomial({x}, 4)` returns a pair of a polynomial
   ...
   * that is, @p p can be decomposed into `mᵀQxm`. It returns a pair of
   ...
   * basis of all indeterminates in the program with degree equal to the
   ...
   * sums-of-squares (SOS), that is, @p p can be decomposed into `mᵀQxm`.
   * Note that it decomposes @p e into a polynomial with respect to

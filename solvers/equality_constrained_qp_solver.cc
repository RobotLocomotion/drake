...
SolutionResult SolveUnconstrainedQP(const Eigen::Ref<const Eigen::MatrixXd>& G,
  ...
  if (llt.info() == Eigen::Success) {
    ...
    *x = llt.solve(-c);
    ...
    bool is_G_psd = false;
    // We first use LDLT (which is fast) decomposition. But I found that LDLT sometimes fails, so we then fall back to Eigen value decomposition.
    ...
    if (ldlt.info() == Eigen::Success && ldlt.isPositive()) {
      ...
      if (es.info() == Eigen::Success && (es.eigenvalues().array() >= -feasibility_tol).all()) {
        is_G_psd = true;
        use_ldlt = false;
      }
    }

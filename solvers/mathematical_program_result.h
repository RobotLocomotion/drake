 ...
 public:
  ...
  Eigen::VectorXd GetDualSolution(const Binding<C>& constraint) const {
    ...
    if (it == dual_solutions_.end()) {
      ...
      // dual solution from Gurobi result when the program contains second order
      // cone constraint, but forgot to explicitly turn on the flag to compute
      // gurobi qcp dual.
      if (solver_id_.name() == "Gurobi") {
        throw std::invalid_argument(fmt::format(
            "You used {} to solve this optimization problem. If the problem is "
            "solved to optimaility and doesn't contain binary/integer "
            "variables, but you failed to get the dual solution, "
            "it is because you have to explicitly tell Gurobi solver to "

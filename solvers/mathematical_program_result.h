...
class MathematicalProgramResult final {
 public:
   ...
   * dual solution. Mathematically dual_solution = ∂optimal_cost / ∂bound.
   *
   * For a linear equality constraint Ax=b where b ∈ ℝⁿ, the dual variables has
   * n rows, dual_solution(i) is the value of the dual variable for the
   * constraint A(i,:)*x = b(i).
   *
   ...
   * :)*x at the solution), then dual_solution(i) is positive (because the
   * objective is to minimize a cost, so the shadow price is positive if the
   * lower bound is increased). If the right hand-side A(i, :)*x<=upper(i) is
   * active (meaning A(i,:)*x=upper(i) at the solution), then dual_solution(i)
   * is negative.
   *
   * For a bounding box constraint lower <= x <= upper, the interpretation of
   * the dual solution is the same as the linear equality constraint.
   *

...
void QuadraticCost::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x,
  ...
  const MatrixXd dx = math::ExtractGradientMatrixFromAutoDiff(x);
  const Eigen::VectorXd x_val = drake::math::ExtractValueMatrixFromAutoDiff(x);
  ...
  if (dx.rows() == x.size() && dx.cols() == x.size() &&
    ...
    *y = math::InitializeAutoDiffFromValueAndGradientMatrix(result, dy);
  } else {
    *y = math::InitializeAutoDiffFromValueAndGradientMatrix(result, dy * dx);
  }
}

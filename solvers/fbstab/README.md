...
It assumes that H is positive semidefinite and can detect primal and dual infeasibility. FBstab requires no regularity assumptions on G or A. A mathematical description of FBstab can be found in the following [research article](https://arxiv.org/pdf/1901.04046.pdf).
...
The algorithm itself is implemented in a abstract way using template metaprogramming. It requires 5 classes which each must implement several methods.
...
These classes should be specialized for different kinds of QPs, e.g., support vector machines, optimal control problems, general dense QPs, in order to exploit structure. 

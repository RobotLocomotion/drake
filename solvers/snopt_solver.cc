...
class SnoptUserFunInfo {
 public:
  ...
  std::vector<int>& G_remove_duplication() { return G_remove_duplication_; }

  const std::vector<int>& G_remove_duplication() const {
    return G_remove_duplication_;
  }
 ...
 private:
  ...
  // G_remove_duplication_ has the same length as G_w_duplicate. We add
  // G_w_duplicate[i] to G[G_remove_duplication[i]].
  std::vector<int> G_remove_duplication_;
  int lenG_;
...
void EvaluateCostsConstraints(
  ...
  std::vector<double> G_w_duplicate(info.G_remove_duplication().size(), 0);
  ...
  for (int i = 0; i < static_cast<int>(info.G_remove_duplication().size());
       ++i) {
    G[info.G_remove_duplication()[i]] += G_w_duplicate[i];
  }
}
...
void PruneGradientDuplication(int nx, const std::vector<int>& iGfun_w_duplicate,
  ...
  G_remove_duplication->reserve(iGfun_w_duplicate.size());
  ...
  for (int i = 0; i < static_cast<int>(iGfun_w_duplicate.size()); ++i) {
    ...
    if (it == gradient_index_to_G.end()) {
      G_remove_duplication->push_back(iGfun->size());
      ...
      G_remove_duplication->push_back(it->second);
    }
  }
}
...
void SolveWithGivenOptions(
  ...
  std::vector<int> iGfun;
  std::vector<int> jGvar;
  auto& G_remove_duplication = user_info.G_remove_duplication();
  PruneGradientDuplication(nx, iGfun_w_duplicate, jGvar_w_duplicate,
                           &G_remove_duplication, &iGfun, &jGvar);

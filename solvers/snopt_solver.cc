...
class WorkspaceStorage {
 public:
  ...
  // The snopt manual states that a trying to use a workspace smaller than this
...
void SolveWithGivenOptions(const MathematicalProgram& prog,
  ...
  // If the user sets the "Total ... workspace" options to be larger than the
  // storage we've allocated, then we must allocate more storage to accomodate.
  //
  // We `Pop` the options here (removing them from the CopyToCallbacks). After
  // `snmema` has a chance to request even more storage, we'll `snseti` by hand.
  ...
  // Ask `snmema` whther the integer and/or real workspace needs to be larger,
  // and increase them size if so. Once we know the final size, tell snopt that

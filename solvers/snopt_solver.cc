...
void EvaluateSingleNonlinearConstraint(const C& constraint,
  ...
  constraint.Eval(drake::math::initializeAutoDiff(this_x), ty);
}
...
void EvaluateSingleNonlinearConstraint<LinearComplementarityConstraint>(
  ...
  ty->resize(1);
  auto tx = drake::math::initializeAutoDiff(this_x);
...
void EvaluateAndAddNonlinearCosts(
  ...
  for (const auto& binding : nonlinear_costs) {
    ...
    AutoDiffVecXd ty(1);
    obj->Eval(drake::math::initializeAutoDiff(this_x), &ty);
...
void UnpackSnoptUserFunInfoFromWorkspace(
    int iu[], const SnoptUserFunInfo** snopt_userfun_info) {
...
void snopt_userfun(int* Status, int* n, double x[], int* needF, int* neF,
  ...
  const SnoptUserFunInfo* snopt_userfun_info = NULL;

  UnpackSnoptUserFunInfoFromWorkspace(iu, &snopt_userfun_info);
...
void SolveWithoutWriteback(
    const MathematicalProgram& prog,
    const Eigen::Ref<const Eigen::VectorXd>& x_init,
    const std::map<std::string, std::string>& additional_options_string,
    const std::map<std::string, int>& additional_options_int,
    const std::map<std::string, double>& additional_options_double,
    int* snopt_info, double* objective, EigenPtr<Eigen::VectorXd> x_val) {
  std::map<std::string, std::string> snopt_options_string =
      prog.GetSolverOptionsStr(SnoptSolver::id());
  std::map<std::string, int> snopt_options_int =
      prog.GetSolverOptionsInt(SnoptSolver::id());
  std::map<std::string, double> snopt_options_double =
      prog.GetSolverOptionsDouble(SnoptSolver::id());
  for (const auto& it : additional_options_string) {
    snopt_options_string[it.first] = it.second;
  }
  for (const auto& it : additional_options_double) {
    snopt_options_double[it.first] = it.second;
  }
  for (const auto& it : additional_options_int) {
    snopt_options_int[it.first] = it.second;
  }
  SolveWithGivenOptions(prog, x_init, snopt_options_string, snopt_options_int,
                        snopt_options_double, snopt_info, objective, x_val);
}

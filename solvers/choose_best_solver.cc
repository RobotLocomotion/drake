...
SolverId ChooseBestLPSolver(const MathematicalProgram& prog) {
  DRAKE_DEMAND(GetProgramType(prog) == ProgramType::kLP);
...
SolverId ChooseBestSolver(const MathematicalProgram& prog) {
  ...
  if (program_type == ProgramType::kLP) {
    return ChooseBestLPSolver(prog);
  }
  // Fall back
  return ChooseFirstMatchingSolver<
      LinearSystemSolver, EqualityConstrainedQPSolver, MosekSolver,
      GurobiSolver, OsqpSolver, ClpSolver, MobyLCPSolver<double>, SnoptSolver,
      IpoptSolver, NloptSolver, CsdpSolver, ScsSolver>(prog);
}

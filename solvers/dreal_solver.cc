...
optional<DrealSolver::IntervalBox> DrealSolver::Minimize(
    ...
    const double delta, const bool use_local_optimization) {
  DrealConverter dreal_converter;
  dreal::Box model;
  dreal::Config config;
  config.mutable_precision() = delta;
  config.mutable_use_local_optimization() = use_local_optimization;
...
// Extracts "precision" value from @p prog. If not specified, returns @p
// default_value.
double GetPrecision(MathematicalProgram* const prog,
                    const double default_value) {
  double precision{default_value};
  const map<string, double>& double_options{
      prog->GetSolverOptionsDouble(DrealSolver::id())};
  auto it = double_options.find("precision");
...
// Extracts "use_local_optimization" value from @p prog. If not specified,
// returns @p default_value.
bool GetUseLocalOptimization(MathematicalProgram* const prog,
                             const bool default_value) {
  bool use_local_optimization{default_value};
  const map<string, int>& int_options{
      prog->GetSolverOptionsInt(DrealSolver::id())};
  auto it = int_options.find("use_local_optimization");
  if (it != int_options.end()) {
    use_local_optimization = (it->second != 0);
  }
  return use_local_optimization;
}
...
symbolic::Formula ExtractRotatedLorentzConeConstraints(
  ...
  for (const Binding<RotatedLorentzConeConstraint>& binding :
    ...
    f = f && (z(0) >= 0) && (z(1) >= 0);
    // z₀ * z₁ ≥ ∑ zᵢ² (for i ∈ [2, n-1])
    symbolic::Expression squaredNorm;
...
SolutionResult DrealSolver::Solve(MathematicalProgram& prog) const {
  ...
  const double precision{GetPrecision(&prog, 0.001 /* default */)};
  const bool use_local_optimization{
      GetUseLocalOptimization(&prog, true /* default */)};
  optional<IntervalBox> result;
  if (costs.size() == 0) {
    // No cost functions in the problem. Call Checksatisfiability.
    result = CheckSatisfiability(constraints, precision /* Read it from MP */);
  } else if (costs.size() == 1) {
    // Single cost function in the problem. Call Minimize.
    result = Minimize(costs[0], constraints, precision, use_local_optimization);
  } else {
    throw logic_error("DrealSolver can have up to a single cost function.");
  }

...
GTEST_TEST(AugmentedLagrangian, TestObjective) {
  ...
  EXPECT_EQ(GetLagrangianSizeForAl(prog, true), 0);
  EXPECT_EQ(GetLagrangianSizeForAl(prog, false), 0);
  std::vector<bool> is_equality = IsEqualityForAl(prog, true);
  EXPECT_TRUE(is_equality.empty());
  is_equality = IsEqualityForAl(prog, false);
  EXPECT_TRUE(is_equality.empty());
  ...
  EXPECT_EQ(constraint_residue.rows(), 0);
  is_equality = IsEqualityForAl(prog, true);
  EXPECT_TRUE(is_equality.empty());
  is_equality = IsEqualityForAl(prog, false);
  EXPECT_TRUE(is_equality.empty());
  ...
  const auto al = EvalAugmentedLagrangian<AutoDiffXd>(
      prog, x_ad, Eigen::VectorXd(0), 0.5, false, &constraint_residue_ad,
      &cost_ad);
...
void CheckAugmentedLagrangianEqualityConstraint(
  ...
  T cost;
  const T al = EvalAugmentedLagrangian<T>(prog, x_val, lambda_val, mu_val,
                                          false, &constraint_residue, &cost);
  ...
  CompareAlResult<T>(al, al_expected, constraint_residue, cnstr_val - cnstr_bnd,
                     cost, T{0}, tol_val);
}
...
GTEST_TEST(AugmentedLagrangian, EqualityConstraints) {
  ...
  EXPECT_EQ(GetLagrangianSizeForAl(prog, true), 4);
  EXPECT_EQ(GetLagrangianSizeForAl(prog, false), 4);
  std::vector<bool> is_equality = IsEqualityForAl(prog, true);
  EXPECT_EQ(is_equality, std::vector<bool>({true, true, true, true}));
...
void CheckAugmentedLagrangianInequalityConstraint(
  ...
  constraint_residue_expected << cnstr_val(0) - lb(0), cnstr_val(1) - lb(1),
      ub(1) - cnstr_val(1), ub(2) - cnstr_val(2), cnstr_val(3) - lb(3);
...
GTEST_TEST(AugmentedLagrangian, InequalityConstraint) {
  ...
  EXPECT_EQ(GetLagrangianSizeForAl(prog, true), 5);
  EXPECT_EQ(GetLagrangianSizeForAl(prog, false), 5);
  std::vector<bool> is_equality = IsEqualityForAl(prog, true);
  EXPECT_EQ(is_equality, std::vector<bool>({true, false, false, false, false}));

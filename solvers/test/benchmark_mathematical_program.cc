...
static void BenchMarkSosProgram1(benchmark::State& state) {
  for (auto _ : state) {
    ...
    for (int i = 0; i < 10; ++i) {
      VectorX<symbolic::Polynomial> p_i(30);
      ...
      for (int j = 0; j < p_i.rows(); ++j) {
        ...
        for (int k = 0; k < monomial_basis.rows(); ++k) {
          p_map.emplace(monomial_basis(k),
                        static_cast<double>(i * j) / 100. + k / 10. + 1);
        }
        p_i(j) = symbolic::Polynomial(p_map);

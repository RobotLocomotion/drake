...
GTEST_TEST(MosekSolver, TestInitialGuess) {
  ...
  // By setting y = (0, 1, 0, 0), pt_C = (0, 1), lambda = (0, 1, 0, 0, 0) point C is on the line segment A2A3. This is a valid integral solution with distance to origin = 1.
  prog.SetInitialGuess(pt_C, Eigen::Vector2d(0, 1));
  prog.SetInitialGuess(lambda, (Eigen::Matrix<double, 5, 1>() << 0, 1, 0, 0, 0).finished());
  ...
  // this case it searchers the the point on line segment A2A3 that is closest

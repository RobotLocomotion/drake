...
NonConvexQPproblem2::NonConvexQPproblem2(CostForm cost_form,
  ...
  switch (cost_form) {
    case CostForm::kGeneric: {
      auto cost = [](const auto& x, auto* y) {
        (*y)(0) = (-50.0 * x(0) * x(0)) + (-10.5 * x(0)) -
                  (50.0 * x(1) * x(1)) + (-7.5 * x(1)) - (50.0 * x(2) * x(2)) +
                  (-3.5 * x(2)) - (50.0 * x(3) * x(3)) + (-2.5 * x(3)) -
...
void GloptiPolyConstrainedMinimizationProblem::AddGenericCost() {
  auto example_cost = [](const auto& x, auto* y) {
    (*y)(0) = -2 * x(0) + x(1) - x(2);
  };

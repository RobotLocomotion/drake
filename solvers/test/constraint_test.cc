...
void TestLorentzConeEvalConvex(const Eigen::Ref<const Eigen::MatrixXd>& A,
  ...
  EXPECT_TRUE(CompareMatrices(
      y_expected, math::ExtractValue(y_autodiff1), 1e-12));
  EXPECT_TRUE(CompareMatrices(
      y_expected, math::ExtractValue(y_autodiff2), 1e-12));
  // With eval_type = kConvexSmooth, we approximate the gradient with some
  // smooth function, which introduces larger error (2e-12).
  EXPECT_TRUE(CompareMatrices(math::ExtractGradient(
      y_autodiff1),
...
void TestLorentzConeEvalNonconvex(const Eigen::Ref<const Eigen::MatrixXd>& A,
  ...
  EXPECT_TRUE(
      CompareMatrices(y, math::ExtractValue(y_taylor)));
...
GTEST_TEST(testConstraint, testExpressionConstraint) {
  ...
  EXPECT_TRUE(CompareMatrices(math::ExtractValue(y_autodiff),
                              y_expected));
  EXPECT_TRUE(
      CompareMatrices(math::ExtractGradient(y_autodiff),
                      y_gradient_expected));

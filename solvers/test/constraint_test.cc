...
void TestLorentzConeEvalConvex(const Eigen::Ref<const Eigen::MatrixXd>& A,
  ...
  const AutoDiffVecXd x_autodiff =
      math::InitializeAutoDiffFromValueAndGradientMatrix(x_test, dx_test);

  AutoDiffVecXd y_autodiff1, y_autodiff2;
  cnstr1.Eval(x_autodiff, &y_autodiff1);
  cnstr2.Eval(x_autodiff, &y_autodiff2);
  EXPECT_TRUE(CompareMatrices(
      y_expected, math::ExtractValueMatrixFromAutoDiff(y_autodiff1), 1e-12));
  EXPECT_TRUE(CompareMatrices(
      y_expected, math::ExtractValueMatrixFromAutoDiff(y_autodiff2), 1e-12));
  // With eval_type = kConvexSmooth, we approximate the gradient with some
  // smooth function, which introduces larger error (2e-12).
  EXPECT_TRUE(CompareMatrices(math::ExtractGradientMatrixFromAutoDiff(
      y_autodiff1),
                              math::ExtractGradientMatrixFromAutoDiff(
                                  y_autodiff2),
                              2e-12));
}
...
void TestLorentzConeEvalNonconvex(const Eigen::Ref<const Eigen::MatrixXd>& A,
  ...
  auto tx = drake::math::InitializeAutoDiffFromValueMatrix(x_test);
  ...
  EXPECT_TRUE(
      CompareMatrices(y, math::ExtractValueMatrixFromAutoDiff(y_taylor)));
...
GTEST_TEST(testConstraint, testLorentzConeConstraintAtZeroZ) {
  ...
  cnstr.Eval(math::InitializeAutoDiffFromValueMatrix(x), &y_autodiff);
  EXPECT_TRUE(CompareMatrices(math::ExtractValueMatrixFromAutoDiff(y_autodiff),
                              Vector1d(0)));
  const Eigen::MatrixXd y_gradient =
      math::ExtractGradientMatrixFromAutoDiff(y_autodiff);
...
GTEST_TEST(testConstraint, testExpressionConstraint) {
  ...
  AutoDiffVecXd x_autodiff = drake::math::InitializeAutoDiffFromValueMatrix(x);
  AutoDiffVecXd y_autodiff;
  ...
  EXPECT_TRUE(CompareMatrices(math::ExtractValueMatrixFromAutoDiff(y_autodiff),
                              y_expected));
  EXPECT_TRUE(
      CompareMatrices(math::ExtractGradientMatrixFromAutoDiff(y_autodiff),
                      y_gradient_expected));
...
GTEST_TEST(testConstraint, testExponentialConeConstraint) {
  ...
  dx << 1, 1;
  const auto x_autodiff = math::InitializeAutoDiffFromValueAndGradientMatrix(
      Eigen::VectorXd(x), dx);
  AutoDiffVecXd y_autodiff;
  constraint.Eval(x_autodiff, &y_autodiff);
  ...
  EXPECT_TRUE(CompareMatrices(
      math::ExtractGradientMatrixFromAutoDiff(y_autodiff),
      math::ExtractGradientMatrixFromAutoDiff(y_autodiff_expected), tol));
}

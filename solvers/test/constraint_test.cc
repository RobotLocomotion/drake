...
GTEST_TEST(ToLatex, LinearComplementarityConstraint) {
  ...
  EXPECT_EQ(
      c.ToLatex(vars),
      "0 \\le \\begin{bmatrix} (-1 + x_{0}) \\\\ (-1 + x_{1}) \\end{bmatrix} "
      "\\\\ 0 \\le \\begin{bmatrix} x_{0} \\\\ x_{1} \\end{bmatrix} \\\\ "
      "\\begin{bmatrix} x_{0} \\\\ x_{1} \\end{bmatrix}^T\\begin{bmatrix} (-1 "
      "+ x_{0}) \\\\ (-1 + x_{1}) \\end{bmatrix} = 0 \\tag{test}");
}

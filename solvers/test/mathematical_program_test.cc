...
GTEST_TEST(TestAddDecisionVariables, TestMatrixInput) {
  ...
  for (int i = 0; i < 2; ++i) {
    for (int j = 0; j < 3; ++j) {
      EXPECT_EQ(prog.FindDecisionVariableIndex(vars(i, j)),
                num_existing_decision_vars + j * 2 + i);
    }
  }
}
...
GTEST_TEST(TestAddIndeterminates, MatrixInput) {
  ...
  for (int i = 0; i < 2; ++i) {
    for (int j = 0; j < 3; ++j) {
      EXPECT_EQ(prog.FindIndeterminateIndex(vars(i, j)),
                num_existing_indeterminates + j * 2 + i);
    }
  }
}
...
GTEST_TEST(TestMathematicalProgram, BoundingBoxTest3) {
  ...
  // Now add constraint on X.row(0). It doesn't occupy a contiguous memory.
  auto cnstr2 =
      prog.AddBoundingBoxConstraint(X_lo.row(0), X_up.row(0), X.row(0));
  EXPECT_EQ(cnstr2.variables().rows(), 3);
  X_bounds.clear();
  for (int j = 0; j < 3; j++) {
    X_bounds.emplace(X(0, j), std::make_pair(X_lo(0, j), X_up(0, j)));
  }
  for (int j = 0; j < 3; ++j) {
    const auto it = X_bounds.find(cnstr2.variables()(j));
    EXPECT_NE(it, X_bounds.end());
    EXPECT_EQ(it->second.first, cnstr2.evaluator()->lower_bound()(j));
    EXPECT_EQ(it->second.second, cnstr2.evaluator()->upper_bound()(j));
  }
}

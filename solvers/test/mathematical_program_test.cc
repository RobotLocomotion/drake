...
GTEST_TEST(testMathematicalProgram, testSetAndGetInitialGuess) {
  ...
  prog.UpdateInitialGuess(x(2), 2, &guess);
  EXPECT_TRUE(std::isnan(guess[0]));
  EXPECT_EQ(guess[2], 2.0);
  prog.UpdateInitialGuess(x.head<2>(), Eigen::Vector2d(0.0, 1.0), &guess);
  EXPECT_EQ(guess[0], 0.0);
  EXPECT_EQ(guess[1], 1.0);
  EXPECT_EQ(guess[2], 2.0);
  EXPECT_THROW(prog.UpdateInitialGuess(y, 0.0, &guess), std::runtime_error);
}

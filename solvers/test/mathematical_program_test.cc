...
symbolic::Monomial transform(const symbolic::Monomial& monomial,
  ...
  for (const pair<const Polynomial<double>::VarType, Variable>& p :
       polynomial_var_to_symbolic_var) {
    const Variable& symbolic_var{p.second};
  ...
  for (const pair<Variable, int> p : monomial.get_powers()) {
    ...
    const Polynomial<double>::VarType var_in_monomial_as_polynomial_var =
        Polynomial<double>::VariableIdToVarType(var_in_monomial.get_id());
    const int exponent{p.second};
    const auto it =
        polynomial_var_to_symbolic_var.find(var_in_monomial_as_polynomial_var);

    // There should be a mapping for the ID in polynomial_var_to_symbolic_var.
    DRAKE_DEMAND(it != polynomial_var_to_symbolic_var.end());

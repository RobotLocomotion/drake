...
void TestLPPoorScaling1(const SolverInterface& solver, double tol,
...
void TestLPPoorScaling2(const SolverInterface& solver, double tol,
                        const std::optional<SolverOptions>& options) {
  ...
  if (solver.available()) {
    ...
    EXPECT_TRUE(CompareMatrices(result.GetSolution(x),
                                Eigen::Vector3d(0.2282, 0, 0.3323), tol));
  }
}
}  // namespace test
}  // namespace solvers

...
TEST_F(TestAggregateCostsAndConstraints, AggregateBoundingBoxConstraints1) {
  ...
  EXPECT_EQ(result.at(x_[0]).first, 1);
  EXPECT_EQ(result.at(x_[0]).second, 2);
  EXPECT_EQ(result.at(x_[2]).first, 3);
  EXPECT_EQ(result.at(x_[2]).second, 5);
  ...
  EXPECT_EQ(result.at(x_[0]).first, 1.5);
  EXPECT_EQ(result.at(x_[0]).second, 2);
  EXPECT_EQ(result.at(x_[1]).first, 4);
  EXPECT_EQ(result.at(x_[1]).second, kInf);
  EXPECT_EQ(result.at(x_[2]).first, 3);
  EXPECT_EQ(result.at(x_[2]).second, 4);
  ...
  EXPECT_EQ(result.at(x_[0]).second, 2);
  EXPECT_EQ(result.at(x_[1]).first, 4);
  EXPECT_EQ(result.at(x_[1]).second, 2);
  EXPECT_EQ(result.at(x_[2]).first, 3);
  EXPECT_EQ(result.at(x_[2]).second, 4);
}
...
TEST_F(TestAggregateCostsAndConstraints, AggregateBoundingBoxConstraints2) {
  ...
  const std::pair<Eigen::VectorXd, Eigen::VectorXd> bounds =
      AggregateBoundingBoxConstraints(prog);
  EXPECT_TRUE(CompareMatrices(bounds.first, Eigen::Vector4d(-1, -kInf, 1, 1)));
  EXPECT_TRUE(CompareMatrices(bounds.second, Eigen::Vector4d(2, kInf, 3, 2)));
}
}  // namespace solvers
}  // namespace drake

...
class StubSolverBase final : public SolverBase {
 public:
  ...
  // in contras to the above constructor which leaves it defaulted.
...
GTEST_TEST(SolverBaseTest, ProgramAttributesDefault) {
  ...
  std::string error1 = dut.ExplainUnsatisfiedProgramAttributes(prog);
  EXPECT_THAT(error1, HasSubstr("StubSolverBase is unable to solve"));

  dut.satisfied_ = true;
  EXPECT_TRUE(dut.AreProgramAttributesSatisfied(prog));
  std::string error2 = dut.ExplainUnsatisfiedProgramAttributes(prog);
  EXPECT_EQ(error2, "");
}

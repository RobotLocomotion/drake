...
AssertionResult CompareAutodiff(const AutoDiffVecXd& tx_expected,
  ...
  const VectorXd x_expected = math::ExtractValueMatrixFromAutoDiff(tx_expected);
  const VectorXd x_actual = math::ExtractValueMatrixFromAutoDiff(tx_actual);
  AssertionResult value_result =
      CompareMatrices(x_expected, x_actual, tolerance);
  if (!value_result) {
    return value_result << "(value)";
  }
  const MatrixXd dx_expected =
      math::ExtractGradientMatrixFromAutoDiff(tx_expected);
  const MatrixXd dx_actual = math::ExtractGradientMatrixFromAutoDiff(tx_actual);
...
void VerifyFunctionEvaluator(F&& f, const VectorXd& x) {
  ...
  const AutoDiffVecXd tx = math::InitializeAutoDiffFromValueMatrix(x);
  AutoDiffVecXd ty_expected(3);
  deref(f).template eval<AutoDiffXd>(tx, &ty_expected);
  Eigen::MatrixXd dy_expected =
      math::ExtractGradientMatrixFromAutoDiff(ty_expected);

...
GTEST_TEST(SpecificOptionsTest, CopyToCallbacksNoop) {
  // This is a code-coverage test to cover the early return no-op.
  const SolverId& solver_id = GetSolverId();
  const SolverOptions solver_options;
  SpecificOptions dut(&solver_id, &solver_options);
  ...
  dut.CopyToCallbacks(fail, fail, fail);
}

GTEST_TEST(SpecificOptionsTest, CopyToCallbacksEarlyReturn) {
  // This is a code-coverage test for the early return no-op.
  const SolverId& solver_id = GetSolverId();
  const SolverOptions solver_options;
  SpecificOptions dut(&solver_id, &solver_options);
  auto fail = [](const auto&...) {

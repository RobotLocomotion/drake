...
GTEST_TEST(testCost, testLinearCost) {
  ...
  const AutoDiffVecXd x_autodiff =
      math::InitializeAutoDiffFromValueAndGradientMatrix(x0, x_grad);
  ...
  const AutoDiffVecXd x_autodiff_identity =
      math::InitializeAutoDiffFromValueMatrix(x0);
  AutoDiffVecXd y_autodiff_identity;
  cost->Eval(x_autodiff_identity, &y_autodiff_identity);
  EXPECT_TRUE(
      CompareMatrices(math::ExtractValueMatrixFromAutoDiff(y_autodiff_identity),
                      Vector1<double>(obj_expected), tol));
  EXPECT_TRUE(CompareMatrices(
      math::ExtractGradientMatrixFromAutoDiff(y_autodiff_identity),
      a.transpose(), tol));
  ...
  EXPECT_TRUE(
      CompareMatrices(math::ExtractValueMatrixFromAutoDiff(y_autodiff_empty),
                      Vector1<double>(obj_expected), tol));
  EXPECT_EQ(math::ExtractGradientMatrixFromAutoDiff(y_autodiff_empty).size(),
            0);
...
GTEST_TEST(TestL2NormCost, Eval) {
  ...
  // Test AutoDiffXd.
  {
    const Vector4<AutoDiffXd> x = math::InitializeAutoDiffFromValueMatrix(x0);
    VectorX<AutoDiffXd> y;
    cost.Eval(x, &y);
    EXPECT_NEAR(z.norm(), math::ExtractValueMatrixFromAutoDiff(y)[0], 1e-16);
    const Matrix<double, 1, 4> grad_expected =
        (x0.transpose() * A.transpose() * A + b.transpose() * A) / (z.norm());
    EXPECT_TRUE(CompareMatrices(math::ExtractGradientMatrixFromAutoDiff(y),
                                grad_expected, 1e-15));
  }

...
GTEST_TEST(TestL2NormCost, Eval) {
  ...
  L2NormCost cost(A, b);
  EXPECT_TRUE(CompareMatrices(A, cost.A_dense()));
  EXPECT_TRUE(CompareMatrices(A, cost.A_sparse().toDense()));
  EXPECT_TRUE(CompareMatrices(b, cost.b()));
...

  {
    ...
    EXPECT_TRUE(CompareMatrices(cost_sparse.A_dense(), A));
    EXPECT_TRUE(CompareMatrices(cost_sparse.A_sparse().toDense(), A));
...
GTEST_TEST(TestL2NormCost, UpdateCoefficients) {
  ...
  EXPECT_EQ(cost.A_sparse().rows(), 4);
  EXPECT_EQ(cost.A_dense().rows(), 4);
  EXPECT_TRUE(CompareMatrices(cost.A_sparse().toDense(), new_A));
  EXPECT_TRUE(CompareMatrices(cost.A_dense(), new_A));
  EXPECT_EQ(cost.b().rows(), 4);

...
GTEST_TEST(testCost, testLinearCost) {
  ...
  EXPECT_TRUE(
      CompareMatrices(math::ExtractValue(y_autodiff_identity),
                      Vector1<double>(obj_expected), tol));
  ...
  EXPECT_TRUE(
      CompareMatrices(math::ExtractValue(y_autodiff_empty),
                      Vector1<double>(obj_expected), tol));
  EXPECT_EQ(math::ExtractGradient(y_autodiff_empty).size(),
            0);

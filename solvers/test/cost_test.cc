...
GTEST_TEST(TestL1NormCost, Eval) {
  ...
  // Test Symbolic.
  {
    ...
    env.insert(x, x0);
    EXPECT_NEAR(z.cwiseAbs().sum(), y[0].Evaluate(env), 2e-15);
  }
}
...
GTEST_TEST(TestL2NormCost, Eval) {
  ...
  // Test Symbolic.
  {
    ...
    symbolic::Environment env;
    env.insert(x, x0);
    EXPECT_NEAR(z.norm(), y[0].Evaluate(env), 2e-15);
  }
}
...
GTEST_TEST(TestLInfNormCost, Eval) {
  ...
  // Test Symbolic.
  {
    ...
    symbolic::Environment env;
    env.insert(x, x0);
    EXPECT_NEAR(z.cwiseAbs().maxCoeff(), y[0].Evaluate(env), 2e-15);
  }
}
...
GTEST_TEST(TestPerspectiveQuadraticCost, Eval) {
  ...
  // Test AutoDiffXd.
  {
    ...
    EXPECT_TRUE(
        CompareMatrices(math::ExtractGradient(y), grad_expected, 3e-14));
  }

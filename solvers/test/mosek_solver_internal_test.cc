...
Eigen::MatrixXd ToSymmetric(const Eigen::MatrixBase<Derived>& X) {
  std::cout << fmt::format(
      "X\n{}\n(X+X.T)/2\n{}\n(X.eval()+X.eval().T)/2\n{}\n", fmt_eigen(X),
      fmt_eigen((X + X.transpose()) / 2),
      fmt_eigen((X.eval() + X.eval().transpose()) / 2));
...
GTEST_TEST(AddLinearMatrixInequalityConstraint, LMIonly) {
  ...
  std::cout << fmt::format("{}\n", fmt_eigen(lmi0.evaluator()->F()[0]));
  std::cout << fmt::format("{}\n", fmt_eigen(lmi0.evaluator()->F()[1]));
  std::cout << fmt::format("{}\n", fmt_eigen(lmi0.evaluator()->F()[2]));
  std::cout << fmt::format("{}\n", fmt_eigen(lmi1.evaluator()->F()[0]));
  std::cout << fmt::format("{}\n", fmt_eigen(lmi1.evaluator()->F()[1]));
...
GTEST_TEST(AddLinearMatrixInequalityConstraint, LMIandPSD) {
  // Test a program with both LMI and PSD constraints. The PSD matrix also shows
  // up in LMI.
  MathematicalProgram prog;
  auto x = prog.NewContinuousVariables(2);

...
TEST_F(CsdpDocExample, TestSdpaFreeFormatConstructor) {
  ...
  CompareBlockInX(dut.X_blocks()[0], BlockInX(csdp::MATRIX, 2));
  CompareBlockInX(dut.X_blocks()[1], BlockInX(csdp::MATRIX, 3));
  CompareBlockInX(dut.X_blocks()[2], BlockInX(csdp::DIAG, 2));
...
TEST_F(LinearProgram1, TestSdpaFreeFormatConstructor) {
  ...
  CompareBlockInX(dut.X_blocks()[0], BlockInX(csdp::DIAG, 7));
  EXPECT_EQ(dut.prog_var_in_sdpa().size(), prog_->num_vars());
...
TEST_F(LinearProgram3, TestSdpaFreeFormatConstructor) {
  ...
  CompareBlockInX(dut.X_blocks()[0], BlockInX(csdp::DIAG, 3));
  CompareBlockInX(dut.X_blocks()[1], BlockInX(csdp::DIAG, 4));

...
GTEST_TEST(GurobiTest, LogFile) {
  ...
  if (solver.available()) {
    ...
    // option. The common solver option should win.
    {
      ...
      auto result = solver.Solve(prog, {}, solver_options);
      EXPECT_TRUE(filesystem::exists({log_file}));
      // Even if we overwrite LogFile to gurobi2.log, Gurobi will still create
      // both gurobi_common2.log and gurobi2.log. It writes the log info into
      // gurobi2.log. So log_file should have more lines than log_file_common;
      EXPECT_TRUE(filesystem::exists({log_file_common}));
      ...
      int log_file_common_lines = 0;
      std::ifstream log_file_common_stream(log_file_common);
      while (std::getline(log_file_common_stream, line)) {
        log_file_common_lines++;
      }
      log_file_common_stream.close();
      EXPECT_GT(log_file_lines, log_file_common_lines);
    }
  }
}

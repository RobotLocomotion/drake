...
GTEST_TEST(OsqpSolverTest, DualSolution1) {
  ...
  if (osqp_solver.is_available()) {
    ...
    constraint2.evaluator()->set_bounds(Vector1d(3 + delta),
                                        Vector1d(3 + delta));
    const MathematicalProgramResult result_updated = osqp_solver.Solve(prog);
    EXPECT_NEAR(
...
GTEST_TEST(OsqpSolverTest, DualSolution3) {
  ...
  if (osqp_solver.is_available()) {
    ...
    const MathematicalProgramResult result1 = osqp_solver.Solve(prog);
    EXPECT_NEAR(
        (result1.get_optimal_cost() - result.get_optimal_cost()) / delta, -6,
        2e-5);
    constraint.evaluator()->UpdateUpperBound(Eigen::Vector2d(2, 2 + delta));
    const MathematicalProgramResult result2 = osqp_solver.Solve(prog);
    ...
    constraint.evaluator()->set_bounds(Eigen::Vector2d(-1 + delta, -1),
                                       Eigen::Vector2d(2, 2));
    const MathematicalProgramResult result3 = osqp_solver.Solve(prog);
    ...
    const MathematicalProgramResult result4 = osqp_solver.Solve(prog);
    EXPECT_NEAR(

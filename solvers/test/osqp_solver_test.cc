...
GTEST_TEST(OsqpSolverTest, ProgramAttributesGood) {
  ...
  EXPECT_TRUE(OsqpSolver::ProgramAttributesSatisfied(prog, &error));
  EXPECT_EQ(error, "");
}
...
GTEST_TEST(OsqpSolverTest, ProgramAttributesBad) {
  ...
  std::string error{"error"};
  EXPECT_FALSE(OsqpSolver::ProgramAttributesSatisfied(prog, &error));
  EXPECT_THAT(error, HasSubstr("GenericCost was declared"));
}
...
GTEST_TEST(OsqpSolverTest, ProgramAttributesMisfit) {
  ...
  std::string error{"error"};
  EXPECT_FALSE(OsqpSolver::ProgramAttributesSatisfied(prog, &error));
  EXPECT_THAT(error, HasSubstr("QuadraticCost is required"));
}

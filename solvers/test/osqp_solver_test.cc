...
GTEST_TEST(OsqpSolverTest, WarmStart) {
  MathematicalProgram prog;
  auto x = prog.NewContinuousVariables<3>();
  AddTestProgram(&prog, x);
  ...
  OsqpSolver osqp_solver;
  if (osqp_solver.available()) {
    for (bool primal_only : {false, true}) {
      MathematicalProgramResult result;
      SolverOptions solver_options;
      solver_options.SetOption(osqp_solver.solver_id(), "drake_warm_start", 1);
      if (primal_only) {
        solver_options.SetOption(
            osqp_solver.solver_id(), "drake_warm_start_primal_only", 1);
      }
      ...
      // Solve with warm-starting using prior solution.
      solver_options.SetOption(CommonSolverOption::kPrintToConsole, 1);
      // Note that warm starting from prior solution can yield the same number
      // of iterations. For this reason, we (aggressively) restrict it to 1
      // iteration.

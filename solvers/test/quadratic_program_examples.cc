...
void TestQPDualSolution3(const SolverInterface& solver, double tol1,
                         double tol2) {
  MathematicalProgram prog;
  auto x = prog.NewContinuousVariables<2>();
  ...
  if (solver.available()) {
    ...
    EXPECT_TRUE(result.is_success());
    EXPECT_TRUE(
        CompareMatrices(result.GetSolution(x), Eigen::Vector2d(2, -1), tol1));
    // At the optimal solution, the active constraints are
    // x[0] <= 2
    // x[1] >= -1
    // Solving the KKT condition, we get the dual solution as
    ...
    EXPECT_NEAR(result2.get_optimal_cost(), result.get_optimal_cost(), tol2);
    ...
    EXPECT_NEAR(result3.get_optimal_cost(), result.get_optimal_cost(), tol2);

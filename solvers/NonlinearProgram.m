...
%       obj.nlcon_eq_idx = obj.nlcon_eq_idx(~c_delete_flag)-c_delta(obj.nlcon_eq_idx(~c_delete_flag));
  ...
  methods(Access=protected)
    ...
    function [x,objval,exitflag,infeasible_constraint_name] = snopt(obj,x0)
      ...
      nx_free = length(x0_free);
      xmul = zeros(nx_free,1);
      xstate = zeros(nx_free,1);
      nF = length(lb);
      Fmul = zeros(nF,1);
      Fstate = zeros(nF,1);
      solveopt = 1;
      [x_free,objval,xmul,Fmul,exitflag] = snoptcmex(solveopt,x0_free, ...
        x_lb_free,x_ub_free,xmul,xstate, ...
        lb,ub,Fmul,Fstate,...
        0,1,Avals,iAfun,jAvar,...

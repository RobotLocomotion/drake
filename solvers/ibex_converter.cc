...
const ExprNode* IbexConverter::VisitCeil(const Expression&) {
  throw std::runtime_error("IbexConverter: Ceil expression is not supported.");
}

const ExprNode* IbexConverter::VisitFloor(const Expression&) {
  throw std::runtime_error("IbexConverter: Floor expression is not supported.");
}

const ExprNode* IbexConverter::VisitIfThenElse(const Expression&) {
  throw std::runtime_error(
      "IbexConverter: If-then-else expression is not supported.");
}
...
const ExprCtr* IbexConverter::VisitTrue(const Formula&, const bool) {
  throw std::runtime_error("IbexConverter: True is detected.");
}
...
const ExprCtr* IbexConverter::VisitEqualTo(const Formula& f,
  ...
  if (polarity) {
    ...
    throw std::runtime_error("IbexConverter: non-equlaity (!=) is detected.");
  }
}

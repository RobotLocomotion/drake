...
void ParsePositiveSemidefiniteConstraints(
  ...
  DRAKE_ASSERT(b->size() == *A_row_count);
  // Make sure that each triplet in A_triplets has row no larger than
  // *A_row_count.
  // With the lambda here, we cannot use DRAKE_ASSERT directly. So we wrap
  // DRAKE_DEMAND within if (kDrakeAssertIsArmed).
  if (kDrakeAssertIsArmed) {
    DRAKE_DEMAND(
        std::all_of(A_triplets->begin(), A_triplets->end(),
                    [A_row_count](const Eigen::Triplet<double>& triplet) {
                      return triplet.row() <= *A_row_count;
                    }));
  }

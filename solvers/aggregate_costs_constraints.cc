...
AggregateBoundingBoxConstraints(
  ...
  std::unordered_map<symbolic::Variable, std::pair<double, double>> bounds;
  for (const auto& constraint : bounding_box_constraints) {
    for (int i = 0; i < constraint.variables().rows(); ++i) {
      ...
      if (it == bounds.end()) {
        bounds.emplace_hint(it, var, std::make_pair(var_lower, var_upper));
      } else {
        if (it->second.first < var_lower) {
          it->second.first = var_lower;
        }
        if (it->second.second > var_upper) {
          it->second.second = var_upper;
        }
      }
    }
  }
...
std::pair<Eigen::VectorXd, Eigen::VectorXd> AggregateBoundingBoxConstraints(
  ...
  Eigen::VectorXd lower = Eigen::VectorXd::Constant(prog.num_vars(), -kInf);
  Eigen::VectorXd upper = Eigen::VectorXd::Constant(prog.num_vars(), kInf);

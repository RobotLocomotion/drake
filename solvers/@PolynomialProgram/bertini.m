...
eq = diff(obj.getPoly_objective, obj.decision_vars)'; % stationarity
vars = obj.decision_vars;

if ~isempty(obj.getPoly_equality_constraints)
  lambda = msspoly('l',length(obj.getPoly_equality_constraints));
  eq(1:obj.num_vars) = eq(1:obj.num_vars) + (lambda'*diff(obj.getPoly_equality_constraints, obj.decision_vars))';
  eq = vertcat(eq, obj.getPoly_equality_constraints); % primal feasibility
  vars = vertcat(vars, lambda);
end

if ~isempty(obj.getPoly_inequality_constraints)
  mu = msspoly('u',length(obj.getPoly_inequality_constraints));
  eq(1:obj.num_vars) = eq(1:obj.num_vars) + (mu'*diff(obj.getPoly_inequality_constraints, obj.decision_vars))';
  eq = vertcat(eq,mu.*obj.getPoly_inequality_constraints); % complementarity
  mu_indices = length(vars) + 1:length(obj.getPoly_inequality_constraints);
...
if ~isempty(obj.getPoly_inequality_constraints)
  ...
  nonlinear_constraint_vals = double(msubs(obj.getPoly_inequality_constraints,obj.decision_vars,sol));
  sol = sol(:,all(nonlinear_constraint_vals<=0)); % primal feasibility
end

vals = double(msubs(obj.getPoly_objective,obj.decision_vars,sol));

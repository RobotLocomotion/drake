...
symbolic::Polynomial MathematicalProgramResult::GetSolution(
  ...
  for (const auto& indeterminate : p.indeterminates()) {
    if (decision_variable_index_->find(indeterminate.get_id()) !=
  ...
  for (const auto& monomial_coefficient_pair :
       p.monomial_to_coefficient_map()) {
    for (const auto& var : monomial_coefficient_pair.second.GetVariables()) {
      const auto it = decision_variable_index_->find(var.get_id());
      if (it != decision_variable_index_->end()) {
        env.insert(var, x_val_(it->second));
      }
    ...
    monomial_to_coefficient_result_map.emplace_hint(
        monomial_to_coefficient_result_map.end(),
        monomial_coefficient_pair.first,
        monomial_coefficient_pair.second.EvaluatePartial(env));
  }

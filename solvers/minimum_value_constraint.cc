...
MinimumValueConstraint::MinimumValueConstraint(
    ...
    : Constraint(NumConstraints(minimum_value_lower, minimum_value_upper),
      ...
      penalties_double_(max_num_values_),
      penalties_autodiff_(max_num_values_) {
...
template <>
std::vector<double>& MinimumValueConstraint::penalties() const {
  return penalties_double_;
}

// Specialization for T=AutoDiffXd
template <>
std::vector<AutoDiffXd>& MinimumValueConstraint::penalties() const {
  return penalties_autodiff_;
}
...
void MinimumValueConstraint::EvalMaxPenalty(
  ...
  if (y_index >= 0) {
    ...
    if constexpr (std::is_same_v<T, double>) {
      penalties = &penalties_double_;
    } else if constexpr (std::is_same_v<T, AutoDiffXd>) {
      penalties = &penalties_autodiff_;
    }
    // Initialize penalty to 0.
    for (auto& p : *penalties) {
      p = T{0.};
    }
    int less_than_influence_count = 0;
    for (int i = 0; i < num_values; ++i) {
      const T& value = values(i);
      if (value < influence_value_) {
        Penalty(value, bound_value, influence_value_, penalty_function_,
                &((*penalties)[less_than_influence_count]));
        (*penalties)[i] *= penalty_output_scaling_;
        less_than_influence_count++;
      }
    }
    if (less_than_influence_count != 0) {
...
void MinimumValueConstraint::DoEvalGeneric(
  ...
  EvalMaxPenalty(minimum_value_lower_, values, y_lower_index, SmoothOverMax<T>,
                 y);
  EvalMaxPenalty(minimum_value_upper_, values, y_upper_index, SmoothUnderMax<T>,
                 y);
}

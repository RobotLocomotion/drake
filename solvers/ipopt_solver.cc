...
class IpoptSolver_NLP : public Ipopt::TNLP {
 public:
  ...
  virtual void finalize_solution(SolverReturn status, Index n, const Number* x,
    ...
    problem_->result_reporting_interface().SetSolverId(IpoptSolver::id());
    ...
    problem_->result_reporting_interface().SetDecisionVariableValues(
        solution);
    problem_->result_reporting_interface().SetOptimalCost(obj_value);
  }

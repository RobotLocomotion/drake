...
void ScsSolver::DoSolve2(const MathematicalProgram& prog,
  ...
  // Note that when psd_cone_length[i] = -1 or lmi_cone_length[i] = -1, that
  // constraint will not be parsed as PSD cone in SCS. So we should filter out
  // these entries with value -1.
  cone->ssize = 0;

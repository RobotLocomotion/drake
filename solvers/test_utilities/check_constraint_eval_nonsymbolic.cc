...
void CheckConstraintEvalNonsymbolic(
  ...
  const Eigen::VectorXd x_double{
      math::ExtractValueMatrixFromAutoDiff(x_autodiff)};
  Eigen::VectorXd y_double;
  constraint.Eval(x_double, &y_double);
  AutoDiffVecXd y_autodiff;
  constraint.Eval(x_autodiff, &y_autodiff);
  EXPECT_TRUE(CompareMatrices(
      y_double, math::ExtractValueMatrixFromAutoDiff(y_autodiff), tol));
  ...
  EXPECT_TRUE(CompareMatrices(
      J * math::ExtractGradientMatrixFromAutoDiff(x_autodiff),
      math::ExtractGradientMatrixFromAutoDiff(y_autodiff), std::sqrt(tol)));
}
}  // namespace test
}  // namespace solvers
}  // namespace drake

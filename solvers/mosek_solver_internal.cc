...
MSKrescodee MosekSolverProgram::AddAffineConeConstraint(
  ...
  switch (cone_type) {
    ...
    default: {
      throw std::runtime_error("MosekSolverProgram: unsupported cone types.");
    }
  }
  if (rescode != MSK_RES_OK) {
    return rescode;
...
MSKrescodee MosekSolverProgram::AddQuadraticCostAsLinearCost(
  ...
  // one_zero_vec = [1;0;...;0]
  Eigen::SparseMatrix<double> one_zero_vec(L.rows() + 2, 1);
  std::array<Eigen::Triplet<double>, 1> one_zero_vec_triplets;
  one_zero_vec_triplets[0] = Eigen::Triplet<double>(1, 0, 1);
  one_zero_vec.setFromTriplets(one_zero_vec_triplets.begin(),
                               one_zero_vec_triplets.end());
  MSKint64t acc_index;
  // g = [1;0;0;...;0]
  ...
  rescode = this->AddAffineConeConstraint(prog, L_bar_sparse, one_zero_vec,
                                          quadratic_vars, slack_vars, g,
                                          MSK_CT_RQUAD, &acc_index);
  if (rescode != MSK_RES_OK) {
    return rescode;
  }

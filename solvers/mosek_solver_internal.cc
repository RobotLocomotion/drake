...
MSKrescodee MosekSolverProgram::AddLinearMatrixInequalityConstraint(
  ...
  for (const auto& binding : prog.linear_matrix_inequality_constraints()) {
    ...
    if (matrix_rows != 2) {
      ...
      const Eigen::Vector3d c(binding.evaluator()->F()[0](0, 0) / 2,
                              binding.evaluator()->F()[0](1, 1),
                              binding.evaluator()->F()[0](0, 1));
      for (int i = 0; i < binding.variables().rows(); ++i) {
        A_triplets.emplace_back(0, i,
                                binding.evaluator()->F()[1 + i](0, 0) / 2);
        A_triplets.emplace_back(1, i, binding.evaluator()->F()[1 + i](1, 1));
        A_triplets.emplace_back(2, i, binding.evaluator()->F()[1 + i](0, 1));
      }

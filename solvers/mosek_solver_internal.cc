...
MSKrescodee MosekSolverProgram::AddQuadraticConstraints(
  ...
  // Store all the linear entries in all quadratic constraints into A *
  // decision_vars, where A is a sparse matrix, whose non-zero entries is
    ...
    for (int i = 0; i < constraint.variables().rows(); ++i) {
      ...
      if (decision_variable_to_mosek_matrix_variable().count(
        ...
        throw std::runtime_error(fmt::format(
            "variable {} shows up in the quadratic constraint {}, but this "
        ...
        if (constraint.evaluator()->Q()(i, j) != 0) {
          ...
          if (i == j) {
            ...
            if (decision_variable_in_mosek_index[i] !=
              ...
              // For off-diagonal term, if we have decision_var_index[i] !=
              // decision_var_index[j], then we are trying to add the term 2 *
              // Q()(i, j) * decision_var[i] * decision_var[j] to the quadratic
              // constraint. Since Mosek only takes the lower-diagonal entries
              // of the Hessian matrix, we append Q()(i, j) to the off-diagonal
              // entry of the Hessian matrix in Mosek.

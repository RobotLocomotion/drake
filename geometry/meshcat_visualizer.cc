...
void MeshcatVisualizer<T>::UpdateDeformableGeometry(
  ...
  if constexpr (std::is_same_v<T, double>) {
    ...
    for (int i = 0; i < ssize(vertex_positions); ++i) {
      ...
      const std::string newSubPath =
          fmt::format("{}/{}", path, i);
      TriangleSurfaceMesh<double> triSurface = internal::MakeTriangleSurfaceMesh(render_meshes[i], vertex_positions[i]);
...
template<typename T>
bool IsDeformableAcceptableGeometry(const SceneGraphInspector<T>& inspector, GeometryId geom_id) {
  const Shape& shape = inspector.GetShape(geom_id);
  auto extension = shape.Visit<std::string>(overloaded{
    [](const Mesh& mesh) {

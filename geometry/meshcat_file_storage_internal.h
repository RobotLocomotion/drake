...
class FileStorage final {
 public:
  ...
  /* Adds the given `content` to the database. All arguments are consumed by
  ...
  content already exists in the cache, the new hint will be dropped -- the hint
  used during the first Insert() always wins.

  @pre if the `content` has already been inserted, `extension` matches the
  previous extension. */

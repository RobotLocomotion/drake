...
std::string MeshcatZipFactory::Build() const {
  ...
  for (const Entry& entry : entries_) {
    ...
    if (int error_code = zip_set_file_compression(archive, /* index = */ index,
                                                  /* comp = */ ZIP_CM_STORE,
                                                  /* comp_flags = */ 0);
        error_code < 0) {
  ...
  if (int error_code = zip_close(archive); error_code < 0) {
  ...
  zip_stat_t stat;
  if (int error_code = zip_source_stat(buffer, &stat); error_code < 0) {
    throw_buffer_error("zip_source_stat");
  }
  const size_t buffer_size = stat.size;
  if (int error_code = zip_source_open(buffer); error_code < 0) {

...
vtkSmartPointer<vtkResourceStream> MeshMemoryLoader::DoLoad(const vtkURI& uri) {
  ...
  if (scheme == "file") {
    ...
    // Note: If the glTF file were to reference a file in a *higher* directory
    ...
    const std::string_view name(path.begin() + pos + kBaseUriPrefxLength,
                                path.end());
    const auto file_source_iter = mesh_.supporting_files.find(name);
    if (file_source_iter == mesh_.supporting_files.end()) {

...
/** The base class for defining a set of geometry properties.
 ...
 they are treated as having different semantics. To be a _set_ or properties,
 ...
 property _consumers_ and values are _defined_ by property _creators_.
 ...
 with SceneGraph by a creator and accessed by some downstream consumer entity.
 Each consumer specifies what properties it expects to find and what default
 values (if any) it provides. For example, the consumer could document that a
 particular property is always required and its absence would throw an
 ...
 the creator to make sure that the geometry propertie values are _correctly_

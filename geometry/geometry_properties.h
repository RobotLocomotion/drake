...
/** The base class for defining geometry properties. Ultimately, this will be
 ...
 All property value typess must either be cloneable or copy-constructible,
 ...
 properties and what kind of default values (if any) it provides. It becomes the
 responsibility of the initializer to make sure that the geometry properties
 are provided in the _correct_ form. Correctness includes such issues as
 key-value pairs placed into a *correctly*-spelled group, property keys being
 likewise correctly spelled, and values of the expected type. Correct spelling
 includes correct case.
 ...
 To read properties from a property set:
   ...
   1. Optionally, test to see if the desired property group exists in the
      property set via has_group(). Property access requires a group name and
 ...
 easily lead to name collisions in the common group (hence the inscrutable
 prefix "PRE").
 ...
 <h4>Targeted look up for _required_ properties</h4>
 ...
 These two examples show both possible workflows. The first in which no special
 action is taken for a missing property; the generic missing property error
 is dispatched. The second example shows a preliminary test in order to dispatch
 a custom message.

 NOTE: calls to `GetProperty()` always require the return type template value
 (`Eigen::Vector3d`) to be specified in the call.
 ...
 // Explicitly detect missing property and send custom property.
 if (!properties.HasProperty("MyGroup", "rgb")) {
   throw std::logic_error(
       "ThisClass: Missing required property 'rgb' in group 'MyGroup');
 }
 ...
 Working with properties in this manner require knowledge of how to work with
...
class GeometryProperties {
 public:
  ...
  const Group& get_group(const std::string& group_name) const {
  ...
  void AddProperty(const std::string& group_name, const std::string& name,
    ...
    if (iter != values_.end()) {
      ...
      throw std::logic_error(fmt::format(
          "Trying to add property `{}` to group `{}`; "
  ...
  /** Retrieves the typed value from the property set (if it exists), otherwise
   ...
   explicitly declare the return `ValueType` template parameter, e.g.:
  ...
  /** Reports the _total_ number of properties in this set.  */

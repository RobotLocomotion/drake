...
/** The base class for defining geometry properties. Ultimately, this will be
 sub-classed into derived properties appropriate for various application
 domains (e.g., proximity queries, sensor simulation, external visualization,
 etc.) This provides the basic underlying functionality of all the sub-classes
 but is _not_ intended to be used directly.

 All of a geometry's properties (its "set of properties"), are partitioned.
 The set can can contain multiple property _groups_. Each group
 has a unique name and contains a set of key-value pairs (the properties).
 All property value types must either be cloneable or copy-constructible,
 otherwise there are no restrictions on a property value type.

 Ultimately, geometry properties are defined when geometry is registered
 with SceneGraph and consumed by some downstream entity. Each "consumer"
 should specify what it expects to find in the geometry properties and what
 kind of default values (if any) it provides. For example, the consumer could
 document that a particular property is always required and its absence would
 throw an exception. Alternatively, it could indicate that a property is only
 optional and a default value will be used in its absence. It is the
 responsibility of the _registering code_ to make sure that the
 geometry properties are _correctly_ defined according to the expected
 consumer's specification. Correctness includes such issues as key-value pairs
 placed into a _correctly_-spelled group, property keys being likewise correctly
 spelled, and values of the expected type. Correct spelling includes correct
 case.
 ...
 To build a set of properties to apply to a geometry:
   ...
   1. Create a named group. Properties can only be assigned to a group that has
      already been added. There is a _default_ group included in every
   ...
   2. Add a _new_ property to the group via AddProperty(). This is used when
      ...
      property group already. Property names must be unique within a group.
 ...
 flexibility at the cost of spelling sensitivity. It would be easy to create
 ...
 ```
 ProximityProperties properties;
 properties.AddProperty(properties.default_group_name(), "count", 7);
 properties.AddProperty(properties.default_group_name(), "length", 7.);
 properties.AddProperty(properties.default_group_name(), "name", "7");
 ```

 <h4>Aggregate properties in a struct</h4>

 In some cases, there is a set of properties that will _always_ be accessed
 together. In these cases, it makes sense to aggregate them into a struct and
 store that as a single value. This reduces the number of lookups required.
 ...
 This section describes how to read properties under several difference
 scenarios: (a) when they are required, (b) when you provide a default, and
 (c) when you need to inspect what properties are available.
 ...
 Alternatively, the default value can be provided as:
 ...
 ```
 const ProximityProperties& properties = FunctionThatGivesProperties();
...
class GeometryProperties {
 public:
  ...
  /** Sets a named property in the given group. Assigns a value to a property,
   ...
   @tparam ValueType the type of data to store with the attribute -- must be
                     copy constructible or cloneable (see
                     systems::AbstractValue).  */
  template <typename ValueType>
  void SetProperty(const std::string& group_name, const std::string& name,
    ...
    if (iter != values_.end()) {
      ...
      throw std::logic_error(
          fmt::format("SetProperty(): Trying to add property {} to group {}. "
  ...
  /** Retrieves the typed value from the set of properties (if it exists),
   ...
   We recommend capturing the return value as a const reference; if the
   implementation changes under the hood to optimize copying, call sites which
   capture as references will immediately benefit.
  ...
  const Group& GetGroupProperties(const std::string& group_name) const {
    const auto iter = values_.find(group_name);
    if (iter != values_.end()) {
      return iter->second;
    } else {
      throw std::logic_error(
          fmt::format("GetGroupProperties(): Can't retrieve properties for a "
  ...
  /** Reports the number of properties in the named group.
   ...
   @param group_name  The name of the group to query.
  ...
  int NumProperties(const std::string& group_name) const {
    ...
    throw std::logic_error(
        fmt::format("NumProperties(): Cannot report number of properties for a "
                    "non-existant group: {}",
                    group_name));
  }

  /** Reports the _total_ number of properties in this set of properties.  */
  ...
  static const std::string& default_group_name() {
    static const never_destroyed<std::string> kDefaultGroup("__^default^__");

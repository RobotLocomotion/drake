...
class MeshcatVisualizer final : public systems::LeafSystem<T> {
 ...
 private:
  ...
  /* Sets or updates a deformable geometries, which replaces the visualizer with
   the new vertex positions. If the deformable geometry has not been registered
   before, it stores all new TriangleSurfaceMesh. Otherwise, it updates their
   positions with the new deformable geometry vertices.
  ...
  void SetOrUpdateDeformableGeometry(const QueryObject<T>& query_object,
                                     GeometryId geom_id,
  ...
  /* A store of the dynamic deformable geometries and their vector of triangle
    surface mesh. It is coupled with the version_. This is only for efficiency;

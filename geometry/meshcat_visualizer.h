...
class MeshcatVisualizer final : public systems::LeafSystem<T> {
 public:
  ...
  /** (Advanced) Creates an instance of %MeshcatVisualizer.

   While %MeshcatVisualizer can be explicitly constructed, proper use requires
   proper construction of the parameters below and subsequent connections
   between %MeshcatVisualizer's input ports to the appropriate sources. This
   process is greatly simplified by using the preferred
  ...
  /** Returns the surface-displacements input port. When connected, this port
   ...
   connected to actually have any effect. The preferred mechanism is to use
   visualization::ApplyVisualizationConfig (which handles the constructor

...
class MeshcatVisualizer final : public systems::LeafSystem<T> {
 public:
  ...
  used to e.g. add additional visualizations to the same scene. Note that a
  mutable Meshcat reference is returned (even with const access), because all
  visualization methods on Meshcat require mutability, but this does come with
  the downside that user code can defeat the goal of having deterministic
  visualizations given a Context. */
 ...
 private:
  ...
  /* The version of the geometry that was last set in Meshcat by this
   instance. If the version found on the input port differs from this value,
   SetObjects is called again before SetTransforms. This is intended to track
   the information in meshcat_, and is therefore also a mutable member

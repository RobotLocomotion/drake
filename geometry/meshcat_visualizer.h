...
class MeshcatVisualizer final : public systems::LeafSystem<T> {
  ...
  */
  MeshcatAnimation* start_recording() {
    recording_ = true;
  ...
  obtained from get_mutable_recording() will be rendered invalid. This does *not* currently remove the animation from Meshcat. */
  void DeleteRecording();

  /* Returns a mutable pointer to the singleton MeshcatAnimation object owned
  by this MeshcatVisualizer in which the frames will be recorded. This pointer
 ...
 private:
  ...
  // design thinking and may require either moving the prefix into the Context
  // as well (e.g. multiple copies of the MeshcatVisualizer publish to the same
  // Meshcat, but on different prefixes) or support for SetObject in
  // MeshcatAnimation (each animation keeps track of the objects, instead of
  // the shared Meshcat instance keeping track).  We may also want to allow
  // users to disable the default publishing behavior (to record without
  // visualizing immediately).

  /* MeshcatAnimation object for recording. It must be mutable so that frames can be added to it during Publish events. */
  mutable std::unique_ptr<MeshcatAnimation> animation_;

  /* Recording status.  True means that each new Publish event will record a frame in the animation. */

...
/** Defines the near and far clipping planes for frustum-based (OpenGL)
 ...
 <h4>The short summary</h4>
  - The clipping range defines the distance of the *closest* and farthest things
    that *can* appear in the rendering.
 ...
 color the same pixel, the triangle with the smallest z-value is selected.

 The z-buffer has fixed precision. That fixed precision is spread over the
 entire depth (as defined by the clipping range). The deeper the range, the
 less precision the z-buffer has. This will lead to artifacts where two objects
 near the same depth will flicker back and forth in front of each other. Two
 objects with measurably different depths relative to the camera, become
 indiscernible in the z-buffer due to these precision issues; the object that
 ends up in front is due to random chance.

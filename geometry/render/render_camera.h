...
/** Collection of core parameters for modeling a pinhole camera in a
  RenderEngine. These parameters are applicable to both depth and color/label
  renderings. Parameters specific to those output image type can be found
  below.
...
class RenderCameraCore {
 ...
 private:
  ...
  // Note: in the future, we'll add a sub-class of GeometryProperties so that
  // arbitrary properties can be added to support arbitrary RenderEngine
  // implementations.
};

/** Collection of camera properties for cameras to be used with color/label
 images. It includes all of the core render camera properties as well as
 request for the rendered image to be displayed.

 @see RenderCameraCore.  */

...
class RenderCameraCore {
 public:
  ...
  /** Expresses `this` camera's pinhole camera properties as the projective
   ...
   it uses the functor `set_element` to write values into the matrix. This
   function will write *all* entries `(i, j), i, j ∈ {0, 1, 2, 3}`. Callers
   waste effort if they pre-initialize the matrix.

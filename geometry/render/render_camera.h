...
class RenderCameraCore {
 public:
  ...
  /** Constructs a %RenderCameraCore from the old, symmetric camera
  ...
  /** Expresses `this` camera's pinhole camera properties as the projective
   ...
   (i.e., a classic OpenGl projection matrix).

   This function has no knowledge about how the matrix is implemented. Instead,
   it uses the functor `set_element` to write values into the matrix.
   Furthermore, this function only writes those entries that directly depend on
   the `camera` model parameters; all other entries are left untouched. As such,
   it requires the caller to make sure the matrix being written to has
   previously been set to all zeros.

   @param set_element  The functor used to set values at arbitrary entries
                       `(i, j), `i, j ∈ {0, 1, 2, 3}`.
   @pre The matrix being written to has all values zeroed out. */
  void SetProjectionMatrix(
      const std::function<void(int, int, double)>& set_element) const;

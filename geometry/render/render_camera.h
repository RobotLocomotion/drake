...
class RenderCameraCore {
 ...
 private:
  ...
  // so that arbitrary properties can be added to support arbitrary RenderEngine
  // implementations.
};
...
/** Defines a depth sensor's functional range. Only points that lie within the
 ...
 incorrect values from beyond the missing geometry. Generally, it's best if the
 near clipping plane is much smaller than the minimum depth range value. A

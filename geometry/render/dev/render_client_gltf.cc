...
void RenderClientGltf::UpdateViewpoint(const math::RigidTransformd& X_WC) {
  /* The vtkGLTFExporter populates the camera matrix in "nodes" incorrectly in
   ...
   When VTK is updated, this method override in the header, as well as the
   implementation here, can be deleted as RenderEngineVtk::UpdateViewpoint will
   correctly update the transforms on the cameras. */

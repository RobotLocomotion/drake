...
int do_main() {
  ...
  if (FLAGS_render_engine == "vtk") {
    ...
    std::unique_ptr<RenderEngine> client{
        MakeRenderClientGLTF(RenderClientGLTFParams())};
    /* TODO(svenevs): make use of the RenderClient inheritance or delete.
     See render_client.h, we can expose customization points.  Provided the user
     adds these customization points *prior* to AddRenderer call.
     */
    RenderClient* render_client{dynamic_cast<RenderClient*>(client.get())};
    DRAKE_DEMAND(render_client != nullptr);
    DRAKE_DEMAND(render_client->ProofOfConcept() == "ProofOfConcept");
    scene_graph->AddRenderer(render_name, std::move(client));
  }

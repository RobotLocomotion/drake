...
void ThrowIfInvalidTempDirectory(const std::string& temp_directory) {
  ...
  // Next, check that we can write to / read from the directory.
  // TODO(svenevs): the alternative is to check the file_status::permissions,
  // but it is not clear if we can assume owner vs group vs other.
  try {

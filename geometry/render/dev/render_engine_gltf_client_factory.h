...
/** Constructs a RenderEngine implementation which generates
 ...
 @note
   @code{.cpp}
   // Setup your server information and create the RenderEngine.
   RenderEngineGltfClientParams params;
   params.url = "http://some-server.url";
   auto render_engine = MakeRenderEngineGltfClient(params);
   // ... libcurl has been initialized ...

   // Situation (2), if needed cleanup and re-initialize curl with your
   // application's specific needs.
   curl_global_cleanup();
   curl_global_init();  // <<< your custom flags here

   // Situations (1) and (2), now that libcurl is initialized in a non-threaded
   // context, you may spawn your threads and continue as normal.
   @endcode

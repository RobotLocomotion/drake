...
GTEST_TEST(RenderClient, LoadColorImage) {
  ...
  // Create a standard drake image with the same size as the testing images.
  ImageRgba8U drake_image;
  CreateZeroDrakeImage(kTestImageWidth, kTestImageHeight, &drake_image);
...

  {
    ...
    for (const auto& [w, h] : width_height) {
      ImageRgba8U test_drake_image;
      CreateZeroDrakeImage(w, h, &test_drake_image);
      DRAKE_EXPECT_THROWS_MESSAGE(
...

  {
    ...
    EXPECT_TRUE(CompareSensorImages(drake_image, CreateTestColorImage(3)));
  }

  {
    ...
    EXPECT_TRUE(CompareSensorImages(drake_image, CreateTestColorImage()));
  }
}
...
GTEST_TEST(RenderClient, LoadDepthImage) {
...

  {
    ...
    const std::vector<std::pair<int, int>> width_height{
        ...
        {kTestImageWidth * 2, kTestImageHeight * 2}};
    for (const auto& [w, h] : width_height) {
      ImageDepth32F test_drake_image;
      CreateZeroDrakeImage(w, h, &test_drake_image);
      DRAKE_EXPECT_THROWS_MESSAGE(
...

  {
    ...
    EXPECT_TRUE(CompareSensorImages(drake_image, CreateTestDepthImage()));
  }
}
...
GTEST_TEST(RenderClient, LoadLabelImage) {
...

  {
    ...
    const std::vector<std::pair<int, int>> width_height{
        ...
        {kTestImageWidth * 2, kTestImageHeight * 2}};
    for (const auto& [w, h] : width_height) {
      ImageLabel16I test_drake_image;
      CreateZeroDrakeImage(w, h, &test_drake_image);
      DRAKE_EXPECT_THROWS_MESSAGE(
...

  {
    ...
    EXPECT_TRUE(CompareSensorImages(drake_image, CreateTestLabelImage()));
  }
}

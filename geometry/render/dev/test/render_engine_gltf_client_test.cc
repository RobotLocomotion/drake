...
GTEST_TEST(RenderEngineGltfClient, UpdateViewpoint) {
  ...
  auto compare = [&](const Matrix4d& vtk, const Matrix4d& gltf) {
    ...
    /* We expect that the MVT from RenderEngineVtk and RenderEngineGltfClient
     ...
     Make a comparison of the rotation components of the matrices directly. */
    ...
    /* Where translation is concerned, there is something wrong with
     ...
     GLTF is correct.  As such, we compare directly against the translation
     vector since the goal of the test is to make sure that the
     ModelViewTransformMatrix coming out of UpdateViewpoint will correctly
     place the camera in the *world* coordinate system. */

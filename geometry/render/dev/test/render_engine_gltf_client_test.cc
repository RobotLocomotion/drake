...
GTEST_TEST(RenderEngineGltfClient, Constructor) {
  std::string temp_dir;
  {
    ...
    auto engine = MakeRenderEngineGltfClient({});
...

  {
    ...
    Params params{std::nullopt, "0.0.0.0", 0, "super_render", true, true};
...
GTEST_TEST(RenderEngineGltfClient, Clone) {
  Params params{std::nullopt, "192.168.1.1", 2222, "endpoint", true, false};
  Engine engine{params};
  Tester tester{&engine};
...
GTEST_TEST(RenderEngineGltfClient, UpdateViewpoint) {
  ...
  auto compare = [&](const Matrix4d& vtk, const Matrix4d& gltf) {
    ...
    Matrix4d vtk_inv = pseudo_inverse(vtk);
    Matrix3d R_vtk = vtk_inv.topLeftCorner<3, 3>();
    Matrix3d R_gltf = gltf.topLeftCorner<3, 3>();
...
class FakeServer : public HttpService {
 public:
  ...
  HttpResponse PostForm(
    ...
    if (image_type == "color") {
      TestPngRgb8 png{path, width, height};
    } else if (image_type == "depth") {
      TestTiffGray32 tiff{path, width, height};
    } else {  // image_type := "label"
      TestPngGray16 label{path, width, height};
    }
    ret.data_path = path;

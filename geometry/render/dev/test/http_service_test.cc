...
class EmptyService : public HttpService {
 public:
  ...
  std::unique_ptr<HttpService> DoClone() const {
...
GTEST_TEST(HttpServiceTest, Constructor) {
  ...
  // Edge cases: temp directory does not exist, or cannot read / write to.
  // Does not exist.
  const std::string not_likely_a_directory{"/not/likely/a/directory"};
  DRAKE_EXPECT_THROWS_MESSAGE(
      EmptyService(not_likely_a_directory, localhost, port, false),
      fmt::format(
          "HttpService: provided temp directory '{}' is not a directory.",
          not_likely_a_directory));

  {
    // With a valid temporary directory, we should be able to construct.
  ...
  /* Used below in manufacturing bad permissions for unusable temp_directories.
   NOTE: directories need their execute bits removed to become unusable. */
  constexpr auto all_read =
      fs::perms::owner_read | fs::perms::group_read | fs::perms::others_read;
  constexpr auto all_write =
      fs::perms::owner_write | fs::perms::group_write | fs::perms::others_write;
  constexpr auto all_exec =
      fs::perms::owner_exec | fs::perms::group_exec | fs::perms::others_exec;

  {
    // Temporary directory that cannot be used case 1: no write perms.
    const auto temp_dir = drake::temp_directory();
    fs::permissions(temp_dir, all_write | all_exec, fs::perm_options::remove);
    DRAKE_EXPECT_THROWS_MESSAGE(
        EmptyService(temp_dir, localhost, port, false),
        fmt::format("HttpService: cannot write to / read from '{}'.",
                    temp_dir));
    fs::remove(temp_dir);
  }

  {
    // Temporary directory that cannot be used case 2: no read perms.
    const auto temp_dir = drake::temp_directory();
    fs::permissions(temp_dir, all_read | all_exec, fs::perm_options::remove);
    DRAKE_EXPECT_THROWS_MESSAGE(
        EmptyService(temp_dir, localhost, port, false),
        fmt::format("HttpService: cannot write to / read from '{}'.",
                    temp_dir));
  ...
  {
    // Temporary directory that cannot be used case 3: no perms.
    const auto temp_dir = drake::temp_directory();
    fs::permissions(temp_dir, fs::perms::all, fs::perm_options::remove);
    DRAKE_EXPECT_THROWS_MESSAGE(
        EmptyService(temp_dir, localhost, port, false),
        fmt::format("HttpService: cannot write to / read from '{}'.",
                    temp_dir));

...
std::string RenderClient::RenderOnServer(
  ...
  if (png_reader->CanReadFile(bin_out_path.c_str())) {
    return RenameToSceneWithExtension(scene_path, bin_out_path, ".png");
  }
  image_types_tried += "PNG";

  vtkNew<vtkTIFFReader> tiff_reader;
  if (tiff_reader->CanReadFile(bin_out_path.c_str())) {
    return RenameToSceneWithExtension(scene_path, bin_out_path, ".tiff");
  }
...
std::string RenderClient::RenameToSceneWithExtension(
    const std::string& input_scene, const std::string& path,
    const std::string& ext) const {
  const fs::path origin{path};
  if (!fs::is_regular_file(origin)) {
    throw std::runtime_error(fmt::format(
        "RenderClient: cannot rename '{}', file does not exist.", path));
  }

  // Require that input path to match is valid.
  if (!fs::exists(input_scene)) {
    throw std::runtime_error(
        fmt::format("RenderClient: cannot rename '{0}' to '{1}' with extension "
                    "'{2}': '{1}' "
                    "does not exist.",
                    path, input_scene, ext));
  }

  fs::path destination{input_scene};
  destination.replace_extension(fs::path{ext});

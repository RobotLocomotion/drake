...
struct HttpResponse {
  /** The HTTP response code from the server.  Note that in the special case of
   ...
   value of `0`.  Use HttpCodeOk() to validate server responses. */
  int32_t http_code{0};
  ...
  /** Whether or not the HTTP response code indicates success.  Note that if
   a server has not been properly implemented, it may always return `200` even
   if the response text indicates failure.  This is an error with the server,
   there is nothing this framework can do to validate such a scenario. */
  bool HttpCodeOk() const { return http_code >= 200 && http_code < 400; }
};
 ...
 public:
  ...
  /** Constructs an %HttpService to be used throughout the duration of a client
   ...
   @throws std::runtime_error
     If the specified `temp_directory` does not exist or cannot be written to.
   @throws std::logic_error
     If the provided `url` is empty, or has any trailing slashes. */
  explicit HttpService(const std::string& temp_directory,
  ...
  /** Post an HTML `<form>` to the specified `endpoint`.
   ...
   @endcode
   Make sure to check HttpResponse::HttpCodeOk() before continuing, or trying
   to load the HttpResponse::data_path as any specific kind of file.

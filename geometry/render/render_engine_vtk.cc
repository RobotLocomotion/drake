...
// So, instead, we render the depth image in the range [epsilon, z_near] and
// then detect the occluding pixels that lie closer than the camera's supported
// range and mark them as too close. Clipping all geometry beyond z_near is not
...
void RenderEngineVtk::InitializePipelines() {
  ...
  for (auto& pipeline : pipelines_) {
    ...
    // Initialize far plane to arbitrary value that will be overwritten by
    // the depth camera's z-far value.

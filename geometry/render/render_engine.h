...
class RenderEngine : public ShapeReifier {
 public:
  ...
  /** Given the poses of *all* geometries in SceneGraph (measured and expressed
   in the world frame), updates the renderer's pose representation of those
   geometries registered with _this_ renderer and were flagged as needing an
   update (see RegisterVisual()).  */
  template <typename T>
  void UpdatePoses(const std::vector<math::RigidTransform<T>>& X_WG) {
    for (auto pair : update_indices_) {
      RenderIndex render_index = pair.first;
      GeometryIndex geometry_index = pair.second;
      DoUpdateVisualPose(geometry::internal::convert(X_WG[geometry_index]),
                         render_index);
    }
  }
 ...
 protected:
  ...
  virtual void DoUpdateVisualPose(const math::RigidTransformd& X_WG,
                                  RenderIndex index) = 0;

...
class RenderEngine : public ShapeReifier {
  ...
  */
  ...
  /** Updates the configurations of all meshes associated with the given
   deformable geometry (see RegisterDeformableVisual()).
  ...
  */
  ...
  /** The NVI-function for sub-classes to implement actual deformable geometry
   ...
   chooses to register this particular geometry, it should return true.

   A derived render engine can use arbitrary criteria to decide if the
   deformable geometry gets registered. Note that when accessing the RenderLabel
   property in `properties` derived class should _exclusively_ use
   GetRenderLabelOrThrow().

   The given `id` is guaranteed to be unique and the given `render_meshes` are

...
class ShaderProgram {
 public:
  ...
  // Change this to ShaderProgramData (and define that in its own header file).
  // to eliminate dependency between OpenGlInstance and ShaderProgram.
  ...
  /* Sets the OpenGl projection matrix state. The projection matrix transforms a
   vertex from the geometry frame G to the OpenGl 2D device frame D -- it
   projects a point in 3D to a point on the image.  */
  void SetProjectionMatrix(const Eigen::Matrix4f& X_DG) const;

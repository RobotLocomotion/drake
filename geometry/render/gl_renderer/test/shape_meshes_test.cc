...
using RotationMatrixf = math::RotationMatrix<float>;
...
GTEST_TEST(LoadMeshFromObjTest, ReadingFile) {
  ...
  EXPECT_EQ(mesh_data.uvs.rows(), 0);
...
// Confirms that non-triangular faces get triangulated. This also tests that
// per-vertex normals are as expected.
GTEST_TEST(LoadMeshFromObjTest, TriangulatePolygons) {
  ...
  */
  ...
  EXPECT_EQ(mesh_data.positions.rows(), 9);
  EXPECT_EQ(mesh_data.normals.rows(), 9);
  EXPECT_EQ(mesh_data.uvs.rows(), 0);
  EXPECT_EQ(mesh_data.indices.rows(), 5);
  // The first three faces have a normal in the <0, 0, 1> direction.
  const auto pos_z = Vector3<GLfloat>::UnitZ().transpose();
  for (int t = 0; t < 3; ++t) {
    for (int i = 0; i < 3; ++i) {
      const int n_index = mesh_data.indices(t, i);
      EXPECT_TRUE(
          CompareMatrices(mesh_data.normals.block<1, 3>(n_index, 0), pos_z));
    }
  }
  // The last two in the <0, 1, 0> direction.
  const auto pos_y = Vector3<GLfloat>::UnitY().transpose();
  for (int t = 3; t < 5; ++t) {
    for (int i = 0; i < 3; ++i) {
      const int n_index = mesh_data.indices(t, i);
      EXPECT_TRUE(
          CompareMatrices(mesh_data.normals.block<1, 3>(n_index, 0), pos_y));
    }
  }
}
...
GTEST_TEST(ShapeMeshesTest, ConfirmUtilities) {
  ...
  */
  data.positions.block<1, 3>(0, 0) << 0, 0, 0;
  data.positions.block<1, 3>(1, 0) << 1.5, 0, 0;
  data.positions.block<1, 3>(2, 0) << 0, 1.5, 0;
  data.indices.resize(1, 3);
  data.indices.block<1, 3>(0, 0) << 0, 1, 2;
...
class NormalCone {
 public:
  NormalCone(const MeshData& mesh_data, int t) {
    ...
    const Vector3f n_0 =
        mesh_data.normals.block<1, 3>(mesh_data.indices(t, 0), 0).normalized();
    const Vector3f n_1 =
        mesh_data.normals.block<1, 3>(mesh_data.indices(t, 1), 0).normalized();
    const Vector3f n_2 =
        mesh_data.normals.block<1, 3>(mesh_data.indices(t, 2), 0).normalized();
    ...
    if (dist_01 < kEps && dist_02 < kEps && dist_12 < kEps) {
      // The normals are all in the same direction. So, the cone's "ray"
      ray_ = n_0;
      cos_theta_ = 1.0 - kEps;
...
GTEST_TEST(PrimitiveMeshTests, NormalCone) {
...

  {
    ...
    for (int i = 0; i < 3; ++i) {
      mesh_data.normals.block<1, 3>(i, 0) = expected_ray;
    }
...

  {
    ...
    const RotationMatrixf R(AngleAxisf(theta, Vector3f(-2, 1, 0).normalized()));
    ...
    for (int axis = 0; axis < 3; ++axis) {
      for (int i = 0; i < 3; ++i) {
        if (i == axis) {
          mesh_data.normals.block<1, 3>(i, 0) = n1;
        } else {
          mesh_data.normals.block<1, 3>(i, 0) = n0;
        }
      }
      const NormalCone cone(mesh_data, 0);
      ...
      // If we rotation the expected ray a bit more than theta, it is no longer
      // inside the cone. Sadly, due to 32-bit float precision, a minimum
      // additional rotation of 0.065 radians is necessary.
      const RotationMatrixf R_outside(
          AngleAxisf(theta + 0.065, Vector3f(-2, -1, 0).normalized()));
...

  {
    ...
    const Vector3f n_base =
        RotationMatrixf(AngleAxisf(theta, Vector3f(-2, 1, 0).normalized())) *
        expected_ray;
    for (int i = 0; i < 3; ++i) {
      const RotationMatrixf R{AngleAxisf(2 * M_PI / 3 * i, expected_ray)};
      mesh_data.normals.block<1, 3>(i, 0) = R * n_base;
    }
    const NormalCone cone(mesh_data, 0);
    // The ray and cos_theta should match within epsilon.
    EXPECT_TRUE(CompareMatrices(cone.ray(), expected_ray, kEps));
    ...
    for (int i = 0; i < 3; ++i) {
      const Vector3f n = mesh_data.normals.block<1, 3>(i, 0);
...
void TestGenericPrimitiveTraits(const MeshData& mesh) {
  ...
  for (int n_i = 0; n_i < mesh.normals.rows(); ++n_i) {
    const Vector3f n = mesh.normals.block<1, 3>(n_i, 0);
    EXPECT_NEAR(n.norm(), 1, kEps);
  }
  ...
  for (int t = 0; t < mesh.indices.rows(); ++t) {
    ...
    EXPECT_GT(n_face.dot(c), 0) << "for triangle " << t;
    const NormalCone cone(mesh, t);
    EXPECT_TRUE(cone.Contains(n_face))
...
GTEST_TEST(PrimitiveMeshTests, MakeLongLatUnitSphere) {
  ...
  for (int resolution : {3, 10, 20, 40}) {
    ...
    for (int v = 0; v < mesh_data.positions.rows(); ++v) {
      const Vector3f v_i = mesh_data.positions.block<1, 3>(v, 0);
      EXPECT_NEAR(v_i.norm(), 1.f, kEps) << "for vertex " << v;
      const Vector3f n_i = mesh_data.normals.block<1, 3>(v, 0);
...
GTEST_TEST(PrimitiveMeshTests, MakeUnitCylinder) {
  ...
  for (int resolution : {3, 10, 20, 40}) {
    ...
    for (int v_i = 1; v_i < mesh_data.positions.rows() - 1; ++v_i) {
      Vector3f v = mesh_data.positions.block<1, 3>(v_i, 0);
      v(2) = 0;
...
GTEST_TEST(PrimitiveMeshTests, MakeSquarePatch) {
  ...
  for (int resolution : {1, 4, 15}) {
    ...
    for (int v = 0; v < mesh_data.positions.rows(); ++v) {
      Vector3f vertex = mesh_data.positions.block<1, 3>(v, 0);
    ...
    for (int v = 0; v < mesh_data.positions.rows(); ++v) {
      ASSERT_TRUE(
          CompareMatrices(mesh_data.normals.block<1, 3>(v, 0), expected_n));
    }
  }
}

...
class DefaultRgbaColorShader final : public internal::ShaderProgram {
 public:
 ...
 private:
  ...
  // The vertex shader:
  //   - Transforms the vertex into device coordinates.
  //     - Technically, we don't have to have model_view and projection
  //       matrices separate to do this. But keeping them separate makes
  //       this trivially compatible with the other shaders.
  //   - Transforms the normal into camera coordinates and performs a
  //     rudimentary phong lighting calculation with a *directional* light
  //     pointing in the camera's view direction.
  // could combine modelview and projection matrices into a single
  // transform, but there's no real value in doing so. Leaving it as is
  // maintains compatibility with the depth shader.
...
void main() {
  ...
  p_CV = model_view_matrix * vec4(p_MV, 1);
  ...
  // For each fragment from a geometry, it simply colors the fragment with
  // the provided label encoded as an RGBA color.
...
void main() {
  ...
  color.rgb = diffuse_color.rgb * max(dot(nhat_C, light_dir_C), 0.0);
})""";
};
...
RenderEngineGl::RenderEngineGl(RenderEngineGlParams params)
  ...
  // texture to be "more preferred" than color from rgba, so we add it the
...
void RenderEngineGl::RenderColorImage(const CameraProperties& camera,
  ...
  //  geometries than need to be sorted from farthest to nearest the camera and

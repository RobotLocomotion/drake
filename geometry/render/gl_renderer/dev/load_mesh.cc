...
std::pair<VertexBuffer, IndexBuffer> LoadMeshFromObj(
  ...
  const bool ret =
      tinyobj::LoadObj(&attrib, &shapes, &materials, &err, input_stream,
                       readMatFn, do_tinyobj_triangulation);
  // As of tinyobj v1.0.6, we expect that `ret` will *always* be true. We
  // capturing it an asserting it so that if the version advances, and false is
  // ever returned, CI will inform us.

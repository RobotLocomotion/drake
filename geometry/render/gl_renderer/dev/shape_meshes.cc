...
/* Creates a triangle mesh for a revolute surface. It is, essentially, a curve
 ...
 the curve path (i.e., every circle's center les on the z axis). The total
 ...
 @pre radius_i(0) and radius_i(curve_sample_count) = 0.
 */
pair<VertexBuffer, IndexBuffer> MakeRevoluteShape(
  ...
  std::cerr << "Revolute expects " << vert_count << " vertices and "
            << tri_count << " tris\n";
  ...
  // Triangles spanning ring C-1 to ring C; a triangle fan around the last
  // vertex.
...
pair<VertexBuffer, IndexBuffer> MakeLongLatUnitSphere(int longitude_bands,
  ...
  */
  ...
  auto calc_z_i = [delta_phi](int ring_i) { return cosf(ring_i * delta_phi); };

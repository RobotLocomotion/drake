...
// These are pseudo-public aliases -- the are exposed to other classes in the
// render::internal namespace, but aren't part of the public API.

/* The representation of all vertex positions in the mesh encoded in a Nx3
 matrix such that the ith row is the position fo the ith vertex in the mesh
...
/* The representation of all mesh triangles, encoded in an Nx3 matrix of index
 values. The ith row represents the ith triangle such that columns 0, 1, and 2
 that row are indices into the corresonding VertexBuffer. All values in this
 data should lie in the range [0, vertices.rows() - 1], for the corresponding
 set of vertices.

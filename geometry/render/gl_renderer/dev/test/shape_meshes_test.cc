...
GTEST_TEST(PrimitiveMeshTests, MakeLongLatUnitSphere) {
  ...
  for (int resolution : {3, 10, 20, 40}) {
    ...
    float area = total_area(vertices, indices);
    EXPECT_GT(area, prev_area) << "for resolution " << resolution;
    EXPECT_LE(area, kIdealArea) << "for resolution " << resolution;
    prev_area = area;
...
GTEST_TEST(PrimitiveMeshTests, MakeSquarePatch) {
  ...
  for (int resolution : {1, 4, 15}) {
    ...
    // first scale factor is the actual expected area. There is a second, more
    // subtle factor. We expect there to be a per-triangle error. The more
    // triangles there the more error we accumulate. So, we also multiply by
    // the number of triangles to account for this additional source of error.

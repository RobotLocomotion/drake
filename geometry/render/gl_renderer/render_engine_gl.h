...
class RenderEngineGl final : public RenderEngine {
 ...
 private:
  ...
  // Computes the projection matrix based on the given camera.
  ...
  // The collection of all shader prorams (grouped by image type).
  ...
  // for each unique render image size (BufferDim) and output image type. The

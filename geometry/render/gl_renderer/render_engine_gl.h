...
class RenderEngineGl final : public RenderEngine {
 ...
 private:
  ...
  // Configure the projection matrix -- computes the matrix and sets the value
  ...
  // Configure the "modelview" matrix and sets the value in the shader program's
  // "model_view_matrix" uniform. For a single camera, this changes on a
  // *per-geometry* basis.

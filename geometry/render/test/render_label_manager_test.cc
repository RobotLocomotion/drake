...
// Tester class to give access to RenderLabel private elements.
class RenderLabelTester {
 public:
  // Reports the maximum allocatable value supported by RenderLabel.
...
class RenderLabelManagerTester {
 public:
  ...
  ::testing::AssertionResult HasConsistentNames(
      const RenderLabelManager& manager) const {
    for (const auto& name_vector_pair : manager.name_label_map_) {
      const std::string& name = name_vector_pair.first;
      for (const auto& label_class : name_vector_pair.second) {
        if (name != label_class.name) {
          return ::testing::AssertionFailure()
                 << "Render label class has name '" << label_class.name
                 << "' stored in the collection named '" << name << "'";
        }
      }
    }
  ...
  ::testing::AssertionResult HasRenderLabelClass(
    ...
    for (const auto& test_label :
         manager.name_label_map_.at(label_class.name)) {
      if (test_label.source_id == label_class.source_id) {
        if (test_label.label == label_class.label) {
          ...
          return ::testing::AssertionFailure()
                 ...
                 << label_class.label << ", found " << test_label.label;
        }
      }
    }
    return ::testing::AssertionFailure()
  ...
  int LabelCount(const RenderLabelManager& manager) const {
    size_t count = 0;
    for (const auto& pair : manager.name_label_map_) {
      // Count the number of labels each name maps to.
  ...
  ::testing::AssertionResult HasReservedLabels(
    ...
    ::testing::AssertionResult result = HasRenderLabelClass(
        manager, RenderLabelClass("empty", source_id,
                                  RenderLabel::empty_label()));
    if (result) {
      result = HasRenderLabelClass(
          manager, RenderLabelClass("ignored", source_id,
                                    RenderLabel::ignored_label()));
      if (result) {
        result = HasRenderLabelClass(
            manager, RenderLabelClass("unclassified", source_id,
                                      RenderLabel::unclassified_label()));
      }
    }
    return result;
  }
...
GTEST_TEST(RenderLabelManagerTest, GetRenderLabel) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      "All render labels have been allocated");
}

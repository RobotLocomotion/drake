...
/* Tests if the `from_mtl` flag is correctly propagated. */
TEST_F(LoadRenderMeshFromObjTest, PropagateFromMtlFlag) {
  for (const bool has_mtl : {false, true}) {
    ...
    EXPECT_EQ(has_mtl, material.from_mtl);
  }
}

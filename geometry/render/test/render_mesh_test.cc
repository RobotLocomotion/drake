...
const fs::path LoadRenderMeshFromObjTest::kTempDir = temp_directory();
...
TEST_F(LoadRenderMeshFromObjTest, GeometryErrorModes) {
  ...
  }
  {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
        LoadRenderMeshFromObj(obj_path.string(), empty_props(), kDefaultDiffuse,
                              diagnostic_policy_),
...
TEST_F(LoadRenderMeshFromObjTest, GeometryTriangulatePolygons) {
  ...
  */
  ...
  struct TriangluateTestParmas {
    ...
    Vector3d n0;
    Vector2d uv0;
    // The expected normal and uvfor the vertices in face 1 (f1).
    Vector3d n1;
    Vector2d uv1;
  };
  ...
  for (const auto& params : test_params) {
    ...
    const RenderMesh mesh_data = LoadRenderMeshFromObj(
        obj_path.string(), empty_props(), kDefaultDiffuse, diagnostic_policy_);
...
/* The OBJ has a single intrinsic material which only defines diffuse texture.
 ...
 has an 60% grey diffuse color and the named texture. This grey color comes
 from encoded tinyobj behavior. */
TEST_F(LoadRenderMeshFromObjTest, MaterialLibraryTextureOnly) {
  ...
  const RenderMesh mesh = LoadRenderMeshFromObj(
      obj_path, empty_props(), kDefaultDiffuse, diagnostic_policy_);
  // Tiny obj uses 0.6 as the default luminance and, by default, uses 32-bit
  // floats. This tolerance is to protect us from the value that is only
  // approximated in binary from varying precision.
  EXPECT_TRUE(mesh.material.diffuse.AlmostEqual(Rgba(0.6, 0.6, 0.6), 1e-7));
...
/* The OBJ has multiple intrinsic materials *applied* to the mesh. The material
 should be the fallback material and should be accompanied by a warning. */

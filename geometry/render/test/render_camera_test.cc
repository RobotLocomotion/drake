...
correctly.  */
GTEST_TEST(RenderCameraCoreTest, SetProjectionMatrix) {
  ...
  Matrix4d X_DC_base = Matrix4d::Constant(M_PI);
  base_cam.SetProjectionMatrix(
      [&X_DC_base](int i, int j, double value) { X_DC_base(i, j) = value; });
  {
    /* Confirm that all of the zeros have also been written.  */
  ...
  Eigen::Matrix4d X_DC;
  auto set_X_DC = [&X_DC](int i, int j, double value) { X_DC(i, j) = value; };

  {
    ...
    X_DC = Matrix4d::Zero();
    cam.SetProjectionMatrix(set_X_DC);
    X_DC(0, 0) *= 2;
    X_DC(0, 2) -= cx / w;
  ...
  GTEST_TEST(ColorRenderCameraTest, Constructor) {
    ...
    EXPECT_TRUE(CompareMatrices(
        camera.core().sensor_pose_in_camera_body().GetAsMatrix4(),
        X_BS.GetAsMatrix4()));

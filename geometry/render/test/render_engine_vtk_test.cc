...
TEST_F(RenderEngineVtkTest, BoxTest) {
  for (const bool use_texture : {false, true}) {
    for (const double texture_scale : {1.0, 0.5}) {
      ...
      // corner will report the expected depth, color, and label. The box
      // needs to be offset *almost* half its width and depth. The "almost"
      // comes from the fact that it must *fill* the center pixel.
      ...
      if (texture_scaled) {
        ...
        // Quick proof that we're testing for a different color.

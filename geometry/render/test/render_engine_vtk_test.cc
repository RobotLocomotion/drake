...
// Performs the shape-centered-in-the-image test with a textured sphere.
TEST_F(RenderEngineVtkTest, TexturedSphereTest) {
  Init(X_WC_, true);
  PopulateSphereTest(renderer_.get(), true /* use_texture */);
  expected_color_ = RgbaColor(kTextureColor, 255);
  PerformCenterShapeTest(renderer_.get(), "Texture sphere test");
}

// Performs the shape-centered-in-the-image test with a sphere.
TEST_F(RenderEngineVtkTest, TransparentSphereTest) {
//  for (bool use_texture : {true, false}) {}

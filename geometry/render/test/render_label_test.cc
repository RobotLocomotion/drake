...
TEST_F(RenderLabelTests, Construction) {
  ...
  EXPECT_EQ(RenderLabel(), RenderLabel::unspecified_label());

  // Bad values throw an exception.
  using ValueType = RenderLabel::ValueType;

  std::vector<ValueType> bad_values{
      -1, RenderLabel::unspecified_label(), RenderLabel::do_not_render_label(),
      RenderLabel::dont_care_label(), RenderLabel::empty_label()};
...
TEST_F(RenderLabelTests, TestForReservedLabels) {
  for (RenderLabel label : labels_) {
    ...
    EXPECT_FALSE(label.is_dont_care());
    EXPECT_FALSE(label.is_do_not_render());
    EXPECT_FALSE(label.is_unspecified());
  }

  EXPECT_TRUE(RenderLabel::empty_label().is_empty());
  EXPECT_FALSE(RenderLabel::empty_label().is_do_not_render());
  EXPECT_FALSE(RenderLabel::empty_label().is_dont_care());
  EXPECT_FALSE(RenderLabel::empty_label().is_unspecified());

  EXPECT_FALSE(RenderLabel::dont_care_label().is_empty());
  EXPECT_FALSE(RenderLabel::dont_care_label().is_do_not_render());
  EXPECT_TRUE(RenderLabel::dont_care_label().is_dont_care());
  EXPECT_FALSE(RenderLabel::dont_care_label().is_unspecified());

  EXPECT_FALSE(RenderLabel::do_not_render_label().is_empty());
  EXPECT_TRUE(RenderLabel::do_not_render_label().is_do_not_render());
  EXPECT_FALSE(RenderLabel::do_not_render_label().is_dont_care());
  EXPECT_FALSE(RenderLabel::do_not_render_label().is_unspecified());

  EXPECT_FALSE(RenderLabel::unspecified_label().is_empty());
  EXPECT_FALSE(RenderLabel::unspecified_label().is_do_not_render());
  EXPECT_FALSE(RenderLabel::unspecified_label().is_dont_care());
  EXPECT_TRUE(RenderLabel::unspecified_label().is_unspecified());
}
...
TEST_F(RenderLabelTests, AssignmentAndComparison) {
  ...
  EXPECT_TRUE(labels_[0] < RenderLabel::unspecified_label());
}

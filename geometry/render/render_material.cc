...
RenderMaterial DefineMaterial(const GeometryProperties& props,
  ...
  // be MemoryFile-valued. If it were FileSource-valued, we'd get both for one
  // GetProperty() call.
  ...
  if (has_diffuse_map) {
    DRAKE_DEMAND(material.diffuse_map.is_path());
    const std::filesystem::path& diffuse_path = material.diffuse_map.path();

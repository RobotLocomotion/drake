...
class RenderLabelManager {
 public:
  ...
  /** Provides a record of all allocated render labels and their semantic
   ...
   the semantic name will map to multiple semantic classes, distingiushed by
   their source id and RenderLabel value.  */
  const std::unordered_multimap<std::string, RenderLabelClass>&
  GetRenderLabelClasses() const { return name_label_map_; }

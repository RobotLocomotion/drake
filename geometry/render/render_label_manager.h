...
class RenderLabelManager {
 ...
 private:
  ...
  // with that name (they *must* all have different source ids). This isn't an
  // unordered_multimap because the multimap is very *bad* at letting you see
  // *which* values have been associated with a particular key (an important
  // operation for GetRenderLabel()). So, we have this ad hoc multimap instead.

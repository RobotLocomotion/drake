...
vector<RenderMesh> LoadRenderMeshesFromObj(
  ...
  map<int, std::set<int>> material_triangles;
  ...
  for (const auto& shape : shapes) {
    ...
    for (int f = 0; f < num_faces; ++f) {
      ...
      material_triangles[mat_index].insert(ssize(triangles));
  ...
  for (const auto& [mat_index, tri_index_set] : material_triangles) {
    ...
    /* Partition the data. For each `RenderMesh` we need the subset of
     *referenced* vertex indices, re-enumerated to the partition. At the same
    ...
    for (const auto& t : tri_index_set) {
    ...
    mesh_data.indices.resize(ssize(tri_index_set), 3);
    int row = -1;
    for (const int t : tri_index_set) {

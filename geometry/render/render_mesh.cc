...
TriangleSurfaceMesh<double> MakeTriangleSurfaceMesh(
    const RenderMesh& render_mesh, const VectorX<double> &new_vertex_positions) {
  ...
  if (new_vertex_positions.size() != 0) {
    DRAKE_DEMAND(3 * num_vertices == static_cast<int>(new_vertex_positions.size()));
  ...
  for (int v = 0; v < num_vertices; ++v) {
    if (usingNewVertices) {
      const int i = 3 * v;
      vertices.emplace_back(new_vertex_positions[i], new_vertex_positions[i + 1],

...
/**
 ...
 - `generic`: the `generic` label is intended as a convenient dumping ground.
   This would be for geometry that _should_ render into the label image, but
   whose class is irrelevant (e.g., the walls of a room a robot is working in
   or the background terrain in driving simulation).
 - `unclassified`: in the absence of an explicitly assigned render label, a
   geometry receives the unclassified label. It is considered a defect for an
   application not to explicitly assign a label to every rendered geometry; this
   tag simply aids in detecting if such has occurred. This marks the distinction
   between an application _actively_ declaring it doesn't care about a
   geometry's class (via the `generic` %RenderLabel) and an oversight that may
   lead to a geometry not receiving a label.
...
class RenderLabel {
 public:
  ...
  /** Returns a label representing the "generic" label.  */
  static RenderLabel generic_label() {
    return RenderLabel{kGeneric, false};
  }

  /** Reports if the label is the "generic" label.  */
  bool is_generic() const { return value_ == kGeneric; }
 ...
 private:
  ...
  static constexpr ValueType kGeneric = kEmpty - 1;
  static constexpr ValueType kMaxUnreserved = kGeneric - 1;

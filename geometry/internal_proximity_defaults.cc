...
void ApplyProximityDefaults(GeometryState<T>* geometry_state,
  ...
  if (geometry_state->IsDeformableGeometry(gid)) {
    return;
  }

  // Get current proximity properties -- some should exist, given the
  // filtering done above.
  const auto* found_props = geometry_state->GetProximityProperties(gid);
  ...
  auto is_gid_source = [&](SourceId& source_id) {
        return geometry_state->BelongsToSource(gid, source_id);
  };
  SourceId gid_source_id = *std::find_if(source_ids.begin(), source_ids.end(),
                                         is_gid_source);

  // Make the final changes to proximity properties.
  geometry_state->AssignRole(gid_source_id, gid, props, RoleAssign::kReplace);
}

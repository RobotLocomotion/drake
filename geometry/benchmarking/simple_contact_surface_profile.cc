...
/** @defgroup contact_surface_profiles Contact Surface Profiles
 ...
 This is a simple simulation where a bouncing ball, respresented by a soft
 sphere, is moved between a rigid cylinder and a rigid box. Eventually we'd like
 to profile a full system through multibody plant, but this is decoupled from
 dynamics so that just the geometric components can be evaluated.

 Arguments include:
 - __sphere_edge_length__: Measure of the soft sphere's edge length, where
   smaller numbers produce a denser, more expensive mesh. Defaults to 1.
 ...
 ```
 perf record -g bazel-bin/geometry/benchmarking/simple_contact_surface_profile --sphere_edge_length=0.5
 ```
...
DEFINE_double(sphere_edge_length, 1.0,
...
class MovingBall final : public LeafSystem<double> {
 public:
  ...
  explicit MovingBall(SceneGraph<double>* scene_graph) {
    ...
    AddSoftHydroelasticProperties(FLAGS_sphere_edge_length, &prox_props);
...
int profile_contact_surface() {
  ...
  // Add arbitrary cylinder to bang into.
...
int main(int argc, char* argv[]) {
  ...
  return drake::geometry::internal::profile_contact_surface();
}

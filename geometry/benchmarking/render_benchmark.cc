...
class RenderEngineTester {
 public:
  ...
  static void SetDefaultLightPosition(const Vector3<double>& X_WL,
                                      RenderEngine* renderer) {
    renderer->SetDefaultLightPosition(X_WL);
  }
};
...
class RenderEngineBenchmark : public benchmark::Fixture {
 public:
  ...
  // Keep track of the paths to the saved images. We use a set because Google
  // Benchmark can run these benchmarks multiple times to get results, and we
  // want to avoid duplicate path names.

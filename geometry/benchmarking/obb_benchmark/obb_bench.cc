...
struct TestData {
  ...
  bool expect_overlap;   // I'm not sure whether to keep this field since
  // we're doing benchmarking to optimize speed not unit testing. The unit test
  // to ensure correctness is already in geometry/proximity/test/obb_test.cc.
};

void Obb_HasOverlap(const std::vector<TestData>& test_data) {
  for (const TestData& t : test_data) {
    EXPECT_EQ(t.expect_overlap, Obb::HasOverlap(t.a_G, t.b_H, t.X_GH));
  }
}
...
void SetUp(std::vector<TestData>* test_data) {
...
static void Benchmark_Obb_HasOverlap(benchmark::State& state) {
  // Perform setup here
...
BENCHMARK(Benchmark_Obb_HasOverlap);

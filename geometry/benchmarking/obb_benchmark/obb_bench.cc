...
// TODO(DamrongGuoy): CalcCornerTransform(), CalcEdgeTransform(), and
//  SetUp31Records() were copied from geometry/proximity/test/obb_test.cc and
//  adapted from Google Test framework to Google Benchmark framework.
//  Consider eliminating the duplication between this file and obb_test.cc.

// TODO(DamrongGuoy): Consider renaming CalcCornerTransform() to
//  CreatePoseAlongBoxAxis() here and also in
//  geometry/proximity/test/obb_test.cc.

// Creates a pose to test the case where the separating axis is one of the axes
// of Box A. Box B is posed such that its minimum corner touches a face on Box
// with the rest of Box B lying in the A_{axis} direction from Bq. If
// expect_overlap is true, the box B is shifted 0.01 units into box A, if false,
// 0.01 units out of box A. We can do so by generating the rotation
// component, R_AB, such that Box B's minimum corner Bq has a minimum value
// along the chosen axis, and we can solve for the translation component,
// p_AoBo_A = p_AoAf_A + p_AfBq_A + p_BqBo_A.
RigidTransformd CalcCornerTransform(const Obb& a, const Obb& b, const int axis,
                                    const bool expect_overlap);

// TODO(DamrongGuoy): Consider renaming CalcEdgeTransform() to
//  CreatePoseAlongEdgeCrossProduct() here and also in
//  geometry/proximity/test/obb_test.cc.
...
// axes from Frame A and Frame B. For each of the 15 axes, we consider both
// overlapping and non-overlapping configurations. We also test that it is
// robust for the case of parallel boxes.
...
class ObbHasOverlap : public benchmark::Fixture {
 public:
  ObbHasOverlap() {
...
BENCHMARK_F(ObbHasOverlap, All_31_Cases)(benchmark::State& state) {
  // Perform setup here
...
// NOLINTNEXTLINE(runtime/references) cpplint disapproves of gbench choices.
BENCHMARK_F(ObbHasOverlap, Overlap_16_Cases)(benchmark::State& state) {
  // Perform setup here
  std::vector<TestData> test_data;
...
// NOLINTNEXTLINE(runtime/references) cpplint disapproves of gbench choices.
BENCHMARK_F(ObbHasOverlap, NonOverlap_15_Cases)(benchmark::State& state) {
  // Perform setup here
  std::vector<TestData> test_data;
  for (const TestData& t : data_for_all_cases_)
    if (!t.expect_overlap) test_data.push_back(t);

...
VolumeMesh<T> MakeConvexVolumeMesh(
  ...
  //   convex hull, such tests are unnecessary. Currently, we do invoke it with
  //   such a mesh, but not signaling it (because this code wouldn't do anything
  //   differently if we did). When we have the ability to test/check arbitrary,
  //   meshes we should exclude testing when we know it is a valid convex mesh
  //   by construction.

...
std::unique_ptr<BvNode<MeshType>> Bvh<MeshType>::BuildBVTree(
  ...
  Obb bv = ComputeBoundingVolume(mesh_M, start, end);
  ...
  if (num_elements <= BvNode<MeshType>::kMaxElementPerLeaf) {
    ...
    return std::make_unique<BvNode<MeshType>>(bv, data);
  } else {
    ...
    const auto& Baxis_M = bv.pose().rotation().col(axis);
    std::sort(start, end,
...
Obb Bvh<MeshType>::ComputeBoundingVolume(
    ...
    const typename std::vector<CentroidPair>::iterator& start,
    const typename std::vector<CentroidPair>::iterator& end) {
  // Prepare unique vertices (by vertex indices) to help covariance method in
  // ComputeObb(). We use set<> instead of unordered_set<> because the order of
  // vertices can numerically change the covariance matrix.

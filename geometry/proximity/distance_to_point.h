...
struct CallbackData {
  /** Constructs the fully-specified callback data. The values are as described
   ...
   @param X_WG_in             The T-valued poses. Aliased.
  ...
  CallbackData(
      ...
      : query_point(DRAKE_GEOMETRY_SAFE_DEREFERENCE(query_in)),
        geometry_map(DRAKE_GEOMETRY_SAFE_DEREFERENCE(geometry_map_in)),
        threshold(threshold_in),
        p_WQ_W(p_WQ_W_in),
        X_WG(DRAKE_GEOMETRY_SAFE_DEREFERENCE(X_WG_in)),
        distances(DRAKE_GEOMETRY_SAFE_DEREFERENCE(distances_in)) {}
...
bool Callback(fcl::CollisionObjectd* object_A_ptr,
  ...
  const math::RigidTransform<T> typed_X_WG(data.X_WG[encoding.index()]);

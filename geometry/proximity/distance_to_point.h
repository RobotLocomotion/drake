...
struct CallbackData {
  CallbackData(
      ...
      const std::vector<Isometry3<T>>* X_WG_in,
...
/** @name Functions for computing distance from point to primitives
 ...
 @param grad_W[out]     The gradient of the distance function w.r.t p_GQ (the
                        ...
                        gradient vector is expressed in the world frame.  */
//@{
...
class DistanceToPoint {
 public:
  ...
  SignedDistanceToPoint<T> operator()(const fcl::Boxd& box) {
    ...
    std::tie(p_GN_G, grad_G) = BoxDistanceXd(h, p_GQ_G);
    Vector3<T> grad_W = X_WG_.rotation() * grad_G;
  ...
  SignedDistanceToPoint<T> operator()(const fcl::Cylinderd& cylinder) {
    ...
    Vector2<T> grad_B;
    Matrix2<T> R_GB;
    R_GB.col(0) = xyz_to_rz(Vector3<T>(0, 0, p_GQ(1)));
    R_GB.col(1) << sqrt(p_GQ(0) * p_GQ(0) + p_GQ(1) + p_GQ(1)), 0;
    std::tie(p_BN, grad_B) = BoxDistanceXd(h, p_BQ);
  ...
  /** Calculates the signed distance from a query point Q to a box, G. The box
   ...
   @param h        The vector from the center of the box to the positive
                   corner of the box measured in frame B (the "half" size of
                   the box).
  ...
  template <int dim>
  std::tuple<Vector<T, dim>, Vector<T, dim>>
  BoxDistanceXd(const Vector<double, dim>& h, const Vector<T, dim>& p_GQ_G) {
    using std::abs;
    ...
    // Note: this quantity should only be used to define the extents of the
    // nearest feature (e.g., a face would be <0, 0, 1>, an edge could be
    // <-1, 1, 0>, and a vertex would be <1, -1, 1>. It serves merely as a
    // selector and not numerical values.
    Vector<T, dim> p_GC_G;
    VectorLoc location;
...
bool Callback(fcl::CollisionObjectd* object_A_ptr,
  ...
  const fcl::CollisionObjectd* geometry_object =
      (data.query_point == object_A_ptr) ?
      object_B_ptr : object_A_ptr;
  ...
  math::RigidTransform<T> typed_X_WG(data.X_WG[encoding.index()]);
  ...
  switch (collision_geometry->getNodeType()) {
    ...
    case fcl::GEOM_BOX:
      distance =
          distance_to_point(*static_cast<const fcl::Boxd*>(collision_geometry));
      break;

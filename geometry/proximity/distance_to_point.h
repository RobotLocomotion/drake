...
bool Callback(fcl::CollisionObjectd* object_A_ptr,
  ...
  //   distance and doesn't do any work if the distance between boxes is zero.
  //   3. We pass in a number smaller than the typical epsilon because we don't
  //   because typically computation tolerances are greater than or equal to
  //   epsilon() and we don't want this value to trip those tolerances. This is
  //   safe because the bounding box test in which this is calculates doesn't
  //   produce a code via calculation; it is a perfect, hard-coded zero.

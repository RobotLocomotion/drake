...
/* Definition of a deformable body's geometry at the reference configuration.
 ...
 2. This class calculates an approximate signed distance field instead of taking
    a prescribed pressure field. */
class ReferenceDeformableGeometry : public ShapeReifier {
 public:
  /* Constructs an empty reference deformable geometry. */
  ReferenceDeformableGeometry() {}

  /* Constructs a deformable geometry at reference configuration with the given
  `shape` and its spacial discretization `mesh`. An internal approximate signed
  ...
  /* Returns the approximate signed distance field to the surface of the
   deformable geometry in the reference configuration. */
...
class DeformableGeometry {
 public:
  ...
  /* Returns the approximate signed distance field for the deformable geometry.
   */
  const VolumeMeshFieldLinear<double, double>& signed_distance_field() const {

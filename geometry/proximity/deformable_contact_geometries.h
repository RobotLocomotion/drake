...
class RigidGeometry {
 public:
  ...
  explicit RigidGeometry(std::unique_ptr<internal::hydroelastic::CompliantMesh> mesh)
      : mesh_(std::move(mesh)) {}
...
std::optional<RigidGeometry> MakeMeshRepresentation(
  ...
  std::optional<internal::hydroelastic::CompliantGeometry> compliant_hydro_geometry =
  ...
  /* hydroelastic::CompliantGeometry is documented as having a mesh or having a half

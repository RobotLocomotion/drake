...
class DeformableGeometry {
 public:
  ...
  /* Constructs a deformable geometry. The deformable volume mesh of the new
   geometry is initialized to the given `mesh`, and the approximate signed
   distance field is interpolated from vertex values that store the signed
   distance (always non-positive) from the vertex to the surface of the volume
   mesh. */

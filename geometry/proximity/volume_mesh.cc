...
template <typename T>
bool VolumeMesh<T>::Equal(const VolumeMesh<T>& mesh, double tolerance) const {
  ...
  for (int i = 0; i < this->num_vertices(); ++i) {
    if ((this->vertex(i) - mesh.vertex(i)).norm() > tolerance) {
      return false;
    }
  }
  // Check inward face normals and edge vectors. If the vertices have an
  // error of O(tolerance) normals and edge vectors likewise have an error of
  // O(tolerance).
  for (int i = 0; i < this->num_elements(); ++i) {
    // Test normals.
    for (int j = 0; j < 4; ++j) {
      if ((this->inward_normal(i, j) - mesh.inward_normal(i, j)).norm() >
          tolerance) {
        return false;
      }
    }
    ...
    for (int j = 0; j < 4; ++j) {
      for (int k = j + 1; k < 4; ++k) {
        if ((this->edge_vector(i, j, k) - mesh.edge_vector(i, j, k)).norm() >
            tolerance) {
          return false;
        }
      }

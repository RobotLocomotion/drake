...
/** Definition of a half space. It is defined by the implicit equation
 `H(x⃗) = n̂⋅x⃗ - d <= 0`. A particular instance is defined in a particular the
 frame F such that `H(p_QF) > 0` if the point Q (measured and expressed in F)
...
/** Calculates the intersection point between an infinite straight line spanning
 ...
 @pre
     ...
     2. One of A and B is inside the half space (and the other is not).
...
//  check whether we can have other degenerate cases that we don't know.
/** Intersects a polygon with the half space H. It keeps the part of
 the polygon inside the half space (signed distance is negative).
 The `H_F` and vertex positions of `polygon_vertices_F` are both defined in
 a common frame F.
  ...
  for (int i = 0; i < size; ++i) {
    ...
    if (current_contained) {
      if (!previous_contained) {
        ...
        output_polygon.push_back(CalcIntersection(current, previous, H_F));
      }
...
std::vector<Vector3<T>> ClipTriangleByTetrahedron(
  ...
  for (auto& face_vertex : faces) {
    const Vector3<T>& p_MA = p_MV[face_vertex[0]];
    const Vector3<T>& p_MB = p_MV[face_vertex[1]];
    const Vector3<T>& p_MC = p_MV[face_vertex[2]];
...
// TODO(DamrongGuoy): Possibly change SampleVolumeFieldOnSurface to
...
/** Computes the contact surface between a soft geometry S and a rigid
 ...
 @return
     The contact surface between M and N. Geometries A and B map to M and N with

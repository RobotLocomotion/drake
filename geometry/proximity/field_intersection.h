...
/* %VolumeIntersector performs an intersection algorithm between two
 ...
 the two fields have equal values.
...
class VolumeIntersector {
 public:
  ...
  /* Creates the mesh and the scalar field of the contact surface between two
   tetrahedral meshes with scalar fields. The output surface mesh is posed in
   frame M of the first tetrahedral mesh.

   This function is used for hydroelastics when the scalar fields are
   pressure fields. It is also used for deformables when the scalar
   fields are signed distance fields.
   ...
   @param[out] surface_01_M  The output mesh of the contact surface between
                        the two geometries expressed in frame M.
                        The surface normal is in the direction of
                        increasing field_0 and decreasing field_1.
  ...
  /* Returns the index of tetrahedron in the first mesh that contains
   the i-th contact polygon.
   @pre IntersectFields() was called already.  */
  int tet0_of_polygon(int i) const { return tet0_of_contact_polygon_[i]; }
  /* Returns the index of tetrahedron in the second mesh that contains
   the i-th contact polygon.
   @pre IntersectFields() was called already.  */
  ...
  /* Computes the contact surface between two compliant hydroelastic
   geometries. The output contact surface is posed in World frame.

   This function is a helper of ComputeContactSurfaceFromCompliantVolumes(),
   which is the main API of this file.

...
class VolumeMesh {
  ...
  */
  ...
  /** Checks to see whether the given VolumeMesh object is equal via deep
   ...
   standard. The `tolerance` is applied to corresponding vertex positions,
   inward face normals, and edge vectors.
   @param mesh              The mesh for comparison.
   @param tolerance         The maximum distance allowed between two
                            corresponding vertices. Derived quantities are
                            compared with a tolerance based on the expected
                            vertex error.
   @returns `true` if the given mesh is equal.
   */
  bool Equal(const VolumeMesh<T>& mesh, double tolerance = 0) const;

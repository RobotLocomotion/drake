...
/** Given the six vertices that form the interior octohedron, decomposes the
 octohedron into the "best" set of four vertices. The vertices are encoded
 by six vertex indices into the collection of vertex positions. The resulting
 tetrahedra are appended to `test`.
 ...
 @param[out] tets           The collection of tetrahedra; the best four
...
void SplitOctohedron(const std::array<VolumeVertexIndex, 6>& vertex_indices,
  ...
  std::unordered_map<SortedPair<VolumeVertexIndex>, double> sqr_edge_len;
  auto store_sqr_edge_len = [&sqr_edge_len, &p_MVs](VIndex a, VIndex b) {
    sqr_edge_len[SortedPair<VIndex>(a, b)] =
        ExtractDoubleOrThrow((p_MVs[a].r_MV() - p_MVs[b].r_MV()).squaredNorm());
  };
  for (int i = 0; i < 6; ++i) {
    for (int j = i + 1; j < 6; ++j) {
      store_sqr_edge_len(vertex_indices[i], vertex_indices[j]);
    }
  }
  ...
  // tetrahedron indices (i.e., in the range [0, 5]).
  auto tet_quality = [&sqr_edge_len, &volume,
    ...
    double denom = 0.0;
    denom += sqr_edge_len.at(SortedPair<VIndex>(a, b));
    denom += sqr_edge_len.at(SortedPair<VIndex>(a, c));
    denom += sqr_edge_len.at(SortedPair<VIndex>(a, d));
    denom += sqr_edge_len.at(SortedPair<VIndex>(b, c));
    denom += sqr_edge_len.at(SortedPair<VIndex>(b, d));
    denom += sqr_edge_len.at(SortedPair<VIndex>(c, d));
  ...
  // Find minimum optimal direction.
  const double gh_cost = split_quality(kGhTets);
  const double fi_cost = split_quality(kFiTets);
  const double ej_cost = split_quality(kEjTets);

  // Encode split axis as GH, FI, and EJ as 0, 1, and 2, respectively.
  int split_axis = 0;
  double min_cost = gh_cost;
  if (fi_cost < min_cost) {
    split_axis = 1;
    min_cost = fi_cost;
  }
  ...
  auto add_tets = [&vertex_indices,
                   tets](const std::array<std::array<int, 4>, 4>& local_tets) {
    for (int t = 0; t < 4; ++t) {
      tets->emplace_back(
  ...
  // Add tetrahedra from optimal split.
  if (split_axis == 0) {
    add_tets(kGhTets);
...
std::vector<VolumeElement> SplitTetrahedron(
    ...
    std::unordered_map<SortedPair<VolumeVertexIndex>, VolumeVertexIndex>&
        edge_map,
    const std::vector<VolumeVertex<T>>& p_MVs) {
  using KeyType = SortedPair<VolumeVertexIndex>;
  std::vector<VolumeElement> tets;
  tets.reserve(8);
  ...
  const VolumeVertexIndex e(edge_map.at(KeyType(a, b)));
  const VolumeVertexIndex f(edge_map.at(KeyType(a, c)));
  const VolumeVertexIndex g(edge_map.at(KeyType(a, d)));
  const VolumeVertexIndex h(edge_map.at(KeyType(b, c)));
  const VolumeVertexIndex i(edge_map.at(KeyType(b, d)));
  const VolumeVertexIndex j(edge_map.at(KeyType(c, d)));
...
/** Creates a finer volume mesh approximating a unit sphere. for refining a
 volume mesh that represents a unit sphere (not generally applicable to refining
 other meshes). The refinement is achieved by decomposing every tetrahedron into
 eight new tetrahedron which encompass the same volume. The new tetrahedron are
 formed by splitting the six edges of the original tetrahedron in half (adding
 six new vertices), and building eight new tetrahedra topology in that region.

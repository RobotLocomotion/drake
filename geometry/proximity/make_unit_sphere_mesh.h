...
/** Creates a volume mesh for the given sphere `s` with the given characteristic
 `edge_length`.
 ...
 @param edge_length     The characteristic edge length for the sphere.
...
VolumeMesh<T> MakeSphereVolumeMesh(const Sphere& sphere, double edge_length) {
  ...
  VolumeMesh<T> unit_mesh = MakeUnitSphereMesh<T>(L);
  std::vector<VolumeVertex<T>> vertices;

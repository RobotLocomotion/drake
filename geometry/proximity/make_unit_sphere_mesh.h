...
void SplitOctohedron(const std::array<VolumeVertexIndex, 6>& vertex_indices,
  ...
  std::unordered_map<SortedPair<VIndex>, double> sqr_edge_len;
  ...
  const double gh_quality = split_quality(kGhTets);
  const double fi_quality = split_quality(kFiTets);
  const double ej_quality = split_quality(kEjTets);

  // Encode split axis as GH, FI, and EJ as 0, 1, and 2, respectively.
  int split_axis = 0;
  double max_quality = gh_quality;
  if (fi_quality > max_quality) {
    split_axis = 1;
    max_quality = fi_quality;
  }
  if (ej_quality > max_quality) {
    split_axis = 2;
  }

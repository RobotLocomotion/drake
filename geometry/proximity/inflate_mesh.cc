...
InflateProgram::InflateProgram(VolumeMesh<double> mesh, double margin)
    : mesh_{std::move(mesh)}, margin_(margin) {
...
VolumeMesh<double> InflateProgram::Solve() const {
  ...
  if (!result.is_success()) {
    throw std::runtime_error(
        ...
        "apply margins to non-convex meshes is guaranteed succeed. You might "
        "also want to check your volume mesh is not somehow degenerate. "

...
/* Generates a tetrahedral volume mesh approximating a given capsule by medial
 ...
 The capsule is parameterized by two parameters (radius, length). It's length
 ...
 The output mesh will have these properties:
   ...
   3. Let n be ⌈2πr/h⌉, where r = cylinder.radius(), and h is `resolution_hint`,
      i.e., n is the number of vertices per one circular rim of the capsule.

 @param[in] capsule The capsule shape specification.
...
//    `resolution_hint` greater than √2 times the radius of the capsule.
//    For any capsule, there is a `resolution_hint` value that serves as the

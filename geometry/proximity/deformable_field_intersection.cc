...
void AddDeformableDeformableContactSurface(
  ...
  std::vector<double> penetration_distances;
  penetration_distances.reserve(num_faces);
  for (int f = 0; f < num_faces; ++f) {
    penetration_distances.push_back(contact_field_W->EvaluateCartesian(
  ...
  deformable_contact->AddDeformableDeformableContactSurface(
      ...
      std::move(penetration_distances), std::move(contact_vertex_indices[1]),

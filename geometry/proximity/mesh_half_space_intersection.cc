    ...
    for (SurfaceVertexIndex i0(0); i0 < 3; ++i0) {
      if (s[i0] <= 0) {
        ...
        const SurfaceVertexIndex i0_new_index = GetVertexAddIfNeeded(
            vertices_F, i0, X_WF, vertices_to_newly_created_vertices,
            new_vertices_W);

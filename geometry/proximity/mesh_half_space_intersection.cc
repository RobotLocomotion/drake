...
ComputeContactSurfaceFromSoftHalfSpaceRigidMesh(
  ...
  // N.B. ComputeContactSurface() below does not use calc_pressure_R() to
  // compute the zero pressure level set, it uses the signed distance to hs_R
  // (it assumes the pressure is linear with distance). calc_pressure_R() is
  // only used to compute pressure values at the contact surface. We exploit
  // this to pass the "right" pressure that includes margin, without affecting

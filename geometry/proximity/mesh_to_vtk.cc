...
void vtk_write_unstructured_grid(std::ofstream& out,
  ...
  for (typename Mesh::VertexIndex i(0); i < numPoints; ++i) {
  ...
  const int cell_type = (Mesh::kDim == 2) ? 5 : 10;
 ...
 */
template<typename Mesh>
void MeshToVtk(const std::string& file_name,
               const Mesh& mesh,
 ...
 */
template <typename Field>
void MeshFieldLinearToVtk(const std::string& file_name,
                          const Field& field,
                          const std::string& title) {
  std::ofstream file(file_name);
  vtk_write_header(file, title);
  vtk_write_unstructured_grid(file, field.mesh());
  vtk_write_scalar_field(file, field.name(), field);
  file.close();
}
...
void VolumeMeshToVtk(const std::string& file_name,
                     ...
                     const std::string& title) {
  MeshToVtk(file_name, mesh, title);
}
...
void VolumeMeshFieldLinearToVtk(
    ...
    const std::string& title) {
  MeshFieldLinearToVtk(file_name, field, title);
}

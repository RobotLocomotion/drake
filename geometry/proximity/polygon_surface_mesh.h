...
class SurfacePolygon {
 public:
  ...
  int vertex(int i) const {
    return mesh_face_data_.at(index_ + 1 + i);
  }
 ...
 private:
  ...
  /* Constructs a SurfacePolygon.
   @param face_data Indices of the polygonal face in counter-clockwise order.
    */
...
class PolygonSurfaceMesh {
 public:
  ...
  /** %PolygonSurfaceMesh doesn't actually support barycentric coordinates. This
   ...
   to be zero to minimize memory footprint in case one is inadvertantly
   instantiated. */
  ...
  /** Constructs a mesh from specified vertex and mesh data.
   ...
   which _encodes_ M polygons. Each polygon can have an arbitrary number of
   vertices. The encoding of the M polygons is as follows:

       |c₁|v₁₀|v₁₁|...|cᵢ|cᵢ₀|cᵢ₁|...|cₘ|cₘ₀|cₘ₁|...|

   Each polygon is defined in sequence. The definition consists of an integer
   indcating the *number* of vertices in that polygon (c₁, cᵢ, and cₘ in the
   ...
   @param vertices   The vertex positions, measured and exprssed in this
                     mesh's frame.
  ...
  const T& area(int f) const {
    DRAKE_DEMAND(0 <= f && f < num_faces()); return areas_[f]; }
  ...
  /** Calculates the gradient ∇u of a linear field u on the polygon `p`
   (measured in the mesh's frame M). Field u is defined by three field values
   `field_value[i]` at the i-th vertex of the polygon -- we are inferring the
   gradient over the whole polygon by examining only the first three vertices.
   As long as:
   
     - the polygon is planar and
     - the vertices are not co-linear

   this is an acceptable calculation.
   
   This exists to suport the MeshFieldLinear interface. */
  template <typename FieldValue>
  Vector3<FieldValue> CalcGradientVectorOfLinearField(
      const std::array<FieldValue, 3>& field_value, int p) const {
      const auto& poly = element(p);
      const Vector3<T>& p_MA = vertices_M_[poly.vertex(0)];
      const Vector3<T>& p_MB = vertices_M_[poly.vertex(1)];
      const Vector3<T>& p_MC = vertices_M_[poly.vertex(2)];
      return field_value[0] * CalcGradBarycentric(p_MA, p_MB, p_MC) +
             field_value[1] * CalcGradBarycentric(p_MB, p_MC, p_MA) +
             field_value[2] * CalcGradBarycentric(p_MC, p_MA, p_MB);
  }
 ...
 private:
  ...
  /* Calculates the spatial gradient of the barycentric function related to
   vertex V in the triangle VAB. See triangle_surface_mesh.h for more details.

   This code is stolen wholesale from triangle_surface_mesh.h. We should
   probably reconcile the two instances. */
  Vector3<T> CalcGradBarycentric(const Vector3<T>& p_MV, const Vector3<T>& p_MA,

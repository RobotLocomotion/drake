...
// TODO(SeanCurtis-TRI): When we do compliant-compliant contact, we'll need ∇p̃(e) as well.
//  ∇p̃(e) is piecewise constant -- one ℜ³ vector per tetrahedron.
/* Defines a compliant mesh -- a volume mesh, its linearized pressure field, p̃(e),
and derived objects:
...
class CompliantMesh {
 public:
  ...
  CompliantMesh(std::unique_ptr<VolumeMesh<double>> mesh,
...
/* Defines a compliant half space. The half space is defined such that the half
 ...
 The pressure in the compliant layer may be modeled in many ways. Currently, we use
...
class CompliantGeometry {
 public:
  ...
  // TODO(SeanCurtis-TRI): remove all of these legacy API wrappers for CompliantMesh
  // and CompliantHalfSpace.
  //@{
  ...
  const CompliantMesh& compliant_mesh() const {
    if (is_half_space()) {
      throw std::runtime_error(
          "CompliantGeometry::compliant_mesh() cannot be invoked for compliant half space.");
    }
  ...
  const CompliantHalfSpace& compliant_half_space() const {
    if (!is_half_space()) {
      throw std::runtime_error(
          "CompliantGeometry::compliant_half_space() cannot be invoked for compliant mesh.");
    }
  ...
  const VolumeMesh<double>& mesh() const {
    if (is_half_space()) {
      throw std::runtime_error(
          "CompliantGeometry::mesh() cannot be invoked for compliant half space");
    }
  ...
  const Bvh<Obb, VolumeMesh<double>>& bvh() const {
    if (is_half_space()) {
      throw std::runtime_error(
          "CompliantGeometry::bvh() cannot be invoked for compliant half space");
    }
  ...
  double pressure_scale() const {
    if (!is_half_space()) {
      throw std::runtime_error(
          "CompliantGeometry::pressure_scale() cannot be invoked for compliant mesh");
    }
  ...
  /* Returns the half space's margin -- calling this will throw if
   ...
   For CompliantMesh instances, the margin is already part of the representative mesh
  ...
  /* Reports the hydroelastic representation type for the given id. The
   following invariants should always hold:
     ...
     - If HydroelasticType::kRigid is returned, there is a rigid geometry
       ...
       valid RigidGeometry.
     - If HydroelasticType::kCompliant is returned, there is a compliant geometry
       associated with that id and calling compliant_geometry() will return a valid
       CompliantGeometry.  */
...
/* Generic interface for handling unsupported compliant Shapes. Unsupported
 geometries will return a std::nullopt.  */
template <typename Shape>
std::optional<CompliantGeometry> MakeCompliantRepresentation(const Shape& shape,
...
/* Creates a compliant cylinder (assuming the proximity properties have sufficient
 information). Requires the ('hydroelastic', 'resolution_hint') and
 ('hydroelastic', 'hydroelastic_modulus') properties.  */
std::optional<CompliantGeometry> MakeCompliantRepresentation(
    const Cylinder& cylinder, const ProximityProperties& props);

/* Creates a compliant capsule (assuming the proximity properties have sufficient
 information). Requires the ('hydroelastic', 'resolution_hint') and
 ('hydroelastic', 'hydroelastic_modulus') properties.  */
std::optional<CompliantGeometry> MakeCompliantRepresentation(
    const Capsule& capsule, const ProximityProperties& props);

/* Creates a compliant ellipsoid (assuming the proximity properties have sufficient
 information). Requires the ('hydroelastic', 'resolution_hint') and
 ('hydroelastic', 'hydroelastic_modulus') properties.  */
std::optional<CompliantGeometry> MakeCompliantRepresentation(
    const Ellipsoid& ellipsoid, const ProximityProperties& props);

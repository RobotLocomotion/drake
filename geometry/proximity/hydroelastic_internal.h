...
/** Defines a soft mesh -- a mesh, its linearized pressure field, p̃(e), and its
 ...
 the pressure field and the bounding volume hierarchy just referencing it. */
struct SoftMesh {
  ...
  SoftMesh(std::unique_ptr<VolumeMesh<double>> mesh_,
...
struct RigidMesh {
  ...
  explicit RigidMesh(std::unique_ptr<SurfaceMesh<double>> mesh_)
      : mesh(std::move(mesh_)),

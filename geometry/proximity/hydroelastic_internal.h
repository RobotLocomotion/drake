...
class Geometries final : public ShapeReifier {
 public:
  ...
  /** Reports the hydroelastic representation type for the given id. There
   should always be an invariant:
  ...
  /** Returns the representation of the soft geometry with the given `id`.
   @pre type(id) returns HydroelasticType::kSoft.  */
  ...
  /** Returns the representation of the rigid geometry with the given `id`.
   @pre type(id) returns HydroelasticType::kRigid.  */
 ...
 private:
  // Data to be used during reification.
  struct ReifyData {
...
/** Generic interface for handling unsupported soft Shapes. Unsupported
 geometries will return a nullopt.  */
template <typename Shape>

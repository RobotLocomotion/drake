...
VolumeMesh<T> MakeCapsuleVolumeMesh(const Capsule& capsule,
  ...
  // Z coordinate of the poles of the caps.
  ...
  const int num_vertices_per_circle = std::max(
      3,
      static_cast<int>(ceil(2. * M_PI * capsule.radius() / resolution_hint)));

  // Each vertex on the rim correspond to a line of longitude on the
  // hemispheres of the caps. num_circles_per_cap is how many lines of longitude
  // to subdivide each cap into.
  const int num_circles_per_cap = (num_vertices_per_circle - 1) / 2;
  ...
  for (int i = 0; i < num_circles_per_cap; ++i) {
    ...
    for (int j = 0; j < num_vertices_per_circle; ++j) {
      ...
      const double x = capsule.radius() * sin(theta) * cos(phi);
      const double y = capsule.radius() * sin(theta) * sin(phi);
  ...
  // of those wedges connect to the corresponding medial vertex, forming a
  // single tetrahedra.
  //

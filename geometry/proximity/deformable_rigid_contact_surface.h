...
class DeformableRigidContactSurface {
 public:
  ...
  /* Constructs a DeformableRigidContactSurface with no data. */
  DeformableRigidContactSurface() = default;
  ...
  /* Creates a contact surface between the deformable geometry with
   ...
   @note Most variables are passed by r-value reference (as opposed to value) to
   facilitate efficient construction from local variables. */
  DeformableRigidContactSurface(PolygonSurfaceMesh<T>&& contact_mesh_W,
  ...
  /* Releases all the per-contact-point data stored in this
   DeformableRigidContactSurface: the contact mesh expressed in the world frame,
   the penetration distances, the indexes of the tetrahedron containing the
   contact points, the barycentric coordinates of the contact points, and the
   inverses of the bases for the per-point contact frames expressed in the world
   frame. After this is called, DeformableRigidContactSurface gives up ownership
   of the data, and should therefore be discarded.
   @pre has_data() == true. */
  std::tuple<PolygonSurfaceMesh<T>, std::vector<T>, std::vector<int>,
             std::vector<Vector4<T>>, std::vector<math::RotationMatrix<T>>>
  release_data();

  /* Returns true if this DeformableRigidContactSurface is not default
   constructed and has not been called `release_data()` on. */
 ...
 private:
  /* Flag to indicate whether the DeformableRigidContactSurface still holds any
   data. */

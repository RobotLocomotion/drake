...
class CollisionFilter {
 public:
  ...
  /* Applies the collision filter declaration to the persistent state. The
   ...
   given `declaration`. The persistent configuration can only be modified if
   there are no transient declarations active.
  ...
  bool has_active_history() const { return filter_history_.size() > 1; }
 ...
 private:
  ...
  static void AddGeometry(GeometryId id, FilterState* filter_state_ptr,
                          PairFilterState initial_state);

  /* Removes the geometry with the given `id` from the given filter state. */
  static void RemoveGeometry(GeometryId id, FilterState* filter_state_ptr);
  ...
  /* Instantiates a FilterState such that all known pairs have `default_state`
   relationships. */
  static FilterState InitializeTransientState(const FilterState& reference,
                                              PairFilterState default_state);

  /* The filter state of all pairs of geometry.

   This is neither the persistent base configuration nor the history. It is
   the cached result of the base with all transient declarations applied. */
  FilterState filter_state_;

  /* The underlying data for transient history. A filter state which represents
   the resultant state from a transient declaration and its associated id. */
  struct StateDelta {

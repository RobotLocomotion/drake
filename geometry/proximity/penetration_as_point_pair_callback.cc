...
class PenetrationPairGeometry {
 public:
  ...
  PenetrationPairGeometry(const GeometryId& id_A, const GeometryId& id_B,
  ...
  void operator()(const fcl::Sphered& sphere_A, const fcl::Sphered& sphere_B) {
    SphereShapePenetration(sphere_A, sphere_B);
  }

  void operator()(const fcl::Sphered& sphere_A, const fcl::Boxd& box_B) {
    SphereShapePenetration(sphere_A, box_B);
  }

  void operator()(const fcl::Sphered& sphere_A,
                  const fcl::Cylinderd& cylinder_B) {
    SphereShapePenetration(sphere_A, cylinder_B);
  }

  void operator()(const fcl::Sphered& sphere_A,
                  const fcl::Halfspaced& halfspace_B) {
    SphereShapePenetration(sphere_A, halfspace_B);
  }

  void operator()(const fcl::Sphered& sphere_A,
                  const fcl::Capsuled& capsule_B) {
    SphereShapePenetration(sphere_A, capsule_B);
  }
 ...
 private:
  ...
  void SphereShapePenetration(const fcl::Sphered& sphere_A,
    ...
    result_->id_A = id_A_;
    result_->id_B = id_B_;
    result_->depth = sphere_A.radius - shape_B_to_point_Ao.distance;
    // p_WCb is the witness point on ∂B measured and expressed in W.
    result_->p_WCb = X_WB_ * (shape_B_to_point_Ao.p_GN);
...
void CalcDistanceFallback(const fcl::CollisionObjectd& a,
  ...
  throw std::logic_error(fmt::format(
      "Collision queries between shapes '{}' and '{}' "
...
void CalcDistanceFallback<double>(const fcl::CollisionObjectd& a,
  ...
  Vector3<double> drake_normal = -contact.normal;
  ...
  // out of A and into B.
  Eigen::Vector3d p_WAc = contact.pos - 0.5 * depth * drake_normal;
  Eigen::Vector3d p_WBc = contact.pos + 0.5 * depth * drake_normal;
  ...
  pair_data->p_WCa = p_WAc;
  pair_data->p_WCb = p_WBc;
  pair_data->nhat_BA_W = drake_normal;
}
...
void ComputeNarrowPhasePenetration(const fcl::CollisionObjectd& a,
  ...
  PenetrationPairGeometry<T> penetration_pair(id_S, id_O, X_WS, X_WO, result);
...
bool Callback(fcl::CollisionObjectd* fcl_object_A_ptr,
  ...
  if (ScalarSupport<T>::is_supported(
    ...
    ComputeNarrowPhasePenetration(fcl_object_A, data.X_WGs.at(id_A),

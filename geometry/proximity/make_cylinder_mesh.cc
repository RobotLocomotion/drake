...
VolumeMesh<T> MakeCylinderVolumeMeshWithMa(const Cylinder& cylinder,
  ...
  // TODO(DamrongGuoy): Instead of one long function to handle all three
  //  classes of cylinders, consider breaking it into subroutines. One
  //  subroutine handles one class of cylinders.
  ...
  // For a long cylinder, these two mesh vertices are at the two end points of
  // the central medial line segment. For a short cylinder, these two virtual
  ...
  for (int i = 0; i < num_vertices_per_circle; ++i) {
    switch (cylinder_class) {
      ...
      case CylinderClass::kShort:
        const double x =
            ExtractDoubleOrThrow(mesh_vertices[v[0][i]].r_MV().x()) *
            scale_cylinder_radius_to_offset_surface;
        const double y =
            ExtractDoubleOrThrow(mesh_vertices[v[0][i]].r_MV().y()) *
            scale_cylinder_radius_to_offset_surface;

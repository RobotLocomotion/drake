...
struct CallbackData {
  ...
  std::unordered_map<GeometryId, internal::InternalGeometry> geometries;
...
SoftGeometry MakeSphereFromFcl(fcl::CollisionObjectd* object_ptr,
  ...
  // Default pressure field is linear in extent with unit scale.
  PressureField static_pressure;
  static_pressure.p0 = [](double e) { return e; };
  static_pressure.dp0_de = [](double) { return 1; };
  static_pressure =
      properties.GetPropertyOrDefault(kHydroGroup, kPressure, static_pressure);
  ...
  for (const auto& v : geometry.mesh->vertices()) {
    ...
    p0_values.push_back(static_pressure.p0(1.0 - p_MV_len / r));
  }

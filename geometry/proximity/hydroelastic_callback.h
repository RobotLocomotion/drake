...
bool Callback(fcl::CollisionObjectd* object_A_ptr,
  ...
  if (can_collide) {
    if (ScalarSupport<T>::is_supported(
      ...
      const fcl::NODE_TYPE object_A_type =
          object_A_ptr->collisionGeometry()->getNodeType();
      const fcl::NODE_TYPE object_B_type =
          object_B_ptr->collisionGeometry()->getNodeType();
      DRAKE_DEMAND((object_A_type == fcl::GEOM_SPHERE &&
                    object_B_type == fcl::GEOM_HALFSPACE) ||
                   (object_A_type == fcl::GEOM_HALFSPACE &&
                    object_B_type == fcl::GEOM_SPHERE));

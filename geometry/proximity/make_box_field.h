...
/**
 ...
 We require the input parameter `pressure_function` f(e) that maps
 extent e ∈ [0,1] to pressure p = f(e) ∈ [0, ∞). Internally we generate the
 extent field e:B→[0, 1] based on the distance to boundary with minimum
 value 0.0 on the boundary and maximum value 1.0 in the interior. Then, we
 return the pressure field on the box, p:B→[0, ∞) that is zero on the boundary.
 @param box    The box B.
 @param mesh  A tetrahedral mesh of the box B.
 @param pressure_function   The user-defined map from extent to pressure.
 @return   The pressure field defined on the tetrahedral mesh.
 @pre   The `pressure_function` f(e) preserves zero, i.e., f(0)=0.
 */
template <typename T>
VolumeMeshFieldLinear<T, T> MakeBoxPressureField(
    const Box& box, const VolumeMesh<T>& mesh,
    const std::function<T(T)>& pressure_function) {
  DRAKE_DEMAND(pressure_function(T(0)) == T(0));
  ...
  for (const VolumeVertex<T>& vertex : mesh.vertices()) {
    ...
    pressure_values.push_back(pressure_function(extent));
  }

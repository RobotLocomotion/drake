...
DeformableContact<double> Geometries::ComputeDeformableContact(
  ...
  // (deformable_j, deformable_i) or (deformalbe_i, deformable_i).
  for (auto it0 = deformable_geometries_.begin();
    ...
    for (auto it1 = std::next(it0, 1); it1 != deformable_geometries_.end();
         ++it1) {

...
void VerifyInvariantsOfThePressureFieldWithMargin(
  ...
  DRAKE_DEMAND(&field_no_magin.mesh() == &field_with_margin.mesh());
  ...
  const double tolerance = relative_tolerance * std::abs(boundary_pressure);
  ...
  EXPECT_NEAR(min_slope, max_slope, relative_tolerance);
}

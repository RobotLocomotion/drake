...
/* Axis-aligned bounding box used in DeformableBvh. The box is
 ...
 H. Because this is an _axis-aligned_ bounding box, `R_HB = I`. Therefore the
...
class Aabb {
  ...
  */
  ...
  /* The point on the axis-aligned box with the smallest measures along the x-,
   y-, and z-directions. */
  Vector3<double> lower() const { return center_ - half_width_; }

  /* The point on the axis-aligned box with the largest measures along the x-,
   y-, and z-directions. */
  ...
  /* Checks whether bounding volume `bv` intersects the given oriented bounding
   box. The bounding volume is centered on its canonical frame B and B is posed
   in the corresponding hierarchy frame H; by construction B is aligned with H.
   The obb is defined in frame O.

   @param aabb      The axis-aligned box to test.
   @param obb_O     The oriented box to test.
   @param X_OH      The relative pose between the hierarchy frame H and the
                    obb frame O.
   @returns `true` if the boxes intersect.   */
  static bool HasOverlap(const Aabb& a_G, const Obb& b_H,
                         const math::RigidTransformd& X_GH);

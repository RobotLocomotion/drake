  ...
  for (int index : indices) {
    if (index < 0 || index >= v_count) {
      throw std::logic_error(fmt::format(
          "Adding a polygon to a ContactSurface mesh builder requires that the "
          "polygon vertex indices refer to vertices that have already been "
...
int TriMeshBuilder<T>::AddPolygon(
  ...
  DRAKE_ASSERT_VOID(ThrowForInvalidVertexIndex(polygon_vertices, vertices_B_));

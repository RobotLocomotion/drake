...
void ThrowIfInvalidForCentroid(const char* prefix,
  ...
  // We've documented "true" orthogonality to make the caller care. In practice,
  // as long as we're well aware from co-planar, the answer is fine. So, we'll
  // test it against a large cone (with an approximately 90-degree angle). We
  // can always tighten it later, if necessary.
...
// Instantiations for the public API -- algorithms that create contact surfaces

...
void AddPolygonToMeshDataAsOneTriangle(
  ...
  // We use this simple check because we expect this function to go away when
  // we implement the final support for discrete hydroelastics.
  if (polygon_area < 1e-13) {
    return;
  }
  ...
  if (polygon_F.size() == 3) {
    ...
    // frame F, is the same as the face normal vector nhat_F. The three vertices
    // of the triangle are at the origin Go, on Gx axis, and on Gy axis, as

...
VolumeMeshFieldLinear<T, T> MakeVolumeMeshPressureField(
  ...
  //  initializing pressure_values to zero's and skip the computation for
  //  boundary vertices.
  ...
  for (const Vector3<T>& p_MV : mesh_M->vertices()) {
    ...
    pressure_values.emplace_back(
      internal::CalcDistanceToSurfaceMesh(p_MV_d, surface_d));
  }
  const T max_value =
      *std::max_element(pressure_values.begin(), pressure_values.end());
  if (max_value <= T(0)) {
    throw std::runtime_error("MakeVolumeMeshPressureField: the computed max "

...
class Plane {
 public:
  ...
  /** Constructs a Plane in frame F which is normal to `nhat_F` and passes
   through the point `p_FP`.
   @param nhat_F
       A unit-length vector perpendicular to the plane expressed in Frame F (the
       `n̂` in the implicit equation).
   @param p_FP
  ...
  Plane(const Vector3<T>& nhat_F, const Vector3<T>& p_FP)
    ...
    DRAKE_THROW_UNLESS(abs(nhat_F_.norm() - 1.0) <= kEps);
  }

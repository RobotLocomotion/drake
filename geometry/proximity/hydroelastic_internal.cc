...
// invalid values. When using _canonical_ proximity properties, we assume that
// type and value has been validated prior to be stored in the properties.
class Validator {
 public:
    ...
    if (!props.HasProperty(property)) {
      throw std::logic_error(
          fmt::format("Cannot create {} {}; missing the {} property",
                      compliance_, shape_name_, property));
    }

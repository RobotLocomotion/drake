...
std::optional<SoftGeometry> MakeSoftRepresentation(
  ...
  // We inflate the ellipsoid so that the zero pressure level set recovers the
  // original ellipsoid dimensions. To this end we use our knowledge documented
  // in MakeEllipsoidPressureField().
  ...
  const double inflation_factor = 1.0 / (1.0 - margin / H);
  const double a_inflated = inflation_factor * ellipsoid.a();
  const double b_inflated = inflation_factor * ellipsoid.b();
  const double c_inflated = inflation_factor * ellipsoid.c();
  const Ellipsoid inflated_ellipsoid(a_inflated, b_inflated, c_inflated);
  auto inflated_mesh =

...
class Validator {
 public:
  ...
  const ValueType& Extract(const ProximityProperties& props,
    ...
    const auto& value = props.Get<ValueType>(property);
 ...
 protected:
  ...
  // validate; the second is the property in question.
  virtual void ValidateValue(const ValueType&,
...
class PositiveDouble : public Validator<double> {
 ...
 protected:
  void ValidateValue(const double& value,
                     const PropertyName& property) const override {
    if (value <= 0) {
      throw std::logic_error(

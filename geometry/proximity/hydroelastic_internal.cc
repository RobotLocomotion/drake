...
// "rigid" or "compliant") strings (to help give intelligible error messages) and
...
void WarnNoCompliantRepresentation(std::string_view shape_type_name) {
  static const logging::Warn log_once(
      "Compliant {} shapes are not currently supported for hydroelastic contact; "
...
std::optional<CompliantGeometry> MakeCompliantRepresentation(
  ...
  return CompliantGeometry(CompliantMesh(std::move(inflated_mesh), std::move(pressure)));
}
...
std::optional<CompliantGeometry> MakeCompliantRepresentation(
  ...
  return CompliantGeometry(CompliantHalfSpace{hydroelastic_modulus / thickness, margin});
}
...
std::optional<CompliantGeometry> MakeCompliantRepresentation(
  ...
  const double hydroelastic_modulus =
      PositiveDouble("Convex", "compliant").Extract(props, kHydroGroup, kElastic);

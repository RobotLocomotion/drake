...
std::optional<SoftGeometry> MakeSoftRepresentation(
  ...
  std::unique_ptr<VolumeMesh<double>> maybe_inflated_mesh;
  ...
  if (mesh_specification.extension() == ".vtk") {
    ...
    maybe_inflated_mesh = make_unique<VolumeMesh<double>>(
  ...
  pressure = make_unique<VolumeMeshFieldLinear<double, double>>(
      MakeVolumeMeshPressureField(maybe_inflated_mesh.get(),
                                  hydroelastic_modulus, margin));

  return SoftGeometry(
      SoftMesh(std::move(maybe_inflated_mesh), std::move(pressure)));
}

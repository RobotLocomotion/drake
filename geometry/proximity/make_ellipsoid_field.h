...
/*
 ...
 p(x)= E⋅e(x) where e(x) = 1−‖D½⋅x‖, with the diagonal matrix D = diag({a-δ,
 b-δ, c-δ}) where a, b, c are the lengths of the principal semi-axes of
 `ellipsoid`, and δ is the `margin` value. The zero pressure level set of this
 field defines an ellipsoid of principal semi-axes lengths a-δ, b-δ, c-δ.
...
VolumeMeshFieldLinear<T, T> MakeEllipsoidPressureField(
  ...
  for (const Vector3<T>& r_EV : mesh_E->vertices()) {
    ...
    if (margin == 0 && extent < kExtentEpsilon) {
      extent = T(0.0);
    }

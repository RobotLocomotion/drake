...
/**
 ...
 (triangular or tetrahedral) mesh; the field value changes linearly within
 each element E (triangle or tetrahedron), and the gradient ∇f is constant
 within each element. The field is continuous across adjacent elements, but
 its gradient is discontinuous from one element to the other.
 ...
 At a point Q in element E expressed as p_MQ in frame M of the mesh, the
 piecewise linear field f has value:

       f(Q) = ∇fᵉ⋅p_MQ + fᵉ(Mo),

 where ∇fᵉ is expressed in frame M, and Mo is the origin of frame M. Notice
 that Mo may or may not lie in element E.

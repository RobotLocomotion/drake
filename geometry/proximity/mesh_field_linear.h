...
class MeshFieldLinear final : public MeshField<T, MeshType> {
 public:
  ...
  // TODO(DamrongGuoy): Update documentation.
  //  - Constructing mesh field linear with gradient:
  //    -- Longer construction time (as the gradients will be calculated).
  //    -- May throw if the mesh is of "poor" quality -- and some definition of
  //       "poor quality" should be given so that the caller can be empowered
  //       to make that decision.
  //    -- Cheap evaluation of the field at arbitrary Cartesian coordinates.
  //    -- Per-element gradient values available (cheap).
  //  - Constructing mesh field linear without gradient:
  //    -- Faster construction, without sensitivity on mesh quality.
  //    -- Slow evaluation of field from cartesian coordinates.
  //    -- No gradients available.
  //  - Things that don't depend on the choice:
  //    -- Evaluating the field at a vertex.
  //    -- Evaluating the field at a user-given barycentric coordinate.
  //    -- Evaluating the field on an element outside that element is a valid
  //       and meaningful operation.
  //    -- To evaluate the field at all for a point Q, some element must always
  //       be selected and the interpretation of the result will depend on
  //       whether Q lies inside that element or not.

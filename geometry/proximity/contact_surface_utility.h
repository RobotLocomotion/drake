...
/** Given an N-sided _convex_ `polygon`, computes its center of mass. The
 `polygon` is represented as an ordered list of indices into the given set of
 `vertices_F`. The resulting center of mass will be measured and expressed in
 the same Frame F as the provided vertices.
 ...
 @param vertices_F
     ...
     expressed in Frame F.
 @retval p_FC, the position of the polygons center of mass C, measured and
         expressed in Frame F.
 @pre `polygon.size()` >= 3.
 @pre `nhat_F` is truly perpendicular to the defined `polygon`'s plane.
 */
template <typename T>
Vector3<T> CalcPolygonCenterOfMass(
    const std::vector<SurfaceVertexIndex>& polygon,
    const Vector3<T>& nhat_F,
    const std::vector<SurfaceVertex<T>>& vertices_F) {
  // The position of the center of mass (or geometric centroid) can be computed
  // by decomposing the polygon into triangles and performing an area-weighted
  // average of each of the triangle's centroids.
  ...
  //   triangle 1: v0, v2, v3
  //   etc.
  // Center of mass is: ∑(kAᵢ * centroidᵢ) / ∑kAᵢ, where Aᵢ and centroidᵢ are
  // the area and centroid of the ith triangle in the fan. We can use the
  ...
  //   - We don't need the actual area; the double area would work just fine.
  ...
  //     _arbitrary_ normal direction simply further scales the area by a
  //     another constant, leaving the relative weights the same.
  auto triangle_area_weight = [&vertices_F, &nhat_F](V v0, V v1, V v2) {
  ...
  for (int i = 2; i < v_count; ++i) {
    const V v1 = v2;
    v2 = polygon[i];
    const T double_area = triangle_area_weight(v0, v1, v2);
    p_FC_accum += double_area * triangle_centroid(v0, v1, v2);
    total_weight += double_area;
  }
...
/** Adds the convex N-sided `polygon` to the given set of `faces` and `vertices`
 as a set of N triangles. A new vertex is introduced at the `polygon`'s center
 of mass and one triangle is added for each edge, formed by the edge and the
 center-of-mass position vertex.
 ...
 @param[in ,out] vertices_F
     ...
     vertices in this vector. One vertex will be added -- the polygon's center
     of mass.
...
void AddPolygonToMeshData(
  ...
  // TODO(SeanCurtis-TRI): I haven't touched this function yet!
  ...
  // a new vertex at the polygon's geometric centroid (which is the center of
  // mass for this "uniform material" polygon).
  Vector3<T> p_FC = CalcPolygonCenterOfMass(polygon, nhat_F, *vertices_F);
  ...
  // v1 will be polygon[0] in the first iteration.

...
/* @name "MeshBuilder" implementations
 ...
 MeshBuilders provide two services:
   ...
   - Collect mesh and field data associated with the polygon that arises from
     tet-tet, tet-tri, tet-plane, tri-half space, etc. interections.
 ...
 builds and subsequently builds is likewise a frame-dependent quantity. As such,
 it should be named with the expected frame clearly identified: e.g., builder_W.
 Please note the frame expectations on the function parameters. In the class
 documentation, we refer to the builder's frame as B. Some of the APIs require
 the fields or meshes that are colliding, and their frames will be named in
...
class TriMeshBuilder {
 public:
  ...
  /* Adds a vertex (and its corresponding field value) to the mesh. This is
   mostly used in mesh_plane_intersection and mesh_half_space_intersection
   where the algorithm is responsible for creating novel vertices and re-using
   them.
  ...
  int AddVertex(Vector3<T> p_BV, T field_value) {
    vertices_B_.push_back(std::move(p_BV));
    pressures_.push_back(std::move(field_value));
  ...
  /* Adds the polygon to the in-progress mesh. The polygon is defined by
   ...
   @param nhat_B            The normal to the polygon, measured and exprssed in
                            the builder's frame B.
   @param grad_e_MN_B       The gradient of the pressure field in the domain of
                            the polygon, measured and expressed in the builder's
                            frame B.
  ...
  /* Reports if a mesh has been built -- even a single face is enough. */
  bool is_empty() const { return faces_.empty(); }
...
class PolyMeshBuilder {
 public:
  ...
  /* Default construct reserves work space to reduce the odds of heap allocation
  ...
  std::vector<Vector3<T>>& mutable_per_element_gradients() {
    return grad_e_MN_B_per_element_;
  }
 ...
 private:
  ...
  std::vector<Vector3<T>> grad_e_MN_B_per_element_;
  /* The vertices of the mesh being built. */
  std::vector<Vector3<T>> vertices_B_;
  /* The pressure values (e) of the surface being built. */
  std::vector<T> pressures_;

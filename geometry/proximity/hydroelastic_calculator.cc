...
std::unique_ptr<ContactSurface<T>> CalcCompliantCompliant(
  ...
  return ComputeContactSurfaceFromCompliantVolumes(
      id_F, compliant_F.compliant_mesh(), X_WF, id_G, compliant_G.compliant_mesh(), X_WG,
      representation);
}
...
ContactCalculator<T>::MaybeMakeContactSurface(GeometryId id_A,
  ...
  if (type_A == HydroelasticType::kCompliant && type_B == HydroelasticType::kCompliant) {
  ...
  std::unique_ptr<ContactSurface<T>> surface =
      CalcRigidCompliant(compliant, X_WS, id_S, rigid, X_WR, id_R, representation_);

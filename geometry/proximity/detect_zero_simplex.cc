...
std::vector<int> DetectTetrahedronWithAllBoundaryVertices(
  ...
  for (int tetrahedron_index = 0; tetrahedron_index < mesh_M.num_elements();
    ...
    if (1 == boundary_vertex_set.count(tetrahedron.vertex(0)) &&
        1 == boundary_vertex_set.count(tetrahedron.vertex(1)) &&
        1 == boundary_vertex_set.count(tetrahedron.vertex(2)) &&
        1 == boundary_vertex_set.count(tetrahedron.vertex(3))) {
...
std::vector<SortedTriplet<int>> DetectInteriorTriangleWithAllBoundaryVertices(
  ...
  for (const SortedTriplet<int>& interior_triangle :
       GetInteriorFaceSet(GetBoundaryFaceSet(mesh_M), mesh_M)) {
...
std::vector<SortedPair<int>> DetectInteriorEdgeWithAllBoundaryVertices(
  ...
  for (const SortedPair<int>& interior_edge :
       GetInteriorEdgeSet(GetBoundaryEdgeSet(mesh_M), mesh_M)) {

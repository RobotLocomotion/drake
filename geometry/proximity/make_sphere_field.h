...
/*
 ...
 from the origin is defined as E * e(r) where e ∈ [0,1] is the extent -- a
 measure of penetration into the volume, and E is the given
 `hydroelastic_modulus`. The extent is defined as e(r) = 1 - r/ (R-δ), with R
 the radius of the sphere and δ the `margin`. Therefore the zero pressure level
 set of this field defines sphere of radius R-δ. Pressure is maximum at the
 center, with value E.
...
VolumeMeshFieldLinear<T, T> MakeSpherePressureField(
  ...
  for (const Vector3<T>& r_SV : mesh_S->vertices()) {
    T extent = T(1.0) - r_SV.norm() / (radius - margin);
    if (abs(extent) < kExtentEpsilon) {
      extent = T(0.0);
    }

...
/*
 ...
 and E is the given `hydroelastic_modulus`. For a point x inside the `cylinder`,
 the extent is defined as e(x) = (d(x)-δ)/(H-δ), with d(x) the (positive)
 distance to the boundary, δ the `margin` and H = min(R, L/2), where R and L are
 the radius and length of the `cylinder` respectively. With the extent defined,
 the pressure at a point x is computed as p(x) = E⋅e(x). The pressure has
 maximum value E in the interior and minimum value at the boundary p = -E⋅δ/H.

...
/** Computes the contact surface between a compliant sphere and rigid halfspace.
 The halfspace is defined in frame H lying on that H's Hx-Hy plane (with its
 normal in the Hz direction).
 The sphere has the given `radius` and is centered at the origin of frame S. The
 poses of the two geometries are given in the common world frame W.
 ...
 @returns The contact surface for the two shapes.
 @tparam T  The scalar type for the query.  */
template <typename T>
ContactSurface<T> CalcHalfspaceSphereContact(

...
VolumeMesh<double> VolumeMeshRefiner::Refine() {
  ...
  for (const SortedPair<int>& edge : DetectZeroInteriorEdge(input_mesh_)) {
    RefineEdge(edge);
  }
  ...
  for (const SortedTriplet<int>& triangle :
      DetectZeroInteriorTriangle(refined_edges)) {
  ...
  for (const int tetrahedron : DetectZeroTetrahedron(refined_triangles)) {

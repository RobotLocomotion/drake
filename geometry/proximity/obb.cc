...
RotationMatrixd ObbMaker<MeshType>::CalcOrientationByPCA() const {
  ...
  // Basis vectors of box frame B are calculated from the eigenvectors of the
  // covariance matrix. Eigen library has the kᵗʰ eigenvector corresponding to
  // the kᵗʰ eigenvalue, which is sorted in increasing order. Eigen library
  // also has the eigenvectors normalized to unit vectors; however, there is
  // no guarantee that the order of the three eigen vectors will make
  // positive orientation. That's why we pick the two eigenvectors of the two
  // largest eigenvalues for Bx and By and use their cross product for Bz.
  const Vector3d& Bx_M = es.eigenvectors().col(2);
  const Vector3d& By_M = es.eigenvectors().col(1);
...
template Obb ComputeObb(
    const SurfaceMesh<double>& mesh_M,
    const std::set<SurfaceMesh<double>::VertexIndex>& vertices);

template Obb ComputeObb(
    const VolumeMesh<double>& mesh_M,
    const std::set<VolumeMesh<double>::VertexIndex>& vertices);

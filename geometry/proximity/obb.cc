...
RotationMatrixd ObbMaker<MeshType>::CalcOrientationByPca() const {
  ...
  // matrix. SelfAdjointEigenSolver performs eigendecomposition A = VDV⁻¹,
  // where D is the diagonal matrix of eigenvalues, and V is the matrix of
  // eigenvectors of A. For a selfadjoint matrix A, V is always invertible.
  //
  // However, an eigenvalue λᵢ of A = covariance_M can have algebraic
  // multiplicity m when the characteristic equation det(A-λI) = 0 has the
  // (λᵢ-λ)ᵐ term, so geometrically the eigenvalue λᵢ corresponds to an
  // m-dimensional subspace (plane for m=2, space for m=3) instead of a line,
  // and any m orthonormal basis vectors of that subspace qualify as the
  // m eigenvectors of the eigenvalue λᵢ.
  //
  ...
  // we will explain at the end of this function.
  Eigen::SelfAdjointEigenSolver<Matrix3d> es;
  es.computeDirect(covariance_M);
  // Eigen documentation says it has the eigenvalues sorted in increasing order.
  // Confirm it. We use "<=" instead of "<" because the eigenvalues are repeated
  // according to their algebraic multiplicity.
  const double eigenvalue0 = es.eigenvalues()[0];
  const double eigenvalue1 = es.eigenvalues()[1];
  const double eigenvalue2 = es.eigenvalues()[2];
  DRAKE_DEMAND(eigenvalue0 <= eigenvalue1);
  DRAKE_DEMAND(eigenvalue1 <= eigenvalue2);

  // TODO(DamrongGuoy): In case of eigenvalues with multiplicity m or near
  //  multiplicity m, consider picking the basis that optimize OBB. Right now
  //  SelfAdjointEigenSolver picks an orthonormal basis of the m-dimensional
  //  subspace arbitrarily; it's not aware of our OBB application.

  // We will use only the last two vectors in the next step.
  // SelfAdjointEigenSolver always gives three valid orthonormal basis
  // vectors even if the input point set is co-planar, collinear, or just one
  // point. However, the valid orthonormal basis is not unique in these
  // rank-deficit cases. For example, collinear points will have
  // es.eigenvectors().col(2), corresponding to the largest
  // es.eigenvalues()[2], along the direction of the line, while the other
  // two orthonormal vectors are deterministic but arbitrary.
  const Vector3d& eigenvector2 = es.eigenvectors().col(2);
  const Vector3d& eigenvector1 = es.eigenvectors().col(1);
  // Eigen documenation says the eigenvectors are normalized. Confirm it.
  // Estimated tolerance from solving eigenvalue problems.
  double kEps = 100. * std::numeric_limits<double>::epsilon();
  DRAKE_ASSERT(eigenvector2.norm() < 1. + kEps);
  DRAKE_ASSERT(eigenvector1.norm() < 1. + kEps);

  // For PCA, the order of eigenvalues should be decreasing. Therefore, we pick
  // the last eigenvector for the first principal component Bx.
  ...
  // N.B. For an eigenvalue with multiplicity m, SelfAdjointEigenSolver will
  // pick an orthonormal basis to span the m-dimensional subspace arbitrarily.
  // Our unit tests showed that this arbitrary process is not invariant under
  // rigid transform and not optimal for computing the oriented bounding box.
  // For example, the six vertices of a standard octahedron (±UnitX(),
  // ±UnitY(), ±UnitZ()) gives a covariance matrix that has only one eigenvalue
  // with multiplicity 3, so mathematically any orthonormal basis of ℝ³ is
  // a PCA solution. Intuitively the octahedron has no preferred direction
  // under PCA.  However, this affects downstream applications.  In our unit
  // tests, we got a different solution of PCA from applying a rigid transform
  // to the standard octahedron that it shrank the volume of OBB by about 67%.
  // See TEST_F(ObbMakerTestOctahedron, CalcOrientationByPca) and
  // TEST_F(ObbMakerTestOctahedron, ObbMakerCompute) in obb_test.cc.
...
Obb ObbMaker<MeshType>::CalcOrientedBox(const RotationMatrixd& R_MB) const {
  // To calculate frame B of the oriented box, we use an intermediate frame F
  // as rotation of the mesh's frame M by the given orientation R_MB of the box.
  // In other words, the box's frame B is aligned with frame F, but the
  // origin Bo differs from Fo (Fo collocates with Mo).
  //
...
Vector3d ObbMaker<MeshType>::CalcVolumeGradient(const Obb& box) const {
  ...
  // TODO(DamrongGuoy): Use a better estimate for the step size h.
...
// TODO(DamrongGuoy): Improve optimization algorithm. The unit test (ObbTest,
//  TestComputeObb) showed that it got stuck at a local optimal for a
//  symmetric configuration.

template <class MeshType>
Obb ObbMaker<MeshType>::OptimizeObbVolume(const Obb& box0) const {
  // Simple volume minimization along a negative direction of a fixed volume
  // gradient vector ∂v/∂(r,p,y) in roll-pitch-yaw space. Although the gradient
  // is fixed, the step size is adjusted iteratively up or down according to
  // the new samplings.
...
Obb ObbMaker<MeshType>::Compute() const {
  RotationMatrixd R_MB = CalcOrientationByPca();
  Obb box = CalcOrientedBox(R_MB);

...
/** @name  Safe dereferencing of pointers

 Provides a macro and supporting function for aliasing pointers to const
 objects in constructors. If the pointer is null, an exception is thrown,
 reporting the pointer variable (by name) is null.
 */
#define DRAKE_GEOMETRY_SAFE_DEREFERENCE(value) DereferenceOrThrow(value, #value)

template <typename T>
T& DereferenceOrThrow(T* const value, const char* value_name) {
  if (value) return *value;
  throw std::logic_error(fmt::format(
      "Unable to dereference null-valued pointer '{}'", value_name));
}

...
class VertexReader final : public ShapeReifier {
 ...
 private:
  ...
  Vector3d FindNormal(const std::vector<Vector3d>& vertices,
    ...
    } while (a.squaredNorm() < 1e-24 && v < ssize(vertices));

    if (v == ssize(vertices)) {
      throw std::runtime_error(
          fmt::format("MakeConvexHull failed because all vertices in the mesh "
                      "were within a sphere with radius 1e-14 for file: {}.",
                      filename));
    }
    a.normalize();

    Vector3d b = vertices[++v] - vertices[0];
    Vector3d n_candidate = a.cross(b);
    while ((b.squaredNorm() < 1e-24 || n_candidate.squaredNorm() < 1e-24) &&
           v < ssize(vertices)) {
      b = vertices[++v] - vertices[0];

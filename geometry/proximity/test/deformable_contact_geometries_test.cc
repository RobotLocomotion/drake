...
/* Constructs a coarse sphere (octahedron) whose vertices are transformed from
 the canoncial sphere frame S to some arbitrary frame F. */
...
GTEST_TEST(DeformableGeometryTest, ConstructorSphere) {
  ...
  // the sphere is the coarsest possible, it is an octahedron with 8 tests and

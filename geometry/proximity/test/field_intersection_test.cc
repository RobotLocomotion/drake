...
void WritePolygonToVtkAsSurfaceField(
  ...
  vector<int> polygon;
  vector<Vector3<double>> vertices_M;
  ...
  for (const Vector3<double>& v_M : vertices_M) {
    pressure_values.push_back(
        field_M.EvaluateCartesian(tetrahedron, v_M));
  }
  TriangleSurfaceMesh<double> polygon_mesh_M(move(faces), move(vertices_M));
  TriangleSurfaceMeshFieldLinear<double, double> field_on_polygon_M(
      move(pressure_values), &polygon_mesh_M, false);
  WriteTriangleSurfaceMeshFieldLinearToVtk(vtk_file_name, "Presure[Pa]",
                                           field_on_polygon_M, description);
}
#endif
...
TEST_F(FieldIntersectionVizTest, HackCompliantBoxRigidBall) {
  ...
  //  don't need this.
  const TriangleSurfaceMesh<double> ball_surface_mesh1_M =
      ConvertVolumeToSurfaceMesh(TransformVolumeMesh(X_MN_, ball_mesh1_N_));
  const Bvh<Obb, TriangleSurfaceMesh<double>> ball_surface_bvh1_M(
...
TEST_F(FieldIntersectionVizTest, HackRigidBoxCompliantBall) {
  // TODO(DamrongGuoy): Remove #include volume_to_surface_mesh.h when we
  //  don't need this.
  const TriangleSurfaceMesh<double> box_surface_mesh0_M =
      ConvertVolumeToSurfaceMesh(box_mesh0_M_);
  Bvh<Obb, TriangleSurfaceMesh<double>> box_surface_bvh0_M(box_surface_mesh0_M);

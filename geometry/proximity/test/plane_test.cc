...
GTEST_TEST(PlaneTest, MixedScalar) {
  ...
  const Vector3<AutoDiffXd> p_FB_ad =
      math::InitializeAutoDiffFromValueMatrix(p_FB_d);
  ...
  const Vector3<AutoDiffXd> p_FQ_ad_derivs =
      math::InitializeAutoDiffFromValueMatrix(p_FQ_d);

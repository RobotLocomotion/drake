...
GTEST_TEST(PlaneTest, Construction) {
...

  {
    ...
    if (kDrakeAssertIsArmed) {
      ...
      DRAKE_EXPECT_THROWS_MESSAGE(Planed(nhat_F * 0.99999, p_FP, true),
                                  std::runtime_error,
                                  "Plane constructed with a normal vector that "
                                  "was declared normalized is insufficiently "
                                  "normal.*");
      // Within a tolerance it is accepted.
      EXPECT_NO_THROW(Planed(nhat_F * (1 - 10 * kEps), p_FP, true));
    } else {
      // In release, it is simply used.
      Planed plane_F{0.5 * nhat_F, p_FP, true /* already_normalized */};

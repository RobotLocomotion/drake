...
GTEST_TEST(DeformableContactSurface, SmokeTest) {
  ...
  const Box cube = Box::MakeCube(2.0);
  const TriangleSurfaceMesh<double> rigid_mesh_R =
      MakeBoxSurfaceMesh<double>(Box::MakeCube(2.0), kVeryCoarseResolution);
  ...
  std::unique_ptr<DeformableContactSurface<double>>
      deformable_contact_surface_W =
          ComputeContactSurfaceFromDeformableVolumeRigidSurface(
              GeometryId(), deformable_W, GeometryId(), rigid_mesh_R,
              rigid_bvh_R, X_WR);

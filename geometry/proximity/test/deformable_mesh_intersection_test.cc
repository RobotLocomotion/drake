...
// contact, and centroid barycentric coordinates. We only test the correctness
// of these newly added operations here in this test. For the remaining data, we
// only check that they have the correct sizes.
GTEST_TEST(ComputeContactSurfaceDeformableRigid, OnePolygon) {
  ...
  ASSERT_EQ(contact_data.R_CWs().size(), kExpectedNumContactPoints);
}

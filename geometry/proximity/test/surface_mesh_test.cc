...
class ScalarMixingTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    p_WQ_d_ /= 3;
    p_WQ_ad_ = math::InitializeAutoDiffFromValueMatrix(p_WQ_d_);

    b_expected_d_ = Vector3d(1, 1, 1) / 3.0;
    b_expected_ad_ = math::InitializeAutoDiffFromValueMatrix(b_expected_d_);
  }
...
TEST_F(ScalarMixingTest, CalcCartesianFromBarycentric) {
...

  {
    ...
    EXPECT_TRUE(CompareMatrices(math::ExtractValueMatrixFromAutoDiff(p_WC),
                                p_WQ_d_, kEps));
  }

  {
    ...
    EXPECT_TRUE(CompareMatrices(math::ExtractValueMatrixFromAutoDiff(p_WC1),
                                p_WQ_d_, kEps));

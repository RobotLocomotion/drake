...
GTEST_TEST(MakeSphereVolumeMesh, ConfirmEdgeLength) {
  ...
  auto test_equator = [r_squared](const VolumeMesh<double>& mesh,
    ...
    // The number of equator vertices should be 4 * 2^L.
    const int v_count = static_cast<int>(equator_vertices.size());
    // Confirm divisible by 4 -- least significant 2 bits are zero.
    EXPECT_EQ(v_count & 0x3, 0) << "Have " << v_count << " equator vertices";
    // Confirm 4 * 2^L -- the value should be all zeros, except for one bit
    // (and we won't test the sign bit).
    const int bit_count = static_cast<int>(sizeof(int)) * 8 - 1;
    int one_count = 0;
    for (int i = 2; i < bit_count; ++i) {
      one_count += (v_count >> i) & 0x1;
    }
    EXPECT_EQ(one_count, 1) << "Have " << v_count << " equator vertices";

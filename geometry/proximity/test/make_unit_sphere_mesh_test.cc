...
GTEST_TEST(MakeSphereMesh, NoDuplicateVertices) {
  ...
  std::vector<int> expected_v_count{7, 25, 129, 833};
  for (int i = 0; i < 4; ++i) {
...
GTEST_TEST(MakeSphereMesh, SplitOctohedron) {
  ...
  std::vector<VolumeVertex<double>> tet_vertices;
  tet_vertices.emplace_back(1, -1, -1);  // point A
  tet_vertices.emplace_back(1, 1, 1);
  tet_vertices.emplace_back(-1, 1, -1);
  tet_vertices.emplace_back(-1, -1, 1);
  ...
  std::vector<VolumeVertex<double>> unit_p_MVs;
  unit_p_MVs.emplace_back(mid_point(a, b));  // e = (a + b) / 2
  unit_p_MVs.emplace_back(mid_point(a, c));  // f = (a + c) / 2
  unit_p_MVs.emplace_back(mid_point(a, d));  // g = (a + d) / 2
  unit_p_MVs.emplace_back(mid_point(b, c));  // h = (b + c) / 2
  unit_p_MVs.emplace_back(mid_point(b, d));  // i = (b + d) / 2
  unit_p_MVs.emplace_back(mid_point(c, d));  // j = (c + d) / 2
  ...
  // 3, 2, and 1. The axis under test is scaled by 3, the "next" axis by 2, and
  // the third axis by 1. SplitOctohedron() will favor splitting along the
  // longest axis. In the tests above, it's clear that EJ is aligned with the
  // x-axis, GH with the y, and FI with the z.
  const std::vector<SortedPair<VIndex>> split_edges{SortedPair<VIndex>{e, j},
                                                    SortedPair<VIndex>{g, h},
                                                    SortedPair<VIndex>{f, i}};
  for (int axis = 0; axis < 3; ++axis) {
    Vector3d scale_factor{1, 1, 1};
    scale_factor(axis) = 3;
    scale_factor((axis + 1) % 3) = 2;

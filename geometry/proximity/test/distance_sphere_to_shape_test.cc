 ...
 protected:
  ...
  ::testing::AssertionResult TestDerivatives(
    ...
    const auto dgrad_dAo_shape =
        math::ExtractGradientMatrixFromAutoDiff(shape_result.nhat_BA_W);
    const auto dgrad_dAo_point =
        math::ExtractGradientMatrixFromAutoDiff(point_result.grad_W);
    ...
    auto dCb_dAo = math::ExtractGradientMatrixFromAutoDiff(shape_result.p_BCb);
    const auto dN_dAo =
        math::ExtractGradientMatrixFromAutoDiff(point_result.p_GN);
    ...
    const auto dCa_dAo =
        math::ExtractGradientMatrixFromAutoDiff(shape_result.p_ACa);
...
Vector3<AutoDiffXd> DistancePairGeometryTest::p_WSphere<AutoDiffXd>() {
  return math::InitializeAutoDiffFromValueMatrix(p_WSphere_);
}

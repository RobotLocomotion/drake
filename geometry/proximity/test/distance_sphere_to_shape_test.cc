...
/*
 X CallbackData is just a struct and needs no explicit testing!
 X CalcDistanceFallback
  X confirm failure for non-whitelisted functions (don't worry about
    correctness); we assume that the correctness of the wrapped implementation
    was tested elsewhere.
 X DistancePairGeometry
  X Ultimately, it calls DistanceToPoint, so all we *really* need to confirm is
    that the wiring in DistanceToPoint::SphereShapeDistance() is correct.
    (Assuming that the results of distance-to-point is correct).
 ComputeNarrowPhaseDistance
  X Confirm that any halfspaces throw an exception
  X Confirm the fallback is called when there is no sphere - do this by
    creating a dummy scalar and a custom specialization of the fallback
    function. It can store data that can be tracked.
  X Confirm that sphere, box, and cylinder are handled by primitives (the
    dual of the previous test).
  X Confirm I get the same answer whether or not the sphere is the first
    shape or the second shape.
 Callback
  - The result value is always ordered id_A < id_B.
  - ScalarSupport what *is* allowed and what *isn't*.
  - No results if can't collide.
  O Confirm for some test from calls on DistancePairGeometry that the right
    result comes out (unnecessary; it doesn't do *anything* to the results
    except accumulate them).  */

...
TEST_F(PenetrationAsPointPairCallbackTest, CollisionFilterRespected) {
  ...
  // rotation, such that it penetrations A 0.1 units. We want to make sure the
  // two spheres have a non-trivial transform between their frames and show that
  // regardless of their ordering in the callback, the result is bit identical.

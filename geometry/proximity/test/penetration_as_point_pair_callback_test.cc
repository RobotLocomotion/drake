    ...
    if constexpr (std::is_same_v<T, AutoDiffXd>) {
      ...
      const RigidTransform<double> X_WB_double(
          RotationMatrix<double>(
              math::ExtractValue(X_WB.rotation().matrix())),
      ...
      EXPECT_TRUE(CompareMatrices(
          math::ExtractValue(first_result.p_WCa),
          point_pairs_double[0].p_WCa));
      EXPECT_TRUE(CompareMatrices(
          math::ExtractValue(first_result.p_WCb),
          point_pairs_double[0].p_WCb, kEps));
      EXPECT_TRUE(CompareMatrices(
          math::ExtractValue(first_result.nhat_BA_W),

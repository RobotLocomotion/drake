...
ExtractMatrixValue(const Derived& v) {
  if constexpr (std::is_same_v<typename Derived::Scalar, double>) {
    ...
    return math::ExtractValueMatrixFromAutoDiff(v);
  } else {
 ...
 protected:
  ...
  void TestCollisionFilterRespected() {
    ...
    if constexpr (std::is_same_v<T, AutoDiffXd>) {
      p_WBo = math::InitializeAutoDiffFromValueMatrix(
    ...
    if constexpr (std::is_same_v<T, AutoDiffXd>) {
      ...
      const RigidTransform<double> X_WB_double(
          RotationMatrix<double>(
              math::ExtractValueMatrixFromAutoDiff(X_WB.rotation().matrix())),
          math::ExtractValueMatrixFromAutoDiff(X_WB.translation()));
      ...
      EXPECT_TRUE(CompareMatrices(
          math::ExtractValueMatrixFromAutoDiff(first_result.p_WCa),
          point_pairs_double[0].p_WCa));
      EXPECT_TRUE(CompareMatrices(
          math::ExtractValueMatrixFromAutoDiff(first_result.p_WCb),
          point_pairs_double[0].p_WCb, kEps));
      EXPECT_TRUE(CompareMatrices(
          math::ExtractValueMatrixFromAutoDiff(first_result.nhat_BA_W),
  ...
  RigidTransform<T> CalcBoxPoseInSphereBox(double target_depth) {
    ...
    if constexpr (std::is_same_v<T, AutoDiffXd>) {
      p_WBo =
          R_WB * math::InitializeAutoDiffFromValueMatrix(
              Vector3d::UnitX() * center_distance);
    } else {
  ...
  std::pair<RigidTransform<T>, double> CalcHalfspacePoseInSphereHalfspace() {
    ...
    Vector3<T> p_WBo;
    if constexpr (std::is_same_v<T, AutoDiffXd>) {
      p_WBo = math::InitializeAutoDiffFromValueMatrix(Eigen::Vector3d(0.5,
                                                                      -0.2,
                                                                      0.9));
    } else {
...
TEST_F(PenetrationAsPointPairCallbackTest, TestGradient) {
  ...
  const Eigen::Vector3d p_WBo_val =
      Vector3d{1, -2, 3}.normalized() * center_distance;
  const Vector3<AutoDiffXd> p_WBo =
      math::InitializeAutoDiffFromValueMatrix(p_WBo_val);

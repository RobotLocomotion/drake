...
// the offset from the nearest point to thq query point Q (p_NQ_G) also in the
...
GTEST_TEST(DistanceToPoint, Sphere) {
  ...
  // frame basis.
  const Vector3d p_NQ_G = Vector3d{2, -3, 6}.normalized();
  const Vector3d p_GN_G = sphere.radius * p_NQ_G;
  ...
  EXPECT_TRUE(
      tester.Test(p_GN_G, -sphere.radius * p_NQ_G, true /* is inside */));
}
...
GTEST_TEST(DistanceToPoint, Cylinder) {
  ...
  // Case: Nearest point is on the cap.
  {
    ...
    for (double sign : {-1, 1}) {
      ...
      tester.Test(p_GN_G, Vector3d{0, 0, -0.1 * sign},
                            true /* is inside */);
    }
  }

...
/* @file A collection of utilities to facilitate proximity query testing. The
 ...
 test scenarios, outcome expectations and provides the common code to exercise
 the various proximity queries. It is the expectation that these utilities will
 be exercised by each set of *callback* unit tests, providing correctness
 coverage and characterization of the query's "quality".  */
...
class DistanceCallback {
 public:
  ...
  /* Reports the distance contained in the first result produced by the last
   call to Invoke().
   @pre GetNumResults() > 0. */
...
/* Render an FCL enumeration into a string to support intelligible error
...
class MakeFclShape : public ShapeReifier {
 public:
  ...
  const std::shared_ptr<fcl::CollisionGeometry<double>> object() const {
    return object_;
  }
...
class CharacterizeResultTest : public ::testing::Test {
 public:
  ...
  /* @name A consistent set of shapes to use across all tests
   ...
   be approximately 20 cm. Other values used are have been used to avoid
   symmetry and perfectly represented numerical values. */
  //@{

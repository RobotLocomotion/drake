...
class ShapeConfigurations : public ShapeReifier {
 public:
  ...
  /* Sample the cylinder on one cap faces (near its edge), one sample on a
   circular edge, and one on the barrel. The sample on the top face is
   guaranteed to be closest to all points up any requested penetration depth
   (assuming the cylinder is large enough).  */

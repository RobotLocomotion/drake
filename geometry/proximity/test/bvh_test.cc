...
TYPED_TEST(BvhTest, TestCollideSurfaceVolume) {
  ...
  /* The meshes are defined as follows:
   ...
   - Volume mesh: two tetrahedra with a single shared vertex (D). Each tetra
     has a triangular face (ABC and EFG, respectively) that lies parallel to the
     x = 0 plane.
   - Surface mesh: Two triangles with a shared edge (bd) lying on the y = 0
     plane.

   We'll pose the two meshes so that vertex a penetrates the tetrahedron face
   ABC. By this we know we'll have a single intersecting element pair (and their
   indices). We'll also confirm that we don't get four candidate pairs (M * N)
   but only two. Each tetrahedron is in its own leaf node, but the two triangles
   are contained in a single leaf node. */

  using SVertex = SurfaceVertex<double>;
  using STri = SurfaceFace;
  using SVIndex = SurfaceVertexIndex;
  std::vector<SVertex> vertices_S{
      {SVertex(Vector3d(0, 0, 0)), SVertex(Vector3d(1, 0, 1)),
       SVertex(Vector3d(2, 0, 0)), SVertex(Vector3d(1, 0, -1))}};
  ...
  std::vector<VVertex> vertices_V{
      {VVertex(Vector3d(1, 0, 1)), VVertex(Vector3d(1, 0, -1)),
       VVertex(Vector3d(1, 1, 0)), VVertex(Vector3d(0, 0, 0)),
       VVertex(Vector3d(-1, 0, 1)), VVertex(Vector3d(-1, 0, -1)),
       VVertex(Vector3d(-1, 0, 1))}};

...
// Test suite for exercising DispatchRigidCompliantCalculation with different scalar
// types.
template <typename T>
...
TYPED_TEST(DispatchRigidCompliantCalculationTests, CompliantMeshRigidHalfSpace) {
  using T = TypeParam;
  const bool colliding{true};
...
TYPED_TEST(DispatchRigidCompliantCalculationTests, CompliantHalfSpaceRigidMesh) {
  using T = TypeParam;
  const bool colliding{true};
...
TYPED_TEST(MaybeMakeContactSurfaceTests, BothCompliantOneHalfSpace) {
  ...
  TestScene<T> scene{ShapeType::kSphere, ShapeType::kHalfSpace};
  scene.ConfigureScene(HydroelasticType::kCompliant, HydroelasticType::kCompliant);
...
// Confirms that MaybeMakeContactSurface relies on DispatchRigidCompliantCalculation

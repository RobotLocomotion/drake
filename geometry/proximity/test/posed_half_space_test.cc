...
GTEST_TEST(PosedHalfSpaceTest, Construction) {
  ...
  for (const Vector3d& dir : dirs) {
    ...
    EXPECT_NO_THROW(PosedHalfSpace<double>(dir.normalized(), point));
  }
}

// Confirms that a PosedHalfSpace normal that is *insufficiently* unit length
// throws.
GTEST_TEST(PosedHalfSpaceTest, Unnormalized) {
  const double kDelta = 4 * std::sqrt(std::numeric_limits<double>::epsilon());
  EXPECT_THROW(
      PosedHalfSpace<double>(Vector3d{1, kDelta, kDelta}, Vector3d{0, 0, 0}),
      std::exception);
}
...
GTEST_TEST(PosedHalfSpaceTest, CalcSignedDistance) {
  ...
  {
    PosedHalfSpace<double> hs_F{nhat_F, p_FB};
    for (int i = 0; i < 2; ++i) {
      EXPECT_NEAR(hs_F.CalcSignedDistance(p_FQs[i]), d_Qs[i], kEps);
    }
  }

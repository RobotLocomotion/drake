...
GTEST_TEST(VolumeMeshRefinerTest, TestRefineTetrahedron) {
  ...
  ASSERT_EQ(DetectZeroTetrahedron(test_mesh).size(), 1);
  ASSERT_EQ(DetectZeroInteriorTriangle(test_mesh).size(), 0);
  ASSERT_EQ(DetectZeroInteriorEdge(test_mesh).size(), 0);
  ...
  EXPECT_EQ(DetectZeroTetrahedron(refined_mesh).size(), 0);
  EXPECT_EQ(DetectZeroInteriorTriangle(refined_mesh).size(), 0);
  EXPECT_EQ(DetectZeroInteriorEdge(refined_mesh).size(), 0);
...
GTEST_TEST(VolumeMeshRefinerTest, TestRefineTriangle) {
  ...
  const VolumeMesh<double> test_mesh(
      ...
      std::vector<Vector3d>{Vector3d::Zero(), Vector3d::UnitX(),
                            Vector3d::UnitY(), Vector3d::UnitZ(),
                            -Vector3d::UnitZ()});
  ASSERT_EQ(DetectZeroTetrahedron(test_mesh).size(), 2);
  ASSERT_EQ(DetectZeroInteriorTriangle(test_mesh).size(), 1);
  ASSERT_EQ(DetectZeroInteriorEdge(test_mesh).size(), 0);
...
GTEST_TEST(VolumeMeshRefinerTest, TestRefineEdge) {
  ...
  const VolumeMesh<double> test_mesh(
      ...
      std::vector<Vector3d>{Vector3d::Zero(), Vector3d::UnitX(),
                            ...
                            Vector3d(-1, 1, 1),
                            Vector3d(-1, 1, -1)});
  ASSERT_EQ(DetectZeroTetrahedron(test_mesh).size(), 3);
  ASSERT_EQ(DetectZeroInteriorTriangle(test_mesh).size(), 3);
  ASSERT_EQ(DetectZeroInteriorEdge(test_mesh).size(), 1);
  ASSERT_EQ(test_mesh.num_vertices(), 5);

...
GTEST_TEST(MakeCapsuleVolumeMesh, SecondCoarsestMesh) {
  ...
  // give the next coarsest mesh (4 vertices on the circlar rim).
...
GTEST_TEST(MakeCapsuleVolumeMesh, FinestMesh) {
  ...
  // give the finest possible mesh (706 vertices on the circlar rim).
...
GTEST_TEST(MakeCapsuleVolumeMesh, FinestMeshClamped) {
  ...
  // the circlar rim). Here we give a hint twice as small as the smallest
  // valid hint.

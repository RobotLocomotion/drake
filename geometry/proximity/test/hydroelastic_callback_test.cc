...
// 4. If the two geometries are not colliding, then we produce no contact.
GTEST_TEST(HydroelasticCallbackAutodiff, AutoDiffBlanketFailure) {
  ...
  // S1 and B2 are not in contact.
  DRAKE_EXPECT_THROWS_MESSAGE(
      Callback<AutoDiffXd>(&object_S1, &object_B2, &data), std::logic_error,
      "Requested AutoDiff-valued contact surface between two geometries with "
      "hydroelastic representation but for scalar type.*");
  // TODO(hongkai.dai): enable the next test. Currently it throws error for
  // kUnsupportedScalar before calling for the penetration as point pair
  // fallback.
  // point_pairs.clear();

...
class TestScene {
 public:
  ...
  void PoseGeometry(bool are_colliding) {
    ...
    //
    // The distance geometry B needs to move in the -z direction to *touch* the
    // bottom of sphere A: the radius of A _plus_ the distance of geometry B
    // to its own origin (radius for sphere, 0 for half space).
...
TYPED_TEST(DispatchRigidSoftCalculationTests, SoftMeshRigidMesh) {
...

  {
    ...
    auto surface = DispatchRigidSoftCalculation(

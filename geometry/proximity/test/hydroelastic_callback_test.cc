...
TYPED_TEST(DispatchCompliantCompliantCalculationTests,
  ...
  //  Right now AutoDiffXd exposed a numerical problem in field_intersection.
  //  IntersectTetrahedra()-> ClipPolygonByHalfSpace() -> CalcIntersection()
  //  of a line defined by practically two identical points (within
  //  1e-14 distance). I don't know why `double` didn't expose the problem.
  //  It's probably by chance that some kinds of numerics happened in the
  //  "right" way with AutoDiffXd.
  ...
  for (const auto representation :
        ...
        fmt::format("representation = {}", static_cast<int>(representation)));
    {
      ...
      switch (representation) {
        case HydroelasticContactRepresentation::kTriangle:
          EXPECT_EQ(48, surface->tri_mesh_W().num_triangles());
          break;
        case HydroelasticContactRepresentation::kPolygon:
          EXPECT_EQ(12, surface->poly_mesh_W().num_elements());
          break;
      }
    }
  }

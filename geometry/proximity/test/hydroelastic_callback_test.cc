...
// that an exception is thrown. It is important that collisions are *not*
// filtered between the two objects.
TYPED_TEST(HydroelasticCallbackTyped, ThrowForMissingHydroelasticData) {
  using T = TypeParam;

  vector<ContactSurface<T>> surfaces;

  const Sphere sphere(this->radius_);
  const Box box(this->cube_size_, this->cube_size_, this->cube_size_);
...
TYPED_TEST(HydroelasticCallbackTyped, ThrowForBadPairBasedOnCompliance) {
  ...
  // Case: both geometries are soft.
  {
    ...
    hydroelastic_geometries.MaybeAddGeometry(sphere, this->id_sphere_,
                                             soft_properties());
    // NOTE: We're swapping a sphere for the box because the box doesn't
    // currently support soft representation.
...
//  When the calling path becomes more hetergeneous, this test will no longer
//  be sufficient.

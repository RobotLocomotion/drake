...
enum class ShapeType { kSphere, kHalfSpace, kZeptoSphere };
...
class TestScene {
 public:
  ...
  unique_ptr<CollisionObjectd> MakeShape(GeometryId id, HydroelasticType type,
    ...
    switch (shape_type) {
      ...
      case ShapeType::kHalfSpace:
        ...
        break;
      case ShapeType::kZeptoSphere:
        shape =
            make_unique<CollisionObjectd>(make_shared<Sphered>(kZeptoRadius));
        MakeHydroelastic(id, type, Sphere(kZeptoRadius));
        break;
    }
  ...
  void PoseGeometry(bool are_colliding) {
    ...
    auto shape_radius = [](ShapeType shape_type) -> double {
      switch (shape_type) {
        case ShapeType::kSphere:
          return kRadius;
        case ShapeType::kZeptoSphere:
    ...
    const double dist_WoA = shape_radius(this->shape_A_type_);
    ...
    const double dist_WoB = shape_radius(this->shape_B_type_);
 ...
 private:
  ...
  static constexpr double kZeptoRadius{1e-7};
...
// Confirms that soft zepto-spheres vanish from contact queries.
TYPED_TEST(HydroelasticCallbackFallbackTyped, SoftZeptosVanish) {
  using T = TypeParam;

  TestScene<T> scene{ShapeType::kSphere, ShapeType::kZeptoSphere};
  scene.ConfigureScene(HydroelasticType::kRigid, HydroelasticType::kSoft);

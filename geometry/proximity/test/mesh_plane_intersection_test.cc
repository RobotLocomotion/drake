...
// Compute a unit-length normal to the plane spanned by the three vertices.
// The direction of the normal will be based on right-handed winding and it is
// assumed the vertex positions are _not_ co-linear.
...
// Infrastructure for testing SliceTetWithPlane().
class SliceTetWithPlaneTest : public ::testing::Test {
 protected:
  void SetUp() override {
    // An arbitrary transform that doesn't contain any identities.
  ...
  ::testing::AssertionResult CallSliceTestWithPlane(
    ...
    SliceTetWithPlane(tet_index, field_F, plane_F, X_WF_, &faces_,
                      &vertices_W_, &surface_pressure_, &cut_edges_);
  ...
  // Determine which vertex is the centroid; it should be the single vertex
  // referenced by every face (all other vertices should be referenced twice).
  ...
  // weight * edge_vertices[0] + (1 - weight) * edge_vertices[1])
  struct EdgeVertex {
  ...
  // For each vertex in the "slice" mesh (that isn't the centroid), determine
  ...
  CharacterizeEdgeVertices(
      SurfaceVertexIndex centroid_index, const vector<SurfaceFace>& slice,
    ...
    set<SurfaceVertexIndex> slice_vertex_indices;
    for (const auto& face : slice) {
      for (int i = 0; i < 3; ++i) {
        ...
        for (int e = 0; e < 6; ++e) {
          ...
          // Test if S lies on the line spanned by V0V1, skip if it doesn't.
          // The dot product of the two vectors should be equal to the product
          // of their magnitudes (within epsilon).
    ...
    for (const auto& edge_vertex : edge_vertices) {
      ...
      // The edge matches; confirm that the vertex matches.
  ...
  ::testing::AssertionResult SliceIsConsistent(
    ...
    for (const auto& V_W : vertices_W_) {
      ...
      const auto b_V = field_F.mesh().CalcBarycentric(p_FV, tet_index);
      for (int i = 0; i < 4; ++i) {
    ...
    // Confirms which vertex is the centroid and that every other perimeter
    // vertex is shared by two triangles.
...
// the sensitivity between intersecting and not intersecting. The correctness
// of the values of those artifacts can be found in subsequent tests.
TEST_F(SliceTetWithPlaneTest, NonIntersectingConfiguration) {
  ...
  for (const auto& X_FM : X_FMs) {
    ...
    // that it is normal to the v1, v2, v3 plane.
    const Vector3d& Mx_F = X_FM.rotation().matrix().col(0);
    const Vector3d& My_F = X_FM.rotation().matrix().col(1);
    const Vector3d normal_F = (Mx_F + My_F + Mz_F).normalized();
    VolumeMesh<double> mesh_F = TrivialVolumeMesh(X_FM);
...
TEST_F(SliceTetWithPlaneTest, TriangleIntersections) {
  ...
  // we want avoid a proliferation of zeros in that vector. We can avoid this
  ...
  vector<RigidTransformd> X_FMs{
      ...
      RigidTransformd{Vector3d{1.5, 2.5, 3.5}},
      RigidTransformd{RotationMatrixd{AngleAxisd{
...
// Similar for the TriangleIntersections test, but for intersections that
// for quads.
TEST_F(SliceTetWithPlaneTest, QuadIntersections) {
  // There are six unique configurations for a plane-tet intersection to be
  // a quad. A pair of vertices lie one one side of the plane and the other pair
  // on the other side. There are 4 choose 2 = 6 options. Alternatively, we
  // can enumerate three pairs of edges. For a given plane normal direction,
  // that represents one of the six options, reversing the normal represents
  // another. So, pick the right three vertex pairs, and evaluate it with
  // a pair of planes which differ by opposing normal directions, we'll cover
  ...
  for (const auto& X_FM : X_FMs) {
    ...
    // a tet projects onto all six edges.
...
// the output (and conversely duplicate input vertices lead to duplicate
// output vertices).

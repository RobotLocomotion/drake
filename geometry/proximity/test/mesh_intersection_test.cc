...
VolumeElementIndex GetTetForTriangle(const SurfaceMesh<T>& surface_S,
  ...
  for (VolumeElementIndex t(0); t < volume_V.num_elements(); ++t) {
    auto bary_t = volume_V.CalcBarycentric(p_VC, t);
    if ((bary_t.array() >= 0).all() && (bary_t.array() <= 1).all()) {
      return t;
    }
  }
...
GTEST_TEST(MeshIntersectionTest, SampleVolumeFieldOnSurface) {
  ...
  // test in the volume mesh. The surface lies parallel to the y = 0.75 plane.
  ...
  using GradientPtr = std::vector<Vector3<double>>*;
  std::vector<Vector3<double>> grad_eM_M;

  for (GradientPtr grad_eM_M_ptr : {GradientPtr(nullptr), &grad_eM_M}) {
    ...
    if (grad_eM_M_ptr) {
      // We've requested gradients of the constituent pressure fields. Only the
      // soft volume mesh has one.
      const std::vector<SurfaceFace>& faces = surface_M->faces();
      ASSERT_EQ(faces.size(), grad_eM_M_ptr->size());
      for (FIndex f(0); f < surface_M->num_elements(); ++f) {
        const VolumeElementIndex t =
            GetTetForTriangle(*surface_M, f, *volume_M, {});
        ASSERT_TRUE(CompareMatrices(grad_eM_M_ptr->at(f),
                                    volume_field_M->EvaluateGradient(t)));
      }
      ...
      EXPECT_FALSE(CompareMatrices(grad_eM_M_ptr->front(),
                                   grad_eM_M_ptr->back(), 1e10));
    }
  }
}
...
void TestComputeContactSurfaceSoftRigid() {
  ...
  auto contact_SR = ComputeContactSurfaceFromSoftVolumeRigidSurface(
      id_A, *field_S, X_WS, id_B, *surface_R, X_WR, false);
  EXPECT_EQ(contact_SR->id_M(), id_A);
  EXPECT_EQ(contact_SR->id_N(), id_B);
  // No gradients requested --> no gradients available.
  EXPECT_FALSE(contact_SR->HasGradE_M());
  ...
  {
    // Confirm that when gradients of constituent pressure fields are requested,
    // that they are returned. ComputeContactSurfaceFromSoftVolumeRigidSurface
    // has *two* responsibilities:
    //  1. Make sure that the request propagates down the call stack so that
    //     gradients are returned *at all*.
    //  2. Make sure that the results are in the *world* frame.
    auto contact = ComputeContactSurfaceFromSoftVolumeRigidSurface(
        id_A, *field_S, X_WS, id_B, *surface_R, X_WR, true);
    ASSERT_LT(id_A, id_B);  // This implies that the soft mesh is M.
    EXPECT_TRUE(contact->HasGradE_M());
    EXPECT_FALSE(contact->HasGradE_N());
    // To determine the world transformation has taken place, we'll find which
    // tetrahedron produced the first triangle in the contact surface. We'll
    // confirm that its gradient has been transformed to the world frame.
    const SurfaceFaceIndex f0(0);
    const VolumeElementIndex t =
        GetTetForTriangle(contact->mesh_W(), f0, *mesh_S, X_WS.inverse());
    EXPECT_TRUE(
        CompareMatrices(contact->EvaluateGradE_M_W(f0),
...
GTEST_TEST(MeshIntersectionTest, ComputeContactSurfaceSoftRigidMoving) {
  ...
  // is at the center of the soft octahedron S.
  {
    ...
    const auto contact = ComputeContactSurfaceFromSoftVolumeRigidSurface(
        s_id, *s_pressure_S, X_WS, r_id, *r_mesh_R, X_WR, false);
...
GTEST_TEST(MeshIntersectionTest, ComputeContactSurfaceSoftRigidBVH) {
  ...
  auto contact_SR = ComputeContactSurfaceFromSoftVolumeRigidSurface(
      id_S, *field_S, X_WS, id_R, *surface_R, X_WR, false);
  ...
  auto bvh_contact_SR = ComputeContactSurfaceFromSoftVolumeRigidSurface(
      id_S, *field_S, bvh_S, X_WS, id_R, *surface_R, bvh_R, X_WR, false);

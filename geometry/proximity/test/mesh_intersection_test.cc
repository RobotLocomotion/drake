...
class SurfaceVolumeIntersectorTester {
 public:
  ...
  const std::vector<Vector3<T>>& ClipTriangleByTetrahedron(
      ...
      int face, const SurfaceMesh<double>& surface_N,
  ...
  bool IsFaceNormalAlongPressureGradient(
      ...
      const VolumeElementIndex& tet_index, int tri_index) {
...
template <typename T>
VolumeElementIndex GetTetForTriangle(const SurfaceMesh<T>& surface_S, int f,

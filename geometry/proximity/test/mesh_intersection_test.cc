 ...
 protected:
  ...
  void TestPositionDerivative(
    ...
    for (const auto& config : configurations) {
      ...
      const Vector3<AutoDiffXd> p_WS =
          math::InitializeAutoDiffFromValueMatrix(p_WS_d);
...
TEST_F(MeshMeshDerivativesTest, FaceNormalsWrtPosition) {
  ...
  auto evaluate_normals = [X_WR = convert_to_double(this->X_WR_)](
    ...
    for (SurfaceFaceIndex f(0); f < mesh_W.num_elements(); ++f) {
      ...
      EXPECT_TRUE(CompareMatrices(math::ExtractValueMatrixFromAutoDiff(tri_n_W),
                                  plane_n_W, 2 * kEps));
...
TEST_F(MeshMeshDerivativesTest, FaceNormalsWrtOrientation) {
  ...
  const Vector3d plane_n_W = math::ExtractValueMatrixFromAutoDiff(plane_n_W_ad);

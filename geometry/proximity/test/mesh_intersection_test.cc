...
/* This test fixture enables some limited testing of the autodiff-valued contact
 ...
 mesh_plane_intersection_test.cc. We've swapped plane with a triangle mesh and
 changed the call to the intersecting algorithm. Otherwise, it's line for line
 and comment for comment identical. We might want to consider refactoring it
 but for the fact that it only appears two places. If we end up suppoting other
 customized implementations between planar rigid objects and soft mesh, it
 would make sense to refactor. */
 ...
 protected:
  ...
  Vector3d GetEdgeDirInS(const Vector3d& p_SE) const {
    ...
    throw std::logic_error(
        "Querying for point I that isn't actually on a tet edge");
  }
...
TEST_F(MeshMeshDerivativesTest, VertexPosition) {
  /* The vertices of the contact surface *always* lie on the tri. Some of the
   ...
   - We treat centroid vertices differently. For centroids of triangles (where
     the intersecting polygon is a triangle), the centroid position and its
     derivative are simply:

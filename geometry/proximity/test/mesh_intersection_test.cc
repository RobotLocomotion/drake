...
class MeshIntersectionFixture : public testing::Test {
 protected:
  ...
  void VerifyComputeContactSurfaceFromSoftRigid(
    ...
    // The gradients for the pressure field of the soft mesh are expresssed in
...
TEST_F(MeshIntersectionFixture, SampleVolumeFieldOnSurface) {
  ...
  // Triangulate polygon.
  {
    SCOPED_TRACE("Trianguate polygon");
  ...
  {
    SCOPED_TRACE("Do not triangualte polygon");
...
TEST_F(MeshIntersectionFixture, TestComputeContactSurfaceSoftRigid) {
  ...
  for (const bool triangulate_polygon : {true, false}) {
    SCOPED_TRACE(fmt::format("triangulate_polygon = {}", triangulate_polygon));
...
TEST_F(MeshMeshDerivativesTest, FaceNormalsWrtOrientation) {
  ...
  for (const double theta : {0.0, M_PI / 6, M_PI / 2 * 0.9, M_PI / 2 * 0.99}) {
    ...
    auto surface = ComputeContactSurfaceFromSoftVolumeRigidSurface(
        ...
        *this->tri_mesh_R_, *this->bvh_R_, this->X_WR_, triangulate_polygon);

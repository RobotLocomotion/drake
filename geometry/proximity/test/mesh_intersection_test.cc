...
//      tetrahedron as being outside the mesh.
...
GTEST_TEST(MeshIntersectionTest, ComputeContactSurfaceSoftRigidMoving) {
  ...
  auto volume_mesh_S = OctahedronVolume<double>();
  auto soft_S = OctahedronPressureField<double>(volume_mesh_S.get());
  // Rigid pyramid surface R.
  auto id_R = GeometryId::get_new_id();
  auto rigid_R = PyramidSurface<double>();
  ...
  // is at the center of the soft octahedron S.
  {
    ...
    const auto contact_C_W = ComputeContactSurfaceFromSoftVolumeRigidSurface(
        id_S, *soft_S, X_WS, id_R, *rigid_R, X_WR);
    // TODO(DamrongGuoy): More comprehensive checks on the mesh of the contact
    //  surface. Here we only check the number of triangles.
    EXPECT_EQ(12, contact_C_W->mesh_W().num_faces());

    // Point Q is at the center vertex of S. Check that C also has a vertex
    // coincident with Q with the same pressure value.
    {
      const Vector3d p_SQ = Vector3d::Zero();
      const Vector3d p_WQ = X_WS * p_SQ;
      // Index of contact surface C's vertex coincident with Q.
      SurfaceVertexIndex vertex_C;
      ASSERT_TRUE(FindVertex(p_WQ, contact_C_W->mesh_W(), &vertex_C));
      const double pressure_C = contact_C_W->EvaluateE_MN(vertex_C);
      // Index of soft octahedron S's vertex coincident with Q.
      VolumeVertexIndex vertex_S;
      ASSERT_TRUE(FindVertex(p_SQ, soft_S->mesh(), &vertex_S));
      const double pressure_S = soft_S->EvaluateAtVertex(vertex_S);

      EXPECT_NEAR(pressure_C, pressure_S, kEps * pressure_S);
    }

    // Check that the pressure gradient on the contact surface C equals
    // the projection onto C of the pressure gradient of S.
    {
      ...
      auto pressure_C_W =
          dynamic_cast<const SurfaceMeshFieldLinear<double, double>*>(
              &contact_C_W->e_MN());
      ASSERT_TRUE(pressure_C_W != nullptr);
      // Use the pressure gradient vector on face 0 of C as the representative.
      // It is expressed in World frame.
      const Vector3d gradp_C_W =
          pressure_C_W->EvaluateGradient(SurfaceFaceIndex(0));

      // Use centroid Q of face 0 of C to find the tetrahedral element in S.
      const Vector3d p_WQ = contact_C_W->mesh_W().CalcCartesianFromBarycentric(
          SurfaceFaceIndex(0), Vector3d(1. / 3., 1. / 3., 1. / 3.));
      const Vector3d p_SQ = X_WS.inverse() * p_WQ;
      ...
      // Pressure gradient vector in S expressed in World frame.
      const Vector3d gradp_S_W = X_WS.rotation() * gradp_S_S;

      // Project ∇p_S on face 0 of C. The projection is calculated by
      // subtracting the normal component, i.e., ∇p_S - (∇p_S⋅n)n, where n is
      // the unit face normal of face 0 of C.
      const Vector3d nhat_C_W =
          contact_C_W->mesh_W().face_normal(SurfaceFaceIndex(0));
      const Vector3d project_gradp_S_W =
          gradp_S_W - gradp_S_W.dot(nhat_C_W) * nhat_C_W;

      EXPECT_TRUE(CompareMatrices(gradp_C_W, project_gradp_S_W, 1e-14,
  ...
  //
  {
    ...
    const auto X_WR = X_WS * X_SR;
    auto contact_C_W = ComputeContactSurfaceFromSoftVolumeRigidSurface(
        id_S, *soft_S, X_WS, id_R, *rigid_R, X_WR);
    // TODO(DamrongGuoy): More comprehensive checks on the mesh of the contact
    //  surface.  Here we only check the number of triangles.
    EXPECT_EQ(12, contact_C_W->mesh_W().num_faces());
    ...
    VolumeMesh<double>::Barycentric b_Q;
    ASSERT_TRUE(FindElement(p_SQ, soft_S->mesh(), &tetrahedron, &b_Q));
    const double pressure_S = soft_S->Evaluate(tetrahedron, b_Q);

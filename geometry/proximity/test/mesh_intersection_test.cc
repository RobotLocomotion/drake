...
class MeshIntersectionFixture : public testing::Test {
 protected:
  ...
  // grad_eS_S) of SampleVolumeFieldOnSurface() according to its control
  // parameter triangulate_polygon.
  ...
  static constexpr double kEps_ = std::numeric_limits<double>::epsilon();
};
  ...
  // Triangulate polygon.
  {
    ...
    SurfaceVolumeIntersector<double>().SampleVolumeFieldOnSurface(
        *field_S_, *bvh_mesh_S_, *surface_R_, *bvh_surface_R_, X_SR_,
        triangulate_polygon, &surface_S, &e_field, &grad_eS_S);

    VerifySampleVolumeFieldOnSurface(surface_S, e_field, grad_eS_S,
                                     triangulate_polygon);
  }
  // Do not triangulate polygon.
  {
    const bool triangulate_polygon = false;
    SurfaceVolumeIntersector<double>().SampleVolumeFieldOnSurface(
        *field_S_, *bvh_mesh_S_, *surface_R_, *bvh_surface_R_, X_SR_,
        triangulate_polygon, &surface_S, &e_field, &grad_eS_S);
...
void MeshIntersectionFixture::VerifySampleVolumeFieldOnSurface(
    const unique_ptr<SurfaceMesh<double>>& surface_S,
    const unique_ptr<SurfaceMeshFieldLinear<double, double>>& e_field,
    const vector<Vector3<double>>& grad_eS_S, bool triangulate_polygon) {
  ...
  EXPECT_NEAR(expect_area, surface_S->total_area(), kEps_);
  ...
  // triangle centroids.
  const double kEpsPressure = kEps_ * 1e10;
...
TEST_F(MeshIntersectionFixture, TestComputeContactSurfaceSoftRigid) {
  ...
  for (const bool triangulate_polygon : {true, false}) {
    ...
    auto contact_SR = ComputeContactSurfaceFromSoftVolumeRigidSurface(
        ...
        *bvh_surface_R_, X_WR, triangulate_polygon);
...
void MeshIntersectionFixture ::
    VerifyComputeContactSurfaceFromSoftRigid(
        const unique_ptr<ContactSurface<double>>& contact_SR,
        const unique_ptr<ContactSurface<double>>& contact_RS,
        const RigidTransformd& X_WS) {
  // Mesh invariants:

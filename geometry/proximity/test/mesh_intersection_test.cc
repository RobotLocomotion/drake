...
// Checks if two surfaces meshes are equivalent, i.e. where the elements are
// in the same domain and have the same face winding. Permutations in order
// are allowed. Note that this definition does not include all vertices, for
// example, dead vertices that aren't linked to any elements, as that is beyond
// the purpose of this test.
bool IsEquivalent(const SurfaceMesh<double>& mesh_a,
                  const SurfaceMesh<double>& mesh_b) {
  // The number of elements should be equal.
  if (mesh_a.num_elements() != mesh_b.num_elements()) {
    return false;
  }

  // Check that the elements are equivalent in that they consist of vertices in
  // the same domain. We do this by representing each element by a vector of its
  // 3 vertices, sorting them in a consistent manner, and comparing them across
  // both meshes. We also keep metadata of the element index so that we can
  // use the original 3 vertices to test face winding order later.
  using SurfaceFacePair = std::pair<SurfaceFaceIndex, std::vector<Vector3d>>;
  std::vector<SurfaceFacePair> element_vertices_a;
  std::vector<SurfaceFacePair> element_vertices_b;
  for (SurfaceFaceIndex i(0); i < mesh_a.num_elements(); ++i) {
    std::vector<Vector3d> vectors_a;
    std::vector<Vector3d> vectors_b;
    for (int j = 0; j < 3; ++j) {
      vectors_a.push_back(mesh_a.vertex(mesh_a.element(i).vertex(j)).r_MV());
      vectors_b.push_back(mesh_b.vertex(mesh_b.element(i).vertex(j)).r_MV());
    }
    element_vertices_a.emplace_back(i, vectors_a);
    element_vertices_b.emplace_back(i, vectors_b);
  }
  ...
  auto comp = [&vector_comp](const SurfaceFacePair& pair_a,
    ...
    // Sort according to the first vertex first, followed by the second and so
    // on if there is a tie.
  ...
  std::sort(element_vertices_a.begin(), element_vertices_a.end(), comp);
  std::sort(element_vertices_b.begin(), element_vertices_b.end(), comp);

  // Now we can compare matching elements across the two meshes.
  for (std::vector<SurfaceFacePair>::size_type i = 0;
       i < element_vertices_a.size(); ++i) {
    // Compare that the vertices are in the same domain.
    for (int j = 0; j < 3; ++j) {
      if (!CompareMatrices(element_vertices_a[i].second[j],
                           element_vertices_b[i].second[j])) {
        return false;
      }
    }
    // Since we kept track of the element index metadata, we can retrieve the
    // original elements and and check on face winding order by comparing the
    // cross product (normal) between the two elements.
    const Vector3d& face_normal_a =
        mesh_a.face_normal(element_vertices_a[i].first);
    const Vector3d& face_normal_b =
        mesh_b.face_normal(element_vertices_b[i].first);
    // Since they've been normalised to unit vectors we can check that the dot
    // product should be equal to 1.
    if (face_normal_a.dot(face_normal_b) != 1) {
      return false;
    }
  }

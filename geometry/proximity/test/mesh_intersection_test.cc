...
GTEST_TEST(MeshIntersectionTest, ComputeContactSurfaceSoftRigidMoving) {
  ...
  // is at the center of the soft octahedron S.
  {
    ...
    const auto contact = ComputeContactSurfaceFromSoftVolumeRigidSurface(
        id_S, *s_pressure_S, X_WS, id_R, *r_mesh_R, X_WR);
    ...
    // Check the pressure gradient on the contact surface C by comparing it to
    // the projection onto C of the pressure gradient of S.
    {
      ...
      const Vector3d grad_s_pressure_S =
          s_pressure_S->EvaluateGradient(tetrahedron);
      const Vector3d grad_s_pressure_W = X_WS.rotation() * grad_s_pressure_S;

      // Project ∇p from S onto face 0 of C. The projection is calculated by
      // subtracting the normal component, i.e., ∇p - (∇p⋅n)n, where n is
      // the unit face normal of face 0 of C.
      const Vector3d nhat_W =
          contact->mesh_W().face_normal(SurfaceFaceIndex(0));
      const Vector3d project_grad_s_pressure_W =
          grad_s_pressure_W - grad_s_pressure_W.dot(nhat_W) * nhat_W;

      EXPECT_TRUE(CompareMatrices(grad_c_pressure_W, project_grad_s_pressure_W,

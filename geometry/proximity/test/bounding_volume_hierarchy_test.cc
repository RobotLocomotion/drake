...
GTEST_TEST(AABBTest, TestVolume) {
  ...
  EXPECT_NEAR(zero_volume, 0, kDefaultTolerance);
...
GTEST_TEST(AABBTest, TestStretchBoundary) {
  ...
  ASSERT_GT(stretch_tolerance, kDefaultTolerance);
  ...
  // Expect the two Vector3d to be exactly equal down to the last bits.
  EXPECT_TRUE(CompareMatrices(aabb.half_width(), stretched));

...
class BVHTester {
 public:
  ...
  template <class MeshType>
  static int GetNumVerticesPerElement(
      const BoundingVolumeHierarchy<MeshType>& bvh) {
    return bvh.num_vertices_per_element_;
  }
};
...
TEST_F(BVHTest, TestComputeBoundingVolume) {
  ...
  // now encompass the whole sphere with a center of 0 and halfwidth of 1.5.
  for (int i = 1; i < bvh_.mesh().num_elements(); ++i) {
    element_centroids.push_back(
        std::pair(SurfaceFaceIndex(i), Vector3<double>(0.5, 0.5, 0.5)));
  }
  aabb = BVHTester::ComputeBoundingVolume<SurfaceMesh<double>>(
      bvh_, element_centroids.begin(), element_centroids.end());
  EXPECT_EQ(aabb.center(), Vector3<double>(0., 0., 0.));
  EXPECT_EQ(aabb.halfwidth(), Vector3<double>(1.5, 1.5, 1.5));
}
...
TEST_F(BVHTest, TestBuildBVTree) {
  ...
  check_node(bv_tree, 0, bv_tree->aabb.CalcVolume());
}

// Tests copy constructor.
GTEST_TEST(BoundingVolumeHierarchyTest, TestCopy) {
  ...
  BoundingVolumeHierarchy<SurfaceMesh<double>> original(mesh.get());

  // Copy constructor.
  BoundingVolumeHierarchy<SurfaceMesh<double>> copy(original);

  // Confirm that it's a copy.
  // TODO(tehbelinda): Update test expectations pending copy semantics.
  EXPECT_NE(&original.mesh(), &copy.mesh());
  EXPECT_EQ(&copy.mesh(), nullptr);
  EXPECT_NE(BVHTester::GetBVTree(original), BVHTester::GetBVTree(copy));
  EXPECT_EQ(BVHTester::GetBVTree(copy), nullptr);
  EXPECT_EQ(BVHTester::GetNumVerticesPerElement(original),
            BVHTester::GetNumVerticesPerElement(copy));
}

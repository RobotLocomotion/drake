...
// local axes out of A, the next 3 cases use the local axes out of B, and the
// remaining 9 cases use axes represented by the cross product of axes from
// A and B. We also test that it is robust for the case of parallel boxes.
GTEST_TEST(AABBTest, TestObbOverlap) {
  ...
  // To cover the cases of local axes out of A, we need to pose box β along
  // each axis. For example, in 2D and along the x-axis, they would look like:
  // z
  // ^     a
  // |  -----------------      *
  // |  |               |    *    *    b
  ...
  // -----------------------------------> x
  //
  // For our test, we want to place point Q to be equal to a point on α's far
  // edge, D, with a slight offset to verify that it either overlaps or doesn't
  // overlap on either side of the border.
  auto calculate_transform = [](const Aabb& a, const Aabb& b, const int axis,
                                const bool expect_overlap) -> RigidTransformd {
    // Let C be the frame at Bo that is aligned with A. Rotate b around the
    // other 2 axes to ensure we thoroughly cover all bits instead of
    // conveniently aligned boxes.
    const int axis1 = (axis + 1) % 3;
    const int axis2 = (axis + 2) % 3;
    RotationMatrixd R_CB =
        RotationMatrixd{AngleAxisd(M_PI / 5, Vector3d::Unit(axis1 % 3))} *
        RotationMatrixd{AngleAxisd(-M_PI / 5, Vector3d::Unit(axis2 % 3))};
    Vector3d p_BQ = b.center() - b.half_width();
    // Find the corner point Q that we want to place next to D.
    Vector3d p_CQ = R_CB * p_BQ;
    // Find the point D, which could be on either side of the border.
    const double offset = expect_overlap ? -0.01 : 0.01;
    Vector3d p_AD = a.half_width();
    p_AD[axis] += offset;
    // Pick some other point on the other axes, i.e. a quarter across, to
    // ensure we thoroughly cover all bits instead of simply using any
    // midpoints or corners.
    // z
    // ^
    // |  -------------
    // |  |     |  o  |
    ...
    p_AD[axis2] /= 2;
    p_AD += a.center();
    Vector3d p_AC = p_AD - p_CQ;
    return RigidTransformd(R_CB, p_AC);
  };

  a = Aabb(Vector3d(1, 2, 3), Vector3d(3, 4, 5));
  b = Aabb(Vector3d(1, 2, 3), Vector3d(3, 4, 5));
  for (int i = 0; i < 3; ++i) {
    X_AB = calculate_transform(a, b, i, false);
    EXPECT_FALSE(Aabb::HasOverlap(a, b, X_AB));
    X_AB = calculate_transform(a, b, i, true);
    EXPECT_TRUE(Aabb::HasOverlap(a, b, X_AB));
  }
  ...
  for (int i = 0; i < 3; ++i) {
    X_AB = calculate_transform(b, a, i, false).inverse();
    EXPECT_FALSE(Aabb::HasOverlap(a, b, X_AB));
    X_AB = calculate_transform(b, a, i, true).inverse();
    EXPECT_TRUE(Aabb::HasOverlap(a, b, X_AB));
  }

  // TODO(tehbelinda): Figure out remaining 9 cases.

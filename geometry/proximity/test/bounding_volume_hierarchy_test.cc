...
TEST_F(BVHTest, TestComputeBoundingVolume) {
  ...
  EXPECT_TRUE(
      CompareMatrices(aabb.half_width(), Vector3d(1.5, 1.5, 1.5), 1e-15));
  ...
  EXPECT_TRUE(CompareMatrices(aabb.center(), Vector3d(0., 0., 0.)));
  EXPECT_TRUE(CompareMatrices(aabb.half_width(), Vector3d(1., 2., 3.), 1e-15));
}
...
GTEST_TEST(AABBTest, TestVolume) {
  ...
  EXPECT_NEAR(aabb.CalcVolume(), 21.6, 1e-13);
  AABB zero_aabb = AABB(Vector3d(3, -4, 1.3), Vector3d(0, 0, 0));
  EXPECT_NEAR(zero_aabb.CalcVolume(), 0, 1e-13);
}

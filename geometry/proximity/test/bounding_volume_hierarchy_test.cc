...
GTEST_TEST(AABBTest, TestVolume) {
  ...
  EXPECT_NEAR(volume, 21.6, AabbTester::kTolerance * 70);
  ...
  EXPECT_NEAR(zero_volume, 0, AabbTester::kTolerance);
...
GTEST_TEST(AABBTest, TestBounds) {
  ...
  EXPECT_TRUE(CompareMatrices(
      aabb.upper(), Vector3d(1, 2.5, 3.7),
      AabbTester::kTolerance + std::numeric_limits<double>::epsilon()));
  EXPECT_TRUE(CompareMatrices(
      aabb.lower(), Vector3d(-3, 1.5, -1.7),
      AabbTester::kTolerance + std::numeric_limits<double>::epsilon()));
}

// Tests padding the boundary of the bounding box volume.
GTEST_TEST(AABBTest, TestPadBoundary) {
  Aabb aabb = Aabb(Vector3d(-1, 0.5, 1), Vector3d(1.2, 2.5, 0.3));
  Vector3d padded = Vector3d(1.2, 2.5, 0.3).array() + AabbTester::kTolerance;
  ...
  ASSERT_GT(padding, AabbTester::kTolerance);

...
TEST_F(BVHTest, TestComputeBoundingVolume) {
  std::vector<std::pair<SurfaceFaceIndex, Vector3d>> surface_centroids;
  ...
  for (SurfaceFaceIndex i(0); i < mesh_.num_elements(); ++i) {
    surface_centroids.emplace_back(i, Vector3d(0.5, 0.5, 0.5));
  }
  AABB aabb = BVHTester::ComputeBoundingVolume<SurfaceMesh<double>>(
      mesh_, surface_centroids.begin(), surface_centroids.end());
  EXPECT_TRUE(CompareMatrices(aabb.center(), Vector3d(0., 0., 0.)));
  EXPECT_TRUE(
      CompareMatrices(aabb.half_width(), Vector3d(1.5, 1.5, 1.5)));
...
TEST_F(BVHTest, TestBuildBVTree) {
  ...
  check_node = [&check_node, &element_indices, num_elements](
    ...
    if (depth < 3) {
      ...
      Vector3d node_bounds_upper =
          node.aabb.center() + node.aabb.half_width();
      Vector3d node_bounds_lower =
          node.aabb.center() - node.aabb.half_width();
      ...
      auto check_child = [&check_node, &node_volume, &node_bounds_upper,
        ...
        Vector3d child_bounds_upper =
            child.aabb.center() + child.aabb.half_width();
        Vector3d child_bounds_lower =
            child.aabb.center() - child.aabb.half_width();
        // Instead of comparing each element, we can use the coefficient-wise
        // min and max functions to ensure that the children are the min of the
        // upper bounds and the max of the lower bounds.
        EXPECT_TRUE(
...
GTEST_TEST(BoundingVolumeHierarchyTest, TestComputeCentroid) {
  ...
  auto volume_mesh =
      MakeEllipsoidVolumeMesh<double>(Ellipsoid(1., 2., 3.), 6);

...
TEST_F(BVHTest, TestBuildBVTree) {
  ...
  check_node = [&check_node, num_elements](BVNode<SurfaceMesh<double>>* node,
    ...
    if (depth < 3) {
      ...
      check_node(node->left, depth + 1, left_volume);
      int right_volume = node->right->aabb.volume();
      EXPECT_LE(right_volume, volume);
      check_node(node->right, depth + 1, right_volume);
    } else {
...
// Tests CloneAndSetMesh(). We use SurfaceMesh<double> to represent the type
// parameters.
GTEST_TEST(BoundingVolumeHierarchyTest, TestClone) {
  const Sphere sphere(1.5);
  const double edge_length = 3 * sphere.get_radius();
  auto mesh1 = std::make_unique<SurfaceMesh<double>>(
      MakeSphereSurfaceMesh<double>(sphere, edge_length));
  BoundingVolumeHierarchy<SurfaceMesh<double>> original(mesh1.get());
  SurfaceMesh<double> mesh2 = *mesh1;

  std::unique_ptr<BoundingVolumeHierarchy<SurfaceMesh<double>>> clone =
      original.CloneAndSetMesh(&mesh2);
  EXPECT_EQ(&mesh2, &clone->mesh());
  EXPECT_NE(
      nullptr,
      dynamic_cast<BoundingVolumeHierarchy<SurfaceMesh<double>>*>(clone.get()));
}

// Tests calculating the bounding box volume.
GTEST_TEST(AABBTest, TestVolume) {
  AABB aabb = AABB{Vector3<double>(-1, 2, 1), Vector3<double>(2, 0.5, 3)};
  EXPECT_EQ(aabb.volume(), 24);

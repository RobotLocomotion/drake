...
GTEST_TEST(AABBTest, TestObbOverlap) {
  ...
  auto calc_corner_transform =
      [](const Aabb& a, const Aabb& b, const int axis,
         const bool expect_overlap) -> RigidTransformd {
    // We want to compute X_AB such that B is posed relative to A as documented
    // above. We can do so by generating the rotation component, R_AB, such
    ...
    // Reality check that the center (p_Bo) and the maximum corner
    // (p_BoBqprime_A) are strictly increasing along the given axis.
    Vector3d p_Bo_A = R_AB * b.center();
    Vector3d p_BoBqprime_A = R_AB * (b.center() + b.half_width());
    DRAKE_DEMAND(p_Bo_A[axis] > p_BoBq_A[axis]);
    DRAKE_DEMAND(p_BoBqprime_A[axis] > p_Bo_A[axis]);
  ...
  // to pose box B so Bq is the uniquely closest point to A at a Point Af in the
  // interior of the +x × +z edge of box A. The rest of the box extends farther
  // along from the -x edge of box B (as suggested in the above illustration).
  ...
  auto calc_edge_transform = [](const Aabb& a, const Aabb& b, const int a_axis,
    ...
    // We want to compute X_AB such that B is posed relative to A as documented
    // above. We can do so by generating the rotation component, R_AB, such
    // that Bq lies on the minimum edge along the chosen axis, and we can solve

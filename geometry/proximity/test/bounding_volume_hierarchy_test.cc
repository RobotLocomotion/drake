...
class BVHTester {
 public:
  ...
  static bool HasOverlap(const Aabb& a, const Aabb& b,
    ...
    return BoundingVolumeHierarchy<MeshType>::HasOverlap(a, b, X_AB);
  }
};
...
TEST_F(BVHTest, TestCollide) {
  ...
  auto pairs = bvh_.GetCollidingPairs(copy, math::RigidTransformd::Identity());
  ...
  pairs = bvh_.GetCollidingPairs(separate, X_WV);
...
// Tests colliding the bvh with early exit and typed transform.
...
GTEST_TEST(AABBTest, TestObbOverlap) {
  ...
  EXPECT_TRUE(BVHTester::HasOverlap<SurfaceMesh<double>>(
      a, b, math::RigidTransformd::Identity()));

  // Parallel / touching the volume.
  ...
  EXPECT_FALSE(BVHTester::HasOverlap<SurfaceMesh<double>>(
      a, b, math::RigidTransformd::Identity()));

  // Separate from the volume using translation along y-axis.
  b = Aabb(Vector3d(0, 0, 0), Vector3d(1, 2, 1));
  EXPECT_FALSE(BVHTester::HasOverlap<SurfaceMesh<double>>(
      a, b, math::RigidTransformd{Vector3<double>{0, 4.1, 0}}));
  ...
  EXPECT_FALSE(BVHTester::HasOverlap<SurfaceMesh<double>>(
      ...
      math::RigidTransformd{
          math::RotationMatrixd{AngleAxisd(M_PI / 2, Vector3d::UnitX())},
          Vector3d{0, 0, 3.1}}));

  // Overlapping with offset center despite translation.
  b = Aabb(Vector3d(-2, 0, 0), Vector3d(1, 2, 1));
  EXPECT_TRUE(BVHTester::HasOverlap<SurfaceMesh<double>>(
      a, b, math::RigidTransformd{Vector3<double>{2.1, 0, 0}}));
  ...
  EXPECT_FALSE(BVHTester::HasOverlap<SurfaceMesh<double>>(
      a, b,
      math::RigidTransformd{math::RollPitchYawd{0, -M_PI / 4, M_PI / 4},
                            Vector3d{0, -3.1, 2.2}}));
  // Separate along a's x-axis and b's y-axis.
  EXPECT_FALSE(BVHTester::HasOverlap<SurfaceMesh<double>>(
      a, b,
      math::RigidTransformd{math::RollPitchYawd{M_PI / 4, 0, M_PI / 4},
                            Vector3d{0, -3.1, 2.2}}));
  ...
  EXPECT_FALSE(BVHTester::HasOverlap<SurfaceMesh<double>>(
      a, b,
      math::RigidTransformd{math::RollPitchYawd{M_PI / 4, M_PI / 4, 0},
                            Vector3d{2.2, 0, 2.2}}));

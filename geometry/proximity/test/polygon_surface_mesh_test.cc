...
/* Simple class for defining an expected polygon. It doesn't do math and assumes
 that the various quantities are defined consistently. */
struct Polygon {
  std::string name;
...
class PolygonSurfaceMeshTest : public ::testing::Test {
 protected:
  ...
  /* Returns a regular pentagon popsed in frame W. */
...
TYPED_TEST(PolygonSurfaceMeshTest, ZeroFaceNormal) {
  ...
  /* The mesh is a single triangle built on co-linear vertices. We leave the
...
/* In the case where a face has co-linear vertices, we want to confirm that we
...
TYPED_TEST(PolygonSurfaceMeshTest, TransformVertices) {
  ...
  ASSERT_NEAR(ExtractDoubleOrThrow(mesh_M.total_area()),
      ...
      << "Area should be invaraint to rigid vertex transformations";

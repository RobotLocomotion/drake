...
// +---------- -- -+---------- -- -+---------------+-----------------+
// |shrink.x() != 0|shrink.y() != 0|shrink.z() != 0| test parameter  |
// +---------- -- -+---------- -- -+---------------+-----------------+
// |       F       |       F       |       F       | Box(2, 2, 2)    |
// |       T       |       T       |       F       | Box(6, 4, 2)    |
// |       F       |       T       |       T       | Box(2, 6, 4)    |
...
// job; however, (T,T,T) is infeasible because at least one of the three
// dimensions has to shrink to zero.
GTEST_TEST(MakeBoxVolumeMeshWithMaTest, ConditionCoverage) {
  // |shrink.x() != 0|shrink.y() != 0|shrink.z() != 0|
  // |       F       |       F       |       F       |
  {
    const Box box(2., 2., 2.);
    const VolumeMesh<double> mesh = MakeBoxVolumeMeshWithMa<double>(box);

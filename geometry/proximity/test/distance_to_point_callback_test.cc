...
// Creates a MeshSdfCache with a single mesh registered for the given id. The
// contents of the cache are ignored unless we perform a query against a mesh
// or convex shape (with the indicated id).
MeshSdfCache MakeSdfCacheWithMesh(const GeometryId id) {
...
void TestScalarShapeSupport() {
  ...
  MeshSdfCache mesh_sdf_cache = MakeSdfCacheWithMesh(other_id);
...
GTEST_TEST(Callback, MeshAndConvex) {
  ...
  {
    MeshSdfCache mesh_sdf_cache = MakeSdfCacheWithMesh(mesh_id);

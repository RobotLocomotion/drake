...
// Converts a Drake Shape to an FCL CollisionGeometry for use in callback
// invocations. Only covers the shape types exercised by the test data above.
std::shared_ptr<fcl::CollisionGeometryd> MakeTestFclShape(const Shape& shape) {
  if (const auto* s = dynamic_cast<const Sphere*>(&shape)) {
    return make_shared<fcl::Sphered>(s->radius());
  }
  if (const auto* b = dynamic_cast<const Box*>(&shape)) {
    return make_shared<fcl::Boxd>(b->width(), b->depth(), b->height());
  }
  if (const auto* c = dynamic_cast<const Cylinder*>(&shape)) {
    return make_shared<fcl::Cylinderd>(c->radius(), c->length());
  }
  if (dynamic_cast<const HalfSpace*>(&shape)) {
    return make_shared<fcl::Halfspaced>(Vector3d::UnitZ(), 0);
  }
  throw std::logic_error("Unsupported shape type in SignedDistanceToPointTest");
}
...
struct SignedDistanceToPointTest
  ...
  vector<SignedDistanceToPoint<double>> RunCallback(double threshold) {
    ...
    EncodedData point_encoding(GeometryId::get_new_id(), true);
    point_encoding.write_to(&query_point);

    auto fcl_geom = MakeTestFclShape(*data.geometry);
    fcl::CollisionObjectd geometry_object(fcl_geom);
    EncodedData geom_encoding(id, true);
    geom_encoding.write_to(&geometry_object);
    ...
    Callback<double>(&query_point, &geometry_object, &callback_data, threshold);
    return distances;
  }
};
...
TEST_P(SignedDistanceToPointTest, SingleQueryPointSymbolic) {
  ...
  Callback<Expression>(&query_point, &geometry_object, &callback_data,
                       threshold);

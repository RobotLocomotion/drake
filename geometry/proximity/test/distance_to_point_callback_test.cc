...
class PointShapeAutoDiffSignedDistanceTester {
 public:
  ...
  ::testing::AssertionResult Test(const Vector3d& p_GN_G,
    ...
    Vector3<AutoDiffXd> p_WQ_ad = math::InitializeAutoDiffFromValueMatrix(p_WQ);
    ...
    const Vector3d grad_W_val =
        math::ExtractValueMatrixFromAutoDiff(result.grad_W);
    ...
    auto p_WQ_val_compare = CompareMatrices(
        math::ExtractValueMatrixFromAutoDiff(p_WQ_ad),
        math::ExtractValueMatrixFromAutoDiff(p_WQ_ad_expected), tolerance_);
    if (!p_WQ_val_compare) {
      if (error) failure << "\n";
      error = true;
    ...
    if (is_grad_W_unique) {
      auto p_WQ_derivative_compare = CompareMatrices(
          math::ExtractGradientMatrixFromAutoDiff(p_WQ_ad),
          math::ExtractGradientMatrixFromAutoDiff(p_WQ_ad_expected),
          tolerance_);

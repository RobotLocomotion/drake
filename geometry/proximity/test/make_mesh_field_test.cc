...
GTEST_TEST(MakeVolumeMeshPressureFieldTest, Double) {
...
GTEST_TEST(MakeVolumeMeshPressureFieldTest, AutoDiffXd) {
  const VolumeMesh<AutoDiffXd> non_convex_mesh {
      {
          {5, 4, 1, 2},
          {5, 4, 2, 3},
          {5, 4, 3, 1},
  ...
  const AutoDiffXd kHydroelasticModulus = 1e7;
  const double kEps = 1e-14;
  VolumeMeshFieldLinear<AutoDiffXd, AutoDiffXd> field =
      MakeVolumeMeshPressureField(&non_convex_mesh, kHydroelasticModulus);

  // All vertices are on the boundary except the last vertex.
  for (int v = 0; v < 5; ++v) {
    SCOPED_TRACE(fmt::format("v: {}", v));
    EXPECT_NEAR(field.EvaluateAtVertex(v).value(), 0, kEps);
  }
  EXPECT_NEAR(field.EvaluateAtVertex(5).value(), kHydroelasticModulus.value(),
              kEps * kHydroelasticModulus.value());
}

...
TYPED_TEST(MakeVolumeMeshPressureFieldTest, PressureOnNonConvexMesh) {
  ...
  // The first five vertices are on the boundary, so they got zero pressure.
  for (int v = 0; v < 5; ++v) {
...
GTEST_TEST(MakeVolumeMeshPressureFieldTest, NoInteriorVertex) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      "MakeVolumeMeshPressureField: the computed max "
      "distance to boundary among all mesh vertices is "
      "non-positive. Perhaps the mesh lacks interior "
      "vertices.");
}

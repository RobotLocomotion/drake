...
// Test fixture class for testing PCA with a unique solution, so it is
// invariant under rigid transform. We use a rectangular box that is not
// unifrom (width < depth < height). M is the canonical frame of the box and
// also its mesh.
...
// Test PCA of the box both at the standard pose and at a general pose.
TEST_F(ObbMakerTestRectangularBox, CalcOrientationByPCA) {
  ...
  // under rigid transform from frame M to frame F. We can do this because this
  // example has the unique PCA solution.
...
// 1. Test ObbMaker::CalcOrientationByPCA() and ObbMaker::CalcOrientedBox().
// 2. Test an example with non-unique PCA solution with eigenvalue of
//    multiplicity 2.
//
...
TEST_F(ObbMakerTestTriangle, CalcOrientationByPCA) {
  ...
  //    preferred line of direction. PCA solution is not unique; it can be any
...
// Test fixture for computing an oriented bounding box of a vertex set of an
// octahedron, which has no unique PCA solution. Its covariance matrix has
// one single eigenvalue with triple multiplicity. There are two test
// functions to demonstrate:
//   1. Triple multiplicty of PCA solution causes different orientation under
//      rigid transform.
//   2. Two solutions of PCA problem can give very different volume of
//      oriented bounding boxes, i.e., our algorithm is not always optimal.
//
...
TEST_F(ObbMakerTestOctahedron, ObbMakerCompute) {
  ...
  // Computationally solution for this example is unstable. A rigid transform
  // from frame M to frame F below can perturb the new solution significantly.
  // As a result, the OBB's volume will shrink about 66%.
...
// TODO(DamrongGuoy): Update or remove this tests if we improve the
//  algorithm. Currently our oriented box can be worse than an axis-aligned box.

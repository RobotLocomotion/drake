...
// Verifies the increasing order (or non-decreasing order for repeated values)
// of eigenvalues from Eigen::SelfAdjointEigenSolver.
// ObbMaker::CalcOrientationByPca() uses eigenvectors from Eigen to set up
// the basis vectors corresponding to the principal components, whose correct
// order depend on the order of eigenvalues. Furthermore, a repeated
// eigenvalue is an indicator of arbitrary basis vectors.
GTEST_TEST(ObbMakerTest, VerifyEigenvalues) {
  // Empirical tolerance from solving eigenvalue problems, about 2e-14.
  ...
  // Covariance matrix of 8 vertices of the box [-1,-1]x[-2,2]x[-3,3] has
  // distinct eigenvalues in increasing order: 1, 2², 3².
  {
  ...
  // UnitY(), UnitZ()} has eigenvalues 0, 1/3, 1/3.
  {
    Matrix3d covariance_M = Matrix3d::Zero();
  ...
  // eigen value repeated three times: 2, 2, 2.
  {
    Matrix3d covariance_M = Matrix3d::Zero();
  ...
  // Covariance matrix of 6 vertices of 2X stretch in Mx direction of the
  // previous octahedron has one eigen value repeated two times followed by
  // the largest eigenvalue: 2, 2, 2²*2.
  {
    Matrix3d covariance_M = Matrix3d::Zero();
    // Use std::vector<> here to instruct compiler to convert every entry to
    // Vector3d. Strictly speaking, Vector3d and -Vector3d have different
...
GTEST_TEST(ObbMakerTest, TestOptimizeObbVolume) {
  ...
  // The elliposid's canonical frame is E, and the general frame is M.

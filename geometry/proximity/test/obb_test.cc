...
// rectangular box that is not unifrom (width < depth < height).
...
TEST_F(ObbMakerTestRectangularBox, CalcOrientationByPca) {
  ...
  EXPECT_GT(R_MB.matrix().determinant(), 0.);
  ...
  // Confirm the basis is right-handed.
  EXPECT_GT(R_FB.matrix().determinant(), 0.);
}

// The purpose of this test fixture is:
// 1. Test coverage of ObbMaker::CalcOrientationByPca() and

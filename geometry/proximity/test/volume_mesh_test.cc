...
class ScalarMixingTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    vertices.emplace_back(math::InitializeAutoDiffFromValueMatrix(
...
TEST_F(ScalarMixingTest, CalcBarycentric) {
  ...
  p_WQ_d /= 4;
  const Vector3<AutoDiffXd> p_WQ_ad =
      math::InitializeAutoDiffFromValueMatrix(p_WQ_d);
...

  {
    ...
    EXPECT_TRUE(CompareMatrices(math::ExtractValueMatrixFromAutoDiff(b),
                                b_expected, kEps));
  }

  {
    ...
    EXPECT_TRUE(CompareMatrices(math::ExtractValueMatrixFromAutoDiff(b1),
                                b_expected, kEps));
...
TEST_F(ScalarMixingTest, CalcGradientVectorOfLinearField) {
...

  {
    ...
    EXPECT_TRUE(CompareMatrices(math::ExtractValueMatrixFromAutoDiff(grad_W),
...

  {
    ...
    EXPECT_TRUE(CompareMatrices(math::ExtractValueMatrixFromAutoDiff(grad_W1),
                                grad_W_expected, kEps));

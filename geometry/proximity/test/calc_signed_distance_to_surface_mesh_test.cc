...
// FeatureNormalSet from a non-watertight mesh gives bad normals for the
// inside-outside test.
GTEST_TEST(FeatureNormalSet, BadMesh) {
  // This non-watertight mesh is a surface mesh of a tetrahedron with
  // vertex v4 coincident with vertex v1. The constructed FeatureNormalSet will
  // give a bad normal at the coincident edges v1v2 and v2v4.
  //
  //              Mz
  //              ┆
  //           v3 ●
  ...
  const TriangleSurfaceMesh<double> mesh_M(
      // The triangles have outward face windings.
      {SurfaceTriangle{0, 2, 4}, SurfaceTriangle{0, 1, 3},
       SurfaceTriangle{0, 3, 2}, SurfaceTriangle{1, 2, 3}},
      {Vector3d::Zero(), Vector3d::UnitX(), Vector3d::UnitY(),
       Vector3d::UnitZ(),
       // Vertex v4 is coincident with vertex v1.
       Vector3d::UnitX()});

  const FeatureNormalSet dut(mesh_M);
  // This query point Q is outside the tetrahedron with its nearest point
  // at the middle of edge v2v4. However, the inside-outside test would
  // classify it as inside. Notice that it is slightly above the plane of
  // the triangle v0v2v4.
  const Vector3d p_MQ(10, 10, 0.1);
  // N is the nearest point of Q in the edge v2v4.
  const Vector3d p_MN(0.5, 0.5, 0);
  const Vector3d p_NQ_M = p_MQ - p_MN;
  // This negative dot product incorrectly classifies Q as being inside.
  EXPECT_LT(p_NQ_M.dot(dut.edge_normal({2, 4})), 0);
}
...
TEST_F(CalcSquaredDistanceToTriangleTest, Inside) {
  // The query point Q projects to C inside the triangle.
  //
  //                   Fy
  //                   ┆
  //                   0.1          v0
  ...
  // The projection C is the closest point.
  const Vector3d p_MC = X_MF_ * Vector3d(0.04, 0.02, 0);
  ...
  EXPECT_NEAR(dut.squared_distance, (p_MQ - p_MC).squaredNorm(), kEps_);
  EXPECT_TRUE(CompareMatrices(dut.closest_point, p_MC, kEps_));
  EXPECT_TRUE(CompareMatrices(dut.feature_normal, nhat_M, kEps_));
}

TEST_F(CalcSquaredDistanceToTriangleTest, OutsideNearEdge) {
  // The query point Q projects to Q' outside the triangle nearest to
  // the edge v1v2.  C is the closest point on the edge v1v2.
  ...
  // edge v1v2 at the closest point C(0.04, 0, 0) in frame F.
  const Vector3d p_MC = X_MF_ * Vector3d(0.04, 0, 0);
...
// Test special cases that the query point Q is in an edge or at a vertex.
class CalcSquaredDistanceToTriangleSpecialCases : public ::testing::Test {
 public:
  CalcSquaredDistanceToTriangleSpecialCases()
      :  // To create the special cases, many things have to line up. We
         // found this set up empirically.
         //
         //                Mz
         //                ┆
         //             v1 ●┄┄┄┄┄┄┄0.1┄┄┄┄ My
...
TEST_F(CalcSquaredDistanceToTriangleSpecialCases, PointInEdge) {
  //
  //                   My
  //                   ┆
  //                   0.1          v0
  ...
  const Vector3d p_MQ = Vector3d(0.04, 0, 0);
  const SquaredDistanceToTriangle dut =
      CalcSquaredDistanceToTriangle(p_MQ, 0, mesh_M_, normal_set_M_);
...
TEST_F(CalcSquaredDistanceToTriangleSpecialCases, PointAtVertex) {
  //
  //                   My
  //                   ┆
  //                   0.1          v0
  ...
  //
  // Q is at v1.
  const Vector3d p_MQ = mesh_M_.vertex(1);
  const SquaredDistanceToTriangle dut =
      CalcSquaredDistanceToTriangle(p_MQ, 0, mesh_M_, normal_set_M_);

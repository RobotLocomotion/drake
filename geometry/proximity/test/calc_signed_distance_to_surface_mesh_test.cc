...
TEST_F(FeatureNormalSetTest, EdgeNormalIsAverageFaceNormal) {
  ...
  EXPECT_TRUE(
      CompareMatrices(dut.edge_normal({1, 2}), kExpectEdgeNormal, kEps_));
}
  ...
  const Vector3d kExpectVertexNormal =
      (M_PI_4 * mesh_M_.face_normal(0) + M_PI_4 * mesh_M_.face_normal(1) +
       (M_PI / 3) * mesh_M_.face_normal(3))
          .normalized();
  EXPECT_TRUE(
      CompareMatrices(dut.vertex_normal(1), kExpectVertexNormal, kEps_));
}
...
TEST_F(CalcSignedDistanceToSurfaceMeshTest, PositiveTriangle) {
  ...
  EXPECT_TRUE(CompareMatrices(d.nearest_point, p_MC, kEps_));
  EXPECT_NEAR(d.signed_distance, 0.25, kEps_);
  EXPECT_TRUE(CompareMatrices(d.gradient, n_M, kEps_));
}

...
template <typename T>
PolygonSurfaceMesh<T> MakePolygonSurfaceMesh() {
...
DeformableRigidContactSurface<T> MakeDeformableRigidContactSurface() {
  return DeformableRigidContactSurface<T>(
      MakePolygonSurfaceMesh<T>(), {{kSignedDistance}},
...
TYPED_TEST(DeformableRigidContactSurfaceTest, ConstructAndRelease) {
  ...
  EXPECT_TRUE(contact_surface_mesh_W.Equal(MakePolygonSurfaceMesh<T>()));
}
...
TYPED_TEST(DeformableRigidContactSurfaceTest, RotationMatrices) {
  ...
  const PolygonSurfaceMesh<double>& contact_surface_mesh_W = std::get<0>(data);

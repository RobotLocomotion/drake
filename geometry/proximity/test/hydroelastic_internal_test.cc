...
GTEST_TEST(Hydroelastic, GeometriesPopulationAndQuery) {
  ...
  // An otherwise valid id used to access the *wrong* representation type
  // throws.
  EXPECT_THROW(geometries.soft_geometry(rigid_id), std::out_of_range);
  EXPECT_THROW(geometries.rigid_geometry(soft_id), std::out_of_range);
  EXPECT_THROW(geometries.soft_geometry(bad_id), std::out_of_range);
  EXPECT_THROW(geometries.rigid_geometry(bad_id), std::out_of_range);

  // Ids that report the correct type, successfully access the appropriate
  // representation.
  EXPECT_NO_THROW(geometries.soft_geometry(soft_id));
  EXPECT_NO_THROW(geometries.rigid_geometry(rigid_id));
}

// Tests the classification of proximity properties.
GTEST_TEST(Hydroelastic, Classify) {
  // Case: props with no declared "stiffness" is, by definition, an unclassified
  // hydroelastic type.
  EXPECT_EQ(Classify(ProximityProperties()), HydroelasticType::kUndefined);

  // Case: Presence of elasticity is insufficient to trigger hydroelastic.
  {
  ...
  // Case: missing elasticity is a classification error.
  {
  ...
  // Case: Infinite "stiffness" should report as rigid.
  {
  ...
  // Case: Really, really huge, but not infinite "stiffness" should report as
  // soft.
  {
    ProximityProperties props(trigger_hydro);
    props.AddProperty(kMaterialGroup, kElastic,
  ...
  // Case: Microscopically small stiffness should report as soft.
  {
    ProximityProperties props(trigger_hydro);
  ...
  // Case: Zero stiffness should throw an error.
  {
    ProximityProperties props(trigger_hydro);
  ...
  // Case: Negative stiffness should throw an error.
  {
    ProximityProperties props(trigger_hydro);
...
ProximityProperties rigid_properties(double edge_length = 0.1) {
...
TEST_F(HydroelasticRigidGeometryTest, UnsupportedRigidShapes) {
  ...
  EXPECT_EQ(MakeRigidRepresentation(Cylinder(1, 1), props), nullopt);

  EXPECT_EQ(MakeRigidRepresentation(HalfSpace(), props), nullopt);
  ...
  EXPECT_EQ(MakeRigidRepresentation(Convex(obj, 1.0), props), nullopt);

  EXPECT_EQ(MakeRigidRepresentation(Mesh(obj, 1.0), props), nullopt);
}
...
TEST_F(HydroelasticRigidGeometryTest, Sphere) {
  ...
  optional<RigidGeometry> sphere =
      MakeRigidRepresentation(sphere_spec, props);
  EXPECT_NE(sphere, nullopt);
...
ShapeType make_default_shape() {
  throw std::logic_error(
      "Running HydroelasticRigidGeometryErrorTests for an unsupported shape "
      "type");
}
...
// @param property_name  The name of the property (in the "hydroelastic" group)
//                       to be tested.
// @param compliance     A string representing the compliance being made.
...
//                       properties are generally tested in some sequence. This
...
void TestPropertyErrors(
    const ShapeType& shape_spec, const char* property_name,
    const char* compliance,
    function<void(const ShapeType&, const ProximityProperties&)> maker,
    optional<ValueType> bad_value, const ProximityProperties& props = {}) {
  ...
  // Error case: property value is wrong type.
  {
    ...
    wrong_value.AddProperty(kHydroGroup, property_name, "10");
    DRAKE_EXPECT_THROWS_MESSAGE(
  ...
  if (bad_value.has_value()) {
    ...
    negative_value.AddProperty(kHydroGroup, property_name, *bad_value);
    DRAKE_EXPECT_THROWS_MESSAGE(
...
// TODO(SeanCurtis-TRI): Add Cylinder, Mesh, and Convex as they become supported
//  by either rigid or soft geometries.
...
TYPED_TEST_CASE_P(HydroelasticRigidGeometryErrorTests);
...
REGISTER_TYPED_TEST_CASE_P(HydroelasticRigidGeometryErrorTests,
                           BadCharacteristicLength);
typedef ::testing::Types<Sphere, Box> RigidErrorShapeTypes;
INSTANTIATE_TYPED_TEST_CASE_P(My, HydroelasticRigidGeometryErrorTests,
...
TEST_F(HydroelasticSoftGeometryTest, UnsupportedSoftShapes) {
  ...
  EXPECT_EQ(MakeSoftRepresentation(Box(1, 1, 1), props), nullopt);

  EXPECT_EQ(MakeSoftRepresentation(Cylinder(1, 1), props), nullopt);

  // Note: the file name doesn't have to be valid for this (and the Mesh) test.
  const std::string obj = "drake/geometry/proximity/test/no_such_files.obj";
  EXPECT_EQ(MakeSoftRepresentation(Convex(obj, 1.0), props), nullopt);

  EXPECT_EQ(MakeSoftRepresentation(Mesh(obj, 1.0), props), nullopt);
}
...
TYPED_TEST_CASE_P(HydroelasticSoftGeometryErrorTests);

TYPED_TEST_P(HydroelasticSoftGeometryErrorTests, BadCharacteristicLength) {
  using ShapeType = TypeParam;
  ShapeType shape_spec = make_default_shape<ShapeType>();
  TestPropertyErrors<ShapeType, double>(
      shape_spec, kRezHint, "soft",
      [](const ShapeType& s, const ProximityProperties& p) {
...
REGISTER_TYPED_TEST_CASE_P(HydroelasticSoftGeometryErrorTests,
                           BadCharacteristicLength);
typedef ::testing::Types<Sphere> SoftErrorShapeTypes;
INSTANTIATE_TYPED_TEST_CASE_P(My, HydroelasticSoftGeometryErrorTests,
                              SoftErrorShapeTypes);

...
// CompliantGeometry can represent either a mesh or a half space (and in the future,
...
// fact that CompliantGeometry's management of these exclusive types is handled by
// std::variant and the move/copy semantics of the underlying data types
// (already tested). If CompliantGeometry changes its implementation details, this
...
GTEST_TEST(CompliantGeometryTest, TestCopyMoveAssignConstruct) {
  ...
  const CompliantGeometry original(CompliantMesh(std::move(mesh), std::move(pressure)));

  // In all of the following tests we are only looking for evidence of
  // copying/moving. Since CompliantGeometry relies on the copy/move semantics of the
  ...
  // Test copy-assignment operator.
  {
    ...
    // Then, change it to a CompliantGeometry representing a mesh by copy-assignment.
  ...
  // ownership.
  {
    CompliantGeometry start(original);  // Assume the copy constructor is correct.
    ...
    // Test move-assignment operator.
    // Initialize `move_assigned` as a CompliantGeometry representing a half spce.
    // Then, change it to a CompliantGeometry of a compliant mesh by move-assignment
...
GTEST_TEST(Hydroelastic, GeometriesPopulationAndQuery) {
  ...
  EXPECT_EQ(geometries.hydroelastic_type(compliant_id), HydroelasticType::kCompliant);
...
GTEST_TEST(Hydroelastic, RemoveGeometry) {
  ...
  AddCompliantHydroelasticProperties(1.0, 1e8, &compliant_properties);
  geometries.MaybeAddGeometry(Sphere(0.5), compliant_id, compliant_properties);
  ASSERT_EQ(geometries.hydroelastic_type(compliant_id), HydroelasticType::kCompliant);
  ...
  // untouched.
  {
    ...
    ASSERT_EQ(copy.hydroelastic_type(compliant_id), HydroelasticType::kCompliant);
  }

  // Case 3: Removing a compliant geometry has no effect on anything else. We copy
  // the geometries to make sure that there is always something else to remain
  // untouched.
  {
    Geometries copy{geometries};
    DRAKE_EXPECT_NO_THROW(copy.RemoveGeometry(compliant_id));
    ASSERT_EQ(copy.hydroelastic_type(compliant_id), HydroelasticType::kUndefined);
...
//   - Invalid value (maybe)
//
// This is sufficiently generic to test both kinds of geometry (rigid and compliant)
...
// Smoke test for shapes that are *known* to be unsupported as compliant objects.
// NOTE: This will spew warnings to the log.
TEST_F(HydroelasticCompliantGeometryTest, UnsupportedCompliantShapes) {
  ProximityProperties props = compliant_properties();

  EXPECT_EQ(MakeCompliantRepresentation(MeshcatCone(2, 1, 1), props), std::nullopt);
}
...
TEST_F(HydroelasticCompliantGeometryTest, HalfSpace) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      "Cannot create compliant HalfSpace; missing the .*slab_thickness.* property");
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      half_space->bvh(), "CompliantGeometry::bvh.* cannot be invoked .* half space");
}
...
// and define a pressure field. This actively excludes Mesh because compliant Mesh
// is not currently supported for hydroelastic contact.
// (See the `CompliantErrorShapeTypes` declaration below.)
// It should include every *other* supported compliant shape type. For HalfSpace,

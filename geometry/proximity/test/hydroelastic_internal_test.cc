...
// is made, and samples the representation to look for evidence of it being
// the *right* representation.
TEST_F(HydroelasticRigidGeometryTest, Mesh) {
  std::string file =
    FindResourceOrThrow("drake/geometry/test/non_convex_mesh.obj");
  const double scale = 1.1;
  // Characteristic length is not used.
  ProximityProperties props = rigid_properties(1.0);
  ...
  // Smoke test the surface mesh. Depend on the specific content of the obj
  // file.

...
GTEST_TEST(Hydroelastic, RemoveGeometry) {
  ...
  // Case 1: Remove a geometry that has no representation is  no-op.
  ...
  // Case 2: Removing a rigid geometry has no effect on anything else.
  DRAKE_EXPECT_NO_THROW(geometries.RemoveGeometry(rigid_id));
  ASSERT_EQ(geometries.hydroelastic_type(rigid_id),
            HydroelasticType::kUndefined);

  // Case 3: Removing a soft geometry has no effect on anything else.
  DRAKE_EXPECT_NO_THROW(geometries.RemoveGeometry(soft_id));
  ASSERT_EQ(geometries.hydroelastic_type(soft_id),
            HydroelasticType::kUndefined);
}
...
TEST_F(HydroelasticRigidGeometryTest, Sphere) {
  ...
  optional<RigidGeometry> sphere = MakeRigidRepresentation(sphere_spec, props);

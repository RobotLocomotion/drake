...
TEST_F(HydroelasticSoftGeometryTest, Sphere) {
  ...
  // property in the following ways:
  {
      ...
      // to sparse. So, sphere 2 must have 4X the tetrahedra as sphere 1.
...

  {
    ...
    // Starting with sphere 1's properties, we'll set the property to be an
    // arbitrary class that *cannot* be implicitly cast to a bool. Should throw.
    ProximityProperties dense_properties(properties1);

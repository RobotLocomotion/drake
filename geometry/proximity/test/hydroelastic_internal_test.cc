...
TEST_F(HydroelasticRigidGeometryTest, Cylinder) {
  ...
  for (SurfaceVertexIndex v(0); v < mesh.num_vertices(); ++v) {
    const double x = mesh.vertex(v).r_MV().x();
    const double y = mesh.vertex(v).r_MV().y();
    const double z = mesh.vertex(v).r_MV().z();
    ...
    ASSERT_LE(x * x + y * y, pow(radius, 2.0) + 1e-15);
    ASSERT_LE(z * z, pow(length / 2.0, 2.0) + 1e-15);
  }
}

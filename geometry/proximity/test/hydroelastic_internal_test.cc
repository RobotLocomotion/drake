...
TEST_F(HydroelasticRigidGeometryTest, Convex) {
...

  {
    ...
    // Even though we don't currently claim to support glTF files for proximity,
    // this is proof that we're using the convex hull as that would be the
    // *only* way this test would pass.
    // TODO(SeanCurtis-TRI): Create cube_with_hole.vtk to complete the set.
    const std::string file =

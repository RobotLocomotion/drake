...
class CollisionFilterTest : public ::testing::Test {
 protected:
  ...
  /* Creates a CollisionFilter with all the same geometries as found in
   `filter`, but with all pairs *unfiltered*. */
...
/* Exercise transient declaration API. This entails five different methods that
 ...
 function, we want to test certain properties (as listed below):
  ...
  E Flatten()
    1 Flattenging an "only-persitent" filter state makes no difference.
    ...
    4 Flatten preserves resultant filter state (i.e., the set of filtered
      pairs is same before and after filtering).
...
TEST_F(CollisionFilterTest, TransientDeclarations) {
  ...
  filter.Apply(Decl().ExcludeBetween(GeometrySet{B}, GeometrySet{C, D, E}),
               extract, !permanent);
  expected_filters.emplace(make_pair(D, E));
  expected_filters.emplace(make_pair(B, C));
  expected_filters.emplace(make_pair(B, D));
  expected_filters.emplace(make_pair(B, E));
  ...
  expected_filters.emplace(make_pair(C, D));
  expected_filters.erase(expected_filters.find(make_pair(B, D)));
  ASSERT_TRUE(is_expected(filter, expected_filters));
  ...
  expected_filters.emplace(make_pair(A, B));
  expected_filters.emplace(make_pair(A, C));
  expected_filters.erase(expected_filters.find(make_pair(B, C)));
  expected_filters.erase(expected_filters.find(make_pair(B, E)));
  ASSERT_TRUE(is_expected(filter, expected_filters));
  ...
  expected_filters.erase(expected_filters.find(make_pair(A, C)));
  expected_filters.emplace(make_pair(B, C));
  ASSERT_TRUE(is_expected(filter, expected_filters));
  ...
  expected_filters.erase(expected_filters.find(make_pair(A, B)));
  ASSERT_TRUE(is_expected(filter, expected_filters));

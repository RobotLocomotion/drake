...
class ContactSurfaceTester {
 public:
  ...
  TriangleSurfaceMesh<T>& mutable_tri_mesh_W() const {
    return const_cast<TriangleSurfaceMesh<T>&>(surface_.tri_mesh_W());
  }
...
ContactSurface<typename MeshType::ScalarType> TestContactSurface(
  ...
  auto e_field = MakeField(move(e_values), *surface_mesh);
...
// Tests copy constructor of ContactSurface. The logic doesn't depend on mesh
...
// Tests the equality comparisons. The logic doesn't depend on mesh
// representation or scalar type, so we simply test it against one mesh type.
GTEST_TEST(ContactSurfaceTest, TestEqual) {
  ...
  ContactSurfaceTester<double>(surface1)
      .mutable_tri_mesh_W()
      .ReverseFaceWinding();

...
unique_ptr<MeshFieldLinear<typename MeshType::ScalarType, MeshType>> MakeField(
  ...
  if constexpr (std::is_same_v<MeshType, TriangleSurfaceMesh<T>>) {
    ...
    // provided. So, we'll create the equivalent triangle mesh and field an
    // ask for *its* gradients.
...
ContactSurface<typename MeshType::ScalarType> TestContactSurface(
  ...
  vector<Vector3<T>> grad_e(surface_mesh->num_elements(), Vector3<T>{1, 1, 1});
  ...
  if (test_return_value) {
    ...
    // We'll confirm the field properly references its field. The previous

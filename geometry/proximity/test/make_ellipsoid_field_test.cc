...
GTEST_TEST(MakeEllipsoidFieldTest, WithMargin) {
  ...
  // If `p` is a point along one of the axes, it returns the index of the
  // non-zero coordinate. Otherwise it returns a negative value.
  auto point_on_axis = [](const Vector3d sizes, const Vector3d& p) {
    for (int i = 0; i < 3; ++i) {
      const int j = (i + 1) % 3;
      const int k = (i + 2) % 3;
  ...
  for (int i = 0; i < mesh.num_vertices(); ++i) {
    ...
    const int k = point_on_axis(radii, v);
    if (k >= 0) {

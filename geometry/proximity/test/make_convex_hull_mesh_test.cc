...
GTEST_TEST(MakeConvexHullMeshTest, GltfMesh) {
  ...
  const fs::path bin_src =
      FindResourceOrThrow("drake/geometry/test/cube_with_hole.bin");
  const fs::path bin_tgt = dir_path / bin_src.filename();
  fs::copy_file(bin_src, bin_tgt);

  const fs::path gltf_src =
      FindResourceOrThrow("drake/geometry/test/cube_with_hole.gltf");
  const fs::path gltf_tgt = dir_path / gltf_src.filename();
  {
    std::ifstream in_gltf(gltf_src);
    ...
    std::ofstream out_gltf(gltf_tgt);
    DRAKE_DEMAND(out_gltf.is_open());
    out_gltf << cube;
  }

  const PolyMesh dut = MakeConvexHull(Mesh(gltf_tgt, scale));

...
/* This test fixture enables some limited testing of the autodiff-valued contact
 ...
 boundary. The function TestQuantity() will compute multiple contact surfaces
 by varying the relative orientation of mesh and and half space. (See notes in
 that function.) It uses AutoDiffXd to automatically compute the derivatives of
 all calculations *with respect to the position of frame R's origin*. This is
 selected because it's a simple derivative to reason about.
 ...
 TestQuantity() invokes that function for each unique configuration. */
class MeshHalfSpaceDerivativesTest : public ::testing::Test {
 protected:
  ...
  /* Test for a particular quantity of the contact surface with multiple
   ...
   of the mesh penetrates into the half space. For each configuration, the
  ...
  void TestQuantity(const std::function<void(const ContactSurface<AutoDiffXd>&,
    ...
    for (const double theta :
      ...
      CategorizeVertices(surface->mesh_W(), X_WR, theta);
  ...
  void CategorizeVertices(const SurfaceMesh<AutoDiffXd>& mesh_W,
                          const RigidTransform<AutoDiffXd>& X_WR,
                          double theta) {
    /* This function relies very directly on the exact structure of the mesh and
     the very heavily curated poses X_WH and W_WR. X_WH is the identity, and
     X_WR has a limited rotation around Wy with an displacement of Ro that
     still guarantees a non-empty intersection with the half space. Changes to
     that aspect of the experiment, will cause this function to become invalid.
     */
   ...
   *amount* of penetration). So,
   ...
   orientation theta as:
    ...
    ∂A/∂Ro = W * ∂L/∂Ro
           ...
           = [0, 0, -width / cos(θ)]
   */
  auto evaluate_area = [w = this->kWidth](
...
TEST_F(MeshHalfSpaceDerivativesTest, VertexPosition) {
  ...
  TestQuantity(evaluate_positions);
}
...
TEST_F(MeshHalfSpaceDerivativesTest, FaceNormalsWrtPosition) {
  ...
  TestQuantity(evaluate_normals);
}

TEST_F(MeshHalfSpaceDerivativesTest, FaceNormalsWrtTheta) {
  /* The normals *do* depend on θ. When θ = 0, the normals point in the +x
   direction. Positive θ rotation around Ry, will cause the normal to dip down.
   Negative rotation causes it to rise up. More generally.

      dn/dθ = [-sin(θ), 0, -cos(θ)]
   */
  auto evaluate_normals = [](const ContactSurface<AutoDiffXd>& surface,
                             const RigidTransform<AutoDiffXd>&, double theta) {
    const Vector3<double> expected_deriv{-std::sin(theta), 0, -std::cos(theta)};
    for (SurfaceFaceIndex t(0); t < surface.mesh_W().num_elements(); ++t) {
      const auto& n = surface.mesh_W().face_normal(t);
      /* Precision decreases as the mesh gets closer to lying *on* the half
       space surface. This simple switch accounts for the observed behavior in
       this test. */
      EXPECT_TRUE(CompareMatrices(math::autoDiffToGradientMatrix(n),
                                  expected_deriv, theta > 1.3 ? 1e-13 : 5e-15));
    }
  };
  TestQuantity(evaluate_normals, false /* wrt position */);
}
...
TEST_F(MeshHalfSpaceDerivativesTest, Pressure) {
  ...
  TestQuantity(evaluate_pressures);
}

...
//      to the mesh.
//    - If requesting gradients of constituent pressure fields, report the
//      values for the soft half space across all triangles.
//
...
GTEST_TEST(ComputeContactSurfaceFromSoftHalfSpaceRigidMeshTest, DoubleValued) {
...

  {
    ...
    const std::unique_ptr<ContactSurface<double>> contact_surface =
        ComputeContactSurfaceFromSoftHalfSpaceRigidMesh(
            hs_id, X_WH, pressure_scale, mesh_id, mesh_F, bvh_F, X_WF, false);
  ...
  {
    // Case: intersecting without gradients of constituent pressure fields.
    const std::unique_ptr<ContactSurface<double>> contact_surface =
        ComputeContactSurfaceFromSoftHalfSpaceRigidMesh(
            hs_id, X_WH, pressure_scale, mesh_id, mesh_F, bvh_F, X_WF, false);
  ...
  {
    // Case: intersecting with gradients of constituent pressure fields.
    // A repeat of the previous test, this time, we simply test for the
    // existence (and correctness) of the pressure field associated with the
    // half space; it should be the negative scaled half space normal. We'll
    // repeat the test twice, swapping ids to make sure the pressure field
    // moves from M to N.
        ...
        -pressure_scale * X_WH.rotation().col(2);
    {
      const std::unique_ptr<ContactSurface<double>> contact_surface =
          ComputeContactSurfaceFromSoftHalfSpaceRigidMesh(
              hs_id, X_WH, pressure_scale, mesh_id, mesh_F, bvh_F, X_WF, true);
      ASSERT_NE(contact_surface, nullptr);
...

    {
      const std::unique_ptr<ContactSurface<double>> contact_surface =
          ComputeContactSurfaceFromSoftHalfSpaceRigidMesh(
              mesh_id, X_WH, pressure_scale, hs_id, mesh_F, bvh_F, X_WF, true);
      ASSERT_NE(contact_surface, nullptr);

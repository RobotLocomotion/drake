...
GTEST_TEST(CompupteContactSurfaceFromSoftHalfSpaceRigidMeshTest, BackfaceCull) {
  // For this test, we don't have to worry about the transformation to world
  // space quantities; it's been tested already. We'll leave this the identity.
  const RigidTransform<double> X_WF;

  // Likewise, we'll have F = B.
  const SurfaceMesh<double> mesh_F = CreateBoxMesh(RigidTransform<double>{});
  const GeometryId mesh_id = GeometryId::get_new_id();
  const BoundingVolumeHierarchy<SurfaceMesh<double>> bvh_F(mesh_F);

  // Construct the half-space.
  const GeometryId hs_id = GeometryId::get_new_id();
  // We will reorient and position the half space. The orientation is so that we
  // know we're getting the normal from the transform, and the positioning is so
  // that the box is completely enclosed in the half space. The normal points
  // along one of the diagonals of the box connecting opposite corners (to ease
  // the prediction of what the outcome should be -- see below).
  ...
  for (SurfaceFaceIndex tri(0); tri < mesh_W.num_faces(); ++tri) {

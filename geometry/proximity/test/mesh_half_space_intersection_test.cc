...
/// Creates a SurfaeMesh of a box. The box is defined in frame B, centered on
...
GTEST_TEST(ComputeContactSurfaceFromSoftHalfSpaceRigidMeshTest, DoubleValued) {
...

  {
    ...
    // that has the same general _topology_ as the BoxMesh test, but not
    ...
    for (SurfaceVertexIndex v(0); v < mesh_W.num_vertices(); ++v) {
      ...
      // Pressure if a function of _penetration depth_ = -signed distance.
...
// message.
GTEST_TEST(ComputeContactSurfaceFromSoftHalfSpaceRigidMeshTest, AutoDiffValued) {
  // An arbitrary relationship between Frames W and F -- avoiding additive and
  // multiplicative identities.
  using T = AutoDiffXd;
  const RigidTransform<T> X_WF;

  ...
  void TestPositionDerivative(
    ...
    for (const auto& config : configurations) {
      ...
      const Vector3<AutoDiffXd> p_WR =
          math::InitializeAutoDiffFromValueMatrix(p_WR_d);
...
TEST_F(MeshHalfSpaceDerivativesTest, Area) {
  ...
  auto evaluate_area = [X_WS = convert_to_double(this->X_WS_)](
    ...
    for (const auto& tri : triangles) {
      ...
      const Matrix3<double> dA_dRo =
          math::ExtractGradientMatrixFromAutoDiff(p_WA_ad);
      const Matrix3<double> dB_dRo =
          math::ExtractGradientMatrixFromAutoDiff(p_WB_ad);
      const Matrix3<double> dC_dRo =
          math::ExtractGradientMatrixFromAutoDiff(p_WC_ad);
    ...
    for (VIndex v(0); v < mesh_W.num_vertices() - 1; ++v) {
      ...
      if (p_SV.z() < -1e-10) {
        /* Because this point lies *below* the half space surface (and we know
         ...
         the identity. */
        const Matrix3<double> J_W =
            math::ExtractGradientMatrixFromAutoDiff(p_WV_ad);
    ...
    switch (pose) {
      ...
      case kSkewTriangle: {
        ...
        for (VIndex v(0); v < 3; ++v) {
          expected_J_W +=
              math::ExtractGradientMatrixFromAutoDiff(mesh_W.vertex(v).r_MV());
        }
        expected_J_W /= 3;
        const Vector3<AutoDiffXd>& p_WC = mesh_W.vertex(VIndex(3)).r_MV();
        const Matrix3<double> J_W =
            math::ExtractGradientMatrixFromAutoDiff(p_WC);
...
TEST_F(MeshHalfSpaceDerivativesTest, FaceNormalsWrtPosition) {
  ...
  auto evaluate_normals = [](const ContactSurface<AutoDiffXd>& surface,
    ...
    for (SurfaceFaceIndex t(0); t < surface.mesh_W().num_elements(); ++t) {
      ...
      EXPECT_TRUE(CompareMatrices(math::ExtractGradientMatrixFromAutoDiff(n_W),
                                  zero_matrix, 32 * kEps));
    }
  };
...
TEST_F(MeshHalfSpaceDerivativesTest, FaceNormalsWrtOrientation) {
  ...
  for (const double theta : {0.0, M_PI / 6, M_PI / 2 * 0.9, M_PI / 2 * 0.99}) {
    ...
    const Vector3d Ry_W =
        math::ExtractValueMatrixFromAutoDiff(X_WR.rotation().col(1));
    ...
    for (SurfaceFaceIndex t(0); t < surface->mesh_W().num_elements(); ++t) {
      ...
      EXPECT_TRUE(CompareMatrices(math::ExtractGradientMatrixFromAutoDiff(n),
...
TEST_F(MeshHalfSpaceDerivativesTest, Pressure) {
  ...
  auto evaluate_pressures = [E = this->pressure_scale_,
    ...
    for (SurfaceVertexIndex v(0); v < surface.mesh_W().num_vertices(); ++v) {
      ...
      const Matrix3<double> dV_dRo =
          math::ExtractGradientMatrixFromAutoDiff(p_WV);

...
class MeshHalfSpaceValueTest : public ::testing::Test {
 public:
  ...
  MeshBuilder& builder_W() { return builder_W_; }
  ...
  ::testing::AssertionResult FieldConsistent(const MeshType& test_mesh_W,
                    const MeshFieldLinear<T, MeshType>& test_field_W,
                    const RigidTransform<T>& X_WF) {
    using std::abs;
    const RigidTransform<T> X_FW = X_WF.inverse();
    ...
    for (int v = 0; v < test_mesh_W.num_vertices(); ++v) {
      const T test_pressure = test_field_W.EvaluateAtVertex(v);
      const Vector3<T> p_FV = X_FW * test_mesh_W.vertex(v);
      const T expected_pressure = pressure_in_F_(p_FV);
      const T error = abs(test_pressure - expected_pressure);
    ...
    if (std::is_same_v<MeshType, PolygonSurfaceMesh<T>>) {
      const Vector3<T> grad_p_W = X_WF.rotation() * grad_p_F_;
    ...
    for (int f = 0; f < test_mesh_W.num_elements(); ++f) {
      ...
      const Vector3<T>& p_WV0 = test_mesh_W.vertex(face.vertex(0));
      const Vector3<T>& p_WV1 = test_mesh_W.vertex(face.vertex(1));
      const Vector3<T> p_WX = (p_WV0 + p_WV1) / 2;
      const T p_0 = test_field_W.EvaluateAtVertex(face.vertex(0));
      const T p_1 = test_field_W.EvaluateAtVertex(face.vertex(1));
      const T p_X_expected = (p_0 + p_1) / 2;
      const T p_X_test = test_field_W.EvaluateCartesian(f, p_WX);
      const T error = (p_X_test - p_X_expected);
      if (error > kPressureEps) {
        return ::testing::AssertionFailure()
               << "\nMesh face " << f
  ...
  void CallConstructTriangleHalfspaceIntersectionPolygon(
    ...
    const Vector3<T> grad_p_W = X_WF.rotation() * this->grad_p_F_;
 ...
 private:
  void SetUp() {
    ...
    Vector3<T> normal_F(0, 0, 1);
    const Vector3<T> point_F(0, 0, 2);
    half_space_F_ = std::make_unique<PosedHalfSpace<T>>(normal_F, point_F);
    ...
    pressure_in_F_ = [grad_p_F = grad_p_F_, point_F](const Vector3<T>& p_VF) {
      return grad_p_F.dot(p_VF - point_F);
    };
  }

  MeshBuilder builder_W_;
  ...
  Vector3<T> grad_p_F_;
  std::function<T(const Vector3<T>&)> pressure_in_F_;
};  // namespace
...
TYPED_TEST_P(MeshHalfSpaceValueTest, InsideOrOnIntersection) {
  ...
  // Case: The triangular mesh lies *completely* inside the half space.
  {
    ...
    // the half space is the Fz <= 2. So, putting these triangles at Fz = 1,
    // they'll lie inside.
...
TYPED_TEST_P(MeshHalfSpaceValueTest, EdgeOnHalfspaceIntersection) {
  ...
  // Case: (a) two vertices on the boundary, one outside.
  {
    ...
    // centroid f.
    //
    // Confirm degenracy: areas are close to zero, and vertices are close to
    // vertices 1 and 2 in tri_mesh_F (as documented above).
    for (int f = 0; f < mesh_W->num_elements(); ++f) {
      EXPECT_NEAR(ExtractDoubleOrThrow(mesh_W->area(f)), 0, 1e-15);
    }
    // The reference vertices. For simplicitly, we'll simply consider the
...
//     among the triangle indicies provided.
...
TYPED_TEST_P(MeshHalfSpaceValueTest, ComputeContactSurfaceInvocation) {
  ...
  // Case no intersections returns nullptr.
  {
    ...
    EXPECT_EQ(ComputeContactSurface<MeshBuilder>(
                  ...
                  pressure_in_F, {8, 9}, X_WF, grad_p_W),
              nullptr);
    // There are no indices provided.
    EXPECT_EQ(ComputeContactSurface<MeshBuilder>(
                  mesh_id, tri_mesh_F, half_space_id, half_space_F,
                  pressure_in_F, {}, X_WF, grad_p_W),
              nullptr);
  }
  ...
  // Case: Plane intersects the mesh.
  {
    ...
    // the TriangleSurfaceMehs will have an addition 24 triangles.
    ...
    // Confirm that we've sampled the half space gradient per face.
    if (surface->id_M() == half_space_id) {
      ...
      GTEST_FAIL()
          << "The half space id was not referenced in the contat surface.";
    }
  }
}

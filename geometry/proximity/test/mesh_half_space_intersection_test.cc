...
//   1. The vertices in the resultant mesh are measured and expressed in
//      Frame W (as opposed to the input mesh's Frame F)
//   2. The new mesh is a triangle soup with duplicated vertices.
template <typename T>
SurfaceMesh<double> CreateMeshWithTransform(const SurfaceMesh<double>& mesh_F,
...
TYPED_TEST_P(MeshHalfSpaceValueTest, NoIntersection) {
  ...
  this->CallConstructTriangleHalfspaceIntersectionPolygon(
      mesh_F, {}, ContactPolygonRepresentation::kCentroidSubdivision);
  ...
  this->CallConstructTriangleHalfspaceIntersectionPolygon(
      mesh_F, {}, ContactPolygonRepresentation::kSingleTriangle);
  EXPECT_TRUE(this->new_vertices_W().empty());
  EXPECT_TRUE(this->new_faces().empty());
  EXPECT_TRUE(this->vertices_to_newly_created_vertices().empty());
  EXPECT_TRUE(this->edges_to_newly_created_vertices().empty());
  ...
  for (const auto representation :
    ...
    switch (representation) {
      ...
      case ContactPolygonRepresentation::kSingleTriangle:
        this->VerifyMeshesEquivalent(
            CreateMeshWithTransform(mesh_F, X_WF),
            SurfaceMesh<T>{move(this->new_faces()),
                           move(this->new_vertices_W())});
        break;
    }
...
// vertices lie within the half space). This covers Case 3 for (a) and
// Case 1 for (b).
TYPED_TEST_P(MeshHalfSpaceValueTest, VertexOnHalfspaceIntersection) {
...

  {
    ...
    this->CallConstructTriangleHalfspaceIntersectionPolygon(
        mesh_F, X_WF, ContactPolygonRepresentation::kCentroidSubdivision);
    ...
    this->CallConstructTriangleHalfspaceIntersectionPolygon(
        mesh_F, X_WF, ContactPolygonRepresentation::kSingleTriangle);
    // This option considers the intersected triangle too small and skips it.
    EXPECT_TRUE(this->new_vertices_W().empty());
    EXPECT_TRUE(this->new_faces().empty());
  }
...
// vertex lies within the half space). This covers Case 2 for (a) and Case 1
// for (b).
TYPED_TEST_P(MeshHalfSpaceValueTest, EdgeOnHalfspaceIntersection) {
...

  {
    ...
    this->CallConstructTriangleHalfspaceIntersectionPolygon(
        mesh_F, X_WF, ContactPolygonRepresentation::kSingleTriangle);
    // This option skips the zero-area quadrilateral.
    EXPECT_TRUE(this->new_vertices_W().empty());
    EXPECT_TRUE(this->new_faces().empty());
    EXPECT_TRUE(this->vertices_to_newly_created_vertices().empty());
    EXPECT_TRUE(this->edges_to_newly_created_vertices().empty());
...
TYPED_TEST_P(MeshHalfSpaceValueTest, QuadrilateralResults) {
  ...
  const Vector3d nhat_F = mesh_F.face_normal(SurfaceFaceIndex(0));
  const RigidTransform<double> X_WF_d = convert_to_double(X_WF);
  ...
  for (const auto representation :
    ...
    const vector<Vector3<double>> quad_W{X_WF_d * Vector3d(2, 5, 1),     // b
                                         X_WF_d * Vector3d(4, 5, 1),     // c
                                         X_WF_d * Vector3d(3.5, 5, 2),   // d
                                         X_WF_d * Vector3d(2.5, 5, 2)};  // e
    vector<SurfaceVertex<double>> expected_vertices_W;
    vector<SurfaceFace> expected_faces;
    switch (representation) {
      case ContactPolygonRepresentation::kCentroidSubdivision: {
        using V = SurfaceVertex<double>;
        expected_vertices_W = std::vector<SurfaceVertex<double>>{
            V(quad_W[0]), V(quad_W[1]), V(quad_W[2]), V(quad_W[3])};
        const SurfaceVertexIndex b(0), c(1), d(2), e(3);
        AddPolygonToMeshData({b, c, d, e}, nhat_W, &expected_faces,
                             &expected_vertices_W);
        break;
      }
      case ContactPolygonRepresentation::kSingleTriangle: {
...
TYPED_TEST_P(MeshHalfSpaceValueTest, OutsideInsideOn) {
  ...
  for (const auto representation :
    ...
    switch (representation) {
      case ContactPolygonRepresentation::kCentroidSubdivision: {
        using V = SurfaceVertex<double>;
        expected_vertices_W = vector<SurfaceVertex<double>>{
            V(quad_W[0]), V(quad_W[1]), V(quad_W[2]), V(quad_W[3])};
        const SurfaceVertexIndex b(0), c(1), d(2), e(3);
        AddPolygonToMeshData({b, c, d, e}, nhat_W, &expected_faces,
                             &expected_vertices_W);
...
TYPED_TEST_P(MeshHalfSpaceValueTest, OneInsideTwoOutside) {
  ...
  for (const auto representation :
    ...
    // Check that the mesh that results is equivalent to the expected mesh.
    vector<Vector3<double>> intersect_W{X_WF_d * Vector3d(3, 6, 1),
    ...
    switch (representation) {
      case ContactPolygonRepresentation::kCentroidSubdivision:
        using V = SurfaceVertex<double>;
        expected_vertices_W = vector<SurfaceVertex<double>>{
            V(intersect_W[0]), V(intersect_W[1]), V(intersect_W[02])};
        typedef SurfaceVertexIndex Index;
        AddPolygonToMeshData({Index(0), Index(1), Index(2)}, nhat_W,
                             &expected_faces, &expected_vertices_W);
        break;
      case ContactPolygonRepresentation::kSingleTriangle:
        AddPolygonToMeshDataAsOneTriangle(
            intersect_W, nhat_W, &expected_faces, &expected_vertices_W);
        break;
    }
  ...
  for (const auto representation :
    ...
    switch (representation) {
      case ContactPolygonRepresentation::kCentroidSubdivision:
        ...
        // total of seven per direction. Total number of faces: 34
        //   -----------------------------------------------
...
GTEST_TEST(ComputeContactSurfaceFromSoftHalfSpaceRigidMeshTest, DoubleValued) {
  ...
  for (const auto representation :
    ...
    // BoxMesh test for the reasoning of the number of vertices and faces.
    switch (representation) {
      case ContactPolygonRepresentation::kCentroidSubdivision:
...
GTEST_TEST(CompupteContactSurfaceFromSoftHalfSpaceRigidMeshTest, BackfaceCull) {
  ...
  for (const auto representation :
    ...
    // passes the culling test. There are two tricky bits in setting up this
    // test.
    //

...
GTEST_TEST(MakeSphereMesh, VolumeConvergence) {
  ...
  for (bool sparse_vertices : {true, false}) {
    auto mesh0 = MakeUnitSphereMesh<double>(0, sparse_vertices);
    ...
    const int tet_growth = sparse_vertices ? 4 : 8;

    for (int level = 1; level < 6; ++level) {
      auto mesh = MakeUnitSphereMesh<double>(level, sparse_vertices);
...
GTEST_TEST(MakeSphereMesh, NoDuplicateVertices) {
  ...
  for (int i = 0; i < 4; ++i) {
    VolumeMesh<double> mesh = MakeUnitSphereMesh<double>(i, false /* sparse */);
...
GTEST_TEST(MakeSphereMesh, AutoDiffRefinement) {
  VolumeMesh<AutoDiffXd> mesh1 =
      MakeUnitSphereMesh<AutoDiffXd>(1, false /* sparse */);
  ...
  for (const bool sparse : {true, false}) {
    ...
    for (const double edge_length : {1.5 * r, r, r / 2, r / 4}) {
      VolumeMesh<double> mesh =
          MakeSphereVolumeMesh<double>(sphere, edge_length, sparse);
...
GTEST_TEST(MakeSphereVolumeMesh, MassiveEdgeLength) {
  ...
  VolumeMesh<double> mesh =
      MakeSphereVolumeMesh<double>(sphere, edge_length, false /* sparse */);
...
GTEST_TEST(SparseSphereTest, Level0Identical) {
  VolumeMesh<double> dense_0 = MakeUnitSphereMesh<double>(0, false);
  VolumeMesh<double> sparse_0 = MakeUnitSphereMesh<double>(0, true);
...
GTEST_TEST(SparseSphereTest, SurfaceMatchesDenseMesh) {
  ...
  for (int level = 1; level < 3; ++level) {
    const VolumeMesh<double> dense_mesh =
        MakeUnitSphereMesh<double>(level, false);
    const SurfaceMesh<double> dense_surface =
        ConvertVolumeToSurfaceMesh<double>(dense_mesh);
    const VolumeMesh<double> sparse_mesh =
        MakeUnitSphereMesh<double>(level, true);
...
GTEST_TEST(SparseSphereTest, RefinesWithSingleInteriorVertex) {
  const bool sparse_vertices{true};

  // Level 0 mesh
  VolumeMesh<double> mesh0 = MakeUnitSphereMesh<double>(0, sparse_vertices);
  ...
  for (int level = 1; level < 6; ++level) {
    VolumeMesh<double> mesh_i =
        MakeUnitSphereMesh<double>(level, sparse_vertices);
...
// sparse_internal_vertices parameter. All other aspects of that function
// (e.g., limits on refinment level and effect of resolution_hint has been
// tested elsewhere).)
GTEST_TEST(SparseSphereTest, SparseSphereOfArbitraryRadius) {
  ...
  VolumeMesh<double> mesh =
      MakeSphereVolumeMesh<double>(sphere, r, true /* sparse */);

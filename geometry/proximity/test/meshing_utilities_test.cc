...
GTEST_TEST(CalcEnclosedVolumeTest, SimpleTest) {
  ...
  obj = FindResourceOrThrow("drake/geometry/test/octahedron_no_volume.obj");
  surface_mesh = ReadObjToTriangleSurfaceMesh(obj, 1.0);
  // We currently expect this particular mesh to return zero volume, because
  // exactly half of the faces are wound in the wrong direction. Meshlab
  // reports volume 0 for this mesh, also! We simply document the known
  // incorrect answer here.
  EXPECT_NEAR(CalcEnclosedVolume(surface_mesh), 0.0, 1e-14);

...
class DeformableDeformableContactTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    collision_filter_.AddGeometry(deformable0_id);
    collision_filter_.AddGeometry(deformable1_id);
    ...
    geometries_.AddDeformableGeometry(deformable0_id, std::move(mesh0));
    geometries_.AddDeformableGeometry(deformable1_id, std::move(mesh1));
  }
...
TEST_F(DeformableDeformableContactTest, OneContactPair) {
  ...
  // geometries.
  auto it0 = GeometriesTester::deformable_geometries(geometries_).begin();

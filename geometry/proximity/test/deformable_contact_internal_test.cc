...
const math::RigidTransform<double> kDefaultPose(math::RollPitchYaw<double>(1, 2,
                                                                           3),
                                                Vector3d(4, 5, 6));
...
GTEST_TEST(GeometriesTest, UnsupportedRigidShapes) {
  ...
  /* MeshcatCone */
  {
    ...
    EXPECT_NO_THROW(geometries.MaybeAddRigidGeometry(
        MeshcatCone(height, a, b), cone_id, props, kDefaultPose));
...
GTEST_TEST(GeometriesTest, UpdateRigidWorldPose) {
  ...
  /* Initially the pose is the default pose. */
  {
    ...
    EXPECT_TRUE(rigid_geometry.pose_in_world().IsExactlyEqualTo(kDefaultPose));
  }

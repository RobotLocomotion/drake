...
GTEST_TEST(MeshSdfCacheTest, CopySemantics) {
  ...
  EXPECT_NE(pre_compute_copy.GetBoundary(id), nullptr);
  EXPECT_NE(pre_compute_copy_assign.GetBoundary(id), nullptr);
  EXPECT_NE(cache.GetBoundary(id), pre_compute_copy.GetBoundary(id));
  EXPECT_NE(cache.GetBoundary(id), pre_compute_copy_assign.GetBoundary(id));
  EXPECT_NE(pre_compute_copy.GetBoundary(id),
            pre_compute_copy_assign.GetBoundary(id));

  // Because of the evaluation above, the cache is now computed.

...
class MeshSdfCacheTester {
 public:
  ...
  static int scale_entry_count(const MeshSdfCache& cache, GeometryId id) {
    ...
    const std::string& file_key = cache.geometry_entries_.at(id).key.file_key;
  ...
  static int ref_count(const MeshSdfCache& cache, GeometryId id) {
    ...
    const auto& boundary_handle =
        cache.geometry_entries_.at(id).boundary_handle;
    return static_cast<int>(boundary_handle.use_count()) - 1;
  }
  ...
  static bool boundary_computed(const MeshSdfCache& cache, GeometryId id) {
    ...
    return cache.geometry_entries_.at(id).boundary_handle->has_value();
  }

...
class MeshSdfCacheTester {
 public:
  ...
  static int ref_count(const MeshSdfCache& cache, GeometryId id) {
    ...
    // Creating a temporary copy increments use_count by 1; subtract it off.
    const auto boundary = cache.geometry_entries_.at(id).boundary;
...
GTEST_TEST(MeshSdfCacheTest, CopyAssignment) {
  ...
  cache.ComputeAll();
  const MeshDistanceBoundary& boundary = cache.GetBoundary(id);
  ...
  EXPECT_EQ(&copy.GetBoundary(id), &boundary);
  ...
  // the same memoized entry. Register() eagerly computes since copy is already
  // in the computed state.
  const GeometryId copy_id = RegisterShape(MakeObjMesh(), &copy);
  const MeshDistanceBoundary& copy_boundary = copy.GetBoundary(copy_id);
...
GTEST_TEST(MeshSdfCacheTest, MoveConstructor) {
  ...
  original.ComputeAll();
  const MeshDistanceBoundary& boundary = original.GetBoundary(id);
  ...
  EXPECT_EQ(&moved.GetBoundary(id), &boundary);
...
GTEST_TEST(MeshSdfCacheTest, MoveAssignment) {
  ...
  const GeometryId source_id = RegisterShape(MakeObjMesh(), &source);
  source.ComputeAll();
  const MeshDistanceBoundary& boundary = source.GetBoundary(source_id);
  ...
  EXPECT_TRUE(dest.contains(source_id));
  EXPECT_EQ(&dest.GetBoundary(source_id), &boundary);
...
GTEST_TEST(MeshSdfCacheTest, Register) {
  ...
  EXPECT_EQ(Tester::num_entries(cache), 0);
  EXPECT_EQ(Tester::num_geometries(cache), 0);
  // We haven't computed the boundaries; accessing the boundary fails.
  EXPECT_THROW(cache.GetBoundary(id1), std::exception);
  ...
  // After computing, the boundary is accessible.
  EXPECT_NO_THROW(cache.GetBoundary(id1));

  // Second instance of the same file and scale increases the ref count, and
  // geometry count; no new entry. Register() eagerly computes since
  // is_computed_ is true.
  ...
  // After computing, a geometry with the same boundary is accessible.
  EXPECT_NO_THROW(cache.GetBoundary(id2));
  ...
  // After computing, a geometry with a different boundary (scale) is valid.
  EXPECT_NO_THROW(cache.GetBoundary(id3));
}
...
GTEST_TEST(MeshSdfCacheTest, RegisterMeshVsConvex) {
  ...
  const MeshDistanceBoundary& mesh_b = cache.GetBoundary(mesh_id);
  const MeshDistanceBoundary& convex_b = cache.GetBoundary(convex_id);
...
GTEST_TEST(MeshSdfCacheTest, ComputeAll) {
  MeshSdfCache cache;

  EXPECT_NO_THROW(cache.ComputeAll());
  ASSERT_EQ(Tester::num_entries(cache), 0);
  EXPECT_EQ(Tester::num_geometries(cache), 0);
  ...
  const GeometryId id_c = RegisterShape(MakeObjMesh(1, kTet), &cache);
  cache.ComputeAll();
  const MeshDistanceBoundary& bound_a1 = cache.GetBoundary(id_a1);
  const MeshDistanceBoundary& bound_a2 = cache.GetBoundary(id_a2);
  const MeshDistanceBoundary& bound_b = cache.GetBoundary(id_b);
  const MeshDistanceBoundary& bound_c = cache.GetBoundary(id_c);
  ASSERT_EQ(Tester::num_entries(cache), 3);
  EXPECT_EQ(Tester::num_geometries(cache), 4);
  ...
  // ComputeAll() is idempotent; repeated GetBoundary() returns same reference.
  cache.ComputeAll();
  EXPECT_EQ(&cache.GetBoundary(id_a1), &bound_a1);
  EXPECT_EQ(&cache.GetBoundary(id_b), &bound_b);
  EXPECT_EQ(&cache.GetBoundary(id_c), &bound_c);
}

...
// Tests the transformation of the field when calling TransformGradients().
// Although the name of the function suggests that only the gradients are
// affected, in practice, there's a secondary effect. If gradients are defined,
// those gradients are used when invoking EvaluateCartesian(). In addition
// to the publicly visible field gradients, there is hidden state that is only
// used in EvaluateCartesian() -- in the field's implementation, it is the field
// value at the field's frame's origin. This value is only used in
// EvaluateCartesian() and *must* be updated when the field is transformed.
// To that end, we test the gradient values themselves directly *and* invoke
// EvaluateCartesian() to confirm full transformation.
GTEST_TEST(MeshFieldLinearTest, TestTransformGradients) {
  ...
  // ignore that.
  mesh_field_N.TransformGradients(X_NM);

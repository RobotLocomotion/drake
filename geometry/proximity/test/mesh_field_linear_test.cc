...
GTEST_TEST(MeshFieldLinearTest, EvaluateCartesianWithAndWithoutGradient) {
  ...
  const MeshFieldLinear<double, VolumeMesh<double>> field_with_gradient(
      std::move(values), &mesh_M, MeshGradientMode::kOkOrThrow);
...
GTEST_TEST(MeshFieldLinearTest, TestDegeneracy) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      (MeshFieldLinear<double, TriangleSurfaceMesh<double>>(
          std::move(e_values), mesh.get(), MeshGradientMode::kOkOrThrow)),

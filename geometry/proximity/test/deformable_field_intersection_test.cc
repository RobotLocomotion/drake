...
TEST_F(AddDeformableDeformableContactSurfaceTest, HaveContact) {
  ...
  // Check DeformableContactSurface.
  {
    ...
    // We assume that we can use contact_mesh_W() without verification
    // because AddDeformableDeformableContactSurface() uses VolumeIntersector
    // to compute the mesh of the contact surface and adds data
    // specific to DeformableContact afterward. The correctness of the
    // contact mesh is already tested in VolumeIntersector.

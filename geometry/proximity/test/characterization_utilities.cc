...
void CharacterizeResultTest<T>::RunCharacterization(
  ...
  fcl::CollisionObjectd obj_A = MakeFclShape(shape_A).object();
  const GeometryId id_A = EncodeData(&obj_A);

  fcl::CollisionObjectd obj_B = MakeFclShape(shape_B).object();
  const GeometryId id_B = EncodeData(&obj_B);
  ...
  auto evaluate_callback =
      [this, &expectation, &worst_error, &worst_config](
          ...
          fcl::CollisionObjectd* obj_B, auto config,
        ...
        SCOPED_TRACE(fmt::format(
            ...
            config.signed_distance, config.X_AB.GetAsMatrix34()));
        RunCallback(expectation, obj_A, obj_B, &collision_filter_, &X_WGs);
        const std::optional<double> error =
            ComputeErrorMaybe(config.signed_distance);
        if (error.has_value() &&
  ...
  for (const auto& X_WA : X_WAs) {
    for (const auto& config : configs) {
      ...
      obj_A.setTransform(convert_to_double(X_WGs.at(id_A)).GetAsIsometry3());
      obj_B.setTransform(convert_to_double(X_WGs.at(id_B)).GetAsIsometry3());

      evaluate_callback('A', &obj_A, 'B', &obj_B, config, X_WGs);
      evaluate_callback('B', &obj_B, 'A', &obj_A, config, X_WGs);
    }
  }
...
template <typename T>
vector<math::RigidTransform<T>> CharacterizeResultTest<T>::X_WAs() {
  return vector<math::RigidTransform<T>>{
      math::RigidTransform<T>{
          math::RotationMatrix<T>(
              AngleAxis<T>(5 * M_PI / 9, Vector3<T>{-1, -1, 1}.normalized())),
          Vector3<T>{-0.2, 0.15, -0.3}},
      math::RigidTransform<T>{
          math::RotationMatrix<T>(
              AngleAxis<T>(22 * M_PI / 9, Vector3<T>{-1, 1, -1}.normalized())),

...
void MakeFclShape::ImplementGeometry(const Mesh&, void*) {
  ...
  // distance_to_point_characterize_test.cc builds a MeshSdfCache entry that
  // must match the geometry. It does so by turning the fcl::Convexd back into
  // a Mesh. If this ever introduces a Mesh that is not its own convex hull,
  // that logic will need to change.

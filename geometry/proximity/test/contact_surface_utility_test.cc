...
// This first test validates the computation of a polygon center of mass. There
// are specific cases in which the center of mass is simply the average vertex
// position. They are:
//
//   - polygon is a triangle
//   - polygon is radially symmetric.
//   - polygon is has duplicate vertices such that the _unique_ set is simply
//     a triangle.
...
// center of mass _isn't_ the average vertex position as a representative
// sample.
//
...
//
//  well-formed triangle: v6, v1, v2 -> com is average vertex.
//  radially symmetric quad: v0, v1, v3, v2 -> com is average vertex.
//  asymmetric quad: v0, v1, v4, v2 -> com is _not_ average vertex.
//  quad duplicate vertices: v0, v1, v1, v2 -> com is triangle average vertex.
//  quad with epsilon edge: v0, v1, v5, v2 -> com is triangle average vertex.
//
// We then transform these vertices into various frames to make sure that it
// works with arbitrary ugliness.
TEST_F(ContactSurfaceUtilityTest, PolygonCenterOfMassTest) {
  ...
  const vector<V> vertices_M{V{Vector3d{0, 0, 0}}, V{Vector3d{1, 0, 0}},
                       ...
                       V{Vector3d{1.5, 1.5, 0}}, V{Vector3d{0, 1 + kEps, 0}},
                       V{Vector3d{-1, -0.25, 0}}};
  ...
  for (const auto X_FM : X_FMs_) {
...

    {
      ...
      const Vector3d p_FCom =
          CalcPolygonCenterOfMass(triangle, nhat_F, vertices_F);
      const Vector3d P_FCom_expected = AverageVertex(triangle, vertices_F);
      EXPECT_TRUE(CompareMatrices(p_FCom, P_FCom_expected));
    }
    {
      // Radially symmetric quad; the center of mass is simply the average
      // vertex position.
      const vector<VIndex> quad{VIndex{0}, VIndex{1}, VIndex{3}, VIndex{2}};
      const Vector3d p_FCom = CalcPolygonCenterOfMass(quad, nhat_F, vertices_F);
      const Vector3d P_FCom_expected = AverageVertex(quad, vertices_F);
      EXPECT_TRUE(CompareMatrices(p_FCom, P_FCom_expected, 2 * kEps));
    }
    {
      // Quad with a duplicate vertex; CoM is the same as for the corresponding
      // triangle.
    ...
    {
      // Quad with a negligible edge; CoM is _almost_ the same as for the
      // corresponding triangle.
      const vector<VIndex> quad{VIndex{6}, VIndex{1}, VIndex{5}, VIndex{2}};
      const vector<VIndex> triangle{VIndex{6}, VIndex{1}, VIndex{2}};
      const Vector3d p_FCom = CalcPolygonCenterOfMass(quad, nhat_F, vertices_F);
      const Vector3d P_FCom_expected = AverageVertex(triangle, vertices_F);
      EXPECT_TRUE(CompareMatrices(p_FCom, P_FCom_expected, kEps));
    }
    {
      // A quad that _isn't_ radially symmetric will not have a CoM in the same
      // location as the average vertex. We're not computing the actual expected
      // value; merely indicating that to some fairly large tolerance, the
      // two points are well separated. That, in conjunction with the previous
      // tests _should_ be enough. Otherwise, the challenge is to compute the
      // CoM through an alternative calculation to compare the two.
      // Note: the size of the threshold used here depends on the distance
      // between the vertices.
      const vector<VIndex> quad{VIndex{6}, VIndex{1}, VIndex{4}, VIndex{2}};
      const vector<VIndex> triangle{VIndex{6}, VIndex{1}, VIndex{2}};
      const Vector3d p_FCom = CalcPolygonCenterOfMass(quad, nhat_F, vertices_F);
      const Vector3d P_FCom_expected = AverageVertex(triangle, vertices_F);
      EXPECT_FALSE(CompareMatrices(p_FCom, P_FCom_expected, 0.1));
    }
  }
}
...
TEST_F(ContactSurfaceUtilityTest, PolygonCenterOfMassTest_NormalUse) {
  using V = SurfaceVertex<double>;
  using VIndex = SurfaceVertexIndex;
  ...
  {
    // A zero vector is catastrophically bad; it produces a NaN center of mass.
    const vector<VIndex> quad{VIndex{0}, VIndex{1}, VIndex{2}, VIndex{3}};
    Vector3d kZeroVec = Vector3d::Zero();
    EXPECT_TRUE(CompareMatrices(
        CalcPolygonCenterOfMass(quad, kZeroVec, vertices_M),
...

  {
    ...
    EXPECT_TRUE(CompareMatrices(
        CalcPolygonCenterOfMass(pseudo_triangle, Fy, vertices_M),
        Vector3d::Constant(-std::numeric_limits<double>::quiet_NaN())));
  }
  ...
  for (const auto X_FM : X_FMs_) {
    ...
    // A "normal" that is too long or too short still produce the right center
    // of mass.
    EXPECT_TRUE(CompareMatrices(CalcPolygonCenterOfMass<double>(
                                    pseudo_triangle, 2 * Mz_F, vertices_F),
                                p_FC_expected, kEps));
    EXPECT_TRUE(CompareMatrices(CalcPolygonCenterOfMass<double>(
                                    pseudo_triangle, 0.5 * Mz_F, vertices_F),
                                p_FC_expected, kEps));
...
//   2. That new vertex is at the CoM of the polygon.
...
TEST_F(ContactSurfaceUtilityTest, AddPolygonToMeshData) {
  ...
  vector<VIndex> quad{VIndex{0}, VIndex{1}, VIndex{2}, VIndex{3}};
  ...
  ASSERT_TRUE(
      CompareMatrices(vertices_M.back().r_MV(),
                      CalcPolygonCenterOfMass(quad, nhat_M, vertices_M)));
  ...
  // possibility but are currently implicitly relying on the idea that all uses
  // of this method would die if such were the case.
}

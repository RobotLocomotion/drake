...
std::vector<Vector3<T>> IntersectTetrahedra(
    ...
    VolumeElementIndex element1,
    const VolumeMeshFieldLinear<double, double>& field1_N,
    const math::RigidTransform<T>& X_MN, Vector3<T>* polygon_nhat_M) {
  ...
  //   It should be out of element 1 and into element 0. Set up a unit test to
  //   confirm that.
  *polygon_nhat_M = equilibrium_plane_M.normal();
  ...
  for (const auto [tet0, tet1] : candidates) {
    ...
    switch (representation) {
      case ContactPolygonRepresentation::kCentroidSubdivision:
        AddPolygonToMeshData(contact_polygon, polygon_normal_M, &surface_faces,
                             &surface_vertices_M);
        break;
      case ContactPolygonRepresentation::kSingleTriangle:
        AddPolygonToMeshDataAsOneTriangle(polygon_vertices_M, polygon_normal_M,

...
std::unique_ptr<ContactSurface<T>> ComputeContactSurfaceFromCompliantVolumes(
  ...
  if (representation == HydroelasticContactRepresentation::kTriangle) {
    VolumeIntersector<TriMeshBuilder<T>, Obb>().IntersectCompliantVolumes(
        id0, field0_M, bvh0_M, X_WM, id1, field1_N, bvh1_N, X_WN,
        &contact_surface_W);
  } else {
    VolumeIntersector<PolyMeshBuilder<T>, Obb>().IntersectCompliantVolumes(
        id0, field0_M, bvh0_M, X_WM, id1, field1_N, bvh1_N, X_WN,
        &contact_surface_W);
  }

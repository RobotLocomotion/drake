...
template <typename MeshBuilder, typename BvType>
void VolumeIntersector<MeshBuilder, BvType>::IntersectCompliantVolumes(
  ...
  std::unique_ptr<MeshType> surface01_M;
  std::unique_ptr<FieldType> field01_M;
  ...
  DRAKE_ASSERT(num_contact_polygons ==
               static_cast<int>(tet0_of_contact_polygon_.size()));
  DRAKE_ASSERT(num_contact_polygons ==
               static_cast<int>(tet1_of_contact_polygon_.size()));
  ...
  for (int i = 0; i < num_contact_polygons; ++i) {
    const Vector3<T>& grad_field0_M =
        field0_M.EvaluateGradient(tet0_of_contact_polygon_[i]);
  ...
  for (int i = 0; i < num_contact_polygons; ++i) {
    const Vector3<T>& grad_field1_N =
        field1_N.EvaluateGradient(tet1_of_contact_polygon_[i]);
    grad_field1_W->emplace_back(X_WN.rotation() * grad_field1_N);
  }

  // The contact surface ContactSurface(id_first, id_second, mesh_W,...)
  // requires that the face normals in `mesh_W` point:
  //     - *out of* the second geometry and
  //     - *into* the first geometry.
  // This is the same convention as IntersectFields() creating a surface mesh
  // with normals in the direction of
  //     - increasing pressure field_0 (going *into* the first geometry) and
  //     - decreasing pressure field_1 (going *out* of the second geometry),
...
std::unique_ptr<ContactSurface<T>> ComputeContactSurfaceFromCompliantVolumes(
  ...
  if (representation == HydroelasticContactRepresentation::kTriangle) {
    VolumeIntersector<TriMeshBuilder<T>, Obb> intersector;
    intersector.IntersectCompliantVolumes(id0, field0_M, bvh0_M, X_WM, id1,
                                          field1_N, bvh1_N, X_WN,
                                          &contact_surface_W);
  } else {
    VolumeIntersector<PolyMeshBuilder<T>, Obb> intersector;
    intersector.IntersectCompliantVolumes(id0, field0_M, bvh0_M, X_WM, id1,
                                          field1_N, bvh1_N, X_WN,
                                          &contact_surface_W);
  }
...
template class VolumeIntersector<PolyMeshBuilder<double>, Obb>;
template class VolumeIntersector<TriMeshBuilder<double>, Obb>;
template class VolumeIntersector<PolyMeshBuilder<AutoDiffXd>, Obb>;
template class VolumeIntersector<TriMeshBuilder<AutoDiffXd>, Obb>;

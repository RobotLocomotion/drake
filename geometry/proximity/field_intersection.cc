...
void IntersectPlaneTetrahedron(
  ...
  std::vector<SurfaceTriangle> unused_faces;
  ...
  SliceTetWithPlane(tetrahedron, field_M, plane_M,
                    math::RigidTransform<T>::Identity(), &unused_faces,
                    &vertices_with_centroid_M, &unused_surface_e,
                    &unused_cut_edges);
...
void FieldIntersection(
  ...
  for (const auto [tet0, tet1] : candidate_tetrahedra) {
    ...
    for (size_t v = num_previous_vertices; v < surface_vertices_M.size();
      ...
      surface_field_values.push_back(
          field0_M.EvaluateCartesian(tet0, surface_vertices_M[v]));
    }
  ...
  *surface_MN_M = std::make_unique<TriangleSurfaceMesh<double>>(
      std::move(surface_faces), std::move(surface_vertices_M));
  const bool calculate_gradient = false;
  *e_01_M = std::make_unique<TriangleSurfaceMeshFieldLinear<double, double>>(
      std::move(surface_field_values), surface_MN_M->get(), calculate_gradient);
}
...
ComputeContactSurfaceFromCompliantVolumes(
  ...
  std::unique_ptr<TriangleSurfaceMesh<double>> surface01;
  std::unique_ptr<TriangleSurfaceMeshFieldLinear<double, double>> field01;
  std::vector<Vector3<double>> grad_field0_Fs;
  std::vector<Vector3<double>> grad_field1_Fs;
  FieldIntersection(field0_F, bvh0_F, field1_G, bvh1_G, X_FG, &surface01,
                    &field01, &grad_field0_Fs, &grad_field1_Fs);

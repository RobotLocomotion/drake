...
class BvNode {
 public:
  ...
  BvNode(const BvNode& node) : aabb_(node.aabb_) {
    if (node.is_leaf()) {
      child_ = node.element_index();
    } else {
      child_ = NodeChildren(std::make_unique<BvNode<MeshType>>(node.left()),
                            std::make_unique<BvNode<MeshType>>(node.right()));
    }
  }

  BvNode& operator=(BvNode node) {
    std::swap(aabb_, node.aabb_);
    std::swap(child_, node.child_);
    return *this;
  }

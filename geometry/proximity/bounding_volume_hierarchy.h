...
class BoundingVolumeHierarchy {
 ...
 private:
  ...
  std::unique_ptr<BVNode<MeshType>> BuildBVTree(
    ...
    std::unique_ptr<BVNode<MeshType>> node =
        std::make_unique<BVNode<MeshType>>(BVNode<MeshType>({aabb}));
    ...
    if (num_elements == 1) {
      ...
      // axes / surface area / etc. Right now the sorting is redundant since
      // we are sorting with the same parameters at every depth. This will soon
      // change with the improvements, for example, sorting by different axes
      // at different depths if we choose the axis with the greatest spread.
      std::sort(start, end, [](const CentroidPair& a, const CentroidPair& b) {

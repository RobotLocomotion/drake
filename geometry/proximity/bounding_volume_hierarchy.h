...
struct BVNode {
  ...
  explicit BVNode(const AABB aabb_in) : aabb(std::move(aabb_in)) {}

  BVNode(const BVNode& node) { *this = node; }
...
/** BoundingVolumeHierarchy is an acceleration structure for performing spatial
 ...
 @pre    Assumes that the mesh remains static and is not mutable. Updates to
         the mesh will leave this BVH stale.
...
class BoundingVolumeHierarchy {
 public:
  ...
  BoundingVolumeHierarchy& operator=(const BoundingVolumeHierarchy& bvh) {
    if (&bvh == this || bvh.root_node_ == nullptr) return *this;

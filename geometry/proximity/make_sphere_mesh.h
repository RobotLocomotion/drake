...
/** Creates a finer volume mesh approximating a unit sphere (not generally
 ...
 @returns  A new mesh (the refined version of the input mesh) and the
 corresponding classifications of the new mesh's vertices. The new mesh is a
 viable mesh for further refinement by this method.
...
std::pair<VolumeMesh<T>, VolumeVertexIndex> RefineUnitSphereMeshOnSurface(
  ...
  // An enumeration of tet edges which lie on the surface of the sphere.
...
/** Creates a volume mesh for the given `sphere`; the level of tessellation is
 ...
 coarsest possible mesh is a tessellated octohedron.

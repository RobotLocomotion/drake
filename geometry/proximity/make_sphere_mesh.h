...
/** Creates a finer volume mesh approximating a unit sphere (not generally
 ...
 splitting each edge. Four tests are created from those four faces and the
 ...
 */
template <typename T>
std::pair<VolumeMesh<T>, VolumeVertexIndex> RefineUnitSphereMeshSurface(
  ...
  // new vertex that sits in the middle of that same edge.
  std::unordered_map<SortedPair<VIndex>, VIndex> edge_vertex_map;
  // An enumeration of the edges of the test that lie on the surface; the
  // expectation that the zeroth vertex of every tet is the center vertex.
  // An enumeration of all of the edges of a tet.
  ...
  if (sparse_internal_vertices) {
    ...
    for (int level = 1; level <= refinement_level; ++level) {
      std::tie(mesh, center_index) =
          RefineUnitSphereMeshSurface<T>(mesh, center_index);
...
/** Creates a volume mesh for the given `sphere`; the level of tessellation is
 ...
 @param sparse_internal_vertices    If `true`, there will be only a single
                            interior vertex (at the sphere center) which may
                            lead to long skinny tetrahedra, depending on the
                            `resolution_hint`. Otherwise, the sphere will be
                            tessellated with interior vertices to maintain
                            tetrahedra with good aspect ratios (but producing
                            a mesh with many more tetrahedra).
...
VolumeMesh<T> MakeSphereVolumeMesh(const Sphere& sphere,
  ...
  VolumeMesh<T> unit_mesh =
      MakeUnitSphereMesh<T>(std::min(L, 8), sparse_internal_vertices);
  std::vector<VolumeVertex<T>> vertices;
...
SurfaceMesh<T> MakeSphereSurfaceMesh(const Sphere& sphere,
  ...
  return ConvertVolumeToSurfaceMesh<T>(
      MakeSphereVolumeMesh<T>(sphere, resolution_hint, false));
}

...
FeatureNormalSet::FeatureNormalSet(const TriangleSurfaceMesh<double>& mesh_M)
  ...
  // Meshes with "sharp" features are bad.  They make calculation of the
  // vertex normals and edge normals too sensitive to numerical rounding.
  //
  // Think of the simplest possible knife model of two adjacent triangles
  // sharing a single edge. The two triangles would have a small angle
  // between them, creating a "sharp edge".  If the edge is too sharp, the
  // mesh will be rejected.  We have a generous tolerance -- the angle can
  // be less than a degree -- but it would be better to stay well away
  // from impractically sharp edges.
  //
  ...
  //
  // We can compute a tolerance based on a minimum dihedral angle θ between
  // adjacent faces using the following formula: ‖nᵢ + nⱼ‖² = 2(1-cos(θ)),
  // where nᵢ and nⱼ are the face normals of the two adjacent faces.

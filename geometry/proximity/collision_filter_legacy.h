...
class CollisionFilterLegacy {
 public:
  ...
  void RemoveGeometry(uintptr_t id) {
    auto iter = collision_cliques_.find(id);
    if (iter != collision_cliques_.end()) collision_cliques_.erase(iter);
  }
  ...
  void UpdateEncoding(uintptr_t old_encoding, uintptr_t new_encoding) {
    ...
    collision_cliques_.insert({new_encoding, std::move(old_iter->second)});

 ...
 protected:
  ...
  static std::unique_ptr<GeometryInstance> make_sphere_instance(
    ...
    return make_unique<GeometryInstance>(Isometry3<double>::Identity(),
                                         make_unique<Sphere>(radius), name);
  }
...
TEST_F(SceneGraphTest, TransmogrifyWithoutAllocation) {
  ...
  (void)s_id;
  (void)scene_graph_ad;
  EXPECT_NO_THROW(scene_graph_ad.RegisterAnchoredGeometry(
      s_id, make_sphere_instance("second_sphere")));
  ...
  SceneGraph<AutoDiffXd>& scene_graph_ad2 =
      *dynamic_cast<SceneGraph<AutoDiffXd>*>(system_ad.get());
  EXPECT_THROW(scene_graph_ad2.RegisterAnchoredGeometry(
                   s_id, make_sphere_instance("third_sphere")),
               std::logic_error);
}

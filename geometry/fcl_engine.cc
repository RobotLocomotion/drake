...
class EncodedData {
 public:
  EncodedData(int index, bool is_dynamic)
    ...
    // NOTE: encoded data is anchored implicitly (the high order bit in
    // converting an int to a size_t is always 0.
  }
  ...
  // Encodes the data into the collision object.
  void encode(fcl::CollisionObject<double>* object) {
...
// Assumes the input vector has already been cleared.
...
void BuildTreeFromReference(
    ...
    const std::unordered_map<const fcl::CollisionObjectd*,
                             fcl::CollisionObjectd*>& object_map,
  ...
  for (auto* other_object : other_objects) {
    target->registerObject(object_map.at(other_object));
  }
}
...
GeometryIndex FclEngine<T>::AddDynamicGeometry(const Shape& shape) {
  ...
  // Implicitly add a tree if one didn't already exist for the given source_id.
  dynamic_tree_.registerObject(fcl_object);
  GeometryIndex index(static_cast<int>(dynamic_objects_.size()));
  EncodedData(index, true /* is dynamic */).encode(fcl_object);
...
AnchoredGeometryIndex FclEngine<T>::AddAnchoredGeometry(
  ...
  EncodedData(index, false /* is dynamic */).encode(fcl_object);

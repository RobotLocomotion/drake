...
class FileStorage final {
 public:
  ...
  /* Adds the given `content` to the database. The `content` argument will be
  empty after this call (you can use the return value, instead). If possible,
  this function re-uses an existing entry, i.e., this function will "intern" the
  string. */
  Handle Insert(std::string&& content);

  /* Returns the database content with the given checksum, or when not found in
  returns default-constructed a Handle (with content == nullptr). */
  [[nodiscard]] Handle Find(const Sha256& sha256) const;

  /* Returns the entire database. */
  std::vector<Handle> DumpEverything() const;
 ...
 private:
  struct Item {
    ...
    // metadata in the Item record, e.g., the original filename, mime type, etc.
  };

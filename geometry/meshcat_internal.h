...
@param[in,out] gltf_contents The glTF data named in `source`. It will be edited
  in place to replace the URIs. (We assume that the caller has created this as
  a unique, editable copy of the glTF data, whether from on-disk or in-memory,
  so here we can just operate on that string as an [in,out] parameter.)

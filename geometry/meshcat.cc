...
class SceneTreeElement {
 public:
  ...
  struct Message {
    ...
    `assets` used by the prior value of bytes). */
    ...
    std::vector<FileStorage::Handle> assets;
  };
...
(value) => this.connection.send(msgpack.encode({{
  ...
  std::string CalcStandaloneHtml() const {
    ...
    std::vector<FileStorage::Handle> assets = file_storage_.DumpEverything();
    ...
    if (url_path.substr(0, 12) == "/cas/sha256/") {
      ...
      if (handle.content == nullptr) {
        drake::log()->warn(
            ...
            suffix, file_storage_.DumpEverything().size());

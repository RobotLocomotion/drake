...
class Meshcat::Impl {
 public:
  ...
  // will first join the websocket thread and throw an exception; the
  // destructor will not be run.
  //
  // Otherwise, the postconditions are that:
  //   loop_ is non-null;
  ...
  void ThrowIfWebsocketThreadExited() const {
    ...
    OperatingMode stopping = kStopping;
    const bool was_stopping = mode_.compare_exchange_strong(stopping, kFinished);
    // XXX
    if (was_stopping) {
...
(value) => this.connection.send(msgpack.encode({{
 ...
 private:
  ...
  // (because the loop isn't running), but they will be correctly destroyed when
  ...
  // throwing, or during the destructor as normal), it sets mode_ to kFinished
  // to indicate that it will never post into the loop again.

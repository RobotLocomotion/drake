...
class SceneTreeElement {
 public:
  ...
  // Returns a rope that implements the entire tree directly in javascript.
...
(value) => this.connection.send(msgpack.encode({{
  ...
  std::string CalcStandaloneHtml() const {
    ...
    // Insert a URL hook that knows how to serve the CAS database.
    // (See FileStorage and GetCasUrl for details about CAS.)
    std::vector<std::string> commands;
    commands.push_back("casResources = {};");
    ...
    // Replay the websocket comments.
    std::vector<std::string> scene_commands = scene_tree_root_.CreateCommands();
    std::move(scene_commands.begin(), scene_commands.end(),
    ...
    // Replace the websockets connection logic with the equivalent commands.
    std::regex block_re(

 ...
 public:
  ...
  void SetObject(std::string_view path, const Shape& shape, const Rgba& rgba) {
    ...
    if (std::holds_alternative<internal::MeshData>(data.object.object)) {
      ...
      // Note: The reifier only provides a buffered geometry data instance for
      // Convex shapes. Make sure the flatShading is set appropriately.
      const bool force_flat = dynamic_cast<internal::BufferGeometryData*>(
                                  data.object.geometry.get()) != nullptr;
      material->flatShading = force_flat;

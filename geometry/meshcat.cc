...
class Meshcat::WebSocketPublisher {
 public:
  ...
  WebSocketPublisher()
      : main_thread_id_(std::this_thread::get_id()), prefix_("drake") {
  ...
  void SetObject(std::string_view path, const Shape& shape, const Rgba& rgba) {
    ...
    loop_->defer([this, app = app_, path = data.path, msg = message.str()]() {
  ...
  void SetProperty(std::string_view path, const std::string& property,
    ...
    // Note: pass all temporaries by value.
    loop_->defer(
        [this, app = app_, path = data.path, property, msg = message.str()]() {
          app->publish("all", msg, uWS::OpCode::BINARY, false);
          SceneTreeElement& e = scene_tree_root_[path];
  ...
  std::string GetPackedProperty(std::string_view path,
    ...
    // Note: pass all temporaries by value.
    loop_->defer(
        [this, path = FullPath(path), property, p = std::move(p)]() mutable {
          if (!scene_tree_root_.Contains(path)) {
            p.set_value("");
          } else {
            const SceneTreeElement& e = scene_tree_root_.At(path);
 ...
 private:
  ...
  std::string FullPath(std::string_view path) const {
    DRAKE_DEMAND(std::this_thread::get_id() == main_thread_id_);
    if (static_cast<int>(path.size()) > 1 && path.back() == '/') {
...
const std::string& Meshcat::prefix() const { return publisher_->prefix(); }

void Meshcat::set_prefix(std::string_view prefix) {
  publisher_->set_prefix(prefix);
}
...
void Meshcat::SetProperty(std::string_view path, const std::string& property,
                          bool value) {

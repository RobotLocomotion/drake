...
class Meshcat::WebSocketPublisher {
 public:
  ...
  WebSocketPublisher(const std::optional<int> port, std::string web_url_pattern)
      : prefix_("/drake"),
        main_thread_id_(std::this_thread::get_id()),
        web_url_pattern_(std::move(web_url_pattern)) {
...
Meshcat::Meshcat(std::optional<int> port, std::string web_url_pattern) {
  // Sanity-check the pattern.
  const std::string url = fmt::format(web_url_pattern, fmt::arg("port", 80));

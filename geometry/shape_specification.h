...
/** Definition of a general (possibly non-convex) triangular surface mesh.
 ...
 Meshes can be used with all three roles but, currently, the mesh is replaced
 by its convex hull when assigned a proximity role. See the documentation of
 QueryObject's proximity queries for more details. The one notable exception is
 for a Mesh specified as rigid hydroelastic; in that case, the general mesh is
 used directly. This convex-hull substitution is a regrettable stop-gap solution
 until we fully support general, non-convex meshes throughout proximity queries.

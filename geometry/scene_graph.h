...
class SceneGraph final : public systems::LeafSystem<T> {
 public:
  ...
  /** @name     SceneGraph management of render labels
   ...
   geometry. The definition of "meaningful" labels can be done in one of two
   ways:

   1. The exercising application can define arbitrary RenderLabel values (e.g.,
   `RenderLabel gripper_label = RenderLabel(17);`) and store that in a database
   for future reference.
   2. The exercising application can defer the responsibility to SceneGraph to
   allocate, manage, and report RenderLabel values. This latter approach is
   of particular significance if there are _multiple_ geometry sources that
   independently need to allocate RenderLabel values that should _not_
   interfere with each other (i.e., two different semantic classes should have
   two different RenderLabel values).

   The methods in this section support this _second_ workflow.
  ...
  /** Removes the indicated `role` from any geometry directly registered to the
   ...
   Note: if removing a _perception_ role, it will remove the geometry from _all_
   registered render engines. To remove a geometry from specific renderers, use
   the @ref RemoveFromRenderer(const std::string&, SourceId, FrameId)
   "RemoveFromRenderer()" variant for frames.
  ...
  /** Removes the indicated `role` from the geometry indicated by `geometry_id`.
   Note: if removing a _perception_ role, it will remove the geometry from _all_
   registered render engines. To remove a geometry from specific renderers, use
   the @ref RemoveFromRenderer(const std::string&, SourceId, GeometryId)
   "RemoveFromRenderer()" variant for individual geometries.
  ...
  /** systems::Context-modifying variant of
   @ref RemoveFromRenderer(std::string,SourceId,Frame) "RemoveFromRenderer()"
   for frames. Rather than modifying %SceneGraph's model, it modifies the copy
   of the model stored in the provided context.  */
  int RemoveFromRenderer(systems::Context<T>* context,
                          const std::string& renderer_name, SourceId source_id,
  ...
  /** systems::Context-modifying variant of
   @ref RemoveFromRenderer(std::string,SourceId,Frame) "RemoveFromRenderer()"
   for individual geometries. Rather than modifying %SceneGraph's model, it
   modifies the copy of the model stored in the provided context.  */
  int RemoveFromRenderer(systems::Context<T>* context,
                          const std::string& renderer_name, SourceId source_id,
                          GeometryId geometry_id) const;

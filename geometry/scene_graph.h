...
with <em style="color:gray">(source name)</em>.
 ...
 geometry source must also provide pose/position values for all of the
geometries the source owns, via a port connection on %SceneGraph. For N geometry
sources, the %SceneGraph instance will have N pose/position input ports.
 ...
 <!-- TODO(xuchenhan-tri): Modify this section when position ports are added.
--> For each registered geometry source, there is one input port for each order
of kinematics values (e.g., pose, velocity, and acceleration). If a source
registers a frame, it must connect to these ports (although, in the current
version, only pose is supported). Failure to connect to the port (or to provide
valid kinematics values) will lead to runtime exceptions.
 ...
 depends on a system's Context. Particularly, a rigid dynamic geometry is
 _fixed_ to a _frame_ whose kinematics values depend on a context. As the frame
 moves, the geometries fixed to it move with it. On the other hands, a
deformable dynamic geometry has a mesh representation whose vertices' positions
can change and are expressed in the frame it is attached to. Therefore, to
...
registered-in frame. The work flow is as follows:
   ...
   (GeometrySource::RegisterGeometry()).
     - For rigid geoemtries,their pose relative to the frame to which it is
     fixed. For deformable geometries, the positions of their mesh vertices are
expressed in the registered-in frame.
 ...
 <!-- TODO(xuchenhan-tri): Modify this section when position ports are added.
  ...
  /** @name             Topology Manipulation
   ...
   - The geometry source can then immediately register "anchored" geometry --
     ...
     move.
   - For rigid geometries that need to move based on the source's state, the
     geometry source must first register a GeometryFrame. In fact, rigid
   geometries never move directly; it is the frames to which they are affixed
   that move. A geometry source can register a frame via the RegisterFrame()
   methods.
   ...
   - (Experimental): Deformable geometries differ from rigid geometries in that
     it must have a meshed representation, and the vertices of the mesh can be
     moved in the frame the geometry is registered-in. Deformable geometries can
   be registered via calls the to (experimental) RegisterDeformableGeometry()
   methods.
  ...
  /** Registers a new deformable geometry G for this source. This hangs geometry
   G on a previously registered frame F (indicated by `frame_id`). The positions
   of the vertices of the deformable geometry are defined in the frame F (i.e.,
   `q_FG`). Returns the corresponding unique geometry id.
   ...
   @throws std::exception  if a) the `source_id` does _not_ map to a
                           ...
                           d) the geometry's name doesn't satisfy the
                           requirements outlined in GeometryInstance, or
                           e) the geometry doesn't have a mesh representation.
  */

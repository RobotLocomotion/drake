...
class SceneGraph final : public systems::LeafSystem<T> {
 public:
  ...
  /** Removes the indicated `role` from any geometry directly registered to the
   ...
   @note When removing a _perception_ role, it will remove the geometry from
   _all_ registered render engines. To remove a geometry from specific
   renderers, use the
   @ref RemoveFromRenderer(const std::string&, SourceId, FrameId)
  ...
  /** Removes the indicated `role` from the geometry indicated by `geometry_id`.
   ...
   _all_ registered render engines. To remove a geometry from specific
   renderers, use the
   @ref RemoveFromRenderer(const std::string&, SourceId, GeometryId)

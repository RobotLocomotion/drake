...
class SceneGraph final : public systems::LeafSystem<T> {
 public:
  ...
  /** Return an inspector on the system's *model* scene graph data.
   This should always be invoked as:

   ```c++
   const SceneGraphInspector<T>& inspector = scene_graph.model_inspector();
   ```

   Failure to capture the returned value in a const reference will lead to
   (possibly inscrutable) compilation errors.

...
/** SceneGraph serves as the nexus for all geometry (and geometry-based
 ...
 geometry sources, the %SceneGraph instance will have N pose/position input
 ports.
 ...
 registered-in frame. The work flow is as follows:
   ...
   3. Register one or more rigid geometries to a frame (RegisterGeometry()),
      and/or one or more deformable geometries to a frame
      (RegisterDeformableGeometry()).
     - For rigid geometries, their poses relative to the frame to which it is
...
class SceneGraph final : public systems::LeafSystem<T> {
 public:
  ...
  /** Registers a new deformable geometry G for this source. This registers
   geometry G on a previously registered frame F (indicated by `frame_id`). The
   registered geometry has a meshed representation. The positions of the
   vertices of this mesh representation are defined in the frame F (i.e.,

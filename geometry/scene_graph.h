...
class SceneGraph final : public systems::LeafSystem<T> {
 public:
  ...
  /** Filters out possible collisions between members of the given `set`
   `G = {g₀, g₁, ..., gₘ}`. The pairs `(gᵢ, gⱼ), ∀ gᵢ, gⱼ ∈ G` will be filtered
   out of consideration.

   @throws std::logic_error if the set includes ids that don't exist in the
                            scene graph.  */
  void DisallowCollisionsWithin(const GeometrySet& set);

  /** Filters out possible collisions between members of the two sets. If
   `A = {a₀, a₁, ..., aₘ}` and `B = {b₀, b₁, ..., bₙ}`, then the pairs
   ...
   @throws std::logic_error if the groups include ids that don't exist in the
                            scene graph.   */
  void DisallowCollisionsBetween(const GeometrySet& setA,

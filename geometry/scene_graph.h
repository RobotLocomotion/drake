...
/** SceneGraph serves as the nexus for all geometry (and geometry-based
 ...
 registers a frame or a deformable geometry, it must connect to these ports.
 Failure to connect to the port (or to provide valid kinematics values) will
 ...
 __configuration port__: An abstract-valued port providing a map from the
 GeometryId of all deformable geometries to their world space configurations
 (i.e. the vertex positions of their mesh representation in the world frame).
 All registered deformable geometries must be accounted for and only geometries
 registered by a source can be included in its output port.
...
class SceneGraph final : public systems::LeafSystem<T> {
 public:
  ...
  /** Registers a new, named source to the geometry system. The caller must save
   ...
   Similarly, if deformable geometry is registered
  ...
  /** Given a valid source `id`, returns a _configuration input port associated
   with that `id`. This port is used to communicate _configuration_ data for
   registered frames.
   @throws std::exception if the source_id is _not_ recognized.  */
 ...
 private:
  ...
  // The cache index for the pose and configuration update cache entry.

...
/** MeshcatContactVisualizer is a system that publishes a ContactResults to
Meshcat; drawing double-sided arrows at the location of the contact force with
length scaled by the magnitude of the contact force.  The most common use of
this system is to connect its input port to the contact results output port of
a `MultibodyPlant`.

 @system
 name: MeshcatContactVisualizer input_ports:
 - contact_results
 @endsystem
...
class MeshcatContactVisualizer final : public systems::LeafSystem<T> {
 public:
  ...
  will remove all geometry/transforms added by the visualizer, or by a previous
  instance of this visualizer using the same prefix.  Use the

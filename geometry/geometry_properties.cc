...
void GeometryProperties::AddGroup(const std::string& group_name) {
  if (values_.count(group_name) == 0) {
    values_.emplace(group_name, Group{});
  } else {
    throw std::logic_error("AddGroup(): Cannot add group named '" + group_name +

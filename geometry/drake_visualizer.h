...
/** A system that publishes LCM messages compatible with the `drake_visualizer`
 ...
 If %DrakeVisualizer is working on illustration geometries, the appearance of
 that geometry in the visualizer will be determined by the values in that
 geometry's IllustrationProperties.
 ...
 The ("phong", "diffuse") generally defines the color of the geometry in
 `drake_visualizer`. The notable exception is if the geometry is a Mesh which
 includes a material file (.mtl) which references a diffuse texture. If the
 property is missing, %DrakeVisualizer will apply a default color (as defined
 in DrakeVisualizerParams).

 If %DrakeVisualizer is working on either proximity or perception properties, it
 uses the following logic to determine the geometry color:

   1. First it will determine if the proximity/perception properties has an
      Rgba-valued ("phong", "diffuse") property and use that if found.
   2. Otherwise, it will determine if the geometry *also* has the illustration
      role assigned and use the color stored in the illustrations properties
      ("phong", "diffuse") property (if it exists).
   3. Otherwise, the default color is applied.

 This has an interesting implication. The appearance of collision geometry can
 be controlled on a per-geometry basis by assigning an explicit Rgba value to
 the ("phong", "diffuse") property in the geometry's ProximityProperties.

 @note If collision geometries are added to SceneGraph by parsing URDF/SDF
 files, they will not have diffuse values. Even if elements are added to the
 specification files; it is not currently parsed. They must be added to the
 geometries after parsing.

 <h3>Thread safety</h3>

 Using this system in multiple, simultaneous threads can easily lead to
 undesirable behavior. All threads will be broadcasting messages on the same
 LCM channels and the consumers of those messages will attempt to display the
 result of interleaving those channels. The System is sufficiently threadsafe
 to allow this usage, but don't expect the visualized result to be meaningful.
 */
class DrakeVisualizer : public systems::LeafSystem<double> {
 public:
  ...
  /** Creates an instance of %DrakeVisualizer.
   ...
   @param lcm     An optional LCM interface. If none is provided, this system
                  will own its own instance. If one is provided it must remain
                  valid for the lifetime of this object.
   @param params  The set of parameters to control this system's behavior.
   @pre `params.publish_period > 0`.
   @pre `params.role != Role::kUnassigned`.
   */
 ...
 private:
  ...
  /* DrakeVisualizer stores a "model" of what it thinks is registered in the
   ...
   DrakeVisualizer can still ensure that drake_visualizer state is consistent
   with the messages it is about to send.

   To support this functionality, we have the model (GeometryVersion) and a
   mutex that will allow updating that model safely. More particularly, we use
   the mutex to make the entire SendGeometryMessage method atomic; we don't want
   a load-draw pair of messages to be interrupted.  */
  ...
  /* The LCM interface: the owned (if such exists) and the work interface
   (whether owned or not). The work method is mutable because we non-const

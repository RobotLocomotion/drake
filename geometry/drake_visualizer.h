...
class DrakeVisualizer final : public systems::LeafSystem<T> {
 ...
 private:
  ...
  /* This function is used in the copy constructor. If this DrakeVisualizer owns
   * its LCM interface, then create a new LCM interface with the same url.
   * Otherwise just return the stored lcm, and the copy-constructed
   * DrakeVisualizer will share the LCM interface with this one. */
  lcm::DrakeLcmInterface* CopyLcmIfNotOwned() const;

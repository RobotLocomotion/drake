...
/** A system that publishes LCM messages compatible with the `drake_visualizer`
 ...
 It is common practice to parallelize computations by exercising a
 systems::Diagram in multiple threads by have unique per-thread systems::Context
 instance. If the systems::Diagram includes an instance of %DrakeVisualizer,
 it can lead to undesirable behavior in the broadcast messages. All threads will
 be broadcasting messages on the same LCM channels. There are no guarantees as
 to how those messages will be interleaved or received by consumers of the
 messages leading to incoherent results, or even visualizer failure.

 It should be emphasized, that sharing a *single* systems::Context instance
 across multiple threads is *strongly* discouraged and can lead to catastrophic
 failure.  */

...
/** A system that publishes LCM messages compatible with the `drake_visualizer`
 ...
 <h3>Ad hoc broadcasting</h3>

 In addition to functioning within the framework, %DrakeVisualizer provides a
 collection of static methods that can be used to broadcast geometry data
 in an ad hoc manner. For more details see
 @ref drake_viz_add_hoc "Ad hoc broadcasting".
*/
class DrakeVisualizer : public systems::LeafSystem<double> {
 public:
  ...
  /** @name Ad hoc broadcasting

   @anchor drake_viz_add_hoc
   (Advanced) These methods provide ad hoc access to broadcasting geometry in
   both draw and load messages. The geometry can be the model geometry stored in
   a SceneGraph instance, or a particular instance of it stored in SceneGraph's
   Context.

   Load and draw messages must be coordinated. If a load message is broadcast
   for one scene, but a draw message is broadcast for another, the result will
   be nonsensical (and most likely lead to errors in the receiving visualizer).
   It is the responsibility of the caller to make sure load and draw messages
   are propertly coordinated.

   The LCM interface *must* be provided by the caller and cannot be `nullptr`.

   The messages can be configured via an instance of DrakeVisualizerParams to
   have the same effect as documented for DrakeVisualizer.

   @note There is no `Context`-free variant of`BroadcastDraw`. The draw message
   requires poses which can only come from the context.   */
  //@{

  /** Broadcasts a load message for the *model* geometry for the given
   SceneGraph instance. The load message's time value will be zero.
   @pre `lcm != nullptr`.  */
  static void BroadcastLoad(const SceneGraph<double>& scene_graph,
                            lcm::DrakeLcmInterface* lcm,
                            DrakeVisualizerParams params = {});

  /** Broadcasts a load message for the geometry stored in the given
   SceneGraph Context instance. The load message's time value will be drawn
   from the context.
   @pre `context` must be allocated by `scene_graph`.
   @pre `lcm != nullptr`.  */
  static void BroadcastLoad(const systems::Context<double>& context,
                            const SceneGraph<double>& scene_graph,
                            lcm::DrakeLcmInterface* lcm,
                            DrakeVisualizerParams params = {});

  /** Broadcasts a draw message for the geometry stored in the given
   SceneGraph Context instance. The draw message's time value will be drawn
   from the context.
   @pre `context` must be allocated by `scene_graph`.
   @pre `lcm != nullptr`.  */
  static void BroadcastDraw(const systems::Context<double>& context,
                            const SceneGraph<double>& scene_graph,
                            lcm::DrakeLcmInterface* lcm,
                            DrakeVisualizerParams params = {});
  //@}

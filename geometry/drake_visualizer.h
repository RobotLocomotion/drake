...
/** A system that publishes LCM messages compatible with the `drake_visualizer`
 ...
 Using a single instance of this system in multiple, simultaneous threads can
 easily lead to undesirable behavior. All threads will be broadcasting messages
 on the same LCM channels and the consumers of those messages will attempt to
 display the result of interleaving those channels. The System is sufficiently
 threadsafe to allow this usage, but don't expect the visualized result to be
 meaningful.
 */

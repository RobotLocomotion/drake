...
template <typename T>
bool GeometryState<T>::has_hydroelastic_representation(
    GeometryId geometry_id) const {
  return geometry_engine_->hydroelastic_geometries().hydroelastic_type(
             geometry_id) != internal::HydroelasticType::kUndefined;
}

template <typename T>
std::variant<std::monostate, const SurfaceMesh<double>*,
             const VolumeMesh<double>*>
GeometryState<T>::get_hydroelastic_mesh_representation(
    GeometryId geometry_id) const {

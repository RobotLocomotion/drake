...
const std::vector<RenderMesh>& GeometryState<T>::GetDrivenRenderMeshes(
  ...
  switch (role) {
    case Role::kPerception:
      return driven_mesh_data_.at(Role::kPerception).render_meshes(id);
    case Role::kIllustration:
      return driven_mesh_data_.at(Role::kIllustration).render_meshes(id);
    case Role::kProximity:
      return driven_mesh_data_.at(Role::kProximity).render_meshes(id);
...
void GeometryState<T>::RegisterDrivenMesh(GeometryId geometry_id, Role role) {
  ...
  internal::DrivenMeshData* meshes_map =
      role == Role::kProximity ? &driven_mesh_data_[Role::kProximity]
      : role == Role::kIllustration ? &driven_mesh_data_[Role::kIllustration]
                                    : &driven_mesh_data_[Role::kPerception];
  meshes_map->SetMeshes(geometry_id, std::move(driven_meshes),

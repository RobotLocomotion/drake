...
void GeometryState<T>::Rename(
  ...
  internal::InternalFrame old_frame = frames_[frame_id];
  SourceId source_id = old_frame.source_id();
  ...
  frames_[frame_id].Rename(new_name);
}
...
template <typename T>
void GeometryState<T>::Rename(GeometryId geometry_id,
  ...
  geometries_[geometry_id].Rename(new_name);
}

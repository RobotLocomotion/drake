...
int GeometryState<T>::NumDynamicGeometries() const {
  ...
  for (const auto& pair : frames_) {
    ...
    // TODO(xuchenhan-tri): Account for deformable geometries.
...
const math::RigidTransform<T>& GeometryState<T>::get_pose_in_world(
  ...
  if (geometry.is_deformable()) {
    throw std::logic_error(
        ...
        "get_positions_in_world() instead.");
  }
...
const VectorX<T>& GeometryState<T>::get_positions_in_world(
  ...
  if (!geometry.is_deformable()) {
    throw std::logic_error(
        "Rigid geometries are characterized by poses. Use get_pose_in_world() "
        "instead.");
  }
...
GeometryId GeometryState<T>::RegisterDeformableGeometry(
  ...
  InternalGeometry internal_geometry(source_id, geometry->release_shape(),
                                     ...
                                     resolution_hint);
  // The mesh defined in the geometry's frame.
  const VolumeMesh<double>* mesh = internal_geometry.mesh();
  DRAKE_DEMAND(mesh != nullptr);
  const InternalFrame& frame = frames_[frame_id];
  const RigidTransform<T> X_WG =
      X_WF_[frame.index()] * geometry->pose().cast<T>();
  VectorX<T> q_WG(mesh->num_vertices() * 3);
  for (int v = 0; v < mesh->num_vertices(); ++v) {
    q_WG.template segment<3>(3 * v) = X_WG * Vector3<T>(mesh->vertex(v));
  }

...
GeometryId GeometryState<T>::GetGeometryFromName(
  ...
  if (frame_id == InternalFrame::get_world_frame_id()) {
    for (const auto& pair : anchored_geometries_) {
      ...
      if (geometry.get_name() == canonical_name) return pair.first;
    }
    throw std::logic_error("The frame 'world' (" + to_string(frame_id) +
                           ") has no geometry with the canonical name '"
                           + canonical_name + "'");
  } else {
    ...
    for (GeometryId geometry_id : frame.get_child_geometries()) {
      ...
      if (geometry.get_name() == canonical_name) return geometry_id;
    }
    throw std::logic_error("The frame '" + frame.get_name() + "' (" +
                           to_string(frame_id) + ") has no geometry with the "
...
GeometryId GeometryState<T>::RegisterGeometry(
  ...
  const std::string trimmed_name = GetValidNameOrThrow(frame_id,
                                                       geometry->name());
...
GeometryId GeometryState<T>::RegisterAnchoredGeometry(
  ...
  // NOTE: It is important to test for name validity *before* adding this
  // geometry to the frame.
  const std::string trimmed_name = GetValidNameOrThrow(
...
bool GeometryState<T>::IsValidGeometryName(
  ...
  return TestNameValidity(frame_id,
                          detail::CanonicalizeStringName(candidate_name)) == OK;
}
...
std::string GeometryState<T>::GetValidNameOrThrow(
    FrameId frame_id, const std::string& name) const {
  std::string trimmed = detail::CanonicalizeStringName(name);

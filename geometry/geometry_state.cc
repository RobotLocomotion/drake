...
template <typename T>
optional<std::reference_wrapper<const VisualMaterial>>
GeometryState<T>::get_visual_material(GeometryId geometry_id) const {
  ...
  // property introspection functions.
  {
    ...
    if (iterator != geometries_.end()) {
      return std::reference_wrapper<const VisualMaterial>{
          iterator->second.get_visual_material()};
    }
  }
  {

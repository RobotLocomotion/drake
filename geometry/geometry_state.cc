...
void GeometryState<T>::ChangeShape(SourceId source_id, GeometryId geometry_id,
  ...
  // TODO(SeanCurtis-TRI) Allow changing deformable after the fact; this
  // requires coordination with MbP because the state of the deformable object
  // must be consistent between the two plants.
  if (geometry->is_deformable()) {
    throw std::logic_error(
  ...
  if (geometry->has_perception_role()) {
    ...
    AddToRenderersUnchecked(*geometry);
  }
}
...
void GeometryState<T>::AssignRole(SourceId source_id, GeometryId geometry_id,
  ...
  const bool added_to_renderer = AddToRenderersUnchecked(geometry);
...
template <typename T>
bool GeometryState<T>::AddToRenderersUnchecked(

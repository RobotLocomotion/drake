...
class RenderEngineGl final : public render::RenderEngine, private ShapeReifier {
 ...
 private:
  ...
  //
  // What gets cached is the unscaled convex hull of the file. If there are
  // multiple instances of the mesh file's convex hull with different scales,
  // then the single convex hull will be scaled accordingly.
  //
  // That may mean that this function must actually compute the convex hull
  // rather than the pre-scaled version provided by `convex`.
  //
  // @throws if `convex.convex_hull()` throws (e.g., no one else has asked for

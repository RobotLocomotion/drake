...
std::string GetPathKey(const MeshSource& mesh_source, bool is_convex) {
  ...
  if (mesh_source.is_in_memory()) {
    ...
    //    instead.
    //
    // It is highly unlikely that this problem will arise. If it were, the
    // in-memory mesh and path mesh would be equally susceptible to this
    // failure. If this ever becomes behavior we want to encourage, we would
    // have to create a cache key predicated on the full file *ecosystem* so
    // that we can detect if any part of the file family is unique, creating,
    // in some sense, a unique geometry.
    //
    // For now, we'll assume that anyone who does such a thing gets what they
    // deserve.
...
class RenderEngineGl::GltfMeshExtractor {
 public:
  ...
  vector<RenderGlMesh> ExtractMeshes(const MeshSource& mesh_source,
                                     const RegistrationData& data) {
    // To make the source available everywhere without passing it ad nauseam,
    // we alias it into a member *temporarily*.
    ScopeExit source_reset([this] {
      source_ = nullptr;
    });
    source_ = &mesh_source;

 ...
 public:
  ...
  /* Returns the unique OpenGl identifier for the texture with the given name.
   ...
   to load the image if:
     ...
     - the file_name isn't prefixed with "data://". Texture names presented as
       a data URI must be pre-loaded as an "in-memory" texture before accessing.
       See AddInMemoryImages().
  ...
  /* Adds a set of in-memory textures to the library. The textures are presented
   ...
   @pre Each name in the map has the prefix "data://" to prevent possible
   collision with real file.
   @pre The OpenGl context "partnered" to this library has been bound. */
  ...
  /* (Internal only) Used for testing. Allows for introspection of registered
   textures. */

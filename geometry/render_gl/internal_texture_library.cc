...
std::optional<GLuint> TextureLibrary::GetTextureId(
  ...
  if (file_name_in.starts_with("data://")) {
    throw std::runtime_error(fmt::format(
        "Trying to access an in-memory texture for a material but the texture "
...
void TextureLibrary::AddInMemoryImages(
    const std::map<std::string, std::vector<unsigned char>>& image_cache) {
  for (const auto& [name, texture] : image_cache) {
    DRAKE_DEMAND(name.starts_with("data://"));
    ...
    if (!maybe_metadata.has_value()) {
      // Failure to register an in-memory image will lead to failure *if* we
      // actually attempt to access the in-memory image. We're going to defer to
      // that act to give feedback.
      //
      // The feedback cannot discern the *reason* the in-memory image is
      // missing and may *suggest* an unsupported file type instead of a defect
      // in Drake where the image was failed to be registered. It is unlikely
      // that it will be attributable to a Drake bug for very long.
      continue;
    }

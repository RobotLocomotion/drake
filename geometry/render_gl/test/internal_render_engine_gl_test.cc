...
class RenderEngineGlTest : public ::testing::Test {
 ...
 protected:
  ...
  void RenderAndCompareAgainstRef(std::string_view image_file_name,
    ...
    // We're testing to see if the images are *coarsely* equal. This accounts
    // for the differences in CI's rendering technology from a local GPU. The
    // images are deemed equivalent if 80% of the channel values are within 20
...
//
// We'll replace the external base color texture with an embedded texture and
// then confirm a) that we get the same rendered image and b) the only image
// included in the engine's texture library is the embedded image.
//
...
TEST_F(RenderEngineGlTest, GltfEmbeddedTextures) {
  ...
  ASSERT_TRUE(library.textures().begin()->first.starts_with("data://"));
}

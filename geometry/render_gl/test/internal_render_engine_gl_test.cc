...
// indications that the updates are as expected.
TEST_F(RenderEngineGlTest, DeformableTest) {
  for (const bool use_texture : {false, true}) {
    ...
    if (use_texture) {
      SCOPED_TRACE("Deformable test with texture");
    } else {
      SCOPED_TRACE("Deformable test without texture");
    }
    PerformCenterShapeTest(renderer_.get());
    ...
    // Test normals are updated by making all vertex normals point to the x-axis
    // and thus perpendicular to both the light direction and camera direction
    // (both parallel to z-axis of world frame).
    auto new_nhat_W = initial_nhat_W;
    for (int r = 0; r < new_nhat_W.rows(); ++r) {
      new_nhat_W.row(r) = Vector3d(1, 0, 0);
    }

    // With the perpendicular normals, we expect to see "black" for inlier.
    // We also expect the object depth to increase by 0.5 as we translate all
    // vertices in the -z direction by 0.5.
    ResetExpectations();
    expected_color_ = Rgba(0.0, 0.0, 0.0, 1.0);
    expected_label_ = deformable_label;

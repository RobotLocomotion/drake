...
TEST_F(RenderEngineGlTest, ConvexTest) {
  ...
  // The cuboctahedron is bound by an the same axis aligned bounding box as
  // box.obj. Because we're scaling it, instead of reaching one in the +Wz
  // direction, it only reaches 0.5, so we need to adjust depth expectations
  // accordingly. But it's important we apply non-unit scale because of how
  // RenderEngineGl caches the convex hull and applies scale.

...
TEST_F(RenderEngineGlTest, DeformableTest) {
  for (const bool use_texture : {false, true}) {
    ...
    renderer_->UpdateDeformableConfigurations(
        id,
        std::vector<VectorXd>{
            flatten(translate_all_vertices(Vector3d(1.01, 0, 0)))},
        std::vector<VectorXd>{flatten(initial_nhat_W)});
    PerformCenterShapeTest(renderer_.get());
    ...
    // With the perpendicular normals, we expect to see "black" for inlier with
    // the inital vertex positions.
    ResetExpectations();
    expected_color_ = Rgba(0.0, 0.0, 0.0, 1.0);
    expected_label_ = deformable_label;

    renderer_->UpdateDeformableConfigurations(
        id, std::vector<VectorXd>{flatten(initial_q_WG)},

...
GTEST_TEST(ThreadSafetyTest, TextureLibrary) {
  ...
  // call GetTextureId(). So, we'll create one per thread.
  ...
  // Allocate per-thread contexts here; each is a copy of a single source
  // context so they share GPU memory. Do this cloning in its own loop so that
  // the actual asynchronous work has better odds of having the library
  // invocations overlap.
  contexts.reserve(kNumThreads);  // Presize; we can't move contexts.
  ...
  for (int i = 0; i < kNumThreads; ++i) {
    // Each thread gets a copy of source, so they all share GPU state.
...
/* We use Vector4<it> to compare the pixel values in an image with an expected
...
Vector4<int> RgbaVector(const Rgba& rgba) {
  ...
  return Vector4<int>(to_T(rgba.r()), to_T(rgba.g()), to_T(rgba.b()),
                      to_T(rgba.a()));
}

...
std::shared_ptr<const MemoryFile> FileStorage::Insert(
  ...
  // files being insert, so we'll simply provide an empty extension.
  ...
  auto fat_handle = std::make_shared<Impl::MemoryFileAndBackreference>();
  fat_handle->backreference = impl_;
  fat_handle->memory_file = std::move(new_memory_file);

  std::shared_ptr<MemoryFile> file(fat_handle, &(fat_handle->memory_file));

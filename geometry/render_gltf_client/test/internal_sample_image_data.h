...
// TODO(zachfang): If we end up deciding to encode kEmpty label value (or
// similar) as a special RGBA, we should update the default values to test that.
/* Recreates an ImageRgba8U image that should be exactly the same as the
 loaded test_colored_label_rgba_8U.png.
 ...
 Returns an image (width=3, height=2) where every combination of R, G, B, & A
 values represents a specific label value after the conversion to an actual
 label image. */
systems::sensors::ImageRgba8U CreateTestColoredLabelImage();

/* Recreates an ImageLabel16I image that each pixel should be properly converted
 to the corresponding label value given the colored label image generated by
 CreateTestColoredLabelImage().

...
static bool valid_render_engine(const char* flagname, const std::string& val) {
  if (val == "vtk")
    ...
    return true;
  drake::log()->error("Invalid value for {}: '{}'; options: 'vtk' or 'client'.",
                      flagname, val);
  return false;
}
...
bool ValidateSaveDirectory(const std::string& save_dir) {
  if (save_dir.empty()) return false;

  if (!filesystem::exists(save_dir) || !filesystem::is_directory(save_dir)) {
    drake::log()->info("Provided save_dir is invalid. Disabling image saving");
    return false;
  }
  return true;
}
...
int DoMain() {
  ...
  // Add the example scene containing various primitives and a textured mustard
  // bottle into a MultibodyPlant. We'll use MbP to *move* the mustard bottle by
  // giving it an initial velocity; we don't want to have to wait for gravity to
  // take effect to observe a difference in position.
  Parser parser{&plant};
  ...
  const bool save_images = ValidateSaveDirectory(FLAGS_save_dir);
  if (save_images) {

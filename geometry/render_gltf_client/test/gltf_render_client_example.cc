/* This example serves as the baseline demonstration of all the features a
 ...
 render images.  Please refer to the README.md in this test folder for
 instructions to launch a rendering server.
 ...
 plan to exercise in this example(s).
...
int DoMain() {
  ...
  // Add the example scene containing various primitives and a textured mesh
  // into a MultibodyPlant. We'll use MBP to *move* the mustard bottle (by
  ...
  if (FLAGS_render_on) {
    ...
    const FrameId world_id =
        plant->GetBodyFrameIdOrThrow(plant->world_body().index());
    RgbdSensor* camera = builder.AddSystem<RgbdSensor>(

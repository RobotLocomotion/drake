...
TEST_F(RenderEngineGltfClientGltfTest, PoseComputation) {
  ...
  /* Given the non-rigid transform of a node (X_WN), the rigid pose of the
   file's frame (X_WF) and the scale factor, extract the pose of the node in the
   file's frame. */
  auto extract_X_FN = [](const Matrix4<double> X_WN,
    ...
    T_inv.block<3, 1>(0, 3) = -X_WF.translation();
    Matrix4<double> R_inv = Matrix4<double>::Identity();
    R_inv.block<3, 3>(0, 0) = X_WF.rotation().inverse().matrix();

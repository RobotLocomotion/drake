...
// Exposes various private access functions for validation.
class RenderEngineGltfClientTester {
 public:
  ...
  void ExportScene(const std::string& export_path, ImageType image_type) const {
    engine_->ExportScene(export_path, image_type);
  }
...
TEST_F(RenderEngineGltfClientTest, ExportScene) {
  RenderEngineGltfClient engine{params_};
  Tester tester{&engine};
  ...
  for (const auto image_type :
    ...
    const std::string scene_file_path =
        fs::path(engine.temp_directory()) /
        fmt::format("{}.gltf", image_type_str);

    // Export the scene and check if the scene file exists.
    tester.ExportScene(scene_file_path, image_type);
    EXPECT_TRUE(fs::is_regular_file(scene_file_path));
  }

  const std::vector<fs::path> files =
      FindRegularFiles(engine.temp_directory());

...
TEST_F(RenderEngineGltfClientGltfTest, PoseComputation) {
  ...
  // 2. Contents are unmodified: glTF-originalroot nodes retain their original
  // file-frame format. In this case, we exploit the fact that all root nodes
...
GTEST_TEST(RenderEngineGltfClientNamesTest, GeometryNamesInExportedScene) {
  ...
  RenderEngineGltfClient* engine_raw = engine.get();
  ...
  const GeometryId obj_id = GeometryId::get_new_id();
  EXPECT_TRUE(engine_raw->RegisterVisual(
      obj_id,
      Mesh(
          FindResourceOrThrow("drake/geometry/render_gltf_client/test/tri.obj"),
          Vector3d::Ones()),
      properties, X_WG, /* needs_update= */ false, "my_obj_geom"));
  ...
  EXPECT_TRUE(engine_raw->RegisterVisual(
      ...
      Mesh(FindResourceOrThrow(
               "drake/geometry/render_gltf_client/test/tri_tree.gltf"),
           Vector3d::Ones()),
      properties, X_WG, /* needs_update= */ false, "my_gltf_geom"));

  // 6. Export the scene.
  const fs::path temp_dir = temp_directory();
  const fs::path output_path = temp_dir / "names_test.gltf";
  RenderEngineGltfClientTester::ExportScene(*engine_raw, output_path,
                                            ImageType::kColor);
  ...
  std::unique_ptr<RenderEngine> clone_base = engine_raw->Clone();

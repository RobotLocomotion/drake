...
class TestIntegration(unittest.TestCase):
    ...
    def _traverse_and_mutate(gltf, entry):
        ...
        if entry_type == dict:
            ...
            # Round float numbers till 12 decimal places due to the precision
            # differences across platforms.
            if all(isinstance(x, float) for x in entry):
                entry = [round(x, 12) for x in entry]
            elif not all(isinstance(x, (int, str)) for x in entry):
                for item in entry:
    ...
    def _normalize_for_compare(gltf):
        ...
        for platform-independent comparisons. Normalization includes:
    ...
    def _check_one_gltf(self, gltf, ground_truth_gltf):
        actual = TestIntegration._normalize_for_compare(gltf)
        expected = TestIntegration._normalize_for_compare(ground_truth_gltf)

...
class TestIntegration(unittest.TestCase):
    def setUp(self):
        ...
        # Wait for the server to spawn up fully.
        time.sleep(1)

        # Read the current stdout and scrape the actual port number.
        server_proc_output = self.server_proc.stdout.read1().decode("utf-8")
        self.server_port = None
        for line in server_proc_output.splitlines():
            match = re.search(r"Running on http://127.0.0.1:([0-9]+)/ ", line)
            if match:
                (self.server_port,) = match.groups()
        self.assertIsNotNone(self.server_port)

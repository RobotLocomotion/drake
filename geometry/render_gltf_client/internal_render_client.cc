...
/* Verifies the loaded image has the correct dimensions.
 If it doesn't, resets it to the correct size and throws an error. */
...
void RenderClient::LoadColorImage(const std::string& path,
  ...
  ImageIo io;
  io.SetFileFormat(ImageFileFormat::kPng);
  io.Load(path, color_image_out);
...
void RenderClient::LoadDepthImage(const std::string& path,
  ...
  ImageIo io;
  io.SetFileFormat([ext = fs::path{path}.extension()]() {
    if (ext == ".png") {
  ...
  ImageAny image_any = io.Load(path);
  std::visit(

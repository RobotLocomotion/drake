...
class DRAKE_NO_EXPORT RenderEngineGltfClient
 ...
 private:
  ...
  /* Override RenderEngineVtk's default handling of the mesh types. We want to
  ...
  // TODO(SeanCurtis-TRI) Based on file path, I should load a gltf *one time*.
  //  Each instance of it should be re-used. Assuming that all instances of
  //  a gltf always use the *same* materials, all I would have to worry about is
  //  the transforms applied to the root nodes. That means a GltfRecord should
  //  contain a reference to the canonical mesh, and a transform instead of the
  //  `contents` it has now.
  //
  //  Exporting the scene:

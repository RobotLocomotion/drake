...
/* Effectively sets the world pose of the single "geometry" represented by one
 ...
 transforms on the `root_nodes` of the glTF. Applying the pose to non-root
 nodes will change the composition of the geometry (e.g., moving pieces
 relative to each other). Presumably, `root_nodes` will be those nodes found by
 FindRootNodes().
...
void SetRootPoses(nlohmann::json* gltf,
  ...
  if (gltf->contains("nodes")) {
    ...
    for (const auto& [n, T_FN] : root_nodes) {
      ...
      nodes[n]["matrix"] = make_json(T_WF * T_FN);
    }
  }
}

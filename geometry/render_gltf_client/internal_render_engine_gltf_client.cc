...
using SceneId = Identifier<class SceneTag>;
...
/* Drake uses an explicit projection matrix with RenderEngineVtk to fine tune
 ...
 matrices, a Drake CameraInfo couldn't be fully expressed through its camera
 node.  To remedy that, RenderClient::RenderOnServer() will emit the complete
...
void RenderEngineGltfClient::DoRenderColorImage(
  ...
  const SceneId color_scene_id = SceneId::get_new_id();
  if (get_params().verbose) {
    LogFrameStart(ImageType::kColor, color_scene_id.get_value());
  }
  ...
  const std::string scene_path =
      ...
      GetSceneFileName(ImageType::kColor, color_scene_id.get_value());
...
void RenderEngineGltfClient::DoRenderDepthImage(
  ...
  const SceneId depth_scene_id = SceneId::get_new_id();

  if (get_params().verbose) {
    LogFrameStart(ImageType::kDepth, depth_scene_id.get_value());
  }
  ...
  const std::string scene_path =
      fs::path(temp_directory()) /
      GetSceneFileName(ImageType::kDepth, depth_scene_id.get_value());
...
void RenderEngineGltfClient::DoRenderLabelImage(
  ...
  const SceneId label_scene_id = SceneId::get_new_id();

  if (get_params().verbose) {
    LogFrameStart(ImageType::kLabel, label_scene_id.get_value());
  }
  ...
  const std::string scene_path =
      fs::path(temp_directory()) /
      GetSceneFileName(ImageType::kLabel, label_scene_id.get_value());

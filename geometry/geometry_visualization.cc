...
// NOTE: This method is declared as a friend of GeometryState so it can poke
// directly into GeometryState's internals. It is not declared in
// geometry_visualization.h because it is not and should not be part of the
// public API. This also allows me to separate the core GeometryState
// implementation from this LCM-dependent cruft. If this signature were to
// change, the friend declaration would likewise change.
//
// It is not "explicitly" declared in any header file. It is, however,
// implicitly declared in geometry_state.h when it is declared a friend of the
// GeometryState class. The signature here and there should be kept in sync.
lcmt_viewer_load_robot BuildLoadMessage(const GeometryState<double>& state) {
...
void DispatchLoadMessage(const GeometryState<double>& state) {
  ...
  lcmt_viewer_load_robot message = BuildLoadMessage(state);
  // Send a load message.
  DrakeLcm lcm;

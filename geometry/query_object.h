...
class QueryObject {
 public:
  ...
  /** @name                Position-dependent Introspection

   These methods provide access to introspect geometry and frame quantities that
   directly depend on the poses of the frames or positions of deformable
  ...
  */
  ...
  /** Reports the position of the deformable geometry indicated by `geometry_id`
   relative to the world frame.
   @sa GetPoseInWorld().
   @throws std::exception if the geometry `geometry_id` is not valid or is not
   a deformable geometry.  */
  const VectorX<T>& GetPositionsInWorld(

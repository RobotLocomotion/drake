 ...
 public:
  ...
  /**
   ...
   implementation. That has several implications, as described below:
     ...
     - We do not currently support conact between two geometries with the *same*
       compliance; one geometry *must* be soft, and the other *must* be rigid.
       If geometries with the same compliance collide, an exception will be
       thrown. More particularly, if such a geometry pair *cannot be culled* an
       exception will be thrown. No exception is thrown if the pair has been
       filtered.

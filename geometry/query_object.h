...
class QueryObject {
 public:
  ...
  /**
   Reports polygonal contact surfaces between pairs of intersecting geometries
   for hydroelastic contact model. It performs the same task as
   ComputeContactSurfaces() with a different representation of the
   output contact surfaces. The computation is subject to collision filtering.
   ...
   triangle with the same properties for the first-order numerical
   integrations. The representative triangle has the same centroid, area,
   pressure value, and pressure gradient as the contact polygon.

   The return type in this implementation is a std::vector of ContactSurface
   that uses the representative triangles, and, in the future, the return type
   will use the true polygonal representation.
  ...
  /** Reports pairwise intersections and characterizes each non-empty
   intersection as a polygonal contact surface _where possible_ and as a
   PenetrationAsPointPair where not. It performs the same task as
   ...
   In the current incarnation, the output parameter `surfaces` in this
   implementation uses ContactSurface consisting of one representative
   triangle for each contact polygon. In the future, it will use the true

...
std::shared_ptr<const FileStorage::Handle> LoadGltfUri(
  ...
  if (uri.substr(0, 5) == "data:") {
    ...
    if (pos == std::string_view::npos) {
      log()->warn(
          "Meshcat ignoring malformed data URI while loading {} for '{}'",
    ...
    if (!asset_data) {
      log()->warn(
          "Meshcat could not open '{}' while loading {} uri '{}' for '{}'",
          asset_filename.string(), array_hint, uri, gltf_filename.string());
      return nullptr;
    }
...
std::vector<std::shared_ptr<const FileStorage::Handle>> UnbundleGltfAssets(
  ...
  try {
    ...
    log()->warn(
        "Meshcat could not unbundle '{}': glTF parse error: {}",

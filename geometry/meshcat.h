...
class Meshcat {
  ...
  */
  ...
  /** @group Realtime Rate Reporting
   ...
   ways:
     ...
     - Immediate - a %Meshcat user can compute realtime rate themselves and
       ...
       computing the value and choosing the frequency at which this is done.
  ...
  /** Updates %Meshcat's knowledge of simulation time. Changes to simulation
   ...
   The realtime rate calculation can be "reset" in two ways. Both mechanisms
   will reset the value reported by GetRealtimeRate() to zero. They differ
   slightly in a subtle way:

     - Passing a value that is strictly less than the value of the previous
       invocation has the effect of re-initializing the calculation *and* making
       a first invocation with the value `sim_time`. So, if the simulation's
       context gets reset to `time = 0`, calls to this method passing the
       context time will implicitly reset realtime rate calculations
       accordingly.
     - Passing std::nullopt re-initializes the calculation. The *next*
       invocation with a defined `sim_time` value will be considered the *first*
       invocation.

...
Elements of the tree are referenced programmatically by a "/"-separated string
...
- A path that *does not* begin with "/" is treated as a relative path.
...
It is considered good practice to add your geometry using relative paths, so
that Delete() can clear all of the geometry from the scene.
...
"/Cameras".  See the meshcat documentation for their details.

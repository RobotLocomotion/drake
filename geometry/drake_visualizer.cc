...
/* Populate the given lcm message data with the given faceted mesh.
...
void SetFacetedMeshData(const TriangleSurfaceMesh<double>& surface_mesh,
                        lcmt_viewer_geometry_data* data_out,
                        const RigidTransformd& X_PG, const Rgba& in_color) {
  DRAKE_DEMAND(data_out != nullptr);
  lcmt_viewer_geometry_data& geometry_data = *data_out;
...
lcmt_viewer_geometry_data MakeHydroMesh(GeometryId geometry_id,
  ...
  SetFacetedMeshData(surface_mesh, &geometry_data, X_PG, in_color);
 ...
 public:
  ...
  void ImplementGeometry(const Convex& convex, void*) override {
    const TriangleSurfaceMesh<double> tri_mesh =
        internal::MakeTriangleFromPolygonMesh(convex.convex_hull());
    ...
    // the convex hull.
    SetFacetedMeshData(tri_mesh, &geometry_data_, RigidTransformd{},
                       Rgba(1, 1, 1, 1));
  }

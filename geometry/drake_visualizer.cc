...
void DrakeVisualizer::BroadcastLoad(const SceneGraph<double>& scene_graph,
...
void DrakeVisualizer::BroadcastLoad(const Context<double>& context,
  ...
  vector<DynamicFrameData> dynamic_frames;
  PopulateDynamicFrameData(query_object.inspector(), params, &dynamic_frames);
  SendLoadMessage(query_object.inspector(), params, dynamic_frames,
                  context.get_time(), lcm);
}

void DrakeVisualizer::BroadcastDraw(const Context<double>& context,
                                    const SceneGraph<double>& scene_graph,
                                    lcm::DrakeLcmInterface* lcm,
                                    DrakeVisualizerParams params) {
  DRAKE_DEMAND(lcm != nullptr);
  ...
  vector<DynamicFrameData> dynamic_frames;
  PopulateDynamicFrameData(query_object.inspector(), params, &dynamic_frames);
  SendDrawMessage(query_object, dynamic_frames, context.get_time(), lcm);
}

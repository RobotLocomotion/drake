...
DrakeVisualizer<T>::DrakeVisualizer(const DrakeVisualizer<U>& other)
    : DrakeVisualizer(other.CopyLcmIfNotOwned(), other.params_) {}
...
template <typename T>
lcm::DrakeLcmInterface* DrakeVisualizer<T>::CopyLcmIfNotOwned() const {
  if (owned_lcm_ != nullptr) {
    return new lcm::DrakeLcm(
        dynamic_cast<lcm::DrakeLcm*>(owned_lcm_.get())->get_lcm_url());
  } else {
    return lcm_;
  }
}

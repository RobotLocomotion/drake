...
void DrakeVisualizer<T>::SendLoadMessage(
  ...
  auto make_link =
      [&inspector, &message, &link_index, &params, &num_roles, &make_geometry](
          int robot_num, FrameId frame_id,
        ...
        for (const auto& role : params.roles) {
          ...
          link.robot_num = robot_num + role_index;
  ...
  if (anchored_links) {
    make_link(0, inspector.world_frame_id(), anchored_counts, "world");
  }

  // Load dynamic geometry into their own frames.
  for (const auto& [frame_id, geometry_counts, name] : dynamic_frames) {
    make_link(inspector.GetFrameGroup(frame_id) * num_roles, frame_id,
              geometry_counts, name);
  }
...
void DrakeVisualizer<T>::SendDrawMessage(
  ...
  for (const auto& [frame_id, geometry_counts, name] : dynamic_frames) {
    int role_index = -1;
    int robot_base = inspector.GetFrameGroup(frame_id) * num_roles;
    ...
    for (const auto& role : params.roles) {
      ...
      int i = link_index;
      message.robot_num[i] = robot_base + role_index;
      if (num_roles > 1) {

...
DrakeVisualizer::DrakeVisualizer(lcm::DrakeLcmInterface* lcm,
  ...
  // TODO(SeanCurtis-TRI) The down side of doing everything in an unrestricted
  //  update event is that *this* system's state will get copied for *every*
  //  publish, even though I will only rarely ever need to write to it. For this
  //  reason, it would be better to separate out the need to change the state
  //  from the read-only operation of updating poses. This will matter most
  //  if/when the state becomes larger.
...
void DrakeVisualizer::SendGeometryMessage(
  ...
  // TODO(SeanCurtis-TRI) When GeometryVersion is available, query the version
  //  availble via QueryObject::inspector() to determine if it matches this
  //  systems known version.

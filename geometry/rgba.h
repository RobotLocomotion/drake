...
class Rgba {
 public:
  ...
  /** Passes this object to an Archive.
   ...
   When serializing a struct with an %Rgba member, it should *not* simply
   "visit" the member. I.e.,

   @code
   struct BadStruct {
    template <typename Archive>
    void Serialize(Archive* a) {
      a->Visit(DRAKE_NVP(color));  // Introduces an extra layer of hierarchy.
    }
   ...
   struct GoodStruct {
    template <typename Archive>
    void Serialize(Archive* a) {
      color.Serialize(a, "color");  // The preferred spelling.
    }

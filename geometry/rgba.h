...
class Rgba {
 public:
  ...
  /** Passes this object to an Archive.
   ...
   When serializing a struct with an %Rgba member, there are two options:

   @code
   // This produces the following yaml, preserving the hierarchical structure.
   // 
   //    color:
   //      rgba: [r, g, b, a]
   ...
   // This produces the following yaml, flattening the structure.
   //
   //    color: [r, g, b, a]
   struct FlatStruct {
    template <typename Archive>
    void Serialize(Archive* a) {
      color.Serialize(a, "color");
    }
   ...
   See drake::visualization::VisualizationConfig::Serialize for a concrete
   example.

   Refer to @ref yaml_serialization "YAML Serialization" for background. */
  template <typename Archive>
  void Serialize(Archive* a, const char* name = "rgba") {
    ...
    a->Visit(MakeNameValue(name, &rgba));
    if (rgba.size() == 3) {

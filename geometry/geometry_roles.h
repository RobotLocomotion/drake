...
/** @defgroup geometry_roles Geometry Roles
 ...
 Drake has partitioned its geometric operations into two classes and has
 ...
 example, there can be multiple contact models that use both physical
 properties (e.g., stiffness and friction coefficients) as well as model
 parameters (e.g., dissipation).

 In such a case, the properties that are defined should be appropriate
 to the calculations performed (i.e., which contact model). The geometry
 properties support multiple parallel sets of properties *per role* (see
 GeometryProperties for how this mechanism works). To make a

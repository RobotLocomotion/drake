...
GTEST_TEST(GeometryProperties, ManagingGroups) {
  ...
  ASSERT_TRUE(properties.HasGroup(properties.default_group_name()));
  ...
  // Redundant add throws exception.
  DRAKE_EXPECT_THROWS_MESSAGE(
      properties.AddGroup(group_name), std::logic_error,
      ".*Cannot add group named '.+'. A group with that name already exists");
  ASSERT_EQ(2, properties.num_groups());

  // Retrieve the group.
  using PropertyGroup = GeometryProperties::Group;
  const PropertyGroup& group = properties.GetGroup(group_name);
  EXPECT_EQ(0u, group.size());

  DRAKE_EXPECT_THROWS_MESSAGE(properties.GetGroup("invalid_name"),
                              std::logic_error,
...
GTEST_TEST(GeometryProperties, AddProperty) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      ".*Trying to add property .* to group .* the group does not exist");
}
...
GTEST_TEST(GeometryProperties, AddPropertyStruct) {
  ...
  TestData data{1, 2., "3"};
  EXPECT_NO_THROW(properties.AddProperty(properties.default_group_name(),
                                         prop_name, data));
...
GTEST_TEST(GeometryProperties, GetPropertyOrDefault) {
  ...
  const std::string& default_ref = properties.GetPropertyOrDefault(
      "strings", "invalid_string", "rvalue_string");
  EXPECT_EQ("rvalue_string", default_ref);
...
GTEST_TEST(GeometryProperties, PropertyCounts) {
  ...
  for (const std::string& group_name : group_names) {
    ...
    for (int i = 0; i < per_group; ++i) {
      EXPECT_NO_THROW(properties.AddProperty(group_name,
                                             fmt::format("prop{}", i), i));
      ++added;
    }
  }

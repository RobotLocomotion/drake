...
GTEST_TEST(GeometryProperties, EigenVector4d) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      std::logic_error,
      ".*exists as a vector / matrix.*"
      "Requested 'Eigen::Vector3d' \\(rows=3, cols=1\\), but found "
      "'Eigen::VectorXd' \\(rows=4, cols=1\\).*");
  EXPECT_THROW(
      properties.GetProperty<Eigen::Vector3d>(group_name, fixed_name),
      std::exception);
...
GTEST_TEST(GeometryProperties, EigenMatrix4d) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      std::logic_error,
      ".*exists as a vector / matrix.*"
      "Requested 'Eigen::Matrix3d' \\(rows=3, cols=3\\), but found "
      "'Eigen::MatrixXd' \\(rows=4, cols=4\\).*");
  DRAKE_EXPECT_THROWS_MESSAGE(
      properties.GetProperty<Eigen::Matrix3Xd>(group_name, fixed_name),
      std::logic_error,
      ".*exists as a vector / matrix.*"
      "Requested 'Eigen::Matrix<double,3,-1,.*>' \\(rows=3, cols=Dynamic\\), "
      "but found 'Eigen::MatrixXd' \\(rows=4, cols=4\\).*");

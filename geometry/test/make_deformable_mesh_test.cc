...
GTEST_TEST(DeformableMeshBuilderTest, Sphere) {
  ...
  const VolumeMesh<double> mesh = MakeDeformableMesh(sphere, kRezHint);
...
GTEST_TEST(DeformableMeshBuilderTest, Cylinder) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(MakeDeformableMesh(c, kRezHint),
                              "Cylinder.*not supported.*");
}

GTEST_TEST(DeformableMeshBuilderTest, Halfspace) {
  const HalfSpace h;
  DRAKE_EXPECT_THROWS_MESSAGE(MakeDeformableMesh(h, kRezHint),
                              "Half space.*not supported.*");
}

GTEST_TEST(DeformableMeshBuilderTest, Box) {
  const Box b(1.5, 2.5, 3.5);
  DRAKE_EXPECT_THROWS_MESSAGE(MakeDeformableMesh(b, kRezHint),
                              "Box.*not supported.*");
}
...
GTEST_TEST(DeformableMeshBuilderTest, Mesh) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(MakeDeformableMesh(m, kRezHint),
                              "Mesh.*not supported.*");
}

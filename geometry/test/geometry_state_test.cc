...
TEST_F(GeometryStateTest, RegisterDeformableGeometry) {
  ...
  // Verify the mesh of the internal geometry matches the input.
  const VolumeMesh<double>* internal_mesh = geometry_state_.GetMesh(g_id);
  ASSERT_NE(internal_mesh, nullptr);
  EXPECT_TRUE(internal_mesh->Equal(mesh));
  ...
  const VectorX<double>& q_WG = geometry_state_.get_positions_in_world(g_id);

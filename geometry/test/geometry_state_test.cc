...
TEST_F(GeometryStateTest, PostHocRenderEngineRespectAcceptingRenderer) {
  ...
  // will be the new render named `second_name`. Otherwise, it will be accepted
  // by a fake name associated with no renderer.
  auto make_properties = [&second_name](bool second_accepts) {
    PerceptionProperties properties =
        DummyRenderEngine().accepting_properties();
  ...
  // We'll assign the perception role to two geometries -- one of them accept
  // the not-yet-existant render engine and one doesn't.
  geometry_state_.AssignRole(source_id_, geometries_[0], make_properties(true));
  geometry_state_.AssignRole(source_id_, geometries_[1],
                             make_properties(false));
  ...
  EXPECT_EQ(second_renderer.num_registered(), 1);
}

...
TEST_F(GeometryStateTest, QueryFrameProperties) {
  ...
  // 1234567890 is the magic number that GeomteryState assigns to the world
  // frame's group id and clique (see GeometryState<T>::GeometryState().
  EXPECT_EQ(geometry_state_.get_frame_group(world), 1234567890);
  DRAKE_EXPECT_THROWS_MESSAGE(

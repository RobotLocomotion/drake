...
class GeometryStateTester {
 public:
  ...
  void ThrowIfInvalidName(FrameId frame_id, const std::string& name) const {
    state_->ThrowIfInvalidName(frame_id, name);
  }
...
class GeometryStateTest : public ::testing::Test {
 protected:
  ...
  std::string anchored_name_{"anchored"};
...
TEST_F(GeometryStateTest, GeometryNameValidation) {
  ...
  // Case: Invalid frame should throw (assuming otherwise valid name).
  EXPECT_THROW(
      geometry_state_.IsValidGeometryName(FrameId::get_new_id(), "name"),
      std::logic_error);

  auto expect_bad_name = [this](const std::string& name,
                                const std::string& message) {
    EXPECT_FALSE(geometry_state_.IsValidGeometryName(frames_[0], name));
    DRAKE_EXPECT_THROWS_MESSAGE(gs_tester_.ThrowIfInvalidName(frames_[0], name),
                                std::logic_error,
                                message);
  };
  ...
  const std::string padded_message{
      "The proposed geometry name has trailing/leading whitespace: '.+'"};
  expect_bad_name(" name", padded_message);
  expect_bad_name("name ", padded_message);
  expect_bad_name(" name ", padded_message);
  expect_bad_name("\tname", padded_message);
  expect_bad_name("name\t", padded_message);
  ...
  EXPECT_NO_THROW(gs_tester_.ThrowIfInvalidName(frames_[0], "unique"));
}

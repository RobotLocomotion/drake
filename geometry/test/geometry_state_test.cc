...
//  3. Property has non-emty set --> only renderers with matching names accept
//     the geometries.
TEST_F(GeometryStateTest, RespectAcceptingRendererProperty) {
  ...
  // We should have a renderer with the name: kDummyRenderName Add a new
  // renderer.
  auto new_renderer = make_unique<DummyRenderEngine>();
  ...
  // Instantiate base properties with the mininum acceptable set of properties.

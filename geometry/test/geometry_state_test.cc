...
class GeometryStateTestBase {
 public:
  ...
  const string dummy_render_name_{"dummy_renderer"};
...
TEST_F(GeometryStateTest, ValidateSingleSourceTree) {
  ...
  // The internal geometries are what and where they should be.
  {
    ...
    for (int i = 0; i < single_tree_dynamic_geometry_count(); ++i) {
      ...
      EXPECT_FALSE(geometry.render_index(dummy_render_name_));
...
TEST_F(GeometryStateTest, AddRendererError) {
  EXPECT_NO_THROW(
      geometry_state_.AddRenderer("unique", make_unique<DummyRenderEngine>()));

  // Non-unique name.
  DRAKE_EXPECT_THROWS_MESSAGE(
      geometry_state_.AddRenderer("unique", make_unique<DummyRenderEngine>()),
      std::logic_error, "A renderer with the name '.+?' already exists");

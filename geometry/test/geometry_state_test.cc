...
TEST_F(GeometryStateTest, ValidateFrameIds) {
  SourceId s_id = SetUpSingleSourceTree();
  FramePoseVector<double> frame_set(s_id, kFrameCount);
  for (auto frame_id : frames_) {
  ...
  // Case: Set has *extra* frame -- already tested in
  // frame_kinematics_vector_test.cc.

  // Case: Right number, wrong frames.
  FramePoseVector<double> frame_set_2(s_id, kFrameCount);
  for (int i = 0; i < kFrameCount; ++i) {
    frame_set_2.set_value(FrameId::get_new_id(), Isometry3<double>::Identity());
  }
  DRAKE_EXPECT_THROWS_MESSAGE(
  ...
  // Case: Too few frames.
  FramePoseVector<double> frame_set_3(s_id, kFrameCount - 1);
  for (int i = 0; i < kFrameCount - 1; ++i) {
    frame_set_3.set_value(frames_[i], Isometry3<double>::Identity());
  }
  DRAKE_EXPECT_THROWS_MESSAGE(
...
TEST_F(GeometryStateTest, SetFramePoses) {
  ...
  auto make_pose_vector =
    ...
    FramePoseVector<double> poses(s_id, count);
...
TEST_F(GeometryStateTest, QueryFrameProperties) {
  ...
  FramePoseVector<double> poses(s_id, kFrameCount);

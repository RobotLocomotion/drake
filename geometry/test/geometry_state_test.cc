...
// Confirms that a GeometrySet can be converted into a set of geometries.
TEST_F(GeometryStateTest, GetGeometryIds) {
  ...
  // Collect up all geometries for convenience.
...

  {
    ...
    EXPECT_EQ(geometry_state_.GetGeometryIds(GeometrySet(all_frames),
                                             Role::kIllustration),
              unordered_set<GeometryId>({geometries_[4], geometries_[5]}));
  }

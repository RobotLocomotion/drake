...
TEST_F(GeometryStateTest, ChangeShapeInternals) {
  ...
  const InternalGeometry ref_geo(*geometry);
  ...
  // Nothing else has.
  EXPECT_EQ(ref_geo.id(), geometry->id());
  EXPECT_EQ(ref_geo.name(), geometry->name());
  EXPECT_EQ(ref_geo.source_id(), geometry->source_id());
  EXPECT_EQ(ref_geo.frame_id(), geometry->frame_id());
  EXPECT_EQ(ref_geo.is_deformable(), geometry->is_deformable());
  EXPECT_EQ(ref_geo.is_dynamic(), geometry->is_dynamic());
}
...
TEST_F(GeometryStateTest, ChangeShapeIllustration) {
  ...
  EXPECT_EQ(
      ref_geo.illustration_properties()->GetProperty<int>("test", "value"),
...
// registered and removed. We'll confirm that both happen in reponse to a call
// to ChangeShape().
...
TEST_F(GeometryStateTest, ChangeShapePerception) {
  ...
  EXPECT_EQ(
      ref_geo.perception_properties()->GetProperty<int>("test", "value"),
...
TEST_F(GeometryStateTest, ChangeShapeProximity) {
  ...
  // as *evidence* that the properties haven't changed.
  EXPECT_EQ(
      ref_geo.proximity_properties()->GetProperty<int>("test", "value"),
...
/* TODO Test that engines have picked up the changed shapes. I probably need
 to test each role independently as well as combinations of roles; i.e.,
 accounting for *one* role doesn't cause the other roles to get skipped. I'll
 make the argument that I know it *doesn't* and if that changes, the
 combinations should be explicitly tested. */

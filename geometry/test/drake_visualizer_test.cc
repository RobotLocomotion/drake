...
/* Confirms that BroadcastLoad works on the scene graph model. We need to test
 the following:
 ...
 all. */
TEST_F(DrakeVisualizerTest, BroadcastLoadFromModel) {
  ConfigureDiagram();
  PopulateScene();

  {
    ...
    // "illustration".
    DrakeVisualizer::BroadcastLoad(*scene_graph_, &lcm_);
    MessageResults results = ProcessMessages();
    ASSERT_EQ(results.num_load, 1);
    ASSERT_EQ(results.load_message.num_links, 1);
  ...
  }
  {
    ...
    DrakeVisualizer::BroadcastLoad(*scene_graph_, &lcm_, params);
    MessageResults results = ProcessMessages();
    ASSERT_EQ(results.num_load, 1);
    ASSERT_EQ(results.load_message.num_links, 1);
    ASSERT_EQ(results.load_message.link[0].name,
...
/* Confirms that BroadcastLoad works on the context geometry. We need to test
 the following:

   1. The contents of the Context (and not SceneGraph model) are broadcast.
   2. The provided parameters are used.
   3. The given lcm is used.
 ...
 We'll make the geometry data in the context *different* from the SceneGraph
 model data and do a single broadcast with non-default parameters.  */
TEST_F(DrakeVisualizerTest, BroadcastLoadFromContext) {
  ConfigureDiagram();
  PopulateScene();

  auto context = diagram_->CreateDefaultContext();
  auto& sg_context = scene_graph_->GetMyMutableContextFromRoot(context.get());

  const GeometryId g_id = scene_graph_->RegisterGeometry(
      &sg_context, source_id_, proximity_frame_id_,
      make_unique<GeometryInstance>(RigidTransformd{}, make_unique<Sphere>(1),
                                    "context_only_proximity"));
  scene_graph_->AssignRole(&sg_context, source_id_, g_id,
                           ProximityProperties());

  DrakeVisualizerParams params;
  params.role = Role::kProximity;
  DrakeVisualizer::BroadcastLoad(sg_context, *scene_graph_, &lcm_, params);
  MessageResults results = ProcessMessages();
  ASSERT_EQ(results.num_load, 1);
  ASSERT_EQ(results.load_message.num_links, 1);
  ASSERT_EQ(results.load_message.link[0].name,
            string(kSourceName) + "::proximity");
  // Show that the new geometry unique two the context was picked up.
  ASSERT_EQ(results.load_message.link[0].num_geom, 2);
  ASSERT_EQ(results.num_draw, 0);
}

/* Confirms that the BroadcastDraw works. As with the previous BroadcastLoad
 tests, we'll simply broadcast a couple of messages to confirm that it is
 sensitive to the passed parameters.  */
TEST_F(DrakeVisualizerTest, BroadcastDraw) {
  ConfigureDiagram();
  PopulateScene();
  auto context = diagram_->CreateDefaultContext();
  const auto& sg_context = scene_graph_->GetMyContextFromRoot(*context);

  {
    // Case: role = illustration (via default) --> one frame labeled with
    // "illustration".
    DrakeVisualizer::BroadcastDraw(sg_context, *scene_graph_, &lcm_);
    MessageResults results = ProcessMessages();
    ASSERT_EQ(results.num_load, 0);
    ASSERT_EQ(results.num_draw, 1);
    ASSERT_EQ(results.draw_message.num_links, 1);
    EXPECT_EQ(results.draw_message.link_name[0],
              string(kSourceName) + "::illustration");
  }
  {
    // Case: role = proximity --> one frame labeled with "proximity".
    DrakeVisualizerParams params;
    params.role = Role::kProximity;
    DrakeVisualizer::BroadcastDraw(sg_context, *scene_graph_, &lcm_, params);
    MessageResults results = ProcessMessages();
    ASSERT_EQ(results.num_load, 0);
    ASSERT_EQ(results.num_draw, 1);
    ASSERT_EQ(results.draw_message.num_links, 1);

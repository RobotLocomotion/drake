...
class DrakeVisualizerTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    draw_subscriber_ =
        make_unique<lcm::Subscriber<lcmt_viewer_draw>>(&lcm_, kDrawChannel);
    load_subscriber_ = make_unique<lcm::Subscriber<lcmt_viewer_load_robot>>(
        &lcm_, kLoadChannel);
  }
  ...
  /* Collection of data for reporting what is waiting int the LCM queue.  */
  ...
  unique_ptr<lcm::Subscriber<lcmt_viewer_draw>> draw_subscriber_;
  unique_ptr<lcm::Subscriber<lcmt_viewer_load_robot>> load_subscriber_;
...
TEST_F(DrakeVisualizerTest, PublishPeriod) {
  for (double period : {1 / 30.0, 1 / 10.0}) {
    ...
    // Advancing to a time < period should send no messages.
    simulator.AdvanceTo(period * 0.999999);
    EXPECT_TRUE(ExpectedMessageCount(0, 0));

    // Advancing beyond the period boundary should send a draw message.
    simulator.AdvanceTo(period * 1.000001);
    EXPECT_TRUE(ExpectedMessageCount(0, 1));

    // Advancing past four more period boundaries --> four more draw messages.
    simulator.AdvanceTo(period * 5.1);
    EXPECT_TRUE(ExpectedMessageCount(0, 4));
  }
}

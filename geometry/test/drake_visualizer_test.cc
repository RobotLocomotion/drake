...
/* Positive confirmation that files with supported extensions are included. */
TYPED_TEST(DrakeVisualizerTest, SupportedMeshes) {
  using T = TypeParam;
  this->ConfigureDiagram(
  ...
  const GeometryId g0_id = this->scene_graph_->RegisterGeometry(
      this->pose_source_id_, f_id,
      make_unique<GeometryInstance>(RigidTransformd{},
                                    make_unique<Mesh>("valid/mesh.gltf", 1),
                                    "a_gltf"));
  this->scene_graph_->AssignRole(this->pose_source_id_, g0_id,
                                 IllustrationProperties());

  const GeometryId g1_id = this->scene_graph_->RegisterAnchoredGeometry(
      this->pose_source_id_,
      make_unique<GeometryInstance>(RigidTransformd{Vector3d{5, 0, 0}},
                                    make_unique<Convex>("valid/mesh.obj", 1),
                                    "an_stl"));
  this->scene_graph_->AssignRole(this->pose_source_id_, g1_id,
                                 IllustrationProperties());
  ...
  // Confirm load message; two links (world and frame), and *two* geometries;
  // convex anchored to world, and dynamic anchored to frame.
  ASSERT_EQ(results.num_load, 1);
  ASSERT_EQ(results.load_message.num_links, 2);
  // We exploit the fact that we always know the world is first.
  ASSERT_EQ(results.load_message.link[0].name, "world");
...
/* DrakeVisualizer should simply ignore Mesh and Convex with unsupported
 extensions. (It also dispatches a warning to the log, but that's harder to
 test). So, we'll simply confirm that a scene with a sphere, Mesh, and Convex
 only includes the sphere in the message when the Mesh and Convex have "bad"
 extensions. We'll make the Mesh anchored and the Convex dynamic to make sure
...
TYPED_TEST(DrakeVisualizerTest, IgnoreUnsupportedMeshes) {
  ...
  const GeometryId g0_id = this->scene_graph_->RegisterGeometry(
      this->pose_source_id_, f_id,
      make_unique<GeometryInstance>(RigidTransformd{},
                                    make_unique<Mesh>("not/an/obj.dae", 1),
                                    "a_gltf"));
  this->scene_graph_->AssignRole(this->pose_source_id_, g0_id,
                                 IllustrationProperties());
  const GeometryId g1_id = this->scene_graph_->RegisterGeometry(

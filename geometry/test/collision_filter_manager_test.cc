...
GTEST_TEST(CollisionFilterManagerTest, ExcludeBetween) {
  ...
  filter_manager.ApplyFilterDeclaration(
  ...
  filter_manager.ApplyFilterDeclaration(
      CollisionFilterDeclaration().ExcludeBetween(GeometrySet({g_id1, g_id2}),
                                                  GeometrySet({g_id3})));
  EXPECT_TRUE(inspector.CollisionFiltered(g_id1, g_id2));
...
// Reality check that we can still modify the model after allocating the
// context.
GTEST_TEST(CollisionFilterManagerTest, PostAllocationCollisionFiltering) {
  SceneGraph<double> scene_graph;
  SourceId source_id = scene_graph.RegisterSource("filter_after_allocation");
  FrameId frame_id =
      scene_graph.RegisterFrame(source_id, GeometryFrame("dummy"));
  auto context = scene_graph.CreateDefaultContext();

  GeometrySet geometry_set{frame_id};
  DRAKE_EXPECT_NO_THROW(
      scene_graph.collision_filter_manager().ApplyFilterDeclaration(
          CollisionFilterDeclaration().ExcludeWithin(geometry_set)));

  DRAKE_EXPECT_NO_THROW(
      scene_graph.collision_filter_manager().ApplyFilterDeclaration(
          CollisionFilterDeclaration().ExcludeBetween(geometry_set,
                                                      geometry_set)));}

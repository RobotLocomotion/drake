...
GTEST_TEST(MeshcatTest, Buttons) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      "Meshcat does not have any buttons named button.");
  ...
  meshcat.ResetControls();
  DRAKE_EXPECT_THROWS_MESSAGE(
      meshcat.GetButtonClicks("button1"),
      "Meshcat does not have any buttons named button1.");
  DRAKE_EXPECT_THROWS_MESSAGE(
      meshcat.GetButtonClicks("button2"),
      "Meshcat does not have any buttons named button2.");
}
...
GTEST_TEST(MeshcatTest, Sliders) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      "Meshcat does not have any sliders named slider.");

  meshcat.AddSlider("slider", 0, 1, 0.01, 0.5);
  EXPECT_EQ(meshcat.GetSliderValue("slider"), 0.5);
  meshcat.SetSliderValue("slider", 0.7);
  EXPECT_EQ(meshcat.GetSliderValue("slider"), 0.7);
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      "Meshcat does not have any sliders named slider1.");
  DRAKE_EXPECT_THROWS_MESSAGE(
      meshcat.GetSliderValue("slider2"),
      "Meshcat does not have any sliders named slider2.");
}

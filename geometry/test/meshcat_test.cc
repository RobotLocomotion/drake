...
GTEST_TEST(MeshcatTest, Paths) {
  Meshcat meshcat;
  // Absolute paths.
  meshcat.SetTransform("/foo/transform", RigidTransformd{});
  EXPECT_TRUE(meshcat.HasPath("/foo/transform"));
  meshcat.Delete("/foo/transform");
  EXPECT_FALSE(meshcat.HasPath("/foo/transform"));

  // Absolute paths with strange spellings.
  meshcat.SetTransform("///bar///transform///", RigidTransformd{});
  EXPECT_TRUE(meshcat.HasPath("//bar//transform//"));
  EXPECT_TRUE(meshcat.HasPath("/bar/transform"));
  meshcat.Delete("////bar//transform///");
  EXPECT_FALSE(meshcat.HasPath("/bar/transform"));
  ...
  meshcat.SetTransform("bar///transform///", RigidTransformd{});
  EXPECT_TRUE(meshcat.HasPath("bar//transform//"));
  EXPECT_TRUE(meshcat.HasPath("/drake/bar/transform"));
  meshcat.Delete("bar//transform//");
  EXPECT_FALSE(meshcat.HasPath("bar/transform"));
  EXPECT_FALSE(meshcat.HasPath("/drake/bar/transform"));
}

...
// joints.
class MeshcatContactVisualizerTest : public ::testing::Test {
 protected:
  MeshcatContactVisualizerTest() : meshcat_(std::make_shared<Meshcat>()) {}

  void SetUpDiagram(const MeshcatContactVisualizerParams& cparams =
    ...
    visualizer_ = &MeshcatContactVisualizerd::AddToBuilder(&builder, plant,
                                                           meshcat_, cparams);
  ...
  const MeshcatContactVisualizer<double>* visualizer_{};
  std::unique_ptr<systems::Diagram<double>> diagram_{};
  std::unique_ptr<systems::Context<double>> context_{};
};

TEST_F(MeshcatContactVisualizerTest, Publish) {
  SetUpDiagram();
...
TEST_F(MeshcatContactVisualizerTest, Parameters) {
  ...
  // visualization; they are covered by meshcat_manual_test.
  SetUpDiagram(params);
...
TEST_F(MeshcatContactVisualizerTest, Delete) {
  SetUpDiagram();
...
GTEST_TEST(MeshcatContactVisualizer, PortTest) {
  auto meshcat = std::make_shared<Meshcat>();
  // Also tests the default constructor.
  MeshcatContactVisualizer<double> visualizer(meshcat);
...
// plant).
GTEST_TEST(MeshcatContactVisualizer, AddToBuilderVariant) {
  auto meshcat = std::make_shared<Meshcat>();
  systems::DiagramBuilder<double> builder;
  auto [plant, scene_graph] =
      multibody::AddMultibodyPlantSceneGraph(&builder, 0.001);
  ...
  MeshcatContactVisualizerd::AddToBuilder(
...
TEST_F(MeshcatContactVisualizerTest, ScalarConversion) {
  SetUpDiagram();

...
- From left to right along the x axis, you should see:
  ...
  std::cout << "Animations:\n";
  meshcat->PoseCamera(animation_target + Vector3d{0, -3, 1.5},
                      animation_target);
  ...
  meshcat->PoseCamera(Vector3d{2, 2, 2}, Vector3d::Zero());
  std::cout << "- Now we have an isometric 3/4 view. The camera is no longer "
               "locked.\n";
  std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
...

  {
    ...
    meshcat->PoseCamera(Vector3d{0, -1.5, 1}, Vector3d{0.25, 0, 0});
  ...
  {
    meshcat->PoseCamera(Vector3d{-1.0, -1.0, 1.5}, Vector3d{0, 0, 0.5});
    systems::DiagramBuilder<double> builder;
    auto [plant, scene_graph] =
  ...
  std::cout << "A standalone HTML file capturing this scene (including the "
               "animation, environment map, and camera targeted at the contact "
               "point) has been written to:\n"
            << "  file://" << html_filename

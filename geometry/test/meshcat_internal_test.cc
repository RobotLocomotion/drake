...
GTEST_TEST(TransformGeometryNameTest, SampledResults) {
  ...
  std::vector<std::pair<std::string, std::string>> test_names{
      {"unchanged", "unchanged"},
      // We shouldn't accidentally create an absolute path.
      {"::prefixed", "::prefixed"},
      {"single:colon", "single:colon"},
      {"simple::path", "simple/path"},
      {"extra:::colon", "extra/:colon"},
      // meshcat treats this to empty/slash.
      {"empty::::slash", "empty//slash"},
      // meshcat strips off terminal '/', so we'll lose the final :: but not
      // gain any path benefit; so we'll leave the "::"" intact.
      {"suffixed::", "suffixed::"},

...
GTEST_TEST(CollisionFilterDeclTest, StatementOrder) {
  ...
  // the end.
  {
    ...
    decl.AllowBetween(set_B, set_A);
    SCOPED_TRACE("AllowBetween");
    validate({Tester::StatementOp::kAllowBetween, set_B, set_A}, decl);
  }

  {
    CollisionFilterDeclaration decl(decl_original);
    decl.AllowWithin(set_B);
    SCOPED_TRACE("AllowWithin");
    validate({Tester::StatementOp::kAllowWithin, set_B, GeometrySet()}, decl);
  }

...
GTEST_TEST(ShapeTest, NumericalValidation) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(Box(2, -1.5, 2), ".*failed. depth =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Box(3, 1, -1.5), ".*failed. height =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Box(3, 1, kNan), ".*failed. height =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Box(3, 1, kInf), ".*failed. height =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Box(Vector3d{-2.5, 1, 3}), ".*failed. width =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Box::MakeCube(-1.5), ".*failed. width =.*");

  DRAKE_EXPECT_THROWS_MESSAGE(Capsule(-0.5, 1), ".*failed. radius =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Capsule(0.5, -1.25), ".*failed. length =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Capsule(0.5, kNan), ".*failed. length =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Capsule(0.5, kInf), ".*failed. length =.*");
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(Cylinder(-0.25, 1), ".*failed. radius =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Cylinder(0.5, -1.5), ".*failed. length =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Cylinder(0.5, kNan), ".*failed. length =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Cylinder(0.5, kInf), ".*failed. length =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Cylinder(Vector2d{0.5, -1.5}),
                              ".*failed. length =.*");

  DRAKE_EXPECT_THROWS_MESSAGE(Ellipsoid(0, 1, 1), ".*failed. a =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Ellipsoid(1, 0, 1), ".*failed. b =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Ellipsoid(1, 1, 0), ".*failed. c =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Ellipsoid(1, 1, kNan), ".*failed. c =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Ellipsoid(1, 1, kInf), ".*failed. c =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Ellipsoid(Vector3d{1, 1, 0}), ".*failed. c =.*");
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(MeshcatCone(0, 1, 1), ".*failed. height =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(MeshcatCone(1, 0, 1), ".*failed. a =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(MeshcatCone(1, 1, 0), ".*failed. b =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(MeshcatCone(1, 1, kNan), ".*failed. b =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(MeshcatCone(1, 1, kInf), ".*failed. b =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(MeshcatCone(Vector3d{1, 1, 0}),
                              ".*failed. b =.*");

  DRAKE_EXPECT_THROWS_MESSAGE(Sphere(-0.5), ".*failed. radius =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Sphere(kNan), ".*failed. radius =.*");
  DRAKE_EXPECT_THROWS_MESSAGE(Sphere(kInf), ".*failed. radius =.*");

...
GTEST_TEST(ShapeTest, MeshAndConvexSharedSource) {
  auto validate_shared_source = []<typename MeshType>(MeshType mesh_src) {
    SCOPED_TRACE(mesh_src.type_name());
    const MeshType mesh_copy(mesh_src);
    EXPECT_EQ(&mesh_src.source(), &mesh_copy.source());
    const MeshType mesh_move(std::move(mesh_src));
    EXPECT_EQ(&mesh_copy.source(), &mesh_move.source());
    EXPECT_NE(&mesh_src.source(), &mesh_move.source());

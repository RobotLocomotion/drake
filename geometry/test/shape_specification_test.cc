...
GTEST_TEST(ShapeTest, CalcObb) {
  constexpr double kTol = 1.0e-13;
  // Box
  {
    const Box box(1, 2, 3);
    const auto obb = CalcObb(box);
  ...
  // Capsule
  {
    ...
    const auto obb = CalcObb(capsule);
    ASSERT_TRUE(obb.has_value());
    EXPECT_TRUE(obb->pose().IsExactlyIdentity());
  ...
  // Cylinder
  {
    ...
    const auto obb = CalcObb(cylinder);
    ASSERT_TRUE(obb.has_value());
    EXPECT_TRUE(obb->pose().IsExactlyIdentity());
  ...
  {
    const Ellipsoid ellipsoid(1, 2, 3);
    const auto obb = CalcObb(ellipsoid);
    ASSERT_TRUE(obb.has_value());
    EXPECT_TRUE(obb->pose().IsExactlyIdentity());
    EXPECT_TRUE(

...
GTEST_TEST(MeshcatVisualizerTest, SurfaceDisplacement) {
  ...
  // unchanged.
  systems::BusValue wrong_bus;
  wrong_bus.Set("wrong_signal", Value<double>(1.0));
  diagram->get_input_port(displacements_input)
      .FixValue(context.get(), wrong_bus);
  diagram->ForcedPublish(*context);
  EXPECT_FALSE(
      GetDoubleProperty(*meshcat, moving_geo_path, "crawl_displacement")
          .has_value());
  ...
  // A bus with the configured signal, sets the crawl_displacement for only the

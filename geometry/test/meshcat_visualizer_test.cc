...
GTEST_TEST(MeshcatVisualizerTest, RealtimeRate) {
  ...
  // calling Meshcat::NoteTimeAdvancement() (which has been tested elsewhere).
  ...
  // Now test the realtime rate calculator reset. Note: resetting the calculator
  // on the visualizer does not change what Meshcat reports as the last
  // broadcast realtime rate.
  const double last_rate = meshcat->GetRealtimeRate();
  visualizer.ResetRealtimeRateCalculator();
  ASSERT_EQ(meshcat->GetRealtimeRate(), last_rate);

  // Waiting a full period won't do anything, because the reset means the next
  // publish will *initialize* the simulation time (no publish).
  Sleep(meshcat_params.realtime_rate_period * 1.05);
  context->SetTime(++step * kDt);
  visualizer.ForcedPublish(vis_context);
  EXPECT_EQ(meshcat->GetRealtimeRate(), last_rate);

  // The timer for the publish period also got reset. So, another quick publish
  // will still be within the period; still no publish.
  context->SetTime(++step * kDt);
  visualizer.ForcedPublish(vis_context);
  EXPECT_EQ(meshcat->GetRealtimeRate(), last_rate);

  // However if we sleep long enough, we will get a publish and a new rate.
  Sleep(meshcat_params.realtime_rate_period * 1.05);
  context->SetTime(++step * kDt);
  visualizer.ForcedPublish(vis_context);
  EXPECT_NE(meshcat->GetRealtimeRate(), last_rate);
}

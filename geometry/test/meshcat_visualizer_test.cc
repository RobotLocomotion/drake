...
GTEST_TEST(MeshcatVisualizerTest, VisualizeDeformableVTKGeometry) {
  ...
  drake::geometry::AddContactMaterial(10.0, {}, surface_friction, &deformable_proximity_props);
  torus_instance->set_proximity_properties(deformable_proximity_props);

  const double unused_resolution_hint = 1.0;
  
  DeformableModel<double>& deformable_model = plant.mutable_deformable_model();
  deformable_model.RegisterDeformableBody(std::move(torus_instance), 
                                    deformable_config, unused_resolution_hint);
  ...
  EXPECT_FALSE(meshcat->HasPath(
    "/drake/visualizer/deformable_geometries/DefaultModelInstance/deformable_torus"));
  diagram->ForcedPublish(*context);     

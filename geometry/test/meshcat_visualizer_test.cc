...
GTEST_TEST(MeshcatVisualizerTest, RealtimeRate) {
  ...
  // on the visualizer changes Meshcat's last reported broadcast realtime rate
  // back to zero.
  ...
  context->SetTime(++step * kDt);
  visualizer.ForcedPublish(vis_context);
  EXPECT_EQ(meshcat->GetRealtimeRate(), 0);

  // Now, if we sleep another period, we will get a publish and a new rate.
  Sleep(meshcat_params.realtime_rate_period * 1.05);
  context->SetTime(++step * kDt);
  visualizer.ForcedPublish(vis_context);
  EXPECT_GT(meshcat->GetRealtimeRate(), 0);
}

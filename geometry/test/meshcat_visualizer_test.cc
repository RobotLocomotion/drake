...
GTEST_TEST(MeshcatVisualizerTest, DeformableGeometry) {
  ...
  // Testing set is:
  // IsDeformableAcceptableGeometry() = true
  // IsDeformableAcceptableGeometry() = false (fallback)
  std::vector<std::pair<const Shape*, std::string_view>> expected_results{
      {&mesh, "deformable_geometries/DefaultModelInstance/object"},
      {&sphere, "DefaultModelInstance/object"}};

  for (const auto& [shape, path_suffix] : expected_results) {
    ...
    auto expected_path =
        fmt::format("/drake/{}/{}", params.prefix, path_suffix);

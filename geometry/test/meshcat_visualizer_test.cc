...
GTEST_TEST(MeshcatVisualizerTest, AcceptingProperty) {
  for (bool is_default_accepting : {true, false}) {
    for (const std::string accepting : {"", "prefix", "no_match"}) {
      SCOPED_TRACE(fmt::format("is_default_accepting = {}, accepting_str = {}",
                               is_default_accepting,
      ...
      MeshcatVisualizerParams params;
      params.is_default_accepting = is_default_accepting;
      params.prefix = "prefix";
      MeshcatVisualizer<double>::AddToBuilder(&builder, scene_graph, meshcat,
                                              params);
      auto diagram = builder.Build();
      ...
      const bool should_show = (accepting == "prefix") ||
                               (is_default_accepting && accepting.empty());
      diagram->ForcedPublish(*context);

...
GTEST_TEST(MeshcatVisualizerTest, AcceptingProperty) {
  for (bool include_geometry_with_unspecified_destination : {true, false}) {
    for (const std::string accepting : {"", "prefix", "no_match"}) {
      SCOPED_TRACE(fmt::format("include = {}, accepting = {}",
                               include_geometry_with_unspecified_destination,
      ...
      MeshcatVisualizerParams params;
      params.include_geometry_with_unspecified_destination =
          include_geometry_with_unspecified_destination;
      params.prefix = "prefix";
      MeshcatVisualizer<double>::AddToBuilder(&builder, scene_graph, meshcat,
                                              params);
      ...
      const bool should_show =
          (accepting == "prefix") ||
          (include_geometry_with_unspecified_destination && accepting.empty());
      diagram->ForcedPublish(*context);

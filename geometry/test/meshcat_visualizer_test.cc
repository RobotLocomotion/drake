...
// Utility function that duplicates MeshcatVisualizer's geometry name prefix
// stripper. When that logic goes away, this function can likewise be replaced
// with calls to inspector.GetName(geom_id).
std::string GetStrippedGeometryName(
    GeometryId geom_id, const SceneGraphInspector<double>& inspector) {
  std::string geometry_name = inspector.GetName(geom_id);
  const FrameId frame_id = inspector.GetFrameId(geom_id);
    ...
    for (GeometryId geom_id : inspector.GetGeometries(iiwa_link_7, role)) {
      EXPECT_TRUE(meshcat_->HasPath(
          fmt::format("visualizer/iiwa14/iiwa_link_7/{}",
                      GetStrippedGeometryName(geom_id, inspector))));
    }
    meshcat_->Delete();
  }
  ...
  for (bool show_hydroelastic : {false, true}) {
    ...
    const std::string data = meshcat->GetPackedObject(
        fmt::format("/drake/{}/two_bodies/body1/{}", prefix,
                    GetStrippedGeometryName(sphere1, inspector)));
...
GTEST_TEST(MeshcatVisualizerTest, ConvexHull) {
  ...
  const std::string data = meshcat->GetPackedObject(
      fmt::format("/drake/{}/box/box/{}", params.prefix,
                  GetStrippedGeometryName(box_id, inspector)));
  ...
  for (bool include_unspecified_accepting : {true, false}) {
    for (const std::string accepting : {"", "prefix", "no_match"}) {
      ...
      const std::string geom_path = fmt::format(
          "prefix/box/box/{}", GetStrippedGeometryName(geom_id, inspector));
...
GTEST_TEST(MeshcatVisualizerTest, AlphaSliderCheckResults) {
  ...
  const GeometryId geom_id = *geom_ids.begin();
  const std::string geom_path = fmt::format(
      "visualizer/box/box/{}", GetStrippedGeometryName(geom_id, inspector));

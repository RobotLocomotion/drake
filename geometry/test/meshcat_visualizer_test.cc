...
TEST_F(MeshcatVisualizerWithIiwaTest, Recording) {
  ...
  animation = visualizer_->start_recording();
  diagram_->Publish(*context_);
  context_->SetTime(1.0);
  diagram_->Publish(*context_);
  EXPECT_TRUE(has_iiwa_frame(*animation, 0));
  EXPECT_TRUE(
      has_iiwa_frame(*animation, std::round(1.0 / params.publish_period)));

  // Confirm that PublishRecording runs.  It's correctness is established by
  // meshcat_manual_test.

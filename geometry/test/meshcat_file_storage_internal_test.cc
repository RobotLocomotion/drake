...
GTEST_TEST(MeshcatFileStorageInternalTest, ThreadingStress) {
  ...
  // This thread imitates the outer Meshcat class, by adding and removing
  // content from the database.
  std::future<void> producer =
      std::async(std::launch::async, [&contents, &dut]() {
  ...
  // This thread imitates the inner (i.e., worker) Meshcat class, by accessing

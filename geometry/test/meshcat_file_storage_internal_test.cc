...
GTEST_TEST(MeshcatFileStorageInternalTest, ThreadingStress) {
  ...
  // of the outer Meshcat class there can be at most one producer (the "main
  // thread"), but here we test with >1 producer in anticipation of enhacing
  // Meshcat to allow concurrent use at some point down the road.

...
GTEST_TEST(MeshcatFileStorageInternalTest, BasicWorkflow) {
  ...
  std::string one_ext{".ext"};
  ContentsPtr one = dut.Insert(std::move(one_content), std::move(one_ext),
                               std::move(one_hint));
  ASSERT_TRUE(one);
  EXPECT_EQ(one.use_count(), 1);
  EXPECT_EQ(one->contents(), "content_1");
  EXPECT_EQ(one->extension(), ".ext");
  ...
  ContentsPtr two = dut.Insert("content_2", ".ext", "hint\r\n2");
  ASSERT_TRUE(two);
  EXPECT_EQ(two.use_count(), 1);
  EXPECT_EQ(two->contents(), "content_2");
  EXPECT_EQ(two->extension(), ".ext");
  ...
  // The extension must match to satisfy the prereq.
  ContentsPtr repeat =
      dut.Insert(std::move(repeat_content), ".ext", std::move(repeat_hint));
  ASSERT_TRUE(repeat);
...
GTEST_TEST(MeshcatFileStorageInternalTest, ReinsertExpired) {
  ...
  ContentsPtr one = dut.Insert("content", ".ext", "old_hint");
  one = {};
  EXPECT_EQ(dut.size(), 0);

  // Re-insert the same content again and make sure it's findable. Note: we've
  // changed the extension which only works because the old was *truly* expired.
  one = dut.Insert("content", ".alt", "new_hint");
  ASSERT_TRUE(one);
  EXPECT_EQ(one.use_count(), 1);
  EXPECT_EQ(one->contents(), "content");
  EXPECT_EQ(one->extension(), ".alt");
  ...
  auto producer_loop = [&contents, &dut]() {
    ...
    for (int i = 0; i < 1000; ++i) {
      if (handles.size() > 0 && random() % 8 == 0) {
        ...
        handles.push_back(dut.Insert(std::move(content), ".txt", "filename"));

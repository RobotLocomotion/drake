...
/* Tests construction of DrivenTriangleSurfaceMesh and the vertex
 position/normal computation for the driven vertices. */
GTEST_TEST(DrivenTriangleSurfaceMesh, VertexPositionAndNormal) {
  const VolumeMesh<double> control_mesh = MakeOctahedronVolumeMesh();
  ...
  TriangleSurfaceMesh<double> surface_mesh(std::move(elements),
                                           std::move(vertices));
  DrivenTriangleSurfaceMesh driven_mesh(surface_mesh, control_mesh);
  ...
  VertexSelector interpolator(std::vector<int>{0, 1, 2, 5}, control_mesh);
  DrivenTriangleSurfaceMesh another_driven_mesh(
      std::variant<BarycentricInterpolator, VertexSelector>(
          std::move(interpolator)),
      DeformableTriangleSurfaceMesh<double>(std::move(surface_mesh)));
...
GTEST_TEST(MakeDrivenSurfaceMesh, OctahedronMesh) {
  const VolumeMesh<double> control_mesh = MakeOctahedronVolumeMesh();
  DrivenTriangleSurfaceMesh driven_mesh = MakeDrivenSurfaceMesh(control_mesh);

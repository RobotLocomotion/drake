...
// Friend class to QueryObjectTest -- left in `drake::geometry` to match
// the friend declaration.
class QueryObjectTest : public ::testing::Test {
 protected:
  void SetUp() override {
    context_ = scene_graph_.AllocateContext();
    query_object_ = MakeQueryObject(context_.get(), &scene_graph_);

    EXPECT_TRUE(is_live(*query_object_));
  }
  ...
  template <typename T>
  void set_live(QueryObject<T>* query_object) {
    query_object->set(context_.get(), &scene_graph_);
  }
...
TEST_F(QueryObjectTest, CopySemantics) {
  ...
  QueryObject<double> baked_from_live{*query_object_};
  EXPECT_TRUE(is_baked(baked_from_live));

  QueryObject<double> from_baked{baked_from_live};
  // Simultaneously test from_baked *is* baked and shares state with
  // baked_from_live.
  EXPECT_TRUE(shared_baked(baked_from_live, from_baked));

  // Confirm a baked object can be reset to be a live object.
  set_live(&from_baked);
  EXPECT_TRUE(is_live(from_baked));
}
...
TEST_F(QueryObjectTest, BakedCopyHasFullUpdate) {
  ...
  const GeometryState<double>& baked_state = get_state(query_object);

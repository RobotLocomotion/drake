...
GTEST_TEST(QueryObjectBakeTest, BakedCopyHasFullUpdate) {
  ...
  // So, if we ask the for the world pose of frame_id, it will *not* be
  // at X_WF. However, when we copy the query_object, the same query on its
  // inspector *will* return that value.

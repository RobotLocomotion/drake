...
GTEST_TEST(GeometryVisualization, SimpleScene) {
  ...
  // Build the message
  // NOTE: this function is *not* declared in geometry_visualization.h. In fact,
  // it is *implicitly* declared as by naming it a friend of GeometryState.
  // See the discussion in geometry_visualization.cc and geometry_state.h for
  // more detail.
  lcmt_viewer_load_robot message =
      BuildLoadMessage(geo_context.get_geometry_state());

...
GTEST_TEST(InternalGeometryTest, DeformableGeometry) {
  ...
  // Create in frame G.
  VolumeMesh<double> expected_mesh = MakeSphereVolumeMesh<double>(
      sphere, kRezHint, TessellationStrategy::kDenseInteriorVertices);
  // Transform to frame F.
  expected_mesh.TransformVertices(X_FG);
  ...
  const VolumeMesh<double>* reference_mesh_F = geometry.reference_mesh();
  ASSERT_NE(reference_mesh_F, nullptr);
  EXPECT_TRUE(expected_mesh.Equal(*reference_mesh_F));

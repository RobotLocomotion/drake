...
GTEST_TEST(InternalGeometryTest, SetPose) {
  ...
  const RigidTransformd X_FG(RotationMatrixd::MakeXRotation(M_PI / 3),
                             Vector3d(1, 2, 3));
  const RigidTransformd X_PG(RotationMatrixd::MakeYRotation(M_PI / 7),
                             Vector3d(4, 5, 6));
  const RigidTransformd X_GoldGNew(RotationMatrixd::MakeZRotation(M_PI / 5),
                                   Vector3d(7, 8, 9));
  const RigidTransformd X_FGNew = X_FG * X_GoldGNew;
...

  {
    ...
    geometry.set_geometry_parent(GeometryId::get_new_id(), X_FG);
    geometry.set_pose(X_FGNew);
    EXPECT_TRUE(CompareMatrices(geometry.X_FG().GetAsMatrix34(),
                                X_FGNew.GetAsMatrix34()));
    EXPECT_TRUE(CompareMatrices(geometry.X_PG().GetAsMatrix34(),
                                (X_PG * X_GoldGNew).GetAsMatrix34(), kTol));
  }
}

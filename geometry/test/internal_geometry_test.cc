...
std::pair<Properties, Properties> MakePropertyPair() {
  Properties p1;
  p1.AddProperty("props1", "value", 1);
  Properties p2;
  p2.AddProperty("props2", "value", 2);
...
GTEST_TEST(InternalGeometryTest, PropertyAssignment) {
...

  {
    ...
    EXPECT_TRUE(
        geometry.proximity_properties()->HasProperty("props1", "value"));
    EXPECT_FALSE(
        geometry.proximity_properties()->HasProperty("props2", "value"));
...

  {
    ...
    EXPECT_TRUE(
        geometry.illustration_properties()->HasProperty("props1", "value"));
    EXPECT_FALSE(

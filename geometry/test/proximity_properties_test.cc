...
GTEST_TEST(ProximityPropertiesTest, AddContactMaterial) {
  ...
  // Case: Correct configuration.
  {
    ...
    EXPECT_EQ(p.Get<double>({kMaterialGroup, kElastic}), E);
    EXPECT_EQ(p.Get<double>({kMaterialGroup, kHcDissipation}), d);
    EXPECT_EQ(p.Get<double>({kMaterialGroup, kPointStiffness}), ps);
    const CoulombFrictiond& mu_stored =
        p.Get<CoulombFrictiond>({kMaterialGroup, kFriction});
  ...
  {
    ProximityProperties p;
    p.Add({kMaterialGroup, kElastic}, E);
  ...
  {
    ProximityProperties p;
    p.Add({kMaterialGroup, kHcDissipation}, d);
    DRAKE_EXPECT_THROWS_MESSAGE(
        AddContactMaterial(E, d, ps, mu, &p), std::logic_error,
        ".+ Trying to add property .+ name already exists");
  }
  ...
  {
    ProximityProperties p;
    p.Add({kMaterialGroup, kPointStiffness}, ps);
    DRAKE_EXPECT_THROWS_MESSAGE(
        AddContactMaterial(E, d, ps, mu, &p), std::logic_error,
        ".+ Trying to add property .+ name already exists");
  }
  ...
  {
    ProximityProperties p;
    p.Add({kMaterialGroup, kFriction}, mu);
    DRAKE_EXPECT_THROWS_MESSAGE(
        AddContactMaterial(E, d, ps, mu, &p), std::logic_error,
        ".+ Trying to add property .+ name already exists");
  }
...
GTEST_TEST(ProximityPropertiesTest, AddRigidProperties) {
  for (double length : {1e-5, 1.25, 1e7}) {
    ...
    EXPECT_EQ(props.Get<HydroelasticType>({kHydroGroup, kComplianceType}),
              HydroelasticType::kRigid);
    EXPECT_TRUE(props.HasProperty({kHydroGroup, kRezHint}));
    EXPECT_EQ(props.Get<double>({kHydroGroup, kRezHint}), length);
  }

...
class SignedDistancePairTestData {
 public:
  SignedDistancePairTestData(shared_ptr<Shape> a, shared_ptr<Shape> b,
...
//
// @param X_WA specifies the pose of A.
// @param R_WB specifies the rotation of B.
...
// Ca ∈ ∂A at (0.25,1,2) in A's frame.  From (2), we set B to have raidius
...
//
// @param X_WA the pose of A
// @param R_WB the rotation matrix of B
...
//
// @param R_WA specifies the rotation of the sphere A.
// @param X_WB specifies the pose of the box B.
...
std::vector<SignedDistancePairTestData> GenDistPairTestSphereBox(
  ...
  auto sphere_A = make_shared<Sphere>(2.);
  auto box_B = make_shared<Box>(16., 12., 8.);
...
// as  p_BAo(s) = (sx * h(x), sy * h(y), sz * h(z)).
//
...
//
// @param R_WA specifies rotation of the sphere
// @param X_WB specifies the pose of the box
// @return  the test data for testing sphere-box pairwise signed distances.
//
...
GenDistPairTestSphereBoxConcentric(
    ...
    const RotationMatrixd& R_WB = RotationMatrixd::Identity()) {
  auto sphere_A = make_shared<Sphere>(2.);
  auto box_B = make_shared<Box>(4., 8., 16.);
  const double radius_A = sphere_A->get_radius();
  const Vector3d half_B = box_B->size() / 2.;
  const RigidTransformd X_WB(R_WB, X_WA.translation());
  const RigidTransformd X_AW = X_WA.inverse();

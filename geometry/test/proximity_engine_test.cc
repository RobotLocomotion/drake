...
class ProximityEngineTester {
 public:
  ...
  get_deformable_contact_geometries(const ProximityEngine<double>& engine) {
...
class ProximityEngineDeformableContactTest : public testing::Test {
 protected:
  ...
  // deformable contact.
  const deformable::Geometries& geometries() const {
  ...
  // Tests registering and removing a rigid (non-deformable) geometry to the
  ...
  void TestRigidRegistrationAndRemoval(const Shape& shape,
    ...
    RigidTransformd X_WG(rpy, translation);
    // Register as dynamic geometry.
    {
      const GeometryId id = GeometryId::get_new_id();
      engine_.AddDynamicGeometry(shape, X_WG, id, props);
      const bool registered = geometries().is_rigid(id);
      ...
      if (registered) {
        ...
        EXPECT_FALSE(geometries().is_rigid(id));
      }
    }
...
// expected behaviors.
TEST_F(ProximityEngineDeformableContactTest, ReplaceProperties) {
  ...
  // declarations (i.e. no resolution hint).
  {
    ...
    EXPECT_FALSE(geometries().is_rigid(sphere.id()));
    DRAKE_EXPECT_NO_THROW(
        engine_.UpdateRepresentationForNewProperties(sphere, {}));
    EXPECT_FALSE(geometries().is_rigid(sphere.id()));
  }
  ...
  // should add a new deformable contact rigid representation.
  {
    ...
    EXPECT_TRUE(geometries().is_rigid(sphere.id()));
  }
...
TEST_F(ProximityEngineDeformableContactTest, AddAndRemoveDeformableGeometry) {
  ...
  EXPECT_TRUE(geometries().is_deformable(id));
  engine_.RemoveDeformableGeometry(id);
  EXPECT_FALSE(geometries().is_deformable(id));
  // Trying to remove a non-existent geometry is a no-op.
  EXPECT_NO_THROW(engine_.RemoveDeformableGeometry(id));
}
...
TEST_F(ProximityEngineDeformableContactTest, ComputeDeformableRigidContact) {
  ...
  // Shift the rigid geometry by the same vector and they are again in
  // contact.

...
TEST_F(ProximityEngineHydroWithFallback, UnsupportedExpression) {
  engine_.UpdateWorldPoses(poses_);
  std::unique_ptr<ProximityEngine<Expression>> sym_engine =
      engine_.ToScalarType<Expression>();
  const unordered_map<GeometryId, RigidTransform<Expression>> X_WGs{};
  vector<ContactSurface<Expression>> surfaces;
  vector<PenetrationAsPointPair<Expression>> points;
  DRAKE_EXPECT_THROWS_MESSAGE(
      sym_engine->ComputeContactSurfacesWithFallback(
          HydroelasticContactRepresentation::kTriangle, X_WGs, &surfaces,
          &points),
      "Hydroelastic contact does not support symbolic::Expression.");
}

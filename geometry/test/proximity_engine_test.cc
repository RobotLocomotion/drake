...
TEST_F(ProximityEngineQueryTest, ComputeSignedDistanceGeometryToPoint) {
  ...
  const double Qx = 1.0;
  const double D1x = 2.0;
  const double A2x = 3.0;
  const double D3x = 4.0;

  // This must be a non-zero point to confirm the query sphere is configured
  // properly.
  const Vector3d p_WQ(Qx, 0.0, 0.0);
  const GeometryId dynamic1 = AddDynamic(sphere, {D1x, 0.0, 0.0});
  const GeometryId anchored2 = AddAnchored(sphere, {A2x, 0.0, 0.0});
  const GeometryId dynamic3 = AddDynamic(sphere, {D3x, 0.0, 0.0});
  ...
  // (4)-(6) - correct distances to dynamic and anchored.
  {
    ...
    EXPECT_EQ(results[0].distance, (D1x - Qx) - sphere.radius());
    EXPECT_EQ(results[1].distance, (A2x - Qx) - sphere.radius());
  }

...
// The interface uses InternalGeometry to add dynamic and anchored geometries.
// This utility is a proxy for GeometryState (which normally constructs and
// manages the internal geometries). This does just enough management to
// satisfy ProximityEngine's needs.
InternalGeometry MakeGeometry(
    const Shape& shape, const RigidTransformd& X_FG = {},
    GeometryId id = GeometryId::get_new_id(),
    const ProximityProperties& props = {},
    const PolygonSurfaceMesh<double>* convex_hull = nullptr) {
  static const never_destroyed<ProximityEngine<double>> engine;
  static const SourceId source_id = SourceId::get_new_id();
  static const FrameId frame_id = FrameId::get_new_id();
...
GTEST_TEST(ProximityEngineTests, AddDynamicGeometry) {
  ...
  engine.AddDynamicGeometry(MakeGeometry(sphere, {}, id), {});
...
GTEST_TEST(ProximityEngineTests, ProcessHydroelasticProperties) {
  ...
  // Case: compliant sphere.
  {
    ...
    engine.AddDynamicGeometry(
        MakeGeometry(sphere, {}, sphere_id, soft_properties), {});
  ...
  // Case: rigid cylinder.
  {
    ...
    engine.AddDynamicGeometry(
        MakeGeometry(cylinder, {}, cylinder_id, rigid_properties), {});
  ...
  // Case: rigid ellipsoid.
  {
    ...
    engine.AddDynamicGeometry(
        MakeGeometry(ellipsoid, {}, ellipsoid_id, rigid_properties), {});
  ...
  // Case: rigid capsule.
  {
    ...
    engine.AddDynamicGeometry(
        MakeGeometry(capsule, {}, capsule_id, rigid_properties), {});
  ...
  // Case: rigid half_space.
  {
    ...
    engine.AddDynamicGeometry(
        MakeGeometry(half_space, {}, half_space_id, rigid_properties), {});
  ...
  // Case: rigid box.
  {
    ...
    engine.AddDynamicGeometry(
        MakeGeometry(box, {}, box_id, rigid_properties), {});
  ...
  // Case: rigid mesh.
  {
    ...
    engine.AddDynamicGeometry(
        MakeGeometry(mesh, {}, mesh_id, rigid_properties), {});
  ...
  // Case: compliant mesh.
  {
    ...
    const GeometryId mesh_id = GeometryId::get_new_id();
    engine.AddDynamicGeometry(
        MakeGeometry(mesh, {}, mesh_id, soft_properties), {});
    EXPECT_EQ(ProximityEngineTester::hydroelastic_type(mesh_id, engine),
              HydroelasticType::kSoft);
  }
  ...
  // Case: compliant convex vtk.
  {
    ...
    engine.AddDynamicGeometry(
        MakeGeometry(convex, {}, convex_id, soft_properties), {});
  ...
  // Case: rigid convex.
  {
    ...
    const GeometryId convex_id = GeometryId::get_new_id();
    engine.AddDynamicGeometry(
        MakeGeometry(convex, {}, convex_id, rigid_properties), {});
    EXPECT_EQ(ProximityEngineTester::hydroelastic_type(convex_id, engine),
              HydroelasticType::kRigid);
  }
...
GTEST_TEST(ProximityEngineTests, ProcessVtkMeshUndefHydro) {
  ...
  // Case: mesh vtk, no hydro type annotation.
  {
    ...
    const GeometryId mesh_id = GeometryId::get_new_id();
    engine.AddDynamicGeometry(
        MakeGeometry(mesh, {}, mesh_id, ProximityProperties()), {});
    EXPECT_EQ(ProximityEngineTester::hydroelastic_type(mesh_id, engine),
...
GTEST_TEST(ProximityEngineTests, ProcessVtkConvexUndefHydro) {
  ...
  // Case: convex vtk, no hydro type annotation.
  {
    ...
    const GeometryId convex_id = GeometryId::get_new_id();
    engine.AddDynamicGeometry(
        MakeGeometry(convex, {}, convex_id, ProximityProperties()), {});
    EXPECT_EQ(ProximityEngineTester::hydroelastic_type(convex_id, engine),
              HydroelasticType::kUndefined);
  }
}
...
std::pair<GeometryId, RigidTransformd> AddShape(ProximityEngine<double>* engine,
  ...
  if (is_anchored) {
    engine->AddAnchoredGeometry(
        MakeGeometry(shape, X_WS, id_S, properties), X_WS);
  } else {
    engine->AddDynamicGeometry(
        MakeGeometry(shape, X_WS, id_S, properties), X_WS);
  }
...
GTEST_TEST(ProximityEngineTests, AddAnchoredGeometry) {
  ...
  const GeometryId id = GeometryId::get_new_id();
  engine.AddAnchoredGeometry(MakeGeometry(sphere, X_WG, id), X_WG);
  EXPECT_EQ(engine.num_geometries(), 1);
...
GTEST_TEST(ProximityEngineTests, AddMixedGeometry) {
  ...
  engine.AddAnchoredGeometry(MakeGeometry(sphere, X_WA, id_1), X_WA);
  EXPECT_TRUE(CompareMatrices(
  ...
  engine.AddDynamicGeometry(MakeGeometry(sphere, X_WD, id_2), X_WD);
...
GTEST_TEST(ProximityEngineTests, ReplaceProperties) {
  ...
  // it mindlessly attempts to update the hydroelastic representation.
  {
    ...
    engine.AddDynamicGeometry(
        MakeGeometry(sphere.shape(), {}, sphere.id(), props), {});
...
GTEST_TEST(ProximityEngineTests, RemoveGeometry) {
  for (bool is_dynamic : {true, false}) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      if (is_dynamic) {
        engine.AddDynamicGeometry(
            MakeGeometry(sphere, poses[id], id, props), poses[id]);
      } else {
        engine.AddAnchoredGeometry(
            MakeGeometry(sphere, poses[id], id, props), poses[id]);
      }
...
GTEST_TEST(ProximityEngineTests, FailedParsing) {
  ...
  // The obj contains multiple objects.
  {
    Convex convex{drake::FindResourceOrThrow(
                  ...
                  1.0};
    DRAKE_EXPECT_THROWS_MESSAGE(
        engine.AddDynamicGeometry(MakeGeometry(convex), {}),
...
GTEST_TEST(ProximityEngineTests, CopySemantics) {
  ...
  ref_engine.AddAnchoredGeometry(MakeGeometry(sphere, pose), pose);

  ref_engine.AddDynamicGeometry(MakeGeometry(sphere, pose), pose);

  Cylinder cylinder{0.1, 1.0};
  ref_engine.AddDynamicGeometry(MakeGeometry(cylinder, pose), pose);

  Box box{0.1, 0.2, 0.3};
  ref_engine.AddDynamicGeometry(MakeGeometry(box, pose), pose);

  Capsule capsule{0.1, 1.0};
  ref_engine.AddDynamicGeometry(MakeGeometry(capsule, pose), pose);

  Ellipsoid ellipsoid{0.1, 0.2, 0.3};
  ref_engine.AddDynamicGeometry(MakeGeometry(ellipsoid, pose), pose);

  HalfSpace half_space{};
  ref_engine.AddDynamicGeometry(MakeGeometry(half_space, pose), pose);

  Convex convex{drake::FindResourceOrThrow("drake/geometry/test/quad_cube.obj"),
                1.0};
  ref_engine.AddDynamicGeometry(MakeGeometry(convex, pose), pose);
...
GTEST_TEST(ProximityEngineTests, MoveSemantics) {
  ...
  Sphere sphere{0.5};
  RigidTransformd pose = RigidTransformd::Identity();
  engine.AddAnchoredGeometry(MakeGeometry(sphere, pose), pose);
  engine.AddDynamicGeometry(MakeGeometry(sphere, pose), pose);
...
GTEST_TEST(ProximityEngineTests, SignedDistanceClosestPointsSingleAnchored) {
  ...
  engine.AddAnchoredGeometry(
      MakeGeometry(sphere, X_WGs.at(id), id), X_WGs.at(id));
...
GTEST_TEST(ProximityEngineTests, SignedDistanceClosestPointsMultipleAnchored) {
  ...
  engine.AddAnchoredGeometry(
      MakeGeometry(sphere, X_WGs.at(id_A), id_A), X_WGs.at(id_A));

  const GeometryId id_B = GeometryId::get_new_id();
  X_WGs[id_B] = RigidTransformd{Translation3d{1.8 * radius, 0, 0}};
  engine.AddAnchoredGeometry(
      MakeGeometry(sphere, X_WGs.at(id_B), id_B), X_WGs.at(id_B));
...
GTEST_TEST(ProximityEngineTests, SignedDistanceClosestPointsMaxDistance) {
  ...
  const double radius = 0.5;
  Sphere sphere{radius};
  engine.AddDynamicGeometry(MakeGeometry(sphere, {}, id_A), {});
  engine.AddDynamicGeometry(MakeGeometry(sphere, {}, id_B), {});
...
GTEST_TEST(SignedDistanceToPointBroadphaseTest, MultipleThreshold) {
  ...
  for (const Vector3d& p_WG : {center1, center2}) {
    ...
    const GeometryId id = GeometryId::get_new_id();
    X_WGs[id] = X_WG;
    engine.AddAnchoredGeometry(MakeGeometry(Sphere(radius), X_WG, id), X_WG);
  }
...
struct SignedDistanceToPointTest
  ...
  SignedDistanceToPointTest() {
    ...
    const GeometryId id = data.expected_result.id_G;
    engine.AddAnchoredGeometry(
        MakeGeometry(*data.geometry, data.X_WG, id), data.X_WG);
    X_WGs[id] = data.X_WG;
  }
};
...
GTEST_TEST(ProximityEngineTests, PenetrationSingleAnchored) {
  ...
  RigidTransformd pose = RigidTransformd::Identity();
  const GeometryId id = GeometryId::get_new_id();
  engine.AddAnchoredGeometry(MakeGeometry(sphere, pose, id), pose);
...
GTEST_TEST(ProximityEngineTests, PenetrationAsPointPairResultOrdering) {
  ...
  for (const auto& pair : poses) {
    engine.AddDynamicGeometry(MakeGeometry(sphere, {}, pair.first), {});
  }
  engine.UpdateWorldPoses(poses);
 ...
 protected:
  void SetUp() override {
    ...
    for (const auto& id : ids) {
      ...
      engine_.AddDynamicGeometry(MakeGeometry(sphere, {}, id, props), {});
      ++n;
    }
  }
...
TEST_F(SimplePenetrationTest, PenetrationDynamicAndAnchored) {
  ...
  engine_.AddAnchoredGeometry(MakeGeometry(sphere_, pose, anchored_id), pose);

  // Set up dynamic geometry.
  const GeometryId dynamic_id = GeometryId::get_new_id();
  engine_.AddDynamicGeometry(MakeGeometry(sphere_, {}, dynamic_id), {});
...
TEST_F(SimplePenetrationTest, PenetrationDynamicAndDynamicSingleSource) {
  ...
  engine_.AddDynamicGeometry(MakeGeometry(sphere_, {}, origin_id), {});

  GeometryId collide_id = GeometryId::get_new_id();
  engine_.AddDynamicGeometry(MakeGeometry(sphere_, {}, collide_id), {});
  EXPECT_EQ(engine_.num_geometries(), 2);
...
GTEST_TEST(ProximityEngineTests, PairwiseSignedDistanceNonPositiveThreshold) {
  ...
  Sphere sphere{kRadius};
  engine.AddDynamicGeometry(MakeGeometry(sphere, {}, id1), {});
  engine.AddDynamicGeometry(MakeGeometry(sphere, {}, id2), {});
  engine.AddDynamicGeometry(MakeGeometry(sphere, {}, id3), {});
  engine.UpdateWorldPoses(X_WGs);
...
class SignedDistancePairTest
 ...
 public:
  SignedDistancePairTest() {
    const auto& data = GetParam();
    engine_.AddAnchoredGeometry(
        MakeGeometry(*data.a_, data.X_WA_, data.expected_result_.id_A),
        data.X_WA_);

    engine_.AddDynamicGeometry(
        MakeGeometry(*(data.b_), {}, data.expected_result_.id_B), {});
...
GTEST_TEST(SignedDistancePairError, HalfspaceException) {
  ...
  engine.AddDynamicGeometry(MakeGeometry(box), {});

  HalfSpace halfspace;
  engine.AddAnchoredGeometry(MakeGeometry(halfspace), {});
...
GTEST_TEST(ProximityEngineCollisionTest, SpherePunchThroughBox) {
  ...
  const GeometryId box_id = GeometryId::get_new_id();
  engine.AddDynamicGeometry(MakeGeometry(Box{w, h, d}, {}, box_id), {});
  const GeometryId sphere_id = GeometryId::get_new_id();
  engine.AddDynamicGeometry(MakeGeometry(Sphere{radius}, {}, sphere_id), {});
...
class BoxPenetrationTest : public ::testing::Test {
 ...
 private:
  ...
  void TestCollision(TangentShape shape_type, double tolerance,
    ...
    engine_.AddDynamicGeometry(
        MakeGeometry(shape(shape_type), {}, tangent_id), {});

    const GeometryId box_id = GeometryId::get_new_id();
    engine_.AddDynamicGeometry(MakeGeometry(box_, {}, box_id), {});
...
GTEST_TEST(ProximityEngineTests, Issue10577Regression_Osculation) {
  ...
  engine.AddDynamicGeometry(MakeGeometry(Box(0.49, 0.63, 0.015), {}, id_A), {});
  engine.AddDynamicGeometry(MakeGeometry(Cylinder(0.08, 0.002), {}, id_B), {});
...
GTEST_TEST(ProximityEngineTests, AnchoredBroadPhaseInitialization) {
  ...
  GeometryId id_A = GeometryId::get_new_id();
  engine.AddDynamicGeometry(MakeGeometry(Sphere(0.5), {}, id_D), {});

  RigidTransformd X_WA{Translation3d{-3, 0, 0}};
  engine.AddAnchoredGeometry(MakeGeometry(Sphere(0.5), X_WA, id_A), X_WA);
...
GTEST_TEST(ProximityEngineTests, ComputePointSignedDistanceAutoDiffAnchored) {
  ...
  // Against an anchored sphere.
  {
    ...
    const GeometryId anchored_id = GeometryId::get_new_id();
    engine.AddAnchoredGeometry(
        MakeGeometry(Sphere(0.7), X_WS, anchored_id), X_WS);
...
GTEST_TEST(ProximityEngineTests, ComputePointSignedDistanceAutoDiffDynamic) {
  ...
  const GeometryId id = GeometryId::get_new_id();
  engine.AddDynamicGeometry(MakeGeometry(Sphere(0.7), {}, id), {});
...
GTEST_TEST(ProximityEngineTests, ComputePairwiseSignedDistanceAutoDiff) {
  ...
  // first sphere.
  const GeometryId id1 = GeometryId::get_new_id();
  engine.AddDynamicGeometry(MakeGeometry(Sphere(radius), {}, id1), {});
  const RigidTransform<AutoDiffXd> X_WS1_ad(p_WQ_ad);

  const GeometryId id2 = GeometryId::get_new_id();
  engine.AddDynamicGeometry(MakeGeometry(Sphere(radius), {}, id2), {});
  const auto X_WS2_ad =
...
GTEST_TEST(ProximityEngineTests,
  ...
  const GeometryId id1 = GeometryId::get_new_id();
  const GeometryId id2 = GeometryId::get_new_id();
  engine.AddDynamicGeometry(MakeGeometry(Box(1, 2, 3), {}, id1), {});
  engine.AddDynamicGeometry(MakeGeometry(Box(2, 4, 6), {}, id2), {});
...
class ProximityEngineDeformableContactTest : public testing::Test {
 protected:
  ...
  void TestRigidRegistrationAndRemoval(const Shape& shape,
    ...
    {
      const GeometryId id = GeometryId::get_new_id();
      engine_.AddDynamicGeometry(MakeGeometry(shape, X_WG, id, props), X_WG);
    ...
    {
      const GeometryId id = GeometryId::get_new_id();
      engine_.AddAnchoredGeometry(MakeGeometry(shape, X_WG, id, props), X_WG);
      const bool registered = deformable_contact_geometries().is_rigid(id);
      EXPECT_EQ(registered, expected_status);
      if (registered) {
        const deformable::RigidGeometry& geometry = rigid_geometry(id);
...
TEST_F(ProximityEngineDeformableContactTest, UpdatePose) {
  ...
  engine_.AddDynamicGeometry(MakeGeometry(Sphere(1.0), pose, id, props), pose);
...
TEST_F(ProximityEngineDeformableContactTest, ComputeDeformableContact) {
  ...
  ProximityProperties props = MakeProximityPropsWithRezHint(1.0);
  engine_.AddDynamicGeometry(
      MakeGeometry(Sphere(kSphereRadius), {}, rigid_id, props), {});
...
// Creates a mesh representing the convex hull of a single tet.
PolygonSurfaceMesh<double> MakeTetHull() {
  return PolygonSurfaceMesh<double>{
      {3, 0, 1, 2, 3, 0, 3, 1, 3, 1, 3, 2, 3, 0, 2, 3},
...
GTEST_TEST(ProximityEngineTests, ImplementedAsFclConvex) {
  ...
  // The convex hull that will define the hydro representation.
  const PolygonSurfaceMesh<double> fake_hull = MakeTetHull();

  // Non-existant file; proof that it isn't used because it can't be.
  const std::string obj_path = "unused.obj";

  auto expect_fcl_convex_is_tet = [&engine](GeometryId id) {
    ...
    EXPECT_EQ(fcl_shape->getVertices().size(), 4);
    EXPECT_EQ(fcl_shape->getFaceCount(), 4);
  };

  {
    ...
    const GeometryId id = GeometryId::get_new_id();
    engine.AddAnchoredGeometry(
        MakeGeometry(Mesh(obj_path), {}, id, {}, &fake_hull), {});

    expect_fcl_convex_is_tet(id);
  }
  {
    SCOPED_TRACE("Convex as fcl::Convexd");
    const GeometryId id = GeometryId::get_new_id();
    engine.AddAnchoredGeometry(
        MakeGeometry(Convex(obj_path), {}, id, {}, &fake_hull), {});

    expect_fcl_convex_is_tet(id);
  }
}

// For Mesh and Convex, ProximityEngine is responsible for passing the convex
// hull to the internal hydroelastic infrastructure. We can detect that this is
// the case by passing a convex hull different from the declared mesh and
// confirm the resulting hydro representation reflects the hull.
// Note: this only uses *soft* geometries; a rigid Mesh will always use the mesh
// itself, and not the convex hull. But, until we have general
// tetrahedralization implemented, a soft mesh *must* use the convex hull.
GTEST_TEST(ProximityEngineTests, ConvexHullToHydro) {
  ProximityEngine<double> engine;

  // The convex hull which will define the compliant geometry.
  const PolygonSurfaceMesh<double> fake_hull = MakeTetHull();
  const ProximityProperties soft_properties = []() {
    ProximityProperties props;
    AddCompliantHydroelasticProperties(1, 1e8, &props);
    return props;
  }();

  // The given id has a compliant hydroelastic geometry defined by a tet.
  auto expect_hydro_is_tet = [&engine](GeometryId id) {
    const auto& hydro_geometries = engine.hydroelastic_geometries();
    ASSERT_EQ(hydro_geometries.hydroelastic_type(id),
                 HydroelasticType::kCompliant);
    const auto& soft_geom = hydro_geometries.soft_geometry(id);
    ASSERT_FALSE(soft_geom.is_half_space());
    const auto& soft_mesh = soft_geom.mesh();
    // The convex tet's four vertices, plus one internal vertex.
    ASSERT_EQ(soft_mesh.num_vertices(), 5);
    ASSERT_EQ(soft_mesh.num_elements(), 4);
  };

  // In both cases, we pass a garbage file name; it won't matter as only the
  // convex hull will be used.
  {
    SCOPED_TRACE("Mesh");
    const GeometryId id = GeometryId::get_new_id();
    engine.AddAnchoredGeometry(
        MakeGeometry(Mesh("unused.obj"), {}, id, soft_properties, &fake_hull),
        {});
    expect_hydro_is_tet(id);
  }

  {
    ...
    const GeometryId id = GeometryId::get_new_id();
    engine.AddAnchoredGeometry(
        MakeGeometry(Convex("unused.obj"), {}, id, soft_properties, &fake_hull),
        {});
    expect_hydro_is_tet(id);
  }
}
...
// For Mesh and Convex, ProximityEngine is responsible for passing the convex
// hull to the internal deformable infrastructure. We can detect that this is
// the case by passing a convex hull different from the declared mesh and
// confirm the resulting rigid deformable representation reflects the hull.
//
// deformable::Geometries uses the hydroelastic representation to create rigid
// meshes. In that case, only Convex depends on the convex hull; that's what
// we'll test on.
GTEST_TEST(ProximityEngineTests, ConvexHullToDeformable) {
  ProximityEngine<double> engine;

  // The convex hull which will define the compliant geometry.
  const PolygonSurfaceMesh<double> fake_hull = MakeTetHull();
  const ProximityProperties rigid_properties = []() {
    ProximityProperties props;
    AddRigidHydroelasticProperties(1, &props);
    return props;
  }();

  const GeometryId id = GeometryId::get_new_id();
  // We pass a garbage file name; it won't matter as only the convex hull will
  // be used.
  engine.AddAnchoredGeometry(
      MakeGeometry(Convex("unused.obj"), {}, id, rigid_properties, &fake_hull),
      {});

  const auto& rigid_geometries = engine.deformable_contact_geometries();
  ASSERT_TRUE(rigid_geometries.is_rigid(id));
  // Note: deformable::Geometries doesn't give us the ability to introspect
  // the rigid geometries. However, we'll take the fact that we didn't throw on
  // the call to AddAnchoredGeometry() combined with the fact that the id
  // reports as a rigid geometry for deformable purpose as success
  // (particularly as the Convex's filename() was garbage).
}

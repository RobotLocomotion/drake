...
// the disk/cylindrical surfaces (GenDistTestDataCylinderBoundarySurface).
// Here, a circle is a 1-dimensional closed curve, and a disk is a
...
// cylinder geometry G. Q can be on the cylindrical surface or on the top and
// bottom circular disk surfaces.
std::vector<SignedDistanceToPointTestData>
GenDistTestDataCylinderBoundarySurface(
    const RigidTransformd& X_WG = RigidTransformd::Identity()) {
  const RotationMatrixd& R_WG = X_WG.rotation();
  ...
  for (const auto& z_vector : {Vector3d(0., 0., 1.), Vector3d(0., 0., -1.)}) {
    for (const auto& xy_vector : all_xy_vectors) {
      const GeometryId id = GeometryId::get_new_id();
      // xy_vector and z_vector are unit vectors in x-y plane and z-axis of G.
      const Vector3d p_GQ = radius * xy_vector +
                            half_length * kZFactor * z_vector;
      const Vector3d p_WQ = X_WG * p_GQ;
      // Q is its own nearest point on ∂G.
      const Vector3d p_GN = p_GQ;
      // The gradient vector on the cylindrical surface is parallel to the x-y
      // plane of G.
  ...
  // Q on the top and bottom circular disks.
  ...
  for (const auto& z_vector : {Vector3d(0., 0., 1.), Vector3d(0., 0., -1.)}) {
    for (const auto& xy_vector : all_xy_vectors) {
      const GeometryId id = GeometryId::get_new_id();
      // xy_vector and z_vector are unit vectors in x-y plane and z-axis of G.
      const Vector3d p_GQ = radius * kRFactor * xy_vector +
                            half_length * z_vector;
      const Vector3d p_WQ = X_WG * p_GQ;
      // Q is its own nearest point on ∂G.
      const Vector3d p_GN = p_GQ;
  ...
  // Q at the centers of the top and bottom circular disks.
  for (const auto& z_vector : {Vector3d(0., 0., 1.), Vector3d(0., 0., -1.)}) {
    const GeometryId id = GeometryId::get_new_id();
...
// circles; however, it can have N on the top and bottom circular disk.
...
// the top and bottom squares of the cube to the top and bottom circular
...
//        p_AoCa_B = -r_A * (f,g,0)
// 3. |w|=1.
//    Ao is on the top or bottom disks.
//        p_AoCa_B = -r_A * (0,0,w)
std::vector<SignedDistancePairTestData> GenDistPairTestSphereCylinderBoundary(
  ...
  std::vector<SignedDistancePairTestData> test_data;
  for (const double i : {-3., -2., -1., 0., 1., 2., 3.}) {
    for (const double j : {-3., -2., -1., 0., 1., 2., 3.}) {
      for (const double k : {-3., -2., -1., 0., 1., 2., 3.}) {

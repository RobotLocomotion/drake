...
//     parameters.
//   - Confirm that meshes can participate.
//     - specifically, it should confirm all combinations of mesh participation.
//       dynamic mesh - dynamic shape  - throws for matching compliance type.
//       anchored mesh - dynamic shape - throws for matching compliance type.
//       dynamic mesh - anchored shape - throws for matching compliance type.
//       dynamic mesh - dynamic mesh   - currently throws for all cases.
//       dynamic mesh - anchored mesh  - currently throws for all cases.
//       anchored mesh - anchored mesh - no throw, no collision reported.
TEST_F(ProximityEngineMeshes, ComputeContactSurfaceWithFallback) {
  ...
  const Mesh mesh{
      drake::FindResourceOrThrow("drake/geometry/test/non_convex_mesh.obj"),
      1.0 /* scale */};
  // All anchored-dynamic configurations to test (excluding anchored-anchored
  // as being a special case).
  const std::vector<std::pair<bool, bool>> anchor_configurations{
      {false, false}, {false, true}, {true, false}};

  // Case: all dynamic-anchored permutations with different compliance should
  // lead to a single contact surface.
  {
  ...
  // Case: all dynamic-anchored permutations with same compliance should throw.
  {
    for (const auto [mesh_anchored, shape_anchored] : anchor_configurations) {
      ProximityEngine<double> engine;
      const auto X_WGs = PopulateEngine(&engine, sphere, shape_anchored, !soft,
  ...
  // Case: mesh-mesh should throw. Note: we're not attempting a soft mesh at
  // all because of a quirk of the mapping from mesh -> box in broad phase
  // without being able to recognize that the box isn't really a box.
  {
    for (const auto [first_anchored, second_anchored] : anchor_configurations) {
      ProximityEngine<double> engine;
      const auto X_WGs = PopulateEngine(&engine, mesh, first_anchored, !soft,
                                        mesh, second_anchored, !soft);

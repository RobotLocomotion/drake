...
GTEST_TEST(ProximityEngineTests, RemoveGeometry) {
  for (bool is_dynamic : {true, false}) {
    ...
    for (int i = 0; i < 3; ++i) {
      const GeometryId id = GeometryId::get_new_id();
      ids.push_back(id);
      poses.insert({id, Isometry3d{Translation3d{0 * 2, 0, 0}}});
      if (is_dynamic) {
        engine.AddDynamicGeometry(sphere, id);
      } else {
...
class SimplePenetrationTest : public ::testing::Test {
 protected:
  ...
  // properties was found. Provide the engine ids of the sphere located at
...
GTEST_TEST(ProximityEngineTests, Issue10577Regression_Osculation) {
  ...
  X_WB.matrix() << 1, 0, 0,      0,
                   ...
                   0, 0, 0,      1;
  // clang-format on
  unordered_map<GeometryId, Isometry3<double>> X_WG{{id_A, X_WA}, {id_B, X_WB}};
...
GTEST_TEST(ProximityEngineTests, ComputePointSignedDistanceAutoDiffAnchored) {
  ...
  {
    unordered_map<GeometryId, Isometry3<AutoDiffXd>> X_WGs;
...
GTEST_TEST(ProximityEngineTests, ComputePointSignedDistanceAutoDiffDynamic) {
  ...
  unordered_map<GeometryId, Isometry3<AutoDiffXd>> X_WGs{{id, X_WS_ad}};
  engine.UpdateWorldPoses(X_WGs);
...
GTEST_TEST(ProximityEngineTests, ComputePairwiseSignedDistanceAutoDiff) {
  ...
  unordered_map<GeometryId, Isometry3<AutoDiffXd>> X_WGs{{id1, X_WS1_ad},
                                                         {id2, X_WS2_ad}};
  engine.UpdateWorldPoses(X_WGs);

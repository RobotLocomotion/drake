...
struct SignedDistanceToPointTest
  ...
  // We use this tolerance of 1e-14 meter for checking the resulted
  // double-precision variables and the resulted Vector3d variables. We do
  // not use the machine epsilon ε because applying a pose X_WG to a geometry G
  // can introduce a round off error beyond ε, which is around 2e-16. For the
  // data that we generate currently, the tolerance 1e-14 seems appropriate.

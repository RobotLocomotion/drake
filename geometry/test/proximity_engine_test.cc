...
GenDistTestDataCylinderBoundaryCircle(
  ...
  // We take 12 points equally spread around a circle. They cover all the
  // combinations of positive/negative/zero (x,y) coordinates on the circle.
  // Furthermore, each (x,y) has |x| ≠ |y| to avoid symmetry that might hide
  // problems.
...
// cylinder geometry G. Q can be on the barrel or on the top and bottom caps.
std::vector<SignedDistanceToPointTestData>
GenDistTestDataCylinderBoundarySurface(
    const RigidTransformd& X_WG = RigidTransformd::Identity()) {
  const RotationMatrixd& R_WG = X_WG.rotation();

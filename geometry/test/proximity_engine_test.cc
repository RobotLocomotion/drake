...
TEST_F(ProximityEngineQueryTest, ComputeSignedDistanceGeometryToPoint) {
  ...
  // properly.
  const Vector3d p_WQ(++x, 0.0, 0.0);
  const GeometryId dynamic1 = AddDynamic(sphere, {++x, 0.0, 0.0});
  const GeometryId anchored2 = AddAnchored(sphere, {++x, 0.0, 0.0});
  const GeometryId dynamic3 = AddDynamic(sphere, {++x, 0.0, 0.0});
  ...
  // (4)-(6) - correct distances to dynamic and anchored.
  {
    ...
    EXPECT_EQ(results[0].distance, 1.0 - sphere.radius());
    EXPECT_EQ(results[1].distance, 2.0 - sphere.radius());
  }

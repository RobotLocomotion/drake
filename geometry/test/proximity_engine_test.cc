...
GTEST_TEST(ProximityEngineTests, ReplaceProperties) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      engine.MaybeUpdateForNewProperties(sphere, {}), std::logic_error,
  ...
  // it mindlessly attempts to update the hydroelastic representation.
  {
    ...
    DRAKE_EXPECT_NO_THROW(engine.MaybeUpdateForNewProperties(sphere, {}));
    EXPECT_EQ(PET::hydroelastic_type(sphere.id(), engine), kUndefined);
  }
  ...
  // required.
  {
    ...
    DRAKE_EXPECT_NO_THROW(engine.MaybeUpdateForNewProperties(sphere, props));
  ...
  // this should remove the hydroelastic representation.
  {
    ...
    DRAKE_EXPECT_NO_THROW(
        engine.MaybeUpdateForNewProperties(sphere, ProximityProperties()));
    EXPECT_EQ(PET::hydroelastic_type(sphere.id(), engine), kUndefined);
  }
  ...
  // efforts to assign those properties throw.
  {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
        engine.MaybeUpdateForNewProperties(sphere, bad_props_no_elasticity),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
        engine.MaybeUpdateForNewProperties(sphere, bad_props_no_length),
        std::logic_error, "Cannot create soft Sphere; missing the .+ property");
  }
}

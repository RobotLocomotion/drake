...
GTEST_TEST(SceneGraphConnectionTest, FullPositionUpdateConnected) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      ".*\n.*context-system-mismatch.*");
  // Check updates to exercise cache entries; glass-box knowledge shows that
  // these all are implemented via systems::CacheEntry, which checks passed
  // contexts.
  DRAKE_EXPECT_THROWS_MESSAGE(
      SceneGraphTester::FullPoseUpdate(*scene_graph, *diagram_context),
      ".*\n.*context-system-mismatch.*");
  DRAKE_EXPECT_THROWS_MESSAGE(
      SceneGraphTester::FullConfigurationUpdate(*scene_graph, ms_context),
      ".*\n.*context-system-mismatch.*");
}

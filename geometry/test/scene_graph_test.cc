...
TEST_F(SceneGraphTest, Hydroelasticate) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      ...
      ".*compliant.* can only use .vtk.*");

  // Change a shape in the context: replace "box3" with a volume mesh.  Note:
  // this currently throws because the prior throw just above left the specific
  // geometry in an invalid state. The use of sequential *Unchecked() calls in
  // the implementation needs to have any possible source of error handled
  // ahead of time; otherwise it is not exception safe. This issue is not
  // specific to context-bearing ChangeShape() calls; the same problem can be
  // induced on the scene graph model.
  scene_graph_.ChangeShape(
      context_.get(), s_id, g_id,

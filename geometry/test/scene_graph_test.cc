...
TEST_F(SceneGraphTest, PostAllocationCollisionFiltering) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      scene_graph_.DisallowCollisionsWithin(geometry_set1), std::logic_error,
      "The call to DisallowCollisionsWithin is invalid; a context has already "
      "been allocated.");

  GeometrySet geometry_set2{FrameId::get_new_id()};
  DRAKE_EXPECT_THROWS_MESSAGE(
      scene_graph_.DisallowCollisionsBetween(geometry_set1, geometry_set2),
      std::logic_error,
      "The call to DisallowCollisionsBetween is invalid; a context has already "
      "been allocated.");
}

...
class DeformableContactData {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(DeformableContactData)
  ...
  const PolygonSurfaceMesh<T>& contact_surface(int i) const {
 ...
 private:
  ...
  /* Per contact surface data. */
  std::vector<std::unique_ptr<PolygonSurfaceMesh<T>>> contact_surface_mesh_W_;

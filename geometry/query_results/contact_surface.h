...
/** The %ContactSurface characterizes the intersection of two geometries M
  ...
  this class). We guarantee that the mapping from (A, B) to (M, N) will always
  be the same. (Although we don't guarantee what that mapping will be.)
...
class ContactSurface {
 public:
  ...
  //  defining them in one particular geometry's frame? It definitely comes
  //  at a cost.
  ...
  void SwapMAndN(const math::RigidTransform<T>& X_NM) {
    ...
    //  mirror them around the origin. Generically, we should apply an arbitrary
    //  3x3 matrix (which encompasses a rotation and a reflection). But that
    //  would *not* be a valid rotation matrix (handed-ness would be wrong).

...
/* Verifies that CalcVertexPermutation gives the expected result with a simple
 deformable geometry. The deformable mesh has 6 vertices, and two tetrahedron (5
 vertices) is in contact. */
...
/* Tests that per-contact point data is as expected when Append invoked multiple
 times. */
GTEST_TEST(DeformableContactDataTest, PerContactPointData) {

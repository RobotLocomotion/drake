...
GTEST_TEST(DeformableContactDataTest, TestConstructorPermutedVertex) {
  ...
  // polygon in tetrahedron 1 of the test_mesh_S that we will define later.
  // The other values are not relevant for this test. The unit normal is
  // only needed for the internal calculation of DeformableRigidContactPair,
  // which is used by DeformableContactData.
  ...
  // whole test_mesh_S of the sphere.  Positions of the vertices are shown
...
GTEST_TEST(DeformableContactDataTest, TestConstructorSignedDistance) {
  ...
  // This mesh has only one tetrahedron, which is inside the unit sphere.
      ...
      {DeformableRigidContactPair<double>(
           DeformableContactSurface<double>(
               std::vector<ContactPolygonData<double>>{
                   ...
                   // The unit normal is only needed for the internal
                   // calculation of DeformableRigidContactPair, which is used
                   // by DeformableContactData.
                   {.unit_normal = Vector3<double>::UnitZ(),
                    .b_centroid = barycentric0,
                    .tet_index = kTetrahedronIndex}}),
           // Only kDeformableId is relevant. It has to be the same for every
           // contact pair.

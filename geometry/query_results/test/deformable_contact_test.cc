...
GTEST_TEST(DeformableContactSurface, Getters) {
  ...
  const std::vector<Vector3<double>> contact_points_W = {{3, 1, 4}};
  ...
  DeformableContactSurface<double> dut(
      kIdA, kIdB, contact_mesh_W, contact_points_W, signed_distances,
      vertex_indexes_A, barycentric_centroids_A, vertex_indexes_B,
      barycentric_centroids_B);
  EXPECT_EQ(dut.id_A(), kIdA);
  EXPECT_EQ(dut.id_B(), kIdB);
...
GTEST_TEST(DeformableContact, AddDeformableRigidContactSurface) {
  ...
  dut.AddDeformableRigidContactSurface(
      kIdA, kIdB, {0, 3, 2, 5}, contact_mesh_W, contact_points_W,
      signed_distances, contact_vertex_indexes, barycentric_centroids);

...
struct SignedDistancePair{
  // Note: While deprecating the is_grad_W_unique, we cannot use the standard
  // Drake mechanism for declaring copy and move semantics. The default
  // implementations insist on writing to the deprecated member and the macro
  // rebels against being enclosed in the deprecation-suppressing pragma. Once
  // deprecation is complete restore the standard boilerplate:
  //
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
/* Deprecation note: This constructor is wrapped in a deprecation silencer
 because gcc will implicitly try to write to the deprecated member and throw
 a build error. When the deprecated member is removed, we *keep* this

...
/* For any vertex in a deformable geometry, we say that it is participating in
 ...
 of freedrom) defined based on contact participation.
...
class ContactParticipation {
 public:
  ...
  /* Returns the permutation p such that p(i) gives the permuted vertex
   ...
   Hence, the permutated vector would be {3, 0, 1, 4, 5, 2}, which means the
  ...
  /* Suppose p is the vertex permutation, this permutation q is defined such
   that q(3*i+d) = 3*p(i)+d for all i in [0, num_vertices) and d = 0, 1, 2. */
  multibody::contact_solvers::internal::PartialPermutation CalcDofPermutation()
      const;

  /* Returns the partial permutation that only considers dofs associated with
   vertices that do participating in contact. */
  multibody::contact_solvers::internal::PartialPermutation
...
/* A discrete representation of the intersection of two geometries A and
 B, at least one of which is a deformable. The intersection is represented as a
 polygonal surface mesh. We call the centroids of the polygon elements in this
 ...
 public:
  ...
  /* Constructs a deformable contact surface with the given data.
   ...
   @param[in] contact_vertex_indexes_A
      The indexes of the vertices of mesh A making up the tetrahedra containing
      each contact point with the same index semantics as `signed_distances`.
   ...
   @param[in] contact_vertex_indexes_B
      The indexes of the vertices of mesh B making up the tetrahedra containing
      each contact point if B is deformable. std::nullopt otherwise.
  ...
  /* Returns the barycentric coordinates of each contact point in its containing
   tetrahedron in the deformable geometry A's mesh. The ordering of contact
  ...
  /* If geometry B is deformable, returns the barycentric coordinates of each
   ...
   mesh. The ordering of contact points is the same as that in
   `signed_distances()`.
...
/* Data structure to hold information about contact information about all
 deformable geometries that have proximity roles. It stores a) a vector of
 DeformableContactSurfaces that charaterizes all contact surfaces that involve
 at least one deformable geometry, and b) a map from geometry id to contact
 participation information for the deformable geometry with the given id. */
template <typename T>
class DeformableContact {
 public:
  ...
  @param[in] contact_points_W
     The positions of the contact points in the world frame.
  @param[in] signed_distances
     _Approximate_ signed distances of penetration sampled on `contact_mesh_W`.
     These values are non-positive.
  @param[in] contact_vertex_indexes
     The indexes of the vertices incident to the tetrahedron containing each

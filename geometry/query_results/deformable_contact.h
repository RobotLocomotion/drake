...
/* Data structure to hold contact information about all deformable geometries
 ...
 that charaterizes all contact surfaces that involve at least one deformable
...
class DeformableContact {
 public:
  ...
  @param[in] signed_distances
     _Approximate_ signed distances of penetration sampled on `contact_mesh_W`
     with the same index semantics as `contact_points_W`. These values are
     non-positive.
  @param[in] contact_vertex_indexes
     The indexes of the vertices incident to the tetrahedron containing each
     contact point with the same index semantics as `contact_points_W`.

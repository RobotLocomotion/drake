...
class ContactParticipation {
 public:
  ...
  /* Returns the partial permutation that only considers vertices that do
   participate in contact. Suppose p is the vertex permutation, this permutation
   q is defined such that q(i) = p(i) if vertex i participates in contact and
   q(i) = -1 otherwise. The permuted vector would be {-1, 0, 1, -1, -1, 2} in
   the example above. */
  multibody::contact_solvers::internal::PartialPermutation
  CalcVertexPartialPermutation() const;

  /* Suppose p is the vertex permutation, this permutation q is defined such
   that q(3*i+d) = 3*p(i)+d for all i in [0, num_vertices) where num_vertices is
   the number of vertices in the deformable mesh (which is always no less than
   num_vertices_in_contact), and d = 0, 1, 2. */
  ...
  /* Returns the partial permutation that only considers dofs associated with
   ...
   Suppose p is the dof permutation, this permutation q is defined such that
   q(i) = p(i) if dof i belongs to a vertex that participates in contact and
   q(i) = -1 otherwise. */
  multibody::contact_solvers::internal::PartialPermutation

...
DeformableContactSurface<T>::DeformableContactSurface(
    ...
    std::vector<Vector3<T>> contact_points_W, std::vector<T> signed_distances,
    ...
    : id_A_(id_A),
      ...
      contact_points_W_(std::move(contact_points_W)),
  ...
  DRAKE_DEMAND(num_contact_points ==
               static_cast<int>(contact_points_W_.size()));
  DRAKE_DEMAND(num_contact_points ==
...
void DeformableContact<T>::AddDeformableRigidContactSurface(
  ...
  contact_surfaces_.emplace_back(
      deformable_id, rigid_id, move(contact_mesh_W), move(contact_points_W),
      move(signed_distances), move(contact_vertex_indexes),
      move(barycentric_coordinates), std::nullopt, std::nullopt);
}

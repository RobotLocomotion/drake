        ...
        FLAGS_soft == "box"
            ...
            : FLAGS_soft == "ball"
                  ? scene_graph->RegisterGeometry(
                        source_id_, frame_id_,
                        make_unique<GeometryInstance>(
                            RigidTransformd(), make_unique<Sphere>(kRadius),
                            "soft ball"))
                  : (std::cout
                         << "Unsupported value for --soft==" << FLAGS_rigid
  ...
  GeometryId rigid_geometry_id =
      FLAGS_rigid == "ball"
          ...
          : FLAGS_rigid == "box"
                ? scene_graph.RegisterAnchoredGeometry(
                      source_id, make_unique<GeometryInstance>(
                                     X_WB, make_unique<Box>(0.15, 0.15, 0.02),
                                     "rigid box"))
                : FLAGS_rigid == "bowl"
                      ? scene_graph.RegisterAnchoredGeometry(
                            source_id,
                            make_unique<GeometryInstance>(
                                X_WB, make_unique<Mesh>(bowl_absolute_path),
                                "rigid bowl"))
                      : (std::cout << "Unsupported value for --rigid=="
                                   << FLAGS_rigid << ", default to bowl.\n"
                                   << "Supported values are ball, bowl, or "
                                      "box." << std::endl,
                         scene_graph.RegisterAnchoredGeometry(
                             source_id,
                             make_unique<GeometryInstance>(

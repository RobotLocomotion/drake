...
class GeometryState {
 ...
 public:
  ...
  /** Reports the name of the geometry.
  ...
  /** Reports the id for the geometry affixed to the indicated frame with the
   given name.
   @param frame_id  The id of the parent frame.
   @param name      The name of the geometry to query for.
   @return The id of the requested frame.
   @throws std::logic_error if no such geometry exists or if the frame doesn't
                            exist. */
  ...
  /** Registers a GeometryInstance with the state. The state takes ownership of
   ...
   @param geometry     The geometry to get the id for. The state takes
                       ownership of the geometry. If the geometry has
                       a name that has no non-whitespace characters, a name will
                       be automatically generated that is unique in the scope
                       of the parent `frame_id`.
  ...
  /** Registers a GeometryInstance with the state. Rather than hanging directly
   ...
   @param geometry     The geometry to get the id for. The state takes
                       ownership of the geometry. If the geometry has a name
                       that has no non-whitespace characters, a name will be
                       automatically generated that is unique in the scope
                       of the *ultimate* parent `frame_id`.
   @returns  A newly allocated geometry id.
   @throws std::logic_error 1. the `source_id` does _not_ map to a registered
                            source,
  ...
  /** Registers a GeometryInstance with the state as anchored geometry. This
   ...
   @param source_id    The id of the source on which the geometry is being
                       declared.
   @param geometry     The geometry to get the id for. The state takes
                       ownership of the geometry. If the
                       geometry has a name that has no non-whitespace
                       characters, a name will be automatically generated that
                       is unique in the scope of the world frame.
   @returns  A newly allocated geometry id.
   @throws std::logic_error  1. the `source_id` does _not_ map to a registered
                             source,
  ...
  /** Utility to report if a candidate geometry name is considered valid. This
   tests the requirements described in the documentation of GeometryInstance.
   When adding a geometry to a frame, if there is doubt if a proposed name is
   valid, the name can be tested prior to registering the geometry.
   ...
   @throws if `candidate_name` is otherwise valid, but `frame_id` does not refer
           to a frame.  */
 ...
 private:
  ...
  enum NameValidityResult {
    OK,                   // Name considered valid.
    DUPLICATE,            // Name is duplicate of a previous geometry.
    EMPTY,                // The name is empty.
    ONLY_WHITE_SPACE,     // The name is nothing but whitespace.
    WHITESPACE_PADDING    // The name has trailing/leading whitespace.
  };

  // Throws an exception if the proposed name is invalid for a geometry to be
  // registered to the indicated frame.
  void ThrowIfInvalidName(FrameId frame_id, const std::string& name) const;

  // Provides a detailed report about the validity of the proposed name.
  NameValidityResult TestNameValidity(FrameId frame_id,
                                      const std::string& candidate_name) const;

...
class GeometryState {
  ...
  */
  ...
  /** Implementation of
   @ref SceneGraph::ChangeShape(SourceId, GeometryId, const Shape&)
 ...
 private:
  ...
  // Attempts to add the given `geometry` to all appropriate render engines. The
  ...
  bool AddToRenderersUnchecked(const internal::InternalGeometry& geometry);

  // Attempts to move the geometry with the given id from *all* render engines.
  // The only GeometryState-level data structure modified is the perception
  // version. All other changes to GeometryState data must happen elsewhere.

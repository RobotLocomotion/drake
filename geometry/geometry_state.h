...
class DrivenMeshData {
 public:
  ...
  driven_perception_meshes() const {
    return driven_perception_meshes_;
  }
  ...
  void SetPerceptionMeshes(GeometryId id,
  ...
  void Remove(GeometryId id) {
    driven_perception_meshes_.erase(id);
 ...
 private:
  ...
  std::unordered_map<GeometryId, std::vector<DrivenTriangleMesh>>
      driven_perception_meshes_;
  // The vector of render meshes used for rendering for each deformable geometry
  // keyed by the corresponding geometry's id.
...
class GeometryState {
 ...
 private:
  ...
  // ("render", "mesh") property containing a valid path to a surface mesh. In
  // that case, the render mesh must be completely contained within the control
  // volume mesh. Otherwise an exception is thrown.
  // @throws std::exception if the ("render", "mesh") property contains a path
  // to a surface mesh that's not completely contained within the control volume
  // mesh.
  ...
  internal::DrivenMeshData& mutable_driven_mesh_data() const {
    GeometryState<T>* mutable_state = const_cast<GeometryState<T>*>(this);

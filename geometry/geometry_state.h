...
class GeometryState {
 ...
 public:
  ...
  /** Reports the total number of geometries directly registered to the given
   frame. This count does _not_ include geometries attached to frames that have
   this frame as ancestor.
  ...
  /** Registers a new frame for the given source as a child of an existing
   frame. The id of the new frame is returned. A pose for this frame *must*
   be provided in the FramePoseVector parameter to SetFramePoses().
   @param source_id    The id of the source for which this frame is allocated.

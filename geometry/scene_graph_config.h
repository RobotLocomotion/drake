...
struct DefaultProximityProperties {
  ...
  /** (Advanced) Specifies a thin layer of thickness "margin" (in meters) around
   ...
   @warning Currently this is only implemented for _compliant_ hydroelastic
   models and not for point contact.

   @warning Currently margin is ignored for non-convex meshes. It is only
   applied to primitives (which are all convex) and convex meshes. */
  std::optional<double> margin{1.0e-4};
  /// @}

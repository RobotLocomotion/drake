...
class SceneGraphInspector {
 public:
  ...
  /** Returns geometry ids that have been registered directly to the frame
   indicated by `frame_id`. If a `role` is provided, it will be only those
   geometries with that role assigned; otherwise all geometries will be
   returned.
  ...
  std::vector<GeometryId> GetGeometries(
      FrameId frame_id, std::optional<Role> role = std::nullopt) const {
    DRAKE_DEMAND(state_ != nullptr);

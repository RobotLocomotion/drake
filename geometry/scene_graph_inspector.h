...
class SceneGraphInspector {
 public:
  ...
  /** Returns the *mesh* used to represent this geometry in hydroelastic contact
   calculations, if it exists. Most primitives (e.g., sphere, etc.) are actually
   represented by discrete approximations (i.e., the mesh). If there is no mesh,
   the returned variant will hold neither the SurfaceMesh<double> nor the
   VolumeMesh<double> alternatives. If either alternative is present, the
   ...
   *not* mean there is necessarily a mesh associated with it. Some shape types
   ...
   The result can be tested via, e.g:

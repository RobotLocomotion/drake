...
void BackfillDefaults(ProximityProperties* properties,
  ...
  multibody::CoulombFriction friction{
    std::max(config.hydroelasticate.default_dynamic_friction,
...
void Hydroelasticate(GeometryState<T>* geometry_state,
  ...
  if (type == HydroelasticType::kUndefined || ignore_compliance_type) {
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS((
    static_cast<void(*)(GeometryState<T>*, const SceneGraphConfig&)>(
        &Hydroelasticate<T>),
    static_cast<void(*)(GeometryState<T>*, const SceneGraphConfig&,
                        GeometryId, bool)>(&Hydroelasticate<T>)
))
}  // namespace internal
}  // namespace geometry
}  // namespace drake

...
class CspaceFreeBox : public CspaceFreePolytopeBase {
 public:
  ...
  ~CspaceFreeBox() override = default;
 ...
 private:
  ...
  /**
   Compute the range of s from the box q_box_lower <= q <= q_box_upper. We also
  ...
  /**
   This class contains the polynomials that we want to certify, that the
   numerator of certain rationals (contained in plane_geometries) are
   non-negative on s_box_lower <= s <= s_box_upper.
   */
  ...
  // Generate the polynomials used for certifying the box s_box_lower <= s <=

 ...
 *
 * where qⱼ is a sampled point outside of the union of geometries Pᵢ. 1(qⱼ ∈
 ...
 * being covered by any sphere. Note that we can still scale the size of the
 ...
 * s.t Pᵢ ⊂ Sₖ for at least one k
 *     ∑ⱼ1(qⱼ ∈ (S₁∪ S₂ ∪...∪ Sₘ)) = optimal_cost_in_step_1.
 *
 */
class SpheresOuterApproximator {
 public:
   ...
   * point.
   * @param max_sphere_radius The maximal value of the sphere radius.
   */
  SpheresOuterApproximator(int num_spheres,
                           std::vector<Eigen::MatrixXd> polytopes,
   ...
   * phi has size num_polytopes * num_spheres.
   * phi(i, j) = 1 => the i'th polytope is covered by the j'th sphere.
   */
 ...
 private:
   ...
   * qⱼᵀqⱼ + aᵢᵀqⱼ + bᵢ ≤ M₂(1-ζ(j, i))
   * where qⱼ = outlier_[outlier_idx], i = sphere_idx. M₁, M₂ are positive big
   * constants.
   */

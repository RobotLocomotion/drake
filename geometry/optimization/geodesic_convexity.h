...
@param maybe_bboxes_A is a vector of Hyperrectangles, allowing the user to
manually pass in the AABBs of each set in `convex_sets_A` to avoid
recomputation. This argument is only used if `preprocess_bbox` is `true`, and
the function does not check that the entries are indeed the AABBs corresponding
to the sets in `convex_sets_A`. If an empty vector is passed, then the AABBs are
computed automatically.
@param maybe_bboxes_B serves the same role to `convex_sets_B` as
`maybe_bboxes_A` does to `convex_sets_A`.
...
@throws if `maybe_bboxes_A.size() > 0 && convex_sets_A.size() !=
maybe_bboxes_A.size()`.
@throws if `maybe_bboxes_B.size() > 0 && convex_sets_B.size() !=
maybe_bboxes_B.size()`.
@throws if not all entries of `convex_sets_A`, `convex_sets_B`,
`maybe_bboxes_A`, and `maybe_bboxes_B` have the same ambient dimension.
*/
std::vector<std::tuple<int, int, Eigen::VectorXd>> CalcPairwiseIntersections(
    ...
    const std::vector<geometry::optimization::Hyperrectangle>& maybe_bboxes_A =
        {},
    const std::vector<geometry::optimization::Hyperrectangle>& maybe_bboxes_B =
        {});
...
not check that the entries are indeed the AABBs corresponding to the sets in
`convex_sets`. If an empty vector is passed, then the AABBs are computed
automatically.
...
@throws if `maybe_bboxes.size() > 0 && convex_sets.size() !=
maybe_bboxes.size()`.
@throws if not all entries of `convex_sets` and `maybe_bboxes` have the same
ambient dimension.
*/
std::vector<std::tuple<int, int, Eigen::VectorXd>> CalcPairwiseIntersections(

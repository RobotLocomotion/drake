...
bool IsBoundedParallel(const ConvexSet& s, Parallelism parallelism) {
  ...
  for (int batch_idx = 0; batch_idx < num_batches; ++batch_idx) {
    ...
    for (int i = 0; i < this_batch_size; ++i) {
      ...
      ConstructBoundednessProgram(&(progs[i]), s, dimension_index, false /* maximize */);
      ConstructBoundednessProgram(&(progs[i + this_batch_size]), s, dimension_index, true /* maximize */);
    }

...
class MakeHPolyhedronReifier : public ShapeReifier {
 public:
  ...
  void ImplementGeometry(const Ellipsoid&, void*) override {
    ThrowUnsupportedGeometry("HyperEllipsoid");
  }
...
HPolyhedron HPolyhedron::MakeFromSceneGraph(
  ...
  return HPolyhedron(Ab_G.first * X_GE.rotation().matrix(),
                     Ab_G.second - Ab_G.first * X_GE.translation());
}
...
HyperEllipsoid::ToShapeWithPose() const {
  ...
  // The eigenvalues here are the eigenvalues of A squared.
  DRAKE_DEMAND((solver.eigenvalues().array() > 1e-12).all());

  Eigen::Matrix3d R_WG = solver.eigenvectors().transpose();
...
namespace {
class MakeHyperEllipsoidReifier : public ShapeReifier {
 public:
...
HyperEllipsoid HyperEllipsoid::MakeFromSceneGraph(
    const QueryObject<double>& query_object, GeometryId geometry_id,
    std::optional<FrameId> expressed_in) {
  MakeHyperEllipsoidReifier reifier;
  Eigen::Matrix3d A_G;
  ...
  return HyperEllipsoid(A_G * X_GE.rotation().matrix(),
                        X_GE.inverse().translation());
}

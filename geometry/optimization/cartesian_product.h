...
coordinates are transformed by the linear map, Ax + b \in S, with the default
values set to the identity map.  This concept is required for reasoning about
cylinders in arbitrary poses as cartesian products, and more generally for
describing any affine transform of a CartesianProduct.
...
class CartesianProduct final : public ConvexSet {
 public:
  ...
  /** Constructs the product of convex sets in the rotated coordinates:
  Ax + b ∈ Y₁ × Y₂ × ⋯ × Yₙ. */
  ...
  Although any geometry that can be used as a ConvexSet could also be a (trivial) CartesianProduct, we restrict this constructor to handling Cylinder geometry, which constructs the (non-trivial) Cartesian product of a HyperEllipsoid and an HPolyhedron.
  ...
  /** The number of terms (or sets) used in the product. */
  ...
  /** Returns true if each subvector is in its corresponding set with tolerance @p tol.  Note: Tolerance support for this query varies in the different convex set implementations.
  */

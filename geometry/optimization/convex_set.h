...
class ConvexSet : public ShapeReifier {
 public:
  ...
  S is bounded, and t is a scalar.

  If S is unbounded, then the constraints imply t ≥ 0, x ∈ t S + rec(S), where
  rec(S) is the recession cone of S (the asymptotic directions in which S is not
  bounded).  For t > 0, this is equivalent to x ∈ t S, but for t = 0, we have
  only x ∈ rec(S). */

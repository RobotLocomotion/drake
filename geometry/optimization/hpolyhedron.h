...
class HPolyhedron final : public ConvexSet {
  ...
  */
  ...
  checked and an exception is thrown if it is not satisfied. If `check_bounded` is set to false, then it is the user's responsibility to ensure that `this` is bounded and the result is not necessarily to be trusted if the precondition is not satisfied.

...
class HPolyhedron final : public ConvexSet, private ShapeReifier {
 public:
  ...
  to the `i`th decision variable of `prog`. The Variable object itself can be
  obtained with `prog.decision_variable(i)`. If this method detects that the
  linear program in `prog` is trivially infeasible, the following trivially
  infeasible HPolyhedron will be returned: {x₁ <= -1, x₁ >= 0}.

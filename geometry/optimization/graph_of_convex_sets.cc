...
Eigen::VectorXd Edge::GetSolutionXu(
    const solvers::MathematicalProgramResult& result) const {
  return result.GetSolution(y_);
}

Eigen::VectorXd Edge::GetSolutionXv(
    const solvers::MathematicalProgramResult& result) const {
...
std::string GraphOfConvexSets::GetGraphvizString(
  ...
  graphviz << "labelloc=t;\n";
  for (const auto& vpair : vertices_) {
    graphviz << "v" << vpair.first << " [label=\"" << vpair.second->name();
    if (result) {
      graphviz << "\n x = "
               << result->GetSolution(vpair.second->x()).transpose();
    }
    graphviz << "\"]\n";
  }
  for (const auto& epair : edges_) {
    const Edge& e = *epair.second.get();
    ...
    if (result) {
      ...
      if (show_slacks) {
        ...
        graphviz << "xu = " << e.GetSolutionXu(*result).transpose() << ",\n";
        graphviz << "xv = " << e.GetSolutionXv(*result).transpose();
      }
    }
    graphviz << "\"];\n";
  }

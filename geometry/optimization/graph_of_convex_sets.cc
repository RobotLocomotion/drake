...
GraphOfConvexSets::GetRandomizedSolutionPath(
    const Vertex& source, const Vertex& target,
  ...
  if (*options.convex_relaxation && *options.max_rounded_paths > 0 &&
    ...
    std::uniform_real_distribution<double> uniform;
    std::vector<std::vector<const Edge*>> paths;
    std::vector<solvers::MathematicalProgramResult> results;
    ...
    int num_trials = 0;
    MathematicalProgramResult best_rounded_result;
    std::vector<const Edge*> best_path;
    ...
    while (static_cast<int>(paths.size()) < *options.max_rounded_paths &&
      ...
      results.push_back(rounded_result);
    ...
    // TODO(bernhardpg): Fix
    throw std::runtime_error{"Cannot find path with these options!"};
  }
}

...
MathematicalProgramResult GraphOfConvexSets::SolveShortestPath(
  ...
  // "Motion Planning around Obstacles with Convex Optimization":
  // https://arxiv.org/abs/2205.04422
  if (options.convex_relaxation && options.round_solution) {
    ...
    while (paths.size() < options.max_rounded_paths &&
      ...
      for (const auto& [edge_id, e] : edges_) {
        ...
        if (std::find(new_path.begin(), new_path.end(), e.get()) !=
          ...
          added_constraints.push_back(prog.AddBoundingBoxConstraint(
              VectorXd::Zero(e->y_.size()), VectorXd::Zero(e->y_.size()),
              e->y_));
          added_constraints.push_back(prog.AddBoundingBoxConstraint(
              VectorXd::Zero(e->z_.size()), VectorXd::Zero(e->z_.size()),
              e->z_));
          added_constraints.push_back(prog.AddBoundingBoxConstraint(
              VectorXd::Zero(e->ell_.size()), VectorXd::Zero(e->ell_.size()),
              e->ell_));
        }
      }
      ...
      if (options.solver) {
        options.solver->Solve(prog, {}, options.solver_options,
                              &rounded_result);
      } else {
        rounded_result = solvers::Solve(prog, {}, options.solver_options);
      }

  ...
  for (const auto& [edge_id, e] : edges_) {
    ...
    for (int i = 0; i < e->ell_.size(); ++i) {
      ...
      if (LinearCost* lc = dynamic_cast<LinearCost*>(cost)) {
        ...
        int c_rows = l1c->A().rows();
        int c_cols = l1c->A().cols();
        auto l1c_slack =
            prog.NewContinuousVariables(c_rows, fmt::format("l1c_slack{}", i));
        VectorXDecisionVariable cost_vars(vars.size() + l1c_slack.size());
        cost_vars << vars, l1c_slack;
        MatrixXd A_linear = MatrixXd::Zero(2 * c_rows + 1, cost_vars.size());
        A_linear.block(0, 0, c_rows, 1) = l1c->b();       // bϕ.
        A_linear.block(0, 2, c_rows, c_cols) = l1c->A();  // Ax.
        A_linear.block(0, c_cols + 2, c_rows, l1c_slack.size()) =
            -MatrixXd::Identity(c_rows, c_rows);  // -δ.

        A_linear.block(c_rows, 0, c_rows, 1) = -l1c->b();       // -bϕ.
        A_linear.block(c_rows, 2, c_rows, c_cols) = -l1c->A();  // -Ax.
        A_linear.block(c_rows, c_cols + 2, c_rows, l1c_slack.size()) =
            -MatrixXd::Identity(c_rows, c_rows);  // -δ.

        A_linear(2 * c_rows, 1) = -1;
        A_linear.block(2 * c_rows, c_cols + 2, 1, l1c_slack.size()) =
        ...
        int c_rows = linfc->A().rows();
        MatrixXd A_linear(2 * c_rows, vars.size());
        A_linear.block(0, 0, c_rows, 1) = linfc->b();                  // bϕ.
        A_linear.block(0, 1, c_rows, 1) = -VectorXd::Ones(c_rows);     // -ℓ.
        A_linear.block(0, 2, c_rows, linfc->A().cols()) = linfc->A();  // Ax.
        A_linear.block(c_rows, 0, c_rows, 1) = -linfc->b();            // -bϕ.
        A_linear.block(c_rows, 1, c_rows, 1) = -VectorXd::Ones(c_rows);  // -ℓ.
        A_linear.block(c_rows, 2, c_rows, linfc->A().cols()) =
            -linfc->A();  // -Ax.
        prog.AddLinearConstraint(A_linear,
                                 VectorXd::Constant(A_linear.rows(), -inf),

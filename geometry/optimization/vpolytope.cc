...
VPolytope VPolytope::MakeBox(const Eigen::Ref<const VectorXd>& lb,
  ...
  MatrixXd vertices = lb.replicate(1, std::pow(2, n));
...
bool VPolytope::DoPointInSet(const Eigen::Ref<const Eigen::VectorXd>& x,
  ...
  VectorXd xstar = vertices_ * result.GetSolution(alpha);
  /*
    std::cout << "vertices: " << vertices_ << std::endl;
    std::cout << "x = " << x << std::endl;
    std::cout << "xstar = " << xstar << std::endl;
    std::cout << "z = " << result.GetSolution(z) << std::endl;
    std::cout << "tol = " << tol << std::endl;
    std::cout << "xstar - x = " << xstar - x << std::endl;
    std::cout << "tol - z = " << tol - result.GetSolution(z[0]) << std::endl;
    */
  return is_approx_equal_abstol(x, xstar, tol);
}

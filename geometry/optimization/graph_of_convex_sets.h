...
The convex relaxation of the GCS problem has two components:
...
relaxation and the true non-convex constraints to the rounding problem. This
can be controlled by the ProblemComponent enum in the AddConstraint method.
Note that when solving the full mixed-integer problem, all constraints from the
relaxation and rounding are considered.
 ...
 public:
  ...
  class Vertex final {
    ...
    */
    ...
    constraints of both problems will be considered in the mixed integer
    problem.
    @throws std::exception if binding.variables() is not a subset of x().
    @throws std::exception if ambient_dimension() == 0.
    @pydrake_mkdoc_identifier{binding}

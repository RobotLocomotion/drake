...
class GraphOfConvexSets {
 public:
  ...
  /** Adds an edge to the graph from VertexId @p uid to VertexId @p vid.  The
  ids must refer to valid vertices in this graph. */
  Edge* AddEdge(const VertexId& u_id, const VertexId& v_id,
                std::string name = "");

  /** Adds an edge to the graph from Vertex @p u to Vertex @p v.  The
  vertex references must refer to valid vertices in this graph. */

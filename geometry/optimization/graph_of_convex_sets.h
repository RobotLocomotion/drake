...
struct SppOptions {
  ...
  /** Optimizaer to be used to solve the shortest path optimization problem. If
  not set, the best solver for the given problem is selected. Note that if the
  solver cannot handle the type of optimization problem generated, it will
  throw. */
  const solvers::SolverInterface* solver = nullptr;

  /** Options passed to the solver once the problem is generated to set the
  solver settings. */
  solvers::SolverOptions solver_options = solvers::SolverOptions();
};
...
class GraphOfConvexSets {
  ...
  */
  ...
  /** Removes any constraints added to each edge with AddPhiConstraint. */
  ...
  */
  ...
  `SppOptions` for further details.
  ...
  */
  solvers::MathematicalProgramResult SolveShortestPath(
      ...
      const SppOptions& options = SppOptions()) const;
  ...
  */
  DRAKE_DEPRECATED("2022-10-01", "Use version with SppOptions")
  solvers::MathematicalProgramResult SolveShortestPath(

...
void CspaceFreeBox::ComputeSBox(
  ...
  if ((q_box_lower.array() > q_box_upper.array()).any()) {
    throw std::runtime_error(
        "CspaceFreeBox: q_box_lower has some entry larger than q_box_upper.");
  }
  ...
  if ((q_box_lower.array() > q_position_upper.array()).any()) {
    throw std::runtime_error(
        "CspaceFreeBox: q_box_lower has some entry larger the the robot "
        "position upper limit.");
  }
  if ((q_box_upper.array() < q_position_lower.array()).any()) {
    throw std::runtime_error(
        "CspaceFreeBox: q_box_upper has some entry smaller than the robot "
        "positoin lower limit.");
  }
...
void CspaceFreeBox::GenerateCertifyPolynomials(

...
HPolyhedron::HPolyhedron(const VPolytope& vpoly)
  ...
  if (affine_hull.AffineDimension() < affine_hull.ambient_dimension()) {
    ...
    if (affine_hull.AffineDimension() == 0) {
      ...
      // and P is the left pseudoinverse of its basis (so that PB is the
      // identity matrix, where B is the matrix whose columns are the basis
      // vectors). Thus, an HPolyhedron in local coordinates

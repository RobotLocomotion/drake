...
HPolyhedron::HPolyhedron(const VPolytope& vpoly)
  ...
  if (affine_hull.AffineDimension() < affine_hull.ambient_dimension()) {
    ...
    if (affine_hull.AffineDimension() == 0) {
      ...
      // convering it to an HPolyhedron. This VPolytope is full dimensional
      ...
      // Now, lift the HPolyhedron to the original ambient space. A point x
      // is projected to P(x-d), where d is the AffineSubspace's translation,
      // and P is the pseudoinverse of its basis. Thus, an HPolyhedron in
      // local coordinates {Ay <= b : y in R^m} can be lifted to an
      // HPolyhedron in global coordinates by substituting P(x-d) for y. After
      // simplification, we have {APx <= b + APd : x in R^n}.

      // First, we obtain the matrix P by converting the standard basis to
      // the local coordinate system, relative to the translation:
      Eigen::MatrixXd P_in =
  ...
  if (qhull.qhullStatus() != 0) {
    throw std::runtime_error(
        fmt::format("Qhull neat with status {} and  message:\n{}",

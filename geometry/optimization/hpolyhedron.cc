...
VectorXd HPolyhedron::UniformSample(
    ...
    const std::optional<const Eigen::MatrixXd>& subspace, double tol) const {
  ...
  const int sampling_dim =
      subspace.has_value() ? subspace.value().cols() : previous_sample.rows();
  ...
  if (subspace.has_value()) {
    for (int i = 0; i < A_.rows(); ++i) {
      skip[i] = true;
      for (int j = 0; j < subspace.value().cols(); ++j) {
        if (abs(A_.row(i) * subspace.value().col(j)) > tol) {
          skip[i] = false;
          break;
        }
      }
  ...
  for (int step = 0; step < mixing_steps; ++step) {
    ...
    direction = subspace.has_value() ? subspace.value() * gaussian_sample
                                     : gaussian_sample;

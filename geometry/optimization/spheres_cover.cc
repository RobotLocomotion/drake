...
SpheresOuterApproximator::SpheresOuterApproximator(
    ...
    const Eigen::Ref<const Eigen::MatrixXd>& outliers, double max_sphere_radius)
    : prog_{},
      dim_{static_cast<int>(outliers.rows())},
      polytopes_{std::move(polytopes)},
      outliers_{outliers},
      max_sphere_radius_{max_sphere_radius} {
  DRAKE_DEMAND(max_sphere_radius > 0);
  ...
  for (int i = 0; i < num_spheres; ++i) {
    for (int j = 0; j < num_outliers; ++j) {
      AddOutlierNotInSphereConstraint(j, i);
    }
  }
  ...
  for (int i = 0; i < num_spheres; ++i) {
    ...
    b_rotated_lorentz(0) = max_sphere_radius_ * max_sphere_radius_;
...
void SpheresOuterApproximator::AddOutlierNotInSphereConstraint(int outlier_idx,
                                                               int sphere_idx) {
  ...
  const double M1 = std::max(-(q.squaredNorm() + a_dot_q_min -
                               max_sphere_radius_ * max_sphere_radius_),
                             0.);

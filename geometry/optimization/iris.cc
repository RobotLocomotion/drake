...
class SamePointConstraint : public solvers::Constraint {
 ...
 private:
  ...
  void DoEval(const Eigen::Ref<const AutoDiffVecXd>& x,
    ...
    // We denote the variables we take gradient as z, namely x.derivatives()
    // store the gradient ∂x/∂z.
    const Eigen::Vector3d y_val =
        X_WA * math::ExtractValue(p_AA) - X_WB * math::ExtractValue(p_BB);
    Eigen::Matrix3Xd dy_dz(3, plant_->num_positions() + 6);
    dy_dz << Jq_v_WA - Jq_v_WB, X_WA.rotation().matrix(),
        -X_WB.rotation().matrix();
    *y = math::InitializeAutoDiff(y_val, dy_dz * math::ExtractGradient(x));
  }

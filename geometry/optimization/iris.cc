...
HPolyhedron Iris(const ConvexSets& obstacles, const Ref<const VectorXd>& sample,
  ...
  // Pre-allocate matrices of the maximum size:
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> A(
      domain.A().rows() + N, dim);
  VectorXd b(domain.A().rows() + N);
  A.topRows(domain.A().rows()) = domain.A();
  b.head(domain.A().rows()) = domain.b();
  // list the {scale, index}, so that I can back out the indices after a sort.
  ...
  while (true) {
    ...
    for (int i = 0; i < N; ++i) {
      ...
      // Only add a constraint if this point is still the set that has been
      // constructed so far on this iteration.
      if (((A.topRows(num_constraints) * point).array() <=
        ...
        A.row(num_constraints) =
            2.0 * E.A().transpose() * E.A() * (point - E.center());
        A.row(num_constraints).normalize();
...
class IrisConvexSetMaker final : public ShapeReifier {
 public:
  ...
  const ConvexSets& sets() { return sets_; }

...
class CspaceFreePolytope {
 public:
  ...
  /**
   ...
   s_upper - s >= 0.
   */
  struct SearchPlaneLagrangians {
    SearchPlaneLagrangians(int C_rows, int s_size)
    ...
    [[nodiscard]] SearchPlaneLagrangians GetSolution(
   ...
   */
  struct SearchPlaneStruct {
    SearchPlaneStruct() : prog{new solvers::MathematicalProgram()} {}

    std::unique_ptr<solvers::MathematicalProgram> prog;
    std::vector<SearchPlaneLagrangians> positive_side_lagrangians;
    std::vector<SearchPlaneLagrangians> negative_side_lagrangians;
  };
 ...
 private:
  ...
  /**
   Constructs a program to search for the separating plane for a pair of
   geometries, for the entire set of configuration C * s <= d and s_lower <= s
   <= s_upper.
   ...
   later.
   */
  [[nodiscard]] SearchPlaneStruct ConstructProgramSearchPlane(

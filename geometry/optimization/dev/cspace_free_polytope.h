...
class CspaceFreePolytope {
 public:
  ...
  /**
   ...
   enabling us using stronger Positivstellasatz theorem. For more explanation on
   this term, refer to the documentation in the private function
   AddUnitLengthConstraint.

   This struct records the Lagrangian multipliers λ₁(y,
   s), λ₂(y, s), λ₃(y, s) and λ₄(y, s).
   */
 ...
 private:
  ...
  // s). To incur the strong Positivstellasatz theorem on a compact set, we
  // notice that p(y, s) is homogeneous in y, hence p(y, s) >= 0 on the polytope
  // if and only if we further restrain y to be on the unit circle. Namely we
  // consider the condition p(y, s) >= 0 if (y, s) ∈ {s | C*s<=d,
  // s_lower<=s<=s_upper} ∩ {y | yᵀy=1}
  //
  // Then we impose the condition
  // p(y, s) - λ₁(y, s)ᵀ(d−Cs) − λ₂(y, s)ᵀ(s−s_lower) − λ₃(y, s)ᵀ(s_upper−s) -
  // λ₄(y, s)(1−yᵀy) is sos.
  // λ₁(y, s), λ₂(y, s), λ₃(y, s) are all sos.
  // λ₄(y, s) is a free polynomial.

...
// -offset - separating_margin -(aᵀ*p_AQ + b) if plane_side = kNegative
...
void CollisionGeometry::ImplementGeometry(const Box& box, void* data) {
  ...
  // if we only require separation but don't explicitly compute the separating
  // margin, then we impose the following constraint:
  ...
  reify_data->rationals->reserve(8);
  ...
  for (int i = 0; i < 8; ++i) {
    reify_data->rationals->push_back(ComputePointOnPlaneSideRational(
        *(reify_data->a), *(reify_data->b), p_GV.col(i), X_BG_,

...
class OnPlaneSideReifier : public ShapeReifier {
 ...
 private:
  ...
  void ImplementGeometry(const Sphere& sphere, void* data) {
    ...
    // it is equivalent to the following condition aᵀ*p_AS + b ≥ r + δ       if
    // plane_side = kPositive   (1a) aᵀ*p_AS + b ≤ -(r + δ)    if plane_side =
    // kNegative   (1b) |a| ≤ 1 (2) where S is the center of the sphere.
    auto* reify_data = static_cast<ReifyData*>(data);

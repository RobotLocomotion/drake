...
CspaceFreePolytope::AddUnitLengthConstraint(
  ...
  for (int i = 0; i < unit_length_vec.rows(); ++i) {
    // We set the Lagrangian multiplier degrees based on the assumping that
  ...
  // − λ₂(y, s)ᵀ(s−s_lower) − λ₃(y, s)ᵀ(s_upper−s) - λ₄(y, s)(1−yᵀy) being sos,
  // we can set λ₁(y, s) λ₂(y, s) and λ₃(y, s) to be quadratic polynomials of y
  // and λ₄(y, s) to be an affine polynomial of s. These Lagrangian multipliers
  // are sufficient to match the degree of p(y, s).

...
void FindMonomialBasis(
  ...
  if (it == map_body_to_monomial_basis->end()) {
    ...
    for (const auto& mobilizer_index : mobilizers) {
...
CspaceFreePolytope::SearchPlaneLagrangians
CspaceFreePolytope::SearchPlaneLagrangians::GetSolution(
    const solvers::MathematicalProgramResult& result) const {
  CspaceFreePolytope::SearchPlaneLagrangians ret(this->polytope.rows(),
                                                 this->s_lower.rows());
...
CspaceFreePolytope::SearchPlaneStruct
CspaceFreePolytope::ConstructProgramSearchPlane(
  ...
  auto add_geometry =
      [&plane, &d_minus_Cs, &s_minus_s_lower, &s_upper_minus_s,
       ...
       &s_upper_redundant_indices, this, map_body_to_monomial_basis](
          ...
          std::vector<SearchPlaneLagrangians>* search_plane_lagrangians) {
        VectorX<symbolic::Monomial> monomial_basis;
        FindMonomialBasis(
  ...
  if (!plane_geometries.unit_length_vectors.empty()) {
    throw std::runtime_error(
        "ConstructProgramSearchPlane: cannot handle unit length vector "
        "constraint yet.");
  }
  return ret;
}

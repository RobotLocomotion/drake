...
CspaceFreePolytope::AddUnitLengthConstraint(
  ...
  // Since p(y, s) is quadratic in y and linear in s, we know that to make
  // p(y, s) - λ₁(y, s)ᵀ(d−Cs) − λ₂(y, s)ᵀ(s−s_lower) − λ₃(y, s)ᵀ(s_upper−s) -
  // λ₄(y, s)(1−yᵀy) being sos, we only need λ₁(y, s) λ₂(y, s) and λ₃(y, s) to
  // be quadratic polynomials of y and λ₄(y, s) to be an affine polynomial of s.
  ...
  // This Lagrangian is just an affine polynomial of s.

...
TEST_F(CollisionGeometryTest, Box) {
  ...
  plant_->CalcPointsPositions(
      *plant_context_, plant_->get_body(body_indices_[0]).body_frame(), p_BV,
...
TEST_F(CollisionGeometryTest, Convex) {
  ...
  plant_->CalcPointsPositions(
      *plant_context_, plant_->get_body(body_indices_[1]).body_frame(), p_BV,
      plant_->get_body(expressed_body).body_frame(), &p_AV);
...
TEST_F(CollisionGeometryTest, Capsule) {
  ...
  CollisionGeometry capsule(&model_inspector.GetShape(body2_capsule_),
                            body_indices_[2], body2_capsule_,
  ...
  plant_->CalcPointsPositions(
      *plant_context_, plant_->get_body(body_indices_[2]).body_frame(), p_BS,

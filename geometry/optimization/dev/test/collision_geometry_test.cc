...
void CheckPolynomialWSlack(const symbolic::RationalFunction& rational,
                           const symbolic::Environment& env,
...
TEST_F(CollisionGeometryTest, Sphere) {
  ...
  CheckPolynomialWSlack(rationals[0], env, -a_expr.dot(p_AS) - b_expr,
                        Eigen::Matrix3d::Identity(), a_expr, y_slack_, radius);
  CheckRationalExpression(rationals[1], env, -1 - a_expr.dot(p_AS) - b_expr);
  ...
  CheckPolynomialWSlack(rationals[0], env, a_expr.dot(p_AS) + b_expr,
                        Eigen::Matrix3d::Identity(), a_expr, y_slack_, radius);
  CheckRationalExpression(rationals[1], env, a_expr.dot(p_AS) + b_expr - 1);
}
...
TEST_F(CollisionGeometryTest, Capsule) {
  ...
  for (int i = 0; i < 2; ++i) {
    CheckPolynomialWSlack(rationals[i], env, -a_expr.dot(p_AS.col(i)) - b_expr,
                          Eigen::Matrix3d::Identity(), a_expr, y_slack_,
                          capsule_shape.radius());
  }
  CheckRationalExpression(rationals[2], env, -a_expr.dot(p_AO) - b_expr - 1);
...
TEST_F(CollisionGeometryTest, Cylinder) {
  ...
  CheckRationalExpression(rationals[0], env, b_G_expr - 1);
  ...
  // clang-format on
  for (int i = 0; i < 2; ++i) {
    CheckPolynomialWSlack(rationals[i + 1], env,
  ...
  cylinder.OnPlaneSide(a_, b_, X_AB_multilinear, rational_forward_kin_,
                       PlaneSide::kNegative, y_slack_, &rationals);
  EXPECT_EQ(rationals.size(), 3);
  CheckRationalExpression(rationals[0], env, -1 - b_G_expr);
  for (int i = 0; i < 2; ++i) {
    CheckPolynomialWSlack(rationals[i + 1], env,
                          -a_G_expr(2) * height[i] - b_G_expr, P, a_G_expr,
                          y_slack_.head<2>(), radius);

...
TEST_F(CIrisToyRobotTest, CspaceFreePolytopeGenerateRationals) {
  ...
  const auto ret = dut.GenerateRationals(q_star, filtered_collision_pairs,
                                         separating_margin);
  ...
  for (const auto& plane_geometries : ret) {
    ...
    if (plane.positive_side_geometry->type() == GeometryType::kCylinder ||
      ...
      throw std::runtime_error("Has not implemented cylinder for C-IRIS yet.");
    ...
    EXPECT_EQ(plane_geometries.rationals.size(),
              plane.positive_side_geometry->num_plane_side_rationals() +
                  plane.negative_side_geometry->num_plane_side_rationals());
  }
}
}  // namespace optimization
}  // namespace geometry

...
class CspaceFreePolytopeTester {
 public:
  ...
  [[nodiscard]] CspaceFreePolytope::SearchPlaneStruct
  ConstructProgramSearchPlane(
    ...
    return cspace_free_polytope_->ConstructProgramSearchPlane(
...
TEST_F(CIrisToyRobotTest, CalcDminusCs) {
  ...
  const auto ret = CalcDminusCs<symbolic::Variable>(C, d, s);
...
TEST_F(CIrisToyRobotTest, ConstructProgramSearchPlane) {
  // Test ConstructProgramSearchPlane with no unit-length-vector constraint.
  CspaceFreePolytopeTester tester(plant_, scene_graph_,
                                  SeparatingPlaneOrder::kAffine);
  ...
  const VectorX<symbolic::Polynomial> d_minus_Cs = CalcDminusCs<double>(
      C, d, tester.cspace_free_polytope().rational_forward_kin().s());
  VectorX<symbolic::Polynomial> s_minus_s_lower;
  VectorX<symbolic::Polynomial> s_upper_minus_s;
  const Eigen::Vector3d q_star(0, 0, 0);
  tester.CalcSBoundsPolynomial(q_star, &s_minus_s_lower, &s_upper_minus_s);
  ...
  auto ret = tester.ConstructProgramSearchPlane(
      plane_geometries, d_minus_Cs, s_minus_s_lower, s_upper_minus_s,
    ...
    auto check_rational_positive_in_polytope =
        [&result, &d_minus_Cs, &s_minus_s_lower, &s_upper_minus_s,
         ...
         &s_upper_redundant_indices, &s, &s_samples](
            ...
            const std::vector<CspaceFreePolytope::SearchPlaneLagrangians>&

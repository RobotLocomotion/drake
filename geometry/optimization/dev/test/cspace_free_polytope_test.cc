...
TEST_F(CIrisToyRobotTest, BinarySearch) {
  ...
  options.epsilon_min = 0.01;
  options.epsilon_max = 0.1;
  solvers::MosekSolver solver;
  if (solver.available()) {
    ...
    options.epsilon_max = 0.01;
    options.epsilon_min = 0;
    ...
    EXPECT_TRUE(CompareMatrices(
        result->d, d + options.epsilon_max * Eigen::VectorXd::Ones(d.rows()),
        1E-10));
    CheckSeparationBySamples(tester, *diagram_, s_samples, result->C, result->d,
                             result->a, result->b, q_star,
                             ignored_collision_pairs);
    ...
    options.epsilon_min = 10;
    options.epsilon_max = 20;
    result = tester.cspace_free_polytope().BinarySearch(ignored_collision_pairs,
                                                        C, d, options);

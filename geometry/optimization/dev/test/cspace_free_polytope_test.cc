...
class CspaceFreePolytopeTester {
 public:
  ...
  ConstructPolytopeSearchProgram(
      const CspaceFreePolytope::IgnoredCollisionPairs& ignored_collision_pairs,
    ...
    return cspace_free_polytope_->ConstructPolytopeSearchProgram(
  ...
  void AddOuterPolytope(
    ...
    cspace_free_polytope_->AddOuterPolytope(prog, Q, s0, C, d,
                                            ellipsoid_margins);
  }
  ...
  HPolyhedron GetPolyhedron(const Eigen::MatrixXd& C,
                            const Eigen::VectorXd& d) const {
    return cspace_free_polytope_->GetPolyhedron(C, d);
  }
...
TEST_F(CIrisToyRobotTest, AddOuterPolytope) {
  const Eigen::Vector3d q_star(0, 0, 0);
  CspaceFreePolytopeTester tester(plant_, scene_graph_,
                                  SeparatingPlaneOrder::kAffine, q_star);
  ...
  Q << 1, 2, -1,
       ...
       2, -1, 3;
  // clang-format on
  const Eigen::Vector3d s0(1, 5, 10);
  tester.AddOuterPolytope(&prog, Q, s0, C, d, ellipsoid_margins);
...
TEST_F(CIrisToyRobotTest, FindPolytopeGivenLagrangian) {
  ...
  if (solver.available()) {
    ...
    const HPolyhedron cspace_h_polyhedron = tester.GetPolyhedron(C, d);

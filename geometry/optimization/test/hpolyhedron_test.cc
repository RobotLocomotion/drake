...
GTEST_TEST(HPolyhedronTest, ConstructorFromLinearProgramThrows) {
  ...
  // Test that programs with trivially infeasible lower bounds throw.
  ...
  prog1.AddLinearConstraint(MatrixXd::Identity(1, 1), Vector1d(0),
                            Vector1d(-kInf), x1);
  EXPECT_THROW(HPolyhedron{prog1}, std::exception);
  ...
  prog2.AddLinearConstraint(MatrixXd::Identity(1, 1), Vector1d(kInf),
                            Vector1d(0), x2);
  EXPECT_THROW(HPolyhedron{prog2}, std::exception);

  MathematicalProgram prog3;
  VectorXDecisionVariable x3 = prog3.NewContinuousVariables(1);
  prog3.AddLinearEqualityConstraint(MatrixXd::Identity(1, 1), Vector1d(kInf),
                                    x3);
  EXPECT_THROW(HPolyhedron{prog3}, std::exception);
}

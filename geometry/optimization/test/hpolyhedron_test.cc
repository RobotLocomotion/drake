...
bool PointInScaledSet(const solvers::VectorXDecisionVariable& x_vars,
  ...
  return prog->CheckSatisfiedAtInitialGuess(constraints, 0);
}
...
GTEST_TEST(HPolyhedronTest, NonnegativeScalingTest2) {
  ...
  std::vector<Binding<Constraint>> constraints =
      H.AddPointInNonnegativeScalingConstraints(&prog, A, x, b, t);

  EXPECT_EQ(constraints.size(), 2);

  EXPECT_TRUE(PointInScaledSet(x, t, .99 * ub.head(2), Vector2d(1.0, 0), &prog,
                               constraints));
  EXPECT_TRUE(PointInScaledSet(x, t, .99 * ub.head(2), Vector2d(0, -1.0), &prog,
                               constraints));
  ...
  EXPECT_FALSE(PointInScaledSet(x, t, 1.01 * ub.head(2), Vector2d(0, -1.0),
                                &prog, constraints));
  EXPECT_FALSE(PointInScaledSet(x, t, .99 * ub.head(2), Vector2d(-0.01, 0),
                                &prog, constraints));
  EXPECT_FALSE(PointInScaledSet(x, t, .99 * ub.head(2), Vector2d(0, -0.01),
                                &prog, constraints));
  EXPECT_TRUE(PointInScaledSet(x, t, .49 * ub.head(2), Vector2d(.5, 0), &prog,
                               constraints));
  EXPECT_TRUE(PointInScaledSet(x, t, .49 * ub.head(2), Vector2d(0, -0.5), &prog,
                               constraints));
  EXPECT_FALSE(PointInScaledSet(x, t, .51 * ub.head(2), Vector2d(.5, 0), &prog,
                                constraints));
  EXPECT_FALSE(PointInScaledSet(x, t, .51 * ub.head(2), Vector2d(0, -0.5),
                                &prog, constraints));

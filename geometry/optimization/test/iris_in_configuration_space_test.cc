...
// Three boxes.  Two on the outside are fixed.  One in the middle on a prismatic
// joint.  Additional constraints are added via options.prog:
//    q <= 0.5
//    sin(q) >= -1/sqrt(2)
...
<robot name="double_pendulum">
...

  {
    ...
    meshcat->SetLine("True CFree", points, 2.0, Rgba(0, 0, 1));
    ...
    meshcat->SetObject("Test point (max)", Sphere(0.01), Rgba(1, 0, 0));
    meshcat->SetTransform("Test point (max)",
                          math::RigidTransform(Eigen::Vector3d(
                              theta1, theta2_max + kOuterTol, 0)));
    meshcat->SetObject("Test point (min)", Sphere(0.01), Rgba(1, 0, 0));

...
TEST_F(ThreeBoxes, NonConvexRounding) {
  ...
  */
  ...
  // Since all constraints were added to the relaxation xor rounding component,
  // we expect kBoth to be empty.
  EXPECT_EQ(source_->GetConstraints(ProblemComponent::kBoth).size(), 0);
  EXPECT_EQ(target_->GetConstraints(ProblemComponent::kBoth).size(), 0);
  EXPECT_EQ(sink_->GetConstraints(ProblemComponent::kBoth).size(), 0);

...
TEST_F(TwoPoints, AddCost) {
  ...
  auto edge_costs = e_->GetCosts();
  EXPECT_TRUE(edge_costs[0] == b0);
  EXPECT_TRUE(edge_costs[1] == b1);
...
TEST_F(TwoPoints, AddConstraint) {
  ...
  // Confirm that they are all accessible.
  auto edge_constraints = e_->GetConstraints();
  EXPECT_TRUE(edge_constraints[0] == b0);
  EXPECT_TRUE(edge_constraints[1] == b1);

...
using ProblemComponent = GraphOfConvexSets::ProblemComponent;
...
TEST_F(TwoPoints, AddConstraint) {
  ...
  const auto& edge_constraints_relaxation =
      e_->GetConstraints(ProblemComponent::kRelaxation);
  ...
  const auto& vertex_constraints_relaxation =
      u_->GetConstraints(ProblemComponent::kRelaxation);
  EXPECT_EQ(vertex_constraints_relaxation[0], u_b0);
  EXPECT_EQ(vertex_constraints_relaxation[1], u_b1);

  const auto& edge_constraints_rounding =
      e_->GetConstraints(ProblemComponent::kRounding);
  EXPECT_EQ(edge_constraints_rounding[0], b0);
  EXPECT_EQ(edge_constraints_rounding[1], b1);
  const auto& vertex_constraints_rounding =
      u_->GetConstraints(ProblemComponent::kRounding);
...
TEST_F(ThreeBoxes, NonConvexRounding) {
  ...
  */
  ...
  constraints_relaxation.push_back(source_->AddConstraint(
      source_->x()[0] <= -0.5, ProblemComponent::kRelaxation));
  constraints_relaxation.push_back(source_->AddConstraint(
      source_->x()[1] >= 0.5, ProblemComponent::kRelaxation));

  constraints_rounding.push_back(source_->AddConstraint(
      pow(source_->x()[0], 2) + pow(source_->x()[1], 2) >= 1.0,
      ProblemComponent::kRounding));
  constraints_rounding.push_back(source_->AddConstraint(
      source_->x()[0] <= -0.1, ProblemComponent::kRounding));
  constraints_rounding.push_back(source_->AddConstraint(
      source_->x()[1] >= 0.1, ProblemComponent::kRounding));

  // Verify the constraints in the relaxation only contain the relaxed
  // constraints, but not the rounding constraints.
  for (const auto& constraint :
       source_->GetConstraints(ProblemComponent::kRelaxation)) {
  ...
  for (const auto& constraint :
       source_->GetConstraints(ProblemComponent::kRounding)) {
  ...
  */
  constraints_relaxation.push_back(target_->AddConstraint(
      target_->x()[0] >= 0.5, ProblemComponent::kRelaxation));
  constraints_relaxation.push_back(target_->AddConstraint(
      target_->x()[1] >= 0.5, ProblemComponent::kRelaxation));

  constraints_rounding.push_back(target_->AddConstraint(
      pow(target_->x()[0], 2) + pow(target_->x()[1], 2) >= 1.0,
      ProblemComponent::kRounding));
  constraints_rounding.push_back(target_->AddConstraint(
      target_->x()[0] >= 0.1, ProblemComponent::kRounding));
  constraints_rounding.push_back(target_->AddConstraint(
      target_->x()[1] >= 0.1, ProblemComponent::kRounding));

  // Verify the constraints in the relaxation only contain the relaxed
  // constraints, but not the rounding constraints.
  for (const auto& constraint :
       target_->GetConstraints(ProblemComponent::kRelaxation)) {
    EXPECT_TRUE(std::find(constraints_relaxation.begin(),
                          constraints_relaxation.end(),
                          constraint) != constraints_relaxation.end());
    EXPECT_FALSE(std::find(constraints_rounding.begin(),
  ...
  // constraints, but not the relaxed constraints.
  for (const auto& constraint :
       target_->GetConstraints(ProblemComponent::kRounding)) {
    EXPECT_FALSE(std::find(constraints_relaxation.begin(),
                           constraints_relaxation.end(),
                           constraint) != constraints_relaxation.end());
    EXPECT_TRUE(std::find(constraints_rounding.begin(),
  ...
  */
  ...
  constraints_relaxation.push_back(sink_->AddConstraint(
      sink_->x()[0] >= 0.5, ProblemComponent::kRelaxation));
  constraints_relaxation.push_back(sink_->AddConstraint(
      sink_->x()[1] <= -0.5, ProblemComponent::kRelaxation));

  constraints_rounding.push_back(
      sink_->AddConstraint(pow(sink_->x()[0], 2) + pow(sink_->x()[1], 2) >= 1.0,
                           ProblemComponent::kRounding));
  constraints_rounding.push_back(
      sink_->AddConstraint(sink_->x()[0] >= 0.1, ProblemComponent::kRounding));
  constraints_rounding.push_back(
      sink_->AddConstraint(sink_->x()[1] <= -0.1, ProblemComponent::kRounding));

  // Verify the constraints in the relaxation only contain the relaxed
  // constraints, but not the rounding constraints.
  for (const auto& constraint :
       sink_->GetConstraints(ProblemComponent::kRelaxation)) {
    EXPECT_TRUE(std::find(constraints_relaxation.begin(),
                          constraints_relaxation.end(),
                          constraint) != constraints_relaxation.end());
    EXPECT_FALSE(std::find(constraints_rounding.begin(),
  ...
  // constraints, but not the relaxed constraints.
  for (const auto& constraint :
       sink_->GetConstraints(ProblemComponent::kRounding)) {
    EXPECT_FALSE(std::find(constraints_relaxation.begin(),
                           constraints_relaxation.end(),
                           constraint) != constraints_relaxation.end());
    EXPECT_TRUE(std::find(constraints_rounding.begin(),
  ...
  constraints_all.insert(
      constraints_all.end(),
      source_->GetConstraints(ProblemComponent::kBoth).begin(),
      source_->GetConstraints(ProblemComponent::kBoth).end());
  constraints_all.insert(
      constraints_all.end(),
      target_->GetConstraints(ProblemComponent::kBoth).begin(),
      target_->GetConstraints(ProblemComponent::kBoth).end());
  constraints_all.insert(constraints_all.end(),
                         sink_->GetConstraints(ProblemComponent::kBoth).begin(),
                         sink_->GetConstraints(ProblemComponent::kBoth).end());

...
// Project each row of q_samples to the box [q_box_lower, q_box_upper].
Eigen::MatrixXd ProjectToBox(const Eigen::Ref<const Eigen::MatrixXd>& q_samples,
                             const Eigen::VectorXd& q_box_lower,
                             const Eigen::VectorXd& q_box_upper) {
  DRAKE_DEMAND(q_samples.cols() == q_box_lower.rows());
  Eigen::MatrixXd ret = q_samples;
  for (int i = 0; i < ret.rows(); ++i) {
    ret.row(i) = ret.row(i).cwiseMax(q_box_lower);
    ret.row(i) = ret.row(i).cwiseMin(q_box_upper);
  }
  return ret;
}
...
TEST_F(CIrisToyRobotTest, ConstructPlaneSearchProgram) {
  ...
  {
    // Test the program for polytopic geometris.
  ...
  {
    // Test the program for non-polytopic geometris.

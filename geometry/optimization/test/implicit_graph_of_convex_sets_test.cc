...
class ThreePointsGcs : public ImplicitGraphOfConvexSets {
 ...
 private:
  ...
  std::vector<Edge*> Expand(Vertex* v) override {
    num_expansions_++;
    if (v == source_) {
      Vertex* target =
          mutable_gcs().AddVertex(Point(Vector2d(-2., 4.)), "target");
      Vertex* sink = mutable_gcs().AddVertex(Point(Vector2d(5., -2.3)), "sink");
      Edge* e_on = mutable_gcs().AddEdge(v, target);
      Edge* e_off = mutable_gcs().AddEdge(v, sink);
      return {e_on, e_off};
    }
    return {};
  }
...
class SimpleLoopGcs : public ImplicitGraphOfConvexSets {
 ...
 private:
  std::vector<Edge*> Expand(Vertex* v) override {
    if (v->name() == "c") {
      return {mutable_gcs().AddEdge(v, GetVertex("a"))};
    } else {
      std::string next_vertex = {static_cast<char>(v->name()[0] + 1)};
      return {mutable_gcs().AddEdge(v, GetVertex(next_vertex))};
    }
  }
 ...
 private:
  std::vector<Edge*> Expand(Vertex* v) override {
    ...
    if (key.type == VertexType::kH) {
      if (key.x < 3) {
        successors.push_back(mutable_gcs().AddEdge(
            v, GetVertex(key.x + 1, key.y, VertexType::kV)));
      }
      if (key.x > 1) {
        successors.push_back(mutable_gcs().AddEdge(
            v, GetVertex(key.x - 1, key.y, VertexType::kV)));
      ...
      if (key.y < 3) {
        successors.push_back(mutable_gcs().AddEdge(
            v, GetVertex(key.x, key.y + 1, VertexType::kH)));
      }
      if (key.y > 1) {
        successors.push_back(mutable_gcs().AddEdge(
            v, GetVertex(key.x, key.y - 1, VertexType::kH)));

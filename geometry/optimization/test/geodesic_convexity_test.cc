...
GTEST_TEST(GeodesicConvexityTest, CalcPairwiseIntersectionsAPI) {
  ...
  EXPECT_THROW(
      CalcPairwiseIntersections(sets, std::vector<int>{}, true, bad_bboxes),
      std::exception);
  EXPECT_THROW(CalcPairwiseIntersections(sets, sets, std::vector<int>{}, true,
  ...
  EXPECT_THROW(
      CalcPairwiseIntersections(sets, sets, std::vector<int>{}, true,
...
GTEST_TEST(GeodesicConvexityTest, CalcPairwiseIntersections1) {
  ...
  // All results should be the same if some or all of the bounding boxes are
  // provided.
  std::vector<Hyperrectangle> bboxes_A = {h1, h2};
  std::vector<Hyperrectangle> bboxes_B = {h3, h4};
  EXPECT_EQ(CalcPairwiseIntersections(sets_A, sets_B, std::vector<int>{}, true,
                                      bboxes_A, {})
                .size(),
            0);
  EXPECT_EQ(CalcPairwiseIntersections(sets_A, sets_B, std::vector<int>{0}, true,
                ...
                .size(),
            1);
  EXPECT_EQ(CalcPairwiseIntersections(sets_A, sets_B, std::vector<int>{1}, true,
                                      bboxes_A, bboxes_B)
                .size(),
            0);
  const auto other_intersection_edges = CalcPairwiseIntersections(
      sets_A, sets_B, std::vector<int>{0, 1}, true, bboxes_A, bboxes_B);

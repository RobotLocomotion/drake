...
GTEST_TEST(SpheresOuterApproximatorTest, SingleBox1) {
  ...
  double max_sphere_radius = 1;
  // Test without setting max_sphere_radius.
  SpheresOuterApproximator dut(1, {box}, outliers, max_sphere_radius);

  auto result = solvers::Solve(dut.prog());
  ...
  Eigen::Vector2d a_val = result.GetSolution(dut.spheres()[0].a);
  double b_val = result.GetSolution(dut.spheres()[0].b);
  for (int i = 0; i < 4; ++i) {
  ...
  // Check the sphere radius constraint
...
GTEST_TEST(SpheresOuterApproximatorTest, SingleBox2) {
  ...
  SpheresOuterApproximator dut1(1, {box}, outliers, max_sphere_radius);
  ...
  // If we allow max_sphere_radius to be larger, then the sphere will only cover
  // one outlier.
  ...
  EXPECT_TRUE(result.is_success());
  // The optimal cost should be 1, that one outlier is covered by the
  // sphere.

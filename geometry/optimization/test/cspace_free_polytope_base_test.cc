...
TEST_F(CIrisToyRobotTest, CspaceFreePolytopeBaseConstructor1) {
  ...
  auto compare_s_indices = [&map_body_pair_to_s_on_chain](
                               multibody::BodyIndex body1,
                               multibody::BodyIndex body2,
                               const std::vector<int>& s_indices_expected) {
    const auto& s_indices = map_body_pair_to_s_on_chain.at(
        SortedPair<multibody::BodyIndex>(body1, body2));

    EXPECT_EQ(s_indices.size(), s_indices_expected.size());
    for (int i = 0; i < static_cast<int>(s_indices.size()); ++i) {
      EXPECT_EQ(s_indices[i], s_indices_expected[i]);
    }
  };
  compare_s_indices(body_indices_[0], plant_->world_body().index(),
                    std::vector<int>());
  compare_s_indices(plant_->world_body().index(), body_indices_[1],
                    std::vector<int>({0}));
  compare_s_indices(plant_->world_body().index(), body_indices_[2],
                    std::vector<int>({0, 1}));
  compare_s_indices(plant_->world_body().index(), body_indices_[3],
                    std::vector<int>({2}));
  compare_s_indices(body_indices_[0], body_indices_[1], std::vector<int>({0}));
  compare_s_indices(body_indices_[0], body_indices_[2],
                    std::vector<int>({0, 1}));
  compare_s_indices(body_indices_[0], body_indices_[3], std::vector<int>({2}));
  compare_s_indices(body_indices_[1], body_indices_[2], std::vector<int>({1}));
  compare_s_indices(body_indices_[1], body_indices_[3],
                    std::vector<int>({0, 2}));
  compare_s_indices(body_indices_[2], body_indices_[3],
  ...
  auto check_plane_s = [&dut](geometry::GeometryId geometry1,
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      for (const auto& s : s_in_a) {
        EXPECT_NE(s_expected.find(s), s_expected.end());
      }
    }
    const auto& s_in_b =

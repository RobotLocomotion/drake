...
// Check that points in a ConvexSet spanning every possible dimension are
// contained in an AffineSubspace.
void CheckAffineHullExtremePoints(const AffineSubspace& as,
                                  const ConvexSet& set, double tol = 0) {
  ASSERT_TRUE(set.MaybeGetFeasiblePoint().has_value());
  Eigen::VectorXd feasible_point = set.MaybeGetFeasiblePoint().value();
  EXPECT_TRUE(as.PointInSet(feasible_point, tol));
  ...
  prog.AddBoundingBoxConstraint(
      Eigen::VectorXd::Constant(set.ambient_dimension(), -1),
      Eigen::VectorXd::Constant(set.ambient_dimension(), 1), x);
  ...
  solvers::Binding<solvers::LinearCost> objective =
      prog.AddLinearCost(Eigen::VectorXd::Zero(set.ambient_dimension()), x);
  ...
  for (int i = 0; i < set.ambient_dimension(); ++i) {
    ...
    auto result = solvers::Solve(prog);
    ASSERT_TRUE(result.is_success());
    EXPECT_TRUE(as.PointInSet(result.GetSolution(y), tol));
...
GTEST_TEST(AffineSubspaceTest, AffineHullCartesianProduct) {
  ...
  CheckAffineHullExtremePoints(as, c, kTol);
}
...
GTEST_TEST(AffineSubspaceTest, AffineHullHPolyhedron) {
  ...
  CheckAffineHullExtremePoints(as1, h1, kTol);
  ...
  CheckAffineHullExtremePoints(as2, h2, kTol2);
}
...
GTEST_TEST(AffineSubspaceTest, AffineHullHyperellipsoid) {
  ...
  CheckAffineHullExtremePoints(as, E);
}
...
GTEST_TEST(AffineSubspaceTest, AffineHullIntersection) {
  ...
  CheckAffineHullExtremePoints(as3, i3, kTol3);
  ...
  CheckAffineHullExtremePoints(as4, i4, kTol4);
  ...
  CheckAffineHullExtremePoints(as5, i5, kTol5);
}
...
GTEST_TEST(AffineSubspaceTest, AffineHullMinkowskiSum) {
  ...
  CheckAffineHullExtremePoints(as1, ms1, kTol);
  ...
  CheckAffineHullExtremePoints(as2, ms2, kTol);
  ...
  CheckAffineHullExtremePoints(as3, ms3, kTol);
}
...
GTEST_TEST(AffineSubspaceTest, AffineHullSpectrahedron) {
  ...
  CheckAffineHullExtremePoints(as2, spect2, kTol);
}
...
GTEST_TEST(AffineSubspaceTest, AffineHullVPolytope) {
  ...
  CheckAffineHullExtremePoints(as1, v);
  ...
  CheckAffineHullExtremePoints(as2, line_segment);
  ...
  CheckAffineHullExtremePoints(as3, triangle);
}

...
GTEST_TEST(HPolyhedronTest, ArbitraryBoxTest) {
  ...
  auto H_P = HPolyhedron::MakeFromSceneGraph(
      query, geom_id, query.inspector().GetFrameId(geom_id));
...
GTEST_TEST(HyperEllipsoidTest, UnitSphereTest) {
  ...
  auto E_scene_graph = HyperEllipsoid::MakeFromSceneGraph(query, geom_id);
...
GTEST_TEST(HyperEllipsoidTest, ScaledSphereTest) {
  ...
  auto E = HyperEllipsoid::MakeFromSceneGraph(query, geom_id);
  EXPECT_TRUE(
...
GTEST_TEST(HyperEllipsoidTest, ArbitraryHyperEllipsoidTest) {
  ...
  // Test expressed_in frame.
  auto E_P = HyperEllipsoid::MakeFromSceneGraph(
      query, geom_id, query.inspector().GetFrameId(geom_id));

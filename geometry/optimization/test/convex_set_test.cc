...
std::tuple<std::unique_ptr<systems::Diagram<double>>, const SceneGraph<double>&,
           const GeometryId>
MakeSceneGraphWithShape(const Shape& shape, const RigidTransformd& X_WG) {
  ...
  return std::tuple<std::unique_ptr<systems::Diagram<double>>,
                    const SceneGraph<double>&, const GeometryId>(
...
GTEST_TEST(HPolyhedronTest, UnitBoxTest) {
  ...
  auto H_scene_graph = HPolyhedron::MakeFromSceneGraph(query, geom_id);
...
GTEST_TEST(HPolyhedronTest, ArbitraryBoxTest) {
  ...
  auto query = scene_graph.get_query_output_port().Eval<QueryObject<double>>(
      scene_graph.GetMyContextFromRoot(*context));
  auto H = HPolyhedron::MakeFromSceneGraph(query, geom_id);
...
GTEST_TEST(EllipsoidTest, UnitSphereTest) {
  ...
  auto E_scene_graph = Ellipsoid::MakeFromSceneGraph(query, geom_id);
  ...
  // Test ToSceneGraph.
  auto [shape, X_WG] = E.ToSceneGraph();
...
GTEST_TEST(EllipsoidTest, ArbitraryEllipsoidTest) {
  const Eigen::Matrix3d D = Eigen::DiagonalMatrix<double, 3>(3.0, 2.0, 1.0);
  ...
  auto [diagram, scene_graph, geom_id] = MakeSceneGraphWithShape(
      geometry::Ellipsoid(1.0 / D(0, 0), 1.0 / D(1, 1), 1.0 / D(2, 2)),
  ...
  // TODO(russt): Add ComputeSignedDistanceToPoint queries once Ellipsoids are
  // supported.
  ...
  EXPECT_LE(ellipsoid->a(), ellipsoid->b());
  EXPECT_LE(ellipsoid->b(), ellipsoid->c());
...
GTEST_TEST(EllipsoidTest, UnitSphere6DTest) {
  Ellipsoid E(Eigen::Matrix<double, 6, 6>::Identity(), Vector6d::Zero());

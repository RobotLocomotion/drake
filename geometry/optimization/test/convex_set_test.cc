...
// Test the exceptions generated by the base class NVI wrapping code.
GTEST_TEST(ConvexSetTest, ZeroDimensionThrow) {
  // A degenerate set with ambient_dimension() == 0.
  const Point dut;

  const Variable t("t");
  const VectorX<Variable> no_vars(0);

  solvers::MathematicalProgram prog;
  DRAKE_EXPECT_THROWS_MESSAGE(dut.AddPointInSetConstraints(&prog, no_vars),
                              ".*ambient_dimension.*>.*0.*");
  DRAKE_EXPECT_THROWS_MESSAGE(
      dut.AddPointInNonnegativeScalingConstraints(&prog, no_vars, t),
      ".*ambient_dimension.*>.*0.*");
  DRAKE_EXPECT_THROWS_MESSAGE(
      dut.AddPointInNonnegativeScalingConstraints(
          &prog, MatrixXd(), VectorXd(), VectorXd(), 0.0, no_vars, no_vars),
      ".*ambient_dimension.*>.*0.*");
}

...
class FclEngine : public ShapeReifier {
 public:
  ...
  /** Returns an independent copy of this engine on the AutoDiffXd scalar
   type. If the engine is already an AutoDiffXd engine, it is equivalent to
  ...
  /** Provides the poses for all of the dynamic geometries in the engine. All
   ...
   [0, num_dynamic() -1]. The iᵗʰ entry contains the pose for the
 ...
 private:
  ...
  // Helper method in reifying geometry in a consistent process.

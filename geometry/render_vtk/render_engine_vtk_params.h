...
struct RenderEngineVtkParams {
  ...
  /** Whether SSAO (screen-space ambient occlusion) is enabled for VTK rendering.
   * Once enabled, VTK simulates the ambient occlusion on surfaces where light
   * is obstructed by the surrounding geometry, produing images with better 
   * qualities and higher fidelity.
   * See https://www.kitware.com/ssao/ for details.
   */
  bool enable_ssao{false};  

  /** An optional SSAO parameters set. When specified, VTK will use the parameters
   * provided for SSAO. Non-specified parameters will use default values:
   * radius: 0.5
   * bias: 0.01
   * kernel size: 32
   * intensity_scale: 1.0

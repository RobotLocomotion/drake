...
RenderEngineVtk::actors() const {
  std::unordered_map<GeometryId, std::array<vtkSmartPointer<vtkActor>, 3>>
      actors;
  for (const auto& [id, prop_array] : props_) {
    if (vtkActor::SafeDownCast(prop_array[0]) != nullptr) {
      std::array<vtkSmartPointer<vtkActor>, 3> actor_array;
      for (int i = 0; i < kNumPipelines; ++i) {
        actor_array[i] = vtkActor::SafeDownCast(prop_array[i]);
      }
      actors.insert({id, std::move(actor_array)});
    }
  }
  return actors;
}
...
RenderEngineVtk::RenderEngineVtk(const RenderEngineVtk& other)
  ...
  for (const auto& [id, source_props] : other.props_) {
    ...
    if (vtkActor::SafeDownCast(source_props[0]) != nullptr) {
      ...
      // We should only be copying actors or assemblies.
      DRAKE_UNREACHABLE();
    }
    for (int i = 0; i < kNumPipelines; ++i) {
...
void RenderEngineVtk::ImplementMesh(const std::string& file_name, double scale,
  ...
  if (extension == ".obj") {
    ImplementObj(file_name, scale, data);
  } else if (extension == ".gltf") {
    ImplementGltf(file_name, scale, data);
  } else {
...
void RenderEngineVtk::ImplementGltf(const std::string& file_name, double scale,
  ...
  // and scale it by the requested scale. This will ultimately become the
  // child of another assembly that takes the geometry's pose.
  ...
  if (data->properties.HasProperty("phong", "diffuse") ||
    ...
    log()->warn(
        ...
        "removed from model specifications. glTF file: '{}'",
        file_name);
  }
  ...
  for (int i = 0; i < kNumPipelines; ++i) {
    vtkNew<vtkAssembly> file_root(make_file_assembly());
    ...
    while (vtkActor* source_actor = actors->GetNextActor()) {
      if (i == ImageType::kColor) {
        ...
        file_root->AddPart(source_actor);
      } else {
        ...
        file_root->AddPart(actor);
      }
    }
    vtkNew<vtkAssembly> assembly;
    assembly->AddPart(file_root.Get());
    assembly->SetUserTransform(vtk_X_WG);
    pipelines_.at(i)->renderer.Get()->AddActor(assembly);
    props[i] = std::move(assembly);
  }

  props_.insert({data->id, std::move(props)});

  // We've successfully processed the .gltf. Mark it accepted.
  data->accepted = true;
}
...
const std::vector<LightParameter>& RenderEngineVtk::active_lights()
    const {
  if (active_lights_ == nullptr) {
    if (parameters_.lights.size() > 0) {
      active_lights_ = &parameters_.lights;
    } else {
      fallback_lights_.push_back({});
      DRAKE_DEMAND(fallback_lights_.size() == 1);
      active_lights_ = &fallback_lights_;
    }
  }

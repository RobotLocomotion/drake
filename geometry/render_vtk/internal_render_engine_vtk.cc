...
// Validate the assumption that the given actor has not been posed using the
// transform components: position, origin, rotation, or scale.
void ThrowUnlessIdentityTransformComponents(vtkActor* p) {
  double values[3];
  p->GetPosition(values);
  if (values[0] != 0.0 || values[1] != 0.0 || values[2] != 0.0) {
    throw std::runtime_error(
        "vtkGLTFImporter imported an actor with non-zero position");
  }
  p->GetOrigin(values);
  if (values[0] != 0.0 || values[1] != 0.0 || values[2] != 0.0) {
    throw std::runtime_error(
        "vtkGLTFImporter imported an actor with non-zero origin");
  }
  p->GetOrientation(values);
  if (values[0] != 0.0 || values[1] != 0.0 || values[2] != 0.0) {
    throw std::runtime_error(
        "vtkGLTFImporter imported an actor with non-zero orientation");
  }
  p->GetScale(values);
  if (values[0] != 1 || values[1] != 1 || values[2] != 1) {
    throw std::runtime_error(
        "vtkGLTFImporter imported an actor with non-unit scale");
  }
}
...
bool RenderEngineVtk::ImplementGltf(const std::string& file_name, double scale,
  ...
  for (int i = 0; i < kNumPipelines; ++i) {
    ...
    while (vtkActor* source_actor = actors->GetNextActor()) {
      ...
      // as X_WG changes.
      //
      // We'd like some indication of whether or not the above paragraph ceases
      // to be valid. So, we'll make sure that the "built-in" transform
      // components (position, origin, orientation, scale) are all identity.
      DRAKE_ASSERT_VOID(ThrowUnlessIdentityTransformComponents(source_actor));

...
double AddShapeRows(RenderEngineVtk* render_engine, bool gltf_mesh = false) {
  ...
  PerceptionProperties texture_material(minimum_material());
  texture_material.AddProperty("phong", "diffuse", Rgba(1, 1, 1));
  texture_material.AddProperty(
      "phong", "diffuse_map",
  ...
  x -= 0.15;
  // This mesh has materials (box_checkered.obj or fully_texture_pyramid.gltf).
...
// Compare the test image against a reference image.
//
...
//                    the literal depth values to disk.
template <typename ImageType>
void CompareImages(const ImageType& test_image, const std::string& ref_filename,
  ...
  Eigen::Map<const VectorX<T>> data_expected(expected_image.at(0, 0),
                                             expected_image.size());
  Eigen::Map<const VectorX<T>> data2(compare_image.at(0, 0),
                                     compare_image.size());
  const auto differences =
      (data_expected.template cast<float>() - data2.template cast<float>())
          .array()
          .abs();
...
TEST_F(RenderEngineVtkTest, WholeImageDefaultParams) {
  ...
  // The camera is above the Wz plane, looking in the -y direction (across the
  // rows of shapes).

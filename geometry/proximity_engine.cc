...
class EncodedData {
 public:
  EncodedData(int index, bool is_dynamic) : data_(static_cast<size_t>(index)) {
 ...
 private:
  ...
  // Because user data is a void*, we can read it like a size_t. The highest
...
class ProximityEngine<T>::Impl : public ShapeReifier {
 public:
  ...
  GeometryIndex AddDynamicGeometry(const Shape& shape) {
    ...
    EncodedData(index, true /* is dynamic */).store(fcl_object.get());
  ...
  AnchoredGeometryIndex AddAnchoredGeometry(const Shape& shape,
    ...
    EncodedData(index, false /* is dynamic */).store(fcl_object.get());

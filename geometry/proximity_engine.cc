...
// construction the geometry object should be unchanged after invocation.
// However, in practice, there is an implicit mutation in this operation.
...
unique_ptr<CollisionObjectd> CopyFclObjectOrThrow(
  ...
  // Calling the CollisionObject constructor mutates the geometry (erasing any
  // effect the geometry's declared margin may have on the AABB). So, we read
  // the values, create the collision object, and write back the stored values
  // all under the protection of the given mutex.
...
class ProximityEngine<T>::Impl : public ShapeReifier {
 ...
 private:
  ...
  // Serializes the transient aabb_local mutation in CopyFclObjectOrThrow().

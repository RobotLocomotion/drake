...
ProximityEngine<T>::ComputeSignedDistanceToPoint(
    ...
    const std::vector<Isometry3<T>>& X_WG, const double threshold) const {
  return impl_->ComputeSignedDistanceToPoint(query, geometry_map, X_WG,
                                             threshold);
}

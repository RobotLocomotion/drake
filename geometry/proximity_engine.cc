...
class ProximityEngine<T>::Impl : public ShapeReifier {
 public:
  ...
  void UpdateRepresentationForNewProperties(
    ...
    if (geometry.is_deformable()) {
      // If an internal geometry is deformable, it must have been registered as
      // a deformable geometry in this ProximityEngine.
      DRAKE_DEMAND(IsRegisteredAsDeformable(id));
  ...
  // Returns true if the geometry with the given Id has be registered in `this`
  // ProximityEngine as a deformable geometry (via "AddDeformableGeometry()")
  // and has not been since removed (via "RemoveDeformableGeometry()").
  bool IsRegisteredAsDeformable(GeometryId id) {
    return geometries_for_deformable_contact_.is_deformable(id);
  }

  // Returns true if the geometry with the given Id has be registered in `this`
  // ProximityEngine as a rigid (non-deformable) geometry (via
  ...
  // Removes a rigid geometry from this engine.
 ...
 private:
  ...
  // `dynamic_objects_` and `dynamics_tree_`.

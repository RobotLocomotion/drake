...
namespace {
  ...
  ReadObjForConvex(const std::string& filename, double scale,
    ...
    int num_faces = TinyObjToFclFaces(shapes[0].mesh, faces.get());
...
class ProximityEngine<T>::Impl : public ShapeReifier {
 public:
  ...
  void ImplementGeometry(const Mesh& mesh, void* user_data) override {
    ...
    unused(num_faces);
    const std::vector<int>& faces = *face_ptr;

    // Note: the strategy here is to use an *invalid* fcl::Convex shape for the
    // mesh. An invalid Convex shape will use the vertices as a sort of
    // point cloud; this supports penetration and signed distance queries using
    // the general convexity algorithm. The convex shape is the *implicit*
    // convex hull of the vertices. This will be *slow* and it will produce
    // an fcl::CollisionGeometry that *lies* about its volume, moment of
    // inertia, and center of mass. But as Drake doesn't make use of those
    // FCL calculations we should be ok. This will break if FCL changes its
    // behavior for an "invalid" Convex shape.

    // Create an "invalid" convex mesh from two, disjoint triangles.
    std::vector<int> disjoint_faces;
    // The first triangle will consist of a triangle from the first polygon.
    disjoint_faces.push_back(3);  // Three vertices.
    disjoint_faces.push_back(faces[1]);
    disjoint_faces.push_back(faces[2]);
    disjoint_faces.push_back(faces[3]);
    disjoint_faces.push_back(3);  // Three vertices for the second triangle.
    // The second face will be the first face we find whose first three vertices
    // contain at most one vertex in the first triangle.
    int f_index = faces[0] + 1;  // The number of vertices in the first face.
    while (f_index < static_cast<int>(faces.size())) {
      DRAKE_DEMAND(faces[f_index] >= 3);
      int v = f_index + 1;
      int count = 0;
      for (int i = 0; i < 3; ++i) {
        count += faces[v + i] == disjoint_faces[1] ||
                 faces[v + i] == disjoint_faces[2] ||
                 faces[v + i] == disjoint_faces[3];
      }
      if (count <= 1) {
          disjoint_faces.push_back(faces[v]);
          disjoint_faces.push_back(faces[v + 1]);
          disjoint_faces.push_back(faces[v + 2]);
          break;
        }
      f_index += faces[f_index] + 1;
    }
    DRAKE_DEMAND(disjoint_faces.size() == 8);
    auto fcl_convex = make_shared<fcl::Convexd>(
        vertices, 2, make_shared<std::vector<int>>(std::move(faces)));

...
class ProximityEngine<T>::Impl : public ShapeReifier {
 public:
  ...
  void AddDeformableGeometry(const VolumeMesh<double>& mesh_W, GeometryId id) {
    deformable_contact_geometries_.AddDeformableGeometry(id, mesh_W);
  }
  ...
  void UpdateRepresentationForNewProperties(
    ...
    // representation and deformable contact representation.
    ...
    if (geometry.is_deformable()) {
      // If deformable geometry had a proximity property before, it must have
      // been registered as a deformable geometry in this engine.
      DRAKE_DEMAND(deformable_contact_geometries_.is_deformable(id));
      // Since all deformable geometries with any proximity property is treated
      // the same way, the "new_properties" don't change anything.
      return;
    }
    ...
    // deformable contact representations of rigid (non-deformable) geometries.
  ...
  void UpdateWorldPoses(
    ...
    for (const auto& id_object_pair : dynamic_objects_) {
      ...
      deformable_contact_geometries_.UpdateRigidWorldPose(id, X_WG_d);
    }

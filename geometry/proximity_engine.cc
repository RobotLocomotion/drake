 ...
 public:
  ...
  bool IsDeepCopy(const Impl& other) const {
    if (this != &other) {
      ...
      //  collision_filter_, and X_MBs_.
      auto are_map_deep_copy =
          [](const unordered_map<GeometryId, unique_ptr<CollisionObjectd>>&
                 this_map,
             const unordered_map<GeometryId, unique_ptr<CollisionObjectd>>&
                 other_map) -> bool {
      ...
      if (!are_map_deep_copy(this->dynamic_objects_, other.dynamic_objects_)) {
        return false;
      }
      if (!are_map_deep_copy(this->anchored_objects_,
                             other.anchored_objects_)) {
        return false;
      }
      if (!are_map_deep_copy(this->dynamic_mesh_objects_,
                             other.dynamic_mesh_objects_)) {
        return false;
      }
      if (!are_map_deep_copy(this->anchored_mesh_objects_,

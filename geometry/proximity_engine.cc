...
template <typename X>
struct dereferenced {
  using type = std::remove_reference<decltype(*std::declval<X&>())>::type;
};

// For a vector of `maybes` (std::optional or various pointer types will work),
// moves the dereferenced objects to `objects`, ignoring any nullish entries.
// Type X must provide a bool conversion operator, and a dereference operator.

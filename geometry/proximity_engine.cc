...
bool DistanceCallback(fcl::CollisionObjectd* fcl_object_A_ptr,
  ...
  // either (A,B) or (B,A), and the nearphase distance will always give the
  // same result.
  GeometryId id_A = EncodedData(*fcl_object_A_ptr).id(geometry_map);
  GeometryId id_B = EncodedData(*fcl_object_B_ptr).id(geometry_map);
  if (id_B < id_A) {
    std::swap(id_A, id_B);
  ...
  // the collision objects. We insure this by a reference to a const version

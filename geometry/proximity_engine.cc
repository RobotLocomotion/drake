...
SignedDistanceToPoint<double> DistanceToPoint::operator()(
  ...
  auto rz_to_xyz = [&p_GQ, &cylinder](Vector2d rz) -> Vector3d {
    const double r = rz(0);
    const double z = rz(1);
    const double r_Q = sqrt(p_GQ(0) * p_GQ(0) + p_GQ(1) * p_GQ(1));
    const bool near_center_line = (r_Q < RelativeTolerance(cylinder.radius));
    // Gr is the 2D unit vector for r coordinate expressed in xy-plane of G;
    // however, it is Gx if Q is within a tolerance from the center line.
    const Vector2d Gr = near_center_line ? Vector2d(1., 0.)
                                         : Vector2d(p_GQ(0), p_GQ(1)) / r_Q;
    return Vector3d (r * Gr(0), r * Gr(1), z);
  };

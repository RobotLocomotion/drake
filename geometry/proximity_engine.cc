...
struct ConvexHullCacheEntry {
  ...
  int num_faces{0};
  // Sub-cache of fcl::Convexd objects keyed by scale (x, y, z). This should
  // only be initialized on finite scale values.
...
class ProximityEngine<T>::Impl : public ShapeReifier {
 ...
 private:
  ...
  void ImplementFromConvexHull(const MeshType& mesh, void* user_data) {
    ...
    // Note: Mesh and Convex both validate their scale values as being finite.
    // So, the scale factor is "safe" as a map key.

...
template <typename T>
void SceneGraph<T>::RenameFrame(FrameId frame_id, std::string_view new_name) {
  return model_.RenameFrame(frame_id, new_name);
}
...
void SceneGraph<T>::RenameGeometry(GeometryId geometry_id,
  ...
  return model_.RenameGeometry(geometry_id, new_name);
}

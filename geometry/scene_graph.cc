...
class SceneGraph<T>::Hub {
 public:
  ...
  SceneGraphConfig& mutable_config() {
    std::lock_guard<std::mutex> guard(hydroelasticated_model_mutex_);
  ...
  const GeometryState<T>& hydroelasticated_model() const {
    // Note: executions that populate the cached model may hold this mutex for
    // quite some time.
    std::lock_guard<std::mutex> guard(hydroelasticated_model_mutex_);
    if (!hydroelasticated_model_) {
      // Hold the model in a temporary non-const smart pointer while it gets
      // provisioned, then transfer it to the const-typed member pointer.
      auto hydro_model = std::make_unique<GeometryState<T>>(model_);
      internal::Hydroelasticate(hydro_model.get(), config_);
      hydroelasticated_model_.reset(hydro_model.release());
    }
 ...
 private:
  ...
  // Access is exclusive, so that construction of new contexts remains
  // thread-safe.
  mutable std::mutex hydroelasticated_model_mutex_;
  mutable std::unique_ptr<const GeometryState<T>> hydroelasticated_model_;
};
...
void SceneGraph<T>::SetDefaultParameters(const Context<T>& context,
  ...
  bool want_hydro = (GetConfiguredHydroelasticType(hub_.config()) !=

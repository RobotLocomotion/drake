...
template <typename T>
int SceneGraph<T>::RemoveFromRenderer(const std::string& renderer_name,
                                      SourceId source_id, FrameId frame_id) {
  return initial_state_->RemoveFromRenderer(renderer_name, source_id, frame_id);
}

template <typename T>
int SceneGraph<T>::RemoveFromRenderer(Context<T>* context,
                                      const std::string& renderer_name,
                                      SourceId source_id,
                                      FrameId frame_id) const {
  auto& g_state = mutable_geometry_state(context);
  return g_state.RemoveFromRenderer(renderer_name, source_id, frame_id);
}
...
int SceneGraph<T>::RemoveFromRenderer(const std::string& renderer_name,
                                      SourceId source_id,
                                      GeometryId geometry_id) {
  return initial_state_->RemoveFromRenderer(renderer_name, source_id,
                                            geometry_id);
}
...
int SceneGraph<T>::RemoveFromRenderer(Context<T>* context,
                                      ...
                                      GeometryId geometry_id) const {
  auto& g_state = mutable_geometry_state(context);
  return g_state.RemoveFromRenderer(renderer_name, source_id, geometry_id);
}

...
/* Hub: Encapsulate some state to internally enforce the invariants for
 augmented model caching in one place, with otherwise unrestricted
 ...
 public:
  ...
  std::unique_ptr<GeometryState<T>> MakeAugmentedModel() const {
    ...
    if (augmented_model_cache_ != nullptr) {
      ...
      internal::ApplyProximityDefaults(result.get(), config_);
...
GeometryId SceneGraph<T>::RegisterGeometry(
  ...
  if (want_hydro && has_proximity) {
    internal::ApplyProximityDefaults(&g_state, config, geometry_id);
  }
  return geometry_id;
}

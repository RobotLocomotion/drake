...
void ValidateConfig(const SceneGraphConfig& config) {
  ...
  DRAKE_THROW_UNLESS(std::isfinite(hydro.default_hunt_crossley_dissipation));
  DRAKE_THROW_UNLESS(std::isfinite(hydro.default_dynamic_friction));
  DRAKE_THROW_UNLESS(std::isfinite(hydro.default_static_friction));
  ...
  DRAKE_THROW_UNLESS(hydro.default_hunt_crossley_dissipation >= 0.0);
  DRAKE_THROW_UNLESS(hydro.default_dynamic_friction >= 0.0);
  DRAKE_THROW_UNLESS(hydro.default_static_friction >= 0.0);

  // Since we can't conveniently use multibody::CoulombFriction, check now that
  // the values are compatible.
  DRAKE_THROW_UNLESS(hydro.default_static_friction >=
                     hydro.default_dynamic_friction);
}
...
void SceneGraph<T>::ChangeShape(
  ...
  if (need_hydroelasticate) {
    ...
    g_state.AssignRole(source_id, geometry_id, new_props, RoleAssign::kReplace);
  }
...
void SceneGraph<T>::AssignRole(Context<T>* context, SourceId source_id,
  ...
  const auto& config = scene_graph_config(*context);
  if (assign == RoleAssign::kNew && config.hydroelasticate.enabled == true) {
    internal::Hydroelasticate(&g_state, config, geometry_id);
  }
}

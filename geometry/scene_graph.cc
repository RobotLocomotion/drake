...
void SceneGraph<T>::CalcConfigurationUpdate(const Context<T>& context,
  ...
  internal::DrivenMeshData& driven_illustration_meshes =
      state.mutable_driven_illustration_meshes();
  internal::DrivenMeshData& driven_proximity_meshes =
      state.mutable_driven_proximity_meshes();
  // Process all sources *except*:
  //   - the internal source and
  ...
  driven_illustration_meshes.SetControlMeshPositions(kinematics_data.q_WGs);
  driven_proximity_meshes.SetControlMeshPositions(kinematics_data.q_WGs);

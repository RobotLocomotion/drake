...
/* We're creating a T-valued abstract value (stored either as State or
 ...
 Context::SetTimeStateAndParamtersFrom()) on an AutoDiff-valued Context with
 a double-valued context. The Value type wwould die in that attempt (because the
...
SceneGraph<T>::SceneGraph(bool data_as_state)
  ...
  if (data_as_state_) {
    ...
    geometry_state_index_ = this->DeclareAbstractParameter(
        *make_unique<GeometryStateValue<T>>());
  }
...
void SceneGraph<T>::SetDefaultState(const Context<T>& context,
  ...
  if (data_as_state_) {
    systems::AbstractValues& xa = state->get_mutable_abstract_state();
    xa.get_mutable_value(geometry_state_index_).set_value(model_);
  }
}
...
void SceneGraph<T>::SetDefaultParameters(const Context<T>& context,
  ...
  if (!data_as_state_) {
    AbstractValue& p =
        parameters->get_mutable_abstract_parameter(geometry_state_index_);
    p.set_value(model_);
  }
}

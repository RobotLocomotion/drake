...
class InternalGeometry {
 public:
  ...
  /* A geometry is deformable iff it has a mesh representation. */
  bool is_deformable() const { return mesh_ != nullptr; }

  /* Returns true if the geometry is not "fixed" in (or "hangs" on) the world
   frame. */
 ...
 private:
  ...
  // Optional mesh representation for deformable geometries.
  // TODO(xuchenhan-tri): InternalGeometry only needs the connectivity of the

...
class InternalGeometry {
 public:
  ...
  /* A geometry is deformable iff an internal mesh representation is required to
   ...
   deformable. */
  bool is_deformable() const { return reference_mesh_F_ != nullptr; }
 ...
 private:
  ...
  // configuration. The vertex positions are expressed in the parent
  // frame of the geometry, F. It's a nullptr if the geometry is rigid.
  copyable_unique_ptr<VolumeMesh<double>> reference_mesh_F_;
};

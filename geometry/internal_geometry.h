...
class InternalGeometry {
 public:
  ...
  void set_shape(const Shape& shape) { shape_spec_ = shape.Clone(); }
  ...
  // 2023-04-01 Simply set X_FG when the deprecation is complete.
  /* Changes the geometry's pose with respect to its frame from X_FG_old to
   `X_FG`. However that pose changes, the same change is applied to X_PG. */
  void set_pose(const math::RigidTransform<double>& X_FG) {
    const math::RigidTransform<double>& X_FGold = X_FG_;
    const math::RigidTransform<double> X_GoldG = X_FGold.inverse() * X_FG;
    X_FG_ = X_FG;

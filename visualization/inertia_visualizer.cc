...
std::pair<Ellipsoid, RigidTransform<double>> CalculateInertiaGeometry(
  ...
  const SpatialInertia<T> M_Bcm = M_Bo.Shift(M_Bo.get_com());

  // Pose the ellipsoid at the center of mass.
  RigidTransform<double> X_BoBcm =
      RigidTransform<double>(ExtractDoubleOrThrow(M_Bcm.get_com()));

  // For a massless body, use a very tiny sphere, expressed as an ellipsoid
  // for simplicity.
  const double mass = ExtractDoubleOrThrow(M_Bcm.get_mass());
  if (mass == 0) {
    return std::pair<Ellipsoid, RigidTransform<double>>(
        Ellipsoid(0.001, 0.001, 0.001), X_BoBcm);
  }

  // We need to find the equivalent solid ellipsoid for M_Bcm, as described by
  ...
  const Vector3d unit_inertia_moments =
      ExtractDoubleOrThrow(M_Bcm.get_unit_inertia().get_moments());
  ...
  // Assuming the density of water for the visualization ellipsoid, we'll scale

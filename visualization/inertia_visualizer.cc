...
std::pair<Ellipsoid, RigidTransform<double>> CalculateInertiaGeometry(
  ...
  const double mass = ExtractDoubleOrThrow(M_BBo.get_mass());
  if (mass == 0) {
  ...
  // Find the equivalent solid ellipsoid for M_BBcm, as described by an assumed
  // density, a computed pose X_BoBcm, and computed semi-major axes (a, b, c).
  RigidTransform<double> X_BoBcm;
  Vector3<double> unit_inertia_abc;
  std::tie(unit_inertia_abc, X_BoBcm) =
      M_BBo.CalcPrincipalSemiDiametersAndPoseForSolidEllipsoid();
  ...
  // each of the unit inertia's ellipsoid's radii.
  unit_inertia_abc =
      unit_inertia_abc.array().max(1e-2 * unit_inertia_abc.maxCoeff());
  // Assuming the ~density of water for the visualization ellipsoid, scale up
  // the ellipsoid representative of the unit inertia to have a volume that
  // matches the body's actual mass.
  const double density = 1000.0;
  const double unit_inertia_ellipsoid_mass =
      density * (4.0 / 3.0) * M_PI * unit_inertia_abc(0) * unit_inertia_abc(1) *
      unit_inertia_abc(2);
  const double volume_scale = mass / unit_inertia_ellipsoid_mass;
  const Vector3d spatial_inertia_abc =
      unit_inertia_abc * std::cbrt(volume_scale);

  return std::make_pair(Ellipsoid(spatial_inertia_abc), X_BoBcm);
}

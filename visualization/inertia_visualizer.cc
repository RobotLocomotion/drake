...
InertiaVisualizer<T>::InertiaVisualizer(
  ...
  for (multibody::BodyIndex i{0}; i < num_bodies; ++i) {
    ...
    props.AddProperty("phong", "diffuse", params.default_color);
...
void InertiaVisualizer<T>::UpdateItems(
  ...
  for (auto& item : items_) {
    ...
    double axisC =
        std::max(sqrt((5 / 2) * (+Ixx + Iyy - Izz)) * params_.scale_factor,
                 std::numeric_limits<double>::epsilon());
    const Eigen::Vector3d dimensions{axisA * params_.scale_factor,
                                     axisB * params_.scale_factor,
                                     axisC * params_.scale_factor};

...
void InertiaVisualizer<T>::UpdateItems(
  ...
  for (auto& item : items_) {
    ...
    item.X_BBcm = X_BBcm;
    scene_graph->ChangeShape(source_id_, item.geometry, *shape);
  }
}
...
template <typename T>
std::pair<std::unique_ptr<Shape>, math::RigidTransform<double>>
CalculateInertiaGeometry(const multibody::Body<T>& body,
  ...
  // TODO(trowell-tri) This is incorrect.
  math::RigidTransform<double> X_BBcm =
      RigidTransform<double>(ExtractDoubleOrThrow(M_B.get_com()));

  // For a massless body, use a very tiny sphere.
  const double mass = ExtractDoubleOrThrow(M_B.get_mass());
  // TODO(trowell-tri) unable to create a massless body with sdf?
  if (mass == 0) {
    return std::pair<std::unique_ptr<Shape>, math::RigidTransform<double>>(
        std::make_unique<Sphere>(0.001), X_BBcm);
  }
  ...
  return std::pair<std::unique_ptr<Shape>, math::RigidTransform<double>>(
      std::make_unique<Ellipsoid>(abc), X_BBcm);
}

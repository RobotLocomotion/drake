...
/** %MeshcatPoseInput adds slider bars to the MeshCat control panel for the
...
template <typename T>
class MeshcatPoseInput final : public systems::LeafSystem<T> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(MeshcatPoseInput)
  ...
  */
  MeshcatPoseInput(
      ...
      std::vector<std::string> decrement_keycodes = {"KeyQ", "KeyW", "KeyA",
                                                     "KeyJ", "KeyI", "KeyO"},
      std::vector<std::string> increment_keycodes = {"KeyE", "KeyS", "KeyD",
                                                     "KeyL", "KeyK", "KeyU"},
      std::string prefix = "",
  ...
  @pre `diagram` must contain this MeshcatPoseInput system.
  */

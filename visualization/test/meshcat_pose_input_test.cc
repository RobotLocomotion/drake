...
GTEST_TEST(MeshcatPoseInputTest, NarrowConstructor) {
  auto meshcat = geometry::GetTestEnvironmentMeshcat();

  const MeshcatPoseInput<double> dut(meshcat);
...
GTEST_TEST(MeshcatPoseInputTest, WideConstructor) {
  auto meshcat = geometry::GetTestEnvironmentMeshcat();
  ...
  const MeshcatPoseInput<double> dut(meshcat, initial_pose, lower_limit,
...
GTEST_TEST(MeshcatPoseInputTest, DeleteFunction) {
  auto meshcat = geometry::GetTestEnvironmentMeshcat();

  auto dut = std::make_unique<MeshcatPoseInput<double>>(meshcat);
...
GTEST_TEST(MeshcatPoseInputTest, Destructor) {
  auto meshcat = geometry::GetTestEnvironmentMeshcat();
...
GTEST_TEST(MeshcatPoseInputTest, Run) {
  auto meshcat = geometry::GetTestEnvironmentMeshcat();
  const RigidTransformd initial_pose(RollPitchYawd(0.1, 0.2, 0.3),
                                     Vector3d(0.4, 0.5, 0.6));

  systems::DiagramBuilder<double> builder;
  auto* dut = builder.AddSystem<MeshcatPoseInput>(meshcat, initial_pose);
...
GTEST_TEST(MeshcatPoseInputTest, SetPose) {
  auto meshcat = geometry::GetTestEnvironmentMeshcat();
  MeshcatPoseInput<double> dut(meshcat);
  auto context = dut.CreateDefaultContext();
...
GTEST_TEST(MeshcatPoseInputTest, InputPort) {
  auto meshcat = geometry::GetTestEnvironmentMeshcat();
  MeshcatPoseInput<double> dut(meshcat);
...
via the Run method. */
GTEST_TEST(MeshcatPoseInputTest, RunWithFixedInputPort) {
  auto meshcat = geometry::GetTestEnvironmentMeshcat();
  MeshcatPoseInput<double> dut(meshcat);

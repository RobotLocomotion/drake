...
class InertiaVisualizerGeometryTest
 ...
 public:
  ...
  by flipping all three angles to be nonnegative. */
  static Vector3d GetCanonicalRpy(const RigidTransform<double>& X) {
    ...
    Vector3d result;
    for (double flip_x : {0.0, M_PI}) {
      for (double flip_y : {0.0, M_PI}) {
        for (double flip_z : {0.0, M_PI}) {
          auto flips = RotationMatrix<double>::MakeXRotation(flip_x) *
                       RotationMatrix<double>::MakeYRotation(flip_y) *
                       RotationMatrix<double>::MakeZRotation(flip_z);
          result = RollPitchYaw<double>(R * flips).vector();
...
INSTANTIATE_TEST_SUITE_P(Poses, InertiaVisualizerGeometryTest, testing::Values(
  //           x,   y,  z,  r,  p,  y
  RowVector6d{ 0,   0,  0,   0,   0,   0},
  RowVector6d{10,  20, 30,   0,   0,   0},
  RowVector6d{ 5,   2,  1,   0,   0,   0},
  RowVector6d{10, 200, 50,   0,   0,   0},
  RowVector6d{ 0,   0,  0, 0.1,   0,   0},
  RowVector6d{ 0,   0,  0,   0, 0.1,   0},
  RowVector6d{ 0,   0,  0,   0,   0, 0.1},
  RowVector6d{ 0,   0,  0, 0.1, 0.2, 0.3},
  RowVector6d{ 5,   2,  1, 0.5, 0.2, 0.1}
));
// clang-format on

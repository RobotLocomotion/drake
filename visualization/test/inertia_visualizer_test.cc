...
GTEST_TEST(InertiaGeometryCalculationTest, Box) {
  ...
  ASSERT_NE(dynamic_cast<Ellipsoid*>(shape.get()), nullptr);

  const double eps = std::numeric_limits<double>::epsilon();
  auto ellipsoid = *dynamic_cast<Ellipsoid*>(shape.get());

...
GTEST_TEST(VisualizationConfigTest, Defaults) {
  ...
  EXPECT_EQ(config.default_illustration_color_rgba, params.default_color);
}
...
GTEST_TEST(VisualizationConfigFunctionsTest, ParamConversionDefault) {
  ...
  EXPECT_EQ(params.at(0).default_color, config.default_illustration_color_rgba);
  ...
  EXPECT_EQ(params.at(1).default_color, config.default_proximity_color_rgba);
...
GTEST_TEST(VisualizationConfigFunctionsTest, ParamConversionSpecial) {
  ...
  config.default_illustration_color_rgba = Rgba(0.25, 0.25, 0.25, 0.25);
  const std::vector<DrakeVisualizerParams> params =
      ConvertVisualizationConfigToParams(config);

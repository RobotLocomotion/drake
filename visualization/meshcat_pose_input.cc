#include "drake/visualization/meshcat_pose_input.h"
...
Vector6d ExtractRpyXyz(const RigidTransformd& pose,
  ...
  Vector6d rpy_xyz;
  rpy_xyz << min(
      fmod(rpy.roll_angle() - lower_limit[0], 2 * M_PI) + lower_limit[0],
      upper_limit[0]),
      min(fmod(rpy.pitch_angle() - lower_limit[1], 2 * M_PI) + lower_limit[1],
          upper_limit[1]),
      min(fmod(rpy.yaw_angle() - lower_limit[2], 2 * M_PI) + lower_limit[2],
          upper_limit[2]),
...
template <typename T>
MeshcatPoseInput<T>::MeshcatPoseInput(
  ...
  auto& output =
      this->DeclareAbstractOutputPort("pose", &MeshcatPoseInput<T>::CalcOutput);
  ...
  this->DeclareInitializationPublishEvent(
      &MeshcatPoseInput<T>::OnInitialization);
}

template <typename T>
void MeshcatPoseInput<T>::Delete() {
...
template <typename T>
void MeshcatPoseInput<T>::CalcOutput(const Context<T>&,
...
template <typename T>
systems::EventStatus MeshcatPoseInput<T>::OnInitialization(
...
template <typename T>
RigidTransformd MeshcatPoseInput<T>::Run(
  ...
  // Add the Stop button.
  constexpr char kButtonName[] = "Stop MeshcatPoseInput";
  ...
  while (meshcat_->GetButtonClicks(kButtonName) < 1) {
    ...
    // If the sliders have not changed, avoid invalidating the conte
...
template <typename T>
void MeshcatPoseInput<T>::SetPose(const RigidTransformd& pose) const {
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(
    class ::drake::visualization::MeshcatPoseInput)

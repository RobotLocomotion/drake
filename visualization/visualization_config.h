...
struct VisualizationConfig {
  ...
  void Serialize(Archive* a) {
    ...
    a->Visit(DRAKE_NVP(default_inertia_color));
    a->Visit(DRAKE_NVP(inertia_scale_factor));
  ...
  The vector must be of size three (rgb) or four (rgba). */
  geometry::Rgba default_inertia_color{0.0, 0.0, 1.0, 0.2};
  ...
  double inertia_scale_factor{0.4};

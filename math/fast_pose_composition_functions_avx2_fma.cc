...
Each matrix is 9 consecutive doubles is in column-major order.
...
we'll pull in the rows and then shuffle) and broadcast elements from right,
performing 4 operations in parallel but ignoring the 4th result. We must be
careful not to load or store past the last element.
...
Each matrix is 12 consecutive doubles is in column-major order.
...
void ComposeXXAvx(const double* X_AB, const double* X_BC, double* X_AC) {
  ...
  // Column stu:                                    s   t   u   _  =
  __m256d stu_ = xyz_;                          //  x   y   z
  stu_ = _mm256_fmadd_pd(abc_, four(X), stu_);  // +aX +bX +cX
  stu_ = _mm256_fmadd_pd(def_, four(Y), stu_);  // +dY +eY +fY
  stu_ = _mm256_fmadd_pd(ghi_, four(Z), stu_);  // +gZ +hZ +iZ

  // Column jkl:                                    j   k   l   _  =
  __m256d jkl_ = _mm256_mul_pd(abc_, four(A));  //  aA  bA  cA  _
  jkl_ = _mm256_fmadd_pd(def_, four(B), jkl_);  // +dB +eB +fB  _
  jkl_ = _mm256_fmadd_pd(ghi_, four(C), jkl_);  // +gC +hC +iC
...
additions, and 3 substractions) but we're doing 84 here and throwing away 21 of
...
void ComposeXinvXAvx(const double* X_BA, const double* X_BC, double* X_AC) {
  ...
  // Column jkl:                                    j   k   l   _  =
  __m256d jkl_ = _mm256_mul_pd(adg_, four(A));  //  aA  dA  gA
  jkl_ = _mm256_fmadd_pd(beh_, four(B), jkl_);  // +bB +eB +hB
  jkl_ = _mm256_fmadd_pd(cfi_, four(C), jkl_);  // +cC +fC +iC

  // Column mno:                                    m   n   o  _  =
  __m256d mno_ = _mm256_mul_pd(adg_, four(D));  //  aD  dD  gD _
  mno_ = _mm256_fmadd_pd(beh_, four(E), mno_);  // +bE +eE +hE _
  mno_ = _mm256_fmadd_pd(cfi_, four(F), mno_);  // +cF +fF +iF

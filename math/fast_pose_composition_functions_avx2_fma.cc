...
void LoadMatrix3Inv(const double* R_BA, Vec4* adg_, Vec4* beh_, Vec4* cfi_) {
  ...
  const auto adgf = hn::InterleaveLower(tag, abgh, defg);   // a.g.|d.f. => adgf
  const auto behg = hn::InterleaveUpper(tag, abgh, defg);   // .b.h|.e.g => behg
  const auto adcf = hn::InterleaveLower(abcd, defg);        // a.c.|d.f. => adcf

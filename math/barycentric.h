...
class BarycentricMesh {
 ...
 public:
  ...
  /// The mesh is represented by a std::set (for logarithmic lookups) of
  /// coordinates in each input dimension. Note: The values are type double, not
  /// T (We do not plan to take gradients, etc w/ respect to them).
  ...
  /// containing the points to interpolate between.
  /// @param input must be a vector of length get_num_inputs().

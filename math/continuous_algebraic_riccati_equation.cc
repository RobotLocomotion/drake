...
Eigen::MatrixXd ContinuousAlgebraicRiccatiEquation(
  ...
  // The Hamiltonian H should have half of its eigen values with negative real
  // part, and half of its eigen values with positive real part, see
  // https://stanford.edu/class/ee363/lectures/clqr.pdf for more discussion on
  // the Hamiltonian.
  if (H.determinant() == 0) {
    // If H.determinant() is 0, then H must have at least one eigen value being
  ...
  if (svd.info() != Eigen::Success) {
    throw std::runtime_error(
        ...
        "semidefinite? Is your R matrix positive "
        "definite?");
  }

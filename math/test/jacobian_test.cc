...
#pragma GCC diagnostic pop
  ...
  auto value = ExtractValueMatrixFromAutoDiff(jac_chunk_size_default);
  EXPECT_TRUE(CompareMatrices(value_expected, value, 1e-12,
                              MatrixCompareType::absolute));

  // Ensure that Jacobian is correct.
  auto jac = ExtractGradientMatrixFromAutoDiff(jac_chunk_size_default);
...
#pragma GCC diagnostic pop
  ...
  auto value_autodiff = ExtractValueMatrixFromAutoDiff(hess_chunk_size_default);
  auto value = ExtractValueMatrixFromAutoDiff(value_autodiff);
  EXPECT_TRUE(CompareMatrices(value_expected, value, 1e-12,
                              MatrixCompareType::absolute));

  // Ensure that the two ways of computing the Jacobian from AutoDiff match.
  auto jac_autodiff =
      ExtractGradientMatrixFromAutoDiff(hess_chunk_size_default);
  auto jac1 = ExtractValueMatrixFromAutoDiff(jac_autodiff);
  auto jac2 = ExtractGradientMatrixFromAutoDiff(value_autodiff);
  ...
  auto hess = ExtractGradientMatrixFromAutoDiff(jac_autodiff);
  EXPECT_TRUE(

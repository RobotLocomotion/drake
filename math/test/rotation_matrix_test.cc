...
GTEST_TEST(RotationMatrix, MakeFromOrthonormalRowsOrColumns) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(
      R = RotationMatrix<double>::MakeFromOrthonormalRows(Vector3d(-1, 0, 0),
                                                          Fy, Fz),
      "Error: Rotation matrix determinant is negative.*");
  DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(
      R = RotationMatrix<double>::MakeFromOrthonormalColumns(Vector3d(-1, 0, 0),
                                                             Fy, Fz),
      "Error: Rotation matrix determinant is negative.*");
...
GTEST_TEST(RotationMatrix, MakeXRotationMakeYRotationMakeZRotation) {
  ...
  Matrix3d zero_matrix = m - R.matrix();
  EXPECT_TRUE((zero_matrix.array() == 0).all());
  EXPECT_TRUE(CompareMatrices(RotationMatrixd::MakeXRotation(M_PI_4) * i, i,
                              tolerance));
  ...
  EXPECT_TRUE(CompareMatrices(RotationMatrixd::MakeYRotation(M_PI_4) * j, j,
                              tolerance));
  ...
  EXPECT_TRUE(CompareMatrices(RotationMatrixd::MakeZRotation(M_PI_4) * j,
                              Vector3d(-M_SQRT1_2, M_SQRT1_2, 0), tolerance));
  EXPECT_TRUE(CompareMatrices(RotationMatrixd::MakeZRotation(M_PI_4) * k, k,
                              tolerance));
...
GTEST_TEST(RotationMatrix, AxialRotationOptimizations) {
  ...
  // should give the same result either with our mangled matrix or a clean one.
  const Rmat R_AB(RollPitchYaw(1.0, 2.0, 3.0));
  Rmat R_AC;
...
GTEST_TEST(RotationMatrix, IsAxialRotationOrThrow) {
  // This is a legitimate axial rotation about the y axis.
  const RotationMatrixd aR_BC = RotationMatrixd::MakeYRotation(0.5);
  // This is a _general_ (non-axial) rotation.
  const RotationMatrixd R_AB(RollPitchYawd(1.0, 2.0, 3.0));

  EXPECT_NO_THROW(aR_BC.IsAxialRotationOrThrow<1>());

  // BTW we don't need these messages to be beautiful; they are issued only
  // in Debug builds. We just want to make sure bad rotations are caught.
  ...
  const RotationMatrixd& badR_BC =
      *reinterpret_cast<const RotationMatrixd*>(badR_BC_matrix.data());

  badR_BC_matrix(0, 0) = .123;  // One of the cosines is broken.
  EXPECT_THROW(badR_BC.IsAxialRotationOrThrow<1>(), std::exception);

  badR_BC_matrix = aR_BC.matrix();  // Restore good value.
  badR_BC_matrix(2, 0) = .123;      // One of the sines is broken.
  EXPECT_THROW(badR_BC.IsAxialRotationOrThrow<1>(), std::exception);
...
void CheckProjectionWithAxis(const Eigen::Matrix3d& M,
                             const Eigen::Vector3d& axis, const double angle_lb,
...
GTEST_TEST(RotationMatrixTest, OperatorMultiplyByMatrix3X) {
  ...
  Eigen::Matrix3d v_B;
  const Vector3d v1_B(-12, -9, 7);
  v_B.col(0) = v1_B;
  const Vector3d v2_B(-11, -8, 10);
  v_B.col(1) = v2_B;
  const Vector3d v3_B(-10, -7, 12);
  v_B.col(2) = v3_B;
  ...
  const Eigen::MatrixXd s_A =
      R_AB * (Eigen::Vector3d(1, 2, 3) * Eigen::RowVector4d(1, 2, 3, 4));
  ...
  const auto z_A_expected =
      R_AB * (Eigen::MatrixXd(3, 4) << Eigen::Vector3d(1, 2, 3),
...
class RotationMatrixConversionTests : public ::testing::Test {
 protected:
  void SetUp() override { SetupQuaternionTestCases(); }
...
TEST_F(RotationMatrixConversionTests, RotationMatrixToQuaternionSymbolic) {
  ...
  for (const Eigen::Quaterniond& qi : quaternion_test_cases_) {
    ...
    const Eigen::Quaterniond q_actual_double(
        q_actual_expr.coeffs().unaryExpr([](const Expression& x) {
...
TEST_F(RotationMatrixConversionTests, RotationMatrixToQuaternionVariable) {
  ...
  for (const Eigen::Quaterniond& qi : quaternion_test_cases_) {
    ...
    const Eigen::Quaterniond q_actual_double(
        q_expr.coeffs().unaryExpr([&env](const Expression& x) {
...
TEST_F(RotationMatrixConversionTests, AngleAxisToRotationMatrix) {
  ...
  if (kDrakeAssertIsArmed) {
    ...
    EXPECT_THROW(const RotationMatrix<double> R_zero(aa_zero), std::exception);
...
// Verify that MakeFromOneUnitVector() throws an exception if its first argument
// is a vector with a non-finite element (e.g., NaN or infinity) or is a vector
// whose magnitude is not super close to 1.0.
...
GTEST_TEST(RotationMatrixTest, MakeFromOneUnitVectorExceptions) {
  ...
  Vector3<double> test_vector(NAN, 1, NAN);
  DRAKE_EXPECT_THROWS_MESSAGE(
  ...
  test_vector = Vector3<double>(kInfinity, 1, 0);
  DRAKE_EXPECT_THROWS_MESSAGE(
      RotationMatrix<double>::MakeFromOneUnitVector(test_vector, axis_index),
      "MakeFromOneUnitVector.* The unit_vector argument .* "
      "is not a unit vector.[^]*");

  // Verify zero vector throws an exception as expected..
  test_vector = Vector3<double>(0, 0, 0);
  DRAKE_EXPECT_THROWS_MESSAGE(
      RotationMatrix<double>::MakeFromOneUnitVector(test_vector, axis_index),
      "MakeFromOneUnitVector.* The unit_vector argument .* "
      "is not a unit vector.[^]*");
  ...
  test_vector = Vector3<double>(1, 2, 3);
  DRAKE_EXPECT_THROWS_MESSAGE(
      RotationMatrix<double>::MakeFromOneUnitVector(test_vector, axis_index),
      "MakeFromOneUnitVector.* The unit_vector argument .* "
      "is not a unit vector.[^]*");

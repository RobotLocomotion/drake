...
GTEST_TEST(RotationMatrix, SetRotationMatrix) {
  ...
  RotationMatrix<double> R;
  EXPECT_NO_THROW(R.SetOrThrowIfNotValid(m));
...
GTEST_TEST(RotationMatrix, SymbolicRotationMatrices) {
  ...
  // SetOrThrowIfNotValid() only sets the rotation matrix, with no checks for
  // validity, e.g., ThrowIfNotValid() is a "no-op" (does nothing).
  m_symbolic << q0, q1, q2,
                q1, q2, q3,
                q2, q3, 4;
  EXPECT_NO_THROW(R1.SetOrThrowIfNotValid(m_symbolic));
  ...
  R2.SetOrThrowIfNotValid(R1.matrix());

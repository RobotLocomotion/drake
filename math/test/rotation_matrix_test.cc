...
GTEST_TEST(RotationMatrix, RotationMatrixConstructor) {
  ...
  m << 1, 2,  3,
       4, 5,  6,
       7, 8, -10;
  DRAKE_EXPECT_THROWS_MESSAGE(RotationMatrix<double> R2(m), std::logic_error,
  ...
  m << 1, 9000*kEpsilon, 9000*kEpsilon,
       0, cos_theta, sin_theta,
       0, -sin_theta, cos_theta;
  DRAKE_EXPECT_THROWS_MESSAGE(RotationMatrix<double> R3(m), std::logic_error,
                              "Error: Rotation matrix is not orthonormal.*");
  ...
  m << 1, 0, 0,
       0, 1, 0,
       0, 0, -1;
  DRAKE_EXPECT_THROWS_MESSAGE(RotationMatrix<double> R4(m), std::logic_error,
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(RotationMatrix<double> R5(m), std::logic_error,
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(RotationMatrix<double> R6(m), std::logic_error,
        "Error: Rotation matrix contains an element that is infinity or NaN.*");
#endif
}

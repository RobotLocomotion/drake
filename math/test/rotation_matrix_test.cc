...
GTEST_TEST(RotationMatrix, SymbolicProjectionTest) {
  ...
  auto identity = Matrix3<symbolic::Expression>::Identity().eval();
  const symbolic::Variable x{"x"};
  identity(2, 0) = x;

  // The ProjectToRotationMatrix() SVD will throw if there is a free variable.
  using RotMatExpr = RotationMatrix<symbolic::Expression>;
  symbolic::Expression quality;
  DRAKE_EXPECT_THROWS_MESSAGE(
      RotMatExpr::ProjectToRotationMatrix(identity, &quality),
      std::runtime_error,
      ".*environment does not have an entry for the variable.*\n*");

  // Removing the variable allows us to succeed.  (The precision of the result
  // is tested elsewhere.)
  identity(2, 0) = 0;
  RotMatExpr::ProjectToRotationMatrix(identity, &quality);
  EXPECT_LT(abs(quality - 1.0), 1e-6);
}

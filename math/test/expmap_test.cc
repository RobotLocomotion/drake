...
using drake::math::InitializeAutoDiffFromValueMatrix;
...
void ConvertQuaternionToExpmapAndBack(const Vector4d& quat) {
  auto quat_autodiff = InitializeAutoDiffFromValueMatrix(quat);
  auto expmap_autodiff = quat2expmap(quat_autodiff);
  auto expmap = ExtractValueMatrixFromAutoDiff(expmap_autodiff);
  auto expmap_grad = ExtractGradientMatrixFromAutoDiff(expmap_autodiff);
  auto quat_back_autodiff =
      expmap2quat(InitializeAutoDiffFromValueMatrix(expmap));
  auto quat_back = ExtractValueMatrixFromAutoDiff(quat_back_autodiff);
  auto quat_back_grad = ExtractGradientMatrixFromAutoDiff(quat_back_autodiff);

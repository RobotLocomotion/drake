...
GTEST_TEST(SoftOverMax, TestAutodiff) {
  std::vector<Eigen::AutoDiffScalar<Eigen::Vector2d>> x;
  x.emplace_back(1, Eigen::Vector2d(1, 3));
  x.emplace_back(2, Eigen::Vector2d(0, 1));
  x.emplace_back(3, Eigen::Vector2d(0, -1));
  const Eigen::AutoDiffScalar<Eigen::Vector2d> x_max = SoftOverMax(x, 10);
...
GTEST_TEST(SoftUnderMax, TestAutoDiff) {
  ...
  x.emplace_back(2, Eigen::Vector2d(0, -1));
  const Eigen::AutoDiffScalar<Eigen::Vector2d> x_max =
      SoftUnderMax(x, 10 /* alpha */);
...
GTEST_TEST(SoftOverMin, TestAutoDiff) {
  ...
  x.emplace_back(1, Eigen::Vector2d(1, 2));
  x.emplace_back(2, Eigen::Vector2d(0, -1));
  const Eigen::AutoDiffScalar<Eigen::Vector2d> x_min =
      SoftOverMin(x, 10 /* alpha */);

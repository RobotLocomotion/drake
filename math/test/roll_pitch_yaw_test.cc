...
GTEST_TEST(RollPitchYaw, RollPitchYawDtToAngularVelocityExpressedInA) {
  ...
  const Eigen::Vector3d rpyDt_calculated =
      rpy.AngularVelocityAToRollPitchYawDt(w_AD_A);
...
GTEST_TEST(RollPitchYaw, AngularAccelerationAToRollPitchYawDDt) {
  const RollPitchYaw<double> rpy(0.2, 0.3, 0.4);
  const Eigen::Vector3d rpyDt(-2.1, 3.3, 5.7);
  const Eigen::Vector3d alpha_AD_A(0.5, 0.7, 0.9);
  const Eigen::Vector3d rpyDDt =
      rpy.AngularAccelerationAToRollPitchYawDDt(rpyDt, alpha_AD_A);
  ...
  const Eigen::Vector3d rpyDDt_expected(rDDt, pDDt, yDDt);

...
const char* not_enough_knots = R"""(
order: 3
knots: [0., 1., 1.5, 1.6, 2.]
)""";
GTEST_TEST(BsplineBasisSerializeTests, NotEnoughKnotsTest) {
  BsplineBasis<double> dut{};
  EXPECT_THROW(YamlReadArchive(YAML::Load(not_enough_knots)).Accept(&dut),
               std::runtime_error);
}

const char* unsorted_knots = R"""(
order: 3
knots: [0., 2.5, 1., 1.5, 1.6, 2., 3.]
)""";
GTEST_TEST(BsplineBasisSerializeTests, UnsortedKnotsTest) {
  BsplineBasis<double> dut{};
  EXPECT_THROW(YamlReadArchive(YAML::Load(unsorted_knots)).Accept(&dut),
               std::runtime_error);
}

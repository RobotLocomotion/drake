...
GTEST_TEST(RigidTransform, SpecializedTransformOperators) {
  ...
  const Xform Xy_BC(Xform::MakeAxialRotation<1>(theta));  // about y
  const Xform Xz_BC(Xform::MakeAxialRotation<2>(theta));  // about z

  // Test MakeAxialRotation().
  EXPECT_TRUE(Xx_BC.IsNearlyEqualTo(
      Xform(RotationMatrixd::MakeXRotation(theta)), kTol));
  EXPECT_TRUE(Xy_BC.IsNearlyEqualTo(
      Xform(RotationMatrixd::MakeYRotation(theta)), kTol));
  EXPECT_TRUE(Xz_BC.IsNearlyEqualTo(
  ...
  EXPECT_TRUE(CompareMatrices(Xform::ApplyAxialRotation<0>(Xx_BC, v_C),
                              Xx_BC * v_C, kTol));
  EXPECT_TRUE(CompareMatrices(Xform::ApplyAxialRotation<1>(Xy_BC, v_C),
                              Xy_BC * v_C, kTol));
  EXPECT_TRUE(CompareMatrices(Xform::ApplyAxialRotation<2>(Xz_BC, v_C),
                              Xz_BC * v_C, kTol));
  ...
  X_BC_update = Xz_BC;
  Xform::UpdateAxialRotation<2>(4 * theta, &X_BC_update);
  EXPECT_TRUE(X_BC_update.IsNearlyEqualTo(
  ...
  const Xform Xr_BC(RollPitchYaw(0.5, 1.5, -2.5), Vector3d::Zero());
  const Xform Xt_BC(Vector3d(-1.0, 2.0, -3.0));
  Xform X_AC;  // reusable result

  // Test ComposeWithRotation() and ComposeWithTranslation().
  X_AB.ComposeWithRotation(Xr_BC, &X_AC);
  EXPECT_TRUE(X_AC.IsNearlyEqualTo(X_AB * Xr_BC, kTol));

  X_AB.ComposeWithTranslation(Xt_BC, &X_AC);
  EXPECT_TRUE(X_AC.IsNearlyEqualTo(X_AB * Xt_BC, kTol));

  // Test ComposeWithAxialRotation().
  X_AB.ComposeWithAxialRotation<0>(Xx_BC, &X_AC);
  EXPECT_TRUE(X_AC.IsNearlyEqualTo(X_AB * Xx_BC, kTol));
  X_AB.ComposeWithAxialRotation<1>(Xy_BC, &X_AC);
  EXPECT_TRUE(X_AC.IsNearlyEqualTo(X_AB * Xy_BC, kTol));
  X_AB.ComposeWithAxialRotation<2>(Xz_BC, &X_AC);
  EXPECT_TRUE(X_AC.IsNearlyEqualTo(X_AB * Xz_BC, kTol));
}

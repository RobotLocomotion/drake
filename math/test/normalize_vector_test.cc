...
void NormalizeVectorTestFun(const Eigen::Matrix<double, nx, 1>& x) {
  ...
  auto x_autodiff = InitializeAutoDiffFromValueMatrix(x);
  auto x_norm = x_autodiff.norm();
  auto x_normalized_autodiff = x_autodiff / x_norm;

  EXPECT_TRUE(CompareMatrices(
      x_normalized, ExtractValueMatrixFromAutoDiff(x_normalized_autodiff),
      1E-10, MatrixCompareType::absolute));
  EXPECT_TRUE(CompareMatrices(
      dx_normalized,
      ExtractGradientMatrixFromAutoDiff(x_normalized_autodiff), 1E-10,

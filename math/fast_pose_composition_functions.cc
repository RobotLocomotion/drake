...
void ComposeXXPortable(const RigidTransform<double>& X_AB,
  ...
  ComposeXXNoAlias(reinterpret_cast<const double*>(&X_AB),
                   reinterpret_cast<const double*>(&X_BC), X_AC_temp);
  std::copy(X_AC_temp, X_AC_temp + 12, reinterpret_cast<double*>(X_AC));
}
...
void ComposeXinvXPortable(const RigidTransform<double>& X_BA,
  ...
  assert(X_AC != nullptr);
  double X_AC_temp[12];  // Protect from overlap with inputs.
  ComposeXinvXNoAlias(reinterpret_cast<const double*>(&X_BA),
                      reinterpret_cast<const double*>(&X_BC), X_AC_temp);
  std::copy(X_AC_temp, X_AC_temp + 12, reinterpret_cast<double*>(X_AC));
}
...
void ComposeXX(const RigidTransform<double>& X_AB,
               const RigidTransform<double>& X_BC,
               RigidTransform<double>* X_AC) {
  ComposeXXAvx(reinterpret_cast<const double*>(&X_AB),
               reinterpret_cast<const double*>(&X_BC),
               reinterpret_cast<double*>(X_AC));
}
void ComposeXinvX(const RigidTransform<double>& X_BA,
                  const RigidTransform<double>& X_BC,
                  RigidTransform<double>* X_AC) {
  ComposeXinvXAvx(reinterpret_cast<const double*>(&X_BA),
                  reinterpret_cast<const double*>(&X_BC),
                  reinterpret_cast<double*>(X_AC));
}

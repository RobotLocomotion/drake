...
class RigidTransform {
 public:
  ...
  /// @name   Advanced methods for transforms with specialized structure
  /// @anchor special_xform_def
  ///
  /// (Advanced) If we have foreknowledge that a %RigidTransform has some
  ...
  /// if we know the particular axis, e.g. zrX_BC would be an axial rotation
  /// transform containing a rotation about the z axis.
  ///
  /// Specialized transforms have the property that of the twelve elements in
  /// their matrix representation some are "inactive", meaning that they never
  /// change during a simulation. Those elements are expected to have _exactly_
  /// the values they would have in an identity transform (that is, 1 on the
  /// diagonal and 0 elsewhere). Even when T is symbolic::Expression, we insist
  /// that the inactive elements have the correct numerical values and don't
  /// require an Environment for evaluation. With _general_ transforms (twelve
  /// active elements), transforming a vector takes 18 floating point
  /// operations, composing two transforms takes 63, and updating requires
  /// writing all twelve elements. The methods in this section take advantage of
  /// the specialized structures to reduce the number of operations required.
  /// Axial methods are templatized by the axis number 0, 1, or 2 (+x, +y, or
  /// +z axis, resp.). The number of operations required is documented with each
  /// method.
  ///
  ...
  /// performance reasons we will not attempt to verify that in Release builds
  /// although we may do so in Debug builds.
  ///
  ...
  /// (Advanced) Creates an axial rotation transform arX_AB consisting of only
  /// an axial rotation of `theta` radians about x, y, or z and no translation.
  /// Of the 12 entries in the transform matrix, only 4 are active; the rest
  /// will be set to 0 or 1. This structure can be exploited for efficient
  /// updating and operating with this transform.
  ...
  /// (Advanced) Given an axial rotation transform arX_BC (just an axial
  /// rotation and no translation), uses it to efficiently re-express a given
  /// vector. This takes only 6 floating point operations.
  /// @param[in] arX_BC the axial transform to be applied.
  /// @param[in] p_C a position vector measured and expressed in frame C,
  ///   to be transformed (re-measured and re-expressed) in frame B (just
  ///   re-expressed in this case since there is zero translation).
  /// @retval p_B the input vector, now re-expressed in frame B.
  /// @tparam axis 0, 1, or 2 corresponding to +x, +y, or +z rotation axis.
  ...
  /// (Advanced) Given a new rotation angle θ, updates the axial rotation
  /// transform arX_BC to represent the new rotation angle. We expect that
  /// arX_BC was already such a transform (about the given x, y, or z axis).
  /// Only the 4 active elements are modified; the other 8 remain unchanged.
  ...
  /// updates the axial rotation transform arX_BC to represent the new rotation
  /// angle. We expect that arX_BC was already such a transform (about the
  /// given x, y, or z axis). Only the 4 active elements are modified; the other
  /// 8 remain unchanged.
  ...
  /// (Advanced) With `this` a general transform X_AB, and given an axial
  /// rotation transform arX_BC, efficiently calculates `X_AC = X_AB * arX_BC`.
  /// This requires only 14 floating point operations.
  ...
  /// (Advanced) Composes `this` general transform X_AB with a given axial
  /// translation transform atX_BC to efficiently calculate
  ...
  /// (Advanced) Composes `this` general transform X_AB with a given

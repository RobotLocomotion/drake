...
class RigidTransform {
 public:
  ...
  void PreMultiplyByAxialTranslation(const RigidTransform<T>& atX_AB,
    ...
    Eigen::Vector3<T> p_AC_A = X_BC.translation();  // Initialize to p_BC_B.
    p_AC_A[axis] += atX_AB.translation()[axis];     // The only flop.
    X_AC->set_translation(p_AC_A);
  }

  /// (Internal use only) Given a new distance d, updates the axial translation
  /// transform atX_BC to represent the new translation by d. We expect that
  /// atX_BC was already such a transform (about the given x, y, or z axis).
  /// Only the 1 active element is modified; the other 11 remain unchanged. No
  /// floating point operations are needed.
  /// @param[in] distance the new translation distance in meters.

...
/// point).
template <typename Derived>
bool IsPositiveDefinite(const Eigen::MatrixBase<Derived>& matrix, const
double threshold) {
  ...
  Eigen::SelfAdjointEigenSolver<typename Derived::PlainObject>
      eigensolver(matrix);
  DRAKE_THROW_UNLESS(eigensolver.info() == Eigen::Success);
  return eigensolver.eigenvalues().minCoeff() >= threshold;
}

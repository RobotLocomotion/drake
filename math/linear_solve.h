...
typename std::enable_if<std::is_same_v<typename DerivedA::Scalar, double> &&
...
LinearSolve(const LinearSolver& linear_solver,
  ...
  Eigen::Matrix<double, DerivedB::RowsAtCompileTime, Eigen::Dynamic> b_grad(
      b.rows(), num_z_b);
  ...
  for (int i = 0; i < b.cols(); ++i) {
    b_grad = autoDiffToGradientMatrix(b.col(i));
    // ∂x/∂zᵢ = A⁻¹*∂b/∂zᵢ
    // To avoid aliasing issue, I create an x_grad matrix. If we are 100% sure
    // that calling solve won't cause the aliasing issue, then we can use a
    // single grad matrix, and call
    ...
    x_grad = linear_solver.solve(b_grad);
    x_ad.col(i) = initializeAutoDiffGivenGradientMatrix(x_val.col(i), x_grad);
  }
  return x_ad;
}

...
/**
 * Specialized when A and b are both double or symbolic::Expression matrices.
 * See @ref linear_solve_given_solver for more details. The user needs to
 * explicitly specify the scalar type in A.
 */
template <typename ScalarA, typename LinearSolver, typename DerivedB>
typename std::enable_if<
    internal::is_double_or_symbolic_v<ScalarA> &&
        internal::is_double_or_symbolic_v<typename DerivedB::Scalar> &&
        std::is_same_v<ScalarA, typename DerivedB::Scalar>,
    Eigen::Matrix<ScalarA, DerivedB::RowsAtCompileTime,
                  DerivedB::ColsAtCompileTime>>::type
SolveLinearSystem(const LinearSolver& linear_solver,
                  const Eigen::MatrixBase<DerivedB>& b) {
  return linear_solver.solve(b);
 ...
 * details. The user needs to explicitly specify the scalar type in A.
 */
template <typename ScalarA, typename LinearSolver, typename DerivedB>
typename std::enable_if<
    std::is_same_v<ScalarA, double> &&
        !internal::is_double_or_symbolic_v<typename DerivedB::Scalar>,
...
SolveLinearSystem(const LinearSolver& linear_solver,
                  ...
                  const Eigen::MatrixBase<DerivedB>& b) {
  unused(A);
  return SolveLinearSystem<double>(linear_solver, b);
}
 ...
 public:
  ...
  typename std::enable_if<
      ...
      Eigen::Matrix<typename DerivedB::Scalar, DerivedA::RowsAtCompileTime,
                    DerivedB::ColsAtCompileTime>>::type
  Solve(const Eigen::MatrixBase<DerivedB>& b) const {
    return SolveLinearSystem<double>(linear_solver_, b);
  }

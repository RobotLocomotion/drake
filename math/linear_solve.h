 ...
 * double/AutoDiffScalar, and b is an Eigen vector of double/AutoDiffScalar.
 ...
 * AutoDiffScalar-valued vector.See @ref linear_solve_given_solver for more
...
LinearSolve(const LinearSolver& linear_solver,
  ...
  // ∂x/∂zᵢ = A⁻¹*∂b/∂zᵢ
  // Before calling linear_solver.solve(grad), grad stores the gradient of b.
  // After calling grad = linear_solver.solve(grad), grad stores the gradient of
  // x. I don't create a separate matrix for the gradient of x, as that might
  // incur memory heap allocation.
  grad = linear_solver.solve(grad);
  return initializeAutoDiffGivenGradientMatrix(x_val, grad);
}

/**
 * Specialized when both A and b are AutoDiffScalar-valued matrices.See @ref
 * linear_solve_given_solver for more details.
 */
template <typename LinearSolver, typename DerivedA, typename DerivedB>
typename std::enable_if<

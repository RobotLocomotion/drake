 ...
 *
 * Create the linear solver for a given matrix A, which will be used to solver
 ...
 * LinearSolve(GetLinearSolve<Eigen::LLT>(A_ad), A_ad, b_val);
 ...
 * LinearSolve(GetLinearSolve<Eigen::LLT>(A_val), A_val, b_ad);
 * // Solve A * x = b with both A and b containing gradient.
 * const Eigen::Matrix<AutoDiffXd, 2, 1> x_ad3 =
 * LinearSolve(GetLinearSolve<Eigen::LLT>(A_ad), A_ad, b_ad);
 * @endcode{cc}
 */
 ...
 * The returned linear solver will have matrix containing double.
 * See @ref get_linear_solver for more details.
 */
template <template <typename, int...> typename LinearSolverType,
          typename DerivedA>

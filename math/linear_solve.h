...
typename std::enable_if<
    internal::is_double_or_symbolic<typename DerivedA::Scalar>::value,
    typename GetLinearSolverReturn<LinearSolverType, DerivedA>::type>::type
GetLinearSolver(const Eigen::MatrixBase<DerivedA>& A) {
  const typename GetLinearSolverReturn<LinearSolverType, DerivedA>::type
      linear_solver(A);

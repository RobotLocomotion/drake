 ...
 * double/AutoDiffScalar/symbolic_expression, and b is an Eigen matrix of
 * double/AutoDiffScalar/symbolic_expression.
 * Notice that if either A or b contains symbolic_expression, then the other
...
//@{
/**
 * Specialized when A and b are both double or symbolic_expression matrices.
...
LinearSolve(const LinearSolver& linear_solver,
  ...
  Eigen::Matrix<double, DerivedB::RowsAtCompileTime, Eigen::Dynamic> grad(
      b.rows(), num_z_b);
  Eigen::Matrix<typename DerivedB::Scalar, DerivedA::RowsAtCompileTime,
                DerivedB::ColsAtCompileTime>
      x_ad(A.rows(), b.cols());

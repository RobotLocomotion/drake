...
class RotationMatrix {
 public:
  ...
  /// with the rotation angle θ. The other five elements are "inactive" and
  /// must have _exactly_ the values they would have in an identity rotation
  /// (that is, 1 on the diagonal and 0 off-diagonal). Even when T is
  ...
  /// arguments are indeed axial; for performance reasons we will not attempt to
  /// verify that in Release builds although we may do so in Debug builds.
  ///
  ...
  /// @pre aR_BC is an @ref axial_rotation_def "axial rotation matrix".
  template <int axis>
  ...
  /// rotation about the indicated axis.

...
class RotationMatrix {
 public:
  ...
  /// uses it to efficiently re-express a vector. This takes only 6 floating
  /// point operations.
  ...
  /// (Advanced) Given a new rotation angle θ, updates the axial rotation aR_BC
  /// to represent the new rotation angle. We expect that aR_BC was already such
  /// an axial rotation about the given x, y, or z axis. Only the 4 active
  /// elements are modified; the other 5 remain unchanged. (However, execution
  /// time is likely to be dominated by the time to calculate sine and cosine.)
  ...
  /// updates the axial rotation aR_BC to represent the new rotation angle. We
  /// expect that aR_BC was already such a rotation (about the given x, y, or z
  /// axis) but by some other angle. Only the 4 active elements are modified;
  /// the other 5 remain unchanged.
  ...
  /// rotation aR_BC, efficiently forms R_AC = R_AB * aR_BC. This requires only

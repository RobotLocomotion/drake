...
class RotationMatrix {
 public:
  ...
  /// to an independent variable `t` (where `t` usually denotes time).  `R` is
  /// regarded as depending on the %RollPitchYaw angles `rpy`, which themselves
  /// are regarded as functions of `t` [i.e., `r(t)`, `p(t)`, `y(t)`].
  /// @param[in] rpy %RollPitchYaw with roll, pitch, yaw angles.
  /// @param[in] rpyDt Ordinary derivative of rpy with respect to an independent
  /// variable `t` (where `t` usually denotes time, but not necessarily).
  ...
  static Matrix3<T> OrdinaryDerivativeRotationMatrix(const RollPitchYaw<T>& rpy,
                                                     const Vector3<T>& rpyDt) {
    // For the rotation matrix R generated by the RollPitchYaw angles rpy,
    // calculate the partial derivatives of R with respect to roll `r`,
    // pitch `p` and yaw `y`, respectively.
    Matrix3<T> R_r, R_p, R_y;  // ∂R/∂r, ∂R/∂p, ∂R/∂y
    RotationMatrix<T>::PartialDerivativeRotationMatrix(rpy, &R_r, &R_p, &R_y);
    ...
    const Matrix3<T> R_r_mult_rDt = R_r * rpyDt(0);
    const Matrix3<T> R_p_mult_pDt = R_p * rpyDt(1);
    const Matrix3<T> R_y_mult_yDt = R_y * rpyDt(2);
    return R_r_mult_rDt + R_p_mult_pDt + R_y_mult_yDt;
  }
 ...
 private:
  ...
  // For the %RotationMatrix `R` that is generated by the %RollPitchYaw rotation
  // sequence `rpy`, calculates the partial derivatives of `R` with respect to
  ...
  static void PartialDerivativeRotationMatrix(const RollPitchYaw<T>& rpy,
                           Matrix3<T>* R_r, Matrix3<T>* R_p, Matrix3<T>* R_y) {

...
class RotationMatrix {
 public:
  ...
  /// @param[in] Ra_BC An axial rotation about the indicated axis.
  /// @param[in] v_C A vector expressed in C to be re-expressed in B.
  /// @retval v_B The input vector re-expressed in B.
  /// @pre Ra_BC is _exactly_ a rotation about the given axis (constant entries
  ///   are exactly 1 or 0).
  template <int axis>
    requires(0 <= axis && axis <= 2)
  static Vector3<T> ApplyAxialRotation(const RotationMatrix<T>& Ra_BC,
                                       const Vector3<T>& v_C) {
    ...
    const Matrix3<T>& M = Ra_BC.matrix();
  ...
  /// (Advanced) Given sin(θ) and cos(θ), update the axial rotation Ra_BC which
  ...
  static void UpdateAxialRotation(const T& sin_theta, const T& cos_theta,
                                  RotationMatrix<T>* Ra_BC) {
    DRAKE_ASSERT(Ra_BC != nullptr);
    Matrix3<T>& M = Ra_BC->mutable_matrix();
    constexpr int x = axis, y = (axis + 1) % 3, z = (axis + 2) % 3;
    DRAKE_ASSERT(M(x, x) == 1 && M(x, y) == 0 && M(x, z) == 0);
    DRAKE_ASSERT(M(y, x) == 0 && M(z, x) == 0);
  ...
  /// (Advanced) Given a new rotation angle θ, update the axial rotation Ra_BC
  ...
  template <int axis>
    requires(0 <= axis && axis <= 2)
  static void UpdateAxialRotation(const T& theta, RotationMatrix<T>* Ra_BC) {
    using std::cos, std::sin;
    DRAKE_ASSERT(Ra_BC != nullptr);
    UpdateAxialRotation<axis>(sin(theta), cos(theta), &*Ra_BC);
  }

  /// With `this` a general rotation R_AB, and given an axial rotation Ra_BC,
  /// efficiently form R_AC = R_AB * Ra_BC.
  /// @param[in] Ra_BC An axial rotation about the indicated axis.
  /// @param[out] R_AC The result. Must not overlap with Ra_BC in memory.
  /// @pre Ra_BC is _exactly_ a rotation about the given axis (constant entries
  ///   are exactly 1 or 0).
  template <int axis>
    requires(0 <= axis && axis <= 2)
  void ComposeWithAxialRotation(const RotationMatrix<T>& Ra_BC,
    ...
    const Matrix3<T>& M_BC = Ra_BC.matrix();
    constexpr int x = axis, y = (axis + 1) % 3, z = (axis + 2) % 3;

...
RotationMatrix<T>::ThrowIfNotValid(const Matrix3<S>& R) {
  ...
  if (!IsOrthonormal(R, get_internal_tolerance_for_orthonormality())) {
    ...
    std::string message = fmt::format(
        ...
        " expensive (but not necessarily closest) rotation matrix, use the"
        " constructor RotationMatrix<T>(RotationMatrix<T>::"
        "Quaternion<T>(your_matrix3))."
        " Alternately, if using quaternions, ensure the quaternion is"
        " normalized.", measure);

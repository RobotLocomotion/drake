...
void BarycentricMesh<T>::EvalBarycentricWeights(
  ...
  std::vector<bool> has_volume(get_num_inputs());
  ...
  for (int i = 0; i < get_num_inputs(); i++) {
    ...
    if (right_iter == coords.end()) {
      ...
      T right_value = *(right_iter);
      T left_value = *(std::prev(right_iter));
  ...
  // Sort the dimensions by their relative position.
...
void BarycentricMesh<T>::Eval(const Eigen::Ref<const MatrixX<T>> mesh_values,
  ...
  DRAKE_DEMAND(input.size() == get_num_inputs());

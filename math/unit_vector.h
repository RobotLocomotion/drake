...
// the calling function might consider normalizing unit_vector even when it
...
//      (mag_squared == 1.0) ? unit_vector : unit_vector / sqrt(mag_squared);
// @endcode
// TODO(Mitiguy) Change calls to this function to ThrowIfNotUnitVector() on the
//  date specified in the message written to the log file. The date is specified
//  in the implementation of WarnIfNotUnitVector() in unit_vector.cc.
template <typename T>

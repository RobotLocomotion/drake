...
Note that, except when explicitly marked "noalias", the methods below must allow
for the output argument's memory to overlap with any of the input arguments'
memory. */
...
This requires 45 flops (9 dot products) but we're doing 60 here
and throwing away 15 of them. */
void ComposeRR_avx(const double* R_AB, const double* R_BC, double* R_AC) {
  constexpr __int64_t yes = __int64_t(1) << 63;
  constexpr __int64_t no  = __int64_t(0);
...
void ComposeXX_avx(const double* X_AB, const double* X_BC, double* X_AC) {
  constexpr uint64_t yes  = uint64_t(1ull << 63);
  constexpr uint64_t no  = uint64_t(0);
  const __m256i mask = _mm256_setr_epi64x(yes, yes, yes, no);
...
*/
void ComposeXinvX_avx(const double* X_BA, const double* X_BC, double* X_AC) {
  constexpr uint64_t yes  = uint64_t(1ull << 63);
  constexpr uint64_t no  = uint64_t(0);
  const __m256i mask = _mm256_setr_epi64x(yes, yes, yes, no);
...
void ComposeRR(const double* R_AB, const double* R_BC, double* R_AC) {
  internal::ComposeRR_cpp(R_AB, R_BC, R_AC);
}
void ComposeRinvR(const double* R_BA, const double* R_BC, double* R_AC) {
  internal::ComposeRinvR_cpp(R_BA, R_BC, R_AC);
}
void ComposeXX(const double* X_AB, const double* X_BC, double* X_AC) {
  internal::ComposeXX_cpp(X_AB, X_BC, X_AC);
}
void ComposeXinvX(const double* X_BA, const double* X_BC, double* X_AC) {
  internal::ComposeXinvX_cpp(X_BA, X_BC, X_AC);
}
#endif

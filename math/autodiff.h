...
/** (Advanced) Determines the appropriate return type for the ExtractValue()
function which extracts the `value()` portion from a matrix of AutoDiffScalar
entries. We expect the template parameter `Derived` to be an Eigen type that
represents a matrix of AutoDiffScalars. The `type` declaration here will
then be an Eigen matrix with the same dimensions (or same variable
dimensions) but containing entries of the Scalar type from the AutoDiffScalar
(commonly, but not necessarily, `double`).
...
@tparam Derived An Eigen type representing a matrix with AutoDiffScalar
    ...
    an ExtractValue() call site.

@see AutoDiffToGradientMatrix */
...
@retval value_matrix An Eigen::Matrix of the same dimensions as the input
...
template <typename Derived>
typename AutoDiffToValueMatrix<Derived>::type ExtractValue(
...
template <typename Derived>
// TODO(sherm1) DRAKE_DEPRECATED("2021-12-01", "Use ExtractValue() instead")
...
(Eigen::Dynamic) and the starting index (0).
...
void InitializeAutoDiff(
    const Eigen::MatrixBase<Derived>& value_matrix,
    Eigen::MatrixBase<DerivedAutoDiff>* auto_diff_matrix) {
  InitializeAutoDiff(value_matrix, Eigen::Dynamic, 0,
      auto_diff_matrix);
}
...
void initializeAutoDiff(const Eigen::MatrixBase<Derived>& value_matrix,
  ...
  InitializeAutoDiff(value_matrix,
      num_derivatives, deriv_num_start, &auto_diff_matrix);
}
...
Creates an AutoDiff matrix that matches `value_matrix` in size with derivative
of compile time size `nq` and runtime size `num_derivatives`. Sets its values to
be equal to `value_matrix`, and for each element i of `auto_diff_matrix`, sets
...
AutoDiffMatrixType<Derived, nq> InitializeAutoDiff(
  ...
  AutoDiffMatrixType<Derived, nq> auto_diff_matrix(value_matrix.rows(),
                                                   value_matrix.cols());
  InitializeAutoDiff(value_matrix, num_derivatives, deriv_num_start,
                     &auto_diff_matrix);
...
AutoDiffMatrixType<Derived, nq> initializeAutoDiff(
    ...
    Eigen::DenseIndex num_derivatives = -1,
    Eigen::DenseIndex deriv_num_start = 0) {
  return InitializeAutoDiff(value_matrix, num_derivatives, deriv_num_start);
}
...
template <typename... Deriveds>
// TODO(sherm1) DRAKE_DEPRECATED("2021-12-01",
//     "Use InitializeAutoDiffTuple() instead")
std::tuple<AutoDiffMatrixType<
    Deriveds, internal::total_size_at_compile_time<Deriveds...>()>...>
initializeAutoDiffTuple(const Eigen::MatrixBase<Deriveds>&... args) {

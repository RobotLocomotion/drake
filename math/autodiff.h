...
@param[in] deriv_num_start starting index into derivative vector (i.e. element
    deriv_num_start in derivative vector corresponds to value(0, 0)).
...
void InitializeAutoDiff(
  ...
  if (num_derivatives == Eigen::Dynamic) num_derivatives = value.size();
  ...
  for (Eigen::DenseIndex i = 0; i < value.size(); i++) {
    (*auto_diff_matrix)(i) =
...
void InitializeAutoDiff(
    const Eigen::MatrixBase<Derived>& value,
    Eigen::MatrixBase<DerivedAutoDiff>* auto_diff_matrix) {
  InitializeAutoDiff(value, Eigen::Dynamic, 0, auto_diff_matrix);
}
...
void initializeAutoDiff(const Eigen::MatrixBase<Derived>& value,
  ...
  InitializeAutoDiff(value, num_derivatives, deriv_num_start,
                     &auto_diff_matrix);
}
...
AutoDiffMatrixType<Derived, nq> InitializeAutoDiff(
    ...
    Eigen::DenseIndex deriv_num_start = 0) {
  if (num_derivatives == -1) num_derivatives = value.size();

...
// Here "_cpp" is a suffix added to styleguide-compatible name to indicate that
// these are written in portable C++. The names are not user-visible.
void ComposeRR_cpp(const double* R_AB, const double* R_BC, double* R_AC);

// TODO(sherm1) ComposeRinvR_cpp(), ComposeXX_cpp(), ComposeXinvX_cpp()

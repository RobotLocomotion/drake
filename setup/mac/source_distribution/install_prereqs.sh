...
pip3.12 install --break-system-packages --user -r "${BASH_SOURCE%/*}/requirements.txt"

if [[ "${with_test_only}" -eq 1 ]]; then
  pip3.12 install --break-system-packages --user -r "${BASH_SOURCE%/*}/requirements-test-only.txt"
fi

...
def _dpkg_download_install(
    *, temp_dir: pathlib.Path, urls: list[str], sha256: str
    ...
    assert temp_dir and urls and sha256
...
def _apt_install_flavor(
    *, lsb_release: dict[str, str], flavor: Flavor, yes: bool
    ...
    and all of its antecedent flavors. The filenames of the package lists are
    governed by the `lsb_release` data. The `yes` flag is passed along to apt
    as `--yes`."""
    assert isinstance(flavor, Flavor)
    distribution = "ubuntu"  # TODO(jwnimmer-tri) Use lsb_release, instead.
    codename = lsb_release["Codename"]
    description = lsb_release.get("Description", "<unknown>")
    packages = []
    for item in Flavor:
        txt_file = (_my_dir() / distribution
...
def _dpkg_install_bazelisk():
    ...
    # TODO(jwnimmer-tri) This probably shouldn't hard-code Ubuntu.
    distribution = "ubuntu"

    # Load the json metadata.
    content = (_my_dir() / distribution / f"packages-bazelisk.json").read_text(
        encoding="utf-8")
    specs = json.loads(content)
    name = specs["name"]
    version = specs["version"]

    # Bail out when already installed at the correct version.
    installed = _get_dpkg_version(name)
    if installed == version:
        logging.debug(f"{name} is already at version {version}")
        return

    # Bail out when already installed at a newer version.
    if installed is not None:
        ...
        if comparison.returncode == 0:
            logging.info(
                f"{name} version {installed} is already installed; "
                f"not downgrading to Drake's nominal version {version}."
            )
            return
...
def main():
    ...
    # TODO(jwnimmer-tri) Add --user-environment-only option.
    ...
    if False:  # user_environment_only

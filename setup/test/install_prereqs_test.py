...
class InstallPrereqsTest(unittest.TestCase):
    def setUp(self):
        ...
        # In the test cases below, we'll check that prereqs flavors install a
        # plausible number of packages as a cross-check that the setup code is
        # probably correct, without overly coupling the test cases to the
        # specific package names of our dependencies.
    ...
    def _check_ubuntu_binary(self, *, already_installed: bool, yes: bool):
        ...
        dut.start(args=["-y"] if yes else ["--verbose"])
        ...
        self.assertGreater(num_packages, 0)
    ...
    def test_ubuntu_build_bootstrap(self):
        ...
        self.assertGreaterEqual(
            num_packages, self._min_num_apt_packages["build"]
        )
    ...
    def test_developer_bootstrap(self):
        ...
        if sys.platform != "darwin":
            ...
            self.assertTrue(dut.did_apt_update)
            self.assertGreaterEqual(
                num_packages, self._min_num_apt_packages["developer"]
            )
    ...
    def test_developer_bump(self):
        ...
        if sys.platform != "darwin":
            ...
            self.assertTrue(dut.did_apt_update)
            self.assertGreaterEqual(
                len(package_names), self._min_num_apt_packages["developer"]
            )
    ...
    def test_developer_completed(self):
        ...
        if sys.platform != "darwin":
            ...
            if _is_ubuntu_distro("noble"):
                ...
                # doesn't install anything further.
                dut.expect_dpkg_query(default_version="14.x.x")

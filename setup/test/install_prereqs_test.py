...
class InstallPrereqsActor:
    ...
    def expect_call(self, *, exact=None, prefix=None, stdout="", returncode=0):
        ...
        out to a subprocess and mocks up the effects of that call. The mocked
        call will return the given `returncode` and `stdout` content. The mocked
        command line is specified as either an `exact` list of arguments (where
        the first argument is the command name), or a `prefix` where only the
        beginning of the arguments need to match. Returns the arguments.
        """
